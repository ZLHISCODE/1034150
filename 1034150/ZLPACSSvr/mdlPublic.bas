...
'-----------------------------------------------------------------------------
'功能:根据现有序列UID查询，返回用“影像类型拆分”或“多帧图像”的新序列UID
'参数： strOldSeriesUID -- 从图像中读取出来的序列UID
'       strImageType -- 图像类型，“LOCALIZER，AXIAL ，MultiFrame”三种，用来匹配“序列描述”字段的内容
'返回：新产生的序列UID
'-----------------------------------------------------------------------------
    Dim rsMatch As New ADODB.Recordset
    ...
    '先将返回值设置成从图像中读取出来的序列UID
    funcGetSeriesUID = strOldSeriesUID
    
    '查询数据库的正式表和临时表，是否有前半段相同的序列UID
    ...
    While Not rsMatch.EOF
        '如果序列UID相同，则标记intCur=0
        If rsMatch("序列UID") = strOldSeriesUID Then
            intCur = 0
        Else
            '提取序列UID最后的一个尾数数值，算是类似序列UID的计数值
            intCur = Val(Right(rsMatch("序列UID"), Len(rsMatch("序列UID")) - InStrRev(rsMatch("序列UID"), ".")))
        End If
        
        'intMax从默认值0开始，记录最大的序列UID计数值
        If intMax < intCur Then intMax = intCur
        
        '如果序列描述相同，则返回查出来的序列UID，而且退出循环
        '如果一个序列中有多个多帧图，这里也会被拆分成多个序列，
        '因为保存到数据库中的rsMatch("序列描述")是图像中的序列描述，而不是传入的参数“MultiFrame”标记。
    ...
    '如果查询出来有结果，但是没有匹配，则序列计数值+1，否则不需要加1
    If rsMatch.RecordCount > 0 And blnMatch = False Then
        '创建新的UID
    ...
    For Each curImage In Images
        ...
        If rsTmp.EOF Then
            ...
            If funGetAEStoreParas(curImage.Tag, strModality, strDeviceIP, blnSplitSeriesUID, intImageMatchItem, intDBMatchItem, blnMatchStudyUID, _
                ...
                '提取图像中的主匹配ID
                strAdviceID = funGetMatchIDInImg(curImage, intImageMatchItem)
                '根据图像类型拆分序列UID
                ...
                If blnNewStudy = True Then      '没有已经匹配成功的记录，则按病人ID或英文名查找
                    ...
                    Else        '没有找到匹配的记录，则插入临时检查记录
                        ...
                        If strAdviceID = -1 Then
                            strAdviceID = getStrNumber(curImage.PatientID)
                        End If
...
Private Function funGetMatchIDInImg(img As DicomImage, intMatchItem As Integer) As String
'-----------------------------------------------------------------------------
'功能:根据条件，提取图像中的匹配ID
'参数： img ----需要匹配的图像
