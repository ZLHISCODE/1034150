...
Public Function OutAdviceCheckWarn_MK(ByVal lngCmd As Long, Optional ByVal lngRow As Long, Optional ByRef blnNoSave As Boolean) As Long
...
'      单药警告：应在用药审查过之后进行调用(有警告值)
    ...
    Dim str开嘱科室 As String, str开嘱医生 As String, str开嘱医生串 As String
    ...
    '美康3.0
        ...
        If glngModel = PM_门诊医嘱清单 Then gobjPati.lng挂号ID = rsTmp!就诊ID
        ...
        With gobjAdvice
            ...
            Else
                ...
                '开嘱医生获取 医嘱清单因为界面只加载了显示行（中药服法）,传病人医嘱时未能将清单中中药信息传人,35.0上已处理,此处暂不处理
                For i = .FixedRows To .Rows - 1
                    If InStr(",5,6,7,", "," & .TextMatrix(i, gobjCOL.intCOL诊疗类别) & ",") > 0 Then
                    ...
                    If blnDo Then
                        ...
                        If str频率 = "" Then
                            ...
                            If Not rs开嘱医生 Is Nothing Then
                                ...
                                If Not rs开嘱医生.EOF Then str开嘱医生 = rs开嘱医生!编号 & "/" & str开嘱医生
                            End If
                        End If
                ...
                For i = .FixedRows To .Rows - 1
                    ...
                    If blnDo Then
                        If glngModel = PM_门诊编辑 Then
                            lngTmp = .RowData(i)
                        Else
                            lngTmp = .TextMatrix(i, gobjCOL.intCOLID)
                        End If
                        k = PassGetWarn(lngTmp)
...
Public Function OutAdviceCheckWarn_TYT(ByVal lngCmd As Long, Optional ByVal lngRow As Long, Optional ByRef blnNoSave As Boolean) As Long
...
'返回值：医嘱保存调用，需要用返回值判断是否存在禁忌用药
    ...
    Dim str给药途径 As String, str药品 As String, str单量 As String, str单量单位 As String
    Dim str开嘱科室ID As String, str医嘱序号 As String, str中药组IDs As String
    Dim str相关ID As String, str医嘱ID As String
    ...
    Dim i As Long, j As Long, k As Long, lngMaxWarn As Long, lng医嘱ID As Long
...
Public Function InAdviceCheckWarn_MK(ByVal lngCmd As Long, Optional ByVal lngRow As Long, Optional blnIsHaveOut As Boolean, Optional ByRef blnNoSave As Boolean) As Long
'功能：调用Pass系统中对医嘱进行合理用药审查等相关功能
'参数：lngCmd=
'        0-检测设置PASS菜单状态
'        1/33-保存自动审查(住院/门诊),2/34-提交自动审查(住院/门诊),3-手工调用审查
...
'      单药警告：应在用药审查过之后进行调用(有警告值)
    Dim rsTmp As New ADODB.Recordset
    Dim rs规格 As ADODB.Recordset, rs开嘱医生 As ADODB.Recordset
        ...
        With gobjAdvice
            ...
            Else
                ...
                For i = .FixedRows To .Rows - 1
                    ...
                    Else
                        blnDo = InStr(",5,6,7,", .TextMatrix(i, gobjCOL.intCOL诊疗类别)) > 0
                        If blnDo Then
                    ...
                    If blnDo Then
                        ...
                        Call PassSetRecipeInfo(str医嘱ID, str药品ID, str药品, _
                                              ...
                                              str用法, .TextMatrix(i, gobjCOL.intCOL相关ID), IIf(glngModel = PM_住院编辑, IIf(.TextMatrix(i, gobjCOL.intCOL期效) = "长嘱", 0, 1), _
                                              IIf(str期效 = "长嘱", 0, 1)), str开嘱医生)
                        lngCount = lngCount + 1
                    End If
                Next
                '无可审查的药品
        ...
        For i = .FixedRows To .Rows - 1
           If glngModel = PM_住院编辑 Then
            ...
            If blnDo Then
                ...
                ElseIf glngModel = PM_住院医嘱清单 Then
                    If Val(.TextMatrix(i, gobjCOL.intCOL相关ID)) <> Val(.TextMatrix(i - 1, gobjCOL.intCOL相关ID)) Then  '一并给药不重复取
                        str给药途径 = Get药嘱给药途径ID(Val(.TextMatrix(i, gobjCOL.intCOL相关ID)))   '传代码
                    End If
                End If
                Call Get频率信息_名称(.TextMatrix(i, gobjCOL.intCOL频率), 0, 0, "", IIf(.TextMatrix(i, gobjCOL.intCOL诊疗类别) = "7", 2, 1), str频率编码)
                ...
                xmlpre.GeneralName = StrToXML(Get项目名称(.TextMatrix(i, gobjCOL.intCOL诊疗项目ID)))
                xmlpre.HosMediCode = .TextMatrix(i, gobjCOL.intCOL收费细目ID)
                xmlpre.MediName = StrToXML(str药品)
...
errH:
    ...
    InAdviceCheckWarn_DT = False
End Function
Public Function InAdviceCheckWarn_TYT(ByVal lngCmd As Long, Optional ByVal lngRow As Long, Optional blnIsHaveOut As Boolean, Optional ByRef blnNoSave As Boolean) As Long
'功能：调用太元通系统中对医嘱进行合理用药审查等相关功能
'参数：lngCmd=
...
'返回值：医嘱保存调用，需要用返回值判断是否存在禁忌用药
    ...
    Dim strSQL As String, strOrderInfo As String, str频率编码 As String, str频率 As String
    Dim int频率次数 As Integer, int频率间隔 As Integer, str间隔单位 As String
    Dim str给药途径 As String, str药品 As String, str期效 As String
    Dim blnDo As Boolean
    Dim curDate As Date
    Dim rsPati As ADODB.Recordset
    Dim rsTmp As ADODB.Recordset
    ...
    Dim i As Long, k As Long, lngMaxWarn As Long
    Dim lngBegin As Long, lngEnd As Long
    Dim strTmp As String, strOld As String
    Dim arrTmp As Variant, colAuditResult As Collection
    Dim arrLight(1 To 3) As String
...
'功能：对PASS接口进行注册和初始化，同时检查PASS接口DLL是否正确安装
    ...
    If gbytPass = MK Then
        ...
        ElseIf gstrVersion = "4.0" Then
            ...
            strHisCode = IIf(gstrNodeNo = "-", "0", gstrNodeNo)
            lngTmp = MDC_Init(strCheckMode, strHisCode, UserInfo.编号)
            If lngTmp <= 0 Then   'YWJ需要传人模式
                MsgBox "PASS4.0系统初始化失败，当前合理用药监测系统不可用，请检查相关配置是否正确。" & vbCrLf & _
                        "返回值:" & lngTmp, vbInformation, gstrSysName
                Exit Function
            End If
        End If
    ElseIf gbytPass = DT Then
        ...
        ElseIf gstrVersion = "4.0" Then 'BS版
            ...
            WriteLog gstrBaseXml: WriteLog strDetails
            lngTmp = CRMS_UI(DTBS_登录, gstrBaseXml, strDetails, "")
            WriteLog "登录接口返回值:" & lngTmp
        End If
    ...
    ElseIf gbytPass = YWS Then '广州保进 药卫士
        ...
        If Err.Number <> 0 Then
            MsgBox "药卫士登录失败,可能合理用药监测系统未正确安装或配置。" & _
                   vbCrLf & "在正确安装和配置合理用药监测系统之前，相应的功能不能使用。", vbInformation, gstrSysName
            WriteLog "动态创建组件——错误号:" & Err.Number & "|错误描述:" & Err.Description
            Exit Function
        End If
        Err.Clear: On Error GoTo 0
        With udtBase
        ...
        WriteLog gstrBaseXml
        strRet = gobjPass.YWS_UI(YWS_登录, gstrBaseXml, "", "")
        WriteLog "PassInitialize 登录接口返回值:" & lngTmp
...
'输液配置中心调用
'--------------------------------------------------------------------------------------------
Public Function PIVAAdviceCheckWarn_MK(ByVal int单据 As Integer, ByVal strNo As String, ByVal lngCmd As Long, Optional ByVal lngRow As Long) As Long
'功能：调用Pass系统相关功能
'参数：lngCmd=
'        0-检测设置PASS菜单状态
'        21-病生状态/过敏史管理(只读)
...
'      单药警告：应在用药审查过之后进行调用(有警告值)
    ...
    If str挂号单 <> "" Then               '门诊病人
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, G_STR_PASS, lngPatiID, str挂号单)
        If rsTmp.EOF Then
            Screen.MousePointer = 0
            Exit Function
        End If
    ...
    Else                                    '住院病人
        strSQL = _
            ...
            " Where A.病人ID=B.病人ID And B.出院科室ID=C.ID" & _
            " And B.住院医师=D.姓名(+) And A.病人ID=[1] And B.主页ID=[2]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, G_STR_PASS, lngPatiID, lng主页ID)
        If rsTmp.EOF Then
            Screen.MousePointer = 0
            Exit Function
        End If
    ...
    If lngCmd = 0 Then
        '取药品名称
         str药品 = gobjAdvice.TextMatrix(lngRow, gobjCOL.intCOL药品名称)
         lng药品id = gobjAdvice.TextMatrix(lngRow, gobjCOL.intCOL收费细目ID)
         str单量单位 = gobjAdvice.TextMatrix(lngRow, gobjCOL.intCOL单量单位)
         '取药品给药途径
         str用法 = gobjAdvice.TextMatrix(lngRow, gobjCOL.intCOL用法)
        ...
        '设置菜单可用状态，在zlPASSPopupCommandBars中设置
        PIVAAdviceCheckWarn_MK = 1 '表示可以弹出菜单
...
'--处方发药明细调用
'--------------------------------------------------------------------------------------------
Public Function RecipeAdviceCheckWarn_MK(ByVal int单据 As Integer, ByVal strNo As String, ByVal lngCmd As Long, Optional ByVal lngRow As Long) As Long
'功能：调用Pass系统相关功能
'参数：lngCmd=
'        0-检测设置PASS菜单状态
'        21-病生状态/过敏史管理(只读)
...
'      单药警告：应在用药审查过之后进行调用(有警告值)
    ...
    With gobjAdvice
        If lngCmd = 0 Then
            '取药品名称
            str药品 = .TextMatrix(lngRow, gobjCOL.intCOL药品名称)
            lng药品id = .TextMatrix(lngRow, gobjCOL.intCOL收费细目ID)
            str单量单位 = Mid(.TextMatrix(lngRow, gobjCOL.intCOL单量), InStr(.TextMatrix(lngRow, gobjCOL.intCOL单量), "(") + 1)
...
'            Call SetPassMenuState
            ...
            RecipeAdviceCheckWarn_MK = 1 '表示可以弹出菜单
    
            Screen.MousePointer = 0: Exit Function
        ElseIf lngCmd = 6 Then
            Call PassSetWarnDrug(Val(.TextMatrix(lngRow, gobjCOL.intCOL相关ID)))  '单药警告(已警告的医嘱唯一码)
        Else
            '用药审核或用药研究
            lngCount = 0
            str药品 = "": str用法 = "": str频率 = ""
            i = 1
            If .TextMatrix(i, gobjCOL.intCOL开嘱医生) <> "" Then
                strSQL = "select 编号 from 人员表 where 姓名=[1]"
                Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "", .TextMatrix(i, gobjCOL.intCOL开嘱医生))
            End If
            For i = .FixedRows To .Rows - 1
                blnDo = Val(.TextMatrix(i, gobjCOL.intCOLID)) <> 0 And Val(.TextMatrix(i, gobjCOL.intCOL收费细目ID)) <> 0
                If blnDo Then
                    '取药品名称
                    str药品 = .TextMatrix(i, gobjCOL.intCOL药品名称)
                    ...
                    '取药品给药途径
                    str用法 = .TextMatrix(i, gobjCOL.intCOL用法)
                    
                    '取用药频率(次/天),都为整数四舍五入
                    If .TextMatrix(i, gobjCOL.intCOL间隔单位) = "天" Then
                        str频率 = .TextMatrix(i, gobjCOL.intCOL频率次数) & "/" & .TextMatrix(i, gobjCOL.intCOL频率间隔)
                    ElseIf .TextMatrix(i, gobjCOL.intCOL间隔单位) = "周" Then
                        str频率 = .TextMatrix(i, gobjCOL.intCOL频率次数) & "/7"
                    ElseIf .TextMatrix(i, gobjCOL.intCOL间隔单位) = "小时" Then
                        If Val(.TextMatrix(i, gobjCOL.intCOL频率间隔)) <= 24 Then
                            str频率 = Format(24 / Val(.TextMatrix(i, gobjCOL.intCOL频率间隔)) * Val(.TextMatrix(i, gobjCOL.intCOL频率次数)), "0") & "/1"
                        Else
                            str频率 = Val(.TextMatrix(i, gobjCOL.intCOL频率次数)) & "/" & Format(Val(.TextMatrix(i, gobjCOL.intCOL频率间隔)) / 24, "0")
                        End If
                    ElseIf .TextMatrix(i, gobjCOL.intCOL间隔单位) = "分钟" Then
                        str频率 = Format((24 * 60) / Val(.TextMatrix(i, gobjCOL.intCOL频率间隔)) * Val(.TextMatrix(i, gobjCOL.intCOL频率次数)), "0") & "/1"
                    End If
    ...
    '                            "相关id：" & .TextMatrix(i, mIntCol相关id) & ";医嘱标志：" & .TextMatrix(i, mIntCol医嘱标志) & ";医生情况：" & rsTmp!编号 & "\" & .TextMatrix(i, mIntCol开嘱医生)
                    '传入医嘱信息
                    Call PassSetRecipeInfo(.TextMatrix(i, gobjCOL.intCOLID), .TextMatrix(i, gobjCOL.intCOL收费细目ID), str药品, _
                        Mid(.TextMatrix(i, gobjCOL.intCOL单量), 1, InStr(1, .TextMatrix(i, gobjCOL.intCOL单量), "(") - 1), Mid(.TextMatrix(i, gobjCOL.intCOL单量), InStr(1, .TextMatrix(i, gobjCOL.intCOL单量), "(") + 1, InStr(1, .TextMatrix(i, gobjCOL.intCOL单量), ")") - InStr(1, .TextMatrix(i, gobjCOL.intCOL单量), "(") - 1), str频率, _
                        Format(.TextMatrix(i, gobjCOL.intCOL开始时间), "yyyy-MM-dd"), Format(.TextMatrix(i, gobjCOL.intCOL终止时间), "yyyy-MM-dd"), str用法, _
                        .TextMatrix(i, gobjCOL.intCOL相关ID), .TextMatrix(i, gobjCOL.intCOL期效), rsTmp!编号 & "\" & .TextMatrix(i, gobjCOL.intCOL开嘱医生))
                    lngCount = lngCount + 1
                End If
            Next
...
Public Function InAdviceCheckWarn_MK4(Optional ByVal bytShow As Byte = 0, Optional ByVal bytSubmit As Byte = 0, _
        Optional blnIsHaveOut As Boolean, Optional ByRef blnNoSave As Boolean) As Long
'功能：调用Pass系统中对医嘱进行合理用药审查等相关功能
'参数：
'      返回：blnIsHaveOut=是否存在离院带药的药品
'返回：本次审核返回的最高级别警示值,为-1,-2,-3表示没有进行审查
...
'      单药警告：应在用药审查过之后进行调用(有警告值)
    ...
    Dim str医生嘱托 As String, str滴速 As String, strGroupIDs As String
    Dim str开嘱医生 As String, str医生编码 As String, str开嘱医生Tag As String
    Dim str总量 As String, str总量单位 As String, str用药目的 As String
    ...
    With gobjAdvice
        ...
        For i = .FixedRows To .Rows - 1
            ...
            Else
                ...
                If blnDo Then
                    ...
                    str开始时间 = Format(.Cell(flexcpData, i, gobjCOL.intCOL开始时间), "yyyy-MM-dd")
                    blnDo = blnDo And (str状态 = "4" And Format(DateAdd("D", 7, str开始时间), "YYYY-MM-dd") > Format(curDate, "yyyy-MM-dd") Or _
            ...
            If blnDo Then
                ...
                If str频率 = "" Then
                    ...
                    If str开嘱医生Tag <> str开嘱医生 And str开嘱医生 <> "" Then
                        str医生编码 = RowValue("人员表", str开嘱医生, "编号", "姓名")
                        str开嘱医生Tag = str开嘱医生
                    End If
                ...
                '开嘱科室名称
                str开嘱科室ID = .TextMatrix(i, gobjCOL.intCOL开嘱科室ID)
                If str开嘱科室ID <> str开嘱科室IDTag And Val(str开嘱科室ID) <> 0 Then
                    str开嘱科室 = RowValue("部门表", Val(.TextMatrix(i, gobjCOL.intCOL开嘱科室ID)), "名称")
        ...
        If glngModel = PM_住院医嘱清单 Then
            ...
            strSQL = "Select a.Id As 医嘱id, a.相关id, a.序号 As 医嘱序号, a.诊疗类别, a.医嘱期效, a.医嘱状态, a.诊疗项目id, NVl(a.收费细目id,f.药品ID) as 药品ID, Decode(a.诊疗类别||'','7',a.医嘱内容,a.标本部位) as 药品名称, a.执行频次 as 频率, a.单次用量, a.总给予量," & vbNewLine & _
                "       a.执行标记, a.开始执行时间, a.开嘱时间,a.开始执行时间 as 开始时间,a.执行终止时间 as 结束时间, a.医生嘱托, a.开嘱科室id, e.名称 As 开嘱科室, a.开嘱医生, a.用药目的, b.计算单位 as 单量单位, c.住院单位 as 总量单位," & vbNewLine & _
        ...
        'PASS审查函数MDC_DoCheck
        Call AdviceCheckWarn_MK4(gobjPati.lng病人ID, "", gobjPati.lng主页ID, bytShow, bytSubmit, rsAdvice, str警示)
        ...
        If str警示 <> "" Then
            ...
            For i = LBound(arrTmp) To UBound(arrTmp)
                ...
                If glngModel = PM_住院医嘱清单 Then
                    r = .FindRow(str医嘱ID, .FixedRows, gobjCOL.intCOLID)
                ElseIf glngModel = PM_住院编辑 Then
                    r = .FindRow(CLng(str医嘱ID))
                End If
                '设置警示灯
                If r <> -1 Then
                    strOld = .Cell(flexcpData, r, gobjCOL.intCOL警示)
                    If Val(str警示值) >= 0 And Val(str警示值) <= 4 Then
                        .Cell(flexcpData, r, gobjCOL.intCOL警示) = str警示值
                        Set .Cell(flexcpPicture, r, gobjCOL.intCOL警示) = frmIcons.imgPass.ListImages(arrLight(Val(str警示值))).Picture
                    Else
                        .Cell(flexcpData, r, gobjCOL.intCOL警示) = ""
                        Set .Cell(flexcpPicture, r, gobjCOL.intCOL警示) = Nothing
                    End If
                    
                    If PM_住院编辑 = glngModel Then
                        If strOld <> CStr(.Cell(flexcpData, r, gobjCOL.intCOL警示)) Then
                            .Cell(flexcpData, r, gobjCOL.intCOL序号) = 1
                            blnNoSave = True    '标记为未保存
                        End If
                    ElseIf PM_住院医嘱清单 = glngModel Then
                        If CStr(.Cell(flexcpData, r, gobjCOL.intCOL警示)) <> strOld Then
                            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
                            arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_更新审查(" & str医嘱ID & "," & IIf(Val(str警示值) >= 0 And Val(str警示值) <= 4, Val(str警示值), "NULL") & ")"
                        End If
                    
                    End If
                Else
                    strAdvicesIds = strAdvicesIds & "," & str医嘱ID & ":" & str警示值
                End If
                ...
                If Val(str警示值) >= 0 Then
                    If lngMaxWarn >= 0 Then
                        If arrLevel(Val(str警示值)) > arrLevel(lngMaxWarn) Then
                            lngMaxWarn = Val(str警示值)
                        End If
                    Else
                        lngMaxWarn = Val(str警示值)
                    End If
...
'说明：用药审查：涉及当天下的临嘱(包括已执行)，和未停止的长嘱
    ...
    Dim rsAdvice As ADODB.Recordset
    Dim rsTmp As ADODB.Recordset
    Dim str药品名称 As String, str用法 As String, str频率 As String, str用法ID As String
    ...
    Dim str总量 As String, str总量单位 As String, str用药目的 As String, str医生嘱托 As String
    ...
    Dim str警示 As String, str警示值 As String, str滴速 As String, str处方序号 As String
    ...
    '* 诊断信息
    If glngModel = PM_门诊编辑 Then
        ' PASS增加一条标准化的诊断记录（多条重复调用）
        Set rs诊断 = InitAdviceRS(FUN_诊断信息)
        If Not gobjDiags Is Nothing Then
            With gobjDiags
                For i = 1 To .Count
                    If .Item(i).str诊断描述 <> "" Then
            ...
            End With
        End If
        If rs诊断.RecordCount > 0 Then rs诊断.MoveFirst
    End If
     '传入病人医嘱信息
    '-------------------------------------------------------------
    ...
    With gobjAdvice
            ...
            If blnDo Then
                ...
                If str用法 = "" Then
                    ...
                    If glngModel = PM_门诊编辑 Then
                        ...
                        End If
                    Else
                        'str用法 = RowValue("病人医嘱记录", Val(.TextMatrix(i, gobjCOL.intCOL相关ID)), "医嘱内容")
                    End If
                End If
                ...
                If str开嘱科室ID <> str开嘱科室Tag And str开嘱科室ID <> "" Then
                    str开嘱科室 = RowValue("部门表", Val(str开嘱科室ID), "名称")
                ...
                If glngModel = PM_门诊编辑 Then
                    str医嘱ID = .RowData(i)
                    str处方序号 = .TextMatrix(i, gobjCOL.intCol处方号)
                    str医嘱序号 = .TextMatrix(i, gobjCOL.intCOL序号)
                    str单次用量 = .TextMatrix(i, gobjCOL.intCOL单量)
                    str单量单位 = .TextMatrix(i, gobjCOL.intCOL单量单位)
                    str总量 = .TextMatrix(i, gobjCOL.intCOL总量)
                ...
                rsAdvice!医嘱ID = str医嘱ID
                rsAdvice!相关ID = str相关ID
                rsAdvice!医嘱期效 = "0" '门诊固定传0,住院:0-长嘱,1-代表临嘱
        ...
        Call AdviceCheckWarn_MK4(gobjPati.lng病人ID, gobjPati.str挂号单, 0, bytShow, bytSubmit, rsAdvice, str警示, rs诊断)
...
Public Function AdviceCheckWarn_MK4_YF(ByVal lngPatiID As Long, ByVal lng主页ID As Long, Optional ByVal str挂号单 As String, _
                Optional ByVal str医嘱IDs As String, Optional str警示 As String) As Long
'功能：调用Pass4系统审查功能
'参数:
'    str医嘱IDs-需要审查的医嘱ID
'    str警示-医嘱ID:警示值,医嘱ID2:警示值2
...
'说明：
    ...
    With rsTmp
        For i = 1 To .RecordCount
            ...
            rsAdvice!医嘱ID = !医嘱ID & ""
            rsAdvice!相关ID = !相关ID & ""
            rsAdvice!医嘱期效 = IIf(str挂号单 <> "", 0, !医嘱期效 & "") '门诊固定传0,
            rsAdvice!医嘱序号 = !医嘱序号 & ""
            ...
            If str医生 <> str开嘱医生 And str开嘱医生 <> "" Then
                ...
                str开嘱医生编码 = RowValue("人员表", str开嘱医生, "编号", "姓名")
            End If
            ...
            Else '临时医嘱
                rsAdvice!结束时间 = Format(!开始时间 & "", "YYYY-MM-dd hh:mm:ss")
            End If
        
            rsAdvice!开嘱时间 = Format(!开始时间 & "", "YYYY-MM-dd hh:mm:ss")
            rsAdvice!开始时间 = Format(!开始时间 & "", "YYYY-MM-dd hh:mm:ss")
            If glngModel = PM_处方发药 Then
                rsAdvice!结束时间 = ""   '门诊药房默认传空串,便于审查重复用药
            End If
            rsAdvice!总量 = !总量 & ""
            If str挂号单 <> "" Then
                rsAdvice!总量单位 = !门诊单位 & ""
    ...
    Call AdviceCheckWarn_MK4(lngPatiID, str挂号单, lng主页ID, 1, 0, rsAdvice, str警示)  '显示审查界面,不采集数据
...
'功能：调用保进药卫士用药监测系统对医嘱进行合理用药审查等相关功能
    ...
    With udtPati
        ...
        Set rsTmp = Get病人过敏记录(lngPatiID, lng主页ID)
        For i = 1 To rsTmp.RecordCount
            If "" & rsTmp!药物ID <> "" Then
                With udt过敏源
        ...
        If strTmp <> "" Then
            ...
            For i = LBound(arrTmp) To UBound(arrTmp)
                With udt诊断
                    .str诊断类型 = "1" '1=病生理状态
                    .str诊断代码 = RowValue("病生理情况", arrTmp(i), "编码", "名称")
                    .str诊断名称 = arrTmp(i)
                End With
                colTmp.Add udt诊断, "_" & colTmp.Count + 1
            Next
    ...
    '刷新界面
    strTmp = gobjPass.YWS_UI(YWS_初始客户端, gstrBaseXml, "", "")
    If udtPres.col药品信息.Count > 0 Then
        ...
        Call WriteLog("返回值:" & strTmp & vbCrLf & strRetXML)
        If str警示 <> "-1" Then
...
Public Function AdviceCheckWarn_MK4(ByVal lngPatiID As Long, ByVal str挂号单 As String, ByVal lng主页ID As Long, ByVal bytShow As Byte, _
                ByVal bytSubmit As Byte, ByVal rsAdvice As ADODB.Recordset, Optional str警示 As String = "", Optional ByVal rs诊断 As ADODB.Recordset) As String
'功能：美康4.0自定义接口
...
'str警示-返回警示串：格式：医嘱ID1:警示值1,医嘱ID2:警示值2（缺省不返回警示值）
    ...
    colTemp.Add 4, "K" & "4"
     ...
     If str挂号单 <> "" Then
        ...
        Call MDC_SetPatient(lngPatiID & "_" & str挂号单, rsTmp!门诊号 & "", rsTmp!门诊号 & "", rsTmp!姓名 & "", Nvl(rsTmp!性别), Format(rsTmp!出生日期, "yyyy-MM-dd"), _
            str身高, str体重, rsTmp!科室ID & "", rsTmp!科室名 & "", rsTmp!医生码 & "", rsTmp!医生名 & "", str急诊标识, _
            CLng(colTemp("K" & lng哺乳)), CLng(colTemp("K" & lng妊娠)), str妊娠日期, CLng(colTemp("K" & lng肝功)), CLng(colTemp("K" & lng肾功)))
     
     Else
         Call MDC_SetPatient(lngPatiID & "", rsTmp!住院号 & "", lng主页ID & "", rsTmp!姓名 & "", rsTmp!性别 & "", _
             Format(rsTmp!出生日期, "yyyy-MM-dd"), rsTmp!身高 & "", rsTmp!体重 & "", _
             rsTmp!科室ID & "", rsTmp!科室名 & "", rsTmp!医生码 & "", rsTmp!医生名 & "", 1, _
             CLng(colTemp("K" & lng哺乳)), CLng(colTemp("K" & lng妊娠)), str妊娠日期, CLng(colTemp("K" & lng肝功)), CLng(colTemp("K" & lng肾功)))
     End If
    ...
    If glngModel <> PM_门诊编辑 Then
        ...
        Next
    Else
        If Not rs诊断 Is Nothing Then
            For i = 1 To rs诊断.RecordCount
                Call MDC_AddMedCond(i & "", rs诊断!诊断编码 & "", rs诊断!诊断名称 & "", "")
                rs诊断.MoveNext
            Next
        End If
    End If
    ...
    On Error GoTo errH
     ...
     With rsAdvice
         For i = 1 To .RecordCount
            ...
            strTmp = ""
            If glngModel = PM_门诊编辑 Or glngModel = PM_门诊医嘱清单 Then strTmp = !处方序号
            ...
            '滴速
            If !滴速 & "" <> "" Then
               strTmp = FuncGetDripInfo(!医嘱ID & "", !滴速 & "")
               If strTmp <> "" Then Call MDC_AddJsonInfo(strTmp)
            End If
            lngCount = lngCount + 1
            .MoveNext
         Next
...
Public Function GetPatiInfo_YF(ByVal lngPatiID As Long, ByVal str挂号单 As String, ByVal lng主页ID As Long) As ADODB.Recordset
    ...
    If gbytPass = MK And gstrVersion = "3.0" Then
        If str挂号单 <> "" Then               '门诊病人
            strSQL = "Select B.ID as 就诊ID,A.姓名,A.性别,A.出生日期," & _
                " C.编码 as 科室码,C.名称 as 科室名,E.编号 as 医生码,E.姓名 as 医生名" & _
                " From 病人信息 A,病人挂号记录 B,部门表 C,人员表 E" & _
                " Where A.病人ID=B.病人ID And B.执行部门ID=C.ID " & _
                " And B.执行人=E.姓名(+) And A.病人ID=[1] And B.NO=[2] and B.记录性质=1 And B.记录状态=1 "
    ...
    ElseIf gbytPass = TYT Then
        ...
        Else
            strSQL = _
            " Select A.住院号,A.当前床号,Nvl(B.姓名,A.姓名) 姓名,Nvl(B.性别,A.性别) 性别 ,A.出生日期,A.年龄,A.身份证号,B.身高,B.体重  " & _
                     " From 病人信息 A,病案主页 B" & _
...
'功能:获取药嘱信息
    ...
    If bytFunc = 0 Then
        If str医嘱IDs <> "" Then
            ...
            "       b.名称 As 药品名称, b.计算单位 As 单量单位, b.执行频率, g.名称 As 用法, c.诊疗项目id As 用法id, a.开嘱科室id, d.名称 As 开嘱科室, a.用药目的,a.用药理由,a.医生嘱托,F.规格, " & vbNewLine & _
        ...
        Else
            strSQL = "Select a.Id As 医嘱id, a.相关id, a.医嘱期效, a.序号 As 医嘱序号,a.紧急标志 as 标志,a.开嘱医生, a.医嘱状态, a.开嘱时间, a.开始执行时间 As 开始时间, a.执行终止时间 As 结束时间, a.诊疗类别," & vbNewLine & _
                     "       a.诊疗项目id, a.收费细目id, a.执行频次 As 频率, a.间隔单位, a.频率次数, a.频率间隔, a.单次用量, a.总给予量 As 总量, e.门诊包装, e.门诊单位, e.住院单位, a.天数," & vbNewLine & _
                     "       b.名称 As 药品名称, b.计算单位 As 单量单位, b.执行频率, g.名称 As 用法, c.诊疗项目id As 用法id, a.开嘱科室id, d.名称 As 开嘱科室, a.用药目的,a.用药理由,a.医生嘱托,F.规格,Decode(G.类别||'_'||G.操作类型||'_'||G.执行分类,'E_2_1',C.医生嘱托,'') As 滴速, NULL As 处方号" & vbNewLine & _
                     "From 病人医嘱记录 A, 诊疗项目目录 B, 病人医嘱记录 C, 部门表 D, 药品规格 E,收费项目目录 F, 诊疗项目目录 G " & vbNewLine & _
                     "Where a.诊疗项目id = b.Id And a.相关id = c.Id(+) And a.开嘱科室id = d.Id(+) And a.收费细目id = e.药品id(+) And a.收费细目id=F.ID(+) And " & vbNewLine & _
    ...
    ElseIf bytFunc = 1 Then
        strSQL = "Select a.Id As 医嘱id, a.相关id, a.医嘱期效, a.序号 As 医嘱序号,a.紧急标志 as 标志,a.开嘱医生, a.医嘱状态, a.开嘱时间, a.开始执行时间 As 开始时间, a.执行终止时间 As 结束时间, a.诊疗类别," & vbNewLine & _
                "       a.诊疗项目id, a.收费细目id, a.执行频次 As 频率, a.间隔单位, a.频率次数, a.频率间隔, a.单次用量, a.总给予量 As 总量, e.门诊包装, e.门诊单位, e.住院单位, a.天数," & vbNewLine & _
                "       b.名称 As 药品名称, b.计算单位 As 单量单位, b.执行频率, g.名称 As 用法, c.诊疗项目id As 用法id, a.开嘱科室id, d.名称 As 开嘱科室, a.用药目的, a.用药理由, a.医生嘱托,F.规格," & vbNewLine & _
                "       Decode(G.类别||'_'||G.操作类型||'_'||G.执行分类,'E_2_1',C.医生嘱托,'') As 滴速,NULL As 处方号,a.处方号 AS 处方ID,A.执行性质 AS A执行性质,C.执行性质 AS B执行性质,G.类别,G.操作类型,G.执行分类  " & vbNewLine & _
                "From 病人医嘱记录 A, 诊疗项目目录 B, 病人医嘱记录 C, 部门表 D, 药品规格 E,收费项目目录 F, 诊疗项目目录 G " & vbNewLine & _
                "Where a.诊疗项目id = b.Id And a.相关id = c.Id(+) And a.开嘱科室id = d.Id(+) And a.收费细目id = e.药品id(+) And a.收费细目id=F.ID(+) And " & vbNewLine & _
                "      c.诊疗项目id = g.Id(+) And a.诊疗类别 In ('5', '6', '7') And Nvl(A.执行标记,0)<>-1 " & strSQL & vbNewLine & _
                "Order By a.序号"
    ElseIf bytFunc = 2 Then
            strSQL = "Select /*+cardinality(H,10) */ a.病人ID,a.挂号单,a.Id As 医嘱id, a.相关id,a.医嘱期效, a.序号 As 医嘱序号,a.紧急标志 as 标志,a.开嘱医生, a.医嘱状态, a.开嘱时间, a.开始执行时间 As 开始时间, a.执行终止时间 As 结束时间, a.诊疗类别," & vbNewLine & _
            "       a.诊疗项目id, a.收费细目id, a.执行频次 As 频率, a.间隔单位, a.频率次数, a.频率间隔, a.单次用量, a.总给予量 As 总量, e.门诊包装, e.门诊单位, e.住院单位, a.天数," & vbNewLine & _
            "       b.名称 As 药品名称, b.计算单位 As 单量单位, b.执行频率, g.名称 As 用法, c.诊疗项目id As 用法id, a.开嘱科室id, d.名称 As 开嘱科室, a.用药目的,a.用药理由,a.医生嘱托,F.规格, " & vbNewLine & _
            "  Decode(G.类别||'_'||G.操作类型||'_'||G.执行分类,'E_2_1',C.医生嘱托,'') As 滴速, H.C2 As 处方号" & vbNewLine & _
            "From 病人医嘱记录 A, 诊疗项目目录 B, 病人医嘱记录 C, 部门表 D, 药品规格 E,收费项目目录 F, 诊疗项目目录 G,Table(f_Str2list2([4])) H " & vbNewLine & _
            "Where a.诊疗项目id = b.Id And a.相关id = c.Id(+) And a.开嘱科室id = d.Id(+) And a.收费细目id = e.药品id(+) And a.收费细目id=F.ID(+) And " & vbNewLine & _
            "      c.诊疗项目id = g.Id(+) And a.诊疗类别 In ('5', '6', '7') And A.相关ID = H.C1" & vbNewLine & _
            "Order By a.序号"
    End If
    On Error GoTo errH
            ...
            If blnDo Then
                ...
                Else
                    ...
                    ElseIf glngModel = PM_住院医嘱清单 Then
                        str医嘱ID = CStr(.TextMatrix(i, gobjCOL.intCOLID))
                        If Val(.TextMatrix(i, gobjCOL.intCOL相关ID)) <> Val(.TextMatrix(i - 1, gobjCOL.intCOL相关ID)) Then  '一并给药不重复取
                            str给药途径 = RowValue("病人医嘱记录", Val(.TextMatrix(i, gobjCOL.intCOL相关ID)), "诊疗项目ID")   '传代码
                        End If
                    End If
                    ...
                    udtMedic.str商品名 = YWS_StrToXML(RowValue("诊疗项目目录", Val(.TextMatrix(i, gobjCOL.intCOL诊疗项目ID)), "名称"))
    ...
    If udtPres.col药品信息.Count > 0 Then
        ...
        If glngModel = PM_住院编辑 Then
            ...
            If strTmp <> "0" Then
                ...
                Call WriteLog("保存处方:返回值" & strTmp)   '保存处方
            End If
        End If
    End If
        ...
        For i = .FixedRows To .Rows - 1
            ...
            Else
                blnDo = (Val(.TextMatrix(i, gobjCOL.intCOLID)) <> 0 And InStr(",5,6,7,", .TextMatrix(i, gobjCOL.intCOL诊疗类别)) > 0 And Val(.TextMatrix(i, gobjCOL.intCOL收费细目ID)) <> 0 _
                    ...
                    And Format(.TextMatrix(i, gobjCOL.intCOL开嘱时间), "yyyy-MM-dd") = Format(curDate, "yyyy-MM-dd")
            End If
            blnDo = blnDo And .TextMatrix(i, gobjCOL.intCOL状态) <> 4 '作废的医嘱不传入
            If blnDo Then
...
'      rsOut=禁忌药品说明
    ...
    If udtPres.col药品信息.Count > 0 Then
        ...
        WriteLog "功能号:" & DTBS_处方分析
        WriteLog "DetailXML:" & strXML
    
        lngTmp = CRMS_UI(DTBS_处方分析, gstrBaseXml, strXML, strRetXML)
        strRetXML = StrConvToNormal(strRetXML)
        WriteLog "处方分析返回值:" & lngTmp & vbCrLf & "RetXML:" & strRetXML
        ...
        If glngModel = PM_门诊编辑 Then
           ...
           If (lngTmp = 2 Or lngTmp = 3) And gbytBlackLamp = 0 Then
                MsgBox "用药监测系统发现当前医嘱存在禁忌用药，操作不能继续!", vbExclamation + vbOKOnly, gstrSysName
                Exit Function
...
Public Function CreateAdviceRS(Optional ByVal lng病人ID As String, Optional ByVal lng主页ID As String, Optional ByVal str挂号单 As String, _
...
Private Function GetDrugInfo_MK4(ByVal str挂号单 As String, ByVal strAdvice As String) As ADODB.Recordset
'功能：获取病人过敏记录
'参数：bytFunc=1 传入病人历次所有过敏记录;0=传入病人本次就诊过敏记录
    ...
    strSQL = "Select a.Id, a.相关id, a.处方序号, d.名称 As 用法 " & vbNewLine & _
            "From 病人医嘱记录 A, 病人医嘱记录 B, 诊疗项目目录 D" & vbNewLine & _
            "Where a.相关id = b.Id(+) And b.诊疗项目id = d.Id(+) And A. 挂号单 = [1] And a.相关id <> 0 And Instr([2], ',' || a.Id || ',') > 0" & vbNewLine & _
            "Order By a.序号"
    Set GetDrugInfo_MK4 = zlDatabase.OpenSQLRecord(strSQL, "mdlPass", str挂号单, "," & strAdvice & ",")
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
