...
'功能：初始化模块参数
    ...
    ElseIf lngModul = P病人入出管理 Then  '1132
...
'功能：产生工本费、就诊卡等填写病人费用记录时的必须信息(收费类别,收费细目ID,计算单位,收入项目ID,收入项目,收据费目,原价,现价,是否变价,科室标志)
'参数：strClass=工本费、就诊卡、病历费
    ...
    If lng收费细目ID = 0 Then
        strSQL = _
            " Select A.类别 as 收费类别,A.ID as 收费细目ID,A.计算单位,C.ID as 收入项目ID, " & _
            " C.名称 as 收入项目,C.收据费目,B.原价,B.现价,Nvl(A.是否变价,0) as 是否变价,Nvl(B.缺省价格,0) 缺省价格,Nvl(A.执行科室,0) as 科室标志 " & _
    ...
    Else
        strSQL = _
            ...
            " And Sysdate Between B.执行日期 And Nvl(B.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlInPatient", strClass, lng收费细目ID)
...
'参数：byt入住方式:0-入院入科，1-转科入科，2-入病区
    ...
    If byt入住方式 = 0 Then '
        strSQL = "Select NVL(B.姓名,A.姓名) 姓名, Nvl(NVL(B.性别,A.性别),'未知') 性别, B.年龄,To_Char(A.出生日期,'YYYY-MM-DD HH24:MI:SS') As 出生日期, B.险类, B.病人性质, B.当前病况, B.护理等级id, B.住院医师, B.门诊医师, B.责任护士, B.出院科室id, B.出院科室id 入住科室id, B.医疗小组id, " & vbNewLine & _
            "       To_Char(B.入院日期, 'YYYY-MM-DD HH24:MI:SS') As 入院时间, B.当前病区id, B.住院号, D.名称 as 当前科室,E.名称 as 当前病区, B.出院病床 as 主要床号,B.是否确诊,B.确诊日期, B.出院日期, B.出院方式, B.尸检标志,B.随诊标志,B.随诊期限,B.再入院" & vbNewLine & _
...
'功能:检查病人出院时间之后是否存在护理数据
    ...
    If rsTemp.RecordCount > 0 Then
        ...
        strSQL = _
            ...
            " From 病人护理文件 a, 病人护理数据 b, 病历文件列表 c" & vbNewLine & _
            " Where a.Id = b.文件id And b.发生时间 >= [3] And a.病人id = [1] And a.主页id = [2] And a.格式id = c.Id And c.种类 = 3 And " & vbNewLine & _
            "      c.保留 <> -1 And (Nvl(a.婴儿, 0) = 0 Or" & vbNewLine & _
...
ByVal strInput As String, ByRef blnCancel As Boolean, Optional ByVal intFlag = -1, Optional ByVal lngDeptID = 0, Optional ByVal bln仅操作员部门 As Boolean = False) As ADODB.Recordset
    ...
    vPoint = GetCoordPos(fra入院.hWnd, obj.Left, obj.Top)
...
'返回：病人占用的床位记录集
    ...
    On Error GoTo errH
    strSQL = "Select A.床号, A.房间号, A.性别分类, A.等级id As 床位等级id, B.名称 As 床位等级, A.科室ID, A.共用, C.性别" & vbNewLine & _
