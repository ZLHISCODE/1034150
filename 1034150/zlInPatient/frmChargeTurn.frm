...
'    Set panBottom = dkpMan.CreatePane(mObjPancel.Pan_Bottom, 250, 580, DockBottomOf, panThis)
'    panBottom.Title = "功能窗体"
'    panBottom.Tag = mObjPancel.Pan_Bottom
'    panBottom.Handle = picBottom.hWnd
'    panBottom.Options = PaneNoCloseable Or PaneNoFloatable Or PaneNoHideable Or PaneNoCaption
'
'    panBottom.MaxTrackSize.Height = 435 / Screen.TwipsPerPixelY
'    panBottom.MinTrackSize.Height = 435 / Screen.TwipsPerPixelY
...
Public Sub ShowMe(objParent As Object, ByVal lngPatient As Long, ByRef strNos As String, _
    ...
    '出参:strNOS:要进行费用转入的单据,票据,结帐ID,险类(非医保为零):H0000001,F000023,81235,901;H0000002,F000045,81263,901;...
    '返回:
    '编制:刘兴洪
...
Private Function IsYBSingle(ByVal strno As String, ByVal intInsure As Integer) As Boolean
    ...
    blnInsureSingle = gclsInsure.GetCapability(83, , intInsure) Or gclsInsure.GetCapability(85, , intInsure)
...
Public Function ExecuteTurn(ByVal frmMain As Object, ByVal lngModule As Long, ByVal strPrivs As String, _
    ...
    '入参:strNOS-要进行费用转入的单据,票据,结帐ID,险类(非医保为零),单据类型:H0000001,F000023,81235,901;H0000002,F000045,81263,901,收费单(记帐单);...
    ...
    Do While i <= UBound(arrNO)
        ...
        For j = i To i + lngcnt - 1
            ...
            If bln医保单张退 Or (Val(arrInfo(3)) = 0 And Not blnTurnAll) Then
                'Zl_门诊费用转住院_Insert
                ...
                Else
                    ...
                    If mbln立即销帐 = True And arrInfo(4) <> "记帐单" Then
...
''Private Function CheckJzData(ByVal lng结帐ID As Long) As Boolean
''    '---------------------------------------------------------------------------------------------------------------------------------------------
''    '功能:检查输入的单据是否允许退费
''    '返回:合法,返回true,否则返回False
''    '编制:刘兴洪
''    '日期:2011-03-28 17:53:17
''    '---------------------------------------------------------------------------------------------------------------------------------------------
''    On Error GoTo errHandle
''       '刘兴洪 问题:消费卡处理 日期:2010-01-14 09:58:02
''        If zlIsCheckCanelFee(lng结帐ID) = False Then Exit Function
''        If mblnOneCard Then
...
'功能:读取并显示病人指定天数内的门诊费用单据
    ...
    strSql = strSql & " UNION ALL " & _
            " Select /*+ rule */   '√' as 选择,'可转入' as 类别,'记帐单' as 单据,Decode(NULL,Null,'','√') as 医保, A.NO As 单据号, A.实际票号 As 票据号, A.开单人," & vbNewLine & _
    ...
    If chkShow.Value = 0 Then
        strSql = strSql & " UNION ALL " & _
            ...
            " From " & _
            " (Select  /*+ rule */  '' as 选择,'不可转入' as 类别,'记帐单' as 单据,Decode(NULL,Null,'','√') as 医保, A.NO As 单据号, A.实际票号 As 票据号, A.开单人," & vbNewLine & _
            "       LTrim(To_Char(Sum(A.应收金额), '999999999" & gstrDec & "')) As 应收金额," & vbNewLine & _
            "       LTrim(To_Char(Sum(A.实收金额), '999999999" & gstrDec & "')) As 实收金额," & vbNewLine & _
            "       To_Char(A.发生时间, 'YYYY-MM-DD HH24:MI:SS') As 发生时间,0 as 结帐ID,0 as 险类" & vbNewLine & _
            " From 门诊费用记录  A" & vbNewLine & _
            ...
            "           And Not Exists (Select 1 From 门诊费用记录 K Where K.NO=A.NO And K.记录性质=A.记录性质 And K.附加标志 <> 9 Group By K.序号 Having Sum(K.数次) <> 0) " & vbNewLine & _
...
Private Sub cmdSave_Click()
    ...
    Dim strSql As String, rsTemp As ADODB.Recordset
    mstrNOS = ""
    If Not mblnSelPati Then
        '直接保存
        With mshList
            For i = 1 To .Rows - 1
                If .TextMatrix(i, .ColIndex("类别")) = "可转入" And .TextMatrix(i, .ColIndex("选择")) = "√" Then
                    mstrNOS = mstrNOS & ";" & .TextMatrix(i, .ColIndex("单据号")) & "," & .TextMatrix(i, .ColIndex("票据号")) & _
                            "," & .TextMatrix(i, .ColIndex("结帐ID")) & "," & .TextMatrix(i, .ColIndex("险类")) & "," & .TextMatrix(i, .ColIndex("单据"))
                End If
            Next
        End With
        mstrNOS = Mid(mstrNOS, 2)
        If mstrNOS = "" Then
            MsgBox "注意:" & vbCrLf & "    你还未选择要转成住院的单据,不能续继!"
            Exit Sub
        End If
        Unload Me
        Exit Sub
    ...
    With mshList
        ...
        For i = 1 To .Rows - 1
            If .TextMatrix(i, .ColIndex("类别")) = "可转入" And .TextMatrix(i, .ColIndex("选择")) = "√" Then
                If InStr(1, vbCrLf & strno, vbCrLf & .TextMatrix(i, .ColIndex("单据号")) & vbCrLf) = 0 Then
                    If Len(strno) < 30 Then
                        strno = strno & .TextMatrix(i, .ColIndex("单据号")) & vbCrLf
                    ElseIf InStr(1, strno, "...") = 0 Then
                        strno = strno & "..." & vbCrLf
                    End If
                End If
                If .TextMatrix(i, .ColIndex("单据")) = "收费单" Then
                    If CheckBillExistReplenishData(1, , .TextMatrix(i, .ColIndex("单据号"))) Then
                        MsgBox "注意:" & vbCrLf & "    单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录存在补充结算,不能继续!"
                        Exit Sub
                    End If
                End If
                If Val(.TextMatrix(i, .ColIndex("险类"))) <> 0 Then
                    '检查医保单据是否全转出
                    If IsYBSingle(.TextMatrix(i, .ColIndex("单据号")), Val(.TextMatrix(i, .ColIndex("险类")))) = False Then
                        If CheckInsureAll(.TextMatrix(i, .ColIndex("结帐ID"))) = False Then
                            MsgBox "注意:" & vbCrLf & "    医保单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录本次未转出全部相关结算单据,不能继续!"
                            Exit Sub
                        End If
                    End If
                Else
                    If CheckAllTurn(.TextMatrix(i, .ColIndex("单据号"))) = True Then
                        If CheckInsureAll(.TextMatrix(i, .ColIndex("结帐ID"))) = False Then
                            MsgBox "注意:" & vbCrLf & "    单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录本次未转出全部相关结算单据,不能继续!"
                            Exit Sub
                        End If
                    End If
                End If
                If .TextMatrix(i, .ColIndex("单据")) = "记帐单" Then
                    If zlIsExistsSquareCard(.TextMatrix(i, .ColIndex("单据号")), 2) Then
                        '消费卡检查
                        MsgBox "在结帐单：" & .TextMatrix(i, .ColIndex("单据号")) & "中存在消费卡，暂不支持对消费卡的门诊转住院费用,请检查!", vbOKOnly + vbInformation, gstrSysName
                        Exit Sub
                    End If
                    strBalanceID = ""
                    strSql = "Select Distinct 结帐ID From 门诊费用记录 Where Mod(记录性质,10)=2 And No=[1] "
                    Set rsTemp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, .TextMatrix(i, .ColIndex("单据号")))
                    ...
                    If strBalanceID <> "" Then
                        ...
                        If mrsOneCard.RecordCount > 0 Then
                            MsgBox "在结帐单：" & .TextMatrix(i, .ColIndex("单据号")) & "中存在一卡通结算，暂不支持门诊转住院费用,请检查!", vbOKOnly + vbInformation, gstrSysName
                            Exit Sub
                        End If
                        Set mrsOneCard = zlGetThreeCard(strBalanceID)
                        If mrsOneCard.RecordCount > 0 Then
                            MsgBox "在结帐单：" & .TextMatrix(i, .ColIndex("单据号")) & "中存在三方卡结算，暂不支持门诊转住院费用,请检查!", vbOKOnly + vbInformation, gstrSysName
                            Exit Sub
                        End If
                    End If
                End If
    ...
    If MsgBox("你是否真要将如下门诊费用转成住院费用吗?" & vbCrLf & strno, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
        mstrNOS = ""
        Exit Sub
    End If
    mstrNOS = Mid(mstrNOS, 2)
    ...
    If Val(Nvl(mrsInfo!主页ID)) = 0 Then
        MsgBox "注意:该病人还未入院,不能门诊费用转住院费用,请检查!", vbInformation + vbOKOnly, gstrSysName
        Exit Sub
    End If
    If ExecuteTurn(Me, mlngModule, mstrPrivs, mstrNOS, Val(Nvl(mrsInfo!住院号)), Val(Nvl(mrsInfo!主页ID)), CDate(Format(mrsInfo!入院日期, "yyyy-mm-dd HH:MM:SS")), Val(Nvl(mrsInfo!入院科室id)), Val(Nvl(mrsInfo!入院病区ID))) = False Then
        '转换
    End If
    Unload Me
    Exit Sub
...
Private Function GetMultiNOs(ByVal strno As String, Optional lng打印ID As Long, Optional blnNOMoved As Boolean) As String
        ...
        If .TextMatrix(lngRow, .ColIndex("类别")) = "可转入" And .TextMatrix(lngRow, .ColIndex("选择")) <> IIf(blnSelect, "√", "") Then
            ...
            If str单据 = "收费单" Then
                ...
                If intInsure > 0 Then      '全部选择或取消
                    If Not IsYBSingle(strno, intInsure) Then
...
Private Function CheckAllTurn(ByVal strno As String) As Boolean
    ...
    strSql = "Select 1" & vbNewLine & _
            ...
            "       (Select 1 From 卡消费接口目录 Where 编号 = a.结算卡序号 And 是否全退 = 1))" & vbNewLine & _
...
Private Function zlIsExistsSquareCard(ByVal strNos As String, Optional int记录性质 As Integer = 3) As Boolean
    ...
    strSql = "" & _
    "   Select /*+ rule */ A.卡结算id " & _
    "   From 病人卡结算对照 A, 病人预交记录 B, 门诊费用记录 C,Table( f_Str2list([1])) J " & _
    "   Where A.预交id = B.ID and B.记录性质=[2] And C.NO = J.Column_Value And C.结帐ID = B.结帐ID And Rownum < 2"
...
Private Sub txtPatient_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        mlngTXTProc = GetWindowLong(txtPatient.hWnd, GWL_WNDPROC)
...
Private Sub txtPatient_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        Call SetWindowLong(txtPatient.hWnd, GWL_WNDPROC, mlngTXTProc)
    End If
End Sub
    ...
    Else '当作姓名
        Select Case objCard.名称
            Case "姓名", "姓名或就诊卡"
                ...
                End If
                 '通过姓名查找
                strPati = "Select A.病人ID as ID,A.病人ID,A.住院号, A.门诊号, Nvl(b.性别, a.性别), Nvl(b.年龄, a.年龄) as 年龄, A.住院次数, A.家庭地址, A.工作单位," & vbNewLine & _
...
errHandle:
    ...
    End If
End Sub
Private Function zlIsCheckCanelFee(ByVal str结帐ID_In As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If gobjSquare.objSquareCard Is Nothing Then
        MsgBox "注意:" & vbCrLf & "    当前没有安装卡结算部件，不能进行退费,请检查！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    '    '     在退费时，需要进行相关的检查
    '    '---------------------------------------------------------------------------------------------------------------------------------------------
    If gobjSquare.objSquareCard.zlCheckDelSquareValied(Me, mlngModule, mstrPrivs, str结帐ID_In) = False Then
        Exit Function
    End If
...
'    '   3.进入预交款时
'    '+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
'    mtySquareCard.blnExistsObjects = mobjSquareCard.zlInitComponents(Me, mlngModule, glngSys, gstrDBUser, gcnOracle, False, strExpend)
'    Dim objCard As Card
'    Call IDKind.zlInit(Me, glngSys, glngModul, gcnOracle, gstrDBUser, mobjSquareCard, "")
'     '初始化相关的本地数据集
'     Set mtySquareCard.rsSquare = New ADODB.Recordset
'End Sub
 
Private Function CheckOneCard(ByVal objICCard As Object, ByVal lng结帐ID As Long, ByVal str结帐单 As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查一卡通是否有效
    '返回:
    '编制:刘兴洪
    '日期:2011-03-29 09:04:47
    '暂不处理
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Set rsOneCard = GetOneCardBalance(lng结帐ID)
    If rsOneCard.RecordCount = 0 Then CheckOneCard = True: Exit Function
    MsgBox "结帐单" & str结帐单 & " 为一卡通结算，暂不支持一卡通退费!", vbInformation + vbOKOnly, gstrSysName
    Exit Function
    
    '暂不支持,以下代码保留,以后再处理
    If mrsOneCard Is Nothing Then
        Set mrsOneCard = GetOneCard
    ElseIf mrsOneCard.State <> 1 Then
        Set mrsOneCard = GetOneCard
    End If
    If mrsOneCard.RecordCount = 0 Then
        MsgBox "注意:" & vbCrLf & "   结帐单为" & str结帐单 & "的是一卡通结算，但又未启用一卡通，请检查!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    If objICCard Is Nothing Then
        On Error Resume Next
...
Private Function DelBalance(ByVal strDelDate As String, ByVal strno As String, ByVal lng结帐ID As Long, _
    ...
    If intInsure <> 0 Then
        If gclsInsure.CheckInsureValid(intInsure) = False Then
             Exit Function
        End If
        If gclsInsure.GetCapability(support门诊结算作废, , intInsure) And IsYBSingle(strno, intInsure) Then
            strAdvance = "1|1"
