...
Private Sub cboUnit_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If Not rsTmp Is Nothing Then
            intIdx = FindCboIndex(cboUnit, rsTmp!ID)
            If intIdx <> -1 Then
...
Private Sub Form_Load()
    ...
    If InStr(mstrPrivs, "病人预出院") = 0 Then
...
'    End If
    ...
    Call InitPatiType
    '初始护理等级
    If Not InitNurse Then mblnUnload = True: Exit Sub
            ...
            If mstrSeekKey <> "床号" Then
                If mlng当前病区id <> mlngUnit Then
                    If InStr(mstrPrivs, "所有病区") <> 0 Then
                        intIdx = FindCboIndex(cboUnit, mlng当前病区id)
                        If intIdx <> -1 Then
                            cboUnit.ListIndex = intIdx
                        End If
'                        mlngUnit = mlng当前病区id
...
Private Sub mnuEdit_Out_Click()
    ...
    If gblnOK Then Call LoadList(True, True, False, True)
End Sub
...
Private Sub mnuQueryInfo_Click()
    If mobjLVW Is lvwBeds_s Then
        frmDegreeCard.mlng病人ID = mrsBeds!病人ID
        frmDegreeCard.mlng主页ID = mrsBeds!主页ID
    ElseIf mobjLVW Is lvwFamily_s Then
        frmDegreeCard.mlng病人ID = mrsFamily!病人ID
        frmDegreeCard.mlng主页ID = mrsFamily!主页ID
    ElseIf mobjLVW Is lvwIn_s Then
        frmDegreeCard.mlng病人ID = mrsIn!病人ID
        frmDegreeCard.mlng主页ID = mrsIn!主页ID
    Else
        frmDegreeCard.mlng病人ID = mrsOut!病人ID
        frmDegreeCard.mlng主页ID = mrsOut!主页ID
    End If
...
'功能：读取指定病区的床位映象表(含床位信息、病人身份信息、病人在院信息),并显示在列表中
    ...
    gstrSQL = _
        ...
        "From (Select A.床号, A.科室id, Nvl(C.名称, Decode(A.共用, 1, '<共用病床>', Null)) As 科室, A.房间号, A.性别分类," & vbNewLine & _
...
'        "      Nvl(B.主页id, 0) <> 0 And B.护理等级id = C.ID(+) And A.病人id = E.病人id(+) And E.性质(+) = 1" & gstrSQL
    ...
    gstrSQL = _
        ...
        "       To_Char(B.出院日期, 'YYYY-MM-DD HH24:MI:SS') As 出院时间, D.名称 As 出院科室,B.出院科室ID, B.出院病床, B.当前病况 As 出院病况," & vbNewLine & _
        "       C.名称 As 护理等级, B.出院方式, B.病人性质, B.险类, Decode(Nvl(E.费用余额, 0), 0, '√', Null) As 结清" & vbNewLine & _
        "       ,A.就诊卡号,A.身份证号,A.IC卡号,Nvl(B.病人类型,Decode(B.险类,Null,'普通病人','医保病人')) 病人类型,A.主页Id 就诊次数 " & vbNewLine & _
        "From 病人信息 A, 病案主页 B, 收费项目目录 C, 部门表 D, (select 病人ID,主页ID,来源途径 性质, Nvl(sum(金额),0) 费用余额 from 病人未结费用 group by 病人ID,主页ID,来源途径) E" & vbNewLine & _
        "Where B.病人id = A.病人id And B.出院科室id = D.ID And B.出院日期 Is Not Null And" & vbNewLine & _
        "      Nvl(B.主页id, 0) <> 0 And B.护理等级id = C.ID(+) And B.病人id = E.病人id(+) And B.主页ID=E.主页ID(+) And E.性质(+) = 2" & gstrSQL
...
Private Sub LoadList(Optional mblnBeds As Boolean = True, Optional mblnFamily As Boolean = True, _
    Optional mblnIn As Boolean = True, Optional mblnOut As Boolean = True)
'功能：刷新界面列表数据
'参数：缺省刷新所有列表,可以分别指定
...
'                    Set objMenu = tbr.Buttons("Undo").ButtonMenus.Add(, , "入科")
        ...
        If InStr(mstrPrivs, "病人预出院") = 0 And objMenu.Text = "预出院" Then
            objMenu.Enabled = False
        End If
...
Dim arrType As Variant, i As Integer
    ...
    Next
End Function
Private Function InitNurse() As Boolean
    Dim rsTmp As ADODB.Recordset
    Dim objButton As Button, lngLen As Long, i As Integer
    ...
    End If
    For i = 1 To rsTmp.RecordCount
        If LenB(rsTmp!名称) > lngLen Then lngLen = LenB(rsTmp!名称)
        rsTmp.MoveNext
    Next
    rsTmp.MoveFirst
    
    For i = 1 To rsTmp.RecordCount
        Set objButton = tbrFilter.Buttons.Add(, "Nurse" & rsTmp!ID, GetLenText(rsTmp!名称, lngLen), , "Check")
        If i <= 10 Then
            objButton.ToolTipText = rsTmp!名称 & "病人(ALT + " & i Mod 10 & ")"
        End If
        rsTmp.MoveNext
    Next
...
'    Dim rsTemp As ADODB.Recordset
    ...
    strSQL = _
        ...
        " Order by A.编码"
        '
    vRect = GetControlRect(cboToDept.hWnd)
