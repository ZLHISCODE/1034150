        ...
        If .RowData(.Row) <> 0 Then
            ...
            If mbln门诊 Then
                ...
                strSQL = _
                    ...
                    "   Where A.病人ID=[1]  And B.ID=[2] And A.婴儿费>=[3] And A.登记时间>=B.登记时间 and B.记录性质=1 and B.记录状态=1" & _
                    " Union ALL Select Distinct 2,A.婴儿 From 病人医嘱记录 A,病人挂号记录 B Where A.病人ID=[1] And A.挂号单=B.NO And B.ID=[2] And A.婴儿>=[3] And B.记录性质=1 and B.记录状态=1" & _
            ...
            Else
                ...
                strSQL = _
                    " Select Distinct 1 as 标志,婴儿费 as 婴儿 From 住院费用记录 Where 病人ID=[1] And 主页ID=[2] And 婴儿费>=[3]" & _
                    " Union ALL Select Distinct 2,婴儿 From 病人医嘱记录 Where 病人ID=[1] And 主页ID=[2] And 婴儿>=[3]" & _
                    " Union ALL Select Distinct 3,婴儿 From 电子病历记录 Where 病人ID=[1] And 主页ID=[2] And 婴儿>=[3]" & _
                    " Union ALL Select Distinct 4,婴儿 From 病人护理记录 Where 病人ID=[1] And 主页ID=[2] And 婴儿>=[3]" & _
                    " Union ALL Select Distinct 4,婴儿 From 病人护理文件 Where 病人ID=[1] And 主页ID=[2] And 婴儿>=[3]"
            End If
            ...
            If Not rsTmp.EOF Then
                MsgBox "该病人的婴儿" & rsTmp!婴儿 & "已存在" & Decode(rsTmp!标志, 1, "费用", 2, "医嘱", 3, "病历", 4, "护理") & "数据，当前行不能删除。", vbInformation, gstrSysName
                Exit Sub
            End If
        End If
    ...
    With vsBaby
        For i = .FixedRows To .Rows - 1
            ...
            arrSQL(UBound(arrSQL)) = "Zl_病人新生儿记录_Insert(" & _
                ...
                "," & ZVal(.TextMatrix(i, mCol.Col体重)) & ",'" & zlCommFun.GetNeedName(.TextMatrix(i, mCol.COl血型)) & "','" & .TextMatrix(i, mCol.Col备注说明) & "')"
...
Private Sub Form_Load()
    ...
    '婴儿信息
    strSQL = "Select 序号,婴儿姓名,婴儿性别,分娩次数,分娩方式,胎儿状况,身长,体重,血型,出生时间,备注说明" & _
...
Private Sub txtBaby_Validate(Index As Integer, Cancel As Boolean)
    ...
    Case B_出生时间
        ...
        Else
            If CDate(Format(Trim(txtBaby(Index).Text), "YYYY-MM-dd HH:ss")) > CDate(Format(zlDatabase.Currentdate, "YYYY-MM-dd HH:ss")) Then
...
'参数:bytFunc =0,选中行内容显示到卡片；=1新增设置缺省值=2清除卡片信息
    With vsBaby
        ...
        If bytFunc = 0 Then
            ...
            cboBaby(B_胎儿状况).Text = zlControl.CboLocate(cboBaby(B_胎儿状况), .TextMatrix(lngRow, mCol.Col胎儿状况), False)
            cboBaby(B_血型).Text = zlControl.CboLocate(cboBaby(B_血型), .TextMatrix(lngRow, mCol.COl血型), False)
            cboBaby(B_分娩方式).Text = zlControl.CboLocate(cboBaby(B_分娩方式), .TextMatrix(lngRow, mCol.Col分娩方式), False)
            cboBaby(B_性别).Text = zlControl.CboLocate(cboBaby(B_性别), .TextMatrix(lngRow, mCol.Col婴儿性别), False)
        ...
        ElseIf bytFunc = 1 Then
            ...
            txtBaby(B_出生时间).Text = Format(zlDatabase.Currentdate, "YYYY-MM-DD HH:ss")
