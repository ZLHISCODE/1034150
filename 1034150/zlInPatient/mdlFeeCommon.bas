...
'功能：读取指定性质的人员列表
    ...
    If str性质 <> "" Then
        If blnBaseInfo Then
            ...
            " Order by a.简码"
        Else
            strSql = "Select a.* From 人员表 a,人员性质说明 b" & _
            " Where a.ID = b.人员ID And b.人员性质=[1] And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null) And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
            " Order by a.简码"
        End If
    Else
...
Public Function GetDepts(ByVal str性质 As String, ByVal str服务对象 As String, Optional ByVal bln仅操作员部门 As Boolean = False, _
    ...
    strSql = _
        ...
        " And B.部门ID=A.ID And Instr(',' || [2]|| ',',',' || B.服务对象 || ',')>0 " & strSql & _
        " Order by A.编码"
    Set GetDepts = zlDatabase.OpenSQLRecord(strSql, App.ProductName, str性质, str服务对象, UserInfo.ID, strDeptIDs)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'功能：通过用户ID获取病人所属部门,同时包含当前部门对应的病区或科室
'返回：部门ID串 ID1,ID2,ID3...
    Dim strSql As String
    Dim rsTmp As ADODB.Recordset
    ...
    strSql = "Select Distinct a.部门id" & vbNewLine & _
            "From (Select Nvl(c.科室id, d.病区id) As 部门id" & vbNewLine & _
            "       From 部门人员 B, 病区科室对应 C, 病区科室对应 D" & vbNewLine & _
            "       Where b.部门id = c.病区id(+) And b.部门id = d.科室id(+) And b.人员id = [1] And Nvl(Nvl(c.病区id, d.科室id), 0) <> 0" & vbNewLine & _
            "       Union All" & vbNewLine & _
            "       Select b.部门id From 部门人员 B Where b.人员id = [1]) A, 部门表 B, 部门性质说明 C" & vbNewLine & _
            "Where a.部门id = b.Id And a.部门id = c.部门id And (b.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or b.撤档时间 Is Null) And" & vbNewLine & _
            "      Instr(',临床,护理,', ',' || c.工作性质 || ',') > 0"

    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, "mdlFeeCommon", UserInfo.ID)
    Do While Not rsTmp.EOF
        strIDs = strIDs & "," & rsTmp!部门ID
        rsTmp.MoveNext
    Loop
        ...
        If Not blnOnlyCheckLimit Then
            If rsTmp!他人单据 = 0 And blnCheckOperator Then
                If strOperator <> UserInfo.姓名 Then
                    MsgBox "你没有权限对" & strOperator & "处理的" & strBill & "单据进行" & strMessage & "！", vbInformation, gstrSysName
                    Exit Function
                End If
            End If
...
Public Function ReadBillInfo(ByVal int来源 As Integer, ByVal strno As String, _
    ...
    '功能：读取一张单据的操作员和登记时间
    '入参：int来源-1-门诊,2-住院
    '      intFlag:-1=结帐,-2=预交,其它同 住院费用记录或门诊费用记录.记录性质(1-收费记录；2(12)-记帐记录；3(13)-自动记帐记录;4-挂号记录；5(15)-就诊卡记录)
    '出参：
    '返回：
    '编制：刘兴洪
...
Public Function GetMoneyInfo(lng病人ID As Long, Optional dblModiMoney As Double, _
    ...
    Optional ByVal lng主页ID As Long) As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取指定病人的剩余额
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTmp As ADODB.Recordset, bln医保 As Boolean, blnDo As Boolean
    Dim strSql As String
    On Error GoTo errH
    blnDo = (lng主页ID = 0)
    ...
    Else
        '查病人某一次住院的未结费用
        strSql = "Select Nvl(Sum(金额),0) as 费用余额 from 病人未结费用 Where 病人id=[1] and 主页id=[2] "
    End If
...
Public Function GetStockSet(ByVal lng药房ID As Long, ByVal lng药品ID As Long, ByVal bln卫材 As Boolean) As Recordset
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng药品ID As Long, ByVal bln卫材 As Boolean, ByRef dblAllTime As Double, ByVal strDec As String, ByRef dblPriceSingle As Double) As Currency
...
'      dblPriceSingle-只有一个批次时返回该批次的单价，避免根据数量先乘再除后因四舍五入引起不同的数量的单价不同
    ...
    Set rsPrice = GetStockSet(lng药房ID, lng药品ID, bln卫材)
    '时价=总金额/总数量
    dblPrice = 0 '本笔总应收金额
