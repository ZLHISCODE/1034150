...
'bytType =0 取首要诊断,=1首要诊断找不到时取次要诊断
    ...
    If bytType = 0 Then
        ...
        Else
            strSQL = "Select 疾病id, 诊断id, 诊断描述,诊断类型 " & vbNewLine & _
            "From 病人诊断记录" & vbNewLine & _
            "Where 记录来源 In (1, 2, 3) And 诊断类型 In (1, 2, 11, 12) And 取消时间 Is Null And 病人id = [1] And 主页id = [2] And 诊断次序 = 1 And Nvl(是否疑诊,0) = 0 And Not (NVl(疾病ID,0)=0 and NVl(诊断ID,0)=0) " & vbNewLine & _
