...
Private mlng本次执行结果Old  As Long '更新时取原始本次执行结果，默认为-1
Private mlng本次次数Old     As Long '更新时取原始本次执行次数，默认为-1
...
Private Sub Form_Load()
    ...
    If mlngModul = p医技工作站 Then
        If Val(zlDatabase.GetPara("代行执行记录", glngSys, p医技工作站)) = 0 Then
    ...
    Else
        ...
        mlng本次执行结果Old = Val(Nvl(rsTmp!执行结果, 1) & "")
        ...
        '修改时获取执行结果
        cboResult.ListIndex = Val(Nvl(rsTmp!执行结果, 1) & "")
        
        mlng完全执行 = mlng完全执行 - IIf(Val(Nvl(rsTmp!执行结果, 1) & "") = 1, Val(txt本次数次.Text), 0)
        Call GetCboIndex(cbo执行人, Nvl(rsTmp!执行人))
...
Private Sub cmdOK_Click()
    ...
    If Val(txt发送数次.Text) <> 0 And Not mbln血库流程 Then '未填写发送数次的不限制(如持续性长嘱)
        ...
        If mlng最大已销 <> 0 And (mlng本次执行结果Old = 0 And cboResult.ListIndex = 1 Or mlng本次次数Old <> dbl本次数次) Then
    ...
    If mstr执行时间 = "" Then
        ...
        strSQL = "ZL_病人医嘱执行_Insert(" & mlng医嘱ID & "," & mlng发送号 & "," & _
            ...
            IIf(mbln单独执行, 1, 0) & "," & "0," & IIf(cboResult.ListIndex = 0, 0, 1) & ",'','" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & mlng执行科室ID & ")"
        Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
    Else
        strSQL = "ZL_病人医嘱执行_Update(To_Date('" & mstr执行时间 & "','YYYY-MM-DD HH24:MI:SS')," & mlng医嘱ID & "," & mlng发送号 & "," & _
            "To_Date('" & Format(dtp要求时间.Value, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS')," & _
            dbl本次数次 & ",'" & txt执行摘要.Text & "','" & zlCommFun.GetNeedName(cbo执行人.Text) & "'," & _
            "To_Date('" & Format(dtp执行时间.Value, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS')" & "," & IIf(cboResult.ListIndex = 0, 0, 1) & ",NULL," & IIf(mbln单独执行, 1, 0) & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & mlng执行科室ID & ")"
        Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
    End If
...
'       lng发送号 本次医嘱发送的发送号
    ...
    If bln单独执行 Then
        ...
        strSQL = "Select Sum(Nvl(b.本次数次,a.发送数次)) 完全执行次数" & vbNewLine & _
            ...
            "Where a.医嘱id = b.医嘱id And a.发送号 = b.发送号 And a.医嘱id = [1] And a.发送号 = [2] And Nvl(b.执行结果, 1) <> 0"
    Else
