...
Private Function GetEmrCISStruct(ByVal lngPatiID As Long, ByVal lngPageID As Long) As ADODB.Recordset
    ...
    '上级ID，ID，名称，参数，图标
    strSQL = "Select Decode(e.Kind,'01','R1', '02', 'R2', '03', 'R4', '04', 'R5', '05', 'R6', 'R2') 上级id, Nvl(d.Subdoc_Id, Rawtohex(b.Id)) As ID," & vbNewLine & _
                ...
                "     (Select Distinct d.Id, c.Antetype_Id, c.Subdoc_Id, c.Subdoc_Title, c.Real_Doc_Id, c.Complete_Time, c.Completor" & vbNewLine & _
                "       From Bz_Act_Log A,Bz_Act_Log D, Bz_Doc_Tasks C" & vbNewLine & _
                "       Where a.Extend_Tag = :etag And (a.Id = d.Id Or a.Id = d.Basiclog_Id) And d.Id = c.Actlog_Id And c.Real_Doc_Id Is Not Null) D, Antetype_List E" & vbNewLine & _
...
'功能：打开LIS报告文件查看，获取临时文件路径
    ...
    strFile = objFile.GetSpecialFolder(TemporaryFolder) & "\" & str报告名 & "." & IIf(lng类型 = 0, "pdf", "html")
