...
Private Sub timNotify_Timer()
    ...
    If mintNotify > 0 Then
        ...
        If DateDiff("s", CDate(strPreTime1), curTime) > mintNotify * CLng(60) Or mblnFirst Then
            strPreTime1 = Format(curTime, "yyyy-MM-dd HH:mm:ss")
            '启用消息平台则不按照固定刷新时限为准
            If mclsMipModule.IsConnect = False Or mblnFirst Then Call LoadNotify
            mblnFirst = False
        End If
     Else
...
'intType:新开医嘱的类型(3:出院,4-转科)
    ...
    Set objItem = objRecord.AddItem(LPAD(CStr(arrInput(Index)), 10, " ")) '床号
    objItem.Caption = CStr(arrInput(Index)): Index = Index + 1
...
Private Sub SetNotifyState()
    ...
    If rptNotify.Rows.Count > 0 Then
        lblTitle.Caption = "医嘱提醒：共有" & rptNotify.Rows.Count & "条消息需要处理"
        lblInfo.Caption = lblTitle.Caption
...
'功能：自动进入医嘱校对、确认停止的执行界面
                ...
                Else
                    ...
                    Select Case .Item(C_消息).Value
                        ...
                        Case "ZLHIS_CHARGE_001" '费用销帐申请
                            intFunc = 12
                        Case "ZLHIS_LIS_003" '检验危急值,护士站不能处理危急值消息
                            'Call ZLHIS_CIS_014(mclsMipModule, .Item(C_病人ID).Value, .Item(c_姓名).Value, .Item(c_住院号).Value, Val(Split(str业务, ",")(0)), _
                                '.Item(C_主页ID).Value, , mlng病区ID, .Item(c_床号).Value, Val(Split(str业务, ",")(0)))
				Exit Sub
                        Case "ZLHIS_PACS_005" '检查危急值,护士站不能处理危急值消息
                            'Call ZLHIS_CIS_025(mclsMipModule, .Item(C_病人ID).Value, .Item(c_姓名).Value, .Item(c_住院号).Value, Val(Split(str业务, ",")(0)), _
                                '.Item(C_主页ID).Value, , mlng病区ID, .Item(c_床号).Value, Val(Split(str业务, ",")(0)))
				Exit Sub
                    End Select
                End If
    ...
    End If
    '108829:进行发送、校对、停止前则检查是否还存在有效医嘱
    Select Case intFunc
    Case 0
        If InStr(strPrivs, ";发送药疗临嘱;") > 0 Or InStr(strPrivs, ";发送药疗长嘱;") > 0 Or InStr(strPrivs, ";发送其他临嘱;") > 0 Or InStr(strPrivs, ";发送其他长嘱;") > 0 Then
            If Not HaveOperateAdvice(lng病人ID, lng主页ID, 0) Then
                ...
                Call SetNotifyState
            Else
                blnTmp = mclsPublicAdvice.AdviceSend(Me, mlng病区ID, lng病人ID, lng主页ID, gstrPrivs, mclsMipModule)
                If Not HaveOperateAdvice(lng病人ID, lng主页ID, 0) Then
                    If strSQL <> "" Then Call zlDatabase.ExecuteProcedure(strSQL, "Zl_业务消息清单_Read")
                    Call ReMoveItemByKey(strNoteKey)
                    Call SetNotifyState
    ...
    Case Else
        If strSQL <> "" Then Call zlDatabase.ExecuteProcedure(strSQL, "Zl_业务消息清单_Read")
        If mblnFirst = False And strNoteKey <> "" Then
            Call ReMoveItemByKey(strNoteKey)
            Call SetNotifyState
        End If
        If intFunc > -1 Then
...
Private Sub BatchRemove(ByVal strPatis As String)
    '功能：批量移除医嘱提示信息(以病人为单位)。
