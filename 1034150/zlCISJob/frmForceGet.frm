...
Object = "{AF9744ED-CAFC-4877-8437-2C20C14CEA4E}#3.12#0"; "zlIDKind.ocx"
Begin VB.Form frmForceGet 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsRegist 
      ...
      FormatString    =   $"frmForceGet.frx":0611
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub imgSentence_Click()
    ...
    blnCancel = True
    On Error GoTo errH
    strSQL = "Select A.病人ID as ID,A.门诊号,A.姓名 as 名称,A.性别,A.年龄,a.发生时间 as 挂号时间 From 病人挂号记录 A,病人信息 B Where A.病人ID=B.病人ID" & IIf(mlng接诊科室ID = 0, "", " And A.执行部门ID+0=[1]") & " And A.发生时间 Between Sysdate - Decode(A.急诊,1," & IIf(gint急诊挂号天数 = 0, 1, gint急诊挂号天数) & "," & IIf(gint普通挂号天数 = 0, 1, gint普通挂号天数) & ") And Sysdate order by A.发生时间 desc"
...
Private Sub cmdOK_Click()
    ...
    With vsRegist
        ...
        If Val(.RowData(.Row)) = 2 Then
            ...
            '对预约进行强制续诊
            If Val(zlDatabase.GetPara("允许挂号划价单", glngSys, p门诊医生站, 1)) <> 1 And Not mobjSquare Is Nothing Then
                ...
                If blnDo Then
                    ...
                    If InStr("," & str科室IDs & ",", "," & rsTmp!科室ID & ",") = 0 Then
                        If InStr(mstrPrivs, "全院病人续诊") = 0 Then
                            ...
                            blnDo = False
                        ElseIf Nvl(rsTmp!执行状态, 0) = 2 Then
...
Private Sub PatiIdentify_FindPatiBefore(ByVal objCard As zlIDKind.Card, blnCard As Boolean, strShowText As String, objCardData As zlIDKind.PatiInfor, blnFindPatied As Boolean, blnCancel As Boolean)
    ...
    If zlCommFun.IsCharAlpha(strShowText) Then
        ...
        strWhere = strWhere & " And 发生时间 Between Sysdate - Decode(急诊,1," & IIf(gint急诊挂号天数 = 0, 1, gint急诊挂号天数) & "," & IIf(gint普通挂号天数 = 0, 1, gint普通挂号天数) & ") And Sysdate"
    ...
    Else
        Select Case objCard.名称
            Case "姓名"
                strSQL = "Select A.病人ID,A.门诊号,A.姓名,A.性别,A.年龄 From 病人挂号记录 A,病人信息 B Where A.病人ID=B.病人ID And b.姓名=[1]"
            Case "挂号单号"
    ...
    If rsTmp.EOF = False Then
        If rsTmp.RecordCount > 1 Then
            vRect = GetControlRect(fraKind.hwnd)
            If blnLikeCode Then
                strSQL = "Select /*+ RULE */ 病人ID as ID,门诊号,姓名 as 名称,性别,年龄 From 病人挂号记录 Where " & strWhere
            Else
                strSQL = "Select A.病人ID as ID,A.门诊号,A.姓名 as 名称,A.性别,A.年龄 From 病人挂号记录 A,病人信息 B Where A.病人ID=B.病人ID And B.姓名=[1]"
            End If
...
'参数：bytSize  0-9号字体，1-12号字体
    ...
    Call zlControl.VSFSetFontSize(vsRegist, IIf(bytSize = 0, 9, 12))
...
'功能：设置界面控件位置
    ...
    Call SetCtrlPosOnLine(False, 0, lblInfo(2), lngDis1, fraKind, 0, imgSentence, lngDis1, imgStaKB)
    imgSentence.Top = imgSentence.Top - 10
    Call SetCtrlPosOnLine(True, -1, lblInfo(2), 120 + Frame1(1).Height + 90, lblInfo(1), 30, vsRegist, 90 + Frame1(2).Height + 180, lbl续诊科室)
    Frame1(1).Top = lblInfo(2).Top + lblInfo(2).Height + 120
    Frame1(2).Top = vsRegist.Top + vsRegist.Height + 90
    Call SetCtrlPosOnLine(False, 0, lbl续诊科室, lngDis1, cbo续诊科室, lngDis2, chk急诊, lngDis2, cmdOK, lngDis2, cmdCancel)
