...
Begin VB.Form frmLabMain 
   ...
   Begin VB.PictureBox PicImage 
      ...
      Begin C1Chart2D8.Chart2D ChartThis 
         ...
         _ExtentY        =   1296
         _StockProps     =   0
         ControlProperties=   "frmLabMain.frx":381B5
      End
   End
...
Private Const con_主界面筛选_检验中 As String = "门诊病人;住院病人;无主标本;已审标本;未审标本;体检病人;紧急医嘱;紧急标本;质控标本;审核已通过;审核未通过;未做完;已做完"
...
Private WithEvents mclsExpenses As zlCISKernel.clsDockExpense       '新的费用
...
Private mblnVerifying(13) As Boolean                                '检验中筛选状态
    ...
    If intType = 0 Then
        ...
        Else
            strSQL = "Select Distinct a.Id, a.病人id, a.主页id, a.标本类型 标本名称,a.申请科室id 送检科室ID, " & _
                     ...
                     "Where a.医嘱id = b.医嘱id and a.Id = [1]"
        ...
        If mlngKey > 0 Then '显示项目数和价格
            ...
            If Val(Me.rptList.FocusedRow.Record(mCol.微生物标本).Value) = 1 Then
                gstrSql = "Select /*+ rule */" & vbNewLine & _
                        ...
                        "       Where F.收费细目id = J.ID And E.收费项目id = F.收费细目id And (F.终止日期 Is Null Or F.终止日期 = To_Date('3000-01-01', 'yyyy-mm-dd'))) B" & vbNewLine & _
            ...
            Else
                ...
                gstrSql = "Select /*+ rule */" & vbNewLine & _
                            ...
                            "             And (F.终止日期 Is Null Or F.终止日期 = To_Date('3000-01-01', 'yyyy-mm-dd'))) B" & vbNewLine & _
                            "Where A.诊疗项目id = B.诊疗项目id"
            End If
...
'    Me.stbThis.Panels(2).Text = "正在读取数据数据请等待..."
    ...
    strSQL = "Select /*+ rule */  distinct      Decode(a.是否传送, 1, '', '传送失败') As 传送," & vbNewLine & _
            ...
            "       a.审核未通过,a.病人来源,a.结果为空,nvl(s.路径状态,0) as 临床路径病人 " & vbNewLine & _
            " From 检验标本记录 a ,部门表 E , 检验仪器 f , 病人信息 b , 部门表 N , 部门表 M, 病人医嘱报告 R,病人医嘱附件 p,病案主页 S " & strTable & vbNewLine & _
            ...
            " and a.病人ID = S.病人ID(+) and a.主页ID = s.主页ID(+)  " & strWhere
...
'            End If
            ...
            If mintUnion = 1 Then
                ...
                Else
                    ...
                    Do Until rsTmp.EOF
                        ...
                        ReDim Preserve astrSQL(UBound(astrSQL) + 1)
                        astrSQL(UBound(astrSQL)) = "Zl_检验报告单_Update(" & rsTmp("ID") & "," & intPrivacy & ")"         '审核后处理病历报告单
            ...
            Else
                ...
                Else
                    ...
                    ReDim Preserve astrSQL(UBound(astrSQL) + 1)
                    astrSQL(UBound(astrSQL)) = "Zl_检验报告单_Update(" & mlngKey & "," & intPrivacy & ")"        '审核后处理病历报告单
...
'            RefreshData
        ...
        Case mActR.写入病历
            ...
            If intMicrobe = 1 Then
                strSQL = "Zl_检验报告单_Update(" & mlngKey & ",0)"        '微生物三级报告单发布
                zlDatabase.ExecuteProcedure strSQL, Me.Caption
            End If
        Case mActR.验证签名
...
Private Sub InsertOneRecored(lngKey As Long, Optional blnNew As Boolean = True, Optional blnGoto As Boolean = True)
    ...
    strSQL = "Select /*+ rule */     Decode(a.是否传送, 1, '', '传送失败') As 传送," & vbNewLine & _
            ...
            "       a.申请类型,nvl(R.查阅状态,0) as 查阅状态,nvl(R.病历ID,0) as 报告发送,a.初审人,a.初审时间,b.工作单位,p.项目,p.内容,b.健康号,  " & vbNewLine & _
            "       a.审核未通过,a.病人来源,a.结果为空,nvl(s.路径状态,0) as 临床路径病人 " & vbNewLine & _
            " From 检验标本记录 a ,部门表 E , 检验仪器 f , 病人信息 B , 部门表 C,病人医嘱报告 R,病人医嘱附件 P,病案主页 S " & vbNewLine & _
            ...
            " And a.医嘱ID = R.医嘱ID(+) and A.医嘱Id = P.医嘱ID(+) " & vbNewLine & vbNewLine & _
...
Private Sub DelButton(Index As Integer)
    ...
    With objBar.Controls
        ...
        If objControl Is Nothing Then
            Set objControl = .Find(, conMenu_Manage_Report) '从预览按钮之后开始加入
