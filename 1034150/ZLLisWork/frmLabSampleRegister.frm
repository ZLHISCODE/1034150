...
Private Sub CreateCbs()
    ...
    '==列表科室
    Set ControlFile = Me.cbrthis.ActiveMenuBar.Controls.Add(xtpControlPopup, 0, "送检科室")
    With ControlFile.CommandBar.Controls
...
Private Sub Form_Resize()
    ...
    Pane1.MaxTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    Pane1.MinTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    
    Me.cbrthis.RecalcLayout
    
    Set Pane2 = Me.dkpMan.FindPane(mDkp.病人列表)
    Pane2.MaxTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    Pane2.MinTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    
    Set Pane3 = Me.dkpMan.FindPane(mDkp.登记计数)
    Pane3.MaxTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    Pane3.MinTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
'
    Set Pane4 = Me.dkpMan.FindPane(mDkp.登记试管)
    Pane4.MaxTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
    Pane4.MinTrackSize.SetSize 7995 / Screen.TwipsPerPixelX, 4545 / Screen.TwipsPerPixelY
...
Private Sub RefreshPatientData(Optional lngPatientType As Long = 0, Optional ByVal intBabyNo As Integer = 0)
    ...
    strSQL = "Select /*+ RULE */ 病人id,病人来源,病人姓名,病人科室,性别,年龄,就诊卡号,标识号,当前床号," & vbNewLine & _
                ...
                "       Sum(decode(紧急,'紧急',1,0)) As 紧急,Sum(重采) As 重采,sum(婴儿)/count(婴儿) as 婴儿号 From ("

    
    strSQL = strSQL & "Select  distinct a.病人id,decode(a.病人来源,1,'门诊',2,'住院',3,'院外',4,'体验') as 病人来源, " & vbCrLf & _
             " decode(nvl(a.婴儿,0),0,c.姓名,t.婴儿姓名)  as 病人姓名,e.名称 as 病人科室,decode(nvl(a.婴儿,0),0,c.性别,t.婴儿性别) as 性别," & vbCrLf & _
             " decode(nvl(a.婴儿,0),0,c.年龄,decode(sysdate-t.出生时间,'','',round(sysdate-t.出生时间)||'天')) as 年龄,c.就诊卡号,b.样本条码, " & vbCrLf & _
             ...
             " decode(b.执行状态,0,'',2,'拒收') as 拒收,nvl(b.接收批次,0) as 接收批次,b.接收时间,b.接收人,nvl(b.重采标本,0) as 重采 " & vbCrLf & _
             ...
             " and a.执行科室ID + 0 in (Select * From Table(Cast(f_Num2list([1]) As zlTools.t_Numlist))) " & vbCrLf & _
    ...
    If Me.rptPlist.Tag <> "" Then
        ...
        If lngPatientType = 1 Then
            ...
            '使用病人ID时不使用
            strSQL = strSQL & " and m.发送时间+0 Between [9] and [10]"
        Else
            strSQL = strSQL & " and m.发送时间 Between [9] and [10]"
        End If
        ...
        If varFilter(mFilter.开始时间) = "" Then
            strDateBegin = Now - Val(varFilter(mFilter.间隔时间))
            strDateEnd = Now
        Else
    ...
    If strTmp = "" And Me.rptPlist.Tag = "" Then
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, gstrSysName, strDeptIDs, "", "", "", "", "", "", "", _
                    CDate(Format(strDateBegin, "yyyy-MM-dd 00:00:00")), CDate(Format(strDateEnd, "yyyy-MM-dd 23:59:59")), mlngSelectBatch, mlngKey, 0, "")
    Else
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, gstrSysName, strDeptIDs, Val(varFilter(mFilter.标识号)), CStr(varFilter(mFilter.就诊卡)) _
                    ...
                    CDate(Format(strDateEnd, "yyyy-MM-dd 23:59:59")), mlngSelectBatch, mlngKey, int主页ID, str挂号单)
    End If
    If lngPatientType <> 1 Then
        '清除记录
    ...
    Else
        ...
        Do Until rsTmp.EOF
            For intLoop = 0 To Me.rptPlist.Rows.Count - 1
                If Me.rptPlist.Rows(intLoop).Record(mPcol.病人ID).Value = mlngKey And Me.rptPlist.Rows(intLoop).Record(mPcol.婴儿号).Value = intBabyNo Then
...
Private Function RefreshAdviceData(lngPatientID As Long, intState As Integer, Optional intPatientType As Integer = 0, Optional blOnlyWhere As Boolean = False, Optional intBabyNo As Integer = 0) As Boolean
    ...
    strSQL = " Select distinct /*+ rule */ B.ID as 医嘱ID,a.主页id, B.相关id, G.颜色 As 试管颜色, D.名称 As 采集方式, B.医嘱内容, C.样本条码,C.采样时间, " & vbCrLf & _
    ...
    If intState <> 3 And intState <> 4 Then
...
'    strSQL = strSQL & " union all " & strSQLbak
    ...
    strSQL = strSQL & IIf(intBabyNo <> 0, " and nvl(a.婴儿,0)= " & intBabyNo, IIf(blOnlyWhere, "", "and nvl(a.婴儿,0) = 0 ")) & " Order By 试管编码, 标本, 相关id, 医嘱id, 开嘱时间 "
    ...
    If strTmp <> "" Or rptPlist.Tag <> "" Then
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngPatientID, strDeptIDs, CDate(Format(strDateBegin, "yyyy-mm-dd 00:00:00")), _
                ...
                int主页ID, str挂号单, Val(txtGoto))
    Else
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngPatientID, strDeptIDs, CDate(Format(strDateBegin, "yyyy-mm-dd 00:00:00")), _
                CDate(Format(strDateEnd, "yyyy-mm-dd 23:59:59")), "", 0, mlngSelectBatch, int主页ID, str挂号单, Val(txtGoto))
    End If
            ...
            Case Else                               '就诊卡和姓名
                ...
                Else
                    If Val(IDKind.GetKindItem("卡类别ID")) <> 0 Then
                        ...
                        strFind = lng病人ID
                    ElseIf blnCard Or IDKind.IDKind = IDKinds.C5就诊卡 Then
...
Private Sub GetBatch()
    ...
    strSQL = "Select /*+ RULE */ distinct b.接收批次 " & vbCrLf & _
             " From 病人医嘱记录 A, 病人医嘱发送 B, 病人信息 C, 部门表 E, 诊疗项目目录 F,病人挂号记录 G,病人医嘱记录 H, " & vbCrLf & _
             "      诊疗项目目录 K ,病案主页 L,病人医嘱发送 M" & vbCrLf & _
             " Where A.ID = H.相关ID And H.id = B.医嘱id And A.病人id = C.病人id And A.病人科室id = E.ID And A.诊疗项目id = f.ID " & vbCrLf & _
             "      And h.诊疗项目ID = k.id " & vbCrLf & _
             " And A.挂号单 = G.No(+) and (g.病人ID is null or (g.记录状态 =1 and g.记录性质 =1) ) and f.类别 = 'E' and F.操作类型 = '6' and a.病人id = l.病人ID(+) and a.执行科室ID + 0 = [1] " & vbCrLf & _
             " And A.ID = M.医嘱ID And k.试管编码 is not null And C.出院时间 is null and b.接收人 is not null "
    ...
    If intType = 1 Then
        ...
        If Len(strAdvice) > 4000 Then
            ...
            Do While Len(strTemp) > 4000
                ...
                strSQL = " Select /*+ rule */ B.ID as 医嘱ID, B.相关id, G.颜色 As 试管颜色, D.名称 As 采集方式, B.医嘱内容, C.样本条码,C.采样时间, " & vbCrLf & _
                        " H.名称 As 执行科室, B.开嘱医生,B.开嘱时间, C.发送人, C.发送时间, G.编码 as 试管编码,b.标本部位 as 标本, " & vbCrLf & vbCrLf & _
            ...
            Do Until rsTmp.EOF
                ...
                If IsNull(rsTmp("试管颜色")) = False Then
                    ...
                    If strOldAdvice <> rsTmp("相关ID") Then
                        ...
                        For intLoop = 0 To Me.rptAlist(TabCtr.Selected.Index).Columns.Count
                            Record(intLoop).ForeColor = Val(Nvl(rsTmp("试管颜色")))
                        Next
