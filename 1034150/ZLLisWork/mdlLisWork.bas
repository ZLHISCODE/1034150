...
'说明：当系统参数为执行后审核费用时才返回。
    ...
    If bln单独执行 Then
        strSQL = _
            ...
            "  And Exists (Select 1 From zlParameters Where 系统=[4] And 模块 is Null And Nvl(私有,0)=0 And 参数号 = 81 And 参数值 = '1')" & _
            " Group by B.编码,B.名称"
...
Public Sub WriterBarCodeToLIS(rsBarcode As ADODB.Recordset, intMode As Integer)
    '功能   把条码写入LIS申请单
    ...
    If rsBarcode.RecordCount > 0 Then
        If Not mobjLisInsideComm Is Nothing Then
            ...
            Do Until rsBarcode.EOF
                If intMode = 3 Then
                    If mobjLisInsideComm.SampleBarcodeWrite(rsBarcode("医嘱ID串"), rsBarcode("样本条码"), UserInfo.姓名, strErr) = False Then
                        MsgBox "写入条码到LIS申请单出错!" & vbCrLf & strErr
                    End If
                Else
                    If mobjLisInsideComm.SampleBarcodeWrite(rsBarcode("医嘱ID串"), "", "", strErr) = False Then
                        MsgBox "写入条码到LIS申请单出错!" & vbCrLf & strErr
                    End If
                End If
                rsBarcode.MoveNext
...
Public Function CheckPlugIn(ByVal lngSys As Long, ByVal lngModual As Long, ByVal rsMoneyNow As ADODB.Recordset) As Boolean
