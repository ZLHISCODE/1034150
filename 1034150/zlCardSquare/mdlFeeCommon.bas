...
errHandle:
    ...
    End If
End Function
Public Function zlGetSpecialItemFee(strClass As String, Optional ByVal lng收费细目ID As Long) As Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Else
        ...
        " Where B.收费细目ID = A.ID And B.收入项目ID = C.ID And A.ID=[2]" & _
        "       And (A.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or A.撤档时间 is NULL)" & _
        "       And ((To_Char(Sysdate,'YYYY-MM-DD HH24:MI:SS') Between To_Char(B.执行日期,'YYYY-MM-DD HH24:MI:SS') and To_Char(B.终止日期,'YYYY-MM-DD HH24:MI:SS')) or (To_Char(Sysdate,'YYYY-MM-DD HH24:MI:SS')>=To_Char(B.执行日期,'YYYY-MM-DD HH24:MI:SS') And (B.终止日期 is NULL Or B.终止日期=To_Date('3000-01-01','YYYY-MM-DD'))))"
    End If
    On Error GoTo errH
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取特定项目的费用集", strClass, lng收费细目ID)
...
Public Function zlAddUpdateSwapSQL(ByVal bln预交 As Boolean, ByVal strIDs As String, ByVal lng卡类别ID As Long, ByVal bln消费卡 As Boolean, _
    ...
    ByRef cllPro As Collection, Optional int校对标志 As Integer = 0, Optional int发送标志 As Integer = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:更新三方交易流水号和流水说明
    '入参: bln预交款-是否预交款
    ...
    '发送标志
    strSQL = strSQL & "" & IIf(int发送标志 = 0, "NULL", int发送标志) & ")"
...
Public Function zlLoadDefaultAddr(ByVal objCtrl As Object) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:读取区域表中的缺省地址
    '入参:objCtrl-结构化地址控件
    '返回:
    ...
    Do While Not rsTmp.EOF
        If Val(Nvl(rsTmp!级数)) < 5 Then
            str地址(Val(Nvl(rsTmp!级数))) = Nvl(rsTmp!名称)
        End If
        rsTmp.MoveNext
    Loop
    Call objCtrl.LoadStructAdress(str地址(0), str地址(1), str地址(2), str地址(3), str地址(4))
    zlLoadDefaultAddr = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Public Function zlReadAddrInfo(ByVal objCtrl As Object, ByVal lng病人ID As Long, ByVal lng主页ID As Long, _
