...
Private WithEvents mobjIDCard As clsIDCard   '身份证读卡
...
Private Sub txt余额_Change()
    ...
    If cbo支付方式.ListIndex < 0 Then Exit Sub
    ...
    With cbo支付方式
        ...
        Do While Not rsTemp.EOF
            ...
            For i = 0 To UBound(varData)
                ...
                End If
            Next
            If Not blnFind And rsTemp!性质 <> 7 Then
                .AddItem Nvl(rsTemp!名称)
...
Private Sub CheckFreeCard(ByVal strCard As String)
    ...
    If Not mCardType.rs医疗卡费 Is Nothing And Val(txt卡费.Text) <> 0 Then
        If mCardType.bln变价 = False Then
            txt卡费.Text = Format(GetActualMoney(NeedName(cbo费别.Text), mCardType.rs医疗卡费!收入项目ID, mCardType.rs医疗卡费!现价, mCardType.rs医疗卡费!收费细目ID), "0.00")
            lbl卡费.Tag = txt卡费.Text
        End If
    End If
End Sub
