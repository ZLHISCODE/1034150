...
Private Sub Form_Load()
    ...
    datTmp = CDate(GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName, "登记开始时间", Format(curDate, "yyyy-MM-dd")))
    dtp登记B.value = datTmp
    datTmp = CDate(GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName, "登记结束时间", Format(dtp登记B.MaxDate, dtp登记E.CustomFormat)))
    dtp登记E.value = datTmp
...
Public Sub MakeFilter()
    ...
    If txt住院号.Text <> "" Then mstrFilter = mstrFilter & " And A.住院号=[11]"
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName, "登记开始时间", Format(Me.dtp登记B.value, "YYYY-MM-DD")
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName, "登记结束时间", Format(Me.dtp登记E.value, "yyyy-MM-dd 23:59:59")
