...
Begin VB.Form frmPatiCureCardMgr 
   ...
   Begin XtremeDockingPane.DockingPane dkpMan 
      Bindings        =   "frmPatiCureCardMgr.frx":3290
      Left            =   1515
      Top             =   90
      _Version        =   589884
      _ExtentX        =   450
...
Private Enum mtxtIdx
    ...
    idx_挂失人 = 1
End Enum
Private WithEvents mobjIDCard As clsIDCard  '身份证接口
...
Private Sub LoadTypeData(ByVal cbrCmb As CommandBarComboBox)
    ...
    intIndex = 1
    '问题号:56599
    strSql = "Select ID,编码,名称,是否自制,是否发卡,Nvl(是否重复使用,0) as 是否重复使用 From 医疗卡类别 where 是否启用=1 Order by 编码"
...
Private Sub PrintReBill()
    ...
    Dim strTemp As String, strCardNo As String
    strTemp = zlCommFun.ShowMsgbox("缴款单打印", "请选择你要打印的缴款单", "发卡(&F),预存(&I),取消(&C)", Me, vbDefaultButton2)
    ...
    If strTemp = "发卡" Then
        ...
        End With
        Call ReportOpen(gcnOracle, glngSys, "ZL" & (glngSys \ 100) & "_BILL_1107", Me, "卡类别ID=" & mlngCardTypeID, "卡号=" & strCardNo, "缴款=" & 0, "找补=" & 0, "ReportFormat=1", 2)
    Else
        '预存款打印
    End If
End Sub
...
Public Sub zlExecuteCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Cardtrade   '换卡
        With vsCardList
            ...
            If strCardNo = "" Then Exit Sub
        End With
        If frmPatiCureCardEdit.zlShowCard(Me, mlngModule, mstrPrivs, Cr_换卡, mlngCardTypeID, strCardNo) = False Then Exit Sub
        If MsgBox("数据已经发生改变,是否重新刷新数据?", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
        Call LoadDataToGrid
    Case conMenu_Edit_CardFill    '补卡(&B)")
        With vsCardList
          ...
          '  If strCardNo = "" Then Exit Sub
        End With
        If frmPatiCureCardEdit.zlShowCard(Me, mlngModule, mstrPrivs, Cr_补卡, mlngCardTypeID, strCardNo) = False Then Exit Sub
        If MsgBox("数据已经发生改变,是否重新刷新数据?", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
        Call LoadDataToGrid
    Case conMenu_Edit_CardLoss        '挂失
        With vsCardList
            strCardNo = Trim(.TextMatrix(.Row, .ColIndex("卡号")))
            lng病人ID = Trim(.TextMatrix(.Row, .ColIndex("病人ID")))
   ...
   Case conMenu_Edit_CancelCardBound
    ...
    Case conMenu_COMBOX_INTERFACE   '点击选择
        ...
        mbln重复使用 = mcolCard(ctrCombox.ListIndex)(2) = 1 '57899
        Call LoadDataToGrid
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_File_PrintSingleBill           '"重打缴款单(&R)"
        Control.Visible = zlCheckPrivs(mstrPrivs, "预交收据") Or zlCheckPrivs(mstrPrivs, "医疗卡收据")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardPay
        Control.Visible = (zlCheckPrivs(mstrPrivs, "发卡") And mbln自制卡) Or (zlCheckPrivs(mstrPrivs, "发卡") And mbln自制卡 = False And mbln发卡 = True) '问题号:56599
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardBound
        Control.Visible = zlCheckPrivs(mstrPrivs, "绑定卡") And ((Not mbln自制卡) Or (mbln自制卡 And mbln重复使用))
        Control.Enabled = Control.Visible
    ...
    Case conMenu_Edit_CancelCardBound
        ...
        Control.Visible = zlCheckPrivs(mstrPrivs, "取消绑定卡") And ((Not mbln自制卡) Or (mbln自制卡 And blnIsBind))
    ...
    Case conMenu_Edit_CardBack
        ...
        End If
        '园内卡退卡,绑定则不能退卡,只能取消
        Control.Visible = zlCheckPrivs(mstrPrivs, "退卡") And mbln自制卡 And Not blnIsBind
        Control.Enabled = Control.Visible
    Case conMenu_Edit_Cardtrade '换卡
        Control.Visible = zlCheckPrivs(mstrPrivs, "换卡") And mbln自制卡
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardFill  '补卡
        Control.Visible = zlCheckPrivs(mstrPrivs, "补卡") And mbln自制卡
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardLoss  '挂失
        Control.Visible = zlCheckPrivs(mstrPrivs, "挂失") And mstrCurStatus = "有效卡"
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardCancelLoss  '取消挂失
        bln挂失 = mstrCurStatus = "已挂失"
        Control.Visible = zlCheckPrivs(mstrPrivs, "取消挂失") ' And mbln自制卡
        Control.Enabled = Control.Visible And zlIsHaveData And bln挂失
    Case conMenu_Edit_CardInFull  '缴预存
        Control.Visible = zlCheckPrivs(mstrPrepayPrivs, "门诊预交") Or zlCheckPrivs(mstrPrepayPrivs, "住院预交") Or zlCheckPrivs(mstrPrepayPrivs, "共用预交")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardInFullBack  '冲预存
        Control.Visible = zlCheckPrivs(mstrPrepayPrivs, "预交退款")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_CardBackMoney  '退款
        Control.Visible = zlCheckPrivs(mstrPrepayPrivs, "预交退款")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_MzToZy    '门诊转住院
        Control.Visible = zlCheckPrivs(mstrPrepayPrivs, "门诊预交转住院")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_ZyToMz    '住院转门诊
        Control.Visible = zlCheckPrivs(mstrPrepayPrivs, "住院预交转门诊")
        Control.Enabled = Control.Visible
   Case conMenu_Edit_ModiyPati
        Control.Visible = zlCheckPrivs(mstrPrivs, "调整病人信息")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_ChangePassWord    '密码调整
        Control.Visible = zlCheckPrivs(mstrPrivs, "修改密码")
        Control.Enabled = Control.Visible
    Case conMenu_View_Refresh   '刷新
    Case Else
        If (Control.id >= conMenu_ReportPopup * 100# + 1 And Control.id <= conMenu_ReportPopup * 100# + 99) And Control.Parameter <> "" Then
...
'---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSql = strSql & vbNewLine & _
           ...
           "                              B.住院号, B.国籍, B.家庭地址, B.家庭电话, B.监护人, B.联系人姓名, B.联系人关系," & vbNewLine & _
           ...
           "                                  Else " & vbNewLine & _
    "                                   '' "
    '问题号:116787,焦博,2017/12/12,增加"门诊预交余额"和"住院预交余额"显示病人余额。
    strSql = strSql & vbNewLine & _
           ...
           "                                To_Char(B.出生日期, 'yyyy-mm-dd hh24:mi:ss'), B.出生地点, B.身份证号, B.门诊号, B.住院号," & vbNewLine & _
           ...
           "          T.姓名, T.性别, T.年龄, T.出生日期, T.出生地点, T.身份证号, T.门诊号, T.住院号, T.国籍, T.家庭地址," & vbNewLine & _
           "          T.家庭电话, T.监护人, T.联系人姓名, T.联系人关系, T.联系人地址, T.联系人电话, T.工作单位, T.单位电话," & vbNewLine & _
           "          T.家庭地址邮编, T.在院, Nvl(Bd.变动类别, 0) As 变动类别, T.是否严格控制, T.是否重复使用," & vbNewLine & _
    ...
    With vsCardList
        ...
        For i = 1 To .Cols - 1
            ...
            ElseIf .ColKey(i) Like "*时间" Or .ColKey(i) Like "*日期" Or .ColKey(i) = "状态" Then
                .ColAlignment(i) = flexAlignCenterCenter
            ElseIf InStr(";变动类别;是否严格控制;是否重复使用;", ";" & .ColKey(i) & ";") > 0 Then
                .ColAlignment(i) = flexAlignCenterCenter
...
Private Sub vsCardList_DblClick()
    ...
    With vsCardList
        ...
        If strCardNo = "" Then Exit Sub
    End With
    If frmPatiCureCardEdit.zlShowCard(Me, mlngModule, mstrPrivs, Cr_查询, mlngCardTypeID, strCardNo) = False Then Exit Sub
End Sub
    ...
    Else
        Select Case IDKind.GetCurCard.名称
            Case "姓名", "姓名或就诊卡"
                ...
                If zlstr.ActualLen(strInput) < 4 Then Exit Function
                strPati = _
                ...
                "       LTrim(To_Char(max(Decode(类型, 1, 0, Nvl(b.预交余额, 0))), '99999999990.00')) As 住院预交余额," & vbNewLine & _
                "       max(c.卡号) as 卡号 " & vbNewLine & _
                "From 病人信息 A, 病人余额 B,病人医疗卡信息 C" & vbNewLine & _
                "Where a.停用时间 Is Null And a.病人id = b.病人id(+) And a.病人id=c.病人id(+) And b.性质(+) = 1 And Rownum < 101 " & vbNewLine & _
                "And a.姓名 Like [1] And c.卡类别ID(+)=[2] " & vbNewLine & _
