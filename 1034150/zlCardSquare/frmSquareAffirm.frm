...
'---------------------------------------------------------------------
'入参变量
Private mbytBillType As Byte, mlngModule As Long, mlngPatiID As Long
...
'        strSQL = strSubTable & strSQL & " And  A.NO in (Select Distinct NO From 门诊费用记录 J,医嘱 P Where A.病人ID=[1] and A.医嘱序号=P.ID)"
    ...
    If mbytBillType <= 1 Then
        strSfTable = "" & _
        "   Select   /*+ rule */ decode(A.记录性质,1,'收费',2,'记帐',4,'挂号') as 类别,A.记录性质,A.执行部门ID,A.发药窗口,A.病人ID, " & _
        "               A.NO,nvl(A.价格父号,A.序号) as 序号,B.编码||'-'||B.名称 as 项目,B.规格,nvl(A.付数,1)*A.数次 as 数次, " & _
        "               B.计算单位,A.收费细目ID,A.标准单价,A.应收金额,A.实收金额,A.收费类别,A.登记时间" & _
        "   From 门诊费用记录 A,收费项目目录 B" & IIf(mstrNos <> "", " ,单据 C", "") & _
        "   Where A.收费细目ID=B.ID And A.记录性质=1  And A.病人ID=[1]    " & _
        "               And  A.记录状态=0 "
        If mstr医嘱IDs <> "" And mstrNos <> "" Then
            '问题:49593
    ...
    If mbytBillType = 2 Or mbytBillType = 0 Then
        ...
        "               B.计算单位,A.收费细目ID,A.标准单价,A.应收金额,A.实收金额,A.收费类别,A.登记时间" & _
        "   From 门诊费用记录 A,收费项目目录 B" & IIf(mstrNos <> "", " ,单据 C", "") & _
        "   Where A.收费细目ID=B.ID And A.记录性质=2  And A.病人ID=[1]    " & _
        "               And  A.记录状态=0 "
        If mstr医嘱IDs <> "" And mstrNos <> "" Then
            '问题:49593
    ...
    Set rsTemp = OpenSQLRecordNew(strSQL, "获取病人费用信息", varPara)
...
Public Function zlSquareAffirm(ByVal frmMain As Object, _
    ...
    Optional ByRef lng结帐ID As Long = 0) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub cbo支付方式_Click()
    ...
    If Not mcolPayMode Is Nothing Then
        With mCurCardPay
         ...
         End With
     Else
            mCurCardPay.str结算方式 = NeedName(cbo支付方式.Text)
     End If
    '创建卡对象
...
Private Function GetSelectNOs() As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取选择的单据号
...
Private Function BrushcardStrikePrepay() As Boolean
    ...
    If zlDatabase.PatiIdentify(Me, glngSys, mlngPatiID, Val(txt冲预交), mlngModule, 1, mlngCardTypeID) Then
...
Private Function SaveCharge() As Boolean
    ...
    If mblnCliniqueRoomPay = False Then '非诊间支付时，需要检查相关的数据合法性
        ...
        If RoundEx(Val(txt冲预交.Text), 4) <> RoundEx(mCurCarge.dbl本次消费合计, 4) Then
           If BrushCardThreeSwapCheck(strNos, Val(txt金额.Text)) = False Then Exit Function
           dblThreeMoney = Val(txt金额.Text)
        End If
    Else
        If BrushCardThreeSwapCheck(strNos, dblThreeMoney) = False Then Exit Function
    End If
...
Private Function Load预交余额(ByVal lng病人ID As Long) As Boolean
    ...
    strSQL = "" & _
    "   Select Sum(nvl(预交余额,0)) as 预交余额,sum(费用余额) as 费用余额 " & _
    "   From 病人余额 " & _
    "   Where 病人ID=[1] and 性质=1 and nvl(类型,2) =1"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取病人余额", lng病人ID)
    With mCurCarge
        .dbl预交余额 = 0
        .dbl费用余额 = 0
        If Not rsTemp.EOF Then
            .dbl预交余额 = Val(Nvl(rsTemp!预交余额))
            .dbl费用余额 = Val(Nvl(rsTemp!费用余额))
        End If
        .dbl可用预交 = .dbl预交余额 - .dbl费用余额
        If .dbl可用预交 < 0 Then .dbl可用预交 = 0
    End With
    lbl预交余额.Caption = "预交余额:" & Format(Val(Nvl(rsTemp!预交余额)), "###0.00;-###0.00;0.00;0.00")
    lbl预交余额.Tag = Val(Nvl(rsTemp!预交余额))
    lbl费用余额.Caption = "未结费用:" & Format(Val(Nvl(rsTemp!费用余额)), "###0.00;-###0.00;0.00;0.00")
    lbl费用余额.Tag = Val(Nvl(rsTemp!费用余额))
    lbl剩余余额.Caption = "剩余款额:" & Format(Val(Nvl(rsTemp!预交余额)) - Val(Nvl(rsTemp!费用余额)), "###0.00;-###0.00;0.00;0.00")
    lbl剩余余额.Tag = Val(Nvl(rsTemp!预交余额)) - Val(Nvl(rsTemp!费用余额))
    Load预交余额 = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
Private Sub Form_Load()
    ...
    RaisEffect picSum, -1: RaisEffect picPayMode, -1
    '初始化数据
...
Private Sub InitPara()
    ...
    Call InitFactPara
    '门诊病人消费时需要刷卡验证
    '= Val(zlDatabase.GetPara(28, glngSys ))
    '费用单价保留位数
...
errHandle:
    ...
    End If
End Function
Private Function zlBrush消费卡(ByVal dblMoney As Double, ByVal rsClassMoney As ADODB.Recordset) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:处理消费卡刷卡
    '返回:成功,返回true,否则返回False
    '编制:刘兴洪
    ...
    zlBrush消费卡 = frmInput.zlBrushPay(Me, mlngModule, mobjPatiCardObject, rsClassMoney, _
        ...
        Nvl(mrsInfo!年龄), dblMoney, mCurCardPay.str刷卡卡号, mCurCardPay.str刷卡密码, , True, , False, cllBalance)
...
Private Function BrushCardThreeSwapCheck(ByVal strNos As String, _
    ...
    If mCurCardPay.bln消费卡 And mCurCardPay.bln自制卡 Then
        ...
        '肯定是处理自制卡
        If zlBrush消费卡(dblMoney, rsMoney) = False Then Exit Function
    Else
        '    zlBrushCard(frmMain As Object, _
        '        ByVal lngModule As Long, _
        '        ByVal lngCardTypeID As Long, _
