...
'功能：清理执行文件夹
    ...
    Dim cllFolders      As New Collection
    ...
    For Each objFolder In gobjFSO.GetFolder(strFolder).SubFolders
        '预升级不会删除预升级下载目录
        If objFolder.Name = "ZLPRETMP" Then
            If blnOk And (gotCurType = OT_OfficialUpgrade Or gotCurType = OT_Repair) Then
                cllFolders.Add objFolder, "K_" & objFolder.Path
            End If
        Else
            cllFolders.Add objFolder, "K_" & objFolder.Path
        End If
    Next
    Do While (cllFolders.Count <> 0)
        Set objFolder = cllFolders(1)
        cllFolders.Remove ("K_" & objFolder.Path)
        For Each objFile In objFolder.Files
            Call gobjFSO.DeleteFile(objFile.Path, True)
        Next
        Call gobjFSO.DeleteFolder(objFolder.Path, True)
    Loop
End Sub
