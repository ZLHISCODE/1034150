...
Private Sub cmdLog_Click()
    ...
    If gobjFSO.FileExists(strNotPad) Then
        ...
        End If
    Else
        MsgBox "本机没有安装记事本程序,不能打开日志文件!" & vbCrLf & "请手工用其它程序打开,记事本路径为:" & vbCrLf & gobjTrace.LogFile, vbInformation, "客户端自动升级"
    End If
End Sub

Private Sub cmdOK_Click()
    Call CallHISEXE(mblnOK)
    ...
    If ctType = CT_KillProcAndSvr Then
        ...
        For i = 1 To lngTotal
            ...
            If gobjFSO.FileExists(grsFileUpgrade!实际路径) Then
                ...
                strReturn = RunCommand("NET STOP " & gobjFSO.GetBaseName(grsFileUpgrade!实际路径), strErr, , 60000)
    ...
    If ctType = CT_StartSvr Then
        ...
        For i = 1 To lngTotal
            ...
            If gobjFSO.FileExists(grsFileUpgrade!实际路径) Then
                ...
                strReturn = RunCommand("NET Start " & gobjFSO.GetBaseName(grsFileUpgrade!实际路径), strErr, , 60000)
                gobjTrace.WriteInfo "ControlProcAndSvr", "启动结果", strReturn, "错误信息", strErr
            End If
            grsFileUpgrade.MoveNext
        Next
...
''功能：升级的主体逻辑代码
    ...
    If Not blnOperateOK Then
        ...
        Call SetOperateProcess(gotCurType, OS_Failure, SumErrMsg, glngFileBatch)  '标识升级完成
    Else
        Call RecordErrMsg(MT_MsgFoot, "消息尾", "结果:升级成功 时间:" & Format(Currentdate, "yyyy-MM-dd HH:mm:ss"))
        lblInfor.Caption = "正在标记本次升级状态"
...
ErrH:
    ...
    MsgBox "升级过程发生致命错误，请联系管理员！信息：" & Err.Description, vbInformation, App.Title
    Err.Clear
ErrEnd:
    Call RecordErrMsg(MT_MsgFoot, "消息尾", "结果:升级失败 时间:" & Format(Currentdate, "yyyy-MM-dd HH:mm:ss"))
    Call SetOperateProcess(gotCurType, OS_Failure, SumErrMsg, glngFileBatch)          '标识升级结束
    FileUpgrade = False
    cmdOK.Visible = True
    mblnOperateCompleted = True
End Function
...
ErrH:
    ...
    MsgBox "加载错误清单致命错误，请联系管理员！信息：" & strErr, vbInformation, App.Title
    ...
    With grsFileUpgrade
        ...
        For lngLoop = 1 To lngTotal
            ...
            gobjTrace.WriteInfo "CheckUpdate", "部件", !文件名, "安装路径", !安装路径, _
                            "自动注册", Decode(!自动注册, RFT_NotReg, "不注册", RFT_NormalReg, "自动识别注册", RFT_NETGAC, "NET全局程序集缓存注册", RFT_NETServer, "NET系统服务器注册", RFT_NETComReg, "NETCOM注册", RFT_VBComReg, "VBCom注册", RFT_DelphiComReg, "DelphiCom注册", RFT_PBComReg, "PBCom注册"), _
...
ErrH:
    ...
    MsgBox "检查文件更新发生致命错误，请联系管理员！信息：" & strTmpErr, vbInformation, App.Title
...
ErrH:
    ...
    MsgBox "下载解压文件发生致命错误，请联系管理员！信息：" & strErrInfo, vbInformation, App.Title
    Err.Clear
End Function
...
ErrH:
    ...
    MsgBox "清理弃用文件发生致命错误，请联系管理员！信息：" & strErr, vbInformation, App.Title
    Err.Clear
End Function
    ...
    With grsFileUpgrade
        ...
        For lngLoop = 1 To .RecordCount
            ...
            If strErrInfo = "" Then
                ...
                If Not gclsRegCom.RegCom(!实际路径, !自动注册, strErrInfo) Then
...
ErrH:
    ...
    MsgBox "安装注册文件发生致命错误，请联系管理员！信息：" & strErrInfo, vbInformation, App.Title
    Err.Clear
    lblInfor.Caption = "正在保存注册失败文件清单"
...
Private Function SetupOneFile(ByVal strSTFileName As String, ByVal strTmpFile As String, ByVal strSetupFile As String, Optional ByVal blnForceCover As Boolean, Optional ByRef strErrReturn As String) As Boolean
'功能：将升级文件放在安装路径
...
ErrH:
    ...
    MsgBox "批处理文件执行发生致命错误，请联系管理员！信息：" & strErrInfo, vbInformation, App.Title
    Err.Clear
End Function
