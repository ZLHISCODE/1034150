...
On Error GoTo ErrH
    ...
    strDataCon = "Provider=OraOLEDB.Oracle.1;User ID=" & strUser & ";password=" & strPassW & ";Data Source=" & strServer & ";Persist Security Info=False"
    
    mcnOracle.ConnectionString = strDataCon
    ...
    Exit Function
ErrH:
    MsgBox "PublicPacs接口连接错误，错误原因:" + err.Description + vbCrLf + "连接串配置:" + strDataCon, vbCritical, "系统信息"
    err.Clear
End Function
...
On Error GoTo Errorhand
    ...
    If Not InitOracleConn Then Exit Sub
...
Private Sub Menu_File_Preview(ByVal Control As XtremeCommandBars.ICommandBarControl)
...
Private Sub cbrMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_File_Preview   '预览
            Call Menu_File_Preview(Control)
        ...
        Case conMenu_File_ViewImage '观片
            If vsfReportList.RowSel <= 0 Then Exit Sub
            Call mdlPublic.ViewImage(vsfReportList.Cell(flexcpText, vsfReportList.RowSel, GetColNum(vsfReportList, "医嘱ID")), Me)
...
On Error GoTo Errorhand
    Select Case Control.ID
        ...
        Case mMenuID.conMenu_File_Preview, mMenuID.conMenu_File_Print
            Call InitButtonEnabled(Control)
...
Private Sub InitButtonEnabled(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    If vsfReportList.Rows <= 1 Then Exit Sub
    Control.Enabled = vsfReportList.Cell(flexcpText, vsfReportList.RowSel, GetColNum(vsfReportList, "医嘱ID")) = mlngAdviceId
    If Control.Enabled Or tabReport.Tab = 1 Then Control.Enabled = vsfReportList.Cell(flexcpText, vsfReportList.RowSel, GetColNum(vsfReportList, "类型")) = 2
End Sub
...
Private Sub LoadReport(strFilter As String)
    Dim strSql As String
    Dim rsTemp As ADODB.Recordset
    Dim strTmp As String
On Error GoTo Errorhand
    If Not InitOracleConn Or mblnIsConfiging Then Exit Sub
    ...
    If tabReport.Tab = 0 Then
        ...
        Else
            strSql = "Select* from (Select distinct m.类型, m.报告ID, m.执行科室id, m.查阅, m.打印, m.姓名,m.影像类别, m.医嘱内容,m.申请时间 , m.性别, m.年龄,  " & _
                ...
                " m.申请人, m.报告人, m.终审人, n.门诊号, n.就诊卡号, n.身份证号,m.病人id,m.医嘱id,m.开嘱科室ID " & _
                "From (Select distinct 2 as 类型,TO_CHAR(RAWTOHEX(b.id)) 报告ID,a.执行科室id,decode(nvl(f.查阅状态,0),0,'','√') 查阅, " & _
                "decode(nvl(b.报告打印,0),0,'','√') 打印,a.姓名,a.性别,a.年龄,a.影像类别,c.医嘱内容,c.病人来源, " & _
                ...
                "(c.病人id = e.病人id Or a.关联id = d.关联id) And c.相关id Is Null and " & strTmp & " and e.id = " & mlngAdviceId & _
                " Union All " & _
                "Select distinct 1 as 类型,TO_CHAR(b.病历id) 报告ID,a.执行科室id,decode(nvl(b.查阅状态,0),0,'','√') 查阅, " & _
                "decode(nvl(a.报告打印,0),0,'','√') 打印,a.姓名,a.性别,a.年龄,a.影像类别,c.医嘱内容,c.病人来源, " & _
                ...
                "where m.病人id = n.病人id and m.病人id=o.病人id(+) and m.医嘱id = p.医嘱id " & _
                IIf(mblnFirst, "", "and p.报到时间 between to_date('" & Format(dtpStart.Value, "yyyy-mm-dd 00:00:00") & "','yyyy-mm-dd hh24:mi:ss') and to_date('" & Format(dtpEnd.Value, "yyyy-mm-dd 23:59:59") & "','yyyy-mm-dd hh24:mi:ss')") & ") " & _
    ...
    If mblnFirst Then rsTemp.Filter = "医嘱id=" & mlngAdviceId
...
Private Sub Form_Resize()
    ...
    vsfReportList.Top = IIf(mblnRIS, 500, 2700)
    vsfReportList.Height = picReportList.Height - 600
End Sub
...
Private Sub picReportList_Resize()
    ...
    Else
        vsfReportList.Height = picReportList.ScaleHeight - picReportControl.Top - picReportControl.Height '- picBarControl.Height - 700
    End If
End Sub
...
Private Sub LoadRichReportContent(ByVal strDocId As String)
    ...
    strSql = "select 内容 from 电子病历格式 where 文件ID='" & strDocId & "'"
    Set rsTemp = GetRecordset(strSql)
    ...
    If rsTemp.RecordCount > 0 Then
        ...
        Call BlobToFile(rsTemp.Fields(0), tmpPath + "\\tmp.zip")
            
        '解压并加载显示
        Dim cc As New cUnzip
        
        cc.ZipFile = tmpPath + "\\tmp.zip"
...
On Error GoTo ErrH
    ...
    If Not InitOracleConn Then Exit Function
    
    If Trim(strRegNo) <> "" Then
        strSql = "Select A.ID As 医嘱ID, RawToHex(B.检查报告ID) As 报告ID, C.文档标题, C.报告内容.getClobVal(), C.报告状态, C.最后编辑时间, C.最后编辑人 " & _
...
On Error GoTo ErrH
    ...
    strSql = "Select A.医嘱ID, RawToHex(A.检查报告ID) As 报告ID, B.文档标题, B.报告内容.getClobVal(), B.报告状态, B.最后编辑时间, B.最后编辑人 " & _
...
On Error GoTo ErrHand
    ...
    If GetRecordset.State <> adStateOpen Then
        If InitOracleConn = False Then Exit Function
    End If
    
    If GetRecordset.State = adStateOpen Then GetRecordset.Close
    '打开
    GetRecordset.Open strSql, mcnOracle, adOpenKeyset, adLockOptimistic
