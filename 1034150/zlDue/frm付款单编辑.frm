...
'--------------------------------------------------------------
    ...
    strSQL = strSQL & _
             ...
             "   From (Select a.药品id, Sum(a.实际数量) 全院库存 From 药品库存 a, T1 b Where a.药品id = b.项目id " & _
             ...
             "   From (Select a.库房id, a.药品id, Sum(a.实际数量) 当前库房库存 From 药品库存 A, T1 B Where a.药品id = b.项目id " & _
...
Private Sub zlBackForward()
    ...
    ChangeMode 2
    zlCtlSetFocus mfrmEdit.vsPayEdit, True
End Sub
...
Private Function SaveCheck() As Boolean
    ...
    If mfrmEdit.zlCheck(cllPro) = False Then
        ChangeMode 2
        zlCtlSetFocus mfrmEdit.vsPayEdit
        Exit Function
    End If
...
Private Function SelMltProvide() As Boolean
    ...
    vRect = GetControlRect(txtDept.hwnd)
    lngH = txtDept.Height
