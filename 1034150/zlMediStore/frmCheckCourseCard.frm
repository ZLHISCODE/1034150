...
'        SQLCondition.str药品ID = ""
        ...
        gstrSQL = " Select distinct a.药品id" & _
                    " From 药品储备限额 A," & _
                         "收费项目目录 C" & _
                    " Where a.库房id = [1] And a.库房货位 in (select * from Table(Cast(f_Str2list([2]) As Zltools.t_Strlist))) and a.药品id=c.id "
...
'    Else
        ...
        gstrSQL = "Select b.编码 As 药品编码, a.药品id, Nvl(a.批次, 0) As 批次, a.批准文号" & _
                   ...
                   " Where A.性质 = 1 And A.药品id = b.Id And A.库房id = [1] And b.Id in (select * from Table(Cast(f_Num2List([2]) As zlTools.t_NumList)))"
...
'    End If
    ...
    With rsProperty
        ...
        For lngRow = 1 To lngRows
            ...
            DoEvents
            Call ShowPercent(lngRow / lngRows)
            DoEvents
...
Private Sub cmbBatch_Click()
    ...
    gstrSQL = "Select B.编码 As 药品编码, A.药品id, Nvl(A.批次, 0) As 批次, A.批准文号 " & _
            " From 药品库存 A, 收费项目目录 B " & _
            " Where A.性质 = 1 And A.药品id = B.Id And A.库房id = [1] "
    ...
    With rsData
        ...
        Do While Not .EOF
            ...
            DoEvents
            Call ShowPercent(.AbsolutePosition / .RecordCount)
            DoEvents
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            gstrSQL = "SELECT * " & _
                ...
                " A.摘要,填制人,填制日期,审核人,审核日期,A.频次 AS 盘点时间,A.成本价 AS 库存金额,A.成本金额 AS 库存差价,B.指导差价率,D.是否变价,B.药房分批 AS 药房分批核算,A.批准文号 " & _
            ...
            With vsfBill
                Do While Not rsInitCard.EOF
                    ...
                    If mbln相同单位 = True Then
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0) * Val(Split(.TextMatrix(intRow, mconIntCol比例系数), "|")(0)), mintPriceDigit)
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(Nvl(rsInitCard!零售价, 0) * Val(Split(.TextMatrix(intRow, mconIntCol比例系数), "|")(0)), mintPriceDigit)
                        
                        .TextMatrix(intRow, mconIntCol大单位数量) = GetFormat(rsInitCard.Fields("实盘数量").Value / Split(获取比例系数(rsInitCard), "|")(0), mintNumberDigit0)
                        .TextMatrix(intRow, mconintCol大单位) = IIf(IsNull(rsInitCard.Fields(Split(mstr单位, "|")(0)).Value), "", rsInitCard.Fields(Split(mstr单位, "|")(0)).Value)
                    Else
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0) * Val(Split(.TextMatrix(intRow, mconIntCol比例系数), "|")(1)), mintPriceDigit)
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(Nvl(rsInitCard!零售价, 0) * Val(Split(.TextMatrix(intRow, mconIntCol比例系数), "|")(1)), mintPriceDigit)
                    
                        .TextMatrix(intRow, mconIntCol大单位数量) = GetFormat(Int(rsInitCard.Fields("实盘数量").Value / Split(获取比例系数(rsInitCard), "|")(0)), mintNumberDigit0)
                        .TextMatrix(intRow, mconintCol大单位) = IIf(IsNull(rsInitCard.Fields(Split(mstr单位, "|")(0)).Value), "", rsInitCard.Fields(Split(mstr单位, "|")(0)).Value)
                        
                        .TextMatrix(intRow, mconIntCol小单位数量) = GetFormat((rsInitCard.Fields("实盘数量").Value / Split(获取比例系数(rsInitCard), "|")(0) - Val(.TextMatrix(intRow, mconIntCol大单位数量))) * Split(获取比例系数(rsInitCard), "|")(0) / Val(Split(获取比例系数(rsInitCard), "|")(1)), mintNumberDigit1)
                        .TextMatrix(intRow, mconintCol小单位) = IIf(IsNull(rsInitCard.Fields(Split(mstr单位, "|")(1)).Value), "", rsInitCard.Fields(Split(mstr单位, "|")(1)).Value)
                        
                        .TextMatrix(intRow, mconintCol数量_合计) = GetFormat(rsInitCard.Fields("实盘数量").Value, mintNumberDigit1)
...
'                End If
                ...
                If mblnNotTrigger <> True Then
                    mblnNotTrigger = True
'                    Set RecReturn = Frm药品选择器.ShowME(Me, 2, txtStock.Tag, txtStock.Tag, , False, True, False, True, IsHavePrivs(mstrPrivs, "查看盘点单库存"), 0, mblnNoStock, 0, False, mbln忽略服务对象)
                    If grsMaster.State = adStateClosed Then
                        Call SetSelectorRS(2, "药品盘点管理", txtStock.Tag, txtStock.Tag, , , , mbln盘停用药品, mblnNoStock, 1, , , mbln忽略服务对象)
                    End If
                    
                    Set RecReturn = frmSelector.ShowME(Me, 0, 2, , , , txtStock.Tag, txtStock.Tag, , 0, False, True, IsHavePrivs(mstrPrivs, "查看盘点单库存"), False, , mstrPrivs)
...
Private Function CheckData(ByVal rsTemp As ADODB.Recordset) As ADODB.Recordset
    ...
    With vsfBill    '把重复的查询出来
        For i = 1 To .rows - 2
            '34.110版本支持同个分批药品录入多个批次，不检查批次=-1(新增批次)的数据
...
'                    Set RecReturn = Frm药品多选选择器.ShowME(Me, 2, txtStock.Tag, txtStock.Tag, , strkey, sngLeft, sngTop, False, True, False, True, IsHavePrivs(mstrPrivs, "查看盘点单库存"), 0, mblnNoStock, 0, False, mbln忽略服务对象)
                    If grsMaster.State = adStateClosed Then
                        Call SetSelectorRS(2, "药品盘点管理", txtStock.Tag, txtStock.Tag, , , , mbln盘停用药品, mblnNoStock, 1, , , mbln忽略服务对象)
                    End If
                    Set RecReturn = frmSelector.ShowME(Me, 1, 2, strkey, sngLeft, sngTop, txtStock.Tag, txtStock.Tag, , 0, False, True, IsHavePrivs(mstrPrivs, "查看盘点单库存"), False, , mstrPrivs)
            ...
            Case mconIntCol大单位数量, mconIntCol小单位数量
                ...
                If strkey <> "" And .TextMatrix(.Row, 0) <> "" Then
                    If .Col = mconIntCol大单位数量 Then
                        strkey = GetFormat(strkey, mintNumberDigit0)
                    Else
                        strkey = GetFormat(strkey, mintNumberDigit1)
                    End If
                    .EditText = strkey
                End If
                ...
                End If
                .TextMatrix(.Row, mconintCol数量_合计) = GetFormat(strkey * Val(Split(.TextMatrix(.Row, mconIntCol比例系数), "|")(1)), mintNumberDigit1)
...
Private Function SaveCard() As Boolean
    ...
    With vsfBill
        ...
        If chrNo = "" Then chrNo = zlDataBase.GetNextNo(62, lng库房ID)
...
Private Sub 提示库存数()
    ...
    On Error GoTo errHandle
    If Not IsHavePrivs(mstrPrivs, "查看盘点单库存") Then Exit Sub
    ...
    With vsfBill
        ...
        staThis.Panels(2).Text = "该药品当前库存数为[" & GetFormat(dbl实际数量, mintNumberDigit1) & "]" & .TextMatrix(.Row, mconIntCol单位)
    End With
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
...
'   2.如果是分批核算药，则分别处理该药的未处理的各批次库存行。
    ...
    With vsfBill
        ...
        .TextMatrix(intRow, mconIntCol指导差价率) = rsData!指导差价率 & "||" & rsData!是否变价 & "||" & rsData!药房分批核算
        
        .TextMatrix(intRow, mconIntCol售价) = GetFormat(Nvl(rsData!售价, 0) * rsData.Fields(Replace(Split(mstr单位, "|")(1), "单位", "系数")).Value, mintPriceDigit)
        .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsData!成本价, 0) * rsData.Fields(Replace(Split(mstr单位, "|")(1), "单位", "系数")).Value, mintPriceDigit)
        
        dbl成本价 = rsData!平均成本价
        If rsData!是否变价 = 1 Then
            gstrSQL = "select Nvl(零售价,0)*" & rsData.Fields(Replace(Split(mstr单位, "|")(1), "单位", "系数")).Value & " As 零售价,nvl(实际金额,0)/nvl(实际数量,0)*" & rsData.Fields(Replace(Split(mstr单位, "|")(1), "单位", "系数")).Value & " as  售价 " _
                    & " from 药品库存 " _
                    & " where 库房id=[1] " _
                    & "   and 药品id=[2] " _
                    & "   and 性质=1 and 实际数量>0 and " _
                    & "   nvl(批次,0)=[3]"
            Set rsPrice = zlDataBase.OpenSQLRecord(gstrSQL, mstrCaption, txtStock.Tag, CLng(rsData!药品ID), CLng(IIf(IsNull(rsData!批次), "0", rsData!批次)))

            If rsPrice.EOF Then
                dbl加成率 = Nvl(rsData!加成率, 0) / 100
                dbl零售价 = dbl成本价 * (1 + dbl加成率)
                .TextMatrix(intRow, mconIntCol售价) = GetFormat(dbl零售价 * rsData.Fields(Replace(Split(mstr单位, "|")(1), "单位", "系数")).Value, mintPriceDigit)
            Else
                If rsData!批次 > 0 Then
                    If rsPrice!零售价 > 0 Then
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsPrice!零售价, mintPriceDigit)
                    Else
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsPrice!售价, mintPriceDigit)
                    End If
                Else
                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsPrice!售价, mintPriceDigit)
                End If
...
Private Function GetPhysicDetail(ByVal lng库房ID As Long, ByVal lng药品ID As Long, _
    ...
    gstrSQL = "" & _
        ...
        "   A.药品来源,A.基本药物,A.药库分批 AS 分批核算,A.药房分批 AS 药房分批核算,E.是否变价,A.指导差价率,A.加成率," & _
        ...
        "   B.批准文号,B.帐面数量,B.盘点数量,B.可用数量" & str单位 & ",Decode(sign(NVL(b.帐面数量,0)), 1,Decode(k.成本价, Null, a.成本价, k.成本价), a.成本价) 成本价 " & _
        ...
        "     ) GROUP BY 库房ID, 药品ID, 批次,批准文号) B," & _
        ...
        " AND A.药品ID=C.收费细目ID(+) AND C.性质(+)=3" & _
        ...
        Select Case Col
            Case mconIntCol大单位数量, mconIntCol小单位数量
                ...
                Else
                    lngSum = Val(.TextMatrix(Row, mconIntCol小单位数量)) + Val(.TextMatrix(Row, mconIntCol大单位数量)) * Val(Split(.TextMatrix(Row, mconIntCol比例系数), "|")(0)) / Val(Split(.TextMatrix(Row, mconIntCol比例系数), "|")(1))
                End If
                .TextMatrix(Row, mconintCol数量_合计) = GetFormat(lngSum * Val(Split(.TextMatrix(Row, mconIntCol比例系数), "|")(1)), mintNumberDigit1)
            Case mconintCol成本价
                If Val(.TextMatrix(Row, 0)) = 0 Then Exit Sub
