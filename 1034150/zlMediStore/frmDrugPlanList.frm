...
Private Sub SetVisable()
    ...
    If Not IsHavePrivs(mstrPrivs, "增加") Then
    ...
    If Not IsHavePrivs(mstrPrivs, "清除") Then
        mnuEditClear.Visible = False
        tlbTool.Buttons("Clear").Visible = False
    End If
    
    If Not IsHavePrivs(mstrPrivs, "取消审核") And Not IsHavePrivs(mstrPrivs, "取消复核") Then
    ...
    If Not IsHavePrivs(mstrPrivs, "复核") Then
        mnuEditCheck.Visible = False
        tlbTool.Buttons("Check").Visible = False
    End If
    
    If Not IsHavePrivs(mstrPrivs, "修改执行数量") Then
...
Private Sub mnuFileParameter_Click()
    ...
    frm参数设置.设置参数 Me, mstrPrivs, mlngMode, MStrCaption
    mint价格显示 = Val(zlDatabase.GetPara("价格显示方式", glngSys, 模块号.药品计划))
    mlastRow = 0
...
Private Sub mnuViewColDefine_Click()
    ...
    Case 模块号.药品计划           '药品计划管理
        strColumn_All = "药名,0|商品名,0|药品来源,1|规格,1|产地,0|单位,1|医保类型,1|前期数量,1|上期数量,1|库存上限,1|库存下限,1|" & _
                        "库存数量,0|库存金额,1|上期销量,1|本期销量,1|计划数量,0|执行数量,0|送货单位,1|送货数量,1|成本价,0|成本金额,0|售价,0|售价金额,0|上次供应商,1|说明,1"
        str选择列 = "药名|商品名|药品来源|规格|产地|单位|医保类型|前期数量|上期数量|库存上限|库存下限|库存数量|库存金额|上期销量|本期销量|计划数量|执行数量|送货单位|送货数量|成本价|成本金额|售价|售价金额|上次供应商|说明"
        str屏蔽列 = ""
    End Select
    ...
    If strColumn_Select <> "" Then
        ...
        If UBound(arr总列) <> UBound(arr设置列) Then
            strColumn_Select = "药名|商品名|药品来源|规格|产地|单位|医保类型|前期数量|上期数量|库存上限|库存下限|库存数量|库存金额|上期销量|本期销量|计划数量|执行数量|送货单位|送货数量|成本价|成本金额|售价|售价金额|上次供应商|说明"
            strColumn_UnSelect = ""
...
Private Sub vsfList_EnterCell()
    ...
    If vsfList.Row >= 1 And LTrim(vsfList.TextMatrix(vsfList.Row, 0)) <> "" Then
        ...
        gstrSQL = "SELECT B.序号" & strSql药名 & ",D.药品来源,D.规格, Decode(" & mintUnit & ", 1, d.计算单位, 2, d.门诊单位, 3, d.住院单位, d.药库单位) As 单位,d.医保类型," & _
                ...
                " Trim(To_Char(b.库存数量 * b.售价, " & mstrMoneyFormat & ")) 库存金额," & _
                ...
                " B.上次供应商,B.上次生产商,NVL(B.说明,'') 说明, " & _
                " TRIM(TO_CHAR(B.执行数量 / " & strUnit & mstrNumberFormat & ")) 执行数量 " & _
                ...
                "      F.规格, a.门诊单位,a.住院单位,A.药库单位,a.送货单位,a.送货包装 " & _
...
Private Sub SetDetailColWidth()
    With vsfDetail
        ...
        .ColAlignment(.ColIndex("库存金额")) = flexAlignRightCenter
        ...
        Else
            ...
            Else    '审核单
                ...
                If .TextMatrix(.Row, .ColIndex("复核人")) = "" Then    '未复核单
                    If mnuEditCancel.Visible = True And IsHavePrivs(mstrPrivs, "取消审核") Then
                ...
                Else
                    '已复核单
                    If mnuEditCancel.Visible = True And IsHavePrivs(mstrPrivs, "取消复核") Then
                        mnuEditCancel.Enabled = True
                        tlbTool.Buttons("Cancel").Enabled = True
                    End If
