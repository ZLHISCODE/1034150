...
Private Sub LoadHisPlane(ByVal lng库房ID As Long, ByVal lng药品ID As Long, ByVal lngRow As Long)
    ...
    With rsTmp
        ...
        Do While Not .EOF
            ...
            mrsHisPlane!计划数量 = GetFormat(Nvl(!计划数量, 0) / Val(mshBill.TextMatrix(lngRow, mconIntCol比例系数)), mintShowNumberDigit)
            mrsHisPlane!执行数量 = GetFormat(Nvl(!执行数量, 0) / Val(mshBill.TextMatrix(lngRow, mconIntCol比例系数)), mintShowNumberDigit)
            mrsHisPlane!No = !No
...
Public Sub ShowCard(FrmMain As Form, ByVal str单据号 As String, _
    ...
    ElseIf mint编辑状态 = 4 Then
        ...
        If Not IsHavePrivs(mstrPrivs, "采购计划打印") Then
...
Private Sub ShowHisPlane(ByVal lngRow As Long, ByVal lng药品ID As Long)
    ...
    With vsfHisPlane
        ...
        Do While Not mrsHisPlane.EOF
            .TextMatrix(.rows - 1, .ColIndex("计划数量")) = GetFormat(mrsHisPlane!计划数量, mintShowNumberDigit)
            .TextMatrix(.rows - 1, .ColIndex("执行数量")) = GetFormat(Nvl(mrsHisPlane!执行数量, 0), mintShowNumberDigit)
    ...
    If mint编辑状态 = 3 Then        '审核
        ...
        If SaveCheck = True Then
            If Val(zlDatabase.GetPara("审核打印", glngSys, 模块号.药品计划)) = 1 Then
                '打印
                If IsHavePrivs(mstrPrivs, "采购计划打印") Then
...
Private Sub Form_Activate()
    ...
    If mint编辑状态 = 1 Then
        ...
        If frmDrugPlanCondition.GetCondition(mfrmMain, str用途ID, str剂型编码, lng库房ID, int计划类型, _
                ...
                strToxicologyClass, mbln按销量产生计划, mstr来源药房, mstr来源库房, mstrAll来源药房) = True Then
            mlng库房ID = lng库房ID
            mint计划类型 = int计划类型
            mint编制方法 = int编制方法
            mbln数量方式 = bln数量方式
            ...
            Select Case mint计划类型
                Case 1       '月计划
                    Str期间 = Format(DateAdd("m", 1, zlDatabase.Currentdate), "yyyyMM")
                ...
                Case 2       '季计划
                    ...
                    Str期间 = Format(DateAdd("Q", 1, zlDatabase.Currentdate), "yyyy") & IIf(intMonth <= 3, 1, IIf(intMonth >= 10, 4, IIf(intMonth <= 9 And intMonth >= 7, 3, 2)))
                ...
                Case 3      '年计划
                    Str期间 = Format(DateAdd("yyyy", 1, zlDatabase.Currentdate), "yyyy")
                ...
                Case 4      '周计划
                    datCurrDate = zlDatabase.Currentdate
...
Private Sub ReFreshALLDrug(ByVal str用途ID, ByVal str剂型 As String, _
    ...
    '取指定条件的药品信息
    gstrSQL = "" & _
         " SELECT DISTINCT A.药品ID,'[' || F.编码 || ']' As 药品编码, F.名称 As 通用名, B.名称 As 商品名,A.药品来源,f.规格," & _
         " Decode(" & mintUnit & ", 1, f.计算单位, 2, a.门诊单位, 3, a.住院单位, a.药库单位) As 单位," & _
         " DECODE(A.成本价,NULL,NVL(A.指导批发价,0),0,NVL(A.指导批发价,0),NVL(A.成本价,0)) AS 单价,F.产地," & _
         " Decode(" & mintUnit & ", 1, 1, 2, a.门诊包装, 3, a.住院包装, a.药库包装) As 比例系数," & _
         " Nvl(G.现价, 0) 售价,a.上次售价,Nvl(F.是否变价,0) 是否变价, a.送货单位, a.送货包装,f.费用类型, a.上次供应商id As 供应商id, d.名称 As 供应商  " & _
    ...
    If lng库房ID = 0 Then
        ...
        gstrSQL = "( " & gstrSQL & ") D," & _
                  " (Select A.药品ID, C.ID As 供应商ID,C.名称 As 供应商, B.上次产地, A.库存数量, A.平均售价 " & _
    ...
    Else
        ...
        gstrSQL = "( " & gstrSQL & ") D," & _
                  " (Select DISTINCT A.药品ID, C.ID As 供应商ID,C.名称 As 供应商, B.上次产地, A.库存数量, A.平均售价 " & _
                  " From (Select 药品id, Sum(实际数量) As 库存数量, " & _
                  " Decode(Sign(Sum(实际数量)), 1, Decode(Sign(Sum(实际金额)), 1, Sum(实际金额), 0) / Sum(实际数量), 0) 平均售价 " & _
                  " From 药品库存 " & _
                  " Where 性质 = 1 "
        ...
        gstrSQL = gstrSQL & " Group By 药品id) A, " & _
                  " (Select 库房id,药品id,上次供应商ID,上次产地 From 药品库存 " & _
                  " Where 性质 = 1 AND (库房ID=[1] or 库房id In(select * from Table(Cast(f_Num2List([5]) As zlTools.t_NumList)))) " & _
                  " And (药品id,Nvl(批次, 0)) In " & _
                  " (Select 药品id,Nvl(Max(Nvl(批次, 0)), 0) 批次 From 药品库存 Where 性质 = 1 AND (库房ID=[1] or 库房id In(select * from Table(Cast(f_Num2List([5]) As zlTools.t_NumList))))  Group By 药品id )) B, " & _
                  " (SELECT ID,名称 FROM 供应商 WHERE SUBSTR(类型,1,1)=1) C " & _
    ...
    gstrSQL = "SELECT d.药品id, d.药品编码, d.通用名, d.商品名, d.药品来源,d.规格, " _
            ...
            & " Decode(D.是否变价, 0, D.售价, Decode(nvl(d.上次售价,0), 0, Decode(Nvl(E.平均售价, 0), 0, D.售价, E.平均售价), d.上次售价)) 售价,d.送货单位,d.送货包装,d.费用类型 from " _
            & gstrSQL _
            & " WHERE d.药品id = e.药品id (+) "
    If lng库房ID <> 0 Then
    ...
    With rsAllDrug
        ...
        Do While Not .EOF
            ...
            If IIf(IsNull(!库存数量), 0, !库存数量) = 0 Then
                If mstr供应商ID = "" Then
                    gstrSQL = "Select B.id 供应商ID, B.名称 供应商, C.上次产地, 0 库存数量 from " & _
            ...
            If blnOk Then
                ...
                mshBill.TextMatrix(intRow, mconIntCol产地) = str产地
                ...
                mshBill.TextMatrix(intRow, mconintCol库存数量) = GetFormat(dbl库存数量, mintShowNumberDigit)
                
                mshBill.TextMatrix(intRow, mconIntCol比例系数) = !比例系数
                
                mshBill.TextMatrix(intRow, mconIntCol成本价) = GetFormat(IIf(IsNull(!单价), "0", !单价 * !比例系数), mintShowPriceDigit)
                mshBill.TextMatrix(intRow, mconIntCol售价) = GetFormat(IIf(IsNull(!售价), "0", !售价 * !比例系数), mintShowPriceDigit)
                mshBill.TextMatrix(intRow, mconintCol库存金额) = GetFormat(mshBill.TextMatrix(intRow, mconintCol库存数量) * mshBill.TextMatrix(intRow, mconIntCol售价), mintShowMoneyDigit)
                ...
                If dbl送货包装 <> 0 Then
                    ...
                    mshBill.TextMatrix(intRow, mconintCol送货单位) = str送货单位 & "(1" & str送货单位 & "=" & GetFormat(dbl送货包装 / !比例系数, 1) & mshBill.TextMatrix(intRow, mconIntCol单位) & ")"
                    mshBill.TextMatrix(intRow, mconintCol送货数量) = GetFormat(Val(mshBill.TextMatrix(intRow, mconintCol计划数量)) / dbl送货包装, 1)
                End If
                
                If intRow = mshBill.rows - 1 Then mshBill.rows = mshBill.rows + 1
                Call ShowPercent(intRow / intRecord)
            End If
            .MoveNext
        Loop
    End With
    ...
    With mshBill
        Select Case int编制方法
            Case 1      '往年同期线形参照   只有月度、季度计划
                dat前期 = DateAdd("m", Choose(int计划类型, 1, 3), DateAdd("yyyy", -2, zlDatabase.Currentdate))
                dat上期 = DateAdd("m", Choose(int计划类型, 1, 3), DateAdd("yyyy", -1, zlDatabase.Currentdate))
                If lng库房ID = 0 Then
                ...
                End If
                .TextMatrix(intCurrentRow, mconIntCol前期数量) = GetFormat(num前期数量, mintShowNumberDigit)
                .TextMatrix(intCurrentRow, mconIntCol上期数量) = GetFormat(num上期数量, mintShowNumberDigit)
                .TextMatrix(intCurrentRow, mconintCol计划数量) = IIf(GetFormat(num计划数量, mintShowNumberDigit) = 0, "", GetFormat(num计划数量, mintShowNumberDigit))
                .TextMatrix(intCurrentRow, mconintCol成本金额) = _
                        IIf(GetFormat(num计划数量 * IIf(mshBill.TextMatrix(intCurrentRow, mconIntCol成本价) = "", 0, mshBill.TextMatrix(intCurrentRow, mconIntCol成本价)), mintShowNumberDigit) = 0 _
                        , "" _
                        , GetFormat(num计划数量 * IIf(mshBill.TextMatrix(intCurrentRow, mconIntCol成本价) = "", 0, mshBill.TextMatrix(intCurrentRow, mconIntCol成本价)), mintShowNumberDigit))
                .TextMatrix(intCurrentRow, mconIntCol售价金额) = _
                        IIf(GetFormat(num计划数量 * IIf(mshBill.TextMatrix(intCurrentRow, mconIntCol售价) = "", 0, mshBill.TextMatrix(intCurrentRow, mconIntCol售价)), mintShowNumberDigit) = 0 _
                        , "" _
                        , GetFormat(num计划数量 * IIf(mshBill.TextMatrix(intCurrentRow, mconIntCol售价) = "", 0, mshBill.TextMatrix(intCurrentRow, mconIntCol售价)), mintShowNumberDigit))
            Case 2      '临近期间平均参照法
                dat前期 = Choose(int计划类型, DateAdd("m", -2, zlDatabase.Currentdate), DateAdd("m", -6, zlDatabase.Currentdate), DateAdd("yyyy", -2, zlDatabase.Currentdate), DateAdd("d", -14, mdatThisMondyDate))
                dat上期 = Choose(int计划类型, DateAdd("m", -1, zlDatabase.Currentdate), DateAdd("m", -3, zlDatabase.Currentdate), DateAdd("yyyy", -1, zlDatabase.Currentdate), DateAdd("d", -7, mdatThisMondyDate))
                If lng库房ID = 0 Then
                    gstrSQL = "SELECT ABS(SUM(NVL(数量, 0))) AS 前期数量 " _
                            & " FROM 药品收发汇总 a, 药品入出类别 b " _
                           & " Where a.类别id = b.id " _
            ...
            Case 3      '药品储备定额参照法
                If mstrBeginDate = "" Or mstrEndDate = "" Then
                    mstrEndDate = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                    mstrBeginDate = Format(DateAdd("m", -1, zlDatabase.Currentdate), "yyyy-mm-dd")
                End If
            ...
            Case 5  '自定义区间
                ...
                If mbln计划数量 Then
                    ...
                    End If
                End If
                .TextMatrix(intCurrentRow, mconIntCol前期数量) = GetFormat(num上期数量, mintShowNumberDigit)
                .TextMatrix(intCurrentRow, mconintCol计划数量) = IIf(GetFormat(num计划数量, mintShowNumberDigit) = 0, "", GetFormat(num计划数量, mintShowNumberDigit))
                .TextMatrix(intCurrentRow, mconintCol成本金额) = _
                        IIf(GetFormat(num计划数量 * IIf(mshBill.TextMatrix(intCurrentRow, mconIntCol成本价) = "", 0, mshBill.TextMatrix(intCurrentRow, mconIntCol成本价)), mintShowNumberDigit) = 0 _
                        , "" _
        ...
        If Not rsNum.EOF Then
            .TextMatrix(intCurrentRow, mconIntCol库存上限) = GetFormat(Nvl(rsNum!上限, 0) / Val(.TextMatrix(intCurrentRow, mconIntCol比例系数)), mintShowNumberDigit)
            .TextMatrix(intCurrentRow, mconIntCol库存下限) = GetFormat(Nvl(rsNum!下限, 0) / Val(.TextMatrix(intCurrentRow, mconIntCol比例系数)), mintShowNumberDigit)
        End If
        ...
        If rsNum.RecordCount > 0 Then
            .TextMatrix(intCurrentRow, mconintCol上期销量) = GetFormat(Nvl(rsNum!销售数量, 0) / Val(.TextMatrix(intCurrentRow, mconIntCol比例系数)), mintShowNumberDigit)
        End If
        ...
        Set rsNum = zlDatabase.OpenSQLRecord(gstrSQL, MStrCaption, lng药品ID, CDate(strBegin), CDate(strEnd), mstr来源药房)
        If rsNum.RecordCount > 0 Then
            .TextMatrix(intCurrentRow, mconintCol本期销量) = GetFormat(Nvl(rsNum!销售数量, 0) / Val(.TextMatrix(intCurrentRow, mconIntCol比例系数)), mintShowNumberDigit)
        End If
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4, 5, 6
            ...
            gstrSQL = "SELECT a.id,nvl(a.库房id,0) as 库房id,nvl(c.名称,'全院') AS 库房,a.药房id, a.no, a.计划类型,a.期间, a.编制方法, a.编制人," _
                    ...
                    & "b.序号,b.药品id,d.药品编码,d.通用名,d.商品名,d.药品来源, d.规格" & str单位 & ", nvl(b.前期数量,0) as 前期数量, nvl(b.上期数量,0) as 上期数量, " _
                    & " nvl(b.上期销量,0) as 上期销量,nvl(b.本期销量,0) as 本期销量,b.库存数量, b.计划数量,nvl(b.执行数量,0) as 执行数量,b.送货数量,d.送货单位,d.送货包装, b.单价, b.金额, b.上次供应商,b.上次生产商,b.说明,b.售价,b.售价金额,d.费用类型 " _
                    & " FROM 药品采购计划 a, 药品计划内容 b,部门表 c," _
                    & " (SELECT DISTINCT a.药品id," _
                    & " '[' || C.编码 || ']' As 药品编码, C.名称 As 通用名, B.名称 As 商品名,a.药品来源,c.规格,a.药库单位,A.药库包装,a.门诊单位,A.门诊包装,a.住院单位,a.住院包装,a.送货单位,a.送货包装,C.计算单位 售价单位,1 售价包装,c.费用类型 " _
            ...
            With mshBill
                For intRow = 1 To intRecordCount
                    ...
                    .TextMatrix(intRow, mconIntCol产地) = IIf(IsNull(rsInitCard!上次生产商), "", rsInitCard!上次生产商)
                    ...
                    .TextMatrix(intRow, mconIntCol前期数量) = GetFormat(rsInitCard!前期数量 / rsInitCard!比例系数, mintShowNumberDigit)
                    .TextMatrix(intRow, mconIntCol上期数量) = GetFormat(rsInitCard!上期数量 / rsInitCard!比例系数, mintShowNumberDigit)
                    .TextMatrix(intRow, mconintCol上期销量) = GetFormat(rsInitCard!上期销量 / rsInitCard!比例系数, mintShowNumberDigit)
                    .TextMatrix(intRow, mconintCol本期销量) = GetFormat(rsInitCard!本期销量 / rsInitCard!比例系数, mintShowNumberDigit)
                    .TextMatrix(intRow, mconintCol库存数量) = GetFormat(rsInitCard!库存数量 / rsInitCard!比例系数, mintShowNumberDigit)
                    .TextMatrix(intRow, mconintCol库存金额) = GetFormat(rsInitCard!库存数量 * rsInitCard!售价, mintShowMoneyDigit)
                    .TextMatrix(intRow, mconintCol计划数量) = IIf(GetFormat(rsInitCard!计划数量, mintShowNumberDigit) = 0, "", GetFormat(rsInitCard!计划数量 / rsInitCard!比例系数, mintShowNumberDigit))
                    .TextMatrix(intRow, mconintCol执行数量) = IIf(GetFormat(rsInitCard!执行数量, mintShowNumberDigit) = 0, "", GetFormat(rsInitCard!执行数量 / rsInitCard!比例系数, mintShowNumberDigit))
                    .TextMatrix(intRow, mconintCol原执行数量) = IIf(GetFormat(rsInitCard!执行数量, mintShowNumberDigit) = 0, "", GetFormat(rsInitCard!执行数量 / rsInitCard!比例系数, mintShowNumberDigit))
                    
                    dbl送货包装 = IIf(IsNull(rsInitCard!送货包装), 0, rsInitCard!送货包装)
                    str送货单位 = IIf(IsNull(rsInitCard!送货单位), "", rsInitCard!送货单位)
                    If dbl送货包装 <> 0 Then
                        .TextMatrix(intRow, mconintCol送货数量) = IIf(IsNull(rsInitCard!送货数量), "", GetFormat(rsInitCard!送货数量, 1))
                        .TextMatrix(intRow, mconintCol送货单位) = str送货单位 & "(1" & str送货单位 & "=" & GetFormat(dbl送货包装 / rsInitCard!比例系数, 1) & rsInitCard!单位 & ")"
                        .TextMatrix(intRow, mconintCol送货包装) = dbl送货包装
                    End If
                    
                    .TextMatrix(intRow, mconIntCol成本价) = GetFormat(rsInitCard!单价 * rsInitCard!比例系数, mintShowPriceDigit)
                    .TextMatrix(intRow, mconintCol成本金额) = IIf(GetFormat(rsInitCard!金额, mintShowMoneyDigit) = 0, "", GetFormat(rsInitCard!金额, mintShowMoneyDigit))
                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!售价 * rsInitCard!比例系数, mintShowPriceDigit)
                    .TextMatrix(intRow, mconIntCol售价金额) = IIf(GetFormat(rsInitCard!售价金额, mintShowMoneyDigit) = 0, "", GetFormat(rsInitCard!售价金额, mintShowMoneyDigit))
...
'                    Set rsTemp = Frm药品多选选择器.ShowME(Me, 1, , mlng库房ID, , strkey, sngLeft, sngTop)
                            ...
                            With mshBill
                                ...
                                Call SetDrugRows(rsTemp!药品ID, _
                                        ...
                                        Switch(strUnit = "售价单位", 1, strUnit = "门诊单位", rsTemp!门诊包装, strUnit = "住院单位", _
                                               rsTemp!住院包装, strUnit = "药库单位", rsTemp!药库包装))
            ...
            Case mconintCol计划数量
                ...
                If .Text = "" Then
                    ...
                    If .TextMatrix(.Row, mconintCol计划数量) <> "" Then
                        ...
                        If .TextMatrix(.Row, mconIntCol成本价) <> "" Then
                            .TextMatrix(.Row, mconintCol成本金额) = GetFormat(.TextMatrix(.Row, mconIntCol成本价) * strkey, mintShowMoneyDigit)
                        End If
                        If .TextMatrix(.Row, mconIntCol售价) <> "" Then
                            .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(.TextMatrix(.Row, mconIntCol售价) * strkey, mintShowMoneyDigit)
                        End If
                    End If
                    .Col = mconintCol说明
                    Cancel = True
                ...
                If strkey <> "" Then
                    strkey = GetFormat(strkey, mintShowNumberDigit)
                    ...
                    If Val(.TextMatrix(.Row, mconintCol送货包装)) <> 0 Then
                        .TextMatrix(.Row, mconintCol送货数量) = GetFormat(Val(.TextMatrix(.Row, mconIntCol比例系数)) / Val(.TextMatrix(.Row, mconintCol送货包装)) * Val(strkey), 1)
                    End If
                End If
            ...
            Case mconIntCol成本价
                ...
                If strkey <> "" Then
                    strkey = GetFormat(strkey, mintShowPriceDigit)
                    .Text = strkey
                    If .TextMatrix(.Row, mconintCol计划数量) <> "" Then
                        .TextMatrix(.Row, mconintCol成本金额) = GetFormat(.TextMatrix(.Row, mconintCol计划数量) * strkey, mintShowMoneyDigit)
                    End If

                End If
                Call 显示合计金额
            Case mconIntCol产地
                If strkey = "" And .TextMatrix(.Row, mconIntCol产地) = "" Then
                    strkey = " "
                    .Text = strkey
                    .TextMatrix(.Row, mconIntCol产地) = strkey
                Else
            ...
            Case mconintCol说明
                ...
                Else
                    If StrIsValid(strkey, 50) = False Then
                        Cancel = True
                        .TxtSetFocus
                        Exit Sub
                    End If
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        NO_IN = Trim(txtNo)
        If NO_IN = "" Then NO_IN = zlDatabase.GetNextNo(32, mlng库房ID)
        ...
        编制日期_IN = Format(zlDatabase.Currentdate, "yyyy-mm-dd hh:mm:ss")
        ...
        '来源库房ID
        gstrSQL = gstrSQL & ",'" & IIf(mstr来源库房 = "", mlng库房ID, mstr来源库房) & "'"
        '来源药房ID
        ...
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                上次生产商_IN = .TextMatrix(intRow, mconIntCol产地)
...
Private Sub 显示合计金额()
    ...
    End With
    If mint价格显示 = 0 Then
        lblPurchasePrice.Caption = "金额合计：" & GetFormat(Dbl金额, mintShowMoneyDigit)
    ElseIf mint价格显示 = 1 Then
        lblPurchasePrice.Caption = "金额合计：" & GetFormat(dbl售价金额, mintShowMoneyDigit)
    Else
        lblPurchasePrice.Caption = "成本金额合计：" & GetFormat(Dbl金额, mintShowMoneyDigit) & "      售价金额合计：" & GetFormat(dbl售价金额, mintShowMoneyDigit)
    End If
End Sub
...
Private Sub 显示库存()
    ...
    If txtNo <> "" Then
        ...
        If rsData!来源库房 <> 0 Then
            mstr来源库房 = rsData!来源库房
        Else
            mstr来源库房 = ""
            mlng库房ID = rsData!来源库房
        End If

        mstr来源药房 = rsData!来源药房
    End If
    ...
    If chk来源药房.Value = 1 Or chk来源库房.Value = 1 Or chk所有库房.Value = 1 Or mstr自定义库房 <> "" Then
        ...
        gstrSQL = "Select B.名称, A.药品id, Nvl(Sum(A.可用数量),0) As 可用数量, Nvl(Sum(A.实际数量),0) As 实际数量 " & _
            " From 药品库存 A, 部门表 B " & _
            " Where A.性质 = 1 And A.库房id + 0 = B.ID And A.药品id = [1] And("
            
        If chk所有库房.Value <> 1 Then
            If chk来源库房.Value = 1 And mlng库房ID <> 0 Then
                gstrSQL = gstrSQL & " A.库房id In(select * from Table(Cast(f_Num2List([2]) As zlTools.t_NumList))) "
                vsfStock.Tag = 1
            ElseIf chk来源库房.Value = 1 And mlng库房ID = 0 Then
                gstrSQL = gstrSQL & " 1=1 "
                vsfStock.Tag = 1
            End If
            
            If chk来源药房.Value = 1 Then
                gstrSQL = gstrSQL & IIf(vsfStock.Tag = 1, "Or", "") & " A.库房id In(select * from Table(Cast(f_Num2List([3]) As zlTools.t_NumList))) "
                vsfStock.Tag = 1
            End If
        
            If mstr自定义库房 <> "" Then
                gstrSQL = gstrSQL & IIf(vsfStock.Tag = 1, "Or", "") & " A.库房id In(select * from Table(Cast(f_Num2List([4]) As zlTools.t_NumList))) "
                vsfStock.Tag = 1
            End If
        End If
        
        If vsfStock.Tag = 1 Then
            gstrSQL = gstrSQL & ")"
        Else
            gstrSQL = gstrSQL & " 1=1)"
        End If
        ...
        Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, "显示库存", lng药品ID, IIf(mstr来源库房 = "", mlng库房ID, mstr来源库房), IIf(mstr来源药房 = "", mstrAll来源药房, mstr来源药房), mstr自定义库房)
...
Private Function SetDrugRows(ByVal lng药品ID As Long, ByVal str药品编码 As String, ByVal str通用名 As String, ByVal str商品名 As String, ByVal str药品来源 As String, _
    ...
    With mshBill
        ...
        .TextMatrix(.Row, mconIntCol产地) = str产地
        .TextMatrix(.Row, 0) = lng药品ID
        .TextMatrix(.Row, mconIntCol比例系数) = dbl比例系数
        
        gstrSQL = "Select a.成本价,a.指导批发价,b.名称 as 供应商 From 药品规格 a ,供应商 b Where a.上次供应商id=b.id(+) and a.药品ID=[1]"
        ...
        gstrSQL = "Select Decode(Nvl(a.是否变价, 0), 0, Nvl(b.现价, 0), Decode(nvl(d.上次售价,0), 0, Decode(Nvl(c.平均售价, 0), 0, b.现价, c.平均售价), d.上次售价)) 售价 " & _
                 ...
                 " (B.终止日期 Is Null Or Sysdate Between B.执行日期 And Nvl(B.终止日期, To_Date('3000-01-01', 'yyyy-MM-dd'))) "
        Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[取售价]", lng药品ID, mlng库房ID)
        dbl销售单价 = rsData!售价
        
        If mlng库房ID = 0 Then
            '如果是全库房，则从药品库存中取库存数量，从药品规格中取供应商，上次产地
            gstrSQL = "Select B.名称 供应商, C.上次产地, Nvl(A.库存数量,0) 库存数量" & _
        ...
        Else
            ...
            gstrSQL = " Select C.名称 As 供应商, B.上次产地, A.库存数量 " & _
            ...
            If rsData.RecordCount = 0 Then
                gstrSQL = "Select B.名称 供应商, C.上次产地, 0 库存数量 from " & _
                          " (Select ID,名称 From 供应商 Where Substr(类型, 1, 1) = 1) B, 药品规格 C " & _
                          " Where C.上次供应商id = B.ID(+) And 药品id = [1] "
                Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[取上次供应商及产地信息]", lng药品ID, mlng库房ID)
            End If
        ...
        If Not rsData.EOF Then
            .TextMatrix(.Row, mconintCol库存数量) = GetFormat(IIf(IsNull(rsData!库存数量), 0, rsData!库存数量) / dbl比例系数, mintShowNumberDigit)
            .TextMatrix(.Row, mconintCol库存金额) = GetFormat(IIf(IsNull(rsData!库存数量), 0, rsData!库存数量) * dbl销售单价, mintShowMoneyDigit)
            ...
            .TextMatrix(.Row, mconIntCol产地) = IIf(IsNull(rsData!上次产地), str产地, rsData!上次产地)
            SetNumer lng药品ID, mlng库房ID, .TextMatrix(.Row, mconintCol库存数量), .Row, mint计划类型, mint编制方法, mbln数量方式
        End If
        
        '加载大包装入库信息
        gstrSQL = "select a.送货单位,a.送货包装,b.费用类型 from 药品规格 a,收费项目目录 b where a.药品id=b.id and a.药品id=[1]"
        ...
        If dbl送货包装 <> 0 Then
            .TextMatrix(.Row, mconintCol送货单位) = str送货单位 & "(1" & str送货单位 & "=" & GetFormat(dbl送货包装 / dbl比例系数, 1) & str单位 & ")"
            .TextMatrix(.Row, mconintCol送货包装) = dbl送货包装
            If Val(.TextMatrix(.Row, mconintCol计划数量)) <> 0 Then
                .TextMatrix(.Row, mconintCol送货数量) = GetFormat(Val(.TextMatrix(.Row, mconintCol计划数量)) / dbl送货包装, 1)
            End If
        End If
        ...
        .TextMatrix(.Row, mconIntCol成本价) = GetFormat(dbl成本单价 * dbl比例系数, mintShowPriceDigit)
        .TextMatrix(.Row, mconIntCol售价) = GetFormat(dbl销售单价 * dbl比例系数, mintShowPriceDigit)
    End With
    rsData.Close
    SetDrugRows = True
    Exit Function
...
Private Sub SetColumnByUserDefine()
    ...
    strColumn_All = "药名,2|商品名,3|药品来源,4|规格,5|产地,6|单位,7|医保类型,9|前期数量,10|上期数量,11|库存上限,12|库存下限,13|" & _
                        "库存数量,14|库存金额,15|上期销量,16|本期销量,17|计划数量,18|执行数量,19|送货单位,21|送货数量,22|成本价,24|成本金额,25|售价,26|售价金额,27|上次供应商,28|说明,29"
    ...
    If strColumn_Selected <> "" Then
        ...
        If UBound(arr总列) <> UBound(arr可设置列) Then
            strColumn_Selected = "药名|商品名|药品来源|规格|产地|单位|医保类型|前期数量|上期数量|库存上限|库存下限|库存数量|库存金额|上期销量|本期销量|计划数量|执行数量|送货单位|送货数量|成本价|成本金额|售价|售价金额|上次供应商|说明"
