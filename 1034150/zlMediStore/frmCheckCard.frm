...
Private Const mconIntCol指导差价率 As Integer = 13
...
Private Sub cmdBatch_Click()
    ...
    gstrSQL = "Select  Distinct a.药品id, b.编码, b.名称, c.库房货位 " & _
        ...
        " where　a.性质 = 1 And a.药品id = b.Id And a.库房id = c.库房id(+) And a.药品id = c.药品id(+) And a.库房id = [1]"
        ...
        Do While Not rsPhysic.EOF
            ...
            Do While Not rsDetail.EOF
                ...
                If rsDetail!是否变价 = 1 Then
                    If Nvl(rsDetail!批次, 0) > 0 Then
                        gstrSQL = "select Nvl(零售价,0) As 零售价 " _
                            & " from 药品库存 " _
                            & " where 性质=1 and 库房id=[1] and 药品id=[2] and nvl(批次,0)=[3]"
                        Set rs时价分批 = zlDataBase.OpenSQLRecord(gstrSQL, mstrCaption, Val(txtStock.Tag), CLng(rsPhysic!药品ID), CLng(rsDetail!批次))
                        If Not rs时价分批.EOF Then
                            If rs时价分批!零售价 > 0 Then
                                dbl零售价 = rs时价分批!零售价
                            Else
                                If Nvl(rsDetail!帐面数量, 0) <> 0 Then
                                    dbl零售价 = rsDetail!实际金额 / rsDetail!帐面数量
                                Else
                                    dbl加成率 = Nvl(rsDetail!加成率, 0) / 100
                                    dbl零售价 = FormatEx(时价药品零售价(rsDetail!药品ID, dbl加成率, dbl成本价), mintPriceDigit)
                                End If
                            End If
                        Else
                            If Nvl(rsDetail!帐面数量, 0) <> 0 Then
                ...
                .TextMatrix(.rows - 1, mconIntCol来源) = Nvl(rsDetail!药品来源)
                .TextMatrix(.rows - 1, mconIntCol基本药物) = Nvl(rsDetail!基本药物)
                .TextMatrix(.rows - 1, mconIntCol规格) = IIf(IsNull(rsDetail!规格), "", rsDetail!规格)
                .TextMatrix(.rows - 1, mconIntCol产地) = Nvl(rsDetail!产地, Nvl(rsDetail!缺省产地))
                ...
                .TextMatrix(.rows - 1, mconIntCol实际金额) = Nvl(rsDetail!实际金额, 0)
                .TextMatrix(.rows - 1, mconIntCol实际差价) = Nvl(rsDetail!实际差价, 0)
                .TextMatrix(.rows - 1, mconIntCol指导差价率) = rsDetail!指导差价率 & "||" & rsDetail!是否变价 & "||" & rsDetail!药房分批核算
                .TextMatrix(.rows - 1, mconintCol标志) = "平"
                .TextMatrix(.rows - 1, mconintCol数量差) = "0"
                .TextMatrix(.rows - 1, mconintCol库存数量) = Nvl(rsDetail!帐面数量, 0)
                
                If mintUnit > 0 Then
                    .TextMatrix(.rows - 1, mconIntCol单位) = IIf(IsNull(rsDetail!单位), "", rsDetail!单位)
                    .TextMatrix(.rows - 1, mconIntCol售价) = GetFormat(dbl零售价 * rsDetail!比例系数, mintPriceDigit)
                    .TextMatrix(.rows - 1, mconintCol帐面数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数, mintNumberDigit)
                    .TextMatrix(.rows - 1, mconIntCol比例系数) = Nvl(rsDetail!比例系数, 0)
                    .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(.TextMatrix(.rows - 1, mconintCol帐面数量), mintNumberDigit)
                    .TextMatrix(.rows - 1, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) * Val(.TextMatrix(.rows - 1, mconIntCol售价)), mintMoneyDigit)
                    .TextMatrix(.rows - 1, mconIntCol可用数量) = Nvl(rsDetail!可用数量, 0) / rsDetail!比例系数
                    .TextMatrix(.rows - 1, mconintCol成本价) = GetFormat(Nvl(rsDetail!成本价, 0) * rsDetail!比例系数, mintCostDigit)
                Else
                    .TextMatrix(.rows - 1, mconIntCol售价) = GetFormat(dbl零售价 * rsDetail!比例系数小, mintPriceDigit0)
                    .TextMatrix(.rows - 1, mconintCol帐面数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                    
                    .TextMatrix(.rows - 1, mconIntCol比例系数大) = Nvl(rsDetail!比例系数大, 0)
                    .TextMatrix(.rows - 1, mconIntCol比例系数小) = Nvl(rsDetail!比例系数小, 0)
                    ...
                    .TextMatrix(.rows - 1, mconintCol大包装帐面数量) = Int(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数大)
                    .TextMatrix(.rows - 1, mconintCol大包装实盘数量) = .TextMatrix(.rows - 1, mconintCol大包装帐面数量)
                    .TextMatrix(.rows - 1, mconintCol小包装帐面数量) = GetFormat((Nvl(rsDetail!帐面数量, 0) Mod rsDetail!比例系数大) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol小包装实盘数量) = .TextMatrix(.rows - 1, mconintCol小包装帐面数量)
                    .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol合计) = .TextMatrix(.rows - 1, mconintCol实盘数量) & .TextMatrix(.rows - 1, mconIntCol实盘数量单位小)
                    .TextMatrix(.rows - 1, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) * Val(.TextMatrix(.rows - 1, mconIntCol售价)), mintMoneyDigit)
                    .TextMatrix(.rows - 1, mconIntCol可用数量) = Nvl(rsDetail!可用数量, 0) / rsDetail!比例系数小
                    .TextMatrix(.rows - 1, mconintCol成本价) = GetFormat(Nvl(rsDetail!成本价, 0) * rsDetail!比例系数小, mintCostDigit0)
                End If
                ...
                .TextMatrix(.rows - 1, mconIntCol批次) = Nvl(rsDetail!批次, 0)
...
'                    .TextMatrix(.Rows - 1, mconIntCol效期) = "2006-06-01"
                ...
                If Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) = 0 Then
                ...
                dbl盘点金额 = GetFormat(Val(.TextMatrix(.rows - 1, mconIntCol售价)) * Val(.TextMatrix(.rows - 1, mconintCol实盘数量)), mintMoneyDigit) '当前售价*实盘数量 = 盘点金额
                .TextMatrix(.rows - 1, mconintCol金额差) = GetFormat(dbl盘点金额 - Val(.TextMatrix(.rows - 1, mconIntCol实际金额)), intMoneyBit)
                .TextMatrix(.rows - 1, mconintCol差价差) = GetFormat((Val(.TextMatrix(.rows - 1, mconIntCol售价)) - Val(.TextMatrix(.rows - 1, mconintCol成本价))) * Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) - Val(.TextMatrix(.rows - 1, mconIntCol实际差价)), intMoneyBit)
                ...
                If mbln盘停用药品 = True Then
                    ...
                    End If
                End If
                '.TextMatrix(.rows - 1, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol成本价)) * Val(.TextMatrix(.rows - 1, mconintCol实盘数量)), mintMoneyDigit)
                '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                .TextMatrix(.rows - 1, mconintCol盘点成本金额) = GetFormat((Nvl(rsDetail!实际金额, 0) + Val(.TextMatrix(.rows - 1, mconintCol金额差))) - (Nvl(rsDetail!实际差价, 0) + Val(.TextMatrix(.rows - 1, mconintCol差价差))), mintMoneyDigit)
                .TextMatrix(.rows - 1, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol金额差)) - Val(.TextMatrix(.rows - 1, mconintCol差价差)), intMoneyBit)
                ...
                rsDetail.MoveNext
            Loop
            Call ShowPercent(rsPhysic.AbsolutePosition / rsPhysic.RecordCount)
            rsPhysic.MoveNext
        Loop
            ...
            If .TextMatrix(lngRow, 0) <> "" Then
                ...
                If Not rsDetail.EOF Then
                    ...
                    If mintUnit = 0 Then
                        ...
                        .TextMatrix(lngRow, mconintCol小包装实盘数量) = zlStr.FormatEx((dbl实盘数量 Mod Val(.TextMatrix(lngRow, mconIntCol比例系数大))) / Val(.TextMatrix(lngRow, mconIntCol比例系数小)), mintNumberDigit0, , True)
                        
                        .TextMatrix(lngRow, mconintCol合计) = zlStr.FormatEx(Int(dbl实盘数量), mintNumberDigit, , True) & .TextMatrix(lngRow, mconIntCol帐面数量单位小)
                    End If
                    
                    .TextMatrix(lngRow, mconintCol实盘数量) = zlStr.FormatEx(Int(dbl实盘数量), mintNumberDigit, , True)
                    ...
                    '金额差=当前售价*实盘数量-实际金额
                    '差价差=金额差*iif(实际金额=0,指导差价率,(实际差价/实际金额))
                    dbl盘点金额 = GetFormat(Val(.TextMatrix(lngRow, mconIntCol售价)) * dbl实盘数量, mintMoneyDigit) '当前售价*实盘数量 = 盘点金额
...
Private Sub CheckDataUpdate()
    ...
    If mint编辑状态 = 3 Then
        With vsfBill
            If .rows > 1 Then
                Call zlCommFun.ShowFlash("正在药品变动,请稍候 ...", Me)
                    ...
                    If Val(.TextMatrix(intRow, 0)) <> 0 Then
                        ...
                        If rsTemp.RecordCount > 0 Then
                            ...
                            If dbl原账面数量 <> dbl现账面数量 Then
                                ...
                                .TextMatrix(intRow, mconIntCol实际金额) = Nvl(rsTemp!实际金额, 0)
                                .TextMatrix(intRow, mconIntCol实际差价) = Nvl(rsTemp!实际差价, 0)
                                If mintUnit > 0 Then
                                    .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(Nvl(rsTemp!账面数量, 0) / Val(.TextMatrix(intRow, mconIntCol比例系数)), mintNumberDigit)
                                Else
                                    .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(Nvl(rsTemp!账面数量, 0) / Val(.TextMatrix(intRow, mconIntCol比例系数小)), mintNumberDigit0)
                                    
                                    .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(Int(Nvl(rsTemp!账面数量, 0) / Val(.TextMatrix(intRow, mconIntCol比例系数大))), mintNumberDigit0)
                                    .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat((Nvl(rsTemp!账面数量, 0) Mod Val(.TextMatrix(intRow, mconIntCol比例系数大))) / Val(.TextMatrix(intRow, mconIntCol比例系数小)), mintNumberDigit0)
                                    .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(Nvl(rsTemp!可用数量, 0) / Val(.TextMatrix(intRow, mconIntCol比例系数小)), mintNumberDigit0)
                                End If

                                If Val(.TextMatrix(intRow, mconintCol实盘数量)) = 0 Then
                                    intMoneyBit = mintMaxMoneyBit
                                Else
                                    intMoneyBit = mintMoneyDigit
                                End If
                                
                                .TextMatrix(intRow, mconintCol数量差) = GetFormat(Abs(Val(.TextMatrix(intRow, mconintCol实盘数量)) - Val(.TextMatrix(intRow, mconintCol帐面数量))), mintNumberDigit)
                                ...
                                '金额差=当前售价*实盘数量-实际金额
                                '差价差=金额差*iif(实际金额=0,指导差价率,(实际差价/实际金额))
                                dbl盘点金额 = GetFormat(Val(.TextMatrix(intRow, mconIntCol售价)) * Val(.TextMatrix(intRow, mconintCol实盘数量)), mintMoneyDigit) '当前售价*实盘数量 = 盘点金额
                                .TextMatrix(intRow, mconintCol金额差) = GetFormat(dbl盘点金额 - Val(.TextMatrix(intRow, mconIntCol实际金额)), intMoneyBit)
                                .TextMatrix(intRow, mconintCol差价差) = GetFormat((Val(.TextMatrix(intRow, mconIntCol售价)) - Val(.TextMatrix(intRow, mconintCol成本价))) * Val(.TextMatrix(intRow, mconintCol实盘数量)) - Val(.TextMatrix(intRow, mconIntCol实际差价)), intMoneyBit)
                                ...
                                If .TextMatrix(intRow, mconintCol标志) = "亏" Then
                                    .TextMatrix(intRow, mconintCol金额差) = GetFormat(-1 * Val(.TextMatrix(intRow, mconintCol金额差)), intMoneyBit)
                                    .TextMatrix(intRow, mconintCol差价差) = GetFormat(-1 * Val(.TextMatrix(intRow, mconintCol差价差)), intMoneyBit)
                                End If
                            
                                '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                                .TextMatrix(intRow, mconintCol盘点成本金额) = GetFormat((Nvl(rsTemp!实际金额, 0) + dbl金额差) - (Nvl(rsTemp!实际差价, 0) + dbl差价差), mintMoneyDigit)
                                .TextMatrix(intRow, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(intRow, mconintCol金额差)) - Val(.TextMatrix(intRow, mconintCol差价差)), intMoneyBit)
...
Private Sub Form_Activate()
    ...
    ElseIf mint编辑状态 = 6 Then
        '全部盘为零
        str盘点时间 = Format(zlDataBase.Currentdate, "yyyy-MM-dd HH:mm:ss")
...
'    On Error Resume Next
    ...
    Call zlCommFun.ShowFlash("正在计算药品库存数据,请稍候 ...", Me)
    DoEvents
    ...
    Call zlCommFun.ShowFlash("正在装入药品数据,请稍候 ...", Me)
    DoEvents
        ...
        Do While Not rsPhysic.EOF
            ...
            Do While Not rsDetail.EOF
                ...
                If rsDetail!是否变价 = 1 Then
                    If Nvl(rsDetail!批次, 0) > 0 Then
                        gstrSQL = "select Nvl(零售价,0) As 零售价 " _
                            & " from 药品库存 " _
                            & " where 性质=1 and 库房id=[1] and 药品id=[2] and nvl(批次,0)=[3]"
                        Set rs时价分批 = zlDataBase.OpenSQLRecord(gstrSQL, mstrCaption, lng库房ID, CLng(rsPhysic!药品ID), CLng(rsDetail!批次))
                        If Not rs时价分批.EOF Then
                            If rs时价分批!零售价 > 0 Then
                                dbl零售价 = rs时价分批!零售价
                            Else
                                '时价分批零售价小于零说明是错误的，则只能计算平均价
                                If Nvl(rsDetail!帐面数量, 0) <> 0 Then
                                    dbl零售价 = rsDetail!实际金额 / rsDetail!帐面数量
                                Else
                                    dbl加成率 = Nvl(rsDetail!加成率, 0) / 100
                        ...
                        End If
                    Else
                        '时价不分批只能求平均价，如果数量也没有那么只能通过差价率计算
                        If Nvl(rsDetail!帐面数量, 0) <> 0 Then
                            dbl零售价 = rsDetail!实际金额 / rsDetail!帐面数量
                        Else
                            dbl加成率 = Nvl(rsDetail!加成率, 0) / 100
                ...
                If mintUnit > 0 Then
                    ...
                    .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(.TextMatrix(.rows - 1, mconintCol帐面数量), mintNumberDigit)
                    .TextMatrix(.rows - 1, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) * Val(.TextMatrix(.rows - 1, mconIntCol售价)), mintMoneyDigit)
                    .TextMatrix(.rows - 1, mconIntCol可用数量) = GetFormat(Nvl(rsDetail!可用数量, 0) / rsDetail!比例系数, mintNumberDigit)
                    .TextMatrix(.rows - 1, mconintCol成本价) = GetFormat(Nvl(rsDetail!成本价, 0) * rsDetail!比例系数, mintCostDigit)
                Else
                    .TextMatrix(.rows - 1, mconIntCol售价) = GetFormat(dbl零售价 * rsDetail!比例系数小, mintPriceDigit0)
                    .TextMatrix(.rows - 1, mconintCol帐面数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                    ...
                    .TextMatrix(.rows - 1, mconIntCol实盘数量单位大) = rsDetail!大包装单位
                    .TextMatrix(.rows - 1, mconIntCol实盘数量单位小) = rsDetail!小包装单位
                    .TextMatrix(.rows - 1, mconintCol大包装帐面数量) = GetFormat(Int(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数大), mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol大包装实盘数量) = .TextMatrix(.rows - 1, mconintCol大包装帐面数量)
                    .TextMatrix(.rows - 1, mconintCol小包装帐面数量) = GetFormat((Nvl(rsDetail!帐面数量, 0) Mod rsDetail!比例系数大) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol小包装实盘数量) = .TextMatrix(.rows - 1, mconintCol小包装帐面数量)
                    .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol合计) = .TextMatrix(.rows - 1, mconintCol实盘数量) & .TextMatrix(.rows - 1, mconIntCol实盘数量单位小)
                    .TextMatrix(.rows - 1, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) * Val(.TextMatrix(.rows - 1, mconIntCol售价)), mintMoneyDigit)
                    .TextMatrix(.rows - 1, mconIntCol可用数量) = GetFormat(Nvl(rsDetail!可用数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol成本价) = GetFormat(Nvl(rsDetail!成本价, 0) * rsDetail!比例系数小, mintCostDigit0)
                End If
...
'    On Error Resume Next
    ...
    With vsfBill
        .Redraw = flexRDNone
        Do While Not rsPhysic.EOF
            Set rsDetail = GetPhysicDetail(lng库房ID, rsPhysic!药品ID, True, IIf(mint编辑状态 = 5, True, False))
            Do While Not rsDetail.EOF
                If (rsPhysic.AbsolutePosition > 1 Or rsDetail.AbsolutePosition > 1) And .TextMatrix(.rows - 1, 0) <> "" Then .rows = .rows + 1
                dbl成本价 = Nvl(rsDetail!平均成本价, 0)
                ...
                If rsDetail!是否变价 = 1 Then
                    gstrSQL = "select Nvl(零售价,0) As 零售价,nvl(实际金额,0)/nvl(实际数量,0) as 售价 " _
                            & " from 药品库存 " _
                            & " where 性质=1 and 库房id=[1] and 药品id=[2] and nvl(批次,0)=[3] and 实际数量>0"
                    Set rs时价分批 = zlDataBase.OpenSQLRecord(gstrSQL, mstrCaption, lng库房ID, CLng(rsPhysic!药品ID), CLng(rsDetail!批次))
...
'                If mint编辑状态 <> 5 Then
'                    .TextMatrix(.rows - 1, mconintCol数量差) = FormatEx(rsDetail!数量差, mintNumberDigit)
'                End If
                If mint编辑状态 = 5 Then
                    If mintUnit > 0 Then
                        .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(Nvl(rsDetail!盘点数量, 0) / rsDetail!比例系数, mintNumberDigit)
                    Else
                        .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(Nvl(rsDetail!盘点数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                ...
                Else
                    ...
                    mintNumberDigit = 5
                    mintNumberDigit0 = 5
                    .TextMatrix(.rows - 1, mconintCol实盘数量) = GetFormat(0, mintNumberDigit)
                End If
                ...
                Else
                    ...
                    .TextMatrix(.rows - 1, mconIntCol实盘数量单位小) = rsDetail!小包装单位
                    .TextMatrix(.rows - 1, mconintCol大包装帐面数量) = GetFormat(Int(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数大), mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol大包装实盘数量) = GetFormat(Int(Nvl(rsDetail!盘点数量, 0) / rsDetail!比例系数大), mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol小包装帐面数量) = GetFormat((Nvl(rsDetail!帐面数量, 0) Mod rsDetail!比例系数大) / rsDetail!比例系数小, mintNumberDigit0)
                    .TextMatrix(.rows - 1, mconintCol小包装实盘数量) = GetFormat((Nvl(rsDetail!盘点数量, 0) Mod rsDetail!比例系数大) / rsDetail!比例系数小, mintNumberDigit0)
                End If
                ...
                .TextMatrix(.rows - 1, mconintCol数量差) = GetFormat(Abs(Val(.TextMatrix(.rows - 1, mconintCol实盘数量)) - Val(.TextMatrix(.rows - 1, mconintCol帐面数量))), mintNumberDigit)
                .TextMatrix(.rows - 1, mconintCol库存数量) = Nvl(rsDetail!帐面数量, 0)
                ...
                If .TextMatrix(.rows - 1, mconintCol标志) = "亏" Then
                    .TextMatrix(.rows - 1, mconintCol金额差) = GetFormat(-1 * Val(.TextMatrix(.rows - 1, mconintCol金额差)), intMoneyBit)
                    .TextMatrix(.rows - 1, mconintCol差价差) = GetFormat(-1 * Val(.TextMatrix(.rows - 1, mconintCol差价差)), intMoneyBit)
                End If
                
                '.TextMatrix(.rows - 1, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol成本价)) * Val(.TextMatrix(.rows - 1, mconintCol实盘数量)), mintMoneyDigit)
                '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                .TextMatrix(.rows - 1, mconintCol盘点成本金额) = GetFormat((Nvl(rsDetail!实际金额, 0) + dbl金额差) - (Nvl(rsDetail!实际差价, 0) + dbl差价差), mintMoneyDigit)
                .TextMatrix(.rows - 1, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(.rows - 1, mconintCol金额差)) - Val(.TextMatrix(.rows - 1, mconintCol差价差)), intMoneyBit)
                '盘亏盘盈行用颜色区分
                Call SetStocktakingColor(vsfBill, .rows - 1)
...
'功能:在状态条上根据百分比显示当前处理进度(█)
    ...
    intAll = staThis.Panels(2).Width / TextWidth("█") - 4
    staThis.Panels(2).Text = Format(sngPercent, "0% ") & String(intAll * sngPercent, "█")
End Sub
...
'            End If
        ...
        Case 2, 3, 4
            ...
            gstrSQL = "Select *" _
                    ...
                    & "             a.摘要,填制人,填制日期,审核人,审核日期,a.频次 as 盘点时间,a.成本价 as 库存金额,a.成本金额 as 库存差价,b.指导差价率,I.是否变价,b.药房分批 as 药房分批核算,A.填写数量,A.批准文号,Nvl(A.发药方式,0) As 新批次, " _
                    & " Nvl(I.撤档时间, To_Date('3000-01-01', 'YYYY-MM-DD')) As 撤档时间,NVL(B.最大效期,0) 最大效期 " _
            ...
            With vsfBill
                ...
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol指导差价率) = GetFormat(rsInitCard!指导差价率, mintMoneyDigit) & "||" & rsInitCard!是否变价 & "||" & rsInitCard!药房分批核算
                    ...
                    End If
                    .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(rsInitCard!帐面数量, intNumberDigit)
                    .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(rsInitCard!实盘数量, intNumberDigit)
                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!售价, mintPriceDigit)
                    .TextMatrix(intRow, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(intRow, mconintCol实盘数量)) * Val(.TextMatrix(intRow, mconIntCol售价)), mintMoneyDigit)
                    
                    If mintUnit > 0 Then
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0), mintCostDigit)
                    Else
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0), mintCostDigit0)
                    End If
                    
                    If mintUnit > 0 Then
                        .TextMatrix(intRow, mconIntCol单位) = rsInitCard!单位
                        .TextMatrix(intRow, mconIntCol比例系数) = rsInitCard!比例系数
                        .TextMatrix(intRow, mconintCol数量差) = GetFormat(rsInitCard!数量差, intNumberDigit)
                    Else
                        Select Case mint大单位
'                            Case mconint售价单位
...
'                                .TextMatrix(intRow, mconintCol大包装帐面数量) = FormatEx(rsInitCard!帐面数量, mintNumberDigit)
'                                .TextMatrix(intRow, mconintCol大包装实盘数量) = FormatEx(rsInitCard!实盘数量, mintNumberDigit)
                            Case mconint门诊单位
                                ...
                                .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(Int(rsInitCard!帐面数量 / rsInitCard!比例系数门诊), intNumberDigit0)
                                .TextMatrix(intRow, mconintCol大包装实盘数量) = GetFormat(Int(rsInitCard!实盘数量 / rsInitCard!比例系数门诊), intNumberDigit0)
                            Case mconint住院单位
                                ...
                                .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(Int(rsInitCard!帐面数量 / rsInitCard!比例系数住院), intNumberDigit0)
                                .TextMatrix(intRow, mconintCol大包装实盘数量) = GetFormat(Int(rsInitCard!实盘数量 / rsInitCard!比例系数住院), intNumberDigit0)
                            Case mconint药库单位
                                ...
                                .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(Int(rsInitCard!帐面数量 / rsInitCard!比例系数药库), intNumberDigit0)
                                .TextMatrix(intRow, mconintCol大包装实盘数量) = GetFormat(Int(rsInitCard!实盘数量 / rsInitCard!比例系数药库), intNumberDigit0)
                        End Select
                        ...
                        Select Case mint小单位
                            Case mconint售价单位
                                ...
                                .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat(rsInitCard!帐面数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大)), intNumberDigit0)
                                .TextMatrix(intRow, mconintCol小包装实盘数量) = GetFormat(rsInitCard!实盘数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大)), intNumberDigit0)
                                .TextMatrix(intRow, mconintCol数量差) = GetFormat(rsInitCard!数量差, intNumberDigit)
                                .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!售价 * rsInitCard!比例系数售价, mintPriceDigit0)
                                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0) * rsInitCard!比例系数售价, mintCostDigit0)
                                .TextMatrix(intRow, mconintCol合计) = .TextMatrix(intRow, mconintCol实盘数量) & rsInitCard!售价单位
                            Case mconint门诊单位
                                .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(rsInitCard!帐面数量 / rsInitCard!比例系数门诊, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(rsInitCard!实盘数量 / rsInitCard!比例系数门诊, intNumberDigit0)
                                ...
                                .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat((rsInitCard!帐面数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大))) / rsInitCard!比例系数门诊, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol小包装实盘数量) = GetFormat((rsInitCard!实盘数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大))) / rsInitCard!比例系数门诊, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol数量差) = GetFormat(rsInitCard!数量差 / rsInitCard!比例系数门诊, intNumberDigit0)
                                .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!售价 * rsInitCard!比例系数门诊, mintPriceDigit0)
                                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0) * rsInitCard!比例系数门诊, mintCostDigit0)
                            Case mconint住院单位
                                .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(rsInitCard!帐面数量 / rsInitCard!比例系数住院, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(rsInitCard!实盘数量 / rsInitCard!比例系数住院, intNumberDigit0)
                                ...
                                .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat((rsInitCard!帐面数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大))) / rsInitCard!比例系数住院, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol小包装实盘数量) = GetFormat((rsInitCard!实盘数量 Mod Val(.TextMatrix(intRow, mconIntCol比例系数大))) / rsInitCard!比例系数住院, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol数量差) = GetFormat(rsInitCard!数量差 / rsInitCard!比例系数住院, intNumberDigit0)
                                .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!售价 * rsInitCard!比例系数住院, mintPriceDigit0)
                                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsInitCard!成本价, 0) * rsInitCard!比例系数住院, mintCostDigit0)
                            Case mconint药库单位
                                .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(rsInitCard!帐面数量 / rsInitCard!比例系数药库, intNumberDigit0)
                                .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(rsInitCard!实盘数量 / rsInitCard!比例系数药库, intNumberDigit0)
...
'                                .TextMatrix(intRow, mconIntCol比例系数大) = rsInitCard!比例系数药库
'                                .TextMatrix(intRow, mconintCol大包装帐面数量) = FormatEx(Int(rsInitCard!帐面数量 / rsInitCard!比例系数药库), mintNumberDigit)
'                                .TextMatrix(intRow, mconintCol大包装实盘数量) = FormatEx(Int(rsInitCard!实盘数量 / rsInitCard!比例系数药库), mintNumberDigit)
                        End Select
                    End If
                    ...
                    Else
                        intMoneyBit = mintMoneyDigit
                    End If
                    .TextMatrix(intRow, mconIntCol实际差价) = GetFormat(Nvl(rsInitCard!库存差价, 0), intMoneyBit)
                    .TextMatrix(intRow, mconIntCol实际金额) = GetFormat(Nvl(rsInitCard!库存金额, 0), intMoneyBit)
                    .TextMatrix(intRow, mconintCol金额差) = GetFormat(Nvl(rsInitCard!金额差, 0), intMoneyBit)
                    .TextMatrix(intRow, mconintCol差价差) = GetFormat(Nvl(rsInitCard!差价差, 0), intMoneyBit)
                    '保持与主界面金额差和差价差算法一致
                    ...
                    '.TextMatrix(intRow, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(intRow, mconintCol成本价)) * Val(.TextMatrix(intRow, mconintCol实盘数量)), mintMoneyDigit)
                    '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                    .TextMatrix(intRow, mconintCol盘点成本金额) = GetFormat((Nvl(rsInitCard!库存金额, 0) + dbl金额差) - (Nvl(rsInitCard!库存差价, 0) + dbl差价差), mintMoneyDigit)
                    .TextMatrix(intRow, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(intRow, mconintCol金额差)) - Val(.TextMatrix(intRow, mconintCol差价差)), intMoneyBit)
                    
                    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                    .TextMatrix(intRow, mconintCol库存数量) = Nvl(rsInitCard!填写数量, 0)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If MsgBox("数据可能已改变，但未存盘，真要退出吗？", vbQuestion Or vbYesNo Or vbDefaultButton2, gstrSysName) = vbNo Then
        ...
        vsfBill.Row = 1
        vsfBill.Col = mconIntCol药名
        If txtCheckDate.Caption = "" Then txtCheckDate.Caption = Format(zlDataBase.Currentdate, "yyyy-MM-dd HH:mm:ss")
        Cancel = 1
        Exit Sub
    Else
        SaveWinState Me, App.ProductName, mstrCaption
    ...
    With vsfBill
        ...
        Case mconIntCol药名
            If mblnNotTrigger <> True Then
                ...
                Set RecReturn = frmSelector.ShowME(Me, 0, 2, , , , txtStock.Tag, txtStock.Tag, , 0, False, True, True, mbln盘停用药品)
            ...
            End If
        Case mconIntCol产地
            vRect = GetControlRect(vsfBill.hWnd)
...
Private Function CheckData(ByVal rsTemp As ADODB.Recordset) As ADODB.Recordset
    ...
    Do While Not rsTemp.EOF
        ...
        gstrSQL = "select 现价 from 收费价目 where 执行日期(+)<=[1] AND NVL(终止日期(+),SYSDATE)>=[1] and 收费细目id=[2]"
...
'                    Set RecReturn = Frm药品多选选择器.ShowME(Me, 2, txtStock.Tag, txtStock.Tag, , strkey, sngLeft, sngTop, False, True, True, True, True, 0, mblnNoStock, 0, mbln盘停用药品, mbln忽略服务对象)
                    ...
                    Set RecReturn = frmSelector.ShowME(Me, 1, 2, strkey, sngLeft, sngTop, txtStock.Tag, txtStock.Tag, , 0, False, True, True, mbln盘停用药品)
                    If RecReturn.RecordCount > 0 Then
                        Set RecReturn = CheckData(RecReturn)  '检查重复记录 并将重复记录的药品id返回回来
                    End If
            ...
            Case mconintCol实盘数量, mconintCol大包装实盘数量, mconintCol小包装实盘数量
                ...
                Select Case .Col
                    ...
                    Case mconintCol大包装实盘数量
                        intDigit = mintPriceDigit1
                    Case mconintCol小包装实盘数量
                        intDigit = mintPriceDigit0
                End Select
...
Private Function SaveCard() As Boolean
    ...
    With vsfBill
        chrNo = Trim(txtNo)
        lng库房ID = txtStock.Tag
        If chrNo = "" Then chrNo = zlDataBase.GetNextNo(29, lng库房ID)
        If IsNull(chrNo) Then Exit Function
        txtNo.Tag = chrNo
        
        str摘要 = Trim(txt摘要.Text)
        str填制人 = Txt填制人
        dat填制日期 = Format(zlDataBase.Currentdate, "yyyy-mm-dd hh:mm:ss")
        str盘点时间 = txtCheckDate.Caption
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                dbl实盘数量 = GetFormat(.TextMatrix(intRow, mconintCol实盘数量) * dbl比例系数, gtype_UserDrugDigits.Digit_数量)
                ...
                Else
                    dbl数量差 = GetFormat(Abs(.TextMatrix(intRow, mconintCol实盘数量) * dbl比例系数 - Val(.TextMatrix(intRow, mconintCol库存数量))), gtype_UserDrugDigits.Digit_数量)
                End If
                
                str批准文号 = IIf(Trim(.TextMatrix(intRow, mconIntCol批准文号)) = "", "", .TextMatrix(intRow, mconIntCol批准文号))
                
                dbl售价 = FormatEx(.TextMatrix(intRow, mconIntCol售价) / dbl比例系数, gtype_UserDrugDigits.Digit_零售价)
                dbl成本价 = FormatEx(.TextMatrix(intRow, mconintCol成本价) / dbl比例系数, gtype_UserDrugDigits.Digit_成本价)
                
                If Val(Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1)) = 0 Or int新批次 = 0 Then
                    '定价药品或不是新增批次取原始售价
                    dbl售价 = Get售价(Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1) = 1, lng药品ID, lng库房ID, lng批次ID)
                Else
                    '新增批次时价按界面价格换算后保存
                    dbl售价 = FormatEx(.TextMatrix(intRow, mconIntCol售价) / dbl比例系数, gtype_UserDrugDigits.Digit_零售价)
                End If
                ...
                If int新批次 = 0 Then
                    '不是新增批次取原始成本价
                    dbl成本价 = Get成本价(lng药品ID, lng库房ID, lng批次ID)
                Else
                    dbl成本价 = FormatEx(.TextMatrix(intRow, mconintCol成本价) / dbl比例系数, gtype_UserDrugDigits.Digit_成本价)
                End If
                ...
                dbl金额差 = GetFormat(Val(.TextMatrix(intRow, mconintCol金额差)), intMoneyBit)
                dbl差价差 = GetFormat(Val(.TextMatrix(intRow, mconintCol差价差)), intMoneyBit)
                dbl库存金额 = GetFormat(Val(.TextMatrix(intRow, mconIntCol实际金额)), intMoneyBit)
                dbl库存差价 = GetFormat(Val(.TextMatrix(intRow, mconIntCol实际差价)), intMoneyBit)
...
Private Sub 显示合计金额()
    ...
    lblPurchasePrice.Caption = "金额差合计：" & GetFormat(dbl金额差, mintMoneyDigit)
    ...
    lblCheckSum.Caption = "盘点金额合计：" & GetFormat(dbl盘点金额, mintMoneyDigit)
    ...
    lblCostPrice.Caption = "盘点成本金额合计：" & GetFormat(dbl成本金额, mintMoneyDigit)
    ...
    With vsfBill
        ...
        Else
            ...
            Else
                ...
                dbl小包装数量 = (rsUseCount!可用数量 Mod (Val(.TextMatrix(.Row, mconIntCol比例系数大)))) / Val(.TextMatrix(.Row, mconIntCol比例系数小))
                dbl小包装实际数量 = (rsUseCount!实际数量 Mod (Val(.TextMatrix(.Row, mconIntCol比例系数大)))) / Val(.TextMatrix(.Row, mconIntCol比例系数小))
        ...
        If mintUnit > 0 Then
            staThis.Panels(2).Text = "该药品当前库存数为[" & GetFormat(dbl大包装实际数量, mintNumberDigit) & "]" & .TextMatrix(.Row, mconIntCol单位)
        Else
            staThis.Panels(2).Text = "该药品当前库存数为[" & GetFormat(dbl大包装实际数量, mintNumberDigit) & "]" & .TextMatrix(.Row, mconIntCol帐面数量单位大) & _
                ",[" & GetFormat(dbl小包装实际数量, mintNumberDigit0) & "]" & .TextMatrix(.Row, mconIntCol帐面数量单位小)
        End If
    End With
    Exit Sub
errHandle:
...
'        For intRow = 1 To .rows - 1
'            If .TextMatrix(intRow, 0) <> "" Then
'                If intRow <> .Row And Val(.TextMatrix(intRow, 0)) = lngID And IIf(.TextMatrix(intRow, mconIntCol批次) = "", "0", .TextMatrix(intRow, mconIntCol批次)) = lng批次 Then
...
'        If IsNull(rsDetail!售价) Then
'            mstrMsg = "对不起，该药品在本次盘点时间时无售价信息，请检查!"
'            Exit Function
'        End If
...
'            If Nvl(rsDetail!帐面数量, 0) > 0 Then
                If rsDetail!批次 > 0 Then
                    gstrSQL = "select Nvl(零售价,0) As 零售价 " _
                            & " from 药品库存 " _
                            & " where 性质=1 and 库房id=[1] and 药品id=[2] and nvl(批次,0)=[3]"
                    Set rs时价分批 = zlDataBase.OpenSQLRecord(gstrSQL, mstrCaption, Val(txtStock.Tag), lngId, CLng(rsDetail!批次))
                    If Not rs时价分批.EOF Then
                        If rs时价分批!零售价 > 0 Then
                            dbl零售价 = rs时价分批!零售价
                        Else
...
'                dbl加成率 = 1 / (1 - rsDetail!指导差价率 / 100) - 1
'                dbl零售价 = FormatEx(时价药品零售价(rsDetail!药品ID, dbl加成率, dbl成本价), mintPriceDigit)
'            End If
        End If
        ...
        .TextMatrix(intRow, mconIntCol来源) = Nvl(rsDetail!药品来源)
        .TextMatrix(intRow, mconIntCol基本药物) = Nvl(rsDetail!基本药物)
        ...
        If mintUnit > 0 Then
            '按常量定义进行格式化
            .TextMatrix(intRow, mconIntCol售价) = GetFormat(dbl零售价 * rsDetail!比例系数, mintPriceDigit)
            
            .TextMatrix(intRow, mconIntCol单位) = IIf(IsNull(rsDetail!单位), "", rsDetail!单位)
            .TextMatrix(intRow, mconIntCol比例系数) = rsDetail!比例系数
            If rsDetail!是否变价 = 1 Then
                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Get成本价(rsDetail!药品ID, Val(txtStock.Tag), CLng(rsDetail!批次)) * dbl比例系数, mintCostDigit)
            Else
                .TextMatrix(intRow, mconintCol成本价) = GetFormat(dbl初始成本价 * dbl比例系数, mintCostDigit)
            End If
        Else
            '按常量定义进行格式化
            .TextMatrix(intRow, mconIntCol售价) = GetFormat(dbl零售价 * rsDetail!比例系数小, mintPriceDigit0)
            ...
            .TextMatrix(intRow, mconIntCol比例系数大) = Nvl(rsDetail!比例系数大, 0)
            .TextMatrix(intRow, mconIntCol比例系数小) = Nvl(rsDetail!比例系数小, 0)
            
            If rsDetail!是否变价 = 1 Then
                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Get成本价(rsDetail!药品ID, Val(txtStock.Tag), CLng(rsDetail!批次)) * rsDetail!比例系数小, mintCostDigit0)
            Else
                .TextMatrix(intRow, mconintCol成本价) = GetFormat(dbl初始成本价 * rsDetail!比例系数小, mintCostDigit0)
            End If
        End If
        ...
        If lng批次 = -1 Or bln盘点入库 = True Then
            ...
            .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(0, mintNumberDigit)
            .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol帐面数量), mintNumberDigit)
            
            If mintUnit = 0 Then
                .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(0, mintNumberDigit0)
                .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat(0, mintNumberDigit0)
                .TextMatrix(intRow, mconintCol大包装实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol大包装帐面数量), mintNumberDigit0)
                .TextMatrix(intRow, mconintCol小包装实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol小包装帐面数量), mintNumberDigit0)
            End If
            
            .TextMatrix(intRow, mconintCol盘点金额) = GetFormat(0, mintMoneyDigit)
            .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(0, mintNumberDigit)
            .TextMatrix(intRow, mconIntCol实际金额) = GetFormat(0, mintNumberDigit)
            .TextMatrix(intRow, mconintCol库存数量) = GetFormat(0, mintNumberDigit)
            .TextMatrix(intRow, mconIntCol实际差价) = GetFormat(0, mintMoneyDigit)
            If mintUnit <= 0 Then
        ...
        Else
            ...
            If mintUnit > 0 Then
                .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数, mintNumberDigit)
                .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol帐面数量), mintNumberDigit)
                .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(Nvl(rsDetail!可用数量, 0), mintNumberDigit)
                
                .TextMatrix(intRow, mconintCol成本价) = GetFormat(Nvl(rsDetail!成本价, 0) * rsDetail!比例系数, mintCostDigit)
            Else
                .TextMatrix(intRow, mconintCol帐面数量) = GetFormat(Nvl(rsDetail!帐面数量, 0) / rsDetail!比例系数小, mintNumberDigit0)
                .TextMatrix(intRow, mconintCol实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol帐面数量), mintNumberDigit0)
                .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(Nvl(rsDetail!可用数量, 0), mintNumberDigit0)
                
                .TextMatrix(intRow, mconintCol大包装帐面数量) = GetFormat(Int(rsDetail!帐面数量 / rsDetail!比例系数大), mintNumberDigit0)
                .TextMatrix(intRow, mconintCol大包装实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol大包装帐面数量), mintNumberDigit0)
                .TextMatrix(intRow, mconintCol小包装帐面数量) = GetFormat((rsDetail!帐面数量 Mod rsDetail!比例系数大) / rsDetail!比例系数小, mintNumberDigit0)
                .TextMatrix(intRow, mconintCol小包装实盘数量) = GetFormat(.TextMatrix(intRow, mconintCol小包装帐面数量), mintNumberDigit0)
                If mintUnit <= 0 Then
                    .TextMatrix(intRow, mconintCol合计) = .TextMatrix(intRow, mconintCol实盘数量) & rsDetail!小包装单位
                End If
            End If
            .TextMatrix(intRow, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(intRow, mconintCol实盘数量)) * Val(.TextMatrix(intRow, mconIntCol售价)), mintMoneyDigit)
            .TextMatrix(intRow, mconIntCol实际金额) = Nvl(rsDetail!实际金额, 0)
            .TextMatrix(intRow, mconintCol库存数量) = Nvl(rsDetail!帐面数量, 0)
            .TextMatrix(intRow, mconIntCol实际差价) = Nvl(rsDetail!实际差价, 0)
        End If
        
        .TextMatrix(intRow, mconIntCol指导差价率) = rsDetail!指导差价率 & "||" & rsDetail!是否变价 & "||" & rsDetail!药房分批核算
        .TextMatrix(intRow, mconintCol标志) = "平"
        ...
        .TextMatrix(intRow, mconintCol数量差) = GetFormat("0", mintNumberDigit)
        ...
        '.TextMatrix(intRow, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(intRow, mconintCol成本价)) * Val(.TextMatrix(intRow, mconintCol实盘数量)), mintMoneyDigit)
        '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
        .TextMatrix(intRow, mconintCol盘点成本金额) = GetFormat((Val(.TextMatrix(intRow, mconIntCol实际金额)) + Val(.TextMatrix(intRow, mconintCol金额差))) - (Val(.TextMatrix(intRow, mconIntCol实际差价)) + Val(.TextMatrix(intRow, mconintCol差价差))), mintMoneyDigit)
        .TextMatrix(intRow, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(intRow, mconintCol金额差)) - Val(.TextMatrix(intRow, mconintCol差价差)), intMoneyBit)
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        If mbln盘停用药品 = True Then
            '如果是停用药品，该行粗体显示
...
Private Function GetPhysic(ByVal lng库房ID As Long, ByVal str盘点属性 As String, _
    ...
    '提取符合盘点条件的所有药品清单
    gstrSQL = "SELECT Distinct A.药品ID,B.编码,B.名称,E.库房货位" & _
...
Private Function GetPhysicDetail(ByVal lng库房ID As Long, ByVal lng药品ID As Long, _
    ...
    '取药品当前库存及盘点时间以后的净发生额
    gstrSQL = "" & _
        " SELECT DISTINCT A.药品ID,A.成本价 As 平均成本价,E.产地 缺省产地,'[' || E.编码 || ']' As 药品编码, E.名称 As 通用名, C.名称 As 商品名,A.药库分批 AS 分批核算,A.药房分批 AS 药房分批核算,E.是否变价,A.指导差价率,A.加成率," & _
        ...
        "        B.批准文号,Nvl(B.帐面数量,0) 帐面数量,B.盘点数量,B.可用数量" & str单位 & ",Decode(b.批次, -1, b.成本价, Decode(sign(NVL(b.帐面数量,0)), 1,Decode(k.成本价, Null, a.成本价, k.成本价), a.成本价)) 成本价, " & _
        "        Nvl(E.撤档时间, To_Date('3000-01-01', 'YYYY-MM-DD')) As 撤档时间,a.最大效期 " & _
        ...
        "             FROM 药品库存 A" & _
        "             Where A.性质 = 1 And A.库房ID=[1] And A.药品ID=[2] " & IIf(bln盘无库存有金额药品 = True, " And A.实际数量=0 And (A.实际金额<>0 Or A.实际差价<>0)", "") & _
        ...
        IIf(bln盘无库存药品, "", " and (b.帐面数量 <> 0 Or b.实际金额 <> 0 Or b.实际差价 <> 0)") & _
...
Private Function 时价药品零售价(ByVal lng药品ID As Long, ByVal dbl加成率 As Double, ByVal dbl采购价 As Double) As Double
    Dim dbl零售价 As Double, dbl指导零售价 As Double, dbl差价让利比 As Double
    Dim rsTemp As New ADODB.Recordset
    ...
    dbl指导零售价 = rsTemp!指导零售价
    dbl差价让利比 = rsTemp!差价让利比
    
    时价药品零售价 = 0
    
    dbl零售价 = dbl采购价 * (1 + dbl加成率)
    dbl零售价 = dbl零售价 + (dbl指导零售价 - dbl零售价) * (1 - dbl差价让利比 / 100)
    时价药品零售价 = IIf(dbl零售价 > dbl指导零售价, dbl指导零售价, dbl零售价)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub vsfBill_SetupEditWindow(ByVal Row As Long, ByVal Col As Long, ByVal EditWindow As Long, ByVal IsCombo As Boolean)
    ...
    vsfBill.EditSelLength = zlCommFun.ActualLen(vsfBill.EditText)
End Sub
    ...
    With vsfBill
        ...
        Select Case Col
            ...
            Case mconintCol实盘数量
                ...
                If strkey <> "" And .TextMatrix(Row, 0) <> "" And Val(strkey) <> Val(.TextMatrix(Row, mconintCol实盘数量)) Then
                    ...
                    .TextMatrix(Row, mconintCol数量差) = GetFormat(Abs(Val(strkey) - Val(.TextMatrix(Row, mconintCol帐面数量))), mintNumberDigit)
                    ...
                    '金额差=当前售价*实盘数量-实际金额
                    '差价差=金额差*iif(实际金额=0,指导差价率,(实际差价/实际金额))
                    dbl盘点金额 = GetFormat(Val(.TextMatrix(Row, mconIntCol售价)) * Val(strkey), mintMoneyDigit) '当前售价*实盘数量 = 盘点金额
                    .TextMatrix(Row, mconintCol金额差) = GetFormat(dbl盘点金额 - Val(.TextMatrix(Row, mconIntCol实际金额)), intMoneyBit)
                    .TextMatrix(Row, mconintCol差价差) = GetFormat((Val(.TextMatrix(Row, mconIntCol售价)) - Val(.TextMatrix(Row, mconintCol成本价))) * Val(strkey) - Val(.TextMatrix(Row, mconIntCol实际差价)), intMoneyBit)
                    dbl金额差 = Val(.TextMatrix(Row, mconintCol金额差))
                    dbl差价差 = Val(.TextMatrix(Row, mconintCol差价差))
                    If .TextMatrix(Row, mconintCol标志) = "亏" Then
                        .TextMatrix(Row, mconintCol金额差) = GetFormat(-1 * Val(.TextMatrix(Row, mconintCol金额差)), intMoneyBit)
                        .TextMatrix(Row, mconintCol差价差) = GetFormat(-1 * Val(.TextMatrix(Row, mconintCol差价差)), intMoneyBit)
                    End If
                    
                    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                    .TextMatrix(Row, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(Row, mconIntCol售价)) * Val(strkey), mintMoneyDigit)
                    .TextMatrix(Row, mconintCol实盘数量) = strkey
                    
                    '.TextMatrix(Row, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(Row, mconintCol成本价)) * Val(.TextMatrix(Row, mconintCol实盘数量)), mintMoneyDigit)
                    '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                    .TextMatrix(Row, mconintCol盘点成本金额) = GetFormat((Val(.TextMatrix(Row, mconIntCol实际金额)) + dbl金额差) - (Val(.TextMatrix(Row, mconIntCol实际差价)) + dbl差价差), mintMoneyDigit)
                    .TextMatrix(Row, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(Row, mconintCol金额差)) - Val(.TextMatrix(Row, mconintCol差价差)), intMoneyBit)
        ...
        Case mconintCol大包装实盘数量, mconintCol小包装实盘数量
            ...
            If strkey <> "" And .TextMatrix(Row, 0) <> "" Then
                strkey = GetFormat(strkey, intNumber)
                .EditText = strkey
                ...
                .TextMatrix(Row, mconintCol实盘数量) = GetFormat(dblSumNum, intNumber)
                .TextMatrix(Row, mconintCol合计) = .TextMatrix(Row, mconintCol实盘数量) & .TextMatrix(Row, mconIntCol实盘数量单位小)
                .TextMatrix(Row, mconintCol数量差) = GetFormat(Abs(Val(.TextMatrix(Row, mconintCol实盘数量)) - Val(.TextMatrix(Row, mconintCol帐面数量))), intNumber)
                ...
                '金额差=当前售价*实盘数量-实际金额
                '差价差=金额差*iif(实际金额=0,指导差价率,(实际差价/实际金额))
                dbl盘点金额 = GetFormat(Val(.TextMatrix(Row, mconIntCol售价)) * dblSumNum, mintMoneyDigit) '当前售价*实盘数量 = 盘点金额
                .TextMatrix(Row, mconintCol金额差) = GetFormat(dbl盘点金额 - Val(.TextMatrix(Row, mconIntCol实际金额)), intMoneyBit)
                .TextMatrix(Row, mconintCol差价差) = GetFormat((Val(.TextMatrix(Row, mconIntCol售价)) - Val(.TextMatrix(Row, mconintCol成本价))) * Val(dblSumNum) - Val(.TextMatrix(Row, mconIntCol实际差价)), intMoneyBit)
                dbl金额差 = Val(.TextMatrix(Row, mconintCol金额差))
                dbl差价差 = Val(.TextMatrix(Row, mconintCol差价差))
                If .TextMatrix(Row, mconintCol标志) = "亏" Then
                    .TextMatrix(Row, mconintCol金额差) = GetFormat(-1 * Val(.TextMatrix(Row, mconintCol金额差)), intMoneyBit)
                ...
                .TextMatrix(Row, mconintCol盘点金额) = GetFormat(Val(.TextMatrix(Row, mconIntCol售价)) * dblSumNum, mintMoneyDigit)
                '.TextMatrix(Row, mconintCol盘点成本金额) = GetFormat(Val(.TextMatrix(Row, mconintCol成本价)) * Val(.TextMatrix(Row, mconintCol实盘数量)), mintMoneyDigit)
                '成本金额=成本价*实盘数量=(账面金额+金额差) -(账面差价+差价差) 用后者是为了控制报表与程序出的盘点单能对账
                .TextMatrix(Row, mconintCol盘点成本金额) = GetFormat((Val(.TextMatrix(Row, mconIntCol实际金额)) + dbl金额差) - (Val(.TextMatrix(Row, mconIntCol实际差价)) + dbl差价差), mintMoneyDigit)
                .TextMatrix(Row, mconintCol盘点成本金额差) = GetFormat(Val(.TextMatrix(Row, mconintCol金额差)) - Val(.TextMatrix(Row, mconintCol差价差)), intMoneyBit)
