...
Private Const mconIntCol数量 As Integer = 19
Private Const mconIntCol冲销数量 As Integer = 20
...
Private Const mconIntCol外调价 As Integer = 25
Private Const mconIntCol外调金额 As Integer = 26
Private Const mconIntCol增值税率 As Integer = 27
Private Const mconIntCol税金 As Integer = 28
Private Const mconintCol差价 As Integer = 29
Private Const mconIntCol药品编码和名称 = 30
Private Const mconIntCol药品编码 = 31
Private Const mconIntCol药品名称 = 32
Private Const mconIntColS  As Integer = 33             '总列数
'=========================================================================================
    ...
    With mshBill
        For n = 1 To .rows - 1
            If Val(.TextMatrix(n, 0)) <> 0 Then
                ...
                Dbl数量 = Val(.TextMatrix(n, mconIntCol数量)) * Val(.TextMatrix(n, mconIntCol比例系数))
                str名称 = .TextMatrix(n, mconIntCol药品名称)
                
                If Not CheckDrugStock(str名称, lng药品ID, lng批次, Dbl数量, n) Then
                    Exit Function
                End If
            End If
        Next
...
'    End With
    ...
    ElseIf KeyAscii >= 32 Then
        If Not cbo外调单位.Locked And cbo外调单位.Style = 2 Then
            lngIdx = zlControl.CboMatchIndex(cbo外调单位.hWnd, KeyAscii)
...
Private Sub cbo外调单位_Validate(Cancel As Boolean)
    ...
    On Error GoTo errh
    vRect = GetControlRect(cbo外调单位.hWnd)
...
'    End With
    ...
    ElseIf KeyAscii >= 32 Then
        If Not cbo外销单位.Locked And cbo外销单位.Style = 2 Then
            lngIdx = zlControl.CboMatchIndex(cbo外销单位.hWnd, KeyAscii)
...
Private Sub cbo外销单位_Validate(Cancel As Boolean)
    ...
    On Error GoTo errh
    vRect = GetControlRect(cbo外销单位.hWnd)
    ...
    With mshBill
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                .TextMatrix(intRow, mconIntCol采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol数量) * .TextMatrix(intRow, mconIntCol采购价), mintMoneyDigit)
...
Private Sub RefreshBill()
    ...
    gstrSQL = " Select '售价' As 类型, a.序号, a.药品id ,nvl(a.批次,0) as 批次, 0 原价, b.现价" & _
            ...
            " Where a.单据 = 11 And a.No = [1] And c.Id = a.药品id And Round(a.零售价," & intPriceDigit & ") <> Round(decode(nvl(b.批次,0),0,b.实际金额 / b.实际数量,b.零售价), " & intPriceDigit & ") And Nvl(c.是否变价, 0) = 1 And" & _
            ...
            " Where a.单据 = 11 And a.No = [1] And a.药品id = b.药品id And Nvl(a.批次, 0) = Nvl(b.批次, 0) and round(a.成本价," & intCostDigit & ")<>round(b.平均成本价," & intCostDigit & ") And a.库房id = b.库房id and a.入出系数=-1 and b.性质=1" & _
            " Order By 类型, 药品id, 序号"
...
Private Sub Form_Load()
    ...
    On Error GoTo errHandle
    mblnEnterCell = False
    mintSelectStock = Val(zlDatabase.GetPara("是否选择库房", glngSys, 模块号.其他出库))
        ...
        Case 2, 3, 4, 6
            ...
            If mint编辑状态 <> 6 Then
                gstrSQL = "SELECT W.*,Z.可用数量,Z.实际金额,Z.实际差价 " & _
                    ...
                    " B.药品来源,B.基本药物,F.规格,F.产地 AS 原产地,A.产地, A.批号,A.批次,B.指导差价率,A.效期," & _
                    strUnitQuantity & _
                    " A.成本金额,A.零售金额, A.差价,A.摘要,填制人,填制日期,审核人,审核日期,A.库房ID,A.入出类别ID,F.是否变价,B.药房分批 AS 药房分批核算," & _
            ...
            Else
                gstrSQL = "SELECT W.*,Z.可用数量,Z.实际金额,Z.实际差价 " & _
                    " FROM " & _
                    " (SELECT DISTINCT A.药品ID,A.序号,'[' || F.编码 || ']' As 药品编码, F.名称 As 通用名, E.名称 As 商品名, " & _
                    " B.药品来源,B.基本药物,F.规格,F.产地 AS 原产地,A.产地, A.批号,A.批次,B.指导差价率,A.效期,G.名称 AS 外调单位,H.名称 AS 外销单位,A.增值税率," & _
                    strUnitQuantity & _
                    " A.成本金额,0 零售金额, 0 差价,A.摘要,A.库房ID,A.入出类别ID,F.是否变价,B.药房分批 AS 药房分批核算,A.批准文号 " & _
                    " FROM " & _
                    "     (SELECT MIN(ID) AS ID, SUM(实际数量) AS 填写数量,SUM(成本金额) AS 成本金额,药品ID,序号,产地, 批号,效期,NVL(批次,0) 批次," & _
                    " 扣率,成本价,零售价,摘要,库房ID,入出类别ID,单量,发药窗口,批准文号, To_Number(Trim(To_Char(Nvl(频次, '0'), '999999999999.0000'))) As 增值税率" & _
                    "     FROM 药品收发记录 X " & _
                    "     WHERE NO=[1] AND 单据=11  " & _
                    "     GROUP BY 药品ID,序号,产地,批号,效期,NVL(批次,0),扣率,成本价,零售价,摘要,库房ID,入出类别ID,单量,发药窗口,批准文号, To_Number(Trim(To_Char(Nvl(频次, '0'), '999999999999.0000'))) " & _
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol外调价) = GetFormat(rsInitCard!外调价, intCostDigit)
                    .TextMatrix(intRow, mconIntCol外调金额) = GetFormat(rsInitCard!外调价 * rsInitCard!数量, intMoneyDigit)
                    ...
                    .TextMatrix(intRow, mconIntCol增值税率) = GetFormat(IIf(IsNull(rsInitCard!增值税率), "0", rsInitCard!增值税率), 2)
                    .TextMatrix(intRow, mconIntCol税金) = GetFormat(rsInitCard!外调价 * rsInitCard!数量 * (Val(.TextMatrix(intRow, mconIntCol增值税率)) / 100 / (1 + Val(.TextMatrix(intRow, mconIntCol增值税率)) / 100)), intMoneyDigit)
                    
                    If mint编辑状态 = 6 Then
                        .TextMatrix(intRow, mconIntCol冲销数量) = GetFormat(0, intNumberDigit)
                        .TextMatrix(intRow, mconIntCol外调金额) = GetFormat(0, intMoneyDigit)
                        .TextMatrix(intRow, mconIntCol税金) = GetFormat(0, intMoneyDigit)
                    End If
    ...
    With mshBill
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                Else
                    .TextMatrix(intRow, mconIntCol采购价) = GetFormat((.TextMatrix(intRow, mconIntCol售价金额) - .TextMatrix(intRow, mconintCol差价)) / (.TextMatrix(intRow, mconIntCol数量)), gtype_UserDrugDigits.Digit_成本价)
                End If
                .TextMatrix(intRow, mconIntCol采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol采购价) * (.TextMatrix(intRow, mconIntCol数量)), mintMoneyDigit)
                    ...
                    If RecReturn.RecordCount > 0 Then
                        ...
                        For i = 1 To RecReturn.RecordCount
                            ...
                            If SetColValue(.Row, RecReturn!药品id, "[" & RecReturn!药品编码 & "]", RecReturn!通用名, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), _
                                    ...
                                    Choose(mintUnit, 1, RecReturn!门诊包装, RecReturn!住院包装, RecReturn!药库包装), _
                                    IIf(IsNull(RecReturn!批次), 0, RecReturn!批次), RecReturn!时价, RecReturn!药房分批, _
                                    IIf(IsNull(RecReturn!批准文号), "", RecReturn!批准文号)) = False Then
                                Cancel = True
                                Exit Sub
                            End If
...
'                        .TextMatrix(.Row, mconIntCol采购价) = FormatEx((.TextMatrix(.Row, mconIntCol售价金额) - .TextMatrix(.Row, mconintCol差价)) / strkey, mintCostDigit)
                    ...
                    .TextMatrix(.Row, mconintCol差价) = GetFormat(Val(Val(.TextMatrix(.Row, mconIntCol售价金额))) - Val(.TextMatrix(.Row, mconIntCol采购金额)), mintMoneyDigit)
                    ...
                    If Val(.TextMatrix(.Row, 0)) <> 0 And cboType.Text = "药品外调" And .TextMatrix(.Row, mconIntCol外调价) = "" Then
                        ...
                        .TextMatrix(.Row, mconIntCol外调价) = GetFormat((1 + rsTemp!比例 / 100) * Val(.TextMatrix(.Row, mconIntCol采购价)), mintCostDigit)
                    End If
                    .TextMatrix(.Row, mconIntCol外调金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(strkey), mintMoneyDigit)
                    
                    '税金=外销金额*增值税率
                    .TextMatrix(.Row, mconIntCol税金) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(strkey) * (Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100 / (1 + Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100)), mintMoneyDigit)
                End If
                显示合计金额
            ...
            Case mconIntCol冲销数量
                ...
                If strkey <> "" Then
                    ...
                    If .TextMatrix(.Row, mconIntCol采购价) <> "" Then
                        .TextMatrix(.Row, mconIntCol采购金额) = GetFormat(.TextMatrix(.Row, mconIntCol采购价) * Val(strkey), mintMoneyDigit)
                    End If
                    If .TextMatrix(.Row, mconIntCol售价) <> "" Then
                        .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(.TextMatrix(.Row, mconIntCol售价) * Val(strkey), mintMoneyDigit)
                    End If
                    If .TextMatrix(.Row, mconIntCol外调价) <> "" Then
                        .TextMatrix(.Row, mconIntCol外调金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(strkey), mintMoneyDigit)
                    End If
                    .TextMatrix(.Row, mconIntCol税金) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(strkey) * (Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100 / (1 + Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100)), mintMoneyDigit)
                    .TextMatrix(.Row, mconintCol差价) = GetFormat(IIf(.TextMatrix(.Row, mconIntCol售价金额) = "", 0, .TextMatrix(.Row, mconIntCol售价金额)) - IIf(.TextMatrix(.Row, mconIntCol采购金额) = "", 0, .TextMatrix(.Row, mconIntCol采购金额)), mintMoneyDigit)
                End If
                显示合计金额
            Case mconIntCol外调价
                ...
                If strkey <> "" Then
                    ...
                    '重算外调金额
                    .TextMatrix(.Row, mconIntCol外调金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(.TextMatrix(.Row, mconIntCol数量)), mintMoneyDigit)
                    
                    '重算税金
                    .TextMatrix(.Row, mconIntCol税金) = GetFormat(Val(.TextMatrix(.Row, mconIntCol外调价)) * Val(.TextMatrix(.Row, mconIntCol数量)) * (Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100 / (1 + Val(.TextMatrix(.Row, mconIntCol增值税率)) / 100)), mintMoneyDigit)
                End If
        End Select
    End With
    Exit Sub
    ...
    With mshBill
        ...
        If cboType.Text = "药品外销" Then
            ...
            If Not rsPrice.EOF Then
                .TextMatrix(intRow, mconIntCol增值税率) = FormatEx(rsPrice!增值税率, 2)
                
                If rsPrice!采购价 > 0 Then
                    .TextMatrix(intRow, mconIntCol外调价) = GetFormat(rsPrice!采购价 * num比例系数, mintPriceDigit)
                Else
                    .TextMatrix(intRow, mconIntCol外调价) = GetFormat(rsPrice!指导批发价 * num比例系数, mintPriceDigit)
                End If
            End If
        End If
    End With
...
'                dblSalePrice = FormatEx(.TextMatrix(intRow, mconIntCol售价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_零售价)
                ...
                If Val(.TextMatrix(intRow, mconIntCol外调价)) = 0 And cboType.Text = "药品外调" Then
                    ...
                    .TextMatrix(intRow, mconIntCol外调价) = FormatEx((1 + rsTemp!比例 / 100) * Val(.TextMatrix(intRow, mconIntCol采购价)), gtype_UserDrugDigits.Digit_成本价)
                    .TextMatrix(intRow, mconIntCol外调金额) = GetFormat(.TextMatrix(intRow, mconIntCol外调价) * Val(.TextMatrix(intRow, mconIntCol数量)), mintMoneyDigit)
                End If
                If cboType.Text = "药品外调" Or cboType.Text = "药品外销" Then
                    dblOutPrice = FormatEx(Val(.TextMatrix(intRow, mconIntCol外调价)) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_成本价)
                End If
...
Private Sub 显示合计金额()
    ...
    lblOther.Caption = "外调(销)合计：" & GetFormat(Cur外调金额, mintMoneyDigit)
End Sub
...
Private Sub 提示库存数()
    ...
    With mshBill
        ...
        staThis.Panels(2).Text = "该药品当前库存数为[" & GetFormat(.TextMatrix(.Row, mconIntCol可用数量), mintNumberDigit) & "]" & .TextMatrix(.Row, mconIntCol单位)
    End With
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
...
Private Sub txtIn_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If Len(txtIn.Text) < 8 Then
        txtIn.Text = GetFullNO(txtIn.Text, intNO, lng库房ID)
    End If
...
'    End If
    ...
    gstrSQL = "SELECT A.药品ID,'['||C.编码||']' As 编码,'['||C.编码||']'|| Nvl(F.名称,C.名称) As 药品名称, C.名称 As 通用名,F.名称 As 商品名,C.规格,C.产地," & _
             ...
             "        B.管理费比例,B.指导差价率,A.实际数量,D.可用数量,D.实际金额,D.实际差价,E.现价,A.批准文号,B.药品来源,B.基本药物,d.平均成本价 " & _
    ...
    With rsBill
        ...
        Do While Not .EOF
            ...
            If Not SetColValue(intRow, !药品id, !编码, !通用名, IIf(IsNull(!商品名), "", !商品名), _
                ...
                Nvl(!指导差价率, 0), Choose(mintUnit, 1, !门诊包装, !住院包装, !药库包装), Nvl(!批次, 0), !时价, _
            ...
            mshBill.TextMatrix(intRow, mconIntCol数量) = GetFormat(Nvl(!实际数量, 0) / Choose(mintUnit, 1, !门诊包装, !住院包装, !药库包装), mintNumberDigit)
            If mshBill.TextMatrix(intRow, mconIntCol售价) <> "" Then
                mshBill.TextMatrix(intRow, mconIntCol售价金额) = GetFormat(Val(mshBill.TextMatrix(intRow, mconIntCol售价)) * Val(mshBill.TextMatrix(intRow, mconIntCol数量)), mintMoneyDigit)
            End If
            
'            mshBill.TextMatrix(intRow, mconintCol差价) = FormatEx(Get出库差价(Val(cboStock.ItemData(cboStock.ListIndex)), Val(mshBill.TextMatrix(intRow, 0)), Val(mshBill.TextMatrix(intRow, mconIntCol批次)), Val(mshBill.TextMatrix(intRow, mconIntCol实际金额)), Val(mshBill.TextMatrix(intRow, mconIntCol实际差价)), Val(mshBill.TextMatrix(intRow, mconIntCol售价金额)), Val(mshBill.TextMatrix(intRow, mconIntCol数量)) * Val(mshBill.TextMatrix(intRow, mconIntCol比例系数))), mintMoneyDigit)
            
            If Nvl(!实际数量, 0) <> 0 Then
                mshBill.TextMatrix(intRow, mconIntCol采购价) = GetFormat(Get成本价(Val(mshBill.TextMatrix(intRow, 0)), Val(cboStock.ItemData(cboStock.ListIndex)), Val(mshBill.TextMatrix(intRow, mconIntCol批次))) * Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)), mintCostDigit)
'                mshBill.TextMatrix(intRow, mconIntCol采购价) = FormatEx((mshBill.TextMatrix(intRow, mconIntCol售价金额) - mshBill.TextMatrix(intRow, mconintCol差价)) / Val(mshBill.TextMatrix(intRow, mconIntCol数量)), mintCostDigit)
            End If
            mshBill.TextMatrix(intRow, mconIntCol采购金额) = GetFormat(Val(mshBill.TextMatrix(intRow, mconIntCol采购价)) * Val(mshBill.TextMatrix(intRow, mconIntCol数量)), mintMoneyDigit)
            mshBill.TextMatrix(intRow, mconintCol差价) = GetFormat(Val(mshBill.TextMatrix(intRow, mconIntCol售价金额)) - Val(mshBill.TextMatrix(intRow, mconIntCol采购金额)), mintMoneyDigit)
            
            '计算外调价及外调金额:外调价=(1+管理费比例)*进价
            mshBill.TextMatrix(intRow, mconIntCol外调价) = GetFormat((1 + !管理费比例 / 100) * Val(mshBill.TextMatrix(intRow, mconIntCol采购价)), mintCostDigit)
            mshBill.TextMatrix(intRow, mconIntCol外调金额) = GetFormat(Val(mshBill.TextMatrix(intRow, mconIntCol外调价)) * Val(mshBill.TextMatrix(intRow, mconIntCol数量)), mintMoneyDigit)
...
Private Function CompareUsableQuantity(ByVal intRow As Integer, ByVal dbl填写数量 As Double) As Boolean
    ...
    With mshBill
        ...
        If .TextMatrix(intRow, mconIntCol批次) > 0 Or Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1) = 1 Then   '对移出库房是库房且药品是分批核算或则是时价药品的判断
            If mint编辑状态 = 1 Then
...
Private Function CheckDrugStock(ByVal str药品名称 As String, ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl药品数量 As Double, Optional ByVal intRow As Integer) As Boolean
    Dim lng库房ID As Long
    ...
    Else
        ...
        dbl原填写数量 = IIf(mbln下可用数量, numUsedCount * Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)), 0)
            
        gstrSQL = "Select Nvl(可用数量,0) 可用数量,Nvl(实际数量,0) 实际数量 From 药品库存 Where 库房ID=[1] And Nvl(批次,0)=[3] And 性质=1 And 药品ID=[2] "
        Set rsCheck = zlDatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[检查库存]", lng库房ID, lng药品ID, lng批次)
        
        '填单和修改单据时取可用数量(如果填单时下了可用库存，要加上原来的填写数量)来检查；
        '审核时取实际数量来检查
        If rsCheck.EOF Then
            Dbl数量 = 0
        Else
            If mint编辑状态 = 1 Or mint编辑状态 = 2 Then
                Dbl数量 = rsCheck!可用数量 + dbl原填写数量
            Else
                Dbl数量 = rsCheck!实际数量
            End If
        End If
        ...
        If blnMsg Then
            If mint库存检查 = 1 Then        '不足提醒
                If MsgBox("药品【" & str药品名称 & "】领用数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else                            '不足禁止
                MsgBox "药品【" & str药品名称 & "】领用数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），不能领用！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
        CheckDrugStock = True
