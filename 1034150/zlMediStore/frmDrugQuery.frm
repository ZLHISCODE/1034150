...
Begin VB.Form frmDrugQuery 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfBatch 
      ...
      EndProperty
      MousePointer    =   0
      MouseIcon       =   "frmDrugQuery.frx":F461
      BackColor       =   -2147483643
      ForeColor       =   -2147483640
      BackColorFixed  =   -2147483633
      ForeColorFixed  =   -2147483630
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frmDrugQuery.frx":F47D
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
'    If DataRecordSet.RecordCount = 0 Then Exit Sub
    ...
    If cboStock.ItemData(cboStock.ListIndex) = 0 Then
        Call ObjReport.ReportOpen(gcnOracle, glngSys, "ZL1_INSIDE_1309_2", Me, "药品=" & vsfList.TextMatrix(vsfList.Row, vsfList.ColIndex("名称")) & "|" & Val(vsfList.TextMatrix(vsfList.Row, vsfList.ColIndex("药品ID"))), "库房=所有库房|is not null", "单位=" & Choose(intChoose级数, "售价单位", "门诊单位", "药库单位", "住院单位") & "|" & Choose(intChoose级数, 1, 3, 2, 4))    ' , "开始日期=" & Format(DateAdd("m", -1, ZlDatabase.Currentdate), "yyyy-MM-DD"), "结束日期=" & Format(ZlDatabase.Currentdate, "yyyy-MM-DD"))
    Else
        Call ObjReport.ReportOpen(gcnOracle, glngSys, "ZL1_INSIDE_1309_2", Me, "药品=" & vsfList.TextMatrix(vsfList.Row, vsfList.ColIndex("名称")) & "|" & Val(vsfList.TextMatrix(vsfList.Row, vsfList.ColIndex("药品ID"))), "库房=" & cboStock.Text & "|=  " & cboStock.ItemData(cboStock.ListIndex), "单位=" & Choose(intChoose级数, "售价单位", "门诊单位", "药库单位", "住院单位") & "|" & Choose(intChoose级数, 1, 3, 2, 4)) ' , "开始日期=" & Format(DateAdd("m", -1, ZlDatabase.Currentdate), "yyyy-MM-DD"), "结束日期=" & Format(ZlDatabase.Currentdate, "yyyy-MM-DD"))
    End If
End Sub
...
Private Sub Form_Activate()
    If Me.tvwSection_S.Tag <> "T" Then ReFreshDrugData Me.cboStock.ItemData(Me.cboStock.ListIndex), IIf(Left(Me.tvwSection_S.SelectedItem.Key, 1) = "R", 0, Mid(Me.tvwSection_S.SelectedItem.Key, 2))
End Sub
...
Private Sub grdPrint(blnIsPreview As Byte)
    ...
    objRow.Add "截止日期：" & Format(zlDatabase.Currentdate, "yyyy年MM月DD日")
...
Private Sub vsfBatch_EnterCell()
    ...
    With vsfBatch
        ...
        .ForeColorSel = IIf(Val(.RowData(.Row)) = 0, glng黑色, glng报警)
...
'            .Row = LngPhysicRow       '清除上次选中行
'            For intCol = 0 To .Cols - 1
'                .Col = intCol
'                .CellBackColor = glng白色
'                .CellForeColor = IIf(.RowData(.Row) = 0, glng黑色, glng报警)
'            Next
'            .Col = 0
'        End If
'
...
'            For intCol = 0 To .Cols - 1
'                .Col = intCol
'                .CellBackColor = IIf(Me.ActiveControl.Name = "vsfBatch", glng蓝色, glng灰色)
'                If Me.ActiveControl.Name = "vsfBatch" Then
'                    lngColor = IIf(.RowData(.Row) = 0, glng白色, glng报警)
'                Else
'                    lngColor = IIf(.RowData(.Row) = 0, glng黑色, glng报警)
'                End If
...
'            .Redraw = flexRDNone
'            .Col = intCol
'            .CellBackColor = glng蓝色
'            .CellForeColor = IIf(.RowData(.Row) = 0, glng白色, glng报警)
'            .Redraw = flexRDDirect
'        Next
'        .Col = 0
'    End With
...
'            .CellForeColor = IIf(Trim(.TextMatrix(.Row, .colindex("撤档时间"))) = "", glng黑色, glng红色)
'            .Redraw = flexRDDirect
'        Next
'        .Col = 0
'    End With
...
Private Sub vsfBatch_LostFocus()
'    Dim intCol As Integer
'    With vsfBatch
...
'    If DataRecordSet.RecordCount = 0 Then Exit Sub
    ...
    If cboStock.ItemData(cboStock.ListIndex) = 0 Then
        Call ObjReport.ReportOpen(gcnOracle, glngSys, IIf(glngSys \ 100 = 1, "ZL1_INSIDE_1309_2", "ZL8_INSIDE_1309_2"), Me, "药品=" & vsfList.TextMatrix(vsfList.MouseRow, vsfList.ColIndex("名称")) & "|" & Val(vsfList.TextMatrix(vsfList.MouseRow, vsfList.ColIndex("药品ID"))), "库房=所有库房|is not null", "单位=" & Choose(intChoose级数, "售价单位", "门诊单位", "药库单位", "住院单位") & "|" & Choose(intChoose级数, 1, 3, 2, 4))    ' , "开始日期=" & Format(DateAdd("m", -1, ZlDatabase.Currentdate), "yyyy-MM-DD"), "结束日期=" & Format(ZlDatabase.Currentdate, "yyyy-MM-DD"))
    Else
...
Private Function ReFreshTreeView() As Boolean
    ...
    gstrSQL = "Select distinct a.ID,(a.编码 || '-' || a.名称) As 名称 From 部门表 a,部门性质说明 b,部门性质分类 C " & _
              ...
              IIf(IsHavePrivs(mstrPrivs, "所有库房"), "", " And A.id In (Select 部门ID From 部门人员 Where 人员ID=[1])") & _
    ...
    With RecDept
        If .RecordCount = 0 Then
            ...
            Exit Function
        End If
        If IsHavePrivs(mstrPrivs, "所有库房") Then
            Me.cboStock.Clear
    ...
    Str材质 = ""
    StrSort = ""
    If IsHavePrivs(mstrPrivs, "西成药") Then
        Bln西成药 = True
        Str材质 = "1"
        StrSort = ",'5'"
    Else
    ...
    If IsHavePrivs(mstrPrivs, "中成药") Then
        Bln中成药 = True
        If Str材质 = "" Then
            Str材质 = "2"
        Else
    ...
    If IsHavePrivs(mstrPrivs, "中草药") Then
        Bln中草药 = True
        If Str材质 = "" Then
            Str材质 = "3"
        Else
...
Private Function ReFreshDrugData(ByVal lngDeptId As Long, _
    ...
    Call zlCommFun.ShowFlash("正在查找数据,请稍候 ...", Me)
    DoEvents
    For Each lstItem In Me.lst剂型_S.ListItems
    ...
    If lngDeptId = 0 Then
        ...
        Case 1
            gstrSQL = ",A.计算单位 as 单位,'' as 上次采购价,nvl(P.现价,0) as 当前售价,nvl(M.剂量系数,0) as 系数1,Sum(B.可用数量) As 可用数量,Sum(B.实际数量) As 实际数量,Sum(B.实际金额) As 实际金额,Sum(B.实际差价) As 实际差价,Nvl(b.平均成本价,m.成本价) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,1 as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, '' 库房货位 "
            strOrder = " Group by M.药品ID,X.分类id,A.编码,M.基本药物,M.标识码,A.名称,L.名称,A.规格,decode(b.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),b.上次产地),M.药库分批,A.计算单位,nvl(P.现价,0),nvl(M.剂量系数,0),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称,m.成本价,b.平均成本价 "
        Case 2
            gstrSQL = ",M.门诊单位 as 单位,'' as 上次采购价,nvl(P.现价,0)*nvl(M.门诊包装,0) as 当前售价,nvl(M.门诊包装,0) as 系数1,Sum(B.可用数量/Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 可用数量,Sum(B.实际数量/Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 实际数量,Sum(B.实际金额) As 实际金额,Sum(B.实际差价) As 实际差价,Nvl(b.平均成本价,m.成本价)*Decode(M.门诊包装,0,1,null,1,M.门诊包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.门诊包装,0,1,null,1,M.门诊包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, '' 库房货位 "
            strOrder = " Group by M.药品ID,X.分类id,A.编码,M.基本药物,M.标识码,A.名称,L.名称,A.规格,decode(b.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),b.上次产地),M.药库分批,M.门诊单位,nvl(P.现价,0)*nvl(M.门诊包装,0),nvl(M.门诊包装,0),Decode(M.门诊包装,0,1,null,1,M.门诊包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称,m.成本价,b.平均成本价 "
        Case 3
            gstrSQL = ",M.药库单位 as 单位,'' as 上次采购价,nvl(P.现价,0)*nvl(M.药库包装,0) as 当前售价,nvl(M.药库包装,0) as 系数1,Sum(B.可用数量/Decode(M.药库包装,0,1,null,1,M.药库包装)) as 可用数量, Sum(B.实际数量/Decode(M.药库包装,0,1,null,1,M.药库包装)) as 实际数量,Sum(B.实际金额) As 实际金额,Sum(B.实际差价) As 实际差价,Nvl(b.平均成本价,m.成本价)*Decode(M.药库包装,0,1,null,1,M.药库包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.药库包装,0,1,null,1,M.药库包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商,'' 库房货位 "
            strOrder = " Group by M.药品ID,X.分类id,A.编码,M.基本药物,M.标识码,A.名称,L.名称,A.规格,decode(b.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),b.上次产地),M.药库分批,M.药库单位,nvl(P.现价,0)*nvl(M.药库包装,0),nvl(M.药库包装,0),Decode(M.药库包装,0,1,null,1,M.药库包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称,m.成本价,b.平均成本价 "
        Case 4
            gstrSQL = ",M.住院单位 as 单位,'' as 上次采购价,nvl(P.现价,0)*nvl(M.住院包装,0) as 当前售价,nvl(M.住院包装,0) as 系数1,Sum(B.可用数量/Decode(M.住院包装,0,1,null,1,M.住院包装)) as 可用数量, Sum(B.实际数量/Decode(M.住院包装,0,1,null,1,M.住院包装)) as 实际数量,Sum(B.实际金额) As 实际金额,Sum(B.实际差价) As 实际差价,Nvl(b.平均成本价,m.成本价)*Decode(M.住院包装,0,1,null,1,M.住院包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.住院包装,0,1,null,1,M.住院包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商,'' 库房货位 "
            strOrder = " Group by M.药品ID,X.分类id,A.编码,M.基本药物,M.标识码,A.名称,L.名称,A.规格,decode(b.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),b.上次产地),M.药库分批,M.住院单位,nvl(P.现价,0)*nvl(M.住院包装,0),nvl(M.住院包装,0),Decode(M.住院包装,0,1,null,1,M.住院包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称,m.成本价,b.平均成本价 "
        End Select
    Else
        Select Case intChoose级数
        Case 1
            gstrSQL = ",A.计算单位 as 单位,Nvl(Avg(S.上次采购价), m.成本价) as 上次采购价,nvl(P.现价,0) as 当前售价,nvl(M.剂量系数,0) as 系数1,Sum(S.可用数量) as 可用数量, Sum(S.实际数量) as 实际数量,sum(i.下限) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,1 as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
            strOrder = " Group by M.药品ID,A.编码,M.基本药物,M.标识码,X.分类id,A.名称,L.名称,A.规格,decode(s.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),s.上次产地),nvl(M.最大效期,0),s.库存效期,s.报警,M.药库分批,A.计算单位,nvl(P.现价,0),nvl(M.剂量系数,0),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称, C.库房货位, m.成本价,s.平均成本价 "
        Case 2
            gstrSQL = ",M.门诊单位 as 单位,Nvl(Avg(S.上次采购价*nvl(M.门诊包装,0)), m.成本价*nvl(M.门诊包装,0)) as 上次采购价,nvl(P.现价,0)*nvl(M.门诊包装,0) as 当前售价,nvl(M.门诊包装,0) as 系数1,Sum(S.可用数量 /Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 可用数量, Sum(S.实际数量 /Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 实际数量,sum(i.下限/Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.门诊包装,0,1,null,1,M.门诊包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.门诊包装,0,1,null,1,M.门诊包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
            strOrder = " Group by M.药品ID,A.编码,M.基本药物,M.标识码,X.分类id,A.名称,L.名称,A.规格,decode(s.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),s.上次产地),nvl(M.最大效期,0),s.库存效期,s.报警,M.药库分批,M.门诊单位,nvl(P.现价,0)*nvl(M.门诊包装,0),nvl(M.门诊包装,0),Decode(M.门诊包装,0,1,null,1,M.门诊包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称, C.库房货位, m.成本价,s.平均成本价 "
        Case 3
            gstrSQL = ",M.药库单位 as 单位,Nvl(Avg(S.上次采购价*nvl(M.药库包装,0)), m.成本价*nvl(M.药库包装,0)) as 上次采购价,nvl(P.现价,0)*nvl(M.药库包装,0) as 当前售价,nvl(M.药库包装,0) as 系数1,Sum(S.可用数量 /Decode(M.药库包装,0,1,null,1,M.药库包装)) as 可用数量,Sum(S.实际数量 /Decode(M.药库包装,0,1,null,1,M.药库包装)) as 实际数量,sum(i.下限/Decode(M.药库包装,0,1,null,1,M.药库包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.药库包装,0,1,null,1,M.药库包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.药库包装,0,1,null,1,M.药库包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
            strOrder = " Group by M.药品ID,A.编码,M.基本药物,M.标识码,X.分类id,A.名称,L.名称,A.规格,decode(s.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),s.上次产地),nvl(M.最大效期,0),s.库存效期,s.报警,M.药库分批,M.药库单位,nvl(P.现价,0)*nvl(M.药库包装,0),nvl(M.药库包装,0),Decode(M.药库包装,0,1,null,1,M.药库包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称, C.库房货位, m.成本价,s.平均成本价 "
        Case 4
            gstrSQL = ",M.住院单位 as 单位,Nvl(Avg(S.上次采购价*nvl(M.住院包装,0)), m.成本价*nvl(M.住院包装,0)) as 上次采购价,nvl(P.现价,0)*nvl(M.住院包装,0) as 当前售价,nvl(M.住院包装,0) as 系数1,Sum(S.可用数量 /Decode(M.住院包装,0,1,null,1,M.住院包装)) as 可用数量, Sum(S.实际数量 /Decode(M.住院包装,0,1,null,1,M.住院包装)) as 实际数量,sum(i.下限/Decode(M.住院包装,0,1,null,1,M.住院包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.住院包装,0,1,null,1,M.住院包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.住院包装,0,1,null,1,M.住院包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
            strOrder = " Group by M.药品ID,A.编码,M.基本药物,M.标识码,X.分类id,A.名称,L.名称,A.规格,decode(s.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),s.上次产地),nvl(M.最大效期,0),s.库存效期,s.报警,M.药库分批,M.住院单位,nvl(P.现价,0)*nvl(M.住院包装,0),nvl(M.住院包装,0),Decode(M.住院包装,0,1,null,1,M.住院包装),Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')),Nvl(A.是否变价, 0),G.名称, C.库房货位, m.成本价,s.平均成本价 "
        End Select
    End If
    ...
    If lngDeptId = 0 Then
        strSQL = "SELECT Distinct M.药品ID,X.分类ID As 用途分类ID,A.编码,M.基本药物,M.标识码 As 药卡号,A.名称 As 通用名,L.名称 As 商品名,A.规格,decode(b.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),b.上次产地) AS 产地,NULL AS 效期,DECODE(M.药库分批,1,'是','否') AS 药库分批 " & gstrSQL & _
                " FROM 药品规格 M,收费价目 P," & str收费项目目录 & _
                " A ,(Select distinct b.药品id, b.上次成本价,b.平均成本价, '' 上次产地, b.可用数量,b.实际数量,b.实际金额, b.实际差价 " & _
                " From 药品库存 a,(Select 药品id, Avg(上次采购价) As 上次成本价,Sum(Nvl(实际数量,0) * 平均成本价) / Decode(Sum(Nvl(实际数量, 0)), 0, 1, Sum(Nvl(实际数量, 0))) as 平均成本价, Max(nvl(批次,0)) As 批次, " & _
                " Sum(Nvl(可用数量,0)) As 可用数量, Sum(Nvl(实际数量,0)) As 实际数量, Sum(Nvl(实际金额,0)) As 实际金额, Sum(Nvl(实际差价,0)) As 实际差价 " & _
                " From 药品库存 Where 性质 = 1 and (Nvl(可用数量,0)<>0 or Nvl(实际数量,0)<>0 or Nvl(实际金额,0)<>0 or Nvl(实际差价,0)<>0) Group By 药品id) b Where a.药品id=b.药品id And a.性质 = 1 And nvl(a.批次,0) = b.批次) B, " & _
                " 诊疗项目目录 X,药品特性 T,收费项目别名 L, 供应商 G "
'        If str剂型 <> "" Then
'            StrSql = StrSql & ",Table(Cast(f_Str2list([11]) As zlTools.t_Strlist)) E"
'        End If
                
        strSQL = strSQL & " WHERE M.药名ID=X.ID And X.ID=T.药名ID And Nvl(M.上次供应商id, 0) = G.ID(+) " & _
...
'            StrSql = StrSql & " And T.药品剂型=E.Column_Value "
    ...
    Else
        strSQL = "SELECT M.药品ID,X.分类ID As 用途分类ID,A.编码,M.基本药物,M.标识码 As 药卡号,A.名称 As 通用名,L.名称 As 商品名,A.规格,decode(s.上次产地,null,decode(m.上次产地,null,a.产地,m.上次产地),s.上次产地) AS 产地,NVL(M.最大效期,0) AS 效期,s.报警,s.库存效期,DECODE(M.药库分批,1,'是','否') AS 药库分批 " & gstrSQL & _
                " FROM 药品规格 M,收费价目 P,(select nvl(下限,0) as 下限,药品id from 药品储备限额 where 库房id=[9]) I ," & str收费项目目录 & _
                " A,诊疗项目目录 X,药品特性 T, " & _
                " (Select b.药品id, b.上次采购价, b.平均成本价,a.上次产地, b.可用数量,b.实际数量,b.实际金额, b.实际差价, A.上次供应商id,Decode(Sign(Add_Months(Sysdate, " & intMonths & ") - 效期), -1, 0, 1) 报警,效期 as 库存效期 " & _
                "  From 药品库存 a,(SELECT 药品ID,avg(上次采购价) AS 上次采购价,Sum(Nvl(实际数量,0) * 平均成本价) / Decode(Sum(Nvl(实际数量, 0)), 0,1, Sum(Nvl(实际数量, 0))) as 平均成本价,Max(nvl(批次,0)) AS 批次, " & _
                " SUM(Nvl(可用数量,0)) AS 可用数量,SUM(Nvl(实际数量,0)) AS 实际数量,SUM(Nvl(实际金额,0)) AS 实际金额,SUM(Nvl(实际差价,0)) AS 实际差价 " & _
                "  FROM 药品库存 WHERE 库房ID=[9] AND 性质=1 and (Nvl(可用数量,0)<>0 or Nvl(实际数量,0)<>0 or Nvl(实际金额,0)<>0 or Nvl(实际差价,0)<>0)  " & _
                "  GROUP BY 药品ID) b Where a.库房ID=[9] and a.药品id=b.药品id And a.性质 = 1 And nvl(a.批次,0) = b.批次) S, " & _
                " 收费项目别名 L, 供应商 G, (Select Distinct 收费细目id, 执行科室id From 收费执行科室 Where 执行科室id=[9]) K, " & _
        ...
        strSQL = strSQL & " WHERE  i.药品id(+)=m.药品id and M.药品ID =A.ID And M.药名ID =X.ID And X.ID=T.药名ID And Nvl(S.上次供应商id, 0) = G.ID(+) " & _
                ...
                IIf(bln包含停用药品, "", " AND (TO_CHAR(A.撤档时间, 'YYYY-MM-DD') = '3000-01-01' OR A.撤档时间 IS NULL)  ") & _
                "       And M.药品ID+0=P.收费细目ID AND SYSDATE BETWEEN P.执行日期 AND NVL(P.终止日期,SYSDATE) "
...
Private Sub RefreshBatch(lng库房ID As Long, lng药品id As Long)
    ...
    If Not rsTemp.EOF And Not rsTemp.BOF Then
        ...
        If lng库房ID = 0 Or (int药房 = 1 And int分批 = 2) Or (int药房 = 0 And int分批 <> 0) Then
            ...
            If lng库房ID = 0 Then
                gstrSQL = "Select 库房, 批号, 平均成本价, 失效期, 报警, 产地, 上次采购价, Sum(可用数量) As 可用数量, Sum(实际数量) As 实际数量, Sum(实际金额) As 实际金额, Sum(实际差价) As 实际差价," & _
                          "     填制日期 , NO, 供药单位, 供应商, 库房货位, 下限, 售价, 是否变价, 现价 " & _
                        " From ( " & _
                        " SELECT (D.编码 || '-' || D.名称) AS 库房,s.上次批号 AS 批号,avg(s.平均成本价) 平均成本价,NULL AS 失效期,0 报警,NULL AS 产地,NULL AS 上次采购价," & _
                        "        SUM(Nvl(S.可用数量,0))/" & dbl包装系数 & " AS 可用数量,SUM(Nvl(S.实际数量,0))/" & dbl包装系数 & " AS 实际数量,SUM(Nvl(S.实际金额,0)) AS 实际金额,SUM(Nvl(S.实际差价,0)) AS 实际差价," & _
                        "        NULL AS 填制日期,NULL AS NO,NULL AS 供药单位,NULL As 供应商, C.库房货位,下限/" & dbl包装系数 & " as 下限, " & _
                        " decode(nvl(s.批次,0),0,decode(sum(Nvl(s.实际数量,0)),0,0,Sum(Nvl(s.实际金额,0)) / Sum(Nvl(s.实际数量,0))),s.零售价)*" & dbl包装系数 & " As 售价,nvl(a.是否变价,0) as 是否变价,b.现价*" & dbl包装系数 & "as 现价 " & _
                        ...
                        "       AND (Nvl(S.可用数量,0)<>0 OR Nvl(S.实际数量,0)<>0 OR Nvl(S.实际金额,0)<>0 OR Nvl(S.实际差价,0)<>0) " & _
                        " GROUP BY D.编码 || '-' || D.名称,s.上次批号, C.库房货位,下限,是否变价,现价,批次,零售价)" & _
                        " Group By 批号,库房, 平均成本价, 失效期, 报警, 产地, 上次采购价, 填制日期, NO, 供药单位, 供应商, 库房货位, 下限, 售价, 是否变价, 现价"
            Else
               gstrSQL = "SELECT (D.编码 || '-' || D.名称) AS 库房,S.上次批号 As 批号,s.平均成本价, S.效期 失效期, S.上次产地 As 产地,DECODE(SIGN(ADD_MONTHS(SYSDATE," & intMonths & ")-S.效期),-1,0,1) 报警," & _
                        "        Nvl(S.可用数量,0)/" & dbl包装系数 & " AS 可用数量,Nvl(S.实际数量,0)/" & dbl包装系数 & " AS 实际数量,Nvl(S.实际金额,0) As 实际金额,Nvl(S.实际差价,0) As 实际差价, " & _
                        "        S.上次采购价*" & dbl包装系数 & " AS 上次采购价, G.名称 As 供应商,'' 库房货位, decode(nvl(s.批次,0),0,decode(Nvl(s.实际数量,0),0,0,Nvl(s.实际金额,0) / Nvl(s.实际数量,0)),s.零售价)*" & dbl包装系数 & " As 售价, " & _
                        " nvl(a.是否变价,0) as 是否变价, b.现价*" & dbl包装系数 & "as 现价" & _
            ...
            Me.vsfBatch.rows = rsTemp.RecordCount + 1
            With rsTemp
                Do While Not .EOF
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("库房")) = !库房
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("批号")) = IIf(IsNull(!批号), "", !批号)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("效期")) = Format(!失效期, "yyyy年MM月dd日")
                    If gtype_UserSysParms.P149_效期显示方式 = 1 And Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("效期")) <> "" And cboStock.Text <> "所有库房" Then
                        '换算为有效期
                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("效期")) = Format(DateAdd("D", -1, Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("效期"))), "yyyy-mm-dd")
                    End If
                    
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("产地")) = IIf(IsNull(!产地), "", !产地)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("可用数量")) = Format(!可用数量, mStr数量)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("实际数量")) = Format(!实际数量, mStr数量)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("实际金额")) = Format(!实际金额, mStr金额)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("实际差价")) = Format(!实际差价, mStr金额)
                    If !是否变价 = 0 Then
                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("售价")) = Format(!现价, mStr单价)
                    Else
                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("售价")) = Format(!售价, mStr单价)
                    End If
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("上次采购价")) = Format(!上次采购价, mStr成本价)
                    If !实际数量 <> 0 Then
'                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("成本价")) = Format((!实际金额 - !实际差价) / !实际数量, mStr数量)
'                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("成本金额")) = Format(!实际金额 - !实际差价, mStr金额)
                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("成本价")) = Format(!平均成本价 * dbl包装系数, mStr成本价)
                        Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("成本金额")) = Format(!平均成本价 * dbl包装系数 * !实际数量, mStr金额)
                    End If
          
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("供应商")) = IIf(IsNull(!供应商), "", !供应商)
                    Me.vsfBatch.TextMatrix(.AbsolutePosition, vsfBatch.ColIndex("库房货位")) = IIf(IsNull(!库房货位), "", !库房货位)
                    
                    With vsfBatch
                        If Me.vsfBatch.TextMatrix(rsTemp.AbsolutePosition, vsfBatch.ColIndex("实际数量")) <> "" And lng库房ID = 0 Then
                            lng下限 = IIf(IsNull(rsTemp!下限), 0, rsTemp!下限)
                            If lng下限 = 0 Then
                                strTemp = "无下限"
                            ElseIf Val(.TextMatrix(rsTemp.AbsolutePosition, .ColIndex("实际数量"))) > lng下限 Then
                                strTemp = "充足"
                            ElseIf Val(.TextMatrix(rsTemp.AbsolutePosition, .ColIndex("实际数量"))) = lng下限 Then
                                strTemp = "持平"
                            ElseIf Val(.TextMatrix(rsTemp.AbsolutePosition, .ColIndex("实际数量"))) < lng下限 Then
                                strTemp = "不足"
                            End If
                            .TextMatrix(rsTemp.AbsolutePosition, .ColIndex("储备情况")) = strTemp
                        End If
                    End With
                    '根据记录状态的不同，进行着色
                    lngColor = IIf(!报警 = 0, glng正常, glng报警)
                    Me.vsfBatch.Cell(flexcpForeColor, .AbsolutePosition, 0, .AbsolutePosition, vsfBatch.Cols - 1) = lngColor
                    '失效期药品在列前加上时钟图标
                    If !报警 = 1 And IsNull(!失效期) = False Then
                        Me.vsfBatch.Cell(flexcpPicture, .AbsolutePosition, 0, .AbsolutePosition, 0) = imglvw.ListImages(3).Picture
                    End If
                    ...
                    If zlCommFun.Nvl(!可用数量, 0) < 0 Or zlCommFun.Nvl(!实际数量, 0) <= 0 Then
                        Me.vsfBatch.Cell(flexcpForeColor, .AbsolutePosition, 0, .AbsolutePosition, vsfBatch.Cols - 1) = vbRed
                    End If
                    
                    Me.vsfBatch.RowData(.AbsolutePosition) = 0 ' CStr(!报警)
                    .MoveNext
                Loop
            End With
        End If
...
Private Sub 设置权限()
    If Not IsHavePrivs(mstrPrivs, "药品明细帐") Then
        tbrThis.Buttons("明细").Visible = False
        mnuFileBatch.Visible = False
    End If
    If Not IsHavePrivs(mstrPrivs, "药品总帐") Then
...
Private Sub SetFormat(ByVal intType As Integer)
    ...
    If intType = IniListType.AllList Or intType = IniListType.MainList Then
        With vsfList
            ...
            .ColWidth(.ColIndex("上次供应商")) = IIf(IsHavePrivs(mstrPrivs, "供应商查询"), IIf(.ColWidth(.ColIndex("上次供应商")) = 0, 2500, .ColWidth(.ColIndex("上次供应商"))), 0)
    ...
    If intType = IniListType.AllList Or intType = IniListType.BatchList Then
        With vsfBatch
            ...
            Else
                ...
                .ColWidth(.ColIndex("供应商")) = IIf(IsHavePrivs(mstrPrivs, "供应商查询"), IIf(.ColWidth(.ColIndex("供应商")) = 0, 2500, .ColWidth(.ColIndex("供应商"))), 0)
...
Private Sub DataBound(ByVal rsData As ADODB.Recordset)
    ...
    With vsfList
        ...
        Do While Not rsData.EOF
            ...
            If rsData!变价 = 0 Then  '定价
                lngPrice = rsData!当前售价
            Else                            '时价
                If Nvl(rsData!实际数量, 0) <> 0 Then
                    lngPrice = rsData!实际金额 / rsData!实际数量
                Else
                    lngPrice = rsData!当前售价
                End If
            End If
