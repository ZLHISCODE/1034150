...
Private Const MCON_规格 = _
    ...
    "|药品编码,,,1000|通用名称,,,1000|药品名称,,1,1000|商品名,,,1000|规格,,0,1000|产地,,,1000|药名ID,,1,0" & _
    ...
    "|时价,,,1000|指导批发价,,1,1000,n|指导差价率,,1,1000,n|库房货位,,,1000|批准文号,,,1000|实际数量,,1,1000,n" & _
    "|合同单位,,0,1000|药价级别,,,1000|留存数量,,1,,n|简码,,1,0|数字简码,,1,0|五笔码,,1,0"

Private Const MCON_批次 = _
    "RID,,1,0|库房,,,1000|批次,,1,1000|入库日期,,0,1000,d|批号,,,1000|生产日期,,,1000,d|有效期,,,1000,d|产地,,,1000" & _
...
Private Const MCON_选定 = _
    ...
    "|剂量单位,,1,1000|药品名称,,1,1000|商品名,,,1000|规格,,0,1000|产地,,0,1000|药名ID,,1,0|药品ID,,1,0|批号,,0,1000" & _
    ...
    "|时价,,1,1000|指导批发价,,1,1000,n|指导差价率,,1,1000,n|批准文号,,1,1000" & _
...
Private mbln包含停用药品 As Boolean         '是否显示停用药品
...
Private mlngLast As Long    '保存最后一次列点击
...
Private Sub cmdRefresh_Click()
    ...
    If mfrmMain.Caption Like "药品移库单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品移库管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品盘点记录单*" Or mfrmMain.Caption Like "药品盘点表*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品盘点管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "库存差价调整单*" Then
        Call SetSelectorRS(mbyt编辑模式, "库存差价调整管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品领用单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品领用管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品其他入库单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品其他入库管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品其他出库单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品其他出库管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品外购入库单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品外购入库管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品移库单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品移库管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品申领单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品申领管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    
    ElseIf mfrmMain.Caption Like "药品计划单*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品计划管理", mlng来源库房, mlng目标库房, , mlng供应商, , mbln包含停用药品)
    End If
...
'    '取系统参数“明确申领药品批次”
'    If mbln申领单 Then
'        mbln明确申领批次 = (gtype_UserSysParms.P73_明确申领药品批次 = 1)
'    End If
...
Private Sub picSetCols_Click(Index As Integer)
    ...
    If Index = 0 Then
        vRect = GetControlRect(vsf规格.hWnd)
    ...
    Else
        vRect = GetControlRect(vsf批次.hWnd)
...
Private Sub tvw类别_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    If tvw类别.Tag <> Node.Key Then
        ...
        '格式化VSF的列
        FormatCols strCols
        If Node.Tag = "3" Then '非中草药不显示中药形态
...
'--------------------------------------------
    ...
    ElseIf zlCommFun.IsCharAlpha(strSimpleCode) Then
        '纯字母
        strReturn = strTemp & " like '" & mstrMatch & strSimpleCode & "%'"
    ElseIf zlCommFun.IsCharChinese(strSimpleCode) Then
        '纯汉字
...
'--------------------------------
    ...
    Case mconint门诊单位
        dblUnit = Nvl(objFields("门诊包装").Value, 0)
    Case mconint住院单位
        dblUnit = Nvl(objFields("住院包装").Value, 0)
    Case mconint药库单位
        dblUnit = Nvl(objFields("药库包装").Value, 0)
    Case Else
        dblUnit = 1
    End Select
    ...
    If objFields(intCol).Name = "上次采购价" Or objFields(intCol).Name = "成本价" Then
        ...
        If strVSFName <> "vsf规格" Then
            strReturn = Format(Nvl(objFields(intCol).Value, 0) * dblUnit, mStr成本价)
        Else
            strReturn = Format(Nvl(objFields(intCol).Value, 0), mStr成本价)
        End If
    ElseIf objFields(intCol).Name = "售价" Then
        If strVSFName <> "vsf规格" Then
            strReturn = Format(Nvl(objFields(intCol).Value, 0) * dblUnit, mStr单价)
        Else
            strReturn = Format(Nvl(objFields(intCol).Value, 0), mStr单价)
        End If
    ElseIf objFields(intCol).Name = "可用数量" Or objFields(intCol).Name = "库存数量" Or objFields(intCol).Name = "实际数量" Then
        If strVSFName <> "vsf规格" Then
            strReturn = Format(Nvl(objFields(intCol).Value, 0) / dblUnit, mStr数量)
        Else
            strReturn = Format(Nvl(objFields(intCol).Value, 0), mStr数量)
        End If
    ElseIf objFields(intCol).Name = "库存金额" Or objFields(intCol).Name = "库存差价" Then
        strReturn = Format(Nvl(objFields(intCol).Value, 0), mStr金额)
    Else
        strReturn = Nvl(objFields(intCol).Value)
    End If
...
'--------------------------
    ...
    If chkChoose.Value = 2 Then
        ...
        With vsf规格
            For i = 1 To .rows - 1
                .RowHidden(i) = InStr(str剂型, ";" & .TextMatrix(i, .ColIndex("剂型")) & ";") = 0
                If .RowHidden(i) = False And blnFirst = False Then
                    .Row = i
                    blnFirst = True
                End If
            Next
    ...
    Else
        ...
        With vsf规格
            For i = 1 To .rows - 1
                If .RowHidden(i) Then .RowHidden(i) = False
                .Row = 1
            Next
        End With
    End If
    vsf规格.Redraw = flexRDDirect
End Sub

Private Sub Filter形态()
'--------------------------
'功能：过滤掉非勾选的形态
'--------------------------
    Dim str形态 As String
    ...
    If str形态 <> ";" Then
        With vsf规格
            For lngRow = 1 To .rows - 1
                .RowHidden(lngRow) = InStr(str形态, ";" & .TextMatrix(lngRow, .ColIndex("中药形态")) & ";") = 0
                If .RowHidden(lngRow) = False And blnFirst = False Then
                    .Row = lngRow
                    blnFirst = True
                End If
            Next
...
'        End If
    ...
    If vsf批次.Visible Or vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("时价")) = "是" Then
        If (DblCurStock > 0) Or Not mblnPrice Or vsf批次.TextMatrix(vsf批次.Row, vsf批次.ColIndex("批号")) = "新增批次药品" Then
            CheckData = True
            Exit Function
        End If
        MsgBox "该" & IIf(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("时价")) = "是", "时价", "批次") _
                    & "药品已经没有库存，不能继续操作！", vbInformation, gstrSysName
        Exit Function
    Else
    ...
    Else
        If mbyt编辑模式 = 2 Then
            If mblnPrice Then
                ...
                End If
            Else
                If Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("可用数量"))) = 0 And GetCheck库房(mlng来源库房) = 1 Then
                    If FillVSF选定(Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("药品ID")))) = False Then Exit Sub
                Else
                    If FillVSF选定(Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("药品ID")))) = False Then Exit Sub
                End If
...
Private Sub vsf规格_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then Call vsf规格_DblClick
End Sub
...
Private Sub vsf批次_DblClick()
    ...
    Dim int不明确批次 As Integer
    
    If GetVSFlexRows(vsf批次) <= 1 Then Exit Sub
    If glngModul = 1305 Then '领用
        With vsf规格
            If .TextMatrix(.Row, .ColIndex("领用标志")) = "0" Or .TextMatrix(.Row, .ColIndex("领用标志")) = "" Then
                MsgBox "该部门不允许领用该药品，请到药品储备限额中设置！", vbInformation, gstrSysName
                Exit Sub
    ...
    If glngModul = 1304 And gtype_UserSysParms.P174_药品移库明确批次 = 0 Then   '移库
        With vsf规格
            For intRow = 1 To vsf选定.rows - 1
                If .TextMatrix(.Row, .ColIndex("药品id")) = vsf选定.TextMatrix(intRow, vsf选定.ColIndex("药品id")) Then
    ...
    If glngModul = 1343 And gtype_UserSysParms.P73_明确申领药品批次 = 0 Then   '申领
        With vsf规格
            For intRow = 1 To vsf选定.rows - 1
                If .TextMatrix(.Row, .ColIndex("药品id")) = vsf选定.TextMatrix(intRow, vsf选定.ColIndex("药品id")) Then
    ...
    int不明确批次 = Val(GetSetting("ZLSOFT", "私有模块\" & gstrDbUser & "\" & App.ProductName & "\" & "药品外购入库管理", "不明确批次填单", 0))
    If glngModul = 1300 And int不明确批次 = 1 Then   '外购
        With vsf规格
            For intRow = 1 To vsf选定.rows - 1
                If .TextMatrix(.Row, .ColIndex("药品id")) = vsf选定.TextMatrix(intRow, vsf选定.ColIndex("药品id")) Then
                    MsgBox "该药品已经在药品选择器中，不允许在同张单据中选择多次！", vbInformation, gstrSysName
                    Exit Sub
                End If
            Next
        End With
...
'    int分批 = Get库房分批()
    ...
    With vsf选定
        ...
        .TextMatrix(lngRow, .ColIndex("指导差价率")) = vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("指导差价率"))
        .TextMatrix(lngRow, .ColIndex("产地")) = vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("产地"))
        '成本价
...
'        End Select
        ...
        If vsf批次.Visible Then
            ...
            .TextMatrix(lngRow, .ColIndex("产地")) = vsf批次.TextMatrix(vsf批次.Row, vsf批次.ColIndex("产地"))
        ...
        Else
            ...
            If rsTemp.RecordCount > 0 Then
                .TextMatrix(lngRow, .ColIndex("批号")) = Nvl(rsTemp!上次批号)
                If Not IsNull(rsTemp!生产日期) Then
                    .TextMatrix(lngRow, .ColIndex("生产日期")) = Nvl(rsTemp!生产日期)
                End If
                .TextMatrix(lngRow, .ColIndex("上次供应商ID")) = Nvl(rsTemp!上次供应商ID)
                .TextMatrix(lngRow, .ColIndex("产地")) = Nvl(rsTemp!上次产地)
                ...
                If Not IsNull(rsTemp!效期) Then
                    ...
                    Else
                        .TextMatrix(lngRow, .ColIndex("有效期")) = Nvl(rsTemp!效期)
                    End If
                End If
                .TextMatrix(lngRow, .ColIndex("批准文号")) = Nvl(rsTemp!批准文号)
            End If
            rsTemp.Close
            
            .TextMatrix(lngRow, .ColIndex("批次")) = ""
            '库存盘点记录单，无“查看盘点单库存”参数处理
            If Not mblnStore Then
                ProcQuantity vsf选定, lngRow, .TextMatrix(lngRow, .ColIndex("药品ID"))
            Else
...
'-----------------------------------
    ...
    gstrSQL = "Select A.现价, B.指导批发价, B.指导零售价, C.编码 药名编码, C.名称 通用名称 " & _
              ...
              "  And Sysdate Between A.执行日期 And Nvl(A.终止日期,Sysdate) And A.收费细目ID=[1] "
    ...
    If Not rsTemp.EOF Then
        ...
        End If
        If strMsg <> "" Then strMsg = "[" & Nvl(rsTemp!药名编码) & Nvl(rsTemp!通用名称) & "]" & strMsg
    End If
    rsTemp.Close
    ...
    With vsf选定
        For i = 1 To .rows - 1
            If Val(.TextMatrix(i, .ColIndex("药品ID"))) > 0 Then
                ...
                mrsReturn!产地 = .TextMatrix(i, .ColIndex("产地"))
                ...
                mrsReturn!批次 = IIf(.TextMatrix(i, .ColIndex("批次")) = "", Null, .TextMatrix(i, .ColIndex("批次")))
                ...
                mrsReturn!指导差价率 = Val(.TextMatrix(i, .ColIndex("指导差价率")))
