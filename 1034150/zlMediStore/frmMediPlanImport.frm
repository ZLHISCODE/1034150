...
Begin VB.Form frmMediPlanImport 
   ...
   Begin VB.PictureBox picDetail 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfDetail 
         ...
         Left            =   0
         TabIndex        =   4
         ToolTipText     =   "执行数量与计划数量字体加粗表示执行数量大于了计划数量及该计划单已经产生了入库单并已审核"
         Top             =   120
         Width           =   2655
         _cx             =   4683
         _cy             =   2143
...
Private Sub cbo指定_Click()
    ...
    With vsfDetail
        ...
        For i = 1 To .rows - 1
            ...
            '判断药品是否设置存储库房在该导入库房中
            blncheck库房 = Check库房(Val(IIf(IsNull(.TextMatrix(i, .ColIndex("id"))), 0, .TextMatrix(i, .ColIndex("id")))), Val(cbo指定.ItemData(cbo指定.ListIndex)))
    ...
    With vsfDetail
        For i = 1 To .rows - 1
            If .TextMatrix(i, .ColIndex("choose")) = "-1" And .Cell(flexcpBackColor, i, 1, i, 2) <> &HFFC0C0 And 是否导入(i) Then
                ...
                rsTmp!加成率 = .TextMatrix(i, .ColIndex("add_rate"))
    ...
    With rsTmp
        ...
        Do While Not .EOF
            If lngWHID <> rsTmp!库房id Or lngPID <> rsTmp!供药单位ID Then
                ...
                lngPID = rsTmp!供药单位ID
                intSN = 1
                strNo = zldatabase.GetNextNo(21, rsTmp!库房id)
            End If
            '执行存储过程，提交数据
...
'初始化VsfView
    ...
    Else
        ...
        strCols = "||选择,choose,440|H_计划id,planid,1000|H_库房id,whid,0|计划单号,planno,880|序号,sn,440|H_药品ID,id,1000|药品名称,name,1800|计划数量,planqty,1500|执行数量,execqty,1500" _
                ...
                & "|上次生产商,producer,2000|H_生产日期,pdate,0|H_最大效期,avail_day,0|H_加成率,add_rate,0|H_药库包装,store_pak,0|H_分批属性,batch,0"
    End If
...
'                End If
                ...
                If Val(.TextMatrix(i, .ColIndex("execqty"))) > Val(.TextMatrix(i, .ColIndex("planqty"))) Then
                    .Cell(flexcpFontBold, i, .ColIndex("execqty"), i, .ColIndex("planqty")) = True
                End If
    ...
    If Col = vsfMain.ColIndex("choose") Then
        If vsfMain.TextMatrix(Row, Col) = -1 Then
            ...
            strSQL = "select -1 选择,b.库房id, B.NO, A.计划id, A.药品id," _
                   ...
                   & "           (Select Max(批次) From 药品库存 Where B.库房id = 库房id And A.药品id = 药品id And 性质 = 1) " _
                   & " and b.库房id = 库房id And a.药品id = 药品id And 性质 = 1 and rownum=1 ) " _
                   & "      else  A.单价 End, C.成本价) 成本价, a.售价,  " _
                   & "A.上次供应商, A.上次生产商, A.说明, F.是否变价, " _
                   & "(1 / (1 - C.指导差价率 / 100)) - 1 加成率," _
                   & "G.id 供应商ID, (select max(上次生产日期) from 药品库存 where 药品id=c.药品id) 上次生产日期, F.最大效期, C.指导零售价, C.差价让利比 " _
                   & "From 药品计划内容 A," _
                   & "     (Select ID, NO, 库房id From 药品采购计划 Where 审核日期 is not null " & strWhere _
