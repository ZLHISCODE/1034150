...
Private Function CheckDepend() As Boolean
    ...
    gstrSQL = "SELECT DISTINCT a.id, a.名称 " _
            ...
            & "Where (a.站点 = '" & gstrNodeNo & "' Or a.站点 is Null) And c.工作性质 = b.名称 " _
             ...
             & " AND a.id = c.部门id " _
              ...
              & IIf(IsHavePrivs(mstrPrivs, "所有库房"), "", " And a.ID IN (Select 部门ID From 部门人员 Where 人员ID=[2])")
    ...
    With cboStock
        ...
        If .ListIndex = -1 Then
            If Not IsHavePrivs(mstrPrivs, "所有库房") Then
...
Private Sub GetList(ByVal StrFind As String)
    ...
    If TabShow.Tab = 1 Then
        ...
        gstrSQL = "Select NO, 盘点时间, 填制人, 填制日期, 审核人, 审核日期, " & _
                ...
                & "     LTrim(To_Char(a.扣率 * a.零售价 , " & mstrMoneyFormat & ")) As 盘点金额," _
                & "ltrim(to_char(零售金额*a.入出系数,decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & "))) 金额差," _
                & "ltrim(to_char((A.扣率-A.填写数量) * a.零售价* Decode(记录状态, 1, 1, Decode(Mod(记录状态, 3), 0, 1, -1)),decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & "))) AS 账面金额差," _
                & "ltrim(to_char((a.成本价+to_char(a.零售金额*a.入出系数*Decode(a.记录状态, 1, 1, Decode(Mod(a.记录状态, 3), 0, 1, -1)),decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & ")))-(a.成本金额+to_char(a.差价*a.入出系数*Decode(a.记录状态, 1, 1, Decode(Mod(a.记录状态, 3), 0, 1, -1)),decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & ")))," & mstrMoneyFormat & ")) as 盘点成本金额," _
                & "ltrim(to_char(a.零售金额*a.入出系数-a.差价*a.入出系数,decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & "))) as 成本金额差," _
                & " a.记录状态, a.摘要 " _
                & " FROM 药品收发记录 a,药品规格 B " & strSqlForm _
                & " Where A.药品ID=B.药品ID And A.单据 = 12  " & strUserPart & StrFind _
                & " Group By a.No,a.序号, 频次, a.填制人, a.审核人, a.成本价, a.入出系数, a.成本价,a.成本金额," & str包装系数 & ", a.零售金额, a.记录状态, a.扣率, a.填写数量, a.零售价,a.扣率, a.单量, a.差价, a.摘要) " _
...
'    End If
    ...
    If Not IsHavePrivs(mstrPrivs, "月度确认") Then
    ...
    If Not IsHavePrivs(mstrPrivs, "冲销") Then
        ...
        End If
    End If
    If Not IsHavePrivs(mstrPrivs, "全部盘为零") Then
    ...
    End If
    If Not IsHavePrivs(mstrPrivs, "药品盘点表") Then
        mnuEditAddTable.Visible = False
        tlbTool.Buttons("Table").Visible = False
...
Private Sub mnuFileParameter_Click()
    ...
    frm参数设置.设置参数 Me, mstrPrivs, mlngMode, mstrTitle
    ...
    If vsfList.Row >= 1 And LTrim(vsfList.TextMatrix(vsfList.Row, 0)) <> "" Then
        ...
        If TabShow.Tab = 1 Then
            gstrSQL = "Select DISTINCT a.序号" & strSql药名 & "," _
                    ...
                    & " From (Select a.入出系数,a.记录状态,a.序号,a.药品id,a.产地,a.批号,a.效期,A.填写数量,A.扣率,A.实际数量,a.成本价,a.成本金额,a.零售价,a.零售金额,a.差价,a.单量,a.批准文号" _
                    & "         From 药品收发记录 a" _
                    & "        Where a.记录状态= [2] " _
...
'            Case 0
'                For n = 1 To .rows - 1
'                    .TextMatrix(n, .ColIndex("实盘数")) = GetFormat(.TextMatrix(n, .ColIndex("实盘数")), mintShowNumberDigit)
'                Next
'            Case 1
'                For n = 1 To .rows - 1
'                    .TextMatrix(n, .ColIndex("帐面数")) = GetFormat(.TextMatrix(n, .ColIndex("帐面数")), mintShowNumberDigit)
'                    .TextMatrix(n, .ColIndex("实盘数")) = GetFormat(.TextMatrix(n, .ColIndex("实盘数")), mintShowNumberDigit)
'                    .TextMatrix(n, .ColIndex("数量差")) = GetFormat(.TextMatrix(n, .ColIndex("数量差")), mintShowNumberDigit)
'                Next
'            End Select
'        End With
...
Private Sub SetEnable()
    ...
    mnuEditVerify.Visible = blnVisible And IsHavePrivs(mstrPrivs, "审核")
    mnuEditStrike.Visible = blnVisible And IsHavePrivs(mstrPrivs, "冲销")
    mnuEditLine2.Visible = blnVisible
    tlbTool.Buttons("Verify").Visible = blnVisible And IsHavePrivs(mstrPrivs, "审核")
    tlbTool.Buttons("Strike").Visible = blnVisible And IsHavePrivs(mstrPrivs, "冲销")
