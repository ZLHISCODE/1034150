        ...
        For n = 1 To mshBill.rows - 1
            If Val(mshBill.TextMatrix(n, 0)) > 0 Then
                If .RecordCount = 0 Then
                    ...
                    !界面实际数量 = Val(mshBill.TextMatrix(n, mconIntCol填写数量))
                ...
                Else
                    ...
                    Else
                        ...
                        !界面实际数量 = !界面实际数量 + Val(mshBill.TextMatrix(n, mconIntCol填写数量))
                        !单位 = Nvl(mshBill.TextMatrix(n, mconIntCol单位))
                        .Update
                    End If
                End If
...
Private Function CheckBillStock() As Boolean
    ...
    With mshBill
        For n = 1 To .rows - 1
            If Val(.TextMatrix(n, 0)) <> 0 Then
                ...
                Dbl数量 = Val(.TextMatrix(n, mconIntCol实际数量)) * Val(.TextMatrix(n, mconIntCol比例系数))
                If Not CheckDrugStock(lng药品ID, lng批次, Dbl数量, n) Then
                    Exit Function
                End If
            End If
        Next
    ...
    CheckBillStock = True
End Function
Private Function CheckDrugStock(ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl药品数量 As Double, Optional ByVal intRow As Integer) As Boolean
    Dim lng库房ID As Long
    ...
    Else
        ...
        If blnMsg Then
            If mint库存检查 = 1 Then        '不足提醒
                If MsgBox("领用数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else                            '不足禁止
                MsgBox "领用数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），不能领用！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
        CheckDrugStock = True
...
Private Sub 提示库存数()
    ...
    With mshBill
        ...
        Else
            gstrSQL = "select 可用数量/" & .TextMatrix(.Row, mconIntCol比例系数) & " as  可用数量 from 药品留存 where 期间=[1] and 库房id=[2] " _
