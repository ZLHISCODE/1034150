...
'            txtCost.Caption = ""
'        Else
'            txtCostPrice.Caption = GetFormat(RecReturn!成本价, mintCostDigit)
'            txtCost.Caption = GetFormat(Val(txtCostPrice.Caption) * Val(Me.TxtNumber) * Val(txt单位.Tag), mintMoneyDigit)
'        End If
'        If IsNull(RecReturn!售价) Then
'            txtSalePrice.Caption = ""
'            txtSale.Caption = ""
'        Else
'            txtSalePrice.Caption = GetFormat(RecReturn!售价, mintPriceDigit)
'            txtSale.Caption = GetFormat(Val(txtSalePrice.Caption) * Val(Me.TxtNumber) * Val(txt单位.Tag), mintMoneyDigit)
'        End If
'
'        If Val(txtProvider.Tag) <> 0 Then
...
'        End If
        ...
        txtCostPrice.Tag = GetFormat(Get成本价(RecReturn!药品id, mlng库房ID, IIf(IsNull(RecReturn!批次), 0, RecReturn!批次)), gtype_UserDrugDigits.Digit_成本价)
        txtCostPrice.Caption = GetFormat(Val(txtCostPrice.Tag) * Val(txt单位.Tag), mintCostDigit)
        txtCost.Caption = GetFormat(Val(txtCostPrice.Caption) * Val(Me.TxtNumber), mintMoneyDigit)
        
        If RecReturn!时价 = 1 Then
            txtSalePrice.Tag = GetFormat(Get零售价(RecReturn!药品id, mlng库房ID, IIf(IsNull(RecReturn!批次), 0, RecReturn!批次), 1), gtype_UserDrugDigits.Digit_零售价)
        Else
            txtSalePrice.Tag = GetFormat(RecReturn!售价, gtype_UserDrugDigits.Digit_零售价)
        End If
        txtSalePrice.Caption = GetFormat(Val(txtSalePrice.Tag) * Val(txt单位.Tag), mintPriceDigit)
...
Private Function SaveHandle() As Boolean
    ...
    If mint审核时减库存 = 1 Then
        ...
        dblQuantity = FormatEx(Val(TxtNumber.Text) * Val(txt单位.Tag), gtype_UserSaleDigits.Digit_数量)
        ...
        gstrSQL = "Select Nvl(A.实际数量,0) 实际数量, Nvl(A.实际金额,0) 实际金额, Nvl(A.实际差价,0) 实际差价, A.效期, A.批准文号, " & _
            " Nvl(B.是否变价, 0) 是否变价, C.现价, D.指导差价率, Nvl(D.管理费比例, 0) 比例,Nvl(A.批次,0) As 批次,Nvl(A.零售价,0) As 零售价 " & _
            ...
            " A.库房id = [1] And A.药品id = [2] And Nvl(A.批次, 0) = [3]"
...
'        End If
        ...
        If cboType.Text = "药品外调" Then
            dblOutPrice = FormatEx((1 + rsTemp!比例 / 100) * dblPurchasePrice, gtype_UserSaleDigits.Digit_成本价)
...
Private Function CheckDrugStock(ByVal lng库房ID As Long, ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl药品数量 As Double) As Boolean
    ...
    Else
        ...
        '审核时取实际数量来检查
        If Dbl药品数量 > rsCheck!实际数量 Then
            blnMsg = True
        End If
        
        If blnMsg Then
            If mint库存检查 = 1 Then        '不足提醒
                If MsgBox("出库数量大于现有的库存数量，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else                            '不足禁止
                MsgBox "出库数量大于现有的库存数量，不能出库！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
        CheckDrugStock = True
...
'                  "Where (站点 = [1] Or 站点 is Null) And (撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or 撤档时间 Is Null) "
    ...
    dtp处理日期.Value = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
    dtp短损日期.Value = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
...
Private Sub initCard()
    ...
    With frmDrugQualityList.vsfList
        ...
        TxtNumber.Text = GetFormat(rsTmp!毁损数量 / .TextMatrix(.Row, 14 + intBit), IIf(mint编辑模式 = 4, mintNumberDigit, mintNumberDigit))
        ...
        txtCostPrice.Tag = GetFormat(rsTmp!成本单价, gtype_UserDrugDigits.Digit_成本价)
        txtCostPrice.Caption = GetFormat(Val(txtCostPrice.Tag) * Val(txt单位.Tag), mintCostDigit)
        If IsNull(rsTmp!成本金额) Then
            txtCost.Caption = ""
        Else
            txtCost.Caption = GetFormat(rsTmp!成本金额, mintMoneyDigit)
        End If
        
        txtSalePrice.Tag = GetFormat(rsTmp!销售单价, gtype_UserDrugDigits.Digit_零售价)
        txtSalePrice.Caption = GetFormat(Val(txtSalePrice.Tag) * Val(txt单位.Tag), mintPriceDigit)
        If IsNull(rsTmp!销售金额) Then
            txtSale.Caption = ""
        Else
            txtSale.Caption = GetFormat(rsTmp!销售金额, mintMoneyDigit)
        End If
...
Private Function Get零售价(ByVal lng药品ID As Long, ByVal lng库房ID As Long, ByVal lng批次 As Long, ByVal dbl比例系数 As Double) As Double
    ...
    gstrSQL = "select Decode(Nvl(批次,0),0,实际金额/实际数量,Nvl(零售价,实际金额/实际数量))*" & dbl比例系数 & " as  售价 " _
        ...
        & " nvl(批次,0)=[3]"
    Set rsPrice = zlDatabase.OpenSQLRecord(gstrSQL, "Get零售价", lng库房ID, lng药品ID, lng批次)

    If rsPrice.EOF Then
        Get零售价 = 0
        Exit Function
    End If
    Get零售价 = rsPrice.Fields(0).Value
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub TxtNumber_Validate(Cancel As Boolean)
    ...
    If Trim(TxtNumber.Text) <> "" Then
        ...
        If CheckDrugStock(lng库房ID, lng药品ID, lng批次, lng数量) = False Then
            Cancel = True
            Exit Sub
        End If
    End If
    ...
    With txtProvider
        ...
        If adoProvider.RecordCount > 1 Then
            ...
            With mshProvider
                ...
                .Top = sstQuality.Top + Fra短损信息.Top + txtProvider.Top - .Height ' + txtProvider.Height
                .Left = sstQuality.Left + Fra短损信息.Left + txtProvider.Left   ' + cmdProvider.Width - .Width
                Exit Sub
            End With
        Else
            .Text = adoProvider!名称
...
Private Sub txt产地_Validate(Cancel As Boolean)
    If Trim(txt产地.Text) <> "" Then
        If LenB(StrConv(txt产地.Text, vbFromUnicode)) > 30 Then
            MsgBox "产地超长，最多能输入15个汉字或30个字符!", vbInformation, gstrSysName
            Cancel = True
        End If
    End If
End Sub
