...
Begin VB.Form frmPurchaseCard 
   ...
   Begin VB.CheckBox chk不明确批次 
      Caption         =   "不明确批次填单"
      Height          =   270
      Left            =   4680
      TabIndex        =   54
      ToolTipText     =   "方便分批且存在多个批次的药品退货"
      Top             =   5400
      Visible         =   0   'False
      Width           =   1560
   End
...
Private mintFixPrice As Integer             '0:采购价格不固定；1：采购价，发票价格固定
...
Private mint退货发票金额方式 As Integer     '退货时发票金额默认为：0－按结算金额为准；1－按售价金额为准
Private mbln修改单据号 As Boolean           '允许修改单据号，由参数控制
...
Private mbln重置 As Boolean        '切换"不明确批次填单"的询问选择
...
Private mconIntCol结算价 As Integer
Private mconIntCol结算金额 As Integer
...
Private Function Check退库(ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl数量 As Double, Optional ByVal intRow As Integer) As Boolean
    Dim lng库房ID As Long
    ...
    gstrSQL = "Select Nvl(可用数量,0) 可用数量,Nvl(实际数量,0) 实际数量 From 药品库存 Where 库房ID=[1] And Nvl(批次,0)=[3] And 性质=1 And 药品ID=[2] "
    Set rsCheck = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[检查库存是否足够——退货]", lng库房ID, lng药品ID, lng批次)
    
    If rsCheck.RecordCount = 0 Then
        blnMsg = True
    Else
        If mint编辑状态 = 1 Or mint编辑状态 = 2 Or mint编辑状态 = 8 Then
            dbl可用数量 = rsCheck!可用数量 + Abs(dbl原填写数量)
        Else
            dbl可用数量 = rsCheck!实际数量
        End If
    End If
    If blnMsg = False Then If Abs(Dbl数量) > Abs(dbl可用数量) Then blnMsg = True
    
    If blnMsg Then
        If rsCheck.RecordCount <> 0 Then
            If mint库存检查 = 1 Then
                If MsgBox("退货数量不能大于现有的库存数量（当前库存数量为：" & Abs(dbl可用数量) / Val(mshBill.TextMatrix(intCurRow, mconIntCol比例系数)) & "）！是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else
                MsgBox "退货数量不能大于现有的库存数量（当前库存数量为：" & Abs(dbl可用数量) / Val(mshBill.TextMatrix(intCurRow, mconIntCol比例系数)) & "）！", vbInformation, gstrSysName
                Exit Function
            End If
        Else
            If mint库存检查 = 1 Then
                If MsgBox("退货数量不能大于现有的库存数量（当前库存数量为：0）！是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else
                MsgBox "退货数量不能大于现有的库存数量（当前库存数量为：0）！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
    End If
...
Private Sub Set时价分批药品零售价(ByVal intRow As Integer, ByVal dblPrice As Double)
    ...
    With mshBill
       ...
       .TextMatrix(intRow, mconintCol零售价) = GetFormat(dblPrice, gtype_UserDrugDigits.Digit_零售价) '零售价字段本来都是最小单位，因此按照最大位数进行显示
        ...
        If Val(.TextMatrix(intRow, mconIntCol结算价)) = Val(.TextMatrix(intRow, mconIntCol售价)) Then
        ...
        End If
        .TextMatrix(intRow, mconintCol零售差价) = GetFormat(Val(.TextMatrix(intRow, mconintCol零售金额)) - Val(.TextMatrix(intRow, mconIntCol结算金额)), mintMoneyDigit)
    End With
End Sub
...
Public Sub ShowCard(FrmMain As Form, ByVal str单据号 As String, ByVal int编辑状态 As Integer, Optional int记录状态 As Integer = 1, Optional BlnSuccess As Boolean = False)
    ...
    mintSelectStock = Val(zldatabase.GetPara("是否选择库房", glngSys, 模块号.外购入库))
    
    mintFixPrice = Val(zldatabase.GetPara("定价采购", glngSys, 模块号.外购入库))
    mbln修改单据号 = (Val(zldatabase.GetPara("修改外购单据号", glngSys, 模块号.外购入库)) = 1)
    ...
    mint退货发票金额方式 = Val(zldatabase.GetPara("退货发票金额", glngSys, 模块号.外购入库, 1))
    mbln供应商校验 = (Val(zldatabase.GetPara("校验供应商资质", glngSys, 模块号.外购入库)) = 1)
    mint时价分段加成方式 = Val(zldatabase.GetPara("时价药品入库采用分段加成", glngSys, 模块号.外购入库, 0))
    mint时价入库时取上次售价 = Val(zldatabase.GetPara("时价药品入库时取上次售价", glngSys, 模块号.外购入库, 0))
    Set mfrmMain = FrmMain
    mblnEdit = False
    If mint编辑状态 = 1 Or mint编辑状态 = 8 Then
        mblnEdit = True
        If mbln修改单据号 = True Then
            txtNO.Locked = False
            txtNO.TabStop = True
        End If
        chk不明确批次.Visible = mint编辑状态 = 8
...
Private Sub chk不明确批次_Click()
    Dim intCol As Integer
    
    If mshBill.rows <= 1 Or Val(mshBill.TextMatrix(1, 0)) <= 0 Then Exit Sub '没有药品不需要清空
    
    If Not mbln重置 Then Exit Sub

    If MsgBox("切换条件，界面中已有数据将清除，是否继续？", vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
        mbln重置 = False
        If chk不明确批次.Value = 1 Then
            chk不明确批次.Value = 0
        Else
            chk不明确批次.Value = 1
        End If
    ...
    With mshBill
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                .TextMatrix(intRow, mconIntCol结算金额) = GetFormat(0, mintMoneyDigit)
                ...
                If Trim(.TextMatrix(intRow, mconintcol发票号)) <> "" Then
                    .TextMatrix(intRow, mconintcol发票金额) = GetFormat(0, mintMoneyDigit)
                End If
    ...
    For intRow = 1 To mshBill.rows - 1
        If (gtype_UserSysParms.P173_经过标记付款后才能进行付款管理 = 1 And mshBill.TextMatrix(intRow, mconIntCol付款标志) <> "是") Or gtype_UserSysParms.P173_经过标记付款后才能进行付款管理 <> 1 Then
            If mshBill.TextMatrix(intRow, 0) <> "" And mshBill.RowData(intRow) = 0 Then
                ...
                mshBill.TextMatrix(intRow, mconIntCol结算金额) = GetFormat(mshBill.TextMatrix(intRow, mconIntCol数量) * mshBill.TextMatrix(intRow, mconIntCol结算价), mintMoneyDigit)
                mshBill.TextMatrix(intRow, mconIntCol售价金额) = GetFormat(mshBill.TextMatrix(intRow, mconIntCol数量) * mshBill.TextMatrix(intRow, mconIntCol售价), mintMoneyDigit)
                mshBill.TextMatrix(intRow, mconintCol差价) = GetFormat(mshBill.TextMatrix(intRow, mconIntCol售价金额) - mshBill.TextMatrix(intRow, mconIntCol结算金额), mintMoneyDigit)
                ...
                If Trim(mshBill.TextMatrix(intRow, mconintcol发票号)) <> "" Then
                    ...
                    If rsDrug.EOF Then
                        mshBill.TextMatrix(intRow, mconintcol发票金额) = mshBill.TextMatrix(intRow, mconIntCol结算金额)
                    Else
                        mshBill.TextMatrix(intRow, mconintcol发票金额) = GetFormat(rsDrug.Fields(0), mintMoneyDigit)
                    End If
                End If
    ...
    With mshBill
        ...
        For i = 1 To .rows - 1
            If i <> .Row And Trim(.TextMatrix(i, mconintcol发票号)) = "" And .TextMatrix(i, 0) <> "" Then    '不是编辑行且发票号为空的批量修改
                ...
                If mint记录状态 = 1 Then .TextMatrix(i, mconintcol发票金额) = .TextMatrix(i, mconIntCol结算金额)
...
Private Sub getInputData()
    ...
    Do While Not rsTemp.EOF
        With vsfInputCost
            ...
            .TextMatrix(.rows - 1, .ColIndex("成本价")) = GetFormat(rsTemp!成本价 * cmdGetData.Tag, lblDate.Tag)
...
'    '此代码保留，35版本中需要此功能 问题号：77592
'    Dim rsData As ADODB.Recordset
'
'    If txtProvider.Tag = "0" Then
'        MsgBox "请先选择一个供药单位！", vbInformation, gstrSysName
...
'        Set rsData = CheckRedo(rsData) '检查重复记录，将重复的记录过滤掉然后返回过滤后的数据集
...
Private Sub cmdNo_Click()
    ...
    With mshBill
        ...
        If mint时价入库时取上次售价 <> 1 Then
            .TextMatrix(.Row, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mconIntCol结算价)), mdbl加价率 / 100, Val(.TextMatrix(.Row, mconIntCol结算价)) * (1 + (mdbl加价率 / 100))), mintPriceDigit)
        End If
        .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol售价)) * Val(.TextMatrix(.Row, mconIntCol数量)), mintMoneyDigit)
        .TextMatrix(.Row, mconintCol差价) = GetFormat(IIf(.TextMatrix(.Row, mconIntCol售价金额) = "", 0, .TextMatrix(.Row, mconIntCol售价金额)) - IIf(.TextMatrix(.Row, mconIntCol结算金额) = "", 0, .TextMatrix(.Row, mconIntCol结算金额)), mintMoneyDigit)
...
Private Sub cmdYes_Click()
    ...
    If Val(Txt加价率) > 900 Or Val(Txt加价率) < 0 Then
        MsgBox "请输入合法的加成率！（0-200）", vbInformation, gstrSysName
        Txt加价率.SetFocus
        Exit Sub
    End If
    ...
    With mshBill
        ...
        dbl成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(.Row, mconIntCol结算价)), Val(.TextMatrix(.Row, mconIntCol采购价)))
                        
        '重新计算零售价、差价
        If mint时价入库时取上次售价 <> 1 Then
            .TextMatrix(.Row, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), dbl成本价, Val(Txt加价率) / 100, dbl成本价 * (1 + (Val(Txt加价率) / 100))), mintPriceDigit)
        End If
        .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol售价)) * Val(.TextMatrix(.Row, mconIntCol数量)), mintMoneyDigit)
        .TextMatrix(.Row, mconintCol差价) = GetFormat(IIf(.TextMatrix(.Row, mconIntCol售价金额) = "", 0, .TextMatrix(.Row, mconIntCol售价金额)) - IIf(.TextMatrix(.Row, mconIntCol结算金额) = "", 0, .TextMatrix(.Row, mconIntCol结算金额)), mintMoneyDigit)
        .TextMatrix(.Row, mconIntCol加成率) = FormatEx(Val(Txt加价率), 2) & "%"
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    ElseIf KeyCode = vbKeyF4 Then
        ...
        If mbln加价率 And (mint编辑状态 = 1 Or mint编辑状态 = 2) Then
            ...
            Txt加价率 = "15.00000"
            With mshBill
                If Val(.TextMatrix(.Row, mconIntCol售价)) <> 0 And Val(.TextMatrix(.Row, mconIntCol结算价)) <> 0 Then
                    Txt加价率 = GetFormat((Val(.TextMatrix(.Row, mconIntCol售价)) / Val(.TextMatrix(.Row, mconIntCol结算价)) - 1) * 100, 5)
                End If
            End With
            Txt加价率.Tag = Txt加价率
            Txt加价率.SetFocus
...
Private Sub CmdSave_Click()
    ...
    mstr审核日期 = Format(zldatabase.Currentdate, "yyyy-mm-dd hh:mm:ss")
...
'            End If
    ...
    If mint编辑状态 = 8 Or (mbln退货 And mint编辑状态 = 2) Then '退货新增和修改检查是否存在分批且未明确批次的药品
        For i = 1 To mshBill.rows - 1
            If Val(mshBill.TextMatrix(i, 0)) <> 0 Then
                If Get分批属性(Val(cboStock.ItemData(cboStock.ListIndex)), Val(mshBill.TextMatrix(i, 0))) = 1 And Val(mshBill.TextMatrix(i, mconIntCol批次)) = 0 Then
                    MsgBox "存在分批且未明确批次的药品，将进行自动分解！", vbInformation, gstrSysName
                    AutoExpend
                    Exit Sub
                End If
            End If
            ...
            If mshBill.TextMatrix(i, 0) <> "" Then
                ...
                If Val(Split(.TextMatrix(i, mconIntCol原销期), "||")(2)) = 0 Then '判断定价
                    ...
                    If .TextMatrix(i, mconIntCol售价) <> dbl零售价 Then
                        ...
                        dbl成本价 = Val(.TextMatrix(i, mconIntCol结算价))
...
'    mbln时价取上次售价 = IIf(Val(zlDataBase.GetPara(183, 100)) = 0, False, True)
    ...
    If InStr(1, "|" & mstrColumn_UnSelected & "|", "|结算价|") = 0 Then mshBill.ColWidth(mconIntCol结算价) = IIf(mblnViewCost Or mint编辑状态 = 7, 1000, 0)
    If InStr(1, "|" & mstrColumn_UnSelected & "|", "|采购价|") = 0 Then mshBill.ColWidth(mconIntCol采购价) = IIf((mblnViewCost Or mint编辑状态 = 7), 1000, 0)
    If InStr(1, "|" & mstrColumn_UnSelected & "|", "|结算金额|") = 0 Then mshBill.ColWidth(mconIntCol结算金额) = IIf(mblnViewCost Or mint编辑状态 = 7, 900, 0)
    If InStr(1, "|" & mstrColumn_UnSelected & "|", "|差价|") = 0 Then mshBill.ColWidth(mconintCol差价) = IIf(mblnViewCost, 900, 0)
'    mshBill.ColWidth(mconintCol零售差价) = IIf(mblnViewCost, 1000, 0)
...
Private Sub initCard()
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4, 5, 6, 7, 9
            ...
            Case 5, 7     '修改发票,财务审核
                If mint记录状态 = 1 Then
                    gstrSQL = "SELECT * FROM (SELECT DISTINCT A.药品ID,A.序号,'[' || D.编码 || ']' As 药品编码, D.名称 As 通用名, E.名称 As 商品名," & _
                        " B.药品来源,B.基本药物,D.规格,D.产地 AS 原产地,A.产地, A.批号,NVL(A.批次,0) 批次," & _
                        ...
                        " A.成本金额 AS 采购金额,B.指导差价率/100 AS 指导差价率,D.是否变价,B.药房分批 药房分批核算," & _
                        " DECODE(A.扣率, NULL, 0, A.扣率) AS 扣率, A.零售价*" & num包装系数 & " AS 零售价,A.零售金额,A.差价, " & _
                        " A.批准文号,C.随货单号, C.发票号 ,c.发票代码, C.发票日期, C.发票金额,A.供药单位ID,F.名称 AS 供应商, A.填制人,A.填制日期," & _
                        " A.审核人,A.审核日期,A.库房ID,G.名称 AS 部门,NVL(C.付款序号,0) AS 付款序号,Nvl(A.发药方式,0) 退货,A.外观,A.验收结论," & _
                ...
                Else
                    gstrSQL = "SELECT * FROM (SELECT DISTINCT A.药品ID,A.序号,'[' || D.编码 || ']' As 药品编码, D.名称 As 通用名, E.名称 As 商品名," & _
                        " B.药品来源,B.基本药物,D.规格,D.产地 AS 原产地,A.产地,A.批号,A.批次," & _
                        " NVL(B.招标药品,0) 招标药品,NVL(B.差价让利比,0) 差价让利比,B.最大效期,A.效期," & strUnitQuantity & _
                        " nvl(A.单量,b.指导批发价)*" & num包装系数 & " AS 指导批发价 ,A.成本价*" & num包装系数 & " AS 采购价," & _
                        " A.成本金额 AS 采购金额,B.指导差价率/100 AS 指导差价率,D.是否变价,B.药房分批 药房分批核算,  " & _
                        " DECODE(A.扣率, NULL, 0, A.扣率) AS 扣率, A.零售价*" & num包装系数 & " AS 零售价 ,A.零售金额,A.差价," & _
                        " A.批准文号, A.随货单号, A.发票号,a.发票代码,A.发票日期,A.发票金额,A.供药单位ID,F.名称 AS 供应商, A.库房ID,G.名称 AS 部门,NVL(A.付款序号,0) AS 付款序号,A.退货,A.生产日期,A.配药人 As 核查人,A.配药日期 As 核查日期,B.药价级别,A.金额差,A.加成率,付款标志,a.计划id " & _
                        " FROM " & _
                        "     (SELECT X.NO, SUM(实际数量) AS 填写数量,SUM(成本金额) AS 成本金额,随货单号,发票号,发票代码,发票日期," & _
                        "      X.药品ID,X.序号,X.产地, X.批号,NVL(X.批次,0) 批次,X.效期,X.扣率,X.成本价,X.零售价,X.单量," & _
                        ...
                        "      FROM 药品收发记录 X,(Select 序号,项目ID,随货单号,发票号,发票代码,发票日期,付款序号,Sum(发票金额) 发票金额,付款标志  From 应付记录 " & _
                        "      Where 系统标识 = 1 And 记录性质 =0 And 入库单据号=[1] Group By 序号,项目ID,随货单号,发票号,发票代码,发票日期,付款序号,付款标志) Y " & _
                        "      WHERE X.序号 = Y.序号(+) And X.药品ID=Y.项目ID(+) AND X.NO=[1] AND 单据=1 " & _
                        "      GROUP BY X.NO,X.药品ID,X.序号,X.产地,X.批号,NVL(X.批次,0),X.效期,X.扣率,X.成本价,X.零售价,X.单量," & _
                        "      X.供药单位ID,X.库房ID,随货单号,发票号,发票代码,发票日期,NVL(Y.付款序号,0),Nvl(X.发药方式,0),X.生产日期,X.批准文号,发票金额,X.配药人,X.配药日期,X.频次,y.付款标志,x.计划id " & _
            ...
            Case 6      '冲销
                gstrSQL = "SELECT * FROM (SELECT DISTINCT A.药品ID,A.序号,'[' || D.编码 || ']' As 药品编码, D.名称 As 通用名, E.名称 As 商品名," & _
                    " B.药品来源,B.基本药物,D.规格,D.产地 AS 原产地,A.产地,A.批号," & _
                    " NVL(B.招标药品,0) 招标药品,NVL(B.差价让利比,0) 差价让利比,B.最大效期,A.效期," & strUnitQuantity & _
                    " nvl(A.单量,b.指导批发价)*" & num包装系数 & " AS 指导批发价 ,A.成本价*" & num包装系数 & " AS 采购价," & _
                    " A.成本金额 AS 采购金额,B.指导差价率/100 AS 指导差价率,D.是否变价,B.药房分批 药房分批核算,  " & _
                    " DECODE(A.扣率, NULL, 0, A.扣率) AS 扣率, A.零售价*" & num包装系数 & " AS 零售价 ,0 AS 零售金额,0 AS 差价,A.金额差, " & _
                    " A.批准文号,A.随货单号, A.发票号,a.发票代码,A.发票日期,A.发票金额,A.供药单位ID,F.名称 AS 供应商, A.库房ID,G.名称 AS 部门,NVL(A.付款序号,0) AS 付款序号,A.退货,A.生产日期,A.批次,A.配药人 As 核查人,A.配药日期 As 核查日期,B.药价级别,A.加成率,a.付款标志,a.计划id,a.外观 " & _
                    " FROM " & _
                    "     (SELECT MIN(X.ID) AS ID, SUM(实际数量) AS 填写数量,SUM(成本金额) AS 成本金额,随货单号,发票号,发票代码,发票日期,Sum(发票金额) As 发票金额," & _
                    "      X.药品ID,X.序号,X.产地, X.批号,X.效期,X.扣率,X.成本价,X.零售价,X.单量," & _
                    ...
                    "      FROM 药品收发记录 X,(SELECT 收发id,付款序号,随货单号,发票号,发票代码,发票日期,发票金额,付款标志 FROM 应付记录 WHERE 系统标识=1 AND 记录性质=0) Y " & _
                    "      WHERE X.ID=Y.收发ID(+) AND X.NO=[1] AND 单据=1 " & _
                    "      GROUP BY X.药品ID,X.序号,X.产地,X.批号,X.效期,X.扣率,X.成本价,X.零售价,X.单量," & _
                    "      X.供药单位ID,X.库房ID,随货单号,发票号,发票代码,发票日期,NVL(Y.付款序号,0),Nvl(X.发药方式,0),X.生产日期,X.批准文号,NVL(X.批次,0),X.配药人,X.配药日期,X.频次,付款标志,x.计划id,x.外观 " & _
                    "      HAVING SUM(实际数量)<>0 ) A," & _
                    "      药品规格 B,收费项目别名 E ,收费项目目录 D,供应商 F,部门表 G " & _
                    " WHERE A.药品ID = B.药品ID AND B.药品ID=D.ID AND A.库房ID=G.ID" & _
                    " AND B.药品ID=E.收费细目ID(+) AND E.性质(+)=3 " & _
            ...
            Case Else
                gstrSQL = "SELECT * FROM (SELECT DISTINCT A.药品ID,A.序号,'[' || D.编码 || ']' As 药品编码, D.名称 As 通用名, E.名称 As 商品名," & _
                    ...
                    " A.成本金额 AS 采购金额,B.指导差价率/100 AS 指导差价率,D.是否变价,B.药房分批 药房分批核算," & _
                    " DECODE(A.扣率, NULL, 0, A.扣率) AS 扣率, A.零售价*" & num包装系数 & " AS 零售价,A.零售金额,A.差价, " & _
                    " A.批准文号,C.随货单号, C.发票号 ,c.发票代码, C.发票日期, C.发票金额,A.供药单位ID,F.名称 AS 供应商, A.摘要,A.填制人,A.填制日期," & _
                    " A.审核人,A.审核日期,A.库房ID,G.名称 AS 部门,NVL(C.付款序号,0) AS 付款序号,Nvl(A.发药方式,0) 退货,A.外观,A.产品合格证,A.验收结论," & _
            ...
            If mint编辑状态 = 2 Or mint编辑状态 = 3 Or mint编辑状态 = 7 Then
            ...
            If mbln退货 And mint编辑状态 = 2 Then chk不明确批次.Visible = True '修改退库单可见
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    If mbln退货 Then
                        .TextMatrix(intRow, mconIntCol数量) = GetFormat(rsInitCard!数量 * IIf(mint编辑状态 = 6, 1, -1), intNumberDigit)
                        .TextMatrix(intRow, mconIntCol结算金额) = GetFormat(rsInitCard!采购金额 * IIf(mint编辑状态 = 6, 1, -1), intMoneyDigit)
                        .TextMatrix(intRow, mconIntCol售价金额) = GetFormat(rsInitCard!零售金额 * IIf(mint编辑状态 = 6, 1, -1), intMoneyDigit)
                        .TextMatrix(intRow, mconintCol差价) = GetFormat(rsInitCard!差价 * IIf(mint编辑状态 = 6, 1, -1), intMoneyDigit)
                        .TextMatrix(intRow, mconintcol发票金额) = IIf(GetFormat(Nvl(rsInitCard!发票金额, 0) * IIf(mint编辑状态 = 6, 1, -1), intMoneyDigit) = "0.00", "", GetFormat(Nvl(rsInitCard!发票金额, 0) * IIf(mint编辑状态 = 6, 1, -1), intMoneyDigit))
                    Else
                        .TextMatrix(intRow, mconIntCol数量) = GetFormat(rsInitCard!数量, intNumberDigit)
                        .TextMatrix(intRow, mconIntCol结算金额) = GetFormat(IIf(mint编辑状态 = 6, 0, rsInitCard!采购金额), intMoneyDigit)
                        ...
                        .TextMatrix(intRow, mconintcol发票金额) = IIf(GetFormat(IIf(IsNull(rsInitCard!发票金额), "0", rsInitCard!发票金额), intMoneyDigit) = "0.00", "", GetFormat(IIf(IsNull(rsInitCard!发票金额), "0", rsInitCard!发票金额), intMoneyDigit))
                    End If
                    .TextMatrix(intRow, mconIntCol结算价) = GetFormat(rsInitCard!采购价, intCostDigit)
                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!零售价, intPriceDigit)
                    .TextMatrix(intRow, mconIntCol扣率) = IIf(mintFixPrice = 0, rsInitCard!扣率, GetFormat(rsInitCard!扣率, 7))
                    ...
                    .TextMatrix(intRow, mconIntCol指导批发价) = GetFormat(rsInitCard!指导批发价, intCostDigit)
                    ...
                    If .TextMatrix(intRow, mconIntCol原销期) <> "" Then
                        If Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2)) = 1 And Val(.TextMatrix(intRow, mconIntCol分批属性)) = 1 Then
                            ...
                            If mbln退货 Then
                                .TextMatrix(intRow, mconintCol零售金额) = GetFormat(-1 * rsInitCard!零售金额, intMoneyDigit)
                                .TextMatrix(intRow, mconintCol零售差价) = GetFormat(-1 * rsInitCard!差价, intMoneyDigit)
                            Else
                            ...
                            If mint编辑状态 <> 6 Then   '不是冲销时
                                If mbln退货 Then
                                    .TextMatrix(intRow, mconIntCol售价金额) = GetFormat(-1 * (rsInitCard!零售金额 - rsInitCard!金额差), intMoneyDigit)
                                    .TextMatrix(intRow, mconintCol差价) = GetFormat(-1 * (rsInitCard!差价 - rsInitCard!金额差), intMoneyDigit)
                                Else
                                ...
                                If Val(.TextMatrix(intRow, mconIntCol数量)) <> 0 Then
                                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(Val(.TextMatrix(intRow, mconIntCol售价金额)) / Val(.TextMatrix(intRow, mconIntCol数量)), intPriceDigit)
                                Else
                                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsInitCard!零售价, intPriceDigit)
                                End If
                            Else
                    ...
                    .TextMatrix(intRow, mconIntCol采购价) = GetFormat(Val(.TextMatrix(intRow, mconIntCol结算价)) * 100 / IIf(Val(.TextMatrix(intRow, mconIntCol扣率)) = 0, 1, Val(.TextMatrix(intRow, mconIntCol扣率))), intCostDigit)
                    ...
                    '根据参数决定时价药品售价公式中成本价的算法
                    dbl成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(intRow, mconIntCol结算价)), Val(.TextMatrix(intRow, mconIntCol采购价)))
                    
                    '计算加成率
                    If Val(.TextMatrix(intRow, mconIntCol售价)) <> 0 And dbl成本价 <> 0 Then
                        If IIf(IsNull(rsInitCard!加成率), "", rsInitCard!加成率) <> "" Then
                            .TextMatrix(intRow, mconIntCol加成率) = FormatEx(Val(rsInitCard!加成率) * 100, 2) & "%"
                        Else
                            .TextMatrix(intRow, mconIntCol加成率) = FormatEx((Val(.TextMatrix(intRow, mconIntCol售价)) / dbl成本价 - 1) * 100, 2) & "%"
                        End If
                    End If
...
Private Sub SetEdit()
    ...
    With mshBill
        If mblnEdit = False Then
            ...
            Next
            If mint编辑状态 = 3 Then
                .ColData(mconIntCol结算价) = IIf(InStr(1, mstrControlItem, ",结算价,") > 0, 4, 5)
                ...
                .ColData(mconIntCol结算金额) = IIf(InStr(1, mstrControlItem, ",结算金额,") > 0, 4, 5)
...
'                .LocateCol = mconIntCol结算价
            ...
            ElseIf mint编辑状态 = 8 Or mbln退货 Then
                ...
                '.ColData(mconIntCol结算价) = 5
                .ColData(mconIntCol结算金额) = 5
                If mbln退货 Then
                    txtProvider.Enabled = False
                    cmdProvider.Enabled = False
                End If
            ...
            If mintFixPrice = 0 Then
                .ColData(mconIntCol结算价) = 4
                .ColData(mconIntCol采购价) = 4
                .ColData(mconIntCol结算金额) = 4
            ...
            .ColAlignment(mconIntCol结算价) = flexAlignRightCenter
            .ColAlignment(mconIntCol结算金额) = flexAlignRightCenter
...
Private Sub initGrid()
    ...
    With mshBill
        ...
        .TextMatrix(0, mconIntCol结算价) = "结算价"
        .TextMatrix(0, mconIntCol结算金额) = "结算金额"
        ...
        .ColWidth(mconIntCol结算价) = 1000
        .ColWidth(mconIntCol结算金额) = 900
...
Private Sub Form_Resize()
    ...
    With lbl核查日期
        .Top = Lbl填制日期.Top
        .Left = mshBill.Left + (mshBill.Width - .Width - txt核查日期.Width - 100) / 2
    End With
    
    With txt核查日期
        .Top = Txt填制日期.Top
        .Left = lbl核查日期.Left + lbl核查日期.Width + 100
    End With
    ...
    If chk不明确批次.Visible Then
        Me.chk不明确批次.Top = txtCode.Top + 45
    End If
        ...
        If mbln退货 = True Then
            For n = 1 To .rows - 1
                If Val(.TextMatrix(n, 0)) <> 0 Then
                    dbl合计数量 = 0
                    lng药品ID = Val(.TextMatrix(n, 0))
                    lng批次 = Val(.TextMatrix(n, mconIntCol批次))
                    For m = 1 To .rows - 1
                        If Val(.TextMatrix(m, 0)) <> 0 And lng药品ID = Val(.TextMatrix(m, 0)) And lng批次 = Val(.TextMatrix(m, mconIntCol批次)) Then
                            dbl合计数量 = dbl合计数量 + Val(.TextMatrix(m, mconIntCol数量)) * Val(.TextMatrix(m, mconIntCol比例系数))
                        End If
                    Next
                    
                    If Not Check退库(lng药品ID, lng批次, dbl合计数量, n) Then
                        Exit Function
                    End If
                End If
            Next
...
'        Set RecReturn = Frm药品选择器.ShowME(Me, IIf(mint编辑状态 = 8 Or mbln退货, 2, 1), cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), , True, True, False, False, True, IIf(mint编辑状态 = 8 Or mbln退货, Val(txtProvider.Tag), 0))
        ...
        '保存注册表信息(是否显示停用药品)
        SaveSetting "ZLSOFT", "私有模块\" & gstrDbUser & "\" & App.ProductName & "\药品外购入库管理", "不明确批次填单", chk不明确批次.Value
            ...
            For i = 1 To RecReturn.RecordCount
                With mshBill
                    ...
                    SetColValue .Row, RecReturn!药品id, "[" & RecReturn!药品编码 & "]", RecReturn!通用名, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), _
                        ...
                        RecReturn!药房分批, RecReturn!指导差价率 / 100, IIf(IsNull(RecReturn!生产日期), "", Format(RecReturn!生产日期, "yyyy-mm-dd")), _
                        RecReturn!售价单位
...
Private Sub mshBill_EditKeyPress(KeyAscii As Integer)
    ...
    With mshBill
        If .Col = mconIntCol数量 Or .Col = mconIntCol冲销数量 Or .Col = mconIntCol采购价 Or .Col = mconIntCol结算价 Or .Col = mconIntCol结算金额 Or .Col = mconintcol发票金额 Or .Col = mconIntCol售价 Or .Col = mconintCol零售价 Then
            strkey = .Text
            If strkey = "" Then
            ...
            Select Case .Col
                ...
                Case mconIntCol采购价, mconIntCol结算价
                   intDigit = mintCostDigit
                Case mconIntCol结算金额, mconintcol发票金额
    ...
    With mshBill
        ...
        If .Row <> .LastRow Then
            ...
            If PicInput.Visible Then
                '重新计算零售价、差价
                dbl售价 = Val(.TextMatrix(lngRow, mconIntCol结算价)) * (1 + (Val(Txt加价率) / 100))
                .TextMatrix(lngRow, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(lngRow, 0)), Val(.TextMatrix(lngRow, mconIntCol结算价)), Val(Txt加价率) / 100, dbl售价, lngRow), mintPriceDigit)
                ...
                .TextMatrix(lngRow, mconintCol差价) = GetFormat(IIf(.TextMatrix(lngRow, mconIntCol售价金额) = "", 0, .TextMatrix(lngRow, mconIntCol售价金额)) - IIf(.TextMatrix(lngRow, mconIntCol结算金额) = "", 0, .TextMatrix(lngRow, mconIntCol结算金额)), mintMoneyDigit)
                PicInput.Visible = False
            End If
        End If
        SetInputFormat .Row
        ...
        Select Case .Col
            ...
            Case mconIntCol扣率
                ...
                If mint编辑状态 = 7 Then
                    Call SetState
                End If
            Case mconIntCol结算价, mconIntCol指导批发价, mconIntCol采购价, mconintCol零售价
                .TxtCheck = True
                .MaxLength = 11
                .TextMask = ".1234567890"
                If mint编辑状态 = 7 Then
                    ...
                    If RecReturn.RecordCount > 0 Then
                        ...
                        For i = 1 To RecReturn.RecordCount
                            ...
                            If SetColValue(.Row, RecReturn!药品id, "[" & RecReturn!药品编码 & "]", RecReturn!通用名, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), _
                                 ...
                                 IIf(IsNull(RecReturn!产地), "!", RecReturn!产地), RecReturn!最大效期, "", _
                                 Choose(mintUnit, 1, RecReturn!门诊包装, RecReturn!住院包装, RecReturn!药库包装), IIf(IsNull(RecReturn!批次), 0, RecReturn!批次), RecReturn!时价, _
                                 RecReturn!药房分批, RecReturn!指导差价率 / 100, IIf(IsNull(RecReturn!生产日期), "", Format(RecReturn!生产日期, "yyyy-mm-dd")), RecReturn!售价单位) = False Then ' RecReturn!简码
                                 Cancel = True
                                 Exit Sub
                             End If
...
'                        .Col = mconIntCol批号
                ...
                Else
                    ...
                    gstrSQL = "Select 编码,简码,名称 From 药品生产商 " _
                            & "Where (站点 = [3] Or 站点 is Null) And (upper(名称) like [1] or Upper(编码) like [1] or Upper(简码) like [2] Or zlSpellCode(名称) Like [2] Or zlWbCode(名称) Like [2]) " _
                            & "Order By 编码 "
                    ...
                    If rs产地.EOF Then
                        ...
                        Else
                            ...
                            Set rsMax = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "-药品生产商编码长度")
                            int编码 = rsMax!length
                            
                            gstrSQL = "SELECT Nvl(MAX(LPAD(编码," & int编码 & ",'0')),'00') As Code FROM 药品生产商"
                            Set rsMax = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "-药品生产商编码")
                            strCode = rsMax!Code
            ...
            Case mconIntCol采购价
                ...
                '计算结算价：结算价=采购价*扣率
                .TextMatrix(.Row, mconIntCol结算价) = GetFormat(Val(.TextMatrix(.Row, mconIntCol采购价)) * Val(.TextMatrix(.Row, mconIntCol扣率)) / 100, mintCostDigit)
...
'                                Txt加价率 = GetFormat(计算加成率(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mconIntCol售价)), dbl成本价), 5)
'                            End If
                            Txt加价率.Tag = Txt加价率
                            Txt加价率.SetFocus
                        Else
                            ...
                            End If
                            If mint时价入库时取上次售价 <> 1 Then  '没有勾选时价取上次售价参数
                                .TextMatrix(.Row, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), dbl成本价, dbl加成率, dblTemp售价), mintPriceDigit)
                            End If
                            
                            .TextMatrix(.Row, mconIntCol加成率) = FormatEx(dbl加成率 * 100, 2) & "%"
                    ...
                    Else
                        '定价药品计算加成率，无实际意义，仅显示
                        If Val(.TextMatrix(.Row, mconIntCol售价)) <> 0 And dbl成本价 <> 0 Then
                            .TextMatrix(.Row, mconIntCol加成率) = FormatEx((Val(.TextMatrix(.Row, mconIntCol售价)) / dbl成本价 - 1) * 100, 2) & "%"
                        End If
                    End If
                End If
                ...
                If strkey <> "" And .TextMatrix(.Row, mconIntCol数量) <> "" Then
                    .TextMatrix(.Row, mconIntCol结算金额) = GetFormat(.TextMatrix(.Row, mconIntCol数量) * Val(.TextMatrix(.Row, mconIntCol结算价)), mintMoneyDigit)
                    .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(.TextMatrix(.Row, mconIntCol数量) * Val(.TextMatrix(.Row, mconIntCol售价)), mintMoneyDigit)
                    .TextMatrix(.Row, mconintcol发票金额) = IIf(Trim(.TextMatrix(.Row, mconintcol发票号)) = "", "", .TextMatrix(.Row, mconIntCol结算金额))
                    .TextMatrix(.Row, mconintCol差价) = GetFormat(IIf(.TextMatrix(.Row, mconIntCol售价金额) = "", 0, .TextMatrix(.Row, mconIntCol售价金额)) - IIf(.TextMatrix(.Row, mconIntCol结算金额) = "", 0, .TextMatrix(.Row, mconIntCol结算金额)), mintMoneyDigit)
                End If
            ...
            Case mconIntCol结算价
                ...
                If Not IsNumeric(strkey) And strkey <> "" Then
                    MsgBox "对不起，结算价必须为数字型,请重输！", vbInformation + vbOKOnly, gstrSysName
                    Cancel = True
                    .TxtSetFocus
                    Exit Sub
                End If
                ...
                If strkey <> "" Then
                    If Val(strkey) < 0 Then
                        MsgBox "对不起，结算价不能为负数,请重输！", vbInformation + vbOKOnly, gstrSysName
                        Cancel = True
                        .TxtSetFocus
                        Exit Sub
                    End If
                ...
                If strkey <> "" Then
                    strkey = GetFormat(strkey, mintCostDigit)
                    .Text = strkey
                    .TextMatrix(.Row, mconIntCol结算价) = .Text
                End If
                
                If Val(strkey) > Val(.TextMatrix(.Row, mconIntCol指导批发价)) Then
                    MsgBox "你输入的结算价大于了采购限价。", vbInformation + vbOKOnly, gstrSysName
                End If
                ...
                If Val(.TextMatrix(.Row, mconIntCol采购价)) <> 0 Then
                    .TextMatrix(.Row, mconIntCol扣率) = GetFormat(Val(.TextMatrix(.Row, mconIntCol结算价)) / Val(.TextMatrix(.Row, mconIntCol采购价)) * 100, 7)
                Else
                    .TextMatrix(.Row, mconIntCol扣率) = "100"
                End If
                ...
                If strkey <> "" And .TextMatrix(.Row, mconIntCol原销期) <> "" And mint编辑状态 <> 8 And mbln退货 = False Then
                    ...
                    If Split(.TextMatrix(.Row, mconIntCol原销期), "||")(2) = 1 Then
                        ...
                        If mbln加价率 And mint时价入库售价加成方式 = 0 Then
                            mbln允许手工输入加成率 = True
                            If mint时价入库时取上次售价 <> 1 Then  '如果未勾选取上次售价，且勾选了手工录入加成率参数则弹出加成率框，让用户选择
                                sngLeft = Pic单据.Left + mshBill.Left + mshBill.MsfObj.CellLeft + Screen.TwipsPerPixelX
                                sngTop = Pic单据.Top + mshBill.Top + mshBill.MsfObj.CellTop + mshBill.MsfObj.CellHeight  '  50
                                If sngTop + 1700 > Screen.Height Then
                                    sngTop = sngTop - mshBill.MsfObj.CellHeight - 1700
...
'                End If
                ...
                dbl成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(.Row, mconIntCol结算价)), Val(.TextMatrix(.Row, mconIntCol采购价)))
                .TextMatrix(.Row, mconIntCol加成率) = FormatEx(计算加成率(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, .Col)), dbl成本价), 2) & "%"
                '重算差价
                .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol售价)) * Val(.TextMatrix(.Row, mconIntCol数量)), mintMoneyDigit)
                .TextMatrix(.Row, mconintCol差价) = GetFormat(Val(.TextMatrix(.Row, mconIntCol售价金额)) - Val(.TextMatrix(.Row, mconIntCol结算金额)), mintMoneyDigit)
            ...
            Case mconintCol零售价
                ...
                If Val(.TextMatrix(.Row, mconIntCol结算价)) = Val(.TextMatrix(.Row, mconIntCol售价)) Then
                '通过技术手段单独处理零差价销售情况下零售价和售价不等的情况
                    .TextMatrix(.Row, mconIntCol售价金额) = GetFormat(Val(.TextMatrix(.Row, mconIntCol售价)) * Val(.TextMatrix(.Row, mconIntCol数量)), mintMoneyDigit)
                Else
                ...
                dbl成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(.Row, mconIntCol结算价)), Val(.TextMatrix(.Row, mconIntCol采购价)))
                .TextMatrix(.Row, mconIntCol加成率) = FormatEx(计算加成率(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mconIntCol售价)), dbl成本价), 2) & "%"
                
                Call Set时价分批药品零售价(.Row, Val(.Text))
                Call 显示合计金额
            Case mconIntCol结算金额
                If Not IsNumeric(strkey) And strkey <> "" Then
                    MsgBox "对不起，结算金额必须为数字型,请重输！", vbInformation + vbOKOnly, gstrSysName
                    Cancel = True
                    .TxtSetFocus
                    Exit Sub
                End If
                ...
                If strkey <> "" Then
                    ...
                    If Val(strkey) * Val(.TextMatrix(.Row, mconIntCol数量)) < 0 Then
                        MsgBox "结算金额符号应与数量符号一致！", vbInformation + vbOKOnly, gstrSysName
                        Cancel = True
                        .TxtSetFocus
                        Exit Sub
                    End If
                ...
                If strkey <> "" And strkey <> .TextMatrix(.Row, mconIntCol结算金额) Then
                    If .TextMatrix(.Row, mconIntCol数量) <> "" Then
                        If mbln加价率 Then
                            '取得改变采购金额前的加价率
                            dbl成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(.Row, mconIntCol结算价)), Val(.TextMatrix(.Row, mconIntCol采购价)))
                        ...
                        '计算结算价、采购价：结算价=采购金额/数量;采购价=(采购金额/数量)/扣率
                        If Val(.TextMatrix(.Row, mconIntCol扣率)) = 0 Then
                            .TextMatrix(.Row, mconIntCol扣率) = "100"
                        End If
                        .TextMatrix(.Row, mconIntCol结算价) = GetFormat(strkey / .TextMatrix(.Row, mconIntCol数量), mintCostDigit)
                        .TextMatrix(.Row, mconIntCol采购价) = GetFormat((strkey / .TextMatrix(.Row, mconIntCol数量)) * 100 / Val(.TextMatrix(.Row, mconIntCol扣率)), mintCostDigit)
                    ...
                    If Val(.TextMatrix(.Row, mconIntCol数量)) <> 0 Then
                        .TextMatrix(.Row, mconIntCol结算价) = GetFormat(strkey / Val(.TextMatrix(.Row, mconIntCol数量)), mintCostDigit)
                        .TextMatrix(.Row, mconIntCol采购价) = GetFormat(strkey / Val(.TextMatrix(.Row, mconIntCol数量)) * 100 / Val(.TextMatrix(.Row, mconIntCol扣率)), mintCostDigit)
                    End If
                    .TextMatrix(.Row, mconintcol发票金额) = IIf(Trim(.TextMatrix(.Row, mconintcol发票号)) = "", "", GetFormat(strkey, mintMoneyDigit))
                    ...
                    .TextMatrix(.Row, mconIntCol结算金额) = GetFormat(strkey, mintMoneyDigit)
                End If
            ...
            Case mconIntCol数量
                ...
                If strkey <> "" Then
                    ...
                    If mint编辑状态 = 8 Or mbln退货 Or Val(strkey) < 0 Then
                        If Get分批属性(Val(cboStock.ItemData(cboStock.ListIndex)), Val(.TextMatrix(.Row, 0))) = 1 And Val(.TextMatrix(.Row, mconIntCol批次)) = 0 Then '对应库房分批单未明确批次
                        Else
                            If Not CheckStock(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mconIntCol批次)), Val(.Text) * Val(.TextMatrix(.Row, mconIntCol比例系数))) Then
                                Cancel = True
                                .TxtSetFocus
                                Exit Sub
                            End If
...
'                        Exit Sub
'                    End If
                    If .TextMatrix(.Row, mconIntCol结算价) <> "" Then
                        .TextMatrix(.Row, mconIntCol结算金额) = GetFormat(.TextMatrix(.Row, mconIntCol结算价) * strkey, mintMoneyDigit)
...
'                                    mdbl加价率 = mdbl加价率 / 100
                                    ...
                                    If mint时价入库时取上次售价 <> 1 Then  '没有勾选时价取上次售价参数
                                        .TextMatrix(.Row, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), dbl成本价, mdbl加价率, dblTemp售价), mintPriceDigit)
                                    End If
                    ...
                    If mint退货发票金额方式 = 0 Then
                        .TextMatrix(.Row, mconintcol发票金额) = .TextMatrix(.Row, mconIntCol结算金额)
                    Else
                        If mint编辑状态 = 8 Or (mint编辑状态 = 2 And mbln退货 = True) Then
                            .TextMatrix(.Row, mconintcol发票金额) = .TextMatrix(.Row, mconIntCol售价金额)
                        Else
                            .TextMatrix(.Row, mconintcol发票金额) = .TextMatrix(.Row, mconIntCol结算金额)
                        End If
                    End If
            ...
            Case mconIntCol冲销数量
                ...
                If strkey <> "" Then
                    ...
                    If rsDrug.EOF Then
                        .TextMatrix(.Row, mconintcol发票金额) = GetFormat(0, mintMoneyDigit)
                    Else
                        .TextMatrix(.Row, mconintcol发票金额) = GetFormat(strkey / .TextMatrix(.Row, mconIntCol数量) * rsDrug.Fields(0), mintMoneyDigit)
                    End If
            ...
            Case mconIntCol验收结论
                ...
                Else
                    ...
                    gstrSQL = "Select 编码,名称 From 入库验收结论 " _
                            & "Where upper(名称) like [1] or Upper(编码) like [1] "
                    Set rs验收结论 = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption, IIf(gstrMatchMethod = "0", "%", "") & strkey & "%")
                    ...
                    If rs验收结论.EOF Then
                        ...
                        Exit Sub
                    Else
                        If rs验收结论.RecordCount = 1 Then
                            .TextMatrix(.Row, mconIntCol验收结论) = rs验收结论.Fields("名称")
                            .Text = rs验收结论.Fields("名称")
                            .Col = GetNextEnableCol(mconIntCol验收结论)
                        Else
                            Set msh产地.Recordset = rs验收结论
                            With msh产地
                                .Redraw = False
                                .Left = Pic单据.Left + mshBill.Left + mshBill.MsfObj.CellLeft + Screen.TwipsPerPixelX
                                .Top = Pic单据.Top + mshBill.Top + mshBill.MsfObj.CellTop + mshBill.MsfObj.CellHeight
    ...
    With mshBill
        ...
        Do Until rsData.EOF
            ...
            If intRow = 0 Then
                ...
                intRow = .rows - 1
            End If
            bln招标药品 = (rsData!招标药品 = 1)
            
            .TextMatrix(intRow, 0) = rsData!药品id
            .TextMatrix(intRow, mconIntCol批次) = IIf(IsNull(rsData!批次), "", rsData!批次)
            
            .TextMatrix(intRow, mconIntCol药品编码和名称) = rsData!编码名称
            .TextMatrix(intRow, mconIntCol药品编码) = Mid(rsData!编码名称, 2, InStr(2, rsData!编码名称, "]") - 2)
            .TextMatrix(intRow, mconIntCol药品名称) = Mid(rsData!编码名称, InStr(2, rsData!编码名称, "]") + 1)
            ...
            .TextMatrix(intRow, mconIntCol商品名) = Mid(rsData!编码名称, InStr(2, rsData!编码名称, "]") + 1)
            .TextMatrix(intRow, mconIntCol来源) = IIf(IsNull(rsData!药品来源), "", rsData!药品来源)
            .TextMatrix(intRow, mconIntCol基本药物) = IIf(IsNull(rsData!基本药物), "", rsData!基本药物)
            .TextMatrix(intRow, mconIntCol药价级别) = IIf(IsNull(rsData!药价级别), "", rsData!药价级别)
            ...
            .TextMatrix(intRow, mconIntCol单位) = rsData!单位
            .TextMatrix(intRow, mconIntCol指导批发价) = GetFormat(rsData!指导批发价, mintCostDigit)
            .TextMatrix(intRow, mconIntCol原产地) = IIf(IsNull(rsData!产地), "", rsData!产地)
            .TextMatrix(intRow, mconIntCol批准文号) = IIf(IsNull(rsData!批准文号), "", rsData!批准文号)
            .TextMatrix(intRow, mconIntCol生产日期) = IIf(IsNull(rsData!生产日期), "", rsData!生产日期)
            .TextMatrix(intRow, mconIntCol批号) = IIf(IsNull(rsData!批号), "", rsData!批号)
            .TextMatrix(intRow, mconIntCol效期) = IIf(IsNull(rsData!效期), "", rsData!效期)
            .TextMatrix(intRow, mconIntCol扣率) = 100
            ...
            .TextMatrix(intRow, mconIntCol采购价) = GetFormat(rsData!成本价, mintCostDigit)
            .TextMatrix(intRow, mconIntCol结算价) = GetFormat(rsData!成本价, mintCostDigit)
            '原效期字段下面保存原效期，指导差价，是否变价，药房分批等，格式为：原效期||指导差价率||是否变价||药房分批
            .TextMatrix(intRow, mconIntCol原销期) = IIf(IsNull(rsData!最大效期), "0", rsData!最大效期) & "||" & rsData!加成率 & "||" & rsData!是否变价 & "||" & rsData!药房分批
            .TextMatrix(intRow, mconintcol简码) = ""
            .TextMatrix(intRow, mconIntCol比例系数) = rsData!比例系数
            .TextMatrix(intRow, mconintcol随货单号) = IIf(IsNull(rsData!随货单号), "", rsData!随货单号)
            .TextMatrix(intRow, mconintcol发票号) = IIf(IsNull(rsData!发票号), "", rsData!发票号)
            .TextMatrix(intRow, mconintcol发票代码) = IIf(IsNull(rsData!发票代码), "", rsData!发票代码)
            .TextMatrix(intRow, mconIntCol发票日期) = IIf(IsNull(rsData!发票日期), "", rsData!发票日期)
            
            SetInputFormat intRow
            
            dbl加成率 = IIf(IsNull(rsData!加成率), 0, rsData!加成率)
            
            If rsData!是否变价 = 1 Then
                gstrSQL = " Select Decode(Nvl(批次,0),0,nvl(实际金额,0)/Nvl(实际数量,0),Nvl(零售价,nvl(实际金额,0)/Nvl(实际数量,1))) 售价 From 药品库存" & _
                          " Where 库房ID=[1] And 药品ID=[2] And 性质=1 And NVL(批次,0)=[3]"
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[按出库的形式计算售价]", cboStock.ItemData(cboStock.ListIndex), rsData!药品id, Val(.TextMatrix(intRow, mconIntCol批次)))
            Else
                gstrSQL = "Select 现价 as 售价 From 收费价目 Where 收费细目id = [1] And Sysdate Between 执行日期 And 终止日期"
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[按出库的形式计算售价]", rsData!药品id)
            End If

            .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsTemp!售价 * rsData!比例系数, mintPriceDigit)
            
            If Val(.TextMatrix(intRow, mconIntCol售价)) <> 0 And rsData!成本价 <> 0 Then
                .TextMatrix(intRow, mconIntCol加成率) = FormatEx((Val(.TextMatrix(intRow, mconIntCol售价)) / rsData!成本价 - 1) * 100, 2) & "%"
            End If
            .TextMatrix(intRow, mconintCol差价) = GetFormat(rsData!差价, mintMoneyDigit)
            
            '定价药品显示加成率，无实际意义，仅显示
            If Val(.TextMatrix(intRow, mconIntCol售价)) <> 0 And rsData!成本价 <> 0 Then
                .TextMatrix(intRow, mconIntCol加成率) = FormatEx((Val(.TextMatrix(intRow, mconIntCol售价)) / rsData!成本价 - 1) * 100, 2) & "%"
            Else
                .TextMatrix(intRow, mconIntCol加成率) = FormatEx(dbl加成率, 2) & "%"
            End If
            
            .TextMatrix(intRow, mconIntCol外观) = rsData!外观
            ...
            If .TextMatrix(intRow, mconIntCol原销期) <> "" Then
                If mintUnit <> mconint售价单位 And Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2)) = 1 And Val(.TextMatrix(intRow, mconIntCol分批属性)) = 1 Then
                    .TextMatrix(intRow, mconintCol零售单位) = rsData!计算单位
                End If
            End If
            ...
            For i = mconIntCol药名 To .Cols - 1
                ...
                .Col = i
                If bln招标药品 Then
                    mshBill.MsfObj.CellForeColor = IIf(rsData!差价让利比 = 0, &H800000, &H800080)
                Else
                    mshBill.MsfObj.CellForeColor = IIf(rsData!差价让利比 = 0, &H0, &H40&)      ' &H40C0&
                End If
                .ColData(i) = j
            Next
            .Col = intCol
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng药品ID As Long, ByVal str药品编码 As String, ByVal str通用名 As String, _
    ...
    ByVal int是否变价 As Integer, ByVal int药房分批 As Integer, ByVal dbl指导差价率 As Double, ByVal str生产日期 As String, _
    ByVal str售价单位 As String) As Boolean
    ...
    Dim rsTemp As New ADODB.Recordset
    Dim i As Long, j As Long
    Dim dbl时价成本价 As Double, dbl加成率 As Double
    Dim str药名 As String
...
'    End If
    ...
    With mshBill
        ...
        gstrSQL = "SELECT Nvl(a.差价让利比,0) 差价让利比,nvl(a.扣率,0) 扣率,Nvl(a.招标药品,0) 招标药品,nvl(a.成本价,0) 成本价, a.批准文号,a.上次批准文号,a.上次产地,b.产地,a.上次生产日期,a.药价级别 " & _
        ...
        .TextMatrix(intRow, mconIntCol售价) = GetFormat(num售价, mintPriceDigit)
        .TextMatrix(intRow, mconIntCol指导批发价) = GetFormat(num指导批发价, mintCostDigit)
        ...
        '取出该药品的批号及效期，以及原采购价
        If mint编辑状态 = 8 Or mbln退货 Then
            gstrSQL = " Select 上次批号 批号,效期,上次生产日期,上次产地,批准文号,上次采购价 From 药品库存" & _
            ...
            If rsPrice.RecordCount <> 0 Then
                ...
                If dbl成本价 > 0 Then
                    .TextMatrix(intRow, mconIntCol采购价) = GetFormat(dbl成本价 * num比例系数, mintCostDigit)
                    .TextMatrix(intRow, mconIntCol结算价) = GetFormat(dbl成本价 * num比例系数 * dblRate / 100, mintCostDigit)
                End If
            End If
        End If
        
        '原效期字段下面保存原效期，指导差价，是否变价，药房分批等，格式为：原效期||加成率||是否变价||药房分批
        ...
        If Not (mint编辑状态 = 8 Or mbln退货) Then
            If mint取上次采购价方式 = 0 Then  ' mintFixPrice = 0 And mint取上次采购价方式 = 0
       
                If Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(0)) > 0 Then
                    If mintUnit = mconint药库单位 Then
                        gstrSQL = "select 上次采购价,上次产地,批准文号,上次生产日期 from 药品库存 where 性质=1 and 库房id=[1] and 药品id=[2] " & _
                            " and nvl(批次,0) =(select max(nvl(批次,0)) from 药品库存 where 性质=1 and 库房id=[1] " & _
                                                                                      " and 药品id=[2] )"
                    Else
                ...
                If Not rsPrice.EOF Then
                    ...
                    If Nvl(rsPrice.Fields(0), 0) = 0 Then
                        ...
                        End If
                    Else
                        .TextMatrix(intRow, mconIntCol采购价) = GetFormat(rsPrice.Fields(0) * num比例系数, mintCostDigit)
                        .TextMatrix(intRow, mconIntCol结算价) = GetFormat(rsPrice.Fields(0) * num比例系数 * dblRate / 100, mintCostDigit)
                    End If
                ...
                If Val(.TextMatrix(intRow, mconIntCol采购价)) <> 0 Then
                    .TextMatrix(intRow, mconIntCol扣率) = GetFormat(Val(.TextMatrix(intRow, mconIntCol结算价)) / Val(.TextMatrix(intRow, mconIntCol采购价)) * 100, 7)
                End If
            Else
                If dbl成本价 > 0 Then
                    .TextMatrix(intRow, mconIntCol采购价) = GetFormat(dbl成本价 * num比例系数, mintCostDigit)
        ...
        '根据参数决定时价药品售价公式中成本价的算法
        dbl时价成本价 = IIf(mint时价入库售价加成方式 = 0, Val(.TextMatrix(intRow, mconIntCol结算价)), Val(.TextMatrix(intRow, mconIntCol采购价)))
        ...
        If int是否变价 = 1 Then
            If mint编辑状态 <> 8 And mbln退货 = False Then
                ...
                If mint时价入库时取上次售价 = 1 Then
                    ...
                    If rs售价!上次售价 > 0 Then
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(rs售价!上次售价 * mlng包装系数, mintPriceDigit)
                    Else
                        .TextMatrix(intRow, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(intRow, 0)), dbl时价成本价, dbl加成率, dblTemp售价), mintPriceDigit)
                    End If
                Else
                    .TextMatrix(intRow, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(intRow, 0)), dbl时价成本价, dbl加成率, dblTemp售价), mintPriceDigit)
                End If
                .TextMatrix(intRow, mconIntCol加成率) = FormatEx(dbl加成率 * 100, 2) & "%"
            Else
                '按出库的形式计算售价（退货肯定是有库存）
                ...
                .TextMatrix(intRow, mconIntCol售价) = GetFormat(rsTemp!售价 * num比例系数, mintPriceDigit)
                
                If Val(.TextMatrix(intRow, mconIntCol售价)) <> 0 And dbl时价成本价 <> 0 Then
                    .TextMatrix(intRow, mconIntCol加成率) = FormatEx((Val(.TextMatrix(intRow, mconIntCol售价)) / dbl时价成本价 - 1) * 100, 2) & "%"
                End If
            End If
        Else
            '定价药品显示加成率，无实际意义，仅显示
...
Private Sub SetDisCount(ByVal intRow As Integer, ByVal intDisCount As Double)
    ...
    With mshBill
        ...
        Else
            .TextMatrix(intRow, mconIntCol扣率) = IIf(mintFixPrice = 0, intDisCount, GetFormat(intDisCount, 7))
        End If
...
'            End If
            If Not (mint编辑状态 = 8 Or mbln退货) Then
                .TextMatrix(intRow, mconIntCol结算价) = GetFormat((Val(.TextMatrix(intRow, mconIntCol采购价)) * intDisCount / 100), mintCostDigit)
            End If
            If .TextMatrix(intRow, mconIntCol数量) <> "" Then
               .TextMatrix(intRow, mconIntCol结算金额) = GetFormat((.TextMatrix(intRow, mconIntCol数量) * .TextMatrix(intRow, mconIntCol结算价)), mintMoneyDigit)
               .TextMatrix(intRow, mconintcol发票金额) = IIf(Trim(.TextMatrix(intRow, mconintcol发票号)) = "", "", .TextMatrix(intRow, mconIntCol结算金额))
            End If
            .TextMatrix(intRow, mconintCol差价) = GetFormat(IIf(.TextMatrix(intRow, mconIntCol售价金额) = "", 0, .TextMatrix(intRow, mconIntCol售价金额)) - IIf(.TextMatrix(intRow, mconIntCol结算金额) = "", 0, .TextMatrix(intRow, mconIntCol结算金额)), mintMoneyDigit)
            ...
            If .TextMatrix(intRow, mconIntCol原销期) <> "" Then
                If Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2) = 1 Then
                    ...
                    If mbln加价率 Then
                        If mint时价入库时取上次售价 <> 1 Then  '没有勾选时价取上次售价参数
                            .TextMatrix(intRow, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), dbl成本价, (mdbl加价率 / 100), dbl成本价 * (1 + (mdbl加价率 / 100))), mintPriceDigit)
                        End If
                    Else
                        If mint时价分段加成方式 = 1 Then
                            If dbl成本价 > 0 Then
                                If get分段加成售价(Val(.TextMatrix(intRow, 0)), Val(.TextMatrix(intRow, mconIntCol比例系数)), dbl成本价, (mdbl加价率 / 100), dbl售价) = False Then
                                    Exit Sub
                                End If
                            End If
                        Else
                            dbl加成率 = Val(Replace(.TextMatrix(intRow, mconIntCol加成率), "%", "")) / 100
                            dbl售价 = dbl加成率 * (1 + dbl加成率)
                        End If
                        
                        If mint时价入库时取上次售价 <> 1 Then  '没有勾选时价取上次售价参数
                            .TextMatrix(intRow, mconIntCol售价) = GetFormat(时价药品零售价(Val(.TextMatrix(.Row, 0)), dbl成本价, dbl加成率, dbl售价), mintPriceDigit)
                        End If
                    End If
                    If .TextMatrix(intRow, mconIntCol数量) <> "" Then
...
Private Sub txtProvider_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    With txtProvider
        ...
        gstrSQL = "Select id,编码,名称,简码 From 供应商 " & _
                  ...
                  "  And (简码 like [1] Or 编码 like [1] or 名称 like [1] Or zlSpellCode(名称) Like [1] Or zlWbCode(名称) Like [1]  )"
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For intLop = 1 To .rows - 1
                If Trim(.TextMatrix(intLop, mconIntCol药名)) <> "" Then
                    ...
                    If Trim(Trim(.TextMatrix(intLop, mconIntCol结算价))) = "" Then
                    ...
                    If Trim(Trim(.TextMatrix(intLop, mconIntCol结算金额))) = "" Then
                    ...
                    If Split(.TextMatrix(intLop, mconIntCol原销期), "||")(0) <> "0" Then
                        If Val(.TextMatrix(intLop, mconIntCol分批属性)) = 1 And (Trim(.TextMatrix(intLop, mconIntCol批号)) = "" Or Trim(.TextMatrix(intLop, mconIntCol效期)) = "") Then
                    ...
                    If Val(.TextMatrix(intLop, mconIntCol结算价)) > 9999999999# Then
                    ...
                    If Val(.TextMatrix(intLop, mconIntCol结算金额)) > 9999999999999# Then
    ...
    With mshBill
        ...
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                dblQuantity = GetFormat(.TextMatrix(intRow, mconIntCol数量) * .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_数量)
                dblDiscount = .TextMatrix(intRow, mconIntCol扣率)
                dbl加成率 = Val(Replace(.TextMatrix(intRow, mconIntCol加成率), "%", "")) / 100
                dblPurchasePrice = Round(.TextMatrix(intRow, mconIntCol结算价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_成本价)
                dblPurchaseMoney = .TextMatrix(intRow, mconIntCol结算金额)
                ...
                '时价分批药品处理
                If Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2)) = 1 And Val(.TextMatrix(intRow, mconIntCol分批属性)) = 1 Then
                    dblSalePrice = FormatEx(Val(.TextMatrix(intRow, mconintCol零售价)), gtype_UserDrugDigits.Digit_零售价)
                ...
                '更新药品目录中的指导批发价
                If mbln修改批发价 Then
                    str指导批发价 = FormatEx(Val(.TextMatrix(intRow, mconIntCol指导批发价)) / Val(.TextMatrix(intRow, mconIntCol比例系数)), gtype_UserDrugDigits.Digit_成本价)
    ...
    With mshBill
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                datTimeLimit = IIf(Trim(.TextMatrix(intRow, mconIntCol效期)) = "", "", .TextMatrix(intRow, mconIntCol效期))
                str批准文号 = IIf(Trim(.TextMatrix(intRow, mconIntCol批准文号)) = "", "", .TextMatrix(intRow, mconIntCol批准文号))
                dblQuantity = GetFormat(Val(.TextMatrix(intRow, mconIntCol数量)) * .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_数量)
                dblDiscount = .TextMatrix(intRow, mconIntCol扣率)
                dbl加成率 = Val(Replace(.TextMatrix(intRow, mconIntCol加成率), "%", "")) / 100
                dblPurchasePrice = Round(Val(.TextMatrix(intRow, mconIntCol结算价)) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_成本价)
                dblPurchaseMoney = Val(.TextMatrix(intRow, mconIntCol结算金额))
                dblSalePrice = FormatEx(.TextMatrix(intRow, mconIntCol售价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_零售价)
    ...
    With mshBill
        ...
        For intRow = 1 To .rows - 1
            If Val(.TextMatrix(intRow, mconIntCol冲销数量)) <> 0 Then
                ...
                '检查可用数量是否足够，参数设置为不检查库存时不进行（财务审核时的冲销，退库时冲销也不检查）
                If mint库存检查 <> 0 And mint编辑状态 <> 7 And mbln退货 = False And .TextMatrix(intRow, 0) <> "" Then
                    If CheckStrickUsable(1, Val(cboStock.ItemData(cboStock.ListIndex)), Val(.TextMatrix(intRow, 0)), .TextMatrix(intRow, mconIntCol药名), _
                        Val(.TextMatrix(intRow, mconIntCol批次)), Val(.TextMatrix(intRow, mconIntCol冲销数量) * .TextMatrix(intRow, mconIntCol比例系数)), mint库存检查) = False Then
                        Exit Function
                    End If
                End If
            End If
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" And (Val(.TextMatrix(intRow, mconIntCol冲销数量)) <> 0 Or mint编辑状态 = 7) Then
                ...
                Else
                    int全部冲销 = 0
                End If
                冲销数量_IN = GetFormat(Val(.TextMatrix(intRow, mconIntCol冲销数量)) * .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_数量)
...
Private Sub 显示合计金额()
    ...
    With mshBill
        For intLop = 1 To .rows - 1
            curTotal = curTotal + Val(.TextMatrix(intLop, mconIntCol结算金额))
'            Cur记帐金额 = Cur记帐金额 + Val(.TextMatrix(intLop, mconIntCol售价金额))
    ...
    lblPurchasePrice.Caption = "结算金额合计：" & GetFormat(curTotal, mintMoneyDigit)
...
Private Function Get加价率() As Boolean
    Get加价率 = (Val(zldatabase.GetPara("时价药品以加价率入库", glngSys, 模块号.外购入库, 0)) = 1)
End Function
...
Private Sub 检查成本价()
    ...
    With mshBill
        If Val(.TextMatrix(.Row, 0)) = 0 Then Exit Sub
        dbl成本价 = Format(Val(.TextMatrix(.Row, mconIntCol结算价)), "#####0.00000;-#####0.00000;0;")
...
'    修改采购价、采购金额及差价（要考虑到存在审核冲销的单据，这时需要修改采购价、采购金额，差价）
    With mshBill
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                dbl采购价 = Val(.TextMatrix(intRow, mconIntCol结算价))
                dbl采购金额 = Val(.TextMatrix(intRow, mconIntCol结算金额))
...
Private Sub SetColumnByUserDefine()
    ...
    strColumn_All = "药名,2|药品来源,4|基本药物,5|药价级别,7|规格,8|产地,13|单位,14|批号,15|生产日期,16|效期,17|数量,18|指导批发价,21|采购价,22|扣率,23|" & _
                    "结算价,24|结算金额,25|加成率,26|售价,27|售价金额,28|差价,29|零售价,30|零售单位,31|零售金额,32|零售差价,33|批准文号, 34|外观,35|" & _
                    "产品合格证,36|随货单号,37|验收结论,38|发票号,39|发票代码,40|发票日期,41|发票金额,42"
    ...
    If strColumn_Selected <> "" Then
        ...
        If UBound(arr总列) <> UBound(arr可设置列) Or InStr(1, "|" & strColumn_Selected & "|", "|产地|") = 0 Or InStr(1, "|" & mstrColumn_UnSelected & "|", "|产地|") <> 0 Then
            strColumn_Selected = "药名|药品来源|基本药物|药价级别|规格|产地|批号|生产日期|效期|单位|数量|指导批发价|采购价|扣率|" & _
                        "结算价|结算金额|加成率|售价|售价金额|差价|批准文号|外观|产品合格证|随货单号|验收结论|发票号|发票代码|发票日期|发票金额"
...
Private Sub SetColumnValue(ByVal str列名 As String, ByVal intValue As Integer, Optional ByVal blnShow As Boolean = True)
    ...
    Case "结算价"
        mconIntCol结算价 = intValue
    Case "结算金额"
        mconIntCol结算金额 = intValue
    Case "售价"
...
Private Function CheckStock(ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl数量 As Double, Optional ByVal intRow As Integer) As Boolean
    ...
    '定价不分批药品不检查库存
'    With mshBill
'        If Split(.TextMatrix(.Row, mconIntCol原销期), "||")(2) = 0 And Val(.TextMatrix(.Row, mconIntCol批次)) = 0 Then
'            CheckStock = True
'            Exit Function
'        End If
'    End With
    ...
    Else               '负数入库
        ...
        Else
            ...
            Else
                dbl可用数量 = rsCheck!可用数量
            End If
            If Abs(Dbl数量) > Abs(dbl可用数量) And mint编辑状态 <> 7 Then
                blnMsg = True
            End If
...
Private Function 库存检查_退货() As Boolean
    Dim lngRow As Long, lngRows As Long, lng药品ID As Long, lng库房ID As Long, lng批次 As Long
    ...
    For lngRow = 1 To lngRows
        ...
        If lng药品ID <> 0 And Val(mshBill.TextMatrix(lngRow, mconIntCol数量)) <> 0 Then
            lng批次 = Val(mshBill.TextMatrix(lngRow, mconIntCol批次))
            lng库房ID = cboStock.ItemData(cboStock.ListIndex)
            
            gstrSQL = " Select Nvl(A.实际数量,0)/" & Choose(mintUnit, "1", "B.门诊包装", "B.住院包装", "B.药库包装") & " As 数量 " & _
                      " From 药品库存 A,药品规格 B " & _
                      " Where A.药品ID=[2] And A.性质=1 And A.药品ID=B.药品ID And Nvl(A.批次,0)=[3] And A.库房ID=[1] "
            Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[检查退货记录的库存是否足够]", lng库房ID, lng药品ID, lng批次)
                      
            If rsTemp.EOF Then
                blnExit = True
            Else
                blnExit = (Abs(rsTemp!数量) < Abs(Val(mshBill.TextMatrix(lngRow, mconIntCol数量))))
            End If
            If blnExit Then
                MsgBox "第" & lngRow & "行的药品库存数不够，不允许审核！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
    Next
...
Private Sub RefreshBill()
    ...
    gstrSQL = "Select A.序号,A.药品ID,B.现价 From 药品收发记录 A,(" & gstrSQL & ") B,收费项目目录 C" & _
            ...
            " Where A.单据 = 1 And A.NO = [1] And C.ID = A.药品id And Round(A.零售价, " & intPriceDigit & ") <> Round(B.零售价, " & intPriceDigit & ") And " & _
    ...
    For lngRow = 1 To lngRows
        ...
        If lng药品ID <> 0 Then
            ...
            If rsPrice.RecordCount <> 0 Then
                ...
                dbl成本价 = Val(mshBill.TextMatrix(lngRow, mconIntCol结算价))
                ...
                mshBill.TextMatrix(lngRow, mconIntCol加成率) = FormatEx((Val(mshBill.TextMatrix(lngRow, mconIntCol售价)) / dbl成本价 - 1) * 100, 2) & "%"
            End If
        End If
    Next
    rsPrice.Filter = 0
...
Private Sub SetState()
    ...
    With mshBill
        If .TextMatrix(.Row, mconIntCol付款标志) = "是" And gtype_UserSysParms.P173_经过标记付款后才能进行付款管理 = 1 Then
            ...
            If InStr(1, mstrControlItem, "结算价") > 0 Then
                .ColData(mconIntCol结算价) = 5
            Else
                .ColData(mconIntCol结算价) = 4
            End If
            
            If InStr(1, mstrControlItem, "结算金额") > 0 Then
                .ColData(mconIntCol结算金额) = 5
            Else
                .ColData(mconIntCol结算金额) = 4
            End If
...
Private Sub vsfInputCost_DblClick()
    ...
    With mshBill
        ...
        .TextMatrix(vsfInputCost.Tag, mconIntCol结算价) = GetFormat(Val(.TextMatrix(vsfInputCost.Tag, mconIntCol采购价)) * Val(.TextMatrix(vsfInputCost.Tag, mconIntCol扣率)) / 100, mintCostDigit)
        '设置金额
        If .TextMatrix(vsfInputCost.Tag, mconIntCol数量) <> "" Then
            .TextMatrix(vsfInputCost.Tag, mconIntCol结算金额) = GetFormat(.TextMatrix(vsfInputCost.Tag, mconIntCol数量) * Val(.TextMatrix(vsfInputCost.Tag, mconIntCol结算价)), mintMoneyDigit)
            .TextMatrix(vsfInputCost.Tag, mconintcol发票金额) = IIf(Trim(.TextMatrix(vsfInputCost.Tag, mconintcol发票号)) = "", "", .TextMatrix(vsfInputCost.Tag, mconIntCol结算金额))
            .TextMatrix(vsfInputCost.Tag, mconintCol差价) = GetFormat(IIf(.TextMatrix(vsfInputCost.Tag, mconIntCol售价金额) = "", 0, .TextMatrix(vsfInputCost.Tag, mconIntCol售价金额)) - IIf(.TextMatrix(vsfInputCost.Tag, mconIntCol结算金额) = "", 0, .TextMatrix(vsfInputCost.Tag, mconIntCol结算金额)), mintMoneyDigit)
            .TextMatrix(vsfInputCost.Tag, mconintCol零售差价) = GetFormat(Val(.TextMatrix(vsfInputCost.Tag, mconintCol零售金额)) - Val(.TextMatrix(vsfInputCost.Tag, mconIntCol结算金额)), mintMoneyDigit)
        End If
...
Private Function AutoExpend(Optional blnCheck As Boolean = False) As Boolean
    ...
    Dim dbl填写数量 As Double, dbl退库数量 As Double, Dbl数量 As Double, dbl比例系数 As Double
    ...
    Do While True
        ...
        lng药品ID = Val(mshBill.TextMatrix(lngRow, 0))
        dbl退库数量 = Val(mshBill.TextMatrix(lngRow, mconIntCol数量))
        dbl填写数量 = dbl退库数量
        dbl比例系数 = Val(mshBill.TextMatrix(lngRow, mconIntCol比例系数))
        lng批次 = Val(mshBill.TextMatrix(lngRow, mconIntCol批次))
        dblRate = Val(mshBill.TextMatrix(lngRow, mconIntCol扣率))
...
'                      " From 药品库存 Where 库房ID=[1] And 药品ID=[2] And 性质=1 And Nvl(可用数量,0)>0" & _
'                      " Order by 效期,Nvl(批次,0)"
            If gtype_UserSysParms.P96_药品填单下可用库存 = 1 Then
                gstrSQL = "Select Sum(Nvl(a.可用数量, 0)) / " & dbl比例系数 & " As 可用数量, Sum(Nvl(a.实际数量, 0)) / " & dbl比例系数 & " As 实际数量, Max(b.平均成本价) 平均成本价, Max(Nvl(b.实际金额, 0)) 实际金额," & vbNewLine & _
                        "       Max(Nvl(b.实际差价, 0)) 实际差价, Max(Nvl(b.批次, 0)) 批次, Max(b.上次批号) 批号, Max(To_Char(b.效期, 'yyyy-MM-dd')) 效期," & vbNewLine & _
                        "       Max(b.上次产地) 产地, Max(Nvl(b.上次供应商id, 0)) 上次供应商id, Max(b.批准文号) 批准文号, Max(Nvl(b.零售价, 0) *" & dbl比例系数 & ") As 零售价" & vbNewLine & _
                        "From (Select Nvl(可用数量, 0) As 可用数量, 实际数量, Nvl(批次, 0) 批次, 库房id, 药品id" & vbNewLine & _
                        "       From 药品库存" & vbNewLine & _
                        "       Where 库房id = [1] And 药品id = [2] And 上次供应商id = [4] And 性质 = 1" & vbNewLine & _
                        "       Union All" & vbNewLine & _
                        "       Select -1 * 入出系数 * Nvl(实际数量, 0) 可用数量, 0 实际数量, Nvl(批次, 0) 批次, 库房id, 药品id" & vbNewLine & _
                        "       From 药品收发记录" & vbNewLine & _
                        "       Where NO = [3] And 库房id = [1] And 药品id = [2]) A, 药品库存 B" & vbNewLine & _
                        "Where a.库房id = b.库房id And a.药品id = b.药品id And a.批次 = Nvl(b.批次, 0) And b.性质 = 1 And Nvl(a.可用数量,0)>0" & vbNewLine & _
                        "Group By b.库房id, b.药品id, Nvl(b.批次, 0)" & vbNewLine & _
                        "Having Sum(Nvl(a.可用数量, 0)) / " & dbl比例系数 & " > 0 " & vbNewLine & _
                        "Order By 效期, Nvl(批次, 0)"
            Else
                gstrSQL = " Select Nvl(可用数量,0)/" & dbl比例系数 & " As 可用数量,Nvl(实际数量,0)/" & dbl比例系数 & " As 实际数量,平均成本价," & _
                      " Nvl(实际金额,0) 实际金额,Nvl(实际差价,0) 实际差价," & _
                      " Nvl(批次,0) 批次,上次批号 批号,to_char(效期,'yyyy-MM-dd') 效期,上次产地 产地,NVL(上次供应商ID,0) 上次供应商ID,批准文号,nvl(零售价,0)*" & dbl比例系数 & " As 零售价 " & _
                      " From 药品库存 Where 库房ID=[1] And 药品ID=[2] And 上次供应商id = [4] And 性质=1 And Nvl(可用数量,0)>0" & _
                      " Order by 效期,Nvl(批次,0)"
            End If
            
            Set rsCheck = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption & "[提取该药品在指定库存的所有库存记录]", lng库房ID, lng药品ID, Trim(txtNO.Text), Val(txtProvider.Tag))
            
            '过滤该药品存在的批次
            strFilter = ""
            For i = 1 To mshBill.rows - 1
                If Val(mshBill.TextMatrix(i, 0)) = lng药品ID And Val(mshBill.TextMatrix(i, mconIntCol批次)) <> 0 Then
                    strFilter = IIf(strFilter = "", "批次 <> " & mshBill.TextMatrix(i, mconIntCol批次), strFilter & " and 批次 <> " & mshBill.TextMatrix(i, mconIntCol批次))
                End If
            Next
            ...
            With rsCheck
                ...
                Do While Not .EOF
                    ...
                    If .AbsolutePosition <> 1 Then
                        ...
                        Next
                        mshBill.TextMatrix(lngRow, mconIntCol数量) = "0"
                        mshBill.RowData(lngRow) = mshBill.RowData(lngRow - 1)
                    End If
                    
                    If intCount = 1 Then
                        dbl实际数量 = Val(mshBill.TextMatrix(lngRow, mconIntCol数量))
                    End If
                    
                    '填写批次相关信息
                    mshBill.TextMatrix(lngRow, mconIntCol行号) = lngRow
                    mshBill.TextMatrix(lngRow, mconIntCol序号) = lngRow
                    ...
                    mshBill.TextMatrix(lngRow, mconIntCol数量) = GetFormat(Dbl数量, mintNumberDigit)
                    
                     
                    If Trim(mshBill.TextMatrix(lngRow, mconIntCol数量)) = "" Then mshBill.TextMatrix(lngRow, mconIntCol数量) = 0
...
'                    mshBill.TextMatrix(lngRow, mconIntCol采购金额) = GetFormat(Val(mshBill.TextMatrix(lngRow, mconIntCol售价金额)) - Val(mshBill.TextMatrix(lngRow, mconintCol差价)), mintMoneyDigit)
                    ...
                    If Dbl数量 <> 0 Then
                        ...
                        mshBill.TextMatrix(lngRow, mconIntCol结算价) = GetFormat(rsCheck!平均成本价 * dbl比例系数 * dblRate / 100, mintCostDigit)
                    End If
                    mshBill.TextMatrix(lngRow, mconIntCol结算金额) = GetFormat(Val(mshBill.TextMatrix(lngRow, mconIntCol结算价)) * Dbl数量, mintMoneyDigit)
                    mshBill.TextMatrix(lngRow, mconintCol差价) = GetFormat(Val(mshBill.TextMatrix(lngRow, mconIntCol售价金额)) - Val(mshBill.TextMatrix(lngRow, mconIntCol结算金额)), mintMoneyDigit)
                    
                    Call Set时价分批药品零售价(lngRow, Val(mshBill.TextMatrix(lngRow, mconIntCol售价)) / Val(mshBill.TextMatrix(lngRow, mconIntCol比例系数)))
                    
                    dbl加成成本价 = IIf(mint时价入库售价加成方式 = 0, Val(mshBill.TextMatrix(lngRow, mconIntCol结算价)), Val(mshBill.TextMatrix(lngRow, mconIntCol采购价)))
                    
                    If Val(mshBill.TextMatrix(lngRow, mconIntCol售价)) <> 0 And dbl加成成本价 <> 0 Then
                        mshBill.TextMatrix(lngRow, mconIntCol加成率) = FormatEx((Val(mshBill.TextMatrix(lngRow, mconIntCol售价)) / dbl加成成本价 - 1) * 100, 2) & "%"
                    End If
                    
                    dbl填写数量 = dbl填写数量 - Dbl数量
                    dbl退库数量 = dbl退库数量 - Dbl数量
            ...
            If dbl填写数量 <> 0 And rsCheck.RecordCount = 0 Then
                ...
                mshBill.TextMatrix(lngRow, mconIntCol结算金额) = ""
...
'                      " From 药品库存 Where 性质=1 And 库房ID=[1] And 药品ID=[2] And Nvl(批次,0)=[3] And Nvl(可用数量,0)>0" & _
'                      " Order by " & IIf(gtype_UserSysParms.P150_药品出库优先算法 = 0, " Nvl(批次,0)", " 效期,Nvl(批次,0)")
