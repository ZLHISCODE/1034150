...
'    gstrSQL = "Select nvl(参数值,'0') From 系统参数表 Where 参数名='西成药以商品名显示'"
    gstrSQL = "Select nvl(参数值,缺省值) From Zlparameters Where 系统 = 100 And Nvl(私有, 0) = 0 And 模块 Is Null And 参数号 = 74"

    Call OpenRecordset(rsTemp, "西成药以商品名显示")
    If Not rsTemp.EOF Then
        mblnTradeName = IIf(Val(rsTemp.Fields(0).Value) = 1, True, False)
    End If
