...
Private mintStyle As Integer                              '嵌入编辑 -1(10.35开始支持) 非模态 vbModeless=0 模态 vbModal=1
...
Private mbln传染病处理 As Boolean                       '是否需要更新  疾病申请记录.处理状态
...
Private Function ShowSharePageHistory(ByVal Document As cEPRDocument, Optional ByVal intNumber As Integer = 5) As Boolean
    ...
    strIDs = GetFileRange(Document.EPRFileInfo.ID, Document.EPRPatiRecInfo.ID, strTime, Document.EPRPatiRecInfo.病历种类, Document.EPRPatiRecInfo.病人ID, Document.EPRPatiRecInfo.主页ID)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, intNumber, varPar(0), varPar(1), varPar(2), varPar(3), varPar(4), varPar(5), varPar(6), varPar(7), varPar(8), varPar(9))
    ...
    Do While Not rsTemp.EOF
        ...
        If lngTail > 0 Then
            With Me.Editor1
                ...
                .SelStart = lngTail
            End With
            Set rsText = zlDatabase.OpenSQLRecord(strVSql, Me.Caption, lngDemoId, CLng(rsTemp!ID))
...
'################################################################################################################
Public Sub ShowMe(frmParent As Object, Optional blnFirst As Boolean = True, Optional blnCanPrint As Boolean = True, Optional ByVal intStyle As Integer)
    '设置窗体显示状态
...
Private Sub ExecPaste(ByRef edtThis As Object)
    ...
    '粘贴剪贴板内容 并替换μ为u，防止崩溃
    strClipboard = Replace(Clipboard.GetText, "μ", "u")
    
    If tblThis.Visible Then
        If tblThis.SelectedCellKey > 0 Then
            If tblThis.Cells("K" & tblThis.SelectedCellKey).Protected Then Exit Sub
            If tblThis.InEdit Then
                tblThis.InsertText strClipboard
            Else
                tblThis.Cells("K" & tblThis.SelectedCellKey).Text = strClipboard
                tblThis.Refresh False, True, tblThis.SelectedCellKey
                tblThis_Resize tblThis.Width, tblThis.Height
            End If
            tblThis.Modified = True
...
Private Sub ExecSaveAsPhrase()
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lngCompendID)
...
Private Function AddSign() As Boolean
    ...
    Dim lS As Long, l As Long, strSQL As String
    Dim rsTmp As ADODB.Recordset, strTime As String
...
'            Editor1.Selection.StartPos = lKSS - 1
    ...
    If mbln传染病处理 And mblnFBContentChanged Then
...
Private Sub cbrThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case ID_FILE_PRINTPREVIEW, ID_FILE_PRINT, ID_FILE_PRINTINWORD
        ...
        If Control.Enabled And (Me.Document.EPRFileInfo.种类 = cpr住院病历 Or Me.Document.EPRFileInfo.种类 = cpr门诊病历) Then
    ...
    Case ID_VIEW_PACSPIC
        Control.Visible = (Me.Document.EPRFileInfo.种类 = cpr诊疗报告)
        Control.Enabled = (Me.Document.EPRFileInfo.种类 = cpr诊疗报告)
        Control.Checked = mfrmSegments.Visible
    Case ID_VIEW_HISTORYREPORT
        Control.Visible = (Me.Document.EPRFileInfo.种类 = cpr诊疗报告)
        Control.Enabled = (Me.Document.EPRFileInfo.种类 = cpr诊疗报告)
    ...
    Case ID_INSERT_DOCADVISE
        ...
        Control.Visible = (Me.Document.EPRFileInfo.种类 = cpr门诊病历)
    Case ID_INSERT_EPRDEMO
        If Me.Editor1.AuditMode Then
            Control.Enabled = False
        Else
    ...
    Case ID_FORMAT_PROTECT
        ...
        Control.Enabled = (Me.Document.EditType = cprET_病历文件定义 Or Me.Document.EditType = cprET_全文示范编辑 _
            Or InStr(1, gstrPrivsEpr, "保护文本处理") > 0)
    Case ID_FORMAT_SUPER
    ...
    Case ID_PATISIGN
        If Me.Document.EditType = cprET_单病历编辑 Then
            Control.Visible = mblnEnPtSign
            If Control.Visible Then Control.Visible = (Me.Editor1.ReadOnly = False)
        Else
...
'        Set objControl = cbpPopup.CommandBar.Controls.Add(xtpControlButton, ID_TABLE_INSERTTABLE, "表格(&T)"): objControl.BeginGroup = True
    ...
    With cbrBar.Controls
        ...
        Dim rs As New ADODB.Recordset
        Set rs = zlDatabase.OpenSQLRecord("select 名称 from 病历常用样式 order by 编号", Me.Caption, "")
        i = 0
        Do While Not rs.EOF
            i = i + 1
            Combo.AddItem rs("名称")
...
'    cbrThis.KeyBindings.Add 0, VK_BACK, ID_EDIT_BACKSPACE
    ...
    ColorHighlight.COLOR = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "HighlightColor", vbYellow)
    ColorFillColor.COLOR = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "CellFillColor", vbWhite)
    Editor1.ShowRuler = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "ShowRuler", False)
    ...
    DkpThis.ShowPane ID_VIEW_PHRASEDEMO
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneCompendHided", False) Then PaneCompend.Close
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneSentenceHided", False) Then PaneSentence.Close
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneSegmentHided", False) Then PaneSegment.Close
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PanePacsPicHided", False) Then PanePacsPic.Close
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneHistoryReportHided", False) Then PaneHistoryReport.Close
    If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneDockSymbol", False) Then PaneDockSymbol.Hide
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Me.WindowState <> vbMinimized Then
        ...
        SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "HighlightColor", ColorHighlight.COLOR
        SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "CellFillColor", ColorFillColor.COLOR
        SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "ShowRuler", Me.Editor1.ShowRuler
    End If
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneCompendHided", DkpThis.FindPane(ID_VIEW_STRUCTURE).Closed
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneSentenceHided", DkpThis.FindPane(ID_VIEW_PHRASEDEMO).Closed
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneSegmentHided", DkpThis.FindPane(ID_VIEW_SEGMENT).Closed
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PanePacsPicHided", DkpThis.FindPane(ID_VIEW_PACSPIC).Closed
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneHistoryReportHided", DkpThis.FindPane(ID_VIEW_HISTORYREPORT).Closed
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "PaneDockSymbol", DkpThis.FindPane(ID_VIEW_Assistant).Hidden
...
'## 功能：  保存文档至数据库
'################################################################################################################
Private Function SaveEMRDoc(Optional ByVal blnNoAsk As Boolean = False) As Boolean
    Dim blnR As Boolean, eEditMode As EditModeEnum
        Dim rsTmp As ADODB.Recordset, strSQL As String, strTime As String
    ...
    If SaveDocToFile(strF) Then
        If gobjFSO.FileExists(strF) Then
            ...
            Else
                ...
                Clipboard.Clear: Editor1.DocHeadCopyWithFormat:           edtBuff.DocHeadPasteWithFormat '转到临时控件中
                edtBuff.DocHeadReplaceKey:              edtBuff.DocHeadCopyWithFormat
                ...
                Clipboard.Clear: Editor1.DocFootCopyWithFormat:          edtBuff.DocFootPasteWithFormat '转到临时控件中
                edtBuff.DocHeadReplaceKey:              edtBuff.DocFootCopyWithFormat
                WordApp.Selection.Paste
                '去掉 其中的总页数,页码
                WordApp.Selection.Start = 0
                WordApp.Selection.End = 99999
...
Private Sub mfrmSentenceDetailed_RowDblClick(ByVal lngSentenceID As Long)
    ...
    gstrSQL = "Select 词句id, 排列次序, 内容性质, 内容文本, 诊治要素id, 替换域, 要素名称, 要素类型, 要素长度, 要素小数, 要素单位, 要素表示, 要素值域, 输入形态, 对象属性" & vbNewLine & _
                ...
                "Order By 排列次序"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lngSentenceID)
    With Editor1
...
Public Sub RefreshPatiInfo()
    ...
    Err = 0: On Error GoTo errHand
    If (Document.EPRPatiRecInfo.医嘱id <> 0 And Document.EPRPatiRecInfo.婴儿 <> 0) Then
...
On Error GoTo errHand
    If Me.Document.EPRPatiRecInfo.病历种类 <> cpr诊断文书 Or mbln传染病处理 Then Exit Function
  
    If isRelated Then   '关联
        If Me.Document.EPRPatiRecInfo.病人来源 = cprPF_门诊 Then
        ...
        ElseIf Me.Document.EPRPatiRecInfo.病人来源 = cprPF_住院 Then
