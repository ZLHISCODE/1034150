...
Private mlngRowNow As Long '记录已经刷新的行号
...
Dim rsTemp As New ADODB.Recordset
    ...
    With Me.vfgWrit
        ...
        For lngRow = .FixedRows To .Rows - 1
            ...
            End If
            If lngCurId = Val(.TextMatrix(lngRow, mCol.wID)) Then .Row = lngRow
        Next
        If .Rows = .FixedRows Then .Rows = .FixedRows + 1
        vfgWrit.Tag = -1: .Row = 0 '促使vfgthis不选中任何行，不显示任何内容，仅当选中某行时才刷新
...
Private Sub vfgWrit_RowColChange()
    ...
    If mblnViewNow = True Or mlngRowNow = vfgWrit.Row Then Exit Sub
    mblnViewTag = True
    ...
    With Me.vfgWrit
        If .Cols < mCol.wID + 1 Then Exit Sub
        lngRecordId = Val(.TextMatrix(.Row, mCol.wID))
        byteEdit = Val(.TextMatrix(.Row, mCol.w编辑方式))
    End With
    Err = 0
    On Error GoTo 0
    
    If Me.Tag = "" And (Val(Me.vfgWrit.Tag) <> Me.vfgWrit.Row) Then
        If Not mfrmContent Is Nothing Then Call mfrmContent.zlRefresh(lngRecordId, IIf(mblnEdit = False, "", mstrPrivs), , mblnMoved, , byteEdit, True)
        Me.vfgWrit.Tag = Me.vfgWrit.Row
    End If
    mblnViewTag = False
    mlngRowNow = vfgWrit.Row
End Sub
