...
Private mobjBHEditor As Object

Dim mlngVersion As Long  '选中的文件版本号
            ...
            If .Cell(flexcpFontUnderline, lngMouseRow, lngMouseCol) = True Then
                If DisplayContent(Val(.TextMatrix(lngMouseRow, mCol.ID))) Then
                    With vsfFeedback
                        ...
                        .Top = vfgThis.Top + 250 * (lngMouseRow + 1)
                        .ZOrder
                        .Visible = True
                        .SetFocus
                    End With
...
Private Sub mfrmNew_NewClick(ByVal FileId As Long, ByVal babyNum As Long)
    ...
    If rs!保留 = 5 Then
        If mobjBHEditor Is Nothing Then
            Set mobjBHEditor = DynamicCreate("ZLEMREdit.EMRComEdit", "BHEditor")
        End If
        Call mobjBHEditor.NewEMRWordEdit(mlngPatiId, mlngPageId, FileId, glngDeptId)
        Exit Sub
    End If
    ...
    Else                    '电子病历RichEpr
        If zlDatabase.GetPara("共享病历必须先书写被共享病历", glngSys, mlngModul, "1") = 1 Then
            ...
            If rs.EOF = False Then '是共享文档
                ...
                Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlngPatiId, mlngPageId, Val(Split(strSQL, "|")(0)))
                If rs.EOF Then
                    MsgBox "该病历的共享病历 [" & Split(strSQL, "|")(1) & "] 尚未书写，请检查。", vbInformation, gstrSysName
                    Exit Sub
                End If
            End If
        End If
...
Private Sub vfgThis_RowColChange()
    ...
    If Me.Tag = "" And (Val(Me.vfgThis.Tag) <> Me.vfgThis.Row) Then
        ...
        Call mfrmContent.zlRefresh(lngRecordId, IIf(mblnEdit = False, "", mstrPrivs), mblnMoved, mblnShowFinal, byteEdit, mblnAllowDelete)
        Me.Tag = ""
...
Dim cbrToolBar As CommandBar
    ...
    With cbrMenuBar.CommandBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_Option, "病历选项(&O)")
    End With
...
Dim bFinded As Boolean, frmThis As Form, bEditor As Integer
    ...
    Case conMenu_File_Open
        ...
        If bEditor = 0 Or bEditor = 4 Then
    ...
    Case conMenu_File_Preview
        If bEditor <> 0 And bEditor <> 1 Then Exit Sub
    ...
    Case conMenu_Edit_Modify
        ...
        With Me.vfgThis
            ...
            ElseIf bEditor = 0 Then
                ...
                End If
            ElseIf bEditor = 4 Then
                If mobjBHEditor Is Nothing Then
                    Set mobjBHEditor = DynamicCreate("ZLEMREdit.EMRComEdit", "BHEditor")
                End If
                Call mobjBHEditor.ModifyEMRWordEdit(lFileId, glngDeptId)
            End If
        End With
    Case conMenu_Edit_Delete
        If bEditor = 4 Then
            If mobjBHEditor Is Nothing Then
                Set mobjBHEditor = DynamicCreate("ZLEMREdit.EMRComEdit", "BHEditor")
            End If
            Call mobjBHEditor.DeleteEMRWord(lFileId, glngDeptId)
        ElseIf bEditor = 0 Or bEditor = 1 Then
    ...
    Case conMenu_Edit_Option
        frmDockInEPRsOption.ShowMe mlngModul, mstrPrivs
        mlngfolding = zlDatabase.GetPara("共享病历折叠起始行数", glngSys, mlngModul, "6")
        Call mfrmContent.RefreshParameter
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Dim blnTmp As Boolean, bEditor As Integer
    ...
    With Me.vfgThis
        ...
        Case conMenu_File_Open, conMenu_File_Excel, conMenu_File_RowPrint
            ...
            If Control.Enabled Then Control.Enabled = (bEditor = 0 Or bEditor = 1)
        ...
        Case conMenu_Edit_Modify
            Control.Enabled = (mblnEdit And mlngPatiId > 0 And InStr(1, mstrPrivs, "病历书写") > 0)
            If Control.Enabled Then Control.Enabled = (bEditor = 0 Or bEditor = 1 Or bEditor = 4)
...
Dim rs传染 As ADODB.Recordset
    ...
    gstrSQL = "Select r.科室id 病人科室, Decode(b.名称, Null, r.病历名称, b.名称) As 页面, r.病历名称, r.创建人 As 创建人," & vbNewLine & _
            ...
            "       Decode(c.编号, b.编号, 1, 0) As 新组, r.Id, r.病历种类, b.编号, r.编辑方式, r.打印人 as 打印,r.婴儿" & vbNewLine & _
            "From 电子病历记录 R, 部门表 D, 病历文件列表 C, 病历页面格式 B" & vbNewLine & _
            "Where r.文件id + 0 = c.Id And r.病人来源 = 2 And r.病历种类 In (2, 5, 6) And r.科室id = d.Id And r.病人id = [1] And r.主页id = [2] And" & vbNewLine & _
            "      c.种类 = b.种类 And c.页面 = b.编号 And Nvl(c.保留, 0) In (0,1,2,3,5) " & vbNewLine & _
            "Order By r.病历种类, b.编号, r.序号, r.创建时间"
...
Dim lFileId As Long                      '文件ID
Dim r As String, blnOrigMode As Boolean  '是否显示原始状态
    
    If NVL(vfgThis.TextMatrix(vfgThis.Row, mCol.编辑方式), 0) = 0 Then
        ...
        mfrmPrintPreview.DoMultiDocPreview Me, cpr住院病历, mlngPatiId, mlngPageId, _
                    ...
                    vfgThis.Cell(flexcpText, vfgThis.Row, mCol.ID), Not blnPreview, blnOrigMode, , mblnMoved, mlngAdviceID, , IIf(InStr(mstrPrivs, "取消打印") > 0, 0, 1)
                    '没有"取消打印"权限不允许重复打印，不允许调整打印份数
...
Dim rsTemp As New ADODB.Recordset, strPage As String, lngLen As Long, blnExport As Boolean
    ...
    '指定目录
    strPath = OpenDir(Me.hWnd, "指定导出目录")
    If strPath = "" Then
