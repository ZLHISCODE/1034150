...
'返回：真-处理成功
    ...
    '一卡通消费验证
    gbln消费验证 = Val(gobjComlib.zlDatabase.GetPara(28, glngSys)) <> 0
    '住院自动发料
    gbln住院自动发料 = Val(gobjComlib.zlDatabase.GetPara(63, glngSys)) <> 0
    '门诊自动发料
    gbln门诊自动发料 = Val(gobjComlib.zlDatabase.GetPara(92, glngSys)) <> 0
    '是否安装血库系统
    gbln血库系统 = IsSetUp(2200)
    '记帐报警包含划价费用
...
'功能：是否安装某个系统
    ...
    Set rsTmp = gobjComlib.zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", lng系统编号)
