...
'功能：返回诊疗项目名称
    ...
    Set rsTmp = gobjComlib.zlDatabase.OpenSQLRecord(strSql, "mdlCISKernel", lng项目ID)
...
'功能：读取当前登录人员或指定人员的人员性质
    ...
    Set rsTmp = gobjComlib.zlDatabase.OpenSQLRecord(strSql, "mdlCISKernel", lng人员id)
...
'      blnNone加班加价=为真时,"gbln加班加价"无效
    ...
    Else
        ...
        If str费别 <> "" Then
            ...
            Set rsTmp = gobjComlib.zlDatabase.OpenSQLRecord(strSql, "CalcDrugPrice", lng药品ID)
...
Public Function AdviceMoneyMake(ByVal lng病人ID As Long, ByVal lng主页ID As Long, rsMoneyNow As Recordset, rsMoneyDay As ADODB.Recordset, _
    ...
    Optional ByVal int费用性质 As Integer, Optional ByVal str诊疗类别 As String) As Boolean
'功能：判断指定的医嘱费用是否应该产生
...
'      rsExec=医嘱执行计价的内容
    ...
    ElseIf int收费方式 = 1 Then '检验试管费用(一次发送只收取一次)
        If str试管编码 <> "" Then
            '相同条码(试管)只收取一次
            rsMoneyNow.Filter = "试管编码='" & str试管编码 & "' And 收费项目ID=" & lng收费项目ID & " And 收费方式<>-1"
            If Not rsMoneyNow.EOF Then blnMakeMoney = False
            ...
            For i = 1 To rsDays.RecordCount
                If i = 1 Then
                    If rsMoneyDay Is Nothing Then
                        Call GetPatiDayMoneyDetail(rsMoneyDay, lng病人ID, lng主页ID, byt来源)
                    End If
                Else
                    If int收费方式 = 7 Then
                        Call GetPatiDayMoneyDetail(rsMoneyDay, lng病人ID, lng主页ID, byt来源, lng诊疗项目ID, lng收费项目ID, rsDays!收费时间)
                    End If
                End If
    ...
    If InStr(",4,5,6,7,", "," & str诊疗类别 & ",") = 0 Then
        If str分解时间 <> "" And Not rsExec Is Nothing Then
            ...
            For i = 0 To UBound(arrTmp)
                ...
                rsExec!费用性质 = int费用性质
                If blnMakeMoney Then
                    If str频率 <> "" And (lng计算方式 = 1 Or lng计算方式 = 2) Then
                        '计量和计时的需要乘以数次
                        If int期效 = 0 Then
                            '1、长嘱可选频率、持续性、必要时和不定时以单量作为数次。
                            dbl数量 = dbl计价数量 * dbl单量
