...
Private mstrRegNo As String '门诊病人挂号单号
'当前病人险类的医保支持参数
...
Private mobjDrugMachine As Object '新发药机
...
Public Function EditCard(ByVal frmMain As Object, ByVal strPrivs As String, _
    ...
    ByVal lng医嘱ID As Long, ByVal lng发送号 As Long, _
    ByVal lng病人ID As Long, ByVal lng主页Id As Long, int病人来源 As Integer, _
    ByVal int记录性质 As Integer, ByVal lng开单科室id As Long, ByVal lng病人科室ID As Long, _
    ByVal lng开嘱科室ID As Long, ByVal str开嘱医生 As String, Optional ByVal strOriginalNO As String, _
    Optional ByVal strInNO As String = "", Optional ByVal strTime As String = "", Optional blnDelete As Boolean, _
    Optional strFeeTable As String = "", Optional bln费用登记 As Boolean, _
    Optional ByRef strOutNos As String, Optional byt应用场合 As Byte = 0, Optional ByRef objSaveData As Object, _
    ...
    '     bln费用登记:仅登记,不计实收金额
    '     lng开单科室ID:为当前主界面医技科室
    '     lng病人科室id:主要是用于确定门诊病人的科室ID
    '     lng开嘱科室ID:
    '     str开嘱医生:
    '     strTime:操作单据内容的登记时间
    '     blnDelete:是否处理退费单据(查阅)
    '     strFeeTab:门诊费用记录或住院费用记录
    '     byt应用场合-0-医嘱附费;1-体检补费(可选参数)
    '出参:strOutNos-成功保存的单据号
    '返回:成功保存一张单据以上,返回true,否则返回False
    '编制:刘兴洪
    '日期:2014-04-10 14:13:53
    '---------------------------------------------------------------------------------------------------------------------------------------------
     mbytInState = bytInState: mstrInNO = strInNO: mstrOriginalNO = strOriginalNO
     mstrTime = strTime: mblnDelete = blnDelete
     mlng医嘱ID = lng医嘱ID: mlng发送号 = lng发送号: mlng病人ID = lng病人ID
     mlng主页ID = lng主页Id: mint病人来源 = int病人来源: mint记录性质 = int记录性质
     mstrFeeTab = strFeeTable: mbln费用登记 = bln费用登记
     mlng开单科室ID = lng开单科室id: mlng病人科室id = lng病人科室ID
     mlng开嘱科室ID = lng开嘱科室ID: mstr开嘱医生 = str开嘱医生
     mbyt应用场合 = byt应用场合
            ...
            With mobjBill.Details(Bill.Row)
                If .执行部门ID <> Bill.ItemData(Bill.ListIndex) Then
                    ...
                    ElseIf .收费类别 = "4" And .Detail.跟踪在用 Then
                        '取库存
                        dblStock = GetStock(.收费细目ID, .执行部门ID, 0, , , .Detail.批次) '29680
                        .Detail.库存 = dblStock
                        Call ShowStock(.Detail.名称, .Detail.库存)
...
Private Sub bill_CommandClick()
    ...
    End If
    mlng批次 = -1
    lng项目ID = frmItemSelect.ShowSelect(Me, mstrPrivs, mint病人来源, int险类, True, str类别, , , str特准项目, zl获取中药形态(Bill.Row), IIf(mbln费用登记, True, False), mbln条码刷卡, mlng批次)
    If lng项目ID <> 0 Then
        Bill.Text = lng项目ID
        mblnSelect = True
        ...
        Select Case Bill.TextMatrix(0, Bill.Col)
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    If mblnSelect Then
                        ...
                        Set mobjDetail = GetInputDetail(Val(Bill.Text), mlng批次)
                    Else
                        If gSysPara.bln收费类别 Then
                            If Bill.RowData(Bill.Row) = 0 Then
                        ...
                        mlng批次 = -1
                        lng项目ID = frmItemSelect.ShowSelect(Me, mstrPrivs, mint病人来源, int险类, True, str类别, Bill.Text, Bill.TxtHwnd, str特准项目, zl获取中药形态(Bill.Row), IIf(mbln费用登记, True, False), mbln条码刷卡, mlng批次)
                        If lng项目ID <> 0 Then
                            Set mobjDetail = GetInputDetail(lng项目ID, mlng批次)
                    ...
                    '医保费用项目是否审批检查
                    If CheckYBFeeItemVerfiy(Val(Nvl(mrsInfo!病人ID)), Val(Nvl(mrsInfo!险类)), mobjDetail) = False Then
                    ...
                    ElseIf mobjDetail.类别 = "4" And mobjDetail.跟踪在用 Then
                        dblStock = GetStock(mobjDetail.ID, lngDoUnit, 0, , , mobjDetail.批次) ''29680
                        mobjDetail.库存 = dblStock
                        Call ShowStock(mobjDetail.名称, mobjDetail.库存)
                    End If
                    ...
                    '83877,冉俊明,2015-4-14,门诊病人根据"病人挂号记录"获取"险类",调用GetItemInfo
                    Else '90304
                        If mrsInfo.State = 1 Then   '问题:主要是大连一院要求,由于BH不能登记,所以没有BugNo
                    ...
                    If mint记录性质 = 2 And mrsWarn.State = 1 And mobjBill.Details.Count = Bill.Row Then
                        ...
                        If curTotal > 0 And mbln费用登记 = False Then
                            cur余额 = Val(txt实收.Tag)
                            If gSysPara.bln报警包含划价费用 Then cur余额 = cur余额 - GetPriceMoneyTotal(IIf(mint病人来源 = 1, 0, 1), mrsInfo!病人ID)
                    ...
                    If mobjBill.Details(Bill.Row).数次 <> 0 Then
                        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, p医嘱附费管理, IIf(mint病人来源 = 2, 1, 0), 0, _
                            MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), Bill.Row)) = False Then
                            mobjBill.Details.Remove Bill.Row  '删除刚刚想要加入的费用行
                            Bill.Text = "": Cancel = True: Exit Sub
                        End If
                    End If
                ...
                If mobjBill.Details.Count >= Bill.Row Then
                    With mobjBill.Details(Bill.Row)
                        ...
                        If Bill.TextMatrix(0, Bill.Col) = "项目" And InStr(",5,6,7,", .收费类别) = 0 Then
                            If (gSysPara.bln从项汇总折扣 And mobjBill.Details(Bill.Row).从属父号 = 0) Or Not gSysPara.bln从项汇总折扣 Then  '(如果有级联,只取一级)
            ...
            Case "付数"
                If mobjBill.Details.Count >= Bill.Row And Bill.Text <> "" Then
                    ...
                    If gSysPara.dblMaxMoney > 0 Then
                        If CSng(Bill.Text) * mobjBill.Details(Bill.Row).数次 * Bill.TextMatrix(Bill.Row, BillCol.单价) > gSysPara.dblMaxMoney Then
                            If MsgBox("当前金额超过了" & gSysPara.dblMaxMoney & ",你确定要继续吗?", vbInformation + vbOKCancel + vbDefaultButton2, gstrSysName) = vbCancel Then
                                Bill.Text = mobjBill.Details(Bill.Row).付数: Cancel = True: Exit Sub
                            End If
                        End If
                    End If
            ...
            Case "数次"
                If mobjBill.Details.Count >= Bill.Row And Bill.Text <> "" Then
                    ...
                    '最大金额检查
                    If gSysPara.dblMaxMoney > 0 Then
                        If CSng(Bill.Text) * mobjBill.Details(Bill.Row).付数 * Bill.TextMatrix(Bill.Row, BillCol.单价) > gSysPara.dblMaxMoney Then
                            If MsgBox("当前金额超过了" & gSysPara.dblMaxMoney & ",你确定要继续吗?", vbInformation + vbOKCancel + vbDefaultButton2, gstrSysName) = vbCancel Then
                                Bill.Text = mobjBill.Details(Bill.Row).数次: Cancel = True: Exit Sub
                            End If
                        End If
                    ...
                    If mint记录性质 = 2 And mrsWarn.State = 1 Then
                        ...
                        If curTotal > 0 And mbln费用登记 = False Then
                            cur余额 = Val(txt实收.Tag)
                            If gSysPara.bln报警包含划价费用 Then cur余额 = cur余额 - GetPriceMoneyTotal(IIf(mint病人来源 = 0, 0, 1), mrsInfo!病人ID)
                            mbytWarn = BillingWarn(Me, mstrPrivs, mrsWarn, mrsInfo!姓名, cur余额, mrsInfo!当日额 - mcurModiMoney, curTotal, _
                                Nvl(mrsInfo!担保额, 0), mobjBill.Details(Bill.Row).收费类别, mobjBill.Details(Bill.Row).Detail.类别名称, mstrWarn, mintWarn)
                            If mbytWarn = 2 Or mbytWarn = 3 Then
                                mobjBill.Details(Bill.Row).数次 = dblPreTime
...
NotCalc:
            ...
            Case "单价"
                If mobjBill.Details.Count >= Bill.Row And Bill.Text <> "" Then
                    ...
                    '最大金额检查
                    If gSysPara.dblMaxMoney > 0 Then
                        If CSng(Bill.Text) * mobjBill.Details(Bill.Row).付数 * mobjBill.Details(Bill.Row).数次 > gSysPara.dblMaxMoney Then
                            If MsgBox("当前金额超过了" & gSysPara.dblMaxMoney & ",你确定要继续吗?", vbInformation + vbOKCancel + vbDefaultButton2, gstrSysName) = vbCancel Then
                                Bill.Text = "": Cancel = True: Exit Sub
                            End If
                        End If
...
If gSysPara.bln从项汇总折扣 Then            '如果主项屏蔽费别,则汇总计算折扣参数无效,不汇总计算
...
With mobjBill
    ...
    cur打折后实收 = CCur(Format(ActualMoney(.费别, .Details(lngMainRow).InComes(1).收入项目ID, cur打折前应收合计), gSysPara.Money_Decimal.strFormt_VB))
    
    cur打折后实收 = cur打折后实收 - cur打折前应收合计 + .Details(lngMainRow).InComes(1).应收金额
    
    .Details(lngMainRow).InComes(1).实收金额 = Format(cur打折后实收, gSysPara.Money_Decimal.strFormt_VB)
    .Details(lngMainRow).InComes(1).Key = "_" & Format(cur打折后实收, gSysPara.Money_Decimal.strFormt_VB)
...
Private Sub Bill_EnterCell(Row As Long, Col As Long)
    ...
    If mobjBill.Details.Count >= Bill.Row And mlngPreRow <> Row Then
        With mobjBill.Details(Bill.Row)
            ...
            ElseIf .收费类别 = "4" And .Detail.跟踪在用 And .收费细目ID <> 0 Then
                .Detail.库存 = GetStock(.收费细目ID, .执行部门ID, 0, , , .Detail.批次) '29680
                Call ShowStock(.Detail.名称, .Detail.库存)
            ...
            Bill.ColData(BillCol.类别) = IIf(gSysPara.bln收费类别, BillColType.ComboBox, BillColType.UnFocus)
    ...
    '如果点击未保存的行,则恢复列的性质
    If mobjBill.Details.Count < Bill.Row Then
        Bill.ColData(BillCol.类别) = IIf(gSysPara.bln收费类别, BillColType.ComboBox, BillColType.UnFocus) '类别列,当主从项时会被改变
    ...
    If gSysPara.bln收费类别 And Bill.TextMatrix(Row, BillCol.类别) = "" And mblnOne Then
...
Private Sub cbo开单科室_Click()
    ...
    If cbo开单科室.ItemData(cbo开单科室.ListIndex) = 0 And cbo开单科室.Text Like "其他科室*" Then
        If gobjDatabase.zlShowListSelect(Me, glngSys, 1150, cbo开单科室, mrsAll开单科室, True, "", "缺省,优先级", rsReturn) = False Then
            mobjBill.开单部门ID = 0
            Exit Sub
        End If
        ...
        rsReturn.MoveFirst
        If gobjControl.CboLocate(cbo开单科室, Val(rsReturn!ID), True) = False Then

             If InStr(mstrPrivs, ";所有科室;") > 0 Then cbo开单科室.RemoveItem cbo开单科室.ListCount - 1
...
'                strSQL = strSQL & " Union ALL Select " & .收费细目ID & "," & .执行部门ID & ",0 From Dual"
   ...
   ' strSQL = _
    ...
    On Error GoTo errH
     Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mobjBill.病人ID, mobjBill.主页ID, strValues(0), strValues(1), strValues(2), strValues(3), strValues(4), strValues(5), strValues(6), strValues(7), strValues(8), strValues(9), strValues(10))
            ...
            If .数次 < 0 And .执行部门ID <> 0 Then
                ...
                If Not rsTmp.EOF Then
                    ...
                    If dbl数次合计 > dbl数量 - dbl已结数量 Then
                        Select Case gSysPara.bytBillOpt '对已结帐的记帐单据的操作权限:0-允许,1-提醒,2-禁止。
                        Case 0  '允许
...
Private Sub cmdOK_Click()
    ...
    If mbytInState = 3 Then
        ...
        If mint病人来源 = 1 And mint记录性质 = 2 Then
            '110319
            If mblnDrugMachine Then
                ...
                Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, "查询本次退费项目", mstrInNO)
...
'        End If
        ...
        If mint记录性质 = 2 And mrsWarn.State = 1 And mstrWarn <> "-" Then
            ...
            If curTotal > 0 Then
                ...
                sta.Panels(3).Text = sta.Panels(3).Text & "/费用:" & Format(Val(cmdCancel.Tag) + curTotal, gSysPara.Money_Decimal.strFormt_VB)
        ...
        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, p医嘱附费管理, IIf(mint病人来源 = 2, 1, 0), 1, _
            MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text))) = False Then
            Exit Sub
        End If
        ...
        For i = 1 To mobjBill.Details.Count
            With mobjBill.Details(i)
                ...
                If InStr(",5,6,7,", .收费类别) > 0 Then
                    If .Detail.分批 Or .Detail.变价 Then
                        '问题号:113637,焦博,2017/09/15,在保存数据之前需要检查药品库存,调用GetDrugTotal过程
                        '读取药品库存时没有传入批次,导致库存检查失效
                        dblTotal = GetDrugTotal(mobjBill, .收费细目ID, .执行部门ID, .Detail.批次)
                        .Detail.库存 = GetStock(.收费细目ID, .执行部门ID, 0) '29680
                        If mbln药房单位 Then
                            .Detail.库存 = .Detail.库存 / .Detail.药房包装
                        End If
        ...
        '刷卡消费验卡
        If mint病人来源 = 1 And mint记录性质 = 2 And gSysPara.bln消费验证 Then
            curTotal = CalcGridToTal
            If curTotal > 0 Then
                If Not gobjDatabase.PatiIdentify(Me, glngSys, mobjBill.病人ID, curTotal) Then Exit Sub
            End If
        End If
        '74231,冉俊明,2014-6-24,项目开单后立即收费或记帐审核
        If mobjSquareCard Is Nothing Then
            If mint病人来源 = 1 And gSysPara.bln开单后立即结算 Then
        ...
        '74231,冉俊明,2014-6-24,项目开单后立即收费或记帐审核
        If mint病人来源 = 1 And gSysPara.bln开单后立即结算 And strNos <> "" Then
            If Not mobjSquareCard Is Nothing Then
                Call mobjSquareCard.zlSquareAffirm(Me, p医嘱附费管理, mstrPrivs, mlng病人ID, , , mint记录性质, strNos)
            End If
        End If
...
'返回：不一致的费用行,为0时正常
    ...
    For i = 1 To mobjBill.Details.Count
        ...
        If mobjBill.Details(i).Detail.服务对象 = 1 And mint病人来源 <> 1 Then
            MsgBox "第 " & i & " 行项目""" & mobjBill.Details(i).Detail.名称 & """仅服务于门诊,该病人不能使用.", vbInformation, gstrSysName
            Check服务对象 = i: Exit Function
        End If

        If mobjBill.Details(i).Detail.服务对象 = 2 And mint病人来源 <> 2 Then
        ...
        With objBill.Details(i)
            ...
            If blnNew Then
                ...
                If InStr(",5,6,7,", .收费类别) > 0 And mbln药房单位 Then
                    ...
                    rsTmp!单价 = Format(dbl单价 / .Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)
                Else
                    rsTmp!数量 = IIf(.付数 = 0, 1, .付数) * .数次
                    rsTmp!单价 = Format(dbl单价, gSysPara.Price_Decimal.strFormt_VB)
                End If
                rsTmp!实收金额 = Format(cur实收, gSysPara.Money_Decimal.strFormt_VB)
            ...
            Else
                ...
                If InStr(",5,6,7,", .收费类别) > 0 And mbln药房单位 Then
                    ...
                    rsTmp!单价 = Format((rsTmp!单价 + Format(dbl单价 / .Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)) / 2, gSysPara.Price_Decimal.strFormt_VB)
                Else
                    rsTmp!数量 = rsTmp!数量 + IIf(.付数 = 0, 1, .付数) * .数次
                    rsTmp!单价 = Format((rsTmp!单价 + Format(dbl单价, gSysPara.Price_Decimal.strFormt_VB)) / 2, gSysPara.Price_Decimal.strFormt_VB)
                End If
                rsTmp!实收金额 = rsTmp!实收金额 + Format(cur实收, gSysPara.Money_Decimal.strFormt_VB)
            End If
...
Private Sub Form_Load()
    ...
    gSysPara.strLike = IIf(gobjDatabase.GetPara("输入匹配") = "0", "%", "")
    gSysPara.bytCode = Val(gobjDatabase.GetPara("简码方式"))
    mblnSetControl = True
...
'                Call FindCboIndex(cbo开单科室, mobjBill.开单部门ID, False)
                Call Find开单科室(mobjBill.开单部门ID)
                Call GetCboIndex(cbo开单人, mobjBill.开单人)
                Call gobjControl.CboLocate(cboBaby, mobjBill.婴儿费, True)
                
                If gSysPara.bln从项汇总折扣 Then CalcMoneys
                Call ShowDetails
                Call ShowMoney
...
Private Sub sta_PanelClick(ByVal Panel As MSComctlLib.Panel)
    If Panel.Key = "BarCode" Then
        ...
        If txtBarCode.Enabled And txtBarCode.Visible Then txtBarCode.SetFocus
        Call gobjDatabase.SetPara("上次选择条码控制", IIf(mblnShowBarCode, 1, 0), glngSys, p医嘱附费管理)
        Exit Sub
    End If
    
    If Not gSysPara.bln简码匹配方式切换 Then Exit Sub
    ...
    If Panel.Bevel = sbrRaised And (Panel.Key = "PY" Or Panel.Key = "WB") Then
        ...
        End If
        Call gobjDatabase.SetPara("简码方式", IIf(sta.Panels("PY").Bevel = sbrInset And sta.Panels("WB").Bevel = sbrInset, 2, IIf(sta.Panels("WB").Bevel = sbrInset, 1, 0)))
...
Private Sub bill_AfterAddRow(Row As Long)
    ...
    With Bill
        ...
        If mbytInState <> 2 Then
            .ColData(BillCol.类别) = IIf(gSysPara.bln收费类别, BillColType.ComboBox, BillColType.UnFocus) '类别列,当主从项时会被改变
...
Private Sub cbo开单科室_KeyPress(KeyAscii As Integer)
    ...
    If InStr(mstrPrivs, ";所有科室;") = 0 Then Exit Sub
...
'        lngIdx = gobjControl.CboMatchIndex(cbo开单科室.Hwnd, KeyAscii)
    ...
    If KeyAscii = 13 Then
        ...
        If cbo开单人.ListIndex <> -1 Then
            ...
            If strText <> cbo开单人.List(cbo开单人.ListIndex) Then
                Call gobjControl.CboSetIndex(cbo开单人.hWnd, -1)
            Else
                gobjCommFun.PressKey vbKeyTab: Exit Sub
            End If
        End If
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        ...
        Case vbKeyF6 '清除当前单据内容,进入新单状态
            If mbytInState = 0 Then
                txt实收.Text = gSysPara.Money_Decimal.strFormt_VB: txt应收.Text = gSysPara.Money_Decimal.strFormt_VB
                Call ClearRows: Call Bill.ClearBill
                Call SetColNum: Call ClearMoney
                Call NewBill
                Bill.SetFocus
...
Private Function InitData() As Boolean
    ...
    '开单科室
    If mbyt应用场合 = 0 Or mlng医嘱ID <> 0 Then
        strSQL = "Select 开嘱科室ID,开嘱医生,病人来源,Decode(Nvl(病人来源,0),4,'',挂号单) As 挂号单 From 病人医嘱记录 Where ID=[1]"
        Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng医嘱ID)
        If Not rsTmp.EOF Then
            mlng开嘱科室ID = Nvl(rsTmp!开嘱科室id, 0)
            mstr开嘱医生 = Nvl(rsTmp!开嘱医生)
            mstrRegNo = Nvl(rsTmp!挂号单)  '99842,体检病人挂号单存的是体检单号
            If Val(Nvl(rsTmp!病人来源, 0)) = 4 Then
                mbyt应用场合 = 1 '如果是体检病人，则修改应用场合，以便填写划价单病人来源为体检
            End If
        End If
    ...
    ElseIf mlng开单科室ID = 0 Then
        If mstrInNO <> "" Then
            strSQL = _
            " Select  A.开单部门ID ,A.病人ID" & IIf(mstrFeeTab = "住院费用记录", ",A.主页ID ", ",0 as 主页ID") & _
            " From " & mstrFeeTab & " A " & _
            " Where Rownum<2 And  NO=[1] And A.记录性质=[2]"
            Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrInNO, mint记录性质)
            If rsTmp.EOF = False Then
                mlng开单科室ID = Val(Nvl(rsTmp!开单部门ID))
                mlng病人ID = Val(Nvl(rsTmp!病人ID))
                mlng主页ID = Val(Nvl(rsTmp!主页ID))
            End If
            If mlng开单科室ID = 0 Then
                MsgBox "未找到单据号:" & mstrInNO & "的记录.", vbInformation, gstrSysName
                Exit Function
            End If
        Else
            MsgBox "指定的开单科室不存在。", vbInformation, gstrSysName
            Exit Function
        End If
    End If
    ...
    "Order By 优先级,编码"
    Set mrsAll开单科室 = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption)
    ...
    Set mrsDept = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng开单科室ID, mlng开嘱科室ID, strOperDoc)
    ...
    Set mrsClass = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mstr收费类别)
    ...
    Set mrsUnit = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mint病人来源)
    If mrsUnit.EOF Then
                    ...
                    Else
                        ...
                        Select Case .Detail.执行科室
                            Case 0 '不明确
                                mrsUnit.Filter = 0
                                '101736,手工记帐缺省执行科室
                                If mint病人来源 = 2 And Not blnEnter Then
                                    ...
                                    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, .收费细目ID, 2)
                            ...
                            Case 4 '指定科室
                                ...
                                Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, .收费细目ID, mint病人来源, Val(Nvl(mrsInfo!科室ID, 0)))
                                If Not rsTmp.EOF Then
...
'功能：根据表单要完成的功能设置界面布局
    ...
    With Bill
        ...
        If mbytInState = 0 Then
            ...
            .ColData(BillCol.类别) = IIf(gSysPara.bln收费类别, 3, 5)
...
'    mobjCard.卡号有效字符 = "0" '输入类型(0-所有字符,1-数字,2-字母;3-数字或字母;4-指定字符)|Ascii码1，Ascii码2....
    ...
    sta.Panels("PY").Visible = mbytInState = 0 And gSysPara.bln简码匹配方式切换 '35242
    sta.Panels("WB").Visible = mbytInState = 0 And gSysPara.bln简码匹配方式切换
    sta.Panels("BarCode").Visible = mbytInState = 0
    If mbytInState = 0 Then
        '简码匹配方式：0-拼音,1-五笔
        i = Val(gobjDatabase.GetPara("简码方式"))
        If i = 0 Then
    ...
    Else
        ...
        End If
    End If
    txt应收.Text = gSysPara.Money_Decimal.strFormt_VB: txt实收.Text = gSysPara.Money_Decimal.strFormt_VB
...
'功能：获取病人信息
    ...
    If mint病人来源 = 2 Or mstrRegNo = "" Then
        strSQL = "Select" & _
    ...
    Else
        strSQL = "Select" & _
            ...
            "       A.担保人," & IIf(mint病人来源 = 2 And mint记录性质 = 2, "Decode(A.担保额,null,A.担保额,Zl_Patientsurety(A.病人ID,B.主页ID)) 担保额,", "A.担保额,") & _
            "       Nvl(B.医疗付款方式,A.医疗付款方式) as 医疗付款方式,Y.编码 as 付款码,zl_PatiWarnScheme(A.病人ID,B.主页ID) as 适用病人," & _
            "       B.住院医师,zl_PatiDayCharge(A.病人ID) as 当日额,Nvl(C.险类,A.险类) as 险类,Nvl(B.病人性质,0) as 病人性质,B.审核标志,B.备注 as 病人备注," & _
            "       NVL(B.病人类型,A.病人类型) as 病人类型" & _
            " From 病人信息 A,病案主页 B,病人挂号记录 C,病人余额 X,医疗付款方式 Y" & _
            " Where A.病人ID=B.病人ID(+) And A.病人ID=X.病人ID(+) And X.类型(+) = " & IIf(mint病人来源 = 1, 1, 2) & strSQL & _
            " And A.病人ID=[1] And B.主页ID(+)=[2] And A.医疗付款方式=Y.名称(+) And A.病人ID=C.病人ID And C.NO=[4]"
    End If
    On Error GoTo errH
    Set mrsInfo = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病人ID, lng主页Id, mlng病人科室id, mstrRegNo)
    If Not mrsInfo.EOF Then
        mstr住院医生 = Nvl(mrsInfo!住院医师)
        If Not IsNull(mrsInfo!险类) And Nvl(mrsInfo!病人类型) = "" Then
            txt姓名.ForeColor = vbRed
        Else
            txt姓名.ForeColor = zlDatabase.GetPatiColor(Nvl(mrsInfo!病人类型))
        End If
        ...
        If mint记录性质 = 2 Then
            ...
            End If
            sta.Panels(3).Text = "预交:" & Format(Val(cmdOK.Tag), "0.00")
            sta.Panels(3).Text = sta.Panels(3).Text & "/费用:" & Format(Val(cmdCancel.Tag), gSysPara.Money_Decimal.strFormt_VB)
            ...
            Set mrsWarn = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(Nvl(mrsInfo!病区ID, 0)), CStr(Nvl(mrsInfo!适用病人)))
...
'说明：ExpenseBill集合的索引对应单据的行号
    ...
    For i = IIf(lngRow = 0, 1, lngRow) To IIf(lngRow = 0, mobjBill.Details.Count, lngRow)
        ...
        bln从项汇总折扣 = False
        If gSysPara.bln从项汇总折扣 And Not mbln费用登记 Then                    '如果主项屏蔽费别,则汇总计算折扣参数无效,不汇总计算
    ...
    If gSysPara.bln从项汇总折扣 And Not mbln费用登记 Then
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    gstrSQL = _
        ...
        " And A.ID=[1]"
    Set rsTmp = gobjDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mobjBill.Details(lngRow).收费细目ID)
    If Not rsTmp.EOF Then
        With mobjBill.Details(lngRow)
            '先获取操作员以前输入的变价金额
            If .Detail.变价 Then
                If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                    ...
                    If dblAllTime <> 0 Then
                        dblPrice = Get时价药品应收金额(.执行部门ID, .收费细目ID, .收费类别 = "4", dblAllTime, gSysPara.Money_Decimal.strFormt_VB, dblPriceSingle, .Detail.批次)
                        If dblAllTime <> 0 Then
                            '数量未分解完毕
                            If InStr(",5,6,7,", .收费类别) > 0 Then
                        ...
                        Else
                            ...
                            End If
                            dblMoney = IIf(dblPriceSingle = 0, Format(dblPrice / dblAllTime, gSysPara.Price_Decimal.strFormt_VB), dblPriceSingle) '这里结果是按售价单位
                        End If
                    Else
                        dblMoney = 0
                    End If
            ...
            With mobjBillIncome
                ...
                If mobjBill.Details(lngRow).Detail.变价 Then
                    If InStr(",5,6,7,", mobjBill.Details(lngRow).收费类别) > 0 And mbln药房单位 Then
                        .标准单价 = Format(dblMoney * mobjBill.Details(lngRow).Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)
                    Else
                        .标准单价 = Format(dblMoney, gSysPara.Price_Decimal.strFormt_VB)
                    End If
                Else
                    If InStr(",5,6,7,", mobjBill.Details(lngRow).收费类别) > 0 And mbln药房单位 Then
                        .标准单价 = Format(Nvl(rsTmp!现价, 0) * mobjBill.Details(lngRow).Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)
                    Else
                        .标准单价 = Format(Nvl(rsTmp!现价, 0), gSysPara.Price_Decimal.strFormt_VB)
                    End If
                End If
                '应收金额=单价 * 付数 * 数次
                ...
                .应收金额 = CCur(Format(.应收金额, gSysPara.Money_Decimal.strFormt_VB))
                ...
                Else
                    ...
                    Else
                        .实收金额 = CCur(Format(ActualMoney(mobjBill.费别, .收入项目ID, .应收金额, _
                            ...
                            dblAllTime, dbl加班加价率), gSysPara.Money_Decimal.strFormt_VB))
                    End If
                End If
                ...
                If Not IsNull(mrsInfo!险类) Then
                    ...
                    If strInfo <> "" Then
                        ...
                        .统筹金额 = Format(Val(Split(strInfo, ";")(2)), gSysPara.Money_Decimal.strFormt_VB)
        ...
        Select Case Bill.TextMatrix(0, i)
            ...
            Case "单价"
                ...
                If mobjBill.Details(lngRow).InComes.Count > 0 Then
                    ...
                    Next
                End If
                Bill.TextMatrix(lngRow, i) = Format(dbl单价, gSysPara.Price_Decimal.strFormt_VB)
            Case "应收金额"
                '应收金额是该收费细目所有收入项目的合计
                cur金额 = 0
                If mobjBill.Details(lngRow).InComes.Count > 0 Then
                    ...
                    Next
                End If
                Bill.TextMatrix(lngRow, i) = Format(cur金额, gSysPara.Money_Decimal.strFormt_VB)
            Case "实收金额"
                '实收金额是该收费细目所有收入项目的合计
                cur金额 = 0
                If mobjBill.Details(lngRow).InComes.Count > 0 Then
...
'功能：刷新显示收入项目费用区
    ...
    For i = 1 To mcolMoneys.Count
        ...
        mshMoney.TextMatrix(i, 1) = Format(mcolMoneys(i).实收金额, gSysPara.Money_Decimal.strFormt_VB)
        cur实收 = cur实收 + mcolMoneys(i).实收金额
        cur应收 = cur应收 + mcolMoneys(i).应收金额
    Next
    
    txt应收.Text = Format(cur应收, gSysPara.Money_Decimal.strFormt_VB)
    txt实收.Text = Format(cur实收, gSysPara.Money_Decimal.strFormt_VB)
...
Private Function GetInputDetail(ByVal lng项目ID As Long, ByVal lng批交 As Long) As Detail
'功能：读取收费项目信息
    ...
    On Error GoTo errH
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng项目ID, IIf(gSysPara.byt药品名称显示 = 1, 3, 1), lngMediCareNO)
    With objDetail
        .ID = rsTmp!ID
...
'说明：仅该行收费项目有从属项目及尚未取才取。
    ...
    On Error GoTo errH
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mobjBill.Details(lngRow).收费细目ID)
...
'出口:保存是否成功
    ...
    Dim rsTmp As ADODB.Recordset
    Dim lng医疗小组ID As Long, lng病人科室ID As Long, lng病区ID As Long
    Dim blnTrans As Boolean
    
    strNos = ""
    If mstrOriginalNO = "" Then
        If mint记录性质 = 1 And mint病人来源 <> 2 Then
    ...
    mobjBill.登记时间 = gobjDatabase.CurrentDate
    
 
    If mlng医嘱ID <> 0 And mint病人来源 = 2 Then
        Call GetPatiInforFromAdvice(mlng医嘱ID, mlng病人ID, mlng主页ID, lng病人科室ID, lng病区ID, lng医疗小组ID)
    End If
    If lng病人科室ID = 0 Then
            ...
            For Each mobjBillIncome In mobjBillDetail.InComes
                ...
                With mobjBill
                    If mint病人来源 = 2 Then
                        gstrSQL = "zl_住院记帐记录_INSERT('" & .NO & "'," & int序号 & "," & .病人ID & "," & ZVal(.主页ID) & "," & _
                            ...
                            ZVal(lng病区ID) & "," & ZVal(lng病人科室ID) & "," & .加班标志 & "," & .婴儿费 & "," & .开单部门ID & ",'" & .开单人 & "',"
                    Else
                        If mint记录性质 = 2 Then
                ...
                With mobjBillIncome
                    ...
                    If InStr(",5,6,7,", mobjBillDetail.收费类别) > 0 And mbln药房单位 Then
                        dbl单价 = Format(.标准单价 / mobjBillDetail.Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)
                    End If
    ...
    '                End If
                ...
                With mobjBillDetail
                    Select Case .收费类别
                    Case "4"    '卫材
                        If (mint病人来源 = 1 And mint记录性质 = 2 And gSysPara.bln门诊自动发料 Or mint病人来源 = 2 And gSysPara.bln住院自动发料) And int划价 = 0 Then
                        ...
                        End If
                    Case "5", "6", "7"  '药品
                            If gSysPara.bln收费后自动发药 And mint病人来源 = 1 And int划价 = 0 Then
                                   If .执行部门ID <> 0 And Not gSysPara.bln分离发药 Then
                ...
                If mint病人来源 = 2 Then
                    gstrSQL = gstrSQL & int划价 & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
                        ...
                        IIf(mobjBill.开单部门ID = mlng开嘱科室ID, "1", "0") & "," & mlng开单科室ID & ",NULL" & _
                        IIf(lng医疗小组ID = 0, ",-1", "," & lng医疗小组ID) & ",0," & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                Else
                    If mint记录性质 = 2 Then
                        gstrSQL = gstrSQL & int划价 & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
                            ...
                            gstrSQL = gstrSQL & "" & IIf(mbyt应用场合 = 1, 4, 1) & ","
                            '  中药形态_In   门诊费用记录.结论%Type := Null,
                            gstrSQL = gstrSQL & "Null,"
                            '  备货材料_In   Number := 0,
                            gstrSQL = gstrSQL & "0,"
                            '  批次_In       药品收发记录.批次%Type := Null
                            gstrSQL = gstrSQL & "" & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                    Else
                        gstrSQL = gstrSQL & "'" & UserInfo.姓名 & "'," & _
                            IIf(mobjBillDetail.收费类别 = "4", mlng卫材类别ID, mlng药品类别ID) & "," & _
                            "'" & mobjBillDetail.摘要 & "'," & ZVal(mlng医嘱ID) & ",NULL,NULL,NULL,NULL,NULL," & _
                            IIf(mbyt应用场合 = 1, 4, 1) & "," & _
                            "'" & mobjBillDetail.保险编码 & "','" & mobjBillDetail.Detail.类型 & "'," & IIf(mobjBillDetail.保险项目否, 1, 0) & "," & _
                            ZVal(mobjBillDetail.保险大类ID) & ",Null,0," & "" & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                    End If
                End If
        ...
        gcnOracle.BeginTrans: blnTrans = True
            ...
            If mblnSendMateria Then
                ...
                If rsTmp.RecordCount > 0 Then
                    str汇总号 = gobjDatabase.GetNextNo(20)
...
'      blnDelete=是否读取要退费的单据
    ...
    If mbytInState = 1 Then
        If mint记录性质 = 1 Or (mint记录性质 = 2 And mint病人来源 = 1) Then
            blnNOMoved = gobjDatabase.NOMoved("门诊费用记录", strNO, "记录性质=", mint记录性质)
        Else
            blnNOMoved = gobjDatabase.NOMoved("住院费用记录", strNO, "记录性质=", mint记录性质)
        End If
    End If
    ...
    End If
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, strNO, mint记录性质, _
    ...
    Call gobjControl.CboLocate(cboBaby, Nvl(rsTmp!婴儿费, 0), True)
    ...
    If mint记录性质 = 2 Then
        ...
        If Not rsPatiMoney Is Nothing Then
            sta.Panels(3).Text = "预交:" & Format(rsPatiMoney!预交余额, "0.00") & _
                "/费用:" & Format(rsPatiMoney!费用余额, gSysPara.Money_Decimal.strFormt_VB) & _
    ...
    Else
        ...
        strSQL = _
            ...
            " Order by 序号"
    End If
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, strNO, mint记录性质, IIf(mint记录性质 = 2, ",9,25,", ",8,24,"), _
        IIf(mblnDelete, "2", "0,1,3"), CDate(IIf(mstrTime = "", "1990-01-01", mstrTime)), IIf(gSysPara.byt药品名称显示 = 1, 3, 1), intSign, mlng医嘱ID)
    If rsTmp.EOF Then
        If blnDelete Then
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        Bill.TextMatrix(i, BillCol.单价) = Format(rsTmp!单价, gSysPara.Price_Decimal.strFormt_VB)
        Bill.TextMatrix(i, BillCol.应收金额) = Format(rsTmp!应收金额, gSysPara.Money_Decimal.strFormt_VB)
        Bill.TextMatrix(i, BillCol.实收金额) = Format(rsTmp!实收金额, gSysPara.Money_Decimal.strFormt_VB)
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        mshMoney.TextMatrix(i, 1) = Format(rsTmp!实收金额, gSysPara.Money_Decimal.strFormt_VB)
        curTotal = curTotal + rsTmp!实收金额
        cur应收Total = cur应收Total + rsTmp!应收金额
        rsTmp.MoveNext
    Next
    
    txt实收.Text = Format(curTotal, gSysPara.Money_Decimal.strFormt_VB)
    txt应收.Text = Format(cur应收Total, gSysPara.Money_Decimal.strFormt_VB)
...
'            " Order by B.服务对象,A.编码"
    ...
    Set mrsWork = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mint病人来源, lng开单科室id, lng药品ID, str药房, bytDay)
    GetWorkUnit = True
    Exit Function
errH:
    If gobjComlib.ErrCenter() = 1 Then Resume
...
'功能：读取当前开单科室中包含的所有人员
    ...
    On Error GoTo errH
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng科室id)
...
'功能：根据当前病人的类型判断指定行的项目是否可以输入,适用于所有类别的项目
    ...
    '读取检查数据
    If bytType = 1 Then
        strSQL = "Select 编码,名称,简码,性质,缺省标志 From 费用类型 Where 编码 In(" & gSysPara.str医保费用类型 & ") Order by 编码"
    Else
        strSQL = "Select 编码,名称,简码,性质,缺省标志 From 费用类型 Where 编码 In(" & gSysPara.str公费费用类型 & ") Order by 编码"
    End If
    On Error GoTo errH
...
'功能：初始化费用本机参数
    ...
    mblnPay = Val(gobjDatabase.GetPara("中药输入付数", glngSys, p医嘱附费管理)) <> 0
    mblnTime = Val(gobjDatabase.GetPara("变价输入数次", glngSys, p医嘱附费管理)) <> 0
    mbln其它药房 = Val(gobjDatabase.GetPara("显示其它药房库存", glngSys, p医嘱附费管理)) = 1
    mbln其它药库 = Val(gobjDatabase.GetPara("显示其它药库库存", glngSys, p医嘱附费管理)) = 1
    mstr收费类别 = gobjDatabase.GetPara("收费类别", glngSys, p医嘱附费管理, "")
    
    '药品单位
    mbln药房单位 = Val(gobjDatabase.GetPara("药品单位", glngSys, p医嘱附费管理)) <> 0
    If mint病人来源 = 1 Then
        mstr药房单位 = "门诊单位": mstr药房包装 = "门诊包装"
    Else
        mstr药房单位 = "住院单位": mstr药房包装 = "住院包装"
    End If
    mbytSendMateria = Val(gobjDatabase.GetPara("记帐后发药", glngSys, p医嘱附费管理))
    mbyt缺省科室 = Val(gobjDatabase.GetPara("补费缺省科室", glngSys, p医嘱附费管理))
    '缺省药房
    ...
    mlng发料部门 = Val(gobjDatabase.GetPara(IIf(mint病人来源 = 2, "住院", "门诊") & "缺省发料部门", glngSys, p医嘱附费管理))
     
    mblnShowBarCode = Val(gobjDatabase.GetPara("上次选择条码控制", glngSys, p医嘱附费管理)) = 1
End Sub
...
Public Function zlCheck北京医保(ByVal intInsure As Integer) As Boolean
    ...
    strTemp = gobjCommFun.ShowMsgBox("处方类型", "请确定当前医保病人本次要发送的药品处方的类型。", "!医保内(&A),医保外(&B),?取消(&C)", Me)
...
Public Function zlCheck定价零价格对码(ByVal lng收费细目ID As Long, bln定价 As Boolean) As Boolean
   ...
   zlCheck定价零价格对码 = False
    If bln定价 Then
        ...
        "       And B.收费细目ID=[1]"
        Set rs价格 = gobjDatabase.OpenSQLRecord(strSQL, "获取当前价格", lng收费细目ID)
...
Public Function Get待发药清单(strNO As String, strTime As String) As ADODB.Recordset
    ...
    strSQL = _
        ...
        " Order by A.药品ID"
    If strTime <> "" Then
        Set Get待发药清单 = gobjDatabase.OpenSQLRecord(strSQL, "mdlInExse", strNO, 9, CDate(strTime))
    Else
        Set Get待发药清单 = gobjDatabase.OpenSQLRecord(strSQL, "mdlInExse", strNO, 9)
    End If
    Exit Function
errH:
    If gobjComlib.ErrCenter() = 1 Then Resume
...
Private Sub cmdSaveWholeSet_Click()
    ...
    If mint记录性质 = 1 Or (mint记录性质 = 2 And mint病人来源 = 1) Then
        blnNOMoved = gobjDatabase.NOMoved("门诊费用记录", mstrInNO, "记录性质=", mint记录性质)
    Else
        blnNOMoved = gobjDatabase.NOMoved("住院费用记录", mstrInNO, "记录性质=", mint记录性质)
    End If
   ...
   If mbytInState = 1 Then
        ...
        " Order by 序号"
        If mstrTime <> "" Then
            Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrInNO, mint记录性质, CDate(mstrTime))
        Else
            Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrInNO, mint记录性质)
        End If
        With rsTemp
            Do While Not .EOF
        ...
        With mobjBill
            ...
            For i = 1 To .Details.Count
                ...
                If InStr(",5,6,7,", .Details(i).收费类别) > 0 And mbln药房单位 Then
                    ...
                    dbl价格 = Format(dbl价格 / .Details(i).Detail.药房包装, gSysPara.Price_Decimal.strFormt_VB)
                End If
...
Public Function ImportWholeSet(frmParent As Object, ByVal intInsure As Integer, rsSel As ADODB.Recordset, Optional lng病人ID As Long = 0, _
    ...
    "       nvl(G.费别,F.费别) as 费别,NVL(NVL(Q.姓名,G.姓名),F.姓名) 姓名,NVL(NVL(Q.性别,G.性别),F.性别) 性别,NVL(NVL(Q.年龄,G.年龄),F.年龄) 年龄,F.担保额," & _
    ...
    "          病案主页 G,病人挂号记录 Q,收费项目别名 H,收费项目别名 E1,材料特性 W,药品规格 X,诊疗项目目录 M1" & _
    ...
    "       And A.收费细目ID=E1.收费细目ID(+) And E1.码类(+)=1 And E1.性质(+)=3" & _
    "       And A.病人ID=F.病人ID(+) And F.病人ID=G.病人ID(+) And F.病人ID=Q.病人ID(+) And F.门诊号=Q.门诊号(+) And F.主页ID=G.主页ID(+)" & _
    "       And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null) " & vbNewLine & _
    "       And Sysdate Between D.执行日期 And Nvl(D.终止日期,To_Date('3000-01-01','YYYY-MM-DD')) "
    
    If Not gSysPara.bln分离发药 Then
        gstrSQL = "Select * From (" & gstrSQL & ")"
    Else
        '分离发药时排开时价和分批药品或卫材
        gstrSQL = "Select * From (" & gstrSQL & ") Where Not( Instr(',5,6,7,',收费类别)>0 And (分批=1 Or 是否变价=1))"
    End If
    Set rsItems = gobjDatabase.OpenSQLRecord(gstrSQL, "mdlExse", strValue(0), strValue(1), strValue(2), strValue(3), strValue(4), strValue(5), strValue(6), strValue(7), strValue(8), strValue(9), strValue(10), IIf(gSysPara.byt药品名称显示 = 1, 3, 1), lng病人ID)
    '没有记录就是空单子
...
NextRecord: Do While Not .EOF
            ...
            If i = 1 Then
                ...
                objBill.发生时间 = gobjDatabase.CurrentDate   ' !发生时间
                objBill.登记时间 = gobjDatabase.CurrentDate
                objBill.多病人单 = 0
            End If
            '处理收费细目=====================================================
           ...
           lngDoUnit = Get收费执行科室ID(Val(Nvl(rsItems!病人ID)), Val(Nvl(rsItems!主页ID)), objBillDetail.收费类别, objBillDetail.收费细目ID, _
            ...
            If InStr(",5,6,7,", rsItems!收费类别) > 0 And gSysPara.bln分离发药 Then
            ...
            Do While Not rsItems.EOF
                ...
                If Val(Nvl(rsItems!是否变价)) = 1 Then
                    If InStr(",5,6,7,", rsItems!收费类别) > 0 Or (rsItems!收费类别 = "4" And Nvl(rsItems!跟踪在用, 0) = 1) Then
                        ...
                        If dblAllTime <> 0 Then
                            dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, CLng(Nvl(!收费细目ID)), rsItems!收费类别 = "4", dblAllTime, gSysPara.Money_Decimal.strFormt_VB, dblPriceSingle, objBillDetail.Detail.批次)
                            If dblAllTime <> 0 Then
                            ...
                            Else
                                '注意：货币型最多只能保留4位小数,且不四舍五入,所以需要手工舍入;而用其它型在计算精度上又有问题
                                objBillIncome.标准单价 = IIf(dblPriceSingle = 0, Format(dblPrice / (Val(Nvl(!数量))), gSysPara.Price_Decimal.strFormt_VB), dblPriceSingle)  '这里是售价价格
                            End If
                        Else
                            objBillIncome.标准单价 = 0
                        End If
                ...
                If InStr(",5,6,7,", rsItems!收费类别) > 0 And mbln药房单位 Then
                    objBillIncome.标准单价 = Format(objBillIncome.标准单价 * Nvl(rsItems!药房包装, 1), gSysPara.Price_Decimal.strFormt_VB)
                Else
...
'
            ...
            With objBillDetail
                ...
                If InStr(",5,6,7,", .收费类别) > 0 And gSysPara.bln分离发药 Then
...
Public Function zlGet医疗小组ID() As Long
    ...
    "               And A.科室ID=[3] "
    On Error GoTo errH
    Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID, mlng主页ID, lng开单部门ID)
...
Private Function CheckValid() As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查是否存在主费用
    '返回:如果存在主费用-False，不存在-True
    '编制:李南春
    '日期:2015/7/14 17:32:53
    '问题:86396
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strTable = IIf(mint病人来源 = 1, "门诊费用记录", "住院费用记录")
    strSQL = "Select 1 From 病人医嘱发送 A, " & strTable & " B " & _
            " Where A.NO=B.NO And A.记录性质=B.记录性质 And A.NO=[1] And A.发送号=[2]"
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrOriginalNO, mlng发送号)
    
    If rsTmp.RecordCount > 0 Then
        MsgBox "已存在一项主费用，不能继续添加！", vbInformation, gstrSysName
        Exit Function
    End If
    CheckValid = True
    Exit Function
...
Private Sub Find开单科室(ByVal lng科室id As Long)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:定位开单科室，如果没有则添加
    '返回:
    '编制:李南春
    '日期:2016/1/18 18:02:57
    '问题:91763
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    On Error GoTo Errhand
    If lng科室id <> 0 Then
        For i = 0 To cbo开单科室.ListCount - 1
            If cbo开单科室.ItemData(i) = lng科室id Then
                cbo开单科室.ListIndex = i: Exit Sub
            End If
        Next
    End If
    '没找到,自动增加
    strSQL = "Select ID,编码,名称,简码 From 部门表 Where ID=[1]"
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, "开单科室", lng科室id)
    If rsTmp.RecordCount = 0 Then Exit Sub
    If Not rsTmp.EOF Then
         If InStr(mstrPrivs, ";所有科室;") > 0 Then cbo开单科室.RemoveItem cbo开单科室.ListCount - 1
        cbo开单科室.AddItem IIf(zlIsShowDeptCode, Nvl(rsTmp!编码) & "-", "") & Nvl(rsTmp!名称)
...
Private Sub CreateDrugPacker()
    ...
    Err = 0: On Error Resume Next
    If Val(gobjDatabase.GetPara("启用药品自动化设备接口", glngSys, Val("9010-药品自动化设备接口"))) = 1 Then
        '优先新接口
    ...
    If mblnDrugMachine Then
        '权限检查
        strPrivs = gobjComlib.GetPrivFunc(glngSys, Val("9010-药品自动化设备接口"))
...
Private Function AddStuffItemFromBarCode(ByVal strBarCode As String) As Boolean
    ...
    Dim lngRow As Long, blnCancel As Boolean, str特准项目 As String
    Dim blnAdd As Boolean
    ...
    If SelectIsNurse = False Then
        If InStr(mstr收费类别, "'4'") = 0 And mstr收费类别 <> "" Then
    ...
    If intInsure <> 0 Then
        If zl_Check特准项目(gclsInsure, intInsure, lng病人ID, (mint记录性质 = 1 Or mint病人来源 = 1)) Then str特准项目 = Get保险特准项目(lng病人ID, "A.ID")
    End If
    
    mlng批次 = -1
    lng项目ID = frmItemSelect.ShowSelect(Me, mstrPrivs, mint病人来源, intInsure, True, str类别, strBarCode, txtBarCode.hWnd, str特准项目, -1, IIf(mbln费用登记, True, False), True, lng批次)
...
Private Function CheckYBFeeItemVerfiy(ByVal lng病ID As Long, ByVal intInsure As Integer, objDetail As Detail) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查医保费用项目是否审批、是否对码等且是否合法
    '入参:lng病ID-病人ID
    '     intInsure-险类
    '     bln要求审批-指定项目是否要求审批
    '     lng项目ID-项目ID
    ...
    If intInsure = 0 Or lng病ID = 0 Then CheckYBFeeItemVerfiy = True: Exit Function
    
    '1.保险支付项目对应检查
    If Not zlCheck定价零价格对码(objDetail.ID, Not objDetail.变价) Then
        If Not ItemExistInsure(lng病ID, objDetail.ID, intInsure) Then
            If gSysPara.int医保对码 = 1 Then
                If MsgBox("项目""" & objDetail.名称 & """没有设置对应的保险项目，要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            ElseIf gSysPara.int医保对码 = 2 Then
                MsgBox "项目""" & objDetail.名称 & """没有设置对应的保险项目。", vbInformation, gstrSysName
                Exit Function
            End If
        End If
    End If
    ...
    '2.医保费用项目是否审批检查
    If objDetail.要求审批 = False Or mrsMedAudit Is Nothing Then CheckYBFeeItemVerfiy = True: Exit Function
    If Not (mint病人来源 = 2 And mint记录性质 = 2) Then CheckYBFeeItemVerfiy = True: Exit Function
    ...
    If mrsMedAudit.RecordCount = 0 Then
        ...
        Exit Function
    End If
    dblNum = FormatEx(Val(Nvl(mrsMedAudit!可用数量)), 6)
    If dblNum <= 0 Then
        dblNum = dblNum / IIf(mbln药房单位, objDetail.药房包装, 1)
        MsgBox "当前病人使用[" & objDetail.名称 & "]已达到批准的使用限量" & FormatEx(dblNum, 5) & "。", vbInformation, gstrSysName
       Exit Function
    End If
