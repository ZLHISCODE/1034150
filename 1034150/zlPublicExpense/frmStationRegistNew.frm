...
Private Sub cmdOK_Click()
    ...
    Dim strDay As String, blnAppointPrint As Boolean, str付款方式 As String, lng病人ID As Long
...
Private Function LoadRegPlans(ByVal blnCache As Boolean) As Boolean
    ...
    If Not blnCache Then
        ...
        strTime = " Union " & _
                ...
                "      And Exists (Select 1 From 临床出诊安排 M,临床出诊表 N Where M.ID=A.ID And M.出诊ID=N.ID And N.发布时间 Is Not Null) And " & _
                "      a.Id Not In (Select 号源id" & vbNewLine & _
                "                   From 临床出诊记录" & vbNewLine & _
                "                   Where 出诊日期 = [6] And (开始时间 < Nvl(a.停诊开始时间,a.终止时间) Or 终止时间 > Nvl(a.停诊终止时间,a.开始时间)) And Nvl(是否锁定, 0) = 0)"
