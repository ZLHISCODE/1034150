...
'        .bln默认输入摘要 = Val(gobjDatabase.GetPara("默认输入摘要", glngSys, 9000, "1")) = 1
        ...
        .int预约挂号打印 = Val(gobjDatabase.GetPara("预约挂号单打印方式", glngSys, 1115, "0"))
...
Private Sub cmdOK_Click()
    ...
    Dim strTimeCheck As String, str医生姓名 As String
    ...
    ReadRegistPrice Val(Split(vsfArrange.TextMatrix(vsfArrange.Row, vsfArrange.ColIndex("IDS")), ",")(1)), chkBook.Value = 1, False, txtFeeType.Text, rsItems, rsIncomes
    ...
    If mblnAppointment Then
        If CDate(strTimeCheck) < CDate(gobjDatabase.CurrentDate) - mty_Para.lng预约有效时间 / 1 / 60 / 24 Then
...
Private Sub LoadRegPlans(ByVal blnCache As Boolean)
    ...
    If Not blnCache Then
        ...
        strSQL = "Select a.Id, b.号码 As 号别, b.号类, b.科室id, c.名称 As 科室, c.编码 As 科室编码, a.项目id, d.名称 As 项目, a.替诊医生id, a.替诊医生姓名, a.替诊开始时间, a.替诊终止时间, a.医生id, a.医生姓名 As 医生, Nvl(a.已挂数, 0) As 已挂," & vbNewLine & _
                ...
                "       a.是否序号控制 As 序号控制, a.上班时段 As 排班, a.号源id, a.缺省预约时间, a.开始时间, a.终止时间, a.出诊日期, a.是否分时段, a.是否独占  " & vbNewLine & _
                ...
                "       And Nvl(a.是否发布,0) = 1 "
        strSQL = strSQL & " And (a.开始时间 < Nvl(a.停诊开始时间,a.终止时间) Or a.终止时间 > Nvl(a.停诊终止时间,a.开始时间)) "
        strSQL = strSQL & "  And Not Exists (Select 1 From 临床出诊记录 Where Id=a.Id And 终止时间 < [6]) And a.开始时间 >= [9] "
...
Private Sub LoadUnitReg(ByVal lng记录ID As Long)
 '加载挂号合作单位控制信息
    ...
    strSQL = "Select 名称 As 合作单位, 控制方式, 序号, 数量 From 临床出诊挂号控制记录 Where 记录id = [1] And 性质 = 1 And 类型 = 1"
...
Private Sub LoadFeeItem(ByVal lngItemID As Long, ByVal blnBook As Boolean)
    ...
    strSQL = "Select 1 As 性质, a.类别, a.Id As 项目id, a.名称 As 项目名称, a.编码 As 项目编码, a.计算单位, a.屏蔽费别, 1 As 数次, c.Id As 收入项目id, c.名称 As 收入项目," & vbNewLine & _
            "       c.编码 As 收入编码, c.收据费目, b.现价 As 单价, Null As 从属父号" & vbNewLine & _
            ...
            "Union All" & vbNewLine & _
            "Select 2 As 性质, a.类别, a.Id As 项目id, a.名称 As 项目名称, a.编码 As 项目编码, a.计算单位, a.屏蔽费别, d.从项数次 As 数次, c.Id As 收入项目id," & vbNewLine & _
            "       c.名称 As 收入项目, c.编码 As 收入编码, c.收据费目, b.现价 As 单价, 1 As 从属父号" & vbNewLine & _
    ...
    If blnBook Then
        strSQL = strSQL & _
                "Union All" & vbNewLine & _
                "Select 3 As 性质, a.类别, a.Id As 项目id, a.名称 As 项目名称, a.编码 As 项目编码, a.计算单位, a.屏蔽费别, 1 As 数次, c.Id As 收入项目id, c.名称 As 收入项目," & vbNewLine & _
                "       c.编码 As 收入编码, c.收据费目, b.现价 As 单价, Null As 从属父号" & vbNewLine & _
                "From 收费项目目录 A, 收费价目 B, 收入项目 C, 收费特定项目 D" & vbNewLine & _
                "Where b.收费细目id = a.Id And b.收入项目id = c.Id And a.Id = d.收费细目id And d.特定项目 = '病历费' And Sysdate Between b.执行日期 And" & vbNewLine & _
                "      Nvl(b.终止日期, To_Date('3000-1-1', 'YYYY-MM-DD'))"
    End If
    strSQL = strSQL & " Order By 性质, 项目编码, 收入编码"
    Set mrsItems = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lngItemID)
    vsfMoney.Clear 1
    vsfMoney.Rows = 2
    ...
    Do While Not mrsItems.EOF
        With vsfMoney
            ...
            .TextMatrix(.Rows - 1, .ColIndex("应收金额")) = Format(Nvl(mrsItems!单价), "0.00")
            .TextMatrix(.Rows - 1, .ColIndex("实收金额")) = Format(GetActualMoney(txtFeeType.Text, mrsItems!收入项目ID, Nvl(mrsItems!单价), mrsItems!项目ID), "0.00")
