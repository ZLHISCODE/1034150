...
Public Function ImportBill(ByVal int来源 As Integer, ByVal str单据号 As String, _
    ...
    On Error GoTo errH
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, "mdlExpense", str单据号, int记录性质, IIf(gSysPara.byt药品名称显示 = 1, 3, 1))
                    ...
                    If !是否变价 = 1 Then
                        If InStr(",5,6,7,", !收费类别) > 0 Or (!收费类别 = "4" And Nvl(!跟踪在用, 0) = 1) Then
                            ...
                            If dblAllTime <> 0 Then
                                dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, CLng(!收费细目ID), !收费类别 = "4", dblAllTime, gSysPara.Money_Decimal.strFormt_VB, dblPriceSingle)
                                If dblAllTime <> 0 Then
                                    If !收费类别 = "4" Then
                                ...
                                Else
                                    objBillIncome.标准单价 = IIf(dblPriceSingle = 0, Format(dblPrice / (!付数 * !数次), gSysPara.Price_Decimal.strFormt_VB), dblPriceSingle)   '这里是售价价格
                                End If
                            Else
                                objBillIncome.标准单价 = 0
                            End If
                ...
                Do
                    ...
                    If !是否变价 = 1 Then
                        If InStr(",5,6,7,", !收费类别) > 0 Or (!收费类别 = "4" And Nvl(!跟踪在用, 0) = 1) Then
                            ...
                            dblAllTime = !付数 * !数次 '这里是售价数量
                            If dblAllTime <> 0 Then
                                dblPrice = Get时价材料应收金额(lng虚拟库房ID, CLng(!收费细目ID), Nvl(!批次, 0), dblAllTime, gSysPara.Money_Decimal.strFormt_VB, dblPriceSingle, True)
                                If dblAllTime <> 0 Then
...
Public Function GetPriceMoneyTotal(ByVal intType As Byte, lng病人ID As Long) As Double
    ...
    '记帐报警包含所有住院划价费用
    If intType = 1 Then
        blnAllFee = Val(gobjDatabase.GetPara(42, glngSys, 1150)) = 1
        If blnAllFee Then
            strWhere = ""
        Else
            strWhere = " And Nvl(主页ID,0) = (Select Nvl(住院次数,0) From 病人信息 Where 病人ID = [1])"
        End If
    Else
        strWhere = ""
    End If
...
Private Function GetStockSet(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, Optional ByVal lng批次 As Long = 0) As Recordset
    '功能:获取药品库存记录集
    Dim strSQL As String, byt出库方式 As Byte
    
    If bln卫材 Then
        byt出库方式 = Val(gobjDatabase.GetPara(156, glngSys))
    Else
        byt出库方式 = Val(gobjDatabase.GetPara(150, glngSys))
    End If
    ...
    On Error GoTo errH
    Set GetStockSet = gobjDatabase.OpenSQLRecord(strSQL, App.ProductName, lng药房ID, lng收费项目ID, lng批次)
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, _
    ...
    ByRef dblPriceSingle As Double, Optional ByVal lng批次 As Long = 0) As Currency
...
'      dblPriceSingle-只有一个批次时返回该批次的单价，避免根据数量先乘再除后因四舍五入引起不同的数量的单价不同
    ...
    For i = 1 To rsPrice.RecordCount
        ...
        End If
        If i = 1 Then
            dblPriceSingle = Format(rsPrice!时价, gSysPara.Price_Decimal.strFormt_VB)
        Else
            dblPriceSingle = 0
        End If
       
        dblPrice = dblPrice + Format(dblCurTime * Format(rsPrice!时价, gSysPara.Price_Decimal.strFormt_VB), strDec)
...
''        " From 住院费用记录 A,(" & strSQL & ") B Where A.NO=B.NO And A.医嘱序号=B.医嘱ID And A.记录性质 IN(2,12) And A.记录状态=1" & _
''        " Group by A.NO,Nvl(A.价格父号,A.序号) Having Sum(Nvl(A.结帐金额,0))<>0"
''    Set rsTmp = gobjDataBase.OpenSQLRecord(strSQL, "mdlPublic", lng发送号, lng医嘱ID)
''    If Not rsTmp.EOF Then
...
''            " )" & _
''        " Group by A.NO,Nvl(A.价格父号,A.序号) Having Sum(Nvl(A.结帐金额,0))<>0"
''    Set rsTmp = gobjDataBase.OpenSQLRecord(strSQL, "mdlPublic", lng发送号, lng医嘱ID, glngSys)
''    If Not rsTmp.EOF Then
''        If gSysPara.bytBillOpt = 1 Then
...
Public Function GetStuffStockSet(ByVal lng虚拟库房ID As Long, _
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String
    If Val(gobjDatabase.GetPara(156, glngSys)) = 0 Then '分批药品出库方式：0-按批次先进先出，1-按效期最近先出,效期相同，则再按批次先进先出
        strSQL = "Nvl(批次,0)"
    Else
        strSQL = "效期,Nvl(批次,0)" '效期为空则排在最后
    End If
    ...
    " Order by " & strSQL
    On Error GoTo errH
    Set GetStuffStockSet = gobjDatabase.OpenSQLRecord(strSQL, App.ProductName, lng虚拟库房ID, lng材料ID, lng批次)
    Exit Function
errH:
    If gobjComlib.ErrCenter() = 1 Then Resume
    Call gobjComlib.SaveErrLog
