...
Public mobjPlugIn As Object '73935,冉俊明,20114-7-3,将渠道定制的界面嵌入到病人信息编辑中
...
Private mblnNotQuery As Boolean
...
Private Sub cbo家庭地址_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii = 13 Then
        ...
        If gobjCommFun.ActualLen(cbo家庭地址.Text) > 50 Then Call gobjCommFun.PressKey(vbKeyTab): Exit Sub
...
Private Function CheckValied() As Boolean
    ...
    If mlng病人ID > 0 And mbln基本信息调整 And (mstr年龄 & mstr年龄单位 <> IIf(IsNumeric(txt年龄.Text), txt年龄.Text & cbo年龄单位.Text, txt年龄.Text) Or mstr性别 <> NeedName(cbo性别.Text) Or mstr姓名 <> txtPatient.Text Or _
        mstr身份证号 <> txt身份证号.Text Or mstr出生日期 <> txt出生日期.Text Or mstr出生时间 <> txt出生时间) Then
        If MsgBox("病人基本信息已发生改变，是否继续", vbInformation + vbYesNo, gstrSysName) = vbNo Then
            '记录病人原始信息
            txtPatient.Text = mstr姓名:  cbo性别.ListIndex = gobjControl.Cbo.FindIndex(cbo性别, mstr性别, True)
            txt年龄.Text = mstr年龄
            txt年龄.Tag = txt年龄.Text
            If mstr年龄单位 <> "" Then cbo年龄单位.ListIndex = gobjControl.Cbo.FindIndex(cbo年龄单位, mstr年龄单位, True): cbo年龄单位.Visible = True: txt年龄.Width = 690
    ...
    If gobjCommFun.ActualLen(cbo家庭地址.Text) > 50 Then
...
Public Function SaveAfterArrList(ByVal blnNewPati As Boolean, ByRef lng病人ID As Long) As Boolean
    ...
    For i = 0 To UBound(mstrArrListSql)
        gobjDatabase.ExecuteProcedure Replace(mstrArrListSql(i), "^新病人^", CStr(lng病人ID)), Me.Caption
    Next
    
    '110269:李南春,2017/5/4,保存HIS数据要提交EMPI数据，失败后所有数据都要回退
    ...
    mblnSavePati = True
    '74430,冉俊明,2014-7-7,挂号中的病人信息编辑功能中提供采集照片功能
    Call SavePatiPic(lng病人ID)
    '73935,冉俊明,20114-7-3,将渠道定制的界面嵌入到病人信息编辑中
    If Not mobjPlugIn Is Nothing And mlngPlugInHwnd <> 0 Then '保存插件附加信息
        On Error Resume Next
        Call mobjPlugIn.PatiInfoSaveAfter(lng病人ID)
...
Private Sub cmdOK_Click()
    ...
    Dim lng病人ID As Long, strNO As String, str病人ID As String
    ...
    Dim cllOtherInfo As New Collection, strSQL As String
    ...
    If mbytFun = 1 And (mstrCard <> "" Or txtPatient.Text <> "") Or (mbytFun = 0 And mlng病人ID <> 0) Or mbytFun = 2 Then
        ...
        strPati = _
        "zl_挂号病人病案_INSERT(" & byt类型 & "," & str病人ID & "," & txt门诊号.Text & "," & _
                  ...
                  Val(txt单位名称.Tag) & ",'" & txt单位电话.Text & "','" & txt单位邮编.Text & "','" & cbo家庭地址.Text & "'," & _
                  "'" & txt家庭电话.Text & "','" & txt家庭邮编.Text & "'," & strDate & ",''," & str出生日期 & ",'" & strMCAccount & "','" & "" & "'," & _
                  "NULL," & IIf(Trim(txt区域.Text) = "", "NULL,", "'" & Trim(txt区域.Text) & "',") & _
                   "'" & Trim(txtRegLocation.Text) & "','" & Trim(txt户口地址邮编.Text) & "'," & IIf(Trim(txt联系人身份证.Text) = "", "NULL,", "'" & Trim(txt联系人身份证.Text) & "',") & _
        ...
        Call Add健康卡相关信息(str病人ID, cllOtherInfo)
        ...
        End If
        For i = 1 To cllOtherInfo.Count
            strSQL = cllOtherInfo(i)
            If strSQL <> "" Then
                mlngListSql = mlngListSql + 1
                ReDim Preserve mstrArrListSql(mlngListSql - 1)
                mstrArrListSql(mlngListSql - 1) = strSQL
            End If
        Next
        If strCard <> "" Then
            mlngListSql = mlngListSql + 1
        ...
        If str其他关系 <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = str其他关系
        End If
    End If

    Call CloseIDCard
    mfrmMain.mlngNewPatiID = 999
    mfrmMain.lblInfo.Caption = "性别:" & cbo性别.Text & "   年龄:" & txt年龄.Text & IIf(cbo年龄单位.Visible, cbo年龄单位.Text, "") & _
                                                "   门诊号:" & txt门诊号.Text & "   费别:" & cbo费别.Text
...
'    End Select
    ...
    mbln扫描身份证 = False
    mbln扫描身份证签约 = IIf(gobjDatabase.GetPara("扫描身份证签约", glngSys, glngModul) = "1", 1, 0) = "1"
...
'功能：初始化必要数据
    ...
    mDateSys = gobjDatabase.CurrentDate
...
'      blnKeepIndex=是否保持原有的费别选择
    ...
    If bln初诊 Then
        mrs费别.Filter = "科室id=0"   'adFilterNone
    Else                        '不允许仅限初诊的项目
        mrs费别.Filter = "初诊=0 and 科室id=0"
    End If
...
Public Sub ShowMe(bytMode As Byte, frmParent As Object)
    ...
    If mbytFun = 1 And mstrCard <> "" Then  '刷卡自动产生"新病人"
        txtPatient.Text = "新病人"
        mbln基本信息调整 = False
        Call cmdOK_Click
    Else
...
'    txtPatient.MaxLength = zlGetPatiInforMaxLen.intPatiName
    ...
    mbln家庭地址输入 = Val(Nvl(gobjDatabase.GetPara("家庭地址输入方式", glngSys, 1111, 1), 1)) = 1
    mbln自动门诊号 = gobjDatabase.GetPara("自动门诊号", glngSys, 1111) = "1"
    If mbln自动门诊号 Then
        txt门诊号.Text = gobjDatabase.GetNextNo(3)
    End If
    mblnCheckNOValidity = Val(Nvl(gobjDatabase.GetPara("门诊号有效性检查", glngSys, 1111, 1), 1)) = 1
    Call InitTagPage
...
'        msh过敏.Enabled = False
'        msh过敏.FocusRect = flexFocusNone
'        vsLinkMan.Enabled = False
'        vsInoculate.Enabled = False
'        vsOtherInfo.Enabled = False
'        cboBloodType.Locked = True
'        cboBH.Locked = True
'        txtMedicalWarning.Locked = True
'        txtOtherWaring.Locked = True
'        cmdPicClear.Visible = False
'        cmdPicCollect.Visible = False
'        cmdPicFile.Visible = False
        chk复诊.Value = IIf(mbln复诊, 1, 0)
        chk复诊.Visible = True
...
Private Sub txt出生时间_Change()
    ...
    If InStr(mstrPlugChange, "出生日期") = 0 Then mstrPlugChange = mstrPlugChange & ",出生日期"
...
Private Function ReCalcOld(ByVal DateBir As Date, ByRef cbo年龄单位 As ComboBox, Optional ByVal lng病人ID As Long, Optional ByVal blnSetControl As Boolean = True) As String
...
'返回:年龄,年龄单位
    ...
    If cbo年龄单位.ListIndex = -1 Then
        ...
        txt年龄.Width = 1395
    Else
        If cbo年龄单位.Visible = False Then cbo年龄单位.Visible = True: txt年龄.Width = 690
    End If
...
Private Sub txt卡号_GotFocus()
    ...
    Call gobjControl.TxtSelAll(txt卡号)
End Sub
...
Private Sub txt门诊号_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii = 32 Then
        ...
        End If
        Call gobjCommFun.PressKey(vbKeyTab)
    ElseIf InStr("1234567890" & Chr(8), Chr(KeyAscii)) = 0 Then
        KeyAscii = 0
    End If
End Sub
...
Private Sub txt密码_GotFocus()
    Call gobjControl.TxtSelAll(txt密码)
...
Private Sub txt密码_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        If txt密码.Text = "" Then
            txt验证.Text = ""
            If tbcPage.Selected.Index = 0 Then txtPatient.SetFocus
        Else
            txt验证.SetFocus
        End If
    End If
...
Private Sub txt验证_GotFocus()
    Call gobjControl.TxtSelAll(txt验证)
...
Public Sub Add健康卡相关信息(ByVal str病人ID As String, ByRef colPro As Collection, Optional ByVal lng就诊ID As Long)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:健康卡数据处理
    '入参:
    '编制:56599
    ...
    With msh过敏
        If .Rows > 1 Then
            '清除该病人所有记录
            strSQL = " Zl_病人过敏药物_Delete(" & str病人ID & ")"
    ...
    With vsInoculate
        If .Rows > 1 Then
            '清除该病人所有记录
            strSQL = " Zl_病人免疫记录_Delete(" & str病人ID & ")"
            zlAddArray colPro, strSQL
...
Private Sub SavePatPicture(lng病人ID As Long, strFile As String)
    ...
    Dim rs As New Recordset
        ...
        If rs.EOF Then
            ...
            rs.Update
        End If
        If gobjDatabase.SavePicture(strFile, rs, "照片") = False Then
...
Private Function ReadPatPricture(lng病人ID As Long)
    ...
    Set rsData = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病人ID)
    If rsData.BOF = False Then
        strTmp = gobjDatabase.ReadPicture(rsData, "照片", strTmp)
        mstr采集图片 = strTmp
...
Private Sub zlQueryEMPIPatiInfo()
    ...
    With rsTmp
        ...
        If IsDate(txt出生日期.Text) Then
            !出生日期 = Format(txt出生日期.Text & " " & IIf(IsDate(txt出生时间.Text), txt出生时间.Text, "00:00"), "YYYY-MM-DD HH:MM:SS")
        Else
            !出生日期 = ""
        End If
    ...
    On Error Resume Next
    With mrsEMPIOut
        '104905:李南春,2017/1/12，查找到病人，或者读身份证查找到病人
        If mlng病人ID <> Val(Nvl(!病人ID)) And Val(Nvl(!病人ID)) <> 0 Then
            mlng病人ID = Val(Nvl(!病人ID))
            mblnNotQuery = True
            Call LoadPatientInfo
            mbytFun = 0: Me.Caption = "病人详细信息"
            mblnNotQuery = False
        End If
        ...
        If Nvl(!家庭地址) <> "" Then cbo家庭地址.Text = Nvl(!家庭地址)
        If Nvl(!家庭地址邮编) <> "" Then txt家庭邮编.Text = Nvl(!家庭地址邮编)
        If Nvl(!户口地址) <> "" Then txtRegLocation.Text = Nvl(!户口地址)
    ...
    If mlng病人ID <> 0 Then
        ...
        If strDiff <> "" Then
            strMsgInfo = "病人的 " & strDiff & " 与EMPI信息不一致，因此处不具有相应的权限，本次不会进行更新。"
        End If
