...
Private Sub vsExpense_DblClick()
    '双击查看
    If vsExpense.TextMatrix(vsExpense.Row, vsExpense.ColIndex("单据号")) = "" Then Exit Sub
    ...
    Call frmTechnicExpense.EditCard(mfrmParent, mstrPrivsAnnexFee, 1, mTYAdviceProperty.lng医嘱ID, mTYAdviceProperty.lng发送号, mTYAdviceProperty.lng病人ID, mTYAdviceProperty.lng主页Id, _
         IIf(mTYAdviceProperty.int病人来源 = 2, 2, 1), Val(vsExpense.TextMatrix(vsExpense.Row, vsExpense.ColIndex("记录性质"))), mTYAdviceProperty.lng开嘱科室ID, mTYAdviceProperty.lng病人科室ID, 0, "", mTYAdviceProperty.strNO, vsExpense.TextMatrix(vsExpense.Row, vsExpense.ColIndex("单据号")))
End Sub
...
Private Function LoadDataFromAdvices(ByVal blnDataMoved As Boolean) As Boolean
    ...
    If InStr(mTYAdviceProperty.str计费状态, ",1,") > 0 Then
        ...
        " Group by A.记录性质,Decode(B.记录状态,0,0,1),A.NO,B.费别,C.名称,B.开单人" & _
        ...
        ElseIf blnDataMoved Then
            strSQL1 = strSQL1 & " Union ALL " & Replace(strSQL1, strFeeTab, "H" & strFeeTab)
            strSQL1 = _
...
Private Function LoadAdvicePrice(ByVal blnMoved As Boolean) As Boolean
    ...
    For i = 1 To rsAdvice.RecordCount
        ...
        Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Tittle, Val(rsAdvice!诊疗项目ID), Val(Nvl(rsAdvice!相关ID, 0)), _
            CStr(Nvl(rsAdvice!标本部位, "None")), CStr(Nvl(rsAdvice!检查方法, "None")), Val(Nvl(rsAdvice!执行标记, 0)), mlng执行科室ID, mTYAdviceProperty.int病人来源)
            ...
            If int来源 = 1 Then
                If mTYAdviceProperty.int记录性质 = 1 Then
                    ...
                    arrSQL(UBound(arrSQL)) = lng项目id & ";" & _
                        ...
                        Nvl(rsAdvice!计价特性, 0) & ",1,'" & str保险编码 & "','" & str费用类型 & "'," & int保险项目否 & "," & ZVal(lng保险大类ID) & ")"
                Else
                    '生成门诊记帐单据
                    arrSQL(UBound(arrSQL)) = lng项目id & ";" & _
...
Public Function zlFuncPlugIn(ByVal frmMain As Object, ByVal Control As CommandBarControl) As Boolean
    ...
    strNO = vsExpense.TextMatrix(vsExpense.Row, vsExpense.ColIndex("单据号"))
...
Public Sub zlUpdateCommandBars(ByVal cbsMain As Object, ByVal Control As CommandBarControl)
    ...
    Case conMenu_Edit_Modify
        Control.Enabled = mTYAdviceProperty.lng医嘱ID <> 0 And (mTYAdviceProperty.int执行状态 = 0 Or mTYAdviceProperty.int执行状态 = 3) And _
...
Private Function GetAdviceMoney(ByVal strAdviceIdAndPayNums As String, _
    ...
    Dim strWith As String, strTemp As String
    Dim rsSub As ADODB.Recordset, str医嘱IDs As String
    Dim strSubWith As String
    
    On Error GoTo errHandle
    '85249:李南春,2015/5/29,with预交嵌套使用
    strWith = "" & _
    "   With 医嘱单据 as (  " & vbNewLine & _
    "   Select A.医嘱ID,a.发送号,A.记录性质,A.NO" & vbNewLine & _
    "   From 病人医嘱发送 A,Table(f_Num2list2([1])) B" & vbNewLine & _
    "   Where a.医嘱id=b.C1 and A.发送号=B.C2" & vbNewLine & _
    "   Union ALL " & vbNewLine & _
    "   Select A.医嘱ID,A.发送号,A.记录性质,A.NO " & vbNewLine & _
    "   From 病人医嘱附费 A,Table(f_Num2list2([1])) B" & vbNewLine & _
    "   Where a.医嘱id=b.C1 and A.发送号=B.C2 )"
    
    strTemp = "Select A.Id As 医嘱ID, a.相关id As 主ID, a.诊疗类别 From 病人医嘱记录 A,Table(f_Num2list2([1])) B Where a.相关id = b.C1"
    Set rsSub = gobjDatabase.OpenSQLRecord(strTemp, Tittle, strAdviceIdAndPayNums)
    If Not rsSub.EOF Then
        Do While Not rsSub.EOF
            str医嘱IDs = str医嘱IDs & "," & rsSub!主ID & ":" & rsSub!医嘱ID
            rsSub.MoveNext
        Loop
        If str医嘱IDs <> "" Then str医嘱IDs = Mid(str医嘱IDs, 2)
        strSubWith = "" & _
        "   ,医嘱关联单据 as (  " & vbNewLine & _
        "   Select A.医嘱ID,a.发送号,A.记录性质,A.NO,C.C1 As 主ID" & vbNewLine & _
        "   From 病人医嘱发送 A,Table(f_Num2list2([1])) B,Table(f_Num2list2([2])) C,病人医嘱发送 D" & vbNewLine & _
        "   Where a.医嘱id=c.c2 and A.发送号=B.C2 And b.c1=c.c1 And d.医嘱ID=b.c1 And d.发送号=b.c2 And a.No=d.No" & vbNewLine & _
        "   Union ALL " & vbNewLine & _
        "   Select A.医嘱ID,A.发送号,A.记录性质,A.NO,C.C1 As 主ID " & vbNewLine & _
        "   From 病人医嘱附费 A,Table(f_Num2list2([1])) B,Table(f_Num2list2([2])) C" & vbNewLine & _
        "   Where a.医嘱id=c.c2 and A.发送号=B.C2 And b.c1=c.c1 )"
