...
Begin VB.Form frmIllItemEdit 
   ...
   Begin TabDlg.SSTab sstMain 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "基本项目(&1)"
      TabPicture(0)   =   "frmIllItemEdit.frx":000C
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "fra(0)"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "常用科室(2)"
      TabPicture(1)   =   "frmIllItemEdit.frx":0028
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "fra(1)"
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "对应病种(&3)"
      TabPicture(2)   =   "frmIllItemEdit.frx":0044
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "fra(2)"
      Tab(2).ControlCount=   1
      TabCaption(3)   =   "适用范围(&4)"
      TabPicture(3)   =   "frmIllItemEdit.frx":0060
      ...
      Begin VB.Frame fra 
         ...
         Begin VB.Frame Frame3 
            Caption         =   "  选择上面所选适用范围的应用范围"
            Height          =   2775
            Left            =   120
            TabIndex        =   49
            Top             =   1680
            Width           =   5175
            Begin VB.OptionButton opt适用应用范围 
               Appearance      =   0  'Flat
               Caption         =   "应用于当前类别"
               ForeColor       =   &H80000008&
               Height          =   240
      ...
      Begin VB.Frame fra 
         ...
         Begin VSFlex8Ctl.VSFlexGrid vs病种 
            ...
            FormatString    =   $"frmIllItemEdit.frx":007C
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmIllItemEdit.frx":00CC
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmIllItemEdit.frx":03E6
            Key             =   ""
         EndProperty
      EndProperty
   End
...
'返回值:成功返回True,否则为False
    ...
    Dim int适用范围 As Integer
    Dim int适用应用范围 As Integer
    ...
    For n = 0 To opt适用范围.UBound
        If opt适用范围(n).Value = True Then
            int适用范围 = n
            Exit For
        End If
    Next
    
    For n = 0 To opt适用应用范围.UBound
        If opt适用应用范围(n).Value = True Then
            int适用应用范围 = n
            Exit For
        End If
    Next
    ...
    If mstrID = "" Then       '新增一条记录
        ...
        '  适用范围_In     In Number := 0 --适用范围:0-全院;1-门诊;2-住院
        gstrSQL = gstrSQL & "" & int适用范围 & ","
        '  适用应用_In     In Number := 0 --适用应用范围:0-应用于当前项目;1-应用于同级项目;2-应用于当前分类
        gstrSQL = gstrSQL & "" & int适用应用范围 & ")"
    Else    '修改
        ' Zl_疾病编码目录_Update
...
'重新显示某一行,用于刷新
    ...
    "          to_char(A.撤档时间,'yyyy-mm-dd') as 撤档时间,Decode(Nvl(A.适用范围,0),0,'全院',1,'门诊',2,'住院') As 适用范围 " & _
...
Public Function 疾病编辑(ByVal bln分类 As Boolean, ByVal str分类项目 As String, ByVal str分类项目ID As String, _
    ByVal str编码类别 As String, ByVal str类别名称 As String, Optional ByVal strID As String = "") As Boolean
...
'返回值:编辑成功返回True,否则为False
    ...
    strShow分类名称 = Mid(str分类项目, 1, 10) & IIF(Len(str分类项目) > 10, "...", "")
    
    opt应用范围(1).Caption = "应用于分类(" & strShow分类名称 & ")下所有项目"
    opt应用范围(2).Caption = "应用于分类(" & strShow分类名称 & ")及子分类下所有项目"
    opt应用范围(3).Caption = "应用于编码类别(" & str类别名称 & ")下所有项目"

    opt病种应用范围(1).Caption = "应用于分类(" & strShow分类名称 & ")下所有项目"
    opt病种应用范围(2).Caption = "应用于分类(" & strShow分类名称 & ")及子分类下所有项目"
    opt病种应用范围(3).Caption = "应用于编码类别(" & str类别名称 & ")下所有项目"

    opt适用应用范围(1).Caption = "应用于分类(" & strShow分类名称 & ")下所有项目"
    opt适用应用范围(2).Caption = "应用于分类(" & strShow分类名称 & ")及子分类下所有项目"
    opt适用应用范围(3).Caption = "应用于编码类别(" & str类别名称 & ")下所有项目"

    If strShow分类名称 = "无" Then
        opt应用范围(1).Enabled = False
        opt应用范围(2).Enabled = False
        opt病种应用范围(1).Enabled = False
        opt病种应用范围(2).Enabled = False
        opt适用应用范围(1).Enabled = False
        opt适用应用范围(2).Enabled = False
    End If
...
'        Height = Height - 450
    ...
    If strID <> "" Then
        ...
        gstrSQL = "select A.ID,A.编码,A.序号,A.附码,A.名称,A.简码 As 拼音码,A.五笔码,A.性别限制,A.疗效限制,A.分娩,A.手术类型,A.统计码,A.说明 " & _
                ",B.序号 as 统计序号,B.ID as 分类ID,B.名称 as 分类名称,B.编码范围,Nvl(A.适用范围,0) As 适用范围 " & _
        ...
        If rsTemp("适用范围") >= 0 And rsTemp("适用范围") <= opt适用范围.UBound Then
            opt适用范围(Val(rsTemp("适用范围"))).Value = True
        Else
            opt适用范围(0).Value = True
        End If
    Else
...
Private Sub txtEdit_Change(Index As Integer)
    If Index = 2 Then
        txtEdit(mconInt拼音码).Text = zlCommFun.SpellCode(txtEdit(mconInt名称).Text)
        txtEdit(mconInt五笔码).Text = zlGetSymbol(txtEdit(mconInt名称).Text, 1, mint简码长度)
    ElseIf Index = 3 Then
...
Private Sub GetDefineSize()
    ...
    mint简码长度 = rsTmp.Fields("五笔码").DefinedSize
