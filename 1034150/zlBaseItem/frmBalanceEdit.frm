...
Private Sub cmb_Click()
    ...
    ElseIf cmb.ListIndex = 2 Or cmb.ListIndex = 3 Then
        ...
        For Each ObjItem In Me.lvw场合.ListItems
            ...
            End If
        Next
    ElseIf cmb.ListIndex = 6 Then '一卡通不用于预交款和就诊卡
        For Each ObjItem In Me.lvw场合.ListItems
            If ObjItem.Text = "就诊卡" Or ObjItem.Text = "预交款" Then
                ObjItem.Checked = False
            End If
        Next
...
'返回值:编辑成功返回True,否则为False
    ...
    gstrSQL = "select a.名称,b.结算方式,b.缺省标志 from 结算场合 A,结算方式应用 B" & _
        " where b.应用场合(+)=a.名称 and b.结算方式(+)= [1] order by A.编码"
...
Private Sub ChangeServer()
    ...
    With lvw场合.SelectedItem
        ...
        If Me.cmb.ListIndex > 1 Then cmb_Click: Exit Sub '医保结算及代收款不能作为缺省项
        '应付款不能设置为缺省的结算方式
...
Private Sub lvw场合_ItemCheck(ByVal Item As MSComctlLib.ListItem)
    ...
    If Item.Text = "补结算" Then
        If cmb.ListIndex <> 0 And cmb.ListIndex <> 1 And cmb.ListIndex <> 6 And cmb.ListIndex <> 7 Then Item.Checked = False
    '代收款项只能应用于预交款,且不能为缺省
    ...
    ElseIf cmb.ListIndex = 7 Then
        '消费卡
        If Item.Text <> "收费" And Item.Text <> "结帐" And Item.Text <> "预交款" Then
            Item.Checked = False
        End If
