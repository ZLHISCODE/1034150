...
'功能：检查指定控件的日期是否小于上次终止日期
    ...
    If dateTemp < mdatBegin Then
        Call zlCommFun.ShowTipInfo(objText.hwnd, "不能小于上次转出的终止日期:" & Format(mdatBegin, "YYYY-MM-DD"))
        CheckLessBegin = True
    Else
        Call zlCommFun.ShowTipInfo(objText.hwnd, "")
    End If
End Function
...
'功能：检查最终截止日期是否小于本次截止日期
    ...
    If dateLast < dateThis Then
        Call zlCommFun.ShowTipInfo(txtDateThis.hwnd, "最终截止日期不能小于本次截止日期")
        CheckLessThis = True
    Else
        Call zlCommFun.ShowTipInfo(txtDateThis.hwnd, "")
    End If
End Function
...
Private Sub txtPati_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 And txtPati.Text <> "" Then
        ...
        vPoint = GetCoordPos(txtPati.Container.hwnd, txtPati.Left, txtPati.Top)
...
'参数：intNum=项目序号,为0时固定按年产生
    ...
    If intType = 1 Then
        ...
        GetFullNO = PreFixNO & gstrSQL & Format(Right(strNo, 4), "0000")
    Else
        '按年编号
        GetFullNO = PreFixNO & Format(Right(strNo, 7), "0000000")
    End If
