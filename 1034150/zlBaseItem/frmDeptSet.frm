...
Private Function Check床位状况(ByVal lng部门ID As Long, ByVal lng病区id As Long, ByVal int性质 As Integer, Optional ByVal intUnCheckType As Integer) As Boolean
    'int性质：0-部门性质检查;1-病区科室对应检查
    'intUnCheckType：部门性质检查时，取消选择的性质。0-护理，1-临床
    ...
    If int性质 = 0 Then
        ...
        If intUnCheckType = 0 Then '护理取消时
            strSQL = "Select 1 From 床位状况记录 Where (病区id = [1]) And Rownum = 1"
        Else '临床性质取消，先检查专用床位
            strSQL = "Select 1 From 床位状况记录 Where (科室id = [1]) And Rownum = 1"
        End If
    Else
        '病区科室对应检查：对应病区或科室取消时，检查床位记录
        strSQL = "Select 1 From 床位状况记录 Where (科室id = [1] And 病区id = [2]) And Rownum = 1"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "床位状况检查", lng部门ID, lng病区id)
...
Private Function Check病人状况(ByVal lng部门ID As Long, ByVal lng病区id As Long, ByVal int性质 As Integer, Optional ByVal intUnCheckType As Integer) As Boolean
    'int性质：0-部门性质检查;1-病区科室对应检查
    'intUnCheckType：部门性质检查时，取消选择的性质。0-护理，1-临床
    ...
    On Error GoTo ErrHandle
    If int性质 = 0 Then
        '部门性质检查：临床或护理性质取消时，在院病人
        If intUnCheckType = 0 Then '护理取消时
            strSQL = "Select 1 From 在院病人 Where 病区id = [1] And Rownum < 2"
        Else '临床性质取消，检查科室病人
            strSQL = "Select 1 From 在院病人 Where 科室id = [1] And Rownum < 2"
        End If
    Else
        '病区科室对应检查：对应病区或科室取消时
        strSQL = "Select 1 From 在院病人 Where (科室id = [1] And 病区id = [2]) And Rownum <2"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "在院病人检查", lng部门ID, lng病区id)
    Check病人状况 = (rsTmp.RecordCount > 0)
    Exit Function
ErrHandle:
    If ErrCenter() = 1 Then
        Resume
...
'    If gstrNodeNo <> "-" Then
    On Error GoTo ErrHandle
        ...
        frmDeptSet.Height = frmDeptSet.Height + 100
        tabMain.Height = tabMain.Height + 10
        fraMain(0).Height = fraMain(0).Height + 10
        fra基本信息.Height = fra基本信息.Height + 50
        fra说明.Top = fra说明.Top + 100
        lvw性质.Height = lvw性质.Height + 250
        fraMain(1).Height = fraMain(1).Height + 10
        Lvw科室.Height = Lvw科室.Height + 50

        cmdHelp.Top = cmdHelp.Top + 50
...
Private Sub cmb诊疗科目编码_KeyPress(KeyAscii As Integer)
    ...
    '        End If
            ...
            vRect = zlGetControlRect(cmb诊疗科目编码.hwnd)
...
Private Function Save部门() As Boolean
    ...
    If mstrID = "" Then       '新增一条记录
        ...
        lng部门ID = zlDatabase.GetNextId("部门表")
        ...
        gstrSQL = gstrSQL & ",'" & Trim(cbo环境类别.Text) & "'," & IIF(cmbStationNo.Visible = False Or cmbStationNo.Text = "", "Null", str站点)
            ...
            If frmDeptManage.tvwMain_S.SelectedItem.Key = IIF(mstr上级部门ID = "", "Root", "C" & mstr上级部门ID) Then
                ...
                For i = 2 To .ColumnHeaders.Count
                    Select Case .ColumnHeaders(i).Text
                        ...
                        Case "建档时间"
                            lst.SubItems(i - 1) = Format(zlDatabase.Currentdate, "yyyy-MM-dd")
                        Case "撤档时间"
    ...
    Else
        ...
        With frmDeptManage.tvwMain_S
            '增加到TreeView中
'            If mint编辑模式 = 1 Then
'                Set nod = .Nodes("C" & mstrID)
'                Set nod.Parent = .Nodes(IIF(mstr上级部门ID = "", "Root", "C" & mstr上级部门ID))
'                nod.Text = "【" & txtTemp.Text & txtEdit(1).Text & "】" & txtEdit(2).Text
'                ChangeCode nod, "【" & mstr编码 & "】", "【" & txtTemp.Text & txtEdit(1).Text
'            Else
'                Set nod = .Nodes(mstr上级码 & "|" & mstrID)
'            End If
        End With
        
'        If frmDeptManage.ActiveControl Is frmDeptManage.tvwMain_S Then
'            Dim intCol As Integer
'            '修改下级编码
'            With frmDeptManage.lvwMain
'                For i = 2 To .ColumnHeaders.Count
'                    If .ColumnHeaders(i).Text = "编码" Then
'                        intCol = i - 1
'                        Exit For
'                    End If
'                Next
'                If intCol > 0 Then
'                    For i = 1 To .ListItems.Count
'                        .ListItems(i).SubItems(intCol) = txtTemp.Text & txtEdit(1).Text & Mid(.ListItems(i).SubItems(intCol), Len(mstr编码) + 1)
'                    Next
'                End If
'            End With
'            nod.EnsureVisible
...
'                        Select Case .ColumnHeaders(i).Text
'                            Case "编码"
'                                .SelectedItem.SubItems(i - 1) = txtTemp.Text & txtEdit(1).Text
'                            Case "名称"
'                                .SelectedItem.Text = txtEdit(2).Text
'                            Case "简码"
'                                .SelectedItem.SubItems(i - 1) = txtEdit(3).Text
'                            Case "位置"
'                                .SelectedItem.SubItems(i - 1) = txtEdit(5).Text
'                            Case "上级部门"
'                                .SelectedItem.SubItems(i - 1) = txtEdit(4).Text
'                        End Select
'                    Next
'                    Call frmDeptManage.lvwMain_ItemClick(.SelectedItem)
'                End If
'            End With
'        End If
    End If
...
'    On Error GoTo errHandle
    ...
    Call zlControl.CboSetHeight(cmb诊疗科目编码, cmb诊疗科目编码.Height * 16)
...
'        lvw性质.Top = cmb诊疗科目编码.Top
    ...
    Else
        lbl诊疗科目编码.Top = lvw性质.Top + lvw性质.Height + 140
...
Private Sub Lvw科室_ItemCheck(ByVal Item As MSComctlLib.ListItem)
    If Item.Checked = False And Val(Lvw科室.ListItems(Item.Index).Tag) = 1 Then
        If Check床位状况(Val(mstrID), Val(Mid(Item.Key, 2)), 1) Then
            ...
            Item.Checked = True
            Exit Sub
        ElseIf Check病人状况(Val(mstrID), Val(Mid(Item.Key, 2)), 1) Then
            MsgBox "该部门存在在院病人，不能取消对应关系！", vbInformation, gstrSysName
            Item.Checked = True
            Exit Sub
        End If
    End If
...
Private Sub lvw性质_ItemCheck(ByVal Item As MSComctlLib.ListItem)
    ...
    If Item.Text = "临床" Or Item.Text = "护理" Then
        If Item.Checked = False And Val(Item.SubItems(2)) = 1 Then
            If Check床位状况(Val(mstrID), 0, 0, IIF(Item.Text = "护理", 0, 1)) Then
                MsgBox "该部门存在床位记录，不能取消该性质！", vbInformation, gstrSysName
                Item.Checked = True
                Exit Sub
            ElseIf Check病人状况(Val(mstrID), 0, 0, IIF(Item.Text = "护理", 0, 1)) Then
                MsgBox "该部门存在在院病人，不能取消对应关系！", vbInformation, gstrSysName
                Item.Checked = True
                Exit Sub
            End If
...
Private Sub txtEdit_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    ElseIf Index = 2 Or Index = 3 Then
        If LenB(StrConv(txtEdit(2).Text, vbFromUnicode)) >= 100 And (KeyAscii <> vbKeyDelete And KeyAscii <> vbKeyBack) Then
            KeyAscii = 0
        End If
    ElseIf Index = 5 Then
...
Private Sub txtFind_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        gstrSQL = " Select Distinct 编码,名称,ID From 部门表 " & _
         ...
         vRect = zlGetControlRect(txtFind.hwnd)
