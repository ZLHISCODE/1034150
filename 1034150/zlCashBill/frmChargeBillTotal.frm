...
Private mintRollingType As Integer  '轧帐类别
...
Public Function LoadChargeAndBillTotalData(ByVal frmMain As Object, _
    ...
    mintRollingType = intRollingType
    
    If Not mblnOlnyView Then
        If Not zlCheckPrivs(mstrPrivs, "轧帐") Then vsChagre.Editable = flexEDNone
    End If
...
Private Function LoadPersonChargeAndBill() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    'mintRollingType:轧帐类别(0-所有类别(按全额轧帐),1-收费,2-预交,3-结帐,4-挂号,5-就诊卡,6-消费卡)
    strRollingType = "": strWhere = ""
    strSQL = ""
    If InStr(",0,1,3,4,5,6,", "," & mintRollingType & ",") > 0 Then
        If mintRollingType > 0 Then
            strWhere = " And instr([6],','|| nvl(A.结算性质,0)||',')> 0"
        End If
        Select Case mintRollingType
        Case 0 '所有类别
    ...
    If InStr(",0,2,", "," & mintRollingType & ",") > 0 Then '预交款
        strSQL = strSQL & _
    ...
    If InStr(",21,", "," & mintRollingType & ",") > 0 Then '门诊预交款
        strSQL = strSQL & _
        IIf(strSQL <> "", "UNION ALL ", "") & vbNewLine & _
        "       Select 3 As 性质, ID As 结帐id, a.结算方式, a.金额, 0 As 冲预交, 0 As 借款合计, 0 As 借出合计" & vbNewLine & _
        "       From 病人预交记录 A" & vbNewLine & _
    ...
    If InStr(",22,", "," & mintRollingType & ",") > 0 Then '住院预交款
        strSQL = strSQL & _
        IIf(strSQL <> "", "UNION ALL ", "") & vbNewLine & _
        "       Select 3 As 性质, ID As 结帐id, a.结算方式, a.金额, 0 As 冲预交, 0 As 借款合计, 0 As 借出合计" & vbNewLine & _
        "       From 病人预交记录 A" & vbNewLine & _
    ...
    If InStr(",0,3,", "," & mintRollingType & ",") > 0 Then '结帐产生预交
        strSQL = strSQL & _
        IIf(strSQL <> "", "UNION ALL ", "") & vbNewLine & _
        "       Select 3 As 性质, ID As 结帐id, a.结算方式, a.金额, 0 As 冲预交, 0 As 借款合计, 0 As 借出合计" & vbNewLine & _
        "       From 病人预交记录 A" & vbNewLine & _
    ...
    If InStr(",0,6,", "," & mintRollingType & ",") > 0 Then '消费卡充值
        strSQL = strSQL & _
        IIf(strSQL <> "", "UNION ALL ", "") & vbNewLine & _
        "       Select 5 As 性质, a.Id As 结帐id, a.结算方式, a.缴款金额 As 金额, 0 As 冲预交, 0 As 借款合计, 0 As 借出合计" & vbNewLine & _
        "       From 消费卡充值记录 A" & vbNewLine & _
        "       Where a.操作员姓名 || '' = [3] And 充值时间 Between [4] And [5]" & vbNewLine & _
        "       Union All" & vbNewLine & _
        "       Select 6 As 性质, a.Id, a.结算方式, a.销售金额 As 金额, 0 As 冲预交, 0 As 借款合计, 0 As 借出合计" & vbNewLine & _
        "       From 消费卡目录 A" & vbNewLine & _
        "       Where 发卡人 || '' = [3] And 发卡时间 Between [4] And [5]" & vbNewLine
    End If
    ...
    With vsChagre
        ...
        txtTotal.Text = Format(dblToTotal, "#,###0.00;-#,###0.00;0.00;-0.00") & "元"
        For i = 0 To .Rows - 1
    ...
    strRollingType = "": strWhere = ""
    If mintRollingType <> 6 Then '消费卡无发票
        If mintRollingType <> 0 Then
            '非所有类别
            strWhere = " And  instr([6],','||A.票种||',')>0"
            Select Case mintRollingType
            Case 0 '所有
                strRollingType = ",,"
            Case 1 '1-收费
                strRollingType = ",1,"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质 In (3,4))"
            Case 2 '预交
                strRollingType = ",2,"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And Nvl(F.结算性质,0) = 12)"
            Case 21 '门诊预交
                strRollingType = ",2,"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And Nvl(F.结算性质,0) = 12)"
                strWhere = strWhere & " And Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And F.预交类别=1 And Nvl(F.结算性质,0) = 2)"
            Case 22 '住院预交
                strRollingType = ",2,"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And Nvl(F.结算性质,0) = 12)"
                strWhere = strWhere & " And Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And F.预交类别=2 And Nvl(F.结算性质,0) = 2)"
            Case 3 '结帐
                strRollingType = ",1,2,3,"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质 In (1,4))"
                strWhere = strWhere & " And Not Exists (Select 1 From 票据使用明细 D,票据打印内容 E,病人预交记录 F Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=2 And E.NO=F.NO And F.记录性质=1 And Nvl(F.结算性质,0) <> 12)"
            Case 4 '挂号
                strRollingType = ",1,4,"
                strWhere = strWhere & " And Exists (Select 1 From 票据使用明细 D,票据打印内容 E Where D.ID=A.ID And D.打印ID=E.ID And E.数据性质=4)"
            Case 5 '就诊卡
                strRollingType = ",5,"
            Case Else '消费卡
                strRollingType = ",-,"
        ...
        "                     And ((A.性质 = 2 And Not Exists(Select 1 From 人员收缴票据 C,人员收缴记录 D,票据使用明细 E,票据领用记录 F Where c.收缴id = d.Id And d.收款员 = [3] And e.打印id = a.打印id And e.领用id = f.Id And Nvl(f.批次,'-') = Nvl(c.批次,'-') And Decode(c.性质, 3, 2, c.性质) = a.性质 And d.作废时间 Is Null And a.票种 = c.票种 And Length(c.开始票号) = Length(a.号码) And a.号码 Between c.开始票号 And c.终止票号)) Or " & _
        "                           (性质 <> 2 And Not Exists(Select 1 From 人员收缴票据 E,人员收缴记录 F,票据使用明细 G,票据领用记录 H Where e.收缴ID=f.ID And f.收款员 = [3] And g.打印id = a.打印id And g.领用id = h.Id And Nvl(h.批次,'-') = Nvl(e.批次,'-') And f.作废时间 Is Null And a.票种=e.票种 And Length(e.开始票号) = Length(a.号码) And a.号码 between e.开始票号 and e.终止票号)))" & strWhere & "), "
        ...
        "               Where a.票种=3 And A.NO=B.NO and b.记录状态 in (1,3) and b.ID=C.结帐ID  " & _
        ...
        "               Where a.行号 = b.断号 And a.票种 = b.票种 and a.票种=M.票种 And M.号码 between a.号码 and b.号码 And Nvl(a.批次,0) = Nvl(b.批次,0) And Length(m.号码) = Length(a.号码) " & _
    ...
    Set mrsListBill = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlngChargeRollingID, bytType, mstrPersonName, mdtStartDate, mdtendDate, strRollingType)
    ...
    With vsReturnBill
        ...
        Do While Not mrsListBill.EOF
            If .TextMatrix(lngRow - 1, .ColIndex("类别")) <> Decode(mrsListBill!票种, 1, "收费", 2, "预交", 3, "结帐", 4, "挂号", 5, "就诊卡", "其他") Then
                .TextMatrix(lngRow, .ColIndex("类别")) = Decode(mrsListBill!票种, 1, "收费", 2, "预交", 3, "结帐", 4, "挂号", 5, "就诊卡", "其他")
            ...
            .RowOutlineLevel(lngRow) = 1
            .TextMatrix(lngRow, .ColIndex("类别")) = Decode(mrsListBill!票种, 1, "收费", 2, "预交", 3, "结帐", 4, "挂号", 5, "就诊卡", "其他")
            .TextMatrix(lngRow, .ColIndex("类型")) = Decode(mrsListBill!性质, 2, "退费", 3, "重打", "其他")
            ...
            If .TextMatrix(lngRow, .ColIndex("类别")) = "收费" And Val(.TextMatrix(lngRow, .ColIndex("金额"))) = 0 Then
    ...
    With vsBill
        ...
        Do While Not mrsListBill.EOF
            ...
            If InStr(1, strTemp & ",", "," & int票种 & ",") = 0 Then
                ...
                .Rows = .Rows + 2
                .TextMatrix(.Rows - 3, 0) = Decode(mrsListBill!票种, 1, "收费", 2, "预交", 3, "结帐", 4, "挂号", 5, "就诊卡", "其他")
                .TextMatrix(.Rows - 2, 0) = Decode(mrsListBill!票种, 1, "收费", 2, "预交", 3, "结帐", 4, "挂号", 5, "就诊卡", "其他")
        ...
        For lngRow = 0 To .Rows - 1 Step 2
             ...
             Loop
             If int票种 <> 0 Then
                .TextMatrix(lngRow, 2) = "使用:" & lngBillTotal(0) & "张; " & "收回(退费和重打):" & lngBillTotal(1) + lngBillTotal(2) & "张"
            End If
        Next
        .Rows = .Rows - 1
    End With
...
Private Function LoadChargeAndBillAndTotal() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '1.加载收款数据汇总
    strSQL = "" & _
    "   Select decode(nvl(M.性质,0),1,1,2,2,3,10,4,11,4) as 序号,  " & _
...
Private Sub vsChagre_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngMode, vsChagre, Me.Name, "结算方式列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub vsReturnBill_AfterUserResize(ByVal Row As Long, ByVal Col As Long)
    zl_vsGrid_Para_Save mlngMode, vsReturnBill, Me.Name, "收回票据列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Function GetBalance(ByVal strCash As String) As ADODB.Recordset
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSQL = strSQL & _
    "               Union All " & _
    "               Select a.结算方式, -1 * a.缴款金额 As 金额 " & _
    "               From 消费卡充值记录 A " & _
    "               Where a.操作员姓名 || '' =[1] And 充值时间 > [2]  " & _
    "               Union All " & _
    "               Select a.结算方式, -1 * Nvl(Sum(a.销售金额), 0) As 金额 " & _
    "               From 消费卡目录 A " & _
    "               Where 发卡人 || '' =[1] And 发卡时间 > [2]  " & _
    "               Group By 结算方式 " & _
    "               Union All " & _
        ...
        For i = 1 To .Rows - 1
            ...
            If Not (strTemp Like "*冲预交*" Or strTemp Like "*借入合计*" _
                ...
                If i = mlngCashRow Then
                    strTemp = strTemp & "," & Val(Replace(.TextMatrix(i, .ColIndex("金额")), ",", "")) - mdblRemain
                Else
                    strTemp = strTemp & "," & Val(Replace(.TextMatrix(i, .ColIndex("金额")), ",", ""))
                End If
...
Public Function SaveData(ByVal strStartDate As String, ByVal strEndDate As String, _
    ...
    Optional ByVal dblRemain As Double = 0, Optional intRollingType As Integer) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:保存轧帐数据
    ...
    With vsChagre
        ...
        Else
            ...
            For i = 1 To cllData(0).Count
                ...
                strSQL1 = strSQL1 & "" & IIf(intRollingType = 0, "NULL", intRollingType) & ")"
                cllPro.Add strSQL1
            Next
        End If
...
Private Function CheckMzFeeValied(ByVal strIDs As String, ByRef cllData As Collection) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSQL = " " & _
    "   Select /*+ rule */ * " & _
    "   From (With c_异常 As (Select a.结帐id, Max(a.记录状态) As 记录状态, Sum(a.结帐金额) As 结算金额 " & _
