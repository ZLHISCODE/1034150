...
Private mbln分诊台签到排队 As Boolean '是否分诊台签到排队
...
Private Sub picDept_Resize()
    ...
    End With
End Sub
Public Sub zlInitVar(ByVal frmMain As Form, _
    ByVal byt分诊台签到排队 As Byte, _
    ByVal byt候诊排序方式 As Byte)
    '------------------------------------------------------------------------------------------------------------------------
    '功能：设置常用变量
    '入参：frmMain-调用的父窗体
    '出参：
...
Public Sub zlExc签道(ByVal bln取消签到 As Boolean)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:执行签道
    '编制:刘兴洪
    ...
    Dim bln已分诊 As Boolean, strNO As String
...
End Function
Private Function ExecuteStatuUpdate(ByVal intType As Integer, ByVal lng挂号ID As Long, ByVal strNO As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:记录状态更改
    '入参:lng挂号ID-挂号的ID
    '       intType:-0-签到;1-取消签到/取消回诊;2-需要回诊;3-取消回诊;4-回诊接诊
    '返回:执行取消回诊成功,返回true,否则返回False
    '编制:刘兴洪
    '日期:2011-01-14 19:31:57
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, blnCancel As Boolean
    On Error GoTo errHandle
    '-操作类型_IN:0-签到;1-取消签到/取消回诊;2-需要回诊;3-取消回诊;4-回诊接诊
    'Zl_病人挂号_记录状态_Update
    '  Id_In       病人挂号记录.ID%Type,
    '  操作类型_In Integer:=0
    strSQL = "Zl_病人挂号_记录状态_Update(" & lng挂号ID & "," & intType & ")"
    zlDatabase.ExecuteProcedure strSQL, Me.Caption
    Select Case intType
    Case 0 '签到
        'intType:intType:1-分诊;2-换号;3-病人不就诊;4-病人待诊;5-病人完成就诊;6-恢复就诊,7-回诊
...
Public Sub zlExcuteFunction()
    ...
    If blnTriage Then
        If mbln分诊台签到排队 Then
            'ListSubItem(3).tag:A.记录标志:表示0表示就诊病人,1-表示签到的病人,2-表示需要回诊就诊的病人; 3-表示已回诊但还未接收的病人;
            If Val(lvwMain.SelectedItem.ListSubItems(3).Tag) = 0 Then
                GoTo EdPati:
            End If
        End If
    ...
    End If
EdPati:
'    If (lngExeState = 0) And mbln分诊台签到排队 And Val(lvwMain.SelectedItem.ListSubItems(3).Tag) = 0 Then
'        Call zlExc签道(False)
'        Exit Sub
'    End If
...
Public Sub zlExcuteAppointmentFunction()
    '------------------------------------------------------------------------------------------------------------------------
    '功能：执行预约队列相关功能
    '编制：
    '日期：
    ...
    If blnTriage Then
        If mbln分诊台签到排队 Then
            'ListSubItem(3).tag:A.记录标志:表示0表示就诊病人,1-表示签到的病人,2-表示需要回诊就诊的病人; 3-表示已回诊但还未接收的病人;
            If Val(LvwYY.SelectedItem.ListSubItems(3).Tag) = 0 Then
                GoTo EdPati:
            End If
        End If
        zlExecuteTriage mfrmMain: Exit Sub
    End If
EdPati:
'    If (lngExeState = 0) And mbln分诊台签到排队 And Val(lvwYY.SelectedItem.ListSubItems(3).Tag) = 0 Then
'        Call zlExc签道(False)
'        Exit Sub
'    End If
...
Public Sub zlExcuteChangeNum(ByVal frmMain As Form)
    ...
    If InStr(lvwMain.SelectedItem.Tag, "|") > 0 Then
        If gbytRegistMode = 0 Then
            ...
            End If
        Else
            If zlDatabase.Currentdate < gdatRegistTime Then
                If frmChangeNum.ShowMe(lngID, Me) Then
                    RaiseEvent zlQueueAsk(2, strNO, mlngPre病人ID, blnCancel)
                    'intType:intType:1-分诊;2-换号;3-病人不就诊;4-病人待诊;5-病人完成就诊;6-恢复就诊
                    ' 0-复诊,1-直呼,2-弃号,3-暂停,4-完成就诊,5-广播
...
Public Sub zlExcuteEditPati(ByVal frmMain As Form)
   ...
   If tbPage.Item(midx.idx_排队队列).Selected Then
    ...
    With frmDistRoomPatiEdit
        ...
        .m验证密码 = str验证密码
        .InitData
        '79912:李南春,2015/1/21,入院病人不能在分诊台修改信息
...
Private Sub ReadRoom()
    ...
    Dim lng记录性质 As Long
    Dim strTmp As String, blnBusy As Boolean
    '没有选择就退出
    If tbPage.Item(midx.idx_排队队列).Selected Then
        '没选择就退出
        If lvwMain.SelectedItem Is Nothing Then Exit Sub
    ...
    '先增加医生
    strSQL = _
        " Select c.编号,c.姓名 From 人员性质说明 a, 部门人员 b ,人员表 c,病人挂号记录 d" & vbCrLf & _
        " Where b.人员id=c.id And b.人员id=a.人员id And a.人员性质='医生' " & vbCrLf & _
        " And (C.站点='" & gstrNodeNo & "' Or C.站点 is Null)" & vbNewLine & _
        " And b.部门id=d.执行部门ID And d.记录性质=[2] and d.记录状态=1 and  d.NO=[1] And (c.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or c.撤档时间 Is Null)"
...
Private Sub LoadPatientInfo(ByVal lng病人ID As Long, Optional ByRef bln入院 As Boolean = False)
    ...
    With frmDistRoomPatiEdit
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            .cbo费别.ListIndex = GetCboIndex(.cbo费别, zlCommFun.Nvl(rsTmp!费别))
            .cbo付款方式.ListIndex = GetCboIndex(.cbo付款方式, zlCommFun.Nvl(rsTmp!医疗付款方式))
            .cbo国籍.ListIndex = GetCboIndex(.cbo国籍, zlCommFun.Nvl(rsTmp!国籍))
            .cbo民族.ListIndex = GetCboIndex(.cbo民族, zlCommFun.Nvl(rsTmp!民族))
            .cbo婚姻.ListIndex = GetCboIndex(.cbo婚姻, zlCommFun.Nvl(rsTmp!婚姻状况))
            .cbo职业.ListIndex = GetCboIndex(.cbo职业, zlCommFun.Nvl(rsTmp!职业))
            ...
            Call zlReadAddrInfo(.padd家庭地址, rsTmp!病人ID, Val(Nvl(rsTmp!主页ID)), 3, .cbo家庭地址.Text)
...
Private Sub Form_Load()
    ...
    '医保号,摘要:21101
    zlControl.LvwSelectColumns lvwMain, "挂号单,1100,0,2;科室,1200,0,1;号类,600,0,1;挂号项目,1350,0,1;门诊号,800,0,1;姓名,960,0,1;性别,400,0,1;年龄,600,0,1;诊室,1600,0,1;医生,960,0,1;发生时间,2000,0,1;挂号时间,2000,0,1;序号,600,0,1;医保号,600,0,1;摘要,2000,0,1;预约,800,0,1", True
    zlControl.LvwSelectColumns LvwYY, "预约单,1100,0,2;科室,1200,0,1;号类,600,0,1;挂号项目,1350,0,1;门诊号,800,0,1;姓名,960,0,1;性别,400,0,1;年龄,600,0,1;诊室,1600,0,1;医生,960,0,1;预约时间,2000,0,1;挂号时间,2000,0,1;序号,600,0,1;医保号,600,0,1;摘要,2000,0,1", True
    zlControl.LvwSelectColumns lvwHZPati, "挂号单,1100,0,2;科室,1200,0,1;号类,600,0,1;挂号项目,1350,0,1;门诊号,800,0,1;姓名,960,0,1;性别,400,0,1;年龄,600,0,1;诊室,1600,0,1;医生,960,0,1;发生时间,2000,0,1;挂号时间,2000,0,1;序号,600,0,1;医保号,600,0,1;摘要,2000,0,1;预约,800,0,1", True
...
Public Sub zlRefreshData(Optional blnFilter As Boolean = False, _
    Optional strFindValue As String = "", Optional bytReadType As Byte = 0, Optional objCard As Card)
    '------------------------------------------------------------------------------------------------------------------------
    '功能：重新刷新数据
    '入参：blnFilter-是否过滤
    ...
    '------------------------------------------------------------------------------------------------------------------------
    Call ShowBills(blnFilter, strFindValue, bytReadType, objCard)
End Sub
Private Sub ShowBills(blnFilter As Boolean, Optional strFindValue As String = "", _
    Optional bytReadType As Byte = 0, Optional objCard As Card)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取分诊数据
    '入参：blnFilter-是否过滤
    '          bytReadType-读取类型(0-不区分;1-刷卡;2-读取身份证;3-读取IC卡)
    ...
    Dim strIco As String, objList As ListItem, strKind As String
    Dim i As Long, j As Long
    Dim strNO As String, strTmp As String, blnUnOutJoin As Boolean
    ...
    Dim lng卡类别ID As String, strPassWord As String, strErrMsg As String, lng病人ID As Long, blnHavePassWord As Boolean
    Dim lng提前分诊小时 As Long, blnBusy As Boolean
    
    mstrRegistIdsed = ""
    
    Call ShowBillsAppointment(blnFilter, strFindValue, bytReadType, objCard)
    On Error GoTo errHandle
    ...
    lng病人ID = Val(mcllFilter("病人ID"))
    If str姓名 <> "" Or str就诊卡号 <> "" Or str门诊号 <> "" Or str医保号 <> "" Then blnUnOutJoin = True
    If str姓名 <> "" Then str姓名 = str姓名 & "%"
    
    If strFindValue <> "" Then
        strKind = objCard.名称
        '问题:42946
        If bytReadType = 1 Then
            ...
            Else
                ...
                If gobjSquare.objSquareCard.zlGetPatiID(lng卡类别ID, strFindValue, False, lng病人ID, strPassWord, strErrMsg) = False Then lng病人ID = 0
            End If
            strFilter = strFilter & " And A.病人ID=[12]"
            blnHavePassWord = True
        ElseIf bytReadType = 2 Or strKind = "身份证号" Then '读取身份证
                If gobjSquare.objSquareCard.zlGetPatiID("身份证号", strFindValue, False, lng病人ID, _
                    strPassWord, strErrMsg) = False Then lng病人ID = 0
                strFilter = strFilter & " And A.病人ID=[12]"
                blnHavePassWord = True
        ElseIf bytReadType = 3 Or strKind = "IC卡号" Then '读取IC卡
        ...
        ElseIf (Left(strFindValue, 1) = "-" And IsNumeric(Mid(strFindValue, 2))) Then
            If Val(Mid(strFindValue, 2)) > 2147483647 Then Screen.MousePointer = 0: Exit Sub
            lng病人ID = Val(Mid(strFindValue, 2)): blnUnOutJoin = True
            strFilter = strFilter & " And A.病人ID = [12]"
        ElseIf (Left(strFindValue, 1) = "*" And IsNumeric(Mid(strFindValue, 2))) Or strKind = "门诊号" Then
            str门诊号 = IIf(Left(strFindValue, 1) = "*", Val(Mid(strFindValue, 2)), Val(strFindValue)): blnUnOutJoin = True
        ...
        Else
                ...
                Case Else
                    ...
                    If lng卡类别ID <> 0 Then
                        If gobjSquare.objSquareCard.zlGetPatiID(lng卡类别ID, strFindValue, False, lng病人ID, strPassWord, strErrMsg) = False Then lng病人ID = 0
                    Else
                        If gobjSquare.objSquareCard.zlGetPatiID(strKind, strFindValue, False, lng病人ID, _
                            strPassWord, strErrMsg) = False Then lng病人ID = 0
                    End If
                    strFilter = strFilter & " And A.病人ID=[12]"
                    blnHavePassWord = True
    ...
    strFilter = strFilter & IIf(mstr分诊科室 <> "", " And Instr(','||[10]||',',','||A.执行部门id||',')>0", "") & _
            ...
            IIf(mbytViewScrop(0) = 1, " Or nvl(A.执行状态,0) = 0 And 诊室 Is Not Null", "") & _
    ...
    strSQL = _
        ...
        "      nvl(A.执行状态,0) as 执行状态,A.号序,A.摘要,decode(A.预约,1,'√','') as 预约,B.医保号,A.记录标志,e.编码" & vbCrLf & _
    ...
    Else
        ...
        Else
            ...
            " Where a.病人id=b.病人id  " & IIf(blnUnOutJoin, "", "(+)") & " And ((nvl(A.执行状态,0)=2 and nvl(A.记录标志,0) in (0,1))   or Nvl(A.执行状态,0)<>2 ) " & _
            "           And a.执行部门id=e.id And a.出诊记录id=c1.id And a.出诊记录ID Is Not Null And c1.号源id=c.id And c.项目id=d.ID" & vbCrLf & strFilter & _
            "           And (E.站点='" & gstrNodeNo & "' Or E.站点 is Null) and a.记录性质=1 and a.记录状态=1" & vbNewLine & _
            "  "
            ...
            "      NVL(B.险类, 0) 险类,A.执行人," & _
            "      nvl(A.执行状态,0) as 执行状态,A.号序,A.摘要,decode(A.预约,1,'√','') as 预约,B.医保号,A.记录标志,e.编码 " & vbCrLf & _
            "  From 病人挂号记录 a,病人信息 b,临床出诊号源 c,收费项目目录 d,部门表 e " & vbCrLf & _
            " Where a.病人id=b.病人id  " & IIf(blnUnOutJoin, "", "(+)") & " And ((nvl(A.执行状态,0)=2 and nvl(A.记录标志,0) in (0,1))   or Nvl(A.执行状态,0)<>2 ) " & _
            "           And a.执行部门id=e.id And a.出诊记录ID Is Null And a.号别=c.号码 And c.项目id=d.ID" & vbCrLf & strFilter & _
            "           And (E.站点='" & gstrNodeNo & "' Or E.站点 is Null) and a.记录性质=1 and a.记录状态=1" & vbNewLine & _
            "  "
        End If
    End If
     ...
     Case 0  '科室编码,号码,NO
        strSQL = strSQL & vbCrLf & " Order By 编码,号别,NO "
     Case 1 '科室编码,号码,挂号时间
        strSQL = strSQL & vbCrLf & _
        " Order By 编码,号别, 登记时间"
     Case 2 '科室编码,号码,发生时间
        strSQL = strSQL & vbCrLf & " Order By 编码,号别, 发生时间,登记时间 " '问题号：51665
     End Select
    ...
    With rsTmp
        ...
        Do While Not .EOF
            ...
            objList.Tag = !病人ID & "|" & !险类 & "|" & !就诊卡号 & "|" & !卡验证码 & "|" & !ID & "|" & !号别 & "|" & !执行状态
            ...
            Case 0
                If Not (IsNull(!诊室) Or !病人ID = 0) Then
                    ...
                    If Val(Nvl(!记录标志)) = 1 Or mbln分诊台签到排队 = False Then objList.ForeColor = &H8000000C
            ...
            End Select
            For j = 1 To lvwMain.ColumnHeaders.Count - 1
    ...
    If gbytRegistMode = 0 Then
        strSQL = _
            ...
            "       A.执行人,nvl(A.执行状态,0) as 执行状态,A.记录标志,A.号序,A.摘要,decode(A.预约,1,'√','') as 预约,B.医保号,e.编码" & vbCrLf & _
            "  From 病人挂号记录 a,病人信息 b,挂号安排 c,收费项目目录 d,部门表 e" & vbCrLf & _
    ...
    Else
        ...
        Else
            strSQL = _
             ...
             " Where a.病人id=b.病人id " & IIf(blnUnOutJoin, "", "(+)") & _
             "           And (A.执行状态=2 and nvl(A.记录标志,0) in (2,3) )" & _
             "           And a.执行部门id=e.id And a.出诊记录id Is Not Null And a.出诊记录id=c1.id And c1.号源id = c.id And c.项目id=d.ID" & vbCrLf & strHzWhere & _
             " And (E.站点='" & gstrNodeNo & "' Or E.站点 is Null) and a.记录性质=1 and a.记录状态=1 " & vbNewLine & _
            "  "
            strSQL = strSQL & " Union " & _
            "Select A.诊室,A.ID,A.号别,C.号类,D.名称 as 挂号项目," & vbCrLf & _
             ...
             " Where a.病人id=b.病人id " & IIf(blnUnOutJoin, "", "(+)") & _
             "           And (A.执行状态=2 and nvl(A.记录标志,0) in (2,3) )" & _
             "           And a.执行部门id=e.id And a.出诊记录id Is Null And a.号别 = c.号码 And c.项目id=d.ID" & vbCrLf & strHzWhere & _
             " And (E.站点='" & gstrNodeNo & "' Or E.站点 is Null) and a.记录性质=1 and a.记录状态=1 " & vbNewLine & _
            "  "
        End If
    End If
...
Public Property Get zlIsTriage() As Boolean
    ...
    If tbPage.Item(midx.idx_排队队列).Selected Then
        ...
        Else
            ...
            Else '未签到
                 zlIsTriage = (lngExeState = 0) And Not mbln分诊台签到排队
            End If
        End If
    Else
        '是否能分诊
...
Public Property Get zlIs允许签道() As Boolean
    '是否允许病人签道
    Dim lng病人ID As Long, lngExeState As Long
    If Me.ActiveControl Is Me.lvwHZPati Then
        '回诊病人
    ...
    ElseIf tbPage.Item(midx.idx_排队队列).Selected Then
        ...
        Else
            ...
            zlIs允许签道 = (lngExeState = 0) And Val(lvwMain.SelectedItem.ListSubItems(3).Tag) = 0
            '!病人ID & "|" & !险类 & "|" & !就诊卡号 & "|" & !卡验证码 & "|" & !ID & "|" & !号别 & "|" & !执行状态
        End If
    ElseIf tbPage.Item(midx.idx_预约队列).Selected Then
        ...
        Else
            ...
            zlIs允许签道 = (lngExeState = 0) And Val(LvwYY.SelectedItem.ListSubItems(3).Tag) = 0
            '!病人ID & "|" & !险类 & "|" & !就诊卡号 & "|" & !卡验证码 & "|" & !ID & "|" & !号别 & "|" & !执行状态
        End If
    End If
End Property
...
Public Property Get zlIs允许取消签道() As Boolean
    ...
    If tbPage.Item(midx.idx_排队队列).Selected Then
        ...
        Else
            ...
            lngExeState = Val(Split(lvwMain.SelectedItem.Tag, "|")(6))
            'ListSubItem(3).tag:A.记录标志:表示0表示就诊病人,1-表示签到的病人,2-表示需要回诊就诊的病人; 3-表示已回诊但还未接收的病人;
            zlIs允许取消签道 = (lngExeState = 0) And Val(lvwMain.SelectedItem.ListSubItems(3).Tag) = 1
            '!病人ID & "|" & !险类 & "|" & !就诊卡号 & "|" & !卡验证码 & "|" & !ID & "|" & !号别 & "|" & !执行状态
        End If
    ElseIf tbPage.Item(midx.idx_预约队列).Selected Then
        If LvwYY.SelectedItem Is Nothing Or Me.ActiveControl Is Me.lvwHZPati Then
        ...
        Else
            ...
            lngExeState = Val(Split(LvwYY.SelectedItem.Tag, "|")(6))
            'ListSubItem(3).tag:A.记录标志:表示0表示就诊病人,1-表示签到的病人,2-表示需要回诊就诊的病人; 3-表示已回诊但还未接收的病人;
            zlIs允许取消签道 = (lngExeState = 0) And Val(LvwYY.SelectedItem.ListSubItems(3).Tag) = 1
            '!病人ID & "|" & !险类 & "|" & !就诊卡号 & "|" & !卡验证码 & "|" & !ID & "|" & !号别 & "|" & !执行状态
        End If
    End If
End Property
...
Public Property Get zlIs允许回诊() As Boolean
    '是否允许回诊
    Dim lng病人ID As Long, lngExeState As Long
    If Me.ActiveControl Is Me.lvwHZPati And Not Me.lvwHZPati.SelectedItem Is Nothing Then
        '回诊病人
        'ListSubItem(3).tag:A.记录标志:表示0表示就诊病人,1-表示签到的病人,2-表示需要回诊就诊的病人; 3-表示已回诊但还未接收的病人;
        zlIs允许回诊 = Val(lvwHZPati.SelectedItem.ListSubItems(3).Tag) = 2
    Else
        zlIs允许回诊 = False
    End If
End Property
...
Private Sub lvwHZPati_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    End If
End Sub
Public Sub zlExc回诊(ByVal bln取消回诊 As Boolean)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:执行签道
    '编制:刘兴洪
    '日期:2010-12-08 10:56:44
...
Private Sub ShowBillsAppointment(blnFilter As Boolean, Optional strFindValue As String = "", _
    Optional bytReadType As Byte = 0, Optional objCard As Card)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取分诊数据
    '入参：blnFilter-是否过滤
