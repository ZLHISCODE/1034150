...
Public Function ReadRegistPrice(ByVal lng项目id As Long, ByVal bln病历 As Boolean, ByVal bln就诊卡 As Boolean, _
    ...
    Optional lng挂号科室ID As Long = 0) As Long
...
'说明：主项数次为1,从项按设定数次处理,但为固定
    ...
    '变价且最低限价为零时,不收卡费
    If bln就诊卡 Then
        If gCurSendCard.lng收费细目ID = 0 Then
            strSQL = strSQL & IIf(strSQL <> "", " Union ALL ", "") & _
                ...
                " And Sysdate Between B.执行日期 And Nvl(B.终止日期,To_Date('3000-1-1', 'YYYY-MM-DD')) And Rownum=1"
        Else
            strSQL = strSQL & IIf(strSQL <> "", " Union ALL ", "") & _
                "Select 4 as 性质,A.类别,A.ID as 项目ID,A.名称 as 项目名称,A.编码 as 项目编码,A.计算单位,A.屏蔽费别," & _
                " 1 as 数次,C.ID as 收入项目ID,C.名称 as 收入项目,C.编码 as 收入编码,C.收据费目,B.现价 as 单价,A.执行科室 as 执行科室类型" & _
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlRegEvent", lng项目id, IIf(gCurSendCard.lng收费细目ID = 0, gCurSendCard.str特准项目, gCurSendCard.lng收费细目ID), str附加项目ID)
...
'功能：初始化本机参数
    ...
    If lngModul = 1111 Then
        ...
        gByt挂号凭条 = Val(zlDatabase.GetPara("挂号凭条打印方式", glngSys, lngModul))
        gByt预约挂号单 = Val(zlDatabase.GetPara("预约挂号单打印方式", glngSys, lngModul))
        gByt退号回单 = Val(zlDatabase.GetPara("退号回单打印方式", glngSys, lngModul))
...
Public Function GetSpecialInfo(strClass As String, Optional ByVal lng收费细目ID As Long) As Recordset
'功能：产生工本费、就诊卡等填写门诊费用记录或住院费用记录时的必须信息(收费类别,收费细目ID,计算单位,收入项目ID,收入项目,收据费目,原价,现价,是否变价,科室标志)
'参数：strClass=工本费、就诊卡、病历费
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String
    ...
    If lng收费细目ID = 0 Then
        strSQL = _
            " Select A.类别 as 收费类别,A.ID as 收费细目ID,A.名称 as 收费细目,A.计算单位,C.ID as 收入项目ID, " & _
            " C.名称 as 收入项目,C.收据费目,B.原价,B.现价,Nvl(A.屏蔽费别,0) as 屏蔽费别,Nvl(A.是否变价,0) as 是否变价," & _
            " Nvl(A.执行科室,0) as 科室标志 " & _
            " From 收费项目目录 A,收费价目 B,收入项目 C,收费特定项目 D " & _
    ...
    Else
        strSQL = _
            ...
            " And Sysdate Between B.执行日期 And Nvl(B.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlRegEvent", strClass, lng收费细目ID)
    
    If Not rsTmp.EOF Then Set GetSpecialInfo = rsTmp
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Public Function Get过敏药物(lng病人ID As Long) As String
    ...
    strSQL = "Select 过敏药物ID,过敏药物 From 病人过敏药物 Where 病人ID=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlRegEvent", lng病人ID)
    If Not rsTmp.EOF Then
        For i = 1 To rsTmp.RecordCount
            Get过敏药物 = Get过敏药物 & "||" & IIf(IsNull(rsTmp!过敏药物ID), "", rsTmp!过敏药物ID) & "|" & IIf(IsNull(rsTmp!过敏药物), "", rsTmp!过敏药物)
            rsTmp.MoveNext
        Next
        Get过敏药物 = Mid(Get过敏药物, 3)
    End If
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
...
Public Function RePrintBill(ByVal frmParent As Object, ByVal bytFunc As Byte, _
        ...
        Optional strUseType As String, Optional ByVal bln重打 As Boolean) As Boolean
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '执行数据处理
    Load frmPrint
    Call frmPrint.ReportPrint(bytFunc, strNO, strBackInvoice, lng领用ID, glng挂号ID, strInvoice, zlDatabase.Currentdate, , , , blnVirtualPrint, bytFunc = 2, strUseType)
...
Public Function zlReadRegThreeBalance(ByVal lng结帐ID As String, ByVal lngCard结帐ID As String, _
    ByRef cllBillBalance As Collection, Optional ByRef objCard As Card, Optional ByRef dblThreeMoney As Double) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取三方结算交易信息
    '入参:strNo-单据号
    ...
    gstrSQL = "" & _
    "   Select  B.卡类别ID,B.结算卡序号,B.卡号,B.交易流水号,B.交易说明,B.合作单位,Sum(Nvl(冲预交,0)) As 金额" & _
    "   From 病人预交记录 B" & _
    "   Where B.结帐ID In ([1],[2]) " & _
    "               And  Mod(B.记录性质,10)<>1" & _
    "               And (nvl(b.卡类别ID,0) <> 0  Or  nvl(b.结算卡序号,0) <> 0) " & _
    "   Group By B.卡类别ID,B.结算卡序号,B.卡号,B.交易流水号,B.交易说明,B.合作单位 "

    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取结算交易信息", lng结帐ID, lngCard结帐ID)
    ...
    dblThreeMoney = Val(Nvl(rsTemp!金额))
    Set objCard = New Card
    '卡类别ID,卡号,是否消费卡(1-是;0-否),交易流水号,交易说明,结帐ID,卡结帐ID
    Set cllBillBalance = New Collection
    cllBillBalance.Add Array(lng医疗卡类别ID, Trim(Nvl(rsTemp!卡号)), byt消费卡, Trim(Nvl(rsTemp!交易流水号)), Trim(Nvl(rsTemp!交易说明)), lng结帐ID, lngCard结帐ID)
...
Public Function SetNullValue(varObj As Variant, Optional strDefault As String = "") As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:设置空值的默认值
    '入参:varObj 集合字段对象 strDefault 默认值
    '返回:默认值
    '编制:王吉
    '日期:2012-08-31 04:36:33
    '问题号:53408
