...
'功能：检查工本框的真实长度是否在指定限制长度内
    ...
    ElseIf intCHeckLen <> 0 Then
        If zlCommFun.ActualLen(objTxt.Text) > intCHeckLen Then
...
Public Function GetStockSet(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean) As Recordset
    ...
    On Error GoTo errH
    Set GetStockSet = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng药房ID, lng收费项目ID)
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, ByRef dblAllTime As Double, ByVal strDec As String, ByRef dblPriceSingle As Double) As Currency
...
'      dblPriceSingle-只有一个批次时返回该批次的单价，避免根据数量先乘再除后因四舍五入引起不同的数量的单价不同
    ...
    Set rsPrice = GetStockSet(lng药房ID, lng收费项目ID, bln卫材)
    '时价=总金额/总数量
    dblPrice = 0 '本笔总应收金额
