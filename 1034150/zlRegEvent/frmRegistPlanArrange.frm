...
Private Function IsValied(Optional ByVal blnSave As Boolean = False) As Boolean
    ...
    Dim strTmp As String, lng已约数 As Long
    Dim j As Integer
...
'    End If
    ...
    If chk立即审核.Value = 1 Then
        strSQL = "Zl_挂号安排计划_Verify(" & lng计划ID & "," & IIf(opt生效时间(0).Value, 1, 0) & ")"
        zlAddArray colPro, strSQL
    End If
...
Private Function SaveVerify() As Boolean
    ...
    strSQL = "Zl_挂号安排计划_Verify(" & Val(mstr计划ID) & "," & chk立即生效.Value & ")"
...
Private Function LoadRegHistory() As Boolean
    ...
    strSQL = strSQL & vbCrLf & " Where a.记录状态 = 1 And a.发生时间 Between [2] And [3] And a.号别 = b.号码 And b.Id=[1]"
