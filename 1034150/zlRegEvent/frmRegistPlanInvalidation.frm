...
Private Sub cmdOthers_Click()
    ...
    Dim str开始停用时间     As String
    Dim str结束停用时间     As String
    ...
    End If
    '谢荣 2017-05-23 105055
    str开始停用时间 = vsList.TextMatrix(vsList.Row, 1)
    str结束停用时间 = vsList.TextMatrix(vsList.Row, vsList.ColIndex("结束停用时间"))
    ...
    strSQL = "" & _
        ...
        "          And Nvl(A.是否删除,0)=0 And A.停用日期 IS Null And not exists(Select 1 From 挂号安排停用状态 Z Where A.ID=Z.安排ID And Z.开始停止时间=to_date([47],'yyyy-MM-dd HH24:MI:SS') And Z.结束停止时间=to_date([48],'YYYY-MM-DD HH24:MI:SS'))" & vbNewLine & _
    ...
    With vsList
        For i = 1 To .Rows - 1
            ...
            If Format(dtpEndDate.Value, "yyyy-mm-dd HH:MM") >= Trim(.TextMatrix(i, .ColIndex("开始停用时间"))) _
               ...
               Exit Function
            End If
            If Trim(.TextMatrix(i, .ColIndex("开始停用时间"))) >= Format(dtpStartDate.Value, "yyyy-mm-dd HH:MM") _
               And Trim(.TextMatrix(i, .ColIndex("开始停用时间"))) >= Format(dtpEndDate.Value, "yyyy-mm-dd HH:MM") Then
               ShowMsgbox "注意:" & vbCrLf & "    第" & i & "行中的停用时间范围已经包含在当前所设置的停用范围中,请检查!"
               If dtpStartDate.Enabled And dtpStartDate.Visible Then dtpEnd.SetFocus
               Exit Function
            End If
            If Trim(.TextMatrix(i, .ColIndex("结束停用时间"))) >= Format(dtpStartDate.Value, "yyyy-mm-dd HH:MM") _
               And Trim(.TextMatrix(i, .ColIndex("结束停用时间"))) >= Format(dtpEndDate.Value, "yyyy-mm-dd HH:MM") Then
               ShowMsgbox "注意:" & vbCrLf & "    第" & i & "行中的停用时间范围已经包含在当前所设置的停用范围中,请检查!"
               If dtpEnd.Enabled And dtpEnd.Visible Then dtpEnd.SetFocus
               Exit Function
            End If
...
Private Function SaveData() As Boolean
    ...
    With vsList
        ...
        End If
        '处理增加的数据
        '127804:李南春,2018/10/22,RowData为1是已经存在的计划，不能重复增加
        For i = 1 To .Rows - 1
...
Public Function SelectItem(ByVal strInput As String) As Boolean
    ...
    vRect = GetControlRect(txtCode.Hwnd)
