...
Public Function GetWorkTimeRange(str时间段 As String, ByVal str站点 As String, ByVal str号类 As String) As 上班时段
    ...
    ElseIf str站点 = "" And str号类 <> "" Then
        ...
        If rsWorkTime.EOF Then rsWorkTime.Filter = "时间段='" & str时间段 & "' And 站点='-' And 号类='-'"
    Else
        rsWorkTime.Filter = "时间段='" & str时间段 & "' And 站点='-' And 号类='-'"
    End If
    '存在站点或号类的
        ...
        Do While Not .EOF
            If Nvl(rsControl!上班时段) <> "" Then
                ...
                If obj上班时段 Is Nothing Then
                    MsgBox "上班时间段“" & Nvl(rsControl!上班时段) & "”不存在，请先在“基础设置>上班时间管理”中添加！", vbInformation, gstrSysName
                End If
                Set obj出诊记录 = New 出诊记录
...
Public Function GetNextPlanDate(frmParent As Object, ByVal byt排班方式 As Byte, _
    ...
    Optional ByVal lng人员ID As Long, Optional ByVal blnShowSelect As Boolean = True, _
    Optional ByRef blnCancel As Boolean) As Boolean
    '功能：确定下一个排班的年月周
    '参数：
    ...
    intYear = 0: intMonth = 0: intWeek = 0: blnCancel = False
'    If lng人员ID <> 0 Then
...
'    End If
    ...
    Set rsData = zlDatabase.OpenSQLRecord(strSQL, "mdlClinicPlanData", byt排班方式, lng人员ID, gstrNodeNo)
...
SetNewDate:
    ...
    If blnShowSelect Then
        ...
        If frm.ShowMe(frmParent, byt排班方式, dtStart, intYear, intMonth, intWeek) = False Then
            blnCancel = True: Exit Function
        End If
    End If
    GetNextPlanDate = True
    Exit Function
