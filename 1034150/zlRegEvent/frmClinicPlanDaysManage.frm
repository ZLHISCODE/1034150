...
Private mintYear As Integer, mintMonth As Integer, mintWeek As Integer, mstrPlanName As String
...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Case conMenu_Edit_Delete '删除出诊表
        If DeletePlan(mlng出诊ID, mbytFun) Then Call mfrmMain.NodeChanged("")
...
Private Sub PrintPlan(ByVal lng出诊ID As Long, ByVal bytFun As Byte, Optional ByVal bytMode As Byte)
    ...
    Else
        If gVisitPlan_ModulePara.byt出诊表打印方式 = 1 Or bytMode = 1 Then
            Call ReportOpen(gcnOracle, glngSys, "ZL" & glngSys \ 100 & "_INSIDE_1114_3", Me, "出诊ID=" & mlng出诊ID, 2)
        ElseIf gVisitPlan_ModulePara.byt出诊表打印方式 = 2 Then
            If MsgBox("要打印出诊表吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
                Call ReportOpen(gcnOracle, glngSys, "ZL" & glngSys \ 100 & "_INSIDE_1114_3", Me, "出诊ID=" & mlng出诊ID, 2)
            End If
...
Private Sub ExecuteFilter()
    ...
    Call LoadPlanDataByRecordset(vsfRegistPlan, gPlanGrid_DataStyle.Data_Plan, mrsPlanRecords, _
        mbytFun, , , Val(lblPublishInfo.Tag) = 1)
    Exit Sub
errHandler:
    If ErrCenter() = 1 Then
        Resume
...
Private Function DeletePlan(ByVal lng出诊ID As Long, ByVal byt排班方式 As Byte) As Boolean
    '功能：删除出诊表
    '入参：
    ...
    strSQL = "Select ID" & vbNewLine & _
            ...
            "       Where a.排班方式 = [1] And a.Id = b.出诊id(+)" & vbNewLine & _
            "             And Nvl(a.站点,'-') = Nvl([2],'-') And a.关联ID Is Null" & vbNewLine & _
    ...
    If rsTemp.EOF Then
        ...
        Exit Function
    Else
        If Val(Nvl(rsTemp!ID)) <> lng出诊ID Then
    ...
    If zlStr.IsHavePrivs(mstrPrivs, "所有科室") = False Then
        ...
        strSQL = "Select 1" & vbNewLine & _
                ...
                " Where a.号源id = b.Id And a.出诊id = [1] And Not (Nvl(b.是否临床排班, 0) = 1" & vbNewLine & _
                "       And Exists (Select 1 From 部门人员 Where 部门id = b.科室id And 人员id = [2]))"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID, UserInfo.ID)
    ...
    If MsgBox("你确定要删除【" & mstrPlanName & "】吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
        Exit Function
    End If
...
Public Sub RefreshData(ByVal bytFun As Byte, ByVal lng出诊ID As Long, Optional ByVal blnClear As Boolean, _
    Optional ByVal intYear As Integer, Optional ByVal intMonth As Integer, Optional ByVal intWeek As Integer)
    '功能：刷新安排详情数据
    '入参：
    '   bytFun - 1-月安排，2-周安排
    '   lng出诊ID - 出诊ID
    ...
    If blnClear Then
        ...
        mintYear = intYear: mintMonth = intMonth: mintWeek = intWeek
        
        chkShowAllPlan.Value = vbUnchecked
        If bytFun = 1 Then
            chkShowAllPlan.Visible = True
            picSelectWeek.Visible = True
            ...
            Next
        Else
            chkShowAllPlan.Visible = False
            picSelectWeek.Visible = False
        End If
        
        If mintMonth = 0 Then '只有选择年节点时才会有此情况
            mintMonth = Month(mdtToday)
            mintWeek = GetDateWeek(mdtToday)
            mstrPlanName = "出诊表"
        Else
            mstrPlanName = mintYear & "年" & mintMonth & "月" & IIf(bytFun = 1, "", "第" & mintWeek & "周") & "出诊表"
        End If
        sccTitle.Caption = "出诊安排>" & mstrPlanName
        
        '显示日期范围确定
        varDateRange = GetDateRange(mintYear, mintMonth, IIf(bytFun = 1, 0, mintWeek))
        ...
        strSQL = "Select b.发布人, b.发布时间 From 临床出诊表 B Where b.Id = [1]"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID)
        If Not rsTemp.EOF Then
        ...
        Else
            lblPublishInfo.Tag = ""
            lblPublishInfo.Caption = "发布人：        发布时间：                    "
        End If
        
        Call InitPlanGrid(vsfRegistPlan, gPlanGrid_DataStyle.Data_Plan, mdtStartDate, mdtEndDate, Val(lblPublishInfo.Tag) = 1)
...
Private Sub RefreshOneData(Optional ByVal lngCurRow As Long = -1, _
    ...
    Call RefreshOnePlanData(vsfRegistPlan, gPlanGrid_DataStyle.Data_Plan, mrsPlanRecords, _
        lngCurRow, Val(lblPublishInfo.Tag) = 1, mbytFun)
...
Public Function AddNewPlan(Optional blnMonth As Boolean) As String
    ...
    Dim varPlanInfo As Variant 'Array(出诊ID,出诊表名,年份,月份,周数,开始时间,终止时间,出诊表Key值,是否删除安排)
    
    Err = 0: On Error GoTo errHandler
    If GetNewPlanInfo(Me, mstrPrivs, blnMonth, varPlanInfo) = False Then Exit Function
    ...
    strSQL = strSQL & "" & varPlanInfo(0) & ","
    '  出诊表名_In 临床出诊表.出诊表名%Type,
    strSQL = strSQL & "'" & varPlanInfo(1) & "',"
    '  站点_In     部门表.站点%Type,
    ...
    strSQL = strSQL & "" & ZDate(varPlanInfo(5)) & ","
    '  终止时间_In 临床出诊安排.终止时间%Type := Null,
    strSQL = strSQL & "" & ZDate(varPlanInfo(6)) & ","
    '  年份_In     临床出诊表.年份%Type := Null,
    strSQL = strSQL & "" & varPlanInfo(2) & ","
    '  月份_In     临床出诊表.月份%Type := Null,
    strSQL = strSQL & "" & varPlanInfo(3) & ","
    '  周数_In     临床出诊表.周数%Type := Null,
    strSQL = strSQL & "" & ZVal(varPlanInfo(4)) & ","
    ...
    '  删除安排_In Number:=0
    strSQL = strSQL & "" & IIf(varPlanInfo(8), 1, 0) & ")"
    zlDatabase.ExecuteProcedure strSQL, Me.Caption
    
    'XX月出诊表节点：K2_年份_月份
    'XX周出诊表节点：K3_年份_月份_周数
    AddNewPlan = varPlanInfo(7)
    Exit Function
errHandler:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub optWeek_Click(index As Integer)
    ...
    intWeek = index
    Screen.MousePointer = vbHourglass
    varDateRange = GetDateRange(mintYear, mintMonth, intWeek, False)
...
Private Sub vsfRegistPlan_DblClick()
    ...
    If lngRow = 0 Or lngRow = 1 Then
        ...
        If strSort <> "" Then
            ...
            Screen.MousePointer = vbHourglass
            Call LoadPlanDataByRecordset(vsfRegistPlan, gPlanGrid_DataStyle.Data_Plan, mrsPlanRecords, _
                mbytFun, , True, Val(lblPublishInfo.Tag) = 1)
'            Call ShowStopVisitPlan(vsfRegistPlan, mdtStartDate, mdtEndDate)
            Screen.MousePointer = vbDefault
        End If
    Else
        ...
        If chkShowAllPlan.Value = vbChecked Then
            ...
            '存储了Array(出诊ID,安排ID),周出诊表时每一行是多个出诊表中的安排
            varTemp = vsfRegistPlan.Cell(flexcpData, vsfRegistPlan.Row, GetPlanItemNameCol(vsfRegistPlan.Col) + 2)
            lng出诊ID = varTemp(0): lng安排ID = varTemp(1)
        Else
            lng出诊ID = mlng出诊ID
        End If
...
Private Function PublishPlan(ByVal lng出诊ID As Long, ByVal blnPublish As Boolean) As Boolean
    ...
    If blnPublish Then
        ...
        strSQL = "Select a.Id" & vbNewLine & _
                ...
                "        Where Nvl(排班方式, 0) = [2] And 发布人 Is Null And Id <> [1] And 关联ID Is Null" & vbNewLine & _
                "              And Nvl(站点,'-') = Nvl([3],'-')) A," & vbNewLine & _
                "      (Select ID, 年份 || LPad(月份, 2, '0') || 周数 As 日期 From 临床出诊表 Where ID = [1]) B" & vbNewLine & _
                " Where a.日期 < b.日期 And Rownum < 2"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID, mbytFun, gstrNodeNo)
        If Not rsTemp.EOF Then
        ...
        strSQL = "Select 1" & vbNewLine & _
                ...
                "       And a.安排ID <> b.Id And b.出诊id = [1] And Rownum < 2"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID)
        If Not rsTemp.EOF Then
        ...
        Do While Not rsTemp.EOF
            ...
            If obj上班时段 Is Nothing Then
                MsgBox "上班时间段“" & Nvl(rsTemp!上班时段) & "”不存在，请先在“基础设置>上班时间管理”中添加！", vbInformation, gstrSysName
                Exit Function
            End If
        ...
        If rsTemp.EOF Then
            ...
            Exit Function
        End If
        If MsgBox("你确定要发布【" & mstrPlanName & "】吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Function
    ...
    Else
        strSQL = "Select a.Id" & vbNewLine & _
                " From (Select ID, 年份 || LPad(月份, 2, '0') || 周数 As 日期" & vbNewLine & _
                "        From 临床出诊表" & vbNewLine & _
                "        Where Nvl(排班方式, 0) = [2] And 发布人 Is Not Null And Id <> [1] And 关联ID Is Null" & vbNewLine & _
                "              And Nvl(站点,'-') = Nvl([3],'-')) A," & vbNewLine & _
                "      (Select ID, 年份 || LPad(月份, 2, '0') || 周数 As 日期 From 临床出诊表 Where ID = [1]) B" & vbNewLine & _
                " Where a.日期 > b.日期 And Rownum < 2"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID, mbytFun, gstrNodeNo)
        ...
        strSQL = "Select Id, 出诊表名" & vbNewLine & _
                " From 临床出诊表" & vbNewLine & _
                " Where Id = [1] And 发布时间 Is Not Null" & _
        ...
        If rsTemp.EOF Then
            ...
            Exit Function
        End If
        If MsgBox("你确定要取消发布【" & mstrPlanName & "】吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Function
...
Private Function ApplyToDay(ByVal lng安排ID As Long, ByVal strCurDate As String) As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng出诊ID, lng号源Id, dtStart, dtEnd)
...
Private Function NextNewPlanByPlan(ByVal lng原出诊ID As Long, Optional ByVal blnMonth As Boolean) As Boolean
    ...
    'XX月出诊表节点：K2_年份_月份
    'XX周出诊表节点：K3_年份_月份_周数
    Call mfrmMain.NodeChanged(varPlanInfo(7))
...
Private Function GetPlanRecords(ByVal blnMonth As Boolean, Optional ByVal lng出诊ID As Long, Optional ByVal blnPublished As Boolean, _
    ...
    Optional ByVal dtStartDate As Date, Optional ByVal dtEndDate As Date) As ADODB.Recordset
    '功能：获取安排记录
    '入数：
    '   blnMonth - 是否月排班
    '   lng出诊ID   - 出诊ID
    ...
    If blnMonth And blnMonthAllPlan Then
        '查询本月所有安排
        '先查找出诊ID
        strSQL = "Select Nvl(m.关联ID,m.Id) As ID" & vbNewLine & _
        ...
        strSQL = "Select " & str是否有效 & vbNewLine & _
                "        h.标识符 ,b.出诊id, b.Id As 安排id, e.名称 As 收费项目, b.医生姓名, g.简码 As 医生简码, g.专业技术职务 as 医生职称, " & vbNewLine & strSqlSub & _
        ...
        Set GetPlanRecords = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, intYear, intMonth, UserInfo.ID, gstrNodeNo)
        Exit Function
    End If
    ...
    If blnPublished Then
        strSQL = "Select " & str是否有效 & vbNewLine & _
                ...
                "        c.是否临时出诊, c.停诊开始时间, c.停诊终止时间, c.停诊原因, c.替诊医生姓名, c.是否锁定, b.开始时间, b.终止时间" & vbNewLine & _
                " From 临床出诊号源 A, 临床出诊安排 B, 临床出诊记录 C, 收费项目目录 E, 部门表 F, 人员表 G,专业技术职务 H" & vbNewLine & _
                " Where a.Id = b.号源id And b.Id = c.安排id(+) And a.科室id = f.Id And b.项目id = e.Id And b.医生ID = g.Id(+)" & vbNewLine & _
                "       And g.专业技术职务=h.名称(+) And b.出诊id = [1]" & strWhere & vbNewLine & _
                "       And Nvl(a.是否删除, 0) = 0 And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null)" & vbNewLine & _
                "       And (e.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or e.撤档时间 Is Null)" & vbNewLine & _
                "       And (f.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or f.撤档时间 Is Null)" & vbNewLine & _
                "       And (g.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or g.撤档时间 Is Null)" & vbNewLine & _
    ...
    Else
        ...
        strSQL = "Select " & str是否有效 & vbNewLine & _
                ...
                "        Decode(b.ID,Null,i.标识符,j.标识符)as 标识符,c.Id As 记录id, c.出诊日期, c.上班时段, c.限号数, c.限约数, b.开始时间, b.终止时间, " & vbNewLine & _
                ...
                "        From 临床出诊安排 Where 出诊id = [1]) B," & vbNewLine & _
                ...
                "       And Nvl(Nvl(f.站点,[8]),Nvl([7],'-')) = Nvl([7],'-')" & vbNewLine & _
                " Order By " & str排序号码 & ", c.出诊日期, c.上班时段"
    End If
    Set GetPlanRecords = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊ID, IIf(blnMonth, 1, 2), _
        UserInfo.ID, dtStartDate, dtEndDate, CDate(Format(dtStartDate, "yyyy-mm-01")), _
