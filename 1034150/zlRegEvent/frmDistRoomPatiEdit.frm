...
Object = "{D01C2596-4FE0-4EA9-9EE8-D97BE62A1165}#3.2#0"; "ZLPATIADDRESS.OCX"
...
Public mbln医嘱业务 As Boolean  '是否发生了医嘱医业务
...
Private Sub cbo年龄单位_LostFocus()
    If Not IsDate(txt出生日期.Text) Then
        mblnChange = False
        txt出生日期.Text = ReCalcBirth(Trim(txt年龄.Text), cbo年龄单位.Text)
        mblnChange = True
    End If
...
Private Sub lvwItems_DblClick()
    ...
    msh过敏.Text = Trim(lvwItems.SelectedItem.Text)
    msh过敏.SetFocus
    msh过敏_EnterCell
...
Private Sub msh过敏_EnterCell()
    ...
    If msh过敏.Row > 1 And msh过敏.TextMatrix(msh过敏.Row - 1, 0) = "" Then Exit Sub
...
'功能：初始化必要数据
    ...
    Else
        ...
        cbo付款方式.Width = txtPatiMCNO(0).Width
        lbl付款方式.Top = 1910 - lngTmp
        cbo付款方式.Top = 1850 - lngTmp
        lbl费别.Top = 1910 - lngTmp
        cbo费别.Top = 1850 - lngTmp
...
Private Sub txt出生时间_Change()
    ...
    If IsDate(txt出生日期.Text) Then
        ...
        txt年龄.Text = ReCalcOld(CDate(str出生时间), cbo年龄单位, mlng病人ID)
    End If
End Sub
...
Private Sub Form_Load()
    ...
    padd家庭地址.Visible = mblnStructAdress
    padd家庭地址.ShowTown = mblnShowTown
...
Private Sub txt年龄_Validate(Cancel As Boolean)
    ...
    If Not IsDate(txt出生日期.Text) Then
        mblnChange = False
        txt出生日期.Text = ReCalcBirth(Trim(txt年龄.Text), "")
        mblnChange = True
    End If
    '69026,冉俊明,2014-8-8,检查输入年龄
    '76703,冉俊明,2014-8-15
...
'---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If Not Me.Caption Like "创建挂号*" Then
        '75909
        If mlng挂号ID <> 0 And mbln医嘱业务 = True Then
