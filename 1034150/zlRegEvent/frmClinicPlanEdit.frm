...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case mMenuID.M_FeeItem
        Control.Visible = mbytPlanType = F_FixedRule _
            And mbytFun = Fun_TempPlan And IsHavePrivs(mstrPrivs, "所有科室")
        Control.Enabled = Control.Visible
    Case mMenuID.M_Verify
        Control.Visible = mbytPlanType = F_FixedRule _
            And (mbytFun = Fun_TempPlan Or mbytFun = Fun_AddSignalSourcePlan) _
            And IsHavePrivs(mstrPrivs, "审核临时固定安排") And mobj出诊安排.发布时间 <> ""
        Control.Enabled = Control.Visible
    Case conMenu_Edit_Save '保存数据
...
Private Function InitData(ByRef obj出诊安排 As 出诊安排, ByVal lng出诊ID As Long, _
    ...
    If mbytPlanType = F_FixedRule _
            And mbytFun = Fun_TempPlan And IsHavePrivs(mstrPrivs, "所有科室") Then
    ...
    If obj出诊安排.安排ID <> 0 Then
        ...
        End With
    Else
        If mbytPlanType = F_FixedRule And mbytFun = Fun_TempPlan And IsHavePrivs(mstrPrivs, "所有科室") Then
            '取最后一次有效安排的收费项目
...
Private Sub txtSignal_KeyPress(KeyAscii As Integer)
    ...
    If strText <> "" Then
        ...
        strSQL = "Select a.Id, a.号类, a.号码, b.名称 As 项目, c.名称 As 科室, a.医生姓名" & vbNewLine & _
                ...
                strWhere
        vRect = GetControlRect(txtSignal.Hwnd)
        ...
        If rsTemp Is Nothing Then
            MsgBox "没有找到需要新增的有效出诊号源！", vbInformation, gstrSysName
        ...
        '在有效时间范围内该号源不能有出诊安排
        strSQL = "Select b.开始时间,b.终止时间" & vbNewLine & _
                " From 临床出诊安排 A, (Select 开始时间,终止时间 From 临床出诊安排 Where 出诊ID=[2] And RowNum < 2) B" & vbNewLine & _
                " Where a.号源id = [1] And Not (a.开始时间 > b.终止时间 Or a.终止时间 < b.开始时间) And RowNum < 2" & vbNewLine & _
                " Union All" & vbNewLine & _
                " Select b.开始时间,b.终止时间" & vbNewLine & _
                " From 临床出诊记录 A, (Select 开始时间,终止时间 From 临床出诊安排 Where 出诊ID=[2] And RowNum < 2) B" & vbNewLine & _
                " Where a.号源id = [1] And a.出诊日期 Between b.开始时间 And b.终止时间 And RowNum < 2"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "检查在有效时间范围内该号源是否有出诊安排", mlng号源Id, mlng出诊ID)
        If Not rsTemp.EOF Then
            MsgBox "当前号源在有效期(" & Format(Nvl(rsTemp!开始时间), "yyyy-mm-dd hh:mm:ss") & "～" & Format(Nvl(rsTemp!终止时间), "yyyy-mm-dd hh:mm:ss") & ")内已存在出诊安排，不能将该号源添加到当前出诊表。", vbInformation, gstrSysName
            mlng号源Id = 0
            Exit Sub
        End If
    ...
    If Item.Checked Then
        ...
        If obj出诊记录 Is Nothing Then
            ...
            With mobj出诊安排.出诊号源
                ...
                Else
                    MsgBox "上班时间段“" & obj出诊记录.时间段 & "”不存在，请先在“基础设置>上班时间管理”中添加！", vbInformation, gstrSysName
                End If
    ...
    If mbytPlanType = F_FixedRule Then
        ...
        If mbytFun = Fun_AddSignalSourcePlan Then
            ...
            strSQL = "Select 1" & vbNewLine & _
                    ...
                    " Where 号源id = [1] And Not (开始时间 > [3] Or 终止时间 < [2]) And ID <> [4] And Rownum < 2" & vbNewLine & _
                    " Union All" & vbNewLine & _
                    " Select 1 From 临床出诊记录 Where 号源id = [1] And 出诊日期 Between [2] And [3] And 安排ID <> [4] And Rownum < 2"
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "检查在有效时间范围内该号源是否有出诊安排", mlng号源Id, dtStart, dtEnd, mlng安排ID)
            If Not rsTemp.EOF Then
                If mbytPlanType = F_FixedRule Then
                    MsgBox "当前号源在有效期(" & Format(dtStart, "yyyy-mm-dd hh:mm:ss") & "～" & Format(dtEnd, "yyyy-mm-dd hh:mm:ss") & ")内已存在出诊安排，请调整有效期，否则不能将该号源添加到当前出诊表。", vbInformation, gstrSysName
                Else
                    MsgBox "当前号源在有效期(" & Format(dtStart, "yyyy-mm-dd hh:mm:ss") & "～" & Format(dtEnd, "yyyy-mm-dd hh:mm:ss") & ")内已存在出诊安排，不能将该号源添加到当前出诊表。", vbInformation, gstrSysName
                End If
                Exit Function
            End If
        End If
        ...
        If mbytFun = Fun_TempPlan And IsHavePrivs(mstrPrivs, "所有科室") Then
            If cboFeeItem.ListIndex = -1 Then
