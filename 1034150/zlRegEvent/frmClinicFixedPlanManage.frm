...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Dim lng出诊ID As Long, varTemp As Variant
    ...
    If Not tbPage.Selected Is Nothing Then
        ...
        With vsfGrid
            ...
            If bytActiveGrid = 3 Then
                ...
                varTemp = .Cell(flexcpData, .Row, GetPlanItemNameCol(.Col) + 2)
...
Private Sub zlDataPrint(bytMode As Byte)
    ...
    If VSFlexGridCopyTo(IIf(tbPage.Selected.index = Pg_出诊规则, vsfRegistRule, vsfRegistPlan), vsfTemp, bytMode) = False Then Exit Sub
...
Private Sub vsfRegistPlan_DblClick()
    ...
    Dim strCurItem As String, varTemp As Variant
    ...
    Else
        ...
        '存储了Array(出诊ID,安排ID),周出诊表时每一行是多个出诊表中的安排
        varTemp = vsfRegistPlan.Cell(flexcpData, vsfRegistPlan.Row, lngCol + 2)
        lng出诊ID = varTemp(0): lng安排ID = varTemp(1)
    ...
    If lng安排ID = 0 Then
        ...
        If blnPublished Then
            strSQL = "Select " & str是否有效 & _
                    ...
                    "Where a.Id = b.号源id And b.安排ID = c.安排id(+) And a.科室id = f.Id And b.项目id = e.Id And b.医生ID = g.ID(+) And b.出诊ID = h.ID and g.专业技术职务=i.名称(+)" & strWhere & vbNewLine & _
...
Private Function GetPlanRecords(ByVal lng出诊ID As Long, _
    ...
    strSQL = "Select " & str是否有效 & vbNewLine & _
            ...
            "        f.名称 As 科室, f.简码 As 科室简码, e.名称 As 收费项目, a.医生姓名, g.专业技术职务 As 医生职称, g.简码 As 医生简码," & vbNewLine & _
            "        h.标识符,a.出诊id, a.Id As 安排id, a.开始时间, a.终止时间," & vbNewLine & _
