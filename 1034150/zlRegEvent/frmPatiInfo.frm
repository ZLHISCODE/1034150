...
Public mblnStation As Boolean
Private mstrStationPrivs As String
...
Private Const C_LinkManColumHeader = "联系人姓名,4,2400,1;联系人关系,4,2400,1;联系人身份证号,4,2400,1;联系人电话,4,2400,1" '格式:"列名","对齐","列宽"(其中对齐取值为:1-左对齐 4-居中 7-右对齐)
...
Public mbln零卡费走票号 As Boolean
Private mblnNameChange As Boolean
'110130,谢荣,2017-07-27,在挂号的同时，才更改病人信息或建档
Private mstrArrListSql() As String  '
Private mlngListSql  As Long
...
Private Sub lblICCard_Click()
    ...
    If gCurSendCard.bln就诊卡 And gCurSendCard.str卡名称 <> "二代身份证" Then
        ...
        If Not mobjICCard Is Nothing Then
            ...
            If txt卡号.Text <> "" And Not mfrmMain Is Nothing Then mfrmMain.mblnICCard = True
        End If
        Exit Sub
    End If
    '读取其卡信息
...
GetPatientTab:
        ...
        '获取病人ID
        If gobjSquare.objSquareCard.zlGetPatiID("身份证", strID, False, lng病人ID, strPassWord, strErrMsg, , , , InStr(gstrPrivs, ";合并病人信息;") > 0) = False Then lng病人ID = 0
             '操作员取消获取新病人
        bln允许签约 = True
        mblnNewPatient = True
        If lng病人ID = 0 Then
...
Private Function CheckValied() As Boolean
    ...
    If mblnCancel = False And mbytFun = 1 And mstrCard <> "" And gbln卡费仅划价 Or (mblnCancel = False And mbytFun = 0 And mlng病人ID <> 0) Then
        If mlng病人ID > 0 And mbln基本信息调整 And (mstr年龄 & mstr年龄单位 <> IIf(IsNumeric(txt年龄.Text), txt年龄.Text & cbo年龄单位.Text, txt年龄.Text) Or mstr性别 <> NeedName(cbo性别.Text) Or mstr姓名 <> txtPatient.Text Or _
            ...
            If MsgBox("病人基本信息已发生改变，是否继续？", vbInformation + vbYesNo, gstrSysName) = vbNo Then
                ...
                txt年龄.Text = mstr年龄
                If mstr年龄单位 <> "" Then cbo年龄单位.ListIndex = GetCboIndex(cbo年龄单位, mstr年龄单位): cbo年龄单位.Visible = True: txt年龄.Width = 690
    ...
    Else
        If zlCommFun.ActualLen(cbo家庭地址.Text) > 100 Then
            MsgBox "现住址输入过长，只允许输入100个字符或50个汉字，请检查!", vbInformation, gstrSysName
...
Private Sub cmdOK_Click()
    Dim strPati As String, strCard As String, strMCAccount As String, strTmp As String
    Dim rsCard As ADODB.Recordset, blnTrans As Boolean, strErrMsg As String, blnNewPati As Boolean
    Dim lng病人ID As Long, strNO As String, strYLKNo As String
    ...
    Dim str家庭地址 As String, str户口地址 As String, blnCard As Boolean
    
    txtPatient.Text = Trim(txtPatient.Text)
    txt年龄.Text = Trim(txt年龄.Text)
    '121819:李南春,2018/3/14,发卡卡号以发卡窗体重新输入的卡号为准
    mstrCard = Trim(txt卡号.Text)
    ...
    End If
    '问题号:57326
    If mlng病人ID <> 0 And mbytFun <> 0 Then
        If Check发卡性质(mlng病人ID, gCurSendCard.lng卡类别ID) = False Then Me.txt卡号 = ""
    End If
    ...
    If mbytFun = 1 And (mstrCard <> "" Or txtPatient.Text <> "") And gbln卡费仅划价 Or (mbytFun = 0 And mlng病人ID <> 0) Then
        ...
        "zl_挂号病人病案_INSERT(" & byt类型 & "," & lng病人ID & "," & txt门诊号.Text & "," & _
                  ...
                  IIf(Trim(txt联系人关系.Text) = "", "NULL,", "'" & Trim(txt联系人关系.Text) & "',")    '问题号:40005
        ...
        If byt类型 <> 3 Or blnCard Then
            '不为发卡,就是绑定卡
            Call ReLoadCardFee
            Set rsCard = GetSpecialInfo(gCurSendCard.str特准项目, gCurSendCard.lng收费细目ID)
            If bln发卡(True) = False Then
                blnBound = True
            Else
                blnBound = False
            End If
            Set rsCard = GetSpecialInfo(gCurSendCard.str特准项目)
            If rsCard Is Nothing Then
                MsgBox "不能正确提取" & gCurSendCard.str卡名称 & "费用信息！", vbInformation, gstrSysName
                Exit Sub
            End If
            '变价并且最低限价为零时,不收卡费
            '92468:李南春,2016/1/25,参数控制严格控制卡费为零的情况下，是否走票据号
            If Not (rsCard!是否变价 = 1 And Val("" & rsCard!原价) = 0 And mbln零卡费走票号 = False) And blnBound = False Then
                If Me.txt卡号.Text <> "" And (Nvl(rsCard!现价, 0) <> 0 Or mbln零卡费走票号) Then
                    '就诊卡费用:不是变价,屏蔽费别
                    ...
                    strCard = "zl_门诊划价记录_Insert('" & strNO & "',1," & lng病人ID & ",NULL," & txt门诊号.Text & "," & _
                              ...
                              strDate & ",NULL,'" & UserInfo.姓名 & "',NULL,'" & strYLKNo & "')"
                    
                    '90143:李南春,2016/1/25,存在卡费需要生成住院费用记录
                    str就诊卡 = zlGetSaveCardFeeSQL(gCurSendCard.lng卡类别ID, 0, strYLKNo, lng病人ID, 0, UserInfo.部门ID, UserInfo.部门ID, 0, _
                    NeedName(cbo费别.Text), mstrCard, Trim(txtPatient.Text), NeedName(cbo性别.Text), txt年龄.Text & cbo年龄单位.Text, _
                ...
                End If
            End If
            If Me.txt卡号.Text <> "" And str就诊卡 = "" Then
                If blnBound = False Then
                    str就诊卡 = GetCardDataSql(1, lng病人ID, gCurSendCard.lng卡类别ID, mstrCard, Me.txt卡号.Text, Me.txt密码.Text, _
                                        Datsys, "", "挂号发卡")
                Else
        ...
        If strPati <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = strPati
        End If
        If str家庭地址 <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = str家庭地址
        End If
        If str户口地址 <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = str户口地址
        End If
        If strCard <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = strCard
        End If
        
        If str就诊卡 <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = str就诊卡
        End If
    End If
...
Public Function SaveAfterArrList() As Boolean
    ...
    For i = 0 To UBound(mstrArrListSql)
        zlDatabase.ExecuteProcedure mstrArrListSql(i), Me.Caption
    Next
...
Private Function Check发卡性质(lng病人ID As Long, lng卡类别ID As Long) As Boolean
    ...
    On Error GoTo ErrHandl:
        ...
        Select Case gCurSendCard.lng发卡性质
            Case 0 '不限制
                Check发卡性质 = True
            Case 1 '同一个病人只允许发一张卡
                MsgBox "该病人已经发过" & gCurSendCard.str卡名称 & ",不能在进行发卡操作!", vbInformation + vbOKOnly
                Check发卡性质 = False
            Case 2 '同一个病人允许发多张卡,但需要提醒
               Check发卡性质 = MsgBox("该病人已经发过" & gCurSendCard.str卡名称 & ",是否要进行发卡操作?", vbQuestion + vbYesNo) = vbYes
        End Select
    Exit Function
ErrHandl:
    If ErrCenter() = 1 Then Resume
...
Private Sub Form_KeyPress(KeyAscii As Integer)
    If KeyAscii = Asc("'") Then
        KeyAscii = 0
    ElseIf KeyAscii = 13 Then
        '89242:李南春,2015/12/7,PatiAddress控件内部处理了跳转，外部不再处理
...
'功能：初始化必要数据
    ...
    If mblnStation Then
        ...
        If InStr(mstrStationPrivs, ";允许修改费别;") = 0 Then
            cbo费别.Enabled = False
        Else
            cbo费别.Enabled = True
        End If
...
'
'        lbl付款方式.Top = lbl付款方式.Top - lngTmp
'        cbo付款方式.Top = cbo付款方式.Top - lngTmp
'        lbl费别.Top = lbl费别.Top - lngTmp
'        cbo费别.Top = cbo费别.Top - lngTmp
'        chk复诊.Top = chk复诊.Top - lngTmp
'        Line1.Y1 = Line1.Y1 - lngTmp + 100
'        Line1.Y2 = Line1.Y2 - lngTmp + 100
'        Line2.Y1 = Line2.Y1 - lngTmp + 100
'        Line2.Y2 = Line2.Y2 - lngTmp + 100
'    End If
...
Private Sub Form_Load()
    ...
    mbln零卡费走票号 = Val(Nvl(zlDatabase.GetPara("零卡费走票号", glngSys, mlngModul, 0), 0)) = 1
...
Private Function LoadPati(ByVal strXmlCardInfor As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:加载病人信息
    '返回:加载成功,返回true,否则返回False
    '编制:刘兴洪
   ...
   '加载病人信息
    ...
    If strValue <> "" Then
        If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And txt年龄.Text = "") Then Call LoadOldData(strValue, txt年龄, cbo年龄单位)
    End If
   ...
   ' If strValue <> "" Then txt其他证件.Text = strValue
    ...
    If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And cbo职业.Text = "") Then
        If strValue <> "" Then
            cbo职业.ListIndex = GetCboIndex(cbo职业, strValue, , True)
...
'    End If
    ...
    If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And cbo婚姻.Text = "") Then
    cbo婚姻.ListIndex = GetCboIndex(cbo婚姻, strValue)
    ...
    If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And cbo家庭地址.Text = "") Then
        cbo家庭地址.Text = strValue
        padd家庭地址.Value = strValue
    End If
   '    户口地址    Varchar2    50
    Call zlXML_GetNodeValue("户口地址", , strValue)
    If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And txtRegLocation.Text = "") Then
   ...
   If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And txt家庭邮编.Text = "") Then txt家庭邮编.Text = strValue
    '    监护人  Varchar2    64
...
'    txt联系人电话.Text = strValue '问题号:40005
    ...
    If byt信息更新模式 = 0 Or mlng病人ID = 0 Or (mlng病人ID <> 0 And byt信息更新模式 = 2 And txt单位名称.Text = "") Then
   ...
   '问题号:56599
    ...
    Next
    lngCount = 0: lngChildCount = 0
   '101170：李南春，2017/5/3，从卡上获取病人信息后,调用EMPI
    Call zlQueryEMPIPatiInfo
    LoadPati = True
    Exit Function
errHandle:
...
'        lng变化长度 = lng变化长度 + 360
    ...
    End If
'
'    txt身份证号.Top = txt家庭电话.Top
'    txt身份证号.Left = txt门诊号.Left
'
'    lbl身份证.Top = txt身份证号.Top + (txt身份证号.Height - lbl身份证.Height) / 2
'    lbl身份证.Left = lbl门诊号.Left + lbl门诊号.Width - lbl身份证.Width
'
'    Me.Height = tbcPage.Top + tbcPage.Height + 1410
'    Me.Height = Me.Height - lng变化长度
'
'    txt支付密码.Top = txt家庭电话.Top + txt家庭电话.Height + lng间隔
'    txt支付密码.Left = txt家庭电话.Left
'
'    lbl支付密码.Top = txt支付密码.Top + (txt支付密码.Height - lbl支付密码.Height) / 2
'    lbl支付密码.Left = lbl家庭电话.Left + lbl家庭电话.Width - lbl支付密码.Width
'
'    txt验证密码.Top = txt支付密码.Top
'    txt验证密码.Left = txt身份证号.Left
'
'    lbl验证密码.Top = txt验证密码.Top + (txt验证密码.Height - lbl验证密码.Height) / 2
'    lbl验证密码.Left = lbl身份证.Left + lbl身份证.Width - lbl验证密码.Width
...
'    '医保号
'    txtPatiMCNO(0).Top = txt支付密码.Top + txt支付密码.Height + 100
'    txtPatiMCNO(0).Left = txt支付密码.Left
'    lblPatiMCNO(0).Top = txtPatiMCNO(0).Top + (txtPatiMCNO(0).Height - lblPatiMCNO(0).Height) / 2
'    lblPatiMCNO(0).Left = lbl支付密码.Left + lbl支付密码.Width - lblPatiMCNO(0).Width
'    '验证医保号
'    lblPatiMCNO(1).Top = lblPatiMCNO(0).Top
'    lblPatiMCNO(1).Left = lbl费别.Left + lbl费别.Width - lblPatiMCNO(1).Width
'    txtPatiMCNO(1).Top = txtPatiMCNO(0).Top
'    txtPatiMCNO(1).Left = cbo费别.Left
'
'    If lblPatiMCNO(0).Visible Then
'        cbo付款方式.Top = txtPatiMCNO(1).Top + txtPatiMCNO(1).Height + lng间隔
'        lbl付款方式.Top = cbo付款方式.Top + (cbo付款方式.Height - lbl付款方式.Height) / 2
'    Else
'        cbo付款方式.Top = txt支付密码.Top + txt支付密码.Height + lng间隔
'        lbl付款方式.Top = cbo付款方式.Top + (cbo付款方式.Height - lbl付款方式.Height) / 2
'    End If
'
'    cbo费别.Top = cbo付款方式.Top
'    lbl费别.Top = cbo费别.Top + (cbo费别.Height - lbl费别.Height) / 2
'    chk复诊.Top = lbl费别.Top
'
'
'    Line1.y1 = cbo付款方式.Top + cbo付款方式.Height + lng间隔
'    Line1.y2 = cbo付款方式.Top + cbo付款方式.Height + lng间隔
'    Line2.y1 = cbo付款方式.Top + cbo付款方式.Height + lng间隔
'    Line2.y2 = cbo付款方式.Top + cbo付款方式.Height + lng间隔
        
'    '三区第一行
'    cbo国籍.Top = Line1.y1 + lng间隔
'    lbl国籍.Top = cbo国籍.Top + (cbo国籍.Height - lbl国籍.Height) / 2
'
'    cbo民族.Top = cbo国籍.Top
'     lbl民族.Top = lbl国籍.Top
'     '三区第二行
'     cbo婚姻.Top = cbo国籍.Top + cbo国籍.Height + lng间隔
'     lbl婚姻.Top = cbo婚姻.Top + (cbo婚姻.Height - cbo婚姻.Height) / 2
'
'     cbo职业.Top = cbo婚姻.Top
'     lbl职业.Top = lbl婚姻.Top
'     '三区第三行
'     txt单位邮编.Top = cbo婚姻.Top + cbo婚姻.Height + lng间隔
'     lbl单位邮编.Top = txt单位邮编.Top + (txt单位邮编.Height - lbl单位邮编.Height) / 2
'
'     txt监护人.Top = txt单位邮编.Top
'     lbl监护人.Top = lbl单位邮编.Top
'
'
'    '三区第四行
'    txt联系人姓名.Top = txt单位邮编.Top + txt单位邮编.Height + lng间隔
'    lbl联系人姓名.Top = txt联系人姓名.Top + (txt联系人姓名.Height - lbl联系人姓名.Height) / 2
'
'    txt联系人身份证.Top = txt联系人姓名.Top
'    lbl联系人身份证.Top = lbl联系人姓名.Top
'
'    '三区第五行
'    txt联系人电话.Top = txt联系人姓名.Top + txt联系人姓名.Height + lng间隔
'    lbl联系人电话.Top = txt联系人电话.Top + (txt联系人电话.Height - lbl联系人电话.Height) / 2
'    txt联系人关系.Top = txt联系人电话.Top
'    lbl联系人关系.Top = lbl联系人电话.Top
'
'    '三区第6行
'    txt单位电话.Top = txt联系人电话.Top + txt联系人电话.Height + lng间隔
'    lbl单位电话.Top = txt单位电话.Top + (txt单位电话.Height - lbl单位电话.Height) / 2
'
'     txt家庭邮编.Top = txt单位电话.Top
'     lbl家庭邮编.Top = lbl单位电话.Top
'
'     txtBirthLocation.Top = txt单位电话.Top + txt单位电话.Height + lng间隔
'     lblBirthLocation.Top = txtBirthLocation.Top + (txtBirthLocation.Height - lblBirthLocation.Height) / 2
'     cmdBirthLocation.Top = txtBirthLocation.Top
'
'     '三区第7行
'     txt单位名称.Top = txtBirthLocation.Top + txtBirthLocation.Height + lng间隔
'     lbl单位名称.Top = txt单位名称.Top + (txt单位名称.Height - lbl单位名称.Height) / 2
'
'     cmd单位名称.Top = txt单位名称.Top
'     '三区第8行
'     txt区域.Top = txt单位名称.Top + txt单位名称.Height + lng间隔
'     lbl区域.Top = txt区域.Top + (txt区域.Height - lbl区域.Height) / 2
'
'     cmd区域.Top = txt区域.Top
'     '三区第9行
'     msh过敏.Top = txt区域.Top + txt区域.Height + lng间隔
'     '三区第10行
'     Line3.y1 = msh过敏.Top + msh过敏.Height + lng间隔
'     Line3.y2 = Line3.y1
'     Line4.y1 = Line3.y1
'     Line4.y2 = Line4.y1
End Sub
...
Private Sub zl加载病人信息(rsPatiInfo As Recordset)
    ...
    Dim strTmp As String
    '病人姓名
    txtPatient.Text = SetNullValue(rsPatiInfo!姓名)
    mlng病人ID = Val(Nvl(rsPatiInfo!病人ID))
    If SetNullValue(rsPatiInfo!性别) <> "" Then
    ...
    End If
    '年龄
    If SetNullValue(rsPatiInfo!年龄) <> "" Then
        Call LoadOldData(rsPatiInfo!年龄, txt年龄, cbo年龄单位)
    End If
    '出生日期
    If SetNullValue(rsPatiInfo!出生日期) <> "" Then
    ...
    '身份证号
    If SetNullValue(rsPatiInfo!身份证号) <> "" Then
    ...
    End If
    '职业
    If SetNullValue(rsPatiInfo!职业) <> "" Then
        cbo职业.ListIndex = GetCboIndex(cbo职业, rsPatiInfo!职业, , True)
        If cbo职业.ListIndex = -1 Then
    ...
    End If
    '民族
    cbo民族.ListIndex = GetCboIndex(cbo民族, SetNullValue(rsPatiInfo!民族))
     If cbo民族.ListIndex = -1 And SetNullValue(rsPatiInfo!民族) <> "" Then
         cbo民族.AddItem rsPatiInfo!民族, 0
         cbo民族.ListIndex = cbo民族.NewIndex
     End If
    '国籍
    cbo国籍.ListIndex = GetCboIndex(cbo国籍, SetNullValue(rsPatiInfo!国籍))
     If cbo国籍.ListIndex = -1 And SetNullValue(rsPatiInfo!国籍) <> "" Then
         cbo国籍.AddItem rsPatiInfo!国籍, 0
         cbo国籍.ListIndex = cbo国籍.NewIndex
     End If
    '婚姻状况
    cbo婚姻.ListIndex = GetCboIndex(cbo婚姻, SetNullValue(rsPatiInfo!婚姻状况))
     If cbo婚姻.ListIndex = -1 And SetNullValue(rsPatiInfo!婚姻状况) <> "" Then
         cbo婚姻.AddItem rsPatiInfo!婚姻状况, 0
         cbo婚姻.ListIndex = cbo婚姻.NewIndex
     End If
    txt区域.Text = SetNullValue(rsPatiInfo!区域)
    '家庭地址
    cbo家庭地址.Text = SetNullValue(rsPatiInfo!家庭地址)
    Call zlReadAddrInfo(padd家庭地址, Val(Nvl(rsPatiInfo!病人ID)), 0, 3, cbo家庭地址.Text)
    '家庭电话
    txt家庭电话.Text = SetNullValue(rsPatiInfo!家庭电话)
    '家庭地址邮编
    txt家庭邮编.Text = SetNullValue(rsPatiInfo!家庭地址邮编)
    '工作单位
    txt单位名称.Text = SetNullValue(rsPatiInfo!工作单位)
    lbl单位名称.Tag = ""
    '单位电话
    txt单位电话.Text = SetNullValue(rsPatiInfo!单位电话)
    '单位邮编
    txt单位邮编.Text = SetNullValue(rsPatiInfo!单位邮编)
    '门诊号
    txt门诊号.Text = SetNullValue(rsPatiInfo!门诊号)
    '问题号:40005
    '联系人姓名
    txt联系人姓名.Text = SetNullValue(rsPatiInfo!联系人姓名)
    '联系人电话
    txt联系人电话.Text = SetNullValue(rsPatiInfo!联系人电话)
    '联系人关系
    txt联系人关系.Text = SetNullValue(rsPatiInfo!联系人关系)
    '问题号:56599
    Load健康卡相关信息 (Val(SetNullValue(rsPatiInfo!病人ID, "0")))
    '手机号
...
Public Sub Load健康卡相关信息(lng病人ID As Long)
    ...
    Set rs病人信息 = zlDatabase.OpenSQLRecord(strSQL, "病人信息联系人信息", lng病人ID)
        ...
        txt联系人关系.Text = Nvl(rs病人信息!联系人关系)
...
'---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If gCurSendCard.bln严格控制 = True Then
        mlng磁卡领用ID = CheckUsedBill(5, IIf(mlng磁卡领用ID > 0, mlng磁卡领用ID, gCurSendCard.lng共用批次), IIf(blnCardNo, Trim(txt卡号.Text), UCase(txtPatient.Text)), gCurSendCard.lng卡类别ID)
...
Private Function SetBrushCardObject(ByVal blnComm As Boolean) As Boolean
    ...
    If gCurSendCard.lng卡类别ID = 0 Or Not gCurSendCard.bln刷卡 Then Exit Function
...
Private Sub zlQueryEMPIPatiInfo()
    ...
    On Error Resume Next
    If gobjPlugIn.EMPI_QueryPatiInfo(glngSys, 1111, rsTmp, rsOut) = False Then
    ...
    If mrsEMPIOut.EOF Then Exit Sub
    On Error Resume Next
    With mrsEMPIOut
        ...
        If Nvl(!职业) <> "" Then cbo职业.ListIndex = GetCboIndex(cbo职业, Nvl(!职业), , True)
        If Nvl(!工作单位) <> "" Then txt单位名称.Text = Nvl(!工作单位)
        If Nvl(!婚姻状况) <> "" Then cbo婚姻.ListIndex = GetCboIndex(cbo婚姻, Nvl(!婚姻状况))
        ...
        If Nvl(!联系人关系) <> "" Then txt联系人关系.Text = Nvl(!联系人关系)
    End With
...
Private Function zlSaveEMPIPatiInfo(ByVal blnNewPati As Boolean, ByVal lngPatiID As Long, ByVal lngClinicID As Long, ByRef strErrMsg As String) As Boolean
    ...
    If mrsEMPIOut Is Nothing Then
        'EMPI没有病人信息，需要新建
        On Error Resume Next
        lngRet = gobjPlugIn.EMPI_AddPatiInfo(glngSys, 1111, lngPatiID, 0, lngClinicID, strErrMsg)
    ...
    Else
        ...
        With mrsEMPIOut
            ...
            If cbo职业.ListIndex <> GetCboIndex(cbo职业, Nvl(!职业), , True) Then blnCharge = True: GoTo EMPIModify
            If txt单位名称.Text <> Nvl(!工作单位) Then blnCharge = True: GoTo EMPIModify
            If cbo婚姻.ListIndex <> GetCboIndex(cbo婚姻, Nvl(!婚姻状况)) Then blnCharge = True: GoTo EMPIModify
            ...
            If txt联系人关系.Text <> Nvl(!联系人关系) Then blnCharge = True: GoTo EMPIModify
        End With
    End If
EMPIModify:
    If blnCharge Then
        On Error Resume Next
        lngRet = gobjPlugIn.EMPI_ModifyPatiInfo(glngSys, 1111, lngPatiID, 0, lngClinicID, strErrMsg)
        Call zlPlugInErrH(Err, "EMPI_AddPatiInfo")
        If lngRet = 0 And Err.Number <> 438 Then Err.Clear: Exit Function
        Err.Clear: On Error GoTo Errhand
    End If
