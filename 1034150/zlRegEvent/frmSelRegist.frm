...
Public Function ShowRegist(ByVal frmMain As Form, ByVal strPrivs As String, _
    ...
    ByRef rsOutPatiInfor As ADODB.Recordset, Optional ByVal lngPatientID As Long) As Boolean
    '------------------------------------------------------------------------------------------------------------------------
    '功能：选择或提取预约单
    ...
    Set mrsInfo = rsOutPatiInfor: strOutNo = "": mlngPatientID = lngPatientID
    Me.Show 1, frmMain
...
Private Sub GetPatient(ByVal strInput As String, Optional blnCard As Boolean)
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strInput, CStr(Mid(strInput, 2)), gSysPara.Sy_Reg.bytNODaysGeneral, gSysPara.Sy_Reg.bytNoDayseMergency)
