...
Private Sub Cmd确定_Click()
    ...
    If mstrSQL = "" Then
        ...
        Set mrsSystems = OpenSQLRecord(mstrSQL, "读取可用报表", gclsLogin.DBUser)
        mstrSQL = ""
...
errH:
    If Not gobjComlib Is Nothing Then
        ...
        End If
    Else
        MsgBox "账套选择发生错误：" & Err.Description, vbInformation, gstrSysName
    End If
End Sub
...
Private Sub Form_Load()
    ...
    Set mrsSystems = OpenSQLRecord(mstrSQL, "读取可用系统", gclsLogin.DBUser)
    gclsLogin.IsSysOwner = mrsSystems.RecordCount > 0
    mstrSQL = "Select 1 From Zlsystems Where Mod(编号, 100) <> 0"
    Set mrsSystems = OpenSQLRecord(mstrSQL, "是否存在非标准帐套", gclsLogin.DBUser)
    ...
    mstrSQL = ""
    If mblnMutilSys = False Or glngHelperMainType <> 0 Then Cmd确定_Click
    Exit Sub
errH:
    If Not gobjComlib Is Nothing Then
        If gobjComlib.ErrCenter() = 1 Then
