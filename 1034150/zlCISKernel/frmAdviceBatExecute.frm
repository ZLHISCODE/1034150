...
Public Sub ShowMe(ByVal intType As Integer, ByRef frmParent As Object, ByVal lng病区ID As Long, ByVal lng病人ID As Long, ByVal int场合 As Integer, ByVal bytUseType As Byte, Optional ByVal lng医护科室ID As Long, _
    ...
    Me.lblInfo(0).Caption = IIF(mint场合 = 1, "当前病区：", IIF(Val(zlDatabase.GetPara("部门显示方式", glngSys, p住院医生站)) = 1, "当前病区：", "当前科室：")) & Get部门名称(IIF(mlng婴儿病区ID <> 0 And (mlng婴儿科室ID = mlng医护科室ID Or mlng婴儿病区ID = mlng医护科室ID), lng婴儿病区ID, lng病区ID))
...
Private Sub cboExecutePoeple_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    Call zlControl.CboSetIndex(cboExecutePoeple(Index).hWnd, zlControl.CboMatchIndex(cboExecutePoeple(Index).hWnd, KeyAscii))
End Sub
        ...
        For i = 0 To .Rows - 1
            If .Cell(flexcpData, i, col选择) = "1" And .TextMatrix(i, col相关ID) = "" Then
                ...
                Else
                    ...
                    arrSQL(UBound(arrSQL)) = "Zl_病人医嘱核对_Insert(" & Val(.TextMatrix(i, col医嘱ID)) & "," & Val(.TextMatrix(i, col发送号)) & ",'" & str核对人 & "')"
                End If
            End If
        Next
...
Private Sub Form_Load()
    ...
    'VSFlexGrid
    '-----------------------------------------------------
    strHead = ",400,1;床位,750,1;姓名,850,1;性别,450,1;住院号,1200,1;住院次数,950,1;期效,450,1;医嘱内容,2500,1;总量;单量,700,1;本次数次,840,7;给药途径;要求时间,1550,1;执行时间,1550,1;执行人,950,1;核对时间,1550,1;核对人,950,1;执行结果,950,1;未执行原因,1000,1;备注,2000,1;医嘱ID;相关ID;诊疗类别;病人ID;主页ID;频率;总量;发送号;待执行ID;医嘱状态;在院;登记人;原始执行时间;是否修改;最后执行人;操作类型;执行分类;病区ID;执行状态;开始执行时间;检查时间"
    ...
    If mbytUseType = T医嘱核对 Then
        Me.Caption = "医嘱批量核对"
        lblInfo(1).Caption = "执行时间"
        picExec.Visible = False
        lblInfo(9).Caption = "执行时间"
        lblInfo(11).Caption = "核对人"
        lblInfo(4).Visible = False
        chk期效(0).Visible = False
        chk期效(1).Visible = False
    ...
    End If
    If mbytUseType = T医嘱核对 Or InStr(GetInsidePrivs(IIF(mint场合 = 0, p住院医嘱下达, p住院医嘱发送)), "代行执行记录") > 0 Then
        picExec.Height = 720
    Else
        picExec.Height = 420
        optExecutePeople(2).value = True
        cboExecutePoeple(1).Visible = False
        lblInfo(11).Visible = False
    End If
End Sub
...
'参数：blnIsExecute=true加载已执行/已核对医嘱,false为加载待执行/待核对医嘱
    ...
    Dim blnHaveNoSelect As Boolean
    
    If Not blnIsExecute Then
        '病人
        ...
        If mbytUseType <> T医嘱核对 Then
            ...
            strSql = "Select b.要求时间, b.医嘱id, b.发送号,g.执行状态,Decode(A.执行频次,'一次性',NVL(A.总给予量,1),'需要时',NVL(A.总给予量,1),'持续性'," & _
                    ...
                    " From 医嘱执行时间 B" & _
                    ", 病人医嘱记录 A, 诊疗项目目录 C,材料特性 F,收费项目目录 E,病人医嘱发送 G" & vbNewLine & _
                    " Where a.Id = b.医嘱id And e.ID=f.材料ID(+) And e.id(+) = a.收费细目id And a.诊疗项目id = c.Id And g.发送号=b.发送号 And b.医嘱ID=g.医嘱ID " & _
                    " And Not Exists (Select 1 From 病人医嘱执行 Where b.要求时间 = 要求时间 And b.医嘱id = 医嘱id And b.发送号 = 发送号) " & _
                    " And (g.执行部门id=[6] Or Exists (Select 1 From 病案主页 H Where a.病人id=h.病人id And a.主页id=h.主页id And (h.出院科室id=g.执行部门id Or h.当前病区id=g.执行部门id)) " & " or A.执行性质=5 and a.执行标记=0) "
            '要求时间
            strSql = strSql & " And b.要求时间 +0 Between [1] And [2]"
            '期效
            If chk期效(0).value And chk期效(1).value Then
                strTmp = ""
            ElseIf chk期效(0).value Then
                strTmp = " And a.医嘱期效=0 "
            ElseIf chk期效(1).value Then
                strTmp = " And a.医嘱期效=1 "
            End If
            ...
            '其他
            strFitter = strFitter & IIF(chkType(Type其它医嘱).value, IIF(strFitter <> "", " Or ", "") & IIF(gbln执行发料, " a.诊疗类别='4' ", " a.诊疗类别='4' And f.跟踪在用<>1 ") & _
        ...
        Else
            ...
            strSql = "Select b.要求时间, b.医嘱id, b.发送号, b.执行人, b.执行时间, b.执行结果, b.说明, b.执行摘要, b.登记人,b.核对人,b.核对时间,g.执行状态,B.本次数次,Decode(g.执行状态,1,g.完成时间,2,g.完成时间, B.登记时间) 检查时间 " & _
                    ...
                    " And (g.执行部门id=[6] Or Exists (Select 1 From 病案主页 H Where a.病人id=h.病人id And a.主页id=h.主页id And (h.出院科室id=g.执行部门id Or h.当前病区id=g.执行部门id))) And b.核对时间 Is Null "
            '要求时间，核对时根据执行时间
            strSql = strSql & " And b.执行时间 Between [1] And [2]"
            '期效,核对跳过期效
            '诊疗类别-核对只包含输血和皮试
            strFitter = strFitter & IIF(chkType(Type输血).value, IIF(strFitter <> "", " Or ", "") & " (a.诊疗类别='K' or a.诊疗类别='E' And c.操作类型='8') ", "")
            If chkType(Type皮试).value = 0 And chkType(Type输血).value = 0 Then
                strFitter = strFitter & IIF(strFitter <> "", " Or ", "") & " a.诊疗类别='K' Or a.诊疗类别='E' And c.操作类型 in ('1','8')"
            End If
        End If
        
        strFitter = strFitter & IIF(chkType(Type皮试).value, IIF(strFitter <> "", " Or ", "") & " a.诊疗类别='E' And c.操作类型='1' and c.执行分类=3", "")
        
        strSql = strSql & IIF(strFitter <> "", " And (" & strFitter & ")", "")
        '保存过滤条件
        blnSetup = InStr(GetInsidePrivs(p住院医嘱发送), ";医嘱选项设置;") > 0
        Call zlDatabase.SetPara("医嘱执行期效", chk期效(0).value & chk期效(1).value & "", glngSys, p住院医嘱发送, blnSetup)
        Call zlDatabase.SetPara("医嘱执行范围", chkType(Type输液).value & chkType(Type注射).value & chkType(Type口服).value & _
                chkType(Type皮试).value & chkType(Type治疗).value & chkType(Type采集).value & chkType(Type其它医嘱).value & chkType(Type输血).value & chkType(Type其它给药途径).value & chkType(Type中药服法).value & "", glngSys, p住院医嘱发送, blnSetup)
        
        
        If blnHaveNoSelect Then
            strSql = strSql & " And (a.病人ID,A.主页ID) In(Select C1 As 病人ID,C2 As 主页ID From Table(f_Num2list2([8])))"
        Else
            '如果是全选，则使用病区ID访问
            strSql = strSql & " And (a.病人ID,A.主页ID) In (Select h.病人id, h.主页id From 病案主页 H, 在院病人 R Where h.病人id = r.病人id " & _
                            IIF(mint场合 = 1, " And h.当前病区ID+0=R.病区ID  And (R.病区ID=[7] Or h.婴儿病区ID=[7])", "  And h.出院科室id+0=R.科室ID And (R.科室ID=[7] Or h.婴儿科室ID=[7])") & ")"
    ...
    Else
        ...
        strSql = "Select b.医嘱id, b.发送号, b.要求时间,B.本次数次, b.执行人, b.执行时间, b.执行结果, b.说明, b.执行摘要, b.登记人,b.核对人,b.核对时间 ,g.执行状态,Decode(g.执行状态,1,g.完成时间,2,g.完成时间, B.登记时间) 检查时间 " & vbNewLine & _
                "       From 病人医嘱执行 B,医嘱执行时间 A,病人医嘱发送 G,病人医嘱记录 C" & vbNewLine & _
                "       Where a.要求时间=b.要求时间 and a.医嘱id=b.医嘱id and a.发送号=b.发送号 And g.发送号=b.发送号 And b.医嘱ID=g.医嘱ID And b.医嘱ID=c.ID and (" & _
                " g.执行部门id=[6] Or Exists (Select 1 From 病案主页 H Where c.病人id=h.病人id And c.主页id=h.主页id And (h.出院科室id=g.执行部门id Or h.当前病区id=g.执行部门id))" & _
                IIF(mbytUseType <> T医嘱核对, " or c.执行性质=5 and c.执行标记=0 ", "") & ") " & _
                IIF(mbytUseType <> T医嘱核对, " And A.要求时间 ", " And b.核对时间 Is Not Null  And b.执行时间 ") & " Between [3] And [4] "
                
         '如果没有代行执行的权限，也不允许取消他人的执行记录
        If mbytUseType = T医嘱执行 And InStr(GetInsidePrivs(IIF(mint场合 = 0, p住院医嘱下达, p住院医嘱发送)), "代行执行记录") = 0 Then
            strSql = strSql & " And B.执行人=[5] "
            strExecPeople = UserInfo.姓名
        Else
            If cboExecutePoeple(1).ListCount >= 0 Then
                strSql = strSql & IIF(cboExecutePoeple(1).Text = "", "", IIF(mbytUseType <> T医嘱核对, " And B.执行人=[5] ", " And B.核对人=[5] "))
    ...
    If blnIsExecute Or mbytUseType = T医嘱核对 Then
        strSql = "Select b.执行人, to_char(b.执行时间,'YYYY-MM-DD HH24:MI') as 执行时间, b.执行结果, b.说明 as 未执行原因, b.执行摘要 as 备注,b.登记人,b.执行时间 as 原始执行时间,b.核对人,to_char(b.核对时间,'YYYY-MM-DD HH24:MI') as 核对时间," & _
            ...
            "  Round(a.总给予量 / Decode(a.病人来源, 2, d.住院包装, d.门诊包装), 5) || Decode(a.病人来源, 2, d.住院单位, d.门诊单位)) As 总量,B.检查时间,F.住院号,G.住院次数" & vbNewLine & _
            ...
            " And (F.婴儿科室ID is null or F.婴儿科室ID is not null and (F.婴儿病区ID=[7] or F.婴儿科室ID=[7]) and NVL(A.婴儿,0)<>0 or F.婴儿科室ID is not null and (F.婴儿病区ID<>[7] and f.婴儿科室ID<>[7]) and NVL(A.婴儿,0)=0) "
    Else
        strSql = "Select distinct" & _
            "   a.Id, b.发送号,b.医嘱id as 待执行ID,a.相关id, a.诊疗类别,b.执行状态,a.开始执行时间, A.姓名, f.出院病床 As 床号, A.性别, Decode(Nvl(a.医嘱期效, 0), 0, '长嘱', '临嘱') As 期效,a.医嘱状态,Decode(f.出院日期,NULL,1,0) as 在院,a.总给予量,a.单次用量," & vbNewLine & _
            "       Decode(a.单次用量, Null, Null, decode(sign(1-A.单次用量),1,'0'||A.单次用量,A.单次用量) || c.计算单位) As 单量,  Decode(a.相关id,Null,a.医嘱内容 || ' ' || a.执行频次  ,a.医嘱内容) as 医嘱内容, to_char(b.要求时间,'YYYY-MM-DD HH24:MI') as 要求时间,f.当前病区ID,B.本次数次" & _
            ", a.执行频次 As 频率, a.病人id, a.主页id, a.诊疗项目id,c.操作类型,c.执行分类,Decode(a.总给予量, Null, Null," & _
            "  Round(a.总给予量 / Decode(a.病人来源, 2, d.住院包装, d.门诊包装), 5) || Decode(a.病人来源, 2, d.住院单位, d.门诊单位)) As 总量,first_value(e.执行人) Over(partition By e.医嘱id Order By e.执行时间 DESC) As 最后执行人,b.要求时间, a.序号,F.住院号,G.住院次数" & vbNewLine & _
            " From (" & strSql & ") B, 病人医嘱记录 A,病案主页 F, 诊疗项目目录 C, 药品规格 D ,病人医嘱执行 E,病人信息 G" & vbNewLine & _
            " Where (a.Id = b.医嘱id Or a.相关id = b.医嘱id) And e.医嘱id(+) = a.Id And f.病人id = a.病人id And f.主页id = a.主页id  And F.病人ID=G.病人ID And a.诊疗项目id = c.Id And a.收费细目id = d.药品id(+) And a.诊疗类别 Not In('C','7') And Not (a.诊疗类别='E' And c.操作类型='3') " & _
            Decode(mint医嘱处理范围, 1, " And nvl(a.婴儿,0) = 0 ", 2, " And nvl(a.婴儿,0) <> 0 ", "") & _
            " And (F.婴儿科室ID is null or F.婴儿科室ID is not null and (F.婴儿病区ID=[7] or F.婴儿科室ID=[7]) and NVL(A.婴儿,0)<>0 or F.婴儿科室ID is not null and (F.婴儿病区ID<>[7] and f.婴儿科室ID<>[7]) and NVL(A.婴儿,0)=0) "
    End If
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, CDate(dpkReqTime(0).value), CDate(dpkReqTime(1).value), CDate(dpkExecuted(0).value), CDate(dpkExecuted(1).value), strExecPeople, mlng病区ID, mlng医护科室ID, strPatis)
...
'功能：初始化界面
    ...
    cboExecutePoeple(0).AddItem ""
    cboExecutePoeple(1).AddItem ""
    Do While Not rsTmp.EOF
        cboExecutePoeple(0).AddItem rsTmp!姓名 & ""
        cboExecutePoeple(0).ItemData(cboExecutePoeple(0).NewIndex) = rsTmp!ID & ""
        cboExecutePoeple(1).AddItem rsTmp!姓名 & ""
        cboExecutePoeple(1).ItemData(cboExecutePoeple(1).NewIndex) = rsTmp!ID & ""
                If rsTmp!姓名 = UserInfo.姓名 Then
            cboExecutePoeple(0).ListIndex = cboExecutePoeple(0).ListCount - 1
            cboExecutePoeple(1).ListIndex = cboExecutePoeple(1).ListCount - 1
        End If
        rsTmp.MoveNext
    Loop
    ...
    chk期效(0).value = Val(Mid(strTmp, 1, 1))
    chk期效(1).value = Val(Mid(strTmp, 2, 1))
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If str病人IDs <> "" Then
        ...
        If UBound(Split(str病人IDs, ",")) = 0 And Val(str病人IDs) = mlng病人ID Then
            Call zlDatabase.SetPara("发送病人", "", glngSys, p住院医嘱发送, blnSetup)
        Else
            Call zlDatabase.SetPara("发送病人", mlng病区ID & ":" & str病人IDs, glngSys, p住院医嘱发送, blnSetup)
        End If
    End If
...
Private Sub picExecuted_Resize()
    On Error Resume Next
    vsgExecAdvice.Width = picExecuted.Width - 200
...
Private Sub picWaitExecAdvice_Resize()
    ...
    vsgWaitExecAdvice.Height = picWaitExecAdvice.Height - vsgWaitExecAdvice.Top
    vsgWaitExecAdvice.Width = picWaitExecAdvice.Width
...
Private Sub picWaitExecute_Resize()
    ...
    rptPati.Height = fraPatiInfo.Height - rptPati.Top - picFitter.Height - 250
...
Private Sub vsgExecAdvice_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    If vsgExecAdvice.Cell(flexcpBackColor, NewRow, NewCol) = COLEditBackColor And vsgExecAdvice.RowData(NewRow) = "Begin" Then
        If NewCol = col执行人 Then
            If (InStr(GetInsidePrivs(IIF(mint场合 = 0, p住院医嘱下达, p住院医嘱发送)), "代行执行记录") > 0 And mbytUseType = T医嘱执行 Or mbytUseType = T医嘱核对) Then
...
Private Sub vsgExecAdvice_Click()
    Dim i As Long
...
Private Sub vsgWaitExecAdvice_Click()
    Dim i As Long
