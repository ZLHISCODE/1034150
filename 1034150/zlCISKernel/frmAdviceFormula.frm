...
Private mblnStock As Boolean '指定药房时限定库存，因为共用窗体，所以不能使用全表变量gblnStock
...
Public Function ShowMe(ByVal frmParent As Object, ByVal objclsInsure As Object, ByVal lngHwnd As Long, ByRef t_Pati As TYPE_PatiInfoEx, ByVal int场合 As Integer, _
             ...
             Optional ByRef str摘要 As String, Optional ByVal lng药品ID As Long, Optional ByVal lngPreRow中药房 As Long) As Boolean
'参数:
'     frmParent         父窗体
...
Private Sub Form_Load()
    ...
    mlng中药房 = Val(zlDatabase.GetPara(IIF(mint调用类型 = 2, "住院", "门诊") & "缺省中药房", glngSys, IIF(mint调用类型 = 2, pm住院医嘱下达, pm门诊医嘱下达)))
    mstr可用中药房 = zlDatabase.GetPara(IIF(mint调用类型 = 2, "住院", "门诊") & "可用中药房", glngSys, IIF(mint调用类型 = 2, pm住院医嘱下达, pm门诊医嘱下达))
    ...
    Call zlControl.VSFSetFontSize(vsExt, IIF(mbytSize = 0, 9, 12))
    Call zlControl.VSFSetFontSize(vs中药规格, IIF(mbytSize = 0, 9, 12))
...
'参数：mstrExtData=包含每味中药信息及煎法信息的串,为空时表示新输入中药配方
    ...
    '指定药房时限制库存
    mblnStock = Val(zlDatabase.GetPara(18, glngSys)) <> 0
    '收费项目输入简码匹配方式:10.输入全是数字时只匹配编码  01.输入全是字母时只匹配简码,11两者均要求
    mstrMatchMode = zlDatabase.GetPara(44, glngSys, , "00")
    ...
    strSql = "Select A.ID,A.编码,A.名称 From 诊疗项目目录 A" & _
        ...
        IIF(mint场合 <> 3, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[2])" & _
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        rsTmp.MoveNext
    Next
    If cboData.ListCount = 1 And cboData.ListIndex = -1 Then Call zlControl.CboSetIndex(cboData.hWnd, 0)
    ...
    End If
    Call zlControl.CboLocate(cbo药房, lng药房ID, True)
    If cbo药房.ListCount > 0 And cbo药房.ListIndex = -1 Then Call zlControl.CboSetIndex(cbo药房.hWnd, 0)
...
'参数：bln配方 true=新开时调用配方
    ...
    Else
        If mstr可用中药房 <> "" Then
            If mblnStock And Not blnFirst Then
...
'      如果是散装形态，则加载可选择的规格下拉列表
    ...
    With vs中药规格
        ...
        If mint场合 <> 3 Then
            '显示中药库存
            strSql = "Select d.编码, d.规格, d.名称, e.名称 As 药房, d.计算单位," & vbNewLine & _
                    "       Sum(Nvl(m.可用数量, 0))  As 可用数量" & vbNewLine & _
                ...
                Else
                    ...
                    Else
                        ...
                        If Nvl(rsTmp!是否变价, 0) = 0 Then
                            .TextMatrix(i, col单价) = Format(CalcPrice(Val("" & rsTmp!药品ID), , , True), gstrDecPrice)
                        Else '时价
                            .TextMatrix(i, col单价) = Format(CalcDrugPrice(Val("" & rsTmp!药品ID), cbo药房.ItemData(cbo药房.ListIndex), Val(.TextMatrix(i, col数量)), , True), gstrDecPrice)
                        End If
                    End If
                End If
            Next
...
Private Sub optMode_Click(Index As Integer)
    ...
    If lng药名ID <> 0 And mblnStock Then     '指定药房时限定库存时，形态改变后，第一味药的缺省规格可能变了，可用药房就变了
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                If lng药品ID <> Val(rsTmp!药品ID) Then
                    ...
                    If cbo药房.ListIndex = -1 And cbo药房.ListCount > 0 Then
                        Call zlControl.CboSetIndex(cbo药房.hWnd, 0)
                    End If
                End If
            End If
        End If
    ...
    With vs中药规格
        If .Col = col规格 Then
            ...
            '散装中药，选择规格之后
                ...
                If Nvl(rsTmp!是否变价, 0) = 0 Then
                    .TextMatrix(Row, col单价) = Format(CalcPrice(Val("" & rsTmp!药品ID), , , True), gstrDecPrice)
                Else '时价
                    .TextMatrix(Row, col单价) = Format(CalcDrugPrice(Val("" & rsTmp!药品ID), cbo药房.ItemData(cbo药房.ListIndex), Val(.TextMatrix(Row, col数量)), , True), gstrDecPrice)
                End If
...
'功能：根据输入内容或按*（或按选择按钮）弹出并返回选择的记录集
    ...
    '中药库存,散装才显示库存,中药房未指定时,读不出库存记录
    If lng形态 = 0 And lng药房ID <> 0 Then
        strStock = _
    ...
    If lng形态 = 0 Then
        str规格 = _
                " And Nvl(C.中药形态,0) = [4] And (D.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or D.撤档时间 IS NULL) And D.服务对象 IN([1],3)" & _
                " And (D.站点='" & gstrNodeNo & "' Or D.站点 is Null)"
                
        If mblnStock And (mlng中药房 <> 0 And mint场合 <> 3 Or lng药房ID <> 0 And mint场合 = 3) Then
        ...
        End If
    Else
         str规格 = " And Exists(Select 1 From 药品规格 C,收费项目目录 S Where C.药名ID=E.药名ID And C.药品id=S.ID And Nvl(C.中药形态,0) = [4] And S.服务对象 IN([1],3) " & _
    ...
    If blnInputKey Then
        ...
        If IsNumeric(strCode) Then
            ...
            If Mid(mstrMatchMode, 1, 1) = "1" Then strInput = " And (A.编码 Like [5] And B.码类=[7] Or B.简码 Like [6] And B.码类=3)"
        ElseIf zlCommFun.IsCharAlpha(strCode) Then
            'X1.输入全是字母时只匹配简码
            If Mid(mstrMatchMode, 2, 1) = "1" Then strInput = " And B.简码 Like [6] And B.码类=[7]"
        ...
        Else
            '非散装时按品种显示，且不显示库存
            strSql = _
                " Select Distinct(a.Id),A.编码,A.名称,A.计算单位 as 单位,r.费用类型 As 费用类型,E.处方职务 as 处方职务ID,A.ID as 药名ID" & _
                " From 药品特性 E,药品规格 Q, 收费项目目录 R,(" & strSql & ") A" & _
                " Where A.ID=E.药名ID And e.药名id = q.药名id(+) And q.药品id = r.Id(+)" & str特性 & str规格 & str存储库房 & _
                IIF(strLike = "", "", " And Rownum<=100") & _
                " Order by A.编码"
        End If
        ...
        Set rsTmp = zlDatabase.ShowSQLSelect(Me, strSql, 0, "中药", False, "", "", False, False, True, vPoint.X, vPoint.Y, vsExt.CellHeight, blnCancel, False, True, _
            ...
            strLike & strCode & "%", mint简码 + 1, mlng病人科室id)
    Else
        If lng形态 = 0 Then
        ...
        Else
            '非散装时按品种显示，且不显示库存
            strSql = "Select 0 as 末级,-1 as ID,-NULL as 上级ID,NULL as 编码," & _
                " CHR(13)||'常用中药' as 名称,NULL as 单位,NULL As 费用类型,NULL as 处方职务ID,NULL as 药名ID From Dual" & _
                " Union ALL" & _
                " Select 0 as 末级,ID,上级ID,编码,名称,NULL as 单位,NULL As 费用类型,NULL as 处方职务ID,NULL as 药名ID" & _
                " From 诊疗分类目录 Where 类型=3 And (撤档时间 Is Null Or 撤档时间=To_Date('3000-01-01','YYYY-MM-DD'))" & _
                " Start With 上级ID is NULL Connect by Prior ID=上级ID"
                    
            strMain = "Select Distinct(1) as 末级,a.Id,A.分类ID as 上级ID,A.编码,A.名称,A.计算单位 as 单位,r.费用类型 As 费用类型,E.处方职务 as 处方职务ID,A.ID as 药名ID" & _
                " From 诊疗项目目录 A,药品特性 E,药品规格 Q, 收费项目目录 R" & _
                " Where A.ID=E.药名ID And e.药名id = q.药名id(+) And q.药品id = r.Id(+)" & str特性 & str诊疗 & str规格 & str存储库房
        End If
    ...
    If lng形态 = 0 Then
        ...
        If lng药品ID <> rsTmp!药品ID And vsExt.Row = vsExt.FixedRows And vsExt.Col = vsExt.FixedCols Then
            ...
            If cbo药房.ListIndex = -1 And cbo药房.ListCount > 0 Then Call zlControl.CboSetIndex(cbo药房.hWnd, 0)
    ...
    If mint场合 <> 3 Then
        If Not mclsInsure Is Nothing And mlng病人ID <> 0 Then  '中药配方
            ...
            If UBound(Split(str中药IDs, ",")) = 0 Then
                str摘要 = mclsInsure.GetItemInfo(mint险类, mlng病人ID, Val(Split(mcol规格数量.Item(1), ",")(0)), "", 0, "", str中药IDs)
            Else
                str摘要 = mclsInsure.GetItemInfo(mint险类, mlng病人ID, 0, "", 0, "", str中药IDs)
            End If
        End If
    End If
    
    mstrExtData = strTmp & "|" & Trim(txtJL.Text)
    mstr摘要 = str摘要
    mblnOK = True
    Unload Me
    Exit Sub
