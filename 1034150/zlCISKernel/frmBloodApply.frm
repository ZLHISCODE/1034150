...
Public Function ShowMe(frmParent As Object, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病人性质 As Long, ByVal intType As Integer, Optional ByVal lngUpdateAdvice As Long, _
            ...
            Optional ByVal lng项目ID As Long, Optional ByRef rsCard As ADODB.Recordset, Optional ByVal bytBaby As Byte, Optional ByVal int调用场合 As Integer) As Boolean
...
'功能：保存数据
    ...
    Else
        ...
        str审核状态 = GetBloodState(int紧急, int分类)
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & ",NULL," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & "," & mbytBaby & ",1,1,'K'," & mlng输血项目ID & _
                                 ...
                                 strPar & ",Null,0,NULL," & IIF(mstr摘要输血 = "", "null", "'" & mstr摘要输血 & "'") & ",'" & UserInfo.姓名 & "',Null,NULL,'" & cboInfo(cbo输血目的).Text & "'," & str审核状态 & "," & lng申请序号 & ")"
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & "," & lng相关ID & "," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & _
                                 "," & mbytBaby & ",1,1,'E'," & mlng输血途径 & ",NULL,NULL,NULL,Null,'" & str输血途径 & "',Null,NULL,'一次性',NULL,NULL,NULL,NULL,0," & _
    ...
    '输血申请其他项目
    ReDim Preserve arrSQL(UBound(arrSQL) + 1)
    arrSQL(UBound(arrSQL)) = "Zl_输血申请记录_Insert(" & lng相关ID & "," & chkWait.value & ",NULL," & cboInfo(cbo输血目的).ListIndex & "," & cboInfo(cbo输血性质).ListIndex & "," & IIF(optHistory(0).value, 0, 1) & _
                             ",NULL,NULL,'" & txtInfo(txt孕) & "/" & txtInfo(txt产) & "'," & IIF(optPossession(0).value, 0, 1) & _
                             "," & cboInfo(cbo输血血型).ListIndex & "," & cboInfo(cboRHD).ListIndex & ",NULL)"
    '检验项目
    With vsLIS
        lngCount = 0
        For i = 0 To .Rows - 1
    ...
    If mstr诊断IDs <> "" Then
        ...
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = "Zl_病人医嘱附件_Insert(" & lng相关ID & ",'申请单诊断',null,null,null,'" & txtInfo(txt诊断信息).Text & "',0)"
    End If
    ...
    If mint场合 = 0 Then
        ...
        ElseIf str审核状态 = "1" Then
            Call ZLHIS_CIS_Audit("ZLHIS_CIS_030", mclsMipModule, mlng病人ID, Trim(txtInfo(txt姓名).Text), Trim(txtInfo(txt住院号).Text), , IIF(mlng病人性质 = 1, 1, 2), _
                mlng主页ID, mlng病区ID, , mlng病人科室id, "", , Trim(txtInfo(txt床号).Text), lng医嘱ID, UserInfo.姓名, _
                Format(txtInfo(txt申请日期).Text, "yyyy-MM-dd HH:mm:ss"), mlng开单科室ID, "", , , "")
        ElseIf str审核状态 = "4" Then
            If Not (mclsMipModule Is Nothing) Then
                If mclsMipModule.IsConnect Then
                    Call ZLHIS_CIS_031(mclsMipModule, mlng病人ID, Trim(txtInfo(txt姓名).Text), Trim(txtInfo(txt住院号).Text), , IIF(mlng病人性质 = 1, 1, 2), _
                        mlng主页ID, mlng病区ID, , mlng病人科室id, "", , Trim(txtInfo(txt床号).Text), lng医嘱ID, UserInfo.姓名, _
                        Format(txtInfo(txt申请日期).Text, "yyyy-MM-dd HH:mm:ss"), mlng开单科室ID, "", , , "")
                End If
            End If
...
'功能：读取申请单信息
    ...
    ElseIf mintType = 1 Or mintType = 3 Or mintType = 2 Then
        ...
        strSql = "Select A.ID,A.相关ID,a.紧急标志,a.用药理由,NVL(to_char(a.手术时间,'yyyy-MM-dd hh24:mi'),a.标本部位) as 预定输血时间,a.开始执行时间,a.诊疗项目ID," & _
                "a.执行科室ID,a.执行性质,a.总给予量,B.计算单位,B.名称 as 项目名称,A.申请序号,A.审核状态,a.医生嘱托" & _
...
'                End If
    ...
    If mlng输血项目ID <> 0 Then
        Set rsTmp = Get诊疗项目记录(mlng输血项目ID)
        Call Set执行科室(Val(rsTmp!执行科室 & ""))
        txtGet(txt预定输血成分).Text = rsTmp!名称 & ""
        txtInfo(txt单位).Text = rsTmp!计算单位 & ""
        txtGet(txt预定输血成分).Tag = txtGet(txt预定输血成分).Text
        mlng录入限量 = Val(rsTmp!录入限量 & "")
        If mrsCard Is Nothing And mint调用场合 = 1 Then Call SetLisResult(mlng输血项目ID)
    End If
...
'功能：初始化输血项目对应的检验项目指标表格
    ...
    Dim strResult As String, str项目名称 As String
...
'            指标2<split1>诊疗编码2<split1>隐私项目2<split1>指标代码2<split1>中文名2<split1>英文名2<split1>取值序列2<split1>
            ...
            Set rs结果 = zldatabase.OpenSQLRecord(strSql, Me.Caption, str项目名称, strPar)
...
'功能：缓存数据
    ...
    With mrsCard
        ...
        !检查结果 = strResult
	!申请其他项目SQL = "Zl_输血申请记录_Insert([相关ID]," & chkWait.value & ",NULL," & cboInfo(cbo输血目的).ListIndex & "," & cboInfo(cbo输血性质).ListIndex & "," & IIF(optHistory(0).value, 0, 1) & _
                             ",NULL,NULL,'" & txtInfo(txt孕) & "/" & txtInfo(txt产) & "'," & IIF(optPossession(0).value, 0, 1) & _
                             "," & cboInfo(cbo输血血型).ListIndex & "," & cboInfo(cboRHD).ListIndex & ",NULL)"
        !检验项目SQL = str检验项目SQL
...
'功能：通过缓存数据加载界面
    ...
    If blnDo Then
        With mrsCard
            ...
            If !输血目的 & "" <> "" Then
                Call zlControl.CboSetText(cboInfo(cbo输血目的), !输血目的 & "", False)
'                Call zlControl.CboSetIndex(cboInfo(cbo输血执行).hWnd, 0)
            End If
            ...
            txtInfo(txt预定输血量).Text = Val(!预定输血量 & "")
