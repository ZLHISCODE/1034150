...
'功能：XML预处理
    ...
    Loop
    '动态的诊疗项目ID
    strSQL = "select C.ID,C.类别,C.名称 from  病历单据应用 B,诊疗项目目录 C where  B.诊疗项目ID=C.ID and B.病历文件ID=[1] and b.应用场合=[2]"
...
'功能：读取指定病人的指定提纲在病历填写的信息，例如：主诉，诊断等
    ...
    If Not mobjEmrInterface Is Nothing Then
        strText = mobjEmrInterface.GetOrderInspectInfo(inttype, lng病人ID, lng就诊ID, strCondition)
        If err.Number <> 0 Then
...
Private Sub Form_Load()
    ...
    End If
    If mlng申请序号 = 0 Then
        strSQL = "Select A.文件ID,A.文件名, A.类别,0 as 医嘱ID,B.名称 From 自定义申请单文件 A,病历文件列表 B Where A.文件ID=B.ID And B.ID = [1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlngFileID)
    Else
        strSQL = "Select a.文件ID,a.文件名, a.类别,A.医嘱ID,B.名称" & vbNewLine & _
                "From 医嘱申请单文件 A,病历文件列表 B " & vbNewLine & _
                "Where A.文件ID=B.ID And a.医嘱id In (Select ID From 病人医嘱记录 Where 申请序号 = [1] And 相关id Is Null)"
...
Private Function SetXML(ByVal vTag As Object) As Boolean
    ...
    ElseIf UCase(vTag.tagName) = "INPUT" And (vTag.Type = "radio" Or vTag.Type = "checkbox") Then
        '单选项和多选项选择处理
        err.Clear
        mobjXML.selectSingleNode("root/indt/" & vTag.ID).Text = IIF(vTag.Checked, 1, 0)
        If err.Number <> 0 Then
            '创建节点
            strNodeName = "root/indt"
            Set objNodes = mobjXML.selectSingleNode(strNodeName)
...
Private Function SaveData() As Boolean
    ...
    If rs项目!类别 = "C" Then
        ...
        lng序号 = lng序号 + 1
        str摘要 = ""
        str摘要 = gclsInsure.GetItemInfo(mint险类, mlng病人ID, 0, "", 0, "", CStr(mrsAdvice!诊疗项目ID))
        ...
        lng序号 = lng序号 + 1
        str摘要 = ""
        str摘要 = gclsInsure.GetItemInfo(mint险类, mlng病人ID, 0, "", 0, "", CStr(mrsAdvice!采集方式ID))
    ...
    If mintType = 0 Then
        ...
        If Not objNode Is Nothing Then
            Set picSet = DrawBarCode128Auto(picTmp, lng相关ID, 0, 2, True)
