...
Public gbln开单后立即结算 As Boolean '74231,冉俊明,2014-6-24,项目开单后立即收费或记帐审核
...
'功能：读取当前登录人员或指定人员的人员性质
    ...
    If Not rsTmp.EOF Then Get专业技术职务 = "" & rsTmp!专业技术职务
    Exit Function
errH:
...
'     lng医嘱处理范围=-1所有病人包括婴儿，0病人，1婴儿
    ...
    If lng医嘱处理范围 = -1 Then
        str范围所有 = " Select 病人id, 主页id, 姓名, 住院号, 床号, 担保额, 剩余款,  适用病人, 险类, 住院医师, 费别, 护理等级, 科室,科室id," & _
            " 入院日期, 出院日期, 病人类型, 性别, 审核标志,婴儿科室ID,婴儿病区ID,Null as 婴儿姓名,Null as 婴儿序号,病人状态 From Pati Union All "
    End If
    ...
    If lng医嘱处理范围 = 1 Or lng医嘱处理范围 = -1 Then
        strOther = "select a.病人id,a.主页id,a.姓名,a.住院号,a.床号,a.担保额,a.剩余款, a.适用病人,a.险类,a.住院医师,a.费别,a.护理等级,a.科室,a.科室id," & _
            " a.入院日期, a.出院日期, a.病人类型, a.性别, a.审核标志,a.婴儿科室ID,a.婴儿病区ID,b.婴儿姓名,B.序号 AS 婴儿序号,病人状态 from Pati A,病人新生儿记录 B" & _
            " where A.病人id=b.病人id and A.主页ID=b.主页id "
    Else
        strOther = "Select 病人id, 主页id, 姓名, 住院号, 床号, 担保额, 剩余款,  适用病人, 险类, 住院医师, 费别, 护理等级, 科室, 科室id," & _
            " 入院日期, 出院日期, 病人类型, 性别, 审核标志,婴儿科室ID,婴儿病区ID,Null as 婴儿姓名,Null as 婴儿序号,病人状态 From Pati "
    End If
    ...
    strSQL = "Select * from (" & _
        ...
        " c.id as 科室id,B.入院日期,B.出院日期,B.病人类型,A.性别,b.审核标志,B.婴儿科室ID,B.婴儿病区ID,Decode(B.状态,3,1,Decode(B.出院日期,Null,0,2)) as 病人状态 " & _
...
'返回：真-处理成功
    ...
    '住院自动发料
    gbln住院自动发料 = Val(zlDatabase.GetPara(63, glngSys)) <> 0
    ...
    '门诊收费与发药分离
    gbln分离发药 = zlDatabase.GetPara(15, glngSys) = "1"
    ...
    gbln开单后立即结算 = Val(zlDatabase.GetPara(232, glngSys)) <> 0
...
'返回：部门名称
    ...
    If rs部门 Is Nothing Then
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlPublic", lngID)
    Else
        Set rsTmp = rs部门
...
Public Function Get缺省用法ID(ByVal int类型 As Integer, ByVal int来源 As Integer, Optional ByVal strFilter As String = "") As Long
'功能：返回缺省的给药途径或中药煎法
...
'      int来源=1-门诊,2-住院
    ...
    strSQL = "Select ID From 诊疗项目目录" & _
        ...
        " Order by 编码"
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", CStr(int类型), int来源)
    If Not rsTmp.EOF Then
        Get缺省用法ID = rsTmp!ID
    End If
    Exit Function
...
'参数：int来源=1-门诊,2-住院
    ...
    strSQL = "Select Count(A.用法ID) as 总数,Max(Decode(A.用法ID,[2],1,0)) as 指定" & _
        ...
        " Where A.用法ID=B.ID And B.服务对象 IN([3],3) And A.项目ID=[1] And A.性质>0" & _
        " And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null)"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", lng项目ID, lng用法ID, int来源)
...
'      lng成套缺省执行科室-缺省执行科室ID
    ...
    If str类别 = "4" Then
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", int范围, lng病人科室ID, lng项目ID)
        If Not rsTmp.EOF Then
...
'      int调用场合=1-门诊,2-住院
    ...
    If InStr(",5,6,7,", str类别) > 0 Then
        ...
        If str类别 = "5" Then
            str药房 = "西药房"
            lng药房 = Val(zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "缺省西药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达)))
            str药房IDs = zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "可用西药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达))
        ElseIf str类别 = "6" Then
            str药房 = "成药房"
            lng药房 = Val(zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "缺省成药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达)))
            str药房IDs = zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "可用成药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达))
        ElseIf str类别 = "7" Then
            str药房 = "中药房"
            lng药房 = Val(zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "缺省中药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达)))
            str药房IDs = zlDatabase.GetPara(IIF(int调用场合 = 2, "住院", "门诊") & "可用中药房", glngSys, IIF(int调用场合 = 2, p住院医嘱下达, p门诊医嘱下达))
        End If
...
'功能：检查指定的执行科室是否有效
'参数：blnIsAllDept=true代表成套方案或临床路径判断时，允许成套中设置的科室可用。
...
Public Function Get可用发料部门IDs(ByVal lng材料ID As Long, ByVal lng科室id As Long, Optional ByVal int范围 As Integer = 2) As String
...
'      int范围=1-门诊,2-住院(缺省)
    ...
    strSQL = _
        ...
        " From 收费执行科室 A,部门性质说明 B,部门表 C" & _
        " Where A.执行科室ID+0=B.部门ID And B.工作性质='发料部门'" & _
        " And B.服务对象 IN([1],3) And B.部门ID=C.ID And A.收费细目ID=[3]" & _
        " And (C.站点='" & gstrNodeNo & "' Or C.站点 is Null)" & _
        " And (C.撤档时间=TO_DATE('3000-01-01','YYYY-MM-DD') Or C.撤档时间 is NULL)" & _
        IIF(int范围 <> 3, " And (A.病人来源 is NULL Or A.病人来源=[1])", "") & _
        IIF(lng科室id <> 0, " And (A.开单科室ID is NULL Or A.开单科室ID=[2])", "")
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", int范围, lng科室id, lng材料ID)
    Do While Not rsTmp.EOF
...
'说明：对非药医嘱,当前的执行科室可能是强行选择出来的,需要显示在选择框中;另选择框中增加一个其它供选择
    ...
    ElseIf InStr(",5,6,7,", str类别) > 0 Then
        ...
        If str类别 = "5" Then
            str药房 = "西药房"
            str药房IDs = zlDatabase.GetPara(IIF(int范围 = 2, "住院", "门诊") & "可用西药房", glngSys, IIF(int范围 = 2, p住院医嘱下达, p门诊医嘱下达))
        ElseIf str类别 = "6" Then
            str药房 = "成药房"
            str药房IDs = zlDatabase.GetPara(IIF(int范围 = 2, "住院", "门诊") & "可用成药房", glngSys, IIF(int范围 = 2, p住院医嘱下达, p门诊医嘱下达))
        ElseIf str类别 = "7" Then
            str药房 = "中药房"
            str药房IDs = zlDatabase.GetPara(IIF(int范围 = 2, "住院", "门诊") & "可用中药房", glngSys, IIF(int范围 = 2, p住院医嘱下达, p门诊医嘱下达))
        End If
        ...
        '是否启用输液配置中心
        If bln屏蔽输液中心 And Val(zlDatabase.GetPara("特殊性质药品允许发送到配制中心", glngSys, p输液配置中心, "0")) = 0 Then
...
'说明：未处理NULL值
    ...
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", lngID)
    If Not rsTmp.EOF Then
        If strField = "" Then
...
'功能：获取组合项目中的项目数
    ...
    strSQL = "Select Count(*) as NUM" & _
        ...
        " And (B.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or B.撤档时间 is NULL) And (B.服务对象 IN([2],3) or b.服务对象 is null) " & _
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", lng组合ID, int来源)
...
'      blnNone加班加价=为真时,"gbln加班加价"无效
    ...
    If "4" = Sys.RowValue("收费项目目录", lng药品ID, "类别") Then
        If 1 = Val(zlDatabase.GetPara(156, glngSys)) Then
            strSQL = strSQL & " Order by 效期,Nvl(批次,0)"
        Else
            strSQL = strSQL & " Order by Nvl(批次,0)"
        End If
    Else
        strSQL = strSQL & " Order by " & IIF(gbytMediOutMode = 1, "效期,", "") & IIF(gbytMediOutMode = 2, "上次批号,", "") & "Nvl(批次,0)"
    End If
        
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "CalcDrugPrice", lng药房ID, lng药品ID)
...
Public Function CalcPrice(ByVal lng项目ID As Long, Optional ByVal str费别 As String, _
    ...
    Optional ByVal lng执行科室ID As Long, Optional ByVal lng卫材医嘱ID As Long) As Double
...
'      lng卫材医嘱ID=如果传入该参数，表示非跟踪在用的时价卫材医嘱的价格从医嘱计价中读取
    ...
    If str费别 = "" Then
        strSQL = _
            ...
            " Where A.ID=B.收费细目ID And A.ID=[1]" & _
            " And ((Sysdate Between B.执行日期 and B.终止日期) or (Sysdate>=B.执行日期 And B.终止日期 is NULL))"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISWork", lng项目ID, dbl单价)
...
'       记录来源:1-病历；2-入院登记；3-首页整理(门诊医生站,诊断摘要);
    ...
    strSQL = "Select a.疾病id, a.诊断id, a.诊断描述, a.诊断次序, Nvl(b.编码, c.编码) As 编码, Nvl(b.名称, c.名称) 名称" & vbNewLine & _
             ...
             "Where a.病人id = [1] And a.主页id = [2] And 取消时间 Is Null And 记录来源 IN (1, 3) And NVL(编码序号,1) = 1 And Instr(',' ||[3]|| ',', ',' || 诊断类型 || ',') > 0 And a.疾病id = b.Id(+) And" & vbNewLine & _
...
Public Function Get疾病编码(ByVal lng疾病ID As Long) As String
'功能：获取疾病ID返回疾病编码
'参数：
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlPublic", lng疾病ID)
    If rsTmp.RecordCount > 0 Then Get疾病编码 = rsTmp!编码
...
'                一并给药开始行或独立药品行才传入,一并给药中间行传入空
    ...
    If rsRXKey.EOF Then
        strNextID = GettUniqueID(str医嘱ID, "")
        ...
        rsRXKey.Update
    ElseIf str医嘱ID <> "" Then
        strNextID = GettUniqueID(str医嘱ID, rsRXKey!医嘱ID)
        intNextCount = UBound(Split(strNextID, ",")) + 1
...
Private Function GettUniqueID(ByVal strIDs As String, ByVal strExists As String) As String
'功能：返回"传入ID串"不在"已有ID串"中不重复的ID串
    Dim arrIDs As Variant, strResult As String, i As Long
    
    arrIDs = Split(strIDs, ",")
    For i = 0 To UBound(arrIDs)
        If InStr("," & strExists & ",", "," & arrIDs(i) & ",") = 0 _
            And InStr(strResult & ",", "," & arrIDs(i) & ",") = 0 Then
            strResult = strResult & "," & arrIDs(i)
        End If
    Next
    
    GettUniqueID = Mid(strResult, 2)
End Function
...
Public Function GetLastSendMediCineID(ByVal lngAdviceID As Long, ByVal DateLast As Date) As Long
'功能：根据药品医嘱ID获取最近一次发送的费用对应的收费项目ID(药品规格ID)
    Dim rsTmp As ADODB.Recordset, strSQL As String
    
    strSQL = "Select B.收费细目ID" & _
            " From 病人医嘱发送 A,住院费用记录 B" & _
...
'参数：strRollNotify=病人ID:主页ID,...
    ...
    strSQL = _
        " Select /*+ Rule*/ A.姓名,A.医嘱内容,A.ID,A.诊疗类别,A.上次执行时间,A.收费细目ID" & _
        " From 病人医嘱记录 A,诊疗项目目录 E,(" & strSQLPati & ") F" & _
        " Where A.诊疗项目ID=E.ID And A.病人ID = F.病人ID And A.主页ID = F.主页ID" & _
    ...
    Do While Not rsTmp.EOF
        ...
        If InStr(",5,6,", rsTmp!诊疗类别) > 0 And strUnRoll <> "" Then
            ...
            Else
                lng药品ID = GetLastSendMediCineID(Val(rsTmp!ID), CDate(rsTmp!上次执行时间))
            End If
            If lng药品ID <> 0 Then
...
Public Function CheckPathItemIsMust(ByVal byt执行方式 As Byte, ByVal int天数 As Integer, ByVal lng路径记录Id As Long, _
                                    ByVal lng阶段Id As Long, ByVal lng项目ID As Long) As Boolean
'功能:检查路径项目是否是必须生成的项目
            ...
            For i = 1 To rsDays.RecordCount
                If i = 1 Then
                    If rsMoneyDay Is Nothing Then
                        If byt来源 = 2 Then '住院病人从入院时间开始查找
                            strSQL = "select To_Char(a.入院日期,'yyyy-mm-dd') as 日期 from 病案主页 a where a.病人id=[1] and a.主页id=[2]"
                            Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "AdviceMoneyMake", lng病人ID, lng主页ID)
                            If Not rsTmp.EOF Then str最小日期 = rsTmp!日期
                        End If
                        If str最小日期 = "" Then
...
Public Function zlSelectDept(ByVal frmMain As Form, ByVal lngModule As Long, ByVal cboDept As ComboBox, ByVal rsDept As ADODB.Recordset, _
    ...
    If lngDeptID < 0 Then lngDeptID = 0
GoOver:
    If zlControl.CboLocate(cboDept, lngDeptID, True) = False Then
...
'      lng病人ID,lng主页ID=单个病人时才传入
    ...
    If InStr(GetInsidePrivs(p住院医嘱发送), "全院医嘱校对") = 0 Then
        strSQL = strSQL & "  And A.开嘱科室ID In (Select Column_Value From Table(f_Num2list([3]))) "
    End If
...
Public Sub Load输液滴速(ByRef cbo滴速 As ComboBox, ByRef lbl滴速单位 As Label, ByVal blnTurn As Boolean)
'功能：加载滴速下拉列表
'参数：blnTurn=是否转换输液单位
    Dim i As Long
...
Public Function Check过敏试验(frmParent As Object, ByVal txt医嘱内容 As Object, ByVal lng病人ID As Long, ByVal lng药名ID As Long, ByVal str名称 As String, ByVal bln自动皮试 As Boolean, Optional ByRef lng皮试ID As Long, _
    Optional ByVal lng主页ID As Long, Optional ByVal lng药品ID As Long, Optional ByRef bln连续用药 As Boolean) As String
'功能：检查西成药，中成药的过敏试验，门诊和住院医嘱下达公用
'参数：frmParent当前调用的窗体
'      txt医嘱内容 控件，医嘱编辑界面的文本输入框控件对象
'      lng药名ID=药品诊疗项目ID
'      str名称=药品名称,用于提示
'      bln连续用药 bln判断连续用药 =true 表明要判断并将结果以 bln连续用药 返回
'返回：为空表示通过
'      lng皮试ID=要自动添加的皮试项目ID
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, frmParent.Caption, lng病人ID, lng药名ID, gint过敏登记有效天数)
    If Not rsTmp.EOF Then
    ...
    Else
        ...
        strSQL = "Select A.用法ID,B.名称" & _
            ...
            " And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null)"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, frmParent.Caption, lng药名ID)
        ...
        If Not rsTmp.EOF Then
            ...
            Set rsOther = zlDatabase.OpenSQLRecord(strSQL, frmParent.Caption, lng病人ID, lng主页ID, IIF(0 <> lng药品ID, lng药品ID, lng药名ID), gint过敏登记有效天数)
            bln连续用药 = Not rsOther.EOF
            If Not bln连续用药 Then
                If bln自动皮试 Then
...
Public Function GetNoneSendID(ByVal lng病人ID As Long, ByVal str标识 As String, ByVal bln自动皮试 As Boolean, ByVal bytType As Byte, Optional ByVal bln发送输液药品 As Boolean) As String
...
'      bln发送输液药品 =输液药品发送页面调用
    Dim rsTmp As New ADODB.Recordset
    Dim strSQL As String, i As Long
    Dim str用法IDs As String, strPatiFilter As String
    ...
    '读取皮试过敏(阳性(+))或无皮试结果(未发送或已发送)的医嘱皮试项目ID
    '不包括已作废的皮试医嘱或未发送的标记为免试的皮试医嘱
    strSQL = "Select b.Id From 病人医嘱记录 A,(Select B.ID,A.病人ID,A.主页ID,A.挂号单,Max(a.开始执行时间) As 时间 From 病人医嘱记录 A,诊疗项目目录 B" & _
        " Where A.诊疗项目ID=B.ID And A.诊疗类别='E' And B.操作类型='1'" & _
        " And A.医嘱状态<>4" & strPatiFilter & " Group By b.Id,A.病人ID,A.主页ID,A.挂号单) b " & _
        " where a.诊疗项目id+0 = b.Id And a.开始执行时间+0 = b.时间 And (a.皮试结果 = '(+)' Or a.皮试结果 Is Null)  And a.医嘱状态 <> 4 " & strPatiFilterAnd
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "GetNoneSendID", lng病人ID, IIF(bytType = 1, str标识, Val(str标识)))
    Do While Not rsTmp.EOF
        str用法IDs = str用法IDs & "," & rsTmp!ID
        rsTmp.MoveNext
    Loop
    If str用法IDs = "" Then Exit Function
    str用法IDs = Mid(str用法IDs, 2)
    
    '读取这些皮试相关的药品医嘱
    strSQL = "Select A.相关ID From 病人医嘱记录 A,诊疗用法用量 B" & _
        " Where A.诊疗类别 IN('5','6') And A.医嘱状态 IN (1, 3)" & _
        " And A.诊疗项目ID=B.项目ID And B.性质=0 And Instr([3],','||B.用法ID||',')>0" & strPatiFilter

    '读取药品医嘱及给药途径的医嘱ID(包括一并给药)
    strSQL = "Select a.ID From 病人医嘱记录 a Where a.诊疗类别 IN('5','6','E') And Nvl(a.相关ID,ID) IN (" & strSQL & ")" & strPatiFilter
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "GetNoneSendID", lng病人ID, IIF(bytType = 1, str标识, Val(str标识)), "," & str用法IDs & ",")
    strSQL = ""
    Do While Not rsTmp.EOF
...
Public Function GetPrice(ByVal lng诊疗项目ID As Long, ByVal str部分方法组合 As String, ByVal lng执行类型 As Long, ByVal lng病人来源 As Long, ByVal lng执行科室ID As Long) As Double
'功能：获取检查项目的费用合计
...
'    病人来源：1-门诊，2住院
    ...
    strSQL = "Select Sum(b.收费数量 * d.现价) As 合计" & vbNewLine & _
            ...
            "              Where c.诊疗项目id = D.ID And d.计价性质 =0 and c.诊疗项目id = [1] And (a.标本部位 Is Null And [2] In (2, 3) And c.费用性质 = 1 Or" & vbNewLine & _
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "GetPrice", lng诊疗项目ID, lng执行类型, lng执行科室ID, lng病人来源)
...
Public Sub SetControlFontSize(ByRef frmMe As Object, ByVal bytSize As Byte, Optional ByVal strOther As String)
'功能：设置窗体及所有控件的字体大小
...
'功能：输血医嘱可否编辑 int审核状态 取值有：0，1，2，4，5；bln急 是否是紧急医嘱。（只检查备血医嘱，这里主要是以前老备血流程的检查）
    ...
    If strMsg <> "" Then
        If blnMsg = True Then MsgBox strMsg, vbInformation, "输血申请"
                ...
                For i = 0 To UBound(objAppPages)
                    If lng医嘱ID = 0 Or objAppPages(i).blnIsModify = True Then
                        ...
                        blnDo = GetPacsAdviceSQLData(objAppPages(i), 0, lng申请序号, lng病人ID, lng主页ID, lng婴儿序号, "", lng科室id, objVBA, objScript, rsDefine, _
                            strRISDel, strRISAdd, arrSQL, lng假医嘱ID, strTabAdvice, strTxtAdvice, lng前提ID)
...
Private Function GetPacsAdviceSQLData(ByRef adviceInf As clsApplicationData, ByVal intType As Integer, ByVal lng申请序号 As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, _
    ...
    ByRef strRISDel As String, ByRef strRISAdd As String, ByRef arySql() As String, ByRef lng假医嘱ID As Long, ByRef strTabAdvice As String, ByRef strTxtAdvice As String, ByVal lng前提ID As Long) As Boolean
'------------------------------------------------
...
'------------------------------------------------
    ...
    If intType = 0 Then
        '组织主医嘱插入语句
        lng医嘱序号 = GetMaxAdviceNO(lng病人ID, lng主页ID, lng婴儿序号) + 1    '病人医嘱记录.序号，递增
        
        int急 = IIF(adviceInf.blnIsAdditionalRec = True, 2, IIF(adviceInf.blnIsPriority, 1, 0))
    Else
        '组织主医嘱插入语句
        lng医嘱序号 = GetMaxAdviceNO(lng病人ID, 0, 0, str挂号单) + 1 '病人医嘱记录.序号，递增
    ...
    '诊断关联信息
    If adviceInf.strDiagnoseId <> "" Then
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = "Zl_病人诊断医嘱_Insert(<FAKEID>" & lng医嘱ID & "</FAKEID>,'" & adviceInf.strDiagnoseId & "')"
    End If
...
'功能：查看检查申请单
    ...
    On Error GoTo errH
    If lngNo = 0 Then
        strSQL = "Select a.申请序号 From 病人医嘱记录 A where a.Id =[1]"
    ...
    ShowApply检查 = frmPacsApplication.ShowApplicationForm(Val(rsTmp!病人ID & ""), Val(rsTmp!应用场合 & ""), Val(rsTmp!就诊ID & ""), Val(rsTmp!主页ID & ""), lngNo, objAppPages(), , False)
    Exit Function
errH:
    If ErrCenter() = 1 Then
        Resume
...
Public Sub DefCommandPlugInPopup(ByVal objBar As Object, ByRef rsBar As ADODB.Recordset)
'功能：在医嘱卡右键弹出菜单
    Dim i As Long
...
'功能：获取申请单相关参数值
    ...
    Dim str必用 As String
    Dim varTmp As Variant
    str启用 = zlDatabase.GetPara(238, glngSys, , "11|11|11")
    str必用 = zlDatabase.GetPara(260, glngSys, , "11")
    varTmp = Split(str启用, "|")
    gstrOutUseApp = Mid(varTmp(0), 1, 1) & Mid(varTmp(1), 1, 1) & Mid(varTmp(2), 1, 1)
    gstrInUseApp = Mid(varTmp(0), 2, 1) & Mid(varTmp(1), 2, 1) & Mid(varTmp(2), 2, 1)
...
Public Function CheckPathInItem(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal str诊疗项目IDs As String, ByRef str分类 As String, ByVal lng阶段Id As Long, ByVal bln中药配方 As Boolean, ByVal byt期效 As Byte) As Long
...
'                "Where 诊疗项目ids = [3]"
        ...
        If Not bln中药配方 Then
            strSQL = "Select 分类, 路径项目id,期效,诊疗项目ids" & vbNewLine & _
                    "From (Select 分类, 路径项目id, 组id,期效,f_List2str(Cast(Collect(To_Char(诊疗项目id)) As t_Strlist)) As 诊疗项目ids" & vbNewLine & _
                    "       From (Select c.路径项目id, b.分类, d.诊疗项目id, Nvl(d.相关id, d.Id) 组id,d.期效,d.序号" & vbNewLine & _
                    "              From 病人临床路径 A, 临床路径项目 B, 临床路径医嘱 C, 路径医嘱内容 D" & vbNewLine & _
...
'返回：str分类=当前天数最后一个路径项目所属的分类
    ...
    strSQL = "Select a.路径记录id, a.当前阶段id, a.当前天数, b.日期, b.分类" & vbNewLine & _
            ...
            "       From 病人临床路径 A, 病人路径执行 B" & vbNewLine & _
            "       Where a.病人id = [1] And a.主页id = [2] And a.Id = b.路径记录id And b.阶段id = a.当前阶段id And b.天数 = a.当前天数 And a.状态 = 1 " & vbNewLine & _
...
Public Function GetPathOutItemID(ByVal lng路径记录Id As Long, ByVal DatAddDate As Date) As Long
    ...
    If Not rsTmp.EOF Then
        ...
        Do While Not rsTmp.EOF
            ...
            If blnDo Then
                ...
                rsPrice!开单科室 = CStr(GetItemField("部门表", lng开单科室ID, "名称"))
...
'功能：诊间支付
    ...
    If Not gobjSquareCard Is Nothing Then
        ...
        If Not IsNull(rsTmp!医嘱IDs) Then
            ...
            Call gobjSquareCard.zlSquareAffirm(frmMain, p门诊医嘱下达, GetInsidePrivs(p门诊医嘱下达), lng病人ID, 0, False, 1, , str结算医嘱IDs, "")
        Else
...
'         lngActiveID--医嘱ID，不为0则计算的量剔除改医嘱
    ...
    Dim strSQL As String, strNoAuditBlood As String
    ...
    ElseIf int场合 = 1 Then
        strSQL = _
            ...
            "        Where a.Id = b.诊疗项目id "
    End If
    If Not gobjPublicBlood Is Nothing And bln计算24H量 Then strNoAuditBlood = gobjPublicBlood.zlGetBloodAuditState(2)
    If strNoAuditBlood <> "" And bln计算24H量 Then
        strSQL = strSQL & " And instr([4],'|' || a.id || '|') = 0) "
    Else
        strSQL = strSQL & ")"
    End If
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取输血量", lng病人ID, lng就诊ID, intBaby, strNoAuditBlood)
...
Public Function GetBloodVerifyState(ByVal int病人来源 As Integer, ByVal lng病人ID As Long, ByVal lng就诊ID As Long, ByVal str申请时间 As String, ByVal dblTotalByML As Double, _
    Optional ByVal int紧急 As Integer, Optional ByVal int备血 As Integer, Optional ByVal intBaby As Integer, Optional ByVal lngActiveID As Long, _
    Optional ByVal strBloodInfo As String = "") As String
...
'   lngActiveID：医嘱ID。注意：不为0时，本次计算的24小时总量将不包含该医嘱对应的申请量。主要是针对申请单修改时剔除之前的总量
    ...
    Dim str审核状态 As String
    Dim strNoAuditBlood As String, strBlood() As String
    Dim intLoop As Integer
    ...
    If str审核状态 = "1" And gbln血库系统 = True Then '审核状态已经决定了是备血申请
        '通过血库血液品种判断是否需要审核
        If Not gobjPublicBlood Is Nothing Then strNoAuditBlood = gobjPublicBlood.zlGetBloodAuditState(1)
        If strBloodInfo <> "" And strNoAuditBlood <> "" Then
            strBlood = Split(strBloodInfo, ";")
            For intLoop = 0 To UBound(strBlood)
                If InStr(1, strNoAuditBlood, "|" & Split(strBlood(intLoop), ",")(0) & "|") = 0 Then Exit For
                If intLoop = UBound(strBlood) Then
                    str审核状态 = 4
