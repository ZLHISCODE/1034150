...
'Pass
Private mobjPassMap As Object         'PASS 映射
...
Private Enum COL医嘱清单
    ...
    COL_查阅状态 = COL_诊疗项目ID + 7
    COL_审核状态 = COL_诊疗项目ID + 8
    COL_申请序号 = COL_诊疗项目ID + 9
    COL_高危药品 = COL_诊疗项目ID + 10
    COL_标本部位 = COL_诊疗项目ID + 11
    COL_收费细目ID = COL_诊疗项目ID + 12   'Pass
    COL_检查报告ID = COL_诊疗项目ID + 13
    COL_开嘱科室ID = COL_诊疗项目ID + 14
    COL_用药目的 = COL_诊疗项目ID + 15
    COL_RIS预约ID = COL_诊疗项目ID + 16
    COL_处方审查状态 = COL_诊疗项目ID + 17
    COL_处方审查结果 = COL_诊疗项目ID + 18
    COL_RIS报告ID = COL_诊疗项目ID + 19
    COL_LIS报告ID = COL_诊疗项目ID + 20
    COL_检查方法 = COL_诊疗项目ID + 21  '输血医嘱区分是备血还是用血
    COL_RIS预约状态 = COL_诊疗项目ID + 22
...
Public Sub zlDefCommandBars(ByVal frmParent As Object, ByVal cbsMain As Object, ByVal int场合 As Integer, _
    ...
    If Not cbsMain Is Nothing Then
        ...
        Call zlPASSMap
        If mblnPass Then
            If InStr(GetInsidePrivs(p住院医嘱下达), "合理用药监测") > 0 Then
...
'功能：设置工具栏按钮，对应于医嘱菜单下面的工具栏的按钮，先将其删掉再添加
    ...
    strTmp = "," & conMenu_Edit_NewItem & "," & conMenu_Edit_Apply & "," & conMenu_Edit_ApplyModi & "," & conMenu_Edit_ApplyView & "," & conMenu_Edit_ApplyDel & "," & _
        conMenu_Edit_Blankoff & "," & conMenu_Edit_TraReaction & "," & conMenu_Edit_SendBilling & "," & conMenu_Edit_Send & "," & conMenu_Edit_Untread & "," & _
        conMenu_Edit_Compend & "," & (conMenu_Edit_Compend * 10# + 2) & "," & (conMenu_Edit_Compend * 10# + 3) & "," & conMenu_Edit_MarkMap & "," & conMenu_Edit_MarkKeyMap & "," & conMenu_Manage_ReportLisView & "," & _
        conMenu_Edit_MediAudit & "," & conMenu_Tool_SignNew & "," & conMenu_Edit_Audit & "," & conMenu_Edit_Price & "," & _
        conMenu_Report_ClinicBill & "," & conMenu_Edit_PacsApply & "," & conMenu_Edit_BloodApply & "," & conMenu_Edit_AdvicePay & ","
...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Case conMenu_Edit_Compend * 10# + 5 '自动标记查阅状态
        ...
        Call zlDatabase.SetPara("自动标记报告查阅状态", IIF(mblnAutoRead, 1, 0), glngSys, p门诊医嘱下达, InStr(GetInsidePrivs(p门诊医嘱下达), ";医嘱选项设置;") > 0)
    ...
    Case conMenu_Tool_Option '医嘱选项
        frmOutAdviceSetup.mMainPrivs = mMainPrivs
        frmOutAdviceSetup.mbln医技站 = mint场合 = 2
        frmOutAdviceSetup.Show 1, mfrmParent
        If gblnOK Then
            Call GetLocalSetting
            Call LoadAdvice
            mSendControl.Category = ""
        End If
...
Public Sub zlUpdateCommandBars(ByVal Control As CommandBarControl)
    ...
    If Between(Control.ID, conMenu_Edit_NewItem, conMenu_Edit_NewItem + 998) _
        ...
        Control.Enabled = mlng病人ID <> 0 _
            ...
            Or Control.ID = conMenu_Edit_MarkMap Or Control.ID = conMenu_Edit_MarkKeyMap Or Control.ID = conMenu_Edit_Compend _
    ...
    Select Case Control.ID
    Case conMenu_Edit_NewItem, conMenu_Edit_LISApply, conMenu_Edit_PacsApply, conMenu_Edit_ApplyCustom * 100# To conMenu_Edit_ApplyCustom * 101#  '新开医嘱
    ...
    Case conMenu_Edit_Modify, conMenu_Edit_Delete '修改医嘱,删除医嘱
        ...
        If Control.ID = conMenu_Edit_Modify Then
            Control.Enabled = Control.Enabled And (mvRegDate <> CDate("3000-01-01"))
        End If
    ...
    Case conMenu_Edit_Compend * 10# + 4 '我已经查阅该报告
        ...
        Control.Enabled = blnAdvice And Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_报告ID)) <> 0 'And Not Control.Checked
    Case conMenu_Edit_Compend * 10# + 5 '自动标记查阅状态
    ...
    '查看检验结果
    '------------------------------------------------------------------------------
    If Control.ID = conMenu_Manage_ReportLisView Then
        Control.Enabled = mlng病人ID <> 0
    End If
    '输血执行单打印
...
'功能：根据权限设置菜单和工具栏的可见状态
    ...
    Case conMenu_Edit_TraReaction
        If gbln血库系统 Then
            '输血反应登记
            If InStr(GetInsidePrivs(9005, , 2200), "输血反应登记") = 0 Then blnVisible = False
        End If
...
'      objMip 消息对象
    ...
    If lng病人ID <> 0 And str挂号单Old <> mstr挂号单 Then
        strSql = "Select A.ID,A.执行部门ID,A.执行状态,A.登记时间,C.险类,Nvl(Nvl(A.续诊科室ID,Decode(A.转诊状态,1,A.转诊科室ID,NULL)),A.执行部门ID) as 病人科室ID,a.门诊号,c.姓名" & _
    ...
    If (lngOld界面科室ID <> mlng界面科室ID Or lng病人IDOld <> mlng病人ID) And mlng前提ID <> 0 Then
        mstr前提IDs = Get医技科室医嘱IDs(mlng病人ID, mstr挂号单, mlng界面科室ID, False, mlng前提ID)
    ...
    If mlngPlugInID <> 0 And lng病人IDOld <> mlng病人ID Then
...
Private Sub cbsSub_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        Case ID_婴儿 * 100# '所有医嘱
            ...
            mvarCond.婴儿 = -1
            Call zlDatabase.SetPara("病人婴儿过滤", mvarCond.婴儿, glngSys, p门诊医嘱下达, InStr(GetInsidePrivs(p门诊医嘱下达), ";医嘱选项设置;") > 0)
            ...
            If .TextMatrix(.Row, COL_诊疗类别) = "K" Then
                ...
                If Val(Mid(gstrOutUseApp, 3, 1)) = 1 Then
                    If gbln血库系统 = True Then
                        Call frmBloodApplyNew.ShowMe(Me, mlng病人ID, 0, 1, 2, lng医嘱ID, mlng挂号科室ID, , mlng挂号科室ID, , , mrsDefine, mclsMipModule, 1, mstr挂号单, mlng前提ID, , , , , IIF(bln用血 = True, 1, 0))
                    Else
                        Call frmBloodApply.ShowMe(Me, mlng病人ID, 0, 1, 2, lng医嘱ID, mlng挂号科室ID, , mlng挂号科室ID, , , mrsDefine, mclsMipModule, 1, mstr挂号单, mlng前提ID)
                    End If
                End If
    ...
    With vsAdvice
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            If mobjReport.ReportPrintSet(gcnOracle, glngSys, strParameter, mfrmParent) Then
                ...
                Call mobjReport.ReportOpen(gcnOracle, glngSys, strParameter, mfrmParent, "NO=" & strNO, "性质=" & lng记录性质, 2)
                mstrBillPrint = ""
            End If
        End If
    End With
...
'功能：新增医嘱
    ...
    Call frmOutAdviceEdit.ShowMe(mfrmParent, mint场合, mMainPrivs, mlng病人ID, mstr挂号单, mlng前提ID, , , mblnModalNew, mlng界面科室ID, mstr前提IDs, mclsMipModule, mlng危急值ID)
End Sub
...
'删除：当前医嘱作废(一组医嘱作废)
    ...
    With vsAdvice
        ...
        If .TextMatrix(.Row, COL_诊疗类别) = "K" And gbln血库系统 Then
        ...
        'RIS回退，回退失败则退出
        If InStr(",D,F,", .TextMatrix(.Row, COL_诊疗类别)) > 0 Or .TextMatrix(.Row, COL_诊疗类别) = "E" And InStr(",0,5,", Val(.TextMatrix(.Row, COL_操作类型))) > 0 Then   '检查、手术、治疗
...
errH:
    ...
    If blnTran Then
        ...
        If HaveRIS(True) And lngRIS医嘱ID <> 0 Then
            ...
            Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lngRIS医嘱ID)
            If Not rsTmp.EOF Then
...
'功能：修改当前医嘱
    ...
    With vsAdvice
        ...
        Call frmOutAdviceEdit.ShowMe(mfrmParent, mint场合, mMainPrivs, mlng病人ID, mstr挂号单, mlng前提ID, _
            Val(.TextMatrix(.Row, COL_婴儿ID)), lng医嘱ID, , mlng界面科室ID, mstr前提IDs, mclsMipModule)
    End With
End Sub
...
'功能：发送病人医嘱(可以设置计价项目)
    ...
    If mfrmSend.ShowMe(mfrmParent, mMainPrivs, mlng病人ID, mstr挂号单, mstr前提IDs, False, mlng界面科室ID, mint场合, mclsMipModule) Then
        Call LoadAdvice
        Call ShowTotalMoney
    End If
End Sub
...
'功能：查阅、打印、预览报告
    ...
    Case 0
        ...
        Exit Sub
    Case 2
        If InStr(GetInsidePrivs(p门诊医嘱下达), "查阅未完成报告") > 0 Then
...
Private Sub Form_Load()
    ...
    mblnAutoReadEnabled = Not ((intType = 3 Or intType = 15) And InStr(GetInsidePrivs(p门诊医嘱下达), "医嘱选项设置") = 0)
...
'功能：读取本地注册表设置
    mbln自动皮试 = Val(zlDatabase.GetPara("自动处理皮试", glngSys, p门诊医嘱下达)) <> 0
...
'功能：读取医嘱过滤设置条件
    ...
    strPar = Val(zlDatabase.GetPara("医嘱状态过滤", glngSys, p门诊医嘱下达, "0"))
...
'功能：保存医嘱过滤设置条件
    ...
    Call zlDatabase.SetPara("科内医嘱过滤", IIF(mvarCond.科内, 1, 0), glngSys, p门诊医嘱下达, blnSetup)
    Call zlDatabase.SetPara("显示模式", mvarCond.显示模式, glngSys, p门诊医嘱下达, blnSetup)
    Call zlDatabase.SetPara("报告查看类型", mvarCond.报告, glngSys, p门诊医嘱下达, blnSetup)
    
    Call zlDatabase.SetPara("过滤条件自动隐藏", IIF(mblnHideFilter, 1, 0), glngSys, p门诊医嘱下达, blnSetup)
    Call zlDatabase.SetPara("医嘱状态过滤", mvarCond.过滤模式, glngSys, p门诊医嘱下达, blnSetup)
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If mblnAppend And Not tbcAppend.Selected Is Nothing Then
        Call zlDatabase.SetPara("医嘱子列表", tbcAppend.Selected.Tag, glngSys, p门诊医嘱下达, InStr(GetInsidePrivs(p门诊医嘱下达), ";医嘱选项设置;") > 0)
    End If
...
'功能：初始化发送清单格式
    ...
    strHead = "发送号;发送时间,1530,1;单据号,850,1;发送医嘱,1800,1;收费项目,1800,1;发送数次,850,1;计费状态,850,1;执行状态,850,1;执行科室,1000,1;执行人,800,1;执行时间,1530,1;执行说明,1800,1;发送人,800,1;记录性质"
    arrHead = Split(strHead, ";")
    With vsAppend
        .Redraw = flexRDNone
        .Clear
...
'功能：初始化医嘱清单格式
    ...
    strHead = "ID;相关ID;婴儿ID;医嘱状态;诊疗类别;操作类型;毒理分类;标志;" & _
              ",240,4;开始时间,1530,1;医嘱内容,3000,1;内容,4000,1;,375,1;总量,850,1;单量,850,1;天数,450,1;频率,1000,1;" & _
              "用法,1000,1;医生嘱托,1000,1;执行时间,1000,1;执行科室,1000,1;执行性质,850,1;" & _
              "开嘱医生,850,1;开嘱时间,1530,1;发送人,850,1;发送时间,1530,1;超量说明,1000,1;基本药物,850,1;诊疗项目ID;试管编码;" & _
              "前提ID;签名否;文件ID;报告项;报告ID;查阅状态;审核状态;申请序号;高危药品;标本部位;收费细目ID;检查报告ID;开嘱科室ID;用药目的;RIS预约ID;处方审查状态;处方审查结果;RIS报告ID;LIS报告ID;检查方法;RIS预约状态;危急值ID"
    arrHead = Split(strHead, ";")
    With vsAdvice
        .Clear
...
'功能：根据当前界面设置读取并显示医嘱清单
    ...
    If mvarCond.显示模式 = 0 And mvarCond.过滤模式 <> 3 Then
        strFormat = "MM-DD HH24:MI"
    Else
        strFormat = "YYYY-MM-DD HH24:MI"
    End If
    ...
    "Select /*+ RULE */ A.ID,A.相关ID," & _
             ...
             " A.审查结果,To_Char(A.开始执行时间,'" & strFormat & "') as 开始时间,A.医嘱内容,Null as 内容,A.皮试结果 as 皮试," & _
             ...
             " A.停嘱医生 as 发送人,A.停嘱时间 as 发送时间,a.超量说明,D.基本药物,A.诊疗项目ID,B.试管编码,A.前提ID,Decode(A.新开签名ID,NULL,0,1) as 签名否," & _
             " M.病历文件ID as 文件ID,Nvl(N.通用,0) as 报告项,Max(y.病历id) As 报告id, Decode(Max(NVL(y.查阅状态,0)),MiN(NVL(y.查阅状态,0)),Max(NVL(y.查阅状态,0)),2) As 查阅状态,A.审核状态,A.申请序号,d.高危药品,A.标本部位,A.收费细目ID,Max(y.检查报告id)||'' As 检查报告id," & _
             " A.开嘱科室id,A.用药目的,f.预约id As RIS预约ID,J.状态 as 处方审查状态,J.审查结果 as 处方审查结果,max(y.RISID) as RIS报告ID,max(y.报告ID) as LIS报告ID,Max(a.检查方法) as 检查方法,f.是否调整 as RIS预约状态,max(h.危急值id) as 危急值ID"
    strSql = strSql & _
             ...
             " And Nvl(A.医嘱期效,0)=1 And A.收费细目ID=D.药品ID(+) And A.收费细目ID=G.ID(+)  and a.id=h.医嘱ID(+)" & _
             ...
             " And A.诊疗项目ID=M.诊疗项目ID(+) And M.应用场合(+)=1 And M.病历文件ID=N.ID(+) And N.种类(+)=7 And a.Id=f.医嘱id(+)" & _
...
'功能：读取指定医嘱的计价,并根据当前的诊疗收费 关系进行更新
    ...
    With vsAppend
        ...
        End If
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Name, lng医嘱ID, lng相关ID, lng材料ID)
            ...
            For i = 1 To rsTmp.RecordCount
                ...
                If InStr(",5,6,7,", rs收费细目!类别) > 0 Then
                    If Nvl(rs收费细目!是否变价, 0) = 1 Then
                        ...
                        If InStr(",5,6,7,", rs诊疗项目!类别) > 0 Then
                            '药嘱药品计算门诊包装的门诊时价
                            .TextMatrix(i, COLPrice("单价")) = CalcDrugPrice(rs收费细目!ID, lng执行科室ID, Nvl(rsTmp!数量, 1))
    ...
    With vsAppend
        ...
        If InStr(",5,6,", vsAdvice.TextMatrix(lngRow, COL_诊疗类别)) > 0 Then
            ...
            strSql = _
                ...
                " B.发送人,B.发送号,B.记录序号 as 发送序号,A.序号 as 费用序号,C.诊疗项目ID,C.诊疗类别,B.完成时间,B.完成人,B.执行说明" & _
        ...
        strSql = "Select /*+ RULE */ A.发送序号,A.费用序号," & _
            ...
            " A.单位,A.发送数次 as 数量,C.名称 as 执行科室,A.执行状态,A.首次时间,A.末次时间,A.计费状态,A.发送人,A.发送号,A.完成时间,A.完成人,A.执行说明" & _
...
'功能：显示指定行的医嘱发送可以打印的诊疗单据在菜单上
    ...
    With vsAdvice
        strSql = "Select Distinct D.编号,D.名称,D.说明,B.NO" & _
            ...
            " Order by D.编号"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Name, Decode(Val(.TextMatrix(.Row, COL_相关ID)), 0, Val(.TextMatrix(.Row, COL_ID)), Val(.TextMatrix(.Row, COL_相关ID))))
        If mblnMoved Then
            strSql = Replace(strSql, "病人医嘱记录", "H病人医嘱记录")
            strSql = Replace(strSql, "病人医嘱发送", "H病人医嘱发送")
        End If
...
'参数：intType=0 新增，=1修改，=2查看，=4核对
    ...
    If gbln血库系统 = True Then
        blnApply = frmBloodApplyNew.ShowMe(Me, mlng病人ID, 0, 1, intType, lngUpdateAdvice, mlng挂号科室ID, , mlng挂号科室ID, , , mrsDefine, mclsMipModule, 1, mstr挂号单, mlng前提ID, , , , , IIF(bln用血 = True, 1, 0))
    Else
        blnApply = frmBloodApply.ShowMe(Me, mlng病人ID, 0, 1, intType, lngUpdateAdvice, mlng挂号科室ID, , mlng挂号科室ID, , , mrsDefine, mclsMipModule, 1, mstr挂号单, mlng前提ID)
    End If
...
'参数：lng申请序号=修改申请单时的申请序号
    ...
    Else
        ...
        If strResult <> "" Then
            ...
            lng序号 = GetMaxAdviceNO(mlng病人ID, 0, 0, mstr挂号单)
            '只产生临嘱
            For i = 0 To UBound(arrTmp)
                ...
                For Y = 8 To UBound(Split(arrTmp(i), "<Split A>"))
                    ...
                    arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & _
                        ...
                        "," & str检验执行性质 & "," & str紧急 & "," & str开始时间 & ",Null," & mlng挂号科室ID & "," & _
                        mlng挂号科室ID & ",'" & UserInfo.姓名 & "'," & strDate & ",'" & mstr挂号单 & "'," & ZVal(mlng前提ID) & "," & _
                ...
                arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & _
                    ...
                    "," & str采集执行性质 & "," & str紧急 & "," & str开始时间 & ",Null," & mlng挂号科室ID & "," & _
                    mlng挂号科室ID & ",'" & UserInfo.姓名 & "'," & strDate & ",'" & mstr挂号单 & "'," & ZVal(mlng前提ID) & "," & _
                    "Null,0,Null," & IIF(str摘要 = "", "Null", "'" & str摘要 & "'") & ",'" & UserInfo.姓名 & "'" & _
                    ",Null,Null,Null,Null," & lng申请序号 & ")"
...
'功能：修改申请单
        ...
        Else
            If Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_操作类型)) = 6 And vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "E" Then
               Call FuncLISApply(Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_申请序号)))
                ElseIf vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "D" Then
                        Call FuncPacsApply(Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_ID)), Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_申请序号)))
                ElseIf vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "K" Then
...
'注意:删除或修改下面列中数据时，请检查合理用药部件中的关联处理。
    ...
    If mblnPass Then
        With mobjPassMap
            ...
            COL_开始时间, COL_开嘱科室ID, , COL_频率, , , , COL_警示, , COL_医嘱状态, , , , , COL_执行性质, COL_标本部位, , , , , , COL_总量, , COL_医生嘱托, COL_用药目的)
