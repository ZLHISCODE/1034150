...
Public Function ShowApplicationForm(ByVal lngPatientID As Long, _
    ...
    If SafeArrayGetDim(objAppPages) > 0 Then
        For i = 0 To UBound(objAppPages)
            mobjAppDatas.Add "_" & objAppPages(i).lngApplicationPageId, objAppPages(i)
        Next
    End If
...
'获取当前申请页面的保存格式
    ...
    objAppData.strApplicationPageName = tbcRequest.Item(lngTabIndex).Caption                '申请单名称
    objAppData.lngApplicationPageId = Val(tbcRequest.Item(lngTabIndex).Tag)                 '申请单Id
...
'载入申请单的基本信息如病人姓名性别,申请科室,申请人等
    ...
    If mCurPatientInf.lngFrom = pfOutPatient Then '查询门诊
    ...
    If rsData.RecordCount > 0 Then
        ...
        If mCurPatientInf.lngFrom = pfOutPatient Then
        ...
        ElseIf mCurPatientInf.lngFrom = pfInPatient Then
    ...
    If mintBabyID > 0 Then
        strSql = "select 婴儿姓名,婴儿性别,出生时间 from 病人新生儿记录 where 病人id=[1] and 主页id=[2] and 序号=[3]"
        ...
        If rsData.RecordCount > 0 Then
            ...
            lngDate = DateDiff("d", Nvl(rsData!出生时间), Date)
            
            If lngDate <= 30 Then
                mCurPatientInf.strAge = lngDate & "天"
            ElseIf lngDate > 30 And lngDate < 365 Then
                mCurPatientInf.strAge = lngDate \ 30 & "月"
            ElseIf lngDate > 365 And lngDate < 365 * 3 Then
                mCurPatientInf.strAge = lngDate \ 365 & "岁" & (lngDate Mod 365) \ 30 & "月"
            Else
                mCurPatientInf.strAge = lngDate \ 365 & "岁"
            End If
        End If
    End If
End Sub
...
'载入申请单共有的基本信息
    ...
    labRequestDoctValue.Caption = mCurDoctorInf.str姓名
...
Private Sub CancelRequest(ByVal lngPageIndex As Long)
'撤销申请
    If MsgBox("撤销本次申请后，数据将不能恢复，是否继续？", vbYesNo, Me.Caption) = vbNo Then Exit Sub
    
    '从mobjAppDatas中移除保存的数据
    If mobjAppDatas.Exists("_" & CStr(tbcRequest.Item(lngPageIndex).Tag)) Then Call mobjAppDatas.Remove("_" & tbcRequest.Item(lngPageIndex).Tag)
    
    If tbcRequest.ItemCount > 1 Then
        If lngPageIndex > 0 Then
            '向前移动申请页面
            Call tbcRequest.RemoveItem(lngPageIndex)
        ElseIf lngPageIndex < tbcRequest.ItemCount - 1 Then
            '向后移动申请页面
            Call tbcRequest.RemoveItem(lngPageIndex)
        End If
    ...
    Else
        Call LoadRequestPage(tbcRequest.Item(lngPageIndex).Tag)
    End If
...
Private Function SaveRequest(ByVal lngPageIndex As Long) As Boolean
    ...
    If lngPageIndex < 0 Then Exit Function
    ...
    strDataKey = tbcRequest.Item(lngPageIndex).Tag
    
    Set objCurAppData = GetCurRequestPageFormat(lngPageIndex)
    ...
    If mCurPatientInf.lngInsure > 0 Then
        ...
        If rsTemp.RecordCount > 0 Then
            lblInsureInfo.Caption = "医保信息：" & gclsInsure.GetItemInfo(mCurPatientInf.lngInsure, mCurPatientInf.lngID, Nvl(rsTemp!收费项目ID))
        End If
    End If
...
On Error GoTo errHandle
    Select Case Control.ID
        ...
        Case TApplicationMenu.amAppType * 100 + 1 To TApplicationMenu.amAppType * 100 + 99
            If Not tbcRequest.Visible Or tbcRequest.ItemCount <= 0 Then Exit Sub
            
            '如果对应申请单已经保存，则设置图标
            Control.IconId = IIF(mobjAppDatas.Exists("_" & Control.Category), 3558, 1)
            
        Case TApplicationMenu.amAppDel
            If Not tbcRequest.Visible Or tbcRequest.ItemCount <= 0 Then
...
On Error GoTo errHandle
    ...
    If Not tbcRequest.Selected Is Nothing Then
        '申请单类型相同时，则不进行切换
        If strName = tbcRequest.Selected.Caption Then Exit Sub
    End If
    ...
    For i = 0 To tbcRequest.ItemCount - 1
        If tbcRequest.Item(i).Caption = strName Or Not mobjAppDatas.Exists("_" & CStr(tbcRequest.Item(i).Tag)) Then
    ...
    If lngFindIndex > -1 Then
        Set objControlItem = tbcRequest.Item(lngFindIndex)
        
        If objControlItem.Caption <> strName Then
            objControlItem.Caption = strName
            
            '清除诊断编辑器中的缓存数据
            If Not mclsDiagEdit Is Nothing Then
                Call mclsDiagEdit.DeleteApplyDiag(Val(tbcRequest.Item(lngFindIndex).Tag))
            End If
    ...
    Else
        '创建新的tab页面
        Set objControlItem = tbcRequest.InsertItem(tbcRequest.ItemCount, strName, picTab.hWnd, 0)
    End If
    
    '重新设置tab页的tag设置
    objControlItem.Tag = CStr(lngRequestPageId)
...
On Error GoTo errHandle
    ...
    '诊断编码
    Set rsTemp = zlDatabase.ShowILLSelect(Me, "1", mCurPatientInf.lngRoomId, mCurPatientInf.strSex, True, False, strCode, , mCurPatientInf.lngFrom)
...
On Error GoTo errHandle
    ...
    'D-ICD-10疾病编码
    Set rsTemp = zlDatabase.ShowILLSelect(Me, "D", mCurPatientInf.lngRoomId, mCurPatientInf.strSex, True, True, strCode, , mCurPatientInf.lngFrom)
...
'打开诊断编辑器
    ...
    lngApplicationPageId = Val(tbcRequest.Selected.Tag)
    
    '判断当前申请单是否在mobjAppDatas中存在
    If mobjAppDatas.Exists("_" & CStr(lngApplicationPageId)) Then
        Set objAppData = mobjAppDatas("_" & CStr(lngApplicationPageId))
...
On Error GoTo errHandle
    ...
    Else
        strSql = "select id, 模板标题,模板内容,使用次数 from 病历附项模板 where 病历文件Id=" & Val(tbcRequest.Selected.Tag) & " and 单据附项='" & strPro & "' order by 使用次数 desc "
...
'############窗口测试代码#######################
'    InitDebugObject 1290, Me, "zlhis", "I39EFQ"
'    mlngCurDeptId = 46
'
'    mblnPageUpdateState = False
'    mblnIsLoadRequestPage = True
'    mblnIsSaveRequestPage = False
'
'    mCurPatientInf.lngFrom = 2
'    mCurPatientInf.lngID = 690825
''    mCurPatientInf.lngRegId = 545133
'    mCurPatientInf.lngPageID = 1
'
'    mlngUpdateAppNo = 0 '8330943
'
'    Call GetUserInfo
...
'功能：获取省页面
    ...
    strSql = "Select Max(病历文件id) As PageId From 病历单据应用 Where 诊疗项目id =[1]"
    Set rsData = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lngProjectId)
...
'    strSQL = "select distinct Id from 病人医嘱记录 where 病人Id=[1] and (申请序号=[2] or id=[2]) and 相关ID is null"
'    Set rsData = zlDatabase.OpenSQLRecord(strSQL, "获取更新医嘱记录", mCurPatientInf.lngID, lngUpdateAppNo)
'    If rsData.RecordCount <= 0 Then Exit Sub
'
'    While Not rsData.EOF
'        Set objAppData = ReadAdviceInf(Nvl(rsData!ID))
...
'        Set curTab = tbcRequest.InsertItem(0, objAppData.strApplicationPageName, picTab.Hwnd, 0)
'
'        Call mobjAppDatas.Add(CStr(objAppData.lngApplicationPageId), objAppData)
'
'        curTab.Tag = CStr(objAppData.lngApplicationPageId)
'
...
'    Wend
    ...
    tbcRequest.Tag = strTbcTag
    
    '载入页面数据
    Call LoadRequestPage(Val(tbcRequest.Item(0).Tag))
End Sub
...
'载入检查申请单类别
    ...
    strSql = "select id, 名称 from 病历文件列表 where 种类=7 and 子类='检查' order by 编号"
    Set rsData = zlDatabase.OpenSQLRecord(strSql, "查询检查申请单")
...
'载入申请单附项录入
    ...
    labTitle.Caption = tbcRequest.Selected.Caption
...
'    strDefExeRoomId = Get诊疗执行科室ID(mCurPatientInf.lngID, mCurPatientInf.lngPageID, "D", , lngProjectRoomType, mCurPatientInf.lngRoomId, mlngCurDeptId, 0)
    ...
    Select Case lngProjectRoomType
        ...
        Case 5  '5-院外执行
            ...
            Call zlControl.CboSetIndex(cbxExeRoom.hWnd, 0)
...
'                " Order by 编码"
    ...
    For i = 1 To rsData.RecordCount
        ...
        If lngDefExeRoomId = rsData!ID Then
            Call zlControl.CboSetIndex(cbxExeRoom.hWnd, i - 1)
        End If
...
On Error GoTo errHandle
    ...
    tbcRequest.Left = 3520 '2580
    tbcRequest.Top = 60
    tbcRequest.Width = ScaleWidth - 3540 ' 2600
    
    picBack.Left = 0
    picBack.Top = picMenu.Height
    picBack.Width = ScaleWidth
    picBack.Height = ScaleHeight - picMenu.Height - stbThis.Height
...
Private Sub InitCommandBars()
    ...
    Set cbrControl = cbrToolBar.Controls.Add(xtpControlButton, TApplicationMenu.amAppDel, "撤销", "撤销检查申请"): cbrControl.IconId = 3014: cbrControl.BeginGroup = True
    Set cbrControl = cbrToolBar.Controls.Add(xtpControlButtonPopup, TApplicationMenu.amAppType, "申请单▼", "选择检查申请单的类型"): cbrControl.IconId = 807
    ...
    With vfgList
        ...
        Do While Not rsTmp.EOF
            If .TextMatrix(.Rows - 1, 1) <> rsTmp!部位 Then
                ...
                .TextMatrix(.Rows - 1, 0) = NeedName("" & rsTmp!分组)
...
On Error GoTo errHandle
    ...
    Set rsData = zlDatabase.OpenSQLRecord(strSql, "查询附项模板", Val(tbcRequest.Selected.Tag), strPro)
...
On Error GoTo errHandle
    ...
    Call LoadRequestPage(lngRequestId)
...
Private Sub LoadRequestPage(ByVal lngApplicationPageId As Long)
'载入申请页面
On Error GoTo errHandle
    Dim objCurAppData As clsApplicationData
    Dim strDataKey As String
    ...
    strDataKey = lngApplicationPageId
    If mobjAppDatas.Exists("_" & strDataKey) Then
...
'将选择的检查项目和方法显示到txt只读文本中
    ...
    '计算检查费用
    dblPrice = GetPrice(vfgRequestProject.Cell(flexcpData, lngProjectIndex, TProjectCol.pcId), _
                        vfgRequestProject.Cell(flexcpText, lngProjectIndex, TProjectCol.pcMethod), _
                        lngExeType, _
                        mCurPatientInf.lngFrom, _
                        lngExeRoomId)
...
'初始化调试状态下的所需对象
    ...
    Call OraDataOpen("006116txyy", strUser, strPwd)
