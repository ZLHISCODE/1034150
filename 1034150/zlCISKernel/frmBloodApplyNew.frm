...
Begin VB.Form frmBloodApplyNew 
   BackColor       =   &H8000000E&
   ...
   ClientTop       =   375
   ClientWidth     =   10650
   Icon            =   "frmBloodApplyNew.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton cmd常用嘱托 
      Height          =   300
      Left            =   5355
      Picture         =   "frmBloodApplyNew.frx":6852
      Style           =   1  'Graphical
      TabIndex        =   95
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton cmdDate 
      ...
      Index           =   1
      Left            =   10215
      Picture         =   "frmBloodApplyNew.frx":6DDC
      Style           =   1  'Graphical
      TabIndex        =   81
      TabStop         =   0   'False
      ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.CommandButton cmdDate 
      ...
      Index           =   0
      Left            =   4200
      Picture         =   "frmBloodApplyNew.frx":6ED2
      Style           =   1  'Graphical
      TabIndex        =   80
      TabStop         =   0   'False
      ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.PictureBox picGet 
      ...
      Begin VB.CommandButton cmdGet 
         ...
         Index           =   1
         Left            =   2970
         Picture         =   "frmBloodApplyNew.frx":6FC8
         Style           =   1  'Graphical
         TabIndex        =   72
         TabStop         =   0   'False
         ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.PictureBox picInfo 
      ...
      Begin VB.ComboBox cboInfo 
         ...
         Height          =   360
         Index           =   1
         ItemData        =   "frmBloodApplyNew.frx":70BE
         Left            =   -25
         List            =   "frmBloodApplyNew.frx":70C0
         Style           =   2  'Dropdown List
         TabIndex        =   27
         Top             =   -30
         Width           =   1335
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsLIS 
      ...
      FormatString    =   $"frmBloodApplyNew.frx":70C2
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   2
   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":7315
            Key             =   "c0"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":78AF
            Key             =   "c1"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":7E49
            Key             =   "o0"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":83E3
            Key             =   "o1"
         EndProperty
      EndProperty
   End
   Begin VB.PictureBox picPreBlood 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfBlood 
         ...
         FormatString    =   $"frmBloodApplyNew.frx":897D
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfList 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmBloodApplyNew.frx":8AF2
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmBloodApplyNew"
...
Public Function ShowMe(frmParent As Object, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病人性质 As Long, ByVal inttype As Integer, Optional ByVal lngUpdateAdvice As Long, _
            ...
            Optional ByRef rsDefine As Recordset, Optional ByRef objMip As Object, Optional ByVal int场合 As Integer, Optional ByVal str挂号单 As String, Optional ByVal lng前提ID As Long, _
            Optional ByVal lng项目ID As Long, Optional ByRef rsCard As ADODB.Recordset, Optional ByVal bytBaby As Byte, Optional ByVal int调用场合 As Integer, Optional ByVal int申请单模式 As Integer = 0) As Boolean
...
'功能：检查数据正确性
    ...
    If mintType <> 3 Then
        ...
        If txtInfo(txt孕).Text <> "" And txtInfo(txt产).Text = "" Then
            ...
            If txtInfo(txt产).Enabled Then txtInfo(txt产).SetFocus
            Exit Function
        End If
        If txtInfo(txt产).Text <> "" And txtInfo(txt孕).Text = "" Then
            MsgBox "输入了孕产情况中的产次，则必须输入孕次。", vbInformation, Me.Caption
            If txtInfo(txt孕).Enabled Then txtInfo(txt孕).SetFocus
            Exit Function
        End If
...
'功能：保存数据
    ...
    Dim str滴速 As String
    Dim strPar As String, strErr As String
    ...
    If cboInfo(cbo滴速).Visible = True Then
        If cboInfo(cbo滴速).ListIndex > 0 Then
            str滴速 = cboInfo(cbo滴速).ItemData(cboInfo(cbo滴速).ListIndex)
        Else
            str滴速 = Val(cboInfo(cbo滴速).Text)
        End If
    End If
    If IsNumeric(str滴速) = True Then
        str滴速 = str滴速 & lblInfo(31).Caption
    End If
    ...
    ElseIf mintType = 4 Or (mintType = 1 And mblnUseBloodSend = True) Then '核对医嘱或修改用血医嘱
        ...
        For i = vsfBlood.FixedRows To vsfBlood.Rows - 1
            ...
            End If
        Next
        If str项目名称 = "" Then str项目名称 = Get项目名称(mlng输血项目ID)
    ...
    Else
        ...
        strPar = strTmp挂号单 & IIF(mlng前提ID = 0, ",NULL", "," & mlng前提ID)
        str项目名称 = ""
        For i = vsfBlood.FixedRows To vsfBlood.Rows - 1
            If Val(vsfBlood.Cell(flexcpData, i, COL_P_选择)) = 1 Then
                str项目名称 = IIF(str项目名称 = "", "", str项目名称 & ",") & vsfBlood.TextMatrix(i, COL_P_名称)
        ...
        str审核状态 = GetBloodVerifyState(int病人来源, mlng病人ID, IIF(int病人来源 = 2, mlng主页ID, mlng挂号ID), txtInfo(txt预定输血时间).Text, dblTmp, int紧急, int分类, CInt(mbytBaby), mlngUpdateAdvice, GetBloodInfo(False))
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & ",NULL," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & "," & mbytBaby & ",1,1,'K'," & mlng输血项目ID & _
                                 ...
                                 ",NULL," & mlng病人科室id & "," & mlng开单科室ID & ",'" & UserInfo.姓名 & "'," & _
                                 "to_date('" & Format(IIF(curDate > CDate(txtInfo(txt申请日期).Text), txtInfo(txt申请日期).Text, curDate), "yyyy-MM-dd HH:mm:ss") & "','yyyy-MM-dd HH24:MI:SS')," & _
                                 strTmp挂号单 & ",NULL," & IIF(int检查方法 = 0, "NULL", "'" & int检查方法 & "'") & ",0,NULL," & IIF(mstr摘要输血 = "", "null", "'" & mstr摘要输血 & "'") & ",'" & UserInfo.姓名 & "',Null,Null,'" & cboInfo(cbo输血目的).Text & "'," & str审核状态 & "," & lng申请序号 & ")"
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & "," & lng相关ID & "," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & _
                                 ...
                                 ",NULL," & mlng病人科室id & "," & mlng开单科室ID & ",'" & UserInfo.姓名 & "'," & _
                                 "to_date('" & Format(IIF(curDate > CDate(txtInfo(txt申请日期).Text), txtInfo(txt申请日期).Text, curDate), "yyyy-MM-dd HH:mm:ss") & "','yyyy-MM-dd HH24:MI:SS')," & _
                                 strPar & ",NULL,0,NULL," & IIF(mstr摘要途径 = "", "null", "'" & mstr摘要途径 & "'") & ",'" & UserInfo.姓名 & "',Null,NULL,''," & str审核状态 & "," & lng申请序号 & ")"
    End If
    '输血申请其他项目
    ReDim Preserve arrSQL(UBound(arrSQL) + 1)
    arrSQL(UBound(arrSQL)) = "Zl_输血申请记录_Insert(" & lng相关ID & "," & chkWait.value & ",'" & cboInfo(cbo输血类型).Text & "','" & cboInfo(cbo输血目的).Text & "'," & cboInfo(cbo输血性质).ListIndex & "," & IIF(optHistory(0).value, 0, 1) & _
                             ...
                             "," & cboInfo(cbo输血血型).ListIndex & "," & cboInfo(cboRHD).ListIndex & ",'" & GetBloodInfo & "')"
    '检验项目
    With vsLIS
        lngCount = 0
        For i = 0 To .Rows - 1
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_File_PrintSet, conMenu_File_Print, conMenu_File_Preview
            blnVisible = ((mint申请单打印模式 = 2 And InStr(GetInsidePrivs(p住院医嘱下达), ";输血申请单;") > 0) Or mint场合 = 1) And mint调用场合 = 0
            If mint申请单打印模式 = 2 And mint场合 = 0 Then
...
'5.补录的医嘱开始时间不能大于当前时间，转科补录不能大于转科开始时间
    ...
    '122807:用血医嘱的执行时间不能小于首次备血的执行时间
    If mblnSpareBloood = False Then
        gstrSQL = "Select c.开始执行时间" & vbNewLine & _
            " From 诊疗项目目录 a, 病人医嘱记录 b, 病人医嘱记录 c" & vbNewLine & _
            " Where a.Id = b.诊疗项目id And (a.操作类型 = '8' And Nvl(a.执行分类, 0) = 0 Or a.操作类型 = '9') And b.诊疗类别 = 'E' And b.相关id = c.Id And" & vbNewLine & _
            "      C.医嘱状态 Not In (-1, 4) And C.诊疗类别 = 'K' And C.病人id = [1] And " & IIF(mint场合 = 0, " C.主页id = [2]", " C.挂号单= [2]") & " And Nvl(C.婴儿, 0) = [3]" & vbNewLine & _
            " Order By c.开始执行时间"
        If mint场合 = 0 Then
            Set rsBlood = zlDatabase.OpenSQLRecord(gstrSQL, "Check开始时间", mlng病人ID, mlng主页ID, mbytBaby)
        Else
            Set rsBlood = zlDatabase.OpenSQLRecord(gstrSQL, "Check开始时间", mlng病人ID, mstr挂号单, mbytBaby)
        End If
        If Not rsBlood.EOF Then
            If Format(strStart, "yyyy-MM-dd HH:mm") < Format(rsBlood!开始执行时间, "yyyy-MM-dd HH:mm") Then
                strMsg = "用血医嘱的开始执行时间不能小于首次备血医嘱的开始执行时间" & Format(rsBlood!开始执行时间, "yyyy-MM-dd HH:mm") & " 。"
                If blnMsg Then MsgBox strMsg, vbInformation, gstrSysName
                Exit Function
            End If
        End If
...
'功能：初始下拉菜单
    ...
    If blnFormLoad = True Then
        '部分固定内容的下拉框
        Call SetCboFromList(cboInfo, Array(" ", "A", "B", "O", "AB", "不详", "未查"), Array(cbo输血血型), 0)
        Call SetCboFromList(cboInfo, Array("普通", "急诊"), Array(cbo用血安排))
        Call Cbo.SetIndex(cboInfo(cbo用血安排).hWnd, 0)
        Call SetCboFromList(cboInfo, Array(" ", "-", "+"), Array(cboRHD), 0)
        Call SetCboFromList(cboInfo, Array("15", "30", "快速", "加压"), Array(cbo滴速))
...
'功能：提取病人基本信息
    ...
    If mint场合 = 0 Then
        ...
        Else
            strSQL = "Select a.住院号, Nvl(q.婴儿姓名, a.姓名 || '之婴' || q.序号) 姓名, q.婴儿性别 性别, Round(Sysdate - q.出生时间) || '天' As 年龄, b.名称 As 科室," & vbNewLine & _
...
GoLoadData:
        ...
        Else
            '修改
            '读取输血相关信息
            strSQL = "Select 是否待诊, 输血类型, 输血目的, 输血性质, 即往输血史, 既往输血反应史, 输血禁忌及过敏史,孕产情况, 受血者属地, 输血血型, Rhd, 受血者血型, Hct, Alt, Hbsag, 梅毒, 血红蛋白, 血小板, Antihcv, Antihiv12" & vbNewLine & _
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                cboInfo(cbo滴速).Text = ""
                lblInfo(31).Visible = True
                If strTmp Like "*滴/分" Then
                    If IsNumeric(Split(strTmp, "滴/分")(0)) = True Then
                        cboInfo(cbo滴速).Text = Split(strTmp, "滴/分")(0)
                    End If
...
'功能：初始化输血项目对应的检验项目指标表格（备血申请可能有所个输血项目）
    ...
    If UBound(arrBloodID) > 0 Then
        strSQL = "Select /*+ CARDINALITY(C 10) */ A.检验项目ID,B.编码 from 输血检验对照 A,诊疗项目目录 B,Table(f_Num2list([1])) C " & _
    ...
    Else
        strSQL = "Select A.检验项目ID,B.编码 from 输血检验对照 A,诊疗项目目录 B Where A.检验项目ID=B.ID And A.项目ID=[1] Order by B.编码"
        ...
        If mint场合 = 0 Then
            ...
            If strTmp1 = "" Then
                If vsLIS.Tag = "YES" Then
                    blnGet = True
                Else
                    blnGet = (MsgBox("本次住院未找到有效的检验指标，是否提取历次就诊七天内的检验指标？", vbQuestion + vbDefaultButton2 + vbYesNo, Me.Caption) = vbYes)
                End If
...
Private Sub txtInfo_GotFocus(Index As Integer)
    Dim intIme As Integer '0：不处理；1-打开输入发;2-关闭输入法
   ...
   If cboInfo(cbo滴速).Visible = True Then
    ...
    With mrsCard
        ...
        !申请其他项目SQL = "Zl_输血申请记录_Insert([相关ID]," & chkWait.value & ",'" & cboInfo(cbo输血类型).Text & "','" & cboInfo(cbo输血目的).Text & "'," & cboInfo(cbo输血性质).ListIndex & "," & IIF(optHistory(0).value, 0, 1) & _
                             "," & IIF(optHistory(2).value, 0, 1) & "," & IIF(optHistory(4).value, 0, 1) & ",'" & txtInfo(txt孕) & "/" & txtInfo(txt产) & "'," & IIF(optPossession(0).value, 0, 1) & _
                             "," & cboInfo(cbo输血血型).ListIndex & "," & cboInfo(cboRHD).ListIndex & ",'" & !申请项目 & "')"
        !检验项目SQL = str检验项目SQL
...
'功能：根据申请类型设置窗体相关属性
    ...
    If blnFormLoad = False Then
        If InitInfo(False) = False Then Exit Function
    End If
    If mblnSpareBloood = True Then
    ...
    '预定输血日期
    lblInfo(lbl预定输血日期).Top = IIF(bln用血申请 = True, lblInfo(lbl孕产情况).Top, lblInfo(lbl既往输血史).Top + lblInfo(lbl既往输血史).Height + 195)
    ...
    With picPreBlood
        ...
        .Top = lblInfo(lbl预定输血日期).Top + lblInfo(lbl预定输血日期).Height + 210
        .Visible = True
    End With
    ...
    End With
    '滴速用血申请才显示(目前暂时不用)
    lblInfo(30).Visible = False 'bln用血申请
    picInfo(3).Visible = False 'bln用血申请
    Line1(25).Visible = False 'bln用血申请
    lblInfo(31).Visible = False 'bln用血申请
'    If bln用血申请 Then
'        If cboInfo(cbo滴速).Text <> "" And IsNumeric(cboInfo(cbo滴速).Text) = False Then lblInfo(31).Visible = False
'    End If
    '发血执行位置变动调整
    If picInfo(3).Visible = False Then
...
'用血申请如果是通过医生选择血液的模式则调用次函数
    ...
    With vsfList
        ...
        .TextMatrix(0, COL_S_编号) = "血袋编号": .ColWidth(COL_S_编号) = 1500:
...
'功能：'下达用血申请时：诊断、输血目的、血型等默认去最后一次备血申请的信息
    ...
    strSQL = "Select 是否待诊,输血类型, 输血目的, 输血性质,即往输血史, 既往输血反应史, 输血禁忌及过敏史, 输血血型, Rhd,受血者属地" & vbNewLine & _
                " From 输血申请记录" & vbNewLine & _
                " Where 医嘱id = [1]"
