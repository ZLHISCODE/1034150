...
Begin VB.Form frmCompoundMedicine 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsSend 
      Align           =   3  'Align Left
      Bindings        =   "frmCompoundMedicine.frx":0159
      Height          =   3000
      Left            =   0
      TabIndex        =   1
      Top             =   0
      ...
      FormatString    =   $"frmCompoundMedicine.frx":016D
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Public Sub RefreshData(ByVal lngAdviceID As Long, ByVal lng病区ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病人性质 As Long, ByVal lng医嘱期效 As Long, Optional ByRef objMip As Object)
        ...
        For i = .FixedRows To .Rows - 1
            If Val(.Cell(flexcpData, i, mCol("状态"))) = 1 Then
                ...
                strSql = "Zl_输液配药记录_Update(" & .RowData(i) & "," & lngTmp & "," & _
                    ZVal(.Cell(flexcpData, i, mCol("配药批次"))) & ",'" & UserInfo.姓名 & "'," & strCurDate & ")"
...
'参数：lngSendNO=医嘱发送号
    ...
    End If
    If strIDs <> "" Then
        strSql = "Select 配药ID,操作类型,操作人员,To_Char(操作时间,'YYYY-MM-DD HH24:MI') as 操作时间,操作时间 as 排序 from 输液配药状态 Where 配药ID in(select Column_Value From Table(Cast(f_num2list([1]) As ZLTOOLS.t_numlist)))"
...
Private Sub vsExec_StartEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    If (Col = mCol("打包") And mbln打包修改 Or Col = mCol("配药批次")) Then
        ...
        If Col = mCol("打包") And vsExec.TextMatrix(Row, mCol("状态")) = "待配药" Then Cancel = False
        If Cancel = False Then
            ...
            '判断权限
            If Not Cancel Then
                If InStr(GetInsidePrivs(p住院医嘱发送), ";修改打包状态和配液批次;") = 0 Then
                    Cancel = True
                    MsgBox "您没有修改打包状态和配液批次权限，不能进调整。", vbInformation, Me.Caption
                End If
            End If
        Else
            If Col = mCol("打包") Then
...
'参数：blnAutoAduit=true自动审核销帐申请
    ...
    With vsExec
        ...
        strSql = "Select b.费用id, b.药品id As 收费细目id, Sum(a.数量) As 数量, c.住院包装, c.住院单位, d.名称, b.No, e.序号,E.记录状态" & vbNewLine & _
            "From 输液配药内容 A, 药品收发记录 B, 住院费用记录 E, 药品规格 C, 收费项目目录 D" & vbNewLine & _
        ...
        Else
            ...
            For i = 1 To rsTmp.RecordCount
                ...
                strSql = "Zl_病人费用销帐_Insert(" & rsTmp!费用ID & "," & rsTmp!收费细目ID & "," & _
                      mlng病区ID & "," & rsTmp!数量 & ",'" & UserInfo.姓名 & "'," & strDate & "," & IIF(blnAutoAduit, "0", "1") & ",1," & Val(.RowData(.Row)) & ")"
                colSQL.Add strSql, "C" & colSQL.Count + 1
                '待摆药状态的自动审核
                If blnAutoAduit Then
