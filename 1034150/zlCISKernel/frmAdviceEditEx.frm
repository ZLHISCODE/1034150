...
'0-检查组合,1-手术输入,4-检验组合，5-输血或治疗类需要填写申请附项的
...
Public Function ShowMe(ByVal frmParent As Object, ByVal lngHwnd As Long, ByRef t_Pati As TYPE_PatiInfoEx, ByVal int场合 As Integer, _
            ...
            Optional ByRef strAppend As String, Optional ByVal strAdvItem As String, Optional ByVal strDiagnosis As String, Optional ByRef str手术部位 As String) As Boolean
'参数:
'     frmParent         父窗体
...
'功能：获取指定的申请附项值
    ...
    '就诊要素的提取门诊住院区别对待
    '1-门诊先取已有医嘱中的附项目，再取病历中的，2-住院先取病历，再取医嘱
    If intType = 1 Then
        ...
        If .Visible And .SelLength = 0 And .SelStart > 0 Then
            ...
            Else
                ...
                ElseIf Mid(.Text, .SelStart, 1) = "`" Then
                    ...
                    Call ShowWordInput
                Else
                    If Not (str项目 = "助手医生" Or str项目 = "主刀医生" Or txtSentence.Tag = "选择单选项") Then txtSentence.Tag = ""
                End If
            End If
        End If
    End With
...
Private Function FindItem() As Recordset
    ...
    vPoint = GetCoordPos(txtSentence.hWnd, txtSentence.Left + 15, txtSentence.Top + 3300 + txtSentence.Height)
...
Private Sub vsExt_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsMethod
        ...
        Next
'        '共选部份
'        arrMethod = Split(strMethod, vbTab)
'        If UBound(arrMethod) >= 1 Then
'            arrMethod = Split(arrMethod(1), ";")
'            For i = 0 To UBound(arrMethod)
'                .Rows = .Rows + 1
'                .MergeRow(.Rows - 1) = True
'                .RowData(.Rows - 1) = 2 '表明是共选项
'
'                .Cell(flexcpText, .Rows - 1, 0, .Rows - 1, 1) = Mid(arrMethod(i), 2) '第一位是造影剂标志
'                If InStr("," & vsExt.TextMatrix(vsExt.Row, 2) & ",", "," & Mid(arrMethod(i), 2) & ",") > 0 Then
'                    Set .Cell(flexcpPicture, .Rows - 1, 0, .Rows - 1, 1) = img16.ListImages("c1").Picture
'                    .Cell(flexcpData, .Rows - 1, 0) = 1
'                Else
'                    Set .Cell(flexcpPicture, .Rows - 1, 0, .Rows - 1, 1) = img16.ListImages("c0").Picture
