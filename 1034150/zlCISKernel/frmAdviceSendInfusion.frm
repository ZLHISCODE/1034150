...
Private mstrInfDepIDs As String  '在输液配置中心发药的病区
    ...
    If mbytShowMode = 1 Then  '模式1方式，显示明天发送医嘱
        ...
        If mbln输液接收 Then
            If Not IsWorking Then
                MsgBox "配液中心已停止接收需配置的医嘱！", vbInformation, gstrSysName
            End If
        End If
    ElseIf mbytShowMode = 2 Then
        ...
        For i = 1 To mrs药房.RecordCount
            If InStr("," & strTmp & ",", "," & Val(mrs药房!ID) & ",") = 0 Then
                ...
                If lngTmp = Val(mrs药房!ID) Then
                    Call zlControl.CboSetIndex(.hWnd, .NewIndex)
                End If
            End If
            mrs药房.MoveNext
        Next
        If .ListIndex = -1 Then Call zlControl.CboSetIndex(.hWnd, 0)
    End With
...
'功能：重置发送条件
        ...
        With vsAdvice
            For i = .FixedRows To .Rows - 1
                If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                    blnHave = True
                End If
                If .Cell(flexcpData, i, COL_选择) = 1 Then
                    blnHave = True
                End If
            Next
        End With
    ...
    blnSetPar = InStr(GetInsidePrivs(p住院医嘱发送), ";医嘱选项设置;") > 0 And Me.Visible
    '单病人模式不保存
    If Not mblnOnePati Then
...
'功能：读取今天的输液药品医嘱
    ...
    With cboDruStoCha
        If mbln输液接收 Then Call GetOrSetDruStoChaPar(mstrParDruDepCha, 2, .ItemData(.ListIndex), blnSetPar)
    End With
    ...
    With vsPrice
        ...
        If Col = COLP_收费项目 Then
            ...
            strSql = strSql & " Union ALL " & _
                ...
                " And (a.类别 Not in ('5','6','7') Or Exists(Select 1 From 收费执行科室 W Where w.收费细目id=a.Id And Nvl(w.开单科室id,[3])=[3]))" & _
                " And (Nvl(a.执行科室,0) <> 4 Or Exists (Select 1 From 收费执行科室 W Where w.收费细目id = a.Id And (w.病人来源=2 or (w.病人来源 is Null And Nvl(w.开单科室id,[3]) = [3]))))"
...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;" & _
        ...
        "药品ID;剂量系数;住院包装;住院单位;可否分零;药房分批;是否变价;库存;次数;分解时间;操作类型;试管编码;标本部位;检查方法;跟踪在用;紧急标志;医嘱状态;执行频率;新开操作时间;开始时间"
...
'功能：按病人读取医嘱发送清单
    ...
    Dim strNoneIDs As String, str执行性质限制 As String
    ...
    If gstr输液配置中心 <> "" Then
        str药房条件 = "Select ID From 病人医嘱记录 X" & _
                ...
                " And Instr(',' || [3] || ',',',' || Nvl(执行科室id, 0) || ',') > 0 And 病人ID=[2]"
    ...
    '自备药、离院带药、不取药、自取药等特殊性质的药品根据参数判断
    If mbln接收自备药 = False Then
        str执行性质限制 = " And Not Exists (Select ID" & vbNewLine & _
                    "       From 病人医嘱记录 X" & vbNewLine & _
                    "       Where (x.相关id = a.相关id Or x.相关id = a.Id  Or x.Id = a.Id Or x.Id = a.相关id) And (x.执行性质 In (0, 5) Or x.执行标记 > 0)) "
    End If
    str长嘱 = ""
    str临嘱 = ""
    str医嘱期效 = ""
    ...
    If gblnKSSStrict Then
        If optState(0).value Or optState(2).value Then
            str医嘱期效 = str医嘱期效 & " And (A.医嘱状态<>1 Or A.医嘱状态=1 And Nvl(A.审核状态,0) Not in(1,3))"
        End If
    End If
    ...
    strSQL药品 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
             ...
             " Decode(A.执行频次,'必要时',Null,A.执行时间方案) As 执行时间方案," & _
             " [5] as 病人病区ID,A.病人科室ID,A.开嘱科室ID,A.开嘱医生," & IIF(mblnAutoVerify, "s.操作时间 as 新开操作时间,", "") & _
             " A.可否分零,A.计价特性,A.执行性质,A.执行标记," & str药房置换 & " as 执行科室ID,Nvl(F.名称,Decode(Nvl(A.执行性质,0),5,'-')) as 执行科室,A.摘要,A.医嘱状态,A.医嘱期效,A.首次用量,g.执行标记 as 给药执行标记" & _
             ...
             " And A.诊疗类别 IN('5','6','7','E')" & str主要条件 & str药房条件 & str给药途径 & str执行性质限制 & str医嘱期效 & " And (NVL(a.执行频次,'无')<>'必要时' And NVL(a.执行频次,'无')<>'需要时') " & _
    ...
    For k = 0 To UBound(arrPati)
        ...
        End If
        strNoneIDs = GetNoneSendID(Val(arrPati(k)), arrPatiPage(k), mbln自动皮试, 2, True)
        strSQL药品 = strtmp3
        If gstr输液配置中心 <> "" Then
            If bln可接收病区 Then
        ...
        strSQL药品 = strSQL药品 & IIF(strNoneIDs <> "", "  And Instr([6],','||A.ID||',')=0  ", "") & str输液药品排除 & str输液营养排除 & " Order by A.医嘱期效,A.婴儿,组号,组ID,A.序号"
        Set rsSend药品 = zldatabase.OpenSQLRecord(strSQL药品, Me.Caption, datEnd, Val(arrPati(k)), gstr输液配置中心, strDepts, mlng病区ID, "," & strNoneIDs & ",")
...
'rsSQL!类型=1-校对(如果不需要先校对),2-医嘱计价,3-住院记帐,4-执行科室替换，5-医嘱发送，6-自动发料,7-输液配药
    ...
    With vsAdvice
        For i = .FixedRows To .Rows - 1
            If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing And Val(.TextMatrix(i, COL_医嘱状态)) = 1 Then
...
MoneyItemBegin:
                                ...
                                If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                                    If Not AdviceMoneyMake(lng病人ID, lng主页ID, rsMoneyNow, rsMoneyDay, _
                                        ...
                                        Val(.TextMatrix(i, COL_ID)), lng发送号, Val(rsPrice!数量 & ""), rsExec, , , , , , .TextMatrix(i, COL_诊疗类别)) Then
                                        '跳过当前收费项目(多个收入项目)
                        ...
                        If Val(.TextMatrix(i, COL_执行性质ID)) <> 0 Then  '叮嘱不发送(给药途径，配方煎法、用法、采集方法可能为)
                            ...
                            rsSQL!Sql = "ZL_病人医嘱发送_Insert(" & _
                                ...
                                "'" & UserInfo.姓名 & "','" & str领药号 & "',Null,'" & str分解时间 & "')"
                            rsSQL.Update
                        End If
...
Private Function GetTimPointsInRange(ByVal strTimRange As String, ByVal strTimPoints As String) As String
'功能：获取在指定时间范围内的时间点
'参数：
