   ...
   Begin VB.PictureBox picPati 
      ...
      Begin VB.Frame fra缺省时间 
         ...
         Begin VB.ComboBox cbo医生 
            ...
            Width           =   1515
         End
         Begin VB.CheckBox chk收回 
            Caption         =   "收回超期的"
            Height          =   195
            Left            =   6735
            TabIndex        =   17
            ToolTipText     =   "末次发送时间大于当前时间的，停止到当前时间(超过当前时间的收回)，否则停止到末次发送时间"
            Top             =   98
            Visible         =   0   'False
            Width           =   1200
         End
         Begin VB.CheckBox chk补发 
            Caption         =   "补发未发的"
            Height          =   195
            Left            =   5490
            TabIndex        =   16
            ToolTipText     =   "末次发送时间小于当前时间的，停止到当前时间(未发送的在下次发送时补上)，否则停止到末次发送时间"
            Top             =   98
            Value           =   1  'Checked
            Visible         =   0   'False
            Width           =   1200
...
Private mblnStopNurseGrade As Boolean '单独停止护理等级
...
Private mobjPassMap As Object   'PASS 映射入口参数
...
'可见列
Private Const COL_标志 = 10
Private Const COL_姓名 = 11
Private Const COL_住院号 = 12
Private Const COL_床号 = 13
Private Const COL_婴儿 = 14
Private Const COL_期效 = 15
Private Const COL_开嘱时间 = 16
Private Const COL_开始时间 = 17
Private Const col_医嘱内容 = 18
Private Const COL_皮试 = 19
Private Const COL_总量 = 20
Private Const COL_单量 = 21
Private Const COL_频率 = 22
Private Const COL_用法 = 23
Private Const COL_医生嘱托 = 24
Private Const COL_执行时间 = 25
Private Const COL_终止时间 = 26 '
Private Const COL_执行科室 = 27
Private Const COL_执行性质 = 28
Private Const COL_上次执行 = 29
...
Private Const COL_诊疗项目ID = 37
Private Const COL_频率次数 = 38
Private Const COL_频率间隔 = 39
Private Const COL_间隔单位 = 40
Private Const COL_执行标记 = 41
Private Const COL_操作类型 = 42
Private Const COL_试管编码 = 43
Private Const COL_执行科室ID = 44
Private Const COL_病人科室ID = 45
Private Const COL_收费细目ID = 46
Private Const COL_单量单位 = 47
Private Const COL_前提ID = 48
Private Const COL_签名ID = 49
Private Const COL_操作人员 = 50
Private Const COL_开嘱科室ID = 51
Private Const COL_操作说明 = 52
Private Const COL_执行分类 = 53
Private Const COL_标本部位 = 54  '合理用药监测中西成药名
Private Const COL_审核状态 = 55
...
Public Function ShowMe(frmParent As Object, ByVal MainPrivs As String, ByVal int类型 As Integer, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病区ID As Long, _
    ...
    Optional ByVal bln术后 As Boolean, Optional ByVal lng医护科室ID As Long, Optional ByVal bln发送调用 As Boolean, Optional ByRef objMip As Object, Optional ByRef strPatisOut As String, Optional ByVal bytSize As Byte) As Boolean
...
Private Sub cboTime_Click(Index As Integer)
    If cboTime(Index).ListIndex <> -1 Then
        If Me.Visible Then
            Call SetDefaultTime
        End If
    End If
...
Private Sub chk补发_Click()
    If Me.Visible Then
        Call SetDefaultTime
    End If
End Sub

Private Sub chk收回_Click()
    If Me.Visible Then
        Call SetDefaultTime
    End If
End Sub
...
Private Sub Form_Load()
    ...
    '缺省时间模式
    If mint类型 = 3 Then
        strTmp = zldatabase.GetPara("医嘱缺省校对时间", glngSys, p住院医嘱发送, "001", Array(fraCheckDate, opt校对时间(1), opt校对时间(2), fra开始时间, lbl条件(0), lbl条件(1), cboTime(0), cboTime(1)), InStr(GetInsidePrivs(p住院医嘱发送), "修改校对时间") > 0)
        For i = cboTime.LBound To cboTime.UBound
            cboTime(i).AddItem "开始时间"
            cboTime(i).AddItem "开嘱时间"
        Next
        If Len(strTmp) = 1 Then strTmp = strTmp & "01"
        If Left(strTmp, 1) = "0" Then
            opt校对时间(2).value = True '当前时间
            fra开始时间.Visible = False
        Else
            opt校对时间(1).value = True
            fra开始时间.Visible = True
        End If
        cboTime(0).ListIndex = IIF(Mid(strTmp, 2, 1) = "1", 1, 0)
        cboTime(1).ListIndex = IIF(Mid(strTmp, 3, 1) = "1", 1, 0)
    ElseIf (mint类型 = 1 Or mint类型 = 7) Then
        strTmp = zldatabase.GetPara("医嘱缺省停止时间", glngSys, p住院医嘱发送, "011", Array(opt停止时间(1), opt停止时间(2), txt时点, cbo时段, chk补发, chk收回), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
        mblnHaveAudit = HaveAuditPriv(UserInfo.姓名)
        mlng停嘱审核 = Val(zldatabase.GetPara("实习医生停止医嘱需要审核", glngSys, p住院医嘱下达))
        If Len(strTmp) = 1 Then strTmp = strTmp & "11"
        If Left(strTmp, 1) = "1" Then
            opt停止时间(1).value = True '上次执行时间
        Else
            opt停止时间(2).value = True
            chk补发.value = IIF(Mid(strTmp, 2, 1) = "1", 1, 0)
            chk收回.value = IIF(Mid(strTmp, 3, 1) = "1", 1, 0)
        End If
        
        lbl缺省时间.Caption = "终止时间(&T)"
        opt停止时间(1).Caption = "上次执行时间"
        opt停止时间(2).Caption = "指定时间"
    End If
    If (mint类型 = 1 Or mint类型 = 7) Then
    ElseIf mint类型 = 3 Then
        fra缺省时间.Top = lblPati.Height + lblPati.Top
        fra缺省时间.Width = fraCheckDate.Left + fraCheckDate.Width
        picPati.Height = fra缺省时间.Top + fra缺省时间.Height + 60
    Else
        picPati.Height = picPati.Height - fra缺省时间.Height
    End If
    If mint类型 = 3 Then
        lbl缺省时间.Visible = False
        opt停止时间(1).Visible = False
        opt停止时间(2).Visible = False
        txt时点.Visible = False
        cbo时段.Visible = False
        chk补发.Visible = False
        chk收回.Visible = False
        fraCheckDate.Visible = True
    Else
        fraCheckDate.Visible = False
    End If
    ...
    End If
    mblnStopNurseGrade = Val(zldatabase.GetPara("单独停止护理等级", glngSys, p住院医嘱下达)) = 1
    ...
    ElseIf (mint类型 = 1 Or mint类型 = 7) Then
        ...
        fra缺省时间.Visible = True
        cbo时段.Visible = True
        txt时点.Visible = True
        chk补发.Visible = opt停止时间(2).value
        chk收回.Visible = opt停止时间(2).value
        If mbln护士站 Then
            lbl医生.Visible = True
            cbo医生.Visible = True
        End If
...
Private Sub Form_Resize()
    ...
    lngTmp = optBaby(0).Width + optBaby(1).Width + optBaby(2).Width + 40
    fraBaby.Width = lngTmp
    fraBaby.Height = optBaby(0).Height
    optBaby(0).Left = 10
    Call SetCtrlPosOnLine(False, 0, lblPati, 30, fraBaby, 30, cmdAlley)
    Call SetCtrlPosOnLine(False, 0, optBaby(0), 10, optBaby(1), 10, optBaby(2))
    ...
    fra缺省时间.Left = 0
    fra缺省时间.Height = cbo时段.Height + 80
    fra缺省时间.Top = cmdAlley.Height + 80
    fra缺省时间.Width = Me.ScaleWidth
    
    lbl缺省时间.Left = 150
    lbl缺省时间.Top = 50
    Call SetCtrlPosOnLine(False, 0, lbl缺省时间, 30, opt停止时间(1), 10, opt停止时间(2), 10, cbo时段, 10, txt时点, 200, chk补发, 10, chk收回, 10, lbl医生, 10, cbo医生)
        
    cbo医生.Left = Me.ScaleWidth - cbo医生.Width - lbl缺省时间.Left
    lbl医生.Left = cbo医生.Left - lbl医生.Width - 100
    fraCheckDate.Left = 0
    fraCheckDate.Top = -30
    fraCheckDate.Width = Me.ScaleWidth
    fra开始时间.Width = Me.ScaleWidth
    fra开始时间.Top = cmdAlley.Height + 80
    fra开始时间.Height = cbo时段.Height + 20
    fra开始时间.Left = 0
    lbl校对时间.Top = 150
    lbl校对时间.Left = 150
    Call SetCtrlPosOnLine(False, 0, lbl校对时间, 30, opt校对时间(2), 10, opt校对时间(1), 50, fra开始时间)
    lbl条件(0).Top = 60
    lbl条件(0).Left = 0
    Call SetCtrlPosOnLine(False, 0, lbl条件(0), 15, cboTime(0), 50, lbl条件(1), 15, cboTime(1))
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If mint类型 = 3 Then
        Call zldatabase.SetPara("医嘱缺省校对时间", IIF(opt校对时间(2).value, 0, 1) & IIF(cboTime(0).ListIndex = -1, 0, cboTime(0).ListIndex) & IIF(cboTime(1).ListIndex = -1, 1, cboTime(1).ListIndex), glngSys, p住院医嘱发送, blnSetup)
    ElseIf mint类型 = 1 Or mint类型 = 7 Then
        If opt停止时间(1).value Then
            Call zldatabase.SetPara("医嘱缺省停止时间", "1", glngSys, p住院医嘱发送, blnSetup)
        Else
            Call zldatabase.SetPara("医嘱缺省停止时间", "0" & IIF(chk补发.value = 1, "1", "0") & IIF(chk收回.value = 1, "1", "0"), glngSys, p住院医嘱发送, blnSetup)
        End If
    End If
                ...
                If mint类型 = 3 Then
                    ...
                    Else
                        If Format(.Cell(flexcpData, i, COL_开始时间), "yyyy-MM-dd HH:mm") > Format(.Cell(flexcpData, i, COL_开嘱时间), "yyyy-MM-dd HH:mm") Then
                            .TextMatrix(i, COL_输入) = .Cell(flexcpData, i, IIF(cboTime(0).ListIndex = 0, COL_开始时间, COL_开嘱时间))
                        Else
                            .TextMatrix(i, COL_输入) = .Cell(flexcpData, i, IIF(cboTime(1).ListIndex = 0, COL_开始时间, COL_开嘱时间))
                        End If
                    End If
                Else
                    If opt停止时间(2).value Then
                        '当前时间或指定时间
...
Private Sub opt停止时间_Click(Index As Integer)
    If Me.Visible Then
        If (mint类型 = 1 Or mint类型 = 7) Then
            chk补发.Visible = opt停止时间(2).value
            chk收回.Visible = opt停止时间(2).value
...
Private Sub opt校对时间_Click(Index As Integer)
    If Index = 2 Then
        fra开始时间.Visible = False
    Else
        fra开始时间.Visible = True
...
Private Sub picPati_Resize()
    fraBaby.Left = Me.Width - fraBaby.Width - 350 - IIF(cmdAlley.Visible, cmdAlley.Width + 100, 0)
End Sub
...
'功能：初始化医嘱清单格式
    ...
    strHead = "ID;相关ID;组ID;序号;诊疗类别;毒理分类;中药;,240,4;,300,4;,1530,1;" & _
        "标志,500,4;姓名,750,1;住院号,750,1;床号,500,1;婴儿,500,1;期效,500,4;开嘱时间,1530,1;开始时间,1530,1;" & _
        ...
        "执行科室ID;病人科室ID;收费细目ID;单量单位;前提ID;签名ID;操作人员;开嘱科室ID;操作说明;执行分类;标本部位;审核状态"
...
'功能：设置一些可见列及编辑属性,应在表格数据装入后调用
    With vsAdvice
        ...
        '可输入列标识
        .Cell(flexcpBackColor, .FixedRows, COL_选择, .Rows - 1, COL_输入) = COLEditBackColor       '浅绿
    End With
End Sub
...
'参数：str病人IDs=用于返回实际有数据的病人串:"病人ID,主页ID,..."
    ...
    If mint类型 = 3 Then
        If InStr(GetInsidePrivs(p住院医嘱发送), "全院医嘱校对") = 0 Then
            strWhere = strWhere & " And A.开嘱科室ID In (Select Column_Value From Table(f_Num2list([4])))"
        End If
        ...
        If gblnKSSStrict Or gbln手术分级管理 Or gbln输血分级管理 Or gbln血库系统 Then
            strWhere = strWhere & " And Nvl(A.审核状态,0) Not in(1,3,7" & IIF(gbln血库系统 = True, "", ",4,5") & ")"
        End If
    End If
    ...
    Else
        If (mint类型 = 1 Or mint类型 = 7) Then
            If Not mblnStopNurseGrade Then '不允许单独停止护理等级
    ...
    For k = 0 To UBound(Split(mstr病人IDs, ";"))
        ...
        strSql = _
            "Select /*+ RULE */ A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,A.序号,Nvl(A.诊疗类别,'*') as 诊疗类别,C.毒理分类,NULL as 中药," & _
                " A.审查结果,NULL as 选择,NULL as 输入,Decode(A.紧急标志,1,'紧急',2,'补录','普通') as 标志,A.姓名,P.住院号,P.当前床号 as 床号," & _
                ...
                " b.执行分类,A.标本部位,a.审核状态," & IIF(mint类型 = 3, "Decode(a.医嘱状态,1,Decode(a.校对护士,Null,0,1),0) as 疑问更正", "Null as 疑问更正") & ",D.高危药品"
        strSql = strSql & _
        ...
        Set rsTmp = zldatabase.OpenSQLRecord(strSql, Me.Caption, lng病人ID, lng主页ID, mstrAdviceOfItem, strDepts, mlng病区ID, mlng医护科室ID)
                    ...
                    If Not (bln给药途径 Or bln输血途径) And .TextMatrix(i, COL_诊疗类别) <> "7" Then
                        ...
                        If (mint类型 = 1 Or mint类型 = 7) Then
                            ...
                            If mint类型 = 7 Then
                                ...
                                If rsTmp!操作说明 & "" <> "" Then
                                    .TextMatrix(i, COL_输入) = Format(rsTmp!操作说明 & "", "yyyy-MM-dd HH:mm")
                                End If
                            End If
                            ...
                            Else
                                If Format(.TextMatrix(i, COL_输入), "yyyy-MM-dd HH:mm") < Format(.Cell(flexcpData, i, COL_上次执行), "yyyy-MM-dd HH:mm") Then
                                    ...
                                    If InStr(",5,6,", .TextMatrix(i, COL_诊疗类别)) > 0 And strUnRoll <> "" Then
                                        ...
                                        Else
                                            lng药品ID = GetLastSendMediCineID(Val(.TextMatrix(i, COL_ID)), CDate(.Cell(flexcpData, i, COL_上次执行)))
                                        End If
                                        If lng药品ID <> 0 Then
...
Private Function ExecuteOperate() As Boolean
    ...
    Dim strMsg As String, varTmp As Variant
                    ...
                    If blnExe Then
                        ...
                        If mint类型 = 0 Then      '医嘱作废
                            ...
                            If .TextMatrix(i, COL_诊疗类别) = "K" And gbln血库系统 Then
                                If Val(.TextMatrix(i, COL_审核状态)) = 2 Then
                                    On Error GoTo errH
                        ...
                        ElseIf (mint类型 = 1 Or mint类型 = 7) Then '停止医嘱
                            ...
                            arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_停止(" & lng医嘱ID & "," & _
                                ...
                                "'" & IIF(mbln护士站, NeedName(cbo医生.Text), UserInfo.姓名) & "',0," & IIF(mblnHaveAudit, 1, 0) & "," & mlng停嘱审核 & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
                            '跟主界面显示相关的医嘱停止
                            If .TextMatrix(i, COL_诊疗类别) = "H" And .TextMatrix(i, COL_操作类型) = "1" _
                                ...
                                mblnRefresh = True
                            End If
                            rsMsgRow.Filter = "病人id=" & Val(.TextMatrix(i, COL_病人ID)) & " And 主页id=" & Val(.TextMatrix(i, COL_主页ID)) & " And 操作类型=1"
                            If rsMsgRow.EOF Then
                                rsMsgRow.AddNew
                                rsMsgRow!病人ID = Val(.TextMatrix(i, COL_病人ID))
                                rsMsgRow!主页ID = Val(.TextMatrix(i, COL_主页ID))
...
Private Sub vsPrice_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsPrice
        If Col = COLP_收费项目 Then
            ...
            End If
            strSql = strSql & " ) A,(" & strStock & ") S Where A.ID=S.药品ID(+)" & _
            " Group by A.末级,A.ID,A.上级ID,A.编码,A.名称,A.单位,A.规格,A.产地,A.类别,A.费用类型,A.医保大类,A.说明,A.是否变价,A.类别ID,A.跟踪在用ID,A.住院单位,A.住院包装,S.库存"
            
            Set rsTmp = zldatabase.ShowSQLSelect(Me, strSql, 2, "收费项目", False, "", "", False, False, False, 0, 0, 0, blnCancel, False, False, _
                "," & str项目IDs & ",", mint险类, mlng西药房, mlng成药房, mlng中药房, mlng发料部门, Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_病人科室ID)))
        ...
        If KeyAscii = 13 Then
            ...
            ElseIf Col = COLP_收费项目 And .EditText <> "" Then
                ...
                Set rsTmp = zldatabase.ShowSQLSelect(Me, strSql, 0, "收费项目", False, "", "", False, False, True, vPoint.X, vPoint.Y, .CellHeight, blnCancel, False, True, _
                    strInput & "%", mstrLike & strInput & "%", mint简码 + 1, "," & str项目IDs & ",", mint险类, mlng西药房, mlng成药房, mlng中药房, mlng发料部门, Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_病人科室ID)))
                If Not rsTmp Is Nothing Then
                    '医保对码检查
                    If CheckItemInsure(rsTmp, Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_病人ID)), Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_主页ID))) Then
                        .TextMatrix(Row, Col) = CStr(.Cell(flexcpData, Row, Col))
            ...
            If InStr(",1,2,", Nvl(rsMan!医嘱状态, 0)) > 0 And InStr(",4,5,6,7,", rsMan!诊疗类别) = 0 Then '包含非药品、卫材医嘱的药品计价
                ...
                If blnLoad Then
                    ...
                    strSql = _
                        ...
                        " Order by 费用性质,从属项目"
                    Set rsAdd = zldatabase.OpenSQLRecord(strSql, Me.Caption, Val(rsMan!ID), lng材料ID, mlng病区ID)
...
'            End If
            ...
            If str收费细目IDs <> "" Then
                strSql = _
                    ...
                    " A.费用类型,A.是否变价,Nvl(D.住院包装,1) as 住院包装" & _
                ...
                Set rs收费细目 = zldatabase.OpenSQLRecord(strSql, Me.Caption, str收费细目IDs, IIF(gbyt药品名称显示 = 0, 1, 3)) 'In
            End If
                ...
                If Not IsNull(mrsPrice!收费细目ID) Then
                    ...
                    If Nvl(rs收费细目!是否变价, 0) = 1 Then
                        If InStr(",5,6,7,", mrsPrice!收费类别) > 0 Then
                            ...
                            Else
                                '非药嘱药品按零售单位计算
                                .TextMatrix(j, COLP_单价) = Format(CalcDrugPrice(rs收费细目!ID, lng执行科室ID, mrsPrice!数量), gstrDecPrice)
                            End If
...
'说明：
    ...
    If mint类型 = 3 Then
        ...
        strSql = "select a.id as 医嘱ID,nvl(a.紧急标志,0) as 紧急,a.病人ID,a.主页ID from 病人医嘱记录 a where A.医嘱状态=1" & strWhere & _
