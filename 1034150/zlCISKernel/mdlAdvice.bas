...
Public Function Get病种ID(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng科室id As Long, Optional ByRef bln中医 As Boolean = False) As ADODB.Recordset
    ...
    If bln中医 Then
        strSql = "Select 疾病id, 诊断id, 诊断描述,诊断类型 " & vbNewLine & _
                ...
                "      Nvl(是否疑诊, 0) = 0" & vbNewLine & _
                "Order By Decode(诊断类型, 12, 1, 2, 2, 11, 3, 1, 4), Decode(记录来源, 1, 4, 记录来源) Desc"
    Else
        strSql = "Select 疾病id, 诊断id, 诊断描述,诊断类型 " & vbNewLine & _
            "From 病人诊断记录" & vbNewLine & _
            "Where 记录来源 In (1, 2, 3) And 诊断类型 In (1, 2, 11, 12) And 取消时间 Is Null And 病人id = [1] And 主页id = [2] And 诊断次序 = 1 And Nvl(是否疑诊,0) = 0" & vbNewLine & _
...
'功能：病人的当前费用是否有效，返回true表明当前费别可用
    Dim rsTmp As ADODB.Recordset, strSql As String
    On Error GoTo errH
    strSql = "Select 1 From 病案主页 A, 费别 B Where a.费别=b.名称 And a.病人id=[1] And a.主页id=[2] And Sysdate Between Nvl(b.有效开始,Sysdate) And Nvl(b.有效结束,Sysdate)"
