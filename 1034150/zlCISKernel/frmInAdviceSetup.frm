...
Begin VB.Form frmInAdviceSetup 
   ...
   Begin TabDlg.SSTab tabPar 
      ...
      Tab(0).Control(1)=   "cbo卫材"
      Tab(0).Control(1).Enabled=   0   'False
      Tab(0).Control(2)=   "fra医嘱下达"
      Tab(0).Control(2).Enabled=   0   'False
      Tab(0).Control(3)=   "fra入院诊断"
      Tab(0).Control(3).Enabled=   0   'False
      Tab(0).Control(4)=   "vsfDrugStore"
      Tab(0).Control(4).Enabled=   0   'False
      Tab(0).Control(5)=   "lbl缺省药房"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).Control(6)=   "lbl卫材"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "lbl可用药房"
    ...
    If mint场合 = 1 Then
        ...
        For i = 0 To lst发药类型.ListCount - 1
            If lst发药类型.Selected(i) Then
                strValue = strValue & "," & NeedName(lst发药类型.List(i))
            End If
        Next
        ...
        Call zlDatabase.SetPara("住院领药部门", IIF(opt领药部门(1).value, 1, 0), glngSys, p住院医嘱发送, blnSetPara)
        Call zlDatabase.SetPara("检验医嘱单独产生单据", chk检验单据.value, glngSys, p住院医嘱发送, blnSetPara)
    End If
...
Private Sub Form_Load()
    ...
    If mint场合 = 1 Then
        ...
        Else
            ...
            If chkInfo(chk长嘱口服药发送结束时间).value = 1 Then
                ...
                dtpEnd.Enabled = chkInfo(chk长嘱口服药发送结束时间).Enabled
            End If
        End If
    Else
    ...
    End If
    chk检验单据.value = Val(zlDatabase.GetPara("检验医嘱单独产生单据", glngSys, p住院医嘱发送, , Array(chk检验单据), bln发送设置))
    ...
    opt领药部门(Val(zlDatabase.GetPara("住院领药部门", glngSys, p住院医嘱发送, "0", Array(opt领药部门(0), opt领药部门(1)), bln发送设置))).value = True
