...
Private mstrInfDepIDs As String  '在输液配置中心发药的病病区
...
Private mint输液配置期效 As Integer '配置中心的医嘱期效
...
Private Sub cboTime_Click()
    ...
    Case 3 '[指定..]
        strTmp = zldatabase.GetPara("发送结束时点", glngSys, p住院医嘱发送, "23:59:59", Array(dtpEnd), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
        lngTmp = Val(zldatabase.GetPara("发送时间间隔", glngSys, p住院医嘱发送, "0", Array(dtpEnd), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0))
...
Private Sub Form_Load()
    ...
    If Not PatiFeeUsable(mlng病人ID, mlng主页ID) Then
        Unload Me: Exit Sub
    End If
    ...
    mlng中药房 = Val(zldatabase.GetPara("住院缺省中药房", glngSys, p住院医嘱下达))
    mlng西药房 = Val(zldatabase.GetPara("住院缺省西药房", glngSys, p住院医嘱下达))
    mlng成药房 = Val(zldatabase.GetPara("住院缺省成药房", glngSys, p住院医嘱下达))
    mlng发料部门 = Val(zldatabase.GetPara("住院缺省发料部门", glngSys, p住院医嘱下达))
    '自动处理皮试
    mbln自动皮试 = Val(zldatabase.GetPara("自动处理皮试", glngSys, p住院医嘱下达)) <> 0
    ...
    mbln接收自备药 = Val(zldatabase.GetPara("特殊性质药品允许发送到配制中心", glngSys, p输液配置中心, "0")) = 1
...
Private Function Load诊疗类别() As Boolean
    ...
    str类别s = zldatabase.GetPara("其他发送诊疗类别", glngSys, p住院医嘱发送, "", Array(lstClass), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
...
Private Function Load给药途径() As Boolean
    ...
    On Error GoTo errH
    str给药IDs = zldatabase.GetPara("药嘱发送给药途径", glngSys, p住院医嘱发送, "", Array(lvwWay), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
...
Private Sub Show药房()
    ...
    str置换 = zldatabase.GetPara("药嘱发送药房置换", glngSys, p住院医嘱发送, "", Array(vsDept), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
    arr置换 = Split(str置换, ",")
...
'参数：lngModle=0 读取常规医嘱，=1读取备用医嘱
    ...
    Dim i As Long, blnSetPar As Boolean
    ...
    If Me.Visible Then  '自动读取(医嘱提醒)时不进行保存操作
        blnSetPar = InStr(GetInsidePrivs(p住院医嘱发送), ";医嘱选项设置;") > 0
        
        Call zldatabase.SetPara("发送结束时间", cboTime.ListIndex, glngSys, p住院医嘱发送, blnSetPar)
        If cboTime.ListIndex = cboTime.ListCount - 1 Then
            Call zldatabase.SetPara("发送结束时点", Format(dtpEnd.value, "HH:mm:ss"), glngSys, p住院医嘱发送, blnSetPar)
            Call zldatabase.SetPara("发送时间间隔", Int(CDate(Format(dtpEnd.value, "yyyy-MM-dd")) - CDate(Format(mdatCurr, "yyyy-MM-dd"))), glngSys, p住院医嘱发送, blnSetPar)
        End If
        
        '单病人模式不保存
        If Not mblnOnePati Then
            Call zldatabase.SetPara("发送病人", strSel, glngSys, p住院医嘱发送, blnSetPar)
        End If
        ...
        If Not (mblnAutoVerify And mblnOnePati) Then
            ...
            Else
                strTmp = "-1"
            End If
            Call zldatabase.SetPara("药嘱发送医嘱期效", strTmp, glngSys, p住院医嘱发送, blnSetPar)
            ...
            Else
                strTmp = "-1"
            End If
            Call zldatabase.SetPara("其他发送医嘱期效", strTmp, glngSys, p住院医嘱发送, blnSetPar)
        End If
        
        
        If chk药品期效(0).Visible And chk药品期效(0).Enabled Or (chk药品期效(1).Visible And chk药品期效(1).Enabled) Then
            Call zldatabase.SetPara("药嘱发送给药途径", str给药IDs, glngSys, p住院医嘱发送, blnSetPar)
            ...
            Next
            strTmp = Mid(strTmp, 2)
            Call zldatabase.SetPara("药嘱发送药房置换", strTmp, glngSys, p住院医嘱发送, blnSetPar)
        End If
        ...
        If chk其他期效(0).Visible And chk其他期效(0).Enabled Or (chk其他期效(1).Visible And chk其他期效(1).Enabled) Then
            If chk其他期效(0).value = 1 Or chk其他期效(1).value = 1 Then
                Call zldatabase.SetPara("其他发送诊疗类别", Replace(str类别s, "'", "''"), glngSys, p住院医嘱发送, blnSetPar)
            End If
        End If
        
        If chkSendNow.Enabled Then Call zldatabase.SetPara("药品长嘱从当天开始发送", chkSendNow.value, glngSys, p住院医嘱发送, blnSetPar)
    ...
    With vsPrice
        ...
        If strSQL <> "" Then
            strSQL = "Select A.行号,A.收费细目ID,A.固定,A.从项,A.计价医嘱,A.类别,C.名称 as 类别名称,A.执行科室ID,G.名称 as 执行科室," & _
                ...
                " And A.收费细目ID=E.收费细目ID(+) And E.码类(+)=1 And E.性质(+)=" & IIF(gbyt药品名称显示 = 0, 1, 3) & _
                " Order by A.序号"
            ...
            For i = 1 To rsTmp.RecordCount
                ...
                If InStr(",5,6,7,", rsTmp!类别) > 0 Then
                    ...
                    Else
                        If InStr(",5,6,7,", vsAdvice.TextMatrix(rsTmp!行号, COL_诊疗类别)) > 0 Then
                            dbl当前单价 = CalcDrugPrice(rsTmp!收费细目ID, Nvl(rsTmp!执行科室ID, 0), Format(int付数 * Nvl(rsTmp!数量, 0) * Nvl(rsTmp!住院包装, 1), gstrDecPrice), , True)
                        Else
                            dbl当前单价 = CalcDrugPrice(rsTmp!收费细目ID, Nvl(rsTmp!执行科室ID, 0), Format(int付数 * Nvl(rsTmp!数量, 0), gstrDecPrice), , True)
                        End If
                    End If
                    If InStr(",5,6,7,", vsAdvice.TextMatrix(rsTmp!行号, COL_诊疗类别)) > 0 Then
...
Private Sub Init医嘱主要条件()
    ...
    Dim blnSetPar As Boolean, strPrivs As String
    
     '可以进入这项表示两者至少有一个
    strPrivs = GetInsidePrivs(p住院医嘱发送)
    blnSetPar = InStr(strPrivs, "医嘱选项设置") > 0
    ...
    ElseIf InStr(strPrivs, ";发送其他临嘱;") = 0 And InStr(strPrivs, ";发送其他长嘱;") = 0 Then
        ...
        cbo药房.Top = lblEatDrug.Top + IIF(lblEatDrug.Visible, 250, 0)
    ...
    Else
        '缺省药品医嘱期效
        lngTmp = Val(zldatabase.GetPara("药嘱发送医嘱期效", glngSys, p住院医嘱发送, "2", Array(chk药品期效(0), chk药品期效(1)), blnSetPar))
        ...
        End If
        '缺省其他医嘱期效
        lngTmp = Val(zldatabase.GetPara("其他发送医嘱期效", glngSys, p住院医嘱发送, "2", Array(chk其他期效(0), chk其他期效(1)), blnSetPar))
        If lngTmp = 0 Or lngTmp = 2 Then
            If chk其他期效(0).Enabled Then chk其他期效(0).value = 1
        End If
        If lngTmp = 1 Or lngTmp = 2 Then
    ...
    lngTmp = Val(zldatabase.GetPara("药品长嘱从当天开始发送", glngSys, p住院医嘱发送, "0", Array(chkSendNow), blnSetPar))
    ...
    '发送结束时间
    strTmp = zldatabase.GetPara("发送结束时间", glngSys, p住院医嘱发送, "0", Array(cboTime), blnSetPar)
    cboTime.ListIndex = Val(strTmp)
    If cboTime.ListIndex = cboTime.ListCount - 1 Then
        mdatCurr = zldatabase.Currentdate
        strTmp = zldatabase.GetPara("发送结束时点", glngSys, p住院医嘱发送, "23:59:59", Array(dtpEnd), blnSetPar)
        lngTmp = Val(zldatabase.GetPara("发送时间间隔", glngSys, p住院医嘱发送, "0", Array(dtpEnd), blnSetPar))
        mblnIsSendInterval = dtpEnd.Enabled
        dtpEnd.value = Format(mdatCurr + lngTmp, "yyyy-MM-dd " & strTmp)
    End If
    Call SetFaceBy医嘱期效
...
'功能：更改成药规格
    ...
    ElseIf Col = COL_规格 Then
        With vsAdvice
            ...
            '医嘱相关信息
            strSQL = _
                " Select A.ID,A.诊疗类别,A.开始执行时间,A.上次执行时间,A.执行终止时间,A.执行时间方案," & _
                " A.频率次数,A.频率间隔,A.间隔单位,A.单次用量,A.可否分零,B.入院日期,A.医嘱状态,A.首次用量" & _
...
Private Sub vsPrice_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsPrice
        ...
        If Col = COLP_收费项目 Then
            ...
            End If
            strSQL = strSQL & " ) Group by 末级,ID,上级ID,类别,编码,名称,单位,规格,产地,费用类型,医保大类,说明,是否变价,类别ID,跟踪在用ID"
            
            Set rsTmp = zldatabase.ShowSQLSelect(Me, strSQL, 2, "收费项目", False, "", "", False, False, False, 0, 0, 0, blnCancel, False, False, "," & str项目IDs & ",", Val(vsAdvice.TextMatrix(lng行号, COL_险类)), Val(vsAdvice.TextMatrix(lng行号, COL_病人科室ID)))
            If Not rsTmp Is Nothing Then
                '非本科执行的医嘱不允许输入变价项目
                If lng行号 <> 0 Then
        ...
        If KeyAscii = 13 Then
            ...
            ElseIf Col = COLP_收费项目 And .EditText <> "" Then
                ...
                Set rsTmp = zldatabase.ShowSQLSelect(Me, strSQL, 0, "收费项目", False, "", "", False, False, True, vPoint.X, vPoint.Y, .CellHeight, blnCancel, False, True, _
                    strInput & "%", mstrLike & strInput & "%", mint简码 + 1, "," & str项目IDs & ",", Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_险类)), Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_病人科室ID)), mlng西药房, mlng成药房, mlng中药房, mlng发料部门)
                If Not rsTmp Is Nothing Then
                    '非本科执行的医嘱不允许输入变价项目
                    If lng行号 <> 0 Then
                        If Nvl(rsTmp!是否变价ID, 0) = 1 And Not (InStr(",5,6,7,", rsTmp!类别ID) > 0 Or rsTmp!类别ID = "4" And Nvl(rsTmp!跟踪在用ID, 0) = 1) Then
...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;婴儿,550,1;期效,550,1;医嘱内容,2000,1;规格,2000,1;总量,600,7;单位,450,1;" & _
        ...
        "紧急标志;医嘱状态;执行频率;新开操作时间;计算方式;执行安排;病人性质;开嘱时间;开始时间;执行分类"
...
'说明：SQL查询中,"病人医嘱记录"别名为"A"
    ...
    mbln药房置换 = False
    If chkDeptExchange.value = 1 Then
        mrs药房.Filter = 0
        If Not mrs药房.EOF Then
            For i = 1 To mrs药房.RecordCount
                    ...
                    If bln分批时价 Then
                        If InStr(GetInsidePrivs(p住院医嘱发送), "显示药品库存") = 0 Then
                            strTmp = "医嘱""" & .TextMatrix(lngRow, col_医嘱内容) & """的分批或时价计价项目：" & vbCrLf & vbCrLf & _
                                """" & rsPrice!名称 & """在" & Get部门名称(lng库房ID) & "库存不足" & _
...
'      str分解时间=具体的执行时间分解
    ...
    If bln口服 Then strEnd = Split(strEnd, " ")(0) & " " & mstrEndPoint
...
Private Function GetWhere(ByVal bytMode As Byte)
...
'参数：0-校对，1=发送
    ...
    If blnDo Then
        strTmp = " And A.开嘱科室ID In (Select /*+cardinality(E,10)*/ Column_Value From Table(f_Num2list([4])) E) "
    End If
    
    GetWhere = strTmp
End Function

Private Function CheckSendPrivs(ByVal lng医嘱ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
'功能：判断当前医嘱行的开嘱科室是否是本病区所属科室
    Dim rsTmp As ADODB.Recordset, strSQL As String
    Dim strDepts As String
    
    strDepts = Get会诊执行科室IDs(lng病人ID, lng主页ID)
    
    strDepts = IIF(strDepts = "", "", strDepts & ",") & GetUser科室IDs(True) '当前操作人员的所属病区的所有科室'

    strSQL = " Select 1 From 病人医嘱记录 D Where D.ID = [1] And D.开嘱科室ID In (Select Column_Value From Table(f_Num2list([2])))"
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng医嘱ID, strDepts)
...
Private Function Get会诊执行科室IDs(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As String
'功能：获取当前病人请会诊科室ID串
'说明利用缓存，当都传入0时表示清除缓存
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String
    ...
    If InStr(strPati, "," & lng病人ID & "," & lng主页ID & ":") > 0 Then
        Get会诊执行科室IDs = Split(strPati, ":")(1)
        Exit Function
    End If
    
    strSQL = "select a.执行科室ID as ID from 病人医嘱记录 a,诊疗项目目录 b where a.诊疗项目id=b.id and b.类别='Z' and b.操作类型='7' and a.医嘱状态=8 and a.病人id=[1] and a.主页id=[2]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病人ID, lng主页ID)
    strTmp = GetUser科室IDs(True)
    Do While Not rsTmp.EOF
        If InStr("," & strDepts & ",", "," & rsTmp!ID & ",") = 0 And InStr("," & strTmp & ",", "," & rsTmp!ID & ",") = 0 Then
            strDepts = strDepts & "," & rsTmp!ID
        End If
        rsTmp.MoveNext
    Loop
    strPati = "," & lng病人ID & "," & lng主页ID & ":" & Mid(strDepts, 2)
    
    Get会诊执行科室IDs = Mid(strDepts, 2)
...
'功能：按病人读取医嘱发送清单
    ...
    If bln药品 Then
        ...
        strSQL药品 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.可否分零,A.计价特性,A.执行性质,A.执行标记," & str药房置换 & " as 执行科室ID,Nvl(F.名称,Decode(Nvl(A.执行性质,0),5,'-')) as 执行科室,A.摘要,A.医嘱状态,A.医嘱期效,A.首次用量,E.计算方式,e.执行安排" & _
            " From 病人医嘱记录 A,病案主页 B,病人信息 C,部门表 D,诊疗项目目录 E,部门表 F,病人医嘱记录 X" & IIF(mblnAutoVerify, ",病人医嘱状态 S", "") & _
    ...
    If bln其他 Then
        ...
        strSQL其他 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " Decode(A.诊疗类别,'4',H.计算单位,E.计算单位) as 计算单位,A.执行终止时间,b.病人性质,a.开嘱时间," & _
            " A.执行频次,Decode(A.执行频次,'必要时',1,A.频率次数) As 频率次数,Decode(A.执行频次,'必要时',1,A.频率间隔) As 频率间隔,Decode(A.执行频次,'必要时','天',A.间隔单位) as 间隔单位,A.医生嘱托," & _
            ...
            " A.执行性质,A.执行科室ID,Nvl(F.名称,Decode(Nvl(A.执行性质,0),5,'-')) as 执行科室,H.是否变价,I.跟踪在用,A.紧急标志,A.医嘱状态,A.医嘱期效,h.撤档时间,A.首次用量,E.计算方式,e.执行安排" & _
    ...
    For k = 0 To UBound(arrPati)
        strTmp1 = Get会诊执行科室IDs(Val(arrPati(k)), arrPatiPage(k))
        If strTmp1 <> "" Then
            strDepts = strDepts & "," & strTmp1
        End If
        If bln药品 Then
            str输液药品排除 = ""
            ...
            If lngModle = 0 Then
                If gstr输液配置中心 <> "" Then
                    If bln可接收病区 Then
                        str输液药品排除 = " And " & _
                                    "(Instr(',' || [9] || ',',',' || Nvl(a.执行科室id, 0) || ',') = 0 " & _
                                    IIF(mbln接收自备药, " And not(a.执行性质 In (0, 5) Or a.执行标记 > 0)", "") & " Or a.诊疗类别 Not In ('5', '6') Or Not Exists" & vbNewLine & _
                                    "       (Select 1" & vbNewLine & _
                                    "        From 诊疗项目目录 Y" & vbNewLine & _
                                    "        Where x.诊疗项目id = y.Id And y.类别 = 'E' And y.操作类型 = '2' And Nvl(y.执行分类, 0) = 1) Or" & vbNewLine & _
                                    "       Instr(',' || [9] || ',',',' || Nvl(a.执行科室id, 0) || ',') > 0 And Exists" & vbNewLine & _
                                    "       (Select 1 From 病人医嘱记录 Z Where z.相关id = a.相关id And (Instr(',' || [9] || ',',',' || Nvl(Z.执行科室id, 0) || ',') = 0 " & IIF(mbln接收自备药, " And Not", " Or") & " (z.执行性质 In (0, 5) Or z.执行标记 > 0))) " & _
                                    IIF(mstr配液给药途径 = "", "", " Or instr(',' || [10]  || ',',',' || x.诊疗项目ID || ',')=0 ") & _
                                    IIF(mbln接收自备药, "", " Or (x.执行性质 In (0, 5) Or x.执行标记 > 0)") & "  Or instr([11],x.医嘱期效)=0 ) And" & vbNewLine & _
                                    "      (Exists" & vbNewLine & _
                                    "       (Select 1 From 病人医嘱记录 Z Where z.诊疗类别 In ('5', '6') And z.相关id = a.Id And (Instr(',' || [9] || ',',',' || Nvl(Z.执行科室id, 0) || ',') = 0 " & IIF(mbln接收自备药, " And Not", " Or") & " (z.执行性质 In (0, 5) Or z.执行标记 > 0))) Or" & vbNewLine & _
                                    "       Not (E.类别 = 'E' And E.操作类型 = '2' And Nvl(E.执行分类, 0) = 1) " & _
                                    IIF(mstr配液给药途径 = "", "", " Or instr(',' || [10]  || ',',',' || a.诊疗项目ID || ',')=0 ") & _
                                    IIF(mbln接收自备药, "", " Or (a.执行性质 In (0, 5) Or a.执行标记 > 0)") & " Or instr([11],a.医嘱期效)=0)"
                        str输液药品排除 = str输液药品排除 & " And NVL(E.执行标记,0)<>2 And (Not Exists(Select 1 From 诊疗项目目录 Y Where X.诊疗项目id = y.Id And NVL(Y.执行标记,0)=2) OR x.诊疗项目id is null)"
                    Else
                ...
                End If
            End If
            strNoneIDs = GetNoneSendID(Val(arrPati(k)), arrPatiPage(k), mbln自动皮试, 2)
            strSQL药品 = strtmp3
            strSQL药品 = strSQL药品 & IIF(strNoneIDs <> "", "  And Instr([8],','||A.ID||',')=0  ", "") & str输液药品排除 & " Order by A.医嘱期效,A.婴儿,组号,组ID,A.序号"
            
            Set rsSend药品 = zldatabase.OpenSQLRecord(strSQL药品, Me.Caption, datEnd, Val(arrPati(k)), lng药房ID, strDepts, mlng病区ID, Format(dkpExecTime.value, "YYYY-MM-DD HH:mm"), Format(dkpExecTime.value, "HH:mm"), "," & strNoneIDs & ",", gstr输液配置中心, mstr配液给药途径, IIF(mint输液配置期效 = -1, "01", mint输液配置期效))
        End If
        If bln其他 Then
            Set rsSend其他 = zldatabase.OpenSQLRecord(strSQL其他, Me.Caption, datEnd, Val(arrPati(k)), lng执行科室ID, strDepts, mlng病区ID, Format(dkpExecTime.value, "YYYY-MM-DD HH:mm"), Format(dkpExecTime.value, "HH:mm"))
        End If
...
'   COL_金额：存放计价内容是否变更过
    ...
    With vsAdvice
        ...
        For i = 1 To rsSend.RecordCount
            ...
            .TextMatrix(lngRow, COL_操作类型) = rsSend!操作类型 & ""
            .TextMatrix(lngRow, COL_执行分类) = rsSend!执行分类 & ""
        ...
        If InStr(",5,6,7,", .TextMatrix(lngRow, COL_诊疗类别)) > 0 Then
            ...
            If Val(.TextMatrix(lngRow, COL_执行性质ID)) <> 5 Then
                ...
                If Nvl(rsDrug!是否变价, 0) = 0 Then
                    mrsPrice!单价 = Format(CalcPrice(rsDrug!药品ID, , , True), gstrDecPrice)
                Else '以售价计算药品时价,自备药时无对应药房
                    mrsPrice!单价 = Format(CalcDrugPrice(rsDrug!药品ID, Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量, , True), gstrDecPrice)
                End If
                mrsPrice.Update
                ...
                If .TextMatrix(lngRow, COL_费别) <> "" Then
                    If Nvl(rsDrug!是否变价, 0) = 0 Then
                        cur金额 = Format(CalcPrice(rsDrug!药品ID, .TextMatrix(lngRow, COL_费别), dbl数量, , Val(.TextMatrix(lngRow, COL_执行科室ID))), gstrDec)
                    Else
                        cur金额 = Format(CalcDrugPrice(rsDrug!药品ID, Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量, .TextMatrix(lngRow, COL_费别)), "0.00000")
                    End If
                Else
                    If gbln加班加价 Then
                        '处理加班加价
                        If Nvl(rsDrug!是否变价, 0) = 0 Then
                            dbl单价 = Format(CalcPrice(rsDrug!药品ID), gstrDecPrice)
                        Else '以售价计算药品时价,自备药时无对应药房
                            dbl单价 = Format(CalcDrugPrice(rsDrug!药品ID, Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量), gstrDecPrice)
                        End If
        ...
        ElseIf .TextMatrix(lngRow, COL_诊疗类别) = "4" Then
            ...
            If Val(.TextMatrix(lngRow, COL_执行性质ID)) <> 5 Then
                ...
                If Val(.TextMatrix(lngRow, COL_是否变价)) = 0 Then
                    '定价卫材
                    mrsPrice!单价 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), , , True), gstrDecPrice)
                ElseIf Val(.TextMatrix(lngRow, COL_跟踪在用)) = 0 Then
                    '非跟踪在用的时价卫材，价格可能已保存在病人医嘱计价中
                    mrsPrice!单价 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), , , True, , Val(.TextMatrix(lngRow, COL_ID))), gstrDecPrice)
                Else
                    '计算跟踪在用卫材时价
                    mrsPrice!单价 = Format(CalcDrugPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量, , True), gstrDecPrice)
                End If
                mrsPrice.Update
                ...
                If .TextMatrix(lngRow, COL_费别) <> "" Then
                    If Val(.TextMatrix(lngRow, COL_是否变价)) = 0 Then
                        cur金额 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), .TextMatrix(lngRow, COL_费别), dbl数量, , Val(.TextMatrix(lngRow, COL_执行科室ID))), gstrDec)
                    ElseIf Val(.TextMatrix(lngRow, COL_跟踪在用)) = 0 Then
                        cur金额 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), .TextMatrix(lngRow, COL_费别), dbl数量, , Val(.TextMatrix(lngRow, COL_执行科室ID)), Val(.TextMatrix(lngRow, COL_ID))), gstrDec)
                    Else
                        cur金额 = Format(CalcDrugPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量, .TextMatrix(lngRow, COL_费别)), "0.00000")
                    End If
                Else
                    If gbln加班加价 Then
                        '处理加班加价
                        If Val(.TextMatrix(lngRow, COL_是否变价)) = 0 Then
                            dbl单价 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID))), gstrDecPrice)
                        ElseIf Val(.TextMatrix(lngRow, COL_跟踪在用)) = 0 Then
                            dbl单价 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), , , , , Val(.TextMatrix(lngRow, COL_ID))), gstrDecPrice)
                        Else '以售价计算药品时价,自备药时无对应药房
                            dbl单价 = Format(CalcDrugPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量), gstrDecPrice)
                        End If
                        cur金额 = Format(mrsPrice!数量 * dbl数量 * dbl单价, gstrDec)
                    Else
                        cur金额 = Format(mrsPrice!数量 * dbl数量 * mrsPrice!单价, gstrDec)
            ...
            If Val(.TextMatrix(lngRow, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(lngRow, COL_执行性质ID))) = 0 Then
                ...
                If mblnAutoVerify And Val(.TextMatrix(lngRow, COL_医嘱状态)) = 1 Then
                    ...
                    Set rsTmp = zldatabase.OpenSQLRecord(strSQL, Me.Caption, Val(.TextMatrix(lngRow, COL_诊疗项目ID)), _
                        Val(.TextMatrix(lngRow, COL_执行科室ID)), .TextMatrix(lngRow, COL_标本部位), .TextMatrix(lngRow, COL_检查方法), lng材料ID, mlng病区ID)
                Else
                    Set rsTmp = zldatabase.OpenSQLRecord(strSQL, Me.Caption, Val(.TextMatrix(lngRow, COL_ID)), Val(.TextMatrix(lngRow, COL_诊疗项目ID)), _
                        Val(.TextMatrix(lngRow, COL_执行科室ID)), .TextMatrix(lngRow, COL_标本部位), .TextMatrix(lngRow, COL_检查方法), mlng病区ID)
                End If
...
Private Function CompletePatiSend(rsPati As ADODB.Recordset, rsSQL As ADODB.Recordset, _
    ...
    ByVal str类别名称 As String, ByVal bln划价 As Boolean, strWarn As String, intWarn As Integer, blnTran As Boolean, ByVal lng发送号 As Long) As Boolean
...
'说明：如果出错,则在调用函数中处理,blnTran返回是否启用了事务
    ...
    Dim cur当日 As Currency, i As Long, j As Long, cur余额 As Currency
    ...
    '病人费用报警
    If cur合计 > 0 Then
        strSQL = "Select Nvl(报警方法,1) as 报警方法,报警值,报警标志1,报警标志2,报警标志3 From 记帐报警线 Where 病区ID=[1] And 适用病人=[2]"
        '错误处理由父过程进行,因为父过程对还要进行一些其他处理
        Set rsWarn = zldatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病区ID, CStr(Nvl(rsPati!适用病人)))   '取当前选择的病区（为了考虑转出病人）
        If Not rsWarn.EOF Then
            If rsWarn!报警方法 = 2 Then cur当日 = GetPatiDayMoney(rsPati!病人ID)
            cur余额 = Nvl(rsPati!剩余款, 0)
            If gbln报警包含划价费用 Then cur余额 = cur余额 - GetPriceMoneyTotal(rsPati!病人ID, 2)
            str类别名称 = Mid(str类别名称, 2)
            For i = 1 To Len(str类别)
                intR = BillingWarn(Me, GetInsidePrivs(p住院医嘱发送), rsWarn, rsPati!姓名, cur余额, cur当日, IIF(gbln报警包含划价费用, cur合计, cur记帐合计), Nvl(rsPati!担保额, 0), Mid(str类别, i, 1), Split(str类别名称, ",")(i - 1), strWarn, intWarn, bln划价)
...
'rsSQL!类型=1-校对(如果不需要先校对),2-医嘱计价,3-住院记帐,4-执行科室替换，5-医嘱发送，6-自动发料,7-输液配药
    ...
    Dim lng发送号Tmp As Long, strNOTmp As String
            ...
            If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                ...
                If Val(.TextMatrix(i, COL_医嘱状态)) = 1 Then   '新开医嘱
                    ...
                    Else
                        ...
                        If Not blnSendPrivs And blnSend Then
                            If Not CheckSendPrivs(Val(.TextMatrix(i, COL_ID)), Val(.TextMatrix(i, COL_病人ID)), Val(.TextMatrix(i, COL_主页ID))) Then
                                blnSend = False
                            End If
                        End If
                    End If
                ...
                If Val(.TextMatrix(i, COL_病人ID)) <> lng病人ID Then
                    ...
                    If lng病人ID <> 0 Then
                        ...
                        If Not CompletePatiSend(rsPati, rsSQL, rsUpload, rsItems, cur合计, cur记帐合计, str类别, str类别名称, bln划价, strWarn, intWarn, blnTran, lng发送号) Then GoTo errH
                    ...
                    cur合计 = 0:  str类别 = "": str类别名称 = "": strWarn = "": cur记帐合计 = 0 '重置报警变量
                    ...
                    strSQL = "Select A.病人ID,B.主页ID,NVL(B.姓名,A.姓名) 姓名,B.险类,B.状态," & _
                ...
                If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                    ...
                    If blnSend Then
                        ...
                        If Len(str分解时间) > 4000 Then
                            Screen.MousePointer = 0
                            MsgBox "病人：""" & rsPati!姓名 & """,医嘱：""" & .TextMatrix(i, col_医嘱内容) & """" & vbCrLf & "执行时间范围太长,共需执行" & CStr(UBound(Split(str分解时间, ",")) + 1) & "次。" & vbCrLf & _
                        ...
                        If strSQL <> "" Then
                            ...
                            Set rsPrice = zldatabase.OpenSQLRecord(strSQL, Me.Caption, Val(.TextMatrix(i, COL_收费细目ID)), Val(.TextMatrix(i, COL_执行科室ID)), Val(Nvl(rsPati!险类, 0)))
...
MoneyItemBegin:
                                ...
                                If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                                    If Not AdviceMoneyMake(lng病人ID, lng主页ID, rsMoneyNow, rsMoneyDay, _
                                        ...
                                        .TextMatrix(i, COL_频率), Val(.TextMatrix(i, COL_单量)), .Cell(flexcpData, i, COL_医嘱期效), rsPrice!费用性质, .TextMatrix(i, COL_诊疗类别), strCuvetteNumber) Then
                                        '跳过当前收费项目(多个收入项目)
                                ...
                                If InStr(",5,6,7", rsPrice!类别) > 0 Then
                                    ...
                                    If Nvl(rsPrice!是否变价, 0) = 1 Then
                                        dbl单价 = Format(CalcDrugPrice(rsPrice!ID, lng执行科室ID, int付数 * dbl数量, , True), gstrDecPrice)
                                    Else
                                ...
                                ElseIf rsPrice!类别 = "4" And Nvl(rsPrice!跟踪在用, 0) = 1 Then
                                    ...
                                    '计算时价卫材单价
                                    If Nvl(rsPrice!是否变价, 0) = 1 Then
                                        dbl单价 = Format(CalcDrugPrice(rsPrice!ID, lng执行科室ID, dbl数量, , True), gstrDecPrice)
                                    Else
                                        dbl单价 = Format(Nvl(rsPrice!单价, 0), gstrDecPrice)
                                    End If
                                Else
                                ...
                                If InStr(",5,6,7,", .TextMatrix(i, COL_诊疗类别)) > 0 Then
                                    ...
                                    '问题号72503
                                    If Val(zldatabase.GetPara("住院领药部门", glngSys, p住院医嘱发送, "0")) = 0 Then
                                        j = .FindRow(CStr(.TextMatrix(i, COL_相关ID)), i + 1, COL_ID)
                                ...
                                If int划价 = 0 Or gbln执行后审核 And int执行状态 = 1 Then
                                    bln划价 = False
                                ...
                                If .TextMatrix(i, COL_医嘱期效) = "长嘱" And (InStr(",I,L,", .TextMatrix(i, COL_诊疗类别)) > 0 Or .TextMatrix(i, COL_诊疗类别) = "E" And InStr(",0,5,", "," & .TextMatrix(i, COL_操作类型) & ",") > 0) Then
                                    ...
                                    lng发送号Tmp = lng发送号: strNOTmp = strNO
                                    lngCount = 0
                                    Do While Not rsExec.EOF
                                        If Val(rsExec!数量 & "") <> 0 Then
                                            ...
                                            If rsDays.RecordCount = 0 Then
                                                ...
                                                If lngCount > 1 Then
                                                    Call GetCurBillSet(strNOKey & "_" & Format(rsExec!要求时间 & "", "yyyy-MM-dd"), strNOTmp, -1, -1)
                                                    If rsSendDays.RecordCount = 0 Then
                                                        lng发送号Tmp = zldatabase.GetNextNo(10)
                                                    End If
                                                End If
                                                If rsSendDays.RecordCount > 0 Then
                                                    lng发送号Tmp = rsSendDays!发送号
                                                End If
                                                rsDays.AddNew
...
'                               '如果小于等于一天，产生一张单据
                                ...
                                Do While Not rsDays.EOF
                                    ...
                                    rsSQL!Sql = "ZL_住院记帐记录_Insert(" & _
                                        ...
                                        "Null,'" & str费用类型 & "',Null," & strTmp & ",NULL,-1,0," & Val(.TextMatrix(i, COL_检查方法)) & ")"
                                    rsSQL.Update
                                ...
                                If Not IsNull(rsPati!险类) And bln实时监控 Then
                                    ...
                                    If rsItems.EOF Then
                                        ...
                                        rsItems!开单科室 = CStr(GetItemField("部门表", Val(.TextMatrix(i, COL_开嘱科室ID)), "名称"))
                        ...
                        If Val(.TextMatrix(i, COL_执行性质ID)) <> 0 Then  '叮嘱不发送(给药途径，配方煎法、用法、采集方法可能为)
                            ...
                            Do While Not rsSendDays.EOF
                                ...
                                rsSQL!Sql = "ZL_病人医嘱发送_Insert(" & _
                                    ...
                                    "'" & UserInfo.姓名 & "','" & str领药号 & "',Null,'" & rsSendDays!分解时间 & "')"
                                rsSQL.Update
                                rsSendDays.MoveNext
                            Loop
        ...
        For i = 1 To .Rows - 1
            If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                If Val(.TextMatrix(i, COL_病人ID)) = lng病人ID And Val(.TextMatrix(i, COL_主页ID)) = lng主页ID Then
                    If InStr(",D,F,", .TextMatrix(i, COL_诊疗类别)) > 0 Or .TextMatrix(i, COL_医嘱期效) = "临嘱" And InStr(",0,5,", Val(.TextMatrix(i, COL_操作类型))) > 0 And .TextMatrix(i, COL_诊疗类别) = "E" Then
                        If Val(.TextMatrix(i, COL_相关ID)) = 0 Then
                            rsData.AddNew
