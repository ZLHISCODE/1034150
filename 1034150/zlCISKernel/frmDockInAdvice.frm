...
Private mblnPass As Boolean    '设为模块变量,便于同电子病案查阅分开
'模块参数
...
Private Enum COL医嘱清单
    ...
    COL_停嘱时间 = COL_ID + 32
    COL_停嘱护士 = COL_ID + 33
    COL_确认停嘱时间 = COL_ID + 34
    '隐藏列
    COL_诊疗项目ID = COL_ID + 35
    COL_试管编码 = COL_ID + 36
    COL_执行标记 = COL_ID + 37
    COL_屏蔽打印 = COL_ID + 38
    COL_前提ID = COL_ID + 39
    COL_签名否 = COL_ID + 40
    COL_文件ID = COL_ID + 41
    COL_报告项 = COL_ID + 42 '0-无报告，1-有报告并按编辑格式打印，2-有报告并按报表格式打印。
    COL_报告ID = COL_ID + 43
    COL_查阅状态 = COL_ID + 44
    COL_收费细目ID = COL_ID + 45
    COL_单量单位 = COL_ID + 46
    COL_开嘱科室ID = COL_ID + 47
    COL_审核状态 = COL_ID + 48
    COL_申请序号 = COL_ID + 49
    COL_审核标记 = COL_ID + 50
    COL_基本药物 = COL_ID + 51
    COL_高危药品 = COL_ID + 52
    COL_标本部位 = COL_ID + 53   'PASS  药品名称
    COL_检查报告ID = COL_ID + 54
    COL_用药目的 = COL_ID + 55
    COL_RIS预约ID = COL_ID + 56
    COL_处方审查状态 = COL_ID + 57
    COL_处方审查结果 = COL_ID + 58
    COL_RIS报告ID = COL_ID + 59
    COL_LIS报告ID = COL_ID + 60
    COL_检查方法 = COL_ID + 61 '区分是备血医嘱还是用血医嘱
    COL_RIS预约状态 = COL_ID + 62
    COL_危急值ID = COL_ID + 63 '医嘱关和危急值关联
End Enum
...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Case conMenu_Edit_Delete, conMenu_Edit_ApplyDel '删除医嘱'取消检验申请
        Call FuncAdviceDel
    Case conMenu_Edit_LISApply  '检验申请
        Call FuncApplyLIS(0)
    ...
    Case conMenu_Edit_BloodApply '输血申请
    ...
    Case conMenu_Edit_Compend * 10# + 5 '自动标记查阅状态
        ...
        Call zldatabase.SetPara("自动标记报告查阅状态", IIF(mblnAutoRead, 1, 0), glngSys, p住院医嘱下达, InStr(GetInsidePrivs(p住院医嘱下达), ";医嘱选项设置;") > 0)
    ...
    Case conMenu_Tool_Option '医嘱选项
        If mint场合 = 1 Then lngParValue = Val(zldatabase.GetPara("发送前自动校对", glngSys, p住院医嘱发送, 0))
        lng待入住病人医嘱 = Val(zldatabase.GetPara("允许给待入住病人下达医嘱", glngSys, p住院医嘱下达, 1))
        frmInAdviceSetup.mMainPrivs = mMainPrivs
        frmInAdviceSetup.mint场合 = mint场合
        frmInAdviceSetup.Show 1, mfrmParent
        If gblnOK Then
            If mint场合 = 1 Then
                If lngParValue <> Val(zldatabase.GetPara("发送前自动校对", glngSys, p住院医嘱发送, 0)) Then
                    If mblnInsideTools Then
                        Set objControl = cbsSub.FindControl(, conMenu_Edit_Audit, , True)
                    Else
                        Set objControl = mcbsMain.FindControl(, conMenu_Edit_Audit, , True)
                    End If
                    objControl.Category = ""    '清除已判断标志
                End If
            End If
            Call GetLocalSetting
            Call LoadAdvice
        End If
        If lng待入住病人医嘱 <> Val(zldatabase.GetPara("允许给待入住病人下达医嘱", glngSys, p住院医嘱下达, 1)) Then
            Call zlCommFun.PressKey(vbKeyF5)
        End If
    ...
    Case conMenu_Manage_ReportLisView  '检验报告浏览
        If mlng病人ID <> 0 Then
            ...
            If Not gobjLIS Is Nothing And CheckLISShowVer(Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_ID))) = False Then
...
'  3.无数据的情况
    ...
    If Between(Control.ID, conMenu_Edit_NewItem, conMenu_Edit_NewItem + 998) _
        ...
        Control.Enabled = mlng病人ID <> 0 And mintPState <> ps已诊 And mintPState <> ps待转入 _
            And (InStr(",0,3,", mint执行状态) > 0 Or Control.ID = conMenu_Edit_MarkMap Or Control.ID = conMenu_Edit_MarkKeyMap Or Control.ID = conMenu_Edit_Compend _
    ...
    Select Case Control.ID
    Case conMenu_Edit_NewItem, conMenu_Edit_LISApply, conMenu_Edit_PacsApply, conMenu_Edit_BloodApply, conMenu_Edit_ApplyCustom * 100# To conMenu_Edit_ApplyCustom * 101#  '新开医嘱(本次住院正常退出路径的病人不能再新开和修改)
        ...
        If blnEnabled Then
            With vsAdvice
                If Not (.TextMatrix(.Row, COL_医嘱状态) = "1" And _
                    (.TextMatrix(.Row, COL_诊疗类别) = "D" Or _
    ...
    Case conMenu_Edit_ApplyView
        ...
        If blnEnabled Then
            With vsAdvice
                If Not (InStr(",K,D,", .TextMatrix(.Row, COL_诊疗类别)) > 0) Then blnEnabled = Val(.TextMatrix(.Row, COL_申请序号)) <> 0
            End With
        End If
        Control.Enabled = blnEnabled
        ...
        If mint场合 = 2 Then '医技医生操作
            With vsAdvice
                blnEnabled = blnAdvice _
                    ...
                    And vsAdvice.TextMatrix(vsAdvice.Row, COL_开嘱医生) = UserInfo.姓名
    ...
    Case conMenu_Edit_Compend * 10# + 4 '我已经查阅该报告
        ...
        Control.Enabled = blnAdvice And (Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_报告ID)) <> 0 Or vsAdvice.TextMatrix(vsAdvice.Row, COL_检查报告ID) <> "") 'And Not Control.Checked
    Case conMenu_Edit_Compend * 10# + 5 '自动标记查阅状态
        Control.Checked = mblnAutoRead
        Control.Enabled = mblnAutoReadEnabled
    Case conMenu_Edit_MarkMap, conMenu_Edit_MarkKeyMap  '观片处理
        Control.Enabled = blnAdvice And InStr(",4,5,6,7,8,9,H,M,Z,", vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别)) = 0 ' And Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_报告ID)) <> 0
...
'功能：根据权限设置菜单和工具栏的可见状态
    ...
    If Control.Category = "已判断" And Control.ID <> conMenu_Edit_SendBilling And Control.ID <> conMenu_Edit_Audit Then Exit Sub
    ...
    Case conMenu_Edit_MarkMap, conMenu_Edit_MarkKeyMap
        '观片处理
        If GetInsidePrivs(pXWPACS观片) <> "" And InStr(GetInsidePrivs(p住院医嘱下达), ";观片处理;") <> 0 Then
            blnVisible = True
        Else
            If Control.ID = conMenu_Edit_MarkMap Then
    ...
    Case conMenu_Edit_ViewDrugExplain '查看药品说明书
        If gobjDrugExplain Is Nothing Then blnVisible = False
...
Public Sub zlRefresh(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病区ID As Long, ByVal lng科室id As Long, _
    ...
    Optional ByVal lng医护科室ID As Long, Optional ByRef objMip As Object, Optional ByVal int婴儿 As Integer = -1)
'功能：刷新住院医嘱数据
'参数：int类型=病人的不同类型
'      lng前提ID=当由医技站调用时传入
...
'      lng医护科室ID=医护站界面科室ID
    ...
    Dim lngPre科室ID As Long, lngPre病区ID As Long, lngPre主页ID As Long
    ...
    If (lngPre界面科室ID <> mlng界面科室ID Or lngPre病人ID <> mlng病人ID) And mlng前提ID <> 0 Then
        mstr前提IDs = Get医技科室医嘱IDs(mlng病人ID, mlng主页ID, mlng界面科室ID, True, mlng前提ID)
...
Private Sub cbsExec_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case ID_完成执行
            ...
            End If
        Case ID_取消完成
            If InStr(GetInsidePrivs(p住院医嘱发送), "取消执行完成") = 0 Then
                Control.Visible = False
            Else
...
Private Sub cbsSub_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case ID_婴儿 * 100# '所有医嘱
            ...
            mvarCond.婴儿 = -1
            mbyt婴儿 = 0
            Call zldatabase.SetPara("病人婴儿过滤", mvarCond.婴儿, glngSys, p住院医嘱下达, InStr(GetInsidePrivs(p住院医嘱下达), ";医嘱选项设置;") > 0)
...
Private Sub cbsSub_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Control.Enabled = mlng病人ID <> 0 And mblnEditState = False
    If Not Control.Enabled Then Exit Sub
    Select Case Control.ID
        Case ID_时间, ID_时间标签
    ...
    With vsAdvice
        ...
        If Not rsTmp.EOF Then
            If MsgBox("该[" & strBillName & "]已经打印了 " & rsTmp.RecordCount & " 次，最近一次由""" & _
                rsTmp!打印人 & """在""" & Format(rsTmp!打印时间, "yyyy-MM-dd HH:mm") & """打印。" & vbCrLf & vbCrLf & "要继续打印吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
        End If
...
'功能：新增医嘱
    ...
    Call frmInAdviceEdit.ShowMe(mfrmParent, mint场合, mMainPrivs, mlng病人ID, mlng主页ID, mlng前提ID, _
            int婴儿, , mblnModalNew, mlng界面科室ID, , , mintPState, mlng病区ID, mlng科室ID, datTurn, mlng医护科室ID, mstr前提IDs, mclsMipModule, mlng危急值ID)
    Exit Sub
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'说明：在主界面删除,对检查组合,手术组合,中药配方,是整个删除,一并给药只删除当前药品
    ...
    If mblnPass And mint场合 = 0 Then
        If gobjPass.zlPassCheck(mobjPassMap) Then
            Call gobjPass.zlPassAdviceSave(mobjPassMap, , 4, strDelDrugIDs) '不显示, 采集
        End If
    End If
...
'功能：修改当前医嘱
    ...
    With vsAdvice
        ...
        Call frmInAdviceEdit.ShowMe(mfrmParent, mint场合, mMainPrivs, mlng病人ID, mlng主页ID, mlng前提ID, Val(.TextMatrix(.Row, COL_婴儿ID)), lng医嘱ID, , mlng界面科室ID, , , mintPState, _
                 mlng病区ID, mlng科室ID, datTurn, mlng医护科室ID, mstr前提IDs, mclsMipModule)
    End With
End Sub
...
'功能：医嘱作废
    ...
    If mlng前提ID = 0 Or mblnDirect Then
        ...
        If frmAdviceOperate.ShowMe(mfrmParent, mMainPrivs, 0, mlng病人ID, mlng主页ID, mlng病区ID, lng医嘱ID, mint场合 = 1, , , , , True, , , , , mlng医护科室ID, , mclsMipModule, , IIF(mlngFontSize = 12, 1, 0)) Then
            ...
            If mblnPass And mint场合 = 0 Then
                If gobjPass.zlPassCheck(mobjPassMap) Then
                    Call gobjPass.zlPassAdviceSave(mobjPassMap, , 2) '不显示, 采集
                End If
            End If
        End If
    Else
...
'删除：当前医嘱作废(一组医嘱作废)
    ...
    With vsAdvice
        ...
        If .TextMatrix(.Row, COL_诊疗类别) = "K" And gbln血库系统 And Val(.TextMatrix(.Row, COL_审核状态)) = 2 Then
            On Error GoTo errH
    ...
    On Error GoTo 0
    If strMsg <> "" Then
        MsgBox "您作废的医嘱中包含已经打印的医嘱，请到医嘱打印模块进行重打。", vbInformation, gstrSysName
    End If
...
'参数：Index=回退内容在菜单上的索引
    ...
    If blnBat Then
        ...
        If vRoll.操作类型 = 9 And strAdviceIDs = "" Then
            strAdviceIDs = GetRollAdviceIDs(lng医嘱ID, 2, vRoll.操作类型, vRoll.操作时间)
        End If
    ...
    If vRoll.操作类型 = 0 And vRoll.发送号 <> 0 Then
        ...
        If Not gobjLIS Is Nothing Then
            If blnBat Then
                strAdvices = GetAdvicesSameSend(vRoll.发送号, "C")
            Else
                strAdvices = lng医嘱ID
            End If
        End If
        ...
        If vRoll.操作类型 = 0 And vRoll.发送号 <> 0 Or vRoll.操作类型 = 4 Then '回退发送或者回退作废操作
            If InitObjBlood() Then
                If blnBat Then
                    ...
                    Else
                        strAdvices输血 = GetAdvicesSameSend(vRoll.发送号, "K")
                    End If
                Else
                    If vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "K" Then strAdvices输血 = lng医嘱ID
                End If
    ...
    '医嘱回退检查
    If Not gobjPath Is Nothing And mlng路径状态 = 1 And (vRoll.操作类型 = 8 Or vRoll.操作类型 = 4) Then
        If blnBat Then    '批量回退
            If GetRollAdviceIDs(lng医嘱ID, 2, vRoll.操作类型, vRoll.操作时间) = "" Then   '批量回退成功
                Call gobjPath.zlAddOutPathItem(strAdviceIDs, mlng病人ID, mlng主页ID)
            End If
        Else
    ...
    If vRoll.操作类型 = 0 And vRoll.发送号 <> 0 Then
        Call InitObjLis(p住院医生站)
        If Not gobjLIS Is Nothing Then
            If gobjLIS.DelLisApplicationForm(strAdvices, strErr) = False Then
                MsgBox "删除检验申请失败：" & strErr, vbInformation, gstrSysName
            End If
        End If
    End If
...
'功能：停止医嘱
    ...
    If mlng前提ID = 0 Or mblnDirect Then
        ...
        If frmAdviceOperate.ShowMe(mfrmParent, mMainPrivs, 1, mlng病人ID, mlng主页ID, mlng病区ID, _
            ...
            If mblnPass And mint场合 = 0 Then
                If gobjPass.zlPassCheck(mobjPassMap) Then
                    Call gobjPass.zlPassAdviceSave(mobjPassMap, , 1) '不显示, 采集
                End If
            End If
        End If
    Else
...
'删除：当前医嘱停止(仅用于住院长嘱)
    ...
    With vsAdvice
        ...
        '停嘱时缺省的医嘱终止时间
        strStopTime = frmAdviceStopTime.ShowMe(Me, lng医嘱ID)
...
'功能：查阅、打印、预览报告
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String, strBill As String, i As Long
    Dim strNO As String, int性质 As Long, bln打印 As Boolean
    Dim lng医嘱ID As Long, lngReportID As Long, blnPrint As Boolean
    ...
    Dim str检查报告ID As String, strTmp As String
    ...
    '先判断是否可以继续操作
    Select Case CheckEPRReport(lng医嘱ID, lngReportID)
    Case 0
        MsgBox "该医嘱的报告没有书写！", vbInformation, gstrSysName
        Exit Sub
    Case 2
...
'      blnAuto=设置为已阅时，是否自动设置在调用
    ...
    With vsAdvice
        ...
        If .TextMatrix(.Row, COL_检查报告ID) = "" Then
            ...
            If blnRead Then
                If Not blnAuto Then
                    If Val(.TextMatrix(.Row, COL_查阅状态)) = 1 Then Exit Sub '自动标记时不计次数
        ...
        Else
            ...
            If Nvl(rsTmp!次数, 0) = 0 Then
                .TextMatrix(.Row, COL_查阅状态) = 0
                Set .Cell(flexcpPicture, .Row, COL_F报告) = frmIcons.imgFlag.ListImages("报告").Picture
                .TextMatrix(.Row, COL_查阅状态) = "查阅"
            End If
        End If
...
'功能：确认执行完成
    ...
    With vsAppend
        ...
        '卫料自动发料时的费用检查
        If Val(zldatabase.GetPara(33, glngSys)) = 1 And Val(zldatabase.GetPara(81, glngSys)) = 0 Then
            If GetAdviceStuffMoney(IIF(lng相关ID = 0, lng医嘱ID, lng相关ID), lng医嘱ID, lng发送号, False, 2, _
                Val(.TextMatrix(.Row, COLSend("记录性质"))), Val(.TextMatrix(.Row, COLSend("门诊记帐")))) > 0 Then
                MsgBox "操作不能继续。" & vbCrLf & vbCrLf & "该医嘱存在尚未审核的卫生材料费用，不能在执行之后自动发料。", vbInformation, gstrSysName
                Exit Sub
            End If
        End If
        
        If Val(zldatabase.GetPara(81, glngSys)) = 1 And Val(.TextMatrix(.Row, COLSend("记录性质"))) = 2 Then
            ...
            If curMoney > 0 Then
                ...
                '记帐报警
                If Not FinishBillingWarn(Me, GetInsidePrivs(p住院医嘱发送), mlng病人ID, mlng主页ID, mlng病区ID, curMoney, str类别, str类别名) Then Exit Sub
                
                '门诊一卡通消费身份验证,只检查门诊记帐费用
                If gbln消费验证 And Val(.TextMatrix(.Row, COLSend("门诊记帐"))) = 1 Then
                    If Not zldatabase.PatiIdentify(Me, glngSys, mlng病人ID, curMoney) Then Exit Sub
                End If
            End If
        End If
    End With
...
Private Sub FuncThingModi()
    ...
    With vsAppend
        ...
        If blnOK Then Call LoadExecList(lng医嘱ID, lng发送号)
    End With
End Sub
...
'参数：blnBar
    ...
    If Not blnBar Or blnBloodExec = True Then blnData = False
...
Private Sub vsAdvice_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    With vsAdvice
        If mblnPass And Me.Visible Then
            If NewRow <> OldRow And OldRow >= 0 And NewRow >= 0 Then
                If gobjPass.zlPassCheck(mobjPassMap) Then
            ...
            If Val(.TextMatrix(NewRow, COL_ID)) <> 0 Then
                ...
                If mblnAppend Then
                    ...
                    ElseIf tbcAppend.Selected.Tag = "配药" Then
                        ...
                        Call mfrmCompoundMedicine.RefreshData(Val(vsAdvice.TextMatrix(NewRow, COL_相关ID)), mlng病区ID, mlng病人ID, mlng主页ID, mlng病人性质, intIdx, mclsMipModule)
                    ElseIf tbcAppend.Selected.Tag = "其他" Then
                        Call ShowOtherAppend(NewRow)
                    ElseIf tbcAppend.Selected.Tag = "血液" Then
                        If InitObjBlood = True Then
                            Call gobjPublicBlood.zlRefreshBloodListInfo(Val(vsAdvice.TextMatrix(NewRow, COL_ID)), mlngFontSize, mblnMoved)
                        End If
                    End If
                End If
            ...
            If .TextMatrix(.Row, COL_诊疗类别) = "K" Then
                ...
                If Val(Mid(gstrInUseApp, 3, 1)) = 1 Then
                    ...
                    If gbln血库系统 = True Then
                        Call frmBloodApplyNew.ShowMe(Me, mlng病人ID, mlng主页ID, IIF(mlng病人性质 = 1, 1, 0), 2, lng医嘱ID, mlng科室ID, mlng病区ID, Get开嘱科室ID(UserInfo.ID, mlng界面科室ID, mlng科室ID, 2), mintPState, , mrsDefine, mclsMipModule, , , mlng前提ID, , , mbyt婴儿, , IIF(bln用血 = True, 1, 0))
                    Else
                        Call frmBloodApply.ShowMe(Me, mlng病人ID, mlng主页ID, IIF(mlng病人性质 = 1, 1, 0), 2, lng医嘱ID, mlng科室ID, mlng病区ID, Get开嘱科室ID(UserInfo.ID, mlng界面科室ID, mlng科室ID, 2), mintPState, , mrsDefine, mclsMipModule, , , mlng前提ID, , , mbyt婴儿)
                    End If
                End If
...
Private Sub vsAdvice_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = 0 And lngRow > 0 Then  '简洁模式才显该列
        If vsAdvice.MouseCol = COL_内容 Then
            If Val(fraMore.Tag) <> lngRow Then
                If InStr(vsAdvice.TextMatrix(lngRow, COL_内容), "重整医嘱") = 0 Then
                    With vsAdvice
                        fraMore.Visible = False
                        fraMore.Tag = lngRow
                        If lngRow = .Row Then
                        ...
                        If fraMore.Top + fraMore.Height > .Top + .Height - IIF(VsScroll(vsAdvice), 230, 0) Then Exit Sub
        ...
        Else
            ...
            strPrompt = ""
            If vsAdvice.MouseCol = COL_F标志 Then
                With vsAdvice
                    ...
                    End If
                End With
            ElseIf vsAdvice.MouseCol = COL_查阅状态 Then
                With vsAdvice
                    ...
                    End If
                End With
            ElseIf vsAdvice.MouseCol = COL_F报告 Then
...
Private Sub Form_Load()
    ...
    mblnAutoReadEnabled = Not ((intType = 3 Or intType = 15) And InStr(GetInsidePrivs(p住院医嘱下达), "医嘱选项设置") = 0)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If mblnAppend And Not tbcAppend.Selected Is Nothing Then
        Call zldatabase.SetPara("医嘱子列表", tbcAppend.Selected.Tag, glngSys, p住院医嘱下达, InStr(GetInsidePrivs(p住院医嘱下达), ";医嘱选项设置;") > 0)
    End If
...
'功能：保存医嘱过滤设置条件
    ...
    Call zldatabase.SetPara("重整医嘱过滤", IIF(mvarCond.重整, 1, 0), glngSys, p住院医嘱下达, blnSetup)
    Call zldatabase.SetPara("科内医嘱过滤", IIF(mvarCond.科内, 1, 0), glngSys, p住院医嘱下达, blnSetup)
    Call zldatabase.SetPara("显示模式", mvarCond.显示模式, glngSys, p住院医嘱下达, blnSetup)
    Call zldatabase.SetPara("医嘱过滤方式", mvarCond.过滤模式, glngSys, p住院医嘱下达, blnSetup)
    Call zldatabase.SetPara("报告查看类型", mvarCond.报告, glngSys, p住院医嘱下达, blnSetup)
    
    Call zldatabase.SetPara("过滤条件自动隐藏", IIF(mblnHideFilter, 1, 0), glngSys, p住院医嘱下达, blnSetup)
    Call zldatabase.SetPara("医嘱显示在用", mvarCond.医嘱显示, glngSys, p住院医嘱下达, blnSetup)
End Sub
...
'功能：初始化发送清单格式
    ...
    strHead = "发送号;发送时间,1530,1;单据号,850,1;发送医嘱,1800,1;收费项目,1800,1;发送数次,850,1;计费状态,850,1;" & _
        "执行状态,850,1;执行科室,1000,1;执行人,800,1;执行时间,1530,1;执行说明,1800,1;首次时间,1530,1;末次时间,1530,1;发送人,800,1;医嘱ID;相关ID;记录性质;门诊记帐;记录状态;诊疗类别;操作类型;跟踪在用;完成时间;输血类型"
    arrHead = Split(strHead, ";")
    With vsAppend
        .Redraw = flexRDNone
        .Clear
...
Private Sub InitExecTable()
    ...
    strHead = "要求时间,1530,1;执行时间,1530,1;本次数次,850,1;执行摘要,2500,1;执行人,750,1;登记时间,1530,1;登记人,750,1;执行结果,1000,1;核对人,750,1;核对时间,1530,1"
    arrHead = Split(strHead, ";")
    With vsExec
        .Clear
        .FixedRows = 1
...
'功能：初始化医嘱清单格式
    ...
    strHead = "ID;相关ID;序号;婴儿ID;医嘱状态;诊疗类别;操作类型;毒理分类;标志;" & _
              ",240,4;期效,500,4;开始时间,1530,1;医嘱内容,3000,1;内容,4000,1;,375,1;总量,850,1;单量,850,1;天数,450,1;频率,1000,1;用法,1000,1;医生嘱托,1000,1;执行时间,1000,1;" & _
              "终止时间,1530,1;执行科室,1000,1;执行性质,850,1;上次执行,1560,1;状态,500,4;开嘱医生,850,1;开嘱时间,1530,1;校对护士,850,1;校对时间,1530,1;停嘱医生,850,1;" & _
              "停嘱时间,1530,1;停嘱护士,850,1;确认停嘱时间,1530,1;" & _
              "诊疗项目ID;试管编码;执行标记;屏蔽打印;前提ID;签名否;文件ID;报告项;报告ID;查阅状态,700,4;收费细目ID;单量单位;开嘱科室ID;审核状态;申请序号;" & _
              "审核标记;基本药物,850,1;高危药品;标本部位;检查报告ID;用药目的;RIS预约ID;处方审查状态;处方审查结果;RIS报告ID;LIS报告ID;检查方法;RIS预约状态;危急值ID"
    arrHead = Split(strHead, ";")
    With vsAdvice
        .Clear
...
'参数：blnRefreshNotify刷新医嘱提醒(F5手动刷新,新开医嘱，停止医嘱，作废医嘱时)
    ...
    If mvarCond.医嘱显示 = 0 And mvarCond.过滤模式 <> 3 Then  '在用医嘱，如果是在报告页面不用区分医嘱的范围，直接取出这个病人的所有报告。
        strWhere = strWhere & " And Nvl(A.医嘱状态,0)<>4"
        strWhere = strWhere & " And (A.医嘱期效=0 and (nvl(a.停嘱时间,a.执行终止时间)>[3] or nvl(a.停嘱时间,a.执行终止时间) is null) or " & _
            " A.医嘱期效=1 and A.开始执行时间 >= [6])"
    End If
    ...
    "Select /*+ RULE */ A.ID,A.相关ID,A.序号,Nvl(A.婴儿,0) as 婴儿ID,A.医嘱状态," & _
             ...
             " A.审查结果 as 警示,Decode(Nvl(A.医嘱期效,0),0,'长嘱','临嘱') as 期效," & _
             ...
             " A.审核标记,D.基本药物,d.高危药品,A.标本部位 , Max(y.检查报告id) || '' As 检查报告id,a.用药目的,f.预约ID as RIS预约ID,J.状态 as 处方审查状态,J.审查结果 as 处方审查结果,max(y.RISID) as RIS报告ID,max(y.报告ID) as LIS报告ID,Max(a.检查方法) as 检查方法,f.是否调整 as RIS预约状态,max(h.危急值id) as 危急值ID"
    strSQL = strSQL & _
             " From 病人医嘱记录 A,部门表 E,药品特性 C,药品规格 D,诊疗项目目录 B,收费项目目录 G,病人医嘱报告 Y,病历单据应用 M,病历文件列表 N,RIS检查预约 f, 处方审查明细 I, 处方审查记录 J,病人危急值医嘱 H" & _
             " Where A.诊疗项目ID=B.ID(+) And A.执行科室ID=E.ID(+) And A.诊疗项目ID=C.药名ID(+) And a.ID = i.医嘱ID(+) And I.审方ID = J.ID(+) and (I.最后提交 =1 Or I.审方ID is NULL) " & _
             " And A.收费细目ID=D.药品ID(+) And A.收费细目ID=G.ID(+) And A.ID=Y.医嘱ID(+) and a.id=f.医嘱ID(+) and a.id=h.医嘱ID(+)" & _
    ...
    strGroupBy = " Group By a.Id, a.相关id, a.序号, a.婴儿, a.医嘱状态, a.诊疗类别, b.操作类型, c.毒理分类, a.紧急标志, a.审查结果, a.医嘱期效, a.开始执行时间, a.医嘱内容, a.皮试结果," & vbNewLine & _
            ...
            "         a.前提id, a.新开签名id, m.病历文件id, n.通用, a.收费细目id, b.计算单位, a.开嘱科室id, a.审核状态, a.申请序号, a.审核标记, d.基本药物, d.高危药品, a.标本部位,a.用药目的,f.预约ID,J.状态,J.审查结果,f.是否调整"
    '重整显示格式处理
    If mdat重整 <> CDate("1900-01-01") Then
        ...
        Else
            ...
            " Select * From (" & strSQL & " And Nvl(A.重整标志,1)=0 And A.医嘱状态 Not IN(1,2) " & strGroupBy & "  Order by 婴儿ID,序号)" & _
                     ...
                     " Null as 毒理分类,-Null as 标志,-Null as 警示,Null as 期效,Null as 开始时间,Null as 医嘱内容,Null as 内容,Null as 皮试,Null as 总量," & _
                     ...
                     " -Null as 签名否,-Null as 文件ID,-Null as 报告项,-Null as 报告ID,-Null as 查阅状态, -Null as 收费细目ID, Null as 单量单位, -Null as 开嘱科室ID,-Null as 审核状态," & _
                     " -Null as 申请序号,-Null as 审核标记 ,Null as 基本药物,-Null as 高危药品,-Null as 标本部位,-NULL AS 检查报告id,-Null as 用药目的,-null as RIS预约ID,-null as 处方审查状态,-null as 处方审查结果,-null as RIS报告ID,-null as LIS报告ID,-null as 检查方法,-null as RIS预约状态,-null as 危急值ID From Dual" & _
                     " Union ALL" & _
    ...
    If Not rsTmp.EOF Then
        strSQL = "Select a.医嘱id,decode(a.输血血型,1,'A',2,'B',3,'O',4,'AB','') As 血型 From 输血申请记录 A, 病人医嘱记录 B Where 医嘱id = b.Id And b.病人ID=[1] and b.主页ID=[2] And a.输血血型>0 and b.诊疗类别='K'"
        ...
        With vsAdvice
            ...
            Do While i <= .Rows - 1
                        ...
                        If InStr(",5,6,", .TextMatrix(i - 1, COL_诊疗类别)) > 0 Then
                            ...
                            For j = i - 1 To .FixedRows Step -1
                                If Val(.TextMatrix(j, COL_相关ID)) = Val(.TextMatrix(i, COL_ID)) Then
                                    '显示成药的给药途径+滴速
                                    .TextMatrix(j, COL_用法) = .TextMatrix(i, COL_用法) & IIF(.TextMatrix(i, COL_医生嘱托) <> "", "," & .TextMatrix(i, COL_医生嘱托), "")  '方便PASS取给药途径
                ...
                If Not (bln给药途径 Or bln输血途径) And .TextMatrix(i, COL_诊疗类别) <> "7" Then
                    ...
                    'Pass:根据审查结果显示警示灯
                    If mblnPass Then
                        If gobjPass.zlPassCheck(mobjPassMap) And .TextMatrix(i, COL_警示) <> "" Then
                ...
                Else
                    ...
                    If mvarCond.过滤模式 = 3 Then
                        ...
                        .Cell(flexcpData, i, COL_查阅状态) = .TextMatrix(i, COL_查阅状态)
...
'功能：读取指定医嘱的计价,并根据当前的诊疗收费 关系进行更新
    ...
    With vsAppend
        ...
        End If
        Set rsTmp = zldatabase.OpenSQLRecord(strSQL, Me.Name, lng医嘱ID, lng相关ID, lng材料ID, mlng病区ID)
        '显示计价内容
        If Not rsTmp.EOF Then
            '确定显示行数
            ...
            For i = 1 To rsTmp.RecordCount
                ...
                If InStr(",5,6,7,", rs收费细目!类别) > 0 Then
                    If Nvl(rs收费细目!是否变价, 0) = 1 Then
                        ...
                        If InStr(",5,6,7,", rs诊疗项目!类别) > 0 Then
                            '药嘱药品计算一个住院包装的住院时价
                            .TextMatrix(i, COLPrice("单价")) = CalcDrugPrice(rs收费细目!ID, lng执行科室ID, Nvl(rsTmp!住院包装, 1))
                            .TextMatrix(i, COLPrice("单价")) = Format(Val(.TextMatrix(i, COLPrice("单价"))) * Nvl(rsTmp!住院包装, 0), gstrDecPrice)
                        Else
                            '非药嘱药品计算相对售价数量的售价实价
                            .TextMatrix(i, COLPrice("单价")) = Format(CalcDrugPrice(rs收费细目!ID, lng执行科室ID, Nvl(rsTmp!数量, 0)), gstrDecPrice)
                        End If
                    Else
                        '药嘱药品为住院单价,非药药品为售价
                ...
                '执行科室
                If lng执行科室ID <> 0 Then
                    .TextMatrix(i, COLPrice("执行科室")) = Get部门名称(lng执行科室ID)
                End If
...
'功能：显示指定行医嘱的发送记录
    ...
    With vsAppend
        ...
        If vsAdvice.TextMatrix(lngRow, COL_期效) = "长嘱" Then
            strTab = "住院费用记录"
        Else
        ...
        If InStr(",5,6,", vsAdvice.TextMatrix(lngRow, COL_诊疗类别)) > 0 Then
            ...
            strSQL = _
                ...
                " B.执行状态 as 执行状态ID,B.计费状态 as 计费状态ID,A.记录状态,NVL(B.完成时间,A.执行时间) as 完成时间,NVL(B.完成人,A.执行人) as 完成人,B.执行说明" & _
        ...
        Else
            ...
            strSQL = _
                ...
                " B.首次时间,B.末次时间,Decode(Nvl(B.计费状态,0),-1,'无需计费',0,'未计费'," & strState & ") as 计费状态," & _
                " B.发送人,B.发送号,B.记录序号 as 发送序号,A.序号 as 费用序号,C.诊疗项目ID,C.诊疗类别," & _
                " B.执行状态 as 执行状态ID,B.计费状态 as 计费状态ID,A.记录状态,B.完成时间,B.完成人,B.执行说明" & _
        ...
        strSQL = "Select /*+ RULE */ A.发送序号,A.费用序号," & _
            ...
            " A.执行部门ID,A.执行状态ID,A.计费状态ID,A.记录状态,D.操作类型,H.跟踪在用,A.完成时间,a.完成人,a.执行说明" & _
            " From (" & strSQL & ") A,收费项目目录 B,部门表 C,诊疗项目目录 D,诊疗项目类别 F,收费项目别名 G,材料特性 H" & _
            " Where A.收费细目ID=B.ID(+) And A.执行部门ID=C.ID(+) And A.诊疗项目ID=D.ID" & IIF(mbln叮嘱发送执行, "(+)", "") & " And A.诊疗类别=F.编码" & IIF(mbln叮嘱发送执行, "(+)", "") & _
...
'功能：读取指定医嘱的执行情况表
    Dim rsTmp As New ADODB.Recordset
    Dim strSQL As String, i As Long
    Dim strPre As String, dblTotal As Double
    ...
    strSQL = "Select A.要求时间,A.执行时间,A.本次数次,D.计算单位,A.执行摘要,A.执行人,A.登记时间,A.登记人,NVL(A.执行结果,1) As 执行结果,a.核对人,a.核对时间,d.操作类型,d.类别" & _
    ...
    With vsExec
        ...
        If Not rsTmp.EOF Then
            ...
            For i = 1 To rsTmp.RecordCount
                ...
                .TextMatrix(i, 7) = IIF(rsTmp!执行结果 = 0, "未执行", "完成")
                ...
                dblTotal = dblTotal + Nvl(rsTmp!本次数次, 0)
...
'功能：显示指定行的医嘱发送可以打印的诊疗单据在菜单上
    ...
    '输血医嘱申请单打印模式=2，新开后可以打印
    intBloodState = 8
    If mint申请单打印模式 = 2 And vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "K" Then
        blnBlood = True
        strTmp = ",-1,4,"
...
'        End If
        strSQL = "Select Distinct D.编号,D.名称,D.说明,A.NO,0 类别" & _
            ...
            If .Redraw <> flexRDNone Then
                If tbcAppend.Selected.Tag = "发送" And vsAppend.Cols = COLSend.Count Then
                    If mint场合 = 1 And Val(.TextMatrix(NewRow, COLSend("发送号"))) <> 0 And (InStr(",5,6,7,", .TextMatrix(NewRow, COLSend("诊疗类别"))) = 0 Or (mbln叮嘱发送执行 And .TextMatrix(NewRow, COLSend("诊疗类别")) = "")) Then
                        If gbln执行发料 Or Not (.TextMatrix(NewRow, COLSend("诊疗类别")) = "4" And Val(.TextMatrix(NewRow, COLSend("跟踪在用"))) = 1) Then
                    ...
                    If blnExec = True And blnBloldExec = True Then
                        If InitObjBlood = True Then
                            Call gobjPublicBlood.zlRefreshBloodExec(Me, glngSys, p住院医嘱发送, Val(.TextMatrix(NewRow, COLSend("医嘱ID"))), mlng医护科室ID, GetInsidePrivs(p住院医嘱发送), 2, mlng病区ID, mblnMoved, mlngFontSize)
                        End If
                    End If
                End If
...
'功能：修改申请单
    ...
    With vsAdvice
        ...
        Else
    If Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_操作类型)) = 6 And vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "E" Then
        Call FuncApplyLIS(Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_申请序号)))
    ElseIf vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "D" Then
        Call FuncApplyPacs(Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_ID)), Val(vsAdvice.TextMatrix(vsAdvice.Row, COL_申请序号)))
    ElseIf vsAdvice.TextMatrix(vsAdvice.Row, COL_诊疗类别) = "K" Then
...
'参数：lng申请序号=修改申请单时的申请序号
    ...
    Dim strDate As String, str开始时间 As String, str医嘱内容 As String, str医嘱IDs As String, blnCancel As Boolean
    Dim strDelIDs As String, arrDelID() As String
    ...
    If CheckAdviceAddModi(IIF(lng申请序号 = 0, 0, 1)) = False Then Exit Sub
    ...
    Else
        ...
        If strResult <> "" Then
            ...
            arrTmp = Split(strResult, "<Split B>")
            strDate = "To_Date('" & Format(zldatabase.Currentdate, "yyyy-MM-dd HH:mm") & "','YYYY-MM-DD HH24:MI')"
            lng序号 = GetMaxAdviceNO(mlng病人ID, mlng主页ID, mbyt婴儿)
            
            '只产生临嘱
            For i = 0 To UBound(arrTmp)
                If arrTmp(i) = "" Then Exit For
                lng假医嘱ID = lng假医嘱ID + 1
                str相关ID = "<FAKEID>" & lng假医嘱ID & "</FAKEID>"
                lng相关ID = lng假医嘱ID
                
                lng采集科室ID = Val(Split(arrTmp(i), "<Split A>")(0))
                lng执行科室ID = Val(Split(arrTmp(i), "<Split A>")(1))
                str开始时间 = "To_Date('" & Split(arrTmp(i), "<Split A>")(2) & "','YYYY-MM-DD HH24:MI')"
                
                str标本 = Split(arrTmp(i), "<Split A>")(3)
                str附项 = Split(arrTmp(i), "<Split A>")(4)
                
                str嘱托 = Split(arrTmp(i), "<Split A>")(5)
                str紧急 = Split(arrTmp(i), "<Split A>")(6)
                lng采集项目ID = Val(Split(arrTmp(i), "<Split A>")(7))
                
                
                'a.先产生检验医嘱
                For Y = 8 To UBound(Split(arrTmp(i), "<Split A>"))
                    
                    lng检验项目ID = Split(arrTmp(i), "<Split A>")(Y)
                    str检验项目 = CStr(GetItemField("诊疗项目目录", lng检验项目ID, "名称"))
                    If Y = 8 Then str检验项目组合 = str检验项目
                    ...
                    lng序号 = lng序号 + 1
                    str摘要 = ""
                    str摘要 = gclsInsure.GetItemInfo(mint险类, mlng病人ID, 0, "", 0, "", CStr(lng检验项目ID))
                    ...
                    arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & _
                        ...
                        "," & str检验执行性质 & "," & str紧急 & "," & str开始时间 & ",Null," & mlng科室ID & "," & _
                        lng开单科室ID & ",'" & UserInfo.姓名 & "'," & strDate & ",NULL," & ZVal(mlng前提ID) & "," & _
                        "Null,0,Null," & IIF(str摘要 = "", "Null", "'" & str摘要 & "'") & ",'" & UserInfo.姓名 & "'" & _
                        ",Null,Null,Null,Null," & lng申请序号 & ")"
                ...
                'b.再产生采集方法医嘱
                str采集方法 = CStr(GetItemField("诊疗项目目录", lng采集项目ID, "名称"))
                Set rsTmp = Get诊疗项目记录(lng采集项目ID)
                str采集计价性质 = Val("" & rsTmp!计价性质)
                str采集执行性质 = "" & rsTmp!执行科室
                str医嘱内容 = AdviceMakeText(str检验项目组合, str采集方法, str标本)
                lng序号 = lng序号 + 1
                str摘要 = ""
                str摘要 = gclsInsure.GetItemInfo(mint险类, mlng病人ID, 0, "", 0, "", CStr(lng采集项目ID))
                ...
                arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & _
                    ...
                    "," & str采集执行性质 & "," & str紧急 & "," & str开始时间 & ",Null," & mlng科室ID & "," & _
                    lng开单科室ID & ",'" & UserInfo.姓名 & "'," & strDate & ",NULL," & ZVal(mlng前提ID) & "," & _
                    "Null,0,Null," & IIF(str摘要 = "", "Null", "'" & str摘要 & "'") & ",'" & UserInfo.姓名 & "'" & _
                    ",Null,Null,Null,Null," & lng申请序号 & ")"
        ...
        '临床路径判断
         ...
         Call ZLHIS_CIS_001(mclsMipModule, mlng病人ID, mstr姓名, mstr住院号, , IIF(mlng病人性质 = 1, 1, 2), mlng主页ID, mlng病区ID, , mlng科室ID, "", , mstr床号, _
               lng相关ID, str紧急, 1, "E", "", UserInfo.姓名, str开始时间, lng开单科室ID, "", , , "")
...
Private Function GetAdvicesSameSend(ByVal lng发送号 As Long, ByVal str诊疗类别 As String) As String
'功能：根据发送号获取一起发送的医嘱的主ID
    Dim rsTmp As ADODB.Recordset, strSQL As String
    Dim strIDs As String, i As Long
 
    If str诊疗类别 = "C" Then
        strSQL = "Select 相关id As 医嘱id" & vbNewLine & _
                "From 病人医嘱记录" & vbNewLine & _
                "Where ID In (Select 医嘱id From 病人医嘱发送 Where 发送号 = [1]) And 诊疗类别 = [2]" & vbNewLine & _
                "Group By 相关id"
    ElseIf str诊疗类别 = "D" Or str诊疗类别 = "K" Then
        strSQL = "Select ID As 医嘱id" & vbNewLine & _
                "From 病人医嘱记录" & vbNewLine & _
                "Where ID In (Select 医嘱id From 病人医嘱发送 Where 发送号 = [1]) And 诊疗类别 = [2] And 相关id is null "
    End If

    On Error GoTo errH
    Set rsTmp = zldatabase.OpenSQLRecord(strSQL, Me.Caption, lng发送号, str诊疗类别)
    For i = 1 To rsTmp.RecordCount
        strIDs = strIDs & "," & rsTmp!医嘱ID
        rsTmp.MoveNext
    Next
    
    GetAdvicesSameSend = Mid(strIDs, 2)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'入参:bytSize：0-小(缺省)，1-大
    ...
    If mvarCond.显示模式 = 0 Then
        Call zlControl.VSFSetFontSize(vsAdvice, mlngFontSize, COL_内容)
    Else
        Call zlControl.VSFSetFontSize(vsAdvice, mlngFontSize, col_医嘱内容)
    End If
    
    Call zlControl.VSFSetFontSize(vsAppend, mlngFontSize)
    If tbcAppend.Selected.Tag = "发送" Then '第一行的字体要特殊处理
        vsAppend.Cell(flexcpFontSize, 0, 0, 0, vsAppend.Cols - 1) = mlngFontSize
    End If
    
    Call zlControl.VSFSetFontSize(vsExec, mlngFontSize)
    Call zlControl.VSFSetFontSize(vsfAdivceDetail, mlngFontSize)
    
    If Not mfrmCompoundMedicine Is Nothing Then
        Call zlControl.VSFSetFontSize(mfrmCompoundMedicine.vsSend, mlngFontSize)
        Call zlControl.VSFSetFontSize(mfrmCompoundMedicine.vsExec, mlngFontSize)
    End If
...
'参数:lng医嘱ID
    ...
    strSQL = "Select b.名称 as 分类,a.编码 as ID,a.编码,a.名称,a.简码 From 变异常见原因 a,变异常见原因 b" & _
             ...
             " Order by 分类,a.编码"
    vPoint = GetCoordPos(vsAdvice.hWnd, vsAdvice.CellLeft, vsAdvice.CellTop)
...
'功能：设置工具栏按钮，对应于医嘱菜单下面的工具栏的按钮，先将其删掉再添加
    ...
    With objCbs(2).Controls
        ...
        Else
            ...
            If mblnPass Then
                If gobjPass.zlPassCheck(mobjPassMap) Then  '合理用药菜单
...
'参数：intType=0 新增，=1修改，=2查看 ,=4 核对
    ...
    If intType <> 2 Then
        If CheckAdviceAddModi(intType, 0, datTurn) = False Then
            If intType = 1 Then intType = 2
    ...
    If gbln血库系统 = True Then
        blnApply = frmBloodApplyNew.ShowMe(Me, mlng病人ID, mlng主页ID, IIF(mlng病人性质 = 1, 1, 0), intType, lngUpdateAdvice, mlng科室ID, mlng病区ID, Get开嘱科室ID(UserInfo.ID, mlng界面科室ID, mlng科室ID, 2), mintPState, datTurn, mrsDefine, mclsMipModule, , , mlng前提ID, , , mbyt婴儿, , IIF(bln用血 = True, 1, 0))
    Else
        blnApply = frmBloodApply.ShowMe(Me, mlng病人ID, mlng主页ID, IIF(mlng病人性质 = 1, 1, 0), intType, lngUpdateAdvice, mlng科室ID, mlng病区ID, Get开嘱科室ID(UserInfo.ID, mlng界面科室ID, mlng科室ID, 2), mintPState, datTurn, mrsDefine, mclsMipModule, , , mlng前提ID, , , mbyt婴儿)
    End If
    If blnApply = True Then
        If mlng路径状态 = 1 And Not gobjPath Is Nothing And (intType = 0 Or intType = 1) And lngUpdateAdvice <> 0 Then
            '获取输血申请序号
            lngNo = Get申请序号(lngUpdateAdvice)
...
'注意:删除或修改下面列中数据时，请检查合理用药部件中的关联处理。
    ...
    If mblnPass Then
        With mobjPassMap
            ...
            Set .VSCOL = .GetVSCOL(COL_ID, COL_相关ID, COL_诊疗类别, COL_诊疗项目ID, COL_收费细目ID, col_医嘱内容, COL_期效, COL_单量, COL_单量单位, COL_用法, _
                COL_天数, , COL_开嘱时间, COL_开嘱医生, COL_开始时间, COL_开嘱科室ID, COL_终止时间, COL_频率, , , _
                , COL_警示, COL_序号, COL_医嘱状态, , , , , COL_执行性质, COL_标本部位, _
                , , , , , COL_总量, , COL_医生嘱托, COL_用药目的)
...
Private Function Get申请序号(ByVal lng医嘱ID As Long) As Long
'功能：获取部门名称
'参数：lngID=部门ID
'返回：部门名称
    Dim strSQL As String
    ...
    If Not rsTmp.EOF Then Get申请序号 = Val(rsTmp!申请序号 & "")
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'功能：按发送号过滤本次发送的医嘱中是否有发到RIS去的医嘱没有
    ...
    strSQL = "Select a.id,a.诊疗项目id,0 as RISItem" & vbNewLine & _
        ...
        "      (a.诊疗类别 In ('F', 'D') Or a.诊疗类别 = 'E' And Nvl(c.操作类型,'0') in ('0','5')) And a.相关id Is Null And a.医嘱期效 = 1"
