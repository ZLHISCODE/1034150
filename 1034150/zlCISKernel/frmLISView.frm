...
Public Sub ShowMe(ByVal lng病人ID As Long, ByVal lngMod As Long, ByVal frmMain As Form)
...
'   >>>>>>>>>>>>>>>>>> 正式版面SQL,只提取已审核的数据 <<<<<<<<<<<<<<<<<<
'    strSQL = "Select ID, 紧急, 申请时间, 检验项目, 标本序号, 微生物标本, 姓名, 性别, 年龄, 报告结果 as 结果次数, 医嘱id, 病人id, 检验人, 审核人," & vbNewLine & _
'            "       婴儿, 主页id, 审核时间" & vbNewLine & _
'            "From 检验标本记录  " & vbNewLine & _
'            "Where 病人id = [1] And 审核人 is Not null " & vbNewLine & _
'            "Order By 申请时间, 审核时间 Desc"
    ...
    If mstrWhere = "" Then
        strSql = "Select A.ID, A.紧急, A.申请时间, A.检验项目, A.标本序号, Nvl(A.微生物标本,0) as 微生物标本, A.姓名, A.性别, A.年龄, A.报告结果 as 结果次数, A.医嘱id, A.病人id, A.检验人, A.审核人," & vbNewLine & _
                ...
                "Where a.审核人 Is Not Null And  a.病人id = [1]" & vbNewLine & _
    ...
    Else
        ...
        strSql = "Select A.ID, A.紧急, A.申请时间, A.检验项目, A.标本序号,  Nvl(A.微生物标本,0) as 微生物标本, A.姓名, A.性别, A.年龄, A.报告结果 as 结果次数, A.医嘱id, A.病人id, A.检验人, A.审核人," & vbNewLine & _
                "       A.婴儿, a.主页id 住院次数, A.审核时间, decode(A.病人来源,1,'门诊',2,'住院','其他') as 病人来源, A.操作类型" & vbNewLine & _
                "From 检验标本记录  A " & vbNewLine & _
                "Where a.审核人 Is Not Null And  a.病人id = [1] And A.申请时间 Between [2] And [3] " & vbNewLine & _
                IIF(str项目 = "", "", " And instr(A.检验项目,[4])>0  ") & _
                "Order By A.申请时间, A.审核时间 Desc"
                
        Set rsTmp = zldatabase.OpenSQLRecord(strSql, Me.Caption, lng病人ID, dateMin, dateMax, str项目)
    End If
    Do Until rsTmp.EOF
...
Private Sub Form_Load()
    ...
    mstrPrivs = IIF(Right(gMainPrivs, 1) = ";", gMainPrivs, gMainPrivs & ";") & gcolPrivs("_" & mlngMod)
    CreateCbs                           '创建工具条
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Call zldatabase.SetPara("隐藏检验图形", IIF(blnCheck, "True", "False"), glngSys, mlngMod, InStr(GetInsidePrivs(mlngMod), IIF(mlngMod = p门诊医嘱下达 Or mlngMod = p住院医嘱下达, ";医嘱选项设置;", ";参数设置;")) > 0)
