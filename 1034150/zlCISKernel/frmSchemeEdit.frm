...
Private Sub cmd适用证候_Click()
    ...
    vPoint = GetCoordPos(txt适用证候.Hwnd, 0, 0)
...
Private Sub cmd用法_Click()
    ...
    With vsAdvice
        ...
        strSql = "Select Distinct A.ID,A.编码,A.名称,C.名称 as 分类,A.执行分类 as 执行分类ID " & _
            ...
            " And A.类别='E' And A.操作类型=[1] And " & IIF(mint范围 = 3, "Nvl(A.服务对象,0)<>0", "A.服务对象 IN([3],3)") & strSql & _
            " And (A.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or A.撤档时间 IS NULL)" & _
            " Order by A.编码"
        vRect = GetControlRect(txt用法.Hwnd)
        Set rsTmp = zldatabase.ShowSQLSelect(Me, strSql, 0, lbl用法.Caption, False, "", "", False, False, True, _
            vRect.Left, vRect.Top, txt用法.Height, blnCancel, False, True, CStr(int类型), Val(.TextMatrix(.Row, COL_诊疗项目ID)), mint范围)
        If rsTmp Is Nothing Then
            If Not blnCancel Then
        ...
        If KeyAscii = 13 Then
            ...
            Else
                ...
                Set rsTmp = zldatabase.ShowSQLSelect(Me, strSql, 0, lbl用法.Caption, False, "", "", False, False, True, _
                    ...
                    strLike & UCase(txt用法.Text) & "%", CStr(int类型), Val(.TextMatrix(.Row, COL_诊疗项目ID)), mint简码 + 1, mint范围)
                If rsTmp Is Nothing Then
                    If Not blnCancel Then
    ...
    If KeyAscii = 13 Then
        ...
        If tbrFree.Buttons(1).value = 0 Then
            ...
            If rsTmp Is Nothing Then '取消或无数据
                ...
                txt医嘱内容.SetFocus: Exit Sub
            ElseIf byt匹配 = 1 Then
                Call zlControl.CboSetIndex(cbo期效.Hwnd, IIF(cbo期效.ListIndex = 0, 1, 0))
            End If
            '新项目的录入
    ...
    With vsAdvice
        If .RowData(lngRow) = 0 Then
            ...
            If i = -1 Or Not Visible Then
                Call zlControl.CboSetIndex(cbo期效.Hwnd, 1)    '缺省为临嘱
            Else
...
'返回：本次录入是否有效
    ...
    With vsAdvice
        ...
        If rsInput!类别ID = "K" Then
            ...
            '缺省与上一行相同
            If lngPreRow <> -1 Then
                If .TextMatrix(lngPreRow, COL_类别) = "K" Then
        ...
        For i = 1 To rsItems.RecordCount
            ...
            If InStr(",0,5,", Val(.TextMatrix(lngCurRow, COL_执行性质))) = 0 And Nvl(rsItems!执行科室ID, 0) <> 0 Then
                If InStr(",5,6,7,", Nvl(rsItems!类别, "*")) > 0 Then
                    ...
                    End If
                ElseIf Nvl(rsItems!类别) = "4" Then
                    str药房IDs = Get可用发料部门IDs(Val(.TextMatrix(lngCurRow, COL_收费细目ID)), 0, mint范围)
                    If InStr("," & str药房IDs & ",", "," & rsItems!执行科室ID & ",") > 0 Then
                        .TextMatrix(lngCurRow, COL_执行科室ID) = Nvl(rsItems!执行科室ID, 0)
                    End If
                Else
...
'      strExtData=检查:包含检查部位方法等信息,手术:包含附加手术及麻醉的信息,可能无附加手术
    ...
    With vsAdvice
        ...
        strSql = "Select A.*,Nvl(B.性质,0) as 性质,B.用法ID," & _
            ...
            " Order by 性质"
        Set rsMore = zldatabase.OpenSQLRecord(strSql, Me.Caption, Val(rsInput!诊疗项目ID), mint范围)
    ...
    With vsAdvice
        ...
        If RowIn配方行(lngRow) Then
            ...
            For i = lngBeginRow To lngRow
                ...
                If .TextMatrix(i, COL_类别) = "7" Then
                    ...
                    '执行性质:配方中所有组成的中药相同
                    If cbo执行性质.Tag <> "" Then
                        .TextMatrix(i, COL_执行性质) = Decode(NeedName(cbo执行性质.Text), "自备药", 5, "不取药", 5, 4)
                        .TextMatrix(i, COL_执行标记) = Decode(NeedName(cbo执行性质.Text), "自取药", 1, "不取药", 2, 0)
        ...
        Else    '其它诊疗项目
            ...
            ElseIf InStr(",5,6,", .TextMatrix(lngRow, COL_类别)) > 0 Then
                ...
                '执行性质
                If cbo执行性质.Tag <> "" Then
                    .TextMatrix(lngRow, COL_执行性质) = Decode(NeedName(cbo执行性质.Text), "自备药", 5, "不取药", 5, 4)
                    .TextMatrix(lngRow, COL_执行标记) = Decode(NeedName(cbo执行性质.Text), "自取药", 1, "不取药", 2, 0)
                    If Val(.TextMatrix(lngRow, COL_执行性质)) = 5 Then
                        .TextMatrix(lngRow, COL_执行科室ID) = 0
                ...
                If Val(cmd用法.Tag) <> 0 And txt用法.Tag <> "" Then
                    ...
                    Call AdviceSet给药途径(lngRow, Val(cmd用法.Tag), NeedName(cbo执行性质.Text), strTmp)
                ElseIf blnCurDo Then    'cbo执行性质.Tag <> "" Then
                    '如果执行性质更改了,需要强行修改对应的给药途径的执行性质和执行科室
                    lngTmp = .FindRow(CLng(.TextMatrix(lngRow, COL_相关ID)), lngRow + 1)
                    Call AdviceSet给药途径(lngRow, Val(.TextMatrix(lngTmp, COL_诊疗项目ID)), NeedName(cbo执行性质.Text), strTmp)
                End If
                    ...
                    For i = lngBeginRow To .Rows - 1
                        If i <> lngRow And .RowData(i) <> 0 Then    '可能现在中间有空行
                            If Val(.TextMatrix(i, COL_相关ID)) = Val(.TextMatrix(lngRow, COL_相关ID)) Then
                                ...
                                If cbo执行性质.Tag <> "" And NeedName(cbo执行性质.Text) = "离院带药" Or NeedName(cbo执行性质.Text) = "自取药" Then
    ...
    For i = lngRow1 To vsAdvice.Rows - 1
        If Val(vsAdvice.TextMatrix(i, COL_相关ID)) = lng相关ID Then
            ...
            If Not (Val(vsAdvice.TextMatrix(i, COL_执行科室ID)) = 0 And Val(vsAdvice.TextMatrix(i, COL_执行性质)) = 5) Then
                If Have部门性质(Val(vsAdvice.TextMatrix(i, COL_执行科室ID)), "配制中心") Then
...
'说明：对非药医嘱,当前的执行科室可能是强行选择出来的,需要显示在选择框中;另选择框中增加一个其它供选择
    ...
    If str类别 = "4" Then
        strSql = _
            ...
            " And " & IIF(mint范围 = 3, "Nvl(B.服务对象,0)<>0", "B.服务对象 IN([2],3)") & " And B.部门ID=C.ID And A.收费细目ID=[3]" & _
...
'      blnBy缺省=获取缺省药房时，如果本地有指定，是否按本地缺省指定的药房来，没有则不返回
    ...
    If str类别 = "4" Then
        ...
        strSql = _
            ...
            " From 收费执行科室 A,部门性质说明 B,部门表 C" & _
            " Where A.执行科室ID+0=B.部门ID And B.工作性质='发料部门'" & _
            " And " & IIF(mint范围 = 3, "Nvl(B.服务对象,0)<>0", "B.服务对象 IN([1],3)") & " And B.部门ID=C.ID And A.收费细目ID=[2]" & _
            " And (C.撤档时间=TO_DATE('3000-01-01','YYYY-MM-DD') Or C.撤档时间 is NULL)" & _
            " Order by B.服务对象,C.编码"
        Set rsTmp = zldatabase.OpenSQLRecord(strSql, "mdlCISKernel", int范围, lng药品ID)
        If Not rsTmp.EOF Then
...
Private Function Get可用发料部门IDs(ByVal lng材料ID As Long, ByVal lng科室id As Long, Optional ByVal int范围 As Integer = 2) As String
...
'      int范围=1-门诊,2-住院(缺省)
    ...
    strSql = _
        ...
        " From 收费执行科室 A,部门性质说明 B,部门表 C" & _
        " Where A.执行科室ID+0=B.部门ID And B.工作性质='发料部门'" & _
        " And " & IIF(mint范围 = 3, "Nvl(B.服务对象,0)<>0", "B.服务对象 IN([1],3)") & " And B.部门ID=C.ID And A.收费细目ID=[3]" & _
        " And (C.撤档时间=TO_DATE('3000-01-01','YYYY-MM-DD') Or C.撤档时间 is NULL)" & _
