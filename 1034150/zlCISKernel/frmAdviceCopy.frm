...
Begin VB.Form frmAdviceCopy 
   ...
   Begin VB.Frame fraPati 
      ...
      Begin VB.PictureBox picDiag 
         ...
         Height          =   255
         Left            =   3255
         Picture         =   "frmAdviceCopy.frx":08B5
         ScaleHeight     =   255
         ScaleWidth      =   255
         TabIndex        =   18
         Top             =   210
      ...
      Begin VB.CommandButton cmdPati 
         Height          =   240
         Left            =   1950
         Picture         =   "frmAdviceCopy.frx":7107
         Style           =   1  'Graphical
         TabIndex        =   2
         TabStop         =   0   'False
         ToolTipText     =   "选择病人(F4)"
    ...
    With cboFinTim
        ...
        If intDateCount = -1 Then
            If Not frmSelectTime.ShowMe(Me, mdtOutBegin, mdtOutEnd, cboFinTim) Then
                '取消时恢复原来的选择
                Call zlControl.CboSetIndex(.hwnd, mintOutPreTime)
                Exit Sub
            End If
        Else
            mdtOutEnd = CDate(.Tag)
...
Private Sub InitSelectTime()
    ...
    Call zlControl.CboSetIndex(cboFinTim.hwnd, mintOutPreTime)
End Sub
...
'功能：读取当前病人指定的医嘱
    ...
    If mstr挂号单 <> "" Then
        strSQL = strSQL & ",A.挂号单" & _
            ...
            " And A.医嘱状态 Not IN(2,4) And A.开始执行时间 is Not Null And Nvl(A.医嘱状态,0)<>-1 And A.病人来源=1" & _
            " And A.病人ID+0=[1] And A.挂号单=R.NO And R.ID=[2] And Nvl(A.婴儿,0)=[3]" & _
            " Order by A.序号"
    Else
        strSQL = strSQL & _
            " From 病人医嘱记录 A,诊疗项目目录 B,部门表 C,收费项目目录 D,药品规格 E,诊疗适用科室 G,药品特性 H" & _
            " Where A.诊疗项目ID=B.ID(+) And A.执行科室ID=C.ID(+) And A.收费细目ID=D.ID(+)  And b.id=g.项目id(+) And h.药名ID(+)=b.ID And g.科室ID(+)=[4] And A.收费细目ID=E.药品ID(+)" & _
            " And A.医嘱状态 Not IN(2,4) And A.开始执行时间 is Not Null And Nvl(A.医嘱状态,0)<>-1 And A.病人来源=2" & _
