...
Private mlng药名ID As Long '读取指定品种药品的所有规格
...
Public Function ShowSelect(frmParent As Object, ByVal int场合 As Integer, ByVal lng病区ID As Long, ByVal lng科室id As Long, _
    ...
    Optional ByVal str操作类型 As String, Optional ByVal str执行分类 As String) As ADODB.Recordset
'功能：显示诊疗项目选择器
...
'      lng药名ID=读取指定品种药品的所有规格，传入药品的诊疗项目ID
...
Private Sub Form_Load()
    ...
    mlng西药房 = Val(zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "缺省西药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0)))
    mlng成药房 = Val(zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "缺省成药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0)))
    mlng中药房 = Val(zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "缺省中药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0)))
    mlng发料部门 = Val(zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "缺省发料部门", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0)))
    
    mstr可用西药房 = zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "可用西药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0))
    mstr可用成药房 = zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "可用成药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0))
    mstr可用中药房 = zlDatabase.GetPara(Decode(mint范围, 1, "门诊", 2, "住院", "") & "可用中药房", glngSys, Decode(mint范围, 1, p门诊医嘱下达, 2, p住院医嘱下达, 0))
    If mint场合 = 0 Then
        '字体设置
    ...
    If mint场合 <> -1 Then
        ...
        chkStock.value = Val(zlDatabase.GetPara("显示药品库存", glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), , Array(chkStock), InStr(GetInsidePrivs(IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达)), "医嘱选项设置") > 0)) '是否显示药品库存
    Else
        mbln价格 = False
        chkStock.value = 0
...
Private Sub Form_Resize()
    ...
    Else
        ...
        vsItem.Height = Me.ScaleHeight - tbrOften.Height + 15 - chkStock.Height
        
        tbrOften.Left = Me.ScaleWidth - tbrOften.Width - 15
        tbrOften.Top = vsItem.Top + vsItem.Height + chkStock.Height - 30
        
        If chkStock.Visible Then
            chkStock.Left = tbrOften.Left - chkStock.Width - 30
            chkStock.Top = tbrOften.Top + tbrOften.Height - chkStock.Height - 30
        End If
        
        If chkShowCause.Visible Then
            chkShowCause.Left = tbrOften.Left - chkStock.Width - chkShowCause.Width - 60
            chkShowCause.Top = tbrOften.Top + tbrOften.Height - chkStock.Height - 30
        End If
...
Private Sub Form_Unload(Cancel As Integer)
    '选择器中的设置
    If chkStock.Visible And chkStock.Enabled Then 'int场合<>-1
        '是否显示药品库存：都为医嘱下达模块为准
        Call zlDatabase.SetPara("显示药品库存", chkStock.value, glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), InStr(mstrPrivs, ";医嘱选项设置;") > 0)
    End If
...
'参数：blnClass=是否重建分类卡(应在树形项目改变时才重建)
    ...
    '------------------------------------------------------------------------
        If mint场合 = 2 Then
        str医技成套 = " and a.类别 <> '9' Or a.类别 = '9' And Exists (Select 1 From 诊疗适用科室 Where 项目id = a.Id And Instr([20], ',' || 科室id || ',') > 0) "
    End If
            ...
            If mint场合 <> -1 Then
                ...
                If chkShowCause.value <> 1 Then
                    If lng药房ID <> 0 Then
                        ...
                        bln显示库存 = True
                    ElseIf blnStock And chkStock.value = 1 And Not (mlng西药房 = 0 And mlng成药房 = 0 And mlng中药房 = 0) Then
                                                strStock = _
            ...
            If mstr输入 <> "" Then
                ...
                strInside = "Select " & IIF(Not mbln简码, "Distinct", "") & _
                    ...
                    " A.计算单位 as 零售单位,1 as 零售包装,A.规格,A.产地,A.费用类型," & _
                        IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & "A.说明,A.是否变价" & _
                ...
                If mbln价格 Then
                    strInside = "Select A.ID,A.类别,A.编码,A.名称,A.商品名," & IIF(mbln简码, "A.简码,", "") & _
                        " A.零售单位,A.零售包装,A.规格,A.产地,A.费用类型,A.医保大类,A.说明,Sum(Decode(A.是否变价,1,NULL,B.现价)) as 价格" & _
                ...
                ElseIf mstrLike = "" And strStock <> "" Then
                    ...
                    strInside = strInside & " Group By A.ID,A.类别,A.编码," & IIF(gbyt输入药品显示 = 1, "C2.名称 ,C1.名称,", "B.名称,") & IIF(mbln简码, "B.简码,", "") & _
                        " A.计算单位,A.规格,A.产地,A.费用类型," & IIF(mint险类 <> 0, "N.名称,", "") & "A.说明,A.是否变价" & IIF(chkShowCause.value = 1, ",a.撤档时间,A.站点,a.服务对象,b.码类", "")
                End If
                ...
                strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & " Select " & _
                        ...
                        IIF(mbln价格, ",Decode(A.价格,NULL,NULL,A.价格*" & str范围 & "包装||'/'||" & str范围 & "单位) as 价格", ",Null as 价格") & _
                        IIF(strStock <> "", _
                    ...
                    " Where A.ID=C.药品ID And C.药名ID=D.药名ID And D.药名ID=E.ID And A.类别=F.编码 And " & IIF(mstr诊疗分类 <> "", IIF(InStr(",5,6,7,", "," & mstr诊疗分类 & ",") > 0, Replace(str诊疗类别, "A.", "E."), "E.类别 = Null"), " E.类别 IN ('5','6','7')") & _
                        ...
                        IIF(strStock <> "" Or str库存限制 <> "", str库存限制, "") & str药品 & Replace(strSub, "A.", "E.") & _
                ...
                If mbln价格 Then
                    strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                        "Select A.类别 AS 类别ID,E.ID as 诊疗项目ID,A.ID as 收费细目ID," & _
                            ...
                            " D.药品剂型,Null as 项目特性,A.费用类型," & IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & "A.说明,D.处方职务 as 处方职务ID," & _
                            " Decode(A.是否变价,1,NULL,Sum(B.现价)*" & str范围 & "包装||'/'||" & str范围 & "单位) as 价格" & _
                            IIF(strStock <> "", _
                                IIF(InStr(strPriv, "显示药品库存") = 0, _
                                    ",Decode(Sign(Nvl(X.库存,0)),1,'有','') as 库存", _
                                    ",Decode(X.库存,NULL,NULL,Round(X.库存/" & str范围 & "包装,5)||" & str范围 & "单位) as 库存"), _
                        ...
                        IIF(mint险类 <> 0, "N.名称,", "") & "A.说明,D.处方职务,A.是否变价," & str范围 & "包装," & str范围 & "单位" & IIF(strStock <> "", ",X.库存", "") & ",d.抗生素,D.临床自管药"
                Else
                    strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                        " Select A.类别 AS 类别ID,E.ID as 诊疗项目ID,A.ID as 收费细目ID," & _
                            ...
                            IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & "A.说明,D.处方职务 as 处方职务ID,Null as 价格" & _
                            IIF(strStock <> "", _
                                IIF(InStr(strPriv, "显示药品库存") = 0, _
                                    ",Decode(Sign(Nvl(X.库存,0)),1,'有','') as 库存", _
                                    ",Decode(X.库存,NULL,NULL,Round(X.库存/" & str范围 & "包装,5)||" & str范围 & "单位) as 库存"), _
    ...
    If mint场合 <> -1 Then
        ...
        End If
        If (blnLoad Or blnStock And chkStock.value = 1) And mlng发料部门 <> 0 And chkShowCause.value <> 1 Then
            strStock = _
                "Select A.药品ID,Nvl(Sum(A.可用数量),0) as 库存 From 药品库存 A" & _
    ...
    If blnLoad Then
        If mstr输入 <> "" Then
            ...
            strInside = "Select " & IIF(Not mbln简码 Or blnBarcode, "Distinct", "") & _
                ...
                IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & "A.说明,A.是否变价," & IIF(blnBarcode, "C.批次 ", "NULL as 批次") & _
                IIF(chkShowCause.value = 1, ",a.撤档时间,A.站点,a.服务对象,b.码类", "") & _
            ...
            If mbln价格 Then
                strInside = "Select A.ID,A.类别,A.编码,A.名称," & IIF(mbln简码, "A.简码,", "") & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明,Sum(Decode(A.是否变价,1,NULL,B.现价)) as 价格," & IIF(blnBarcode, "a.批次", "null as 批次") & _
                    IIF(chkShowCause.value = 1, ",a.撤档时间,A.站点,a.服务对象,A.码类", "") & _
                    " From 收费价目 B,(" & strInside & ") A" & _
                    " Where A.ID=B.收费细目ID And Sysdate Between B.执行日期+0 And Nvl(B.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
            ...
            ElseIf mstrLike = "" And strStock <> "" Then
                ...
                " A.费用类型," & IIF(mint险类 <> 0, "N.名称,", "") & "A.说明,A.是否变价" & IIF(chkShowCause.value = 1, ",a.撤档时间,A.站点,a.服务对象,b.码类", "") & IIF(blnBarcode, ",C.批次 ", "")
            End If
            ...
            strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                " Select " & _
                    ...
                    IIF(mbln价格, ",Decode(A.价格,NULL,NULL,A.价格||'/'||A.计算单位) as 价格", ",Null as 价格") & _
                    IIF(strStock <> "", _
                        IIF(InStr(strPriv, "显示药品库存") = 0, _
                            ",Decode(Sign(Nvl(X.库存,0)),1,'有','') as 库存", _
                ...
                " Where A.ID=C.材料ID And C.诊疗ID=E.ID And A.类别=F.编码 And " & IIF(mstr诊疗分类 <> "", IIF(mstr诊疗分类 = "4", Replace(str诊疗类别, "A.", "E."), " E.类别 = Null"), " E.类别 ='4'") & _
                    ...
                    " And (E.服务对象 IN([8],3) Or [8]=3 And Nvl(E.服务对象,0)<>0) And Nvl(E.执行频率,0) IN(0,[9])", "") & _
                    IIF(strStock <> "", " And A.ID=X.药品ID(+)", "") & Replace(strSub, "A.", "E.")
        Else
            If mbln价格 Then
                strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                    "Select A.类别 AS 类别ID,E.ID as 诊疗项目ID,A.ID as 收费细目ID," & _
                        " F.名称 AS 类别,Null as 基本,A.编码,A.名称,Null as 商品名," & IIF(mbln简码, "Null as 简码,", "") & "A.计算单位,A.规格,A.产地,Null as 药品剂型," & _
                        " Null as 项目特性,A.费用类型," & IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & "A.说明,Null as 处方职务ID," & _
                        " Decode(A.是否变价,1,NULL,Sum(B.现价)||'/'||A.计算单位) as 价格" & _
                        IIF(strStock <> "", _
                            IIF(InStr(strPriv, "显示药品库存") = 0, _
                                ",Decode(Sign(Nvl(X.库存,0)),1,'有','') as 库存", _
                                ",Decode(X.库存,NULL,NULL,X.库存||A.计算单位) as 库存"), _
                    ...
                    " Where A.ID=C.材料ID And C.诊疗ID=E.ID And A.类别=F.编码 And " & IIF(mstr诊疗分类 <> "", IIF(mstr诊疗分类 = "4", Replace(str诊疗类别, "A.", "E."), " E.类别 = Null"), " E.类别 ='4'") & " And C.核算材料=0" & _
                        ...
                        " And (E.服务对象 IN([8],3) Or [8]=3 And Nvl(E.服务对象,0)<>0) And Nvl(E.执行频率,0) IN(0,[9])" & _
                        " And (E.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or E.撤档时间 IS NULL)" & _
                        IIF(strStock <> "", " And A.ID=X.药品ID(+)", "") & Replace(strSub, "A.", "E.") & _
                        IIF(mint险类 <> 0, " And A.ID=M.收费细目ID(+) And M.大类ID=N.ID(+) And M.险类(+)=[18]", "") & _
                        " And A.ID=B.收费细目ID And Sysdate Between B.执行日期+0 And Nvl(B.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                    " Group by A.类别,E.ID,A.ID,F.名称,A.编码,A.名称,A.计算单位,A.规格,A.产地,A.费用类型," & IIF(mint险类 <> 0, "N.名称,", "") & _
                    " A.说明,A.是否变价" & IIF(strStock <> "", ",X.库存", "")
            Else
                strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                    " Select A.类别 AS 类别ID,E.ID as 诊疗项目ID,A.ID as 收费细目ID," & _
                        " F.名称 AS 类别,Null as 基本,A.编码,A.名称 as 名称,Null as 商品名," & IIF(mbln简码, "Null as 简码,", "") & "A.计算单位,A.规格,A.产地," & _
                        " Null as 药品剂型,Null as 项目特性,A.费用类型," & IIF(mint险类 <> 0, "N.名称 as 医保大类,", "Null as 医保大类,") & _
    ...
    Set mrsItem = zlDatabase.OpenSQLRecord(strSQL, Me.Name, int类型, str类别, lng分类id, mlng分类ID, _
        ...
        "," & str性别 & ",", UserInfo.ID, lng药房ID, mlng西药房, mlng成药房, mlng中药房, mlng发料部门, mstr科室ID, mint险类, mlng药名ID, "," & UserInfo.部门ID & ",")
    ...
    For i = 1 To vsItem.Cols - 1
        ...
        ElseIf InStr(",产地,费用类型,医保大类,说明,价格,库存,", vsItem.TextMatrix(0, i)) > 0 Then
...
'参数：bytSize  0-9号字体，1-12号字体
    ...
    Call zlControl.VSFSetFontSize(vsItem, IIF(bytSize = 0, 9, 12))
