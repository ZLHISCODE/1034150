...
Private Sub cmdUnUse_Click()
    ...
    If MsgBox("确实要将选择的" & IIf(mblnICD10, "疾病", "诊断") & "从" & gobjComLib.zlCommFun.GetNeedName(cbo科室.Text) & "中取消吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
    ...
    With vsList
        If mblnMultiSel Then
            For i = .FixedRows To .Rows - 1
                If Val(.TextMatrix(i, 0)) <> 0 And .RowData(i) <> 0 Then
                    ReDim Preserve arrSQL(UBound(arrSQL) + 1)
                    If mblnICD10 Then
                        arrSQL(UBound(arrSQL)) = "Zl_疾病编码科室_Delete(" & .RowData(i) & "," & cbo科室.ItemData(cbo科室.ListIndex) & ")"
                    Else
                        arrSQL(UBound(arrSQL)) = "Zl_疾病诊断科室_Delete(" & .RowData(i) & "," & cbo科室.ItemData(cbo科室.ListIndex) & ")"
                    End If
                End If
            Next
        End If
        If UBound(arrSQL) = -1 Then
            ...
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            If mblnICD10 Then
                arrSQL(UBound(arrSQL)) = "Zl_疾病编码科室_Delete(" & .RowData(.Row) & "," & cbo科室.ItemData(cbo科室.ListIndex) & ")"
            Else
                arrSQL(UBound(arrSQL)) = "Zl_疾病诊断科室_Delete(" & .RowData(.Row) & "," & cbo科室.ItemData(cbo科室.ListIndex) & ")"
            End If
        End If
    End With
...
'功能：初始化医嘱清单格式
    ...
    If mblnICD10 Then
        strHead = ",255,4;编码,1000,1;附码,550,1;名称,2500,1;说明,1500,1"
    Else
        strHead = ",255,4;编码,1000,1;名称,2500,1;说明,1500,1;编者,850,1"
    End If
...
Private Sub FillListData()
    ...
    If mblnICD10 Then
        ...
        If cbo科室.ItemData(cbo科室.ListIndex) <> 0 Then
            strSQL = _
                " Select A.ID as 项目ID,A.编码,A.序号,A.附码,A.名称,A.说明" & _
        ...
        Else
            strSQL = "Select A.ID as 项目ID,A.编码,A.序号,A.附码,A.名称,A.说明 From 疾病编码目录 A" & _
    ...
    Else
        ...
        Else
            strSQL = "Select A.ID as 项目ID,A.编码,A.名称,A.说明,A.编者" & _
                ...
                " Where Instr([2],A.类别)>0 And A.ID=B.诊断ID And B.分类ID=[3]" & _
                " Order by A.编码"
        End If
...
Private Sub vsList_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    If vsList.RowData(vsList.Row) <> 0 And cbo科室.ListIndex > 0 Then
        For i = 0 To cbo常用.ListCount - 1
            If cbo科室.ItemData(cbo科室.ListIndex) = cbo常用.ItemData(i) Then
