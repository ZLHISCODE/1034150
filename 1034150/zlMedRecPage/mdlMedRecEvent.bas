...
Private mstrMsgXY As String
Private mstrMsgZY As String
...
'返回：Ture-成功，False-失败
    ...
    With gclsPros.CurrentForm
        ...
        If gclsPros.MedPageSandard <> ST_门诊首页 Then
            If Not blnChange Then
                ...
                If gBlnNew = True And (Not gfrmMecCol Is Nothing) Then
                    ...
                    For i = 1 To gfrmMecCol.Count
                        ...
                        gfrmMecCol(i).Top = IIf(gclsPros.FuncType = f病案首页, IIf(gclsPros.MedPageSandard = ST_卫生部标准, -300, -1900), -300)
                        gfrmMecCol(i).Left = 0: gfrmMecCol(i).Tag = gPic外挂附页
                        gfrmMecCol(i).Show
            ...
            Call SetAllObject
        Else
            gclsPros.CurrentForm.cmdModify.Visible = InStr(GetInsidePrivs(p病人信息公共部件), "基本信息调整") > 0
        End If
...
'CboBaseInfo_KeyPress事件
    ...
    If intKeyAscii = vbKeyReturn And (intIndex = BCC_民族 Or intIndex = BCC_职业 Or intIndex = BCC_国籍) And cboTmp.Style = 0 Then
        strInput = Trim(cboTmp.Text)
        If strInput = "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
        '相同的项目则不进行处理
        If cboTmp.ListIndex <> -1 Then
            If zlStr.NeedName(strInput) = zlStr.NeedName(cboTmp.List(cboTmp.ListIndex)) Then
                zlCommFun.PressKey vbKeyTab
...
'功能：monInfo_KeyDown
    If intKeyCode = 13 Then Call zlCommFun.PressKey(vbKeyTab)
End Sub
...
'功能：修改病人基本信息
    ...
    If gobjPatient.ModiPatiBaseInfo(gclsPros.CurrentForm, "住院首页", gclsPros.病人ID, gclsPros.主页ID, gclsPros.PatiType, False) Then
        Set gclsPros.PatiInfo = GetPatiMainInfoData(gclsPros.病人ID, gclsPros.主页ID, IIf(gclsPros.PatiType = PF_门诊, gclsPros.RegistNo, "")) '病案主页以及病人信息
    ...
    Case GC_转入医疗机构, GC_入院转入
        ...
        Else
            ...
            If strSql <> "" Then
                ...
                Else
                    objTXTBox.Tag = ""
                    If zlDatabase.GetPara("医疗机构不允许自由录入", 100, , 0) = "1" Then
...
'vsICUInstruments_ValidateEdit
    ...
    With vsICUInstruments
        ...
        Select Case LngCol
            ...
            Case TI_器械及导管
                For i = 1 To .Rows - 1
                    If .TextMatrix(i, TI_器械及导管) = .EditText And Val(.Cell(flexcpData, i, TI_ICU类型)) = Val(.Cell(flexcpData, LngRow, TI_ICU类型)) And .Cell(flexcpData, LngRow, TI_ICU类型) <> "" And i <> LngRow Then
                        MsgBox "同一个ICU记录不能对应多个相同的器械。", vbInformation, gstrSysName
                        blnCancel = True
                        Exit Sub
                    End If
                Next
        ...
        ElseIf intKeyCode = vbKeyDelete Then
            If (.TextMatrix(.Row, DI_诊断描述) <> "" Or gclsPros.PatiType = PF_门诊 And .Rows = .FixedRows + 1) And Not (gclsPros.FuncType = f病案首页 And gclsPros.CNIndent) Or gclsPros.FuncType = f病案首页 And gclsPros.CNIndent Then
                ...
                If MsgBox("确实要清除该行诊断信息吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
                    '删除主/次要诊断后调用外挂接口
                    If Not gobjPlugIn Is Nothing And gclsPros.PatiType = PF_门诊 Then
                        On Error Resume Next
                        Call gobjPlugIn.DiagnosisDeleted(gclsPros.SysNo, p门诊医生站, gclsPros.病人ID, gclsPros.主页ID, Val(.TextMatrix(.Row, DI_诊断ID)), .TextMatrix(.Row, DI_诊断描述))
            ...
            Select Case .Col
                ...
                Case DI_诊断编码, DI_诊断描述, DI_中医证候, DI_ICD附码 '西医中医证候隐藏,中医无ICD附码隐藏
                    ...
                    Else
                        .ComboList = "" '使按钮状态进入输入状态
                        Clipboard.Clear
                        If vsDiag.Name = "vsDiagXY" Then
                            If .TextMatrix(.MouseRow, DI_诊断分类) = DT_出院诊断XY And .MouseCol = DI_诊断编码 Then
                                mstrMsgXY = .TextMatrix(.MouseRow, DI_诊断编码) & ";" & .TextMatrix(.MouseRow, DI_诊断描述) & ";" & .TextMatrix(.MouseRow, DI_备注)
                                Debug.Print mstrMsgXY
                                Clipboard.SetText mstrMsgXY
                            End If
                        ElseIf vsDiag.Name = "vsDiagZY" Then
                            If .TextMatrix(.MouseRow, DI_诊断分类) = DT_出院诊断ZY And .MouseCol = DI_诊断编码 Then
                                mstrMsgZY = .TextMatrix(.MouseRow, DI_诊断编码) & ";" & .TextMatrix(.MouseRow, DI_诊断描述) & ";" & .TextMatrix(.MouseRow, DI_中医证候) & ";" & .TextMatrix(.MouseRow, DI_备注)
                                Debug.Print mstrMsgZY
                                Clipboard.SetText mstrMsgZY
                            End If
                        End If
                    End If
            End Select
        ...
        Select Case LngCol
            ...
            Case PI_主刀医师, PI_助手1, PI_助手2, PI_麻醉医师
                ...
                Else
                    ...
                    If rsTmp Is Nothing Then
                        If Not blnInputCancel Then
                            If (LngCol = PI_主刀医师 Or LngCol = PI_助手1 Or LngCol = PI_助手2) And zlCommFun.IsCharChinese(.EditText) And Not gclsPros.IsOutDocCtrl Then
...
'vsSpirit_KeyDown事件
    ...
    With vsSpirit
        ...
        Else
            If .Row = .Rows - 1 Then
                If Trim(.TextMatrix(.Row, SI_药物名称)) = "" Then Call zlCommFun.PressKey(vbKeyTab)
            End If
        End If
    End With
End Sub
    ...
    With vsDiagTmp
        ...
        If gclsPros.PatiType <> PF_门诊 Then
                ...
                For i = lngBegin To lngEnd
                    ...
                    Select Case strDiagTypeName
                        ...
                        Case "其他诊断", "出院诊断"
                            Call SetDiagMatchInfo(IIf(bln西医, BCC_门诊与出院XY, BCC_门诊与出院ZY))
                            Call SetDiagMatchInfo(IIf(bln西医, BCC_入院与出院XY, BCC_入院与出院ZY))
                            If bln西医 And (strDiagTypeName = "出院诊断" Or strDiagTypeName = "其他诊断") Then
                                '根据出院诊断相关信息设置相关控件属性
                                strTmp = UCase(Trim(.TextMatrix(i, DI_诊断编码)))
                                If Val(.TextMatrix(i, DI_诊断分类)) = DT_出院诊断XY Then
            ...
            If bln西医 Then
                ...
                Next
                blnOld分化程度 = gclsPros.CurrentForm.cboBaseInfo(BCC_分化程度).Locked
                Call SetCtrlLocked(gclsPros.CurrentForm.cboBaseInfo(BCC_分化程度), Not bln分化程度, Not bln分化程度)
                Call SetCtrlLocked(gclsPros.CurrentForm.cboBaseInfo(BCC_最高诊断依据), Not bln分化程度, Not bln分化程度)
                If gclsPros.CurrentForm.Visible And bln分化程度 And blnOld分化程度 Then
                    Call SetCboDefaultValue(BCC_分化程度)
                    Call SetCboDefaultValue(BCC_最高诊断依据)
                End If
...
Public Sub DiagMouseDown(ByRef vsDiag As VSFlexGrid, ByRef intButton As Integer, ByRef intShift As Integer, ByRef sngX As Single, ByRef sngY As Single)
    ...
    If intButton = 2 Then
        ...
        End If
    ElseIf intButton = 1 Then
        With vsDiag
            LngRow = .MouseRow
            LngCol = .MouseCol
            If LngRow >= vsDiag.FixedRows And LngRow <= vsDiag.Rows - 1 Then
                If vsDiag.Name = "vsDiagXY" Then
                    If LngRow = DT_门诊诊断XY And LngCol = DI_诊断编码 Then
                        Debug.Print mstrMsgXY
                        If mstrMsgXY <> "" Then
                            strArryMsg = Split(mstrMsgXY, ";")
                            For i = 0 To UBound(strArryMsg)
                                If i = 0 Then
                                    .TextMatrix(LngRow, DI_诊断编码) = strArryMsg(i)
                                ElseIf i = 1 Then
                                    .TextMatrix(LngRow, DI_诊断描述) = strArryMsg(i)
                                ElseIf i = 2 Then
                                    .TextMatrix(LngRow, DI_备注) = strArryMsg(i)
                                End If
                            Next
                            mstrMsgXY = ""
                            Debug.Print mstrMsgXY
                    ...
                    End If
                ElseIf vsDiag.Name = "vsDiagZY" Then
                    If .TextMatrix(LngRow, DI_诊断类型) = "门（急）诊诊断" And LngCol = DI_诊断编码 Then
                        Debug.Print mstrMsgZY
                        If mstrMsgZY <> "" Then
                            strArryMsg = Split(mstrMsgZY, ";")
                            For i = 0 To UBound(strArryMsg)
                                If i = 0 Then
                                    .TextMatrix(LngRow, DI_诊断编码) = strArryMsg(i)
                                ElseIf i = 1 Then
                                    .TextMatrix(LngRow, DI_诊断描述) = strArryMsg(i)
                                ElseIf i = 2 Then
                                    .TextMatrix(LngRow, DI_中医证候) = strArryMsg(i)
                                ElseIf i = 3 Then
                                    .TextMatrix(LngRow, DI_备注) = strArryMsg(i)
                                End If
                            Next
