...
'返回：病案主页信息或病人信息
    ...
    Else
        ...
        Else
            strSql = "Select D.病人id, [2] 主页id, Nvl(A.姓名, D.姓名) As 姓名, Nvl(A.性别, D.性别) As 性别, Nvl(A.年龄, D.年龄) As 年龄, Nvl(A.职业, D.职业) 职业," & vbNewLine & _
...
'返回：病人的手麻信息
    ...
    If gclsPros.FuncType <> f病案首页 Then
        strSql = "Select A.Id, A.病人id, A.主页id, A.手术情况, A.记录来源, A.手术日期, A.手术开始时间, A.手术结束时间, Nvl(B.编码, C.编码) As 手术编码, A.已行手术 手术名称," & vbNewLine & _
                ...
                "       A.切口部位, A.麻醉类型, Decode(A.Asa分级, 'I级', 'P1', 'II级', 'P2', 'III级', 'P3', 'IV级', 'P4', 'V级', 'P5', A.Asa分级) Asa分级, A.Nnis分级, Decode(A.手术级别, 1, '一级手术', 2, '二级手术', 3, '三级手术', 4, '四级手术',9, '〇级手术', ' ') As 手术级别, A.切口," & vbNewLine & _
    ...
    Else
        If bln编目 Then
            strSql = "Select A.Id, A.手术情况, A.病人id, A.主页id, A.记录来源, A.手术日期, A.手术开始时间, A.手术结束时间, B.编码 As 手术编码, A.已行手术 手术名称, B.名称 手术原名, A.主刀医师," & vbNewLine & _
                    ...
                    "       Decode(A.手术级别, 1, '一级手术', 2, '二级手术', 3, '三级手术', 4, '四级手术',9, '〇级手术', ' ') As 手术级别, A.切口, A.愈合, A.再次手术, A.术前抗菌用药, A.非预期的二次手术," & vbNewLine & _
    ...
    With rsReturn
            ...
            Else
                While Not rsTmp.EOF
                    ...
                    strSql = "Select a.Id 手麻手术id, a.记录性质, A.手术名称, b.名称 手术原名, Nvl(D.编码,B.编码) 手术编码 , a.诊疗项目id, d.Id 手术操作id, a.主手术, a.切口分类 切口, a.愈合情况 愈合," & vbNewLine & _
                        "Decode(d.手术类型, '甲', '四级手术', '乙', '三级手术', '丙', '二级手术', '丁', '一级手术', '四级', '四级手术', '三级', '三级手术', '二级', '二级手术', '一级', '一级手术', Null) 手术级别,d.名称 对应手术名称" & vbNewLine & _
                    ...
                    Set rsOther = zlDatabase.OpenSQLRecord(strSql, "读取手术操作信息", Val(rsTmp!ID & ""))
                    If Not rsOther.EOF Then
                        !已行手术 =IIf(Val(gclsPros.OPSInput) = 0, rsOther!手术名称, rsOther!对应手术名称)
                        !手术原名 = rsOther!手术原名
        ...
        If blnReadAdvice Then
            ...
            While Not rsTmp.EOF
                ...
                !手术结束时间 = rsTmp!手术结束时间
                !手术操作ID = rsTmp!手术操作ID
                !已行手术 = IIf(Val(gclsPros.OPSInput) = 0, rsTmp!手术名称, rsTmp!对应手术名称)
                !手术级别 = rsTmp!手术级别
                '麻醉信息读取
...
'返回=True-存在该号码,False-不存在该号码
    ...
    Select Case intType
        ...
        Case CT_档案号
            strSql = "Select A.档案号" & vbNewLine & _
                "From 住院病案记录 A" & vbNewLine & _
                "Where A.档案号 = [1] And Not Exists" & vbNewLine & _
                " (Select 1 From 住院病案记录 Where 病人id = [2] And 主页id = [3] And A.病人id = 病人id And A.主页id = 主页id)"
        Case CT_病案号
