...
'    Call SQLTest
    ...
    If bln检查站点 Then
        ...
        Set rsTmp = OpenSQLRecord(strSQL, "检查并确定所属院区", gclsLogin.DBUser)
        If rsTmp Is Nothing Then
            MsgBox Err.Description & vbNewLine & "不能正常访问系统，请您联系系统管理员重新进行角色授权！", vbInformation, gstrSysName
            Exit Function
        End If
...
Private Sub UpdateEmrInterface()
    ...
    If Err.Number = 0 Then
        Call objEMR.CheckUpdate1(gclsLogin.InputUser, IIf(gclsLogin.IsTransPwd, "", "[DBPASSWORD]") & gclsLogin.InputPwd, IIf(gstrCommand <> "", False, True))
        If Err.Number <> 0 Then
            Err.Clear
            Call objEMR.CheckUpdate(gclsLogin.InputUser, IIf(gclsLogin.IsTransPwd, "", "[DBPASSWORD]") & gclsLogin.InputPwd)
        End If
