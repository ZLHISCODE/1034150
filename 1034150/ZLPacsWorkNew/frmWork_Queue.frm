...
Public Event OnCalled(ByVal lngAdviceID As Long, ByVal strRoom As String)
...
'读取排队叫号参数
    ...
    mstrCurTechnicRoomName = Trim(NeedNo(strRoomName))
    mstrCurTechnicDevice = Trim(NeedName(strRoomName))
...
'呼叫后触发事件
    ...
    If lngCallWay = cwOrder Or lngCallWay = cwSpecify Or lngCallWay = cwWaitRoom Then
        ...
        RaiseEvent OnCalled(lngAdviceID, strRoom)
    End If
End Sub
