...
Private Sub CmdOK_Click()
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        ToVarchar(Txt体重, 16) & "',Null,Null,Null,Null,Null,'" & txt附加主述.Text & "',Null," & mlngCurDeptId & ",'" & NeedName(cbo待处理人.Text) & "')"
    ...
    If mintEditMode = 3 Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查记录_UPDATE(" & mlngAdviceID & ", " & mlngSendNo & ",Null,'" & _
            Trim(Me.PatiIdentify.Text) & "','" & Trim(Txt英文名.Text) & "','" & NeedName(cbo性别.Text) & "','" & _
            txt年龄.Text & IIf(cboAge.Visible, cboAge.Text, "") & "'," & To_Date(dtp出生日期.value) & ",'" & ToVarchar(Txt身高, 16) & "','" & _
            ToVarchar(Txt体重, 16) & "',Null,Null,Null,'" & txt附加主述.Text & "',Null," & To_Date(dtp(1).value) & ",'" & NeedName(cbo待处理人.Text) & "')"
...
'返回： True--复制成功；False--复制信息不完整
    ...
    gstrSQL = "SELECT nvl(B.姓名,E.姓名) 姓名,nvl(B.性别,E.性别) 性别,nvl(B.年龄,E.年龄) 年龄,B.出生日期,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",F.发送时间 开嘱时间,NVL(C.简码,0) 科室简码,NVL(C.名称,'未知') AS 开嘱科室,A.开嘱医生,A.紧急标志,F.首次时间,F.执行间,E.检查设备,A.医嘱内容,E.检查号,E.检查技师" & _
...
Dim i As Integer
    ...
    gstrSQL = "SELECT I.名称 as 号别名称,H.病理号,H.检查类型,nvl(B.姓名,E.姓名) 姓名,nvl(B.性别,E.性别) 性别,nvl(B.年龄,E.年龄) 年龄,B.出生日期,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",F.发送时间 开嘱时间,NVL(C.简码,0) 科室简码,NVL(C.名称,'未知') AS 开嘱科室,A.开嘱医生,A.紧急标志,F.首次时间,F.执行间,E.检查设备,A.医嘱内容,E.检查号,E.报告人,E.检查技师" & _
                ...
                " Where H.号码规则ID=I.ID(+) And F.医嘱ID=[1] And F.发送号=[2] AND F.医嘱ID=A.ID and F.医嘱ID=H.医嘱ID(+)" & _
                        " AND F.医嘱ID=E.医嘱ID(+) And F.发送号=E.发送号(+)  And A.病人ID=B.病人ID" & _
                        " And A.开嘱科室ID=C.ID And A.病人科室ID=D.ID And A.诊疗项目ID=G.诊疗项目ID(+)"
...
Normal:
    ...
    Else
        ...
        Else
            ...
            If bln报到 Or mintEditMode = 1 Then
                If bln报到 And IsNull(rsTemp!出生时间) Then
                ...
                End If
            Else
                ReCalcOld Format(Nvl(rsBaby!出生时间, curDate), "yyyy-mm-dd hh:mm:ss"), cboAge
            End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\" & Me.Name, "待处理人", NeedName(cbo待处理人.Text)
