...
'lngType:窗口类型，0-图像处理窗口；1-图像预览窗口；2-标记图处理窗口
    ...
    If mblnIsChanged Then
        If MsgBox("图像处理尚未保存，该操作将清空这些处理，是否继续？", vbYesNo, Me.Caption) = vbNo Then
            Exit Sub
'        Else
...
'        ClearLable Me.DViewer.Images(1)
        Me.DViewer.Images(1).Labels.Clear
    End If
...
'------------------------------------------------
    ...
    If mlngWinType = 1 And mlngState = 2 Then
        mlngState = 3
        Timer1.Enabled = False
        Timer2.Enabled = False
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    If mblnIsChanged Then
        If MsgBox("图像处理尚未保存，是否继续退出？", vbYesNo, Me.Caption) = vbNo Then
            Cancel = 1
            Exit Sub
        End If
    End If
...
Private Sub ucMiniature_OnDbClick(ByVal lngSelectedIndex As Long, blnContinue As Boolean)
On Error GoTo err
...
'            End If
            If mblnIsChanged Then
                If MsgBox("图像操作尚未保存，是否继续？", vbYesNo, Me.Caption) = vbNo Then
                    Exit Sub
                End If
            End If
        End If
...
Public Sub AfterSaveStudy(dcmImage As DicomImage)
    ...
    ucMiniature.AddImage dcmImage, ucPage.PageRecord
