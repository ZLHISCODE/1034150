...
'图片展示方式(按采集时间顺序或倒序)
Property Get IsShowImgDesc() As Boolean
    IsShowImgDesc = gblnShowImgDesc
End Property

Property Let IsShowImgDesc(value As Boolean)
    gblnShowImgDesc = value
End Property
...
'根据报告图象 字段获取相关图像
    ...
    strSql = "Select rownum As 顺序号,a.医嘱id,a.姓名,a.性别,a.年龄, rownum As 图像号,Replace(Trim(D.Column_Value),'.jpg','') as 图像UID, A.检查UID, " & _
            "'' As 序列UID, 0 as 动态图,'' as 编码名称,'' as 采集时间, '' as 录制长度, " & _
...
'intSearchType:0-按检查uid搜索,1-按序列UID搜索,2-按图像UID搜索
    ...
    strSql = "select * from (" & strSql & ") where 顺序号>=" & lngStartRecord & " and 顺序号<=" & lngEndRecord
...
'增加图像
    Dim i As Long
    Dim lngImgIndexDoBorder As Long
    
    If gblnShowImgDesc Then
        '新图像靠左上的处理
        ...
        Call dcmMiniImage.Images.Move(dcmMiniImage.Images.Count, 1)
        
        lngImgIndexDoBorder = 1
        
        '倒序情况选中图像索引自动加一  用于后面 UpdateSelectIndex 取消红框的处理。 UpdateSelectIndex 中 mlngSelectIndex 会被更新，因此这里改变mlngSelectIndex对程序无其他影响
        If mlngSelectIndex <> dcmMiniImage.Images.Count Then mlngSelectIndex = mlngSelectIndex + 1
    ...
    Else
        ...
        lngImgIndexDoBorder = dcmMiniImage.Images.Count
    End If
    
    '设置选中的边框颜色
    With dcmMiniImage.Images(lngImgIndexDoBorder)
        .BorderWidth = 1
        .BorderStyle = 6
        .BorderColour = vbRed
        ...
        If Not .Tag Is Nothing Then
            If UCase(TypeName(.Tag)) = UCase("clsImageTagInf") Then Call ShowAVInf(dcmMiniImage.Images(lngImgIndexDoBorder), .Tag)
        End If

    End With
    
    If mblnIsShowCheckbox Then Call DrawImageSelectBorder(dcmMiniImage)
    
    Call UpdateSelectIndex(lngImgIndexDoBorder)
            ...
            If mblnEnable Then
                ...
                If Shift = 0 Then
                    If blnClickCheck = False Then
                        For i = 0 To dcmMiniImage.Images.Count
                            If i <> lngImgIndex Then ImgChecked(i) = False
                        Next i
                        
                        ImgChecked(lngImgIndex) = Not ImgChecked(lngImgIndex)
                    End If
                Else
                    If blnClickCheck = False Then ImgChecked(lngImgIndex) = Not ImgChecked(lngImgIndex)
                End If
...
On Error GoTo errHandle
    ...
    If blnShowImg Then        '显示图像
        ...
        If intCurrImg <> 0 And intCurrImg <> mintImage Then
            ...
            '加载图像并显示
                Call ShowImageProcess(intCurrImg, 1, mlngPreViewTime)
...
Public Sub ShowImageProcess(intIndex As Integer, lngType As Long, lngTime As Long)
    
    If mobjImageProcess Is Nothing Then
        Set mobjImageProcess = New clsImageProcess
    End If
    
    mobjImageProcess.ShowImageProcess mstrQueryValue, dcmMiniImage.Images(intIndex), ucPage.PageRecord * (ucPage.PageNumber - 1) + intIndex, Me, mblnMoved, mslQueryLevel, lngType, lngTime, mblnDo
End Sub
