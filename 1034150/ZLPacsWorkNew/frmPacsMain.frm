...
Public Sub ShowStation(ByVal lngModule As Long, owner As Object)
    ...
    If mSysPar.lngImageValid > 0 Then
        If Len(Dir(Mid(App.Path, 1, InStrRev(App.Path, "\")) & "zlPacsImageValid.exe")) > 0 Then
           Shell Mid(App.Path, 1, InStrRev(App.Path, "\")) & "zlPacsImageValid.exe   " & "" & "||" & "" & "||" & "" & "||" & mlngCur科室ID & "||" & mSysPar.lngImageValid & "||" & gcnOracle.ConnectionString & "||2", 1
        End If
    End If
End Sub
...
On Error GoTo errHandle
    ...
    If mSysPar.blnFinishCommit And CheckPopedom(mstrPrivs, "检查完成") Then '无报告完成后无需再次确认完成,但需要有检查完成的权限
        ...
        If mblnAllDepts Then
            If mListAdviceInf.lngExeDepartmentId > 0 Then
                lngID = mListAdviceInf.lngExeDepartmentId
            Else
                lngID = 0
            End If
        Else
        ...
        If mrtReportType = 报告文档编辑器 Then
            gstrSQL = "ZL_影像检查_状态更新(" & mListAdviceInf.lngAdviceID & "," & mListAdviceInf.lngSendNO & ",''," _
            & "6,1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & To_Date(zlDatabase.Currentdate) & ")"
        Else
            gstrSQL = "ZL_影像检查_STATE(" & mListAdviceInf.lngAdviceID & "," & mListAdviceInf.lngSendNO & "," _
            & "6,1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & To_Date(zlDatabase.Currentdate) & ")"
        End If
    Else
        If mrtReportType = 报告文档编辑器 Then
    ...
    If lngIndex <= 0 And lngAdviceID > 0 Then
        ...
        If mlngModule <> G_LNG_PATHOLSYS_NUM Then
            strSQL = "Select A.ID,A.姓名, A.性别,A.年龄, A.病人科室id, A.开嘱医生,A.病人来源, A.医嘱内容,A.紧急标志,  Nvl(A.婴儿, 0) 婴儿,A.病人id,e.当前床号,e.住院号,e.门诊号,decode(A.病人来源,2,F.病人类型,E.病人类型) AS 病人类型, " & vbNewLine & _
                    " A.主页id, A.挂号单, B.检查号,B.影像类别,B.绿色通道,B.检查技师, B.检查uid,B.图像位置,B.报告人,B.报告操作,B.关联ID, C.名称, D.发送号,D.执行状态,D.执行过程,D.执行间, 0 as 转出,A.执行科室ID " & vbNewLine & _
        ...
        Else
            strSQL = "Select A.ID,A.姓名, A.性别,A.年龄, A.病人科室id, A.开嘱医生,A.病人来源, A.医嘱内容, A.紧急标志, Nvl(A.婴儿, 0) 婴儿, A.病人id,F.当前床号,F.住院号,F.门诊号,decode(A.病人来源,2,G.病人类型,F.病人类型) AS 病人类型, " & vbNewLine & _
                    " A.主页id, A.挂号单, E.病理号,B.待处理人,B.影像类别,B.绿色通道,B.检查技师, B.检查uid,B.图像位置,B.报告人,B.报告操作,B.关联ID, C.名称, D.发送号,D.执行状态,D.执行过程,D.执行间,0 as 转出,A.执行科室ID " & vbNewLine & _
...
On Error GoTo errHandle
    ...
    If mblnAllDepts Then
        If curAdviceInf.lngExeDepartmentId > 0 Then
            lngID = curAdviceInf.lngExeDepartmentId
        Else
            lngID = 0
        End If
    Else
    ...
    If mrtReportType = 报告文档编辑器 Then
        strSQL = "Zl_影像检查_状态更新(" & curAdviceInf.lngAdviceID & "," & curAdviceInf.lngSendNO & ",''," _
        & "6,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & To_Date(zlDatabase.Currentdate) & ")"
        Call zlDatabase.ExecuteProcedure(strSQL, "改变检查过程")
    Else
        strSQL = "ZL_影像检查_STATE(" & curAdviceInf.lngAdviceID & "," & curAdviceInf.lngSendNO & "," _
        & "6,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & To_Date(zlDatabase.Currentdate) & ")"
        Call zlDatabase.ExecuteProcedure(strSQL, "改变检查过程")
    End If
        ...
        Case conMenu_Manage_PacsCriticalReg     '危急患者登记
            If mcurAdviceInf.lngPatientFrom = 1 Then        '门诊
                Call mobjPublicAdvice.ShowAppCritical(Me, True, 0, 1, _
                            ...
                            mcurAdviceInf.lngAdviceID, , , , mlngCur科室ID, UserInfo.姓名, mobjMsgCenter.Msg)
...
On Error GoTo errHandle
    ...
    If mrtReportType = 报告文档编辑器 Then
        blnCanPrint = ufgStudyList.CurText("紧急") = "  " Or ufgStudyList.CurText("绿色通道") = "  "
...
On Error GoTo ErrorHand
    ...
    strSQL = "Select a.id,a.名称 as 程序名称,a.是否启用 as 程序启用,a.执行类型,b.功能序号,b.名称 as 功能名称,b.是否启用 as 功能启用,b.是否加入右键菜单,b.是否加入工具栏,b.vbs脚本 from 影像插件挂接 a, 影像插件功能 b " & _
             "Where a.id = b.插件id And (a.所属模块=0 or a.所属模块=[1]) and a.是否启用=1 and  b.是否启用=1 Order By a.id,b.功能序号"
...
On Error GoTo errHandle
    ...
    If Not mobjWork_ImageCap Is Nothing Then
        With mobjWork_ImageCap
            ...
            Call .zlUpdateStudyInf(mcurAdviceInf.lngAdviceID, mcurAdviceInf.lngSendNO, mcurAdviceInf.intStep, mcurAdviceInf.intMoved)
...
Private Function GetPatholExecuteStateByList(ByVal lngRow As Long) As String
    Dim strState As String
    ...
    GetPatholExecuteStateByList = strState
End Function
...
On Error GoTo errHandle
    ...
    Select Case lngEventType
        ...
        Case TVideoEventType.vetImgCaped '发生采集图像 用于插件自动执行
        Case TVideoEventType.vetUseAfterImage, TVideoEventType.vetNotUseAfterImage
            If lngEventType = TVideoEventType.vetUseAfterImage And mlngModule = 1291 Then
...
Private Sub mobjQueue_OnCalled(ByVal lngAdviceID As Long, ByVal strRoom As String)
...
'---------------------------工具----------------
        Case conMenu_Tool_Valid         '图像校对工具
            ...
            If Len(Dir(Mid(App.Path, 1, InStrRev(App.Path, "\")) & "zlPacsImageValid.exe")) > 0 Then
               Shell Mid(App.Path, 1, InStrRev(App.Path, "\")) & "zlPacsImageValid.exe   " & "" & "||" & "" & "||" & "" & "||" & mlngCur科室ID & "||" & mSysPar.lngImageValid & "||" & gcnOracle.ConnectionString & "||1", 1
            End If
...
Private Sub ExecuteCustomQuery(ByVal lngSchemeId As Long)
    ...
    If mlngModule <> G_LNG_PATHOLSYS_NUM Then
        ...
        strSQL = "Select  Distinct" & vbNewLine & _
                    ...
                    "       Nvl(B.紧急标志, 0) 紧急标志, Nvl(B.婴儿, 0) 婴儿,B.开嘱医生,A.NO,C.当前床号,C.当前病区ID,Decode(B.病人来源,2,D.住院号,C.门诊号) 标识号," & vbNewLine & _
...
Private Sub Menu_View_Filter_click()
    ...
    With mfrmPACSFilter
        ...
        If NeedName(.cbo待处理人.Text) = "所有医生" Then
            SQLCondition.待处理人 = ""
        Else
            SQLCondition.待处理人 = NeedName(.cbo待处理人.Text)
        End If
...
'                End If
    ...
    Select Case control.ID
        ...
        Case conMenu_Manage_Release, conMenu_Manage_ReportFilmRelease     '报告发放,报到后，完成后都可以执行
            ...
            If mrtReportType = 报告文档编辑器 Then
                If control.ID = conMenu_Manage_ReportFilmRelease Then
                    If mobjWork_Report.GetReportReleaseState(mcurAdviceInf.lngAdviceID) = 3 And Nvl(ufgStudyList.CurText("胶片发放"), "0") = "√" Then
            ...
            Else
                If Not blnNoRecord Then
                  '修改报告发放按钮的标题
                     If Not blnNoRecord Then
                         If Nvl(ufgStudyList.CurText("报告发放"), "0") = "√" And Nvl(ufgStudyList.CurText("胶片发放"), "0") = "√" Then
                             control.Caption = "收回"
                             control.ToolTipText = "收回已经发放的报告或胶片"
                         Else
                             control.Caption = "发放"
        ...
        Case conMenu_Manage_FilmRelease
            ...
            If Not blnNoRecord Then
                 If Nvl(ufgStudyList.CurText("胶片发放"), "0") = "√" Then
                     control.Caption = "胶片收回"
                     control.ToolTipText = "收回已经发放的胶片"
                     
                     If CheckPopedom(mstrPrivs, "取消发放") Then
                         control.Enabled = True
                     Else
                         control.Enabled = False
                     End If
...
'            End If
        ...
        Case conMenu_Manage_Disease, conMenu_Manage_DiseaseQuery, conMenu_Manage_DiseaseRegist
            ...
            If control.ID = conMenu_Manage_Disease Then
                control.Visible = CheckPopedom(mstrPublicAdvicePrivs, "传染病阳性结果登记") Or CheckPopedom(mstrPublicAdvicePrivs, "传染病阳性结果查询")
                control.Enabled = mListAdviceInf.lngAdviceID > 0
            ElseIf control.ID = conMenu_Manage_DiseaseQuery Then
                control.Visible = CheckPopedom(mstrPublicAdvicePrivs, "传染病阳性结果查询")
                control.Enabled = mListAdviceInf.lngAdviceID > 0
            Else
                control.Visible = CheckPopedom(mstrPublicAdvicePrivs, "传染病阳性结果登记")
...
'    mSysPar.lngCriticalValues = Val(GetDeptPara(mlngCur科室ID, "危急情况判断", 0))           '危急情况判断
    ...
    If CheckPopedom(mstrPrivs, "排队叫号") And mlngModule <> G_LNG_PATHOLSYS_NUM Then     '有权限使用才根据参数启用
...
Private Sub InitForm()
    ...
    mblnSetXWParam = CheckPopedom(";" & GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE) & ";", "PACS参数设置")
    ...
    If mSysPar.blnUseQueue And CheckPopedom(";" & GetPrivFunc(glngSys, 1160) & ";", "基本") Then
        Set mobjQueue = New frmWork_Queue
        Call mobjQueue.zlInitPacsQueueCfg(mlngModule, mlngCur科室ID, NeedName(mstrCur科室), mstrPrivs)
    Else
        Set mobjQueue = Nothing
...
Private Sub RefreshStatusBarInf()
    ...
    For i = 1 To ufgStudyList.GridRows - 1
        strStudyProName = ufgStudyList.Text(i, "检查过程")
        If Trim(strStudyProName) = "" Then
            strTag = ufgStudyList.Text(i, "报告操作")
            strStudyProName = IIf(Val(ufgStudyList.Text(i, "执行状态")) = 2, "已拒绝", _
                                                                Decode(Val(ufgStudyList.Text(i, "检查状态")), -1, "已驳回", 0, "已登记", 1, "已登记", _
                                                                2, IIf(strTag <> "", "处理中", _
                                                                        IIf(ufgStudyList.Text(i, "报告人") = "", "已报到", "报告中")), _
                                                                3, IIf(strTag <> "", "处理中", _
                                                                        IIf(ufgStudyList.Text(i, "报告人") = "", "已检查", "报告中")), _
                                                                4, IIf(strTag <> "", "处理中", _
                                                                        IIf(ufgStudyList.Text(i, "复核人") <> "", "审核中", "已报告")), _
                                                                5, "已审核", "已完成"))
        End If
        
        Select Case strStudyProName
            Case "已登记"
...
'功能：取得当前过滤的SQL
    ...
    Dim blnQueryName As Boolean
    Dim rsTemp As Recordset
    Dim rsHTemp As Recordset
    ...
    With SQLCondition
        ...
        strWithOrderCols = "A.医嘱ID,A.发送号,Decode(nvl(A.执行过程,0),0,'',1,'',to_char(A.首次时间,'yyyy-mm-dd hh24:mi:ss')) 首次时间,A.发送时间,A.执行状态,A.执行过程,nvl(A.执行间,' ') as 执行间, A.结果阳性 , " & _
                            ...
                            " C.健康号,C.就诊卡号, C.身份证号, C.结算模式, decode(B.病人来源,2,D.病人类型,C.病人类型) as 病人类型, C.门诊号, C.当前床号, C.当前病区ID, D.路径状态, D.住院号, E.名称,J.医嘱ID as 申请单医嘱 "
        ...
        If mblnMoved Then
            ...
            If blnQueryName Then
            '通过dblink方式查询远程数据库的数据时，会屏蔽历史表的索引。目前对按姓名查询历史表做特殊处理：在SQL中加入提示字driving_site
            '由于处理后Union all 会导致效率降低，故将在线库和历史库分开处理
                strSQL = strSQL & " Order by 检查过程,报到时间,申请时间"
                Set rsTemp = OpenQuerySql(strSQL)
                
                strSQL = "Select /*+driving_site(A) driving_site(B) driving_site(H)*/" & vbNewLine & _
                        "Distinct a.医嘱id, b.相关id, a.发送号, Decode(Nvl(a.执行过程, 0), 0, '', 1, '', To_Char(a.首次时间, 'yyyy-mm-dd hh24:mi:ss')) 报到时间," & vbNewLine & _
                        "         a.发送时间 申请时间, a.执行状态, Nvl(a.执行过程, 0) 检查过程, Nvl(a.执行间, ' ') As 执行间, a.结果阳性 阳性, h.危急状态 危急, b.病人id, b.主页id, b.挂号单," & vbNewLine & _
                        "         b.病人科室id, b.病人来源 来源, b.医嘱内容, b.标本部位, Nvl(b.紧急标志, 0) 紧急标志, Nvl(b.婴儿, 0) 婴儿, b.开嘱医生, a.No, c.当前床号, c.当前病区id," & vbNewLine & _
                        "         Decode(b.病人来源, 2, c.住院号, c.门诊号) 标识号, Nvl(H.姓名, B.姓名) 姓名, F.影像类别, H.检查号, Nvl(H.性别, B.性别) 性别, Nvl(H.年龄, B.年龄) 年龄," & vbNewLine & _
                        "         H.身高, H.体重, H.影像质量, H.报告质量, H.符合情况, Decode(b.病人来源, 3, b.开嘱医生, a.发送人) 登记人, h.报到人, h.报告发放, h.发放胶片, h.关联id, a.记录性质," & vbNewLine & _
                        "         h.待处理人, h.完成人, h.是否电子胶片, h.是否打印, h.报告操作, h.绿色通道, h.报告打印, h.报告人, h.复核人, h.是否技师确认, h.检查技师, h.检查技师二, h.接收日期 采图时间," & vbNewLine & _
                        "         h.随访描述, h.诊断分类, h.检查uid, h.图像位置, a.执行部门id As 执行科室id, 0 As 转出, e.名称 As 病人科室, a.采样时间, c.健康号, c.就诊卡号, a.No As 单据号," & vbNewLine & _
                        "         c.身份证号, c.结算模式, Decode(b.病人来源, 2, d.病人类型, c.病人类型) 病人类型, d.路径状态, a.计费状态," & vbNewLine & _
                        "         Decode(a.记录性质, 2, 1, Decode(a.计费状态, 3, 1, 0)) As 收费, J.医嘱ID 申请单医嘱" & vbNewLine & _
                        IIf(strPatholCol = "", "", _
                                    "    ,p.ID 会诊ID,p.当前状态 会诊状态,p.会诊医师,nvl((select count(1) from 病理检查信息 V , 病理申请信息 W where V.病理医嘱ID=w.病理医嘱id and v.医嘱id=H.医嘱ID and w.补费状态=1),0) as 补费, '' as 病理执行状态,decode(o.病理号,null,'未核收','已核收') as 核收情况," & _
                                    "    decode(o.检查类型,0,'常规',1,'冰冻',2,'细胞',3,'会诊',4,'尸检',5,'快速石蜡',null) as  检查类别, " & _
                                    Replace(strPatholCol, "M.号别名称", "q.名称 as 号别名称") & vbNewLine) & _
                        "From H病人医嘱发送 a, H病人医嘱记录 b, H影像检查记录 h, 病人信息 c, 病案主页 d, 部门表 e, 影像检查项目 f, 影像申请单图像 j " & IIf(strPatholTab = "", "", " ," & strPatholTab) & _
                        " Where a.医嘱ID = b.ID AND A.医嘱ID=J.医嘱ID(+) AND A.医嘱ID=h.医嘱ID(+) And b.病人ID = c.病人ID " & vbNewLine & _
                                      "     And B.病人科室ID=E.ID " & vbNewLine & _
                                      "     And B.病人ID = D.病人ID(+) And B.主页ID+0 = D.主页ID(+) And B.诊疗项目ID+0 =F.诊疗项目ID " & vbNewLine & _
                                      "     " & strFilterOrder & IIf(strFilterDate = "", "", "     And " & strFilterDate) & "  and B.医嘱状态 <> 4" & vbNewLine & _
                                    IIf(strPatholFilter = "", "", "     And " & strPatholFilter) & vbNewLine & _
                        "Order by 检查过程,报到时间,申请时间"
                Set rsHTemp = OpenQuerySql(strSQL)
                
                If rsTemp.RecordCount > 0 Then
                    Set GetFilterData = ConnectRecordset(rsHTemp, rsTemp)
                Else
                    Set GetFilterData = rsHTemp
                End If
                Exit Function
            End If
        ...
        Set GetFilterData = OpenQuerySql(strSQL)
    End With
End Function

Private Function OpenQuerySql(strSQL As String) As Recordset
    With SQLCondition
        ...
        '24: 执行部门Id    25: 当前所属科室Ids    26: 报告内容    27: 性别    28: 开始年龄    29: 结束年龄    30: 结果阳性    31: 病人ID     32待处理人
        Set OpenQuerySql = GetDataToLocal(strSQL, "提取病人列表", .门诊号, .住院号, .就诊卡, .姓名, .身份证, _
...
Private Function ConnectRecordset(rsTempA As Recordset, rsTempB As Recordset) As Recordset
    Dim i As Long
    
    If rsTempB.RecordCount <= 0 Then
        Set ConnectRecordset = rsTempA
        Exit Function
    End If
    
    Do While Not rsTempB.EOF
        rsTempA.AddNew
        For i = 0 To rsTempA.Fields.Count - 1
            rsTempA.Fields(i).value = rsTempB.Fields(i).value
        Next
        rsTempA.Update
        rsTempB.MoveNext
    Loop
...
Private Sub SwitchUser()
    ...
    If frmSwitchUser.blnOk Then
        ...
        If frmSwitchUser.blnCnOracleIsNew = True Then
            ...
            '查找用户权限
            strPrivs = GetPrivFunc(100, mlngModule)
            If strPrivs = "" Then
                MsgBoxD Me, "你不具备使用“影像工作站”" & mlngModule & "模块的权限！"
...
On Error GoTo errHandle
    ...
    With ufgStudyList
        ...
        .CurText("胶片发放") = IIf(Trim(.CurText("胶片发放")) = "", "√", "")
...
On Error GoTo errHandle
    ...
    With ufgStudyList
        ...
        If mrtReportType = 报告文档编辑器 Then
            If mobjWork_Report.GetReportReleaseState(mcurAdviceInf.lngAdviceID) = 3 And .CurText("胶片发放") = "√" Then
        ...
        Select Case ufgStudyList.DataGrid.TextMatrix(0, i)
            ...
            Case "婴儿"
                ...
                If Val(ufgStudyList.Text(lngRow, "婴儿")) <> 0 Then
                    ...
                    Set rsBaby = zlDatabase.OpenSQLRecord(gstrSQL, "提取婴儿信息", Val(ufgStudyList.Text(lngRow, "病人ID")), Val(ufgStudyList.Text(lngRow, "主页ID")), Val(ufgStudyList.Text(lngRow, "婴儿")))
            ...
            Case "病理执行状态"
                ufgStudyList.Text(lngRow, "病理执行状态") = GetPatholExecuteStateByList(lngRow)
...
'        End If
'
'        rsClone.Filter = "相关ID = " & Nvl(rsData!医嘱ID) & " or 医嘱ID = " & Nvl(rsData!医嘱ID)
'        Do While rsClone.EOF = False
'            If Nvl(rsClone!来源, 0) = 2 Then
'                gstrSQL = "Select 执行状态 From 住院费用记录 where 记录状态 = 2 And NO = [1]"
'            Else
'                gstrSQL = "Select 执行状态 From 门诊费用记录 where 记录状态 = 2 And NO = [1]"
'            End If
'
'            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "是否退费", Nvl(rsClone!NO))
'
'            If rsTemp.RecordCount > 0 Then
'                mlngTempCharged = 7  '销账
'                Exit Do
'            End If
'
'            rsClone.MoveNext
...
'
'                    rsTmpClone.Filter = ""
'                    If mlngTempCharged <> 6 Then '没有退费的
'                        Do While rsTmpClone.EOF = False
'                            If Nvl(rsTmpClone!计费状态, -1) = 2 Then
'                                mlngTempCharged = 0      '未收
'                                Exit Do
'                            End If
'
'                            rsTmpClone.MoveNext
...
'            End If
'        Else
'            If Nvl(rsData!计费状态, -1) = 1 Or Nvl(rsData!计费状态, -1) = 2 Or Nvl(rsData!计费状态, -1) = 3 Then
'                rsClone.Filter = "医嘱ID = " & Nvl(rsData!医嘱ID) & " or " & "相关ID = " & Nvl(rsData!医嘱ID)
'
'                Set rsTmpClone = zlDatabase.CopyNewRec(rsClone)
'                rsTmpClone.Filter = "计费状态 <> 3"
'
...
'    End If
'
'    If Nvl(rsData!相关ID) = "" And (mlngTempCharged = 7 Or (mblncmd已缴 = True And mlngTempCharged = 1) Or (mblncmd未缴 = True And (mlngTempCharged = 0 Or mlngTempCharged = 4)) _
'        Or (mblncmd无费 = True And mlngTempCharged = 2) Or (mblncmd补缴 = True And mlngTempCharged = 4) Or (mblncmd记账 And mlngTempCharged = 5) _
'        Or (mblncmd退费 = True And mlngTempCharged = 6) _
'        Or (mblncmd已缴 = False And mblncmd未缴 = False And mblncmd补缴 = False And mblncmd无费 = False And mblncmd记账 = False And mblncmd退费 = False)) Then
'        blnFilterOut = False
'
...
Private Sub ufgStudyList_OnFilterData(rsData As ADODB.Recordset, rsClone As ADODB.Recordset)
    ...
    If mstrcmd部位 = "" Or rsData.RecordCount <= 0 Or rsClone.RecordCount <= 0 Then Exit Sub
...
On Error GoTo errHandle
    ...
    '0--欠费；1--已收费；2--无收费，3-记账记录，4-补缴，5-退费，6-销账，7-有调动
    ...
    If Nvl(rsData!相关ID) = "" And ((mblncmd已缴 = True And mlngTempCharged = 1) Or (mblncmd未缴 = True And (mlngTempCharged = 0 Or mlngTempCharged = 4)) _
        Or (mblncmd无费 = True And mlngTempCharged = 2) Or (mblncmd补缴 = True And mlngTempCharged = 4) Or (mblncmd记账 And mlngTempCharged = 3) _
        Or (mblncmd已缴 = False And mblncmd未缴 = False And mblncmd补缴 = False And mblncmd无费 = False And mblncmd记账 = False)) Then
        blnFilterOut = False
        rsData!收费 = mlngTempCharged
    Else
        blnFilterOut = True
    End If
errHandle:
...
On Error GoTo errHandle
    ...
    With ufgStudyList
        ...
        If Not mblnIsHistory Then  '---------------------------非历次检查直接用列表中记录填充
            ...
            '0--欠费；1--已收费；2--无收费，3-记账记录，4-补缴，5-退费，6-销账，7-有调动
            intChargeState = Val(.DataGrid.Cell(flexcpData, .SelectionRow, .GetColIndex("收费")))
...
On Error GoTo errHandle
    ...
    '刷新视频采集模块的医嘱信息
    If Not mobjWork_ImageCap Is Nothing Then
        Call mobjWork_ImageCap.zlUpdateStudyInf(mcurAdviceInf.lngAdviceID, mcurAdviceInf.lngSendNO, mcurAdviceInf.intStep, mcurAdviceInf.intMoved = 1)
    End If
...
On Error GoTo errHandle
    ...
    Select Case TabWindow.Selected.tag
        ...
        Case "报告填写"
            ...
            Call mobjWork_Report.zlRefreshFace(False, False, True, mobjWork_Report.IsDockActive)
        ...
        Case "影像采集"
            If Not mobjWork_ImageCap Is Nothing Then
                Call mobjWork_ImageCap.zlUpdateStudyInf(mcurAdviceInf.lngAdviceID, mcurAdviceInf.lngSendNO, mcurAdviceInf.intStep, mcurAdviceInf.intMoved)
...
Private Sub Menu_Manage_病案查阅()
    ...
    If Not CheckPopedom(";" & GetPrivFunc(100, 1259) & ";", "基本") Then
    ...
    If mlngModule <> G_LNG_PATHOLSYS_NUM Then
        strSQL = "Select * From (" & vbNewLine & _
             ...
             "Select  Distinct" & vbNewLine & _
                    "       A.医嘱ID,B.相关ID,A.发送号,A.首次时间 报到时间,A.发送时间 申请时间,A.执行状态,nvl(A.执行过程,0) 检查过程,A.执行间,A.结果阳性 阳性,h.危急状态 危急," & vbNewLine & _
                    "       B.病人ID,B.主页ID,B.挂号单,B.病人科室ID,B.病人来源 来源,B.医嘱内容,B.标本部位," & vbNewLine & _
                    "       Nvl(B.紧急标志, 0) 紧急标志, Nvl(B.婴儿, 0) 婴儿,B.开嘱医生,A.NO,C.当前床号,C.当前病区ID,Decode(B.病人来源,2,D.住院号,C.门诊号) 标识号,b.开嘱时间,c.门诊号,c.住院号," & vbNewLine & _
...
On Error GoTo errH
    ...
    strSQL = "Select a.名称 as 程序名, b.名称 as 功能名 , b.自动执行时机,b.vbs脚本  from 影像插件挂接 a, 影像插件功能 b " & _
             "Where a.id = b.插件id And (a.所属模块=0 or a.所属模块=[1]) and b.是否启用=1 and a.是否启用=1 Order By a.id,b.功能序号"
