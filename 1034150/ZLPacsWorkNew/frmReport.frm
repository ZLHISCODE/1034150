...
Private mblOneReportFormat As Boolean           '是否只使用一种打印格式
...
'同步检查医嘱信息
    ...
    If Not mobjWork_ImageCap Is Nothing Then Call mobjWork_ImageCap.zlUpdateStudyInf(lngAdviceID, lngSendNO, lngStudyState, blnMoved)
End Sub
...
On Error GoTo errHandle
    ...
    If mlngAdviceID <> 0 Then
        ...
        Else
            ...
            If str检查报告ID <> "" Then
                 MsgBoxD Me, "此检查请使用PACS智能报告编辑器打开以执行相关操作。", vbExclamation, gstrSysName
            Else
...
'    Dim arrSQL() As Variant
    ...
    End If
    strSQL = "ZL_影像报告内容_update(" & mlngAdviceID & "," & mReportID & ",'" & Replace(strReport, "'", "’") & " ','" & strElements & "'," & m目标版本 & "," & intLevel & ")"
    ReDim Preserve arrSQL(UBound(arrSQL) + 1)
    arrSQL(UBound(arrSQL)) = strSQL
...
Private Sub Form_Activate()
    ...
    If Not mobjWork_ImageCap Is Nothing Then
        If mblnSingleWindow Then
            Call mobjWork_ImageCap.zlUpdateStudyInf(mlngAdviceID, mlngSendNo, mlngStudyState, mblnMoved)
...
'    mintCriticalValues = Val(GetDeptPara(mlngDeptId, "危急情况判断", 0))           '危急情况判断
    ...
    mintReportLevel = Val(GetDeptPara(mlngDeptId, "报告质量判定", 0))              '报告质量判定
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If mblnShowVideoCapture Then
        If Not mobjWork_ImageCap Is Nothing Then
'                Call mobjWork_ImageCap.FreeResource
...
Private Sub subShowHistoryList()
    ...
    strSQL = "Select c.Id As 医嘱id, a.影像类别, c.开嘱时间, c.医嘱内容, b.病历id,a.接收日期 as 检查时间  " & _
            ...
            " (c.病人id = e.病人id Or a.关联id = d.关联id) And c.相关id Is Null And Nvl(RawToHex(B.检查报告ID),' ') =' '"
...
'       blnSilent ---强制静默打印，批量打印时需要
    ...
    For i = 0 To UBound(arr报表格式)
        ...
        If mlngModule = G_LNG_VIDEOSTATION_MODULE And blnIsImageReport Then
...
Private Function CheckSignRollbackType(ByVal lngID As Long, lngReportID As Long) As Integer
...
'返回 0无数据  1诊断签名  2/3审核签名
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngID, lngReportID)
...
'载入巨检描述内容
    ...
    strFormatContext = strFormatContextOld
    strSQL = "select  a.巨检描述,b.序号,b.标本名称, b.形状,b.取材位置,b.蜡块数,b.主取医师 ,b.取材时间, b.标本名称, c.制片数 " & _
    ...
    If rsTemp.RecordCount <> 0 Then str巨检描述 = Nvl(rsTemp!巨检描述)
    While Not rsTemp.EOF
    
        strTitle = "蜡块" & Nvl(rsTemp!序号) & "："
        strText = "取材位置：" & Nvl(rsTemp!取材位置) & " ," & "标本名称：" & Nvl(rsTemp!标本名称) & " ，" & "形状：" & Nvl(rsTemp!形状) & " ," & _
                       "材块数：" & Nvl(rsTemp!蜡块数) & " ," & "制片数：" & Nvl(rsTemp!制片数) & " ," & "主取医师：" & Nvl(rsTemp!主取医师) & " ," & _
                       "取材时间：" & getShortDate(Nvl(rsTemp!取材时间)) & vbCrLf  '& vbCrLf
        strFormatContext = strFormatContext & "\b\cf2\fs24 " & strTitle & " \par\b0\cf0\fs" & strSize & " " & Replace(strText, vbCrLf, " \par\cf0\fs" & strSize & " ") & "\par"
        
        rsTemp.MoveNext
    Wend
    
    strFormatContext = strFormatContext & "\b\cf2\fs24 " & "巨检描述:" & " \par\b0\cf0\fs" & strSize & " " & Replace(str巨检描述, vbCrLf, " \par\cf0\fs" & strSize & " ") & "\par"
