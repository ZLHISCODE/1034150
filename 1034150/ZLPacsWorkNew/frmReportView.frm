...
Private mintMenuFontSize As Integer  '字号菜单选择的字号

Public pModified As Boolean          '记录当前内容是否有改变
Private mingFlag As Integer          '为1时说明已经执行过检查所见的GetFocue方法

Private mblnAdviceFocusing As Boolean
Private mblnResultFocusing As Boolean
Private mblnChkViewFocusing As Boolean
...
'           strModifyEdit----当前报告是否在修订状态被其他人修订保存后没有签名？记录保存人的姓名，空表示不是这种情况
    ...
    If mReportID = 0 Then       '报告为空，需要创建报告，从报告模板中提取信息
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mFileID)
            ...
            strTmp = Nvl(rsTemp!对象属性)
                               
            Select Case Nvl(rsTemp!标题)
                Case "检查所见"

                    rtxtCheckView.tag = strTmp
                    
                    zlWriteReport Nvl(rsTemp!正文), 0
                Case "诊断意见"
     
                    rtxtResult.tag = strTmp
                    
                    zlWriteReport Nvl(rsTemp!正文), 1
                Case "建议"

                    rTxtAdvice.tag = strTmp
...
Public Sub zlWriteReport(strText As String, intType As Integer)
    ...
    If mintMenuFontSize <> 0 Then
        rText.SelFontSize = mintMenuFontSize
    Else
...
Private Sub InitLoaclParas()
    ...
    mintMenuFontSize = Val(zlDatabase.GetPara("报告显示字号", glngSys, glngModul))
...
On Error GoTo err
    ...
    If mblnAdviceFocusing Then
        mblnAdviceFocusing = False
        Exit Sub
    End If
    ...
    If GetFocus <> rTxtAdvice.hWnd Then
        mblnAdviceFocusing = True
        rTxtAdvice.SetFocus
    End If
err:
End Sub
...
Private Sub rtxtCheckView_GotFocus()
    ...
    If mblnChkViewFocusing Then
        mblnChkViewFocusing = False
        Exit Sub
    End If
    ...
    If GetFocus <> rtxtCheckView.hWnd Then
        mblnChkViewFocusing = True
        rtxtCheckView.SetFocus
    End If
End Sub
...
On Error GoTo err
    ...
    If mblnResultFocusing Then
        mblnResultFocusing = False
        Exit Sub
    End If
    ...
    If GetFocus <> rtxtResult.hWnd Then
        mblnResultFocusing = True
        rtxtResult.SetFocus
    End If
err:
End Sub
...
Public Sub SetFontSize(ByVal bytFontSize As Byte)
    ...
    If mintMenuFontSize <> 0 Then Exit Sub '只有mintMenuFontSize=0才需要下面的流程
    
    mblnCheckModify = False
        
    rtxtCheckView.Font.Size = gbytFontSize
    rtxtResult.Font.Size = gbytFontSize
    rTxtAdvice.Font.Size = gbytFontSize
    
    rtxtCheckView.SelStart = 0
    rtxtCheckView.SelLength = Len(rtxtCheckView.Text)
    rtxtCheckView.SelFontSize = gbytFontSize
    
    rtxtResult.SelStart = 0
    rtxtResult.SelLength = Len(rtxtResult.Text)
    rtxtResult.SelFontSize = gbytFontSize
    
    rTxtAdvice.SelStart = 0
    rTxtAdvice.SelLength = Len(rTxtAdvice.Text)
    rTxtAdvice.SelFontSize = gbytFontSize
    
    mblnCheckModify = True
    
End Sub
 
Public Property Get MenuFontSize() As Integer
    MenuFontSize = mintMenuFontSize
End Property

Public Property Let MenuFontSize(ByVal intValue As Integer)
    mintMenuFontSize = intValue
    
    If intValue <> 0 Then
        
        mblnCheckModify = False
        rtxtCheckView.Font.Size = mintMenuFontSize
        rtxtResult.Font.Size = mintMenuFontSize
        rTxtAdvice.Font.Size = mintMenuFontSize
    
        rtxtCheckView.SelFontSize = mintMenuFontSize
        rtxtResult.SelFontSize = mintMenuFontSize
        rTxtAdvice.SelFontSize = mintMenuFontSize
        mblnCheckModify = True
    Else
