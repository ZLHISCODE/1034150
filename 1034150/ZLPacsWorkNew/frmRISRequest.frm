...
On Error GoTo errHandle
    ...
    mstrCur科室 = NeedName(cbo执行科室.Text)
...
Private Sub CmdOK_Click()
    ...
    '如果是婴儿医嘱,而且是修改信息或者报到，此时把信息(姓名，性别，年龄，出生日期)恢复成母亲的信息
    ...
    If mlngBaby <> 0 And mintEditMode <> 0 Then
        ...
        Else
            ReCalcOld Format(Nvl(rsMother!出生日期, zlDatabase.Currentdate), "yyyy-mm-dd hh:mm:ss"), cboAge
        End If
        
        If Trim(Nvl(rsMother!出生日期)) = "" Then
            Call ReCalcBirthDay(txt年龄.Text, cboAge.Text)
        Else
            dtp出生日期.value = Format(Nvl(rsMother!出生日期), "yyyy-mm-dd hh:mm:ss")
        End If
    End If
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查_BEGIN('" & mstrTechnicRoom & "'," & txt检查号.Text & "," & mlngAdviceID & "," & mlngSendNo & ",'" & mstrItemType & "','" & _
            ...
            txt年龄.Text & IIf(cboAge.Visible, cboAge.Text, "") & "'," & To_Date(dtp出生日期.value) & ",'" & ToVarchar(Txt身高, 16) & "','" & _
            ToVarchar(Txt体重, 16) & "',Null,Null,'" & NeedNo(cboDevice.Text) & "','" & NeedName(cbo技师一.Text) & "','" & NeedName(cbo技师二.Text) & "','" & txt附加主述.Text & "'," & To_Date(CDate(dtp(1).value)) & "," & mlngCurDeptId & ",Null)"
        ...
        If Trim(cbo造影剂.Text) <> "" Then
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            arrSQL(UBound(arrSQL)) = "ZL_服用造影剂_INSERT(" & mlngAdviceID & ",'" & ToVarchar(Trim(cbo造影剂.Text), 30) & "','" & ToVarchar(Txt造影用量.Text, 30) & "','" & ToVarchar(Txt造影浓度.Text, 30) & "')"
        Else
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        ...
        If GetDeptPara(mlngCurDeptId, "报道时自动发送WorkList") = "1" And mlngModul = 1290 Then
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            arrSQL(UBound(arrSQL)) = "ZL_影像检查记录_发送安排(" & mlngAdviceID & "," & mlngSendNo & ",1," & "'" & NeedName(cbo技师一.Text) & "','" & NeedName(cbo技师二.Text) & "','" & NeedNo(cboRoom.Text) & "')"
        End If
    End If  '报到的if
    ...
    If mintEditMode = 3 Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查记录_UPDATE(" & mlngAdviceID & ", " & mlngSendNo & "," & txt检查号.Text & ",'" & _
            Trim(Me.PatiIdentify.Text) & "','" & Trim(Txt英文名.Text) & "','" & NeedName(cbo性别.Text) & "','" & _
            txt年龄.Text & IIf(cboAge.Visible, cboAge.Text, "") & "'," & To_Date(dtp出生日期.value) & ",'" & ToVarchar(Txt身高, 16) & "','" & _
            ToVarchar(Txt体重, 16) & "','" & NeedNo(cboDevice.Text) & "','" & NeedName(cbo技师一.Text) & "','" & NeedName(cbo技师二.Text) & "','" & txt附加主述.Text & "','" & NeedNo(cboRoom.Text) & "'," & To_Date(dtp(1).value) & ",Null)"
    ...
    If mintEditMode = 3 And gblnUseXinWangView And mlngModul = G_LNG_PACSSTATION_MODULE Then
        Call XWStudyUpdate(mlngAdviceID, Me.PatiIdentify.Text, NeedName(cbo性别.Text), txt年龄.Text)
    End If
...
'返回：mstrExtData "部位名1;方法名1,方法名2|部位名2;方法名1,方法名2|...<vbTab>0-常规/1-床旁/2-术中"
    ...
    If mblnRegToCheck And Trim(txt检查号.Text) = "" And Not mblnUseAdviceNo And Not mblnUsePatientNo Then
...
BuildNo: '按生成规则重取
        ...
        Next检查号 = zlDatabase.GetNextNo(123, mlngCurDeptId, mstrItemType, lngCurNo)
...
'        mstrRegNo = strNO
    ...
    gstrSQL = gstrSQL & " Union " & _
                "Select null 病人ID,'新病人' 姓名,'未知' 性别,'' 年龄,null 出生日期,3 As 来源ID,0 As 主页ID," & _
                        "0 As 病人科室ID,'' As 医生,null as 门诊号,null as 住院号,'' as 就诊卡号,'' 卡验证码,'' as 当前床号," & _
...
'返回： True--复制成功；False--复制信息不完整
    ...
    gstrSQL = "SELECT nvl(B.姓名,E.姓名) 姓名,nvl(B.性别,E.性别) 性别,nvl(B.年龄,E.年龄) 年龄,B.出生日期,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",F.发送时间 开嘱时间,NVL(C.简码,0) 科室简码,NVL(C.名称,'未知') AS 开嘱科室,A.开嘱医生,A.紧急标志,F.首次时间,F.执行间,E.检查设备,A.医嘱内容,E.检查号,E.检查技师,E.检查技师二 " & _
...
Dim intTMP As Integer
    ...
    gstrSQL = "SELECT nvl(B.姓名,E.姓名) 姓名,nvl(B.性别,E.性别) 性别,nvl(B.年龄,E.年龄) 年龄,B.出生日期,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",F.发送时间 开嘱时间,NVL(C.简码,0) 科室简码,NVL(C.名称,'未知') AS 开嘱科室,A.开嘱医生,A.紧急标志,F.首次时间,F.执行间,E.检查设备,A.医嘱内容,E.检查号,E.检查技师,F.执行部门ID" & _
...
Normal:
        ...
        If bln报到 Or mintEditMode = 1 Then
            If bln报到 And IsNull(rsTemp!出生日期) Then
                ...
                If intTMP = vbYes Then
                    ...
                    If txt年龄.Text = "" Then
                        MsgBoxD Me, "无年龄数据，不能进行报到，请联系对应人员处理。", vbInformation, gstrSysName
                        RefreshPatiInfor = False
                        Exit Function
                    End If
                Else
        ...
        Else
            ...
            If bln报到 Or mintEditMode = 1 Then
                If bln报到 And IsNull(rsBaby!出生时间) Then
                    ...
                    intTMP = MsgBoxD(Me, "出生时间为空，是否根据年龄推算？", vbYesNo, gstrSysName)
                    ...
                    If intTMP = vbYes Then
                        ...
                        End If
                    Else
                          MsgBoxD Me, "出生时间为空，不能进行报到，请联系对应人员处理。", vbInformation, gstrSysName
                          RefreshPatiInfor = False
                          Exit Function
                    End If
                Else
                    txt年龄.Text = ReCalcOld(Format(Nvl(rsBaby!出生时间, curDate), "yyyy-mm-dd hh:mm:ss"), cboAge)
                End If
            Else
                Call ReCalcOld(Format(Nvl(rsBaby!出生时间, curDate), "yyyy-mm-dd hh:mm:ss"), cboAge)
            End If
    ...
    End If
    If Val(Nvl(rsTemp!检查号)) <> 0 Then
        txt检查号.Text = CStr(Nvl(rsTemp!检查号))
        txt检查号.tag = txt检查号.Text
    End If
    ...
    If intChargeState = 5 And bln报到 Then
        cmdOK.Enabled = False
        Me.Caption = Me.Caption & "(当前病人已退费，不能报到)"
    End If
    
    If intChargeState = 6 And bln报到 Then
        cmdOK.Enabled = False
...
Private Sub txt检查号_Change()
    If Trim(txt检查号.Text) = "" Then Exit Sub
    If Not IsNumeric(txt检查号.Text) Then
        MsgBoxD Me, "检查号只能是数字，请重新输入。", vbOKOnly, "提示信息"
        txt检查号.Text = ""
    End If
End Sub
...
'                        End If
                    ...
                    txt年龄.Text = ReCalcOld(dtp出生日期.value, cboAge) & cboAge.Text '加上单位，不然分别配置txt年龄和cboage时，会将单位改成年，bugNo 89538
                Else
                    txt年龄.Text = ""
                End If
...
CheckNoValidate = True
    ...
    '判断检查号是否超过数据库允许的值(Number("))
    If txt检查号.Text > "999999999999999552" Then
        MsgBoxD Me, "当前检查号超过允许的最大号码，请重新输入！" & vbCrLf & "允许的最大号码为：999999999999999552！"
...
On Error GoTo errH
    ...
    '报到、登记（登记后直接报到）：加载注册表值
        For i = 0 To cbo技师一.ListCount - 1
            If NeedName(cbo技师一.list(i)) = mstrExamineDoctorFst Then
                cbo技师一.ListIndex = i
                Exit For
            Else
                cbo技师一.ListIndex = -1
