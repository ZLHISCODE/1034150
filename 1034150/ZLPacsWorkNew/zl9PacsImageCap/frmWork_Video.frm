...
'初始化参数设置
    ...
    mstrAfterDir = IIf(Len(App.Path) > 3, App.Path & "\TmpAfterImage\", App.Path & "TmpAfterImage\")
    ...
    ucPreview.IsShowImgDesc = gobjCapturePar.IsShowImgDesc
...
step2:
    ...
    gobjOwner.Caption = mAfterCaptureInf.strAfterParentTitle & "      当前后台采集标识：" & intTmp & "  当前标识图像数：0" & "        "
...
BUGEX "VideoForm_UnLoad 11"
    ...
    SetParent picPreview.hwnd, Me.hwnd
    For i = 1 To dkpReportImage.PanesCount
        dkpReportImage.Panes(i).Handle = 0
    Next
    
    dkpReportImage.CloseAll
...
'------------------------------------------------
    ...
    With ucPreview.ImgViewer
        ...
        If gobjCapturePar.IsShowImgDesc Then
            Set ImgTmp = .Images(1)
        Else
...
'功能创建工具条
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Cap_After_Tag, "标识检查"): cbrControl.ToolTipText = "标识检查"
