...
Private mintImageLocation As Integer            '记录图像所在的位置，null或0在中联数据库；1在新网数据库
...
'创建影像记录对应的菜单
    ...
    If Not HasMenu(objMenuBar, conMenu_Img_Group) Then
        ...
        With cbrMenuBar.CommandBar
            ...
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Manage_TechDoctorExecute, "技师执行", "", 807, True)
...
'创建工具栏
    ...
    Set cbrControl = CreateModuleMenu(objToolBar.Controls, xtpControlButton, conMenu_Img_Look3D, "3D观片", "3D观片", 8115, False, lngIndex + 2)
    Set cbrControl = CreateModuleMenu(objToolBar.Controls, xtpControlButton, conMenu_Manage_TechDoctorExecute, "技师执行", "", 807, False, lngIndex + 3)
    
    '如果启用三维重建功能，则创建对应菜单
    If mblnUse3D = True Then
        Set cbrControl = CreateModuleMenu(objToolBar.Controls, xtpControlButtonPopup, conMenu_Img_3D, "三维", "三维重建", 8115, False, lngIndex + 4)
            If mstr3DFunctions <> "" Then
                str3DFuncs = Split(mstr3DFunctions, ",")
                For i = 1 To UBound(str3DFuncs)
                    lng3DFunc = Val(str3DFuncs(i))
    ...
    If gblnUseXinWangView = True Then
        '判断是否使用新版观片
        Set cbrControl = CreateModuleMenu(objToolBar.Controls, xtpControlButton, conMenu_Manage_FilmPrint, "胶片打印", "", 3202, False, lngIndex + 5)
    End If
End Sub
...
'根据菜单ID执行对应功能
    Select Case lngMenuId
        ...
        Case conMenu_Manage_ImageFirst, conMenu_Manage_ImageSecond, conMenu_Manage_ImageThird, conMenu_Manage_ImageFourth
            Call Menu_Manage_影像质量(lngMenuId, mstrImageLevel, mobjStudyList)
    End Select
End Sub
...
'更新菜单
    Select Case control.ID
        ...
        Case conMenu_Img_Contrast   '观片对比
            control.Enabled = (mstrStudyUID <> "") Or (mlngStudyHistoryCount > 1)
            control.Visible = IIf(mintImageLocation = 1, False, True)
            
        Case conMenu_Img_Look3D     '3D观片
            control.Enabled = mstrStudyUID <> "" And mlngCurImageCount >= 50
            control.Visible = mintImageLocation = 1
        ...
        Case conMenu_Img_Delete '清除图像
            If Not CheckPopedom(mstrPrivs, "清除图像") Then
                control.Visible = False
            Else
                control.Visible = True
            End If
        ...
        Case conMenu_Img_Query ',获取图像
            If (Not CheckPopedom(mstrPrivs, "清除图像")) Or mintImageLocation = 1 Then
                control.Visible = False
            Else
                control.Visible = True
            End If
        ...
        Case conMenu_View_Show, conMenu_View_Expend_AllCollapse, conMenu_View_Expend_AllExpend  '图像显示，全选序列，全清序列
            control.Enabled = lvwSeq.ListItems.Count > 0 And Me.Visible
            control.Visible = (mintImageLocation <> 1) And Me.Visible
        ...
        Case conMenu_Manage_ImageFirst, conMenu_Manage_ImageSecond, conMenu_Manage_ImageThird, conMenu_Manage_ImageFourth, conMenu_Manage_ImageQuality
            ...
            ElseIf (mlngStudyState >= 3 And mlngStudyState <= 5) Or mlngStudyState = -1 Then
                control.Visible = True
                control.Enabled = mobjStudyList.CurText("检查UID") <> ""
            Else
                control.Visible = True
                control.Enabled = False
            End If
...
Public Sub zlUpdateOtherInf(objStudyList As Object, cboStudyHistory As Object)
    Set mobjStudyList = objStudyList
    
    '技师是否确认
    If objStudyList.CurText("是否技师确认") = "  " Then
        mblnExamineDoctorVerify = True
        mstrExamineDoctorName = objStudyList.Text(objStudyList.DataGrid.RowSel, "检查技师")
    Else
...
Private Sub Menu_Manage_影像质量(ByVal lngID As Long, ByVal strImageLevel As String, ByVal objStudyList As Object)
On Error GoTo errHandle
    ...
    strGrades = Split(strImageLevel, ",")
    
    If strResult <= UBound(strGrades) And strResult >= LBound(strGrades) Then
        objStudyList.CurText("影像质量") = strGrades(strResult)
        Call objStudyList.UpdateSourceData(mlngAdviceID, "影像质量", strResult + 1)
    End If
Exit Sub
errHandle:
    If ErrCenter = 1 Then Resume
...
On Error GoTo errHandle
    ...
    If mblnExamineDoctorVerify Then     '技师取消
        ...
        mblnExamineDoctorVerify = False
        mobjStudyList.CurText("是否技师确认") = " "
    Else
        If mobjStudyList.CurText("检查技师") <> "" Then
    ...
    Call SendMsgToMainWindow(Me, wetTechDo, mlngAdviceID)
...
Private Sub lvwSeq_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If mintImageLocation <> 1 And lvwSeq.ListItems.Count >= 1 And Button = 2 Then
...
On Error GoTo err
    ...
    lvwImage.Enabled = IIf(mintImageLocation = 1, False, True)
    lvwImage.HideColumnHeaders = IIf(mintImageLocation = 1, True, False)
