...
Private Sub cmdCancel_Click()
    ...
    strTemp = txt分类.Text & "|" & txt编码.Text & "|" & txt名称.Text & "|" & txt拼音.Text & "|" & txt五笔.Text & "|" & txt英文.Text & "|" & cbo单位.Text & "|" & _
                ...
                chk原研药.Value & "|" & chk专利药.Value & "|" & chk单独定价.Value & "|" & chk皮试.Value & "|" & chk品种医嘱.Value & "|" & chk抗生素.Value & "|" & cbo抗生素.Text & "|" & str别名 & "|" & txtAtccode.Text
...
Private Sub cmdOK_Click()
    ...
    If Me.cmdCancel.Tag = "增加" Then
        gstrSql = "zl_成药品种_INSERT('" & IIf(Me.Tag = 1, "5", "6") & "'," & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1", "Null") & "," & IIf(Trim(txtAtccode.Text) = "", "NULL,", "'" & txtAtccode.Text & "',") & Me.chkTumour.Value & "," & Me.chkSolvent.Value & "," & Me.chk原研药.Value & "," & Me.chk专利药.Value & "," & Me.chk单独定价.Value & ")"
    Else
        gstrSql = "zl_成药品种_UPDATE(" & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1", "Null") & "," & IIf(Trim(txtAtccode.Text) = "", "NULL,", "'" & txtAtccode.Text & "',") & Me.chkTumour.Value & "," & Me.chkSolvent.Value & "," & Me.chk原研药.Value & "," & Me.chk专利药.Value & "," & Me.chk单独定价.Value & ")"
    End If
    ...
    '            Me.txt编码.Text = IIf(Me.Tag = 1, "5", "6") & strTemp & Right(String(13, "0") & Val(rsTemp!编码) + 1, Len(rsTemp!编码) - 1 - Len(strTemp))
        ...
        Else
            ...
            gstrSql = "select I.分类ID,I.编码,I.名称,I.计算单位,T.药品剂型," & _
                    ...
                    "        nvl(T.急救药否,0) as 急救药否,nvl(T.是否原料,0) as 是否原料,nvl(t.抗生素,0) as 抗生素,nvl(T.是否新药,0) as 是否新药,nvl(T.是否皮试,0) as 是否皮试,nvl(T.是否原研药,0) as 是否原研药,nvl(T.是否专利药,0) as 是否专利药,nvl(T.是否单独定价,0) as 是否单独定价," & _
