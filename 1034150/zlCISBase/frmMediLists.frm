...
Begin VB.Form frmMediLists 
   ...
   Begin TabDlg.SSTab tabContent 
      ...
      Tab(1).Control(0)=   "fraComment(1)"
      Tab(1).Control(1)=   "hgdPrice"
...
Private Sub GetCostAdjust(ByVal lngDrug As Long)
    ...
    gstrSql = " Select B.NO, I.ID As 药品id, '[' || I.编码 || ']' || I.名称 || ' ' || I.规格 || ' ' || I.产地 As 药品, P.名称 As 库房,A.批号,A.效期,A.产地, " & _
            " I.计算单位 As 单位, S.药库单位, Nvl(S.药库包装, 1) 药库包装, A.原成本价,A.新成本价 As 成本价, A.执行日期, B.摘要 " & _
    ...
    With rsTemp
        ...
        Do While Not .EOF
            ...
            Me.hgdCost.TextMatrix(.AbsolutePosition, colCost说明) = IIf(IsNull(!摘要), "", !摘要)
...
'        .MergeCol(1) = True
    ...
    If mstrPrivs Like "*西成药*" Then
        gstrSql = "select 参数值 from zlParameters where 模块=1023 and 参数名='西成药收入项目'"
        Set rs收入项目 = zlDatabase.OpenSQLRecord(gstrSql, "收入项目", "西成药收入项目")
    ...
    If mstrPrivs Like "*中成药*" And bln收入项目 = False Then
        gstrSql = "select 参数值 from zlParameters where 模块=1023 and 参数名='中成药收入项目'"
        Set rs收入项目 = zlDatabase.OpenSQLRecord(gstrSql, "收入项目", "中成药收入项目")
        If rs收入项目.RecordCount > 0 Then
            If IsNull(rs收入项目!参数值) Then
                bln收入项目 = True
            End If
    ...
    End If
    If mstrPrivs Like "*中成药*" And bln收入项目 = False Then
        gstrSql = "select 参数值 from zlParameters where 模块=1023 and 参数名='中草药收入项目'"
        Set rs收入项目 = zlDatabase.OpenSQLRecord(gstrSql, "收入项目", "中草药收入项目")
        If rs收入项目.RecordCount > 0 Then
            If IsNull(rs收入项目!参数值) Then
                bln收入项目 = True
            End If
...
Private Sub hgdPrice_RowColChange()
    ...
    On Error GoTo errHandle
    gstrSql = "Select Nvl(招标药品,0) 招标药品 From 药品规格 Where 药品ID=(Select 收费细目ID From 收费价目 Where ID=[1])"
...
Private Sub lvwItems_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    gstrSql = "select Distinct I.ID,I.编码,I.规格,I.产地 as 厂牌,N.名称 as 商品名,I.费用类型 as 医保类型,S.药品来源," & _
...
Private Sub mnuEditSpecBatch_Click()
    '规格修改
    frmBatchUpdate.ShowMe 2, mstrPrivs
End Sub

Private Sub mnuEditVariBatch_Click()
    '1是品种修改
    frmBatchUpdate.ShowMe 1, mstrPrivs
End Sub
...
Private Sub mnuEditItemUsage_Click()
    ...
    Else
        ...
        Else
            If Right(Me.lvwItems.SelectedItem.Icon, 1) = "S" Then MsgBox "停用药品，不能设置用法用量！", vbExclamation, gstrSysName
...
'        Me.tlbThis.Buttons("Spec").Visible = True
        ...
        With Me.lvwSpecs.ColumnHeaders
            ...
            .Add , "备选码", "备选码", 1000: .Add , "站点", "站点", IIf(gstrNodeNo = "-", 0, 1000)
        End With
        With Me.lvwSpecs
...
Public Sub zlRefRecords(Optional lngItem As Long)
    ...
    If Val(Me.tvwClass.Tag) <= 2 Then
        ...
        End With
'    ElseIf Val(Me.tvwClass.Tag) = 2 Then
'        gstrSql = "select I.ID,I.编码,I.名称,D.产地,I.计算单位,D.规格,S.门诊单位||decode(S.门诊单位,D.计算单位,'','(='||decode(sign(S.门诊包装-1),-1,'0','')||to_char(S.门诊包装)||D.计算单位||')') as 药房单位," & _
'                "        S.药库单位||decode(S.药库单位,D.计算单位,'','(='||decode(sign(S.药库包装-1),-1,'0','')||to_char(S.药库包装)||D.计算单位||')') as 药库单位," & _
'                "        D.费用类型,decode(I.服务对象,1,'门诊',2,'住院',3,'门诊和住院','不直接应用于病人') as 服务对象," & _
'                "        decode(T.药品类型,1,'处方药',2,'甲类非处方药',3,'乙类非处方药',4,'非处方药',5,'其它药品',' ') as 药品类型,to_char(nvl(T.处方限量,0)) as 处方限量," & _
'                "        T.毒理分类,T.货源情况,T.价值分类,T.用药梯次," & _
'                "        nvl(I.撤档时间,to_date('3000-01-01','YYYY-MM-DD')) as 撤档时间,G.名称 合同单位,D.说明,D.备选码,D.站点,S.免煎,Nvl(I.适用性别,0) AS 适用性别 " & _
'                " from 诊疗项目目录 I,药品特性 T,药品规格 S,收费项目目录 D,(Select Id,名称 From 供应商 Where 末级 = 1 And substr(类型,1,1) = '1' And " & _
'                " 撤档时间 Is Null Or 撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD')) G" & _
'                " where I.ID=T.药名ID and T.药名ID=S.药名ID and G.id(+)=S.合同单位id and S.药品ID=D.ID and "
'        If mnuViewShowAll.Checked = False Then
'            gstrSql = gstrSql & " I.分类ID=[1] "
'        Else
'            gstrSql = gstrSql & " I.分类ID IN " & _
'                " (Select ID From 诊疗分类目录 Where 类型 = 3 " & _
'                " Start With ID=[1] Connect By Prior ID=上级ID)"
'        End If
'        If Me.mnuViewStoped.Checked = False Then
'            gstrSql = gstrSql & " and (I.撤档时间 is null or I.撤档时间=to_date('3000-01-01','YYYY-MM-DD'))"
...
'                End If
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_编码").Index - 1) = !编码
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_规格").Index - 1) = IIf(IsNull(!规格), "", !规格)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_产地").Index - 1) = IIf(IsNull(!产地), "", !产地)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_处方单位").Index - 1) = IIf(IsNull(!计算单位), "", !计算单位)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_药房单位").Index - 1) = !药房单位
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_药库单位").Index - 1) = !药库单位
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_医保类型").Index - 1) = IIf(IsNull(!费用类型), "", !费用类型)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_服务对象").Index - 1) = !服务对象
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_处方类型").Index - 1) = !药品类型
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_处方限量").Index - 1) = !处方限量
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_毒理").Index - 1) = !毒理分类
...
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_价值").Index - 1) = !价值分类
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_梯次").Index - 1) = !用药梯次
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_合同单位").Index - 1) = IIf(IsNull(!合同单位), "", !合同单位)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_说明").Index - 1) = IIf(IsNull(!说明), "", !说明)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_备选码").Index - 1) = IIf(IsNull(!备选码), "", !备选码)
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_免煎药").Index - 1) = IIf(!免煎 = 0, "否", "是")
'                ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_适用性别").Index - 1) = IIf(!适用性别 = 1, "男性", IIf(!适用性别 = 2, "女性", "无性别区分"))
...
'                "        decode(T.药品类型,1,'处方药',2,'甲类非处方药',3,'乙类非处方药',4,'非处方药',5,'其它药品',' ') as 药品类型,to_char(nvl(T.处方限量,0)) as 处方限量," & _
'                "        T.毒理分类,T.货源情况,T.价值分类,T.用药梯次," & _
'                "        nvl(I.撤档时间,to_date('3000-01-01','YYYY-MM-DD')) as 撤档时间,I.分类id,G.名称 合同单位,D.说明,D.备选码,D.站点,S.免煎,Nvl(I.适用性别,0) As 适用性别 " & _
'                " from 诊疗项目目录 I,药品特性 T,药品规格 S,收费项目目录 D,(Select Id,名称 From 供应商 Where 末级 = 1 And substr(类型,1,1) = '1' And " & _
'                " 撤档时间 Is Null Or 撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD')) G" & _
'                " where I.ID=T.药名ID and T.药名ID=S.药名ID and G.id(+)=S.合同单位id and S.药品ID=D.ID And I.ID IN ( " & mstrDrugId & ") " & strType
'            If Me.mnuViewStoped.Checked = False Then
'                gstrSql = gstrSql & " and (I.撤档时间 is null or I.撤档时间=to_date('3000-01-01','YYYY-MM-DD'))"
'            End If
'            gstrSql = gstrSql & " order by I.编码"
...
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_价值").Index - 1) = !价值分类
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_梯次").Index - 1) = !用药梯次
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_类型").Index - 1) = "3"
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_分类id").Index - 1) = !分类id
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_合同单位").Index - 1) = IIf(IsNull(!合同单位), "", !合同单位)
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_说明").Index - 1) = IIf(IsNull(!说明), "", !说明)
'                    ObjItem.SubItems(Me.lvwItems.ColumnHeaders("_备选码").Index - 1) = IIf(IsNull(!备选码), "", !备选码)
...
Private Sub mnuEdit_Click()
    '点击“药品”菜单时，将下面的菜单的可用状态重新判断一次
    '先默认都是不可用
'    If Val(Me.tvwClass.Tag) = 3 Then
'        mnuEditItemAdd.Enabled = False   '新增品种
'        mnuEditItemMod.Enabled = False   '修改品种
'        mnuEditItemDel.Enabled = False   '删除品种
'        mnuEditSpecAdd.Enabled = False   '新增规格
'        mnuEditSpecMod.Enabled = False   '修改规格
'        mnuEditSpecDel.Enabled = False   '删除规格
'        mnuEditStart.Enabled = False     '启用
'        mnuEditStop.Enabled = False      '停用
'    End If
'    If Val(mstr分类) = 3 Then Exit Sub
'    If Not tvwClass.SelectedItem.Key Like "_A*" Or Not tvwClass.SelectedItem.Key Like "_L*" Then Exit Sub
...
'
'    If lvwItems.ListItems.Count = 0 Then Exit Sub
'    If lvwItems.ListItems.Count > 0 And lvwItems.SelectedItem.Icon Like "*U" = True Then  '当有品种时，并且是未停用品种
'        mnuEditItemMod.Enabled = True   '修改品种
'        mnuEditItemDel.Enabled = True   '删除品种
'
'        mnuEditSpecAdd.Enabled = True   '新增规格
'        mnuEditStop.Enabled = True      '停用
'    ElseIf lvwItems.ListItems.Count > 0 And lvwItems.SelectedItem.Icon Like "*S" = True Then  '当有品种时，并且已停用
'        mnuEditStart.Enabled = True     '启用
'        mnuEditItemMod.Enabled = False   '修改品种
'    End If
'
'    If lvwSpecs.ListItems.Count = 0 Then Exit Sub
'    If lvwSpecs.ListItems.Count > 0 And lvwSpecs.SelectedItem.Icon Like "*U" = True Then '有规格，并且未停用
'        mnuEditSpecMod.Enabled = True   '修改规格
'        mnuEditSpecDel.Enabled = True   '删除规格
'        mnuEditStop.Enabled = True      '停用
'    ElseIf lvwSpecs.ListItems.Count > 0 And lvwSpecs.SelectedItem.Icon Like "*S" = True Then '有规格，并且已停用
'        mnuEditSpecDel.Enabled = True   '删除规格
'        mnuEditStart.Enabled = True     '启用
'    End If
End Sub
