...
Public Function ShowMe(ByVal lngModle As Long, ByVal objForm As Object, ByVal strText As String, ByVal strID As String, ByVal intType As Integer) As Boolean
...
Private Sub cmdOK_Click()
    ...
    If txtName.Text <> strTmp Then
        txtName.Text = strTmp
        Me.txtSymbol.Text = Mid(zlGetSymbol(Me.txtName.Text), 1, 10)
    End If
    ...
    If Me.chkCodeLen.Value = 1 Then
        If Me.txtParent.Tag = 0 Then
            gstrSql = "select nvl(max(length(编码)),0) LEN from 诊疗分类目录 start with 上级id is null connect by prior id=上级id"
        Else
            gstrSql = "select nvl(max(length(编码)),0) LEN from 诊疗分类目录 start with 上级id=[1] connect by prior id=上级id"
        End If
