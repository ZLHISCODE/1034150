...
Private Sub LvwMain_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    If str类别 <> "8" Then
        ...
        Else
            gstrSql = "Select a.Id, c.分类id, a.编码, A.名称, A.规格, A.产地, a.计算单位, " & _
                    ...
                    " and (A.撤档时间 is null or A.撤档时间 = to_date('3000-01-01', 'YYYY-MM-DD')) " & _
                    " And e.执行日期 <= SYSDATE AND (e.终止日期 > SYSDATE OR e.终止日期 IS NULL) " & _
                    " And " & strTemp & _
                    " Order By a.编码 "
        End If
    ...
    Else
        ...
        Else
            gstrSql = "select b.id , b.编码 , b.名称 ,b.规格,b.产地, b.计算单位,b.售价 " & _
                    ...
                    " (I.撤档时间 is null or I.撤档时间 = to_date('3000-01-01', 'YYYY-MM-DD')) And D.执行日期 <= SYSDATE AND (D.终止日期 > SYSDATE OR D.终止日期 IS NULL) " & _
                    " And " & strTemp & _
                    " Group By i.Id, 分类id, i.编码, i.名称, i.规格, i.产地, i.计算单位, i.是否变价 ) b " & _
                    " Where b.分类id = a.ID  Order By b.编码 "
        End If
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSql, "选择器", str分类, mstr服务对象)
    ...
    Do Until rsTmp.EOF
        ...
        ItmX.SubItems(1) = zlCommFun.Nvl(rsTmp("名称"))
        ItmX.SubItems(2) = zlCommFun.Nvl(rsTmp("规格"))
        ItmX.SubItems(3) = zlCommFun.Nvl(rsTmp("产地"))
        ItmX.SubItems(4) = zlCommFun.Nvl(rsTmp("计算单位"))
        ItmX.SubItems(5) = zlCommFun.Nvl(rsTmp("售价"))
        rsTmp.MoveNext
    Loop
    rsTmp.Close
