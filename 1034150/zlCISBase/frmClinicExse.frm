    ...
    With Me.lvwItems
        ...
        Else
            ...
            gstrSql = "select decode(I.是否变价,1,'变价',to_char(P.价格)) As 价格" & _
                    ...
                    "      From 收费价目 " & _
                    "      Where 收费细目id=[1] " & _
                    "           and 执行日期<=Sysdate And (终止日期 Is Null Or 终止日期>=Sysdate)) P"
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, Val(Me.msfExse.RowData(Me.msfExse.Row)))
...
Private Sub msfExse_KeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
    ...
    With Me.msfExse
        ...
        If .TxtVisible = False Then
            ...
            strTemp = UCase(Trim(.TextMatrix(.Row, ExseCol.项目名)))
        Else
    ...
    End If
    gstrSql = "select distinct I.ID,I.编码,I.名称,I.规格,I.产地,I.计算单位," & _
            " Decode(Nvl(I.是否变价,0),0,ltrim(rtrim(to_char(nvl(D.现价,0),'9999999990.0000'))),Decode(Instr('4567', 类别), 0, ltrim(rtrim(to_char(nvl(D.缺省价格,0),'9999999990.0000'))), '时价')) As 售价 " & _
            " from 收费项目目录 I,收费项目别名 N,收费价目 D " & _
            " where I.ID=D.收费细目ID(+) And I.ID=N.收费细目id and I.类别 not in ('1','J')" & _
            "       and (I.撤档时间 is null or I.撤档时间=to_date('3000-01-01','YYYY-MM-DD')) " & str服务对象 & _
...
110     strSql = "Select i.Id, '[' || i.编码 || ']' || i.名称 As 名称, i.规格, i.计算单位, Decode(i.是否变价, 1, '变价', To_Char(Sum(p.现价))) As 价格," & vbNewLine & _
            ...
            "Where r.收费项目id = i.Id And i.Id = p.收费细目id(+) And p.执行日期 <= Sysdate And (p.终止日期 Is Null Or p.终止日期 >= Sysdate) And" & vbNewLine & _
            "      (r.费用性质 = 0 Or r.费用性质 Is Null) And r.检查部位 Is Null And r.适用科室id = b.Id(+) And r.诊疗项目id = [1]" & vbNewLine & _
...
112     Set rsCharge = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lngItemID)
114     strDeptList = ""
...
192         strSql = "Select /*+ RULE */" & vbNewLine & _
                    ...
                    " Decode(i.是否变价, 1, '变价', To_Char(p.价格)) As 价格, Nvl(r.收费数量, 0) As 数量, Nvl(r.固有对照, 0) As 固定," & vbNewLine & _
                    ...
                    "            Where p.执行日期 <= Sysdate And (p.终止日期 Is Null Or p.终止日期 >= Sysdate)" & vbNewLine & _
...
194         Set rsCharge = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lngItemID)
...
256             Set rsCharge = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lngItemID)
...
132         .TextMatrix(0, 7) = "固定": .TextMatrix(0, 8) = "性质": .TextMatrix(0, 9) = "收费ID"
134         .ColKey(0) = "分组": .ColKey(1) = "部位": .ColKey(2) = "方法": .ColKey(3) = "项目名"
136         .ColKey(4) = "单位": .ColKey(5) = "价格"
138         .ColKey(6) = "数量": .ColKey(7) = "固定": .ColKey(8) = "性质": .ColKey(9) = "收费id"
...
174                 If lngDeptID = 0 Or lngDeptID = Val(Split(strPla(lngRow), "|")(11)) Then
176                     .Rows = .Rows + 1
                        'strPla格式：分组|部位|方法|项目名|单位|价格|数量|固定|性质|收费ID|病人来源|适用科室id
...
194                     .TextMatrix(.Rows - 1, .ColIndex("性质")) = Val(Split(strPla(lngRow), "|")(8))
196                     .TextMatrix(.Rows - 1, .ColIndex("收费ID")) = Val(Split(strPla(lngRow), "|")(9))
...
'                Next
            ...
            Else
                ...
                "      Where 收费细目id=[1] " & _
                "           and 执行日期<=Sysdate And (终止日期 Is Null Or 终止日期>=Sysdate)) P"
                Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, Val("" & varReturn(0)))
...
Private Function CheckArrData(ByRef ArryA() As String) As Boolean
    ...
    If bln存在从项 Then
        gstrSql = "Select Id From 收费价目 Where 收费细目id=[1] And 执行日期 <= SYSDATE AND (终止日期 > SYSDATE OR 终止日期 IS NULL) "
        Set rs = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, Val(Split(ArryA(int主项所在行), "|")(0)))
...
Private Function SaveArryData(ByVal lngSource As Long, lngDeptID As Long, ArryGen() As String, ArryPlan() As String, ArryAppend() As String) As Boolean
    ...
    '检查项目 增加附加费用
        ...
        'Pla格式：分组|部位|方法|项目名|单位|价格|数量|固定|性质|收费ID|病人来源|适用科室id
    For lngCount = LBound(ArryPlan) To UBound(ArryPlan)
        If ArryPlan(lngCount) <> "" Then
            If lngSource = Val(Split(ArryPlan(lngCount), "|")(10)) And lngDeptID = Val(Split(ArryPlan(lngCount), "|")(11)) Then
                If Val(Split(ArryPlan(lngCount), "|")(9)) > 0 And Val(Split(ArryPlan(lngCount), "|")(6)) > 0 _
                  And Split(ArryPlan(lngCount), "|")(1) <> "" And Split(ArryPlan(lngCount), "|")(2) <> "" Then
                  
                    strItemList = strItemList & "|" & Val(Split(ArryPlan(lngCount), "|")(9)) & "^" & _
                              Val(Split(ArryPlan(lngCount), "|")(6)) & "^" & _
                             IIf(Split(ArryPlan(lngCount), "|")(7) = "√", 1, 0) & "^0^0^" & _
                             Split(ArryPlan(lngCount), "|")(1) & "^" & _
                             Split(ArryPlan(lngCount), "|")(2) & "^0"
                End If
            End If
        End If
    Next
