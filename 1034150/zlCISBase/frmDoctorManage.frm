...
Private Sub LoadData(ByVal intIndex As Integer)
    ...
    If mblnPic = True Then
        strSql = "select 照片 from 人员照片 where 人员ID=[1]"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, Val(picInfo.Item(intIndex).Tag))
        If rsTemp.RecordCount > 0 Then
            strTempFile = zlDatabase.ReadPicture(rsTemp, "照片")
...
'功能：加载可授权的用户
    ...
    strSql = "Select a.Id, c.部门id, a.姓名, a.编号, a.性别, a.简码, b.名称 As 所属部门, b.编码 As 部门编码, a.专业技术职务,nvl(a.处方权标志,0) as 处方权标志 ,nvl(a.门诊特殊医嘱权限,'0000') as 门诊特殊医嘱权限,nvl(a.住院特殊医嘱权限,'0000') as 住院特殊医嘱权限,a.管理职务," & vbNewLine & _
                ...
                "       Decode(e.级别, 1, '非限制使用', 2, '限制使用', 3, '特殊使用', 'null') As 门诊抗菌药物权限, nvl(a.手术等级,'null') as 手术等级," & vbNewLine & _
