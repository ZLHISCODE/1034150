    ...
    With vsfDetails
        If mint状态 = 1 Then '品种
            ...
            If mstrNode Like "中草药*" Then
                ...
                .ColHidden(mVaricolumn.品种_原料药) = True
            Else
...
'    End If
    ...
    If mint状态 = 1 Then    '品种
        gstrSql = "Select Distinct a.id,c.id as 类别id,a.参考目录ID, '['||c.编码||']'||c.名称 as 名称, a.编码, a.名称 As 通用名称, d.英文名, d.拼音码, d.五笔码, e.毒理分类, e.价值分类, e.货源情况, e.用药梯次, nvl(e.药品类型,0) as 药品类型, e.药品剂型, nvl(e.急救药否,0)  as 急救药否," & _
                        "  e.是否原研药, e.是否专利药, e.是否单独定价, nvl(e.是否新药,0) as 是否新药, nvl(e.是否原料,0) as 是否原料, f.名称 As 参考项目, nvl(e.处方职务,'00') as 处方职务, nvl(e.处方限量,0) as 处方限量, Nvl(a.适用性别,0) AS 适用性别, a.计算单位 As 剂量单位, nvl(e.是否皮试,0) as 是否皮试, nvl(e.抗生素,0) as 抗生素, nvl(e.品种医嘱,0) as 品种医嘱,a.单独应用 as 单味使用" & _
    ...
    Else    '规格
        ...
        If str分类 Like "*品种" Then '选中的是品种节点
            gstrSql = "Select a.Id, c.药名id, a.编码 As 规格编码, a.规格, j.编码 As 品种编码, j.名称 As 通用名称, m.数字码, c.标识码, a.备选码," & _
                              ...
                              " i.配药类型, i.是否不予配置 As 不予调配,C.本位码" & _
...
'                .TextMatrix(i, mSpecColumn.规格_药品编码) = IIf(IsNull(rsRecord!品种编码), "", rsRecord!品种编码)
                ...
                .TextMatrix(i, mSpecColumn.规格_保质期) = IIf(IsNull(rsRecord!保质期), "", rsRecord!保质期)
                ...
                Else
                    ...
                    ElseIf rsRecord!住院可否分零 = -3 Then
                        intTemp = 5
                    End If
                    .TextMatrix(i, mSpecColumn.规格_住院分零使用) = ShowValue(.ColComboList(mSpecColumn.规格_住院分零使用), IIf(IsNull(rsRecord!门诊可否分零), "", intTemp))
                    ...
                    ElseIf rsRecord!门诊可否分零 = -3 Then
                        intTemp = 5
                    End If
                    .TextMatrix(i, mSpecColumn.规格_门诊分零使用) = ShowValue(.ColComboList(mSpecColumn.规格_门诊分零使用), IIf(IsNull(rsRecord!门诊可否分零), "", intTemp))
                End If
...
Private Sub vsfDetails_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    If mint次数 = 2 Then
        With vsfDetails
            ...
            Else    '品种
                Select Case Col
                    Case mVaricolumn.品种_原研药, mVaricolumn.品种_专利药, mVaricolumn.品种_单独定价, mVaricolumn.品种_急救药, mVaricolumn.品种_新药, mVaricolumn.品种_原料药, mVaricolumn.品种_品种下长期医嘱, mVaricolumn.品种_皮试, mVaricolumn.品种_单味使用
                        If .TextMatrix(Row, Col) <> mstrOldValue Then
                            .Cell(flexcpBackColor, Row, Col) = mlngApplyColor
                        End If
                End Select
            ...
            Select Case OldCol
                ...
                Case mVaricolumn.品种_参考项目
                    ...
                    If rsRecord Is Nothing Then
                        ...
                        vsfDetails.TextMatrix(OldRow, mVaricolumn.品种_参考项目id) = ""
                        Exit Sub
                    End If
        ...
        Else    '规格
            ...
            If mstrNode Like "中草药*" Then '中草药
                For i = 1 To vsfDetails.Rows - 1
                    ...
                    gstrSql = gstrSql & .TextMatrix(i, mSpecColumn.规格_加成率) & ","
            ...
            Else    '西成药、中成药
                For i = 1 To vsfDetails.Rows - 1
                    ...
                    gstrSql = gstrSql + strTemp & ","
                    gstrSql = gstrSql & .TextMatrix(i, mSpecColumn.规格_保质期) & ","
                    gstrSql = gstrSql & .TextMatrix(i, mSpecColumn.规格_差价让利) & ","
                    ...
                    If .TextMatrix(i, mSpecColumn.规格_存储温度) <> "" Then
                    ...
                    If .TextMatrix(i, mSpecColumn.规格_配药类型) <> "" Then
                        strTemp = Mid(.TextMatrix(i, mSpecColumn.规格_配药类型), 1, 1)
                    Else
                        strTemp = "Null"
                    End If
                    gstrSql = gstrSql + strTemp & ","
                    ...
                    gstrSql = gstrSql + "Null,Null,Null,Null,"
