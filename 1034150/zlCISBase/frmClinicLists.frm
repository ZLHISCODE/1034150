...
Begin VB.Form frmClinicLists 
   ...
   Begin TabDlg.SSTab tabContent 
      ...
      Tab(0).Control(0)=   "fraSubInfo(0)"
      Tab(0).Control(1)=   "hgd定向执行"
      ...
      Begin VB.Frame fraSubInfo 
         ...
         Top             =   360
         Width           =   7845
         Begin MSHierarchicalFlexGridLib.MSHFlexGrid hgdExse 
            Height          =   855
            Left            =   240
            TabIndex        =   42
            Top             =   360
...
Private Sub cmdKind_Click(Index As Integer)
    ...
    '设置导入的菜单属性
    Me.mnuEditImport.Enabled = (Index = 0)
    '装数据并调整界面
    ...
    Me.tlbThis.Buttons("Add").ButtonMenus("addcopy").Visible = Me.mnuAddCopy.Visible
End Sub
...
Private Sub Form_Load()
    ...
    '可直接通过菜单进行的权限控制
    If InStr(1, mstrPrivs, "项目编辑") = 0 Then
...
'        .Visible = Me.fraSubInfo(0).Visible
    ...
    End With
    With Me.hgdExse
        .Left = 0: .Top = 90: .Width = Me.fraSubInfo(1).Width: .Height = Me.fraSubInfo(1).Height - .Top
    End With
    For i = 0 To 3
        With hgdExse
            .Left = 30: .Top = 30
...
'                Me.hgd定向执行.TextMatrix(.AbsolutePosition, 1) = !执行部门名称
    ...
    If Val(Me.tvwClass.Tag) = 0 Then
        ...
        gstrSql = "Select i.Id, r.检查部位, r.检查方法, r.费用性质, '[' || i.编码 || ']' || i.名称 As 名称, i.规格, i.计算单位," & vbNewLine & _
            "       Decode(i.是否变价, 1, '变价', To_Char(Sum(p.现价))) As 价格, Nvl(r.收费数量, 0) As 数量, Nvl(r.固有对照, 0) As 固定," & vbNewLine & _
            ...
            "Where r.收费项目id = i.Id And i.Id = p.收费细目id(+) And r.适用科室id = b.Id(+) And p.执行日期 <= Sysdate And" & vbNewLine & _
            "      (p.终止日期 Is Null Or p.终止日期 >= Sysdate) And r.诊疗项目id = [1]" & vbNewLine & _
            "Group By i.Id, r.检查部位, r.检查方法, r.费用性质, i.编码, i.名称, i.规格, i.计算单位, i.是否变价, p.现价, r.收费数量, r.固有对照, r.从属项目, i.撤档时间, r.收费方式," & vbNewLine & _
            "         r.病人来源, b.编码, b.名称" & vbNewLine & _
            "order by r.病人来源,B.名称,nvl(R.从属项目,0)"
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, Val(Mid(Item.Key, 2)))
        
        With rsTemp
            Me.hgdExse.Redraw = False
            Do While Not .EOF
                intIndex = IIf(IsNull(!病人来源), 0, !病人来源)
                'If Me.hgdExse(intIndex).Rows - 1 < .AbsolutePosition Then Me.hgdExse(intIndex).Rows = Me.hgdExse(intIndex).Rows + 1
                i = Me.hgdExse.Rows - 1
                Me.hgdExse.Rows = Me.hgdExse.Rows + 1
                Me.hgdExse.TextMatrix(i, col选择) = i '.AbsolutePosition
                Me.hgdExse.TextMatrix(i, col部位) = "" & !检查部位
                Me.hgdExse.TextMatrix(i, col方法) = "" & !检查方法
                Me.hgdExse.TextMatrix(i, col项目名) = !名称
                Me.hgdExse.TextMatrix(i, col规格) = IIf(IsNull(!规格), "", !规格)
                Me.hgdExse.TextMatrix(i, col收费单位) = IIf(IsNull(!计算单位), "", !计算单位)
                Me.hgdExse.TextMatrix(i, col价格) = IIf(IsNull(!价格), "0", !价格)
                Me.hgdExse.TextMatrix(i, col数量) = FormatEx(Format(!数量, "0.00000"), 5)
                Me.hgdExse.TextMatrix(i, col固定) = IIf(!固定 = 0, "", "√")
                Me.hgdExse.TextMatrix(i, col从项) = IIf(!从项 = 0, "", "√")
                Me.hgdExse.TextMatrix(i, col加收) = IIf(0 + !费用性质 = 1, "√", "")
                Me.hgdExse.TextMatrix(i, col状态) = IIf(Format(!撤档时间, "YYYY-MM-DD") <> "3000-01-01", "停用", "")
                
                Select Case !收费方式
                Case 0
                    Me.hgdExse.TextMatrix(i, col收费方式) = "0-正常收取"
                Case 1
                    Me.hgdExse.TextMatrix(i, col收费方式) = "1-检验试管费用"
                Case 2
                    Me.hgdExse.TextMatrix(i, col收费方式) = "2-一次发送只收取一次"
                Case 3
                    Me.hgdExse.TextMatrix(i, col收费方式) = "3-当天只收取一次"
                Case 4
                    Me.hgdExse.TextMatrix(i, col收费方式) = "4-当天未执行收取一次"
                Case 5
                    Me.hgdExse.TextMatrix(i, col收费方式) = "5-当天只收取一次，排斥其他项目"
                Case 6
                    Me.hgdExse.TextMatrix(i, col收费方式) = "6-当天未执行收取一次，排斥其他项目"
                Case 7
                    Me.hgdExse.TextMatrix(i, col收费方式) = "7-每天首次不收取"
                                Case 9
                    Me.hgdExse.TextMatrix(i, col收费方式) = "9-自定义"
                Case Else
                    Me.hgdExse.TextMatrix(i, col收费方式) = "0-正常收取"
                End Select
                Me.hgdExse.ColAlignment(col收费方式) = AlignmentSettings.flexAlignLeftCenter
                
                Select Case Val("" & !病人来源)
                Case 0: Me.hgdExse.TextMatrix(i, col适用场合) = "所有科室"
                Case 1: Me.hgdExse.TextMatrix(i, col适用场合) = "门诊科室"
                Case 2: Me.hgdExse.TextMatrix(i, col适用场合) = "住院科室"
                Case 3: Me.hgdExse.TextMatrix(i, col适用场合) = "体检科室"
                End Select
                If Trim("" & !适用科室名称) <> "" Then
                    Me.hgdExse.TextMatrix(i, col适用科室) = "" & !适用科室名称 & "(" & !适用科室编码 & ")"
                End If
                ...
                Else
                    lngForeColor = &H0&
                End If
                iRow = Me.hgdExse.Row: iCol = Me.hgdExse.Col
                Me.hgdExse.Row = i
                For j = 0 To Me.hgdExse.Cols - 1
                    Me.hgdExse.Col = j
                    Me.hgdExse.CellForeColor = lngForeColor
                Next
                Me.hgdExse.Row = iRow: Me.hgdExse.Col = iCol
                
                .MoveNext
            Loop
            Me.hgdExse.Redraw = True
            If Me.hgdExse.Rows > 2 Then Me.hgdExse.Rows = Me.hgdExse.Rows - 1
            Me.hgdExse.MergeCells = flexMergeFree
            Me.hgdExse.MergeCol(col适用场合) = True
        End With
    End If
    
    '------------------------------------------------
    '检验指标显示
'    If Val(Me.tvwClass.Tag) = 0 And Item.Tag = "C" Then
'        Me.tabContent.TabVisible(conTab检验指标) = True
'        Me.hgdLabs.Redraw = False
'
'        gstrSql = "select R.检验标本,R.报告项目ID,I.中文名||decode(I.英文名,null,'',' ('||I.英文名||')') as 报告项目,I.类型,I.单位" & _
...
'        With rsTemp
'            Do While Not .EOF
'                If Me.hgdLabs.Rows - 1 < .AbsolutePosition Then Me.hgdLabs.Rows = Me.hgdLabs.Rows + 1
'                Me.hgdLabs.TextMatrix(.AbsolutePosition, 0) = .AbsolutePosition
'                Me.hgdLabs.TextMatrix(.AbsolutePosition, 1) = Nvl(!检验标本)
'                Me.hgdLabs.TextMatrix(.AbsolutePosition, 2) = IIf(IsNull(!报告项目), "", !报告项目)
'                Select Case IIf(IsNull(!类型), 0, !类型)
'                Case 0
'                    Me.hgdLabs.TextMatrix(.AbsolutePosition, 3) = "数值"
'                Case 1
'                    Me.hgdLabs.TextMatrix(.AbsolutePosition, 3) = "文字"
'                Case 2
'                    Me.hgdLabs.TextMatrix(.AbsolutePosition, 3) = "日期"
'                Case 3
'                    Me.hgdLabs.TextMatrix(.AbsolutePosition, 3) = "逻辑"
'                End Select
'                Me.hgdLabs.TextMatrix(.AbsolutePosition, 4) = IIf(IsNull(!单位), "", !单位)
'                .MoveNext
'            Loop
'        End With
'        Me.hgdLabs.Redraw = True
'    Else
'        Me.tabContent.TabVisible(conTab检验指标) = False
'    End If
...
'        Me.mnuEditRefer.Enabled = False
        ...
        '不能再禁止,只有启用
        Me.mnuEditStart.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
        Me.mnuEditStop.Enabled = False
    Else
        '可以删除和修改
        Me.mnuEditDelete.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
        Me.mnuEditModify.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
...
'        Me.mnuEditRefer.Enabled = (InStr(1, mstrPrivs, "参考编辑") > 0)
        '对应单据
        Me.mnuEditBill.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
        '只能停用
        Me.mnuEditStart.Enabled = False
        Me.mnuEditStop.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
        '根据类别分别判断禁止
        If Val(Me.tvwClass.Tag) = 0 And lvwItems.SelectedItem.Tag = "C" Then
            'Me.mnuEditExams.Enabled = False
            Me.mnuEditLabs.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
            Me.mnuEditGather.Enabled = (InStr(1, mstrPrivs, "项目编辑") > 0)
...
Private Sub mnuClassAdd_Click()
    With frmClinicClass
        ...
        If Me.tvwClass.SelectedItem Is Nothing Then
            If .ShowMe(1, Me, "(无)", 0, 1) Then
                Call zlRefClasses
            End If
        Else
            If .ShowMe(1, Me, Me.tvwClass.SelectedItem.Text, Mid(Me.tvwClass.SelectedItem.Key, 2), 1) Then
...
Private Sub mnuClassMod_Click()
    ...
    With frmClinicClass
        ...
        .Tag = Mid(Me.tvwClass.SelectedItem.Key, 2)
        If Me.tvwClass.SelectedItem.Parent Is Nothing Then
            If .ShowMe(1, Me, "(无)", 0, 2) Then Call zlRefClasses(Mid(Me.tvwClass.SelectedItem.Key, 2))
        Else
            If .ShowMe(1, Me, Me.tvwClass.SelectedItem.Parent.Text, Mid(Me.tvwClass.SelectedItem.Parent.Key, 2), 2) Then Call zlRefClasses(Mid(Me.tvwClass.SelectedItem.Key, 2))
        End If
    End With
End Sub
...
Private Sub tlbThis_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Case "Class"
        If InStr(1, mstrPrivs, "项目编辑") > 0 Then Call PopupMenu(Me.mnuClass, 2)
    Case "Add"
        Call mnuEditAdd_Click
    Case "Modify"
...
Private Sub tvwClass_MouseUp(Button As Integer, Shift As Integer, x As Single, y As Single)
    If Button = 2 And InStr(1, mstrPrivs, "项目编辑") > 0 Then Call PopupMenu(Me.mnuClass, 2)
End Sub
...
'                    " where I.类别=K.编码 and I.分类ID=[1] And (I.站点 = '" & gstrNodeNo & "' Or I.站点 is Null)"
        ...
        With rsTemp
            ...
            Do While Not .EOF
                ...
                objItem.SubItems(Me.lvwItems.ColumnHeaders("_站点").Index - 1) = IIf(IsNull(!站点), "", !站点)
                Select Case !类别码
                Case "E"
...
Private Sub zlClearDetail()
    ...
    If Val(Me.tvwClass.Tag) = 0 Then
        ...
        If Val(Me.tvwClass.Tag) = 0 Then
            ...
            End If
            With Me.hgdExse
                .Rows = .FixedRows + 1: .Cols = 15: .FixedCols = 1
                .TextMatrix(0, col选择) = "": .TextMatrix(0, col部位) = "部位": .TextMatrix(0, col方法) = "方法"
                .TextMatrix(0, col项目名) = "项目名": .TextMatrix(0, col规格) = "规格": .TextMatrix(0, col收费单位) = "单位": .TextMatrix(0, col价格) = "价格"
                .TextMatrix(0, col数量) = "数量": .TextMatrix(0, col固定) = "固定": .TextMatrix(0, col从项) = "从项"
                .TextMatrix(0, col加收) = "加收": .TextMatrix(0, col状态) = "状态": .TextMatrix(0, col收费方式) = "收费方式"
                .TextMatrix(0, col适用场合) = "适用场合": .TextMatrix(0, col适用科室) = "适用科室"
                .ColWidth(col选择) = 250: .ColWidth(col部位) = 900: .ColWidth(col方法) = 1200: .ColWidth(col项目名) = 3000: .ColWidth(col规格) = 1000
                .ColWidth(col收费单位) = 800: .ColWidth(col价格) = 1200: .ColWidth(col数量) = 1200: .ColWidth(col固定) = 600
                .ColWidth(col从项) = 600: .ColWidth(col加收) = 600: .ColWidth(col状态) = 0: .ColWidth(col收费方式) = 3000
                .ColWidth(col适用场合) = 850: .ColWidth(col适用科室) = 1800
                
                If str类别 <> "检查" Then
                    .ColWidth(col部位) = 0: .ColWidth(col方法) = 0: .ColWidth(col加收) = 0
                End If
                For intCount = 0 To .Cols - 1
                    .TextMatrix(.FixedRows, intCount) = "": .ColAlignmentFixed(intCount) = 4
                Next
                .ColAlignment(col收费方式) = 2
            End With
        End If
