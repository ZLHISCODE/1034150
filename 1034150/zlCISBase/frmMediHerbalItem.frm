...
Private mstr所有记录 As String      '记录界面中所有的值
...
Private Sub chk原料药_KeyDown(KeyCode As Integer, Shift As Integer)
...
Private Sub cmdCancel_Click()
    ...
    strTemp = txt分类.Text & "|" & txt编码.Text & "|" & txt名称.Text & "|" & txt拼音.Text & "|" & txt五笔.Text & "|" & txt英文.Text & "|" & cbo单位.Text & "|" & _
                txt参考.Text & "|" & cbo毒理.Text & "|" & cbo价值.Text & "|" & cbo货源.Text & "|" & cbo梯次.Text & "|" & cbo药品类型.Text & "|" & _
                cbo处方职务.Text & "|" & cbo医保职务.Text & "|" & txt处方限量.Text & "|" & cbo适用性别.Text & "|" & chk单独应用.Value & "|" & chk原料药.Value & "|" & _
                str别名
    If strTemp <> mstr所有记录 And Me.cmdCancel.Tag <> "查阅" Then
        mblnCancel = True
...
Private Sub cmdOK_Click()
    ...
    gstrSql = gstrSql & ",'" & MoveSpecialChar(Trim(Me.cbo单位.Text)) & "'"
    ...
    gstrSql = gstrSql & "," & Me.chk单独应用.Value & "," & Me.chk原料药.Value
    gstrSql = gstrSql & "," & Left(Me.cbo适用性别.Text, 1)
    gstrSql = gstrSql & "," & ZVal(Me.txt参考.Tag) '& "'"
    strTemp = ""
    With Me.msf别名
        For intCount = 1 To .Rows - 1
            If Trim(.TextMatrix(intCount, 1)) <> "" Then
    ...
    If Me.cmdCancel.Tag = "增加" Then
        gstrSql = "zl_草药品种_INSERT('7'," & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1)", "Null)")
    Else
        gstrSql = "zl_草药品种_UPDATE(" & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1)", "Null)")
    End If
...
'    End If
    ...
    If Me.cmdCancel.Tag <> "增加" Then cmdSaveAddItem.Enabled = False: cmdSaveAddSpec.Enabled = False
    If mblnLoad = False Then
        Me.Caption = "中草药品种" & Me.cmdCancel.Tag
        ...
        With rsTemp
            Me.txt分类.Text = "[" & rsTemp!编码 & "]" & rsTemp!名称
        ...
        If Me.cmdCancel.Tag = "增加" Then
            ...
            Else
                ...
                gstrSql = "select nvl(max(编码),'') as 编码" & _
                        " From 诊疗项目目录" & _
                        " Where 类别 = '7' and 编码 like [1] and length(编码)>=[2]"
                ...
                Else
                    ...
                    If Not IsNumeric(strCode) Then
                        ...
                        If Not IsNumeric(strCode) Then
                            strCode = rsTemp!编码
                        Else
                            strCode = Val(strCode) + 1
                        End If
                    Else
                        strCode = Val(strCode) + 1
                    End If
                    Me.txt编码.Text = strTemp & Right(String(13, "0") & Val(strCode), Len(rsTemp!编码) - Len(strTemp))
                End If
            End If
        ...
        Else
            '基本信息项目
            gstrSql = "select I.分类ID,I.编码,I.名称,I.计算单位,I.单独应用,T.是否原料,T.药品剂型," & _
                    "        T.毒理分类,T.货源情况,T.价值分类,T.用药梯次," & _
                    "        nvl(T.药品类型,0) as 药品类型,nvl(T.处方职务,'00') as 处方职务,nvl(T.处方限量,0) as 处方限量," & _
                    "        nvl(T.急救药否,0) as 急救药否,nvl(T.是否原料,0) as 是否原料,nvl(t.抗生素,0) as 抗生素,nvl(T.是否新药,0) as 是否新药,nvl(T.是否皮试,0) as 是否皮试," & _
                    "        I.建档时间,nvl(I.撤档时间,to_date('3000-01-01','YYYY-MM-DD')) as 撤档时间,B.名称 as 参考名称,I.参考目录id,nvl(T.品种医嘱,0) as 品种医嘱,Nvl(I.适用性别,0) AS 适用性别" & _
            ...
            With rsTemp
                If Not .EOF Then
                    Me.lblNote.Caption = "注：该药品建立于" & Format(rsTemp!建档时间, "YYYY-MM-DD")
                    If Format(rsTemp!撤档时间, "YYYY-MM-DD") <> "3000-01-01" Then
                        Me.lblNote.Caption = Me.lblNote.Caption & "，于" & Format(rsTemp!撤档时间, "YYYY-MM-DD") & "停用。"
                    End If
                    Me.txt编码.Text = rsTemp!编码
                    Me.txt名称.Text = rsTemp!名称
                    Me.cbo单位.Text = IIf(IsNull(rsTemp!计算单位), "", rsTemp!计算单位)
                    Me.txt参考.Text = Nvl(rsTemp!参考名称)
                    Me.txt参考.Tag = Nvl(rsTemp!参考目录ID)
    ...
    '                Next
                    For intCount = 0 To Me.cbo毒理.ListCount - 1
                        If Mid(Me.cbo毒理.List(intCount), InStr(1, Me.cbo毒理.List(intCount), "-") + 1) = IIf(IsNull(rsTemp!毒理分类), "", rsTemp!毒理分类) Then
                            Me.cbo毒理.ListIndex = intCount: Exit For
                        End If
                    Next
                    For intCount = 0 To Me.cbo价值.ListCount - 1
                        If Mid(Me.cbo价值.List(intCount), InStr(1, Me.cbo价值.List(intCount), "-") + 1) = IIf(IsNull(rsTemp!价值分类), "", rsTemp!价值分类) Then
                            Me.cbo价值.ListIndex = intCount: Exit For
                        End If
                    Next
                    For intCount = 0 To Me.cbo货源.ListCount - 1
                        If Mid(Me.cbo货源.List(intCount), InStr(1, Me.cbo货源.List(intCount), "-") + 1) = IIf(IsNull(rsTemp!货源情况), "", rsTemp!货源情况) Then
                            Me.cbo货源.ListIndex = intCount: Exit For
                        End If
                    Next
                    For intCount = 0 To Me.cbo梯次.ListCount - 1
                        If Mid(Me.cbo梯次.List(intCount), InStr(1, Me.cbo梯次.List(intCount), "-") + 1) = IIf(IsNull(rsTemp!用药梯次), "", rsTemp!用药梯次) Then
                            Me.cbo梯次.ListIndex = intCount: Exit For
                        End If
                    Next
                    Me.cbo药品类型.ListIndex = rsTemp!药品类型
                    Me.cbo处方职务.ListIndex = IIf(CInt(Left(Format(rsTemp!处方职务, "00"), 1)) <> 9, CInt(Left(Format(rsTemp!处方职务, "00"), 1)), Me.cbo处方职务.ListCount - 1)
                    Me.cbo医保职务.ListIndex = IIf(CInt(Right(Format(rsTemp!处方职务, "00"), 1)) <> 9, CInt(Right(Format(rsTemp!处方职务, "00"), 1)), Me.cbo医保职务.ListCount - 1)
                    Me.txt处方限量.Text = rsTemp!处方限量
                    Me.chk单独应用.Value = IIf(rsTemp!单独应用 = 0, 0, 1)
                    Me.chk原料药.Value = IIf(rsTemp!是否原料 = 0, 0, 1)
                    'Me.chk新药.Value = IIf(!是否新药 = 0, 0, 1)
                    'Me.chk皮试.Value = IIf(!是否皮试 = 0, 0, 1)
                    '刘兴宏:2008/03/17加入抗生素,主要应用于院感系统的抗菌药物的监测:12753
                    'Me.chk抗生素.Value = IIf(!抗生素 = 1, 1, 0)
                    'Me.chk品种医嘱.Value = IIf(!品种医嘱 = 0, 0, 1)
                    Me.cbo适用性别.ListIndex = rsTemp!适用性别
                End If
            End With
            ...
            With rsTemp
                Do While Not .EOF
                    ...
                    If !性质 = 1 And !码类 = 2 Then Me.txt五笔.Text = rsTemp!简码
                    If rsTemp!性质 = 2 Then Me.txt英文.Text = rsTemp!名称
                    .MoveNext
                Loop
            End With
            ...
            With rsTemp
                Do While Not .EOF
                    If Me.msf别名.Rows - 1 < rsTemp.AbsolutePosition Then Me.msf别名.Rows = Me.msf别名.Rows + 1
                    Me.msf别名.TextMatrix(rsTemp.AbsolutePosition, 0) = .AbsolutePosition
                    Me.msf别名.TextMatrix(rsTemp.AbsolutePosition, 1) = rsTemp!名称
                    Me.msf别名.TextMatrix(rsTemp.AbsolutePosition, 2) = IIf(IsNull(rsTemp!拼音), "", rsTemp!拼音)
                    Me.msf别名.TextMatrix(rsTemp.AbsolutePosition, 3) = IIf(IsNull(rsTemp!五笔), "", rsTemp!五笔)
                    rsTemp.MoveNext
                Loop
            End With
        End If
        ...
        If Me.cmdCancel.Tag = "查阅" Then
            ...
            Me.chk单独应用.Enabled = False: Me.chk原料药.Enabled = False
        ...
        End If
    End If
    If Me.cmdCancel.Tag <> "查阅" Then Me.txt名称.SetFocus
    mstr所有记录 = ""
    str别名 = ""
    With msf别名
        For i = 1 To .Rows - 1
...
'        Me.cbo剂型.Clear
'        Do While Not .EOF
'            Me.cbo剂型.AddItem .Fields(0).Value
'            .MoveNext
'        Loop
