...
'参数：如果没有标题，则取报表名称作为缺省标题
    ...
    Do While i + lngStep <= lngBtn
        ...
        If strTmp <> "" Then
            ...
            With frmSource.lbl(CInt(Split(Split(strTmp, "|")(0), ",")(1)))
                gcolUpItem.Add .Index, 0, "标签", 0, 2, 0, "", 0, .Caption, "", .Left, .Top, .Width, .Height, 0, 0, True, .FontName, .FontSize, .FontBold, .FontUnderline, .FontItalic, 0, .ForeColor, .BackColor, (.BorderStyle = 1), 0, "", "", "", False, , , , , , "_" & .Index
                lngTmp = .Index
            End With
        ...
        lngYStart = 32767
        For Each objLbl In frmSource.lbl
            If objLbl.Index > 0 And objLbl.Container Is objPaper And objLbl.Top + objLbl.Height <= lngBtn And objLbl.Top >= i + lngStep And InStr(strIDs & ",", "," & objLbl.Index & ",") = 0 Then
                If objLbl.Top < lngYStart Then lngYStart = objLbl.Top
            End If
        Next
        i = lngYStart
    ...
    Do While i <= lngBtn And lngBtn <> 32767
        ...
        If strTmp <> "" Then
            ...
            '每行对齐方式在输出时根据标签数量及坐标来决定
            With frmSource.lbl(CInt(Split(Split(strTmp, "|")(0), ",")(1)))
                gcolDownItem.Add .Index, 0, "标签", 0, 2, 0, "", 0, .Caption, "", .Left, .Top, .Width, .Height, 0, 0, True, .FontName, .FontSize, .FontBold, .FontUnderline, .FontItalic, 0, .ForeColor, .BackColor, (.BorderStyle = 1), 0, "", "", "", False, , , , , , "_" & .Index
                lngTmp = .Index
            End With
            For j = 1 To UBound(Split(strTmp, "|"))
