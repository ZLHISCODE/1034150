...
Public gstrBigTable As String   '大表
Public gstrMiddleTable As String '中表
...
'返回：intMaxID=当前可用的最大控件索引,ReadReport=报表对象
    ...
    Else
        strSQL = "Select ID,编号,名称,说明,密码,打印机,进纸,票据,系统,程序ID,功能,修改时间,发布时间,打印方式,禁止开始时间,禁止结束时间 From zlReports Where ID=[1]"
    ...
    If Not rsReport.EOF Then
        ...
        '数据源
        strSQL = "Select ID,报表ID,数据连接编号,名称,字段,对象,类型,说明 From zlRPTDatas Where 报表ID=[1] Order by 名称"
        ...
        For i = 1 To rsData.RecordCount
            ...
            For j = 1 To rsPar.RecordCount
                ...
                '！！！以参数序号为关键字加入集合
                tmpData.Pars.Add tmpPar.组名, tmpPar.序号, tmpPar.名称, tmpPar.类型, tmpPar.缺省值, tmpPar.格式, tmpPar.值列表, tmpPar.分类SQL, tmpPar.明细SQL, tmpPar.分类字段, tmpPar.明细字段, tmpPar.对象, "_" & tmpPar.序号, , tmpPar.是否锁定
                
                rsPar.MoveNext
            Next
            
            '！！！以数据源名称作为关键字加入集合
            tmpReport.Datas.Add tmpData.名称, tmpData.数据连接编号, tmpData.SQL, tmpData.字段, tmpData.对象, tmpData.类型, tmpData.说明, tmpData.Pars, "_" & tmpData.名称
        ...
        If blnOnlyData = False Then
            ...
            '关联报表参数对照
            strSQL = "select A.元素ID,A.关联报表ID,A.参数名,A.参数值来源,b.名称 || '(' || b.编号 || ')' as 关联报表名称 from zlrptrelation A ,zlreports B where a.关联报表id=b.id  and a.报表ID=[1]"
            Set rsRelation = OpenSQLRecord(strSQL, "ReadReport", lngRPTID)
            
            '列特性设置
            strSQL = "select A.报表ID,A.元素ID,A.条件名称,A.条件字段,A.条件关系,A.条件值,A.字体颜色,A.背景颜色,A.是否加粗,A.是否整行应用 from zlRPTColProterty A where a.报表ID=[1]"
            Set rsColProtertys = OpenSQLRecord(strSQL, "ReadReport", lngRPTID)
            
            '报表元素(！切记：表格在前,表格元素在后,按次序号(及XY)排序)
            strSQL = "Select RowNum,系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐,自调,字体,字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,父ID,源行号,源ID,上下间距,左右间距,横向分栏,纵向分栏,表格线加粗 From zlRPTItems A Where A.报表ID=[1]" & _
            ...
            For i = 1 To rsItem.RecordCount
                ...
                With tmpItem
                    ...
                    '！！！以ID(控件索引)作为关键字加入集合
                    Set tmpItem = tmpReport.Items.Add(.ID, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, .X, .Y, .W, .H, .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, .父ID, .SubIDs, .CopyIDs, "_" & .ID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏)
                    ...
                    For j = 1 To rsRelation.RecordCount
                        ...
                        With tmpRelation
                            ...
                            tmpItem.Relations.Add .关联报表ID, .参数名, .参数值来源, .关联报表名称
                        End With
                    ...
                    For j = 1 To rsColProtertys.RecordCount
                        tmpItem.ColProtertys.Add rsColProtertys!条件名称, rsColProtertys!条件字段, _
                                                rsColProtertys!条件关系, rsColProtertys!条件值, rsColProtertys!字体颜色, rsColProtertys!背景颜色, _
                                                rsColProtertys!是否加粗, rsColProtertys!是否整行应用, "_" & rsColProtertys!条件名称
...
'功能:保存报表内容(对象objReport)
    ...
    With objReport
        ...
        gcnOracle.Execute "Insert Into Zlrptsqlshistory" & vbNewLine & _
                        ...
                        "  Select b.报表id, b.名称, '" & gstrLoginUserName & "', Sysdate, a.行号, a.内容" & vbNewLine & _
                        "  From zlRPTSQLs A, zlRPTDatas B" & vbNewLine & _
                        "  Where a.源id = b.Id And b.报表id = " & lngRPTID
        ...
        For Each tmpData In .Datas
            ...
            rsPar.Open "Select 源ID,组名,序号,名称,类型,缺省值,格式,值列表,分类SQL,明细SQL,分类字段,明细字段,对象,锁定 From zlRPTPars Where 源ID=0", gcnOracle, adOpenStatic, adLockOptimistic
        ...
        For Each tmpItem In .Items
            ...
            If tmpItem.类型 = 14 Then '子项除外
                ...
                gcnOracle.Execute "Insert Into zlRPTItems(ID,报表ID,格式号,名称,上级ID,类型,序号,参照,性质,内容,表头," & _
                    ...
                    "源ID,源行号,左右间距,上下间距,横向分栏,纵向分栏,表格线加粗) Values(" & _
                    ...
                    "," & tmpItem.上下间距 & "," & tmpItem.横向分栏 & "," & tmpItem.纵向分栏 & "," & Abs(CInt(tmpItem.表格线加粗)) & ")"
        ...
        For Each tmpItem In .Items
            ...
            If InStr(",1,2,3,4,5,10,11,12,13,", "," & tmpItem.类型 & ",") > 0 Then '子项除外
                ...
                gcnOracle.Execute "Insert Into zlRPTItems(ID,报表ID,格式号,名称,上级ID,类型,序号,参照,性质,内容,表头," & _
                    ...
                    Abs(CInt(tmpItem.下线)) & "," & Abs(CInt(tmpItem.边框)) & "," & tmpItem.网格 & "," & tmpItem.前景 & "," & _
                    tmpItem.背景 & ",'" & tmpItem.排序 & "','" & tmpItem.格式 & "','" & tmpItem.汇总 & "'," & _
                    IIF(tmpItem.分栏 = 0, 1, tmpItem.分栏) & "," & Abs(CInt(tmpItem.系统)) & "," & IIF(lngParentID = 0, "Null", lngParentID) & _
                    "," & "Null" & "," & tmpItem.源行号 & "," & tmpItem.左右间距 & "," & tmpItem.上下间距 & "," & tmpItem.横向分栏 & _
                    "," & tmpItem.纵向分栏 & "," & Abs(CInt(tmpItem.表格线加粗)) & ")"
...
'                    End If
                ...
                '处理表格的子项
                If tmpItem.类型 = 4 Or tmpItem.类型 = 5 Then
                    For Each tmpID In tmpItem.SubIDs
                        With .Items("_" & tmpID.ID)
                            ...
                            gcnOracle.Execute "Insert Into zlRPTItems(ID,报表ID,格式号,上级ID,类型,序号,内容,表头,X,Y,W,H," & _
                                "行高,对齐,字体,字号,粗体,斜体,下线,边框,网格,前景,背景,排序,格式,汇总,分栏,系统,自调,父ID,表格线加粗) Values(" & lngItemSubID & "," & _
                                lngRPTID & "," & .格式号 & "," & lngItemID & "," & .类型 & "," & .序号 & ",'" & .内容 & "','" & _
                                .表头 & "'," & .X & "," & .Y & "," & .W & "," & .H & "," & _
                                .行高 & "," & .对齐 & ",'" & .字体 & "'," & .字号 & "," & Abs(CInt(.粗体)) & "," & _
                                Abs(CInt(.斜体)) & "," & Abs(CInt(.下线)) & "," & Abs(CInt(.边框)) & "," & .网格 & "," & .前景 & "," & _
                                .背景 & ",'" & .排序 & "','" & .格式 & "','" & .汇总 & "'," & .分栏 & "," & Abs(CInt(.系统)) & "," & _
                                Abs(CInt(.自调)) & "," & IIF(Val(lngParentID) = 0, "NULL", IIF(lngParentID = 0, "Null", lngParentID)) & "," & Abs(CInt(.表格线加粗)) & ")"
                            
                            '关联报表参数对照
                            For j = 1 To .Relations.count
                                gcnOracle.Execute "Insert Into zlRPTRelation(报表ID,关联报表ID,元素ID,参数名,参数值来源) Values(" & _
                                    lngRPTID & "," & .Relations.Item(j).关联报表ID & "," & lngItemSubID & ",'" & .Relations.Item(j).参数名 & _
                                    "','" & .Relations.Item(j).参数值来源 & "')"
                                If Not objPan Is Nothing Then
                                    i = i + 1: Call ShowPercent(i / intCount, objPan)
                                End If
                            Next
                            ...
                            For j = 1 To .ColProtertys.count
                                gcnOracle.Execute "Insert Into zlRPTColProterty(报表ID,元素ID,条件名称,条件字段,条件关系,条件值,字体颜色,背景颜色,是否加粗,是否整行应用) Values(" & _
                                    lngRPTID & "," & lngItemSubID & ",'" & .ColProtertys.Item(j).条件名称 & "','" & .ColProtertys.Item(j).条件字段 & _
                                    "','" & .ColProtertys.Item(j).条件关系 & "','" & .ColProtertys.Item(j).条件值 & "'," & Val(.ColProtertys.Item(j).字体颜色) & "," & _
                                    Val(.ColProtertys.Item(j).背景颜色) & "," & IIF(.ColProtertys.Item(j).是否加粗, 1, 0) & "," & IIF(.ColProtertys.Item(j).是否整行应用, 1, 0) & ")"
                                If Not objPan Is Nothing Then
                                    i = i + 1: Call ShowPercent(i / intCount, objPan)
                                End If
                            Next
                ...
                For j = 1 To tmpItem.Relations.count
                    gcnOracle.Execute "Insert Into zlRPTRelation(报表ID,关联报表ID,元素ID,参数名,参数值来源) Values(" & _
                        lngRPTID & "," & tmpItem.Relations.Item(j).关联报表ID & "," & lngItemID & ",'" & tmpItem.Relations.Item(j).参数名 & _
                        "','" & tmpItem.Relations.Item(j).参数值来源 & "')"
                    If Not objPan Is Nothing Then
                        i = i + 1: Call ShowPercent(i / intCount, objPan)
                    End If
                Next
...
'功能：拷贝参数集对象
    ...
    For Each tmpPar In objSPars
        With tmpPar
            objOPars.Add .组名, .序号, .名称, .类型, .缺省值, .格式, .值列表, .分类SQL, .明细SQL, .分类字段, .明细字段, .对象, "_" & .Key, .Reserve, .是否锁定
        End With
    Next
End Sub
...
'      2.输出后定位光标位置在本次输出范围的右上角
    ...
    If TypeName(Dev) = "Printer" Then
        lngDC = GetDC(0)                                '获取DC句柄
        lngPPI = GetDeviceCaps(lngDC, LOGPIXELSY)       '获取PPI数值
        ...
        ReleaseDC 0, lngDC                              '使用GetDC()后必须释放
    Else
...
'        If blnBold Then Dev.DrawWidth = 2
        ...
        If Val(Data) < 0 Then
            Dev.FillStyle = vbFSSolid: Dev.FillStyle = vbFSTransparent '奇怪:必须先设置为0,再为1
            If bytShape = 0 Then
...
'        Dev.DrawWidth = lngLineWidth
    ElseIf TypeName(Data) = "String" Then
        ...
        If W >= Dev.TextWidth("A") * LenB(StrConv(strTemp, vbFromUnicode)) Then
            blnW = True And InStr(strTemp, vbCr) = 0 '宽度是否够用(加回车的为不够用,以便拆行)
        End If
...
'功能：拷贝报表对象,防止因Set造成地址的访问
    ...
    For Each objItem In objS.Items
        With objItem
            objO.Items.Add .ID, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, .X, .Y, .W, .H, .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, IIF(.分栏 < 1 And .类型 <> 6, 1, .分栏), .排序, .格式, .汇总, .表格线加粗, .图片, .系统, .父ID, .SubIDs, .CopyIDs, "_" & .ID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏, .Relations, .ColProtertys
        End With
    Next
    For Each objData In objS.Datas
        With objData
            ...
            For Each objPar In .Pars
                objPars.Add objPar.组名, objPar.序号, objPar.名称, objPar.类型, objPar.缺省值, objPar.格式, objPar.值列表, objPar.分类SQL, objPar.明细SQL, objPar.分类字段, objPar.明细字段, objPar.对象, "_" & objPar.序号, objPar.Reserve, objPar.是否锁定
            Next
...
'      2.目前不支持图片元素内容的导出
    ...
    '报表元素
    'Set rsTmp = New ADODB.Recordset
    strSQL = "Select 系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐,自调,字体,字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,ID as 原ID,父ID,源ID,上下间距,左右间距,源行号,横向分栏,纵向分栏 From zlRPTItems Where 报表ID=[1] Start With 上级ID is NULL Connect by Prior ID=上级ID"
    Set rsTmp = OpenSQLRecord(strSQL, "ExportReport", lngRPTID)
    
    strSQL = "Select 报表ID,元素ID,条件名称,条件字段,条件关系,条件值,字体颜色,背景颜色,是否加粗,是否整行应用 From zlRPTColProterty Where 报表ID=[1]"
    ...
    strSQL = "Select B.源ID,B.组名,B.序号,B.名称,B.类型,B.缺省值,B.格式,B.值列表,B.分类SQL,B.明细SQL,B.分类字段,B.明细字段,B.对象,B.锁定 From zlRPTDatas A,zlRPTPars B Where A.ID=B.源ID And A.报表ID=[1]"
    Set rsSub = OpenSQLRecord(strSQL, "ExportReport", lngRPTID)
...
'      2.覆盖已有报表时,当前报表信息不变,除了纸张信息
    ...
    If lngCurrID = 0 Then
        ...
        rsReport.Open "Select ID,编号,名称,说明,密码,打印机,进纸,票据,打印方式,系统,程序ID,功能,修改时间,发布时间,禁止开始时间,禁止结束时间 From zlReports Where Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
        strReport = GetFieldNames(rsReport)
    End If
    ...
    If Not blnOnlyData Then
        ...
        rsItem.CursorLocation = adUseClient
        rsItem.Open "Select 系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐,自调,字体,字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,父ID,源ID,上下间距,左右间距,源行号,横向分栏,纵向分栏 From zlRPTItems Where Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
    ...
    rsCol.Open "Select 报表ID,元素ID,条件名称,条件字段,条件关系,条件值,字体颜色,背景颜色,是否加粗,是否整行应用 From zlRPTColProterty Where  Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
    ...
    If lngCurrID <> 0 Then
        gcnOracle.Execute _
            "Update zlReports" & _
            " Set 修改时间=Sysdate,发布时间=Decode(发布时间,NULL,NULL,Sysdate)" & ",禁止开始时间=to_date('" & str禁止开始时间 & "','HH24:MI:SS')" & ",禁止结束时间=to_date('" & str禁止结束时间 & "','HH24:MI:SS')" & _
            " Where ID=" & lngCurrID
    End If
    ...
    If lngCurrID <> 0 Then
        'Set rsTmp = New ADODB.Recordset
        strSQL = "Select 系统,程序ID,功能,说明 From zlReports" & _
        ...
        If Not rsTmp.EOF Then
            ...
            strSQL = "Select Distinct B.对象 From zlReports A,zlRptDatas B" & _
                " Where A.ID=B.报表ID And B.对象 is Not NULL And A.ID=[1]"
            Set rsPriv = OpenSQLRecord(strSQL, "ExportReport", lngCurrID)
            Do While Not rsPriv.EOF
                For i = 0 To UBound(Split(rsPriv!对象, ","))
                    If InStr(strObject & ",", "," & Split(rsPriv!对象, ",")(i) & ",") = 0 Then
                        If InStr(",SYS,SYSTEM,ZLTOOLS,", "," & UCase(Split(Split(rsPriv!对象, ",")(i), ".")(0)) & ",") = 0 Then
                            strObject = strObject & "," & Split(rsPriv!对象, ",")(i)
                        End If
                    End If
                Next
                rsPriv.MoveNext
            ...
            '参数所涉及的对象
            'Set rsPriv = New ADODB.Recordset
            strSQL = "Select Distinct Replace(C.对象,'|',',') as 对象" & _
                " From zlReports A,zlRptDatas B,zlRptPars C" & _
                " Where A.ID=B.报表ID And B.ID=C.源ID And C.对象 is Not NULL And A.ID=[1]"
            Set rsPriv = OpenSQLRecord(strSQL, "ExportReport", lngCurrID)
            Do While Not rsPriv.EOF
                For i = 0 To UBound(Split(rsPriv!对象, ","))
                    If InStr(strObject & ",", "," & Split(rsPriv!对象, ",")(i) & ",") = 0 And Split(rsPriv!对象, ",")(i) <> "" Then
                        If InStr(",SYS,SYSTEM,ZLTOOLS,", "," & UCase(Split(Split(rsPriv!对象, ",")(i), ".")(0)) & ",") = 0 Then
                            strObject = strObject & "," & Split(rsPriv!对象, ",")(i)
                        End If
                    End If
                    ...
                    If strObject <> "" Then
                        For i = 0 To UBound(Split(strObject, ","))
                            gcnOracle.Execute "Insert Into zlProgPrivs(系统,序号,功能,对象,所有者,权限) Values(" & _
                                IIF(IsNull(rsTmp!系统), "NULL", rsTmp!系统) & "," & rsTmp!程序ID & ",'" & rsTmp!功能 & "'," & _
                ...
                If InStr(StrPar & ",", "," & tmpPar.名称 & ",") = 0 Then
                    ...
                    With tmpPar '！！以名称(唯一)关键字加入
                        If .Reserve Like "*…|*" And Not blnFirst Then
                            ...
                            tmpPars.Add .组名, .序号, .名称, .类型, CStr(Split(.Reserve, "|")(0)), .格式, .值列表, .分类SQL, .明细SQL, .分类字段, .明细字段, .对象, "_" & .名称, Split(.Reserve, "|")(1) & "|" & .缺省值, .是否锁定
                        Else
                            '第一次进入(非重置条件)或其它类型的条件
                            tmpPars.Add .组名, .序号, .名称, .类型, .缺省值, .格式, .值列表, .分类SQL, .明细SQL, .分类字段, .明细字段, .对象, "_" & .名称, .Reserve, .是否锁定
                        End If
                    End With
                End If
            Next
                                    ...
                                    For j = 0 To objBody.FixedCols - 1
                                        ...
                                        If InStr(strSkip, "[" & i & "," & j & "]") = 0 Then
                                            ...
                                            If Not DrawCell(objOut, objBody.Text, .X + X, .Y + Y, W, H, .X + .W, , objBody.GridColor, lngForeColor, objBody.BackColor, _
                                                ...
                                                True, sngScale, , objItem.表格线加粗) Then Exit Function '合并时允许换行
                                        End If
                                    ...
                                    For j = .ColB To .ColE
                                        ...
                                        If InStr(strSkip, "[" & i & "," & j & "]") = 0 Then
                                            ...
                                            If Not DrawCell(objOut, objBody.Text, .X + X, .Y + Y, W, H, .X + .W, , objBody.GridColor, lngForeColor, lngBackColor, _
                                                ...
                                                objItem.自调, sngScale, , objItem.表格线加粗) Then Exit Function
                                        End If
                                        X = X + objBody.ColWidth(j)
                                    Next
                                End If
                                        ...
                                        If Not blnMeasure Then
                                            ...
                                            If Not objCurDLL Is Nothing And TypeName(objOut) = "Printer" Then
                                                ...
                                                If j <= objBody.Cols - 1 Then
                                                    Call objCurDLL.Act_PrintSheetRow(frmSource.mobjReport.编号, objBody, intPage + intBasePage, i + 1 - .RowB, colRowIDs("_" & objBody.Index)(i))
                                                End If
                                            End If
                                                    ...
                                                    If W > 0 Then
                                                        ...
                                                        If Not objPic Is Nothing Then
                                                            ...
                                                            If Not DrawCell(objOut, objPic, .X + X, .Y + Y, W, H, .X + .W * .Copys, , objBody.GridColor, , , _
                                                                    ...
                                                                    , GetVscAlign(objBody.CellAlignment), True, sngScale, , objItem.表格线加粗) Then Exit Function
                                                        Else
                                                            Set objFont = objBody.Font
                                                            
                                                            '检查并设置列的自动字体设置,利用缓存,尽量加快速度
...
'返回：lngID=报表或组ID
    ...
    '是报表组
    If TypeName(varReport) = "String" Then
        strSQL = "Select ID From zlRPTGroups Where Nvl(系统,0)=[3] And Upper(编号)=[1]"
    Else
        strSQL = "Select ID From zlRPTGroups Where Nvl(系统,0)=[3] And 程序ID=[2]"
    End If
    'Set rsTmp = New ADODB.Recordset
    Set rsTmp = OpenSQLRecord(strSQL, "isGroup", UCase(varReport), Val(varReport), lngSys)
        ...
        With grsConnect
            ...
            Do While .EOF = False
                If Nvl(!编号, 0) = Index Then
                    ...
                    Set cn = mdlPublic.GetConnection(strServer, Nvl(!用户名), strPass _
                                    , CBool(Val("0-不转换密码")) _
                                    , intDeviceType)
...
'      arrInput=不定个数的参数值,按参数号顺序依次传入,必须是明确类型。第一个参数格式如果是“数据连接=x”。x表示数据连接的编号
    ...
    If Not rsTmp Is Nothing Then
        If gstrBigTable = "" Then
            ...
            Do While Not rsIndex.EOF
                gstrBigTable = gstrBigTable & "," & rsIndex!表名
                rsIndex.MoveNext
            Loop
            gstrBigTable = Mid(gstrBigTable, 2)
        End If
        '再取中表（统计信息，User_tab_statistics:num_rows>3000）
        StrPar = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\SQLPlanEx", "MiddleTable", "1")
        If Val(StrPar) = 1 Then
            StrPar = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\SQLPlanEx", "MiddleTableRows", "3000|1000000")
            If StrPar <> gstrMiddleTableRows Then
                ...
                gstrMiddleTable = ""
                Do While Not rsIndex.EOF
                    If InStr("," & gstrBigTable & ",", "," & rsIndex!表名 & ",") = 0 Then
                        gstrMiddleTable = gstrMiddleTable & "," & rsIndex!表名
                    End If
                    rsIndex.MoveNext
                Loop
...
'参数：lngSel=默认选择某一行，绑定值行=lngsel则选中
    ...
    If strFind <> "" Then
        ...
        strSQL = strSQL & " Where upper(编号)='" & strFind & "' or upper(名称) like " & "'%" & strFind & "%' or  upper(zlpinyincode(名称)) like " & "'%" & strFind & "%'"
    End If
    strSQL = strSQL & " Order by 编号"
    
    frmNewSelect.strSQLList = strSQL
    frmNewSelect.strFLDList = "ID," & adNumeric & ",&B|" & "编号," & adVarChar & ",&S|" & "名称," & adVarChar & ",&S|显示值," & adVarChar & ",&D"
    frmNewSelect.strParName = "关联报表"
...
'功能：复制一个关联报表对象
    ...
    For i = 1 To objRelations.count
        objRelationsCopy.Add objRelations.Item(i).关联报表ID, objRelations.Item(i).参数名, objRelations.Item(i).参数值来源, objRelations.Item(i).关联报表名称
    Next
End Sub
...
'功能：复制一个关联报表对象
    ...
    For i = 1 To objColProtertys.count
        objColProtertysCopy.Add objColProtertys.Item(i).条件名称, objColProtertys.Item(i).条件字段, objColProtertys.Item(i).条件关系, _
                                objColProtertys.Item(i).条件值, objColProtertys.Item(i).字体颜色, objColProtertys.Item(i).背景颜色, objColProtertys.Item(i).是否加粗, objColProtertys.Item(i).是否整行应用, "_" & objColProtertys.Item(i).Key
    Next
End Sub
...
'功能：模拟zlRegister部件的GetConnection接口函数（10.34不支持该部件）
'参数：
'  vServer：服务器名，或者可以直接指定IP:Port/SID
'  vUser：用户名
'  vPass：密码
'  vTransPass：是否转换密码；True转换；False不转换
'  vDeviceType：数据连接的驱动类型；0-Microsoft ODBC；1-Oracle OLEDB
'  vErrInfo：错误信息
