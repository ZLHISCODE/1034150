...
Private Sub lbl_Click(Index As Integer)
    ...
    If lbl(Index).Tag <> "" Then
        ...
        If Not CheckReportPriv(objRelations.Item(1).关联报表ID) Then
            MsgBox "你没有权限查询该报表某些数据源中的对象！", vbInformation, App.Title: Exit Sub
        End If
        '执行报表
        If CheckPass(objRelations.Item(1).关联报表ID) = False Then
            MsgBox "报表数据错误，不能执行该报表！", vbInformation, App.Title: Exit Sub
        End If
        
        Set gobjReport = ReadReport(objRelations.Item(1).关联报表ID)
        '初始化参数
        garrPars = Array()
        '定位记录集
        On Error Resume Next
...
Private Sub mnuFile_Print_Click()
    ...
    Do While blnGoOn
        ...
        blnExit = False
        'If (Not mblnPrintEmpty Or mobjReport.打印方式 = 1) And mbytStyle = 2 Then
...
Private Sub msh_EnterCell(Index As Integer)
    ...
    If Not mobjCurDLL Is Nothing And IsNumeric(msh(Index).Tag) Then
        ...
        Call mobjCurDLL.Act_EnterCell(mobjReport.编号, intRow, intCol, strText)
        '改变后的值
        ...
        If strRow <> Index & "," & msh(Index).Row Then
            ...
            Next
            Call mobjCurDLL.Act_EnterRow(mobjReport.编号, msh(Index).Row, Mid(strRowText, 2), msh(Index))
...
'                    msh(Index).Cell(flexcpBackColor, msh(Index).Row, i) = &H808080 '突出汇总行列单元
...
'                    msh(Index).Cell(flexcpBackColor, i, msh(Index).Col) = &H808080 '突出汇总行列单元
...
Private Sub msh_LeaveCell(Index As Integer)
    ...
    If blnRefresh = False Then Exit Sub
    If Not mobjCurDLL Is Nothing And IsNumeric(msh(Index).Tag) Then
        Call mobjCurDLL.Act_LevelCell(mobjReport.编号, msh(Index).Row, msh(Index).Col, msh(Index).Text)
    End If
...
'            lngLeftCol = tmpMsh.LeftCol
            For i = 0 To tmpMsh.Cols - 1
                If tmpMsh.ColWidth(i) <> msh(tmpMsh.Tag).ColWidth(i) Then
                    ...
                    If Not mobjCurDLL Is Nothing Then
                        Call mobjCurDLL.Act_ColResize(mobjReport.编号, i, sngWidth, tmpMsh.ColWidth(i))
                    End If
...
'功能：在查询界面上组织显示一个任意表格
    ...
    With objItem
        ...
        For Each tmpID In .SubIDs
            Set tmpItem = mobjReport.Items("_" & tmpID.ID)
            mshBody.ColData(tmpItem.序号) = tmpItem
            mshBody.ColWidth(tmpItem.序号) = tmpItem.W
            mshBody.ColAlignment(tmpItem.序号) = Switch(tmpItem.对齐 = 0, 1, tmpItem.对齐 = 1, 4, tmpItem.对齐 = 2, 7)
            If mshBody.FixedRows - 1 >= 0 And mshBody.Rows - 1 >= 0 Then mshBody.Cell(flexcpAlignment, mshBody.FixedRows - 1, tmpItem.序号, mshBody.Rows - 1, tmpItem.序号) = mshBody.ColAlignment(tmpItem.序号)
            mshBody.MergeCol(tmpItem.序号) = tmpItem.自调
        Next
        ...
        Do While blnDo
            ...
            For i = 0 To UBound(arrFormula)
                strFormula = Split(arrFormula(i), "|")(0) '公式
                strFormat = Split(arrFormula(i), "|")(1) '格式
                lngCol = CInt(Split(arrFormula(i), "|")(2)) '列号
                strState = Split(arrFormula(i), "|")(3) '汇总
        ...
        If blnDo Then
            ...
            For i = 0 To UBound(arrFormula)
                strState = Split(arrFormula(i), "|")(3) '汇总
                lngCol = Split(arrFormula(i), "|")(2) '列号
                strFormat = Split(arrFormula(i), "|")(1) '格式
                strFormula = Split(arrFormula(i), "|")(0) '公式
        ...
        For Each tmpID In .SubIDs
            ...
            If tmpItem.Relations.count > 0 Then
                For i = 0 To mshBody.Rows - 1
                    '合计列不设置
                    If TypeName(mshBody.Cell(flexcpData, i, tmpItem.序号)) <> "Empty" Then
                        ...
                        On Error Resume Next
                        mshBody.Cell(flexcpData, i, tmpItem.序号).Remove 2
                        On Error GoTo 0
                        mshBody.Cell(flexcpData, i, tmpItem.序号).Add tmpItem.Relations
                    End If
                Next
...
'功能：根据报表内容，设置表格、标签的相对位置
    ...
    For Each tmpItem In mobjReport.Items
        If tmpItem.格式号 = bytFormat And tmpItem.类型 = 4 And tmpItem.分栏 > 1 _
...
'说明：如果运行该函数之后isArray(marrPage)=False,则表明没有表格输出
    ...
    For k = 0 To UBound(arrGrids)
        ...
        blnApp = (objItem.类型 = 4 And objItem.性质 = 1 And objItem.参照 <> "") '是否附加表格
        ...
        If objItem.类型 = 4 Then blnHaveApp = GridHaveApp(objItem.ID)
        ...
        '页间循环(每个表格在多页中计算)
        lngBaseRows = Int(lngBodyH / objItem.行高) '该表格一页内一栏可以打印的行数
        Do
            '页内循环(两个DO)
            ...
            Loop Until lngColB > objBody.Cols - 1 Or _
                (objItem.类型 = 4 And (objItem.分栏 > 1 Or objItem.性质 = 1 Or blnHaveApp))
        ...
        If objBody.Index <> 0 And Not mobjReport.Items("_" & objBody.Index) Is Nothing Then
            If mobjReport.Items("_" & objBody.Index).数据源 <> "" Then
                ...
                With mLibDatas("_" & mobjReport.Items("_" & objBody.Index).数据源).DataSet
                    ...
                    On Error Resume Next
                    If !分组标识 & "" <> "" Or !分组标识 & "" = "" Then
                        If Err.Number = 0 Then
                            '存在分组标识字段则计算有多少组医嘱，间隔开的分组标识相同的算两组
...
rePage:
                                    If colCard.count >= lngC Or blnRePage Then
                                        If Not IsArray(marrPageCard) Then
                                            ReDim marrPageCard(intPage) As PageCards  '第一次初始页
            ...
            For Each objPageCell In arrPageTmp(0)
                If IsArray(marrPageCard) Then
                    For i = 0 To UBound(marrPageCard)
                        On Error Resume Next
                        If marrPageCard(i).Item("_" & mobjReport.Items("_" & objPageCell.ID).父ID).ID <> 0 Then
                            If Err.Number = 0 Then
                                With marrPageCard(i).Item("_" & mobjReport.Items("_" & objPageCell.ID).父ID)
                                        For j = 1 To .Item.count
                                            ...
                                            End If
                                            arrPage(i).Add objPageCell.ID, objPageCell.X + ((j - 1) Mod .Col) * (mobjReport.Items("_" & mobjReport.Items("_" & objPageCell.ID).父ID).W + mobjReport.Items("_" & mobjReport.Items("_" & objPageCell.ID).父ID).左右间距), _
...
'注意：将元素的真实关键字也一并作调整
    ...
    For Each tmpItem In objReport.Items
        With tmpItem
            ...
            Next
            objItems.Add .ID & intIndex, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, .X, .Y, .W, .H, .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, IIF(.父ID = 0, 0, .父ID & intIndex), objSubIDs, objCopyIDs, "_" & .ID & intIndex, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏, .Relations, .ColProtertys
        End With
    Next
    ...
    For Each tmpItem In objItems
        With tmpItem
            objReport.Items.Add .ID, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, .X, .Y, .W, .H, .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, .父ID, .SubIDs, .CopyIDs, "_" & .ID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏, .Relations, .ColProtertys
        End With
    Next
End Sub
                                ...
                                If Not objStatusGridItem Is Nothing Then
                                    For k = 1 To objStatusGridItem.ColProtertys.count
                                        ...
                                        If lngCol + j = mshBody.FixedCols And objColProp.是否整行应用 Then
                                            If CheckColProtertys(mshBody.TextMatrix(lngRow, lngCol + j), objColProp.条件关系, varIFValue) Then
...
'说明：只处理附加表体,且按设计尺寸处理,只在报表显示前调用一次
    ...
    For Each tmpItem In mobjReport.Items
        If tmpItem.格式号 = bytFormat And (tmpItem.类型 = 4 Or tmpItem.类型 = 5) _
            ...
            If GridHaveApp(tmpItem.ID) Then
                ...
                If lngMaxW <> -1 Then
                    For i = 0 To UBound(Split(strIDs, ","))
                        intCurID = CInt(Split(strIDs, ",")(i))
                        If mobjReport.Items("_" & intCurID).类型 = 4 Then
                             '任意表只需处理表头
                            Set tmpMsh = msh(CInt(msh(intCurID).Tag))
                        ElseIf mobjReport.Items("_" & intCurID).类型 = 5 Then
                            Set tmpMsh = msh(intCurID)
                        End If
                        tmpMsh.Redraw = False
                        lngCurW = GetGridColWidth(tmpMsh)
...
Private Sub ReplaceSysNo(objReport As Report)
    ...
    For i = 1 To objReport.Datas.count
        objReport.Datas(i).SQL = Replace(objReport.Datas(i).SQL, "[系统]", IIF(glngSys <> 0, glngSys, objReport.系统))
        For j = 1 To objReport.Datas(i).Pars.count
            objReport.Datas(i).Pars(j).明细SQL = Replace(objReport.Datas(i).Pars(j).明细SQL, "[系统]", IIF(glngSys <> 0, glngSys, objReport.系统))
            objReport.Datas(i).Pars(j).分类SQL = Replace(objReport.Datas(i).Pars(j).分类SQL, "[系统]", IIF(glngSys <> 0, glngSys, objReport.系统))
        Next
    Next
End Sub
...
Public Function GetReportForm(objParent As Object, objCurDLL As clsReport, LibDatas As Object, arrPars As Variant, ByVal bytStyle As Byte) As Object
    ...
    If Err.Number = 0 Then
        ...
        If lbl(lngCurInx).BackColor = CON_SETFOCES And lngCurInx <> 0 Then
            ...
            lngCurInx = 0: lngTmpColor = 0
        End If
        mobjfrmShow.BorderStyle = FormBorderStyleConstants.vbBSNone '设置为无边框
        mobjfrmShow.Caption = mobjfrmShow.Caption       '重点是这一句
