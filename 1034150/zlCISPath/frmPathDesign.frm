...
Private Sub cmdCheck_Click(Index As Integer)
    ...
    If Index = 0 Then
        strSql = "Select a.Id From 路径医嘱内容 A, 临床路径医嘱 B Where a.Id = b.医嘱内容id And b.路径项目id = [1] Order By a.序号"
        strSql = "Select a.Id, a.相关id, a.序号, a.期效, a.诊疗项目id, a.收费细目id, a.医嘱内容, a.单次用量, a.总给予量, a.标本部位, a.检查方法, a.医生嘱托, a.执行频次, a.频率次数," & vbNewLine & _
                ...
                "Where a.Id = b.医嘱内容id And b.路径项目id = [1]" & vbNewLine & _
                "Order By a.序号"
    ...
    strSql = "Select a.项目id, a.医嘱内容id" & vbNewLine & _
                ...
                "      a.操作时间 = (Select Max(操作时间) From 路径医嘱变动 C Where c.项目id = [1] And c.审核时间 Is Null)" & vbNewLine & _
...
'功能：根据所选择的路径版本，加载路径表相应的数据进行显示
    ...
    Else
        ...
        With vsPath
            ...
            '--医嘱定义内容集
            strSql = _
            " Select Distinct A.ID,A.相关ID,A.序号,A.期效,A.诊疗项目ID,A.收费细目ID," & _
            ...
            If strItems <> "" Then
                ...
                strSql = "Select /*+cardinality(b,10)*/" & vbNewLine & _
                    ...
                    "      a.操作时间 = (Select Max(操作时间) From 路径医嘱变动 C Where c.项目id = a.项目id And c.审核时间 is Null)" & vbNewLine & _
                    "Order By a.项目id, a.医嘱内容id"
...
Private Sub FuncShowItemAdvice()
    ...
    With vsPath
        ...
        If TypeName(.Cell(flexcpData, .Row, .Col)) <> "Empty" Then
            If (.Cell(flexcpBackColor, .Row, .Col) = Color_DiffBack Or .Cell(flexcpForeColor, .Row, .Col) = Color_NeedAuditFore) And mbytFunc <> 0 Then
...
'功能：审核/取消审核当前版本
'参数：blnAudit=审核/取消审核
'参数：intNum 1=医务科审核 -1=医务科取消审核 2-药剂科审核 -2=药剂科取消审核
    Dim objCombo As CommandBarComboBox
    Dim strSql As String
...
'功能:查找存在路径医嘱变动的路径项目,并将不存在医嘱变动的路径项目背景设置为灰色
    ...
    On Error GoTo errH
    If mbytFunc = 1 Then
        'mbytFunc=1 查看路径医嘱变动记录时设置路径表单中不存在变动记录的项目背景色
        '=0 隐藏路径医嘱变动记录时恢复路径表单中不存在变动记录的项目背景色
        strSql = "Select Distinct b.项目id From 临床路径项目 A, 路径医嘱变动 B Where a.路径id = [1] And a.版本号 = [2] And a.Id = b.项目id And B.审核状态=1 "
...
Private Sub FuncLoadChangeTimes()
'功能:医嘱变动详情变动次数加载
'说明:mbytFunc=1 医嘱变动历史记录(审核人\审核时间 不为空
