...
Begin VB.Form frmPathSend 
   ...
   Begin VB.ListBox lstPati 
      Appearance      =   0  'Flat
      Height          =   1080
      ItemData        =   "frmPathSend.frx":6852
      Left            =   4800
      List            =   "frmPathSend.frx":685F
      Sorted          =   -1  'True
      Style           =   1  'Checkbox
      TabIndex        =   17
      Top             =   4080
...
Private mstrSQLPhase  As String
...
'     strSQLPhase-护理路径生成时传人 SQL语句,字段列：阶段ID,日期,天数
    ...
    Set mrsPhase = GetPhase(mPP.路径ID, mPP.版本号, mlng病人阶段ID, mPP.当前阶段分支ID, mlng天数)
...
Private Function GetPhase(ByVal lng路径ID As Long, ByVal lng版本号 As Long, ByVal lng当前阶段ID As Long, ByVal lng当前阶段分支ID As Long, _
        ByVal lng天数 As Long, Optional ByVal lng合并路径记录ID As Long) As ADODB.Recordset
'功能：读取当前时间可用的阶段
    ...
    Else
        ...
        If mlngFun = 0 Then
            If mint场合 = 1 Then
                ...
                strSql = "Select b.日期,b.天数,a.Id, Nvl(a.父id,0) as 父id, a.序号, a.名称, a.说明,a.开始天数, a.结束天数, a.分类,NVL(a.分支ID,0) AS 分支ID From (" & mstrSQLPhase & ") B, 临床路径阶段 A Where a.Id = b.阶段ID order by b.排序 "
                On Error GoTo errH
                Set GetPhase = zlDatabase.OpenSQLRecord(strSql, "获取适用阶段")
                Exit Function
        ...
        For i = k To lng天数 - 1
            ...
            If Not mrsPhase.EOF Then
                strSql = "Zl_病人路径生成_Insert(1," & mPati.病人ID & "," & mPati.主页ID & ",NULL," & mPati.科室ID & "," & _
                        ...
                        "," & strDate & "," & mlng提前天数 & _
                        ",'" & strFirstType & "',Null" & _
                        ",'','','','" & UserInfo.姓名 & "',To_Date('" & Format(AddDate, "yyyy-MM-dd HH:mm:ss") & _
    ...
    If Not blnIsSend Then
        If mlngFun = 0 Then
            ...
            strSql = "Zl_病人路径生成_Insert(" & lng序号 & "," & mPati.病人ID & "," & mPati.主页ID & ",NULL," & mPati.科室ID & "," & _
                    ...
                    "," & strDate & "," & mlng提前天数 & _
                    ",'" & strFirstType & "',Null" & _
                    ",'','','','" & UserInfo.姓名 & "',To_Date('" & Format(AddDate, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS'),'未生成任何项目',Null,'已经执行|1" & vbTab & "已经执行',Null,Null,'',Null" & _
