...
Begin VB.Form frmPathManage 
   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPathManage.frx":058A
            Key             =   "Path"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPathManage.frx":0B24
            Key             =   "File"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPathManage.frx":10BE
            Key             =   "branch"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPathManage.frx":7920
            Key             =   "Merge"
         EndProperty
      EndProperty
   End
   Begin VB.PictureBox picList 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsFile 
         ...
         FormatString    =   $"frmPathManage.frx":E182
         ScrollTrack     =   -1  'True
         ScrollBars      =   0
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.PictureBox picInfo 
      ...
      Begin VB.Label lbl科室 
         ...
         Index           =   1
         Left            =   330
         MouseIcon       =   "frmPathManage.frx":E1BF
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2355
            MinWidth        =   882
            Picture         =   "frmPathManage.frx":E311
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
         EndProperty
   ...
   End
End
Attribute VB_Name = "frmPathManage"
...
Private Sub FuncPathDelete()
    ...
    With rptPath.SelectedRows(0)
        ...
        strSql = "Zl_临床路径目录_Delete(" & .Record(COL_ID).Value & ")"
        On Error GoTo errH
...
Private Sub FuncPathFileDelete()
    ...
    With vsFile
        ...
        strSql = "Zl_临床路径文件_Delete(" & rptPath.SelectedRows(0).Record(COL_ID).Value & ",'" & .TextMatrix(.Row, 1) & "')"
        On Error GoTo errH
        zlDatabase.ExecuteProcedure strSql, Me.Caption
        On Error GoTo 0
...
'功能：打开临床路径文件查看
    ...
    strFile = Sys.ReadLob(glngSys, 10, rptPath.SelectedRows(0).Record(COL_ID).Value & "," & vsFile.TextMatrix(vsFile.Row, 1), strFile)
...
'参数：lngModle=0 新增普通文件，=1 新增患者版路径表
    ...
    cdgFile.DialogTitle = "选择要添加的临床路径文件"
    If lngModle = 0 Then
        cdgFile.Filter = "所有文件|*.*"
    Else
    ...
    cdgFile.InitDir = GetSetting("ZLSOFT", "公共模块\" & App.ProductName, "临床路径文件目录")
    ...
    End If
    On Error GoTo 0
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName, "临床路径文件目录", gobjFile.GetFile(cdgFile.FileName).ParentFolder.Path
    ...
    ReDim arrSQL(0)
    arrSQL(0) = "Zl_临床路径文件_Insert(" & rptPath.SelectedRows(0).Record(COL_ID).Value & ",'" & strFileName & "'," & lngModle & ")"
    If Not zlBlobSql(10, rptPath.SelectedRows(0).Record(COL_ID).Value & "," & strFileName, strFile, arrSQL()) Then
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Tool_Option    '字典管理
        ...
        Call zlAppTool.zlAppointDict("路径结果性质,路径常见结果,变异常见原因", glngSys)
    Case conMenu_Edit_Report    '出径登记表
        Call frmPathOutDefinition.ShowMe(Me, rptPath.SelectedRows(0).Record(COL_ID).Value, rptPath.SelectedRows(0).Record(COL_名称).Value)
    ...
    Case Else
        ...
        Else
            ...
            Select Case Control.ID
            Case conMenu_Edit_Audit, conMenu_Edit_Untread, conMenu_Edit_MedicalAudit, conMenu_Edit_MedicalUntread  '审核,取消审核,药剂科审核,药剂科取消审核
                Call RefreshData
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Archive * 10# + 1, conMenu_Edit_Archive * 10# + 2    '增加文件,患者临床路径表
        ...
        Else
            Control.Enabled = lng路径ID <> 0
            If lng路径ID <> 0 Then
                If Control.ID = conMenu_Edit_Archive * 10# + 2 And rptPath.SelectedRows(0).Record(COL_性质).Value = 1 Then
    ...
    Case conMenu_Edit_Report  '出径登记表
        ...
        If rptPath.SelectedRows.count > 0 Then
            If Not rptPath.SelectedRows(0).GroupRow Then
                If rptPath.SelectedRows(0).Record(COL_性质).Value <> 1 Then
                    blnEnabled = True
                End If
            End If
        End If
...
Private Sub Form_Load()
    ...
    gbln双审核 = zlDatabase.GetPara("双审核模式", glngSys, mlngModul) = 1
...
'功能：主窗口菜单定义部份
    ...
    With objMenu.CommandBar.Controls
        ...
        objControl.BeginGroup = True
        If gbln双审核 Then
            Set objControl = .Add(xtpControlButton, conMenu_Edit_MedicalAudit, "药剂科审核(&W)"): objControl.BeginGroup = True
            objControl.IconId = conMenu_Edit_Audit
            Set objControl = .Add(xtpControlButton, conMenu_Edit_MedicalUntread, "药剂科取消审核(&S)")
            objControl.IconId = conMenu_Edit_Untread
            Set objControl = .Add(xtpControlButton, conMenu_Edit_Audit, "医务科审核(&A)"): objControl.BeginGroup = True
            Set objControl = .Add(xtpControlButton, conMenu_Edit_Untread, "医务科取消审核(&R)")
        Else
    ...
    With objBar.Controls
        ...
        If gbln双审核 Then
            Set objControl = .Add(xtpControlButton, conMenu_Edit_MedicalAudit, "药剂科审核"): objControl.BeginGroup = True
            objControl.IconId = conMenu_Edit_Audit
            Set objControl = .Add(xtpControlButton, conMenu_Edit_MedicalUntread, "药剂科取消审核")
            objControl.IconId = conMenu_Edit_Untread
            Set objControl = .Add(xtpControlButton, conMenu_Edit_Audit, "医务科审核"): objControl.BeginGroup = True
            Set objControl = .Add(xtpControlButton, conMenu_Edit_Untread, "医务科取消审核")
        Else
...
'功能：显示临床路径文件内容和患者临床路径
    ...
    strSql = "Select 文件名,创建人,创建时间,类别 From 临床路径文件 Where 路径ID=[1] Order by 创建时间"
    ...
    With vsFile
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            Set .Cell(flexcpPicture, i, 0) = GetFileIcon(rsTmp!文件名, ICON_SMALL, picTemp)
...
'参数：用于定位
    ...
    strSql = "Select Distinct a.Id, a.分类, a.编码, a.名称, a.病例分型, a.适用病情, a.适用性别, a.适用年龄, a.说明, a.通用, a.最新版本," & vbNewLine & _
             "                Decode(b.Id, Null, Null, 1) As 是否存在分支, a.性质, Min(Decode(c.审核时间, Null, 0, 1)) As 已审核" & vbNewLine & _
             "From 临床路径目录 A, 临床路径分支 B, 临床路径版本 C" & vbNewLine & _
             "Where a.Id = b.路径id(+) And a.Id = c.路径id(+)"
    ...
    strSql = strSql & " Group By a.Id, a.分类, a.编码, a.名称, a.病例分型, a.适用病情, a.适用性别, a.适用年龄, a.说明, a.通用, a.最新版本, Decode(b.Id, Null, Null, 1), a.性质"
    ...
    Do While Not rsTmp.EOF
        ...
        If Val(rsTmp!性质 & "") = 1 Then
            objItem.Icon = img16.ListImages("Merge").Index - 1
        Else
            objItem.Icon = img16.ListImages("Path").Index - 1
        End If
        Set objItem = objRecord.AddItem("")
        If Nvl(rsTmp!是否存在分支, 0) <> 0 Then
            objItem.Icon = img16.ListImages("branch").Index - 1
        End If
        Set objItem = objRecord.AddItem("")
...
'功能：批量导出为XML文件
    ...
    strPath = BrowseForFolder(Me.Hwnd, "临床路径导出目录", GetSetting("ZLSOFT", "公共模块\" & App.ProductName, "临床路径XML目录"))
    If strPath = "" Then Exit Sub
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName, "临床路径XML目录", strPath
    
    Screen.MousePointer = 11
    For i = 0 To rptPath.Records.count - 1
        With rptPath.Records(i)
            Call zlCommFun.ShowFlash(i + 1 & "/" & rptPath.Records.count & "，正在导出临床路径""" & .Item(COL_名称).Value & """ ...", Me)
            If .Item(COL_最新版本).Value > 0 Then
                strFile = strPath & "\" & .Item(COL_分类).Value & "-" & .Item(COL_名称).Value & ".xml"
                If ExportPathToXML(.Item(COL_ID).Value, .Item(COL_最新版本).Value, strFile) Then
                    intCount = intCount + 1
                Else
    ...
    strMsg = "导出完成，共成功导出 " & intCount & " 个临床路径文件。" & _
        IIf(strFail <> "", "以下临床路径导出失败：" & vbCrLf & strFail, "")
...
'功能：批量导入XML文件
    ...
    strMsg = _
        "批量导入多个临床路径XML文件时：" & vbCrLf & vbCrLf & _
        "　1.如果系统中不存在相同分类和名称的临床路径，则导入将新增加该临床路径。" & vbCrLf & _
        "　2.如果系统中已存在相同分类和名称的临床路径，则将导入到该临床路径新的版本中。" & vbCrLf & _
        "　　如果该临床路径存在未审核的版本，则导入将覆盖该版本的内容。" & vbCrLf & vbCrLf & _
        "要继续吗？"
    ...
    cdgFile.InitDir = GetSetting("ZLSOFT", "公共模块\" & App.ProductName, "临床路径XML目录")
    cdgFile.FileName = ""
    ...
    End If
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName, "临床路径XML目录", gobjFile.GetParentFolderName(arrFile(0))
    
    Screen.MousePointer = 11
    For i = 0 To UBound(arrFile)
        Call zlCommFun.ShowFlash(i + 1 & "/" & UBound(arrFile) + 1 & "，正在导入文件""" & gobjFile.GetFileName(arrFile(i)) & """ ...", Me)
        If ImportPathFromXML(arrFile(i), , , intLimit, blnLimit) Then
            intCount = intCount + 1
        Else
            If blnLimit Then
    ...
    strMsg = "导入完成，共成功导入 " & intCount & " 个临床路径文件。" & _
        IIf(strFail <> "", "以下临床路径文件导入失败：" & vbCrLf & strFail, "") & _
