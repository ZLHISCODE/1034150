...
'功能：加载病人已生成的路径项目
    ...
    With vsPath
        ...
        "Select 分类, 序号" & vbNewLine & _
                 ...
                 "              Where a.名称 = b.分类 And b.阶段id = c.Id And b.路径记录id = [1] And b.项目id Is Null And a.路径id = c.路径id And" & vbNewLine & _
        ...
        "Select a.Id, Nvl(b.图标id, a.图标id) 图标id, a.分类, To_Char(a.日期, 'yyyy-mm-dd') 日期, a.天数, a.阶段id, Nvl(a.项目序号, b.项目序号) As 项目序号," & vbNewLine & _
                 ...
                 "Where a.路径记录id = [1] And a.项目id = b.Id(+) And a.变异原因 = c.编码(+) And a.阶段id = d.Id" & vbNewLine & _
        ...
        For lngCol = .FixedCols To .Cols - 1
            ...
            Do While Not rsTmp.EOF
                ...
                ElseIf rsTmp!执行方式 = 3 Then                          '可选项，深蓝色
                    ...
                    If Not IsNull(rsTmp!变异原因) Then '93648 中药配方 变异原因
                        .Cell(flexcpBackColor, lngRow, lngCol) = &HE0EFED    '浅灰色
                        .TextMatrix(lngRow, lngCol) = .TextMatrix(lngRow, lngCol) & vbCrLf & "变异原因：" & rsTmp!变异原因
                    End If
                End If
        ...
        Else
            If Val(optSelect(IX_ALL).Tag) <> IX_护士 Then '目前未考虑护士参与评估,故选中护士不显示评估信息
                ...
                strSql = "Select a.阶段id, a.天数, a.评估结果, a.评估说明, a.评估人,c.名称 As 变异原因, a.变异审核人, Nvl(a.时间进度, 0) 时间进度, a.跳转审核人, a.原路径id" & vbNewLine & _
...
Private Sub Get病人路径信息(ByVal lng病人Id As Long, ByVal lng主页ID As Long, ByVal lng科室ID As Long, Optional ByVal lng路径记录ID As Long)
'功能：获取病人的临床路径信息
'参数：lng路径记录ID=当一个病人有多条路径时，刷新指定路径记录ID的路径表
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lng病人Id, lng主页ID, lng科室ID, lng路径记录ID)
    If rsTmp.RecordCount > 0 Then
...
Public Function zlRefresh(ByVal lng病人Id As Long, ByVal lng主页ID As Long, ByVal lng病区ID As Long, ByVal lng科室ID As Long, _
        ...
        Optional ByVal lng路径记录ID As Long, Optional ByVal lng医护科室ID As Long, Optional ByRef objMip As Object) As Long
'参数：lng路径记录ID=当一个病人有多条路径时，刷新指定路径记录ID的路径表
...
'      objMip 消息对象
    ...
    Call Get病人路径信息(lng病人Id, lng主页ID, lng科室ID, lng路径记录ID)
...
Public Sub zlDefCommandBars(ByVal frmParent As Object, ByVal cbsMain As Object, ByVal int场合 As Integer, Optional ByVal blnInsideTools As Boolean = False)
    ...
    End If
    With objPopup.CommandBar.Controls
        Set objControl = .Add(xtpControlButton, conMenu_Tool_Option, "路径选项(&O)"): objControl.BeginGroup = True
...
'    单个执行时，执行完后不检查是否存在需要停止医嘱的项目
'    If Check医嘱项目(lng执行Id) Then
'        If InStr(GetInsidePrivs(p住院医嘱下达), ";医嘱停止;") = 0 Then
'            MsgBox "你没有医嘱停止的权限，不能执行医嘱类的路径项目。", vbInformation + vbOKOnly, gstrSysName
'            Exit Sub
'        End If
'    End If
...
LineBegin:
    ...
    If mPP.当前天数 = 0 Then '第一天
        ...
        strSql = "Select To_number(Trunc(Sysdate)-Trunc(a.开始时间)+1) as 入院天数,Nvl(b.确诊天数,0) as 确诊天数" & _
                " From 病人临床路径 a,临床路径目录 b Where a.ID = [1] And a.路径id = b.id"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mPP.病人路径ID)
        If rsTmp!确诊天数 > 0 And rsTmp!入院天数 > rsTmp!确诊天数 Then
                    ...
                    Else
                        If MsgBox("该病人在" & mPP.当前日期 & "还没有进行评估，必须先评估。" & vbCrLf & "你现在要进行评估操作吗？", vbQuestion + vbYesNo + vbDefaultButton1, gstrSysName) = vbYes Then
                            ...
                            Else
                                ...
                                '当前病人路径状态发生变化时更新Lis病人路径状态
                                If lngPPStatus <> mPP.病人路径状态 Then
                                    RaiseEvent RequestRefresh(mPP.病人路径状态) '更新主窗体路径状态
                                    If Not gobjLIS Is Nothing Then
                                       Call gobjLIS.ModifyPathState(mPati.病人ID, mPati.主页ID, mPP.病人路径状态)
                                    End If
                                End If
...
'功能：取消生成当前选择的未执行的路径项目
    ...
    If blnMust Then
        ...
        strSql = "Select b.名称 as 分类,a.编码 as ID,a.编码,a.名称,a.简码 From 变异常见原因 a,变异常见原因 b" & _
                ...
                " Order by 分类,a.编码"
        vPoint = GetCoordPos(vsPath.Hwnd, vsPath.CellLeft, vsPath.CellTop)
...
Public Function FuncAppendItem(ByVal bytUseType As Byte, Optional ByVal strItemType As String, Optional ByVal strAdviceIDs As String, Optional ByVal lng执行ID As Long, Optional ByVal datDate As Date) As Boolean
...
Public Function FuncImport() As Boolean
'功能：导入路径
    Dim rsTmp As ADODB.Recordset
    '----
    Dim strSql As String
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mPati.病人ID)
    If rsTmp.RecordCount > 0 Then
        MsgBox "该病人在[" & rsTmp!名称 & "]还有正在执行的临床路径，不允许导入新的路径。", vbInformation, gstrSysName
        Exit Function
    End If
    
    lngPPStatus = mPP.病人路径状态
    
    FuncImport = frmPathImport.ShowMe(mfrmParent, mPati, 0)
    Call zlRefresh(mPati.病人ID, mPati.主页ID, mPati.病区ID, mPati.科室ID, mPati.病人状态)
    ...
    If mPP.病人路径状态 = 1 Then
        ...
        Call frmPathImport.ShowMe(mfrmParent, mPati, 2, , , , True, mPP.病人路径ID)
    End If
    Call zlRefresh(mPati.病人ID, mPati.主页ID, mPati.病区ID, mPati.科室ID, mPati.病人状态)
    RaiseEvent RequestRefresh(mPP.病人路径状态)
    Exit Function
...
'功能：导入合并路径
    ...
    FuncImportMerge = frmPathImport.ShowMe(mfrmParent, mPati, 2, , , , False, mPP.病人路径ID)
    Call zlRefresh(mPati.病人ID, mPati.主页ID, mPati.病区ID, mPati.科室ID, mPati.病人状态)
    RaiseEvent RequestRefresh(mPP.病人路径状态)
    Exit Function
errH:
...
'功能：取消导入合并路径
    ...
    ElseIf rsTmp.RecordCount = 1 Then
        ...
        strIDs = rsTmp!ID & ""
    Else
        If Not frmPathImport.ShowMe(mfrmParent, mPati, 3, , , , , , rsTmp, True) Then Exit Sub
...
'功能：查看合并路径导入评估
    ...
    If rsTmp.RecordCount = 0 Then
        ...
        Exit Sub
    End If
    If Not frmPathImport.ShowMe(mfrmParent, mPati, 3, , , , , , rsTmp) Then Exit Sub
    Unload frmPathImport
    If rsTmp.RecordCount = 0 Then Exit Sub
    ...
    Else
        ...
        If rsTmp.RecordCount > 0 Then
            If Format(rsTmp!日期, "YYYY-MM-DD") <= strDate Then  '允许评估的
                If mPP.当前日期 = Format(rsTmp!日期, "YYYY-MM-DD") Then
                    '非补录评估,没有提前生成的路径阶段
                    bln补录 = False
...
'入参:bytSize：0-小(缺省)，1-大
    ...
    Call zlControl.VSFSetFontSize(vsPath, mlngFontSize)
    ...
    If rsTmp.RecordCount = 0 Then
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            If bytType = 1 Then
                ...
                strDate = rsTmp!日期 & ""
                If strPhase <> "-1" Then
                    strPhase = FuncRecordSetToSQL(rsTmp)
                End If
                Exit Sub
            End If
        End If
...
'   -转换后的路径表单
    ...
    zlCopyVSFlexGrid vsPathPrint(0), vsPath
...
Private Sub DefCommandPlugInPopup(ByRef objBar As Object, ByRef rsBar As ADODB.Recordset)
'功能：在医嘱卡右键弹出菜单
    Dim i As Long
    Dim objControl As CommandBarControl
