...
Public Function GetFile(strIDs As String) As ADODB.Recordset
'功能：获取路径项目对应的病历文件记录集
    Dim strSql As String

    strSql = "select A.项目ID as 路径项目ID,A.文件ID,A.原型ID,B.保留  from 临床路径病历 a,病历文件列表 b  where a.项目id in (Select Column_Value From Table(f_Num2list([1]))) and a.文件ID=b.id(+)"
...
Public Function GetNextCode(ByVal str分类 As String) As String
'功能：获取指定分类的临床路径的缺省新编码
    ...
    If str分类 = "" Then
        strSql = "Select Max(Length(编码)) As 长度 From 临床路径目录 Where 分类 Is Null"
    Else
        strSql = "Select Max(Length(编码)) As 长度 From 临床路径目录 Where 分类=[1]"
    End If
    ...
    '按最大长度编码
    If str分类 = "" Then
        strSql = "Select Max(编码) As 编码 From 临床路径目录 Where 分类 Is Null And Length(编码)=[2]"
    Else
        strSql = "Select Max(编码) As 编码 From 临床路径目录 Where 分类=[1] And Length(编码)=[2]"
    End If
...
Public Function zlBlobRead(ByVal Action As Long, ByVal KeyWord As String, Optional ByVal strFile As String) As String
'功能：将指定的LOB字段复制为临时文件
...
'返回：存放内容的文件名，失败则返回零长度""
    ...
    strSql = "Select Zl_Lob_ReadForPath([1],[2],[3]) as 片段 From Dual"
    lngCount = 0
    Do
...
Public Function zlCopyVSFlexGrid(vsgTemp As VSFlexGrid, vsgCopy As VSFlexGrid) As Boolean
...
'功能：读取病历文件的RTF数据到editor控件中
    ...
    On Error GoTo errH
    strZipFile = zlBlobRead(5, lng病历ID)
...
Public Function SaveRTFData(ByVal lng病历ID As Long, ByVal lng病人Id As Long, lng主页ID As Long, lngBaby As Long, edtEditor As Editor) As Boolean
'功能：保存病人病历格式RTF数据
'参数：
    Dim strZipFile As String, strTempFile As String, i As Long
        
    '要素内容更新
    Call ElementsUpdate(lng病历ID, lng病人Id, lng主页ID, lngBaby, edtEditor)
    ...
    '保存格式
    zlBlobSave 5, lng病历ID, strZipFile
...
Private Function ElementsUpdate(ByVal lng病历ID As Long, ByVal lng病人Id As Long, lng主页ID As Long, lngBaby As Long, edtEditor As Editor) As Boolean
    ...
    Next
     ...
     For i = 1 To ThisElements.count
        If ThisElements(i).替换域 = 1 Then
            ThisElements(i).内容文本 = GetReplaceEleValue(ThisElements(i).要素名称, lng病人Id, lng主页ID, cprPF_住院, 0, lngBaby)
...
'             =0则和以前一样，按次序取入院、门诊、中医入院、中医门诊诊断。;如果是中医科时, 优先级：中医入院、入院、中医门诊、门诊
...
'剔除自由录入诊断
    ...
    If lngType = 0 Then
        ...
        Else
            strSql = "Select 疾病id, 诊断id, 诊断描述,诊断类型,记录来源" & vbNewLine & _
                "From 病人诊断记录" & vbNewLine & _
                "Where 记录来源 In (1, 2, 3) And 诊断类型 In (1, 2, 11, 12) And 取消时间 Is Null And 病人id = [1] And 主页id = [2] And 诊断次序 = 1 And" & _
                "   Nvl(是否疑诊,0) = 0 And Not (NVl(疾病ID,0)=0 and NVl(诊断ID,0)=0)   " & vbNewLine & _
    ...
    ElseIf lngType = 2 Then
        strSql = "Select 疾病id, 诊断id, 诊断描述,诊断类型,记录来源" & vbNewLine & _
            "From 病人诊断记录" & vbNewLine & _
            "Where 记录来源 In (1, 2, 3) And 诊断类型 In ( 2,3,12,13) And 取消时间 Is Null And 病人id = [1] And 主页id = [2] And Nvl(是否疑诊,0) = 0 And Not (NVl(疾病ID,0)=0 and NVl(诊断ID,0)=0) " & vbNewLine & _
...
'功能：如果填写了出院诊断，则判断出院诊断是否和导入路径的诊断相同
    ...
    If rsTmp.RecordCount > 0 Then
        ...
        strSql = "Select 疾病ID,诊断ID From 病人诊断记录 Where 诊断次序=1 and 记录来源=3 And 病人ID=[1] And 主页ID=[2]"
...
'返回：结果记录集
    ...
    strSql = "Select 疾病ID,b.编码,记录来源,Mod(诊断类型,10) as 大类 From 病人诊断记录 a ,疾病编码目录 b" & _
        " Where 病人ID=[1] And 主页ID=[2] And 诊断类型 IN(" & IIf(int来源 = 1, "1,11", "1,2,3,11,12,13") & ") and a.疾病ID=b.ID and 疾病ID is not null and nvl(是否疑诊,0)=0" & _
...
Public Sub AddOutPathItem(ByVal strAdviceIDs As String, ByVal lngMode As Long, ByVal lng病人Id As Long, ByVal lng主页ID As Long)
'功能:将医生没有停止的长嘱，生成为路径外项目
'参数：strAdviceIds - id,id...lngMode=1:传人医嘱ID中包停止和未停止的医嘱，lngMode=2：所有回退的医嘱ID
'       lngMode：1-路径生成 ，
'                2-医生护士站，医嘱状态为：作废或停止的医嘱在回退时调用

    Dim strSql As String, strStopIds As String
    ...
    If strStopIds <> "" Then
        ...
        For i = 1 To rsTmp.RecordCount
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            arrSQL(UBound(arrSQL)) = "Zl_病人路径生成_Insert(1," & lng病人Id & "," & lng主页ID & ",Null,0," & _
        ...
        For i = LBound(arrSQL) To UBound(arrSQL)
            Call zlDatabase.ExecuteProcedure(CStr(arrSQL(i)), "AddOutPathItem")
        Next
...
Public Function FuncRecordSetToSQL(ByVal rsTemp As ADODB.Recordset, Optional ByVal strHiddenCol As String) As String
'功能：将记录集转成SQL串
'参数:
