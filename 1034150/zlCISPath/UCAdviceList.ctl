...
Public Sub ShowAdvice(ByVal bytUseType As Byte, Optional ByVal strSQL As String, Optional ByVal lng路径执行ID As Long, _
    Optional ByVal str医嘱IDs As String, Optional ByVal blnReadOnly As Boolean, Optional ByVal lng路径项目ID As Long, Optional ByVal strSelectedIDs As String)
    ...
    If bytUseType <> 0 And bytUseType <> 4 And bytUseType <> 3 Then
        If bytUseType = 1 Then
            strSQL = "Select a.*" & vbNewLine & _
                    "From 病人医嘱记录 A, 病人路径医嘱 B, 诊疗项目目录 C" & vbNewLine & _
                    ...
                    "From 病人医嘱记录 A, 病人路径医嘱 B, 病人医嘱记录 C" & vbNewLine & _
...
'功能：设置医嘱清单的字体，并调整行高和列宽
    ...
    Call zlControl.VSFSetFontSize(vsAdvice, lngFontSize, col内容)
End Sub
