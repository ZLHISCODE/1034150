...
'------------------------------------------------
    ...
    Else        '开始重建
        ...
        '判断是否满足矢冠状位重建的条件
        If Not LeagelToACRebuild(thisViewer.Images) Then
            MsgBox "图像不能进行矢冠状重建，不满足以下条件之一：" & vbCrLf & _
                  "3幅图像以上；500幅图像以下；同一序列；相同层厚；不同位置；相同像素距离。", vbInformation, gstrSysName
        ...
        Call funGetMPRImageAndShow(thisViewer.Images(1).Labels(G_INT_SYS_LABEL_MPRV), thisForm, thisViewer, _
                                    1, intMPRViewerIndex2, ToltalHeight, 1, True)
                
        '创建第二个重建结果图
        intMPRViewerIndex3 = funcReplaceViewer(3, thisForm, Nothing)
        '根据传入的控制线，对MPRViewer内的图像进行重建，并将返回重建的结果图显示到ShowViewerIndex指定的Viewer中
        Call funGetMPRImageAndShow(thisViewer.Images(1).Labels(G_INT_SYS_LABEL_MPRH), thisForm, thisViewer, _
                                    1, intMPRViewerIndex3, ToltalHeight, 2, True)
...
Private Function funAddAllImages(thisViewer As DicomViewer) As Boolean
'------------------------------------------------
...
Public Function funGetMPRImageAndShow(la As DicomLabel, thisForm As frmViewer, MPRViewer As DicomViewer, _
    ...
    blnFirst As Boolean) As Boolean
'------------------------------------------------
