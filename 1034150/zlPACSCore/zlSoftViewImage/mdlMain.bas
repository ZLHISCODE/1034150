...
'传入的参数定义，参数的连接符是三个字符“{+}”
    ...
    If App.PrevInstance Then
        ...
        If lngWinHandle = 0 Then
            Call WriteCommLog("zlSoftViewImage.Sub Main", "★★查找窗口句柄=0", "自动杀掉原来的进程，开启新进程。", ltDebug)
    ...
    If UBound(Split(strMsgs, MSG_SPLIT)) >= 3 Then
        ...
        If UBound(Split(strDBConnection, "[+]")) = 3 Then
            gstrZLHIS主机字符串 = Split(strDBConnection, "[+]")(0)
            gstr用户名 = Split(strDBConnection, "[+]")(1)
            gstr密码 = Split(strDBConnection, "[+]")(2)
            gbln是否转换密码 = Val(Split(strDBConnection, "[+]")(3)) = 1
        Else
            Exit Sub
        End If
    Else
    ...
    Exit Sub
err:
    If errHandle("exe Main", "显示观片窗口出现错误") = 1 Then Resume
End Sub
