...
'  bytFun：1-合格；2-不合格
    ...
    '更新最后操作时间
    strSQL = mdlDefine.FormatString("ZL_处方审查参数_SAVE(2, '[1]', 0, 1, Null)", mstrPCName)
    AddArray colSQL, strSQL
...
'  blnLock：True加锁；False解锁
    ...
    strSQL = mdlDefine.FormatString("Zl_处方审查记录_Lock([1], '[2]', 0, [3])", _
                    IIf(blnLock, 1, 0), _
                    mstrPCName, _
...
'返回：True成功；False失败
    ...
    Else
        ...
        '更新是否开启审方、最后操作时间
        strSQL = mdlDefine.FormatString("ZL_处方审查参数_SAVE(2, '[1]', 0, 0, Null)", mstrPCName)
