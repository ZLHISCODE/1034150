...
Private Const VER_NT_WORKSTATION = &H1                  'The operating system is Windows 10, Windows 8, Windows 7, Windows Vista, Windows XP Professional, Windows XP Home Edition,
'dwPlatformId
...
'警告：由于还没有初始化公共部件的连接对象，该函数中不能使用公共部件中的数据库访问方法
    ...
    Dim strComuterInfo As String, arrComputer As Variant, strComputerName As String
    Dim strTmp As String, arrTmp As Variant, arrTmpName As Variant, i As Integer
    Dim bln检查站点 As Boolean, lng有站点 As Long, bln空站点 As Boolean, bln多站点 As Boolean
    Dim str站点       As String, str院区编号 As String, str名称 As String, str缺省院区, str站点名称
    ...
    '检查是否有重名机器
    If CheckRepeatLogin(gstrIP, strComputerName) = True Then
    ...
    If rsTmp.EOF Then
        ...
        Set rsTmp = OpenSQLRecord(strSQL, "检查工作站-以站点为主", gstrIP)
        If rsTmp.RecordCount > 1 Then
            ...
            Set rsTmp = OpenSQLRecord(strSQL, "检查工作站-以站点为主", gstrIP, CStr(arrComputer(2)), CStr(arrComputer(3)), CStr(arrComputer(4)))
        End If
    End If
    
    bln检查站点 = True
    '如果还存在多个,则可能存在IP冲突的情况,因此不能判定需要更新相关的站点.只能当成新的站点
    ...
    If bln检查站点 Then
        strSQL = "Select b.名称, a.站点, a.缺省" & vbNewLine & _
                ...
                "       From 部门人员 b, 部门表 c" & vbNewLine & _
                "       Where b.部门id = c.Id And b.人员id = [1]) a, Zlnodelist b" & vbNewLine & _
                "Where a.站点 = b.编号(+)" & vbNewLine & _
                "Order By 站点+0"

        Set rsTmp = OpenSQLRecord(strSQL, "检查并确定所属院区", gstrUserID)
        If rsTmp Is Nothing Then
            MsgBox Err.Description & vbNewLine & "不能正常访问系统，请您联系系统管理员重新进行角色授权！", vbInformation, gstrSysName
            Exit Function
        End If
        Do While Not rsTmp.EOF
            If NVL(rsTmp!站点, "") <> "" Then
                If InStr("," & str站点 & ",", "," & rsTmp!站点 & ",") = 0 Then
                    str站点 = str站点 & "," & rsTmp!站点
                    str名称 = str名称 & "," & rsTmp!名称
                    lng有站点 = lng有站点 + 1
                End If
            Else
                bln空站点 = True
        ...
        If str站点 = "" Or (bln空站点 And lng有站点 <> 1) Then
            ...
            Else
                str站点 = "": str名称 = ""
                strSQL = "select distinct (A.站点),B.名称 from 部门表 A,zlNodeList B where A.站点=B.编号 And A.站点 is not null order by A.站点+0"
        ...
        If str站点 <> "" Then
            ...
            str名称 = Mid(str名称, 2)
            arrTmp = Split(str站点, ",")
            arrTmpName = Split(str名称, ",")
            For i = LBound(arrTmp) To UBound(arrTmp)
                If i = LBound(arrTmp) Then
                    str院区编号 = arrTmp(i)
                    str站点名称 = arrTmpName(i)
                Else
                    If str院区编号 <> arrTmp(i) Then
                        bln多站点 = True
                        Exit For
                    End If
                End If
            ...
            If glngHelperMainType = 0 Then
                If bln多站点 Then '提示用户选择当前计算机位置所在的院区。
                    str院区编号 = GetSetting("ZLSOFT", "私有模块\" & gclsLogin.DBUser & "\" & App.ProductName & "\" & App.EXEName, "当前站点选择", "")
                    Call frmSelClient.ShowEdit(str站点, str名称, str院区编号, str缺省院区)
                    str院区编号 = IIf(frmSelClient.gstr站点 = "无", "", frmSelClient.gstr站点)
                    str站点名称 = IIf(frmSelClient.gstr站点 = "无", "", frmSelClient.gstr站点名称)
                    Call SaveSetting("ZLSOFT", "私有模块\" & gclsLogin.DBUser & "\" & App.ProductName & "\" & App.EXEName, "当前站点选择", str院区编号)
                End If
            End If
        End If
    End If
    gclsLogin.NodeNo = IIf(str院区编号 <> "", str院区编号, "-")
    gclsLogin.NodeName = IIf(str站点名称 <> "", str站点名称, "-")
    If gclsLogin.DeptName = "" Then gclsLogin.DeptName = gstrDeptName
        
    '新增的工作站，还没有该工作站的数据，上传（IP、机器名、CPU、内存、硬盘、操作系统）
    If strRowID = "" Then
        ...
        Else
            int升级标志 = 1
        End If
        strSQL = "Zl_Zlclients_Set(0,Null,'" & strComputerName & "','" & gstrIP & "','" & arrComputer(2) & "','" & arrComputer(3) & _
                    "','" & arrComputer(4) & "','" & arrComputer(5) & "','" & gclsLogin.DeptName & "','" & gclsLogin.UsageOccasion & "',Null," & int服务器编号 & "," & int升级标志 & _
                    ",0,'" & str院区编号 & "',0,Null,Null," & int启用视频源 & ")"
    ...
    '更新工作上的已有信息
    Else
        strSQL = "Zl_Zlclients_Set(1,'" & strRowID & "','" & strComputerName & "','" & gstrIP & "','" & arrComputer(2) & "','" & arrComputer(3) & _
                    "','" & arrComputer(4) & "','" & arrComputer(5) & "','" & gclsLogin.DeptName & "','" & gclsLogin.UsageOccasion & "',Null,Null,Null," & int连接数 & ",'" & str院区编号 & "',0,Null,Null," & int启用视频源 & ")"
        '需要更新相关的站点信息
        ...
        '连接数检查限制
        If int连接数 > 0 Then
            strSQL = "Select SID From gv$Session Where Upper(PROGRAM) Like 'ZL%.EXE' And Status<>'KILLED' And MACHINE=(Select Max(MACHINE) From v$Session Where AUDSID=[1])"
            Set rsTmp = OpenSQLRecord(strSQL, "检查连接数量", gstrSessionID)
    ...
    '保存clentSession信息
    strSQL = "Zl_Zlclientsession_Save('" & strComputerName & "','" & gstrSessionID & "',Null, '" & gstrIP & "','" & gstrDBUser & "','" & gstrUserName & "'," & gstrDeptID & ",'',1,'','')"
    ExecuteProcedure strSQL, "保存ClientSession信息"
    
    On Error GoTo Errhand
AutoUpGrude:      '执升升级程序
    If blnUpdate Then
        If glngHelperMainType > 0 Then
            blnAllow = True '升级助手控制时，不升级
        Else
...
'返回：是否成功
    ...
    On Error Resume Next
    If strJobName = "OfficialUpgrade" And gstrExeFile = "" Then
        gstrExeFile = GetCurExePath
    End If
    
    If gstrExeFile <> "" And strJobName = "OfficialUpgrade" Then
    ...
    '支持加密命令行
    If UCase(strJobName) = UCase("CheckUpgrade") Then
        GoTo makCheck
    ElseIf UCase(GetFileDesInfo(strUPFile, "ProductName")) = "ZLHISINSTALLUPDATE" Then
        If IsDesinMode Then
            '组装命令行，以及生成命令行校验位
            strCommand = "ZLUPDATE:" & Cipher("USER=ZLHIS PASS=HIS SERVER=TESTBASE MODE=" & Decode(strJobName, "Repair", 0, "OfficialUpgrade", 1, "PreUpgrade", 2, "CheckUpgrade", 3, 1) & " EXEPATH=" & strExeName & " WATI=" & lngWait)
        Else
            '组装命令行，以及生成命令行校验位
            strCommand = "ZLUPDATE:" & Cipher("USER=" & gclsLogin.InputUser & " PASS=" & strPass & " SERVER=" & gclsLogin.ServerName & " MODE=" & Decode(strJobName, "Repair", 0, "OfficialUpgrade", 1, "PreUpgrade", 2, "CheckUpgrade", 3, 1) & " EXEPATH=" & strExeName & " WATI=" & lngWait)
        End If
    Else
makCheck:
        If IsDesinMode Then
    ...
    If UCase(strJobName) = UCase("CheckUpgrade") Then
        '“部件检查”不在升级外壳处理（升级外壳尽量不修改，避免再次加入杀毒软件的白名单）
        Call CheckComponent(strCommand)
        StartHisCrust = True
    Else
...
Private Function AnalyseConfigure() As String
    ...
    If Err.Number <> 0 Then
        strCPU = "(unknown)"
    End If
    If Len(strCPU) > 30 Then strCPU = Mid(strCPU, 1, 30)
    On Error GoTo 0
    ' 获得剩余内存
    '先判断系统是否为win2000及以下
        ...
        Select Case .dwMajorVersion
            ...
            Case 6
                ...
                ElseIf .dwMinorVersion = 1 Then
                    ...
                    End If
                Else
                    'win8版本是6.2，win8.1版本可能是6.2或者6.3,；而win10版本也是6.3，故直接从注册表中获取版本信息
                    Call GetRegValue("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion", "ProductName", strOS)
                End If
        End Select
    End With
...
Private Function CheckRepeatLogin(ByVal strIpAddress As String, ByVal strComputerName As String) As Boolean
    '检查是否有重复登录
...
'    strProgram = GetCallEXE
    strSQL = "Select A.UserName, A.Program, C.IP, B.名称" & vbNewLine & _
            "From gv$Session A, 部门表 B, Zlclientsession C" & vbNewLine & _
            "Where A.Terminal = C.工作站 And B.ID=C.当前部门ID " & vbNewLine & _
            "      And A.Terminal = [3]" & vbNewLine & _
            "      And C.IP <> [2] And C.状态=1"

    Set rsTemp = OpenSQLRecord(strSQL, "检查重复工作站", strProgram, strIpAddress, strComputerName, gstrSessionID)
    If rsTemp.RecordCount = 0 Then '可以登录
        CheckRepeatLogin = False
        Exit Function
    Else
        MsgBox "局域网中可能存在相同名称的计算机登录," & vbCrLf & "对方IP是:[" & NVL(rsTemp!IP) & "]" & vbCrLf & IIf(rsTemp!名称 & "" = "", "", "对方部门是:[" & rsTemp!名称 & "]") & vbCrLf & _
                "如果检查发现不存在相同名称的计算机登录，请检查后台作业(客户端会话状态更新)是否自动执行；如未自动执行，则需后台将Zlclientsession中对应的该计算机的数据的状态字段改为0。", vbInformation, gstrSysName
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    strSQL = "Select Distinct A.站点, B.名称" & vbNewLine & _
            ...
            "       From 检验仪器记录 A, 检验仪器人员 B" & vbNewLine & _
            "       Where A.Id = B.仪器id And A.站点 Is Not Null And B.人员id = [1]) A, Zlnodelist B" & vbNewLine & _
            "Where A.站点 = B.编号" & vbNewLine & _
            "Order By A.站点"
    Set rsTmp = OpenSQLRecord(strSQL, "站点查询", gstrUserID)
    Do While Not rsTmp.EOF
        str站点 = str站点 & "," & rsTmp!站点
...
Private Sub UpdateEmrInterface()
    ...
    If Err.Number = 0 Then
        ...
        End If
        Set gclsLogin.EMR = objEMR
    Else
        Set gclsLogin.EMR = Nothing
        Err.Clear
        Exit Sub
    End If
    On Error GoTo 0
...
'返回：是否获取成功（当存在只存在2500系统，则从配置文件获取，若不存在100与2500系统，则返回FALSE
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & strSQL, vbExclamation, "GetEMRLoginUser"
End Function
...
'      blnBrwCall=是否导航台调用,导航台调用升级时检查预升级时点
    ...
    '非授权的程序不自动升级
    strTmpGet = gobjRegister.GetPassword(App.hInstance)
    ...
    If StartHisCrust(strUpdateExePath, strJobName, lngWait, strTmpGet) Then
        If strJobName <> "PreUpgrade" And strJobName <> "CheckUpgrade" Then
            Exit Function
        End If
    End If
    UpdateZLHIS = True
...
'      blnMustNowUpdate=是否现在必须升级
    ...
    Else
        ...
        On Error Resume Next
        strSQL = "Select 预升时点,Nvl(预升完成, 0) 预升完成, Nvl(升级标志, 0) 升级标志, Nvl(收集标志, 0) 收集标志,Nvl(是否立即升级,0) 是否立即升级 From Zlclients Where 工作站 = [1]"
        Set rsTmp = OpenSQLRecord(strSQL, "检查当前任务", strComputerName)
        If Err.Number <> 0 Then
            Err.Clear
            strSQL = "Select 预升时点,Nvl(预升完成, 0) 预升完成, Nvl(升级标志, 0) 升级标志, Nvl(收集标志, 0) 收集标志,0 是否立即升级 From Zlclients Where 工作站 = [1]"
            Set rsTmp = OpenSQLRecord(strSQL, "检查当前任务", strComputerName)
        End If
        On Error GoTo errH
        If Not rsTmp Is Nothing Then
...
Public Function GetFileDesInfo(ByVal strFileName As String, ByVal strEntryName As String) As String
    Dim i               As Long
...
Private Function CheckComponent(ByVal strCommand As String, Optional ByRef strErrInfo As String) As Boolean
'功能：部件检查
'说明：尽量不修改升级外壳程序，通过zlHisCrustCom实现“部件、版本”信息写入zlClientUpdateLog表
    
    Dim objHISCrushDLL As Object
    Dim objFSO As New FileSystemObject
    ...
    If IsDesinMode Then
        strFile = "C:\APPSOFT\ZLHISCRUSTCOM.DLL"
    Else
        strFile = objFSO.GetParentFolderName(App.Path) & "\ZLHISCRUSTCOM.DLL"
    End If
    Set objHISCrushDLL = CreateObject("zlHisCrustCom.clsHisCrsutCom")
    Call objHISCrushDLL.InitHisCrustCom(objHISCrushDLL _
        , IIf(IsDesinMode, "C:\APPSOFT\ZLHISCRUSTCOM.DLL", objFSO.GetParentFolderName(App.Path)) _
        , strCommand _
        , strFile)
    Call objHISCrushDLL.UpdateFile
    CheckComponent = True
    Exit Function
