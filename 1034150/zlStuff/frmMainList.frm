...
Public Sub SetMenu()
    ...
    If TabShow.Tab = 0 Then
        If mlngMode = 1716 Then
            mint移库处理流程 = IIf(Val(zldatabase.GetPara("移库流程", glngSys, mlngMode, "0")) = 1, 1, 0)
            ...
            Else
                ...
                mnuEditPrepare.Visible = IsHavePrivs(mstrPrivs, "发送")
            ...
            Else
                ...
                tlbTool.Buttons("Strike").Visible = IsHavePrivs(mstrPrivs, "冲销")
...
'    End If
    ...
    mstrPrintRange = "查询范围:" & Format(zldatabase.Currentdate, "yyyy年MM月dd日") & "至" & Format(zldatabase.Currentdate, "yyyy年MM月dd日")
...
Private Sub imgLeft_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(mshDetail.hwnd)
...
Private Function CheckSelfMakeStock(ByVal str单据号 As String) As Boolean
    ...
    With rsTemp
        ...
        Do While Not .EOF
            If Check可用数量(Val(NVL(rsTemp!库房id)), Val(NVL(rsTemp!药品ID)), _
                Val(NVL(rsTemp!批次, 0)), Val(NVL(rsTemp!实际数量)), int库存检查) = False Then Exit Function
            .MoveNext
        Loop
    End With
    ...
    With mshList
        ...
        Select Case mlngMode
            Case 1712
                ...
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption & "[判断是否是退库单]", .TextMatrix(.Row, 0), Val(.TextMatrix(.Row, GetCol(mshList, "记录状态"))))
                bln退库单 = (rsTemp!标志 = 1)
                If CheckPrint(.TextMatrix(.Row, 0), 15, cboStock.ItemData(cboStock.ListIndex)) = True Then
                ...
                End If
            Case 1713
                If CheckPrint(.TextMatrix(.Row, 0), 16, cboStock.ItemData(cboStock.ListIndex)) = True Then
                ...
                End If
            Case 1714
                If CheckPrint(.TextMatrix(.Row, 0), 17, cboStock.ItemData(cboStock.ListIndex)) = True Then
            ...
            Case 1716
                ...
                End If
            Case 1717
                If CheckPrint(.TextMatrix(.Row, 0), 20, cboStock.ItemData(cboStock.ListIndex)) = True Then
                ...
                End If
            Case 1718
                If CheckPrint(.TextMatrix(.Row, 0), 21, cboStock.ItemData(cboStock.ListIndex)) = True Then
                ...
                End If
            Case 1719
                If CheckPrint(.TextMatrix(.Row, 0), 22, cboStock.ItemData(cboStock.ListIndex)) = True Then
...
Private Sub mnuViewColDefine_Click()
    ...
    Select Case mlngMode
    Case 1712           '卫材外购入库管理
        strColumn_All = "卫材,0|规格,1|产地,1|批号,0|生产日期,1|商品条码,1|灭菌日期,1|灭菌失效期,1|效期,0|注册证号,1|单位,1|数量,0|指导批发价,1|采购价,0|扣率,1|" & _
...
'    If mshList.Row >= 1 And LTrim(mshList.TextMatrix(mshList.Row, 0)) <> "" Then
        ...
        Select Case mlngMode
            Case 1712       '卫材外购入库
                ...
                If mintUnit <> 0 Then
                    str列名 = "序号,卫材信息,规格,产地,批准文号,批号,生产日期,失效期,注册证号,数量,单位,结算价,结算金额,扣率,售价,售价金额,差价,零售价,零售单位,零售金额,零售差价,随货单号,发票号,发票代码,发票日期,付款序号,发票金额,收发id,商品条码,内部条码"
                    str零售价 = "" & _
                ...
                Else
                    str列名 = "序号,卫材信息,规格,产地,批准文号,批号,生产日期,失效期,注册证号,数量,单位,结算价,结算金额,扣率,售价,售价金额,差价,随货单号,发票号,发票代码,付款序号,发票金额,收发id,商品条码,内部条码"
                    str零售价 = "" & _
                    "                   ltrim(rtrim(to_char(A.零售价*" & str包装系数 & "," & mOraFMT.FM_零售价 & "))) as 售价 , " & _
                    "                   ltrim(rtrim(to_char(A.零售金额," & mOraFMT.FM_金额 & ")))  as 售价金额, " & _
                ...
                gstrSQL = "" & _
                    "   SELECT " & str列名 & _
                    "   From (  SELECT distinct a.序号, '[' || D.编码 || ']' || D.名称 AS 卫材信息,D.规格,d.编码,zlSpellCode(d.名称) 名称, A.产地,A.批准文号, A.批号, to_char(A.生产日期,'yyyy-mm-dd') as 生产日期, to_char(A.效期,'yyyy-mm-dd') as 失效期 ,a.注册证号," & _
                                        strUnitQuantity & _
                    ...
                    "           Where  A.药品id = B.材料id and A.药品id=D.id AND A.Id = C.收发id (+)  " & strTemp & _
                    "                   AND A.记录状态 =[3] " & _
                    "                   AND A.单据 = [1] " & _
                    "                   AND A.No =[2]" & _
                    "       ) " & _
...
Private Sub ListSort()
    ...
    With mshList
        If .Rows > 1 Then
            ...
            End Select
            mintPreCol = intCol
            .Row = FindRow(mshList, intTemp, intCol)
