...
Begin VB.Form frmSystemParaSet 
   ...
   Begin TabDlg.SSTab stbPage 
      ...
      Tab(0).Control(5)=   "cmb出库算法"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).Control(6)=   "txt卫材条码前缀"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "vsfParameter"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).Control(8)=   "optUnit(1)"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).Control(9)=   "optUnit(0)"
      Tab(0).Control(9).Enabled=   0   'False
      Tab(0).Control(10)=   "cbo产地"
      Tab(0).Control(10).Enabled=   0   'False
      Tab(0).Control(11)=   "fra(10)"
      Tab(0).Control(11).Enabled=   0   'False
      Tab(0).ControlCount=   12
      TabCaption(1)   =   "卫材流向控制(&1)"
      TabPicture(1)   =   "frmSystemParaSet.frx":0A14
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "vsf流向"
      Tab(1).Control(1)=   "Image1(0)"
      Tab(1).Control(2)=   "lbl流向(23)"
      ...
      Begin VB.Frame fra 
         ...
         End
      End
      Begin VB.ComboBox cbo产地 
         Height          =   300
         ItemData        =   "frmSystemParaSet.frx":0A68
         Left            =   5760
         List            =   "frmSystemParaSet.frx":0A6A
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsf流向 
         ...
         FormatString    =   $"frmSystemParaSet.frx":0A6C
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfParameter 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmSystemParaSet.frx":0B70
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VB.ComboBox cmb出库算法 
         Height          =   300
         ItemData        =   "frmSystemParaSet.frx":103E
         Left            =   1800
         List            =   "frmSystemParaSet.frx":1040
         Style           =   2  'Dropdown List
         TabIndex        =   6
         Top             =   5797
         Width           =   2145
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsf库房检查 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmSystemParaSet.frx":1042
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VB.Label lbl虚拟库房 
         Caption         =   $"frmSystemParaSet.frx":1138
         Height          =   540
         Left            =   -74280
         TabIndex        =   28
         Top             =   450
      ...
      Begin VB.Image Image1 
         ...
         Index           =   3
         Left            =   -74880
         Picture         =   "frmSystemParaSet.frx":11FB
         Top             =   480
         Width           =   480
      End
      ...
      Begin VB.Image Image1 
         ...
         Index           =   1
         Left            =   -74880
         Picture         =   "frmSystemParaSet.frx":1AC5
         Top             =   450
         Width           =   480
      End
      Begin VB.Image Image1 
         ...
         Index           =   0
         Left            =   -74880
         Picture         =   "frmSystemParaSet.frx":2146
         Stretch         =   -1  'True
         Top             =   480
         Width           =   435
      End
...
Const mstrParSplit1 As String = "^"  '批量保存参数的模块、参数号、参数值分隔符
Const mstrParSplit2 As String = "#"  '批量保存参数组分隔符
...
Private Sub txt_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    ElseIf KeyAscii = Asc(mstrParSplit1) Or KeyAscii = Asc(mstrParSplit2) Then
        KeyAscii = 0
    Else
        Select Case Index
        Case txt_结存时间模式
...
Private Function Save数据() As Boolean
    ...
    With vsfParameter
        ...
        Call zldatabase.SetPara(258, IIf(.TextMatrix(mPara.mint按批次领用卫生材料, 1) = "1", 1, 0), glngSys, 0)
        Call zldatabase.SetPara(280, IIf(.TextMatrix(mPara.mint按批次移库卫生材料, 1) = "1", 1, 0), glngSys, 0)
    End With
    ...
    Call zldatabase.SetPara(268, IIf(cbo产地.ListIndex = -1, 0, cbo产地.ListIndex), glngSys, 0)

    Call zldatabase.SetPara(281, IIf(opt结存方式(0).Value, -1, IIf(opt结存时间模式(0).Value, 0, txt(txt_结存时间模式))), glngSys, 0)

    If IsHavePrivs(mstrPrivs, "设置条码前缀") = True Then
    ...
    '保存虚拟库房对照
    If IsHavePrivs(mstrPrivs, "设置虚拟库房对照") = True Then
        strTemp = ""
        With vsf对照
            For i = 1 To .Rows - 1
...
'    Me.lvwCheckMed.Sorted = True
    ...
    If IsHavePrivs(mstrPrivs, "设置虚拟库房对照") = False Then
        stbPage.TabVisible(3) = False
    End If
    
    If IsHavePrivs(mstrPrivs, "设置条码前缀") = False Then
...
Private Function LoadPara()
    ...
    mblnChkClick = True
'    chkCheck(5).Tag = ""
    Set rsTemp = ReturnParaData(glngSys, "83,88,95,120,123,127,132,136,140,156,159,258,268,280,281")
    With rsTemp
        Do While Not .EOF
            Select Case NVL(!参数号, 0)
                Case 88
                    optUnit(0).Value = IIf(NVL(!参数值, 0) = 0, True, False)
                    optUnit(1).Value = IIf(NVL(!参数值, 0) = 0, False, True)
                Case 156
                    '卫材出库算法
                    If NVL(!参数值, 0) = 1 Then
                        cmb出库算法.ListIndex = 1
            ...
            With vsfParameter
                For i = 1 To .Rows - 1
                    If NVL(rsTemp!参数号, 0) = .TextMatrix(i, 0) And rsTemp!参数值 = 1 Then
                        .TextMatrix(i, 1) = 1
                    End If
                    If NVL(rsTemp!参数号, 0) = "95" Then
                        mintOldChkValue = Val(rsTemp!参数值)
                    End If
                    If NVL(rsTemp!参数号, 0) = "127" Then
            ...
            Select Case .Row
                Case mPara.mint卫材填单下可用库存
                    If .TextMatrix(.Row, 1) = "" Then
                        ...
                        .Cell(flexcpBackColor, mPara.mint按批次领用卫生材料, 0, mPara.mint按批次领用卫生材料, .Cols - 1) = mlngColor
                        .TextMatrix(mPara.mint按批次领用卫生材料, 1) = "1"
                        
                        .Cell(flexcpBackColor, mPara.mint按批次移库卫生材料, 0, mPara.mint按批次移库卫生材料, .Cols - 1) = mlngColor
                        .TextMatrix(mPara.mint按批次移库卫生材料, 1) = "1"
                    Else
                        .TextMatrix(.Row, 1) = ""
                        .Cell(flexcpBackColor, mPara.mint按批次移库卫生材料, 0, mPara.mint按批次移库卫生材料, .Cols - 1) = MCON_LNGCOLOR
                        .Cell(flexcpBackColor, mPara.mint按批次申领卫生材料, 0, mPara.mint按批次申领卫生材料, .Cols - 1) = MCON_LNGCOLOR
                        .Cell(flexcpBackColor, mPara.mint按批次领用卫生材料, 0, mPara.mint按批次领用卫生材料, .Cols - 1) = MCON_LNGCOLOR
                    End If
                Case Else
                    If .TextMatrix(.Row, 1) = "" Then
                        .TextMatrix(.Row, 1) = "1"
