...
Public Function ShowMe(ByVal FrmParent As Object, ByVal 系统号 As Long, ByVal 票据号 As String, _
                       ByVal 记录状态 As Long, ByVal 单位系数 As Integer, ByVal 单据类型 As Integer, ByVal 单据名称 As String, ByVal 单据号 As String, Optional ByVal lng库房ID As Long)
    lng系统号 = 系统号
    str票据号 = 票据号
    lng记录状态 = 记录状态
    int单位系数 = 单位系数
...
Private Sub BillPrint(ByVal intPrintMode As Integer)
    ...
    If intPrintMode = 2 Then
        Select Case int单据类型
            Case 1712 '外购入库
                If CheckPrint(str单据号, 15, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1713 '自制入库
                If CheckPrint(str单据号, 16, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1714   '其他入库
                If CheckPrint(str单据号, 17, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1716   '移库
                If CheckPrint(str单据号, 19, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1717   '领用
                If CheckPrint(str单据号, 20, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1718   '其他出库
                If CheckPrint(str单据号, 21, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1719   '盘点表
                If CheckPrint(str单据号, 22, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1722   '申领单
                If CheckPrint(str单据号, 6, mlng库房id) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case Else
