...
Private Sub SetAllNotReturn()
    Dim n As Long
    
    With vsGrid
        For n = 1 To .Rows - 1
            If .IsSubtotal(n) = False And .TextMatrix(.Row, .ColIndex("单据号")) <> "" Then
                .TextMatrix(n, .ColIndex("退料数量")) = 0
                Call SetExecuteStaut(n, 1)
            End If
        Next
    End With
End Sub

Private Sub SetAllReturn()
    Dim n As Long
    
    With vsGrid
        For n = 1 To .Rows - 1
            If .IsSubtotal(n) = False And .TextMatrix(.Row, .ColIndex("单据号")) <> "" Then
                .TextMatrix(n, .ColIndex("退料数量")) = .TextMatrix(n, .ColIndex("准退数量"))
                Call SetExecuteStaut(n, 1)
            End If
        Next
    End With
...
Private Sub SetComandBars()
    ...
    With objMenu.CommandBar.Controls
        Set cbrControl = .Add(xtpControlButton, conMenu_Status_AllReturn, "全部退料(&R)")
...
Private Function WhiteDataToRecord(ByVal rsSource As ADODB.Recordset) As Boolean
    ...
    With rsSource
        Do While Not .EOF
            ...
            mrsBackStuff!金额 = !金额
            mrsBackStuff!单量 = IIf(IsNull(!单量), "", FormatEx(!单量, 5) & NVL(!计算单位))
...
Public Sub zlPrintBill(ByVal bln发料单 As Boolean, Optional str发料日期 As String = "", Optional int格式 As Integer = 1, Optional strPrivs As String = "", Optional blnPrintAsk As Boolean = False)
    ...
    If IsHavePrivs(mstrPrivs, "退料通知单") Then
        ...
        If intMsg = 0 Then
            '提示打印
            If MsgBox("你需要打印退料清单吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) <> vbYes Then Exit Sub
...
Private Sub PrintPayBill(Optional str发料日期 As String = "", Optional int格式 As Integer)
    ...
    If bln已发料清单 = False Then
        ShowMsgBox "你不具备打印已发料清单的权限,请与系统管理员联系!"
        Exit Sub
    End If
    If intMsg = 0 Then
        '提示打印
...
End Sub
 ...
 Private Sub vsGrid_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    With vsGrid
        Select Case Col
        Case .ColIndex("状态")
            If IsHavePrivs(mstrPrivs, "卫生材料退料") = False Then Cancel = True: Exit Sub
            '历史数据是不能更改的
...
Private Sub vsGrid_DblClick()
    Dim str状态 As String
    If IsHavePrivs(mstrPrivs, "卫生材料退料") = False Then Exit Sub
...
Private Sub vsGrid_MouseUp(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    If Button = 2 Then
        ...
        If vsGrid.IsSubtotal(vsGrid.Row) = True Then Exit Sub
        
        '退药状态弹出菜单
        If vsGrid.TextMatrix(vsGrid.Row, vsGrid.ColIndex("单据号")) = "" Then Exit Sub
