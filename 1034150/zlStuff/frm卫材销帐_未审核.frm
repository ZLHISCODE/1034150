...
Begin VB.Form frm卫材销帐_未审核 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsBatch 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frm卫材销帐_未审核.frx":021C
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Function RefreshData() As Boolean
    ...
    gstrSQL = "" & _
        "   Select 单据, NO, 药品ID as 材料ID, 申请时间, 标识号, 姓名, 床号, 单位, 换算系数, Sum(数量) As 销帐数量,零售价 " & _
        "   From (  Select Distinct C.单据, C.NO, C.药品ID, A.申请时间, B.标识号,B.标准单价 as 零售价, nvl(F.姓名,B.姓名) 姓名, B.床号, " & strFields & " " & _
        ...
        "           Group By 单据, NO, 药品ID, 申请时间, 标识号, 姓名, 床号, 单位, 换算系数,零售价 "
...
Private Function SaveData() As Boolean
    ...
    If Not mobjPlugIn Is Nothing And bln是否有退料 Then
        mobjPlugIn.StuffReturnByID Val(mArrFilter("发料部门id")), strReturnInfo, CDate(strCurDate), strReserve
    End If
