...
'    On Error Resume Next
    ...
    '构造材料查询条件(材料特性 B)
    strPhysic = " And (c.撤档时间>[2] Or c.撤档时间 is NULL)"
    strPhysic = strPhysic & IIf(str库房货位 = "", "And Substr(E.盘点属性," & int盘点方式 & ",1)='1'", " and (Instr([6], ',' || e.库房货位 || ',') > 0 ) And Substr(E.盘点属性," & int盘点方式 & ",1)='1'")
    
    Call zlCommFun.ShowFlash("正在计算卫生材料库存数据,请稍候 ...", Me)

    DoEvents    ': Me.Refresh
    Set rsData = GetDateStock(str盘点时间, lng库房ID, strPhysic, IIf(int盘无库存材料 = 0, False, True), , str分类ID, , bln盘点零数量且有金额, str库房货位)
    
    Call zlCommFun.StopFlash   ': Me.Refresh
    ...
    Call zlCommFun.ShowFlash("正在装入卫生材料数据,请稍候 ...", Me)
    DoEvents: 'Me.Refresh
...
'
            dbl成本价 = Val(NVL(rsData!最后进价)) ' rsTemp!成本价
            
            '时价材料重算售价
            If rsData!是否变价 = 1 Then
                .TextMatrix(i, mBillCol.C_售价) = Format(Get零售价(Val(NVL(rsData!材料ID)), Val(txtStock.Tag), Val(NVL(rsData!批次)), rsData!比例系数), mFMT.FM_零售价)
            Else
            ...
            Else
                .TextMatrix(i, mBillCol.C_成本价) = Format(Val(NVL(rsData!成本价)), mFMT.FM_成本价)
            End If
            .TextMatrix(i, mBillCol.C_效期) = IIf(IsNull(rsData!效期), "", Format(rsData!效期, "yyyy-MM-dd"))
            .TextMatrix(i, mBillCol.C_帐面数量) = Format(Val(NVL(rsData!帐面数量)), mFMT.FM_数量)
...
'    On Error Resume Next
    On Error GoTo ErrHandle
    '构造材料查询条件(材料特性 B)
    strPhysic = " And (c.撤档时间>[2] Or c.撤档时间 is NULL)" & _
                " "          ' And E.库房ID+0=" & lng库房id
    ...
    Else
        Set rsData = GetDateStock(str盘点时间, lng库房ID, strPhysic, False, IIf(mint编辑状态 = 5, True, False))
    End If
    Call zlCommFun.StopFlash
    ...
    Call zlCommFun.ShowFlash("正在装入材料数据,请稍候 ...", Me)
    DoEvents
    mshBill.Redraw = False
    ...
    With mshBill
        Do While Not rsData.EOF
            ...
            dbl成本价 = Val(NVL(rsData!最后进价))
            
            '如果是时价材料，重算其售价
            If rsData!是否变价 = 1 Then
                sinPrice = Get零售价(Val(NVL(rsData!材料ID)), lng库房ID, Val(NVL(rsData!批次)), rsData!比例系数)
            ...
            If (rsData!批次 = -1) Then
                ...
                gstrSQL = "" & _
                    "   Select /*+rule*/ Nvl(A.批次,0) 批次,A.批号,A.效期,A.产地" & strUnitQuantity & _
                    ...
                    "   Group By Nvl(批次,0),批号,效期,产地,序号"
                
                Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption & "--读取记录单批次数据", lngPhysic, str盘点时间, str盘点单NO串) '查询同一药品在记录单中所有数据
...
Private Function GetDateStock(str盘存时间 As String, lng库房ID As Long, str材料条件 As String, _
    ...
    gstrSQL = "" & _
        ...
        "           and a.库房id = e.库房id And a.药品id = e.材料id " & _
        "           AND a.性质=1 " & _
    ...
    gstrSQL = gstrSQL & _
        ...
        "           and a.库房id + 0 = e.库房id And a.药品id + 0 = e.材料id " & _
        "           AND a.库房id + 0 =[1] " & _
    ...
    If bln汇总 Then
        gstrSQL = gstrSQL & _
            ...
            "           AND a.库房id + 0 = e.库房id And a.药品id + 0 = e.材料id AND A.库房ID + 0 =[1] " & _
...
Private Function Get汇总记录单(ByVal lng库房ID As Long, ByVal str盘点时间 As String) As ADODB.Recordset
    ...
    str盘点单 = "" & _
            "   SELECT A.库房ID,B.材料ID,NVL(A.批次, 0) AS 批次,0 AS 实际数量,SUM(A.扣率) 盘点数量," & _
            "           0 AS 实际金额,0 AS 实际差价,0 AS 可用数量,0 as 成本价,A.批号,A.产地,A.批准文号,A.效期" & _
            "   FROM 药品收发记录 A,材料特性 b " & _
    ...
    gstrSQL = "" & _
        ...
        IIf(mbln只统计盘点单卫材, _
            ...
            "                where T1.NO in (select * from Table(Cast(f_Str2list([3]) as zlTools.t_Strlist))) and T1.单据=23 and a.材料ID +0 =T1.药品id ) ", _
            "") & _
    ...
    Select Case mintUnit
        Case 0
            strUnitQuantity = "c.计算单位 AS 单位, nvl(a.帐面数量,0) AS 帐面数量, nvl(a.盘点数量,0) AS 盘点数量,nvl(a.可用数量,0) AS 可用数量, '1' as 比例系数," & _
             " f.售价,decode(nvl(a.成本价,0),0,B.成本价,A.成本价) as 成本价,b.成本价 as 最后进价,"
        Case Else
            strUnitQuantity = "b.包装单位 AS 单位, (nvl(a.帐面数量,0) / b.换算系数) AS 帐面数量, (nvl(a.盘点数量,0) / b.换算系数) AS 盘点数量,(nvl(a.可用数量,0) / b.换算系数) AS 可用数量,b.换算系数 as 比例系数," & _
             "f.售价*b.换算系数 as 售价,decode(nvl(a.成本价,0),0,B.成本价,A.成本价)*b.换算系数 as 成本价,b.成本价*b.换算系数 as 最后进价, "
    End Select
    ...
    gstrSQL = "" & _
        ...
        "        (SELECT 收费细目id, 现价 as 售价 From 收费价目  WHERE ((SYSDATE BETWEEN 执行日期 AND 终止日期) OR (SYSDATE >= 执行日期 AND 终止日期 IS NULL))) f " & _
        ...
        Case 2, 3, 4
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    .TextMatrix(lngRow, mBillCol.C_成本价) = Format(NVL(rsTemp!成本价, 0), mFMT.FM_成本价)
                    ...
                    .TextMatrix(lngRow, mBillCol.C_盘点成本金额) = Format((NVL(rsTemp!库存金额, 0) + dbl金额差) - (NVL(rsTemp!库存差价, 0) + dbl差价差), mFMT.FM_金额)
...
Private Sub mshbill_CommandClick()
    ...
    If mshBill.Col = mBillCol.C_材料 Then
        Set RecReturn = Frm材料选择器.ShowMe(Me, 2, txtStock.Tag, txtStock.Tag, txtStock.Tag, False, True, True, True, , , , , txtCheckDate.Caption, , , mbln盘无存储库房材料, mstrPrivs)
...
'        .ColData(mBillCol.C_产地) = IIf(lng批次 = -1, 1, 5)
'        .ColData(mBillCol.c_批号) = IIf(lng批次 = -1, 4, 5)
'        .ColData(mBillCol.C_效期) = IIf(lng批次 = -1, 2, 5)
'
'        If mint编辑状态 = 2 Then
'            lng新批次 = Val(.TextMatrix(.Row, mBillCol.c_新批次))
'            .ColData(mBillCol.C_产地) = IIf(lng新批次 = 1 Or lng批次 = -1, 1, 5)
'            .ColData(mBillCol.c_批号) = IIf(lng新批次 = 1 Or lng批次 = -1, 4, 5)
        ...
        Select Case .Col
            ...
            Case mBillCol.C_材料
                If strKey <> "" Then
                    ...
                    Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, txtStock.Tag, txtStock.Tag, txtStock.Tag, strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, False, True, True, True, , , , Me.txtCheckDate.Caption, , , mbln盘无存储库房材料, mstrPrivs)
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For lngLop = 1 To .Rows - 1
                If Trim(.TextMatrix(lngLop, mBillCol.C_材料)) <> "" Then
                    ...
                    If Val(.TextMatrix(lngLop, mBillCol.C_批次)) = -1 Or Val(.TextMatrix(lngLop, mBillCol.c_新批次)) = 1 Then
            ...
            '检查分批卫材新增批次的产地，批号是否重复
            For intLop = 1 To .Rows - 1
                If Val(.TextMatrix(intLop, mBillCol.C_批次)) = -1 Or Val(.TextMatrix(intLop, mBillCol.c_新批次)) = 1 Then
                    lng卫材ID = Val(.TextMatrix(intLop, 0))
                    str产地 = .TextMatrix(intLop, mBillCol.C_产地)
                    str批号 = .TextMatrix(intLop, mBillCol.c_批号)
                    dbl成本价 = Val(.TextMatrix(intLop, mBillCol.C_成本价))
                    lng批次 = Val(.TextMatrix(intLop, mBillCol.C_批次))
                    
                    For intRow = 1 To .Rows - 1
                        If intLop <> intRow And _
                            lng卫材ID = Val(.TextMatrix(intRow, 0)) And _
                            str产地 = .TextMatrix(intRow, mBillCol.C_产地) And _
                            str批号 = .TextMatrix(intRow, mBillCol.c_批号) And _
                            dbl成本价 = Val(.TextMatrix(intRow, mBillCol.C_成本价)) And _
                            (Val(.TextMatrix(intLop, mBillCol.C_批次)) = -1 Or Val(.TextMatrix(intLop, mBillCol.c_新批次)) = 1) Then
                            
                            MsgBox "第" & intLop & "行的卫材(" & Trim(.TextMatrix(intLop, mBillCol.C_材料)) & ")新增批次的产地，批号，成本价和第" & intRow & "行重复了！" & vbCrLf & "请重新录入产地和批号信息！", vbInformation, gstrSysName
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Or mint编辑状态 = 5 Or mint编辑状态 = 6 Then 'mbln单据增加 Or
            ...
            End If
            If chrNo = "" Then chrNo = zlDatabase.GetNextNo(75, lng库房ID)
...
Private Sub 提示库存数()
    ...
    strKc = "" & _
        ...
        "           -SUM (DECODE (a.入出系数, 1, a.实际数量*a.付数, -a.实际数量*a.付数))  AS 可用数量" & _
        "   FROM 药品收发记录 a" & _
        "   Where  a.药品id+0=[2]  " & _
        "          and  nvl(a.批次,0)=[3] " & _
        "           AND a.库房id + 0 =[1]" & _
...
'   2.如果是库房分批药，则分别处理该药的未处理的各批次库存行。
    ...
    SetPhiscRows = False
    Set rsData = GetDateStock(txtCheckDate.Caption, txtStock.Tag, "", True, , , lngId)
    lngRecordCount = rsData.RecordCount
    ...
    With mshBill
            ...
            For lngRow = 1 To .Rows - 1
                If .TextMatrix(lngRow, 0) <> "" Then
                    If .TextMatrix(lngRow, 0) = rsData!材料ID And IIf(.TextMatrix(lngRow, mBillCol.C_批次) = "", "0", .TextMatrix(lngRow, mBillCol.C_批次)) = lng批次 Then
                        MsgBox "已有该卫生材料，请检查！", vbOKOnly, gstrSysName
                        Exit Function
                    End If
                End If
            Next
        ...
        Else
            ...
            .TextMatrix(lngRow, mBillCol.C_实盘数量) = .TextMatrix(lngRow, mBillCol.C_帐面数量)
            .TextMatrix(lngRow, mBillCol.C_售价) = Format(IIf(IsNull(rsData!售价), 0, rsData!售价), mFMT.FM_零售价)
            .TextMatrix(lngRow, mBillCol.C_成本价) = Format(Val(NVL(rsData!成本价)), mFMT.FM_成本价)
            .TextMatrix(lngRow, mBillCol.C_盘点金额) = Format(Val(.TextMatrix(lngRow, mBillCol.C_实盘数量)) * Val(.TextMatrix(lngRow, mBillCol.C_售价)), mFMT.FM_金额)
        ...
        If rsData!是否变价 = 1 Then
            .TextMatrix(lngRow, mBillCol.C_售价) = Format(Get零售价(Val(NVL(rsData!材料ID)), Val(txtStock.Tag), Val(NVL(rsData!批次)), rsData!比例系数), mFMT.FM_零售价)
        End If
...
Private Sub printbill()
    Dim strNo As String
    strNo = txtNO.Tag
    Call FrmBillPrint.ShowMe(Me, glngSys, "zl1_bill_1719", mint记录状态, mintUnit, 1719, "卫生材料盘点表", strNo, txtStock.Tag)
End Sub
