...
Option Explicit
Private mblnSelectStock As Boolean             '是否可选部门
...
Public Sub ShowCard(frmMain As Form, ByVal str单据号 As String, _
    ...
    mblnProvider = IsHavePrivs(mstrPrivs, "查看供应商")
...
Private Sub cboEnterStock_Click()
    ...
    If cboEnterStock.ItemData(cboEnterStock.ListIndex) = -1 And Visible Then
        ...
        If Not rsTmp Is Nothing Then
            intIdx = SeekCboIndex(cboEnterStock, rsTmp!Id)
            If intIdx <> -1 Then
...
'                cboEnterStock.ListIndex = cboEnterStock.NewIndex
        ...
        Else
            ...
            intIdx = SeekCboIndex(cboEnterStock, cboEnterStock.Tag)
            Call zlControl.CboSetIndex(cboEnterStock.hwnd, intIdx)
        End If
    Else
...
Private Sub cboEnterStock_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii >= 32 Then
        If Not cboEnterStock.Locked And cboEnterStock.Style = 2 Then
            lngIdx = zlControl.CboMatchIndex(cboEnterStock.hwnd, KeyAscii)
...
Private Sub cboStock_Click()
    ...
    If cboStock.ItemData(cboStock.ListIndex) = -1 And Visible Then
        strSql = "" & _
            ...
            "   Order by 简码"
        vRect = GetControlRect(cboStock.hwnd)
        ...
        If Not rsTmp Is Nothing Then
            intIdx = SeekCboIndex(cboStock, rsTmp!Id)
            If intIdx <> -1 Then
        ...
        Else
            ...
            intIdx = SeekCboIndex(cboStock, cboStock.Tag)
            Call zlControl.CboSetIndex(cboStock.hwnd, intIdx)
        End If
    Else
...
Private Sub cboStock_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii >= 32 Then
        If Not cboStock.Locked And cboStock.Style = 2 Then
            lngIdx = zlControl.CboMatchIndex(cboStock.hwnd, KeyAscii)
...
Private Sub Form_Load()
    mFMT.FM_金额 = GetDigit
    mblnSelectStock = IIf(Val(zldatabase.GetPara("是否选择部门", glngSys, mlngModule, "0")) = 1, 1, 0) = 1
    Me.cboStock.Enabled = mblnSelectStock
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            If rsInitCard.EOF Then
                ...
                Exit Sub
            End If
            mlng库房id = Val(NVL(rsInitCard!部门ID))
            With cboStock
                blnNO库房 = True
                For i = 0 To .ListCount - 1
            ...
            With mshBill
                For intRow = 1 To intRecordCount
                    ...
                    .TextMatrix(intRow, mHeadCol.请购数量) = IIf(Format(Val(NVL(rsInitCard!请购数量)), mFMT.FM_数量) = 0, "", Format(rsInitCard!请购数量 / rsInitCard!比例系数, mFMT.FM_数量))
                    If mint编辑状态 = 3 Then
...
Private Sub mshbill_CommandClick()
    ...
    If mshBill.Col = mHeadCol.材料 Then
        Set RecReturn = Frm材料选择器.ShowMe(Me, 1, Val(cboStock.ItemData(cboStock.ListIndex)), mlng库房id, , , , , , , , , , , , , , mstrPrivs)
        If RecReturn.RecordCount > 0 Then
            If RecReturn.RecordCount = 1 Then
                ...
                SetStuffRows RecReturn!材料ID, "[" & RecReturn!编码 & "]" & RecReturn!名称, _
                            ...
                            Switch(strUnit = "散装单位", NVL(RecReturn!散装单位), strUnit = "包装单位", NVL(RecReturn!包装单位)), Val(NVL(RecReturn!指导批发价)), _
                            Switch(strUnit = "散装单位", 1, strUnit = "包装单位", Val(NVL(RecReturn!换算系数)))
            End If
            RecReturn.Close
        End If
    Else
...
Private Function Get产地(Optional strKey As String = "") As Boolean
    ...
    '应用：可用于各个程序中数据量不是很大的选择器,输入匹配列表等。
     ...
     If strKey <> "" Then
        ...
        gstrSQL = "" & _
            ...
            "   order by 编码"
         vRect = GetControlRect(mshBill.TxtHwnd)
    ...
    With mshBill '
        .TextMatrix(.Row, mHeadCol.产地) = NVL(rsTemp!名称)
        ...
        Select Case .Col
            ...
            Case mHeadCol.材料
                If strKey <> "" Then
                    ...
                    Set rsTemp = FrmMulitSel.ShowSelect(Me, 1, Val(cboStock.ItemData(cboStock.ListIndex)), mlng库房id, , strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, , , , , , , , , , , , mstrPrivs)
                    If rsTemp.RecordCount = 1 Then
                        ...
                        If SetStuffRows(rsTemp!材料ID, "[" & rsTemp!编码 & "]" & rsTemp!名称, _
                            ...
                            Switch(strUnit = "散装单位", NVL(rsTemp!散装单位), strUnit = "包装单位", NVL(rsTemp!包装单位)), Val(NVL(rsTemp!指导批发价)), _
                            Switch(strUnit = "散装单位", 1, strUnit = "包装单位", Val(NVL(rsTemp!换算系数)))) = False Then
                            Cancel = True
                            Exit Sub
                        End If
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If NO_IN = "" Then NO_IN = zldatabase.GetNextNo(85, mlng库房id)
        ...
        For intCount = 1 To .Rows - 1
            If intCount <> intRow And .TextMatrix(intCount, 0) <> "" Then
                If .TextMatrix(intCount, 0) = lng材料ID Then
                    MsgBox "该卫生材料已有了，不能再输！", vbOKOnly + vbExclamation, gstrSysName
                    Exit Function
                End If
            End If
        Next
...
Private Sub SetEnterStock(ByVal lngStockID As Long)
    ...
    With cboEnterStock
        ...
        Do While Not rsSQL.EOF
            .AddItem NVL(rsSQL!名称)
            .ItemData(.NewIndex) = Val(NVL(rsSQL!Id))
            rsSQL.MoveNext
        Loop
        rsSQL.Close
    End With
