...
Private Sub cboStock_Click()
    Dim rsTemp As New ADODB.Recordset
    If mblnNoClick Then Exit Sub
    If cboStock.ListIndex >= 0 Then mlngPreEnterId = cboStock.ItemData(cboEnterStock.ListIndex)
    ...
    With rsTemp
        ...
        Do While Not .EOF
            ...
            If mint编辑状态 = 11 Then
                If Val(NVL(!Id)) = mfrmMain.cboEnterStock.ItemData(mfrmMain.cboEnterStock.ListIndex) Then
...
Private Sub cboStock_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode <> vbKeyReturn Then Exit Sub
    If cboStock.ListCount = 0 Then zlCommFun.PressKey vbKeyTab: Exit Sub
    ...
    If Select部门选择器(Me, cboStock, Trim(cboStock.Text), "V,K,W", Not IsHavePrivs(mstrPrivs, "所有库房")) = False Then
        Exit Sub
    End If
    If cboStock.ListIndex >= 0 Then
...
'                End If
                ...
                If blnDo = False Then
                    ...
                    With mshBill
                        ...
                        .TextMatrix(lngRow, mBillCol.C_差价) = Format(Val(.TextMatrix(lngRow, mBillCol.C_售价金额)) - Val(.TextMatrix(lngRow, mBillCol.C_采购金额)), mFMT.FM_金额)
...
Private Sub initCard()
    ...
    On Error GoTo ErrHandle
    mbln申领单 = False
     mint移库处理流程 = IIf(Val(zldatabase.GetPara("移库流程", glngSys, mlngModule, "0")) = 1, 1, 0)
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4, 6, 10, 11
            ...
            '检查该单据是否是申领单据
            gstrSQL = "" & _
                "   Select Nvl(发药方式,0) 申领 From 药品收发记录 " & _
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mBillCol.C_一次性材料) = NVL(rsInitCard!一次性材料)
                    .TextMatrix(intRow, mBillCol.C_灭菌效期) = NVL(rsInitCard!灭菌效期)
                    ...
                    .TextMatrix(intRow, mBillCol.C_售价金额) = Format(IIf(mint编辑状态 = 6 And mint冲销方式 = 2, -1, 1) * rsInitCard!零售金额, mFMT.FM_金额)
                    .TextMatrix(intRow, mBillCol.C_差价) = Format(IIf(mint编辑状态 = 6 And mint冲销方式 = 2, -1, 1) * rsInitCard!差价, mFMT.FM_金额)
...
Private Sub SetEdit()
    ...
    With mshBill
        ...
        Else
            ...
            If mstrSelectStock = "0" Or mint编辑状态 = 11 Then
                '入库转入也不能进行编辑
                cboStock.Enabled = False
            Else
...
Private Function SaveCheck(Optional ByVal bln强制保存 As Boolean = False) As Boolean
    ...
    str审核日期 = Format(zldatabase.Currentdate, "yyyy-mm-dd hh:mm:ss")
    With mshBill
        '按材料id顺序更新数据
...
Private Sub mshbill_CommandClick()
    ...
    If Not mbln申领单 Then
        Set RecReturn = Frm材料选择器.ShowMe(Me, 2, cboStock.ItemData(cboStock.ListIndex), cboEnterStock.ItemData(cboEnterStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), _
            mbln移库明确批次, True, False, False, True, , , , , mbln仅显示有库存物资, , , mstrPrivs, mbln移库明确批次)
    Else
        Set RecReturn = Frm材料选择器.ShowMe(Me, 2, cboStock.ItemData(cboStock.ListIndex), cboEnterStock.ItemData(cboEnterStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), _
            mbln明确批次, mbln明确批次, False, False, True, , , , , mbln仅显示有库存物资, , , mstrPrivs, IIf(mbln申领单 = True, mbln明确批次, True))
    End If
                ...
                If strKey <> "" Then
                    ...
                    If Not mbln申领单 Then
                        Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, cboStock.ItemData(cboStock.ListIndex), cboEnterStock.ItemData(cboEnterStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), _
                            strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, mbln移库明确批次, True, False, False, True, , , , mbln仅显示有库存物资, , , mstrPrivs, mbln移库明确批次)
                    Else
                        Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, cboStock.ItemData(cboStock.ListIndex), cboEnterStock.ItemData(cboEnterStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), _
                            strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, mbln明确批次, mbln明确批次, False, False, True, , , , mbln仅显示有库存物资, , , mstrPrivs, IIf(mbln申领单 = True, mbln明确批次, True))
                    End If
            ...
            Case mBillCol.C_灭菌日期
                ...
                If strKey <> "" Then
                    ...
                    If Not IsDate(strKey) Then
                        ...
                        Exit Sub
                    End If
                    If Format(zldatabase.Currentdate, "yyyy-mm-dd") >= Format(DateAdd("m", Val(.TextMatrix(.Row, mBillCol.C_灭菌效期)), CDate(strKey)), "yyyy-mm-dd") Then
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, _
    ...
    On Error GoTo ErrHandle
    If str灭菌失效期 <> "" Then
        If Format(str灭菌失效期, "yyyy-mm-dd") <= Format(zldatabase.Currentdate, "yyyy-mm-dd") Then
            If MsgBox("该卫材的灭菌效期已经过期,是否还要进行移库?", vbYesNo + vbDefaultButton1 + vbQuestion) = vbNo Then
                Exit Function
            End If
        End If
    End If
    ...
    With mshBill
        ...
        .TextMatrix(intRow, mBillCol.C_一次性材料) = NVL(rs效期!一次性材料)
        .TextMatrix(intRow, mBillCol.C_灭菌效期) = NVL(rs效期!灭菌效期)
...
Private Function SaveCard(Optional ByVal bln强制保存 As Boolean = False) As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Or mint编辑状态 = 11 Then  ' Or mbln单据增加
            ...
            If chrNo = "" Then chrNo = zldatabase.GetNextNo(72, lng库房ID)
                ...
                If Not mbln申领单 Or bln强制保存 Then
                    gstrSQL = "zl_材料移库_INSERT('" & chrNo & "'," & lng序号 & "," & lng库房ID & "," & _
                        ...
                        IIf(str灭菌效期 = "", "Null", "to_date('" & Format(str灭菌效期, "yyyy-MM-dd") & "','yyyy-mm-dd')") & ",'" & _
                         str摘要 & "',to_date('" & str填制日期 & "','yyyy-mm-dd HH24:MI:SS'))"
                Else
    ...
    With mshBill
        ...
        For intRow = 1 To .Rows - 1
            If Val(.TextMatrix(intRow, mBillCol.C_实际数量)) <> 0 Then
                ...
                If int库存检查 <> 0 Or lng批次 > 0 Then '分批的检查,不足禁用
                    ...
                    If Check可用数量(lng库房ID, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, IIf(lng批次 > 0, 2, int库存检查), IIf(mint冲销方式 = 2, 1, 0)) = False Then .Row = intRow: Exit Function
                End If
...
Private Sub 提示库存数()
    ...
    With mshBill
        ...
        Else
            ...
            With rsUseCount
                Do While Not .EOF
                    strNote = strNote & "," & !库房 & ":" & Format(NVL(!可用数量, 0), mFMT.FM_数量) & mshBill.TextMatrix(mshBill.Row, mBillCol.c_单位)
                    .MoveNext
                Loop
            End With
            ...
            If blnInput = False Then
                ...
                If !实际数量 > !可用数量 Then
                    ...
                    If !批次 <> 0 Or !时价 <> 0 Then
                        MsgBox !药品名称 & "库存不足，不允许出库！（时价或分批卫材）", vbInformation, gstrSysName
                        blnInput = True
                    End If
                    '只提示一次
                    If blnInput = False Then
...
Private Function CompareUsableQuantity(ByVal intRow As Integer, ByVal dbl填写数量 As Double, Optional ByVal blnSave As Boolean = False) As Boolean
    ...
    With mshBill
        ...
        If Val(.TextMatrix(intRow, mBillCol.C_批次)) <> 0 Then '分批卫材
            If mint编辑状态 = 1 Then
                If dbl填写数量 > dblUsableQuantity Then
                    MsgBox "第" & intRow & "行[" & .TextMatrix(intRow, C_材料) & "]是分批核算材料，可用库存数量不够！", vbExclamation + vbOKOnly, gstrSysName
                    Exit Function
                End If
        ...
        ElseIf mint库存检查 = 1 Then
            ...
            ElseIf mint编辑状态 = 2 Or mint编辑状态 = 3 Then
                numUsedCount = 0
                For Each vardrug In mcolUsedCount
                    If vardrug(0) = .TextMatrix(.Row, 0) & .TextMatrix(.Row, mBillCol.C_批次) Then
                        numUsedCount = vardrug(1)
                        Exit For
                    End If
                Next
...
Private Sub printbill()
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1716", mint记录状态, mintUnit, 1716, "卫材调拨单", strNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
        ...
        If lng材料ID <> 0 Then
            ...
            If rsCheck.RecordCount <> 0 Then
                If mint编辑状态 = 10 Then '发送时应该用可用数量判断
                    dbl可用数量 = NVL(rsCheck!可用数量, 0) / dbl比例系数
                ElseIf mint编辑状态 = 3 Then  '审核时应该用实际数量判断
                    dbl可用数量 = NVL(rsCheck!实际数量, 0) / dbl比例系数
                End If
            End If
