    ...
    Case 0
        With Msf批次
            ...
            If BlnSetHeader Then
                ...
                .TextMatrix(0, 10) = "售价"
                .TextMatrix(0, 11) = "上次购价"
                .TextMatrix(0, 12) = "产地"
                .TextMatrix(0, 13) = "生产日期"
                .TextMatrix(0, 14) = "上次供应商ID"
                .TextMatrix(0, 15) = "批准文号"
                .TextMatrix(0, 16) = "成本价"
            ...
            If mblnStartUp = False Then
                ...
                .ColWidth(16) = IIf(mblnCostView = False, 0, 1200)
            End If
        End With
    End Select
End Sub
...
'    On Error Resume Next
    ...
    If In_编辑状态 = 2 Then
        ...
        If mbln空批次 Then
            gstrSQL = "" & _
                "   Select " & IIf(mstr盘点时间 <> "", "/*+ Rule*/", "") & " 1 RID,名称,0 批次,'新增批次卫生材料' 批号,sysdate 失效期,0 可用数量,0 库存数量,0 库存金额" & _
                "           ,0 库存差价,sysdate as 灭菌失效期,0 售价,'' As 成本价,0 上次购价,'' 产地 , Sysdate As 生产日期, 0 As 上次供应商id,'' 批准文号" & _
                "   From 部门表" & _
                "   Where ID=[1]" & _
                "   Union "
        End If
        ...
        If mstr盘点时间 <> "" Then
            strKc = strKc & _
                ...
                "           -SUM (DECODE (a.入出系数, 1, a.差价, -a.差价)) AS 实际差价,0 AS 可用数量,Max(零售价) as 零售价,0 as 平均成本价,a.批号,a.产地 , A.效期,a.灭菌效期,a.生产日期,a.批准文号" & _
                "   FROM 药品收发记录 a " & _
                "   Where  a.药品id+0=[4]  " & _
                "           AND a.库房id + 0 ="
            If mlng源库房ID <> 0 Or mlng目库房ID <> 0 Then
        ...
        gstrSQL = gstrSQL & strTmp & _
                 IIf(mblnStock, " K.实际金额 as 库存金额,", "'' 库存金额,") & _
                 IIf(mblnStock, " K.实际差价 as 库存差价", "'' 库存差价") & ",K.灭菌效期 灭菌失效期," & _
                 IIf(mblnStock, "Decode(nvl(M.是否变价,0),0,G.现价,decode(nvl(K.零售价,0),0,nvl(K.实际金额,0)/decode(K.实际数量,null,1,0,1,K.实际数量),K.零售价))" & IIf(mbln散装单位, "", "*nvl(D.换算系数,1)") & " 售价,", "0 售价,") & _
        " to_char(k.平均成本价," & gOraFmt_Max.FM_成本价 & ") as 成本价, " & _
                 IIf(mblnStock, "decode(nvl(K.上次采购价,0),0,(nvl(K.实际金额,0)-nvl(K.实际差价,0))/decode(K.实际数量,null,1,0,1,K.实际数量),K.上次采购价)" & IIf(mbln散装单位, "", "*nvl(D.换算系数,1)") & " 上次购价", "0 上次购价") & _
        ...
        " And m.Id = g.收费细目id And (Sysdate Between g.执行日期 And Nvl(g.终止日期, Sysdate)) "
...
'        If mlng供应商ID <> 0 Then gstrSQL = gstrSQL & " And K.上次供应商ID=[3]"
        ...
        Set mrsStock = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlng目库房ID, mlng源库房ID, mlng供应商ID, mlngLastSelect材料ID, dtDate, gstrNodeNo)
...
Private Sub tvwClass_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    '对列头排顺序
    gstrSQL = "" & _
        " Select " & IIf(mstr盘点时间 <> "", "/*+ Rule*/", "") & " D.诊疗id,D.材料id,D.分类ID,D.编码,D.通用名称,D.商品名,D.规格,D.产地,d.批准文号,x.名称 As 上次供应商 ,D.售价,to_char(d.成本价," & gOraFmt_Max.FM_成本价 & ") as 最新成本价,D.散装单位,D.换算系数,D.包装单位," & _
          IIf(mblnStock, "  S.可用数量 " & IIf(mbln散装单位, "", "/D.换算系数") & " 可用数量, S.库存数量 " & _
          IIf(mbln散装单位, "", "/D.换算系数") & " 库存数量 ,S.库存金额,S.库存差价,", "'' 可用数量,'' 库存数量,'' 库存金额,'' 库存差价,") & _
        "     D.最大效期 有效期,D.灭菌效期,S.灭菌失效期,D.一次性材料,D.无菌性材料,D.库房分批,D.在用分批,D.时价,D.指导批发价,D.指导差价率,E.库房货位" & _
        " From "

    '材料信息，材料目录
    If mbln只显示跟踪材料 Then
        gstrSQL = gstrSQL & _
                "     (Select Distinct u.诊疗id,u.材料id,H.分类ID,V.编码,V.名称 As 通用名称,B.名称 As 商品名,V.规格," & IIf(int入库产地取值方式 = 0, "decode(u.上次产地,null,v.产地,u.上次产地)", "decode(v.产地,null,u.上次产地,v.产地)") & " as 产地,u.批准文号,V.计算单位 as 散装单位,U.包装单位," & _
                ...
                "          AND U.材料id=i.收费细目ID " & IIf(mbln盘无存储库房材料, "(+)", "") & _
                           IIf(mbln只显示跟踪材料, " and  U.跟踪在用 =1 ", IIf(mblnTrackUsing = True, " and  U.跟踪在用 =0 ", "")) & " And v.Id = p.收费细目id And (Sysdate Between p.执行日期 And Nvl(p.终止日期, Sysdate)) "
    Else
        gstrSQL = gstrSQL & _
                "     (Select Distinct u.诊疗id,u.材料id,H.分类ID,V.编码,V.名称 As 通用名称,B.名称 As 商品名,V.规格," & IIf(int入库产地取值方式 = 0, "decode(u.上次产地,null,v.产地,u.上次产地)", "decode(v.产地,null,u.上次产地,v.产地)") & " as 产地,u.批准文号,V.计算单位 as 散装单位,U.包装单位," & _
                "          To_Char(U.换算系数," & GFM_XS & " ) 换算系数,nvl(To_Char(U.灭菌效期,'9999990'),0) 灭菌效期,nvl(To_Char(U.最大效期,'9999990'),0) 最大效期," & _
...
Private Function CombinateRec() As Boolean
    ...
    With mrsReturn
        ...
        Else '入库或不分批
            ...
            If In_编辑状态 = 2 And mint分批 = 0 Then  '出库不分批
...
Public Function ShowMe(ByVal frmMain As Form, ByVal 编辑模式 As Integer, Optional ByVal 源库房 As Long, _
                    ...
                    Optional ByVal bln显示批次 As Boolean = True) As ADODB.Recordset
    '------------------------------------------------------------------------------------------------------
    '功能:显示选择器
