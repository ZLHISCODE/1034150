        ...
        Case 2, 3, 4, 6
            ...
            With mshBill
                ...
                Do While Not rsTemp.EOF
                    intRow = intRow + 1
                    .TextMatrix(intRow, .ColIndex("材料ID")) = NVL(rsTemp!材料ID)
                    .TextMatrix(intRow, .ColIndex("卫材信息")) = NVL(rsTemp!卫材信息)
                    .TextMatrix(intRow, .ColIndex("序号")) = NVL(rsTemp!序号)
                    .TextMatrix(intRow, .ColIndex("规格")) = NVL(rsTemp!规格)
                    .TextMatrix(intRow, .ColIndex("产地")) = NVL(rsTemp!产地)
                    .TextMatrix(intRow, .ColIndex("批准文号")) = NVL(rsTemp!批准文号)
                    .TextMatrix(intRow, .ColIndex("单位")) = NVL(rsTemp!单位)
                    .TextMatrix(intRow, .ColIndex("批号")) = NVL(rsTemp!批号)
                    .TextMatrix(intRow, .ColIndex("生产日期")) = NVL(rsTemp!生产日期)
                    .TextMatrix(intRow, .ColIndex("效期")) = IIf(IsNull(rsTemp!效期), "", Format(rsTemp!效期, "yyyy-mm-dd"))
                    .TextMatrix(intRow, .ColIndex("一次性材料")) = Val(NVL(rsTemp!一次性材料))
                    .TextMatrix(intRow, .ColIndex("条码管理")) = Val(NVL(rsTemp!条码管理))
                    .TextMatrix(intRow, .ColIndex("灭菌效期")) = NVL(rsTemp!灭菌效期)
                    ...
                    .TextMatrix(intRow, .ColIndex("商品条码")) = NVL(rsTemp!商品条码)
                    If rsTemp!数量 <> 0 Then
                    ...
                    .TextMatrix(intRow, .ColIndex("零售价")) = Format(Val(NVL(rsTemp!零售价)), mFMT.FM_散装零售价)          'If Val(.TextMatrix(.row, .colindex("零售价"))) = 0 Then
                    
                    '反算售价
                    .TextMatrix(intRow, .ColIndex("售价")) = Format((Val(NVL(rsTemp!零售金额)) - Val(NVL(rsTemp!零售差价))) / Val(NVL(rsTemp!数量)), mFMT.FM_零售价)
                    .TextMatrix(intRow, .ColIndex("零售单位")) = NVL(rsTemp!零售单位)
                    ...
                    Else
                        .TextMatrix(intRow, .ColIndex("零售差价")) = Format(Val(NVL(rsTemp!差价)), mFMT.FM_金额)
                        .TextMatrix(intRow, .ColIndex("零售金额")) = Format(Val(NVL(rsTemp!零售金额)), mFMT.FM_金额)
                        '反算售价及售价金额
                        .TextMatrix(intRow, .ColIndex("差价")) = Format(Val(NVL(rsTemp!差价)) - Val(NVL(rsTemp!零售差价)), mFMT.FM_金额)
                        .TextMatrix(intRow, .ColIndex("售价金额")) = Format(Val(NVL(rsTemp!零售金额)) - Val(NVL(rsTemp!零售差价)), mFMT.FM_金额)
                        .TextMatrix(intRow, .ColIndex("采购金额")) = Format(Val(NVL(rsTemp!成本金额)), mFMT.FM_金额)
                    End If
                    .TextMatrix(intRow, .ColIndex("原产地")) = IIf(IsNull(rsTemp!原产地), "!", rsTemp!原产地)
                    
                    '存储格式:最大效期||指导差价率||是否变价||在用分批||库房分批
                    .TextMatrix(intRow, .ColIndex("原销期")) = IIf(IsNull(rsTemp!最大效期), "0", rsTemp!最大效期) & "||" & rsTemp!加成率 & "||" & IIf(IsNull(rsTemp!是否变价), 0, rsTemp!是否变价) & "||" & IIf(IsNull(rsTemp!在用分批), 0, rsTemp!在用分批) & "||" & NVL(rsTemp!库房分批, 0)
                    .TextMatrix(intRow, .ColIndex("比例系数")) = rsTemp!比例系数
                    If mint编辑状态 = 6 Then
                        .TextMatrix(intRow, .ColIndex("冲销数量")) = Format(0, mFMT.FM_数量)
                        .TextMatrix(intRow, .ColIndex("真实数量")) = NVL(rsTemp!真实数量)
                    End If
                    rsTemp.MoveNext
                Loop
            End With
...
Private Sub imgLeft_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(mshBill.hwnd)
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, ByVal str诊疗id As String, ByVal str规格 As String, _
    ...
    int库房分批 = Val(NVL(rsTemp!库房分批))
    dbl成本价 = NVL(rsTemp!成本价, 0)
    str散装单位 = NVL(rsTemp!散装单位)
    ...
    With mshBill
        ...
        .TextMatrix(intRow, .ColIndex("一次性材料")) = NVL(rsTemp!一次性材料)
        .TextMatrix(intRow, .ColIndex("条码管理")) = NVL(rsTemp!条码管理)
        .TextMatrix(intRow, .ColIndex("灭菌效期")) = NVL(rsTemp!灭菌效期)
...
'            End If
        ...
        '时价材料处理
        dbl成本价 = Val(.TextMatrix(intRow, .ColIndex("采购价")))
        
        If int是否变价 = 1 Then
            If mbln加价率 = True Then
                '由于存在差价让利比的存在,需要按加成率计算,因此将指导差价率转换成加成率计算 公式：加成率=1/(1-差价率)-1
                .TextMatrix(intRow, .ColIndex("售价")) = Format(校正零售价(dbl成本价 * (1 + dbl加成率) + _
                                                时价材料零售价(lng材料ID, dbl成本价, dbl加成率)) _
                                                , mFMT.FM_零售价)
'                .TextMatrix(intRow, mcol加成率) = Format(dbl加成率 * 100, "###0.00") & "%"
            ElseIf mbln分段加成率 = True Then
                dbl加成率 = 0
                If Get分段加成售价(dbl成本价, Val(.TextMatrix(intRow, .ColIndex("比例系数"))), mstrCaption, sng分段售价) = False Then
                    .TextMatrix(intRow, .ColIndex("售价")) = Format(校正零售价(dbl成本价 * (1 + dbl加成率) + _
                                                    时价材料零售价(lng材料ID, dbl成本价, dbl加成率)) _
                                                    , mFMT.FM_零售价)
'                    .TextMatrix(intRow, mcol加成率) = Format(dbl加成率 * 100, "###0.00") & "%"
            ...
            Else '取上次售价模式和没有勾选任何取售价方式
                If mbln时价卫材取上次售价 = True Then
                    ...
                    If rsTemp!上次售价 > 0 Then
                        .TextMatrix(intRow, .ColIndex("售价")) = Format(zlStr.NVL(rsTemp!上次售价, 0) * Val(.TextMatrix(intRow, .ColIndex("比例系数"))), mFMT.FM_零售价)
                        If dbl成本价 <> 0 Then
'                            .TextMatrix(intRow, mcol加成率) = Format((Val(.TextMatrix(intRow, .ColIndex("售价"))) / dbl成本价 - 1) * 100, "###0.00") & "%"
                        End If
                    Else
                        '由于存在差价让利比的存在,需要按加成率计算,因此将指导差价率转换成加成率计算 公式：加成率=1/(1-差价率)-1
                        .TextMatrix(intRow, .ColIndex("售价")) = Format(校正零售价(dbl成本价 * (1 + dbl加成率) + _
    ...
    With mshBill
        ...
        Select Case Col
          Case .ColIndex("灭菌日期")
                ...
                If Format(zldatabase.Currentdate, "yyyy-mm-dd") >= Format(DateAdd("m", Val(.TextMatrix(Row, .ColIndex("灭菌效期"))), CDate(strKey)), "yyyy-mm-dd") Then
           ...
           Case .ColIndex("生产日期"), .ColIndex("效期")
            ...
            Case .ColIndex("采购价")
                If strKey = "" Then
                    MsgBox "该卫材的采购价为空了，请检查！", vbInformation, gstrSysName
                End If
                ...
                If strKey <> "" Then
                    gstrSQL = "Select nvl(a.指导批发价,0) as 指导批发价, b.现价" & vbNewLine & _
                                ...
                                " Where a.材料id = b.收费细目id And Sysdate Between b.执行日期 And b.终止日期 And a.材料id = [1]"
                    ...
                    If .TextMatrix(Row, .ColIndex("原销期")) <> "" Then
                        ...
                        If arrSplit(2) = 1 Then
                            ...
                            ElseIf mbln分段加成率 Then
                                    ...
                                    .TextMatrix(Row, .ColIndex("售价")) = Format(校正零售价(sng分段售价 + _
                                                                          ...
                                                                          , mFMT.FM_零售价)
                            ElseIf mbln时价卫材取上次售价 = True Then
                                gstrSQL = "Select Nvl(上次售价, 0) As 上次售价 From 材料特性 Where 材料id = [1]"
                                Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(.TextMatrix(Row, .ColIndex("材料ID"))))
                                If rsTemp!上次售价 > 0 Then
                                    .TextMatrix(.Row, .ColIndex("售价")) = Format(zlStr.NVL(rsTemp!上次售价, 0) * Val(.TextMatrix(.Row, .ColIndex("比例系数"))), mFMT.FM_零售价)
                                Else
                                    '由于存在差价让利比的存在,需要按加成率计算,因此将指导差价率转换成加成率计算 公式：加成率=1/(1-差价率)-1
                                    mdbl加价率 = Round(arrSplit(1) * 100, 2)

                                    .TextMatrix(.Row, .ColIndex("售价")) = Format(校正零售价(Val(.TextMatrix(Row, .ColIndex("采购价"))) * (1 + (mdbl加价率 / 100)) + _
                                    时价材料零售价(Val(.TextMatrix(Row, .ColIndex("材料ID"))), Val(.TextMatrix(Row, .ColIndex("采购价"))), (mdbl加价率 / 100))), mFMT.FM_零售价)
                                End If
                        ...
                        Else
                            '定价检查成本价大于了售价提示
                            If Val(Format(rsTemp!现价 * Val(.TextMatrix(Row, .ColIndex("比例系数"))), mFMT.FM_零售价)) < Val(Format(Val(strKey), mFMT.FM_成本价)) Then
                ...
                strKey = Format(Val(strKey), mFMT.FM_成本价)
                
                '设置金额
                .TextMatrix(Row, .ColIndex("采购金额")) = Format(.TextMatrix(Row, .ColIndex("数量")) * Val(strKey), mFMT.FM_金额)
            ...
            Case .ColIndex("数量")
                ...
                If zlDblIsValid(strKey, 16, True, True, 0, .ColKey(Col)) = False Then
                   Cancel = True: Exit Sub
                End If
            ...
            Case .ColIndex("售价")
                ...
                If strKey = "" Then
                    MsgBox "该卫材的售价为空了，请检查！", vbInformation, gstrSysName
                End If
...
Private Function ValidData() As Boolean
    ...
    If Trim(txtNO.Text) = "" And mbln修改单据号 Then
        ShowMsgBox "单据号不能为空"
        Exit Function
    End If
...
'
                    ...
                    If Trim(Trim(.TextMatrix(intLop, .ColIndex("采购价")))) = "" Then
                        MsgBox "第" & intLop & "行卫材的采购价为空了，请检查！", vbInformation, gstrSysName
                        mshBill.SetFocus
                        .Row = intLop
                        .TopRow = intLop
                        .Col = .ColIndex("采购价")
                    ...
                    If Trim(Trim(.TextMatrix(intLop, .ColIndex("售价")))) = "" Then
                        MsgBox "第" & intLop & "行卫材的售价为空了，请检查！", vbInformation, gstrSysName
                        mshBill.SetFocus
                        .Row = intLop
                        .TopRow = intLop
                        .Col = .ColIndex("售价")
                    ...
                    If zlDblIsValid(.TextMatrix(intLop, .ColIndex("零售价")), 16, False, False, , "第" & intLop & "行卫生材料的零售价") = False Then
                        mshBill.SetFocus
                        .Row = intLop: .TopRow = intLop: .Col = .ColIndex("零售价")
                        Exit Function
                    End If
                    If zlDblIsValid(.TextMatrix(intLop, .ColIndex("零售金额")), 16, False, False, , "第" & intLop & "行卫生材料的零售金额") = False Then
                        mshBill.SetFocus
                        .Row = intLop: .TopRow = intLop: .Col = .ColIndex("零售价")
                        Exit Function
                    End If
                    If zlDblIsValid(.TextMatrix(intLop, .ColIndex("零售差价")), 16, False, False, , "第" & intLop & "行卫生材料的零售差价") = False Then
                        mshBill.SetFocus
                        .Row = intLop: .TopRow = intLop: .Col = .ColIndex("零售价")
                        Exit Function
                    End If
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Then   'mbln单据增加 Or
            ...
            End If
            If chrNo = "" Then chrNo = zldatabase.GetNextNo(70, lng库房ID)
...
Private Sub 提示库存数()
    ...
    With recTmp
        ...
        stbThis.Panels(2).Text = "该卫材当前库存数为[" & Format(dbl数量, mFMT.FM_数量) & "]" & NVL(!单位)
    End With
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
...
Private Sub printbill()
    ...
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1714", mint记录状态, mintUnit, 1714, "卫材其它入库单", strNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
...
Private Function 校正零售价(ByVal sin零售价 As Double, Optional lngLastRow As Long = -1) As Double
    ...
    If rsTemp.EOF Then Exit Function
    sin指导零售价 = NVL(rsTemp!指导零售价, mshBill.ColIndex("材料ID"))
    sin指导零售价 = sin指导零售价 * Val(mshBill.TextMatrix(lngLastRow, mshBill.ColIndex("比例系数")))
...
Private Function Select卫材信息(Optional strSearch As String) As Boolean
    ...
    With mshBill
        lng库房ID = cboStock.ItemData(cboStock.ListIndex)
        If strSearch = "" Then
            Set rsTemp = Frm材料选择器.ShowMe(Me, 1, , lng库房ID, lng库房ID, , , , , , , , , , , 1714, , mstrPrivs)
        Else
            Call CalcPosition(sngLeft, sngTop, mshBill)
            Set rsTemp = FrmMulitSel.ShowSelect(Me, 1, lng库房ID, lng库房ID, lng库房ID, strSearch, sngLeft, sngTop, mshBill.CellWidth, mshBill.CellHeight, , , , , , , , , , 1714, , mstrPrivs)
        End If
...
Private Function SelDate(ByVal intCol As Integer) As Boolean
    ...
    With mshBill
        ...
        If strDate = "" Then strDate = Format(zldatabase.Currentdate, "yyyy-mm-dd")
        lngH = .CellHeight
        ...
        If intCol = .ColIndex("生产日期") Then strMaxDate = Format(zldatabase.Currentdate, "yyyy-mm-dd")
    End With
...
Private Function Show加成率(ByVal intCol As Integer) As Boolean
    ...
    blnreturn = frm扣率Set.ShowCalc(Me, sngX, sngY, lngH, lng材料ID, mintUnit, dbl零售价, dbl结算价, dbl加成率)
    With mshBill
    ...
    With recSort
        ...
        For n = 1 To mshBill.Rows - 1
            If mshBill.TextMatrix(n, 0) <> "" Then
                ...
                !材料ID = Val(mshBill.TextMatrix(n, mshBill.ColIndex("材料ID")))
