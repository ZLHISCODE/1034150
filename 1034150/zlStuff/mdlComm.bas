...
Public Function zlGetSymbol(strInput As String, Optional bytIsWB As Byte, Optional intOutNum As Integer = 10) As String
    ...
    Set rsTmp = zldatabase.OpenSQLRecord(strSql, "生成字符串的简码", strInput, intOutNum)
...
'取指定列头的列位置
Public Function GetCol(mshFlex As MSHFlexGrid, ByVal ColName As String) As Integer
    Dim i As Integer
...
'    err = 0
    ...
    With rsPar
        ...
        'Call OpenRecordset(rsPar, "取字段长度")
...
Public Function Get分段加成售价(ByVal dbl采购价 As Double, ByVal dbl比例系数 As Double, ByVal strFormCaption As String, ByRef sng售价 As Double) As Boolean
    ...
    If rsTemp.EOF Then
        ...
        Exit Function
    End If
    byt计算方法 = Val(NVL(rsTemp!计算方法))
    dbl总限价 = Val(NVL(rsTemp!限价))
    dbl售价1 = 0
    dblTemp = 0
    ...
    If byt计算方法 = 0 Then
        ...
        With rsTemp
            Do While Not .EOF
                If (dbl成本价 > Val(NVL(!最低价)) And dbl成本价 <= Val(NVL(!最高价))) Or _
                   (dbl成本价 > Val(NVL(!最低价)) And Val(NVL(!最高价)) = 0) _
                Then
                    dbl限价 = Val(NVL(!限价))
                    dblTemp = Val(NVL(!加成率))
                    If dbl限价 > 0 Then
    ...
    With rsTemp
        Do While Not .EOF
            dbl限价 = Val(NVL(rsTemp!限价))
            If dbl成本价 <= Val(NVL(!最高价)) Or Val(NVL(!最高价)) = 0 Then
                dblTemp = (dbl成本价 - Val(NVL(!最低价))) * (Val(NVL(!加成率)) - 1)
                If dbl限价 > 0 Then
                ...
                End If
                GoTo Check售价:
            ElseIf dbl成本价 > Val(NVL(!最高价)) Then
                dblTemp = (Val(NVL(!最高价)) - Val(NVL(!最低价))) * (Val(NVL(!加成率)) - 1)
                If dbl限价 > 0 Then
                    If Round(dblTemp, 7) > Round(dbl限价, 7) Then
                        dbl售价1 = dbl售价1 + dbl限价
                    Else
...
Public Sub 初始小数位数()
    ...
    Do While Not rsTemp.EOF
        If Val(NVL(rsTemp!单位)) = 2 Then
            '包装单位
            If Val(NVL(rsTemp!内容)) = 1 Then
                g_小数位数.obj_包装小数.成本价小数 = Val(NVL(rsTemp!精度))
            ElseIf Val(NVL(rsTemp!内容)) = 2 Then
                g_小数位数.obj_包装小数.零售价小数 = Val(NVL(rsTemp!精度))
            ElseIf Val(NVL(rsTemp!内容)) = 3 Then
                g_小数位数.obj_包装小数.数量小数 = Val(NVL(rsTemp!精度))
            End If
        ElseIf Val(NVL(rsTemp!单位)) = 1 Then
            '散装单位
            If Val(NVL(rsTemp!内容)) = 1 Then
                g_小数位数.obj_散装小数.成本价小数 = Val(NVL(rsTemp!精度))
            ElseIf Val(NVL(rsTemp!内容)) = 3 Then
                g_小数位数.obj_散装小数.数量小数 = Val(NVL(rsTemp!精度))
            Else
                g_小数位数.obj_散装小数.零售价小数 = Val(NVL(rsTemp!精度))
            End If
        ElseIf Val(NVL(rsTemp!单位)) = 5 Then
            '金额
            If glngModul <> 1723 Then '卫材发料的话 就不再用该精度，而是直接用费用业务设置的精度
                g_小数位数.obj_包装小数.金额小数 = Val(NVL(rsTemp!精度))
                g_小数位数.obj_散装小数.金额小数 = Val(NVL(rsTemp!精度))
            End If
        End If
        rsTemp.MoveNext
    Loop
...
Public Function Check出院病人(ByVal strPrivs As String, ByVal lng单据 As Long, ByVal strNo As String, ByVal int记录性质 As Integer, ByVal int门诊标志 As Integer, Optional ByVal lng病人id As Long) As Boolean
    ...
    If IsHavePrivs(strPrivs, "发退出院病人处方") = False Then
        '检查病人已预出院或出院
...
Public Function Check结帐处方(ByVal strPrivs As String, ByVal lng单据 As Long, ByVal strNo As String, ByVal str序号 As String, ByVal int记录性质 As Integer, ByVal int门诊标志 As Integer) As Boolean
    ...
    If IsHavePrivs(strPrivs, "发退结帐处方") = 0 Then
