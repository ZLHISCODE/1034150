...
Private Sub cbo库房_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If cbo库房.ListCount = 0 Then Call zlCtlSetFocus(vsfList): Exit Sub
...
Public Function AviShow(frmMain As Form, Optional ByVal blnShow As Boolean = True)
    ...
    If blnShow Then
        zlCommFun.ShowFlash "正在提取数据,请稍候...", frmMain
    Else
        zlCommFun.StopFlash
    End If
...
Private Sub GetDetailRecord(ByVal intType As Integer, ByVal lng结存id As Long, ByVal str期初日期 As String, ByVal str期末日期 As String)
    ...
    If str期初日期 <> "" Then
        ...
        "       Group By A.库房id, A.药品id, B.名称, Decode(B.系数, 1, '2-入库', '3-出库')) A" & _
        "     Where a.数量 <> 0 Or a.金额 <> 0 Or a.差价 <> 0"
    End If
