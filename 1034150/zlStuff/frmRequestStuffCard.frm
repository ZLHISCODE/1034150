...
Private Const mBillCols  As Integer = 27              '总列数
'=========================================================================================
        ...
        Case 2, 4, 6, 7
            ...
            Else
                gstrSQL = "" & _
                    ...
                    "                   B.材料来源,D.规格,D.产地 AS 原产地,A.产地,A.批准文号, A.批号,A.批次,B.指导差价率,B.库房分批 ," & _
                    "                   B.最大效期,A.效期,A.灭菌效期,A.填写数量 as 原始数量," & strUnitQuantity & _
                    "                   A.成本金额,0 零售金额, 0 差价,A.摘要,A.库房ID,A.对方部门ID,D.是否变价,B.在用分批" & _
                    "           FROM ( " & _
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mBillCol.c_序号) = NVL(rsInitCard!序号)
                    ...
                    .TextMatrix(intRow, mBillCol.C_采购金额) = Format(IIf(mint编辑状态 = 7 And mint处理方式 <> 2, 0, IIf(mint编辑状态 = 7 And mint处理方式 = 2, -1, 1)) * rsInitCard!成本金额, mFMT.FM_金额)
                    .TextMatrix(intRow, mBillCol.C_售价) = Format(rsInitCard!零售价, mFMT.FM_零售价)
                    .TextMatrix(intRow, mBillCol.C_售价金额) = Format(IIf(mint编辑状态 = 7 And mint处理方式 = 2, -1, 1) * rsInitCard!零售金额, mFMT.FM_金额)
                    ...
                    .TextMatrix(intRow, mBillCol.c_原始数量) = Val(NVL(rsInitCard!原始数量))
...
Private Sub initGrid()
    With mshBill
        ...
        ElseIf mint编辑状态 = 4 Or mint编辑状态 = 6 Or mint编辑状态 = 7 Then
...
Private Sub mshbill_CommandClick()
    ...
    Set RecReturn = Frm材料选择器.ShowMe(Me, 2, cboStock.ItemData(cboStock.ListIndex), _
        ...
        False, False, (InStr(1, mstrPrivs, "显示对方库存")), , , , , mint仅显示有库存物资, , , mstrPrivs, IIf(mint明确批次 = 0, False, True))
            ...
            Case mBillCol.C_材料
                If strKey <> "" Then
                    ...
                    Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, cboStock.ItemData(cboStock.ListIndex), _
                        ...
                        IIf(mint明确批次 = 0, False, True), False, False, (InStr(1, mstrPrivs, "显示对方库存")), , , , mint仅显示有库存物资, , , mstrPrivs, IIf(mint明确批次 = 0, False, True))
                    ...
                    If RecReturn.RecordCount = 1 Then
                        If SetColValue(.Row, RecReturn!材料ID, "[" & RecReturn!编码 & "]" & RecReturn!名称, _
                                ...
                                IIf(IsNull(RecReturn!效期), "", RecReturn!效期), _
                                IIf(IsNull(RecReturn!最大效期), "0", RecReturn!最大效期), _
                                IIf(NVL(RecReturn!一次性材料, 0) = 1, True, False), _
                                IIf(NVL(RecReturn!灭菌失效期) = "", "", Format(RecReturn!灭菌失效期, "yyyy-mm-dd")), _
                                RecReturn!库房分批, _
                                IIf(IsNull(RecReturn!可用数量), "0", RecReturn!可用数量), _
                                IIf(IsNull(RecReturn!实际金额), "0", RecReturn!实际金额), _
                                IIf(IsNull(RecReturn!实际差价), "0", RecReturn!实际差价), _
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, _
    ...
    If bln一次性材料 = True Then
        If Format(str灭菌失效期, "yyyy-mm-dd") < Format(zldatabase.Currentdate, "yyyy-mm-dd") And Trim(str灭菌失效期) <> "" Then
           If MsgBox("该卫材已经过了灭菌失效期,是否还要申领！", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) <> vbYes Then
                Exit Function
           End If
        End If
    End If
...
Private Sub printbill()
    ...
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1722", mint记录状态, mintUnit, 1722, "卫材申领单", strNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
