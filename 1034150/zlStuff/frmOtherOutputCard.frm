...
Private Sub mshbill_CommandClick()
    ...
    Set RecReturn = Frm材料选择器.ShowMe(Me, 2, cboStock.ItemData(cboStock.ListIndex), , cboStock.ItemData(cboStock.ListIndex), , , , , , , , , , , , , mstrPrivs)
        ...
        Select Case .Col
            ...
            Case mconIntCol材料
                If strKey <> "" Then
                    ...
                    Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, cboStock.ItemData(cboStock.ListIndex), , cboStock.ItemData(cboStock.ListIndex), strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, , , , , , , , , , , , mstrPrivs)
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, _
    ...
    If Format(str灭菌失效期, "yyyy-mm-dd") < Format(zldatabase.Currentdate, "yyyy-mm-dd") And Trim(str灭菌失效期) <> "" Then
       If MsgBox("该材料已经过了灭菌失效期,是否还要领用！", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) <> vbYes Then
            Exit Function
       End If
    End If
    ...
    With mshBill
        ...
        If Not rsTemp.EOF Then
            .TextMatrix(intRow, mconintCol增值税率) = FormatEx(rsTemp!增值税率, 2)
...
Private Function SaveCard(Optional ByVal bln强制保存 As Boolean = False) As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Then   'mbln单据增加 Or
            ...
            If chrNo = "" Then chrNo = zldatabase.GetNextNo(74, lng库房ID)
...
'        For intRow = 1 To .Rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                If cboType.Text = "材料外销" Then
                    dblOutPrice = FormatEx(Val(.TextMatrix(intRow, mconintCol外销价)) / Val(.TextMatrix(intRow, mconIntCol比例系数)), g_小数位数.obj_最大小数.零售价小数)
                End If
...
'        If .TextMatrix(intRow, mconIntCol批次) > 0 Or Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1) = 1 Then '对移出库房是库房且卫材是分批核算的卫材的判断
'            If mint编辑状态 = 1 Then
'                dbltotal = 0
'                For intLop = 1 To .Rows - 1
...
'                            dbltotal = dbltotal + .TextMatrix(intLop, mconIntCol数量)
'                        End If
'                    End If
'                Next
'
...
'                    If .TextMatrix(intLop, 0) <> "" Then
'                        If intLop <> intRow And .TextMatrix(intLop, 0) = .TextMatrix(intRow, 0) And .TextMatrix(intRow, mconIntCol批次) = .TextMatrix(intLop, mconIntCol批次) Then
'                            dbltotal = dbltotal + .TextMatrix(intLop, mconIntCol冲销数量)
'                        End If
'                    End If
'                Next
'
'                If dbl填写数量 + dbltotal > dblUsableQuantity + numUsedCount Then
'                    MsgBox "你输入的数量“" & dbl填写数量 & "”大于了该卫材的可用库存数量“" & dblUsableQuantity + numUsedCount - dbltotal & "”，请重输！", vbExclamation + vbOKOnly, gstrSysName
'                    Exit Function
'                End If
'            End If
...
Private Sub printbill()
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1718", mint记录状态, mintUnit, 1718, "卫材其他出库", strNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
