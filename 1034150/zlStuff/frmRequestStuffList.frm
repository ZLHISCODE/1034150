...
Begin VB.Form frmRequestStuffList 
   ...
   Begin MSComctlLib.ImageList ilsHot 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":259C
            Key             =   "Print"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":27BC
            Key             =   "Add"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":29D8
            Key             =   "Modify"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":2BF8
            Key             =   "Delete"
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":2E18
            Key             =   "Verify"
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":3034
            Key             =   "Restore"
         EndProperty
         BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":3250
            Key             =   "Search"
         EndProperty
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
         ...
         EndProperty
         BeginProperty ListImage10 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":35C4
            Key             =   "Help"
         EndProperty
         BeginProperty ListImage11 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRequestStuffList.frx":37E4
            Key             =   "Quit"
         EndProperty
      EndProperty
   End
...
Public Sub ShowList(ByVal frmMain As Variant)
    ...
    With mshList
        .ColWidth(2) = IIf(mblnCostView = True, 900, 0) '发料部门人员是否可以看成本价
        .ColWidth(4) = IIf(mblnCostView = True, 900, 0)
    End With
...
Private Sub GetList(ByVal strFind As String)
    ...
    gstrSQL = "SELECT A.NO, C.名称 AS 发料库房,LTRIM(TO_CHAR (SUM (A.成本金额), " & mOraFMT.FM_金额 & ")) AS 成本金额, " & _
        ...
        " GROUP BY A.NO,C.名称,A.填制人,A.审核人,A.记录状态,A.配药人,A.摘要 " & _
...
Private Sub SetStrikeColor()
    ...
    With mshList
        ...
        For intRow = 1 To .Rows - 1
            intStatus = Val(.TextMatrix(intRow, .Cols - 3))
            ...
            If intStatus Mod 3 = 2 Then
                ...
                For intCol = 0 To .Cols - 1
                    .Col = intCol
                    If .TextMatrix(intRow, .Cols - 4) = "" Then
...
Private Sub SetListColWidth()
    ...
    With mshList
        ...
        .ColWidth(2) = IIf(mblnCostView = False, 0, 1000) '发料部门人员是否可以看成本价
        If mblnCostView = False Then
            .ColWidth(4) = 0 '发料部门人员是否可以看成本价
        End If
    End With
End Sub
...
Private Sub SetVisable()
    '基本，申领
    If Not IsHavePrivs(gstrPrivs, "申领") Then
...
'    End If
    ...
    lblRange.Caption = "查询范围:" & Format(zlDatabase.Currentdate, "yyyy年MM月dd日") & "至" & Format(zlDatabase.Currentdate, "yyyy年MM月dd日")
    
    mstrPrivs = gstrPrivs
    mblnCostView = IsHavePrivs(mstrPrivs, "查看成本价")
    If Not IsHavePrivs(mstrPrivs, "参数设置") Then
...
Private Sub mnuEditDel_Click()
    ...
    With mshList
        ...
        If intReturn = vbYes Then
            gstrSQL = "zl_材料移库_Delete('" & StrBillNo & "'," & Val(.TextMatrix(.Row, .Cols - 3)) & " )"
...
Private Sub mnuEditDisplay_Click()
    ...
    With mshList
        ...
        frmRequestStuffCard.ShowCard Me, StrNo, 4, .TextMatrix(.Row, .Cols - 3), mstrPrivs, , cboStock.ItemData(cboStock.ListIndex)
    End With
End Sub
...
Private Sub mnuEditDisReceive_Click()
    ...
    With mshList
        StrNo = .TextMatrix(.Row, 0)
        frmRequestStuffCard.ShowCard Me, StrNo, 7, .TextMatrix(.Row, .Cols - 3), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex), int处理方式
...
Private Sub mnuEditModify_Click()
    ...
    With mshList
        ...
        frmRequestStuffCard.ShowCard Me, StrNo, 2, mshList.TextMatrix(mshList.Row, mshList.Cols - 3), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex)
        If blnSuccess = True Then
            mnuViewRefresh_Click
        End If
    End With
...
Private Sub mnuEditReceive_Click()
    ...
    With mshList
        StrNo = .TextMatrix(.Row, 0)
        frmRequestStuffCard.ShowCard Me, StrNo, 6, .TextMatrix(.Row, .Cols - 3), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex)
    End With
    If blnSuccess = True Then
        mnuViewRefresh_Click
    End If
...
Private Sub mnuFileBillPreview_Click()
    With mshList
        If .TextMatrix(.Row, 0) = "" Then Exit Sub
        ReportOpen gcnOracle, glngSys, "zl1_bill_1722", Me, "单据编号=" & .TextMatrix(.Row, 0), "记录状态=" & .TextMatrix(.Row, .Cols - 3), "单位系数=" & mintUnit, 1
    End With
End Sub
...
Private Sub mnuFileBillPrint_Click()
    ...
    With mshList
        If .TextMatrix(.Row, 0) = "" Then Exit Sub
        If CheckPrint(.TextMatrix(.Row, 0), 19, cboStock.ItemData(cboStock.ListIndex)) = True Then
            ReportOpen gcnOracle, glngSys, "zl1_bill_1722", Me, "单据编号=" & .TextMatrix(.Row, 0), "记录状态=" & .TextMatrix(.Row, .Cols - 3), "单位系数=" & mintUnit, 2
        End If
    End With
End Sub
...
Private Sub mshlist_EnterCell()
    ...
    If mshList.Row >= 1 And LTrim(mshList.TextMatrix(mshList.Row, 0)) <> "" Then
        ...
        Set rsDetail = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, mshList.TextMatrix(mshList.Row, 0), Val(mshList.TextMatrix(mshList.Row, mshList.Cols - 3)))
        ...
        End With
        mshDetail.Redraw = True
    ElseIf LTrim(mshList.TextMatrix(mshList.Row, 0)) = "" Then
        With mshDetail
        ...
        If .TextMatrix(.Row, 0) = "" Or .Row = 0 Then         '没有单
         ...
         Else
            ...
            If .TextMatrix(.Row, .Cols - 4) = "" Then    '未审核单
                bln已发送 = (mshList.TextMatrix(mshList.Row, 10) <> "")
                If mnuEditModify.Visible = True Then
...
'    If gstrUserName = "" Then Call GetUserInfo
    ...
    Set objRow = New zlTabAppRow
    objRow.Add "移出库房：" & mshList.TextMatrix(mshList.Row, GetCol(mshList, "发料库房"))
...
Private Function IsHavePrivs(ByVal strPrivs As String, ByVal strCurPriv As String) As Boolean
    IsHavePrivs = InStr(1, ";" & strPrivs & ";", ";" & strCurPriv & ";") <> 0
End Function
