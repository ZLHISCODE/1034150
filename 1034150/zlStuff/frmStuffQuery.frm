...
Private Sub cob库房_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If cob库房.ListCount = 0 Then Call zlCtlSetFocus(Msf材料信息_S): Exit Sub
    
    If cob库房.ListIndex >= 0 Then
        If Val(cob库房.Tag) = cob库房.ItemData(cob库房.ListIndex) Then
            Call zlCtlSetFocus(Msf材料信息_S, True)
            Exit Sub
        End If
    End If
...
Private Sub showReportMXZ()
    ...
    If cob库房.ItemData(cob库房.ListIndex) = 0 Then
        Call mobjReport.ReportOpen(gcnOracle, glngSys, "ZL1_INSIDE_1721_2", Me, "材料=" & mrsData.Fields("名称").Value & "|" & mrsData.Fields("材料id").Value, "库房=所有库房|is not null", "单位=" & Choose(mbytUint, "散装单位", "包装单位") & "|" & mbytUint, "开始日期=" & Format(DateAdd("m", -1, zldatabase.Currentdate), "yyyy-MM-DD"), "结束日期=" & Format(zldatabase.Currentdate, "yyyy-MM-DD"))
    Else
        Call mobjReport.ReportOpen(gcnOracle, glngSys, "ZL1_INSIDE_1721_2", Me, "材料=" & mrsData.Fields("名称").Value & "|" & mrsData.Fields("材料id").Value, "库房=" & cob库房.Text & "|=  " & cob库房.ItemData(cob库房.ListIndex), "单位=" & Choose(mbytUint, "散装单位", "包装单位") & "|" & mbytUint, "开始日期=" & Format(DateAdd("m", -1, zldatabase.Currentdate), "yyyy-MM-DD"), "结束日期=" & Format(zldatabase.Currentdate, "yyyy-MM-DD"), "单位=" & mbytUint)    '"包含未审核单据=0| And A.审核人 Is Not NULL"
    End If
    ...
    With mrsTreeData
        ...
        Do While Not .EOF
            ...
            If IsNull(.Fields("上级id").Value) Then
                Set objNode = Me.tvwSection_S.Nodes.Add("Root", 4, "_" & .Fields("id").Value, NVL(!编码) & " -" & .Fields("名称").Value, "child")
            Else
                Set objNode = Me.tvwSection_S.Nodes.Add("_" & .Fields("上级id").Value, 4, "_" & .Fields("id").Value, NVL(!编码) & " -" & .Fields("名称").Value, "child")
            End If
            .MoveNext
         Loop
   ...
   If lngDeptId = 0 Then
    ...
    If lngDeptId = 0 Then
        strSql = "Select  M.材料ID,L.分类id,Q.编码,Q.名称 as 名称,Q.规格,Q.产地,Null as 效期,Decode(M.库房分批,1,'是','否') as 库房分批 " & gstrSQL & _
                ...
                "     (SELECT 库房id, 药品id, 批次, 效期, 性质, 可用数量, 实际数量, 实际金额, 实际差价, 上次供应商id, 上次采购价, 上次批号, 上次生产日期" & _
                "           ,上次产地, 灭菌效期, 批准文号, 零售价, 上次扣率 " & _
                "       FROM 药品库存 WHERE 性质=1) B " & _
    ...
    Else
        strSql = "Select M.材料ID,L.分类id,Q.编码,Q.名称 as 名称,Q.规格,Q.产地,nvl(M.最大效期,0) as 效期,Decode(M.库房分批,1,'是','否') as 库房分批 " & gstrSQL & _
                " From 材料特性 M,收费项目目录 Q,诊疗项目目录 L,收费价目 P,供应商 G,  " & _
                "      (Select 药品ID,上次采购价,sum(可用数量) as 可用数量,sum(实际数量) as 实际数量,sum(实际金额) as 实际金额,sum(实际差价) as 实际差价 " & _
                "       From 药品库存 Where 库房id+0=[2] And 性质=1 Group by 药品ID,上次采购价) S,(Select Distinct 收费细目id, 执行科室id From 收费执行科室 Where 执行科室id = [2]) K " & _
        ...
        If lng库房ID = 0 Or (int在用 = 1 And int分批 = 2) Or (int在用 = 0 And int分批 <> 0) Then
            ...
            If lng库房ID = 0 Then
                gstrSQL = "" & _
                    ...
                    " Group By D.名称"
            Else
               gstrSQL = "Select D.名称 as 库房,L.批号,L.效期 失效期,L.产地,Decode(sign(Add_Months(Sysdate," & mintMonths & ")-L.效期),-1,0,1) 报警," & _
                        "        S.可用数量/" & mrsData("除数").Value & " as 可用数量,S.实际数量/" & mrsData("除数").Value & " as 实际数量,S.实际金额,S.实际差价," & _
                        "        L.填制日期,L.No,S.上次采购价*" & mrsData("除数").Value & " as 最后进价,S.商品条码,S.内部条码,s.平均成本价*" & mrsData("除数").Value & "平均成本价," & _
                        "decode(c.是否变价,1,decode(s.批次,null,decode(s.实际数量,0,0,s.实际金额/s.实际数量),s.零售价 ),b.现价) * " & mrsData("除数").Value & " as 售价 , g.名称 As 供应商  " & _
                        " From 药品库存 S,部门表 D,药品收发记录 L,材料特性 A,收费价目 B,收费项目目录 C, 供应商 G " & _
                        " Where s.上次供应商id = g.Id(+) And S.库房id=D.id And S.批次=L.id(+) And S.药品id=A.材料id" & _
...
Private Sub ShowBill(frmObject As Object, strNo As String, int记录状态 As Integer, int单据 As Integer, Optional bln在用 As Boolean = False)
    ...
    Select Case int单据
        ...
        Case 13
            ...
            Call frmStuffPrice.ShowBill(frmObject, B_查阅, Val(NVL(rsTemp!价格id)), 0)
'            With frmStuffPrice
...
Private Sub DataBound()
    ...
    With Msf材料信息_S
        ...
        Do While Not mrsData.EOF
            ...
            .TextMatrix(.Row, 4) = zlCommFun.NVL(mrsData!规格, "")
            .TextMatrix(.Row, 5) = zlCommFun.NVL(mrsData!产地, "")
            .TextMatrix(.Row, 6) = zlCommFun.NVL(mrsData!效期, "")
            .TextMatrix(.Row, 7) = zlCommFun.NVL(mrsData!库房分批, "否")
            .TextMatrix(.Row, 8) = zlCommFun.NVL(mrsData!单位, "")
            ...
            .TextMatrix(.Row, 16) = zlCommFun.NVL(mrsData!撤档时间, "")
            .TextMatrix(.Row, 17) = zlCommFun.NVL(mrsData!除数, 1)
            .TextMatrix(.Row, 18) = zlCommFun.NVL(mrsData!供应商, "")
            '上色
            If mbln包含停用 Then
                lngColor = IIf(Trim(.TextMatrix(.Row, 16)) = "", MLNG黑色, MLNG红色)
