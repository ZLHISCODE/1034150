...
Private Function GetDeptRequestDataBill(ByVal strNOIn As String, ByVal lng库房ID As Long, _
    ...
    If str分类IDIN <> "" Then
          strSQL = "" & _
                  "   Select  /*+ Rule*/ A.材料id,('['|| q.编码 || ']' || q.名称) as 材料信息,b.招标材料,q.规格,nvl(max(A.上次生产商),max(q.产地)) as 产地,max(A.上次供应商) as 上次供应商," & _
                  ...
                  "         And (C.审核日期 between [4] and [5]) And C.No =L.Column_Value" & _
      ...
      Else
          strSQL = "" & _
                  "   Select /*+ Rule*/ A.材料id, ('['|| q.编码 || ']' || q.名称) as 材料信息,B.招标材料,q.规格 ,nvl(max(A.上次生产商),q.产地) as 产地,max(A.上次供应商) as 上次供应商," & _
                  "      sum(nvl(A.请购数量,0)) as 请购数量," & _
                  "      sum(nvl(A.计划数量,0)) as 审批数量," & _
                         IIf(mintUnit = 0, "Q.计算单位", "B.包装单位") & " as 单位, " & _
                         IIf(mintUnit = 0, "1", "B.换算系数") & " as 换算系数, " & _
    ...
    strSQL = "" & _
    "   Select  A.材料ID,A.材料信息,A.规格,A.招标材料,nvl(max(a.产地),a.产地) as 产地,nvl(max(上次供应商),A.上次供应商) as 上次供应商," & _
    "           A.请购数量,A.审批数量,A.单位,A.换算系数," & _
    "           nvl(max(b.上次采购价),a.单价)  as 单价," & _
    "           sum(nvl(B.实际数量,0)) as 库存数量" & _
    "   From (" & strSQL & ") A,药品库存 B,供应商 C" & _
    "   Where a.材料id=b.药品id(+) and nvl(B.上次供应商id,0)=C.id(+) and b.性质(+)=1  " & IIf(lng库房ID = 0, "", " And B.库房id(+)=[3]") & _
    ...
    With rsplan
        ...
        For intLop = 1 To .RecordCount
            mshBill.TextMatrix(intLop, 0) = Val(NVL(!材料ID))
            mshBill.TextMatrix(intLop, mHeadCol.材料) = NVL(!材料信息)
            mshBill.TextMatrix(intLop, mHeadCol.规格) = NVL(!规格)
            mshBill.TextMatrix(intLop, mHeadCol.产地) = NVL(!产地)
            mshBill.TextMatrix(intLop, mHeadCol.单位) = NVL(!单位)
            mshBill.TextMatrix(intLop, mHeadCol.前期数量) = ""
            mshBill.TextMatrix(intLop, mHeadCol.上期数量) = ""
            mshBill.TextMatrix(intLop, mHeadCol.库存数量) = Format(Val(NVL(!库存数量)) / Val(NVL(!换算系数)), mFMT.FM_数量)
            mshBill.TextMatrix(intLop, mHeadCol.计划数量) = Format(Val(NVL(!审批数量)) / Val(NVL(!换算系数)), mFMT.FM_数量)
            mshBill.TextMatrix(intLop, mHeadCol.单价) = Format(Val(NVL(!单价)) * Val(NVL(!换算系数)), mFMT.FM_成本价)
            mshBill.TextMatrix(intLop, mHeadCol.金额) = Format(Val(NVL(!单价)) * Val(NVL(!审批数量)), mFMT.FM_金额)
            mshBill.TextMatrix(intLop, mHeadCol.比例系数) = NVL(!换算系数)
            mshBill.TextMatrix(intLop, mHeadCol.上次供应商) = NVL(!上次供应商)
            mshBill.TextMatrix(intLop, mHeadCol.中标材料) = NVL(!招标材料)
            
            Call Calc销量(Val(NVL(!材料ID)), intLop)
    ...
    If mint编辑状态 = 1 Then
        ...
        If frmStuffPlanCondition.GetCondition(mfrmMain, str分类ID, lng库房ID, int计划类型, int编制方法, mbln下限, mint上限, mint下限, mstr供货商ID, mbln中标单位, mbln计划数量) = True Then
            ...
            Select Case mint计划类型
                Case 1       '月计划
                    mstr期间 = Format(DateAdd("m", 1, zlDatabase.Currentdate), "yyyyMM")
...
'                    mshBill.TextMatrix(0, mHeadCol.本期销量) = "本月销量"
                Case 2       '季计划
                    ...
                    mstr期间 = Format(DateAdd("Q", 1, zlDatabase.Currentdate), "yyyy") & IIf(intMonth <= 3, 1, IIf(intMonth >= 10, 4, IIf(intMonth <= 9 And intMonth >= 7, 3, 2)))
...
'                    mshBill.TextMatrix(0, mHeadCol.本期销量) = "本季度销量"
                Case 3    '年计划
                    mstr期间 = Format(DateAdd("yyyy", 1, zlDatabase.Currentdate), "yyyy")
    ...
    With rsAllStuff
        ...
        Do While Not .EOF
            ...
            If mstr供货商ID = "" Then
                blnOK = True
            Else
                If Val(NVL(!上次供应商id)) = 0 And mbln中标单位 Then
                    ...
                    Set rspurchase = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(NVL(!药品ID)), mstr供货商ID, gstrNodeNo)
                    If rspurchase.RecordCount <> 0 Then
                        blnOK = True
                        str上次供应商 = NVL(rspurchase!名称)
                    End If
                Else
                     If "," & mstr供货商ID & "," Like "*," & Val(NVL(!上次供应商id)) & ",*" Then
                         blnOK = True
                     End If
                End If
            End If
            If blnOK Then
                    ...
                    mshBill.TextMatrix(lngRow, mHeadCol.单价) = Format(Val(NVL(!单价)) * NVL(!比例系数, 1), mFMT.FM_成本价)
                    
                    mshBill.TextMatrix(lngRow, mHeadCol.上次供应商) = IIf(IsNull(!供应商), str上次供应商, !供应商)
                    mshBill.TextMatrix(lngRow, mHeadCol.库存数量) = Format(Val(NVL(!库存数量)), mFMT.FM_数量)
                    mshBill.TextMatrix(lngRow, mHeadCol.比例系数) = NVL(!比例系数, 1)
                    mshBill.TextMatrix(lngRow, mHeadCol.中标材料) = IIf(NVL(!招标材料) = 1, "√", "")
                    
                    mshBill.TextMatrix(lngRow, mHeadCol.存储上限) = Format(Val(NVL(!上限)), mFMT.FM_数量)
                    mshBill.TextMatrix(lngRow, mHeadCol.存储下限) = Format(Val(NVL(!下限)), mFMT.FM_数量)
                    
                    SetNumer !药品ID, lng库房ID, Val(NVL(!库存数量)), lngRow, int计划类型, int编制方法
...
Private Sub SetNumer(ByVal lng药品id As Long, ByVal lng库房ID As Long, _
    ...
    With mshBill
        Select Case int编制方法
            ...
            Case 2      '临近期间平均参照法
                dat前期 = Choose(int计划类型, DateAdd("m", -2, zlDatabase.Currentdate), DateAdd("m", -6, zlDatabase.Currentdate), DateAdd("yyyy", -2, zlDatabase.Currentdate), DateAdd("d", -14, sys.Currentdate))
                dat上期 = Choose(int计划类型, DateAdd("m", -1, zlDatabase.Currentdate), DateAdd("m", -3, zlDatabase.Currentdate), DateAdd("yyyy", -1, zlDatabase.Currentdate), DateAdd("d", -7, sys.Currentdate))
...
Private Sub Calc销量(ByVal lng药品id As Long, ByVal intCurrentRow As Integer)
    ...
    With mshBill
        ...
        Set rsNum = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, lng药品id, CDate(strBegin), CDate(strEnd))
        If rsNum.RecordCount > 0 Then
            .TextMatrix(intCurrentRow, mHeadCol.本期销量) = Format(NVL(rsNum!销售数量, 0) / Val(.TextMatrix(intCurrentRow, mHeadCol.比例系数)), mFMT.FM_数量)
        End If
    End With
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            If rsInitCard.EOF Then
                ...
                Exit Sub
            End If
            mlng库房id = Val(NVL(rsInitCard!库房ID))
...
'                End Select
                For intRow = 1 To intRecordCount
                    ...
                    .TextMatrix(intRow, mHeadCol.中标材料) = IIf(Val(NVL(rsInitCard!招标材料)) = 1, "√", "")
                    .TextMatrix(intRow, mHeadCol.存储上限) = Format(Val(NVL(rsInitCard!上限)) / rsInitCard!比例系数, mFMT.FM_数量)
                    .TextMatrix(intRow, mHeadCol.存储下限) = Format(Val(NVL(rsInitCard!下限)) / rsInitCard!比例系数, mFMT.FM_数量)
                    .TextMatrix(intRow, mHeadCol.前期数量) = Format(Val(NVL(rsInitCard!前期数量)) / rsInitCard!比例系数, mFMT.FM_数量)
                    .TextMatrix(intRow, mHeadCol.上期数量) = Format(Val(NVL(rsInitCard!上期数量)) / rsInitCard!比例系数, mFMT.FM_数量)
                    .TextMatrix(intRow, mHeadCol.库存数量) = Format(Val(NVL(rsInitCard!库存数量)) / rsInitCard!比例系数, mFMT.FM_数量)
                    
                    .TextMatrix(intRow, mHeadCol.上期销量) = Format(Val(NVL(rsInitCard!上期销量)) / rsInitCard!比例系数, mFMT.FM_数量)
                    .TextMatrix(intRow, mHeadCol.本期销量) = Format(Val(NVL(rsInitCard!本期销量)) / rsInitCard!比例系数, mFMT.FM_数量)
                    
                    .TextMatrix(intRow, mHeadCol.计划数量) = IIf(Format(Val(NVL(rsInitCard!计划数量)), mFMT.FM_数量) = 0, "", Format(rsInitCard!计划数量 / rsInitCard!比例系数, mFMT.FM_数量))
                    .TextMatrix(intRow, mHeadCol.单价) = Format(Val(NVL(rsInitCard!单价)) * rsInitCard!比例系数, mFMT.FM_成本价)
                    .TextMatrix(intRow, mHeadCol.金额) = IIf(Format(Val(NVL(rsInitCard!金额)), mFMT.FM_金额) = 0, "", Format(Val(NVL(rsInitCard!金额)), mFMT.FM_金额))
...
Private Sub mshbill_CommandClick()
    ...
    If mshBill.Col = mHeadCol.材料 Then
        Set RecReturn = Frm材料选择器.ShowMe(Me, 1, , mlng库房id, , , , , , , , , , , , , , mstrPrivs)
        ...
        Select Case .Col
            ...
            Case mHeadCol.材料
                If strKey <> "" Then
                    ...
                    Set rsTemp = FrmMulitSel.ShowSelect(Me, 1, , mlng库房id, , strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, , , , , , , , , , , , mstrPrivs)
...
Private Function SaveCard() As Boolean
    ...
    SaveCard = False
    With mshBill
        ID_IN = zlDatabase.GetNextId("材料采购计划")
        NO_IN = Trim(txtNO)
        
        If NO_IN = "" Then NO_IN = zlDatabase.GetNextNo(77, mlng库房id)
...
Private Function SaveCheckCard() As Boolean
    ...
    With mshBill
        ...
        编制日期_IN = IIf(Txt填制日期.Caption <> "", Format(Txt填制日期.Caption, "yyyy-mm-dd hh:mm:ss"), Format(zlDatabase.Currentdate, "yyyy-mm-dd hh:mm:ss"))
        编制说明_IN = Trim(txt摘要.Text)
        期间_IN = mstr期间
...
Private Function SetStuffRows(ByVal lng材料ID As Long, ByVal str材料 As String, _
        ...
        For intCount = 1 To .Rows - 1
            If intCount <> intRow And .TextMatrix(intCount, 0) <> "" Then
                If .TextMatrix(intCount, 0) = lng材料ID Then
                    MsgBox "对不起，该卫生材料已有了，不能再输！", vbOKOnly + vbExclamation, gstrSysName
                    Exit Function
                End If
            End If
        Next
    ...
    With mshBill
        ...
        dbl成本价 = NVL(rsData!成本价, 0)
        If dbl成本价 = 0 Then dbl成本价 = NVL(rsData!指导批发价, 0)
        .TextMatrix(.Row, mHeadCol.中标材料) = IIf(Val(NVL(rsData!招标材料)) = "1", "√", "")
        ...
        If NVL(rsData!库存数量, 0) = 0 And NVL(rsData!供应商) = "" And NVL(rsData!上次产地) = "" Then
        ...
        If Not rsData.EOF Then
            .TextMatrix(.Row, mHeadCol.存储上限) = Format(Val(NVL(rsData!上限)) / dbl比例系数, mFMT.FM_数量)
            .TextMatrix(.Row, mHeadCol.存储下限) = Format(Val(NVL(rsData!下限)) / dbl比例系数, mFMT.FM_数量)
        End If
