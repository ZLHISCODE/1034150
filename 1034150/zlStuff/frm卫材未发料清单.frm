...
Private Function SaveChargeOffData(ByVal strDate As String) As Boolean
    ...
    If bln是否有退料 = True Then
        If IsHavePrivs(mstrPrivs, "退料通知单") Then
            intMsg = Val(zlDatabase.GetPara("退料打印提醒方式", glngSys, mlngModule, "0"))
            
            If intMsg = 0 Then
                '提示打印
                If MsgBox("你需要打印相关单据吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
    ...
    If Not mobjPlugIn Is Nothing And bln是否有退料 Then
        mobjPlugIn.StuffReturnByID Val(mArrFilter("发料部门id")), strReturnInfo, CDate(strDate), strReserve
    End If
...
Private Function GetNext汇总标识号() As String
    ...
    GetNext汇总标识号 = zlDatabase.GetNextNo(20)
    Exit Function
End Function
...
Private Sub Form_Load()
    ...
    mstr默认单据格式 = Trim(zlDatabase.GetPara("发料单据打印格式", glngSys, mlngModule, , Array(cboEdit(mcboIdx.idx_单据格式)), IsHavePrivs(mstrPrivs, "参数设置")))
...
Private Function WhiteDataToRecord(ByVal rsSource As ADODB.Recordset) As Boolean
    ...
    With rsSource
        ...
        Do While Not .EOF
            ...
            mrsNotPayStuff!单量 = IIf(IsNull(!单量), "", FormatEx(!单量, 5) & NVL(!计算单位))
...
Private Function FullDataToVsGrid() As Boolean
    ...
    With vsGrid
        ...
        Do While Not mrsNotPayStuff.EOF
            ...
            .TextMatrix(lngRow, .ColIndex("状态")) = IIf(IsHavePrivs(mstrPrivs, "卫生材料发料") Or IsHavePrivs(mstrPrivs, "卫生材料拒发"), NVL(mrsNotPayStuff!状态), "")
...
Private Function RefreshData() As Boolean
    ...
    ElseIf Val(mArrFilter("请求类型")) = 2 Then
        ...
        gstrSQL = Replace(gstrSQL, "And Nvl(c.费用状态,0)<>1", " and r.病人id=c.病人id and r.主页id=c.主页id " & IIf(Trim(mArrFilter("床号")) = "", "", "   AND c.床号 =[1] "))
    End If
...
Private Sub vsGrid_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    With vsGrid
        Select Case .Col
        Case .ColIndex("状态")
                  If IsHavePrivs(mstrPrivs, "卫生材料发料") = False And IsHavePrivs(mstrPrivs, "卫生材料拒发") = False Then
                        Cancel = True
                        Exit Sub
                  End If
...
Private Sub vsGrid_DblClick()
    Dim str状态 As String
    If IsHavePrivs(mstrPrivs, "卫生材料发料") = False And IsHavePrivs(mstrPrivs, "卫生材料拒发") = False Then Exit Sub
    With vsGrid
'         If .Row = 1 And .IsSubtotal(.Row) = True Then
'            Exit Sub
'        End If
...
Private Sub BillListPrint(Optional strDate As String = "", Optional IntStyle As Integer = 0)
    ...
    bln退料单 = IsHavePrivs(mstrPrivs, "退料通知单")
    bln已发料清单 = IsHavePrivs(mstrPrivs, "打印已发料清单")
    bln单据打印 = IsHavePrivs(mstrPrivs, "单据打印")
    If intMsg = 0 Then
        '提示打印
