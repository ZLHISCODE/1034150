...
Private Sub cmdDraw_Click()
    ...
    On Error GoTo ErrHandle
    vRect = GetControlRect(txtDraw.hwnd)
    ...
    End If
    blnClear = False
    If Val(txtDraw.Tag) <> Val(NVL(rsTemp!Id)) And Val(txtDraw.Tag) <> 0 Then
            '需要检查是否已经有设置好的跟踪部分信息
            With mshBill
                For i = 1 To .Rows - 1
    ...
    Me.txtDraw = NVL(rsTemp!编码) & "-" & NVL(rsTemp!名称)
    Me.txtDraw.Tag = NVL(rsTemp!Id)
    
    gstrSQL = "Select 工作性质, 部门id, 服务对象 From 部门性质说明 Where 部门id=[1] And 工作性质='护理'"
    Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(NVL(rsTemp!Id)))
    If rsTemp.EOF Then
...
Private Sub initCard()
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4, 5, 6, 7
            ...
            txtDrawPerson.Text = NVL(rsTemp!领用人)
            txtDrawPerson.Tag = NVL(rsTemp!领用人)
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    .TextMatrix(intRow, mBillCol.c_原始数量) = Val(NVL(rsTemp!原始数量))
                    ...
                    .TextMatrix(intRow, mBillCol.C_跟踪标志) = NVL(rsTemp!跟踪病人)
                    .TextMatrix(intRow, mBillCol.C_分批属性) = Check分批属性(intRow, rsTemp!在用分批, rsTemp!库房分批)
                    If mint编辑状态 <> 7 Then
                        '病人ID|使用时间|条码
                        .TextMatrix(intRow, mBillCol.C_跟踪信息) = NVL(rsTemp!病人ID) & "|" & IIf(IsNull(rsTemp!使用时间), "", Format(rsTemp!使用时间, "yyyy-mm-dd")) & "|" & NVL(rsTemp!条码)
                        .TextMatrix(intRow, mBillCol.C_跟踪病人) = NVL(rsTemp!姓名)
                    End If
...
Private Sub mshbill_CommandClick()
    ...
    Case Else
            Set RecReturn = Frm材料选择器.ShowMe(Me, 2, cboStock.ItemData(cboStock.ListIndex), _
                                ...
                                IIf(mint领用明确批次 = 1, True, False), , , , , , , , , , mlngModule, , mstrPrivs, IIf(mint领用明确批次 = 1, True, False))
...
Private Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, _
    ...
    On Error GoTo ErrHandle
    SetColValue = False
    If Format(str灭菌失效期, "yyyy-mm-dd") < Format(zldatabase.Currentdate, "yyyy-mm-dd") And Trim(str灭菌失效期) <> "" Then
       If MsgBox("该卫材已经过了灭菌失效期,是否还要领用！", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) <> vbYes Then
            Exit Function
       End If
    End If
...
Private Function SaveCard(Optional ByVal bln强制保存 As Boolean = False) As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Or mint编辑状态 = 7 Then 'mbln单据增加 Or
            ...
            If chrNo = "" Then chrNo = zldatabase.GetNextNo(73, lng库房ID)
...
Private Sub txt摘要_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        End If
        strKey = Trim(txt摘要)
        If txt摘要.Tag <> "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
...
Private Function CompareUsableQuantity(ByVal intRow As Integer, ByVal dbl填写数量 As Double) As Boolean
    ...
    With mshBill
        ...
        ElseIf mint编辑状态 = 2 Then
            ...
            End If
        ElseIf mint编辑状态 = 3 Or mint编辑状态 = 5 Then '审核和核查可以修改实际数量
            If gSystem_Para.para_卫材填单下可用库存 = False Then
                '如果没有预减可用数量，则不算界面的原始数量
                dblUsableQuantity = Val(Format(.TextMatrix(intRow, mBillCol.C_可用数量), mFMT.FM_数量))
            Else
...
'        If .TextMatrix(intRow, mBillCol.c_批次) > 0 Or Split(.TextMatrix(intRow, mBillCol.C_指导差价率), "||")(1) = 1 Then '对移出库房是库房且卫材是分批核算的卫材的判断
'            dbltotal = 0
'            For intLop = 1 To .Rows - 1
'                If .TextMatrix(intLop, 0) <> "" Then
'                    If intLop <> intRow And .TextMatrix(intLop, 0) = .TextMatrix(intRow, 0) And .TextMatrix(intRow, mBillCol.c_批次) = .TextMatrix(intLop, mBillCol.c_批次) Then
'                        dbltotal = dbltotal + .TextMatrix(intLop, mBillCol.C_填写数量)
'                    End If
'                End If
'            Next
'            If mint编辑状态 = 6 Then
'                If Abs(dbl填写数量 + dbltotal) > dblUsableQuantity Then
'                    MsgBox "你输入的数量“" & Abs(dbl填写数量) & "”大于了该卫材的实际库存数量“" & dblUsableQuantity - dbltotal & "”，请重输！", vbExclamation + vbOKOnly, gstrSysName
'                    Exit Function
'                End If
'            Else
'                If dbl填写数量 + dbltotal > dblUsableQuantity Then
'                    MsgBox "你输入的数量“" & dbl填写数量 & "”大于了该卫材的可用库存数量“" & dblUsableQuantity - dbltotal & "”，请重输！", vbExclamation + vbOKOnly, gstrSysName
'                    Exit Function
'                End If
'            End If
'        End If
        
        ' 对移出库房是库房且卫材是分批核算的卫材以外的判断
        '加ABS是考虑可以负数冲销情况
        If mint库存检查 = 0 Then
            '0-不检查
        ...
        ElseIf mint库存检查 = 2 Then
            '2-检查，不足禁止
            If IIf(mint编辑状态 = 6, Abs(dbl填写数量), dbl填写数量) > dblUsableQuantity Then
                MsgBox "你输入的数量“" & IIf(mint编辑状态 = 6, Abs(dbl填写数量), dbl填写数量) & "”大于了该卫材的" & IIf(mint编辑状态 = 6, "实际", "可用") & "库存数量“" & dblUsableQuantity + numUsedCount & "”，请重输！", vbExclamation + vbOKOnly, gstrSysName
                Exit Function
            End If
        End If
    End With
...
Private Sub printbill()
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1717", mint记录状态, mintUnit, 1717, "卫材领用单", strNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
