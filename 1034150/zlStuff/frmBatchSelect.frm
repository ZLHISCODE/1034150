...
Public Sub ShowMe(ByVal FrmParent As Form, ByRef rsTemp As ADODB.Recordset, ByRef blnOK As Boolean, ByRef intType As Integer, ByRef dbl比率 As Double, Optional int调价 As Integer)
    mint调价 = int调价
...
'            dbl换算系数 = Val(.TextMatrix(intRow, vsfSelectDrugCol.单位系数))
...
Private Sub vsfDrug_DblClick()
    ...
    With vsfDrug
        ...
        gstrSQL = "Select Distinct a.材料id, c.编码 As 材料编码, c.名称 As 通用名, d.商品名, c.规格, c.是否变价 As 时价, c.产地, c.计算单位,a.换算系数, a.包装单位," & _
                                    " a.成本价, e.现价, a.指导批发价, a.指导零售价,a.跟踪在用" & _
                    " From 材料特性 A, 诊疗项目目录 B, 收费项目目录 C, (Select 名称 As 商品名, 收费细目id From 收费项目别名 Where 性质 = 3) D,收费价目 E" & _
                    " Where a.诊疗id = b.Id And a.材料id = c.Id And c.Id = d.收费细目id(+) and a.材料id=e.收费细目id and sysdate between e.执行日期 and e.终止日期 and b.id=[1] " & _
