...
Begin VB.Form frmStuffSpec 
   ...
   Begin TabDlg.SSTab stbSpec 
      ...
      Tab(1).Control(0)=   "lbl(19)"
      Tab(1).Control(1)=   "lbl(18)"
      Tab(1).Control(2)=   "lbl(20)"
      Tab(1).Control(3)=   "lbl病案费目"
      Tab(1).Control(4)=   "txt病案费目"
      Tab(1).Control(5)=   "fra(2)"
      Tab(1).Control(6)=   "cbo收入项目"
      Tab(1).Control(7)=   "cbo费用类型"
      Tab(1).Control(8)=   "cbo服务对象"
      Tab(1).Control(9)=   "chk屏蔽费别"
      Tab(1).Control(10)=   "fra分批核算"
      Tab(1).Control(11)=   "cmd病案"
...
Private Function ISValied() As Boolean
    ...
    For i = 0 To txtEdit.UBound
        ...
        Case "指导批价", "指导售价"
            ...
            End If
'        Case "拼音简码"
'            Me.txtEdit(i).Text = zlGetSymbol(Me.txtEdit(GetTxtIdx("商品名")).Text, 0)
'        Case "五笔简码"
'            Me.txtEdit(i).Text = zlGetSymbol(Me.txtEdit(GetTxtIdx("商品名")).Text, 1)
        Case Else
...
Private Function SaveData() As Boolean
    ...
    If mintEditType = g新增 Then
        lng材料ID = zldatabase.GetNextId("收费项目目录")
...
Private Function GetMaxCode() As String
    ...
    If intCodeType = 0 Then
        '取最大编码
        gstrSQL = "select nvl(max(a.编码),'00000000000000') as 编码" & _
                " From 收费项目目录 a,材料特性 B" & _
                " Where a.类别 = [1] and a.id=b.材料id  "       'and b.诊疗id=[2]
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, "4", mlng诊疗ID)
        With rsTemp
            GetMaxCode = Right(String(mintCodeLength, "0") & Val(!编码) + 1, Len(!编码))
        End With
        Exit Function
    End If
    ...
    gstrSQL = "select nvl(max(编码),'00000000000000') as 编码" & _
            " From 收费项目目录" & _
            " Where 类别 = '4' and 编码 like [1]"
    ...
    With rsTemp
        If intCodeType = 1 Then
            If Len(!编码) - 1 - Len(strTemp) < 0 Then
                GetMaxCode = "4" & strTemp & Right(String(mintCodeLength, "0") & Val(!编码) + 1, 0)
            Else
                GetMaxCode = "4" & strTemp & Right(String(mintCodeLength, "0") & Val(!编码) + 1, Len(!编码) - 1 - Len(strTemp))
            End If
        Else
            If Len(!编码) - Len(strTemp) < 0 Then
                GetMaxCode = strTemp & Right(String(mintCodeLength, "0") & Val(!编码) + 1, 0)
            Else
                GetMaxCode = strTemp & Right(String(mintCodeLength, "0") & Val(!编码) + 1, Len(!编码) - Len(strTemp))
            End If
        End If
...
Private Function InitCardData(Optional bln单位 As Boolean = True) As Boolean
    ...
    If bln单位 Then
        ...
        With rsTemp
            ...
            str计算单位 = ""
            Do While Not .EOF
                If str计算单位 <> NVL(!计算单位) Then
                    str计算单位 = NVL(!计算单位)
                    cbo单位(0).AddItem NVL(!计算单位)
                End If
                .MoveNext
            Loop
            .Sort = "包装单位"
            str计算单位 = ""
            Do While Not .EOF
                If str计算单位 <> NVL(!包装单位) Then
                    str计算单位 = NVL(!包装单位)
                    cbo单位(1).AddItem NVL(!包装单位)
                End If
                .MoveNext
            Loop
        End With
   ...
   If Not rsTemp.EOF Then
        ...
        With cmbStationNo
            For i = 1 To .ListCount - 1
                If Mid(cmbStationNo.List(i), 1, InStr(1, cmbStationNo.List(i), "-") - 1) = NVL(rsTemp!站点) Then
        ...
        lbl品种说明.Caption = "品种信息：[" & NVL(rsTemp!编码) & "] " & NVL(rsTemp!名称)
        str计算单位 = NVL(rsTemp!计算单位)
        gstrSQL = "Select 名称 From 诊疗项目别名 where 诊疗项目id=[1] and 性质=2"
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlng诊疗ID)
        If rsTemp.RecordCount = 0 Then
            lbl品种说明.Caption = lbl品种说明.Caption & Space(8) & "英文名称:"
        Else
            lbl品种说明.Caption = lbl品种说明.Caption & Space(8) & "英文名称:" & NVL(rsTemp!名称)
        End If
        For i = 0 To cbo单位(0).ListCount
   ...
   If mintEditType = g新增 Then
        ...
        With cbo单位(0)
            ...
            End If
        End With
        dtp许可证效期.Value = zldatabase.Currentdate
        dtp许可证效期.Value = ""
        dtp注册证有效期.Value = zldatabase.Currentdate
        dtp注册证有效期.Value = ""
   ...
   '其他需读取卡片数据
    ...
    gstrSQL = "select I.编码 as 规格编码,I.名称,I.规格,I.产地 as 生产商,S.货源情况,S.材料来源, " & _
             ...
             "        S.跟踪在用,S.核算材料,S.许可证号,S.许可证有效期,S.材质分类,S.存储条件,S.跟踪病人,I.站点,S.增值税率,I.说明,S.是否植入耗材 " & _
    ...
    With rsTemp
        ...
        txtEdit(0).MaxLength = mintCodeLength
        If .RecordCount > 0 Then
            txt批准文号.Text = NVL(!批准文号)
            txt注册商标.Text = NVL(!注册商标)
            txt注册证号.Text = NVL(!注册证号)
            txt备选码.Text = NVL(!备选码)
            For i = 0 To txtEdit.UBound
                ...
                Case "指导批价", "成本价格"
                    txtEdit(i).Text = Format(NVL(.Fields(strFieldsName), 0) * IIf(mintUnit = 0, 1, NVL(!换算系数, 1)), mFMT.FM_成本价)
                Case "指导售价"
                    txtEdit(i).Text = Format(NVL(.Fields(strFieldsName), 0) * IIf(mintUnit = 0, 1, NVL(!换算系数, 1)), mFMT.FM_零售价)
                Case "采购扣率", "指导差价率", "差价让利", "加成率"
                        txtEdit(i).Text = Format(NVL(.Fields(strFieldsName), 0), GFM_VBCJL)
                Case "拼音", "五笔", "结算价", "当前售价"
                Case "许可证号"
                    txtEdit(i).Text = NVL(!许可证号)
                Case "许可证有效期"
                Case "增值税率"
                    txtEdit(i).Text = Format(NVL(.Fields(strFieldsName), 0), GFM_VBJCL)
                Case Else
                    '“商品名,拼音简码,五笔简码”单独处理
                    If InStr("商品名;拼音简码;五笔简码", strFieldsName) = 0 Then txtEdit(i).Text = NVL(.Fields(strFieldsName))
                End Select
            Next
            chk跟踪病人.Value = IIf(Val(NVL(!跟踪病人)) = 1, 1, 0)
            chk核算材料.Value = IIf(Val(NVL(!核算材料)) = 1, 1, 0)
            ...
            For mintCount = 0 To Me.cbo单位(0).ListCount - 1
                If cbo单位(0).List(mintCount) = NVL(!计算单位) Then
            ...
            For mintCount = 0 To Me.cbo单位(1).ListCount - 1
                If cbo单位(1).List(mintCount) = NVL(!包装单位) Then
            ...
            For mintCount = 0 To Me.cbo货源.ListCount - 1
                If Mid(Me.cbo货源.List(mintCount), InStr(1, Me.cbo货源.List(mintCount), "-") + 1) = NVL(!货源情况) Then
            ...
            For mintCount = 0 To Me.cbo材料来源.ListCount - 1
                If Mid(Me.cbo材料来源.List(mintCount), InStr(1, Me.cbo材料来源.List(mintCount), "-") + 1) = NVL(!材料来源) Then
            ...
            For mintCount = 0 To Me.cbo材质分类.ListCount - 1
                If Mid(Me.cbo材质分类.List(mintCount), InStr(1, Me.cbo材质分类.List(mintCount), "-") + 1) = NVL(!材质分类) Then
            ...
            For mintCount = 0 To Me.cbo存储条件.ListCount - 1
                If Mid(Me.cbo存储条件.List(mintCount), InStr(1, Me.cbo存储条件.List(mintCount), "-") + 1) = NVL(!存储条件) Then
          ...
          '时价
            For mintCount = 0 To Me.cbo价格属性.ListCount - 1
                If cbo价格属性.ItemData(mintCount) = NVL(!是否变价, 0) Then
                    cbo价格属性.ListIndex = mintCount
                    Exit For
                End If
            Next
            
            lbl批价单位(0).Caption = "元/" & IIf(mintUnit = 0, NVL(!计算单位), NVL(!包装单位))
            lbl批价单位(1).Caption = "元/" & IIf(mintUnit = 0, NVL(!计算单位), NVL(!包装单位))
            
            cbo价格属性.ListIndex = IIf(IsNull(!是否变价), 0, !是否变价)
            
            Chk一次性材料.Value = NVL(!一次性材料, 0): Call Chk一次性材料_Click
            chk无菌性材料.Value = NVL(!无菌性材料, 0)
            chk原料.Value = NVL(!原材料, 0)
            ...
            Me.chk库房.Value = NVL(!库房分批, 0)
            Me.chk在用.Value = NVL(!在用分批, 0)
            Me.chk跟踪.Value = NVL(!跟踪在用, 0)
            Me.chkCostly.Value = NVL(!高值材料, 0)
            Me.chkCode.Value = NVL(!是否条码管理, 0)
            If mblnInStrument = True Then
                Me.chkInstrument.Value = NVL(!器械包卫材单件, 0)
            End If
   ...
   End With
    ...
    If Me.cbo价格属性.ListIndex <> 0 Then
        ...
        gstrSQL = "select Decode(K.库存数量,0,P.现价,K.库存金额/Nvl(K.库存数量,1)) as 现价,P.收入项目id" & _
                ...
                " where P.收费细目id=[1] and (Sysdate Between p.执行日期 And p.终止日期 or Sysdate>=p.执行日期 And p.终止日期 Is Null)"
    Else
        '非时价材料调价，取其价格记录中的价格
    ...
    If Val(mstr材料ID) <> 0 Then
        '--较证执行数据
        gstrSQL = "Select ID from 收费价目 where 收费细目id=[1] and nvl(变动原因,0)=0"
        
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(mstr材料ID))
        
        Do While Not rsTemp.EOF
                gstrSQL = "zl_材料收发记录_Adjust(" & Val(NVL(rsTemp!Id)) & ")"
...
'            Me.cbo收入项目.Enabled = cbo价格属性.Enabled
    ...
    Else
        gstrSQL = "Select 1 " & _
            ...
            " Where Rownum < 2 "
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(mstr材料ID))
        Me.chk跟踪.Enabled = rsTemp.EOF
    End If
    
    If Val(mstr材料ID) <> 0 Then
        '如果存在未执行的价格,则不充许修改相关价格
        gstrSQL = "Select 现价 from 收费价目 where 收费细目id=[1] and nvl(变动原因,0)=0"
...
Private Sub SetPopedom()
    ...
    chkCode.Visible = IsHavePrivs(mstrPrivs, "设置条码管理")
...
Private Sub LoadReg()
    ...
    blnHavePriv = IsHavePrivs(mstrPrivs, "参数设置") And IsHavePrivs(mstrPrivs, "指导价格管理")
