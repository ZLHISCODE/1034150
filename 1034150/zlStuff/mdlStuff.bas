...
Public Function CheckPrint(ByVal strNo As String, ByVal int单据 As Integer, lng库房ID As Long) As Boolean
    '检查是否已经打印，并返回是否继续打印，继续则返回true，否则返回false
    Dim rsTemp As ADODB.Recordset
    
    gstrSQL = "Select 1 From 药品收发主表 Where NO = [1] And 单据 = [2] And 库房id = [3]"
    Set rsTemp = zlDataBase.OpenSQLRecord(gstrSQL, "检查是否已经打印单据", strNo, int单据, lng库房ID)
    If rsTemp.RecordCount > 0 Then
