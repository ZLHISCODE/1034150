        ...
        Case 2, 3, 4
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol一次性材料) = NVL(rsTemp!一次性材料)
                    .TextMatrix(intRow, mconIntCol灭菌效期) = NVL(rsTemp!灭菌效期)
                    ...
                    .TextMatrix(intRow, mconIntCol数量) = Format(NVL(rsTemp!数量, 0), mFMT.FM_数量)
                    .TextMatrix(intRow, mconIntCol采购价) = Format(NVL(rsTemp!成本价, 0), mFMT.FM_成本价)
                    .TextMatrix(intRow, mconIntCol采购金额) = Format(NVL(rsTemp!成本金额, 0), mFMT.FM_金额)
                    .TextMatrix(intRow, mconIntCol售价) = Format(NVL(rsTemp!零售价, 0), mFMT.FM_零售价)
                    .TextMatrix(intRow, mconIntCol售价金额) = Format(NVL(rsTemp!零售金额, 0), mFMT.FM_金额)
                    .TextMatrix(intRow, mconintCol差价) = Format(NVL(rsTemp!差价, 0), mFMT.FM_金额)
    ...
    With mcolUseCount
        Do While Not rsTemp.EOF
            ...
            For Each vardrug In mcolUseCount
                If vardrug(0) = NVL(rsTemp!费用ID) & "!" & NVL(rsTemp!材料ID) & "!" & Val(NVL(rsTemp!批次)) Then
                    numUsedCount = vardrug(1)
                    .Remove vardrug(0)
                    Exit For
                End If
            Next
            .Add Array(NVL(rsTemp!费用ID) & "!" & NVL(rsTemp!材料ID) & "!" & Val(NVL(rsTemp!批次)), Val(rsTemp!填写数量)), NVL(rsTemp!费用ID) & "!" & NVL(rsTemp!材料ID) & "!" & Val(NVL(rsTemp!批次))
            rsTemp.MoveNext
        Loop
        rsTemp.Close
    End With
            ...
            Select Case .Col
                ...
                Case mconIntCol材料
                    If strKey <> "" Then
                        ...
                        ElseIf zlCommFun.IsCharAlpha(strKey) Then         '输入全是字母时只匹配简码
            ...
            Case mconIntCol灭菌日期
                ...
                If strKey <> "" Then
                    ...
                    If Not IsDate(strKey) Then
                        ...
                        Exit Sub
                    End If
                    If Format(zlDatabase.Currentdate, "yyyy-mm-dd") >= Format(DateAdd("m", Val(.TextMatrix(.Row, mconIntCol灭菌效期)), CDate(strKey)), "yyyy-mm-dd") Then
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Then 'mbln单据增加 Or
            ...
            If chrNo = "" Then chrNo = zlDatabase.GetNextNo(69, lngStockID)
...
Private Sub 提示原料库存数()
    ...
    With rsTemp
        ...
        End If
        dbl数量 = !数量
        stbThis.Panels(2).Text = "该卫材当前库存数为[" & dbl数量 & "]" & NVL(!单位)
    End With
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
...
Private Sub printbill()
    Dim StrNo As String
    StrNo = txtNo.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1713", mint记录状态, mintUnit, 1713, "卫材自制入库单", StrNo, cboStock.ItemData(cboStock.ListIndex)
End Sub
...
Private Function Set组成材料(ByVal lng材料ID As Long, Optional dbl自制数量 As Double = 0, Optional bln检查库存 As Boolean = True, Optional ByRef dblOut成本价 As Double = 0) As Boolean
    ...
    If mint编辑状态 <> 4 Then
        ...
        "        (  SELECT 收费细目id,现价 as 售价 From 收费价目 WHERE  ((SYSDATE BETWEEN 执行日期 AND 终止日期) OR (SYSDATE >= 执行日期 AND 终止日期 IS NULL)) ) d " & _
        ...
        With vs组成材料
            ...
            Do While Not rsTemp.EOF
                    ...
                    Set rsSort = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(NVL(rsTemp!材料ID)), lng库房id, mstr单据号)
                    dbl剩于数量 = dbl自制数量 * Val(NVL(rsTemp!组成))
                    .TextMatrix(lngRow, .ColIndex("原材料编码及名称")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!商品名称)
                    .Cell(flexcpData, lngRow, .ColIndex("原材料编码及名称")) = NVL(rsTemp!材料ID)
                    .TextMatrix(lngRow, .ColIndex("规格")) = NVL(rsTemp!规格)
                    .TextMatrix(lngRow, .ColIndex("单位")) = NVL(rsTemp!单位)
                    dblSum成本金额 = 0
                    .Cell(flexcpData, lngRow, .ColIndex("单位")) = 0 & "," & 0 & "," & 0 & "," & 0 & "," & Val(NVL(rsTemp!指导差价率))
                    Do While Not rsSort.EOF
                        .TextMatrix(lngRow, .ColIndex("原材料编码及名称")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!商品名称)
                        .Cell(flexcpData, lngRow, .ColIndex("原材料编码及名称")) = NVL(rsTemp!材料ID)
                        .TextMatrix(lngRow, .ColIndex("规格")) = NVL(rsTemp!规格)
                        .TextMatrix(lngRow, .ColIndex("单位")) = NVL(rsTemp!单位)
                        .Cell(flexcpData, lngRow, .ColIndex("单位")) = NVL(rsSort!实际可用数量) & "," & NVL(rsSort!实际数量) & "," & NVL(rsSort!实际差价) & "," & NVL(rsSort!实际金额) & "," & Val(NVL(rsTemp!指导差价率))
                        .TextMatrix(lngRow, .ColIndex("批号")) = NVL(rsSort!上次批号)
                        
                        .Cell(flexcpData, lngRow, .ColIndex("批号")) = NVL(rsSort!批次)
                        
                        If Val(NVL(rsTemp!是否变价)) = 0 Then
                            '定价
                            .TextMatrix(lngRow, .ColIndex("售价")) = Format(Val(NVL(rsTemp!售价)), mFMT.FM_零售价)
                            .Cell(flexcpData, lngRow, .ColIndex("售价")) = NVL(rsTemp!售价)
                        ElseIf Val(NVL(rsSort!实际数量)) <> 0 Then
                            If Val(NVL(rsSort!零售价)) <> 0 Then
                                .TextMatrix(lngRow, .ColIndex("售价")) = Format(Val(NVL(rsSort!零售价)), mFMT.FM_零售价)
                                .Cell(flexcpData, lngRow, .ColIndex("售价")) = Val(NVL(rsSort!零售价))
                            Else
                                .TextMatrix(lngRow, .ColIndex("售价")) = Format(Val(NVL(rsSort!实际金额)) / Val(NVL(rsSort!实际数量)), mFMT.FM_零售价)
                                .Cell(flexcpData, lngRow, .ColIndex("售价")) = Val(NVL(rsSort!实际金额)) / Val(NVL(rsSort!实际数量))
                            End If
                        Else
                            .TextMatrix(lngRow, .ColIndex("售价")) = ""
                            .Cell(flexcpData, lngRow, .ColIndex("售价")) = ""
                        End If
                        dbl可用数量 = Val(NVL(rsSort!可用数量))
...
'                        Call 验证出库差价计算(lng库房ID, Val(NVL(rsTemp!材料ID)), Val(.Cell(flexcpData, lngRow, .ColIndex("批号"))), 1, Val(NVL(rsSort!实际差价)), Val(NVL(rsSort!实际金额)), Val(NVL(rsTemp!指导差价率)) / 100, dbl当前数量, Val(.TextMatrix(lngRow, .ColIndex("售价金额"))), dbl差价, dbl购价, dbl成本金额)
...
'                        .TextMatrix(lngRow, .ColIndex("差价")) = Format(dbl差价, mFMT.FM_金额)
                        ...
                        .TextMatrix(lngRow, .ColIndex("成本价")) = Format(Get成本价(Val(NVL(rsTemp!材料ID)), lng库房id, Val(.Cell(flexcpData, lngRow, .ColIndex("批号")))), mFMT.FM_成本价)
                        ...
                        dbl剩于数量 = dbl剩于数量 - Val(NVL(rsSort!可用数量))
                        If dbl剩于数量 <= 0 Then
                            Exit Do
                        End If
                        rsSort.MoveNext
...
'                        .TextMatrix(lngRow, .ColIndex("差价")) = Format(dbl差价, mFMT.FM_金额)
                        ...
                        bln实价 = Val(NVL(rsTemp!是否变价)) = 1
                        
                        If bln检查库存 Then
                            If mint库存检查 = 0 Then
                                '不检查
                                If bln实价 Or Val(NVL(rsTemp!在用分批)) = 1 Then
                                    vs组成材料.Redraw = flexRDBuffered
    ...
    Else            '查看
        ...
        With vs组成材料
            ...
            Do While Not rsTemp.EOF
                .TextMatrix(lngRow, .ColIndex("原材料编码及名称")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!商品名称)
                .Cell(flexcpData, .ColIndex("原材料编码及名称")) = NVL(rsTemp!材料ID)
                .TextMatrix(lngRow, .ColIndex("规格")) = NVL(rsTemp!规格)
                .TextMatrix(lngRow, .ColIndex("单位")) = NVL(rsTemp!单位)
                .TextMatrix(lngRow, .ColIndex("批号")) = NVL(rsTemp!批号)
                .TextMatrix(lngRow, .ColIndex("数量")) = Format(rsTemp!实际数量, mFMT.FM_数量)
                .TextMatrix(lngRow, .ColIndex("售价")) = Format(Val(NVL(rsTemp!零售价)), mFMT.FM_零售价)
                .TextMatrix(lngRow, .ColIndex("售价金额")) = Format(Val(NVL(rsTemp!零售金额)), mFMT.FM_金额)
                .TextMatrix(lngRow, .ColIndex("成本价")) = Format(Val(NVL(rsTemp!成本价)), mFMT.FM_成本价)
                .TextMatrix(lngRow, .ColIndex("成本金额")) = Format(Val(NVL(rsTemp!成本金额)), mFMT.FM_金额)
                .TextMatrix(lngRow, .ColIndex("差价")) = Format(Val(NVL(rsTemp!差价)), mFMT.FM_金额)
                .Rows = .Rows + 1
                lngRow = lngRow + 1
                rsTemp.MoveNext
            Loop
...
Private Function Check原料库存(ByVal lng材料ID As Long, ByVal dbl自制数量 As Double) As Boolean
    ...
    If rsTemp.EOF Then
        ...
        If Not rsTemp.EOF Then
            ShowMsgBox "自制材料:" & NVL(rsTemp!编码) & "-" & NVL(rsTemp!名称) & vbCrLf & " 没有相关的组成材料,请检查!"
        End If
        Exit Function
    End If
    ...
    Do While Not rsTemp.EOF
        Set rsStorck = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(NVL(rsTemp!材料ID)), lng库房id, mstr单据号)
        If rsStorck.EOF Then
            dbl可用数量 = 0
        Else
            dbl可用数量 = Val(NVL(rsStorck!可用数量))
        End If
        If Round(dbl可用数量, 7) < Round(dbl自制数量 * Val(NVL(rsTemp!组成)), 7) Then
            bln实价 = Val(NVL(rsTemp!是否变价)) = 1
            If mint库存检查 = 0 Then
                '不检查
                If bln实价 Or Val(NVL(rsTemp!在用分批)) = 1 Then
                    vs组成材料.Redraw = flexRDBuffered
                    MsgBox "该自制卫材的原料卫材“" & NVL(rsTemp!编码) & "-" & NVL(rsTemp!商品名称) & "”可用库存数不够，请检查该原料卫材的库存！", vbInformation + vbOKOnly, gstrSysName
                    Exit Function
                End If
            ElseIf mint库存检查 = 1 Then
                '检查，提醒
                If bln实价 Or Val(NVL(rsTemp!在用分批)) Then
                    MsgBox "该自制卫材的原料卫材“" & NVL(rsTemp!编码) & "-" & NVL(rsTemp!商品名称) & "”可用库存数不够，请检查该原料卫材的库存！", vbInformation + vbOKOnly, gstrSysName
                    Exit Function
                ElseIf blnContinue = False Then
                    If MsgBox("该自制卫材的原料卫材“" & NVL(rsTemp!编码) & "-" & NVL(rsTemp!商品名称) & "”可用库存数不够，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                        Exit Function
                    End If
                    blnContinue = True
                End If
