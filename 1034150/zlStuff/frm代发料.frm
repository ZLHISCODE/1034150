...
'    If ReadData(gstrSQL) = False Then GoTo ExitSub  '读取所有未发料记录
    ...
    '检测当前卫材是否满足当前库房的存储条件,则给予提示
    If Check存储库房(IntBill, strNo) = False Then Exit Function
...
Private Function Check存储库房(ByVal intBillStyle As Integer, ByVal strNo As String) As Boolean
    ...
    '筛选当前单据中的所有药品
    gstrSQL = "Select Distinct a.药品id, '[' || b.编码 || ']' || b.名称 As 品名" & vbNewLine & _
            "From 药品收发记录 A, 收费项目目录 B" & vbNewLine & _
            "Where a.药品id = b.Id And a.No = [1] And a.单据 = [2]"

    Set rsData = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, strNo, intBillStyle)
    
    Do While Not rsData.EOF
        gstrSQL = "Select 1 From 收费执行科室 Where 收费细目id = [1] And 执行科室id = [2]"
    
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, rsData!药品ID, mlng发料部门ID)
        
        If rsTemp.EOF Then
            MsgBox "该单据中的  " & rsData!品名 & "  不属于当前存储库房！", vbInformation, gstrSysName
            Exit Function
        End If
...
Private Function CheckStock() As Boolean
    ...
    If Msf待发列表.TextMatrix(1, 1) <> "" Then
        For n = 1 To Msf待发汇总.Rows - 2
            '检查当前药品的时价属性
            gstrSQL = "Select 是否变价 From 收费项目目录 Where ID = [1]"
            
            Set rsCheckStock = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(Msf待发汇总.TextMatrix(n, 7)))
            
            bln分批 = Val(Msf待发汇总.TextMatrix(n, 8))
            bln时价 = rsCheckStock!是否变价
