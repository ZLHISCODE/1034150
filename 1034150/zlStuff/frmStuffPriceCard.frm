...
Private Sub cmdItem_Click()
    ...
    frmBatchSelect.ShowMe Me, mrsReturn, mblnOk, mintType, mdbl比率, mint调价
...
Private Sub CmdOK_Click()
    ...
    With Me.vsfPrice
        ...
        For intCount = 1 To IIf(Trim(.TextMatrix(.Rows - 1, 0)) = "", .Rows - 2, .Rows - 1)
            ...
            If lng材料ID <> 0 Then
                If Val(.TextMatrix(intCount, menuPriceCol.现零售价)) <> Val(.TextMatrix(intCount, menuPriceCol.原零售价)) And mint调价 <> 1 Then
                    lngID = zlDatabase.GetNextId("收费价目")
                ...
                End If
                If lng材料ID <> 0 Then
                    If Val(.TextMatrix(intCount, menuPriceCol.原指导售价)) <> Val(.TextMatrix(intCount, menuPriceCol.现指导售价)) And mint调价 <> 1 Then
                    ...
                    End If
                    '更新采购限价
                    If Val(.TextMatrix(intCount, menuPriceCol.原采购限价)) <> Val(.TextMatrix(intCount, menuPriceCol.现采购限价)) And mint调价 <> 0 Then
...
Private Sub cmdPrint_Click()
    ...
    Set objRow = New zlTabAppRow
    objRow.Add "执行时间:" & Format(IIf(Me.opt时间(0).Value, zlDatabase.Currentdate, Me.dtpRunDate.Value), "yyyy年MM月DD日 HH:mm:ss")
...
Private Sub initGrid()
    ...
    ElseIf mintMethod = 1 Then
        gstrSQL = "Select Distinct i.是否变价, Nvl(s.指导批发价, 0) As 指导批价, Nvl(s.扣率, 0) As 扣率, Nvl(s.指导零售价, 0) As 指导售价," & vbNewLine & _
            ...
            "                Decode(k.药品id, Null, 0, 1) 是否有库存" & vbNewLine & _
            "From (Select Min(原成本价) As 原成本价, Min(新成本价) As 新成本价, min(产地) as 产地,调价汇总号,药品id,执行日期 From 成本价调价信息 Where 调价汇总号 = [1] Group By 调价汇总号,药品id,执行日期) M, (Select 药品id From 药品库存 where 性质=1) K, 调价汇总记录 A, 收费项目别名 B, 材料特性 S, 收费项目目录 I, 收费价目 P" & vbNewLine & _
...
Private Sub GetDrugPirce(ByVal rsReturn As ADODB.Recordset, ByVal Row As Integer)
    ...
    For i = 0 To rsReturn.RecordCount - 1
        With vsfPrice
            ...
            .TextMatrix(Row, menuPriceCol.是否跟踪在用) = NVL(rsReturn!跟踪在用)
            .TextMatrix(Row, menuPriceCol.现成本价) = Format(Val(NVL(rsReturn!成本价)) * db包装系数, mFMT.FM_成本价)
            .TextMatrix(Row, menuPriceCol.原采购限价) = Format(Val(NVL(rsReturn!指导批发价)) * db包装系数, mFMT.FM_成本价)
            .TextMatrix(Row, menuPriceCol.现采购限价) = .TextMatrix(Row, menuPriceCol.原采购限价)
            .TextMatrix(Row, menuPriceCol.原指导售价) = Format(Val(NVL(rsReturn!指导零售价)) * db包装系数, mFMT.FM_零售价)
            ...
            If intCurrentPrice = 0 Then '定价卫材
                gstrSQL = "Select b.Id, Decode(Nvl(k.库存数量, 0), 0, a.成本价, (k.库存金额 - k.库存差价) / k.库存数量) As 成本价, a.指导批发价, a.指导零售价, b.现价, a.差价让利比," & vbNewLine & _
                            ...
                            "Where a.材料id = b.收费细目id And a.材料id = [1] And (b.终止日期 Is Null Or b.终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD'))"
            Else '时价卫材
                ...
                gstrSQL = "" & _
                        ...
                        "       and (P.终止日期 is null or P.终止日期=to_date('3000-01-01','YYYY-MM-DD'))"
            End If
                ...
                Else
                    Select Case mintType
                        Case 1      '根据成本价加成
                            ...
                            .TextMatrix(Row, menuPriceCol.现零售价) = Format(Val(NVL(rsTemp!成本价)) * dbl比率 * db包装系数, mFMT.FM_零售价)
                        Case 2      '根据零售价按比例
                            dbl比率 = 1 + Val(mdbl比率) / 100
                            .TextMatrix(Row, menuPriceCol.现零售价) = Format(Val(NVL(rsTemp!现价)) * dbl比率 * db包装系数, mFMT.FM_零售价)
                        Case 3      '根据零售价按固定金额加减
                            dbl比率 = Val(mdbl比率)
                            .TextMatrix(Row, menuPriceCol.现零售价) = Format((Val(NVL(rsTemp!现价)) * db包装系数) + dbl比率, mFMT.FM_零售价)
                    End Select
                End If
...
Private Sub GetDrugStore(ByVal lngDrugID As Long, ByVal intRow As Integer)
    ...
    If mintModal = 0 Or mblnUpdateAdd = True Or mblnBatchItem = True Then
        gstrSQL = "Select s.库房id, s.药品id As 材料id, d.名称 As 库房, '[' || m.编码 || ']' || m.名称 As 药品, m.规格, m.产地," & vbNewLine & _
                        "       m.计算单位 售价单位, p.包装单位,s.上次批号 As 批号, s.实际数量 As 数量, s.批次, Nvl(m.是否变价, 0) 变价, m.Id," & vbNewLine & _
                        "       Decode(Nvl(m.是否变价, 0), 0, e.现价, Decode(s.零售价,null,Decode(Nvl(s.实际数量, 0), 0, e.现价, s.实际金额 / s.实际数量),s.零售价)) As 时价售价," & vbNewLine & _
                        "       p.指导差价率 As 差价率, nvl(p.加成率,0) as 加成率 ,Decode(s.平均成本价, null, p.成本价, s.平均成本价) As 成本价," & vbNewLine & _
                        "       s.上次供应商id, n.名称 As 供应商, s.效期, s.上次产地 As 产地" & vbNewLine & _
            ...
            ElseIf cboPriceMethod.Text = "仅调售价" Then
                ...
                If rsTemp!是否执行 > 0 Then
                    gstrSQL = "Select Distinct a.单据, a.库房id, c.名称 As 库房, b.收费细目id As 材料id, a.供药单位id As 上次供应商id, '[' || e.编码 || ']' || e.名称 As 药品, e.规格," & vbNewLine & _
                        ...
                        "      b.调价汇总号 = [1] And 单据 = 13 order by 库房,批号"
                Else
...
'                 .TextMatrix(.Rows - 1, menuStoreCol.现零售价) = Format(IIf(rsTemp!变价 = 1, rsTemp!时价售价 * dbl包装换算, dblOldPrice), mFMT.FM_零售价)
                ...
                If mdbl比率 = 0 Or mint调价 = 1 Then
                    .TextMatrix(.Rows - 1, menuStoreCol.现零售价) = Format(IIf(rsTemp!变价 = 1, rsTemp!时价售价 * dbl包装换算, dblOldPrice), mFMT.FM_零售价)
                Else
                    .TextMatrix(.Rows - 1, menuStoreCol.现零售价) = vsfPrice.TextMatrix(intRow, menuPriceCol.现零售价)
                End If
            ...
            Else
                If mintModal = 2 And (cboPriceMethod.Text = "仅调售价" Or cboPriceMethod.Text = "售价成本价一起调价") Then   '查阅
                    ...
                    Set rsPirce = zlDatabase.OpenSQLRecord(gstrSQL, "获取售价", txtNO.Text, rsTemp!库房id, rsTemp!材料ID, NVL(rsTemp!批次, 0))
    ...
    With vsfPrice
        ...
        Select Case Col
            Case menuPriceCol.现成本价
                ...
                If strKey <> "" Then
                    If Val(strKey) > Val(.TextMatrix(Row, menuPriceCol.原采购限价)) And Val(.TextMatrix(Row, menuPriceCol.原采购限价)) <> 0 Then
                        If MsgBox("成本价不能大于指导采购价，（" & Format(Val(.TextMatrix(Row, menuPriceCol.原采购限价)), mFMT.FM_成本价) & "），继续吗？", vbQuestion + vbYesNo + vbDefaultButton1, gstrSysName) = vbNo Then
                            Cancel = True
                            Exit Sub
                        Else
            ...
            Case menuPriceCol.现零售价
                ...
                If strKey <> "" Then
'                    If zlDblIsValid(strkey, 12, , False, , "现价") = False Then Cancel = True: Exit Sub
