...
Private Sub cmdProvider_Click()
    ...
    If blnCancel = False Then
        If Not rsTmp Is Nothing Then
            txtProvider.Text = NVL(rsTmp!名称)
            txtProvider.Tag = NVL(rsTmp!Id)
        Else
            txtProvider.Text = ""
            txtProvider.Tag = "0"
        End If
...
Private Sub Form_Load()
    dtpDateBegin.Value = DateAdd("M", -3, zlDatabase.Currentdate)
    dtpDateEnd.Value = zlDatabase.Currentdate
    Call chkDate_Click
...
Const conHead = "H_病人ID,,|姓名,,1000|H_科室ID,,|科室,,2000|发票号,,1000|发票日期,,1000,d|发票金额,,1000,N|成本金额,,1000,n"
...
Private Sub FillLVWPatient()
    ...
    If mbln需要核查 Then
        If mintMode = 1 Then
            '核查
            gstrSQL = "Select Distinct a.病人ID, a.姓名, a.性别, a.年龄, a.住院号, b.病人科室ID, d.名称 科室 " & _
                      "From 病人信息 A, 住院费用记录 B, 药品收发记录 C, 部门表 D " & _
                      "Where a.病人id = b.病人id And b.Id = c.费用id And b.病人科室id = d.Id " & _
                      "    And c.费用ID > 0 And c.配药日期 is null And c.供药单位ID = [1] And c.库房id = [2] " & _
                      "    And c.单据 = 15 "
        Else
            '审核
            gstrSQL = "Select Distinct a.病人ID, a.姓名, a.性别, a.年龄, a.住院号, b.病人科室ID, d.名称 科室 " & _
                      ...
                      "    And c.供药单位ID = [1] And c.库房id = [2] And c.单据 = 15 "
        End If
    Else
        '直接审核
        gstrSQL = "Select Distinct a.病人ID, a.姓名, a.性别, a.年龄, a.住院号, b.病人科室ID, d.名称 科室 " & _
                  "From 病人信息 A, 住院费用记录 B, 药品收发记录 C, 部门表 D " & _
                  "Where a.病人id = b.病人id And b.Id = c.费用id And b.病人科室id = d.Id " & _
                  "    And c.费用ID > 0 And c.审核日期 is null And c.供药单位ID = [1] And c.库房id = [2] " & _
                  "    And c.单据 = 15 "
    End If
    ...
    Else
        mdatBegin = DateAdd("M", -3, zlDatabase.Currentdate)
    ...
    gstrSQL = gstrSQL & " And c.填制日期 between to_date('" & Format(mdatBegin, "yyyy-mm-dd 00:00:00") & "', 'yyyy-mm-dd hh24:mi:ss') " & _
              " And to_date('" & Format(mdatEnd, "yyyy-mm-dd 23:59:59") & "', 'yyyy-mm-dd hh24:mi:ss') " & vbNewLine & _
              "Order By 科室, 姓名 "
    ...
    With lvwPatient
        ...
        Do While Not rsTmp.EOF
            Set lsItem = .ListItems.Add(, "_" & NVL(rsTmp!病人ID) & "_" & NVL(rsTmp!病人科室ID), NVL(rsTmp!科室))
            lsItem.SubItems(1) = NVL(rsTmp!姓名)
            lsItem.SubItems(2) = NVL(rsTmp!性别)
            lsItem.SubItems(3) = NVL(rsTmp!年龄)
            lsItem.SubItems(4) = NVL(rsTmp!住院号)
            rsTmp.MoveNext
        Loop
...
Private Sub txtProvider_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    With txtProvider
        ...
        If Not rsProvider Is Nothing Then
            txtProvider.Text = NVL(rsProvider!名称)
            txtProvider.Tag = NVL(rsProvider!Id)
            chkDate.SetFocus
        Else
            txtProvider.Text = ""
            txtProvider.Tag = "0"
...
Private Sub SelPatient(ByVal lsItem As ListItem)
    ...
    Dim strIVNO As String, strIVDate As String
    ...
    gstrSQL = _
        "Select * " & _
        "From ( SELECT distinct a.药品id 材料id, A.NO, A.序号, ('[' || D.编码 || ']' || D.名称) AS 卫材信息, D.规格, D.产地 as 原产地, A.产地," & _
        ...
        IIf(mintMode = 1, "'' 发票号, '' 发票日期, '' 发票金额, ", "f.发票号, f.发票日期, f.发票金额, ") & _
        ...
        "         a.供药单位id = [1] and a.库房id = [2] AND A.记录状态 = 1 And A.单据 = 15 AND B.病人ID = [3] And B.病人科室ID = [4] And " & _
    ...
    End If
    gstrSQL = gstrSQL & "     ) " & vbLf & "ORDER BY NO, 序号 "
    ...
    With vsfMaterial
        ...
        Do While Not rsTmp.EOF
            ...
            .TextMatrix(lngCurRow, .ColIndex("材料ID")) = NVL(rsTmp!材料ID)
            .TextMatrix(lngCurRow, .ColIndex("材料名称")) = NVL(rsTmp!卫材信息)
            .TextMatrix(lngCurRow, .ColIndex("规格")) = NVL(rsTmp!规格)
            .TextMatrix(lngCurRow, .ColIndex("数量")) = NVL(rsTmp!实际数量)
            .TextMatrix(lngCurRow, .ColIndex("成本价")) = NVL(rsTmp!成本价)
            .TextMatrix(lngCurRow, .ColIndex("成本金额")) = NVL(rsTmp!结算金额)
            .TextMatrix(lngCurRow, .ColIndex("发票金额")) = IIf(mintMode = 1, NVL(rsTmp!结算金额), NVL(rsTmp!发票金额))
            .TextMatrix(lngCurRow, .ColIndex("NO")) = NVL(rsTmp!NO)
            .TextMatrix(lngCurRow, .ColIndex("序号")) = NVL(rsTmp!序号)
            .TextMatrix(lngCurRow, .ColIndex("库房ID")) = NVL(rsTmp!库房ID)
            .TextMatrix(lngCurRow, .ColIndex("供药单位ID")) = NVL(rsTmp!供药单位ID)
            .TextMatrix(lngCurRow, .ColIndex("产地")) = NVL(rsTmp!产地)
            .TextMatrix(lngCurRow, .ColIndex("批号")) = NVL(rsTmp!批号)
            .TextMatrix(lngCurRow, .ColIndex("换算系数")) = NVL(rsTmp!换算系数)
            .TextMatrix(lngCurRow, .ColIndex("单位")) = NVL(rsTmp!单位)
            .TextMatrix(lngCurRow, .ColIndex("生产日期")) = NVL(rsTmp!生产日期)
            .TextMatrix(lngCurRow, .ColIndex("效期")) = NVL(rsTmp!效期)
            .TextMatrix(lngCurRow, .ColIndex("灭菌日期")) = NVL(rsTmp!灭菌日期)
            .TextMatrix(lngCurRow, .ColIndex("灭菌效期")) = NVL(rsTmp!灭菌效期)
            .TextMatrix(lngCurRow, .ColIndex("扣率")) = NVL(rsTmp!扣率)
            .TextMatrix(lngCurRow, .ColIndex("零售价")) = NVL(rsTmp!零售价)
            .TextMatrix(lngCurRow, .ColIndex("零售金额")) = NVL(rsTmp!零售金额)
            .TextMatrix(lngCurRow, .ColIndex("差价")) = NVL(rsTmp!差价)
            .TextMatrix(lngCurRow, .ColIndex("差价金额")) = NVL(rsTmp!零售差价)
            .TextMatrix(lngCurRow, .ColIndex("摘要")) = NVL(rsTmp!摘要)
            .TextMatrix(lngCurRow, .ColIndex("注册证号")) = NVL(rsTmp!注册证号)
            .TextMatrix(lngCurRow, .ColIndex("填制人")) = NVL(rsTmp!填制人)
            .TextMatrix(lngCurRow, .ColIndex("填制日期")) = NVL(rsTmp!填制日期)
            .TextMatrix(lngCurRow, .ColIndex("核查人")) = NVL(rsTmp!核查人)
            .TextMatrix(lngCurRow, .ColIndex("核查日期")) = NVL(rsTmp!核查日期)
            .TextMatrix(lngCurRow, .ColIndex("批次")) = NVL(rsTmp!批次)
            .TextMatrix(lngCurRow, .ColIndex("高值材料")) = NVL(rsTmp!高值材料)
            .TextMatrix(lngCurRow, .ColIndex("商品条码")) = NVL(rsTmp!商品条码)
            .TextMatrix(lngCurRow, .ColIndex("内部条码")) = NVL(rsTmp!内部条码)
            .TextMatrix(lngCurRow, .ColIndex("费用ID")) = NVL(rsTmp!费用ID)
            If mintMode = 1 Then
                dblCostAmount = dblCostAmount + NVL(rsTmp!结算金额, 0)
            Else
                dblCostAmount = dblCostAmount + NVL(rsTmp!发票金额, 0)
            End If
            dbl成本金额 = dbl成本金额 + NVL(rsTmp!结算金额)
            If strIVNO = "" Then strIVNO = NVL(rsTmp!发票号)
        ...
        For i = 1 To .Rows - 1
            If Val(.TextMatrix(i, .ColIndex("病人ID"))) = lngPatientID And Val(.TextMatrix(i, .ColIndex("科室ID"))) = lngPatientDrugID Then
                If Trim(.TextMatrix(i, .ColIndex("发票号"))) <> "" Or _
                   Trim(.TextMatrix(i, .ColIndex("发票日期"))) <> "" Then
        ...
        ElseIf .ColIndex("发票日期") = Col Then
            ...
            If strKey <> "" Then
                If Len(strKey) = 8 And InStr(1, strKey, "-") = 0 Then
                    ...
                    If strKey = "" Then
                        MsgBox "发票日期必须为日期型如（2017-10-10）或（20171010）,请重新录入！", vbInformation + vbOKOnly, gstrSysName
                        .TxtSetFocus
                        Exit Sub
                    End If
                    .Text = strKey
...
Private Sub vsfPatient_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    With vsfPatient
        If .ColIndex("发票号") = Col Or .ColIndex("发票日期") = Col Or .ColIndex("发票金额") = Col Then
'            If mintMode <> 1 Then
                Cancel = False
                mdblIVAmountOld = 0
                If .ColIndex("发票金额") = Col Then
    ...
    With vsfMaterial
        ...
        For intRow = 1 To .Rows - 1
            ...
            If mbln需要核查 Then
                If mintMode = 1 Then
                    str核查人 = UserInfo.姓名
                    str核查日期 = Format(zlDatabase.Currentdate, "yyyy-mm-dd hh:mm:ss")
                Else
            ...
            If GetInvoiceInfo(.TextMatrix(intRow, .ColIndex("病人ID")), str发票号, str发票日期) = False Then
                Exit Function
            End If
...
Private Function GetInvoiceInfo(ByVal lngPatientID, ByRef strIVNO As String, ByRef strIVDate As String) As Boolean
'获取发票号
    Dim i As Long
