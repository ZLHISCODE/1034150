...
Private Const mconintCol金额差 As Integer = 22
Private Const mconintCol差价差 As Integer = 23
Private Const mconintCol盘点金额 As Integer = 24
Private Const mconintCol批号编辑 As Integer = 25
Private Const mconintCol产地编辑 As Integer = 26
Private Const mconIntColS  As Integer = 27             '总列数
'=========================================================================================
...
Private Function GetDateStock(str盘存时间 As String, lng库房ID As Long, str材料条件 As String, Optional blnZero As Boolean = False, Optional ByVal bln新增 As Boolean = False, Optional lng材料ID As Long = 0) As ADODB.Recordset
    ...
    '取得当前库存
    gstrSQL = "" & _
        "   SELECT a.库房id, b.材料id, NVL (批次, 0) AS 批次, a.实际数量,a.实际金额, a.实际差价, a.可用数量,a.上次批号 AS 批号,a.上次产地 AS 产地,a.效期 " & _
        "   FROM (Select 库房id,药品id,批次,实际数量,实际金额,实际差价,可用数量,上次批号,上次产地,效期 From 药品库存 Where 性质=1 And 库房ID+0=[1]) a, 材料特性 b,(Select 库房id, 材料id, 上限, 下限, 盘点属性, 库房货位 From 材料储备限额 Where 库房ID+0=[1] )e " & _
    ...
    gstrSQL = gstrSQL & _
        ...
        "               -SUM (DECODE (a.入出系数, 1, a.差价, -a.差价)) AS 实际差价,0 AS 可用数量,a.批号,a.产地,a.效期 " & _
    ...
    gstrSQL = "" & _
        ...
        "           SUM(可用数量) As 可用数量,max(批号) as 批号,max(产地) as 产地 ,max(效期) as 效期 " & _
        "   FROM ( " & gstrSQL & ") " & _
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            gstrSQL = "" & _
                ...
                "                   a.摘要,填制人,填制日期,审核人,审核日期,a.频次 as 盘点时间,a.成本价 as 库存金额,a.成本金额 as 库存差价,b.指导差价率,D.是否变价,b.在用分批,decode(E.上次批号,null,1,0) 批号编辑,decode(E.上次产地,null,1,0) 产地编辑 " & _
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol单位) = NVL(rsTemp!单位)
                    ...
                    .TextMatrix(intRow, mconIntCol比例系数) = NVL(rsTemp!换算系数)
...
Private Sub mshbill_CommandClick()
    ...
    If mshBill.Col = mconIntCol材料 Then
        If Not IsDate(TxtCheckDate) Then
            ...
            Exit Sub
        End If
        Set RecReturn = Frm材料选择器.ShowMe(Me, 2, txtStock.Tag, txtStock.Tag, txtStock.Tag, False, True, True, True, , , , , TxtCheckDate.Text, , , mbln盘无存储库房材料, mstrPrivs)
    ...
    With mshBill
        ...
        Select Case .Col
            ...
            Case mconIntCol效期
                ...
                .TextMask = "1234567890-"
                .MaxLength = 10
                If .ColData(mconIntCol效期) <> 2 Then Exit Sub
        ...
        Select Case .Col
            ...
            Case mconIntCol材料
                If strKey <> "" Then
                    ...
                    Set RecReturn = FrmMulitSel.ShowSelect(Me, 2, txtStock.Tag, txtStock.Tag, txtStock.Tag, strKey, sngLeft, sngTop, mshBill.MsfObj.CellWidth, mshBill.MsfObj.CellHeight, False, True, True, True, , , , TxtCheckDate.Text, , , mbln盘无存储库房材料, mstrPrivs)
                    If RecReturn.RecordCount = 1 Then
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For intLop = 1 To .Rows - 1
                If Val(.TextMatrix(intLop, mconIntCol批次)) = -1 Then
                    ...
                    lng批次 = Val(.TextMatrix(intLop, mconIntCol批次))
                    ...
                    For intRow = 1 To .Rows - 1
                        If intLop <> intRow And _
                            ...
                            lng批次 = Val(.TextMatrix(intRow, mconIntCol批次)) Then
                            
                            MsgBox "第" & intLop & "行的卫材(" & Trim(.TextMatrix(intLop, mconIntCol材料)) & ")新增批次的产地，批号和第" & intRow & "行重复了！" & vbCrLf & "请重新录入产地和批号信息！", vbInformation, gstrSysName
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If mint编辑状态 = 1 Then
            ...
            End If
            If chrNo = "" Then chrNo = zlDatabase.GetNextNo(76, lng库房ID)
...
Private Function SetPhiscRows(ByVal lngId As Long, ByVal lng批次 As Long) As Boolean
    ...
    With mshBill
        ...
        .TextMatrix(intRow, mconIntCol单位) = NVL(rsData!单位)
        ...
        .TextMatrix(intRow, mconIntCol比例系数) = NVL(rsData!换算系数) ' 获取比例系数(rsData)
        .TextMatrix(intRow, mconIntCol指导差价率) = rsData!指导差价率 & "||" & rsData!是否变价 & "||" & rsData!在用分批
        
        If rsData!是否变价 = 1 Then
            gstrSQL = "" & _
                "   Select 实际金额/实际数量*" & IIf(mintUnit = 0, "1", NVL(rsData!换算系数)) & " as  售价 " & _
                "   From 药品库存 " & _
                "   Where 库房id=[1] " & _
                "           and 药品id=[2]" & _
            ...
            Set rsprice = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, Val(txtStock.Tag), Val(NVL(rsData!材料ID)), Val(NVL(rsData!批次)))
