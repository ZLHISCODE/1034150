...
Begin VB.Form FrmMulitSel 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsColSet 
      ...
      FormatString    =   $"FrmMulitSel.frx":0E42
      ScrollTrack     =   -1  'True
      ScrollBars      =   2
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsHead 
      ...
      FormatString    =   $"FrmMulitSel.frx":0E8F
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsBatch 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"FrmMulitSel.frx":14EE
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   ' On Error Resume Next
    ...
    If In_编辑状态 = 2 Then
        ...
        With mrsStock
            ...
            gstrSQL = ""
            If mbln空批次 Then
                gstrSQL = "Select " & IIf(mstr盘点时间 <> "", "/*+ Rule*/", "") & " 1 RID,名称,0 批次,'新增批次卫生材料' 批号,sysdate 失效期," & _
                          "0 可用数量,0 库存数量,0 库存金额,0 库存差价,sysdate as 灭菌失效期,0 售价,'' As 成本价,0 上次购价,'' 产地 , Sysdate As 生产日期, 0 As 上次供应商id,'' 批准文号,'' 商品条码,'' 内部条码 " & _
                          " From 部门表" & _
                          " Where ID=[1]" & _
                          " Union "
            End If
            ...
            If mblnStock Then
                If mbln散装单位 Then
                    strTmp = " K.可用数量," & _
                             " K.实际数量 as 库存数量,"
                Else
                    strTmp = " K.可用数量" & mstrUnitString & " 可用数量," & _
                             " K.实际数量" & mstrUnitString & " 库存数量,"
                End If
            Else
                strTmp = "'' 可用数量,'' 库存数量,"
            End If
            ...
            gstrSQL = gstrSQL & strTmp & _
                     IIf(mblnStock, "K.实际金额 as 库存金额,", "0 库存金额,") & _
                     IIf(mblnStock, " K.实际差价 as 库存差价", "0 库存差价") & " ,K.灭菌效期 as 灭菌失效期," & _
                     IIf(mblnStock, "Decode(nvl(M.是否变价,0),0,G.现价,decode(nvl(K.零售价,0),0,nvl(K.实际金额,0)/decode(K.实际数量,null,1,0,1,K.实际数量),nvl(K.零售价,0)))" & IIf(mbln散装单位, "", "*nvl(D.换算系数,1)") & " 售价,", "0 售价,") & _
...
Private Function RefreshData() As Boolean
    ...
    ElseIf zlCommFun.IsCharAlpha(mstrInput) Then         '输入全是字母时只匹配简码
        If Mid(gSystem_Para.Para_输入方式, 2, 1) = "1" Then strSerach = " And B.简码 Like [4] And B.码类=[6] "
    ElseIf zlCommFun.IsCharChinese(mstrInput) Then
    ...
    '对列头排顺序
    gstrSQL = "" & _
    " Select  D.诊疗id,D.材料id,D.分类ID,D.编码,D.通用名称,D.商品名,D.规格,D.产地,D.批准文号,x.名称 As 上次供应商,D.售价,to_char(d.成本价," & gOraFmt_Max.FM_成本价 & ") as 最新成本价,D.散装单位 散装,D.换算系数 系数,D.包装单位 包装," & _
                 IIf(mblnStock, "S.可用数量 " & IIf(mbln散装单位, "", "/D.换算系数") & " 可用数量, " & _
                                "S.库存数量 " & IIf(mbln散装单位, "", "/D.换算系数") & " 库存数量, " & _
                                "S.库存金额 库存金额,S.库存差价 ,", _
                      "'' 可用数量,'' 库存数量,'' 库存金额,'' 库存差价,") & _
    ...
    '材料信息，材料目录
    If mbln只显示在用物资 Then
        gstrSQL = gstrSQL & " (Select Distinct u.诊疗id,u.材料id,H.分类ID,V.编码,v.名称 As 通用名称,B.名称 As 商品名,V.规格," & IIf(int入库产地取值方式 = 0, "decode(u.上次产地,null,v.产地,u.上次产地)", "decode(v.产地,null,u.上次产地,v.产地)") & " as 产地,u.批准文号,V.计算单位 as 散装单位,U.包装单位," & _
                    ...
                    "                       And U.材料id=i.收费细目Id " & IIf(mbln盘无存储库房材料, "(+)", "") & _
                                            IIf(mbln只显示在用物资, " And U.跟踪在用=1 ", IIf(mblnTrackUsing = True, " and  U.跟踪在用 =0 ", "")) & " And v.Id = p.收费细目id And (Sysdate Between p.执行日期 And Nvl(p.终止日期, Sysdate)) "
    Else
        gstrSQL = gstrSQL & " (Select Distinct u.诊疗id,u.材料id,H.分类ID,V.编码,v.名称 As 通用名称,B.名称 As 商品名,V.规格," & IIf(int入库产地取值方式 = 0, "decode(u.上次产地,null,v.产地,u.上次产地)", "decode(v.产地,null,u.上次产地,v.产地)") & " as 产地,u.批准文号,V.计算单位 as 散装单位,U.包装单位," & _
                    "                       To_Char(U.换算系数," & GFM_XS & " ) 换算系数,nvl(To_Char(U.灭菌效期,'9999990'),0) 灭菌效期,nvl(To_Char(U.最大效期,'9999990'),0) 最大效期," & _
                    "                       Decode(U.库房分批,1,'是','否') 库房分批,Decode(U.在用分批,1,'是','否') 在用分批,Decode(U.一次性材料,1,'是','否')  一次性材料,Decode(U.无菌性材料,1,'是','否') 无菌性材料,Decode(V.是否变价,1,'是','否') 时价," & _
