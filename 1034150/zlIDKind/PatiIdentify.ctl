...
'读卡之前
Event FindPatiBefore(ByVal objCard As Card, ByRef blnCard As Boolean, ByRef strShowText As String, ByRef objCardData As PatiInfor, ByRef blnFindPatied As Boolean, ByRef blnCancel As Boolean)
Event FindPatiArfter(ByVal objCard As Card, ByVal blnCard As Boolean, ByRef ShowName As String, ByRef objHisPati As PatiInfor, _
        ByRef objCardData As PatiInfor, ByRef strErrMsg As String, ByRef blnCancel As Boolean)

Event ItemClick(Index As Integer, objCard As Card) 'MappingInfo=IDKInd,IDKInd,-1,ItemClick
Event Click(objCard As Card) 'MappingInfo=IDKInd,IDKInd,-1,Click
...
Const m_def_OlnyReadDefaultCard = True
Const m_def_ReadThreePatiInfor = True
Const m_def_InputAppearance = 1
Const m_def_OnlyThreeCard = False
'属性变量:
Dim m_MustBrushCard As Boolean
Dim m_InputBoxAlignment As Pati_InputBoxAlignment
Dim m_HiddenMoseRightKey As Boolean
Dim m_ShowCardNO As Pati_ShowCardNo
Dim m_FindPatiShowName As Boolean
Dim m_病人病区ID As Long
Dim m_IDKindWidth As Long
Dim m_OlnyReadDefaultCard As Boolean
Dim m_ReadThreePatiInfor As Boolean
Dim m_InputAppearance As Input_Appearance
Dim m_OnlyThreeCard As Boolean
Dim mblnNotAutoSel As Boolean
'----------------------------------------------------------------------
'控件引用
Dim mobjParent As Object
Dim mobjCurPati As PatiInfor
Dim mobjCurCardData As PatiInfor   '当前三方卡病人数据或IC卡类存储的病人信息
...
Private Sub IDKind_ReadCard(ByVal objCard As Card, objPatiInfor As PatiInfor, blnCancel As Boolean)
...
Private Function FindPatiBefor(ByVal strInput As String, ByVal blnCard As Boolean, _
    objCard As Card, objPati As PatiInfor) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:查找病人之前
...
End Function
 ...
 Private Sub FindPatiedArfter(ByVal strInput As String, ByVal blnCard As Boolean, _
    objCard As Card, objPati As PatiInfor)
...
Private Sub SetShowCardNo(ByVal objCard As Card, ByVal objPati As PatiInfor)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:设置显示的卡号
    '入参:objCard-当前的卡类别
    '        objPati-当前的卡信息
    ...
    If ShowCardNo = ShowFullNameAndCardNo Then
        ...
        Exit Sub
    End If
    If ShowCardNo = ShowOlnyCardNo Then
        If objCard.接口序号 > 0 Then
...
Private Function SetBrushCardObject(ByVal blnComm As Boolean) As Boolean
    ...
    If objCurCard Is Nothing Or AllowAutoCommCard = False Then Exit Function
    If objCurCard.接口序号 = 0 Or objCurCard.接口程序名 = "" Or Not objCurCard.是否刷卡 Then Exit Function
    If gobjSquare.zlSetBrushCardObject(objCurCard.接口序号, IIf(blnComm, txtPatient, Nothing), strExpend, objCurCard.消费卡) Then
...
Private Sub txtPatient_KeyPress(KeyAscii As Integer)
    ...
    Dim lng病人ID As Long, objPati As New PatiInfor, objThreePati As New PatiInfor
    ...
    If IDKind.GetCurCard.名称 Like "*姓*名*" Or IDKind.GetCurCard.模糊查找项 Then
        ...
        If Not blnCard Then
            ...
            If Left(strInputText, 1) = "-" And IsNumeric(Mid(strInputText, 2)) Then
                ...
                Call zlGetPati(mcnOracle, lng病人ID, objPati, strErrMsg)
            ElseIf Left(strInputText, 1) = "*" And IsNumeric(Mid(strInputText, 2)) Then '门诊号
                Call zlGetPati(mcnOracle, 0, objPati, strErrMsg, "门诊号", Mid(strInputText, 2))
            ElseIf Left(strInputText, 1) = "+" And IsNumeric(Mid(strInputText, 2)) Then '住院号
                Call zlGetPati(mcnOracle, 0, objPati, strErrMsg, "住院号", Mid(strInputText, 2))
            ElseIf Left(strInputText, 1) = "/" Then
                '床号需特殊处理:无病区,不能根据床号来查找
                If 病人病区ID = 0 Then Exit Sub
                lng病人ID = zlGetFromBedNumberToPatiID(mcnOracle, 病人病区ID, Mid(strInputText, 2))
                If lng病人ID <> 0 Then Call zlGetPati(mcnOracle, lng病人ID, objPati, strErrMsg)
            ElseIf IsMobileNo(strInputText) Then
                '103000：李南春，2017/2/7，按手机号查找
                If gobjSquare.zlGetPatiID("手机号", strInputText, False, lng病人ID, , strErrMsg) = False Then lng病人ID = 0
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_OlnyReadDefaultCard = PropBag.ReadProperty("OlnyReadDefaultCard", m_def_OlnyReadDefaultCard)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("OlnyReadDefaultCard", m_OlnyReadDefaultCard, m_def_OlnyReadDefaultCard)
...
Private Sub UserControl_InitProperties()
    ...
    Call SetInputAppearance
    m_OlnyReadDefaultCard = m_def_OlnyReadDefaultCard
...
Public Sub zlInit(ByVal frmMain As Object, Optional ByVal lngSys As Long, Optional ByVal lngModul As Long, _
    ...
    Call IDKind.zlInit(frmMain, lngSys, lngModul, cnOracle, strDBUser, objCardSquare, strIDKindStr, , strProductName)
...
Private Function GetPatient(ByVal blnBrushCard As Boolean, ByVal strInput As String, ByRef objCard As Card, _
    ...
    '指定卡类别
    If objCard.接口序号 > 0 Then
       If gobjSquare.zlGetPatiID(objCard.接口序号, strInput, False, lng病人ID, strCardPassWord, strErrMsg, lngCardTypeID) = False Then lng病人ID = 0
...
'注意！不要删除或修改下列被注释的行！
'MemberInfo=0,0,0,True
Public Property Get OlnyReadDefaultCard() As Boolean
    OlnyReadDefaultCard = m_OlnyReadDefaultCard
End Property

Public Property Let OlnyReadDefaultCard(ByVal New_OlnyReadDefaultCard As Boolean)
    m_OlnyReadDefaultCard = New_OlnyReadDefaultCard
    PropertyChanged "OlnyReadDefaultCard"
End Property
...
Public Property Get GetCurCard() As Card
    Set GetCurCard = FromObjectToCard(IDKind.GetCurCard)
End Property
...
Public Function GetIDKindCard(ByVal strCardType As String, Optional MachMode As Mach_Mode) As Card
    Set GetIDKindCard = FromObjectToCard(IDKind.GetIDKindCard(strCardType, MachMode))
End Function
...
Public Property Get GetfaultCard() As Card
    Set GetfaultCard = FromObjectToCard(IDKind.GetfaultCard)
End Property
