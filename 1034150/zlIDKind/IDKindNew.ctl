...
Dim vRect As RECT
Dim mobjCurCard As Card
Dim mobjCards As Cards  '有效卡类别
Dim mobjDefaultCard As Card '当前缺省卡类别
...
Dim WithEvents mobjIDCard As zlIDCard.clsIDCard
...
Const m_def_OlnyReadCardNo = True
...
Dim m_OlnyReadCardNo As Boolean
...
Public Event Click(objCard As Card)
Public Event ItemClick(Index As Integer, objCard As Card)
Public Event ReadCard(ByVal objCard As Card, objPatiInfor As PatiInfor, blnCancel As Boolean)

Private Sub InitCardsObject()
    Dim strValue As String
    Dim objCard As Card, strCardTypes As String
    Dim objDatabase As Object, objTemp As Object
    ...
    Set objTemp = GetCardSquareDataBaseObject(mcnOracle)
    Set objDatabase = objTemp
    If objDatabase Is Nothing Then Set objDatabase = gobjDatabase
    
    '78768:李南春,2016/3/3,将快键保存到参数表
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("向前滚动-功能键", glngSys, 1153)
    End If
    If strValue = "" Then Call GetRegInFor(mRegType, "医疗卡类别", "向前滚动-功能键", strValue)
    Cards.向前滚动功能键 = strValue
    
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("向前滚动-快键", glngSys, 1153)
    End If
    ...
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("向后滚动-功能键", glngSys, 1153)
    End If
    ...
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("向后滚动-快键", glngSys, 1153)
    End If
    ...
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("读卡-功能键", glngSys, 1153)
    End If
    ...
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("读卡-快键", glngSys, 1153)
    End If
    ...
    '78768:李南春,2016/3/3,缺省读卡类别
    If Not mcnOracle Is Nothing Then
        strValue = objDatabase.getPara("缺省读卡类别", glngSys, 1153, 0)
    Else
    ...
    With mobjCards
        For i = 1 To Cards.Count
            ...
            Cards(i).快键 = strValue
            '缺省的医疗卡类别
'1.以IDKindNew的DefaultCardType为缺省类别
'2.以医疗卡类别.缺省标志=1的医疗卡为缺省类别
'3.以第一个启用的医疗卡类别为缺省医疗卡类别
            '76843:李南春,2014/8/22,设置缺省的医疗卡对象
            If m_DefaultCardType <> "" And (Cards(i).名称 = m_DefaultCardType _
            ...
            End If
            If Cards(i).缺省标志 And mobjDefaultCard Is Nothing Then
                DefaultCardType = Cards(i).接口序号
                Set mobjDefaultCard = Cards(i)
            End If
...
Private Sub RePicKindStatu()
    ...
    If Appearance = ShowSunken3D Then
        ...
        ElseIf CheckReadCard Then
            intType = 2
        ElseIf Not mobjCurCard.是否刷卡 Then
            intType = 2
        Else
            intType = 0
        End If
...
Private Sub SetCommandStatu(ByVal intStyle As Integer, Optional intType As Integer)
    ...
    End If
    'intType:0-所有;1-OlnyDown;2-OlnyKind
...
Private Sub mobjCommEvents_ShowCardInfor(ByVal strCardType As String, ByVal strCardNO As String, ByVal strXmlCardInfor As String, strExpended As String, blnCancel As Boolean)
    Dim objPati As PatiInfor, strErrMsg As String
    Dim objCard As Card
    ...
    If strXmlCardInfor <> "" Then
        Set objPati = FromXMLPati(strXmlCardInfor, strErrMsg)
    End If
    If objPati Is Nothing Then Set objPati = New PatiInfor
...
Private Function IDKindClick(ByVal objCard As Card, Optional ByVal blnFastKeyUse As Boolean = False) As Boolean
    ...
    If gobjSquare.zlReadCard(mobjParent, glngModul, objCard.接口序号, m_OlnyReadCardNo, strExpand, strOutCardNO, strOutPatiInforXML, strPhotoFile) = False Then Exit Function
    If Not m_OlnyReadCardNo Then
        Set objPati = FromXMLPati(strOutPatiInforXML, strErrMsg)
...
Private Sub mobjTxtInput_KeyPress(KeyAscii As Integer)
    ...
    '问题:51488
    If Not mobjCurCard Is Nothing Then
         If mobjCurCard.是否刷卡 = True Then Exit Sub
    End If
...
Private Sub picKind_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    '78768:李南春,2016/3/3,缺省读卡类别
    If mobjCurCard.是否刷卡 And CheckReadCard = False Then Exit Sub
    SetCommandStatu -1, 2
End Sub
...
Private Sub picKind_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If mobjCurCard.是否刷卡 And bln缺省读卡 = False Then Exit Sub
    If bln缺省读卡 Then
...
'    m_AutoSize = m_def_AutoSize
    ...
    m_OlnyReadCardNo = m_def_OlnyReadCardNo
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    Set m_Cards = zlGetKindCards(m_IDKindStr, , NotAutoAppendKind)
...
'    m_AutoSize = PropBag.ReadProperty("AutoSize", m_def_AutoSize)
    ...
    m_OlnyReadCardNo = PropBag.ReadProperty("OlnyReadCardNo", m_def_OlnyReadCardNo)
   ...
   If Ambient.UserMode And _
        UCase(TypeName(UserControl.Parent)) <> UCase("PatiIdentify") Then Set mobjParent = UserControl.Parent
    Call SetCtrlVisible
    Call ReInitCards
    If Ambient.UserMode Then
...
'    Call PropBag.WriteProperty("AutoSize", m_AutoSize, m_def_AutoSize)
    ...
    Call PropBag.WriteProperty("OlnyReadCardNo", m_OlnyReadCardNo, m_def_OlnyReadCardNo)
...
'注意！不要删除或修改下列被注释的行！
'MemberInfo=9,0,2,0
Public Property Get Cards() As Cards
    Set Cards = m_Cards
End Property

Public Property Set Cards(ByVal New_Cards As Cards)
...
'注意！不要删除或修改下列被注释的行！
'MemberInfo=14
Public Function GetIDKindCard(ByVal strCardType As String, Optional MachMode As Mach_Mode) As Card
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取指定卡
    '入参:strCardType-卡类别(数字为指定的卡类别ID,字符为按名称匹配)s
    '       MachMode-匹配方式
    ...
    If MachMode <> CardTypeName Then
        ...
        If MachMode = CardTypeID Then
            Set GetIDKindCard = Cards("K" & lngCardTypeID)
        Else
...
End Property
Public Property Get GetCurCard() As Card
...
Public Property Let DefaultCardType(ByVal New_DefaultCardType As String)
    ...
    End If
End Property
Public Property Get GetfaultCard() As Card
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取缺省卡
    '编制:刘兴洪
...
Public Sub zlInit(ByVal frmMain As Object, Optional ByVal lngSys As Long, Optional ByVal lngModul As Long, _
    ...
    Optional objCardSquare As Object, _
    Optional strIDKindStr As String = "", _
    Optional objBoundTextBox As Object, _
    Optional strProductName As String = "")
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:初始化
    '入参:frmMain-调用的主窗口
    '     lngSys : 系统编号
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Set gobjSquare = objCardSquare: gstrDBUser = strDBUser
    ...
    Set mobjTxtInput = objBoundTextBox
    Call zlInitCommLib
...
GoInitKind:
    If strIDKindStr = "" Then
     ...
     End If
    Set Cards = zlGetKindCards(strIDKindStr, False, NotAutoAppendKind)
...
Private Sub CreateComEvtsObject()
    ...
    End If
    If mobjCommEvents Is Nothing And AllowAutoCommCard Then
        Set mobjCommEvents = New clsCommEvents
        If Not mobjParent Is Nothing Then
             Call gobjSquare.zlInitEvents(mobjParent.Hwnd, mobjCommEvents)
        Else
             Call gobjSquare.zlInitEvents(UserControl.Hwnd, mobjCommEvents)
        End If
    End If
...
Private Sub CloseComEvntsObject()
    ...
    If Not gobjSquare Is Nothing Then
        Call gobjSquare.SetEnabled(False)
    End If
...
Public Sub SetAutoReadCard(ByVal blnAutoReadCard As Boolean)
    ...
    End If
    Err = 0: On Error Resume Next
    gobjSquare.SetEnabled (blnAutoReadCard)
...
Private Function ActiveFastKeyInside() As Boolean
    ...
    Else
     ...
     End If
    '读卡
    If mobjCurCard.是否刷卡 = True And bln缺省读卡 = False Then Exit Function
...
'注意！不要删除或修改下列被注释的行！
'MemberInfo=0,0,0,True
Public Property Get OlnyReadCardNo() As Boolean
    OlnyReadCardNo = m_OlnyReadCardNo
End Property
Public Property Let OlnyReadCardNo(ByVal New_OlnyReadCardNo As Boolean)
    m_OlnyReadCardNo = New_OlnyReadCardNo
    PropertyChanged "OlnyReadCardNo"
End Property
...
Public Property Get AvailabilityIdkindStr() As String
  '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取有效的IDKindStr
    '返回:有效的IDkindStr:短名|全名|是否刷卡|接口序号|卡号长度|缺省标志|是否存在帐户|卡号密文规则;...
    '编制:刘兴洪
    ...
    For i = 1 To mobjCards.Count
        ...
        strNewIdKindStr = strNewIdKindStr & "|" & IIf(objCard.是否刷卡, 0, 1)
...
Public Property Get AvailabilityCards() As Cards
  '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取有效的Cards对象
    '返回:有效的Cards对象
    '编制:刘兴洪
...
Private Function CheckReadCard() As Boolean
    ...
    If (mobjCurCard.名称 = "姓名" Or mobjCurCard.名称 = "姓名或就诊卡") And mlngDefaultCardID > 0 Then
        ...
        If objCurCard.是否刷卡 Then Exit Function
        CheckReadCard = True
    End If
End Function
...
Public Function SetBrushCardObject(ByVal blnComm As Boolean) As Boolean
    ...
    If mobjCurCard Is Nothing Or AllowAutoCommCard = False Or mobjTxtInput Is Nothing Then Exit Function
    If mobjCurCard.接口序号 = 0 Or mobjCurCard.接口程序名 = "" Or Not mobjCurCard.是否刷卡 Then Exit Function
    If gobjSquare.zlSetBrushCardObject(mobjCurCard.接口序号, IIf(blnComm, mobjTxtInput, Nothing), strExpend) Then
        If mobjCommEvents Is Nothing Then
            Set mobjCommEvents = New clsCommEvents
            Call gobjSquare.zlInitEvents(UserControl.Hwnd, mobjCommEvents)
        End If
    End If
...
Public Function zlFindPatient(ByVal strInput As String, objPati As PatiInfor, _
    Optional objCard As Card, Optional strErrMsg As String, Optional ByVal blnBrushCard As Boolean) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    zlFindPatient = GetPatient(strInput, objPati, objCard, strErrMsg)
End Function

Public Function zlGetPatiInfor(ByVal lng病人ID As Long, ByRef objPati As PatiInfor, _
    ...
    zlGetPatiInfor = zlGetPati(mcnOracle, lng病人ID, objPati, strErrMsg)
End Function

Private Function GetPatient(ByVal strInput As String, objPati As PatiInfor, ByRef objCard As Card, _
    ...
    On Error GoTo errHandle
    If gobjSquare Is Nothing Then GoTo NotFindPati:
    
    '指定卡类别
    If objCard Is Nothing Then Set objCard = GetCurCard
    If objCard.接口序号 > 0 Then
        If gobjSquare.zlGetPatiID(objCard.接口序号, strInput, False, lng病人ID, strCardPassWord, strErrMsg, _
        ...
        End If
        If zlGetPati(mcnOracle, lng病人ID, objPati, strErrMsg) = False Then Exit Function
    ...
    If objCard.名称 Like "姓名*" Or objCard.模糊查找项 Then
        ...
        If blnBrushCard Then
            If gobjSquare.zlGetPatiID(lngCardTypeID, strInput, False, lng病人ID, strCardPassWord, strErrMsg, lngCardTypeID) Then
                If lngCardTypeID <> objCard.接口序号 And lngCardTypeID <> 0 Then
                     Set objCard = GetIDKindCard(lngCardTypeID, CardTypeID)
            ...
            ElseIf IsMobileNo(strInput) Then
                If gobjSquare.zlGetPatiID("手机号", strInput, False, lng病人ID, strCardPassWord, strErrMsg) Then
    ...
    Case "姓名", "姓名或就诊卡"
        If Left(strInput, 1) = "-" And IsNumeric(Mid(strInput, 2)) Then '病人ID
            If Val(Mid(strInput, 2)) > 2147483647 Then
        ...
        ElseIf Left(strInput, 1) = "*" And IsNumeric(Mid(strInput, 2)) Then '门诊号
            If zlGetPati(mcnOracle, 0, objPati, strErrMsg, "门诊号", Mid(strInput, 2)) = False Then Exit Function
        ElseIf Left(strInput, 1) = "+" And IsNumeric(Mid(strInput, 2)) Then '住院号
            If zlGetPati(mcnOracle, 0, objPati, strErrMsg, "住院号", Mid(strInput, 2)) = False Then Exit Function
        ElseIf IsMobileNo(strInput) Then
            '103000：李南春，2017/2/7，按手机号查找
            If gobjSquare.zlGetPatiID("手机号", strInput, False, lng病人ID, strCardPassWord, strErrMsg) = False Then lng病人ID = 0
            If lng病人ID = 0 Then GoTo NotFindPati:
            If zlGetPati(mcnOracle, lng病人ID, objPati, strErrMsg) = False Then Exit Function
        Else
            '按姓名全字匹配查找
            If zlGetPati(mcnOracle, 0, objPati, strErrMsg, "姓名", strInput) = False Then Exit Function
        End If
        GetPatient = True: Exit Function
    Case "医保号", "门诊号", "住院号", "就诊卡", "IC卡号", "手机号"
        If gobjSquare.zlGetPatiID(objCard.名称, strInput, False, lng病人ID, strCardPassWord, strErrMsg, lngCardTypeID) = False Then lng病人ID = 0
        If lng病人ID = 0 Then GoTo NotFindPati:
        If lngCardTypeID <> objCard.接口序号 And lngCardTypeID <> 0 Then
             Set objCard = GetIDKindCard(lngCardTypeID, CardTypeID)
        End If
