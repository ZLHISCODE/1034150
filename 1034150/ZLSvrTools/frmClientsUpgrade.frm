...
Begin VB.Form frmClientsUpgrade 
   ...
   Begin MSComctlLib.ImageList ilsIcon 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgrade.frx":06FF
            Key             =   ""
         EndProperty
      EndProperty
   End
   ...
   Begin VB.Image imgMain 
      Height          =   480
      Left            =   150
      Picture         =   "frmClientsUpgrade.frx":11C9
      Top             =   615
      Width           =   480
   End
...
Private Sub Form_Resize()
    ...
    Call SetCtrlPosOnLine(False, 0, cmdRefresh, 45, cmd应用, 45, cmd保存, 60, OptType(0), 45, OptType(1))
    cmdFile.Top = cmdRefresh.Top
    cmdFile.Left = Me.ScaleWidth - cmdFile.Width - 120
    
    Call SetCtrlPosOnLine(False, 0, cmdFile, (45 + cmd预升级设置.Width + cmdFile.Width) * -1, cmd预升级设置, (45 + cmd用户密码设置.Width + cmd预升级设置.Width) * -1, cmd用户密码设置, (45 + cmd用户密码设置.Width + cmdClientModify.Width) * -1, cmdClientModify, (700 + cmdClientModify.Width + cmdkillProcess.Width) * -1, cmdkillProcess)
End Sub
...
Private Sub LoadClientsInfor(Optional blnRefresh As Boolean)
    ...
    "           升级标志, 收集标志, 禁止使用, 连接数, 升级服务器,FTP服务器,预升时点,预升完成,管理员用户,管理员密码" & _
    "   From zlClients" & _
    "   Order by IP"
...
'            strKey = IIf(Val(strKey) = 0, "", strKey)
            ...
            .TextMatrix(i, .ColIndex("管理员")) = Nvl(mrsClients!管理员用户)
            .TextMatrix(i, .ColIndex("密码")) = Decipher(Nvl(mrsClients!管理员密码))
