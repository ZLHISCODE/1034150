...
Private Function GetRegFunFile() As String
    With cdgFile
        ...
        .Filter = "注册函数文件(ZLREGIST.PLB)|ZLREGIST.PLB"
...
'功能：验证系统注册授权的正确性，并返回单位名称
    ...
    If strRegErr <> "" Then
        Me.Visible = False
        If strRegErr Like "注册验证函数不正确*" Then
            If Not (gblnDBA Or gblnOwner) Then
                Call MsgBox(strRegErr & vbCrLf & "可能是服务器管理工具版本和数据库不匹配！", vbInformation, "提示")
                Exit Function
            End If
            If MsgBox(strRegErr & vbCrLf & "现在要重新创建注册函数吗（只有当系统升级的时候需要）？", vbYesNo + vbDefaultButton2, "提示") = vbNo Then
                End
            Else
                '先用缺省密码ZLTOOLS执行连接
                strPassword = "ZLTOOLS"
...
openconn:       Set cnTools = gobjRegister.GetConnection(strServerName, "ZLTOOLS", strPassword, False, OraOLEDB, strError, False)
                ...
                If strError <> "" Then
                    strPassword = InputBox("注册函数验证失败，即将重新创建注册函数(ZLREGIST.PLB)。" & vbCrLf & "需要以ZLTOOLS用户登录执行，请输入该用户的密码", "提示")
                    If strPassword = "" Then
                        End
                    Else
                        strError = ""
                ...
                If Not gblnInIDE Then '增加多环境支持
                    strRegFunFile = App.Path & "\TOOLS\zlRegist.plb"
                Else
                    strRegFunFile = "C:\APPSOFT\TOOLS\zlRegist.plb"
                End If
