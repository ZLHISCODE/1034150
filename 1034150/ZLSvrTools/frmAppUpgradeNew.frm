...
'==公共接口
'====================================================================
Public Function ShowMe(frmParent As Object, ByVal rsSysInfo As ADODB.Recordset, ByVal rsSysFiles As ADODB.Recordset, Optional ByVal blnExecBef As Boolean) As Boolean
 '功能：公共入口
 '返回：是否升级完成后退出
...
Private Sub Form_Load()
    ...
    Call SetCtrlEnabled(lblDba.Tag <> "" Or chkStat.value = 1 And optStatType(ST_升级收集).value, lblDBAUser, txtDBAUser, lblDBAPwd, txtDBAPwd)
...
ReDo:
    ...
    If Not mblnExecBef Then
        ...
        If MsgBox("数据升迁完成后，需要对客户端站点进行部件升级," & vbCrLf & "需要对站点部件升级进行配置吗?", vbYesNo + vbQuestion + vbDefaultButton1, gstrSysName) = vbYes Then
            On Error Resume Next    '为了保证延迟脚本的运行
            If GetUpType Then
                frmFilesFTPSet.ShowEdit Me, False, True
            Else
                frmFilesSet.ShowEdit Me, False, True
            End If
            On Error GoTo 0
        End If
        Call SetStepStateImg(vsnStep, True)  '开始执行
