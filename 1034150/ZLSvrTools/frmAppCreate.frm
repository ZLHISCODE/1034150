...
Private Function SysInstall() As Boolean
    ...
    End If
    '运行大对象数据安装
    If InstallFileData(mcnOwner, "zlAppData", False) = 3 Then GoTo ErrHand
    ...
    If chkSelData.value = 1 Then
        For intCount = 0 To optData.UBound
            If optData(intCount).value = True Then
                ...
                End If
                '运行大对象数据安装
                Call InstallFileData(mcnOwner, "zlSelData" & optData(intCount).Tag, True)
                ...
                For intItems = 0 To lstData(intCount).ListCount - 1
                    If lstData(intCount).Selected(intItems) = True Then
                        ...
                        End If
                        '运行大对象数据安装
                        Call InstallFileData(mcnOwner, "zlSelData" & optData(intCount).Tag & lstData(intCount).ItemData(intItems), True)
                    End If
                Next
            End If
        Next
...
Private Function GrantToRole(ByVal cnThis As ADODB.Connection, ByVal cllRoles As Collection, ByVal strOwnerName As String) As Boolean
    ...
    For i = 0 To cllRoles.Count
        ...
        Call GrantSpecialToRole(cnThis, strRoleName, False, strOwnerName, True)
...
Private Function InstallFileData(cnThisDB As ADODB.Connection, ByVal strFileName As String, _
    Optional ResumeNext As Boolean, Optional IgnoreUnique As Boolean = False) As Byte
'----------------------------------------------
'功能：安装大对象数据，数据来源都是一个个独立文件
'参数：
'   cnThisDB:   指定的数据库连接
'   FileName:   脚本文件，注意不带扩展名
'   ResumeNext: 是否错误继续
'   IgnoreUnique:  对于发生的唯一约束错误，是否忽略
'返回：1-执行成功；2-存在错误但继续执行完毕；3-错误而中断
'----------------------------------------------
    Dim lngLines As Long
    Dim strPath As String
    Dim str描述文件  As String, strTemp As String
    Dim strZip As String, zipfilesIn As ZIPnames, zipfilesEx As ZIPnames
    Dim arrFields As Variant, arrSection As Variant
    Dim lngCount As Long
    
    Dim rsTemp As New ADODB.Recordset
    Dim rs数据 As New ADODB.Recordset
    Dim bln有效行 As Boolean
    
    Const conChunkSize As Integer = 10240
    Dim lngFileSize As Long, lngCurSize As Long, lngModSize As Long
    Dim intBolcks As Integer, lngFileNum As Long
    Dim aryChunk() As Byte
    
    rs数据.CursorLocation = adUseClient
    '重新组合成文件名
    strZip = mstrIniPath & "附加文件\" & strFileName & "-附加.zip"
    If gobjFile.FileExists(strZip) = False Then
        '如何不存在附加文件，就认为当前安装的数据没有附加文件
        InstallFileData = 1
        Exit Function
    End If
    ...
    '获得临时目录的位置
    strPath = Space(256)
    lngCount = GetTempPath(256, strPath)
    strPath = Mid(strPath, 1, InStr(strPath, Chr(0)) - 1) & strFileName & "-附加"
    
    '解压文件
    If VBUnzip(strZip, strPath, 1, 1, 0, 0, 0, 0, zipfilesIn, zipfilesEx) = False Then
        InstallFileData = 3
        Exit Function
    End If
    
    str描述文件 = strPath & "\setup.ini"
    On Error Resume Next
    Set objText = gobjFile.OpenTextFile(str描述文件)
    If err <> 0 Then
        err.Clear
        MsgBox "无法打开大对象描述文件 " & str描述文件 & "，执行中断。", vbExclamation, gstrSysName
        InstallFileData = 3
        Exit Function
    End If
    ...
    Do While Not objText.AtEndOfStream
        ...
        If Left(strTemp, 2) = "--" Or strTemp = "" Then
            bln有效行 = False
        Else
            '处理数据
            bln有效行 = True
            arrSection = Split(strTemp, " ")
            If UBound(arrSection) - LBound(arrSection) <> 5 Then
                If ResumeNext Then
                    InstallFileData = 2
                    '该行数据已不能安装
                    bln有效行 = False
                Else
                    MsgBox "文件" & str描述文件 & "第" & objText.Line & "行描述不完整，导致执行中断。", vbExclamation, gstrSysName
                    InstallFileData = 3
                    Exit Function
                End If
            End If
        ...
        If bln有效行 = True Then
            '该行数据可以安装
            gstrSQL = "select * from " & arrSection(sec表名) & " where rownum<1"
            If rsTemp.State = adStateOpen Then
                If rsTemp.Source <> gstrSQL Then
                    '只有在数据表不同时才重新打开
                    rsTemp.Close
                    rsTemp.Open gstrSQL, cnThisDB, adOpenStatic, adLockReadOnly
                End If
            Else
                rsTemp.Open gstrSQL, cnThisDB, adOpenStatic, adLockReadOnly
            End If
            
            If rs数据.State = adStateOpen Then
                '关闭对记录集的编辑
                If rs数据.EditMode <> adEditNone Then rs数据.CancelUpdate
                rs数据.Close
            End If
            arrFields = Split(arrSection(sec主键), "|")
            If UCase(Trim(arrSection(sec操作方法))) = "INSERT" Then
                '该项数据是新增
                rs数据.Open gstrSQL, cnThisDB, adOpenStatic, adLockOptimistic
                rs数据.AddNew
                For lngCount = LBound(arrFields) To UBound(arrFields) Step 2
                    '字段名与其值是间隔列出
                    '首先把主键各字段的值更新
                    Select Case rsTemp.Fields(arrFields(lngCount)).Type
                        Case adNumeric, adVarNumeric
                            '数值
                            rs数据(arrFields(lngCount)) = Val(arrFields(lngCount + 1))
                        Case adVarChar, adChar
                            '字符串
                            rs数据(arrFields(lngCount)) = arrFields(lngCount + 1)
                        Case adDBTimeStamp
                            '日期
                            rs数据(arrFields(lngCount)) = CDate(arrFields(lngCount + 1))
                    End Select
                Next
            Else
                '该项数据是更新
                '生成SQL语句
                gstrSQL = "select * from " & arrSection(sec表名) & " where "
                For lngCount = LBound(arrFields) To UBound(arrFields) Step 2
                    Select Case rsTemp.Fields(arrFields(lngCount)).Type
                        Case adNumeric, adVarNumeric
                            '数值
                            gstrSQL = gstrSQL & arrFields(lngCount) & "=" & arrFields(lngCount + 1) & " And "
                        Case adVarChar, adChar
                            '字符串
                            gstrSQL = gstrSQL & arrFields(lngCount) & "='" & Replace(arrFields(lngCount + 1), "'", "''") & "' And "
                        Case adDBTimeStamp
                            '日期
                            gstrSQL = gstrSQL & arrFields(lngCount) & "=To_date('" & arrFields(lngCount + 1) & "','yyyy-MM-dd HH24:mi:ss') And "
                    End Select
                Next
            ...
            If rs数据.RecordCount = 1 Then
                '只有这种情况才需要更新内容
                
                strTemp = strPath & "\" & arrSection(sec文件名)
                
                If gobjFile.FileExists(strTemp) = False Then
                    '需要安装的大对象文件不存在
                    If ResumeNext Then
                        '文件不存在，那就没有更新的必要了
                        rs数据.CancelUpdate
                        InstallFileData = 2
                    Else
                        MsgBox "文件" & strTemp & "不存在，导致执行中断。", vbExclamation, gstrSysName
                        InstallFileData = 3
                        Exit Function
                    End If
                Else
                    '文件存在，可以安装
                    If UCase(Trim(arrSection(sec字段类型))) = "LONG" Then
                        '文件字段
                        rs数据(arrSection(sec字段名)) = gobjFile.OpenTextFile(strTemp).ReadAll
                    Else
                        '图片字段
                        lngFileNum = FreeFile
                        Open strTemp For Binary Access Read As lngFileNum
                        ...
                        '清空数据
                        rs数据(arrSection(sec字段名)) = Null
                        For lngCount = 0 To intBolcks
                            ...
                            rs数据(arrSection(sec字段名)).AppendChunk aryChunk()
                        Next lngCount
                        '及时关闭文件
                        Close lngFileNum
                    End If
                ...
                If err <> 0 Then
                    ...
                    If ResumeNext Then
                        InstallFileData = 2
                    Else
                        If IgnoreUnique = True And InStr(strTemp, "ORA-00001:") > 0 Then '违反唯一约束条件
                            InstallFileData = 2
                        Else
                            MsgBox "由于文件" & str描述文件 & "中存在下面错误导致执行中断：" & vbCr & gstrSQL, vbExclamation, gstrSysName
                            InstallFileData = 3
                            Exit Function
                        End If
                    End If
    ...
    objText.Close
    '删除临时目录
    gobjFile.DeleteFolder strPath, True
