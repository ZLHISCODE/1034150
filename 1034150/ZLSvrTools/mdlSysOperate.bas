        ...
        Else
            ...
            Do While Not rsCurFiles.EOF
                ...
                If strBanner <> strTmp Then '排序后的记录,小版本不同,需要重新获取该小版本的最大特殊SP版本
                    ...
                    Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, "获取最大SP版本", lngSys)
...
Public Sub ReGrantToRole(ByVal cnOracle As ADODB.Connection, ByVal strRoleNames As String, ByVal blnGrantBase As Boolean, Optional ByVal strOwners As String, Optional ByRef objProcess As Object, Optional ByRef objlblPer As Object, Optional ByRef lngRoleCount As Long)
'功能：对角色进行重新授权。
'参数：cnOracle=连接
...
Public Function GrantSpecialToRole(ByVal cnOracle As ADODB.Connection, ByVal strRoleNames As String, ByVal blnGrantBase As Boolean, strOwners As String, Optional ByVal blnCreateRole As Boolean) As Boolean
    ...
    On Error GoTo errH
    Call ExecuteProcedure("Zl_Grantspecialtorole('" & strRoleNames & "','" & strOwners & "'," & IIf(blnGrantBase, 1, 0) & "," & IIf(blnCreateRole, 1, 0) & ")", "特殊授权", cnOracle)
...
'返回：参数id, 站点, 部门id,部门, 部门简码,用户名, 人员id,人员,人员简码,机器名,机器名简码,参数值
    ...
    On Error GoTo errH
    strSQL = "Select Nvl(系统, 0) 系统, Nvl(私有, 0) 私有, Nvl(本机, 0) 本机 From zlParameters A Where ID =[1]"
    ...
    End If
    If Not (int本机 = 0 And int私有 = 0) And rsSysTems Is Nothing Then
