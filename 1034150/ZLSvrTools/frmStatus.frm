    ...
    With rsTemp
        ...
        Do While Not .EOF
            If strSID <> .Fields("AUDSID").value Then
                ...
                objItem.SubItems(1) = "" & .Fields("部门").value
                objItem.SubItems(2) = "" & .Fields("Terminal").value
                objItem.SubItems(3) = "" & .Fields("姓名").value
                objItem.SubItems(4) = "" & .Fields("USERNAME").value
                objItem.SubItems(5) = "" & .Fields("PROGRAM").value
                objItem.SubItems(6) = "" & .Fields("OsUser").value
                objItem.SubItems(7) = "" & .Fields("进入时间").value
                Select Case "" & .Fields("STATUS").value
                Case "ACTIVE"
                    objItem.SubItems(8) = "当前"
                Case "INACTIVE"
                    objItem.SubItems(8) = "正常"
                Case Else
                    objItem.SubItems(8) = "其他"
                End Select
                If gblnRac Then
                    objItem.SubItems(9) = "" & .Fields("INST_ID").value
                Else
                    objItem.SubItems(9) = 1
                End If
                objItem.SubItems(10) = "" & .Fields("SID").value
                objItem.SubItems(11) = "" & .Fields("SERIAL#").value
            End If
            .MoveNext
        Loop
    End With
...
Private Sub cmdKillSession_Click()
    ...
    With lvw上机用户
                ...
                For i = 1 To .ListItems.Count
                    If .ListItems(i).Checked = True Then
                        If .ListItems(i).SubItems(8) <> "当前" Then
                            Call KillSessions(Trim(.ListItems(i).SubItems(10)) & "," & Trim(.ListItems(i).SubItems(11)))
                        Else
        ...
        Else
            ...
            Next
            If .ListItems(i).SubItems(8) <> "当前" Then
                If MsgBox("结束会话可能导致该用户未保存的数据丢失，你确定要立即结束用户 " & .ListItems(i).SubItems(4) & "(AUDSID:" & .ListItems(i).Text & ") 的会话吗？", vbQuestion + vbYesNo, gstrSysName) = vbYes And blnSelect = True Then
                    Call KillSessions(Trim(.ListItems(i).SubItems(10)) & "," & Trim(.ListItems(i).SubItems(11)))
                End If
            Else
                Call MsgBox("不能结束状态为""当前""的会话", vbInformation, gstrSysName)
    ...
    If KeyCode = vbKeyReturn And txtFind.Text <> "" Then
        ...
        With lvw上机用户
            For intRow = mintCount + 1 To .ListItems.Count
                If InStr(UCase(.ListItems(intRow).SubItems(3)), UCase(txtFind.Text)) > 0 Or InStr(.ListItems(intRow).SubItems(4), UCase(txtFind.Text)) > 0 Then
                    mintCount = intRow
