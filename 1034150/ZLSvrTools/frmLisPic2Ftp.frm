...
Private Sub ContinueUpload()
    '功能:将本地未上传的图片上传至FTP服务器
    Dim dblTime  As Double, i As Long
        
    '进度显示
    pctResult.Visible = False
    pctProgress.Visible = True
    lblProgress(0).Caption = "共有" & mlngImgUp & "张图片待转存"
    lblProgress(1).Caption = "已经转存0条"
    pgsBar.Max = mlngImgUp
           
    dblTime = GetTickCount
    With fileList
        For i = 0 To mlngImgUp - 1
            DoEvents
            If InStr(1, .List(i), " ") = 0 Then
                If mclsFtp.FuncUploadFile(mstrFtpPath, mstrTmpPath & "\" & .List(i), Mid(.List(i), InStrRev(.List(i), "\") + 1)) = 0 Then
                    Kill mstrTmpPath & "\" & .List(i)
                End If
            Else
                If i > 0 Then
                    mclsFtp.FuncFtpCommand mstrFtpPath, "cdup"
                    mclsFtp.FuncFtpCommand mstrFtpPath, "cdup"
                End If
                mclsFtp.FuncFtpCommand mstrFtpPath, "mkd " & Split(.List(i), " ")(0) '这里要先创建首级目录
                mclsFtp.FuncFtpCommand mstrFtpPath, "mkd " & Split(.List(i), " ")(0) & "/" & Split(.List(i), " ")(1)
                If mclsFtp.FuncUploadFile(Split(.List(i), " ")(0) & "/" & Split(.List(i), " ")(1), mstrTmpPath & "\" & .List(i), "" & Split(.List(i), " ")(2)) = 0 Then
                    Kill mstrTmpPath & "\" & .List(i)
                End If
            End If
            lblProgress(1).Caption = "已经上传" & i + 1 & "张数据"
            pgsBar.value = i + 1
        Next
    End With
    ...
    dblTime = GetTickCount - dblTime
    lblReult(1).Caption = "需上传图片-" & i & ";上传失败-" & mlngImgUp
    lblReult(2).Caption = "共计耗时:" & Format(dblTime / 1000, "0.00") & "S"
End Sub
...
Private Sub cmdMulti_Click()
    ...
    '检查当前文件夹下是否有未上传的图片
    If CheckImg Then
         If MsgBox("当前临时路径下共有" & mlngImgUp & "张图片未上传，是否继续上传" _
            , vbYesNo + vbQuestion + vbDefaultButton1, "确认") = vbYes Then
            Call ContinueUpload
            SetCmdEnable True
            Exit Sub
        End If
    ...
    For i = 1 To intProcNum
        ...
        End If
        Shell """" & strPath & "\zlLisPic2FtpSub.exe"" ""zlUserName=" & gstrUserName & "zlPassword=" & gstrLoginUserPwd & "zlServer=" & gstrServer & " ", vbMaximizedFocus
    Next
    Call Timer_Timer
