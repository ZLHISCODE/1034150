...
'====7z压缩解压=========
'CompressionCmd:生成压缩命令
'DeCompressionCmd：生成解压命令
'GetCmdTxt:管道执行CMD命令并截取输出
'====进程操作==============
...
' 返回：获取的参数记录集
    ...
    If lngSys <> -9 Then
        ...
        strSQL = "Select 0 类型, Nvl(a.系统, 0) 系统," & vbNewLine & _
                        "       Nvl(a.模块, 0) 模块, Nvl(a.私有, 0) 私有, Nvl(a.本机, 0) 本机,  Nvl(a.授权, 0) 授权, Nvl(a.固定, 0) 固定, a.参数号, a.参数名, a.参数值, a.缺省值, a.参数说明," & vbNewLine & _
                        "       0 部门id, Null 用户名, Null 机器名," & vbNewLine & _
        ...
        If blnDetails Then
            ...
            strSQL = strSQL & vbNewLine & _
                            ...
                            "Select 2 类型, Nvl(a.系统, 0) 系统, Nvl(a.模块, 0) 模块, Null  私有,  Null 本机,Null  授权, Null  固定,a.参数号, a.参数名, Null 参数值,Null 缺省值," & vbNewLine & _
                            "       Null 参数说明," & vbNewLine & _
                            "       Null 部门id, c.用户名 用户名, c.机器名 机器名, c.参数值 详细参数值" & vbNewLine & _
                            "From zlParameters A, zlUserParas C" & vbNewLine & _
                            "Where a.Id = c.参数id And (Nvl(a.私有, 0) = 1 Or Nvl(a.本机, 0) = 1)" & IIf(lngSys = -1, "", " And Nvl(a.系统, 0)=[1]")
    ...
    strSQL = IIf(lngSys <> -9, strSQL & vbNewLine & _
                    "Union All " & vbNewLine, "") & _
                    "Select -9 类型, Nvl(编号, 0) 系统, Null 模块, Null 私有, Null 本机, Null 授权,Null 固定,Null 参数号, 名称 参数名, 版本号 参数值, b.计数 || ''   缺省值," & vbNewLine & _
                    "       Null 参数说明," & vbNewLine & _
    ...
    If blnAddSets And lngSys <> -9 Then
        ...
        strSQL = strSQL & vbNewLine & _
                        "Union All " & vbNewLine & _
                        "Select -99 类型, Null 系统, Null 模块, " & IIf(blnDetails, 1, 0) & " 私有, Null 本机, Null 授权,Null 固定,  " & lngSys & " 参数号, Null 参数名, To_Char(Sysdate, 'yyyy-mm-dd HH24:mi:ss')  参数值, User 缺省值," & vbNewLine & _
                        "       Null 参数说明," & vbNewLine & _
                        "       Null 部门id, Null 用户名, Null 机器名, Null 详细参数值" & vbNewLine & _
                        "From Dual"
    End If
