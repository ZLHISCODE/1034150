...
Private Function LoadHistorySpace(ByVal lng系统 As Long) As Boolean
    ...
    With lvwHistory
        ...
        Do While Not rsbakspaces.EOF
            ...
            Call OpenRecordset(rsTmp, gstrSQL, gstrSysName, , , gcnOracle) '创建历史空间后，所有权限都授给了应用系统的所有者的，所以应该能访问
...
Private Sub cmdFunction_Click(Index As Integer)
    ...
    Case F7结构修正
        If gstrUserName = cmbSystem.Tag Then
...
Private Sub Form_Resize()
    ...
    With cmdFunction(F0创建)
        ...
        cmdFunction(F7结构修正).Top = .Top
        
        cmdFunction(F1拆卸).Left = .Left + .Width + 15
        cmdFunction(F2再植).Left = cmdFunction(F1拆卸).Left + cmdFunction(F1拆卸).Width + 15
        
        cmdFunction(F7结构修正).Left = ScaleWidth - cmdFunction(F7结构修正).Width - 60
        cmdFunction(F6转移).Left = cmdFunction(F7结构修正).Left - cmdFunction(F6转移).Width - 15
