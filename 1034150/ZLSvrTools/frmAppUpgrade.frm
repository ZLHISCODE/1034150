...
Begin VB.Form frmAppUpgrade 
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsSysSel 
         ...
         FormatString    =   $"frmAppUpgrade.frx":05D4
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VB.Label lblExplain 
         ...
         BackColor       =   &H80000005&
         BackStyle       =   0  'Transparent
         Caption         =   $"frmAppUpgrade.frx":06BC
         ForeColor       =   &H80000008&
         Height          =   465
         Left            =   120
         TabIndex        =   17
...
Private Sub cmdExec_Click()
    ...
    If VilidateUpgrade Then
        ...
        If optUpMode(1).value Then
            If Not CheckRushHours("当前时段处于业务高峰期，提前执行可能会对系统使用造成一定影响，是否继续？") Then
                Exit Sub
            End If
        End If
        If MsgBox("请检查当前勾选升级的系统是否符合要求，是否确认升级？", vbInformation + vbYesNo, gstrSysName) = vbYes Then
            ...
            Call objfrmUpSys.ShowMe(frmMDIMain, mrsSysInfo, mrsSysUpFiles, optUpMode(1).value)
            vsSysSel.Tag = ""
            Call LoadSystems
            Call LoadData(0)
