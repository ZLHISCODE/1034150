...
RUMMan:
    ...
    strInfo = "角色分组"
RUMGroup:
    strSQL = "Select Distinct  组名 From zlrolegroups Where 角色 Is Not Null order by 组名"
    Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, Me.Caption)
    With cboRoleGroups
        .Clear
        .AddItem "所有角色"
...
'功能，获取角色数据以及角色对应的模块数据
    ...
    If gblnDBA Then
        strSQL = "Select User As Grantee, g.Role, Decode(n.Granted_Role, Null, 0, 1) As Granted," & vbNewLine & _
                        "       Decode(n.Admin_Option, 'YES', 1, 0) As Admin, b.组名, Zlspellcode(Substr(g.Role, 3)) As 简码," & vbNewLine & _
                        "       Substr(g.Role, 4) Rolename, Decode(n.Granted_Role, Null, 0, 1) 勾选,Decode(n.Admin_Option, 'YES', 1, 0) 转授,1 展示,0 改变" & vbNewLine & _
                        "From (Select Role From Dba_Roles Where Role Like 'ZL_%') g," & vbNewLine & _
                        "     (Select Granted_Role, Admin_Option From Dba_Role_Privs Where Grantee = '" & mstrUser & "' And Granted_Role Like 'ZL_%') n," & vbNewLine & _
                        "     Zlrolegroups b" & vbNewLine & _
                        "Where g.Role = n.Granted_Role(+) And g.Role = b.角色(+)" & vbNewLine & _
                        "Order By g.Role"
    Else
        strSQL = "Select User As Grantee, g.Role, Decode(n.Granted_Role, Null, 0, 1) As Granted," & vbNewLine & _
                        "       Decode(n.Admin_Option, 'YES', 1, 0) As Admin, b.组名, Zlspellcode(Substr(g.Role, 3)) As 简码," & vbNewLine & _
                        "       Substr(g.Role, 4) Rolename, Decode(n.Granted_Role, Null, 0, 1) 勾选,Decode(n.Admin_Option, 'YES', 1, 0) 转授,1 展示,0 改变" & vbNewLine & _
                        "From (Select Grantee, Granted_Role" & vbNewLine & _
                        "       From Dba_Role_Privs" & vbNewLine & _
                        "       Where Grantee = '' And Admin_Option = 'YES' And Granted_Role Like 'ZL_%'" & vbNewLine & _
                        "       Union All" & vbNewLine & _
                        "       Select Grantee, Granted_Role" & vbNewLine & _
                        "       From Dba_Role_Privs a" & vbNewLine & _
                        "       Where Grantee = 'SYSTEM' And Admin_Option = 'YES' And Granted_Role Like 'ZL_%' And Not Exists" & vbNewLine & _
                        "        (Select 1 From Dba_Role_Privs x Where x.Granted_Role = a.Granted_Role And Grantee = '" & mstrUser & "')) g," & vbNewLine & _
                        "     (Select Granted_Role, Admin_Option From Dba_Role_Privs Where Grantee = '" & mstrUser & "' And Granted_Role Like 'ZL_%') n," & vbNewLine & _
                        "     Zlrolegroups b" & vbNewLine & _
                        "Where g.Granted_Role = n.Granted_Role(+) And g.Granted_Role = b.角色(+)" & vbNewLine & _
                        "Order By g.Granted_Role"
    End If
    strInfo = "角色数据"
    On Error GoTo ErrH
RUMRole:
    Set mrsRole = Nothing
    Set mrsRole = CopyNewRec(gclsBase.OpenSQLRecord(gcnOracle, strSQL, Me.Caption))
    strSQL = "Select a.角色, a.序号, a.功能, b.标题, b.说明" & vbNewLine & _
                    "From Zlrolegrant a, Zlprograms b" & vbNewLine & _
                    "Where a.序号 = b.序号 And Nvl(a.系统, 0) = Nvl(b.系统, 0)" & vbNewLine & _
                    "Order By a.角色, a.序号"
    strInfo = "角色模块数据"
RUMModule:
...
Private Function GetRoleFilter() As String
    ...
    strGroup = cboRoleGroups.Text
    If strGroup = "所有角色" Then strGroup = ""
    '进行组过滤
    strFilter = IIf(strGroup = "", "", "组名='" & strGroup & "'")
...
'按所选取的角色列出所有的模块及功能
    ...
    Do While Not mrsRole.EOF
        strRole = strRole & " OR (角色='" & mrsRole!Role & "')"
        mrsRole.MoveNext
    Loop
    strRole = Mid(strRole, Len(" OR "))
    '过滤界面上展示并勾选的角色对应的模块，并以序号功能排序
    mrsModule.Filter = strRole: mrsModule.Sort = "序号,功能"
    Do While Not mrsModule.EOF
        If strModule <> mrsModule!序号 & "" Then
            '处理上一个模块的功能
            If strModule <> "" Then
                lst.SubItems(2) = Mid(strAllFun, 2)
            End If
            Set lst = lvwModule.ListItems.Add(, "K" & mrsModule!序号, mrsModule!序号, , "Module")
            lst.SubItems(1) = mrsModule!标题
            lst.SubItems(3) = mrsModule!说明 & ""
            strAllFun = "": strFun = "基本"
            strModule = mrsModule!序号 & ""
        End If
        If strFun <> mrsModule!功能 Then
            If strFun <> "基本" Then
                strAllFun = strAllFun & "," & strFun
            End If
            strFun = mrsModule!功能
        End If
        mrsModule.MoveNext
