...
Private Function Save() As Boolean
    ...
    If mEditType = 新增 Or mStrIP <> strIp Then
    ...
    If mEditType = 修改 And mStrIP <> strIp Then
    ...
    If err <> 0 Then
        MsgBox "已经设置了相同IP地址或工作站,不能再设!" & vbNewLine & err.Description, vbInformation + vbDefaultButton1, gstrSysName
        err.Clear
        Exit Function
    End If
    Save = True
...
Public Sub ShowEdit(ByVal strIp As String, ByVal strCompterName As String, ByVal intEditType As EditType, ByRef blnRetun As Boolean, _
    ...
    mStrIP = strIp
    mstrCompterName = UCase(strCompterName)
