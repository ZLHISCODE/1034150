...
Public gstrLoginPwd As String           '授权用户登陆时输入的密码
...
Public Sub Main()
    ...
    gblnTrace = gblnInIDE
    '是否启用管理工具跟踪
    gblnTrace = Val(GetSetting("ZLSOFT", "公共模块\服务器管理工具", "启用跟踪", "0")) = 1
    ...
    Do
        If (Timer - gdtStart) > 2 Then Exit Do
        DoEvents
    Loop
    
    Set gobjRegister = New clsRegister
...
Public Function CheckRushHours(ByVal strMsg As String, Optional ByVal lngBegin As Long = 8, Optional ByVal lngEnd As Long = 12, Optional ByVal vmrAllowAswner As VbMsgBoxResult = vbYes) As Boolean
'功能：检查当前时间是否是业务高峰期
'参数：strMsg=消息提示内容,当没有内容时，检查到业务高峰立即禁止
'      lngBegin=业务高峰开始小时
'      lngEnd=业务高峰结束小时
'      vmrAllowAswner=允许进行操作的回答
'返回：是否可以进行操作

    Dim lngH As Long
    
    lngH = Val(Mid(Format(CurrentDate, "yyyy-MM-dd HH:mm:ss"), Len("yyyy-MM-dd H"), 2))
    If lngH >= lngBegin And lngH <= lngEnd Then
        If strMsg <> "" Then
            If MsgBox(strMsg, vbInformation + vbYesNo + vbDefaultButton2, gstrSysName) <> vbYes Then
                Exit Function
            End If
        End If
    End If
...
'    D类IP：224.0.0.0到239.255.255.255
    '第一段
    If Val(arrStart(0)) >= 1 And Val(arrStart(0)) <= 239 Then
        ...
        '第二段
        If Val(arrStart(1)) >= 0 And Val(arrStart(1)) <= 255 Then
        ...
        '第三段
        If Val(arrStart(2)) >= 0 And Val(arrStart(2)) <= 255 Then
            If arrEnd(2) <> "" Then
                If Val(arrEnd(2)) >= 0 And Val(arrEnd(2)) <= 255 Then
                    If Val(arrEnd(2)) < Val(arrStart(2)) Then
        ...
        '第四段
        If Val(arrStart(3)) > 0 And Val(arrStart(3)) <= 255 Then
            If arrEnd(3) <> "" Then
                If Val(arrEnd(3)) > 0 And Val(arrEnd(3)) <= 255 Then
                    If Val(arrEnd(3)) < Val(arrStart(3)) Then
