...
Dim strHead As String, strFoot As String    '页眉页脚
    ...
    DocHeadCopyWithFormat
    RTBTmp.SelectAll: RTBTmp.Text = "": RTBTmp.PasteWithFormat: RTBTmp.ClearEndCrlfChar
    ...
    SendMessage RTBNormal.hWndRTB, EM_FORMATRANGE, 0, ByVal CLng(0)
End Sub
...
Public Function PrintDoc(Optional ByVal blnNoAsk As Boolean, Optional ByVal lngStartPage As Long, Optional ByVal lngBlankHeight As Long, _
    ...
    Dim lngCount As Long, lngNumber As Long, lngPWidth As Long, lngPHeight As Long, lngPaperKind As Long
    Err = 0: On Error Resume Next
    Dim intOrientation As Integer
    lngPWidth = Printer.Width
    lngPHeight = Printer.Height
    lngPaperKind = Printer.PaperSize
    '记录打印机初始方向
    intOrientation = Printer.Orientation
    '设置打印机方向
    Printer.Orientation = Me.PaperOrient
'    Printer.Width = mvarPaperWidth
'    Printer.Height = mvarPaperHeight
    '设置纸张，自定义纸张的设置必须放到最后
    ...
    Printer.EndDoc
'    Printer.Width = lngPWidth
'    Printer.Height = lngPHeight
    If lngPaperKind = vbPRPSUser Then
        Call SetCustomPager(UserControl.Hwnd, lngPWidth, lngPHeight)
    Else
        Printer.PaperSize = lngPaperKind
    End If
    If blnNoAsk = False Then
        '恢复默认打印机
        For j = 1 To Printers.Count
            If Printers(j).DeviceName = strOldPrinterName Then Set Printer = Printers(j)
        Next
    End If
    '恢复打印机初始方向
