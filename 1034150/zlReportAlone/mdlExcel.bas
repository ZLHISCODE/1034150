   ...
   If gobjHead Is Nothing Then
            ...
            If gobjBody.RowHeight(i) <> 0 Then
                For j = 0 To gobjBody.Cols - 1
                    If gobjBody.ColWidth(j) <> 0 Then
                        Put intFile, , Replace(Replace(gobjBody.TextMatrix(i, j), "<换行分隔符>", ""), GSTR_SPACE_EX, "")
                        Put intFile, , vbTab
                    End If
                Next
                Put intFile, , vbCrLf
            ...
            If gobjHead.RowHeight(i) <> 0 Then
                For j = 0 To gobjHead.Cols - 1
                    If gobjHead.ColWidth(j) <> 0 Then
                        Put intFile, , Replace(Replace(gobjHead.TextMatrix(i, j), "<换行分隔符>", ""), GSTR_SPACE_EX, "")
                        Put intFile, , vbTab
                    End If
                Next
                Put intFile, , vbCrLf
    ...
    For i = gobjBody.FixedRows To gobjBody.Rows - 1
        If gobjBody.RowHeight(i) <> 0 Then
            For j = 0 To gobjBody.Cols - 1
                If gobjBody.ColWidth(j) <> 0 Then
                    strText = Replace(Replace(gobjBody.TextMatrix(i, j), "<换行分隔符>", ""), GSTR_SPACE_EX, "")
...
'      strExcelFile=指定的输出文件
    ...
    Else
        ...
        gobjExcel.Cells.Select
        gobjExcel.Selection.PasteSpecial Paste:=-4122, Operation:=-4142, SkipBlanks:=False, Transpose:=False
        'gobjExcel.Selection.PasteSpecial Operation:=-4142, SkipBlanks:=False, Transpose:=False
    End If
