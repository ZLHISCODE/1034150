...
Public mblnNonVB As Boolean         '非VB父窗体对象
...
Private Sub CboFormat_Click()
    ...
    '格式的纸张信息
    lblPaper.Caption = "纸张: " _
        & GetPaperName(Nvl(mrsFormat!纸张, 9) _
            , Nvl(mrsFormat!W, INIT_WIDTH) _
            , Nvl(mrsFormat!H, INIT_HEIGHT))
    lblSize.Caption = "尺寸: " _
        & format(Nvl(mrsFormat!W, INIT_WIDTH) / Twip_mm, "#0.0") & "mm(宽) × " _
        & format(Nvl(mrsFormat!H, INIT_HEIGHT) / Twip_mm, "#0.0") & "mm(高)； " _
