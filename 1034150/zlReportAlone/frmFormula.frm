...
Begin VB.Form frmFormula 
   ...
   Begin VB.CommandButton cmdAutoFill 
      ...
      Width           =   990
   End
   Begin VB.CheckBox chkCellSplit 
      Caption         =   "允许单元格自动拆分为多行"
      Height          =   180
      Left            =   1100
      TabIndex        =   11
      Top             =   4620
...
Public mblnCellSplit As Boolean
...
Public mstrAutoFillInfo As String
...
Private mblnAllowCellSplit As Boolean
Private mblnAllowAutoFill As Boolean
...
Private Sub chkAutoRowHeight_Click()
    ...
    chkCellSplit.Enabled = chkAutoRowHeight.Value = 1 And mblnAllowCellSplit
    If Not chkCellSplit.Enabled Then
        chkCellSplit.Value = 0
    End If
...
Private Sub UpdateAutoFillState()
    Dim tmpID As RelatID, tmpItem As RPTItem
    
    '首先通过chkCellSplit判断
    cmdAutoFill.Enabled = chkCellSplit.Value = 1 And mblnAllowAutoFill
    If cmdAutoFill.Enabled Then Exit Sub
    
    '其次通过Item对象“拆分单元格”属性判断
    For Each tmpID In objReport.Items("_" & intCur).SubIDs
        Set tmpItem = objReport.Items("_" & tmpID.id)
        If Not tmpItem Is Nothing Then
            If tmpItem.类型 = Val("6-任意表的列") And tmpItem.拆分单元格 And intCol <> tmpItem.序号 Then
                cmdAutoFill.Enabled = mblnAllowAutoFill
                Exit For
            End If
        End If
    Next
...
Private Sub chkCellSplit_Click()
    If Not Me.Visible Then Exit Sub
    cmdOk.Enabled = True
    Call UpdateAutoFillState
End Sub
...
Private Function UpdateString(ByVal strInfo As String, ByVal intCurrentCol As Integer, ByVal blnSplitCol As Boolean) As String
    ...
    For i = LBound(arrInfo) To UBound(arrInfo)
        If Val("" & arrInfo(i)) = intCurrentCol And blnSplitCol Then
            '隐藏当前列，并且是“拆分单元格”的列
            '略...
        Else
            UpdateString = UpdateString & "|" & arrInfo(i)
        End If
    Next
    If UpdateString <> "" Then UpdateString = Mid$(UpdateString, 2)
End Function

Private Sub cmdAutoFill_Click()
    Dim frmSetup As New frmAutoFill
    
    With frmSetup
        '传入设置信息
        mstrAutoFillInfo = UpdateString(mstrAutoFillInfo, intCol, chkCellSplit.Value = 1 And Me.chkCellSplit.Enabled)
        .mstrColFillInfo = mstrAutoFillInfo
        .Show vbModal, Me
        '接收设置确认的信息
        If .mstrResult <> "" Then
            '更新各列的自动填充属性
            mstrAutoFillInfo = .mstrResult
        End If
    End With
...
Private Sub cmdVerify_Click()
    ...
    '检查“拆分单元格”
    If chkCellSplit.Value = 1 Then
        For Each tmpID In objReport.Items("_" & intCur).SubIDs
            Set tmpItem = objReport.Items("_" & tmpID.id)
            If Not tmpItem Is Nothing Then
                If tmpItem.类型 = Val("6-任意表的列") And tmpItem.拆分单元格 And tmpItem.序号 <> intCol Then
                    If UBound(Split(tmpItem.表头, "^")) >= 2 Then
                        strColName = "【" & Split(tmpItem.表头, "^")(2) & "】"
                    Else
                        strColName = "其他"
                    End If
                    cmdOk.Enabled = False
                    MsgBox "“单元格自动拆分”只支持单列，请检查表格中" & strColName & "列的设置！", vbInformation, App.Title
                    Exit Sub
                End If
            End If
        Next
...
Private Sub Form_Load()
    ...
    If chkAutoRowHeight.Value = 1 Then
        chkCellSplit.Value = IIF(mblnCellSplit, 1, 0)
    Else
        chkCellSplit.Value = 0
    End If
    
    mblnAllowCellSplit = gobjCompatibility.FieldExists("ZLRPTITEMS", "拆分单元格")
    mblnAllowAutoFill = gobjCompatibility.FieldExists("ZLRPTITEMS", "自动填充")
