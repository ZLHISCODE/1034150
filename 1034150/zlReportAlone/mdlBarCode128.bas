Attribute VB_Name = "mdlBarCode128"
'/****************************************************************************
' * Summary   : 条形码生成程序
' * Version   : 1.00
...
' ****************************************************************************/
Public Const STR_CODE_128 = " !""#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~"
...
'Public Function DrawBarCode128(zObj As Object, ByVal zBarH As Integer, ByVal BarText As String, Optional ByVal HasCaption As Boolean) As StdPicture
...
'    Else
'        xObj.Height = (BarH + 5) * Screen.TwipsPerPixelY
'    End If
'
'    'xObj.Height = (xObj.TextHeight(zBarText) + BarH + 25) * Screen.TwipsPerPixelY
''    xObj.Width = ((Test_String(zBarText) + 3) * 11 + 15) * Screen.TwipsPerPixelX
'
'    xObj.Cls
'    Paint_Code zBarText
...
Private Sub PrintB(xstr As String)
    ...
    lngPos = InStr(STR_CODE_128, xstr)
    If lngPos = 0 Then lngPos = 1 'byZT容错显示
    
    posCtr = posCtr + 1
    xTotal = xTotal + ((lngPos - 1) * posCtr)
...
Private Sub Draw_Bar(Encoding As String)
    ...
    For ii = 1 To Len(Encoding)
        xPos = xPos + 1
        xObj.Line (xPos + 2, xTop - 5)-(xPos + 2, xTop + BarH - 5), IIF(Mid(Encoding, ii, 1), vbBlack, vbWhite)
    Next
    ii = 0
End Sub
