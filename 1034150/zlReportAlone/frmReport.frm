...
Private arrReportPars() As RPTPars          '报表组中各个报表的当前参数值
Private arrReportDefPars() As RPTPars       '报表组中各个报表的缺省参数值
Private mstrExcelFile As String
Private mblnAllFormat As Boolean
Private lngPreX As Long
Private lngPreY As Long
...
Private mlngInParamsCondID As Long
Private mintInParamsMenuIdx As Integer
Private marrCurMenuIdx() As Integer
Private marrCurCondID() As Long
...
Private Sub CboFormat_Click()
    ...
    If mobjReport.blnLoad Then
        ...
        If strErr <> "" Then
            MsgBox "在读取报表数据源""" & strErr & """时遇到意外错误，报表不能产生！", vbInformation, App.Title
            Exit Sub
        End If
...
Private Sub Form_Load()
    ...
    Dim strStartTime As String
    Dim strParamName As String, strParamVal As String
    ...
    gtypSpecialPrintPage.Mode = enuPrintMode.常规打印
    bytFormat = 0
    blnExcel = False
    ...
    If gobjReport Is Nothing Then
        ...
        If rsTmp Is Nothing Then
            MsgBox "该报表组中没有任何报表可以执行或无可用的报表格式！", vbInformation, App.Title
        ...
        Do While Not rsTmp.EOF
            ...
            If blnPriv Then
                If i = 0 Then
                    ...
                    ReDim arrReportPars(0)
                    ReDim arrReportDefPars(0)
                    ReDim marrCurCondID(0)
                    ReDim marrCurMenuIdx(0)
                Else
                    Load picPaper(i): picPaper(i).Visible = False
                    ReDim Preserve arrReport(i)
                    ReDim Preserve arrLibDatas(i) '数据暂时未装入
                    ReDim Preserve arrReportPars(i)
                    ReDim Preserve arrReportDefPars(i)
                    ReDim Preserve marrCurCondID(i)
                    ReDim Preserve marrCurMenuIdx(i)
                End If
                
                '报表内容
                Set arrReport(i) = New Report
                Set arrReport(i) = ReadReport(rsTmp!报表id, , , True)
                ...
                '缺省参数内容
                Set arrReportPars(i) = New RPTPars
                Set arrReportPars(i) = MakeNamePars(arrReport(i))
                Call CopyPars(arrReportPars(i), arrReportDefPars(i))
                marrCurCondID(i) = 0
                marrCurMenuIdx(i) = 0
                ...
                '从注册表读上次格式,缺省1
                arrReport(i).bytFormat = CByte(GetSetting("ZLSOFT" _
                        , "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.name & arrReport(i).编号 _
                        , "格式", 1))
...
makContinue:
        ...
        If rsTmp.RecordCount > 0 And lvw.ListItems.count = 0 Then
            MsgBox "你没有权限执行该报表组中的所有报表，请检查报表的合法性，以及是否正确授权！", vbInformation, App.Title
            Unload Me: Exit Sub
    ...
    Else
        ...
        If IsArray(marrPars) Then
            If UBound(marrPars) <> -1 Then
                For i = 0 To UBound(marrPars)
                    ...
                    If j > 0 Then
                        strParamName = Split(marrPars(i), "=")(0)
                        strParamVal = Split(marrPars(i), "=")(1)
                        If UCase(Trim(strParamName)) = UCase("ReportFormat") Then
                            '指定打印的报表格式
                            If IsNumeric(strParamVal) Then
                                bytFormat = CByte(strParamVal)
                                mblnAllFormat = False '程序指定了格式则打印设置无效
                            End If
                        ElseIf UCase(Trim(strParamName)) = UCase("DisabledPrint") Then
                            '只预览不允许打印
                            If IsNumeric(strParamVal) Then
                                mblnDisabledPrint = CByte(strParamVal) = 1
                            End If
                        ElseIf UCase(Trim(strParamName)) = UCase("PrintEmpty") Then
                            '空表打印（False表示强制不打印空表）
                            If IsNumeric(strParamVal) Then
                                mblnPrintEmpty = CByte(strParamVal) = 1
                            End If
                        ElseIf UCase(Trim(strParamName)) = UCase("ExcelFile") Then
                            '输出Excel路径与文件名
                            mstrExcelFile = Trim(strParamVal)
                        ElseIf UCase(Trim(strParamName)) = UCase("PDF") Then
                            '输出PDF路径与文件名
                            mstrPDFFile = Trim(strParamVal)
                        ElseIf UCase$(Trim$(strParamName)) = UCase$("SpecialPrintPage") And mbytStyle = Val("2-打印") Then
                            '特殊打印（3-特殊打印开始；2-特殊打印中；1-特殊打印结束；0-正常打印）
                            If IsNumeric(strParamVal) Then
                                gtypSpecialPrintPage.Mode = CByte(strParamVal)
                            End If
                        End If
                    End If
                Next
        ...
        For i = 1 To mobjReport.Fmts.count
            If mobjReport.Fmts(i).停用 = False Then
        ...
        If cboFormat.ComboItems.count <= 0 Then
            If mbytStyle = 2 And (mobjReport.票据 Or gblnSilentMode) Then
                Call ExportLog(False, "frmReport", "Form_Load", "“" & mobjReport.编号 & "-" & mobjReport.名称 & "”报表无可用的格式！", "")
            Else
                MsgBoxEx "" & mobjReport.编号 & "-" & mobjReport.名称 & vbCr _
                    & "报表无可用的格式，请与管理员联系！", vbInformation, App.Title
            End If
            Unload Me: Exit Sub
        End If
...
'        End If
        ...
        If ParCount(mobjReport) > 0 Then
            If Not ReplaceUserPars(mobjReport) Then
                ...
                Call ShowParInputForm(frmNewParInput, lngReport, 0, 0, mobjPars, mobjDefPars, mobjReport.Datas, False)
                If frmNewParInput.mblnOK Then
                    ...
                    mlngInParamsCondID = frmNewParInput.mlngCondID
                    mintInParamsMenuIdx = frmNewParInput.mintMenuIdx
                    ReplaceInputPars frmNewParInput.mobjPars
                    Unload frmNewParInput
                Else
                    mlngInParamsCondID = 0
                    mintInParamsMenuIdx = 0
...
Private Sub lvw_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    If Not mobjReport Is Nothing Then
        ...
        Call CopyPars(mobjPars, arrReportPars(intReport))   '保存上次报表的参数值至arrReportPars数组
    ...
    Call CopyPars(arrReportPars(intReport), mobjPars)       '当前报表的参数赋给mobjPars（当前参数对象）其他地方会使用该变量
    Set mobjDefPars = arrReportDefPars(intReport)           '当前报表的缺省参数
    ...
    For Each objItem In mobjReport.Items
        If objItem.格式号 = bytFormat Then
            If objItem.类型 = 4 Then
                ...
                End With
            ElseIf objItem.类型 = Val("5-汇总表格") And objItem.性质 = Val("0-无附加表格") Then
...
Private Sub mnuFile_Preview_Click()
    ...
    Set frmShow = New frmPreview
    Set frmShow.frmParent = Me
    Set frmShow.ReportDatas = mLibDatas
    '空表打印只针对业务模块的直接打印方式生效，因此报表执行界面、预览界面都不做控制
    frmShow.NeedPrintEmpty = False
    frmShow.FormatNo = bytFormat
...
Private Sub mnuFile_Print_Click()
    Dim blnDo As Boolean, blnCancel As Boolean, blnReset As Boolean, blnExit As Boolean
    Dim blnPrint As Boolean, blnALLEmpty As Boolean, blnGoOn As Boolean
    Dim k As Integer, i As Integer, j As Integer, intCopy As Integer
    Dim arrBill As Variant
    Dim objFmt As RPTFmt
    Dim strTmp As String, strDefault As String
    Dim lngEndPage As Long, lngPrintH As Long, lngOffsetHeight As Long
    ...
    If gtypSpecialPrintPage.Mode = 常规打印 Or gtypSpecialPrintPage.Mode = 特殊打印开始 Then
        strTmp = GetRegPrinterInfo("PaperCopy", mobjReport.编号, strDefault)
        intCopy = Val(strTmp)
        gtypSpecialPrintPage.Copies = intCopy
    Else
        intCopy = gtypSpecialPrintPage.Copies
    End If
    If intCopy < 1 Then intCopy = 1
    
    gbytExportMode = mbytStyle
    ...
    Do While blnGoOn
        ...
        '直接打印(mbytStyle=2)时,当前格式的表格都为空时,则不打印
        blnExit = False
        'mblnPrintEmpty=False表示程序强制不打印空表； 打印方式=1表示不“空表打印”
        If mblnPrintEmpty = False And mobjReport.打印方式 = Val("1-空表不打印") And mbytStyle = Val("2-打印") Then
            '检查空表
            blnExit = mdlPublic.IsReportEmpty(mobjReport, mLibDatas, bytFormat)
            If blnExit Then
                GoTo NextFormat
            End If
        End If
        ...
        If mbytStyle = Val("4-PDF") Then
            If PDFInitialize(objFmt) Then
                If PDFFile(mstrPDFFile, , , True) = False Then
                    MsgBoxEx "未指定PDF输出路径和文件名，请检查！", vbInformation, App.Title
                    Exit Sub
                End If
            Else
                Exit Sub
            End If
        Else
            If gtypSpecialPrintPage.Mode = enuPrintMode.常规打印 Or gtypSpecialPrintPage.Mode = enuPrintMode.特殊打印开始 Then
                '特殊打印数据初始化
                gtypSpecialPrintPage.Index = 0
                '初始化打印机信息
                If Not InitPrinter(Me, intCopy) Then
                    MsgBoxEx "设备初始化失败，可能是系统没有安装打印机或与当前设置不兼容！", vbInformation, App.Title
                    gblnError = True
                    GoTo ExitHandle
                End If
            End If
        ...
        If Not CalcCellPage Then
            gblnError = True
            MsgBoxEx "无法处理的表格格式,操作不能继续！", vbInformation, App.Title: GoTo ExitHandle
        End If
        If mbytStyle <> 2 And mbytStyle <> 4 Then
            If MsgBoxEx("报表内容即将输出,打印设备准备就绪吗？", vbQuestion + vbYesNo, App.Title) = vbNo Then GoTo ExitHandle
        End If
        ...
        Do
            ...
            If Not IsArray(marrPage) Then
                ...
                '动态计算及设置纸张高度
                If objFmt.动态纸张 And objFmt.纸向 = 1 And gtypSpecialPrintPage.Mode = enuPrintMode.常规打印 Then
                ...
                blnPrint = True
                If gtypSpecialPrintPage.Mode = 常规打印 Then
                    lngOffsetHeight = 0
                Else
                    lngOffsetHeight = (gtypSpecialPrintPage.Index Mod 2) * objFmt.H
                End If
                Call PrintPage(0, Printer, Me, , , , , lngOffsetHeight)
                gtypSpecialPrintPage.Index = gtypSpecialPrintPage.Index + 1
            Else
makPage:
                ...
                For i = 0 To lngEndPage
                    ...
                    Else
                        lngOffsetHeight = (gtypSpecialPrintPage.Index Mod 2) * objFmt.H
                    End If
                    If Not PrintPage(i, Printer, Me, , , , , lngOffsetHeight) Then
                        Exit For
                    End If
                    gtypSpecialPrintPage.Index = gtypSpecialPrintPage.Index + 1

                    If i <> lngEndPage Then
                        If gtypSpecialPrintPage.Mode > enuPrintMode.常规打印 Then
                            If (gtypSpecialPrintPage.Index - 1) Mod 2 = 1 Then
                                Printer.NewPage
                            End If
                        Else
                            Printer.NewPage
            ...
            If k > 0 Then
                ...
                End If
                blnPrint = True
            ElseIf gtypSpecialPrintPage.Mode = enuPrintMode.特殊打印中 Or gtypSpecialPrintPage.Mode = 特殊打印开始 Then
                '特殊打印是否换页
                If (gtypSpecialPrintPage.Index - 1) Mod 2 = 1 Then
                    Printer.NewPage
                End If
...
NextFormat:
        '检查是否继续打印下一格式
        blnGoOn = False
        If InStr(";0;2;4;", ";" & mbytStyle & ";") > 0 And mblnAllFormat And cboFormat.ComboItems.count > 1 _
            And cboFormat.SelectedItem.Index < cboFormat.ComboItems.count Then
...
ExitHandle:
    ...
    If blnPrint Then
        If gtypSpecialPrintPage.Mode > enuPrintMode.常规打印 Then
            If gtypSpecialPrintPage.Mode = enuPrintMode.特殊打印结束 Then
                Printer.EndDoc
                gtypSpecialPrintPage.Index = 0
            End If
        ElseIf gblnSingleTask Then
        ...
        '输出到打印结束，激活打印事件
        If Not mobjCurDLL Is Nothing _
            And (gtypSpecialPrintPage.Mode = enuPrintMode.特殊打印结束 _
                Or gtypSpecialPrintPage.Mode = enuPrintMode.常规打印) Then
...
errH:
    ...
    gtypSpecialPrintPage.Index = 0
    SetRedraw True
    MsgBoxEx Err.Number & ":" & Err.Description & vbCrLf & "打印过程被强行中断！", vbExclamation, App.Title
    Err.Clear
    gblnError = True
End Sub
...
Private Sub mnuPop_Cond_Click(Index As Integer)
    ...
    If Not mobjPars Is Nothing Then
        marrCurMenuIdx(lvw.SelectedItem.Index - 1) = Index
        marrCurCondID(lvw.SelectedItem.Index - 1) = Val(mnuPop_Cond(Index).Tag)
        Call InitReportPars
...
Private Sub mnuPop_Default_Click()
    ...
    If Not mobjPars Is Nothing Then
        marrCurMenuIdx(lvw.SelectedItem.Index - 1) = 0
        marrCurCondID(lvw.SelectedItem.Index - 1) = 0
        Call InitReportPars
        If cmdLoad.Enabled And cmdLoad.Visible Then cmdLoad.SetFocus
    End If
End Sub
...
Private Sub mnuPop_Del_Click()
    If mdlPublic.RPTParsCondDel(mlngRPTID, marrCurCondID(lvw.SelectedItem.Index - 1)) Then
...
Private Sub mnuPop_Save_Click()
    ...
    '保存条件
    If mdlPublic.RPTParsCondSave(mlngRPTID, marrCurCondID(lvw.SelectedItem.Index - 1), mobjPars, mobjDefPars, Me) Then
        '更新参数控件
        If marrCurCondID(lvw.SelectedItem.Index - 1) = 0 Then
            '从缺省状态下保存，更新为新增的条件
            Call mnuPop_Cond_Click(mnuPop_Cond.count - 1)
        Else
            '从条件状态下保存
            intIdx = marrCurCondID(lvw.SelectedItem.Index - 1)
            Call mnuPop_Cond_Click(intIdx)
        End If
    End If
End Sub
...
Private Sub mnuPop_SaveAs_Click()
    ...
    intIdx = marrCurCondID(lvw.SelectedItem.Index - 1)
    If mdlPublic.RPTParsCondSave(mlngRPTID, marrCurCondID(lvw.SelectedItem.Index - 1), mobjPars, mobjDefPars, Me, True) Then
        '更新参数控件
        If intIdx = 0 Then
            '从缺省状态下保存，更新为新增的条件
            Call mnuPop_Cond_Click(mnuPop_Cond.count - 1)
...
'                MsgBox mdlPublic.FormatString("“[1]”报表未发布，请联系管理员！", strName), vbInformation, App.Title
    ...
    If lvw.ListItems.count > 0 Then
        '报表组的子报表需要记录“最后执行时间、执行人”、“历史执行记录”
        Call RecordsExecute(Val(Mid(lvw.SelectedItem.Key, 2)), strStartTime, 3)
    Else
        Call RecordsExecute(mlngReportID, strStartTime, 2)
    End If
End Sub
...
Private Sub msh_Click(Index As Integer)
    ...
    Else
        lngMouseRow = msh(Index).MouseRow
        lngMouseCol = msh(Index).MouseCol
    End If
...
Private Sub picPaper_GotFocus(Index As Integer)
    Call SetPubHook(GWL_WNDPROC, picPaper(Index).hwnd, AddressOf FlexScroll)
End Sub
...
Private Sub picPaper_LostFocus(Index As Integer)
    Call SetPubUnHook(GWL_WNDPROC, picPaper(Index).hwnd)
End Sub
...
'功能：重置报表条件
    ...
    If Not mobjCurDLL Is Nothing Then
        ...
        Else
            ...
            Call ShowParInputForm(frmNewParInput, lngReport, mlngInParamsCondID, mintInParamsMenuIdx, objPars _
                , mobjDefPars, mobjReport.Datas, True)
            If frmNewParInput.mblnOK Then
                '激活条件提交事件
        ...
        End If
    Else
        Call ShowParInputForm(frmNewParInput, lngReport, mlngInParamsCondID, mintInParamsMenuIdx _
            , MakeNamePars(mobjReport), mobjDefPars, mobjReport.Datas, True)
        If frmNewParInput.mblnOK Then
            mlngInParamsCondID = frmNewParInput.mlngCondID
            mintInParamsMenuIdx = frmNewParInput.mintMenuIdx
            ReplaceInputPars frmNewParInput.mobjPars
...
Private Sub Form_Resize()
    ...
    If Me.Visible Then
        If Width < Me.ScaleX(640, vbPixels, vbTwips) Then Width = Me.ScaleX(640, vbPixels, vbTwips)
        If Height < Me.ScaleY(480, vbPixels, vbTwips) Then Height = Me.ScaleY(480, vbPixels, vbTwips)
    Else
        '缺省工具栏高度
        cbr.Height = 780
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    '卸载消息钩子
    For i = picPaper.LBound To picPaper.UBound
        If Not picPaper(i) Is Nothing Then
            If GetProcAddr(picPaper(i).hwnd) <> 0 Then
                Call SetPubUnHook(GWL_WNDPROC, picPaper(i).hwnd)
            End If
        End If
    Next
    ...
    Erase arrReport, arrLibDatas, arrReportPars, arrReportDefPars, marrCurMenuIdx, marrCurCondID
...
Private Sub mnuFile_Setup_Click()
    ...
    sta.Panels(2) = "打印机: " & strTmp & _
        ...
        IIF(objFmt.纸张 = 256, format(objFmt.W / Twip_mm, "#0.0") & "mm × " & format(objFmt.H / Twip_mm, "#0.0") & "mm", "") & _
...
'说明：不依赖于打印设备
    ...
    With mobjReport.Fmts("_" & mobjReport.bytFormat)
        sta.Panels(2).Text = "打印机: " & strPrinter & _
            "   纸张: " & GetPaperName(.纸张, .W, .H) & " " & _
            IIF(.纸张 = 256, format(.W / Twip_mm, "#0.0") & "mm × " & format(.H / Twip_mm, "#0.0") & "mm", "") & _
            IIF(.纸向 = 1, "   纵向", "   横向")
    End With
    On Error GoTo errH
...
'3.允许特殊写法变量绑定。如：select '[0]' 名称 from ...
    ...
    Dim strTmp As String, strSQLtmp1 As String, strErr As String, strSub As String
        ...
        Select Case objData.Pars("_" & CInt(bytPar)).类型
            ...
            Case 1 '数字
                ...
                If objData.数据连接编号 = 0 Then
                    cmdData.Parameters.Append _
                        cmdData.CreateParameter("PAR" & cmdData.Parameters.count + 1, adVarNumeric, adParamInput, 30, Val(StrPar))
                Else
                    cmdData.Parameters.Append _
                        cmdData.CreateParameter("PAR" & cmdData.Parameters.count + 1, adBigInt, adParamInput, , Val(StrPar))
                End If
                strSQL = strLeft & "?" & strRight
            Case 2 '日期
                If left(objData.Pars("_" & CInt(bytPar)).缺省值, 1) = "&" Then
...
'                      只识别常用的加减算法（1：+1-1/24/60/60 ； 2：-1/24/60/60+1 ； 3：-1/24/60/60 ； 4：其他算术表示式直接使用（书写方式太灵活）
                            ...
                            For j = i + 1 To Len(strRight)
                                If Mid(strRight, j, 1) <> " " Then
                                    ...
                                    For k = j + 1 To Len(strRight)
                                        strSub = Mid(strRight, j, k - j + 1)
                                        If Mid(strRight, k, 1) = " " Or IsNumeric(Mid(strRight, j, 1)) And Not IsNumeric(strSub) Then
...
'                                            strTmp = Mid(strRight, k)
                                            ...
                                            '其他条件不调整绑定变量值，直接用表达式。比如：[0] + 1 - 7、 [0] + 1 + 字段
                                            strTmp = strRight
                                        End If
                                        If strTmp <> "" And InStr("+-*/", Mid(Replace(strTmp, " ", ""), 1, 1)) > 0 And Replace(strRight, " ", "") <> "" Then
...
LineBand:
    Call ExportLog(False, "frmReport", "OpenReportSQL", "", strLog)
    Call SQLTest(App.ProductName, "OpenReportSQL", strLog)

    If UCase(cmdData.ActiveConnection.ConnectionString) Like "*ORACLE*" Then
    ...
    '1.本代码段为了避免自定义函数返回异常（Err=-2147467259），以及错误执行LineBlob代码段
    '2.OLEDB Oracle驱动在上一段就会报错“Set rsTmp = cmdData.Execute”就不会错误执行LineBold代码段；但MS-ODBC驱动不报错，并且rsTmp.State=1
    l = 0
    On Error Resume Next
    l = rsTmp.RecordCount
    On Error GoTo errHandle
    Call ExportLog(False, "frmReport", "OpenReportSQL", "RecordCount=" & l, "")
...
LineBlob:
    ...
    If objData.数据连接编号 <= 0 Then
        ...
        If gcnOLEDB Is Nothing Then
            '创建新连接对象
            Call ExportLog(False, "frmReport", "OpenReportSQL", "gcnOLEDB.String=" & gcnOracle.ConnectionString, "")
            Set gcnOLEDB = gobjRegister.ReGetConnection(Val("1-OracleOLEDB"), strErr, gcnOracle)
            Call ExportLog(False, "frmReport", "OpenReportSQL", "strErr=" & strErr, "")
            If gcnOLEDB Is Nothing Then
                Call ExportLog(False, "frmReport", "OpenReportSQL", "gcnOLEDB=nothing", "")
            Else
                Call ExportLog(False, "frmReport", "OpenReportSQL", "gcnOLEDB.NewString" & gcnOLEDB.ConnectionString, "")
            End If
            If gcnOLEDB.State = adStateClosed Then
                strTmp = "创建数据连接失败，请检查：" & Chr(10) & _
                         "1.业务程序使用zlRegister部件方法的参数是否正确；" & Chr(10) & _
                         "2.非导航台程序调用报表部件传入的连接对象（微软驱" & Chr(10) & _
                         "  动），必须通过zlRegister部件创建连接对象。"
                If gblnSilentMode Then
                    'glngErrorNumber = 3709
                    gstrErrorContent = strTmp
                End If
                MsgBoxEx strTmp, vbInformation, App.Title
                Exit Function
            End If
        ...
        Else
            Call ExportLog(False, "frmReport", "OpenReportSQL", "gcnOLEDB.State=" & IIF(gcnOLEDB.State = adStateOpen, "Open", "Close"), "")
        End If
    ...
    If Not cmdData.ActiveConnection Is Nothing Then
        ...
        rsTmp.Open cmdData, , adOpenStatic, adLockOptimistic
        Set rsTmp.ActiveConnection = Nothing        '与zl9ComLib的Recordset对象处理一致
        Call ExportLog(False, "frmReport", "OpenReportSQL", "RecordCount=" & rsTmp.RecordCount, "")
        Set OpenReportSQL = rsTmp
    End If
    
    Call ExportLog(True, "frmReport", "OpenReportSQL", "完成", "")
    Exit Function
...
errHandle:
    ...
    If Err.Description Like "*ORA-00920*" Then
        MsgBoxEx "参数输入错误，导致不能正确读取数据""" & objData.名称 & """！", vbExclamation, App.Title
    ElseIf ErrCenter() = 1 Then
        If mbytStyle = 0 Or mbytStyle = 1 Then ShowFlash "正在读取数据""" & objData.名称 & """，请稍候．．．", , Me
        Resume
    End If
    Call ExportLog(True, "frmReport", "OpenReportSQL", "Err=" & Err.Description, "")
    Call SaveErrLog
End Function
...
Private Sub mnuFile_Excel_Click()
    ...
    Dim blnStatisticsGrid As Boolean
    ...
    '带|表示汇总表格，反之表示自由表格
    blnStatisticsGrid = InStr(gobjBody.Tag, "|") > 0
    For i = 0 To gobjBody.Rows - 1
        For j = 0 To gobjBody.Cols - 1
            ...
            If blnStatisticsGrid Then
                '处理空格（程序避免单元格合并，特别用添加空格避免）
                gobjBody.TextMatrix(i, j) = ReplaceHeadTailBlack(gobjBody.TextMatrix(i, j), GSTR_SPACE_EX)  '只处理头尾的空格
            End If
        Next
    Next
    ...
    For i = 0 To gobjBody.Rows - 1
        For j = 0 To gobjBody.Cols - 1
            ...
            gobjBody.TextMatrix(i, j) = Replace(gobjBody.TextMatrix(i, j), GSTR_SPACE_EX, Space$(1))
        Next
    Next
...
'功能：在查询界面上组织显示一个任意表格
    ...
    Dim strAutoColsIndex As String, strAutoFill As String
    ...
    With objItem
        ...
        mshBody.RowHeightMin = .行高                            '限制最小行高，避免自适应行高小于用户设置行高
        ...
        '--------------------------------------------------------------------------------------
        '1.搜索该表格用到的数据源
        strSource = GetGridSource(mobjReport, objItem) '"病人信息,药品信息,..."
        ...
        For Each tmpID In .SubIDs
            ...
            If Not tmpItem Is Nothing Then
                If tmpItem.自适应行高 Then
                    ...
                    strAutoColsIndex = strAutoColsIndex & tmpItem.序号 & ";"
                End If
                If tmpItem.自动填充 Then
                    strAutoFill = strAutoFill & tmpItem.序号 & ";"
                End If
            End If
        Next
        strAutoColsIndex = ";" & strAutoColsIndex
        strAutoFill = ";" & strAutoFill
        ...
        Do While blnDo
            ...
            For i = 0 To UBound(arrFormula)
                ...
                Else
                    ...
                    mshBody.Cell(flexcpFont, j, lngCol) = mshBody.Font          '强制更新字体对象，VSF的单元格自适应行高才能生效
                    
                    '【自适应行高】才自动拆分单元格
                    If strAutoColsIndex Like "*;" & lngCol & ";*" Then
                        '列
                        For Each tmpID In .SubIDs
                            Set tmpItem = mobjReport.Items("_" & tmpID.id)
                            If Not tmpItem Is Nothing Then
                                If tmpItem.序号 = lngCol And tmpItem.拆分单元格 Then
                                    '拆分
                                    Call SplitCellToMultiRows(j, lngCol, mshBody, strValue, intCount)
                                    Exit For
                                End If
                            End If
                        Next
...
'                        mshBody.Cell(flexcpData, j, lngCol) = colRelation
            ...
            '遍历处理“自动填充内容”的列
            For i = 0 To UBound(arrFormula)
                lngCol = Val(Split(arrFormula(i), "|")(2))
                If strAutoFill Like "*;" & lngCol & ";*" Then
                    '填充内容
                    For k = 1 To intCount
                        mshBody.TextMatrix(j + k, i) = mshBody.TextMatrix(j, i)
                    Next
                End If
            Next
                    ...
                    Case 2, 3 '标签,标签绑定图片
                        ...
                        If gobjFile.FileExists(strValue) Then
                            ...
                            If Err.Number = Val("481-图片无效") Then
                                Set .图片 = PngToBitmap(strValue)   'Png转Bitmap
                            ElseIf .图片 Is Nothing Then
                                Set .图片 = New StdPicture          '以此区分是图片还是文字
                            End If
                            Kill strValue
                            Err.Clear
                            On Error GoTo errH
            ...
            For i = 2 To tmpItem.分栏
                With msh(tmpItem.id)
                    DrawCell picPaper(intReport), "数据分栏位置", tmpItem.X + ((i - 1) * .Width), tmpItem.Y, .Width _
                        , msh(CInt(.Tag)).Height - 15, Nothing, , , .GridColor, .ForeColor, .BackColor _
                        , .Font, , 1, 1
                End With
            Next
        End If
...
'说明：如果运行该函数之后isArray(marrPage)=False,则表明没有表格输出
    ...
    Dim objItem As RPTItem, objItemTmp As RPTItem
    Dim blnHaveApp As Boolean
    ...
    Dim lngL As Long, lngW As Long, lngC As Long
    ...
    For Each objBody In msh
        If objBody.Index <> 0 And (objBody.Container Is picPaper(intReport) Or LCase(objBody.Container.name) = "pic") _
            And left(objBody.Tag, 2) <> "H_" Then
            '卡片的PictureBox
            If LCase(objBody.Container.name) = "pic" Then
                    ...
                    If i >= 0 And i <= .Fields.count - 1 Then
                        ...
                        For Each vsfTmp In msh
                            If vsfTmp.Index > 0 And Not vsfTmp.Container Is Nothing Then
                                Set objItemTmp = mobjReport.Items("_" & vsfTmp.Index)
                                If objBody.Index = vsfTmp.Container.Index And Not objItemTmp Is Nothing Then
                                    '排除空表格（无数据源的表格）
                                    If objItemTmp.类型 = Val("4-自由表格") Then
                                        If GetGridSource(mobjReport, objItemTmp) <> "" Then
                        ...
                        If intGridID >= 0 Then
                            ...
                            Do While .EOF = False
                                ...
                                If lngRow > msh(intGridID).Rows - 1 Then lngRow = msh(intGridID).Rows - 1
            ...
            For Each objPageCell In arrPageTmp(0)
                If IsArray(marrPageCard) Then
                    For i = 0 To UBound(marrPageCard)
                        On Error Resume Next
                        Set objItemTmp = mobjReport.Items("_" & objPageCell.id)
                        j = marrPageCard(i).Item("_" & objItemTmp.父ID).id
                        If Err.Number = 0 Then
                            On Error GoTo 0
                            With marrPageCard(i).Item("_" & objItemTmp.父ID)
                                    For j = 1 To .Item.count
                                        ...
                                        End If
                                        If mobjReport.票据 Then
                                            lngVRowE = (objItemTmp.H - objPageCell.FixH) \ objItemTmp.行高 _
                                        ...
                                        arrPage(i).Add objPageCell.id _
                                            , objPageCell.X _
                                                + ((j - 1) Mod .Col) * (mobjReport.Items("_" & objItemTmp.父ID).W _
                                                + mobjReport.Items("_" & objItemTmp.父ID).左右间距) _
                                            , objPageCell.Y _
                                                + ((j - 1) \ .Col) * (mobjReport.Items("_" & objItemTmp.父ID).H _
                                                + mobjReport.Items("_" & objItemTmp.父ID).上下间距) _
                                            , objPageCell.W, objPageCell.FixH _
                                                + (Val(Mid(.Item(j), InStr(.Item(j), "-") + 1, Len(.Item(j)))) _
                                                - Val(Mid(.Item(j), 1, InStr(.Item(j), "-") - 1)) + 1) * objItemTmp.行高 _
                                            , objPageCell.MaxH, Val(Mid(.Item(j), 1, InStr(.Item(j), "-") - 1)) - 1 _
                                            , Val(Mid(.Item(j), InStr(.Item(j), "-") + 1, Len(.Item(j)))) - 1 _
    ...
    For Each tmpItem In objReport.Items
        With tmpItem
            ...
            objItems.Add .id & intIndex, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, _
                ...
                .ColProtertys, .水平反转, .拆分单元格, .自动填充, .关联表格
        End With
    Next
...
'功能：根据报表组ID,获取其子报表的信息
    Dim rsTmp As New ADODB.Recordset
    Dim strSQL As String, strSub As String
    
    On Error GoTo errH
    
    strSub = "报表id = a.报表id and Nvl(是否停用, 0) = 0"
    If gobjCompatibility.BuildSelectString("zlRPTFMTs", "1", strSub, strSub, False) = False Then
        Err.Raise 60001, , "GetSubReport发生异常！"
    End If

    strSQL = _
        "Select 组ID, 报表ID, 序号, 功能 " & vbNewLine & _
        "From zlRPTSubs A " & vbNewLine & _
        "Where Exists(" & strSub & ") And 组ID=[1] " & vbNewLine & _
        "Order by 序号"
...
Private Sub cmd_Click(Index As Integer)
    ...
    For Each tmpPar In mobjPars
        If tmpPar.名称 = lblName(Index).ToolTipText Then
            ...
            Set colValue = mdlPublic.GetValues(Me, mobjPars)
            ...
            If mLibDatas("_" & .内容).DataSet.RecordCount > 0 Then
                    ...
                    For i = 0 To UBound(Split(strVscOrder, "|"))
                        If left(Split(strVscOrder, "|")(i), 1) = "," Then
                            With rsGroup.Fields(Mid(Split(strVscOrder, "|")(i), 2))
                                ...
                                rsVsc.Fields.Append .name _
                                    , IIF(IsType(.Type, adNumeric) _
                                    , IIF(.NumericScale = 0, adBigInt, adDouble) _
                                    , IIF(.Type = adWChar, adVarWChar, .Type)) _
                                    , .DefinedSize
                            End With
                        Else
                ...
                For i = 1 To rsGroup.RecordCount
                    ...
                    If Not rsVsc Is Nothing And lngGrid = 0 Then
                            ...
                            Select Case rsGroup.Fields(Split(strVsc, "|")(j)).Type
                                Case adChar, adVarChar, adLongVarChar, adWChar, adVarWChar, adLongVarWChar, adBSTR
                                    ...
                                    Else
                                        strFilter = strFilter & "'#'"
                                    End If
                                Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt, adInteger, adSmallInt, adTinyInt _
                        ...
                        'Filter对字段值后面的空格用忽略。比如：a_ 与 a__ 与 a 等值，_表示空格符
                        'rsVsc.Filter = Replace(Mid(strFilter, 6), "♂♂", " ")
                        'If rsVsc.EOF Then
                        If RecordsetFind(rsVsc, Replace(Mid(strFilter, 6), "♂♂", " ")) = False Then
                            rsVsc.AddNew
                                ...
                                Else
                                    Select Case rsGroup.Fields(rsVsc.Fields(j).name).Type
                                        ...
                                        Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt _
                                            , adInteger, adSmallInt, adTinyInt, adUnsignedBigInt, adUnsignedInt, adUnsignedSmallInt, adUnsignedTinyInt
                    ...
                    If Not rsHsc Is Nothing Then
                        ...
                        If RecordsetFind(rsHsc, Mid(strFilter, 6)) = False Then
                            rsHsc.AddNew
                ...
                If Not rsVsc Is Nothing And lngGrid = 0 Then
                    ...
                    ReDim arrMerge(X - 1) '用于处理同级汇总不同上级，防止合并
                    For i = 1 To X - 1
                    ...
                    For i = 1 To rsVsc.RecordCount
                        ...
                        '转换特殊符号
                        Call SpecialSymbolTrans(strKey)
                ...
                If Y > 0 And Not rsHsc Is Nothing Then
                    ...
                    ReDim arrMerge(Y - 1) '用于处理同级汇总不同上级，防止合并
                    For i = 1 To Y - 1
                        arrMerge(i) = Space(i Mod 2)
                    Next
                    ...
                    Select Case tmpItem.类型
                        ...
                        Case 9 '统计项
                            ...
                            For i = lngCurCols + IIF(lngGrid = 0, X, 0) To mshBody.Cols - 1 Step Z
                                ...
                                If mshBody.FixedRows - 1 >= 0 And mshBody.Rows - 1 >= 0 Then
                                    mshBody.Cell(flexcpAlignment, mshBody.FixedRows - 1, i + tmpItem.序号, mshBody.Rows - 1, i + tmpItem.序号) _
                                        = mshBody.ColAlignment(i + tmpItem.序号)
                                End If
                ...
                For i = 1 To rsGroup.RecordCount
                    ...
                    For j = 0 To UBound(Split(strVsc, "|"))
                        strKey = strKey & "^" & IIF(IsNull(rsGroup.Fields(CStr(Split(strVsc, "|")(j))).value) _
                                                  , "" _
                                                  , Replace(Nvl(rsGroup.Fields(CStr(Split(strVsc, "|")(j))).value, ""), " ", "♂"))
                    Next
                    ...
                    If lngRow > 0 Then
                        ...
                        If strHsc <> "" Then
                            ...
                            For j = 0 To UBound(Split(strHsc, "|"))
                                strKey = strKey & "^" & IIF(IsNull(rsGroup.Fields(CStr(Split(strHsc, "|")(j))).value) _
                                                          , "" _
                                                          , Replace(rsGroup.Fields(CStr(Split(strHsc, "|")(j))).value & "", " ", "♂"))
                            Next
                        ...
                        For j = 0 To Z - 1
                            ...
                            If Not IsNull(rsGroup.Fields(strTmp).value) Then
                                ...
                                If StrFmt <> "" Then
                                    ...
                                    Select Case rsGroup.Fields(strTmp).Type
                                        Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt, adInteger _
                                            ...
                                            mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                format(Val(Replace(mshBody.TextMatrix(lngRow, lngCol + j), ",", "")) _
                                                     + rsGroup.Fields(strTmp).value, StrFmt)
                                        Case adChar, adVarChar, adLongVarChar, adWChar, adVarWChar, adLongVarWChar, adBSTR
                                            mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                format(Val(mshBody.TextMatrix(lngRow, lngCol + j)) _
                                                     + Val(rsGroup.Fields(strTmp).value), StrFmt)
                                        Case adDBTimeStamp, adDBTime, adDBDate, adDate
                                            If mshBody.TextMatrix(lngRow, lngCol + j) = "" Then
                                                mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                    format(CDate(rsGroup.Fields(strTmp).value), StrFmt)
                                            Else
                                                mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                    format(CDate(mshBody.TextMatrix(lngRow, lngCol + j)) _
                                                         + rsGroup.Fields(strTmp).value, StrFmt)
                                            End If
                                    End Select
                                ...
                                Else
                                    Select Case rsGroup.Fields(strTmp).Type
                                        Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt _
                                            ...
                                            , adUnsignedTinyInt
                                            mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                Val(Replace(mshBody.TextMatrix(lngRow, lngCol + j), ",", "")) _
                                              + rsGroup.Fields(strTmp).value
                                        Case adChar, adVarChar, adLongVarChar, adWChar, adVarWChar, adLongVarWChar, adBSTR
                                            mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                Val(mshBody.TextMatrix(lngRow, lngCol + j)) _
                                              + Val(rsGroup.Fields(strTmp).value)
                                        Case adDBTimeStamp, adDBTime, adDBDate, adDate
                                            If mshBody.TextMatrix(lngRow, lngCol + j) = "" Then
                                                mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                    CDate(rsGroup.Fields(strTmp).value)
                                            Else
                                                mshBody.TextMatrix(lngRow, lngCol + j) = _
                                                    CDate(mshBody.TextMatrix(lngRow, lngCol + j)) _
                                                  + rsGroup.Fields(strTmp).value
                                            End If
                                    End Select
                                End If
                                ...
                                Select Case CByte(Split(strAlign, ",")(j))
                                    Case 0 '左对齐
                                        mshBody.TextMatrix(lngRow, lngCol + j) = _
                                            mshBody.TextMatrix(lngRow, lngCol + j) & Space((lngRow + lngCol + j) Mod 2)
                                    Case 1 '中对齐
                                        mshBody.TextMatrix(lngRow, lngCol + j) = _
                                            Space((lngRow + lngCol + j) Mod 2) & mshBody.TextMatrix(lngRow, lngCol + j) _
                                                & Space((lngRow + lngCol + j) Mod 2)
                                    Case 2 '右对齐
                                        mshBody.TextMatrix(lngRow, lngCol + j) = Space((lngRow + lngCol + j) Mod 2) _
                                            & mshBody.TextMatrix(lngRow, lngCol + j)
                                End Select
                                        ...
                                        For k = 0 To Z - 1
                                            If strStat = "AVG" Then
                                                ...
                                                Select Case rsGroup.Fields(strTmp).Type
                                                    Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal _
                                                        , adBigInt, adInteger, adSmallInt, adTinyInt, adUnsignedBigInt, adUnsignedInt _
                                            ...
                                            If StrFmt <> "" Then
                                                ...
                                                Select Case Split(strAlign, ",")(k)
                                                    ...
                                                    Case 1 '中
                                                        mshBody.TextMatrix(i, j + k) = Space((i + j + k) Mod 2) & format(arrStat(i, k), StrFmt) _
                                                            & Space((i + j + k) Mod 2)
                                                    Case 2 '右
...
'    End If
    ...
    If InStr(mshBody.Tag, "|") <= 0 Then
        mshBody.Tag = mshBody.Tag & "|" & Val("1-汇总表格")
    End If
...
'说明：只处理附加表体,且按设计尺寸处理,只在报表显示前调用一次
    ...
    For Each tmpItem In mobjReport.Items
        If tmpItem.格式号 = bytFormat _
            ...
            And tmpItem.参照 = "" And tmpItem.性质 = Val("0-无参照表格") Then
...
Private Sub txt_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        Call SetPubHook(GWL_WNDPROC, txt(Index).hwnd, AddressOf WndMessage)
    End If
End Sub

Private Sub txt_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        Call SetPubUnHook(GWL_WNDPROC, txt(Index).hwnd)
    End If
End Sub
...
'                            Debug.Print objControl.TextMatrix(j, k)
...
'                End If
                ...
                If lngIndex <> i Then lngMshRow = objControl.FixedRows: lngMshcol = objControl.FixedCols
                lngIndex = i
                
                For j = lngMshRow To objControl.Rows - 1
                    For k = lngMshcol To objControl.Cols - 1
                        If objControl.TextMatrix(j, k) Like "*" & strFind & "*" Then
                            objControl.Row = j: objControl.Col = k
                            objControl.ShowCell j, k
                            objControl.SetFocus
                            '确定下次查找的开始位置
                            lngMshcol = k + 1: lngMshRow = j
                            If lngMshcol >= objControl.Cols - 1 Then
                                lngMshcol = objControl.FixedCols
                                lngMshRow = lngMshRow + 1
                            End If
                            Exit Sub
                        End If
                    Next
...
Private Sub LoadCondsMenu()
    ...
    With rsPara
        ...
        Else
            ...
            mnuPop_Del.Enabled = marrCurCondID(lvw.SelectedItem.Index - 1) > 0
            Do While .EOF = False
                i = .AbsolutePosition
                Load mnuPop_Cond(i)
                ...
                If marrCurCondID(lvw.SelectedItem.Index - 1) = Nvl(!条件号, 0) Then
    ...
    mnuPop_Default.Checked = marrCurCondID(lvw.SelectedItem.Index - 1) = 0
...
Private Function RecordsetFindChild(ByVal rsSource As ADODB.Recordset, ByVal strFieldInfo As String) As Boolean
'功能：检查当前行的字段与字段值是否匹配
    
    Dim arrExpression() As String
    Dim strTemp As String, strName As String, strVal As String
    ...
    '拆分表达式
    strTemp = Replace(strFieldInfo, " And ", " and ")
    strTemp = Replace(strTemp, " AND ", " and ")
    arrExpression = Split(strTemp, " and ")
    For i = LBound(arrExpression) To UBound(arrExpression)
        strTemp = arrExpression(i)
        If strTemp Like "*=*" Then
            intPos = InStr(strTemp, "=")
            strName = left$(strTemp, intPos - 1)
            strVal = Trim(Mid$(strTemp, intPos + 1))
            
            With rsSource
                '不区分大小写，但区分空格
                Select Case .Fields(strName).Type
                Case adDBTimeStamp, adDBTime, adDBDate, adDate
                    '将日期类型转成双精度类型来判断相同
                    If CDbl(.Fields(strName).value) = CDbl(CDate(Replace(strVal, "#", ""))) Then
                        '略...
                    Else
                        blnMatch = False
                        Exit For
                    End If
                Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt _
                    , adInteger, adSmallInt, adTinyInt, adUnsignedBigInt, adUnsignedInt _
                    , adUnsignedSmallInt, adUnsignedTinyInt
                    If .Fields(strName).value = strVal Then
                        '略...
                    Else
                        blnMatch = False
                        Exit For
                    End If
                Case Else
                    If "'" & UCase(.Fields(strName).value) & "'" = UCase(strVal) Then
                        '略...
                    Else
                        blnMatch = False
                        Exit For
    ...
    RecordsetFindChild = blnMatch
End Function

Private Function RecordsetFind(ByVal rsSource As ADODB.Recordset, ByVal strFieldInfo As String) As Boolean
'功能：替换Filter方法，判断字段和字段值是否已存在匹配的记录

    RecordsetFind = True
    If rsSource Is Nothing Then Exit Function
    If Not strFieldInfo Like "*=*" Then Exit Function
    
    rsSource.Filter = strFieldInfo
    If rsSource.RecordCount > 0 Then
        With rsSource
            Do While .EOF = False
                '查找匹配的字段值和字段
                If RecordsetFindChild(rsSource, strFieldInfo) Then
                    Exit Function
                End If
                .MoveNext
            Loop
...
Private Function SymbolEqual(ByVal strChar As String, ByVal arrReference As Variant) As Integer
    Dim i As Integer
    
    SymbolEqual = -1
    For i = LBound(arrReference) To UBound(arrReference)
        If strChar = arrReference(i) Then
            SymbolEqual = i
            Exit For
        End If
    Next
...
Private Sub SpecialSymbolTrans(ByRef strSource As String)
'功能：将特殊全角符号转换成特殊字符串
'备注：由于集合对象的Key值对部分全角符号统一为半角符号，可能引起Key重复的错误

    Const STR_SYMBOL As String = "（|）|！|｛|｝|：|；|，|？"
    Const STR_SYMBOL_TRANS As String = "<kh1>|<kh2>|<gth>|<dkh1>|<dkh2>|<mh>|<fh>|<dh>|<wh>"
    
    Dim arrSymbol() As String, arrTrans() As String
    Dim i As Long
    Dim intIdx As Integer
    
    arrSymbol = Split(STR_SYMBOL, "|")
    arrTrans = Split(STR_SYMBOL_TRANS, "|")
    
    i = 1
    Do While i <= Len(strSource)
        '检查是否为特殊符号
        intIdx = SymbolEqual(Mid$(strSource, i, 1), arrSymbol)
        If intIdx >= 0 Then
            '转换符号
            strSource = left(strSource, i - 1) & arrTrans(intIdx) & Mid$(strSource, i + 1)
            i = i + Len(arrTrans(intIdx))
        Else
            i = i + 1
        End If
    Loop
...
Private Sub ShowParInputForm(ByVal objParInputForm As Object, ByVal lngReport As Long, ByVal lngCondID As Long _
    , ByVal intMenu As Integer, ByVal objPars As RPTPars, ByVal objDefPars As RPTPars, ByVal objDatas As RPTDatas _
    , Optional ByVal blnReset As Boolean = False)
    
    With objParInputForm
        Set .mobjPars = objPars
        Set .mobjDefPars = objDefPars
        Set .mobjRPTDatas = objDatas
        .mlngReport = lngReport
        .mlngCondID = lngCondID
        .mintMenuIdx = intMenu
        .mstrTitle = mobjReport.名称
        .mblnReset = blnReset
        .Show vbModal, Me
    End With
...
Private Sub SplitCellToMultiRows(ByVal lngRow As Long, ByVal lngCol As Long, ByRef objVSF As Object _
    , ByVal strCellText As String, ByRef intCount As Integer)
'功能：拆分单元格文本成多行
'参数：
'  lngRow：
'  lngCol：
...
'  strCellText：
    ...
    Dim objFontOld As StdFont
    Dim lngCellWidth As Long
    Dim strText As String
    Dim arrNewCellText() As String
    Dim i As Integer
    Dim blnNewLine As Boolean
    
    Set objFontOld = Me.Font
    Set Me.Font = objVSF.Font
    
    On Error GoTo hErr

    '单元格内宽
    lngCellWidth = objVSF.ColWidth(lngCol) - (1 + 2) * 2 * 15
    
    '计算需要的行数和各行的文本
    strText = ""
    ReDim arrNewCellText(0)
    For i = 1 To Len(strCellText)
        strText = strText & Mid$(strCellText, i, 1)
        blnNewLine = Right(strText, 2) = vbCrLf Or Right(strText, 1) = vbCr Or Right(strText, 1) = vbLf
        If Me.TextWidth(strText) > lngCellWidth Or blnNewLine Then
            '记录拆分的文本
            If arrNewCellText(0) <> "" Then
                ReDim Preserve arrNewCellText(UBound(arrNewCellText) + 1)
            End If
            If blnNewLine Then
                arrNewCellText(UBound(arrNewCellText)) = left$(strText, Len(strText) - 1)
                strText = ""    '屏蔽换行符、回车符
            Else
                arrNewCellText(UBound(arrNewCellText)) = left$(strText, Len(strText) - 1)
    ...
    If strText <> "" Then
        If arrNewCellText(0) <> "" Then
            ReDim Preserve arrNewCellText(UBound(arrNewCellText) + 1)
        End If
        arrNewCellText(UBound(arrNewCellText)) = strText
    End If
    
    If UBound(arrNewCellText) <= 0 Then
        '无需拆分
        objVSF.TextMatrix(lngRow, lngCol) = strCellText
    Else
        '新增行数与更新文本内容(倒加)
        For i = UBound(arrNewCellText) To LBound(arrNewCellText) Step -1
            If i <= LBound(arrNewCellText) Then
                objVSF.TextMatrix(lngRow, lngCol) = arrNewCellText(i)
            Else
                If objVSF.Rows = 1 Then
                    '添加
                    objVSF.Rows = objVSF.Rows + 1
                Else
                    '插入
                    objVSF.AddItem "", lngRow + 1
                End If
                objVSF.TextMatrix(lngRow + 1, lngCol) = arrNewCellText(i)
            End If
        Next
    End If
    
    intCount = UBound(arrNewCellText)
    Set Me.Font = objFontOld
    Exit Sub
