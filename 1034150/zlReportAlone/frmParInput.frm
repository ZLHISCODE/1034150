...
Public mlngCondID As Long           '当前报表的条件项目
Public mintMenuIdx As Integer       '当前选择的条件菜单的索引
...
Private Sub cmd_Click(Index As Integer)
    ...
    For Each tmpPar In mobjPars
        If tmpPar.名称 = lbl(Index).ToolTipText Then
            If tmpPar.类型 = Val("3-无类型") And Not CheckContentPass(txt(Index).Text) Then
                MsgBox "录入的内容不允许，请检查！", vbInformation, App.Title
                Exit For
            End If
...
Private Sub PopMenu_Cond_Click(Index As Integer)
    ...
    If Not objCondPars Is Nothing Then
        ...
        mlngCondID = Val(PopMenu_Cond(i).Tag)
        mintMenuIdx = i
        Call UpdateMenuItemCheck(mintMenuIdx)
    End If
End Sub
...
Private Sub PopMenu_Del_Click()
    If mdlPublic.RPTParsCondDel(mlngReport, mlngCondID) Then
        mlngCondID = 0
        mintMenuIdx = 0
        Call LoadCondsMenu
...
Private Sub PopMenu_Save_Click()
    If mdlPublic.RPTParsCondSave(mlngReport, mlngCondID, mobjPars, mobjDefPars, Me) Then
        If mintMenuIdx = 0 Then
            '从缺省状态下保存，更新为新增的条件
            Call PopMenu_Cond_Click(PopMenu_Cond.count - 1)
        Else
            '从条件状态下保存
            Call PopMenu_Cond_Click(mintMenuIdx)
        End If
    End If
End Sub

Private Sub PopMenu_Saveas_Click()
    If mdlPublic.RPTParsCondSave(mlngReport, mlngCondID, mobjPars, mobjDefPars, Me, True) Then
        If mintMenuIdx = 0 Then
            '从缺省状态下保存，更新为新增的条件
            Call PopMenu_Cond_Click(PopMenu_Cond.count - 1)
        Else
...
'    End If
    Call UpdateMenuItemCheck(mintMenuIdx)
    PopMenu_Del.Enabled = mintMenuIdx > 0
...
'            '可否输入匹配
'            If tmpPar.类型 = Val("3-无类型") And Not CheckContentPass(txt(i).Text) Then
'                '限制文本框手工录入时，人为注入SQL
'                txt(i).Locked = True
'            Else
...
'                End If
            ...
            Else
                ...
                txt(i).Visible = True
'                '限制文本框手工录入时，人为注入SQL
'                txt(i).Locked = tmpPar.类型 = Val("3-无类型") And Not CheckContentPass(txt(i).Text)
            End If
        End If
...
Private Function CheckContentPass(ByVal strContent As String) As Boolean
    Dim strSQL As String
    Dim rsTemp As ADODB.Recordset
    
    strSQL = "Select zlTools.Zl_Rptinparams_Check([1]) Val From Dual"
    
    On Error Resume Next
    Set rsTemp = mdlPublic.OpenSQLRecord(strSQL, "检查无类型参数", strContent)
    If Err.Number = 0 Then
        CheckContentPass = Nvl(rsTemp!Val, 0) = 1
    Else
        CheckContentPass = True                             '函数不存在就不检查
    End If
