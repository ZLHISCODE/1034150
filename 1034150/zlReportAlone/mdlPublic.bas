...
Public gobjLogin As Object                      '登录部件
...
Public glngSessionID As Long                    '当前数据库连接的SessionID
Public glngInstance As Long                     'clsReport类的实例数
Public gobjCompatibility As clsCompatibility    '兼容处理类
Public gblnGeneralLog As Boolean                '通用日志；True通用日志输出；False旧的日志输出
Public gblnHaveQueryCategory As Boolean         'zlLogCategory数据表存在
...
Public Type SpecialPrintPage
    Mode As enuPrintMode
    Index As Long
    Copies As Integer
End Type
...
Public gbytExportMode As Byte           '输出模式（与ReportOpen的最后一个参数一致）
...
Public gcolProcAddr As Collection
Public gtypSpecialPrintPage As SpecialPrintPage
...
Private Const MSTR_COMMA_SPLIT As String = "<comma>"
...
Public gblnAppInfo As Boolean '是否启用dbms_application_info
...
'
Public Const GSTR_SPACE_EX As String = "<原有空格>"
...
Private Declare Function sm4_crypt_ecb Lib "zlSm4" (ByVal Mode As Long, ByVal Length As Long, Key As Byte, in_put As Byte, out_put As Byte) As Long
Private Declare Function sm4_crypt_cbc Lib "zlSm4" (ByVal Mode As Long, ByVal Length As Long, iv As Byte, Key As Byte, in_put As Byte, out_put As Byte) As Long

Private Const SM4_CRYPT_RANDOMIZE_KEY As Long = 999  'sm4加密算法密钥生成器的随机种子
Private Const SM4_CRYPT_RANDOMIZE_IV As Long = 666   'sm4加密算法初始向量生成器的随机种子
...
'支持滚轮的滚动
    Dim lngOldAddr As Long

    On Error GoTo makEnd

    Select Case wMsg
    Case WM_MOUSEWHEEL
        Select Case wParam
        Case -7864320  '向下滚
            If Not InDesign Then SendKeys "{PGDN}"
        Case 7864320   '向上滚
            If Not InDesign Then SendKeys "{PGUP}"
        End Select
    End Select
    
makEnd:
    lngOldAddr = GetProcAddr(hwnd)
    If lngOldAddr <> 0 Then
         FlexScroll = CallWindowProc(lngOldAddr, hwnd, wMsg, wParam, lParam)
    End If
End Function

Public Function Pad(ByVal strSource As String, ByVal intLenByte As Integer, ByVal strPad As String _
    , Optional ByVal bytLeft As Boolean) As String
'--------------------------------------------------------------------------------
'功能：效仿Oracle的LPad、RPad函数
'参数：
'  strSource：原字符串
'  intLenByte：字符串填充后的总长度
'  strPad：填充字符串
'  bytLeft：True左边填充字符串；False右边填充字符串
'--------------------------------------------------------------------------------
    Dim intSource As Integer, intInc As Integer, i As Integer
    Dim strInc As String, strTemp As String
    
    On Error GoTo hErr
    
    intSource = LenB(StrConv(strSource, vbFromUnicode))
    If intLenByte >= intSource Then
        strTemp = strSource
        intInc = intLenByte - intSource
        For i = 1 To intInc
            strInc = strInc & strPad
        Next
    Else
        strTemp = StrConv(LeftB(StrConv(strSource, vbFromUnicode), intLenByte), vbUnicode)
    End If
    
    If bytLeft Then
        If strInc <> "" Then
            strTemp = Left(strInc, intLenByte - intSource) & strTemp
        End If
        Pad = StrConv(RightB(StrConv(strTemp, vbFromUnicode), intLenByte), vbUnicode)
    Else
        strTemp = strTemp & strInc
        Pad = StrConv(LeftB(StrConv(strTemp, vbFromUnicode), intLenByte), vbUnicode)
    End If
...
'       resume      返回 1
'------------------------------------------------
    Dim strNote As String, strTemp As String, strConnError As String
    Dim bytReturnType As Byte
    Dim blnExeSQLTest As Boolean, blnConnError As Boolean
    Dim lngConnError As Long, lngPos As Long
    ...
    If Not gcnOracle Is Nothing Then
        If gcnOracle.Errors.count <> 0 Then
            '忽略：OraOLEDB驱动，0x00040EC9-没有更多的结果
            Set objErr = Nothing
            For Each objErr In gcnOracle.Errors
                If objErr.Number = CLng(&H40EC9) _
                    Or UCase(objErr.Description) Like "*ORAOLEDB*40EC9*" Then
                    Set objErr = Nothing
                Else
                    Exit For
                End If
            ...
            If Not objErr Is Nothing Then
                '取首个真实的Oracle.Error对象
                lngConnError = objErr.NativeError
                strConnError = objErr.Description
                blnConnError = True
            End If
        End If
    End If
    ...
    If blnConnError Then
        'PL/SQL存储过程错误
        If objErr.NativeError >= 20000 And objErr.NativeError <= 20200 Then
            '日志变量
            mbytErrType = 1
            mlngErrNum = objErr.NativeError
            mstrErrInfo = objErr.Description
            strNote = objErr.Description
        ...
        Case Else
            ...
            Else
                strNote = "未知错误，发生在" & objErr.Source
            End If
        End Select
    ...
    Else
        ...
        Select Case Err.Number
            ...
            Case 47, 47 - 2146828288
                strNote = " 太多的动态链接库（DLL）应用客户"
            Case 48, 48 - 2146828288
                strNote = " 调用动态链接库（DLL）错误"
            Case 49, 49 - 2146828288
                strNote = " 动态链接库（DLL）约定错误"
            ...
            Case 442, 442 - 2146828288
                strNote = "到远程类库或对象库的连接丢失，按OK进入对话移去参照"
            ...
            Case 3709
                strNote = "应用程序要求一个关闭的引用对象或无效的连接对象。"
            Case Else
                strNote = "发生在界面未知错误"
        End Select
        bytReturnType = 0
...
Public Function ReadReport(ByVal lngRPTID As Long, _
    Optional ByRef intMaxID As Integer, _
    Optional ByVal blnOnlyData As Boolean, _
    Optional ByVal blnWithoutFormatStop As Boolean) As Report
'功能：从数据库中读取指定报表到报表对象
'参数：lngRPTID=报表ID,intMaxID=设计界面处理的最大控件索引,读取过程中改变
'      blnOnlyData=只读取报表数据源
'      blnWithoutFormatStop=不含停用的报表格式
'返回：intMaxID=当前可用的最大控件索引,ReadReport=报表对象
    ...
    Dim lng父ID As Long
    Dim strSQL As String, strReport As String, strWhere As String
    Dim intCopyID As Integer, i As Integer, j As Integer
    ...
    If gstrLoginUser = "" Then
        '导航台才输出该函数的日志
        Call ExportLog(False, "mdlPublic", "ReadReport", "开始", "")
    End If
    ...
    Else
        strSQL = "Select ID,编号,名称,说明,密码,打印机,进纸,票据,系统,程序ID,功能,修改时间,发布时间 " & vbCr & _
                 "    ,打印方式,禁止开始时间,禁止结束时间 " & vbCr & _
    ...
    If Not rsReport.EOF Then
        ...
        If Not rsData.EOF Then
            '数据源SQL
            strSQL = "Select A.源ID,A.行号,A.内容 " & vbCr & _
                     "From zlRPTSQLs A, zlRPTDatas B " & vbCr & _
                     "Where A.源ID=B.ID And B.报表ID=[1] " & vbCr & _
                     "Order by A.源ID,A.行号"
            Set rsSQL = OpenSQLRecord(strSQL, "ReadReport", lngRPTID)
            
            '数据源参数
            strSQL = "Select A.源ID,A.组名,A.序号,A.名称,A.类型,A.缺省值,A.格式,A.值列表,A.分类SQL,A.明细SQL" & vbCr & _
                     "    ,A.分类字段,A.明细字段,A.对象,A.锁定 " & vbCr & _
                     "From zlRPTPars A,zlRPTDatas B " & vbCr & _
                     "Where A.源ID=B.ID And B.报表ID=[1] Order by A.源ID,A.序号,A.名称,A.类型"
        ...
        If blnOnlyData = False Then
            '报表格式
            strSQL = "报表ID,序号,说明,W,H,纸张,纸向,动态纸张,图样,是否停用,停用原因"
            strWhere = "报表ID = [1] " & IIF(blnWithoutFormatStop, " And Nvl(是否停用, 0) = 0 ", "")
            If gobjCompatibility.BuildSelectString("zlRPTFmts", strSQL, strWhere, strSQL, True) = False Then
                Err.Raise 60001, , "ReadReport异常！"
            End If
            strSQL = strSQL & vbCr & "Order by 序号"
            ...
            For i = 1 To rsFormat.RecordCount
                If IsNull(rsFormat!纸张) And IsNull(rsFormat!W) And IsNull(rsFormat!H) _
                    ...
                    '兼容考虑：统一为报表统一设置
                    tmpReport.Fmts.Add rsFormat!序号, rsFormat!说明, Nvl(rsReport!W, INIT_WIDTH), _
                        Nvl(rsReport!H, INIT_HEIGHT), Nvl(rsReport!纸张, 9), Nvl(rsReport!纸向, 1), _
                        Nvl(rsReport!动态纸张, 0) = 1, Nvl(rsFormat!图样, 0), Nvl(rsReport!是否停用, 0) = 1, _
                        Nvl(rsFormat!停用原因), "_" & rsFormat!序号
                Else
                    '缺省为A4幅面,纵向
                    tmpReport.Fmts.Add rsFormat!序号, rsFormat!说明, Nvl(rsFormat!W, INIT_WIDTH), _
                        Nvl(rsFormat!H, INIT_HEIGHT), Nvl(rsFormat!纸张, 9), Nvl(rsFormat!纸向, 1), _
                        Nvl(rsFormat!动态纸张, 0) = 1, Nvl(rsFormat!图样, 0), Nvl(rsFormat!是否停用, 0) = 1, _
                        Nvl(rsFormat!停用原因), "_" & rsFormat!序号
                End If
                rsFormat.MoveNext
            Next
            ...
            strSQL = _
                "RowNum,系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐" & _
                ",自调,字体,字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,父ID,源行号,源ID,上下间距" & _
                ",左右间距,横向分栏,纵向分栏,表格线加粗,自适应行高,水平反转,拆分单元格,自动填充,关联表格"
            Call gobjCompatibility.BuildSelectString("zlRPTItems" _
                    , strSQL _
                    , "报表ID=[1]" _
                    , strSQL _
                    , True)
            strSQL = strSQL & vbCr & " Order By NVL(父ID,0), 格式号, 上级ID Desc, 类型, 序号, X, Y"
            ...
            For i = 1 To rsItem.RecordCount
                ...
                With tmpItem
                    ...
                    .类型 = Nvl(rsItem!类型, 0)
                    .序号 = Nvl(rsItem!序号, 0)
                    If .类型 = 13 And .序号 = Val("1-Code128遗留") Then
                        .序号 = Val("3-Code128Auto")
                    End If
                    .参照 = Nvl(rsItem!参照)
                    .性质 = Nvl(rsItem!性质, 0)
                    .内容 = Nvl(rsItem!内容)
                    ...
                    .拆分单元格 = Nvl(rsItem!拆分单元格, 0) = 1
                    .自动填充 = Nvl(rsItem!自动填充, 0) = 1
                    ...
                    Set tmpItem = tmpReport.Items.Add(.id, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容 _
                        ...
                        , .CopyIDs, "_" & .id, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏, , , .水平反转 _
                        , .拆分单元格, .自动填充, .关联表格)
    ...
    If gstrLoginUser = "" Then
        Call ExportLog(True, "mdlPublic", "ReadReport", "完成", "")
    End If
    Exit Function
...
errH:
    ...
    End If
    If gstrLoginUser = "" Then
        Call ExportLog(True, "mdlPublic", "ReadReport", "Err=" & Err.Description, "")
    End If
    Call SaveErrLog
...
'功能:保存报表内容(对象objReport)
    ...
    Dim lngSQLID As Long, lngItemID As Long
    ...
    With objReport
        ...
        gcnOracle.Execute _
            "Update zlReports" & _
            "   Set 打印机='" & .打印机 & "',进纸=" & .进纸 & "," & _
            "       票据=" & IIF(.票据, 1, 0) & ",打印方式=" & .打印方式 & ",修改时间=Sysdate" & _
            "      ,禁止开始时间=to_date('" & format(.禁止开始时间, "HH:mm:ss") & "','HH24:MI:SS')" & _
            "      ,禁止结束时间=to_date('" & format(.禁止结束时间, "HH:mm:ss") & "','HH24:MI:SS')" & _
            " Where ID=" & lngRPTID
        ...
        For Each tmpData In .Datas
            ...
            If Len(Trim(tmpData.SQL)) > 0 Then
                ...
                '统一CR\LF\CRLF符号
                strTmp = Replace(tmpData.SQL, vbCrLf, vbLf)
                strTmp = Replace(strTmp, vbCr, vbLf)
                arrSQLLine = Split(strTmp, vbLf)
                
                For j = 0 To UBound(arrSQLLine)
                    rsSQL.AddNew
                    rsSQL!源ID = lngSQLID
                    rsSQL!行号 = j + 1
                    rsSQL!内容 = arrSQLLine(j)
                    rsSQL.Update
                    If Not objPan Is Nothing Then
        ...
        For j = 1 To .Fmts.count
            strInfo = "报表ID=" & lngRPTID
            strInfo = strInfo & ", 序号=" & .Fmts(j).序号
            strInfo = strInfo & ", 说明='" & Replace(.Fmts(j).说明, ",", MSTR_COMMA_SPLIT) & "'"
            strInfo = strInfo & ", W=" & .Fmts(j).W
            strInfo = strInfo & ", H=" & .Fmts(j).H
            strInfo = strInfo & ", 纸张=" & .Fmts(j).纸张
            strInfo = strInfo & ", 纸向=" & .Fmts(j).纸向
            strInfo = strInfo & ", 动态纸张=" & IIF(.Fmts(j).动态纸张, 1, 0)
            strInfo = strInfo & ", 图样=" & .Fmts(j).图样
            strInfo = strInfo & ", 是否停用=" & IIF(.Fmts(j).停用, "1", "Null")
            strInfo = strInfo & ", 停用原因=" & IIF(.Fmts(j).停用原因 <> "", "'" & .Fmts(j).停用原因 & "'", "Null")
            
            If gobjCompatibility.BuildInsertString("zlRPTFmts", strInfo, strSQL) = False Then
                Err.Raise 60001, , "保存报表时上传zlRPTFmts数据发生异常！"
            End If
            gcnOracle.Execute Replace(strSQL, MSTR_COMMA_SPLIT, ",")
        ...
        For Each tmpItem In .Items
            ...
            If tmpItem.类型 = Val("14-卡片元素") Then '子项除外
                ...
                strInfo = "ID=" & lngItemID
                strInfo = strInfo & ", 报表ID=" & lngRPTID
                strInfo = strInfo & ", 格式号=" & tmpItem.格式号
                strInfo = strInfo & ", 名称='" & Replace(tmpItem.名称, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", 上级ID=Null"
                strInfo = strInfo & ", 类型=" & tmpItem.类型
                strInfo = strInfo & ", 序号=" & tmpItem.序号
                strInfo = strInfo & ", 参照='" & Replace(tmpItem.参照, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", 性质=" & tmpItem.性质
                strInfo = strInfo & ", 内容='" & Replace(tmpItem.内容, ",", MSTR_COMMA_SPLIT) & "'"
                
                strInfo = strInfo & ", 表头='" & Replace(tmpItem.表头, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", X=" & tmpItem.X
                strInfo = strInfo & ", Y=" & tmpItem.Y
                strInfo = strInfo & ", W=" & tmpItem.W
                strInfo = strInfo & ", H=" & tmpItem.H
                strInfo = strInfo & ", 行高=" & tmpItem.行高
                strInfo = strInfo & ", 对齐=" & tmpItem.对齐
                strInfo = strInfo & ", 自调=" & Abs(CInt(tmpItem.自调))
                strInfo = strInfo & ", 字体='" & Replace(tmpItem.字体, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", 字号=" & tmpItem.字号
                
                strInfo = strInfo & ", 粗体=" & Abs(CInt(tmpItem.粗体))
                strInfo = strInfo & ", 斜体=" & Abs(CInt(tmpItem.斜体))
                strInfo = strInfo & ", 下线=" & Abs(CInt(tmpItem.下线))
                strInfo = strInfo & ", 边框=" & Abs(CInt(tmpItem.边框))
                strInfo = strInfo & ", 网格=" & tmpItem.网格
                strInfo = strInfo & ", 前景=" & tmpItem.前景
                strInfo = strInfo & ", 背景=" & tmpItem.背景
                strInfo = strInfo & ", 排序='" & tmpItem.排序 & "'"
                strInfo = strInfo & ", 格式='" & tmpItem.格式 & "'"
                strInfo = strInfo & ", 汇总='" & tmpItem.汇总 & "'"
                
                strInfo = strInfo & ", 分栏=" & IIF(tmpItem.分栏 = 0, 1, tmpItem.分栏)
                strInfo = strInfo & ", 系统=" & Abs(CInt(tmpItem.系统))
                strInfo = strInfo & ", 父ID=Null"
                strInfo = strInfo & ", 源ID=" & IIF(lngTmp = 0, "Null", lngTmp)
                strInfo = strInfo & ", 源行号=" & tmpItem.源行号
                strInfo = strInfo & ", 左右间距=" & tmpItem.左右间距
                strInfo = strInfo & ", 上下间距=" & tmpItem.上下间距
                strInfo = strInfo & ", 横向分栏=" & tmpItem.横向分栏
                strInfo = strInfo & ", 纵向分栏=" & tmpItem.纵向分栏
                strInfo = strInfo & ", 表格线加粗=" & Abs(CInt(tmpItem.表格线加粗))
                
                strInfo = strInfo & ", 自适应行高=" & IIF(tmpItem.自适应行高, "1", "Null")
                strInfo = strInfo & ", 拆分单元格=" & IIF(tmpItem.拆分单元格, "1", "Null")
                strInfo = strInfo & ", 自动填充=" & IIF(tmpItem.自动填充, "1", "Null")
                strInfo = strInfo & ", 水平反转=" & IIF(tmpItem.水平反转, "1", "Null")
                strInfo = strInfo & ", 关联表格=" & IIF(tmpItem.关联表格 <> "", "'" & tmpItem.关联表格 & "'", "Null")
                
                If gobjCompatibility.BuildInsertString("zlRPTItems", strInfo, strSQL) = False Then
                    Err.Raise 60001, , "保存报表时上传zlRPTItems数据发生异常！"
                End If
                gcnOracle.Execute Replace(strSQL, MSTR_COMMA_SPLIT, ",")
        ...
        For Each tmpItem In .Items
            ...
            If InStr(",1,2,3,4,5,10,11,12,13,", "," & tmpItem.类型 & ",") > 0 Then '子项除外
                ...
                strInfo = strInfo & ", 前景=" & tmpItem.前景
                strInfo = strInfo & ", 背景=" & tmpItem.背景
                strInfo = strInfo & ", 排序='" & Replace(tmpItem.排序, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", 格式='" & Replace(tmpItem.格式, ",", MSTR_COMMA_SPLIT) & "'"
                strInfo = strInfo & ", 汇总='" & Replace(tmpItem.汇总, ",", MSTR_COMMA_SPLIT) & "'"
                
                strInfo = strInfo & ", 分栏=" & IIF(tmpItem.分栏 = 0, 1, tmpItem.分栏)
                strInfo = strInfo & ", 系统=" & Abs(CInt(tmpItem.系统))
                strInfo = strInfo & ", 父ID=" & IIF(lngParentID = 0, "Null", lngParentID)
                strInfo = strInfo & ", 源ID=Null"
                strInfo = strInfo & ", 源行号=" & tmpItem.源行号
                strInfo = strInfo & ", 左右间距=" & tmpItem.左右间距
                strInfo = strInfo & ", 上下间距=" & tmpItem.上下间距
                strInfo = strInfo & ", 横向分栏=" & tmpItem.横向分栏
                ...
                If tmpItem.类型 = Val("4-任意表") Or tmpItem.类型 = Val("5-汇总表") Then
                    For Each tmpID In tmpItem.SubIDs
                        With .Items("_" & tmpID.id)
                            ...
                            strInfo = "ID=" & lngItemSubID
                            strInfo = strInfo & ", 报表ID=" & lngRPTID
                            strInfo = strInfo & ", 格式号=" & .格式号
                            strInfo = strInfo & ", 上级ID=" & lngItemID
                            strInfo = strInfo & ", 类型=" & .类型
                            strInfo = strInfo & ", 序号=" & .序号
                            strInfo = strInfo & ", 内容='" & Replace(.内容, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", 表头='" & Replace(.表头, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", X=" & .X
                            strInfo = strInfo & ", Y=" & .Y
                            
                            strInfo = strInfo & ", W=" & .W
                            strInfo = strInfo & ", H=" & .H
                            strInfo = strInfo & ", 行高=" & .行高
                            strInfo = strInfo & ", 对齐=" & .对齐
                            strInfo = strInfo & ", 字体='" & Replace(.字体, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", 字号=" & .字号
                            strInfo = strInfo & ", 粗体=" & Abs(CInt(.粗体))
                            strInfo = strInfo & ", 斜体=" & Abs(CInt(.斜体))
                            strInfo = strInfo & ", 下线=" & Abs(CInt(.下线))
                            strInfo = strInfo & ", 边框=" & Abs(CInt(.边框))
                            
                            strInfo = strInfo & ", 网格=" & .网格
                            strInfo = strInfo & ", 前景=" & .前景
                            strInfo = strInfo & ", 背景=" & .背景
                            strInfo = strInfo & ", 排序='" & Replace(.排序, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", 格式='" & Replace(.格式, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", 汇总='" & Replace(.汇总, ",", MSTR_COMMA_SPLIT) & "'"
                            strInfo = strInfo & ", 分栏=" & .分栏
                            strInfo = strInfo & ", 系统=" & Abs(CInt(.系统))
                            strInfo = strInfo & ", 自调=" & Abs(CInt(.自调))
                            strInfo = strInfo & ", 父ID=" & IIF(lngParentID = 0, "Null", lngParentID)
                            
                            strInfo = strInfo & ", 表格线加粗=" & Abs(CInt(.表格线加粗))
                            strInfo = strInfo & ", 自适应行高=" & IIF(.自适应行高, "1", "Null")
                            strInfo = strInfo & ", 拆分单元格=" & IIF(.拆分单元格, "1", "Null")
                            strInfo = strInfo & ", 自动填充=" & IIF(.自动填充, "1", "Null")
...
'     失败=空
                    ...
                    Case 3 '无类型(直接替换)
                        ...
                        ElseIf objPars("_" & CInt(bytPar)).缺省值 = "选择器定义…" Then
                            ...
                            ElseIf objPars("_" & CInt(bytPar)).明细SQL <> "" And objPars("_" & CInt(bytPar)).明细字段 <> "" Then
                                ...
                                StrPar = GetDefaultValue(strSQLInfo, objPars("_" & CInt(bytPar)).明细字段, , intCurConnect)
    ...
    If intCurConnect = 0 Then
        strSQLref = strCheck
        If InStr(UCase(strCheck), STR_WHERE) > 0 Then
            'strCheck = Replace(UCase(strCheck), "WHERE ", "Where Rownum<1 And ")   '该行简单处理会使单引号内的字符串大写，引起XML操作函数的不适应。如： Extractivalue函数的xml字串参数
            intLen = Len(STR_WHERE)
            strSQLInfo = ""
            lngPos = 1
            Do While lngPos <= Len(strCheck)
                If UCase$(Mid$(strCheck, lngPos, intLen)) = STR_WHERE Then
                    strSQLInfo = strSQLInfo & STR_WHERE_NEW
                    lngPos = lngPos + intLen
                Else
                    strSQLInfo = strSQLInfo & Mid$(strCheck, lngPos, 1)
                    lngPos = lngPos + 1
                End If
            Loop
            If strSQLInfo <> "" Then
...
Public Function PngToBitmap(ByVal strFile As String) As StdPicture
    Dim lngToken As Long, lngImage As Long
    
    lngToken = InitGDIPlus()
    On Error GoTo hErr
    
    '判断PNG
    Set PngToBitmap = New StdPicture
    If GdipLoadImageFromFile(StrPtr(strFile), lngImage) = 0 Then
        If GetRawFormat(lngImage) = GpImageFormatPNG Then
            Set PngToBitmap = LoadPictureGDIPlus(strFile, , , vbWhite)
        End If
    End If
    
hErr:
    Call FreeGDIPlus(lngToken)
End Function
        ...
        For i = 0 To UBound(arrPars)
            If UCase(CStr(arrPars(i))) Like UCase(strName) & "=*" Then
                ...
                If gobjFile.FileExists(strFile) Then
                    ...
                    If Err.Number = Val("481-图片无效") Then
                        Set LoadPictureFromPar = PngToBitmap(strFile)
                    End If
...
Public Function GetWindowClassName(ByVal objForm As Object) As String
'功能：通过API获取窗体的类名称
    Dim lngHandle As Long
    Dim strName As String * 255
    
    If Not objForm Is Nothing Then
        lngHandle = GetWindow(objForm.hwnd, GW_CHILD)
        Call GetClassName(lngHandle, strName, Len(strName))
        If InStr(strName, Chr(0)) > 0 Then strName = Left(strName, InStr(strName, Chr(0)) - 1)
        GetWindowClassName = strName
    End If
End Function
...
'参数：blnOutCall=是否外部通过接口在调用
    ...
    If rsTmp.RecordCount = 1 Then
        '非VB父窗体对象
        If frmParent Is Nothing Then
            frmShow.mblnNonVB = True
        Else
            frmShow.mblnNonVB = Not UCase(GetWindowClassName(frmParent)) Like "THUNDERPICTUREBOXDC*"
        End If
...
'   2.使用该函数之前必须初始gobjReport,garrPars,glngGroup的值
    ...
    Call ExportLog(False, "mdlPublic", "ShowReport", "开始", "")
    ...
    ShowReport = True
    Call ExportLog(True, "mdlPublic", "ShowReport", "完成", "")
End Function
...
Private Sub SetFont(ByVal sftSource As StdFont, ByRef sftTarget As StdFont)
    With sftTarget
        .name = sftSource.name
        .Size = sftSource.Size
        .Bold = sftSource.Bold
        .Underline = sftSource.Underline
        .Italic = sftSource.Italic
        .Strikethrough = sftSource.Strikethrough
    End With
End Sub
...
Public Function DrawCell(Dev As Object, ByVal Data As Variant, ByVal X As Long _
    ...
    , ByVal objInfoEx As PropertyBag _
...
'   objInfoEx=参数扩展对象
'       OffsetHeight: 偏移高度（Twip）
'说明：
'   1.在使用该函数之前,应该没有改变设备的作图初始值
'   2.输出后定位光标位置在本次输出范围的右上角

'    Const STR_SYMBOL_TAIL As String = _
'        "。，、；：？！…·‘’“”～‖∶＇〃〔〕〈〉《》「」『』〖〗【】" & _
'        "~`!@#$%^&*()_+-={}|[]\:"";'<>?,./"
    '排除成对符号的首符号提行  ‘“〔〈《「『〖【({[<
    Const STR_SYMBOL_TAIL As String = _
    ...
    If objInfoEx Is Nothing Then
        lngOffsetH = 0
    Else
        lngOffsetH = objInfoEx.ReadProperty("OffsetHeight", 0)
    End If
    
    Y = Y + lngOffsetH
    If lngOffsetH > 0 Then
        TH = 0
    End If
    ...
    ElseIf TypeName(Data) = "String" Then
        ...
        '不要用Set Dev.Font=Font,对象是byRef
        Call SetFont(Font, Dev.Font)
...
'        End If
        ...
        If blnH Then
            ...
            Else
                ...
                If Wrap = False And blnCellWordWrap = False And InStr(strTemp, vbCr) <= 0 Then
                    '不自动拆行时超宽部分不输出
                ...
                Else
                    ...
                    '通过打印机对象统一计算折行逻辑，避免预览与打印输出效果不一致的情况
                    If UCase(TypeName(Dev)) = UCase("Printer") Then
                        Set objTmp = Dev
                    Else
                        '预览时，获取当前打印机对象实现统一计算折行逻辑
                    ...
                    Call SetFont(Font, objTmp.Font)
                    
                    ReDim arrText(0) '在此,第一行不可能超高
                    For i = 1 To Len(strTemp)
                        If Mid(strTemp, i, 1) = vbCr Then
                            '多行超高则退出,超高部份不输出
                            If (objTmp.TextHeight("字") + sngYDistance) * (UBound(arrText) + 2) - sngYDistance > Round(H / Ratio) Then Exit For
                            ReDim Preserve arrText(UBound(arrText) + 1)
                        ElseIf objTmp.TextWidth(arrText(UBound(arrText)) & Mid(strTemp, i, 1)) + intCellBorder + intBase > Round(W / Ratio) Then
                            '多行超高则退出,超高部份不输出
                            If (objTmp.TextHeight("字") + sngYDistance) * (UBound(arrText) + 2) - sngYDistance > Round(H / Ratio) Then Exit For
                            ReDim Preserve arrText(UBound(arrText) + 1)
                        End If
                        '有可能一行一个字符宽度都不够
                        If objTmp.TextWidth(arrText(UBound(arrText)) & Mid(strTemp, i, 1)) - intCellBorder <= Round(W / Ratio) And Mid(strTemp, i, 1) <> vbCr Then
                            arrText(UBound(arrText)) = arrText(UBound(arrText)) & Mid(strTemp, i, 1)
                        End If
                    Next
                    
                    '标签元素处理行首字符为标点符号（只支持2位以内连续的标点符号）
                    '注意：对成对符号的首符号不提行处理（依据Word逻辑）
                    If Border = "0000" Then
                        For i = LBound(arrText) To UBound(arrText)
                            If i > LBound(arrText) Then
                                If InStr(STR_SYMBOL_TAIL, Left(arrText(i), 1)) > 0 Then
                                    arrText(i - 1) = arrText(i - 1) & Left(arrText(i), 1)       '上一行
                                    arrText(i) = Mid(arrText(i), 2)                             '当前行
                                    If InStr(STR_SYMBOL_TAIL, Left(arrText(i), 1)) > 0 Then
                                        arrText(i - 1) = arrText(i - 1) & Left(arrText(i), 1)   '上一行
                                        arrText(i) = Mid(arrText(i), 2)                         '当前行
                                    End If
...
'返回：如果无打印机或纸张不对,则失败
    Const STR_ITEMS As String = "PaperBin=|PaperCopy=1|PaperForm=0|PaperFormName=|Printer="
    ...
    Dim strTmp As String
    Dim strDefault As String
    Dim arrItems() As String, arrTmp() As String
    ...
    If Printers.count = 1 Then
        ...
        If strPrinter <> Printers(0).DeviceName Then
            '保存注册表信息
            strPrinter = Printers(0).DeviceName
            arrItems = Split(STR_ITEMS, "|")
            For i = LBound(arrItems) To UBound(arrItems)
                arrTmp = Split(arrItems(i), "=")
                SaveSetting "ZLSOFT" _
                    , "私有模块\" & App.ProductName & "\LocalSet\" & objReport.编号 & "\所有格式" _
                    , arrTmp(0) _
                    , IIF(UCase(arrTmp(0)) = "PRINTER", Printer.DeviceName, arrTmp(1))
            Next
        End If
    Else
        '本地设置
    ...
    If strPrinter = "" Then
        ...
        If Not ReportLocalSet(objReport.系统, objReport.编号, False, objReport.bytFormat, frmMain) Then
            Exit Function
        End If
        strPrinter = GetRegPrinterInfo("Printer", objReport.编号, objFmt.说明, objReport)
    End If
    ...
    '纸张
    If gtypSpecialPrintPage.Mode > enuPrintMode.常规打印 Then
        '特殊打印场景下，不设置物理打印纸张
    Else
        If objFmt.纸张 < Val("256-自定义纸张") Then
            Printer.PaperSize = objFmt.纸张
        ...
        If objFmt.纸张 = Val("256-自定义纸张") Then
            If IsWindowsNT Then
                ...
                If Val(strTmp) = 1 Then
                    ...
                    If Not SetNTPrinterPaper_Form(frmMain.hwnd, objFmt.W / Twip_mm, objFmt.H / Twip_mm _
                        , IIF(objFmt.纸向 = 0, 1, objFmt.纸向), intCopies, , strFormName, Printer) Then
                        InitPrinter = False
                    End If
                Else
                    If Not SetNTPrinterPaper(frmMain.hwnd, objFmt.W / Twip_mm, objFmt.H / Twip_mm _
                        , IIF(objFmt.纸向 = 0, 1, objFmt.纸向), intCopies) Then
                        InitPrinter = False
                    End If
                End If
            End If
...
'    End If
    ...
    If blnIsBusinessTable Then
        strSQL = _
            ...
            "Where Object_Type In ('TABLE', 'VIEW') " & vbNewLine & _
            "  And Owner Not In ('CTXSYS', 'EXFSYS', 'LBACSYS', 'MDSYS', 'DMSYS', 'OLAPSYS', 'OWBSYS', 'ORDSYS', 'TSMSYS', " & vbNewLine & _
            "                    'WKSYS', 'WMSYS', 'DBSNMP', 'OUTLN', 'SCOTT', 'SYSMAN', 'XDB', 'SYSTEM', 'SYS', 'DEMO', " & vbNewLine & _
            "                    'ZLTOOLS') " & vbNewLine & _
            "  And Object_Name Not Like 'BIN$%' " & vbNewLine & _
            "Order By Sort Desc, Object_Name, Owner"
    Else
        strSQL = _
            "Select Owner, Object_Name, Sign(Ascii(Object_Name) - 256) As Sort " & vbNewLine & _
            ...
            "Where Object_Type In ('TABLE', 'VIEW') " & vbNewLine & _
            "  And Owner Not In ('CTXSYS', 'EXFSYS', 'LBACSYS', 'MDSYS', 'DMSYS', 'OLAPSYS', 'OWBSYS', 'ORDSYS', 'TSMSYS', " & vbNewLine & _
            "                    'WKSYS', 'WMSYS', 'DBSNMP', 'OUTLN', 'SCOTT', 'SYSMAN', 'XDB') " & vbNewLine & _
            "  And Object_Name Not Like 'BIN$%' " & vbNewLine & _
            "Order By Sort Desc, Object_Name, Owner"
    End If
...
'功能：将SQL语句变换为只为空格间隔的形式,以便于分析
    ...
    If lngB = -1 Then
        For i = 0 To UBound(arrSeg)
            ...
            For j = lngB To lngE
                '将单引号内的空格符转换，避免后面代码误替换
                If Mid(strSQL, j, 1) = " " Then
                    strSQL = Left(strSQL, j - 1) & STR_SPLIT & Mid(strSQL, j + 1)
                    If blnDo = False Then blnDo = True
                End If
            Next
        Next
    End If
    ...
    If blnDo Then
        '还原单引号内的空格
        strSQL = Replace(strSQL, STR_SPLIT, " ")
    End If
...
'功能：拷贝报表对象,防止因Set造成地址的访问
    ...
    For i = 1 To objS.Fmts.count
        With objS.Fmts(i)
            objO.Fmts.Add .序号, .说明, .W, .H, .纸张, .纸向, .动态纸张, .图样, .停用, .停用原因, "_" & .序号
        End With
    Next
    ...
    For Each objItem In objS.Items
        With objItem
            objO.Items.Add .id, .格式号, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头, .X, .Y, .W, .H _
                ...
                , .纵向分栏, .Relations, .ColProtertys, .水平反转, .拆分单元格, .自动填充, .关联表格
        End With
    Next
...
'      2.目前不支持图片元素内容的导出
    ...
    strSQL = "Select ID,编号,名称,说明,密码,打印机,进纸,票据,打印方式,系统,程序ID,功能,修改时间,发布时间" & vbCr & _
    ...
    '报表元素
    strSQL = "系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐,自调,字体" & _
             ",字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,ID as 原ID,父ID,源ID,上下间距,左右间距" & _
             ",源行号,横向分栏,纵向分栏,表格线加粗,自适应行高,水平反转,拆分单元格,自动填充,关联表格"
    If gobjCompatibility.BuildSelectString("zlRPTItems", strSQL, "报表ID=[1]", strSQL, True) = False Then
        Err.Raise 60001, , "ExportReport异常！"
    End If
    strSQL = strSQL & " " & vbCr & "Start With 上级ID is NULL Connect by Prior ID=上级ID"
    Set rsTmp = OpenSQLRecord(strSQL, "ExportReport", lngRPTID)
...
'      2.覆盖已有报表时,当前报表信息不变,除了纸张信息
    ...
    If Not blnOnlyData Then
        ...
        rsFMT.Open "Select 报表ID,序号,说明,W,H,纸张,纸向,动态纸张,图样 From zlRPTFmts Where Rownum<1" _
            , gcnOracle, adOpenKeyset, adLockOptimistic
        StrFmt = GetFieldNames(rsFMT)
        
        strSQL = "系统,ID,报表ID,格式号,名称,类型,上级ID,序号,参照,性质,内容,表头,X,Y,W,H,行高,对齐,自调" & _
                 ",字体,字号,粗体,斜体,下线,前景,背景,边框,排序,格式,汇总,分栏,网格,父ID,源ID,上下间距,左右间距" & _
                 ",源行号,横向分栏,纵向分栏,表格线加粗,自适应行高,水平反转,拆分单元格,自动填充,关联表格"
        If gobjCompatibility.BuildSelectString("zlRPTItems", strSQL, "Rownum<1", strSQL, False) = False Then
            Err.Raise 60001, , "ImportReport异常！"
        End If
        rsItem.CursorLocation = adUseClient
        rsItem.Open strSQL, gcnOracle, adOpenKeyset, adLockOptimistic
        strItem = GetFieldNames(rsItem)
    End If
    
    rsData.CursorLocation = adUseClient
    rsData.Open "Select ID,报表ID,数据连接编号,名称,字段,对象,类型,说明 " & vbCr & _
                "From zlRPTDatas " & vbCr & _
                "Where Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
    ...
    rsPar.CursorLocation = adUseClient
    rsPar.Open "Select 源ID,组名,序号,名称,类型,缺省值,格式,值列表,分类SQL,明细SQL,分类字段,明细字段,对象,锁定 " & vbCr & _
               "From zlRPTPars Where Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
    StrPar = GetFieldNames(rsPar)
    
    rsCol.CursorLocation = adUseClient
    rsCol.Open "Select 报表ID,元素ID,条件名称,条件字段,条件关系,条件值,字体颜色,背景颜色,是否加粗,是否整行应用,对齐 " & vbCr & _
               "From zlRPTColProterty Where  Rownum<1", gcnOracle, adOpenKeyset, adLockOptimistic
                ...
                If Not rsProgram.EOF Then
                    ...
                    If strObject <> "" Then
                        For i = 0 To UBound(Split(strObject, ","))
                            gcnOracle.Execute _
                                GetInsertProgPrivs(Nvl(rsTmp!系统, 0), Nvl(rsTmp!程序id), Nvl(rsTmp!功能) _
        ...
        If gcnOracle.State = 1 Then
            ...
            If Not mrsForms Is Nothing Then
                ...
                If gstrComputerName <> "" Then
                    'ReportMan方法才设置gstrComputerName值，也就是管理工具才执行该存储过程
...
'      strUserDef：主要适用于工程中，一个窗体多个程序使用(程序使用 set frmxxx=new frm设计窗体形式)，为了按不同应用保存恢复各自的个性化状态，需要直接确定命名。
    ...
    If Not gcnOracle Is Nothing Then
        Call SaveSessAction(objForm.name & "," & strUserDef)
    End If
    ...
    If Not gcnOracle Is Nothing And strProjectName <> "" And gblnRunLog Then
        If gcnOracle.State = 1 Then
            ...
            With mrsForms
                strEnter = format(mdlPublic.Currentdate, "yyyy/mm/dd hh:mm:ss")     '记录下窗体的进入时间
                .Filter = "Handle = " & objForm.hwnd
                If .RecordCount <= 0 Then
                    .AddNew
                End If
...
Public Function PrintPage(ByVal intPage As Integer, objOut As Object, frmSource As Object, _
    ...
    Optional ByVal blnMeasure As Boolean, Optional lngMaxH As Long, _
    Optional ByVal lngOffsetH As Long = 0) As Boolean
'功能：打印(预览)一页
'参数：intPage=要输出的页号,>=0
...
'      lngOffsetH=特殊打印场景（gtypSpecialPrintPage）的偏移高度(Twip)
'说明：该函数不处理换页或结束输出
'参数：frmSource.mobjReport,marrPage,mLibDatas
    ...
    Dim intDPI_X, intDPI_Y As Integer
    Dim X1, Y1, X2, Y2 As Long
    Dim objDrawCellEx As New PropertyBag
    Dim colPageStartRec As New Collection
    Dim objData As LibData
    Dim lngRecNo As Long, lngRecNoGrid As Long
                    ...
                    With objPageCell
                                ...
                                If Not blnMeasure And Not blnPressWork Then
                                    ...
                                    For j = 0 To objBody.FixedCols - 1
                                        ...
                                        If InStr(strSkip, "[" & i & "," & j & "]") = 0 Then
                                            ...
                                            '汇总表格-表头
                                            objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                            If Not DrawCell(objOut, objBody.Text, .X + X, .Y + Y, W, H _
                                                , objDrawCellEx, .X + .W, , objBody.GridColor, lngForeColor _
                                                , objBody.BackColor, objFont, strBdr _
                                        ...
                                        For j = .ColB To .ColE
                                            ...
                                            If InStr(strSkip, "[" & i & "," & j & "]") = 0 Then
                                                ...
                                                If W > 0 Then
                                                    ...
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, strValue, .X + X, .Y + Y, W, H _
                                                        , objDrawCellEx, .X + .W * .Copys, _
                        ...
                        ElseIf .H >= .FixH Then
                            ...
                            For k = 1 To .Copys '处理任意表格数据分栏
                                ...
                                '票据需要对最后一页补充空白行
                                If frmSource.mobjReport.票据 And (UBound(arrPage) = intPage _
                                    Or GridAtCard(frmSource.mobjReport, objItem.id)) Then
                                    '求实际行范围的总高
                                    lngRangeHeight = 0
                                    lngRowCount = 0
                                    For i = .RowB + lngRowTotal To .RowE
                                        ...
                                        If lngRangeHeight + lngRowHeight >= objBody.Height Then
                                            If lngRowHeight >= objBody.Height And lngRangeHeight = 0 Then
                                                '当实际行高大于标准表体高，并且lngRangeHeight为0时，强制一行
                                                lngRowCount = 1
                                            End If
                                            Exit For
                                        Else
                                ...
                                Else
                                    ...
                                    '记录任意表格当前页的开始记录行号
                                    On Error Resume Next
                                    colPageStartRec.Add lngRowB + 1, objItem.名称
                                    On Error GoTo 0
                                End If
                                    ...
                                    If i > .RowE Then
                                        ...
                                        If Not blnMeasure Then
                                            ...
                                            For j = .ColB To .ColE
                                                ...
                                                If W > 0 Then
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, "", .X + X, .Y + Y, W, H, objDrawCellEx _
                                                        , .X + .W * .Copys, , objBody.GridColor, objBody.ForeColor _
                                                        , objBody.BackColor, objFont, strBdr _
                                                ...
                                                If InStr(strSkip, "[" & i & "," & j & "]") = 0 Then
                                                    ...
                                                    If W > 0 Then
                                                        ...
                                                        If Not objPic Is Nothing Then
                                                            ...
                                                            End If
                                                            objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                            If Not DrawCell(objOut, objPic, .X + X, .Y + Y, W, H, objDrawCellEx _
                                                                , .X + .W * .Copys, , objBody.GridColor, , , , strBdr _
                                                                , GetHscAlign(objBody.Cell(flexcpAlignment, i, j), objBody.Text) _
                                                                , GetVscAlign(objBody.CellAlignment) _
                                                                , True, sngScale, , objItem.表格线加粗) Then
                                                                Exit Function
                                                        ...
                                                        Else
                                                            ...
                                                            '自由表格-表体
                                                            objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                            If Not DrawCell(objOut, objBody.Text, .X + X, .Y + Y, W, H, objDrawCellEx _
                                                                , .X + .W * .Copys, , objBody.GridColor, lngForeColor, lngBackColor _
                                                                , objFont, strBdr, GetHscAlign(objBody.Cell(flexcpAlignment, i, j) _
                                                                , objBody.Text, objBody, j), GetVscAlign(objBody.ColAlignment(j)) _
                    ...
                    If blnWithData = False Then
                        Select Case .类型
                            Case 10 '框线
                                If Not blnMeasure Then
                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                    If Not DrawCell(objOut, -1, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                        , lngPaperW, lngPaperH, 0, .前景, , , , , , , sngScale, , .粗体 _
                                        , IIF(.边框, 1, 0)) Then Exit Function
                                Else
                                    lngCurH = .Y + .H
                            ...
                            Case 11 '图片
                                ...
                                If Not blnMeasure Then
                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                    If Not DrawCell(objOut, objPic, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                        , lngPaperW, lngPaperH, 0, .前景, , , IIF(.边框, "1111", "0000"), 0, 0 _
                                        , .粗体, sngScale) Then Exit Function
                                Else
                                    lngCurH = .Y + .H
                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
...
'                                    Else
                                    ...
                                    If (.序号 = 3 Or .序号 = 1) And blnPrint = False Then
                                        '128码自动调整宽度
                                        If Val(Mid(.表头, 3, 1)) = 0 Then
                                ...
                                If Not blnMeasure Then
                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                    If Not DrawCell(objOut, objPic, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                        , lngPaperW, lngPaperH, 0, , , , IIF(.边框, "1111", "0000"), , , _
                                        , sngScale) Then Exit Function
                                Else
                                    lngCurH = .Y + .H
                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
                            ...
                            Case 1 '线条
                                If Not blnMeasure Then
                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                    If Not DrawCell(objOut, 1, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                        , lngPaperW, lngPaperH, .前景, .前景, , , , , , _
                                        , sngScale, , .粗体) Then Exit Function
                                Else
                                    lngCurH = .Y + .H
                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
                            ...
                            Case 12 '图表@@@
                                If intPage = 0 Then '只在第一页打印
                                    If Not blnMeasure Then
                                        ...
                                        objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                        If Not DrawCell(objOut, objPic, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                            , lngPaperW, lngPaperH, , , , , IIF(.边框, "1111", "0000"), , , _
                                            , sngScale) Then Exit Function
                                    Else
                                        lngCurH = .Y + .H
                                        If lngCurH > lngPaperH Then lngCurH = lngPaperH
                            ...
                            Case 2, 3 '标签,标签绑定图片
                                ...
                                If strData <> "" Then
                                    For i = 0 To UBound(Split(strData, "|"))
                                        ...
                                        '标签关联表格更新内容
                                        If .关联表格 <> "" And strData Like "*.*" Then
                                            On Error Resume Next
                                            Set objData = Nothing
                                            Set objData = frmSource.mLibDatas("_" & Split(strData, ".")(0))
                                            On Error GoTo 0
                                            
                                            lngRecNo = -1
                                            lngRecNoGrid = -1
                                            If Not objData Is Nothing Then
                                                If objData.DataSet.RecordCount > 0 Then
                                                    lngRecNo = objData.DataSet.AbsolutePosition
                                                    
                                                    On Error Resume Next
                                                    lngRecNoGrid = Val(colPageStartRec(.关联表格))
                                                    On Error GoTo 0
                                                    
                                                    If lngRecNoGrid >= 0 Then
                                                        objData.DataSet.Move lngRecNoGrid - 1, 1
                                                    Else
                                                        objData.DataSet.Move objData.DataSet.RecordCount
                                                    End If
                                                End If
                                            End If
                                        End If
                                        ...
                                        '标签关联表格的指针还原
                                        If lngRecNo >= 0 And Not objData Is Nothing Then
                                            If objData.DataSet.AbsolutePage <> lngRecNo Then
                                                objData.DataSet.Move lngRecNo - 1, 1
                                            End If
                                        End If
                                    Next
                                End If
                                ...
                                If gobjFile.FileExists(strValue) Then
                                    ...
                                    If Not blnMeasure Then
                                        objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                        If Not DrawCell(objOut, .图片, .X + lngChildX, .Y + lngChildY, .W, .H, objDrawCellEx _
                                            , lngPaperW, lngPaperH, 0, .前景, , , IIF(.边框, "1111", "0000"), 0, 0, .粗体 _
                                            , sngScale) Then Exit Function
                                    Else
                                        lngCurH = .Y + .H
                                        If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                    ...
                                    If objItem.性质 > 0 And objItem.参照 <> "" And blnHaveGrid Then
                                        ...
                                        If strValue <> "" Then
                                            If Not blnMeasure Then
                                                ...
                                                objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                If .水平反转 Then
                                                    If Not DrawCell(objOut, frmSource.picRotate(.id).Image, .X + lngChildX, .Y + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, .背景, objFont, "0000" _
                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                Else
                                                    If Not DrawCell(objOut, strValue, lngOutX, lngOutY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, .背景, objFont, IIF(.边框, "1111", "0000") _
                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                End If
                                            Else
                                                lngCurH = lngOutY + .H
                                            ...
                                            If Not blnMeasure Then
                                                ...
                                                If .水平反转 Then
                                                    If Not DrawCell(objOut, frmSource.picRotate(.id).Image, .X + lngChildX, .Y + lngChildY _
                                                        ...
                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                Else
                                                    If Not DrawCell(objOut, strValue, .X + lngChildX, .Y + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, .背景, objFont, IIF(.边框, "1111", "0000") _
                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                End If
                                            Else
                    ...
                    For Each objPageCard In arrPageCard(intPage).Items
                        ...
                        For Y = 1 To objPageCard.Item.count
                            ...
                            With objTemp
                                ...
                                If Not blnMeasure Then
                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                    If Not DrawCell(objOut, objPic, .X + lngY, .Y + lngX, .W, .H, objDrawCellEx _
                                        , lngPaperW, lngPaperH, 0, .前景, , , IIF(.边框, "1111", "0000"), 0, 0 _
                                        , .粗体, sngScale) Then
                                        Exit Function
                                    End If
                                    ...
                                    With objItem
                                        ...
                                        If .父ID = objPageCard.id Then
                                            ...
                                            Case 10 '框线
                                                If Not blnMeasure Then
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, -1, .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, , , , , , _
                                                        , sngScale, , .粗体, IIF(.边框, 1, 0)) Then Exit Function
                                                Else
                                                    lngCurH = .Y + .H
                                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
                                            ...
                                            Case 1 '线条
                                                If Not blnMeasure Then
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, 1, .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, .前景, .前景, , , , , , _
                                                        , sngScale, , .粗体) Then Exit Function
                                                Else
                                                    lngCurH = .Y + .H
                                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                            ...
                                            Case 11 '图片
                                                ...
                                                If Not blnMeasure Then
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, objPic, .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, , _
                                                        , IIF(.边框, "1111", "0000"), 0, 0, .粗体, sngScale) Then Exit Function
                                                Else
                                                    lngCurH = .Y + .H
                                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
...
'                                                    Else
                                                ...
                                                If Not blnMeasure Then
                                                    objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                    If Not DrawCell(objOut, objPic, .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, , , _
                                                        , IIF(.边框, "1111", "0000"), , , , sngScale) Then Exit Function
                                                Else
                                                    lngCurH = .Y + .H
                                                    If lngCurH > lngPaperH Then lngCurH = lngPaperH
                                                    If lngCurH > lngMaxH Then lngMaxH = lngCurH
                                            ...
                                            Case 2, 3 '标签,标签绑定图片
                                                ...
                                                If gobjFile.FileExists(strValue) Then
                                                    ...
                                                    If Not blnMeasure Then
                                                        objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                        If Not DrawCell(objOut, .图片, .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                            , .W, .H, objDrawCellEx, lngPaperW, lngPaperH, 0, .前景, , _
                                                            , IIF(.边框, "1111", "0000"), 0, 0, .粗体, sngScale) Then Exit Function
                                                    Else
                                                        lngCurH = .Y + .H
                                                    ...
                                                    If objItem.性质 > 0 And objItem.参照 <> "" And blnHaveGrid Then
                                                        ...
                                                        If strValue <> "" Then
                                                            If Not blnMeasure Then
                                                                ...
                                                                objDrawCellEx.WriteProperty "OffsetHeight", lngOffsetH
                                                                If .水平反转 Then
                                                                    If Not DrawCell(objOut, frmSource.picRotate(.id).Image, lngOutX + lngY _
                                                                        , lngOutY + lngX + lngChildY, .W, .H, objDrawCellEx, lngPaperW, lngPaperH _
                                                                        , 0, .前景, .背景, objFont, "0000", .对齐, 0, True _
                                                                        , sngScale, .网格) Then Exit Function
                                                                Else
                                                                    If Not DrawCell(objOut, strValue, lngOutX + lngY _
                                                                        , lngOutY + lngX + lngChildY, .W, .H, objDrawCellEx, lngPaperW, lngPaperH _
                                                                        , 0, .前景, .背景, objFont, IIF(.边框, "1111", "0000"), .对齐, 0, True _
                                                                        , sngScale, .网格) Then Exit Function
                                                                End If
                                                            Else
                                                                lngCurH = lngOutY + .H
                                                    ...
                                                    Else
                                                        If strValue <> "" Then
                                                            If Not blnMeasure Then
                                                                ...
                                                                If .行高 = 1 Then Set objFont = GetAutoFont(strValue, .W, .H, objFont, objOut, True, .网格)
                                                                If .水平反转 Then
                                                                    If Not DrawCell(objOut, frmSource.picRotate(.id).Image _
                                                                        , .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH _
                                                                        , 0, .前景, .背景, objFont, "0000" _
                                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                                Else
                                                                    If Not DrawCell(objOut, strValue _
                                                                        , .X + lngY + lngChildX, .Y + lngX + lngChildY _
                                                                        , .W, .H, objDrawCellEx, lngPaperW, lngPaperH _
                                                                        , 0, .前景, .背景, objFont, IIF(.边框, "1111", "0000") _
                                                                        , .对齐, 0, True, sngScale, .网格) Then Exit Function
                                                                End If
                                                            Else
                                                                lngCurH = .Y + .H
    ...
    If Not blnMeasure Then
        '打印的实际可输出区域（预览）
        If blnSure Then
            intDPI_X = GetDeviceCaps(Printer.hDC, LOGPIXELSX)
            intDPI_Y = GetDeviceCaps(Printer.hDC, LOGPIXELSY)
            X1 = GetDeviceCaps(Printer.hDC, PHYSICALOFFSETX) / intDPI_X * 1440 * sngScale
            Y1 = GetDeviceCaps(Printer.hDC, PHYSICALOFFSETY) / intDPI_Y * 1440 * sngScale
            X2 = GetDeviceCaps(Printer.hDC, PHYSICALWIDTH) / intDPI_X * 1440 * sngScale
            Y2 = GetDeviceCaps(Printer.hDC, PHYSICALHEIGHT) / intDPI_Y * 1440 * sngScale

            objOut.DrawStyle = 2
            objOut.Line (X1, Y1)-(X2 - X1 - Screen.TwipsPerPixelX, Y2 - Y1 - Screen.TwipsPerPixelY) _
                , &H808080, B
            objOut.DrawStyle = 0
        End If
    End If
...
'返回：lngID=报表ID或组ID
    ...
    Call ExportLog(False, "mdlPublic", "IsGroup", "开始", "")
    ...
    If TypeName(varReport) = "String" Then
        strSQL = "Select ID,编号,名称,说明,密码,打印机,进纸,票据,打印方式,系统,程序ID,功能,修改时间,发布时间" & _
                 ...
                 "Where Nvl(系统,0)=[3] And Upper(编号)=[1]"
    Else
        strSQL = "Select ID,编号,名称,说明,密码,打印机,进纸,票据,打印方式,系统,程序ID,功能,修改时间,发布时间" & _
                 "    ,禁止开始时间,禁止结束时间 " & vbCr & _
                 "From zlReports " & vbCr & _
    ...
    If Not rsTmp.EOF Then
        ...
        gdatModiTime = grsReport!修改时间
        lngID = rsTmp!id
        Call ExportLog(True, "mdlPublic", "IsGroup", "报表", "", Nvl(rsTmp!编号), Nvl(rsTmp!名称))
        Exit Function
    End If
    ...
    If Not rsTmp.EOF Then
        lngID = rsTmp!id
        Call ExportLog(False, "mdlPublic", "IsGroup", "报表组", "", Nvl(rsTmp!编号), Nvl(rsTmp!名称))
    End If
    IsGroup = True
    Call ExportLog(True, "mdlPublic", "IsGroup", "完成", "")
    Exit Function
    
errH:
    Call ExportLog(True, "mdlPublic", "IsGroup", "Err=" & Err.Description, "")
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
End Function
...
'说明：因调用报表时未传入调用位置(系统,模块),报表的多个授权位置，只要有一个位置授权即可使用
    ...
    Call ExportLog(False, "mdlPublic", "ReportCanQuery", "开始", "")
    ...
    Else
        ...
        If Err.Number = 0 Then
            ...
            Call ExportLog(True, "mdlPublic", "ReportCanQuery", "Break-1", "", strPriv)
            Exit Function
        End If
    End If
    ...
    Call ExportLog(True, "mdlPublic", "ReportCanQuery", "完成", "")
    Exit Function
    
errH:
    Call ExportLog(True, "mdlPublic", "ReportCanQuery", "Err=" & Err.Description, "")
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
    ReportCanQuery = 3
End Function
...
Public Function WndMessage(ByVal hwnd As OLE_HANDLE, ByVal Msg As OLE_HANDLE, ByVal wp As OLE_HANDLE, ByVal lp As Long) As Long
    ...
    lngOldAddr = GetProcAddr(hwnd)
    If lngOldAddr <> 0 And Msg <> WM_CONTEXTMENU Then
        WndMessage = CallWindowProc(lngOldAddr, hwnd, Msg, wp, lp)
    End If
End Function
...
'      (取DevMode也反映不出来，可能要用GetJob才能获取最近的打印文档属性)
    ...
    If OpenPrinter(strPrtName, lngHandle, 0&) Then
        ...
        '检查打印机的表单是否生效
'        If gbytExportMode = Val("2-直接打印") Or gbytExportMode = Val("4-输出PDF") Then
'            '暂无...
'        Else
        If vDevMode.dmPaperSize = 256 Then
            lngSize = DocumentProperties(lngHWND, lngHandle, strPrtName, arrDevMode(1), 0&, DM_OUT_BUFFER)
            Call CopyMemory(vDevMode, arrDevMode(1), Len(vDevMode))
            If Abs(vDevMode.dmPaperLength - sngHeight * 10) > 10 Then
                If gbytExportMode = Val("2-直接打印") Or gbytExportMode = Val("4-输出PDF") Then
                    '...
                Else
                    MsgBox mdlPublic.FormatString("打印机（[1]）不支持该自定义纸张的尺寸，强制转成A4纸张。", strPrtName) _
                        , vbInformation, "注意"
                End If
                
                '自定义纸张失败，强制恢复A4
                On Error Resume Next
                vDevMode.dmOrientation = Val("1-竖向")
                vDevMode.dmPaperSize = Val("9-A4")
                vDevMode.dmPaperWidth = 2100
                vDevMode.dmPaperLength = 2960
                Call CopyMemory(arrDevMode(1), vDevMode, Len(vDevMode))
                If blnPrompt Then
                    Call DocumentProperties(lngHWND, lngHandle, strPrtName, arrDevMode(1), arrDevMode(1), DM_IN_BUFFER Or DM_IN_PROMPT Or DM_OUT_BUFFER)
                Else
                    Call DocumentProperties(lngHWND, lngHandle, strPrtName, arrDevMode(1), arrDevMode(1), DM_IN_BUFFER Or DM_OUT_BUFFER)
                End If
                Call ResetDC(lngPrtDC, arrDevMode(1))
                Call ClosePrinter(lngHandle)
                On Error GoTo 0
            End If
        End If
...
'      (取DevMode也反映不出来，可能要用GetJob才能获取最近的打印文档属性)
    ...
    Dim vDevMode As DEVMODE, vDevModeCache As DEVMODE
    ...
    If OpenPrinter(strPrtName, lngHandle, 0&) Then
        ...
        vDevMode.dmOrientation = intOrient
        vDevMode.dmCopies = intCopys
        vDevMode.dmFields = DM_FORMNAME Or DM_ORIENTATION Or DM_COPIES Or DM_PAPERSIZE
        vDevModeCache = vDevMode                        '缓存（兼容处理）
        vDevMode.dmPaperWidth = sngWidth * 10           '单位 = 十分之一毫米
        vDevMode.dmPaperLength = sngHeight * 10         '单位 = 十分之一毫米
        vDevMode.dmPaperSize = 256
        
        'Set the dmFields bit flag to indicate what you are changing.
        vDevMode.dmFields = vDevMode.dmFields Or DM_PAPERLENGTH Or DM_PAPERWIDTH
        ...
        If vDevMode.dmPaperSize = 256 Then
            ...
            If Abs(vDevMode.dmPaperLength - sngHeight * 10) > 10 Then
                If gbytExportMode = Val("2-直接打印") Or gbytExportMode = Val("4-输出PDF") Then
                    '...
                Else
                    MsgBox mdlPublic.FormatString("打印机（[1]）不支持该“表单”的尺寸，强制转成A4纸张。", strPrtName) _
                        , vbInformation, "注意"
                End If
                    
                '自定义纸张失败，强制恢复A4
                On Error Resume Next
                vDevModeCache.dmOrientation = Val("1-竖向")
                vDevModeCache.dmPaperSize = Val("9-A4")
                vDevModeCache.dmPaperWidth = 2100
                vDevModeCache.dmPaperLength = 2960
                Call CopyMemory(arrDevMode(1), vDevModeCache, Len(vDevModeCache))
                Call DocumentProperties(lngHWND, lngHandle, strPrtName, arrDevMode(1), arrDevMode(1), DM_IN_BUFFER Or DM_OUT_BUFFER)
                Call ResetDC(lngPrtDC, arrDevMode(1))
                Call ClosePrinter(lngHandle)
                On Error GoTo 0
    ...
    If rsPar Is Nothing And Not gcnOracle Is Nothing Then '静态记录集,只读取一次
        If gcnOracle.State = adStateOpen Then
            ...
            If Not rsPar.EOF Then
                ...
                rsPar.Filter = "参数号=33"
                If Not rsPar.EOF Then gblnAppInfo = Nvl(rsPar!参数值, 0) = 1
            End If
        End If
    End If
    If Not gcnOracle Is Nothing Then
...
Public Sub CallAppInfo(ByVal intType As Integer, ByVal strInfo1 As String, Optional ByVal strInfo2 As String)
    '调用dbms_application_info包
    Dim strSQL As String
    
    On Error GoTo errH
    If gblnAppInfo = False Then Exit Sub
    
    Select Case intType
        Case 1
            strSQL = "dbms_application_info.Set_Client_Info('" & strInfo1 & "')"
        Case 2
            strSQL = "dbms_application_info.Set_Module('" & strInfo1 & "', '" & strInfo2 & "')"
        Case Else
            strSQL = "dbms_application_info.Set_Action('" & strInfo1 & "')"
    End Select
  
    ExecuteProcedure strSQL, "调用dbms_application_info包"
    Exit Sub
errH:
    If ErrCenter() = 1 Then
        Resume
...
Public Sub SaveClientInfo(ByVal strInfo As String)
    '向V$sesssion的client_info字段写入客户端信息
    '参数说明: strInfo  -用户信息, 建议格式 : 部门,人员姓名,客户端
    Call CallAppInfo(1, strInfo)
End Sub

Public Sub SaveSessAction(ByVal strAction As String)
    '向会话信息中写入操作信息,以便执行SQL语句后,在v$sql的Action字段中可查
    '参数说明:  strAction -操作信息
    Call CallAppInfo(3, strAction)
End Sub

Public Sub SaveSessModule(ByVal strModul As String, ByVal strAction As String)
    '向会话信息中写入模块信息,以便执行SQL语句后,在v$sql的Module字段中可查
    '参数说明: strModul -模块信息, strAction -操作信息
    Call CallAppInfo(2, strModul, strAction)
End Sub
...
Public Sub GetChartDataName(ByVal str内容 As String, Optional strFX As String, _
    Optional strFS As String, Optional strFY As String, Optional strData As String, _
    Optional strFY2 As String, Optional strFS2 As String)
'功能：根据Chart数据内容获取相关字段的名称
    Dim arrData As Variant
    
    strFX = "": strFS = "": strFY = "": strData = "": strFY2 = "": strFS2 = ""
    If str内容 <> "" Then
        arrData = Split(str内容, "|")
        ...
        If UBound(arrData) > 2 Then
            If InStr(arrData(3), ".") > 0 Then
                If strData = "" Then
                    strData = Split(arrData(3), ".")(0)
                End If
                strFS2 = Split(arrData(3), ".")(1)
            End If
            If InStr(arrData(4), ".") > 0 Then
                If strData = "" Then
                    strData = Split(arrData(4), ".")(0)
                End If
                strFY2 = Split(arrData(4), ".")(1)
            End If
        End If
    End If
...
Public Function SetChartDataArray(objChart As Object, rsData As ADODB.Recordset, _
    ...
    Optional arrLabelX As Variant, Optional arrLabelS As Variant, Optional strFY2 As String, _
    Optional arrLabelS2 As Variant, Optional strFS2 As String) As Boolean
'功能：设置图表数据,按照公共X轴方式
'参数：strFX=X字段,strFS=序列字段,strFY=Y字段,strFY2=Y2字段
'返回：arrLabelX=包含X轴标签的数组
'      arrLabelS=包含序列标签的数组
    Dim colFS As New Dictionary
    Dim colFS2 As New Dictionary
    Dim colFX As New Dictionary
    Dim colFY As New Dictionary
    Dim colFY2 As New Dictionary
    Dim arrS As Variant, arrS2 As Variant, arrX As Variant, arrY As Variant, arrY2 As Variant
    Dim blnByDate As Boolean, strX As String, strS As String, strS2 As String
    Dim i As Long, j As Long
    ...
    For i = 1 To rsData.RecordCount
        ...
        End If
        If strFS2 <> "" Then
            strS2 = Nvl(rsData.Fields(strFS2).Value)
            If Not IsNull(rsData.Fields(strFS2).Value) Then '不管NULL序列
                '产生序列集合
                If Not colFS2.Exists("_" & strS2) Then
                    colFS2.Add "_" & strS2, strS2
                End If
                '产生序列对应X轴点Y2值集合
                If Not colFY2.Exists("_" & strX & "_" & strS2) Then
                    colFY2.Add "_" & strX & "_" & strS2, Val(Nvl(rsData.Fields(strFY2).Value, 0))
                Else
                    '同一个序列在同一个点有多个值,则累加Y值
                    colFY2("_" & strX & "_" & strS2) = _
                        colFY2("_" & strX & "_" & strS2) + Val(Nvl(rsData.Fields(strFY2).Value, 0))
                End If
            End If
        End If
        '产生X轴点集合
    ...
    End With
    If strFY2 <> "" Then
        With objChart.ChartGroups(2).Data
            .Layout = oc2dDataArray
            .NumSeries = colFS2.count '统计序列数
            .NumPoints(1) = colFX.count '每个序列公共点数
            ...
            '产生序列对应X轴点Y2值
            arrS2 = colFS2.Items
            ReDim arrY2(UBound(arrX), UBound(arrS2))
            For i = 0 To UBound(arrS2)
                For j = 0 To UBound(arrX)
                    If blnByDate Then
                        strX = format(arrX(j), "yyyy-MM-dd HH:mm:ss")
                    Else
                        strX = arrX(j)
                    End If
                    If colFY2.Exists("_" & strX & "_" & arrS2(i)) Then
                        arrY2(j, i) = colFY2("_" & strX & "_" & arrS2(i))
                    Else
                        arrY2(j, i) = .HoleValue '该序列不存在的X点
                    End If
                Next
            Next
            Call .CopyYArrayIn(arrY2)
        End With
    End If
...
Public Function SetChartDataGeneral(objChart As Object, rsData As ADODB.Recordset, _
    ByVal strFX As String, ByVal strFS As String, ByVal strFY As String, Optional arrLabelS As Variant, _
    Optional strFY2 As String, Optional arrLabelS2 As Variant, Optional ByVal strFS2 As String) As Boolean
'功能：设置图表数据,按照公共X轴方式
'参数：strFX=X字段,strFS=序列字段,strFY=Y字段,strFY2=Y2字段
'返回：arrLabelX=包含X轴标签的数组
'      arrLabelS=包含序列标签的数组
    Dim colFS As New Dictionary, colFS2 As New Dictionary
    Dim arrS As Variant, arrS2 As Variant, arrX As Variant, arrY As Variant, arrY2 As Variant
    Dim i As Long, j As Long
    ...
    For i = 1 To rsData.RecordCount
        ...
        End If
        If Not IsNull(rsData.Fields(strFS2).Value) Then '不管NULL序列
            If Not colFS2.Exists("_" & rsData.Fields(strFS2).Value) Then
                colFS2.Add "_" & rsData.Fields(strFS2).Value, rsData.Fields(strFS2).Value
            End If
        End If
        rsData.MoveNext
    Next
    ...
    If strFY2 <> "" Then
        With objChart.ChartGroups(2).Data
            ...
            arrS2 = colFS2.Items
            For i = 0 To UBound(arrS2)
                rsData.Filter = strFS2 & "='" & arrS2(i) & "'"
                .NumPoints(i + 1) = rsData.RecordCount '当前序列点数
                
                '产生当前序列对应的Y2值
                ReDim arrY2(rsData.RecordCount - 1)
                For j = 1 To rsData.RecordCount
                    If Not IsNull(rsData.Fields(strFY2).Value) Then
                        arrY2(j - 1) = rsData.Fields(strFY2).Value
                    Else
                        arrY2(j - 1) = .HoleValue
                    End If
                    rsData.MoveNext
                Next
                Call .CopyYVectorIn(i + 1, arrY2)
            Next
        End With
    End If
...
'      sngScale=显示比例,blnDesign=是否在报表设计环境中使用
    ...
    Dim arrLabelX As Variant, arrLabelS As Variant, varTemp As Variant, arrLabelS2 As Variant
    Dim blnByDate As Boolean, i As Long, j As Long
    Dim str表头 As String, strColor As String
        
    '图表标题
    If objItem.表头 <> "" Then
        If InStr(objItem.表头, ";序列颜色：") > 0 Then
            str表头 = Mid(objItem.表头, 1, InStr(objItem.表头, ";序列颜色：") - 1)
            strColor = Mid(Replace(objItem.表头, str表头, ""), 7)
        Else
            str表头 = objItem.表头
        End If
    End If
    ...
    '数据内容
    Call GetChartDataName(objItem.内容, strFX, strFS, strFY, , strFY2, strFS2)
    objChart.ChartArea.Axes("X").Title.Text = strFX '字段名作为XY轴标题
    objChart.ChartArea.Axes("Y").Title.Text = strFY
    If strFY2 <> "" Then
        objChart.ChartGroups(2).ChartType = IIF(objItem.序号 = 0, 1, objItem.序号)
        objChart.ChartArea.Axes("Y2").Title.Text = strFY2
    End If
    '绑定数据
    arrLabelX = Array(): arrLabelS = Array(): arrLabelS2 = Array()
    If Not rsData Is Nothing Then
        ...
        If objItem.序号 = 0 Then
            '按DataGeneral方式设置数据
            Call SetChartDataGeneral(objChart, rsData, strFX, strFS, strFY, arrLabelS, strFY2, arrLabelS2, strFS2)
        Else
            '按DataArray方式设置数据
            Call SetChartDataArray(objChart, rsData, strFX, strFS, strFY, arrLabelX, arrLabelS, strFY2, arrLabelS2, strFS2)
        End If
    ...
    Else
        ...
        End If
        If strFY2 <> "" Then
            objChart.ChartGroups(2).Data.NumSeries = 4
            objChart.ChartGroups(2).Data.NumPoints(1) = 5
        End If
    End If
...
'    objChart.ChartGroups(1).PointLabels.RemoveAll
    ...
    Else
        ...
        '序列图例2
        If strFS2 <> "" Then
            If UBound(arrLabelS2) <> -1 Then
                For i = 0 To UBound(arrLabelS2)
                    objChart.ChartGroups(2).SeriesLabels.Add arrLabelS2(i)
                Next
            Else
                For i = 1 To objChart.ChartGroups(2).Styles.count
                    If strFS2 <> "" Then
                        objChart.ChartGroups(2).SeriesLabels.Add strFS2 & i
                    Else
                        objChart.ChartGroups(2).SeriesLabels.Add "序列" & i
                    End If
                Next
            End If
        End If
    ...
    For i = 1 To objChart.ChartGroups(1).Styles.count
        ...
        If i < UBound(varTemp) + 2 Then
            objChart.ChartGroups(1).Styles(i).Fill.Interior.ForegroundColor = varTemp(i - 1)
        End If
    Next
    For i = 1 To objChart.ChartGroups(2).Styles.count
        If objItem.自调 Then
            '依优先级循环显示结点类型
            objChart.ChartGroups(2).Styles(i).Symbol.Shape = arrTmp(((i - 1) Mod UBound(arrTmp)) + 1)
            objChart.ChartGroups(2).Styles(i).Symbol.Size = 7 * sngScale
        Else
            objChart.ChartGroups(2).Styles(i).Symbol.Shape = oc2dShapeNone
        End If
        objChart.ChartGroups(2).Styles(i).Line.Pattern = IIF(objItem.下线, 2, 1) 'oc2dLineSolid/oc2dLineNone
        objChart.ChartGroups(2).Styles(i).Line.Width = 1 * sngScale
        If i + objChart.ChartGroups(1).Data.NumSeries < UBound(varTemp) + 2 Then
            objChart.ChartGroups(2).Styles(i).Fill.Interior.ForegroundColor = varTemp(objChart.ChartGroups(1).Data.NumSeries + i - 1)
        End If
    Next
    If objItem.汇总 <> "" Then
        varTemp = Split(objItem.汇总, "|")
        For i = 1 To objChart.ChartLabels.count
            objChart.ChartLabels(i).IsShowing = IIF(varTemp(0) = 1, True, False)
            If varTemp(0) = 1 Then
                objChart.ChartLabels(i).Border.Type = IIF(varTemp(1) = -1, 0, varTemp(1))
                objChart.ChartLabels(i).IsConnected = IIF(varTemp(2) = 1, True, False)
                objChart.ChartLabels(i).offset = Val(varTemp(3))
            End If
        Next
    End If
    '线条粗细
    If objItem.排序 <> "" Then
        For i = 1 To objChart.ChartGroups(1).Data.NumSeries
            objChart.ChartGroups(1).Styles.Item(i).Line.Width = Val(objItem.排序)
        Next
        For i = 1 To objChart.ChartGroups(2).Data.NumSeries
            objChart.ChartGroups(2).Styles.Item(i).Line.Width = Val(objItem.排序)
        Next
    End If
...
'功能：按拷贝图表对象,按比例缩放,并获取相应的图表图片
    Dim strFX As String, strFY As String, strFS As String, strFS2 As String, strFY2 As String
    ...
    objDesc.ChartArea.Axes("Y2").Title.Text = objSource.ChartArea.Axes("Y2").Title.Text
...
'    Kill strFile
    ...
    If strFX <> "" And strFS <> "" And strFY <> "" And Not rsData Is Nothing Then
        ...
        If objItem.序号 = 0 Then
            Call SetChartDataGeneral(objDesc, rsData, strFX, strFS, strFY, , strFY2, , strFS2)
        Else
            Call SetChartDataArray(objDesc, rsData, strFX, strFS, strFY, , , strFY2, , strFS2)
        End If
    Else
        '使用数组拷贝时,如果系列较多,会比较慢
    ...
    '标签
    Call AddLable(objDesc)
    For i = 1 To objDesc.ChartLabels.count
        objDesc.ChartLabels(i).IsShowing = objSource.ChartLabels(i).IsShowing
        If objSource.ChartLabels(i).IsShowing = True Then
            objDesc.ChartLabels(i).Border.Type = objSource.ChartLabels(i).Border.Type
            objDesc.ChartLabels(i).IsConnected = objSource.ChartLabels(i).IsConnected
            objDesc.ChartLabels(i).offset = objSource.ChartLabels(i).offset
        End If
    Next
    ...
    Next
    For i = 1 To objSource.ChartGroups(2).SeriesLabels.count
        objDesc.ChartGroups(2).SeriesLabels.Add objSource.ChartGroups(2).SeriesLabels(i).Text
    Next
    ...
    For i = 1 To objDesc.ChartGroups(1).Styles.count
        ...
        objDesc.ChartGroups(1).Styles(i).Fill.Interior.ForegroundColor = objSource.ChartGroups(1).Styles(i).Fill.Interior.ForegroundColor
    Next
    For i = 1 To objDesc.ChartGroups(2).Styles.count
        objDesc.ChartGroups(2).Styles(i).Symbol.Shape = objSource.ChartGroups(2).Styles(i).Symbol.Shape
        objDesc.ChartGroups(2).Styles(i).Symbol.Size = objSource.ChartGroups(2).Styles(i).Symbol.Size * sngScale
        objDesc.ChartGroups(2).Styles(i).Line.Pattern = objSource.ChartGroups(2).Styles(i).Line.Pattern
        objDesc.ChartGroups(2).Styles(i).Line.Width = objSource.ChartGroups(2).Styles(i).Line.Width * sngScale
        objDesc.ChartGroups(2).Styles(i).Fill.Interior.ForegroundColor = objSource.ChartGroups(2).Styles(i).Fill.Interior.ForegroundColor
    Next
...
'      未安装且注册失败返回False
    ...
    If Not blnInstall Then
        ...
        If Err.Number <> 0 Then
            ...
            If Err.Number <> 0 Then
                MsgBoxEx "图表控件未正确注册，重新安装HIS客户端可以解决这个问题。", vbExclamation, App.Title
                Exit Function
            End If
        Else
            Unload frmFlash
...
'  Index：数据连接编号
    ...
    Dim strKey As String, strPass As String, strServer As String, strTmp As String
        ...
        With grsConnect
            ...
            Do While .EOF = False
                If Nvl(!编号, 0) = Index Then
                    strPass = Nvl(!密码)
                    '解密
                    strTmp = mdlPublic.Sm4DecryptEcb(strPass)
                    If strTmp = "" Then
                        strPass = mdlPublic.Decipher(MSTR_DBLINK_KEY, strPass)
                    Else
                        strPass = strTmp
                    End If
                    strTmp = GetConString(!数据库类型 & "", intDeviceType & "", !驱动版本 & "")
                    Set cn = GetDBConnectionSub(!数据库类型 & "", !用户名, strPass, !端口 & "", !实例名, !IP, !驱动版本 & "", intDeviceType & "", strTmp)
                ...
                With grsConnect
                    ...
                    Do While .EOF = False
                        If Nvl(!编号, 0) = Index Then
                            ...
                            strTmp = !连接字符串 & ""
                            If strTmp = "" Then
                                strTmp = GetConString(!数据库类型 & "", !提供者类型 & "", !驱动版本 & "")
                            End If
                            Set cn = GetDBConnectionSub(!数据库类型 & "", !用户名, strPass, !端口 & "", !实例名, !IP, !驱动版本 & "", !提供者类型 & "", strTmp)
...
Private Function GetDBConnectionSub(ByVal strType As String, ByVal strUser As String, ByVal strPass As String, _
    ByVal strImpot As String, ByVal strDateBase As String, strIP As String, ByVal strVersion As String, _
    ByVal strProvider As String, ByVal strString As String) As ADODB.Connection
'功能：连接数据库
'参数：lng编号-数据库连接编号，strType-传入的连接类型，用于连接测试
    Dim rsTemp As ADODB.Recordset, cnTemp As New ADODB.Connection
    Dim strServer As String, strError As String
    Dim strTemp As String, lngType As Long, lngProvider As Long
            
    cnTemp.CursorLocation = adUseClient     '必须，涉及RecortCount取值和ActionConnection=Nothing的常规使用
    lngType = Val(strType)
    lngProvider = Val(strProvider)
    Select Case lngType
        Case 0 'Oracle
            On Error GoTo errH
            strServer = Nvl(strIP) & IIF(Nvl(strUser) = "", ":1521", ":" & Nvl(strImpot)) & _
                        IIF(Nvl(strDateBase) = "", "", "/" & Nvl(strDateBase))
            Set cnTemp = gobjRegister.GetConnection(strServer, strUser, strPass, False, IIF(lngProvider = 0, 0, 1))
        Case 1 'SQLService
            On Error Resume Next
            strServer = Replace(strString, "[IP,端口]", strIP & "," & strImpot)
            strServer = Replace(strServer, "[实例名]", strDateBase)
            strServer = Replace(strServer, "[用户名]", strUser)
            strServer = Replace(strServer, "[密码]", strPass)
            strServer = Replace(strServer, "[IP]", strIP)
            strServer = Replace(strServer, "[端口]", strImpot)
            cnTemp.Open strServer
            If Err.Number <> 0 Then
                strError = UCase(Err.Description)
                If InStr(strError, "不存在或访问被拒绝") Then
                    MsgBox "连接出错，SQLServer服务未启动！", vbInformation, App.Title
                ElseIf InStr(strError, "无法打开登录所请求的数据库") > 0 Then
                    MsgBox "连接出错，" & strDateBase & "实例名不存在！", vbInformation, App.Title
                ElseIf strError Like "*用户*登录失败*" Then
                    MsgBox "连接出错，用户名或密码无效！", vbInformation, App.Title
                ElseIf Err.Description = "未指定的错误" Then
                    MsgBox "连接出错，可能是以下原因：" & vbNewLine & _
                            "1.SQLServer服务未启动；" & vbNewLine & _
                            "3.实例名不存在；" & vbNewLine & _
                            "2.用户名或密码无效；", vbInformation, App.Title
                Else
        ...
        Case 2 'MySQL
            ...
            strServer = Replace(strServer, "[IP]", strIP)
            strServer = Replace(strServer, "[端口]", strImpot)
            strServer = Replace(strServer, "[驱动版本]", strVersion)
            cnTemp.Open strServer
            If Err.Number <> 0 Then
                strError = UCase(Err.Description)
                If InStr(strError, "未发现数据源名称并且未指定默认驱动程序") > 0 Then
                    MsgBox "连接出错，未安装MySQL对应版本驱动！", vbInformation, App.Title
                ElseIf InStr(strError, "CAN'T CONNECT TO MYSQL SERVER") Then
                    MsgBox "连接出错，MySQL服务未启动！", vbInformation, App.Title
                ElseIf InStr(strError, "UNKNOWN DATABASE") > 0 Then
                    MsgBox "连接出错，" & strDateBase & "实例名不存在！", vbInformation, App.Title
                ElseIf InStr(strError, "ACCESS DENIED FOR USER") > 0 Then
                    MsgBox "连接出错，用户名或密码无效！", vbInformation, App.Title
                Else
                    MsgBox Err.Description, vbInformation, App.Title
                End If
        ...
        Case 3 'PostgreSQL
            ...
            If Err.Number <> 0 Then
                strError = UCase(Err.Description)
                If InStr(strError, "未发现数据源名称并且未指定默认驱动程序") > 0 Then
                    MsgBox "连接出错，未安装PostgreSQL对应版本驱动！", vbInformation, App.Title
                ElseIf InStr(strError, "COULD NOT CONNECT TO SERVER") Then
                    MsgBox "连接出错，PostgreSQL服务未启动！", vbInformation, App.Title
                ElseIf InStr(strError, "DOES NOT EXIST") > 0 Then
                    MsgBox "连接出错，" & strDateBase & "实例名不存在！", vbInformation, App.Title
                ElseIf InStr(strError, "PASSWORD AUTHENTICATION FAILED FOR USER") > 0 Then
                    MsgBox "连接出错，用户名或密码无效！", vbInformation, App.Title
                ElseIf InStr(strError, "TIMEOUT EXPIRED") > 0 Then
                    MsgBox "连接超时，请检查后重试！", vbInformation, App.Title
                Else
                    MsgBox Err.Description, vbInformation, App.Title
                End If
                Err.Clear
            End If
    End Select
    Set GetDBConnectionSub = cnTemp
    Exit Function
errH:
    MsgBox Err.Description, vbInformation, App.Title
End Function

Private Function GetConString(ByVal strType As String, ByVal strProvider As String, ByVal strVersion As String) As String
'获取连接的字符串
'strType=数据库类型;strProvider=提供者类型;strVersion=驱动版本
    Dim lngType As Long, lngProvider As Long, strConnString As String
    
    lngType = Val(strType)
    lngProvider = Val(strProvider) '0-ODBC;1-OLEDB
    Select Case lngType
        Case 0 'Oracle
            If lngProvider = 0 Then
                strConnString = "Driver={Microsoft ODBC for Oracle};UID=[用户名];PWD=[密码];" & _
                "SERVER=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=[IP])(PORT=[端口]))(CONNECT_DATA=(SERVICE_NAME =[实例名]):Persist Security Info=True"
            Else
                strConnString = "Provider=OraOLEDB;UID=[用户名];PWD=[密码];" & _
                "Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=[IP])(PORT=[端口]))(CONNECT_DATA=(SERVICE_NAME =[实例名]):Persist Security Info=True"
            End If
        Case 1 'SQLService
            If lngProvider = 0 Then
                strConnString = "Driver={SQL Server};Server=[IP];Address=[IP,端口];Uid=[用户名];Pwd=[密码];Database=[实例名]"
            Else
                strConnString = "Provider=SQLOLEDB;Data Source=[IP,端口];Network Library=DBMSSOCN;Initial Catalog=[实例名];User Id=[用户名];Password=[密码];"
            End If
        Case 2 'MySQL；不支持OLEDB连接
            strConnString = "Driver={MySQL ODBC [驱动版本] Unicode Driver};Server=[IP];Database=[实例名];User=[用户名]; Password=[密码];Option=3;"
        Case 3 'PostgreSQL；暂不支持OLEDB连接
            If lngProvider = 0 Then
                strConnString = "Driver={PostgreSQL Unicode};Server=[IP];Port=[端口];Database=[实例名];Uid=[用户名];Pwd=[密码];"
            Else
                'strConnString = "Provider=PostgreSQL OLE DB Provider;Data Source=[IP];location=[实例名];User ID=[用户名];password=[密码];timeout=1000;"
            End If
    End Select
    GetConString = strConnString
End Function
...
'调用方式为：Set rsPati=OpenSQLRecord(strSQL, Me.Caption, CDate(Format(rsMove!转出日期,"yyyy-MM-dd")),dtp时间.Value, lng病人ID, "张%", "男", 20, 21)
    ...
    Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "开始", "")
    ...
    If intQueryMode = Val("1-LOB") Then
        '判断LOB字段类型的查询
        If UCase(gcnOracle.ConnectionString) Like UCase("*OraOLEDB*") Then
            'OraOLEDB驱动的连接，直接通过Open方式
            If rsTmp Is Nothing Then Set rsTmp = New ADODB.Recordset
            rsTmp.Open cmdData, , adOpenStatic, adLockOptimistic
            Set OpenSQLRecord = rsTmp
        Else
            GoTo makLOB
        End If
    Else
...
makLOB:
        ...
        If gcnOLEDB Is Nothing Then
            Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "Extended Properties=" & gcnOracle.Properties("Extended Properties").Value, "")
            Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "ConnectString=" & gcnOracle.ConnectionString, "")
            strMsg = ""
            Set gcnOLEDB = gobjRegister.ReGetConnection(Val("1-OracleOLEDB"), strMsg, gcnOracle)
            If gcnOLEDB Is Nothing Then
                Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "gcnOLEDB-1=nothing", "")
            Else
                Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "gcnOLEDB-1=" & IIF(gcnOLEDB.State = adStateOpen, "Open", "Close"), "")
            End If
            Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "Msg" & strMsg, "")
            '缓存连接对象
            If Not gcnOLEDB Is Nothing Then
                Call CacheOLEDBConnect(gcolOLEDBConnect, gcnOLEDB, gobjRegister)
            End If
        Else
            Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "gcnOLEDB-2=" & IIF(gcnOLEDB.State = adStateOpen, "Open", "Close"), "")
        End If
        If Not gcnOLEDB Is Nothing Then
    ...
    Call ExportLog(False, "mdlPublic", "OpenSQLRecord", "SQL", strSQL)
    Call ExportLog(True, "mdlPublic", "OpenSQLRecord", "完成", "")
...
'        Loop
        ...
        Call CommandExecuteStoredProc(cmdData)
    Else
        GoTo NoneVarLine
    End If
    Exit Sub
...
Public Function GetRegisterName() As String
'    If UCase(App.EXEName) = "ZL9REPORTALONE" Then
'        GetRegisterName = "zlRegisterAlone"
'    ElseIf UCase(App.EXEName) = "ZL9REPORTHEC" Then
'        GetRegisterName = "zlRegisterHEC"
'    Else
        GetRegisterName = "zlRegister"
'    End If
    'LogWrite "授权部件（" & GetRegisterName & "）"
End Function
...
'功能：初始化注册部件对象gobjRegister
    ...
    If gobjRegister Is Nothing Then
        On Error Resume Next
        Set gobjRegister = CreateObject(GetRegisterName & ".clsRegister")
        If Not gobjRegister Is Nothing Then
            If gobjRegister.GetServerName = "" Then
...
'返回：分号间隔的功能串,为空表示没有权限
    GetPrivFunc = gobjRegister.zlRegFunc(lngSys, lngProgID, gcnOracle)
End Function
...
Public Function zlRegInfo(strItem As String, Optional blnTemp As Boolean, Optional intBits As Integer) As String
    zlRegInfo = gobjRegister.zlRegInfo(strItem, blnTemp, intBits, gcnOracle)
End Function
...
Public Function zlRegTool(Optional blnTemp As Boolean) As Long
    zlRegTool = gobjRegister.zlRegTool(blnTemp, gcnOracle)
End Function
...
'功能：加载数据连接信息至控件
    ...
    '数据获取
'    strSQL = "Select Nvl(数据库类型, 0) 数据库类型, Nvl(提供者类型,0) 提供者类型, 驱动版本, 编号, 名称, 用户名, 密码, Ip, 端口, 实例名, 连接字符串" & vbNewLine & _
'            "From Zlconnections" & vbNewLine & _
'            "Order By 编号"
    strSQL = "编号,名称,用户名,密码,Ip,端口,实例名,数据库类型,提供者类型,驱动版本,连接字符串"
    If mdlPublic.gobjCompatibility.BuildSelectString("zlConnections", strSQL, "", strSQL, True) = False Then
        Err.Raise 60001, , "SetControlDBConnect发生异常！"
    End If
    strSQL = strSQL & vbCr & "Order By 编号 "
...
Public Function Decipher(ByVal vPassword As String, ByVal vFrom_Text As String) As String
    ...
    Decipher = Replace(Decipher, Chr(0), "")
End Function
...
'  bytType：0-指定返回数据连接的名称；1-用户名
    ...
    strSQL = "Select 名称, 用户名 From Zlconnections Where 编号 = [1] "
...
Public Sub CacheOLEDBConnect(ByRef colCache As Collection, ByVal cnSource As ADODB.Connection _
    , ByVal objRegister As Object)
'功能：缓存OLEDBConnect对象
'参数：
'  colCache：缓存的集合对象
'  cnSource：缓存的连接对象
'  objRegister：注册部件对象
    ...
    If colCache Is Nothing Then
        Set colCache = New Collection
    End If
    
    Call objRegister.GetConnectionInfo(cnSource, strServer, strUser, strPass)
    If strServer <> "" And strUser <> "" Then
        Call colCache.Add(cnSource, strServer & "_" & strUser)
    End If
End Sub
...
'  objRegister：注册部件对象
    ...
    Dim strServer As String, strUser As String, strPass As String
    Dim blnReCreate As Boolean, strErr As String, strIPOld As String, strIPNew As String
    ...
    If colCache Is Nothing Then
        blnReCreate = True
    Else
        If colCache.count <= 0 Then blnReCreate = True
    End If

    '通过Register部件和cnSource连接对象，获取该连接对象的数据库服务名、用户名、密码
    Call objRegister.GetConnectionInfo(cnSource, strServer, strUser, strPass)
    
    If blnReCreate = False Then
        '获取连接对象中的数据库服务名、用户名信息
        If strServer <> "" And strUser <> "" Then
            On Error Resume Next
            Set GetOLEDBConnect = colCache(strServer & "_" & strUser)
            On Error GoTo 0
        End If
    End If
    
    If GetOLEDBConnect Is Nothing Then
        If gobjLogin Is Nothing Then
            '以ZLHIS导航台登录部件ZLLogin获取数据库服务名、用户名、密码
            On Error Resume Next
            Call ExportLog(False, "mdlPublic", "GetOLEDBConnect", "CreateObject-ZLLogin", "")
            Set gobjLogin = CreateObject("ZLLogin.clslogin")
            If Err.Number = 0 Then
                blnReCreate = gobjLogin.InputUser = ""
            Else
                Err.Clear
                Call ExportLog(False, "mdlPublic", "GetOLEDBConnect", "CreateObject-ZLLogin-Failed", "")
                blnReCreate = True
            End If
            
'            If blnReCreate Then
'                LogWrite "Create Object ZLLoginAlone"
'                '以ZLLIS导航台登录
'                Set gobjLogin = CreateObject("ZLLoginAlone.clslogin")
'                If Not gobjLogin Is Nothing Then
'                    If gobjLogin.InputUser = "" Then Exit Function
'                End If
'            End If
            
            '独立安装的LIS，通过LIS接口部件传入的连接是配置中存储的连接库和用户，跟登录导航台的连接不是同一个
            '返回空连接后，通过ReGetconnetion来获取新连接
            If strUser = gobjLogin.InputUser And strUser <> "" Then
                '当以“IP:端口/实例”方式打开连接对象时，从连接对象cnSource获取的服务名可能是这种格式：
                '(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.0.60)(PORT=1522))(CONNECT_DATA=(SERVICE_NAME=qzyy)))
                '由于普通用户没有权限访问v$database视图，所以暂时只以IP来区分，同一台服务器有多个不同名称的数据库时，无法区分出来
                strIPOld = ""
                If strServer <> gobjLogin.ServerName Then strIPOld = GetServerIP(cnSource)
                               
                Set GetOLEDBConnect = objRegister.GetConnection(gobjLogin.ServerName, gobjLogin.InputUser _
                    , gobjLogin.InputPwd, CBool(Val("1-要密码转换")), Val("1-OraOLEDB"), strErr _
                    , CBool(Val("0-不保存登录信息")))
                    
                If strIPOld <> "" Then
                    strIPNew = GetServerIP(GetOLEDBConnect)
                    Call ExportLog(False, "mdlPublic", "GetOLEDBConnect", "传入的连接对象的服务器IP为：" & strIPOld & ",导航台登录的服务器IP为：" & strIPNew, "")
                    
                    If strIPOld <> strIPNew Then
                        Set GetOLEDBConnect = Nothing
                        Call ExportLog(True, "mdlPublic", "GetOLEDBConnect", "Break-1", "")
                        Exit Function
                    End If
                End If
                
                If strErr <> "" Then
                    Call ExportLog(False, "mdlPublic", "GetOLEDBConnect", "GetConnection-Error=" & strErr, "")
                End If
                
                Call CacheOLEDBConnect(gcolOLEDBConnect, GetOLEDBConnect, gobjRegister)
                Call ExportLog(True, "mdlPublic", "GetOLEDBConnect", "GetOLEDBConnect=" & IIF(GetOLEDBConnect.State = adStateOpen, "Open", "Close"), "")
            End If
        End If
    End If
End Function

Private Function GetServerIP(cnThis As ADODB.Connection) As String
'功能：获取服务器IP地址
    ...
    On Error Resume Next
    '这个包是给了Public权限的
    strSQL = "Select Utl_Inaddr.Get_Host_Address as IP From Dual"
    rsTmp.CursorLocation = adUseClient
    rsTmp.Open strSQL, cnThis
    
    If rsTmp.State = 1 Then
        If rsTmp.RecordCount > 0 Then
            GetServerIP = rsTmp!IP
        End If
    End If
End Function

Private Function GetLogConfig(ByRef lngLogNo As Long, ByRef strLogName As String) As Boolean
'--------------------------------------------------------------------------------
'功能：获取日志配置信息
'参数：
'  lngLogNo：日志号
...
'--------------------------------------------------------------------------------
    ...
    strSQL = "Select 系统, 名称 From zlLogConfig Where Nvl(系统, 0) = 0 And 编号 = [1] And RowNum < 2"
    Set rsTemp = mdlPublic.OpenSQLRecord(strSQL, "获取报表工具的日志配置信息", 1)
    If rsTemp.EOF = False Then
        lngLogNo = mdlPublic.Nvl(rsTemp!系统, 0)
        strLogName = mdlPublic.Nvl(rsTemp!名称)
    End If
    rsTemp.Close
    Exit Function
...
Private Sub InitLogOld(ByRef clsSource As clsLog, ByRef cnOracle As ADODB.Connection _
...
'--------------------------------------------------------------------------------
    ...
    Dim lngLogNo As Long
    Dim strLogName As String, strRegPath As String, strFilePath As String
    Dim intDay As Integer
    Dim blnOn As Boolean

    If cnOracle Is Nothing Then Exit Sub
    If cnOracle.State <> adStateOpen Then Exit Sub

    If clsSource Is Nothing Then Set clsSource = New clsLog

    If clsSource.Path = "" Or blnReset = True Then
        '初始化

        '1.日志设置信息
        If clsSource.FileName = "" Or blnReset = True Then
            Call GetLogConfig(lngLogNo, strLogName)
            If strLogName = "" Then
                '缺省赋值，避免数据库无连接时日志无法输出的情况
                lngLogNo = 0
                strLogName = "报表工具跟踪日志"
            End If
        End If

        strRegPath = "公共全局\日志\" & strLogName
        blnOn = Val(GetSetting("ZLSOFT", strRegPath, "启用")) = 1
        intDay = Val(GetSetting("ZLSOFT", strRegPath, "保存天数"))
        strFilePath = FormatString("[1]log\日志跟踪\[2]_[3]" _
                        , Left(App.Path, InStrRev(UCase(App.Path), "\")) _
                        , lngLogNo _
                        , strLogName)
        ...
        With clsSource
            .FileName = strLogName
            .SaveDays = IIF(intDay < 3, 3, intDay)
            .Path = strFilePath
            .Action = blnOn
        End With
    End If
...
Private Sub InitLogNew(ByVal cnOracle As ADODB.Connection)
'功能：通用日志的初始化
    If gclsLog Is Nothing Then Exit Sub
    Call gclsLog.SetBusinessDB(cnOracle)
End Sub

Public Sub InitLog(ByVal cnOracle As ADODB.Connection)
'功能：日志初始化的兼容方法
    Dim strSQL As String
    Dim rsTemp As ADODB.Recordset
    ...
    '查询zlLogCategory表（只一次）
    If gblnHaveQueryCategory = False Then
        On Error GoTo hErr
        strSQL = "Select Count(1) Rec From All_Tables Where Owner = [1] And Table_Name = [2] "
        Set rsTemp = mdlPublic.OpenSQLRecord(strSQL, "获取zlLogCategory表是否存在", "ZLTOOLS", "ZLLOGCATEGORY")
        If rsTemp.EOF = False Then
            gblnGeneralLog = rsTemp!Rec >= 1
        Else
            gblnGeneralLog = False
        End If
        rsTemp.Close
        gblnHaveQueryCategory = True
        On Error GoTo 0
    End If
    
    If gblnGeneralLog Then
        '通用日志
        If gclsLog Is Nothing Then
            On Error Resume Next
            Set gclsLog = CreateObject("zlLog.clsLog")
            On Error GoTo 0
        End If
        Call InitLogNew(cnOracle)
    Else
        '旧日志
        If gclsLog Is Nothing Then
            Set gclsLog = New clsLog
        End If
        Call InitLogOld(gclsLog, cnOracle)
    End If
...
Private Sub LogWriteNew(ByVal blnCallEnd As Boolean, ByVal strModule As String, ByVal strProcName As String _
    , ByVal strInfo As String, ByVal strSQL As String, ParamArray arrProcParas() As Variant)
'功能：通用日志输出
    
    Const STR_CATEGORY As String = "报表工具跟踪日志"
    Dim arrPars() As Variant
    ...
    If strSQL = "" Then
        Call gclsLog.LogCall(STR_CATEGORY, App.ProductName, strModule, strProcName, strInfo _
...
Private Sub LogWriteOld(ByVal strLog As String, Optional ByVal blnSection As Boolean = False)
'--------------------------------------------------------------------------------
'功能：立即输出日志至文件
'参数：
'  strLog：日志内容
'  blnSection：True以段头形式输出日志；False常规形式输出日志
'--------------------------------------------------------------------------------

    If gclsLog Is Nothing Then Exit Sub

    If blnSection Then
        gclsLog.AddSave vbNewLine _
            & mdlPublic.Pad("[" & format(Now, "YYYY-MM-DD hh:mm:ss") & "]", 30, " ") _
            & mdlPublic.Pad(gstrDBUser, 20, " ") _
            & mdlPublic.Pad(App.Title, 20, " ") _
            & mdlPublic.Pad(strLog, 20, " ")
    Else
        gclsLog.AddSave format(Now, "hh:mm:ss, ") & strLog
    End If
End Sub

Public Sub ExportLog(ByVal blnCallEnd As Boolean, ByVal strModule As String, ByVal strProcName As String _
    , ByVal strInfo As String, ByVal strSQL As String, ParamArray arrProcParas() As Variant)
'功能：日志输出的兼容方法
    ...
    If gblnGeneralLog Then
        '通用日志
        Call LogWriteNew(blnCallEnd, strModule, strProcName, strInfo, strSQL, arrProcParas)
    Else
        '旧日志
        blnSection = False
        If UBound(arrProcParas) >= 0 Then
            If UCase(TypeName(arrProcParas(0))) = "STRING" Then
                blnSection = UCase(arrProcParas(0)) = UCase("<Section>")
            End If
        End If
        If strSQL = "" Then
            Call LogWriteOld(mdlPublic.FormatString("[1].[2]: [3]", strModule, strProcName, strInfo), blnSection)
        Else
            Call LogWriteOld(mdlPublic.FormatString("[1].[2]: [3]=[4]" _
                                            , strModule, strProcName, IIF(strInfo = "", "SQL", strInfo), strSQL) _
                , blnSection)
        End If
    End If
End Sub

Private Function HaveAdditionTable(objRPT As Report, objRPTItem As RPTItem) As Boolean
'--------------------------------------------------------------------------------
'功能：检查RPTItem有无附加表格
'参数：
'  objRPTItem：参考对象
...
Public Function LastAdditionTable(objRPT As Report, objRPTItem As RPTItem) As Boolean
'--------------------------------------------------------------------------------
'功能：是否为最后一个（坐标最底）表格
'参数：
'  objRPT：当前报表对象
...
'--------------------------------------------------------------------------------
    ...
    Dim tmpItem As RPTItem, LastItem As RPTItem
    Dim blnMax As Boolean
        
    If Not (objRPTItem.类型 = Val("4-自由表格") Or objRPTItem.类型 = Val("5-汇总表格")) Then
        Exit Function
    End If
    
    blnMax = True
    For Each tmpItem In objRPT.Items
        If objRPTItem.格式号 = tmpItem.格式号 And tmpItem.类型 = Val("4-自由表格") Or tmpItem.类型 = Val("5-汇总表格") Then
            '最大Y坐标
            If tmpItem.Y > objRPTItem.Y Then
                blnMax = False
                Exit For
            End If
        End If
    ...
    LastAdditionTable = blnMax
End Function

Public Function MainTable(objRPT As Report, objAddition As RPTItem) As RPTItem
'--------------------------------------------------------------------------------
'功能：获取主表格对象
'参数：
'  objRPT：当前报表对象
...
'--------------------------------------------------------------------------------
    ...
    If objAddition.类型 <> Val("4-自由表格") Or objAddition.参照 = "" Then Exit Function
    
    For Each tmpItem In objRPT.Items
        If objAddition.格式号 = tmpItem.格式号 And objAddition.参照 = tmpItem.名称 Then
            Set MainTable = tmpItem
            Exit For
        End If
    Next
End Function
...
'功能：对字符串进行加密（按电码本方式进行解密）
'strInput：明文
'strKey：加密密钥，不传时，使用缺省的密钥
'返回值：密文

    Dim arrKey() As Byte, arrInput() As Byte, arrOutPut() As Byte
    ...
    Else
        If strKey <> "" Then
            arrKey = HexToArr(strKey, 16)
        Else
            arrKey = GetRndKey()
        End If
        arrInput = Sm4InputEnc(strInput, lngLength)
        ReDim arrOutPut(UBound(arrInput))

        Call sm4_crypt_ecb(CM_Encrypt, lngLength, arrKey(0), arrInput(0), arrOutPut(0))
        Sm4EncryptEcb = "ZLSV1:" & DecToHex(arrOutPut())
    End If
End Function

Public Function Sm4DecryptEcb(ByVal strInput As String, Optional ByVal strKey As String) As String
'功能：对字符串进行解密（按电码本方式进行解密）
'strInput：密文
'strKey:解密密钥，也就是产生该密文的加密密钥，不传时，使用缺省的密钥
'返回值：明文
    
    Dim arrKey() As Byte, arrInput() As Byte, arrOutPut() As Byte
    Dim lngLength As Long
    '判断当前密文是否是由其他加密算法加密的
    If strInput Like "ZLSV#:*" Or strInput Like "ZLSV##:*" Then
        strInput = Split(strInput, ":")(1)
        If strInput = "" Then
            Sm4DecryptEcb = ""
        Else
            If strKey <> "" Then
            ...
            Else
                arrKey = GetRndKey()
            End If
            lngLength = Len(strInput) / 2
            arrInput = HexToArr(strInput, lngLength)
            ReDim arrOutPut(UBound(arrInput))
            
            Call sm4_crypt_ecb(CM_Decrypt, lngLength, arrKey(0), arrInput(0), arrOutPut(0))
            Sm4DecryptEcb = Trim(StrConv(arrOutPut(), vbUnicode))
        End If
    End If
    Sm4DecryptEcb = Replace(Sm4DecryptEcb, Chr(0), "")
End Function

Private Function Sm4InputEnc(ByVal strInput As String, ByRef lngLength As Long) As Byte()
'对加密时的入参进行初始化处理
    Dim arrReturn() As Byte
    Dim lngLengthBefore As Long
    Dim i As Long

    '先将字符串由 Unicode 转成系统的缺省码页
    arrReturn = StrConv(strInput, vbFromUnicode)
    lngLengthBefore = UBound(arrReturn) + 1
    '判断得到的数组的长度，若不是16的整数倍，则补空格，其对应编码为32
    If lngLengthBefore Mod 16 <> 0 Then
        lngLength = -Int(-lngLengthBefore / 16) * 16
        ReDim Preserve arrReturn(lngLength - 1)

        For i = lngLengthBefore To lngLength - 1
            arrReturn(i) = 32
        Next
    End If
    Sm4InputEnc = arrReturn()
End Function

Private Function HexToArr(ByVal strInput As String, ByVal lngLength As Long) As Byte()
'将16进制字符串转换为数组
    Dim arrReturn() As Byte
    Dim i As Long

    ReDim arrReturn(lngLength - 1)
    For i = 0 To lngLength - 1
        arrReturn(i) = "&H" & Mid(strInput, 2 * i + 1, 2)
    Next
    HexToArr = arrReturn()
End Function

Private Function DecToHex(arrDec() As Byte) As String
'将十进制数组转换为一条十六进制的字符串
    Dim lngLength As Long
    Dim i As Long
...
Private Function GetRndKey() As Byte()
'获取密钥
    Dim arrReturn() As Byte
    Dim i As Long
...
'功能：获取Oracle数据库连接的SessionID
    ...
    Set rsTemp = mdlPublic.OpenSQLRecord(strSQL, "获取SessionID")
    If rsTemp.EOF = False Then
...
Public Function MsgBoxEx(ByVal Prompt As String, Optional ByVal Buttons As VbMsgBoxStyle _
    , Optional ByVal Title As String) As VbMsgBoxResult

    If gblnSilentMode Then
        Call ExportLog(False, "mdlPublic", "MsgBoxEx", Prompt, "")
    Else
        MsgBoxEx = MsgBox(Prompt, Buttons, Title)
    End If
End Function

Public Function IsReportEmpty(ByVal objRPT As Report, ByVal objDatas As LibDatas _
    , ByVal bytFormatNo As Byte) As Boolean
'功能：检查“空表打印”，通过报表元素的数据源判断
'返回：False表示非空表；True表示空表
'注意：“空表打印”的“空表”是指报表的“表格”元素，并且只作用于业务模块调用，管理工具调用不做控制

    Dim objItem As RPTItem
    Dim strItem As String, strSource As String, strTmp As String
    Dim arrSources() As String
    Dim blnNoExist As Boolean
    Dim i As Integer
    
    strSource = ""
    For Each objItem In objRPT.Items
        If objItem.格式号 = bytFormatNo Then
            Select Case objItem.类型
                Case Val("4-任意表格")
                    strItem = GetGridSource(objRPT, objItem, True)
    ...
    If strSource <> "" Then
        ...
        arrSources = Split(strSource, ",")
        For i = LBound(arrSources) To UBound(arrSources)
            blnNoExist = True
            If Not UCase(TypeName(arrSources(i))) = "ERROR" Then
                On Error Resume Next
                blnNoExist = objDatas("_" & arrSources(i)).DataSet.RecordCount = 0
                On Error GoTo 0
                If blnNoExist = False Then
                    '存在数据源且有记录
                    IsReportEmpty = False
                    Exit Function
                End If
...
Public Function GetGridSource(objRPT As Report, objItem As RPTItem, Optional ByVal blnHead As Boolean = False) As String
'功能：从任意表格中搜索出所用到的数据源名
'参数：objItem=任意表格主项
'      blnHead=是否从表头标签中检查
'返回："病人信息,药品信息,...",""
    ...
    For Each tmpID In objItem.SubIDs
        strFormula = objRPT.Items("_" & tmpID.id).内容
        ...
        If blnHead Then
            strFormula = objRPT.Items("_" & tmpID.id).表头
            Do While InStr(strFormula, "[") > 0
                strSource = Trim(Mid(strFormula, InStr(strFormula, "[") + 1, InStr(strFormula, "]") - InStr(strFormula, "[") - 1))
                strFormula = Mid(strFormula, InStr(strFormula, "]") + 1)
                If InStr(strSource, ".") > 0 Then
...
Public Function GetReportObjects(ByVal lngID As Long, Optional ByVal AllConnect As Boolean = False) As String
'功能：获取指定报表的数据库访问对象
'参数：
'  lngID：报表ID
...
Public Function GetNewProgID() As Long
...
Public Function ReplaceHeadTailBlack(ByVal strSource As String, Optional ByVal strNew As String = "") As String
'功能：替换字符串头尾的空格符
    ...
    If InStr(strSource, " ") <= 0 Then
        ReplaceHeadTailBlack = strSource
        Exit Function
    End If
    ...
    For i = 1 To Len(strSource)
        If Mid(strSource, i, 1) = Space(1) Then
            strResult = strResult + strNew
        Else
            strResult = strResult + Mid(strSource, i)
            Exit For
        End If
    Next
    ...
    ReplaceHeadTailBlack = strResult
End Function

Public Function GetProcAddr(ByVal lngHandle As Long) As Long
    GetProcAddr = 0
    On Error Resume Next
    GetProcAddr = Val(Mid(gcolProcAddr("_" & lngHandle), 2))
    On Error GoTo 0
End Function

Public Function SetPubHook(ByVal lngWndProc As Long, ByVal lngControlHandle As Long, ByVal lngProcAddr As Long) As Boolean
'功能：公共消息钩子（加载）
    ...
    '先获取缓存
    lngOldAddr = GetProcAddr(lngControlHandle)
    If lngOldAddr = 0 Then
        lngOldAddr = GetWindowLong(lngControlHandle, lngWndProc)
        If lngOldAddr <> 0 Then
            Call SetWindowLong(lngControlHandle, lngWndProc, lngProcAddr)
            gcolProcAddr.Add _
                  "_" & lngOldAddr & "_" & lngControlHandle _
                , "_" & lngControlHandle
        Else
            SetPubHook = False
        End If
...
Public Function SetPubUnHook(ByVal lngWndProc As Long, ByVal lngControlHandle As Long) As Boolean
'功能：公共消息钩子（卸载）

    Dim lngOldAddr As Long, l As Long
    Dim strVals() As String

    lngOldAddr = GetProcAddr(lngControlHandle)
    If lngOldAddr <> 0 Then
        Call SetWindowLong(lngControlHandle, lngWndProc, lngOldAddr)
        On Error Resume Next
        gcolProcAddr.Remove "_" & lngControlHandle
'    ElseIf Not gcolProcAddr Is Nothing Then
'        '卸载集合中所有的(通常情况下gcolProcAddr只有一个项目)
'        For l = gcolProcAddr.count To 1 Step -1
'            strVals = Split(Mid$(CStr(gcolProcAddr(l)), 2), "_")
'            lngOldAddr = Val(strVals(0))
'            lngControlHandle = Val(strVals(1))
'            Call SetWindowLong(lngControlHandle, lngWndProc, lngOldAddr)
'            gcolProcAddr.Remove l
'        Next
    End If
End Function

Public Sub AdjustSQL(ByRef strSQLIn As String)
...
Private Sub AddLable(objDesc As Object)
    Dim i As Long, j As Long, M As Long, N As Long
    
    '默认情况下是没有标签的，但是标签要添加上
    objDesc.ChartLabels.RemoveAll
    With objDesc.ChartGroups(1).Data
        For i = 1 To .NumSeries
            For j = 1 To .DataCount(i)
                objDesc.ChartLabels.Add
                M = (i - 1) * .DataCount(i) + j
                objDesc.ChartLabels(M).Text = .Y(i, j)
                objDesc.ChartLabels(M).AttachMethod = oc2dAttachDataIndex ' oc2dAttachDataIndex
                objDesc.ChartLabels(M).AttachDataIndex.Series = i
                objDesc.ChartLabels(M).AttachDataIndex.Point = j
                objDesc.ChartLabels(M + N).Anchor = oc2dAnchorNorth
            Next
        Next
    End With
    N = M
    With objDesc.ChartGroups(2).Data
        For i = 1 To .NumSeries
            For j = 1 To .DataCount(i)
                objDesc.ChartLabels.Add
                M = (i - 1) * .DataCount(i) + j
                objDesc.ChartLabels(M + N).Text = .Y(i, j)
                objDesc.ChartLabels(M + N).AttachMethod = oc2dAttachDataIndex ' oc2dAttachDataIndexDataY
                objDesc.ChartLabels(M + N).AttachDataIndex.ChartGroup = 2
                objDesc.ChartLabels(M + N).AttachDataIndex.Series = i
                objDesc.ChartLabels(M + N).AttachDataIndex.Point = j
                objDesc.ChartLabels(M + N).Anchor = oc2dAnchorNorth
            Next
        Next
    End With
    For i = 1 To objDesc.ChartLabels.count
        objDesc.ChartLabels(i).IsShowing = False
    Next
End Sub

Public Sub GetOSFont(ByVal strFontName As String, ByRef objDev As Object)
'功能：获取系统或设备的字体名称

    Static blnWarning As Boolean

    On Error GoTo hErr
    objDev.FontName = strFontName
    Exit Sub
    
hErr:
    If blnWarning = False Then
        blnWarning = True
        If UCase(TypeName(objDev)) = "PRINTER" Then
            MsgBoxEx "打印机缺少（" & strFontName & "）字体，自定义报表部件将缺省一个字体！", vbInformation, App.Title
        Else
            MsgBoxEx "操作系统缺少（" & strFontName & "）字体，自定义报表部件将缺省一个字体！", vbInformation, App.Title
        End If
    End If
End Sub

Private Sub SetControlArray(ByVal objSource As Object, ByRef arrTarget As Variant)
    Dim i As Long
    
    With objSource
        ReDim arrTarget(.UBound)
        For i = .LBound To .UBound
            Set arrTarget(i) = objSource(i)
        Next
    End With
End Sub

Public Function GetValues(ByVal objForm As Object, ByVal objPars As RPTPars) As Collection
'功能：获取现有的界面上的参数值
    Dim i As Integer, j As Integer
    Dim strParName As String, strTmp As String, strDisp As String
    Dim colValue As New Collection
    Dim arrLabel() As Label
    Dim arrCombo() As ComboBox
    Dim arrOption() As OptionButton
    Dim arrCheck() As CheckBox
    Dim arrText() As TextBox
    Dim arrDTPicker() As DTPicker
    
    '统一数组变量（收集控件数组）
    If UCase(objForm.name) = UCase("frmReport") Then
        Call SetControlArray(objForm.lblName, arrLabel)
    Else
        Call SetControlArray(objForm.lbl, arrLabel)
    End If
    Call SetControlArray(objForm.cbo, arrCombo)
    Call SetControlArray(objForm.opt, arrOption)
    Call SetControlArray(objForm.chk, arrCheck)
    Call SetControlArray(objForm.txt, arrText)
    Call SetControlArray(objForm.dtp, arrDTPicker)
    
    For i = 1 To UBound(arrLabel)
        strParName = arrLabel(i).ToolTipText
        
        If objPars("_" & strParName).缺省值 = "固定值列表…" Then
            Select Case objPars("_" & strParName).格式
                Case 0
                    If GetCboIndex(arrCombo(i), arrCombo(i).Text) = -1 Then '是否人为输入
                        'Reserve字段保存本次条件的"宏条件值|显示值"
                        colValue.Add arrCombo(i).Text, "_" & strParName
                    Else
                        '列表选择
                        'Reserve字段保存本次条件的"宏条件值|显示值"
                        '不好的分隔符
                        ...
                        For j = 0 To UBound(Split(strTmp, "|"))
                            ...
                            If strDisp = arrCombo(i).Text Then
                    ...
                    End If
                Case 1
                    For j = 1 To UBound(arrOption)
                        If arrOption(j).Container.Index = i Then
                            If arrOption(j).Value Then
                                colValue.Add arrOption(j).Tag, "_" & strParName
                            End If
                        End If
                    Next
                Case 2
        ...
        ElseIf objPars("_" & strParName).缺省值 = "选择器定义…" Then
            If arrText(i).Tag = "" Then '是否人为输入
                'Reserve字段保存本次条件的"宏条件值|显示值"
                colValue.Add arrText(i).Text, "_" & strParName
            Else
                '列表选择
                'Reserve字段保存本次条件的"宏条件值|显示值"
                colValue.Add arrText(i).Tag, "_" & strParName
            End If
        Else
            Select Case objPars("_" & strParName).类型
                Case 0, 1, 3
                    colValue.Add arrText(i).Text, "_" & strParName
                Case 2
                    strTmp = arrDTPicker(i).CustomFormat
                    If strTmp Like "* *:*:*" Then
                        colValue.Add format(arrDTPicker(i).Value, "YYYY-MM-DD hh:mm:ss"), "_" & strParName
                    Else
                        colValue.Add format(arrDTPicker(i).Value, "YYYY-MM-DD"), "_" & strParName
                    End If
            End Select
        End If
    Next
