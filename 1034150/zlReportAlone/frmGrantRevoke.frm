...
Private mblnColsResize As Boolean
...
Private Sub RefreshSelected()
    ...
    Else
        strSQL = _
            "Select nvl(a.组别, '缺省') 组别, c.序号 Pid, nvl(a.标题, c.标题) 菜单, b.名称 系统, a.Id Menuid" & vbCr & _
            "  , e.名称 报表, e.Id ReportID, c.序号 程序id, c.系统 SysNo " & vbCr & _
            ...
            "Where a.系统(+) = c.系统 And a.模块(+) = c.序号 " & vbCr & _
            "  And c.系统 = b.编号 " & vbCr & _
...
Private Sub RefreshMenuGroup()
    ...
    Else
        strSQL = _
            ...
            "  From zlSystems A " & vbCr & _
            "  Where Exists(Select 1 From zlMenus B Where b.组别 = [1]) " & vbCr & _
            "  Union All " & vbCr & _
            ...
            "    And Exists(Select 1 From zlSystems X Where x.编号 = b.系统) " & vbCr & _
...
Private Sub vsfSelected_AfterUserResize(ByVal Row As Long, ByVal Col As Long)
    mblnColsResize = False
End Sub

Private Sub vsfSelected_BeforeUserResize(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    mblnColsResize = Row <= 0 And Col >= 0 And Cancel = False
End Sub

Private Sub vsfSelected_DblClick()
    If mblnColsResize Then
        '避免双击列的边界（VSF自动调整合适列宽）误调用 cmdSingleClear_Click
        mblnColsResize = Not mblnColsResize
        Exit Sub
    End If
