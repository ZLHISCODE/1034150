...
Private Sub cmdVerify_Click()
    ...
    Dim blnLoginConnect As Boolean
    
    strSQL = RemoveNote(SyntaxEditSQL.Text)
    strSQL = TrimChar(strSQL)
    strSQL = Replace(strSQL, "[系统]", mlngSys)
    mblnCheck = True
    blnLoginConnect = cboConn.ListIndex = 0
    ...
    If blnLoginConnect Then
        '检查SQL的特殊要求
        If CheckSpecial(strSQL, strMsg) = False Then
...
Private Sub txtName_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        Call SetPubHook(GWL_WNDPROC, txtName.hwnd, AddressOf WndMessage)
    End If
End Sub

Private Sub txtName_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        Call SetPubUnHook(GWL_WNDPROC, txtName.hwnd)
    End If
End Sub
...
Private Function CheckSpecial(ByVal strSQL As String, ByRef strMsg As String) As Boolean
'功能：SQL的特殊检查
'返回：True-检查通过；False-检查不通过

    strMsg = ""
    CheckSpecial = False

    '首行不允许多行注释的检查
    If RTrim$(strSQL) Like "[/][*]*[*][/]*" Then
        strMsg = "不允许首行书写多行注释“/*...*/”，因为这样在“Microsoft ODBC”驱动连接时会引起查询错误！"
        Exit Function
    End If
