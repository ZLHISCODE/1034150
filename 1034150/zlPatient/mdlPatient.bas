...
'参数：strClass=工本费、就诊卡、病历费
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlPatient", strClass, lng收费细目ID)
...
'        glng预交ID = zldatabase.GetPara("共用预交票据批次", glngSys, lngModul, 0)
    ...
    ElseIf lngModul = P预交款管理 Then
        ...
        gblnAllowOut = zlDatabase.GetPara("允许出院病人缴预交", glngSys, lngModul) = "1"
