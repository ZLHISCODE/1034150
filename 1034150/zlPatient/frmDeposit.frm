...
Begin VB.Form frmDeposit 
   ...
   Begin VB.PictureBox picFace 
      ...
      Begin zlIDKind.IDKindNew IDKind 
         ...
         NotContainFastKey=   "F1;CTRL+F1;F12;CTRL+F12;F2;F3;F4;CTRL+F4;F11;F10;F8;ESC"
...
Private mblnStyleChange As Boolean
...
Private Sub cboPatiPage_Click()
    ...
    If Not mrsInfo Is Nothing Then
        If mrsInfo.State = 1 Then
            Call SetMoneyInfo(False, Val(mrsInfo!病人ID))
        End If
    End If
End Sub
...
''    '短|全名|刷卡标志|卡类别ID(消费卡序号)|长度|是否消费卡|结算方式|是否密文|是否自制卡;…
   If Not mcolPayMode Is Nothing Then
        '问题:56478
        If NeedName(cboStyle.Text) = NeedName(mstr个人帐户) Then
            mlngCardTypeID = 0
            mbln消费卡 = False
            If mstr结算方式 <> NeedName(mstr个人帐户) Then
                mblnStyleChange = True
            Else
                mblnStyleChange = False
            End If
            mstr结算方式 = NeedName(mstr个人帐户)
        Else
            mlngCardTypeID = Val(mcolPayMode(lngIndex)(3))
            mbln消费卡 = Val(mcolPayMode(lngIndex)(5)) = 1
            If mstr结算方式 <> Trim(mcolPayMode(lngIndex)(6)) Then
                mblnStyleChange = True
            Else
                mblnStyleChange = False
            End If
...
Private Sub cboStyle_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        If KeyAscii >= 32 Then
            lngIdx = zlControl.CboMatchIndex(cboStyle.hWnd, KeyAscii)
    ...
    If cboStyle.ItemData(cboStyle.ListIndex) = BalanceType.C5代收款 Then
         If mbytInState = 0 Then
             If InStr(mstrPrivs, ";代收款收取;") = 0 Then
                 ...
                 If zlControl.CboLocate(cboStyle, BalanceType.C1现金, True) Then Cancel = True
             End If
         Else
     ...
     ElseIf mbytInState = 0 Then
         If InStr(1, mstrPrivs, ";预交收款;") = 0 Then
             ...
             If zlControl.CboLocate(cboStyle, BalanceType.C5代收款, True) Then Cancel = True
         End If
     Else
...
Private Sub cboUnit_KeyPress(KeyAscii As Integer)
    ...
    lngIdx = MatchIndex(cboUnit.hWnd, KeyAscii)
...
'   退款
    ...
    If MsgBox("确实要将单据 " & cboNO.Text & " 作废吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
        ...
        If mcurBill.lng卡类别ID = 0 Then
            If Not is代收款(cboNO.Text) And gbln消费验证 Then
                If mbln住院退预交验证 Or cboType.ItemData(cboType.ListIndex) = 1 Then
                    If Not zlDatabase.PatiIdentify(Me, glngSys, Val(txtPatient.Tag), Val(StrToNum(txtMoney.Text))) Then Exit Sub
                End If
            End If
        End If
        '医保相关检查
...
Private Function CheckDataValied() As Boolean
  ...
  If mbytInState = 0 Then
    ...
    If cboType.ItemData(cboType.ListIndex) = 2 Then
        ...
        If Val(Nvl(mrsInfo!在院)) = 0 And gblnAllowOut = False Then
            strSQL = "Select 1 From 病案主页 Where 病人ID=[1] And Nvl(主页ID,0)=0 And Nvl(病人性质,0)=0"
  ...
  If mbytInState = 3 Then
        ...
        If gbln消费验证 Then
            ...
            End If
            If mbln住院退预交验证 = True Or cboType.ItemData(cboType.ListIndex) = 1 Then
                If Not zlDatabase.PatiIdentify(Me, glngSys, lng病人ID, Val(StrToNum(txtMoney.Text))) Then Exit Function
            End If
        End If
...
Private Function Select三方退款() As Boolean
    ...
    strSQL = "" & _
    "   Select A.卡类别ID,A.结算卡序号," & _
    "           A.卡号,A.交易流水号,A.交易说明," & _
    "           max(decode(sign(金额), 1, ID, decode(A.记录状态,1,0,2,0,ID))) as 预交ID," & _
    "           sum(nvl(金额,0))-sum(nvl(冲预交,0)) as 预交余额 " & _
    "   From 病人预交记录 A " & _
    "   Where   A.病人ID=[1] and A.预交类别=[2] And nvl(A.交易说明,' ') Like '%%'" & strWhere & _
    "   Group by A.卡类别ID,A.结算卡序号,A.卡号,A.交易流水号,A.交易说明"
        
    strSQL = "" & _
    "   Select RowNum as ID, A.预交ID,A.卡类别ID,A.结算卡序号 as 消费接口ID, " & _
    "          nvl(B.编码,C.编号) as 编码,nvl(B.名称,C.名称) as 名称," & _
    "          A.卡号,A.交易流水号,A.交易说明, Nvl(B.是否转帐及代扣,0) As 转帐," & _
    "          A.预交余额,Nvl(B.是否退款验卡,0) as 是否退款验卡 " & _
    "   From (" & strSQL & ") A,医疗卡类别 B,卡消费接口目录 C" & _
    "   Where   A.卡类别ID=B.ID(+) and A.结算卡序号=C.编号(+)  and nvl(A.预交余额,0)>0" & _
    "    Order by 编码"
     Set rsMoney = zlDatabase.ShowSQLSelect(Me, strSQL, 0, cboStyle.Text & "退款选择", False, "", "请选择需要退款的交易", False, False, False, vRect.Left, vRect.Top, cboStyle.Height, blnCancel, True, True, lng病人ID, int预交类型, _
        mlngCardTypeID)
    ...
    With rsMoney
        ...
        mcurBill.bln转账 = Val(Nvl(!转帐)) = 1
        mcurBill.bln退款验卡 = !是否退款验卡 = 1
...
Private Function CheckBrushCard() As Boolean
    ...
    '问题号:55666
    If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModul, Nothing, mlngCardTypeID, mbln消费卡, Nvl(mrsInfo!姓名), Nvl(mrsInfo!性别), Nvl(mrsInfo!年龄), dblMoney, mstrBrushCardNo, mstrbrPassWord, False, False, False, False) = False Then Exit Function
    ...
    '判断预交金是否超出刷卡的账户余额
    bln消费卡刷卡 = IIf(Trim(txtMoney.Text) = "", "0", Trim(txtMoney.Text)) <> "0" And mbln消费卡
    bln三方卡刷卡 = Not mbln消费卡 And dbl账户余额 <> 0 And IIf(Trim(txtMoney.Text) = "", "0", Trim(txtMoney.Text)) <> "0"
    If bln消费卡刷卡 Or bln三方卡刷卡 Then
        lblRepairMoney.Visible = CDbl(txtMoney.Text) > dbl账户余额
        If lblRepairMoney.Visible Then lblRepairMoney.Caption = "补交额:" & Format((CDbl(txtMoney.Text) - dbl账户余额), "###0.00;-###0.00;;"): txtMoney.Text = Format(dbl账户余额, "###0.00;-###0.00;;")
    End If
    CheckBrushCard = True
    Exit Function
errHandle:
...
Private Sub cmdOK_Click()
    ...
    Else
        ...
        '81693:李南春,2015/5/30,评价器
        If Not mobjPlugIn Is Nothing Then
            On Error Resume Next
            Call mobjPlugIn.PatiPrePayAfter(mrsInfo!病人ID, mbytPrepayType, lng预交ID)
            Err.Clear
        End If
    End If
    '问题号:55666
...
Private Sub GetFact(Optional blnFirst As Boolean = False, Optional blnRed As Boolean = False)
   ...
   '票据领用检查及初始
    ...
    Else
        '松散：取下一个号码
        If Not blnRed Then
            txtFact.Text = IncStr(UCase(zlDatabase.GetPara("当前预交票据号", glngSys, mlngFactModule, "")))
        Else
            mstrRedFact = IncStr(UCase(zlDatabase.GetPara("当前预交票据号", glngSys, mlngFactModule, "")))
        End If
    End If
End Sub
...
Private Sub InitFace(Optional blnSave As Boolean = False)
    ...
    Select Case mbytInState
        Case 0, 3 '收取预交款,余额退款
            ...
            End If
            '只有代收款收取权限
            If InStr(mstrPrivs, ";预交收款;") = 0 Then Call zlControl.CboLocate(cboStyle, 5, True)
            If mbytInState = 3 Then
    ...
    If lbl帐户余额.Visible Then
        Line2(14).Visible = True: Line2(11).X2 = 3050
    Else
...
Private Sub FindPati(ByVal objCard As Card, ByVal blnCard As Boolean, ByVal strInput As String)
    ...
    If Not GetPatient(objCard, strInput, blnCancel, blnCard) Then
        If blnCancel Then '取消输入
            Call SelAll(txtPatient): txtPatient.SetFocus: Exit Sub
        End If
    ...
    '46764
    cboUnit.ListIndex = FindCboIndex(cboUnit, IIf(Val(Nvl(mrsInfo!当前科室id)) = 0, Val(Nvl(mrsInfo!科室ID)), Val(Nvl(mrsInfo!当前科室id))))
    If cboUnit.ListIndex = -1 And cboUnit.ListCount > 0 Then cboUnit.ListIndex = 0
                
    '医保改动-在院病人转个人帐户
    If Not IsNull(mrsInfo!险类) And InStr(mstrPrivs, ";保险转帐;") > 0 And mstr个人帐户 <> "" Then
        If GetCboIndex(cboStyle, mstr个人帐户) = -1 Then
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, blnCancel As Boolean, Optional blnCard As Boolean = False) As Boolean
  ...
  '72828,冉俊明,2014-5-9,增加工作单位信息的显示，处理方式：增加A.工作单位字段
    ...
    "            Nvl(C.姓名, a.姓名) As 姓名, Nvl(C.性别, a.性别) As 性别,A.年龄,Nvl(A.门诊号,0) as 门诊号,Nvl(A.住院号,0) as 住院号,Nvl(C.出院病床,0) as 床号,A.家庭地址,A.卡验证码," & _
...
Once:
   ...
   '  结算卡序号_in 病人预交记录.结算卡序号%type:=NULL,
    ...
    If mbytInState = 3 Then
        ...
        End If
    Else
        If zlInterfacePrayMoney(lng预交ID, strNo) = False Then
            '删除无效的预交数据
            gcnOracle.RollbackTrans: Exit Function
...
Private Function ReadBill(strNo As String) As Integer
    ...
    If mbytCallObject <> 0 Or mbytInState = 3 Then
   ...
   gstrSQL = "" & _
    ...
    "               M.名称 as 卡类别名称, nvl(J.名称,Q.名称) as 消费卡名称,Nvl(M.是否退款验卡,0) as 是否退款验卡 " & _
    "   From " & IIf(mblnNOMoved, "H", "") & "病人预交记录 A, " & _
    "               病人卡结算对照 B,病人卡结算记录 C,卡消费接口目录 J,卡消费接口目录 Q,医疗卡类别 M " & _
    "   Where   A.记录性质=1 And A.No=[1]   " & strWhere & _
    "                And A.ID=B.预交ID(+) And b.卡结算ID=C.ID(+) and C.接口编号=Q.编号(+)" & _
    ...
    "           B.当前科室ID,B.当前床号,B.家庭地址,A.ID,A.记录状态,A.摘要,A.金额," & _
    ...
    "           A.卡类别ID,A.结算卡序号,A.卡号,A.交易流水号,A.交易说明,A.合作单位,A.卡类别名称, A.消费卡名称,A.是否退款验卡 " & _
    ...
    cboUnit.ListIndex = FindCboIndex(cboUnit, IIf(IsNull(rsTemp!当前科室id), 0, rsTemp!当前科室id))
    If cboUnit.ListIndex = -1 And cboUnit.ListCount > 0 Then cboUnit.ListIndex = 0
    cboType.ListIndex = -1
    lngPrepayType = Val(Nvl(rsTemp!预交类别))
    ...
    If mcurBill.lng卡类别ID <> 0 Then
        cboStyle.ListIndex = GetCboIndex(cboStyle, mcurBill.str名称)
    Else
        cboStyle.ListIndex = GetCboIndex(cboStyle, IIf(IsNull(rsTemp!结算方式), "", rsTemp!结算方式))
     End If
    If cboStyle.ListIndex = -1 Then
        If mcurBill.lng卡类别ID <> 0 Then
...
Private Sub ShowPremayBalance(ByVal blnreReadData As Boolean, ByVal lng病人ID As Long, _
    ...
    Dim dbl退现 As Double, dbl不能退现 As Double
    ...
    '73199:刘尔旋,2014-5-22,光标从支付方式离开后退预交金额变更的问题
    If mblnStyleChange = True Or blnreReadData = True Then
        If mbytInState = 3 Then
            If bln三方接口 Then
                If mdbl剩余款额 - mdbl预交余额_三方 >= 0 Then
                    txtMoney.Text = Format(mdbl预交余额_三方, "#,##0.00;-#,##0.00;;")
                Else
                    txtMoney.Text = ""
                End If
            Else
...
Private Sub SetMoneyInfo(blnClear As Boolean, Optional lng病人ID As Long, _
    ...
    Dim dblYB As Double, dbl为审 As Double, dbl未缴 As Double
    ...
    Else
        ...
        If Not mrsInfo Is Nothing Then
            If mrsInfo.State = 1 Then
                If lng主页ID = 0 Then
                    ...
                    Set rsMoney = zlDatabase.OpenSQLRecord(strSQL, "提取医保预结", Val(Nvl(mrsInfo!病人ID)))
                Else
                    strSQL = "Select Sum(金额) As 医保预结 From 保险模拟结算 Where 病人ID = [1] And 主页ID = [2]"
                    Set rsMoney = zlDatabase.OpenSQLRecord(strSQL, "提取医保预结", Val(Nvl(mrsInfo!病人ID)), lng主页ID)
                End If
        ...
        End If
        '显示预交余额
        dbl为审 = GetUnAuditedFee(lng病人ID, , mbytPrepayType)
        dbl未缴 = GetUnAuditedFee(lng病人ID, False, mbytPrepayType)
...
Private Sub ShowHistoryPrepay(ByVal strBackNo As String)
    ...
    On Error GoTo errHandle
    '84217,李南春,2015/6/15,显示指定的住院期间缴纳的预交
...
Private Sub InitIDKind()
    ...
    '86292:李南春,2015/7/10,医保调试
    If Not gobjSquare.objSquareCard Is Nothing Then gobjSquare.objSquareCard.mblnYLMgr = mbytCallObject = 2
End Sub
Private Function zlSequareBlanceToDeposit(ByVal lng预交ID As Long, ByVal lng接口编号 As Long, strBlanceInfor As String) As Boolean
    ...
    '    '          出参一定要传入正确,否则会出现程序错误
    '    '------------------------------------------------------------------------------------------------------------------------
     If gobjSquare.objSquareCard.zlBrushCardToDeposit(lng预交ID, lng接口编号, strBlanceInfor) = False Then
          Exit Function
     End If
goEnd:
    ...
    Exit Function
End Function
Private Function zlSequareBlanceToSimulation(lng接口编号 As Long, strBlanceInfor As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:结算卡的模拟结算
    '返回:成功,返回true,否则返回False
    '编制:刘兴洪
    ...
    '    '------------------------------------------------------------------------------------------------------------------------
     If gobjSquare.objSquareCard.zlSimulationBrushCard(Me, StrToNum(txtMoney.Text), lng接口编号, strBlanceInfor) = False Then
          zlSequareBlanceToSimulation = False
          Exit Function
     End If
goEnd:
    zlSequareBlanceToSimulation = True
    Exit Function
End Function
        ...
        '返回:退款合法,返回true,否则返回Flase
          If gobjSquare.objSquareCard.zlReturnCheck(Me, mlngModul, mcurBill.lng卡类别ID, mcurBill.bln消费卡, mcurBill.str卡号, _
            "1|" & lng预交ID, dbl退款金额, mcurBill.str交易流水号, mcurBill.str交易说明, strXMLExend) = False Then
              ...
              Exit Function
         End If
         '100610:李南春,2017/10/11，预交退款和余额退款是否验证刷卡
         If mcurBill.bln退款验卡 Then
            '   zlBrushCard(frmMain As Object, _
            ByVal lngModule As Long, _
            ByVal rsClassMoney As ADODB.Recordset, _
            ByVal lngCardTypeID As Long, _
            ...
            If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModul, Nothing, mcurBill.lng卡类别ID, False, _
                Trim(txtPatient.Text), mstrPatiSex, mstrPatiOld, dbl退款金额, mstrBrushCardNo, mstrbrPassWord, _
                True, False, False, False, , False, "<IN><CZLX>2</CZLX></IN>") = False Then Exit Function
            mcurBill.str卡号 = mstrBrushCardNo
        End If
     End If
...
Private Function zlDepositDel(ByRef lng预交ID As Long, ByRef lng冲预交ID As Long, ByVal dblMoney As Double) As Boolean
    ...
    '日期:2010-02-08 16:40:12
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, strSwapNO As String, strSwapMemo As String, strSwapExtendInfor As String
    ...
    End If
    If Save三方交易(lng冲预交ID, mcurBill.lng卡类别ID, mcurBill.bln消费卡, mcurBill.str卡号, _
         strSwapNO, strSwapMemo, strSwapExtendInfor, True, "1|" & lng冲预交ID) = False Then Exit Function
    zlDepositDel = True
    Exit Function
Errhand:
    If ErrCenter = 1 Then
...
Private Function Save三方交易(ByVal lng预交ID As Long, _
    ...
    If strExpend <> strExpendOld Then
        For i = 0 To UBound(varData)
...
Private Function zlInterfacePrayMoney(ByVal lng预交ID As Long, ByVal strNo As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:接口支付金额
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    If gobjSquare.objSquareCard.zlPaymentMoney(Me, mlngModul, mlngCardTypeID, mbln消费卡, mstrBrushCardNo, "", strNo, StrToNum(txtMoney.Text), strSwapGlideNO, strSwapMemo, strSwapExtendInfor) = False Then Exit Function
    If Save三方交易(lng预交ID, mlngCardTypeID, mbln消费卡, mstrBrushCardNo, strSwapGlideNO, strSwapMemo, strSwapExtendInfor) = False Then Exit Function
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    With cboPatiPage
        Do While Not rsTemp.EOF
            ...
            rsTemp.MoveNext
        Loop
        If bln留观 = True Then Call zlControl.CboSetWidth(cboPatiPage.hWnd, 2000)
    End With
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
