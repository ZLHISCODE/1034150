...
'公共事件
Public Event InsertPicture(ByRef pic As StdPicture)
...
'加载预览图像到界面
    ...
    Dim Inet1 As New cTabFtp
    Dim Inet2 As New cTabFtp
...
Private Sub DViewer_Click()
    ...
    If mlngCurIndex > 0 Then Set pic = Me.DViewer.Images(mlngCurIndex).Picture
...
Private Sub DViewer_DblClick()
    Dim pic As StdPicture
    If mlngCurIndex > 0 Then Set pic = Me.DViewer.Images(mlngCurIndex).Picture
    If Not pic Is Nothing Then RaiseEvent InsertPicture(pic)
End Sub
...
Private Sub mfrmShow_DblClick(pic As stdole.StdPicture)
    RaiseEvent InsertPicture(pic)
End Sub
