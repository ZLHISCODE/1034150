...
108             MsgBox "创建签名对象【JITComVCTK_S.dll】失败！请检查该控件是否安装并注册。", vbInformation, gstrSysName
                Exit Function
            End If
        End If
110     Err.Clear
...
118             MsgBox "创建签名对象【JITClientCOMAPI.dll】失败！请检查该控件是否安装并注册。", vbInformation, gstrSysName
                Exit Function
            End If
        End If
...
128         MsgBox "电子签名参数值设置有误,请检查。" & vbCrLf & _
...
140                 MsgBox "创建签章对象【BicengEsealInterface.dll】失败！请检查该控件是否安装并注册。", vbInformation, gstrSysName
                    Exit Function
                End If
            End If
        End If
...
150     NMG_InitObj = True
        Exit Function
errH:
152  MsgBox "创建接口部件失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
104     If blnCheck Then                 '验证当前USB是否是签名用户的，并获取签名证书
            '证书ID进行签名
106         strSignData = mobjJLClient.AttachSignStr("", strSource)      'DetachSignStr-不带原文签名;AttachSignStr-带原文签名
108         If Not GetErrorInfo("AttachSignStr") Then Exit Function
...
122             If strTimeStampCode = "" Then MsgBox "获取时间戳失败！", vbInformation, gstrSysName: Exit Function
                '日期格式化
124             strTimeStamp = Mid(strTimeStamp, 1, 14)
126             strTimeStamp = String14ToDate(strTimeStamp, strErr)
128             If strErr <> "" Then MsgBox strErr, vbInformation, gstrSysName: Exit Function
                '转东八区时间
130             strTimeStamp = Format(DateAdd("h", 8, strTimeStamp), "YYYY-MM-DD HH:MM:SS")
            Else
132             MsgBox "签名失败！", vbInformation, gstrSysName
                Exit Function
            End If

        Else
134         MsgBox "签名失败！", vbInformation, gstrSysName
            Exit Function
        End If
136     NMG_Sign = True
        Exit Function
errH:
138     MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
102 lngRet = mobjJLServer.VerifyAttachedSign(strSignData) '服务器验证数据 不带原文签名:VerifyDetachedSign(string, string);带原文签名  VerifyAttachedSign
104 If lngRet <> 0 Then
106     MsgBox "签名验证失败:" & mobjJLServer.GetErrorMessage(lngRet), vbInformation, gstrSysName
...
116 If strTS = "" Then
118       MsgBox "时间戳验证失败！", vbInformation, gstrSysName
120       Call mobjJLServer.FinalizeServerConnectEx   '断开时间戳服务器
          Exit Function
    End If
122 Call mobjJLServer.FinalizeServerConnectEx   '断开时间戳服务器
 
124 MsgBox "验证成功，该电子签名数据有效!", vbInformation, gstrSysName
    
126  NMG_VerifySign = True
     Exit Function
errH:
128     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
104         MsgBox "该证书未注册在您的名下，不能使用！" & vbCrLf & _
...
138     NMG_CheckCert = blnRet
        Exit Function
errH:
140      MsgBox "检查USBKEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
114             NMG_RegCert = True
            ...
            Exit Function
errH:
116      MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
114             MsgBox "您的证书还有" & lngDay & "天过期", vbInformation, gstrSysName
116             gblnShow = True
                Exit Function
118         ElseIf (lngDay <= 0) Then
120             MsgBox "您的证书已过期 " & Abs(lngDay) & " 天"
                Exit Function
            End If
        End If
...
134         If Trim(strPic) = "" Then MsgBox "获取签章失败!", vbInformation, gstrSysName: Exit Function
136         strPicPath = App.Path & "\" & Format(Now, "yyyyMMdd") & "_" & strUniqueID & ".bmp"
138         mobjCertInfo.getSealPicturePath strPic, strPicPath
140         If Dir(strPicPath) = "" Then MsgBox "获取签章失败!", vbInformation, gstrSysName: Exit Function
        End If
142     GetCertList = True
        Exit Function
errH:
144     MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
104         MsgBox "调用接口：【" & strName & "】后出错,错误描述:" & vbCrLf & mobjJLClient.GetErrorMessage(lngRet), vbInformation, gstrSysName
            Exit Function
        End If
106     GetErrorInfo = True
        Exit Function
errH:
108     MsgBox "获取错误描述！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
102     If lngRet <> 0 Then
104         MsgBox mobjJLServer.GetErrorMessage(lngRet), vbInformation, gstrSysName
            Exit Function
        End If
106     lngRet = mobjJLServer.SetServerUriEx("/signserver/service/xml")
108     If lngRet <> 0 Then
110         MsgBox mobjJLServer.GetErrorMessage(lngRet), vbInformation, gstrSysName
...
114     ConnectToTsaServer = True
        Exit Function
errH:
116     MsgBox "连接时间戳服务器！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
102     If lngRet <> 0 Then
104         MsgBox mobjJLServer.GetErrorMessage(lngRet), vbInformation, gstrSysName  '连接服务器失败
            Exit Function
        End If
106     lngRet = mobjJLServer.SetServerUriEx("/signserver/service/xml")
108     If lngRet <> 0 Then
...
118         MsgBox mobjJLServer.GetErrorMessage(lngRet), vbInformation, gstrSysName
...
122     ConnectToSignServer = True
        Exit Function
errH:
124     MsgBox "连接签名戳服务器！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
126         gudtPara.blnSignPic = False
        ...
        Exit Function
errH:
128     MsgBox "读取参数失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
