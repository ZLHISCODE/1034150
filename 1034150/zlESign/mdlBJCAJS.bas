...
122             BJCAJS_RegCert = True
            ...
            Exit Function
errH:
124      MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
...
114         MsgBox "您的身份证号为空,请联系管理员到人员管理中录入！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
...
126     If strPIN = "" Then
128        MsgBox "请输入证书密码！", vbOKOnly + vbInformation, gstrSysName
           Exit Function
        Else
130         If mintLogin >= 8 Then
132             MsgBox "已经输入了" & mintLogin & "次错误密码，超过了最大输入次数！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
134         blnRet = BJCAJS_Client.SOF_Login(strCertID, strPIN)
136         If Not blnRet Then
138             mintLogin = mintLogin + 1
140             MsgBox "证书密码可能不正确，您已经输入了" & mintLogin & "次密码，还可以输入" & 8 - mintLogin & "次!", vbOKOnly + vbInformation, gstrSysName
142             mstrLastPwd = ""
                Exit Function
            End If
         End If
...
188     mstrLastPwd = strPIN
        Exit Function
errH:
190      MsgBox "检查USBKEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
118                     MsgBox "生成时间戳失败！", vbOKOnly + vbInformation, gstrSysName
...
124                 strTimeStamp = String14ToDate(strDate)
                Else
126                 MsgBox "时间戳请求失败！", vbOKOnly + vbInformation, gstrSysName
...
130             BJCAJS_Sign = True
            Else
132             MsgBox "签名失败！", vbOKOnly + vbInformation, gstrSysName
            End If
        End If
    
        Exit Function
errH:
134      MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
134 If strTmp <> "" Then
136     MsgBox strTmp, vbOKOnly + vbInformation, gstrSysName
    End If
    
138 BJCAJS_VerifySign = blnOk
    Exit Function
errH:
140     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
110         MsgBox "服务端签名验证失败！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
        '验证证书是否过期
...
122             MsgBox "您的证书还有" & intDay & "天过期。", vbOKOnly + vbInformation, gstrSysName
124             gblnShow = True
126         ElseIf (intDay <= 0) Then
128             MsgBox "您的证书已过期 " & Abs(intDay) & " 天。", vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
        End If
...
136             MsgBox "不是所信任的根!", vbOKOnly + vbInformation, gstrSysName
                Exit Function
138         ElseIf lngRet = -2 Then
140             MsgBox "证书超过有效期！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
142         ElseIf lngRet = -3 Then
144             MsgBox "证书已经作废！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
146         ElseIf lngRet = -4 Then
148             MsgBox "证书被放入黑名单！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
150         ElseIf lngRet = -5 Then
152             MsgBox "证书未生效！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
        End If
...
162         MsgBox "客户端证书验失败！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
...
164     GetCertLogin = True
        Exit Function
errH:
166     MsgBox "登录验证失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
108         strName = ""
110         MsgBox "请插入证书Key！", vbOKOnly + vbInformation, gstrSysName
