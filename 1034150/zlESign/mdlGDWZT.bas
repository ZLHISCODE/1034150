...
130         varRet = mobjSign.SignWithTSATimeStamp(strTsaUrl, varRet, NETCAPKI_CMS_ENCODE_BASE64)
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "signedDataWithTSA" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:PKCS7签名验证并获取签名证书
    ...
    If objSign Is Nothing Then
        MsgBox "签名对象创建失败！", vbInformation + vbOKOnly, gstrSysName: Exit Function
    End If
    blnSignFormat = objSign.IsSign(varTemp)
    If Not blnSignFormat Then
        MsgBox "签名信息验签未通过:签名数据格式不正确!", vbInformation + vbOKOnly, gstrSysName: Exit Function
    End If
    ...
    '不带原文 mobjSign.Detached = true
        ...
        If Not blnRet Then
             MsgBox "签名信息验签未通过!", vbInformation + vbOKOnly, gstrSysName: Exit Function
        End If
    ...
    Else '带原文
        ...
        If Not blnRet Then
            MsgBox "签名信息验证未通过:原文与签名信息不一致!", vbInformation + vbOKOnly, gstrSysName: Exit Function
        End If
    End If
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "VerifySignedData" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能: 5.4.5 PKCS7时间戳签名验证并获取证书
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "VerifySignedDataWithTSA" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:  网关数字证书验证函数 网关验证证书方式一：服务接口【推荐使用】
    ...
    If strGWSeverCert = "" Then
        MsgBox "公网网关证书内容为空!", vbInformation + vbOKOnly, gstrSysName: Exit Function
    End If
    ...
    If colList Is Nothing Then
        MsgBox "服务端返回数据包有误或服务端返回数据为空！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    If Not blnRet Then
        MsgBox "服务端签名无效！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    If strDigest <> strHexDigest Then
        MsgBox "被验证的证书摘要不匹配,可能遭到恶意攻击！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    Set VerifyCert = colList
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "VerifyCert" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
134     Set GetX509Certificate = objCert
      ...
      Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "GetX509Certificate" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:[常用]获取证书对象（从证书BASE64编码信息中）
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "GetX509CertificateByBase" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:[常用]获取证书属性信息
    ...
    If objCert Is Nothing Then
        MsgBox "证书信息为空!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
...
ReturnLine:
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "GetX509CertificateInfo" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:字节数组转Hex编码字符串
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "ConvertHex" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
 End Function
...
'功能:字符串Base64解码为字节数组
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "Base64Decode" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
 End Function
...
'功能:字节数组Base64编码为字符串
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "Base64Encode" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
 End Function
...
'功能:字符串转字节数组
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "ConvertByte" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:获取随机数
    ...
    If objDevice Is Nothing Then
        MsgBox "创建网证通对象【Device】失败！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "GetRandom" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能: 带PIN码PKCS7签名
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "SignedDataByPwd" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'参数:blnNoHasSource=false 带原文签名(缺省) ;Ture=不带原文签名
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "SignedDataByCertificate" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
' <returns></returns>
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "ParseXML" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'功能:解析证书状态码
    ...
    Exit Function
errH:
    MsgBox Err.Description & vbCrLf & "ParseCertStatus" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
'说明：encodeURI-javaScript方法不会对 ASCII 字母和数字进行编码，也不会对这些 ASCII 标点符号进行编码： - _ . ! ~ * ' ( )
    ...
    EnCodeURL = strRet
errH:
    MsgBox Err.Description & vbCrLf & "EnCodeURL" & "行 " & Erl, vbExclamation + vbOKOnly, gstrSysName
End Function
...
Public Function WZT_InitObj() As Boolean
    ...
    If glngSign > 1 Then WZT_InitObj = True: Exit Function

    On Error GoTo errH
    Set mobjSign = CreateObject("NetcaPki.SignedData.1")
    Set mobjUtil = CreateObject("NetcaPki.Utilities.1")
    
    If mobjSign Is Nothing Or mobjUtil Is Nothing Then GoTo errMsg
        
    mstrKeyType = "NETCA"
        
    gstrPara = gobjComLib.zlDatabase.GetPara(90000, glngSys, , "") '读取配置内容
    LogWrite "WZT_InitObj", "CA参数:" & gstrPara
    If gstrPara = "" Then
    ...
    Exit Function
errMsg:
    MsgBox "创建网证通对象失败！请确认【网证通安全客户端】已安装且该部件【NetcaPkiCom.dll】已注册成功。" & vbCrLf & "错误行:" & Erl, vbExclamation, gstrSysName
    Exit Function
errH:
178  MsgBox "创建接口部件失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
    ...
    If blnCheck Then                 '验证当前USB是否是签名用户的，并获取签名证书
        ...
        If gudtPara.blnISTS Then
            ...
            If strSignData <> "" Then
                ...
                End If
            Else
                MsgBox "签名失败!" & IIf(strErr <> "", vbCrLf & "原因:" & vbCrLf & strErr & "!", ""), vbInformation, gstrSysName
                Exit Function
            End If
        Else
...
Public Function WZT_VerifySign(ByVal strSource As String, ByVal strSignData As String, Optional ByVal strTimeStampCode As String) As Boolean
    ...
    Exit Function
errH:
140     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
...
Public Function WZT_CheckCert() As Boolean
    ...
    If colList Is Nothing Then
        MsgBox "验证失败!", vbInformation + vbOKOnly, gstrSysName: Exit Function
        Exit Function
    End If
    lngRet = Val(colList("status"))
    If lngRet <> 0 Then
        strMsg = ParseCertStatus(lngRet)
        MsgBox "网关验证失败!" & IIf(strMsg <> "", "原因:" & strMsg, ""), vbInformation + vbOKOnly, gstrSysName: Exit Function
        Exit Function
    End If
    WZT_CheckCert = True
    Exit Function
...
'      5-签名图片文件名,空串表示没有签名图片
        ...
        If GetCertList(strKeyId, strCertUserName, strCertDN) Then
...
Private Function GetCertList(Optional ByRef strUniqueID As String = "-1", Optional ByRef strName As String = "-1", Optional ByRef strCertDN As String = "-1", Optional ByRef strDate As String = "-1") As Boolean
    '功能:获取网证通证书详情
        Dim varTemp As Variant
        Dim lngDay As Long
        On Error GoTo errH
'        If mobjCert Is Nothing Then
        Set mobjCert = GetX509Certificate(NETCAPKI_CERT_PURPOSE_SIGN) '签名证书
'        End If
        
        If mobjCert Is Nothing Then
            MsgBox "没有符合的证书供选择,请检查是否插入KEY!", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
...
'                "", "GDCA信任号TrustID")
        ...
        If strUniqueID = "" Then strUniqueID = GetX509CertificateInfo(mobjCert, 1) '证书姆印
        If strCertDN = "" Then strCertDN = GetX509CertificateInfo(mobjCert, 3) '证书主题DN
        If strName = "" Then strName = GetX509CertificateInfo(mobjCert, 12) 'Subject的人名(CN)
        If strDate = "" Then strDate = GetX509CertificateInfo(mobjCert, 6) '证书有效期止
        
116     If IsDate(strDate) Then
            '检查证书是否过期
118         lngDay = CheckValidaty(strDate)
120         If (lngDay <= 30 And lngDay > 0 And Not gblnShow) Then
122             MsgBox "您的证书还有" & lngDay & "天过期", vbInformation, gstrSysName
124             gblnShow = True
                Exit Function
126         ElseIf (lngDay <= 0) Then
128             MsgBox "您的证书已过期 " & Abs(lngDay) & " 天"
                Exit Function
            End If
        End If
...
210     GetCertList = True
        Exit Function
errH:
212     MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
100     gstrPara = gobjComLib.zlDatabase.GetPara(90000, glngSys, , "")
        ...
        Exit Sub
errH:
146     MsgBox "读取参数失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Sub
