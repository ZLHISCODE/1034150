...
122    mblnInit = BJCAGX_InitObj
       Exit Function
errH:
124 MsgBox "创建接口部件失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
Public Function BJCAGX_RegCert(arrCertInfo As Variant) As Boolean
        ...
        Exit Function
errH:
     MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
...
Public Function BJCAGX_CheckCert(ByRef blnReDo As Boolean) As Boolean
    ...
    If Not BJCAGX_InitObj() Then
        MsgBox "部件未初始化！", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    If gudtPara.bytSignVersion = V_RSA Then
        If mUserInfo.strCertSn <> strCertSn Then
            MsgBox "该证书未注册在您的名下，不能使用！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
    ...
    Exit Function
errH:
     MsgBox "检查USBKEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
158                     strTimeStampCode = BJCAGX_TS.CreateTS(strRequest)  '产生时间戳（带证书）
160                     If strTimeStampCode = "" Then
162                         MsgBox "生成时间戳失败！", vbOKOnly + vbInformation, gstrSysName
164                         blnRet = False
                        Else
166                         strDate = BJCAGX_TS.gettimestampinfo(strTimeStampCode, 1)  '取得时间戳时间
...
186     BJCAGX_Sign = blnRet
        ...
        Exit Function
errH:
188      MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
120                 blnRet = False
        ...
        Else
            '验证时间戳
122         strTmp = ""
124         intRet = BJCAGX_TS.VerifyTS(strTStampCode, "")  '不验证源文
126         If intRet <> 0 Then
128             strTmp = "验证时间戳失败！" & GetReturnInfo(intRet)
130             blnRet = False
            Else
132             strDate = BJCAGX_TS.gettimestampinfo(strTStampCode, 1)  '取得时间戳时间
134             strTimeStamp = String14ToDate(strDate)
136             strTmp = "验证时间戳成功！" & vbTab & "签名时间:" & strTimeStamp
138             blnRet = True
            End If
...
144             strTmp = IIf(strTmp <> "", strTmp & vbCrLf, "") & "数据签名验证成功！"
146             blnRet = True And blnRet
            Else
148             strTmp = IIf(strTmp <> "", strTmp & vbCrLf, "") & "数据签名验证失败！"
150             blnRet = False
            End If
        End If
...
156     BJCAGX_VerifySign = blnRet
        Exit Function
errH:
158     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
152             MsgBox "服务端签名验证失败！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
            '验证证书是否过期
...
164                 MsgBox "您的证书还有" & intDay & "天过期。", vbOKOnly + vbInformation, gstrSysName
166                 gblnShow = True
168             ElseIf (intDay <= 0) Then
170                 MsgBox "您的证书已过期 " & Abs(intDay) & " 天。", vbOKOnly + vbInformation, gstrSysName
                    Exit Function
                End If
            End If
            '验证证书是否过期
...
180             MsgBox "客户端证书验失败！", vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
...
182         GetCertLogin = True
        ...
        Exit Function
errH:
184     MsgBox "登录验证失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
110        MsgBox "请您插入Key！", vbOKOnly + vbInformation, gstrSysName
           Exit Function
       End If
...
132        strName = ""
134        MsgBox "请您插入Key！", vbOKOnly + vbInformation, gstrSysName
...
184     GetCertList = True
        ...
        Exit Function
errH:
186 MsgBox "读取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
Private Function GetTimeStamp(ByVal strData As String) As String
    ...
    If Len(strData) = 14 Then
        ...
        If Not IsDate(strTimeStamp) Then
            MsgBox "获取的时间戳不是一个日期！" & strTimeStamp, vbExclamation, gstrSysName
            GetTimeStamp = ""
            Exit Function
        End If
    End If
...
Public Function BJCAGX_SetParaStr() As String
    BJCAGX_SetParaStr = IIf(gudtPara.bytSignVersion = 0, "0", "1") & G_STR_SPLIT & IIf(gudtPara.blnSignPic, "1", "0")
End Function
