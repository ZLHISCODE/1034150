...
Private Sub UpdateEmrInterface()
    ...
    If Err.Number = 0 Then
        Call objEMR.CheckUpdate1(gobjRelogin.InputUser, IIf(gobjRelogin.IsTransPwd, "", "[DBPASSWORD]") & gobjRelogin.InputPwd, IIf(gstrCommand <> "", False, True))
        If Err.Number <> 0 Then
            Err.Clear
...
'      blnBrwCall=是否导航台调用,导航台调用升级时检查预升级时点
    ...
    End If
    '预升级可以在导航台运行中进行
    If StartHisCrust(strUpdateExePath, strJobName, lngWait, strTmpGet) And strJobName <> "PreUpgrade" Then
        Exit Function
    End If
    UpdateZLHIS = True
End Function
