...
Private Sub cmdOK_Click()
    ...
    If Not OraDataOpen(strServer, mstrUserName, IIf(blnTransPassword, TranPasswd(strPassword), strPassword)) Then
        Exit Sub
    Else
        strPassword = Trim(txtNewPWD.Text)
        If Not CheckPWDComplex(gcnOracle, strPassword, strToolTip) Then
        ...
        End If
        If UpdatePassword(mstrUserName, strPassword) Then
...
Private Function UpdatePassword(ByVal strUserName As String, ByVal strPasswd As String) As Boolean
    '-------------------------------------------------------------
    '功能：按人员ID，修改其密码
    '参数：CurrUser
    '      当前用户集
    ...
    DoEvents
    gcnOracle.Execute "alter user " & strUserName & " identified by """ & strPasswd & """", , adCmdText
    UpdatePassword = True
    Exit Function
ErrorHand:
    MsgBox Err.Description, vbInformation, "修改密码"
