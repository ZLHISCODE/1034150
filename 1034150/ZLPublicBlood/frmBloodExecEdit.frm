...
Public Function ShowEdit(ByVal frmParent As Object, ByVal lngModul As Enum_Inside_Program, ByVal lng医嘱ID As Long, _
    ByVal lng发送号 As Long, ByVal lng科室ID As Long, ByVal lng收发ID As Long, ByVal lng执行科室ID As Long, Optional ByVal strPrivs As String) As Boolean
    mblnOk = False
    mlngModul = lngModul
    mlng医嘱ID = lng医嘱ID
    mlng发送号 = lng发送号
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then
        If Me.ActiveControl.name <> "cbo执行人" Then
            Call gobjCommFun.PressKey(vbKeyTab)
        End If
    End If
End Sub
...
Private Sub Form_Load()
    ...
    If mlngModul = p医技工作站 Then
        If Val(gobjDatabase.GetPara("代行执行记录", 100, p医技工作站)) = 0 Then
    ...
    With mExeInfo
        If mExeInfo.开始时间 = "" Then '新增
            ...
            mint血袋数 = GetBloodNum
            mint已执行血袋数 = gobjComlib.FormatEx(Val(txt发送数次.Tag) * mint血袋数, 0) '上次执行的血袋数。已经有5位小数，用四舍五入就可以满足
        ...
        Else '修改
            ...
            mint血袋数 = GetBloodNum
            mint已执行血袋数 = gobjComlib.FormatEx(Val(txt发送数次.Tag) * mint血袋数, 0) '本次的执行的血袋数
...
'功能相应科室的医护人员信息
    ...
    If InStr(mstrPrivs, "执行他科项目") > 0 Then
    ...
    Else
        ...
        If blnYn Or strWhere <> "" Then
            strSQL = " Select a.Id, a.编号, a.姓名, a.简码 From (" & strSQL & ") a" & vbNewLine & _
                " Where " & IIf(blnYn, "  (A.编号 Like [2] Or A.简码 Like [3] Or A.姓名 Like [3])  ", "") & IIf(blnYn = True And strWhere <> "", " And ", "") & strWhere
        End If
    End If
    If blnYn = True Then
        Set GetDataToPersons = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng科室ID, UCase(strIn) & "%", gstrLike & UCase(strIn) & "%")
    Else
        Set GetDataToPersons = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng科室ID)
    End If
