...
'                            UCE.DataChanged = True
        ...
        Case conMenu_View_Find '查找、
            Call UCP.SetPIFocus
        Case conMenu_View_FindNext '继续查找
            Call UCP.SetPIFocus
            Call UCP.FindPati(True)
    End Select
End Sub
...
Public Sub BloodReactionRecord(frmMain As Variant, lng阶段 As Long, ByVal lngSys As Long, ByVal lngMoudle As Long, Optional strPrivs As String, Optional lngisModul As Long = 0)
    ...
    Set objPane = dkpPeoPle.CreatePane(2, 700, 100, DockRightOf, objPane): objPane.Title = "记录": objPane.Options = PaneNoCaption Or PaneNoCloseable Or PaneNoFloatable Or PaneNoHideable
    ...
    UCE.InitEdit
    '初始化usrCardPeople
    UCP.UserInit Me, "颜色|ID|1||||255;住院情况|主页ID;床号;姓名;挂号单或住院号;性别和年龄;入院日期;填写人", , p输血反应管理
        ...
        Select Case varCmd(intLoop)
            Case "初始表格"
                ...
                With mclsVsf
                    ...
                    Call .AppendColumn("挂号单或住院号", 1400, flexAlignLeftCenter, flexDTString, "", , True)
            ...
            Case "基础病人查询":
                ...
                Select Case mlng阶段
                    Case 0: '门诊病人   去掉语句"and f.执行人=[4]" ,添加了对输血反应记录的记录人的判断，
                        strSQL = " Select f.病人ID || '-' || f.id As id, f.病人id, f.Id As 主页id, f.No As 挂号单或住院号, '门' As 住院情况, f.姓名, f.性别 || '/' || f.年龄 As 性别和年龄, f.执行部门id As 科室id, " & vbNewLine & _
                    ...
                    Case 1: '住院病人
                        If mlngtbcIndex = 0 Then '1表示在院
                            strSQL = " Select a.病人ID || '-' || a.主页id As id,a.病人id, a.主页id, a.住院号 || '' As 挂号单或住院号, '住' As 住院情况, a.姓名, a.性别 || '/' || a.年龄 As 性别和年龄," & vbNewLine & _
                    ...
                    Case 2: '输血科
                        ...
                        strSQL = strSQL & " Union ALL" & vbNewLine & _
                                " Select f.病人ID || '-' || f.id As id,f.病人id, f.Id As 主页id, f.No As 挂号单或住院号, '门' As 住院情况, f.姓名, f.性别 || '/' || f.年龄 As 性别和年龄, f.执行部门id As 科室id, " & vbNewLine & _
                                "        b.名称 As 科室名称, f.执行时间 As 入院日期, '' As 床号, f.险类, '' As 类型, 255 As 颜色, f.执行人 As 填写人 " & vbNewLine & _
                                " From 病人挂号记录 f, 部门表 b, " & vbNewLine & _
                        ...
                        If mblnADDPeoPle And mstrFindKey <> "" Then
                            ...
                            If Val(Arr部门(2)) = 0 Then '住院
                                strSQL = strSQL & " Union ALL" & vbNewLine & _
                                    " Select a.病人id || '-' || a.主页id As Id, a.病人id, a.主页id, a.住院号 || '' As 挂号单或住院号, '住' As 住院情况, a.姓名," & vbNewLine & _
                            ...
                            Else '门诊
                                strSQL = strSQL & " Union ALL" & vbNewLine & _
                                    " Select a.病人id || '-' || a.Id As Id, a.病人id, a.Id As 主页id, a.No As 挂号单或住院号, '门' As 住院情况, a.姓名," & vbNewLine & _
            ...
            Case "刷新数据"
                '会调用“基础病人查询”，当过滤条件都没有变化时，“基础病人查询”不会查询病人信息，
                '病人数据会从集合中提取
...
Private Sub UCP_CardChanged()
    ...
    Set rsData = mRsBR
    '将过滤数据整合成字符串，方便传递
    strFilter = mArr过滤数据(0) & "|" & mArr过滤数据(1) & "|" & mArr过滤数据(2) & "|" & mArr过滤数据(3)
