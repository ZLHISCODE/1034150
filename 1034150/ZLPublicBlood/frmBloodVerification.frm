...
Private Sub CMDok_Click()
    ...
    If mblnUserIsOk = False Then '在需要用户手动填写接收人的情况下，要对接收人的数据进行判断
        ...
        If Len(Trim(strPassword接收人)) = 0 Then
            ...
            GoTo InputError
        End If
        If Not OraDataOpen(mstrServer, strUserName接收人, IIf(UCase(strUserName接收人) = "SYS" Or UCase(strUserName接收人) = "SYSTEM", strPassword接收人, TranPasswd(strPassword接收人))) Then
            TXT密码(0).Text = ""
            Call gobjControl.ControlSetFocus(TXT密码(0))
            GoTo InputError
        End If
    ...
    If Not OraDataOpen(mstrServer, strUserName核收人, IIf(UCase(strUserName核收人) = "SYS" Or UCase(strUserName核收人) = "SYSTEM", strPassword核收人, TranPasswd(strPassword核收人))) Then
        TXT密码(1).Text = ""
        Call gobjControl.ControlSetFocus(TXT密码(1))
        GoTo InputError
    End If
...
Private Sub Form_Load()
    ...
    arrConn = Split(gcnOracle.ConnectionString, ";")
    j = UBound(arrConn)-1'下标越界，减一
    For i = 0 To j
        If UCase(Split(arrConn(i), "=")(0)) = UCase("Server") Then
            mstrServer = Split(arrConn(i), "=")(1)
            mstrServer = Mid(mstrServer, 1, Len(mstrServer) - 1)
            Exit For
        End If
    Next
End Sub
