...
Public Function GetBloodAuditState(ByVal intMode As Integer) As String
'功能：intMode  1-返回不审核的血液品种；2-返回不计量的血液品种
    Dim rsTmp As New ADODB.Recordset
    Dim strsql As String, str品种IDs As String
    
    str品种IDs = ""
    strsql = "select 品种id from 血液品种 "
    If intMode = 1 Then
        strsql = strsql & "where 是否审核 = 0 "
    ElseIf intMode = 2 Then
        strsql = strsql & "where 是否计入输血量 = 0 "
    End If
    Set rsTmp = gobjDatabase.OpenSQLRecord(strsql, "血液审核信息")
    Do While Not rsTmp.EOF
        str品种IDs = str品种IDs & "|" & rsTmp!品种id
        rsTmp.MoveNext
    Loop
    If str品种IDs <> "" Then str品种IDs = str品种IDs & "|"
    GetBloodAuditState = str品种IDs
    Exit Function
errH:
    If gobjComlib.ErrCenter() = 1 Then
        Resume
