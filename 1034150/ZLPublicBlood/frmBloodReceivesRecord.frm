...
Private Sub ShowVsf(lngstatu As Long, Optional rs As ADODB.Recordset, Optional strP As String)
    ...
    If optOccasion(0).Value = True Then
        strsql = _
            " Select " & IIf(blnBatch = True, " /*+ CARDINALITY(T 10) */ ", "") & " e.收发id, d.病人id, d.主页id, 0 As 选择, g.入院病床 As 床号, g.姓名, g.性别,g.出院科室ID 当前科室ID, a.名称 As 血液名称, a.规格, f.血袋编号, f.Abo, f.Rh," & vbNewLine & _
            ...
            Case "基础病人查询":
                ...
                If optOccasion(0).Value = True Then '住院病人
                    If marrFilter(mtbcThisIndex).DeptID <> -1 And mtbcThisIndex = 0 Then
                        ...
                        strsql = _
                            " Select a.病人id || '-' || a.主页ID As Id,a.病人ID, a.主页id, a.住院号 门住号,  Decode(Nvl(a.病人性质, 0), 0, '住', '留')As 住院情况, a.姓名, a.性别 || '/' || a.年龄 As 性别年龄, a.出院科室id As 科室id, d.名称," & vbNewLine & _
                    ...
                    Else
                        ...
                        End If
                        strsql = _
                            " Select Distinct a.病人id || '-' || a.主页ID As Id,a.病人ID, a.主页id, a.住院号 门住号,  Decode(Nvl(a.病人性质, 0), 0, '住', '留')As 住院情况, a.姓名, a.性别 || '/' || a.年龄 As 性别年龄, a.出院科室id As 科室id, d.名称," & vbNewLine & _
                            "                a.入院日期 As 日期, a.出院病床 As 床号, a.险类, a.病人类型 As 类型, 255 As 颜色, '' As Aborh" & vbNewLine & _
                ...
                Else '门诊病人
                    ...
                    strsql = _
                        " Select Distinct a.病人id || '-' || A.id As Id, A.病人ID,A.ID 主页ID, a.门诊号, Decode(a.急诊,1,'急',decode(a.复诊,1,'复','普')) As 住院情况, a.姓名, a.性别 || '/' || a.年龄 As 性别年龄, a.执行部门id As 科室id, e.名称 科室名称," & vbNewLine & _
...
'    Set mfrmBloodPeoPle = New frmBloodPeoPle
    '初始化UCP控件
    UCP.UserInit Me, "颜色|ID|1||||255;住院情况|主页ID;床号;姓名;门住号;性别年龄;日期;ABORH;图标", ils16, p血液接收登记
...
'    Dim rs部门 As New ADODB.Recordset
    ...
    cboDepart.Tag = cboDepart.Text
    '调用usrCardPeople控件
    strsql = "Select '' 颜色,'' ID,'' 住院情况, '' 主页ID,'' 床号,'' 姓名,'' 门住号,'' 性别年龄,'' 日期,'' ABORH From dual where 1<>1"
