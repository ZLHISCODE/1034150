...
' int场合 =1 门诊调用,2-住院调用, 当场合=2时，传入lng病区ID
    ...
    If mblnExecFresh = False Then  '执行过程中避免重复刷新，应为本窗体内部已经调用了刷新
        ...
        mlngModul = lngModul
        mlng医嘱ID = lng医嘱ID
        mlng医护科室ID = lng医护科室ID '当前选择的科室ID
        mint场合 = int场合
        mlng病区ID = lng病区ID
        mstrPrivs = strPrivs
        mblnMoved = blnMoved
...
Private Sub RefreshData(Optional ByVal blnRefreshBlood As Boolean = True)
    ...
    strSQL = _
        ...
        "               0," & vbNewLine & _
        "               Decode(Nvl(h.接收状态, 0), 0, '待接收', 2, '拒绝接收', '已接收')," & vbNewLine & _
        "               1," & vbNewLine & _
        "               '正在执行'," & vbNewLine & _
        "               2," & vbNewLine & _
        "               '完成执行'," & vbNewLine & _
        ...
        "       g.名称 待执行部门, f.开始执行人, f.开始时间, Decode(f.前15分钟滴速,-1,'快速',-2,'加压',f.前15分钟滴速) 前15分钟滴速, f.输注前输血反应, f.输注前反应时间, " & vbNewLine & _
        "       f.输注中执行人,Decode(f.后15分钟滴速,-1,'快速',-2,'加压',f.后15分钟滴速) 后15分钟滴速, f.输注后输血反应, f.输注后反应时间, f.结束执行人, f.结束时间," & vbNewLine & _
        "       f.核查者, f.复查者, f.核对时间 核查时间, f.执行科室id, p.名称 执行科室" & vbNewLine & _
        " From 部门表 c, 收费项目目录 e, 血液品种 k, 血液规格 l, 血液收发记录 a, 部门表 g, 部门表 p, 血液执行记录 f, 血液发送记录 h, 血液配血记录 b" & vbNewLine & _
        " Where c.Id = a.库房id And e.Id = a.血液id And k.品种id = l.品种id And l.规格id = a.血液id And a.Id = h.收发id And f.执行科室id = p.Id(+) And" & vbNewLine & _
        "      h.收发id = f.收发id(+) And h.执行科室id = g.Id(+) And h.配发id = b.Id And b.申请id = [1]" & vbNewLine & _
...
Private Function FuncExec(ByVal intExecId As CMD_EXEC) As Boolean
    ...
    Select Case intExecId
        ...
        Case ID_执行记录, conMenu_Manage_ThingAdd
            ...
            lngID = Val(vsExec.TextMatrix(vsExec.Row, vsExec.ColIndex("ID")))
            If frmBloodExecEdit.ShowEdit(Me, mlngModul, mlng医嘱ID, mlng发送号, mlng医护科室ID, lngID, mlng执行部门ID, mstrPrivs) = False Then
                Exit Function
            End If
        ...
        Case ID_完成执行, conMenu_Manage_Complete
            ...
            If mint记录性质 = 2 Then
                ...
                If curMoney > 0 Then
                    ...
                    If gbln消费验证 And mint门诊记帐 = 1 Then
                        If Not gobjDatabase.PatiIdentify(Me, mlngSys, mlng病人ID, curMoney) Then Exit Function
                    End If
                End If
            End If
...
'    End If
    ...
    Select Case Control.id
        ...
        Case ID_执行前核对, conMenu_Manage_ThingAudit * 100# + 1 '执行前核查
            Control.Visible = Not (InStr(GetInsidePrivs(mlngSys, mlngModul), "执行情况登记") = 0)
            Control.Enabled = mblnShowExec And blnEnable And Control.Visible And (int接收状态 = 1 Or int接收状态 = 3) And int血液执行状态 = 0 And vsExec.TextMatrix(vsExec.Row, vsExec.ColIndex("核查者")) = "" '已经接收的为核对血液才能核对
