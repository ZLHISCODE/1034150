...
Public Sub showInfor(lng病人ID As Long, lng病人来源 As Long, lng主页id As Long, lng阶段 As Long, cnMain As ADODB.Connection, objfrmMain As Object, lng模块号 As Long, Optional strFilter As String = "", Optional bln输血科新增权限 As Boolean = False)
    ...
    For intLoop = 0 To UBound(varCmd)
        ...
        Case "控件布局"
            ...
            Else
                ...
                '显示标签状态
                Fra1.Visible = True
                Fra1.Move Picright.ScaleLeft, Picright.ScaleTop + Picright.ScaleHeight - 1500, 1200, 1455
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        Case "初始病人信息"
            ...
            '余浪   2017年2月8日
            strSqlBR = " Select b.病人id, a.住院号 As 住院号, b.姓名, b.性别, b.年龄, b.Id As 医嘱id, b.医嘱内容, b.医生嘱托, d.Abo, d.Rh, d.血袋编号, d.Id As 收发id, h.诊断描述 " & _
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        Case "获取反应记录"
            ...
            If mlng阶段 = 2 And mstrFilter <> "" Then  '输血科要按时间过滤反应记录
                strSqlFY = strSqlFY & " and d.反应时间 Between [5] and [6] order by Nvl(d.反应时间,d.记录时间)"
            Else
                    ...
                    For lngi = 0 To .RecordCount - 1 '数据条数
                        ...
                        If lngi = mlngSelNum Then '选中选单数据
                            ...
                            strXD = " Select e.Id, b.名称, e.血袋编号, c.即往输血史, c.孕产情况 " & _
                                    " From 病人医嘱记录 a, 诊疗项目目录 b, 输血申请记录 c, 血液配血记录 d, 血液收发记录 e " & _
                                    " Where e.配发id = d.Id And d.申请id = a.Id and d.记录性质=1 And c.医嘱id(+) = a.Id And b.Id = a.诊疗项目id  " & _
                            ...
                            TXT21(0).Text = .Fields("输血次数").Value '输血次数
                            '孕产情况
...
Private Sub Msk52_Change(Index As Integer)
...
Private Function GetSecondUserName(ByVal objControl As TextBox, ByVal lngDeptID As Long, ByVal lng病人ID As Long, lng主页id As Long, lng病人来源 As Long, lngID As Long, str名称 As String, str血袋编号 As String, str即往输血史 As String, str孕产情况 As String) As Boolean
    ...
    '余浪
    strSQL = " Select id,血液名称,血袋编号,医嘱号,decode(即往输血史,0,'无','有') as 即往输血史, 孕产情况 " & _
            " From (Select e.Id As Id, b.名称 As 血液名称, e.血袋编号 As 血袋编号, a.Id As 医嘱号, c.即往输血史 As 即往输血史, c.孕产情况," & _
            " Decode(f.收发id, Null, 1, f.接收状态) As 接收状态 " & _
            " From 病人医嘱记录 a, 诊疗项目目录 b, 输血申请记录 c, 血液配血记录 d, 血液收发记录 e, 血液发送记录 f " & _
            " Where e.配发id = d.Id And d.申请id = a.Id and d.记录性质=1 And c.医嘱id(+) = a.Id And b.Id = a.诊疗项目id And a.相关id Is Null And Not Exists " & _
            " (Select 1 From 输血反应记录 h Where e.Id = h.收发id " & IIf(mstrST = 修改, " And h.收发ID<>[3]", " ") & ") And e.Id = f.收发id(+) And e.审核人 Is not Null And " & _
            " Mod(e.记录状态, 3) = 1 And a.诊疗类别 = 'K' And a.病人id = [1]"
    If lng病人来源 = 2 Then
...
Private Function GetZXR(lng收发ID As Long) As String
    ...
    Dim strSQL As String
    Dim rsUser As ADODB.Recordset
    strSQL = " select 开始执行人 from 血液执行记录 where 收发id=[1]"
