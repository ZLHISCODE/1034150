...
Public Function Get结算方式(str场合 As String, Optional str性质 As String, Optional blnCharge As Boolean = False) As ADODB.Recordset
    ...
    If blnCharge Then
        strSql = strSql & " Union " & _
                 " Select Null As 编码, '冲预交' As 名称,0 As 缺省,999 as 性质,0 as 应付款 " & _
                 " From Dual Order By 性质"
    Else
...
Public Function GetShareInvoiceGroupID(ByVal bytKind As Byte) As ADODB.Recordset
    ...
    ElseIf bytKind = 10 Then
        strSql = "" & _
        "   Select A.ID,nvl(M.编码,' ') as 使用类别编码,A.使用类别,A.领用人,A.登记时间,A.开始号码,A.终止号码,A.剩余数量 " & _
        "   From 票据领用记录 A,人员表 B,票据使用类别 M" & vbNewLine & _
        "   Where A.票种 In (1,3) And A.使用方式=2 And A.剩余数量>0 And A.领用人=B.姓名" & _
        "           And A.使用类别=M.名称(+) " & _
        "           And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null)" & vbNewLine & _
        "   Order by 使用类别编码,剩余数量 Desc"
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, _
                                    ByRef dblAllTime As Double, ByVal strDec As String, ByRef dblPriceSingle As Double, _
                                    Optional ByVal lng批次 As Long) As Currency
