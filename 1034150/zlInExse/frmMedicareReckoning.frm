...
Public Function ShowMe(ByRef frmParent As Object, ByVal lng结帐ID As Long, _
    ByVal lng病人ID As Long, ByVal bln中途结帐 As Boolean, _
        ...
        ByRef objPayCards As Cards, ByRef objCard As Card, ByVal strPrivs As String) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:医保预结算与结算不一致的较对窗体
...
Private Function ShowMoney(Optional ByVal blnAutoSet As Boolean) As String
    ...
    If blnAutoSet Then
        ...
        If InStr(1, mstrPrivs, ";仅用预交结帐;") > 0 Then
            ...
            For i = 1 To vsDeposit.Rows - 1
                If cur结帐合计 = 0 Then
                    vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = "0.00"
                Else
                    If Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("余额"))) <= Format(cur结帐合计, "0.00") Then
                        vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = Format(Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("余额"))), "0.00")
                    Else
                        vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = Format(cur结帐合计, "0.00")
                    End If
                    cur结帐合计 = cur结帐合计 - Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")))
                End If
            Next i
        Else
            With vsDeposit
                If .TextMatrix(1, .ColIndex("ID")) <> "" Then    '可能没有预交,全部现款
                    If mblnThreeDepositAfter Then
                        For i = 1 To vsDeposit.Rows - 1
                            varData = Split(vsDeposit.Cell(flexcpData, i, vsDeposit.ColIndex("ID")) & "||||||", "||")
                            If Val(varData(0)) <> 0 Or Val(varData(3)) = 1 Then
                                If bln负数 And Val(varData(3)) <> 1 Then
                                    vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = Format(0, "0.00")
                                ElseIf Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("余额"))) <= Format(cur结帐合计, "0.00") Then
                                    vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = Format(Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("余额"))), "0.00")
                                    cur结帐合计 = cur结帐合计 - Val(vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")))
                                Else
                                    vsDeposit.TextMatrix(i, vsDeposit.ColIndex("冲预交")) = Format(cur结帐合计, "0.00")
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    cur结帐合计 = cur结帐合计 - Val(vsDeposit.TextMatrix(i, .ColIndex("冲预交")))
                                End If
                            End If
                        Next i
                    Else
