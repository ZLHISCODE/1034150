...
Private Sub PrintBill()
    ...
    If Not gblnPrintByPatient Then
        '不按病人打印
        Call frmPrint.ReportPrint(1, mtyBalanceInfor.strNo, _
            mtyBalanceInfor.lng结帐ID, mobjFact.LastUseID, _
            mobjFact.共享批次ID, mobjFact.使用类别, txtInvoice.Text, _
            mtyBalanceInfor.dtBalanceDate, CStr(mtyBalanceInfor.dbl缴款), _
            CStr(mtyBalanceInfor.dbl找补), , _
             mobjFact.打印格式, , 1)
        Exit Sub
    End If
    '按病人打印
    ...
    For i = 1 To mrsPatients.RecordCount
        ...
        Call frmPrint.ReportPrint(1, mtyBalanceInfor.strNo, _
            mtyBalanceInfor.lng结帐ID, mobjFact.LastUseID, _
            mobjFact.共享批次ID, mobjFact.使用类别, _
            txtInvoice.Text, mtyBalanceInfor.dtBalanceDate, _
            CStr(mtyBalanceInfor.dbl缴款), CStr(mtyBalanceInfor.dbl找补), _
            lng病人ID, mobjFact.打印格式, , 1)
