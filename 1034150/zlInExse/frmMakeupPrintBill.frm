...
Private mstrRptName  As String  '报表名称
...
Private Function zlMakeupPrint(ByVal lng病人ID As Long, lng主页ID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Call frmPrint.ReportPrint(2, strNo, lng结帐ID, mlng领用ID, mlngShareUseID, mstrUseType, strInvoice, , , , lng病人ID, mintInvoiceFormat)
...
Private Sub Form_Load()
    ...
    mblnFirst = True
    mstrRptName = IIf(gbytInvoiceKind = 0, "ZL" & glngSys \ 100 & "_BILL_1137", "ZL" & glngSys \ 100 & "_BILL_1137_2")
    mblnStartFactUseType = zlStartFactUseType(mbytInvoiceKind)
...
Private Function ReadListData() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    "         And A.id=X.记录ID(+) And X.性质(+)=2 And X.险类=Y.序号(+)  " & _
...
Private Sub ShowBillFormat()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:根据当前登录的收费操作员显示它所使用收费票据格式
    '编制:刘兴洪
    '日期:2013-01-05 15:38:44
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If intFormat = 0 Then
        intFormat = Val(GetSetting("ZLSOFT", "私有模块\" & UserInfo.用户名 & "\zl9Report\LocalSet\" & mstrRptName, "Format", 1))
    End If
    strSql = "Select B.说明 From zlReports A,zlRptFmts B" & _
        " Where A.ID=B.报表ID And A.编号=[1] And B.序号=[2]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mstrRptName, intFormat)
...
Private Sub RefreshFact()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If (lblFact.Tag <> "" And txtInvoice.Text <> "") Or Trim(txtInvoice.Text) = "" Then
        '松散：取下一个号码
        txtInvoice.Text = IncStr(UCase(zlDatabase.GetPara("当前结帐票据号", glngSys, mlngModule)))
        'Tag：问题：24363:刘兴洪：主要是解决自动生成的号是否被用户更改，主要解决：
        '    1.更改的票据号需要检查是否重复，重复后直接返回不更改发票号
        '    2.并发操作，不更改的情况下，检查是否重复，如果重复，自动取下一个号码！
    End If
...
Private Function zlGetInvoiceGroupUseID(ByRef lng领用ID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '编制:刘兴洪
    '日期:2013-01-05 15:40:47
    '说明:
    '---------------------------------------------------------------------------------------------------------------------------------------------
    
    On Error GoTo errHandle
    lng领用ID = GetInvoiceGroupID(mbytInvoiceKind, intNum, lng领用ID, mlngShareUseID, strInvoiceNO, mstrUseType)
    If lng领用ID <= 0 Then
        Select Case lng领用ID
            Case 0 '操作失败
            Case -1
