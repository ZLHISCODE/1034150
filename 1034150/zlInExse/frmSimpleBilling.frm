   ...
   Begin MSComctlLib.ImageList imgList 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmSimpleBilling.frx":1E68
            Key             =   ""
         EndProperty
      EndProperty
   End
...
Private Sub Bill_CommandClick()
    ...
    Else
        int病人来源 = 2
    End If
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, False, "'Z'", , , str特准项目)
    If lng项目id <> 0 Then
        Bill.Text = lng项目id
        mblnSelect = True
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        Else
                            int病人来源 = 2
                        End If
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, False, "'Z'", Bill.Text, Bill.TxtHwnd)
                        If lng项目id <> 0 Then
                    ...
                    If mrsInfo.State = 1 Then
                        If Not IsNull(mrsInfo!险类) Then
                            ...
                            '医保对码
                            If Not CheckMediCareItem(objDetail.ID, mrsInfo!险类, objDetail.名称, objDetail.变价 = False) Then
                                Bill.Text = "": Bill.TxtSetFocus: Cancel = True: Exit Sub
                            End If
                        End If
                    End If
                    ...
                    If mrsWarn.RecordCount > 0 And mrsInfo.State = 1 And mobjBill.Details.Count = Bill.Row Then
                        ...
                        If cur合计 > 0 Then
                            ...
                            gbytWarn = BillingWarn(mstrPrivsOpt, mrsInfo!姓名, Val("" & mrsInfo!病区ID), mrsInfo!适用病人, mrsWarn, cur余额, mrsInfo!当日额 - mcurModiMoney, cur合计, IIf(IsNull(mrsInfo!担保额), 0, mrsInfo!担保额), mobjBill.Details(Bill.Row).收费类别, mobjBill.Details(Bill.Row).Detail.类别名称, mstrWarn, , , curItemMoney)
    ...
    ElseIf Bill.Active And chkCancel.Value = 0 Then '正常输入单据状态
        ...
        If mrsWarn.RecordCount > 0 Then
            ...
            If curTotal > 0 Then
                ...
                gbytWarn = BillingWarn(mstrPrivsOpt, mrsInfo!姓名, Val("" & mrsInfo!病区ID), Val(txt医疗付款.Tag) = 1 Or Not IsNull(mrsInfo!险类), _
...
Private Sub Form_Load()
    ...
    Else '新增
        ...
        If mstrInNO <> "" Then '修改单据
            Set mobjBill = ImportBill(mstrInNO, False, Me, True, True)
            If mobjBill.NO = "" Then
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    If Not rsTmp.EOF Then
        ...
        If mobjBill.Details(lngRow).Detail.变价 Then
            ...
            Else
                dblMoney = mobjBill.Details(lngRow).InComes(1).标准单价
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            With mobjBillIncome
                ...
                If mobjBill.Details(lngRow).Detail.屏蔽费别 Then
                    .实收金额 = .应收金额
                Else
                    .实收金额 = CCur(Format(ActualMoney(mobjBill.费别, .收入项目ID, .应收金额, 0, 0, 0, 0), gstrDec))
                End If
