...
'参数：str序号=指定单据中要审核的行号,为空表示所有行
    ...
    If cur金额 > 0 Then
        '病人相关信息
        strSql = "Select B.当前病区ID 病区ID,nvl(B.姓名,A.姓名) as 姓名,C.预交余额-C.费用余额 as 余额,zl_PatiDayCharge(A.病人ID) as 当日额," & _
        ...
        For i = 0 To UBound(Split(str类别s, ","))
            intWarn = BillingWarn(mstrPrivsOpt, rsTmp!姓名, Val("" & rsTmp!病区ID), rsTmp!适用病人, rsWarn, _
