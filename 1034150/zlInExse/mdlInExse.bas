...
Public gbln自动发料 As Boolean '记帐完成后是否自动发料
...
Public gbyt检查门诊未执行 As Byte    '门诊结帐时检查是否有未执行项目及未发药品:0-不检查,1-检查并提示,2-检查并禁止
Public gbyt检查门诊未发药 As Byte   '门诊结帐时是否检查病人的未发药品项目:0-不检查,1-检查并提示,2-检查并禁止
...
Public Function GetVBalance(ByVal byt业务类型 As Byte, strPrivs As String, int险类 As Integer, lng病人ID As Long, Optional strTime As String, _
     ...
     Optional strItem As String, Optional strDeptIDs As String, Optional strClass As String, Optional strChargeType As String = "", Optional strDiagnose As String = "") As ADODB.Recordset
    '------------------------------------------------------------------------------------------------------------------------
...
''    End If
    ...
    If blnOnly门诊 Then
        ...
        If BytType = 2 Then
            ...
            "              Sum(统筹金额) As 统筹金额,费用类型, A.医嘱序号" & vbNewLine & _
            ...
            "       Group By NO, Mod(记录性质, 10), 记录状态, Nvl(价格父号, 序号), 病人id, 收费类别, 收据费目, 计算单位, 开单人, 收费细目id, 是否急诊," & vbNewLine & _
            "                开单部门id, 执行部门id,费用类型,A.医嘱序号" & vbNewLine & _
        ...
        "       Max(A.开单部门id) 开单部门id, Max(A.执行部门id) 执行部门id, Max(A.开单人) 开单人,Max(A.费用类型) 费用类型,Max(E.诊断描述) 诊断 " & vbNewLine & _
        " From ( " & strTable & ") A, 保险支付项目 B, 收费项目目录 C, 病人诊断医嘱 D, 病人诊断记录 E " & vbNewLine & _
        " Where A.收费细目id = C.ID And a.医嘱序号 = D.医嘱id(+) And D.诊断id = E.Id(+) And  A.收费细目id = B.收费细目id " & IIf(blnRelation, "(+)", "") & " And B.险类" & IIf(blnRelation, "(+)", "") & " = [5] " & vbNewLine & _
                    IIf(strClass = "", "", " And Instr([9],','''||Nvl(A.费用类型,Nvl(C.费用类型,'无'))||''',')>0") & IIf(strDiagnose = "", "", " And (Instr([11],','''||E.诊断描述||''',')>0 Or E.诊断描述 Is Null)") & _
    ...
    Else
        ...
        If BytType = 2 Then
            ...
            If InStr(1, "," & strTime & ",", ",0,") > 0 Or strTime = "" Then  '包含门诊:33189
                    ...
                    "              A.登记时间, Nvl(A.是否上传, 0) As 是否上传, Nvl(A.是否急诊, 0) As 是否急诊,Nvl(A.保险项目否, 0) As 保险项目否, Max(Decode(A.记录性质,2,A.摘要,Null)) As 摘要,A.费用类型,A.医嘱序号" & vbNewLine & _
                    ...
                    "                Nvl(A.保险大类id, 0), A.发生时间, A.登记时间, Nvl(A.是否上传, 0), Nvl(A.是否急诊, 0), Nvl(A.保险项目否, 0),A.费用类型,A.医嘱序号" & vbNewLine & _
        ...
        Else
            ...
            If BytType = 0 Then
                ...
                "              A.保险编码, Nvl(A.保险大类id, 0) As 保险大类id, Avg(Nvl(A.付数, 1) * A.数次) As 数量," & vbNewLine & _
                "              Avg(A.标准单价) As 标准单价, Sum(Nvl(A.实收金额, 0)) - Sum(Nvl(A.结帐金额, 0)) As 金额, A.发生时间," & vbNewLine & _
                "              A.登记时间, Nvl(A.是否上传, 0) As 是否上传, Nvl(A.是否急诊, 0) As 是否急诊,Nvl(A.保险项目否, 0) As 保险项目否, Max(Decode(A.记录性质,5,A.摘要,Null)) As 摘要,A.费用类型,A.医嘱序号" & vbNewLine & _
                "       From " & IIf(blnDateMoved, zlGetFullFieldsTable("住院费用记录"), "住院费用记录 A") & " , 收入项目 B" & vbNewLine & _
        ...
        strSQL = "Select A.记录性质, A.记录状态, A.NO, A.序号, A.门诊标志, A.病人id, A.主页id, A.婴儿费, C.项目编码 As 医保项目编码," & vbNewLine & _
                ...
                "       A.医生, A.发生时间, A.登记时间, A.是否上传, A.是否急诊, A.保险项目否, Max(Decode(A.记录性质,2,A.摘要,5,A.摘要,Null)) As 摘要,A.费用类型,Max(G.诊断描述) As 诊断" & vbNewLine & _
                "From ( " & strTable & ") A, 收费项目目录 B, 保险支付项目 C, 收费项目别名 E,部门表 X, 病人诊断医嘱 F, 病人诊断记录 G" & vbNewLine & _
                "Where A.收费细目id = B.ID And a.医嘱序号 = f.医嘱id(+) And f.诊断id = g.Id(+) And B.ID = C.收费细目id" & IIf(blnRelation, "(+)", "") & " And C.险类" & IIf(blnRelation, "(+)", "") & " = [5] And A.开单部门id = X.ID " & vbNewLine & _
                        IIf(strClass = "", "", " And Instr([9],','''||Nvl(A.费用类型,Nvl(B.费用类型,'无'))||''',')>0") & IIf(strDiagnose = "", "", " And (Instr([11],','''||G.诊断描述||''',')>0 Or G.诊断描述 Is Null)") & _
                "      And A.收费细目id = E.收费细目id(+) And E.码类(+) = 1 And E.性质(+) = " & IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1) & _
                " Group By A.记录性质, A.记录状态, A.NO, A.序号, A.门诊标志, A.病人id, A.主页id, A.婴儿费, C.项目编码," & vbNewLine & _
                "       A.保险编码, A.保险大类id, A.收费类别, A.收费细目id, Nvl(E.名称, B.名称), A.计算单位," & vbNewLine & _
                "       X.名称, B.规格, B.产地, A.数量, A.标准单价, A.金额," & vbNewLine & _
                "       A.医生, A.发生时间, A.登记时间, A.是否上传, A.是否急诊, A.保险项目否,A.费用类型"
    End If
    Set GetVBalance = zlDatabase.OpenSQLRecord(strSQL, "mdlInExse", lng病人ID, "," & strTime & ",", DateBegin, DateEnd, int险类, bytBaby - 1, "," & strItem & ",", "," & strDeptIDs & ",", "," & strClass & ",", "," & strChargeType & ",", "," & strDiagnose & ",")
    '问题:strDeptIDs:42478
    Exit Function
errH:
...
Public Function GetBalance(ByVal byt业务类型 As Byte, strPrivs As String, lng病人ID As Long, Optional strTime As String, Optional strDeptIDs As String, _
    ...
    Optional bln消费卡启用 As Boolean = False, Optional strChargeType As String = "", Optional strDiagnose As String = "") As ADODB.Recordset
    '------------------------------------------------------------------------------------------------------------------------
    ...
    If blnZero Then
        If BytType = 2 Then
            ...
            "                Decode(Nvl(A.婴儿费,0),0,'','√') as 婴儿费,A.收费细目ID,A.收入项目ID,A.收据费目,Nvl(A.实收金额,0) as 未结金额,费用类型, A.收费类别, A.医嘱序号" & _
    ...
    If BytType = 2 Then
        ...
        If InStr(1, "," & strTime & ",", ",0,") > 0 Or strTime = "" Then  '包含门诊
            ...
            "               Sum(Nvl(A.实收金额,0))-Sum(Nvl(A.结帐金额,0)) as 未结金额,A.费用类型 , max(A.收费类别) as 收费类别, A.医嘱序号" & _
            ...
            "        Group by A.NO,A.序号,Mod(A.记录性质,10),A.记录状态,A.执行状态, A.医嘱序号," & _
        ...
        "                    Or (Sum(Nvl(A.实收金额, 0)) = 0 And Sum(Nvl(A.应收金额, 0)) <> 0 and Sum(Nvl(A.结帐金额,0)) =0 And Mod(Count(*),2)=0) " & _
        "                    Or Sum(Nvl(A.结帐金额, 0))=0 And Sum(Nvl(A.应收金额,0))<>0 And Mod(Count(*),2)=0" & _
        "        Group by A.NO,A.序号,Mod(A.记录性质,10),A.记录状态,A.执行状态, A.医嘱序号,Decode(A.主页ID,NULL,'门诊','第'||A.主页ID||'次'),A.主页ID," & _
        "               A.开单部门ID,To_Char(A.发生时间,'YYYY-MM-DD HH24:MI:SS'),A.登记时间,Decode(Nvl(A.婴儿费,0),0,'','√'),A.收费细目ID,A.收入项目ID,A.收据费目,A.费用类型" & _
        ""
    Else
        If BytType = 0 Then
        ...
        "                    Or (Sum(Nvl(A.实收金额, 0)) = 0 And Sum(Nvl(A.应收金额, 0)) <> 0 and  Sum(Nvl(A.结帐金额,0))=0  And Mod(Count(*),2)=0) " & _
        "                     Or Sum(Nvl(A.结帐金额, 0))=0 And Sum(Nvl(A.应收金额,0))<>0 And Mod(Count(*),2)=0" & _
        "        Group by A.NO,A.序号,Mod(A.记录性质,10),A.记录状态,A.执行状态, A.医嘱序号," & IIf(BytType = 0, "", "Decode(A.主页ID,NULL,'门诊','第'||A.主页ID||'次'),A.主页ID,") & _
        "               A.开单部门ID,To_Char(A.发生时间,'YYYY-MM-DD HH24:MI:SS'),A.登记时间,Decode(Nvl(A.婴儿费,0),0,'','√'),A.收费细目ID,A.收入项目ID,A.收据费目,A.费用类型" & _
        ""
        If BytType = 0 Then
    ...
    strSQL = _
        ...
        "       Nvl(A.未结金额,0) 未结金额,Nvl(A.未结金额,0) 结帐金额,Nvl(A.费用类型,C.费用类型) as 类型, A.收费类别, Max(G.诊断描述) As 诊断 " & _
                IIf(bln消费卡启用, ",A.费别, A.执行部门id, A.开单人,A.数量, A.标准单价 as 价格, A.统筹金额, A.保险大类id,A.收费细目ID,C.计算单位", "") & _
        " From (  " & strTable & ") A,部门表 B,收费项目目录 C,收入项目 D,收费项目别名 E, 病人诊断医嘱 F, 病人诊断记录 G  " & _
        " Where A.开单部门ID=B.ID(+) And A.收费细目ID=C.ID And A.收入项目ID=D.ID " & IIf(strClass = "", "", " And Instr([8],','''||Nvl(A.费用类型,Nvl(C.费用类型,'无'))||''',')>0") & _
        "       And A.收费细目ID=E.收费细目ID(+) And a.医嘱序号 = f.医嘱id(+) And f.诊断id = g.Id(+) And E.码类(+)=1 And E.性质(+)=" & IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1) & IIf(strDiagnose = "", "", " And (Instr([10],','''||G.诊断描述||''',')>0 Or G.诊断描述 Is Null)") & _
        ...
        "       Nvl(A.未结金额,0),Nvl(A.未结金额,0),Nvl(A.费用类型,C.费用类型), A.收费类别" & _
                IIf(bln消费卡启用, ",A.费别, A.执行部门id, A.开单人,A.数量, A.标准单价, A.统筹金额, A.保险大类id,A.收费细目ID,C.计算单位", "") & _
    ...
    Set GetBalance = zlDatabase.OpenSQLRecord(strSQL, "mdlInExse", lng病人ID, "," & strTime & ",", DateBegin, DateEnd, _
                    "," & strDeptIDs & ",", bytBaby - 1, "," & strItem & ",", "," & strClass & ",", "," & strChargeType & ",", "," & strDiagnose & ",")
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Public Sub NurseDeposit(ByVal frmMain As Object, ByVal lng病人ID As Long, ByVal lng主页ID As Long)
    On Error GoTo errH
    If gobjPati Is Nothing Then
        Set gobjPati = CreateObject("zl9Patient.clsPatient")
    End If
    Call gobjPati.NurseDeposit(glngSys, gcnOracle, frmMain, gstrDBUser, lng病人ID, lng主页ID)
...
Public Function GetDeposit(lng病人ID As Long, _
   ...
   strSQL = "" & _
    "Select a.No, a.票据号, a.Id, a.金额, a.记录状态, a.预交id, a.日期, a.结算方式, a.卡类别id, a.结算卡序号, a.卡号, a.交易流水号, a.交易说明, Nvl(c.是否转帐及代扣,0) As 代扣," & vbNewLine & _
    "       b.性质 As 结算性质, Nvl(c.是否退现,0) As 退现, Nvl(c.是否全退,0) As 全退, c.名称 As 三方卡名称, c.是否缺省退现 As 缺省退现 " & vbNewLine & _
    ...
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.实际票号), '')) As 票据号," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, To_Char(a.收款时间, 'yyyy-mm-dd hh24:mi:ss')), Null)) As 日期," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.结算方式), '')) As 结算方式," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.卡类别id), 0)) As 卡类别id," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.结算卡序号), 0)) As 结算卡序号," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.卡号), '')) As 卡号," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.交易流水号), '')) As 交易流水号," & vbNewLine & _
    "              Max(Decode(a.记录性质, 1, Decode(a.记录状态, 2, Null, a.交易说明), '')) As 交易说明" & vbNewLine & _
    "       From " & strTable & vbNewLine & _
    "       Where a.记录性质 In (1, 11) And a.病人id = [1] " & strWhere & strWherePage & _
    "       Having Sum(Nvl(金额, 0) - Nvl(冲预交, 0)) <> 0" & vbNewLine & _
    "       Group By a.No) A, 结算方式 B, 医疗卡类别 C" & vbNewLine & _
    " Where a.结算方式 = b.名称(+) And a.卡类别id = c.Id(+) And b.性质 <> 5" & vbNewLine & _
    " Order By decode(sign(金额),-1,0,1),Decode(Nvl(a.卡类别id, 0), 0, 0, Decode(Nvl(c.是否退现, 0), 0, 2, 1)) Desc,Decode(Nvl(a.卡类别id, 0), 0, 0, Decode(Nvl(c.是否全退, 0), 0, 1, 2)) Desc, Nvl(a.卡类别id, 0) Desc, a.No, a.结算方式"
    
    '主要是适用支付宝更改,代扣标志为1,其他的在10.35版本中支持(先使用支付宝缴的预交部分)
    Set GetDeposit = zlDatabase.OpenSQLRecord(strSQL, "mdlInExse", lng病人ID, strTime, strDate, int预交类别)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'返回：真-处理成功
    ...
    gbyt检查门诊未执行 = Val(zlDatabase.GetPara(266, glngSys))
    gbyt检查门诊未发药 = Val(zlDatabase.GetPara(265, glngSys))
    ...
    gbln执行后发料 = zlDatabase.GetPara(33, glngSys) = "1"
    ...
    gbln自动发料 = zlDatabase.GetPara(63, glngSys) = "1"
...
Public Function ImportBill(strNo As String, blnBat As Boolean, frmParent As Object, _
    ...
    Optional ByVal lngUnitID As Long, Optional ByVal bln不读执行性质 As Boolean = True) As ExpenseBill
...
'      不管是导入还是修改单据,都不应包含已停用收费细目
    ...
    Dim strTable  As String, str摘要 As String
    ...
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlInExse", strNo, lngUnitID, IIf(blnBat, 1, 0), IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1))
                    ...
                    If !是否变价 = 1 Then
                        If InStr(",5,6,7,", !收费类别) > 0 Or (!收费类别 = "4" And Nvl(!跟踪在用, 0) = 1) Then
                            ...
                            If dblAllTime <> 0 Then
                                dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, CLng(!收费细目ID), (Nvl(!收费类别) = "4"), dblAllTime, gstrDec, dblPriceSingle)
                                If dblAllTime <> 0 Then
                                    '数量未分解完毕
                                    If !收费类别 = "4" Then
...
Public Function RePrintBalance(strNo As String, frmParent As Object, lng结帐ID As Long, _
    ...
    If Not gobjTax Is Nothing And gblnTax Then
        If lngPatientCount > 1 Then
            '合约单位结帐
            strUseType = zlDatabase.GetPara("合约单位结帐打印", glngSys, 1137)
        Else
            Call zlGetBalancePati(lng结帐ID, lng病人ID, lng主页ID)
            strUseType = zl_GetInvoiceUserType(lng病人ID, lng主页ID, intInsure)
        End If
        intFormat = zl_GetInvoicePrintFormat(1137, strUseType, bytKind)
        intPrintMode = zl_GetInvoicePrintMode(1137, strUseType)
        lngShareUseID = zl_GetInvoiceShareID(1137, strUseType)
        blnDo = True
    Else
            ...
            SaveSetting "ZLSOFT", "私有模块\" & UserInfo.用户名 & "\zl9Report\LocalSet\" & strRptName, "Format", intFormat
...
'            If rstmp Is Nothing Then Exit Function
        ...
        Call GetNextInvoice(frmParent, lngPatientCount, lng领用ID, lngShareUseID, strUseType, strInvoice, , bytInvoiceKind)
        If gblnStrictCtrl And strInvoice = "" Then Exit Function
        
        If gblnPrintByPatient And lngPatientCount > 1 Then
            For i = 1 To rsTmp.RecordCount
                lngPatientID = rsTmp!病人ID
                '合药单位,按普通住院病人打印票据
                Call frmPrint.ReportPrint(2, strNo, lng结帐ID, lng领用ID, lngShareUseID, strUseType, strInvoice, , , , lngPatientID, intFormat, False, bytInvoiceKind + 1)
                If i < rsTmp.RecordCount Then
                    strInvoice = ""
                    Call GetNextInvoice(frmParent, lngPatientCount + 1 - i, lng领用ID, lngShareUseID, strInvoice, i = 1)
                    If gblnStrictCtrl And strInvoice = "" Then Exit Function
                End If
                rsTmp.MoveNext
            Next
        Else
            Call frmPrint.ReportPrint(2, strNo, lng结帐ID, lng领用ID, lngShareUseID, strUseType, strInvoice, , , , , intFormat, , bytInvoiceKind + 1)
        End If
        RePrintBalance = True
    End If
End Function

Public Sub GetNextInvoice(ByRef frmParent As Object, _
    ByVal lngLeastNum As Long, ByRef lng领用ID As Long, ByVal lngShareUseID As Long, _
    ByVal strUseType As String, ByRef strInvoice As String, _
    Optional ByRef blnFirst As Boolean = True, _
    Optional bytInvoiceKind As Byte = 3)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:重打结帐票据时,获取下一票据号
    '入参:blnFirst-按病人打印时，是否首次打印（仅首次打印提示确定票据号）
    '       lngShareUseID-共享批次ID
    '       strUseType-使用类别
    '出参:lng领用ID-返回领用ID
    '        strInvoice-返回发票号
    '编制:刘兴洪
    ...
    If gblnStrictCtrl Then
        lng领用ID = GetInvoiceGroupID(IIf(bytInvoiceKind = 3, IIf(gbytInvoiceKind = 0, 3, 1), IIf(bytInvoiceKind = 0, 3, 1)), lngLeastNum, 0, lngShareUseID, , strUseType)
        Select Case lng领用ID
            Case -1
                If strUseType <> "" Then
                    MsgBox "你没有自用和共用『" & strUseType & "』的结帐票据,请先领用一批票据或设置本地共用票据！", vbInformation, gstrSysName
                Else
                    MsgBox "你没有自用和共用的结帐票据,请先领用一批票据或设置本地共用票据！", vbInformation, gstrSysName
                End If
            Case -2
                If strUseType <> "" Then
                    MsgBox "本地的共用『" & strUseType & "』的结帐票据已经用完,请先领用一批票据或重新设置本地共用票据！", vbInformation, gstrSysName
                Else
    ...
    Else
        Do
            ...
            '检查输入有效性
            If blnInput Then
                If GetInvoiceGroupID(IIf(bytInvoiceKind = 3, IIf(gbytInvoiceKind = 0, 3, 1), IIf(bytInvoiceKind = 0, 3, 1)), lngLeastNum, lng领用ID, lngShareUseID, strInvoice, strUseType) = -3 Then
...
Public Function GetBalanceID(ByVal strNo As String, Optional ByRef lng病人ID As Long) As Long
'功能：获取一张结帐单据的ID
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String
    ...
    strSQL = "Select ID,病人ID From 病人结帐记录 Where 记录状态=1 And NO=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlInExse", strNo)
    If Not rsTmp.EOF Then
        GetBalanceID = Val(Nvl(rsTmp!ID))
...
'功能：获取一张结帐单据的冲预交记录
    ...
    strSQL = "" & _
    "Select ID,NO as 单据号,实际票号 as 票据号, " & _
    "       To_Char(收款时间,'YYYY-MM-DD') as 日期,结算方式," & _
    "       nvl(冲预交,0) as 冲预交,卡类别ID" & _
    " From 病人预交记录   " & _
    " Where mod(记录性质,10)=1 And 结帐ID =[1] And nvl(冲预交,0)<>0"
    
    strSQL = "" & _
    "   Select A.ID,A.单据号,A.票据号,To_Char(Max(B.收款时间),'YYYY-MM-DD') as 日期," & _
    "          A.结算方式," & _
    "          Ltrim(To_Char(max(A.冲预交),'9999999990.00')) as 金额, " & _
    "          A.卡类别ID,Max(C.是否转帐及代扣) as 代扣" & _
    "   From (" & strSQL & ") A,病人预交记录 B,医疗卡类别 C" & _
    "   Where A.单据号=B.NO And B.记录性质=1 And A.卡类别ID=C.ID(+)" & _
    "   Group By a.Id,a.单据号,a.票据号,a.结算方式,A.卡类别ID " & _
    "   Order by 日期,a.结算方式"
...
Public Function GetBalancePay(ByVal lngBalanceID As Long, ByVal blnNOMoved As Boolean) As ADODB.Recordset
    ...
    "       A.结算号码,Nvl(B.性质,0) as 性质,A.卡类别ID,A.卡号,A.交易流水号,A.交易说明" & _
...
Public Function BillCanBeOperate(ByVal strNo As String, ByVal strPriv As String, _
    ...
    Optional str病人IDs As String, Optional ByVal BytType As Byte = 2) As Boolean
...
Public Function BillCanDelete(ByVal strNo As String, ByVal bytFlag As Byte, _
    Optional ByVal blnBat As Boolean, Optional ByVal strTime As String, Optional ByVal strPrivs As String, Optional ByRef blnFlagPrint As Boolean) As Integer
'功能：判断一张单据是否可以退费或销帐
...
'    blnFlagPrint=检查对应的条码是否已打印(检验医嘱中的采集方式已执行)
    ...
    If strPrivs <> "" Then
        ...
        blnYP = zlCheckPrivs(";" & strPrivs & ";", "药品销帐")
        blnZL = zlCheckPrivs(";" & strPrivs & ";", "诊疗销帐")
        blnWC = zlCheckPrivs(";" & strPrivs & ";", "卫材销帐")
    ...
    strSQL = "Select Distinct Nvl(A.执行状态,0) as 执行状态,B.样本条码" & _
        ...
        " Where A.NO=[1] And A.记录性质=[2] And A.记录状态 IN(0,1,3) And Nvl(多病人单,0)=[3]" & vbNewLine & _
...
Public Function CheckMediCareItem(ByVal lng收费细目ID As Long, ByVal int险类 As Integer, ByVal str收费项目名称 As String, ByVal bln定价 As Boolean) As Boolean
...
Public Function CheckDelPriv(ByVal strNo As String, ByVal strPrivs As String, Optional ByVal strTime As String, _
        Optional ByVal bytFlag As Byte = 2, Optional ByVal bytMode As Byte = 1) As Boolean
'功能：检查是否权限冲销住院记帐单
...
Public Function Check留观病人(ByVal strNo As String, ByVal strPrivs As String, Optional ByVal strTime As String, Optional ByVal bytFlag As Byte = 2) As String
...
'参数:lngDeptID-当前操作的病区ID,所有病区时为0
    ...
    Else
        ...
        If lngDeptID <> 0 Then Call zlControl.CboSetIndex(cbo开单科室.hWnd, FindCboIndex(cbo开单科室, lngDeptID))
...
'       主要目的在于禁止隐式调用Click时对开单人，开单科室的相互影响，及相关数据影响(例如：会改变单据对象中的对应值)
    ...
    'a.开单人定开单科室
    If gblnFromDr Then
        Call zlControl.CboSetIndex(cbo开单人.hWnd, GetCboIndex(cbo开单人, str开单人))
        ...
        End If
        Call zlControl.CboSetIndex(cbo开单科室.hWnd, FindCboIndex(cbo开单科室, lng开单科室ID))
...
'     否则,按编码排序,取第一个
    ...
    Else
        ...
        If rs开单科室.RecordCount > 1 And lng开单科室ID > 0 Then
            ...
            If blnDo Then Call zlControl.CboSetIndex(cbo开单科室.hWnd, FindCboIndex(cbo开单科室, lng开单科室ID))
        End If
...
Public Function ExistWaitExe(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional ByVal bln门诊 As Boolean = False) As String
...
'返回：医技科室名称
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "ExistWaitExe", lng病人ID, lng主页ID, IIf(bln门诊, 1, 0))
...
Public Function ExistWaitDrug(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional ByVal lng检查离院带药 As Long = 0, Optional ByVal bln门诊 As Boolean = False) As String
...
'返回：药房和发料部门名称
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "ExistWaitDrug", lng病人ID, lng主页ID, lng检查离院带药, IIf(bln门诊, 1, 0))
...
'功能：检查指定病人是否具有相关权限
    ...
    strSQL = "Select Nvl(b.姓名, a.姓名) As 姓名,B.出院日期,B.状态,X.费用余额" & _
        " From 病人信息 A,病案主页 B,病人余额 X" & _
        " Where A.病人ID=B.病人ID And A.病人ID=X.病人ID(+) And X.性质(+)=1 And X.类型(+)=2 " & _
...
Public Function CheckRecalcRecord(ByVal strNo As String) As Boolean
...
Public Function CheckNONegative(ByVal strNo As String, Optional ByVal BytType As Byte = 2) As Boolean
'功能：判断指定单据是否包含负数明细
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        ElseIf Not IsNull(rsMedAudit!可用数量) Then
            ...
            If ObjBillDetails(j).付数 * ObjBillDetails(j).数次 * IIf(gbln住院单位, ObjBillDetails(j).Detail.住院包装, 1) > rsMedAudit!可用数量 Then
                MsgBox "第" & j & "行收费项目的数次超过了批准的使用限量" & FormatEx(rsMedAudit!可用数量 / IIf(gbln住院单位, ObjBillDetails(j).Detail.住院包装, 1), 5) & "。", vbInformation, gstrSysName
...
Public Function zlIsExistsSquareCard(ByVal strNos As String) As Boolean
    ...
    strSQL = "Select A.卡结算id " & _
    "   From 病人卡结算对照 A, 病人预交记录 B, " & _
    "        (Select Column_Value From Table(Cast(f_Str2list([1]) As Zltools.t_Strlist))) J " & _
    "   Where A.预交id = B.ID and ( B.记录性质=2 or B.记录性质=12) And B.NO = J.Column_Value And Rownum = 1"
...
Public Function ImportWholeSet(frmParent As Object, ByVal intInsure As Integer, rsSel As ADODB.Recordset, Optional lng病人ID As Long = 0, _
     ...
     Optional str划价人 As String = "", Optional str开单人 As String = "") As ExpenseBill
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:读取费用单据到单据对象中
    '入参:rsSel-选中的成套项目
    '       lngUnitID    当前操作病区ID
    ...
    End If
    Set rsItems = zlDatabase.OpenSQLRecord(gstrSQL, "mdlExse", strValue(0), strValue(1), strValue(2), strValue(3), strValue(4), strValue(5), strValue(6), strValue(7), strValue(8), strValue(9), strValue(10), IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1), lng病人ID)
    '没有记录就是空单子
    Set objBill = New ExpenseBill
    Set objBill.Details = New BillDetails
            ...
            Do While Not rsItems.EOF
                ...
                If Val(Nvl(rsItems!是否变价)) = 1 Then
                    If InStr(",5,6,7,", rsItems!收费类别) > 0 Or (rsItems!收费类别 = "4" And Nvl(rsItems!跟踪在用, 0) = 1) Then
                        ...
                        If dblAllTime <> 0 Then
                            dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, CLng(Nvl(!收费细目ID)), (Nvl(!收费类别) = "4"), dblAllTime, gstrDec, dblPriceSingle)
                            If dblAllTime <> 0 Then
...
Public Sub zlPreBalanceAll(ByVal frmMain As Object, ByVal lng病区ID As Long)
    ...
    Dim str医保号 As String, str密码 As String, str姓名 As String, str登记时间 As Date
    ...
    End If
    strSQL = "" & _
    "   Select distinct A.病人ID, B.主页ID,B.住院号,B.出院病床 as 床号, A.登记时间,B.险类, E.密码, Nvl(b.姓名, a.姓名) As 姓名," & vbNewLine & _
    "       E.医保号" & vbNewLine & _
...
errHandle:
    ...
    End If
End Function
Public Function zlExecBilling(ByVal int场合 As Integer, ByVal frmMain As Object, ByVal lng病区ID As Long, _
    ...
    Optional bln补费 As Boolean = False, Optional lng科室ID As Long = 0, Optional lng医嘱ID As Long = 0, _
    Optional bytModeType As Byte = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:调用指定的记帐单
    ...
    frmCharge.Show IIf(frmMain Is Nothing, bytModeType, 1), frmMain
...
Public Function zl_GetInvoicePrintFormat(ByVal lngModule As Long, _
    Optional str使用类别 As String = "", Optional bytKind As Byte) As Integer
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取发票打印格式
    ...
    If bytKind = 1 Then
        strShareTypeFormat = Trim(zlDatabase.GetPara("门诊结帐发票格式", glngSys, lngModule, ""))
    Else
        strShareTypeFormat = Trim(zlDatabase.GetPara("住院结帐发票格式", glngSys, lngModule, ""))
    End If
...
Public Function zl_GetInvoiceRedFormat(ByVal lngModule As Long, _
    ...
    '因为Getpara就缓存了的,所以不用先用变量进行记录
    strShareTypeFormat = Trim(zlDatabase.GetPara("作废发票格式", glngSys, lngModule, ""))
    '格式:使用类别1,格式1|使用类别2,格式2...
    varData = Split(strShareTypeFormat, "|")
    For i = 0 To UBound(varData)
        varTemp = Split(varData(i) & ",,", ",")
        ...
        If Trim(varTemp(0)) = "" Then lngFormat1 = lngFormat
        If Trim(varTemp(0)) = str使用类别 Then
            zl_GetInvoiceRedFormat = lngFormat: Exit Function
        End If
    Next
    zl_GetInvoiceRedFormat = lngFormat1
End Function
...
Public Function zl_GetInvoiceRedMode(ByVal lngModule As Long, _
    Optional str使用类别 As String = "") As Integer
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取发票打印格式
    '出参:int打印方式-打印方式()
    ...
    Dim varTemp As Variant, varData As Variant, i As Long, strShareTypeFormat As String
     Dim intPrintMode As Long, intPrintMode1 As Long
    '因为Getpara就缓存了的,所以不用先用变量进行记录
    strShareTypeFormat = Trim(zlDatabase.GetPara("作废发票打印方式", glngSys, lngModule, ""))
    '格式:使用类别1,打印方式1|使用类别2,打印方式2...
    varData = Split(strShareTypeFormat, "|")
    For i = 0 To UBound(varData)
        varTemp = Split(varData(i) & ",,", ",")
        ...
        If Trim(varTemp(0)) = "" Then intPrintMode1 = intPrintMode
        If Trim(varTemp(0)) = str使用类别 Then
            zl_GetInvoiceRedMode = intPrintMode: Exit Function
        End If
    Next
    zl_GetInvoiceRedMode = intPrintMode1
End Function
...
Public Function zlFromIDGetChargeBalance(ByVal BytType As Byte, _
    ...
    If blnDel Then
        ...
        strSQL = strSQL & " Union ALL " & _
        "   Select A.ID,decode(A.记录状态,2,A.结帐ID,NULL) as 结帐ID,5 as  类型,Mod(A.记录性质,10) as 记录性质,A.结算方式,nvl(b.结算金额,0) as 冲预交,A.摘要," & _
        ...
        "   From  " & strTable & " A ,病人卡结算对照 C,病人卡结算记录 B, " & _
        "        卡消费接口目录 M ,结算方式 q " & strTable1 & _
        "   Where  a.Id = c.预交id And c.卡结算id = b.Id  And a.结算卡序号 = m.编号  " & _
    ...
    strSQL = strSQL & " Union ALL " & _
    "   Select /*+ Rule */ A.ID,A.结帐ID,5 as  类型,Mod(A.记录性质,10) as 记录性质,A.结算方式,nvl(b.结算金额,0) as 冲预交,A.摘要," & _
    "        decode(A.记录状态,2,1,0) as 退费,A.卡类别ID,A.结算卡序号," & _
