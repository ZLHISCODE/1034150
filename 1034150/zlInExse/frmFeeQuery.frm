...
Private mbln出院 As Boolean, mstrBalancePrivs As String
...
Private Sub InitBaseData()
    ...
    Select Case mbytList
        Case ListType.C0费用清单, ListType.C1分科室明细, ListType.C2分项目明细, ListType.C3分类别明细, ListType.C4分类分项明细 '明细清单,分科明细,项目明细,分类明细,(按收入项目(或收据费目),收费项目,明细分级查询)
            ...
            chk不包含原始单据.Visible = True
            chk不包含原始单据.Enabled = chk仅显示销帐单据.Value = 1
...
'功能：刷新数据
    ...
    If LoadPatiClass Then '初始化选项卡
        ...
        tabClass.Tabs(1).Selected = True '调用tabClass_Click显示费用
    End If
...
'功能：根据权限设置菜单和工具栏的可见状态
    ...
    Select Case Control.ID
        ...
        Case conMenu_Edit_Billing, conMenu_Edit_Copy
            '54274
        ...
        Case conMenu_Edit_Balance
            blnVisible = InStr(mstrBalancePrivs, ";门诊费用结帐;") > 0 Or InStr(mstrBalancePrivs, ";住院费用结帐;") > 0
            If mint场合 = 1 Then
                blnVisible = InStr(mstrBalancePrivs, ";仅用预交结帐;") > 0
            End If
    End Select
...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_Edit_Billing   '记帐
            '问题:33744
            If zlExecBilling(IIf(mint场合 = 0, 6, mint场合), gfrmMain, mlng病区ID, mlng病人ID, mbln出院, mbln结清, mstrUnitIDs, mlng主页ID, mbln补费, mlng科室ID, , GetModuleType) Then Call RefreshAllData
        Case conMenu_Edit_Copy '复制记账单
            '54274
            If zlCopyBill(IIf(mint场合 = 0, 6, mint场合), gfrmMain, mlng病区ID, mlng病人ID, mbln出院, mbln结清, mstrUnitIDs, mlng主页ID, mlng科室ID) Then Call RefreshAllData
        Case conMenu_Edit_ReBilling '销帐
            Call ExecUnBilling
        Case conMenu_Edit_CardBackMoney '余额退款
            Call NurseDeposit(mfrmParent, mlng病人ID, mlng主页ID)
        Case conMenu_Edit_ReBillingApply
...
Private Sub cboDept_Click()
    ...
    If cboDept.Tag = "不刷新" And mblnNotClick Then Exit Sub
    Call FilterDetail
End Sub
...
Private Sub LoadFeeOutline(ByVal lng病人ID As Long, ByVal blnDateMoved As Boolean, ByVal lng结帐ID As Long)
    ...
    If lng结帐ID = 0 Then
        If lng病人ID > 0 Then
            strSql = "Select Nvl(预交余额,0) 预交余额,Nvl(费用余额,0) 费用余额,0 as 预结费用 From 病人余额 Where 性质=1 And   类型=2 And 病人ID=[1]"
            If mblnPreBalance Then
                ...
                strSql = "Select Nvl(Sum(预交余额),0) as 预交余额,Nvl(Sum(费用余额),0) as 费用余额,Nvl(Sum(预结费用),0) as 预结费用 From (" & strSql & ")"
            End If
    ...
    Else
        ...
        strSql = "" & _
        " Select nvl(结帐金额,0) as 结帐金额 From 门诊费用记录 where 结帐ID=[1] Union ALL "
        strSql = strSql & " Select nvl(结帐金额,0) as 结帐金额 From 住院费用记录 where 结帐ID=[1]  "
        ...
        "   Group by Decode(Substr(A.记录性质,Length(A.记录性质),1),1,'冲预交',A.结算方式),Decode(Substr(a.记录性质, Length(a.记录性质), 1), 1, 0, b.性质, 3, 1, b.性质, 4, 1, 0) Order by 结算方式"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lng结帐ID)
...
'参数:lng病人ID:0-表示仅显示初始信息.
    ...
    If lng结帐ID = 0 Then
        ...
        strSql = "Select To_Char(Max(Decode(a.记录性质, 1, a.收款时间, Null)), 'YYYY-MM-DD') As 日期, " & vbNewLine & _
                ...
                " Where A.科室id = B.ID(+) And A.记录性质 In (1, 11) and Nvl(A.预交类别, 2) = 2 And A.病人id = [1]" & vbNewLine & _
     ...
     Else
        ...
        strSql = "Select To_Char(c.收款时间, 'YYYY-MM-DD') As 日期, a.No As 单据号, b.名称 As 科室, a.结算方式, a.结算号码," & vbNewLine & _
                "       LTrim(To_Char(Nvl(a.冲预交, 0), '9999999990.00')) As 冲预交金额, a.摘要, a.实际票号, a.操作员姓名 As 结帐操作员, c.操作员姓名 As 预交收款操作员 " & vbNewLine & _
                " From " & IIf(blnDateMoved, zlGetFullFieldsTable("病人预交记录"), "病人预交记录 A") & ", 部门表 B, " & IIf(blnDateMoved, zlGetFullFieldsTable("病人预交记录", , , , "C"), "病人预交记录 C") & vbNewLine & _
                " Where a.科室id = b.Id(+) And a.记录性质 In (1, 11) And c.记录性质 = 1 And c.No = a.No And c.记录状态 In (1, 3) And Nvl(a.预交类别, 2) = 2 And" & vbNewLine & _
                "       Nvl(a.冲预交, 0) <> 0 And a.结帐id = [1] " & vbNewLine & _
                " Order By 日期, 单据号"
...
Private Function LoadCardData(Optional blnHead As Boolean = True, _
    ...
    Select Case mbytList
        Case ListType.C0费用清单, ListType.C1分科室明细, ListType.C2分项目明细, ListType.C3分类别明细, ListType.C4分类分项明细 '明细清单,分科明细,项目明细,分类明细,(按收入项目(或收据费目),收费项目,明细分级查询)
            If chk不包含原始单据.Value = 1 Then
                strWhere = strWhere & " And 记录状态 = 2"
            Else
                strWhere = strWhere & IIf(chk仅显示销帐单据.Value = 0, "", " And A.记录状态<>1")
            End If
        Case ListType.C5分项目汇总, ListType.C6分类别汇总, ListType.C7分月分类汇总, ListType.C8逐日单据汇总, ListType.C9逐日费目汇总
    ...
    If blnMoney Then
        If lng结帐ID = 0 Then '默认显示未结费用,其它情况是某次结帐的费用
            ...
            If mvs.ZeroFee Or (chk仅显示销帐单据.Value = 1 And chk仅显示销帐单据.Visible) Then
                ...
                str病人费用 = _
                    ...
                    "           Or (Sum(Nvl(A.实收金额, 0)) = 0 And Sum(Nvl(A.应收金额, 0)) <> 0 and Sum(Nvl(A.结帐金额,0)) =0 And (Mod(Count(*),2)=0 or  sum(decode(a.结帐ID,null,0,0,1)) = 0)) " & _
                    "           Or (Sum(Nvl(A.结帐金额, 0))=0 And Sum(Nvl(A.应收金额,0))<>0 And (Mod(Count(*),2)=0 Or sum(decode(a.结帐ID,null,0,0,1))=0))" & _
                    "  Group by A.NO,Mod(A.记录性质,10),Nvl(A.价格父号,A.序号),A.发生时间,A.登记时间,A.记录状态,A.收费细目ID,A.收据费目,A.收费类别,A.执行状态," & _
                    "          A.开单人,A.开单部门ID,A.执行部门ID,A.计算单位,A.数次,Nvl(A.付数,1),A.标准单价,A.操作员姓名,A.费用类型,Decode(Nvl(A.医嘱序号,0),0,0,(Decode(Sign(A.数次),-1,1,0))) " & _
                    "  Union ALL " & _
                    "  Select Mod(A.记录性质,10) as 记录性质,A.记录状态,A.发生时间,A.登记时间,A.NO,A.收费细目ID,A.收据费目,A.收费类别,A.开单人,A.开单部门ID,A.执行部门ID,A.计算单位,Max(A.摘要) as 摘要,Max(A.保险编码) as 保险编码," & _
                    "       A.数次,Nvl(A.付数,1) as 付数,A.标准单价,Sum(A.实收金额) As 实收金额,Sum(A.结帐金额) As 结帐金额,A.操作员姓名,A.费用类型,Decode(Nvl(A.医嘱序号,0),0,0,(Decode(Sign(A.数次),-1,1,0))) 超期收回,Nvl(A.价格父号,A.序号) as 序号,A.执行状态 as 执行状态" & _
                    "  From 门诊费用记录 A" & _
                    "  Where A.病人ID=[1]" & strIF & strWhereCheckFee & _
                    "           And (Nvl(A.实收金额,0)<>Nvl(A.结帐金额,0) Or Nvl(A.结帐金额, 0)=0)" & _
                    "  Having Nvl(Sum(A.实收金额),0)-Nvl(Sum(A.结帐金额),0)<>0 " & _
                    "           Or (Sum(Nvl(A.实收金额, 0)) = 0 And Sum(Nvl(A.应收金额, 0)) <> 0 and Sum(Nvl(A.结帐金额,0)) =0 And (Mod(Count(*),2)=0 or  sum(decode(a.结帐ID,null,0,0,1)) = 0)) " & _
                    "           Or (Sum(Nvl(A.结帐金额, 0))=0 And Sum(Nvl(A.应收金额,0))<>0 And (Mod(Count(*),2)=0  Or  sum(decode(a.结帐ID,null,0,0,1)) = 0))" & _
            ...
            Else
                ...
                strSql = _
                    " Select NO,Mod(记录性质,10) as 记录性质" & _
                    " From 住院费用记录 A" & _
                ...
                str病人费用 = _
                 ...
                 str病人费用 = str病人费用 & " Union ALL " & Replace(Replace(str病人费用, "住院费用记录", "门诊费用记录"), strWhere主页, "")
                 str病人费用 = "(" & str病人费用 & ") "
            End If
        ...
        Else
            ...
            " From 住院费用记录 A" & _
            " Where A.结帐ID=[1]  " & strWhereCheckFee & strBaby & IIf(chkAdivce.Value = 0, "", " And A.医嘱序号 is Null") & strWhere
            If blnDateMoved Then
                str病人费用 = str病人费用 & vbCrLf & " UNION ALL " & vbCrLf & Replace(str病人费用, "住院费用记录", "H住院费用记录")
            End If
            ...
            Case ListType.C4分类分项明细 '按收入项目(或收据费目),收费项目,明细分级查询
                If str分类方式 = "收入项目" Then
                    If lng结帐ID = 0 Then
                        ...
                        " Where A.NO = G.NO And Mod(A.记录性质,10)=G.记录性质 And Nvl(A.价格父号,A.序号)=G.序号 And A.记录状态<>0 " & _
                        " Union ALL " & _
                        " Select distinct  A.发生时间,A.NO,A.开单人,A.收据费目, " & _
                        "        A.付数*decode(A.记录性质,12,0,13,0,1) as 付数,A.数次*decode(A.记录性质,12,0,13,0,1) as 数次, " & _
                        "       A.计算单位,A.标准单价,A.实收金额,A.结帐金额,A.费用类型,A.操作员姓名,A.登记时间," & _
...
Private Sub chkNotCheckFee_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then zlCommFun.PressKey vbKeyTab
End Sub
...
Private Sub chk按规格进行小计_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then zlCommFun.PressKey vbKeyTab
End Sub

Private Sub chk不包含原始单据_Click()
    If Visible = False Then Exit Sub
    Call LoadCardData(False, False, True)
End Sub

Private Sub chk不包含原始单据_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then zlCommFun.PressKey vbKeyTab
End Sub

Private Sub chk仅显示销帐单据_Click()
    If Visible = False Then Exit Sub
    If chk仅显示销帐单据.Value = 1 Then
        chk不包含原始单据.Enabled = True
    Else
        chk不包含原始单据.Value = 0
        chk不包含原始单据.Enabled = False
    End If
    Call LoadCardData(False, False, True)
End Sub
...
Private Sub Form_Load()
    ...
    mstrPrivs = gstrPrivs
    mstrBalancePrivs = GetPrivFunc(glngSys, 1137)
...
Private Sub Form_Resize()
    ...
    If picNum.Enabled And picNum.Visible Then
        picNum.Left = IIf(995 + 1680 * (tabClass.Tabs.Count - 1) + 120 < Me.ScaleWidth - picNum.Width, 995 + 1680 * (tabClass.Tabs.Count - 1) + 120, Me.ScaleWidth - picNum.Width - 30)
        tabClass.Width = Me.ScaleWidth - picNum.Width
    End If
...
Private Sub imgColSel_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picDetail.hWnd)
...
'    Call BandRectoGrid(mshInsure, rsTmp)
...
Private Sub ShowBilling(ByVal strNo As String, ByVal byt记录性质 As Byte, ByVal byt记录状态 As Byte)
    ...
    If BillisBatch(strNo) Then '批量记帐
        ...
        frmBillings.Show GetModuleType, Me
    ElseIf BillisSimple(strNo) Then '简单记帐
        ...
        frmSimpleBilling.Show GetModuleType, Me
    Else '记帐单
...
Private Sub ExecUnBilling()
    ...
    If Not (mbytList = ListType.C0费用清单 Or mbytList = ListType.C1分科室明细 Or mbytList = ListType.C2分项目明细 Or mbytList = ListType.C3分类别明细 Or mbytList = ListType.C4分类分项明细) Then Exit Sub
    ...
    '权限判断
    If Not BillOperCheck(5, strUser, CDate(strTime), "销帐", strNo, , BytType) Then Exit Sub
        
    '是否已转入后备数据表中
    If zlDatabase.NOMoved("住院费用记录", strNo, , CStr(BytType), Caption) Then
        If Not ReturnMovedExes(strNo, BytType, Caption) Then Exit Sub
    End If
            
    '项目冲销权限
    If Not CheckDelPriv(strNo, GetInsidePrivs(Enum_Inside_Program.p记帐操作), strTime, BytType) Then Exit Sub
        
    '留观病人权限
    strInfo = Check留观病人(strNo, GetInsidePrivs(Enum_Inside_Program.p记帐操作), strTime, BytType)
    If strInfo <> "" Then
    ...
    '是否已执行
    blnBat = BillisBatch(strNo)
    i = BillCanDelete(strNo, BytType, blnBat, strTime, GetInsidePrivs(Enum_Inside_Program.p记帐操作), blnFlagPrint)
    If i <> 0 Then
        Select Case i
            Case 1 '该单据不存在
    ...
    '出院病人操作权限判断
    If Not BillCanBeOperate(strNo, GetInsidePrivs(Enum_Inside_Program.p记帐操作), "销帐", strTime, str病人IDs, BytType) Then Exit Sub
...
Private Function zlCopyBill(ByVal int场合 As Integer, ByVal frmMain As Object, ByVal lng病区ID As Long, _
    ...
    Optional strUnitIDs As String = "", Optional lng主页ID As Long = 0, Optional lng科室ID As Long = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:拷贝单据
