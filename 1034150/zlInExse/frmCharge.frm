...
Private Sub cmdSelWholeSet_Click()
    ...
    If frmWholeSelect.ShowSelect(Me, mlngModule, mstrPrivsOpt, rsSel) = False Then Exit Sub
    ...
    Set tmpBill = ImportWholeSet(Me, intInsure, rsSel, lng病人ID, gbln住院单位, lng开单部门ID, byt婴儿费, 2, chk加班.Value = 1, _
        0, Get病人来源, UserInfo.姓名, NeedName(cbo开单人.Text))
    '处理数据
    '清除导入的病人信息
...
Private Sub Bill_CommandClick()
    ...
    End If
    mlng批次 = -1
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, gbln住院单位, str类别, , , str特准项目, 0, str排除类别, False, mbln条码刷卡, mlng批次)
    If lng项目id <> 0 Then
        Bill.Text = lng项目id
        mblnSelect = True
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        End If
                        mlng批次 = -1
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, gbln住院单位, str类别, Bill.Text, Bill.TxtHwnd, str特准项目, 0, , False, mbln条码刷卡, mlng批次)
                        If lng项目id <> 0 Then
                    ...
                    If mrsInfo.State = 1 Then
                        If Not IsNull(mrsInfo!险类) Then
                            If Not CheckMediCareItem(mobjDetail.ID, mrsInfo!险类, mobjDetail.名称, mobjDetail.变价 = False) Then
                                Bill.Text = "": Bill.TxtSetFocus: Cancel = True: Exit Sub
                            End If
                        End If
                    End If
                    ...
                    Else
                        If mrsInfo.State = 1 Then '90304
                            str摘要 = gclsInsure.GetItemInfo(Val("" & mrsInfo!险类), mrsInfo!病人ID, mobjBill.Details(Bill.Row).收费细目ID, str摘要, 2)
                        Else
                    ...
                    If mrsWarn.RecordCount > 0 And mrsInfo.State = 1 And mobjBill.Details.Count = Bill.Row Then
                        ...
                        If curTotal > 0 Then
                            ...
                            gbytWarn = BillingWarn(mstrPrivsOpt, mrsInfo!姓名, Val("" & mrsInfo!病区ID), mrsInfo!适用病人, mrsWarn, cur余额, mrsInfo!当日额 - mcurModiMoney, curTotal, _
                    ...
                    If mrsInfo.State = 1 Then
                        If Not IsNull(mrsInfo!险类) And MCPAR.实时监控 And mobjBill.Details(Bill.Row).数次 <> 0 Then
                            If gclsInsure.CheckItem(Val(mrsInfo!险类), 1, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), Bill.Row)) = False Then
                                mobjBill.Details.Remove Bill.Row '删除刚刚想要加入的费用行
                                Bill.Text = "": Cancel = True: Exit Sub
                            End If
                        End If
                    ...
                    If mobjBill.Details(Bill.Row).数次 <> 0 Then
                        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, mlngModule, 1, 0, _
                            MakeDetailRecord(mobjBill, zlStr.NeedName(cbo开单人.Text), zlStr.NeedName(cbo开单科室.Text), Bill.Row)) = False Then
                            mobjBill.Details.Remove Bill.Row '删除刚刚想要加入的费用行
                            Bill.Text = "": Cancel = True: Exit Sub
                        End If
                    End If
                     ...
                     With mobjBill.Details(Bill.Row)
                        ...
                        If mrsWarn.RecordCount > 0 And mrsInfo.State = 1 Then
                            ...
                            If curTotal > 0 Then
                                ...
                                End If
                                If gbln报警包含划价费用 Then cur余额 = Val(txt实收.Tag) - GetPriceMoneyTotal(1, mrsInfo!病人ID) + IIf(gbytBilling = 1, mcurModiMoney, 0)
                                gbytWarn = BillingWarn(mstrPrivsOpt, mrsInfo!姓名, Val("" & mrsInfo!病区ID), mrsInfo!适用病人, mrsWarn, cur余额, mrsInfo!当日额 - mcurModiMoney, _
...
'调用者:Bill_KeyDown中输入项目后
    ...
    With mobjBill.Details(lngMainRow)
        ...
        For i = 1 To mcolDetails.Count
            ...
            If mrsInfo.State = 1 Then
                If Not IsNull(mrsInfo!险类) Then
                    If Not CheckMediCareItem(mcolDetails(i).ID, mrsInfo!险类, mcolDetails(i).名称, mcolDetails(i).变价 = False) Then
                        Exit Sub
                    End If
                End If
            End If
            ...
            If mrsInfo.State = 1 Then '90304
                str摘要 = gclsInsure.GetItemInfo(Val("" & mrsInfo!险类), mrsInfo!病人ID, mcolDetails(i).ID, str摘要, 2)
            Else
...
Private Sub cbo费别_Click()
    If cbo费别.ListIndex <> -1 And Not mobjBill Is Nothing And Bill.Active Then
        If mobjBill.费别 <> NeedName(cbo费别.Text) And Not mbln不重算价格 Then
    ...
    Else
        If mrsInfo.State = 1 Then
            If Not IsNull(mrsInfo!住院医师) Then
                Call zlControl.CboSetIndex(cbo开单人.hWnd, GetCboIndex(cbo开单人, mrsInfo!住院医师))
            End If
        End If
    End If
End Sub
...
Private Sub cbo开单人_Click()
    ...
    If Not (mbytInState = 0 And chkCancel.Value = 0) Then Exit Sub
    If mobjBill.开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text)) Then Exit Sub
        ...
        ElseIf Bill.Active Then '正常输入单据状态'(清除后当作是新病人单据)
            ...
            If mbln补费 Then
                ...
                With mobjBill
                    ...
                    .开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
                End With
                 Bill.SetFocus
...
Private Sub cmdOK_Click()
    ...
    ElseIf Bill.Active And chkCancel.Value = 0 Then '正常输入单据状态
        ...
        '33744
        If mbln补费 Then
            If txtDate.Text > mstr最后转科时间 Then
        ...
        If Not IsNull(mrsInfo!险类) And MCPAR.实时监控 Then
            If gclsInsure.CheckItem(Val(mrsInfo!险类), 1, 2, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text))) = False Then
                Bill.SetFocus: Exit Sub
            End If
        End If
        '处方职务检查
        ...
        If mrsWarn.RecordCount > 0 And mstrWarn <> "-" And Not mblnSavePrice Then
            ...
            If curTotal > 0 Then
                ...
                For i = 1 To mobjBill.Details.Count
                    gbytWarn = BillingWarn(mstrPrivsOpt, mrsInfo!姓名, Val("" & mrsInfo!病区ID), mrsInfo!适用病人, mrsWarn, cur余额, cur当日额 - mcurModiMoney, curTotal, _
...
'调用中药配方输入功能
    ...
    If cbo医疗付款.ListIndex <> -1 Then
        ...
        '问题:45605
        Call zlIsCheckMedicinePayMode(NeedName(cbo医疗付款), bln医保)
    End If
    ...
    If mrsInfo.State = 1 Then
        ...
        '问题:45605
        int险类 = IIf(mrsInfo!险类 <> "" And bln医保, mrsInfo!险类, 0)   '医保病人可能本次记帐不用医保
    Else
        int病人来源 = 2
        int险类 = 0
    End If
    ...
    Set objDetails = frmCHRecipe.ShowMe(Me, mstrPrivs, gbytBilling, mcurModiMoney, mobjBill.病人ID, int病人来源, lng科室ID, Get开单科室ID, _
                    glng中药房, mobjBill.Details, NeedName(cbo费别.Text), int险类, chk加班.Value = 1, mobjBill.煎法, mrsWarn, mcolStock1, zl获取中药形态(Bill.Row, True))
...
Private Sub Form_Load()
    ...
    Else '新增,修改
        ...
        If mstrInNO <> "" Then '修改单据
            Set mobjBill = ImportBill(mstrInNO, False, Me, True, gbln住院单位, , , False)
...
'    mobjCard.卡号有效字符 = "0" '输入类型(0-所有字符,1-数字,2-字母;3-数字或字母;4-指定字符)|Ascii码1，Ascii码2....
    ...
    If mbytInState = 0 Then
        ...
        Else
            ...
            sta.Panels("WB").Bevel = sbrInset
        End If
        mblnShowBarCode = Val(zlDatabase.GetPara("上次选择条码控制", glngSys, 1150)) = 1
...
Private Sub FindPati(ByVal objCard As Card, ByVal blnCard As Boolean, ByVal strInput As String)
    ...
    '读取成功
     ...
     If Mid(gstrCardPass, 6, 1) = "1" _
        And (blnCard Or (blnICCard And mstrPassWord <> "") _
               Or (blnIDCard And mstrPassWord <> "")) Then
    ...
    If gblnFromDr Then
        If Not IsNull(mrsInfo!住院医师) Then
            cbo开单人.ListIndex = -1
            cbo开单人.ListIndex = GetCboIndex(cbo开单人, mrsInfo!住院医师)
        End If
    Else
        '33744
        If mbln补费 Then
            If cbo开单科室.ListIndex >= 0 Then
                If cbo开单科室.ItemData(cbo开单科室.ListIndex) <> mlngDeptID And mlngDeptID <> 0 Then
                    cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, mlngDeptID)
                ElseIf mlngDeptID = 0 Then
                
                    cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, IIf(IsNull(mrsInfo!科室ID), 0, mrsInfo!科室ID))
                End If
            ElseIf mlngDeptID = 0 Then
                cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, IIf(IsNull(mrsInfo!科室ID), 0, mrsInfo!科室ID))
            Else
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, ByVal blnCard As Boolean, Optional blnOutMsg As Boolean = False) As Boolean
    ...
    strSql = _
            ...
            "   A.就诊卡号,A.卡验证码,A.住院号,B.出院病床 as 床号, " & _
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    Dim dblPrice As Double, dbl加班加价率 As Double, dblPriceSingle As Double
    ...
    If Not rsTmp.EOF Then
                ...
                If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                    ...
                    If dblAllTime <> 0 Then
                        ...
                        Else
                            ...
                            dblAllTime = .付数 * .数次
                            If gbln住院单位 And InStr(",5,6,7,", .收费类别) > 0 Then
                                dblAllTime = dblAllTime * .Detail.住院包装 '按售价数量计算实价
                            End If
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            With mobjBillIncome
                ...
                Else
                    If InStr(",5,6,7,", mobjBill.Details(lngRow).收费类别) > 0 And gbln住院单位 Then
                        .标准单价 = Format(Nvl(rsTmp!现价, 0) * mobjBill.Details(lngRow).Detail.住院包装, gstrFeePrecisionFmt)
                    Else
        ...
        If mobjBillDetail.数次 <> 0 Then
            ...
            For Each mobjBillIncome In mobjBillDetail.InComes
                ...
                With mobjBillDetail
                    ...
                    '收集卫料发料部门,以便自动发料
                    If Not (gbytBilling = 1 Or mblnSavePrice) And gbln自动发料 Then
                ...
                gstrSQL = gstrSQL & _
                    ...
                    ",Null,Null,'|" & mobjBill.煎法 & "', " & strTmp & ",NULL,'" & mobjBillDetail.Detail.类型 & "',0," & mobjBill.领药部门ID & "," & str中药形态 & ",-1,0," & _
                    IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
...
'      bytType=2:普通记帐单,3-自动记帐单
    ...
    If blnDelete Then
        ...
        blnYP = zlCheckPrivs(mstrPrivsOpt, "药品销帐")
        blnZL = zlCheckPrivs(mstrPrivsOpt, "诊疗销帐")
        blnWC = zlCheckPrivs(mstrPrivsOpt, "卫材销帐")
    ...
    '费别
    cbo费别.ListIndex = GetCboIndex(cbo费别, Nvl(rsTmp!费别))
    ...
    cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, str医疗付款)
...
'功能：保存当前修改的费用单据
    ...
    strSql = "zl_病人费用记录_Update('" & cboNO.Text & "',2,'" & NeedName(cbo开单人.Text) & "'," & _
...
Private Sub txtIn_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        Set tmpBill = ImportBill(txtIn.Text, False, Me, False, gbln住院单位)
        If tmpBill.NO = "" Then
...
Public Sub SetStatuPatiInfor(ByVal dbl预交 As Double, dblFee As Double, dbl剩余 As Double, Optional dbl应收 As Double = 0)
    ...
    strTemp = "预交:" & Format(Val(cmdOK.Tag), "0.00")
