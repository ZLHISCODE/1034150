...
Private Const HeadApplyDetail = "执行状态,4,1000|婴儿费,4,600|NO,4,1000|发生时间,1,2100|执行科室,1,1200|开单科室,1,1200|单价,7,1250|付数,7,850|数次,7,850|应收金额,7,1050|实收金额,7,1050|销帐数量,7,1000|销帐金额,7,1000|原始销帐数量,7,0|原始销帐金额,7,0"
Private Const HeadAppliedDetail = "NO,4,1000|发生时间,1,2100|执行科室,1,1200|开单科室,1,1200|销帐数量,7,1000|销帐金额,7,1000"
Private Const HeadAuditDetail = "NO,4,1000|发生时间,1,2100|开单科室,1,1200|销帐数量,7,1000"
Private Const HeadAuditedDetail = "NO,4,1000|发生时间,1,2100|开单科室,1,1200|销帐数量,7,1000"
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    ElseIf KeyCode = vbKeyF6 Then  '定位到病人输入框
        txtPatient.SetFocus
...
Private Sub InitData()
    ...
    On Error Resume Next
    If mbytFun = E申请 Then
        dtpApplyB.Value = CDate(zlDatabase.GetPara("费用开始时间", glngSys, mlngModul, Format(dtpApplyB.Value, "YYYY-MM-DD HH:MM:SS"), Array(dtpApplyB, dtpApplyE), zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")))
        chkDate.Value = IIf(zlDatabase.GetPara("忽略期间", glngSys, mlngModul, "0", Array(chkDate), zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")) = "0", 0, 1)
        ...
        '59051
        chkDateAudit.Value = IIf(zlDatabase.GetPara("申请明细忽略期间", glngSys, Enum_Inside_Program.p记帐操作, "0", Array(chkDateAudit), zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")) = "0", 0, 1)
    Else
        dtpAuditB.Value = zlDatabase.GetPara("审核开始时间", glngSys, mlngModul, Format(dtpAuditB.Value, "YYYY-MM-DD HH:MM:SS"), Array(dtpAuditB, dtpApplyE), zlCheckPrivs(mstrPrivsOpt, "记帐选项设置"))
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If mbytFun = E申请 Then
        zlDatabase.SetPara "费用开始时间", Format(dtpApplyB.Value, "YYYY-MM-DD HH:MM:SS"), glngSys, mlngModul, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
        zlDatabase.SetPara "忽略期间", chkDate.Value, glngSys, mlngModul
        zlDatabase.SetPara "项目显示方式", IIf(chk项目(0).Value = 1 And chk项目(1).Value = 0, 1, IIf(chk项目(0).Value = 0 And chk项目(1).Value = 1, 2, 0)), glngSys, mlngModul, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
        zlDatabase.SetPara "销帐申请同时审核", IIf(chkVerfy.Value = 1, 1, 0), glngSys, Enum_Inside_Program.p记帐操作, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
        zlDatabase.SetPara "申请明细忽略期间", chkDateAudit.Value, glngSys, Enum_Inside_Program.p记帐操作, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
        zlDatabase.SetPara "显示他科执行费用", IIf(chkShowOthers.Value = 1, 1, 0), glngSys, Enum_Inside_Program.p记帐操作, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
    Else
        zlDatabase.SetPara "审核开始时间", Format(dtpAuditB.Value, "YYYY-MM-DD HH:MM:SS"), glngSys, mlngModul, zlCheckPrivs(mstrPrivsOpt, "记帐选项设置")
    End If
...
'    End If
'    strSQL = "Select Distinct A.ID,A.编码 ,B.名称 ,A.规格 " & _
'          " From 收费项目目录 A,收费项目别名 B Where A.id=B.收费细目ID " & strIF & _
'          " And rownum<101 Order by 名称"
'
'    vRect = GetControlRect(txtFeeItem.hwnd)
'    Set rsTmp = zlDatabase.ShowSQLSelect(Me, strSQL, 0, "销帐费用项目", 1, "", "请选择", False, False, True, vRect.Left, vRect.Top, txtFeeItem.Height, blnCancel, False, True, strMatch & strInput & "%")
'
'    If Not rsTmp Is Nothing Then
'        txtFeeItem.Text = rsTmp!名称
'        Call LoadMainData(rsTmp!ID)
'        stbThis.Panels(2).Text = ""
'    Else
'        stbThis.Panels(2).Text = "未找到该项目!"
'    End If
...
Private Function zlSelectItem(ByVal strKey As String) As Boolean
    ...
    If mlngDeptID <> 0 Then
        If mbytUseType <> 1 Then
            If Not mblnOperatorICU Then
                strIF = strIF & " And Instr(','||[6]||',',','||A.病人病区id||',')>0"
                ...
                '问题:36462
                strIF = strIF & " And (exists(select 1 From 部门性质说明 where A.开单部门id=部门ID And 工作性质='临床') And " & _
                         "       (Instr(',5,6,7,', ',' || A.收费类别 || ',') > 0 Or (A.收费类别 = '4' And Nvl(C.跟踪在用, 0) = 1)) Or " & _
                         "       (Instr(',5,6,7,', ',' || A.收费类别 || ',') = 0 Or A.收费类别 = '4' And Nvl(C.跟踪在用, 0) = 0))"
            ElseIf Not mblnPatiDeptICU Then
                '以当时病人科室是否为ICU部分:42526
                strIF = strIF & "  And (  exists(Select 1 From  部门性质说明 J1  Where A.病人科室ID=J1.部门ID And J1.工作性质='ICU') "
                strIF = strIF & "  or  (exists(select 1 From 部门性质说明 where A.开单部门id=部门ID And 工作性质='临床') And " & _
                         "       (Instr(',5,6,7,', ',' || A.收费类别 || ',') > 0 Or (A.收费类别 = '4' And Nvl(C.跟踪在用, 0) = 1)) Or " & _
                         "       (Instr(',5,6,7,', ',' || A.收费类别 || ',') = 0 Or A.收费类别 = '4' And Nvl(C.跟踪在用, 0) = 0)) )"
            End If
        Else
    ...
    strDosage = " And Not Exists (Select  Rownum as 序号 From 住院费用记录 J, 药品收发记录 B1, 输液配药内容 C1 Where j.NO = a.NO And a.记录性质 = j.记录性质 And  nvl(A.价格父号, A.序号) = Nvl(J.价格父号, J.序号) And B1.费用id = j.ID And B1.ID = C1.收发id And instr( ',8,9,10,21,24,25,26,',','||B1.单据||',')>0)  "
    
'    '问题:29887,55380
    Dim blnYP As Boolean, blnZL As Boolean, blnWC As Boolean
    blnYP = zlCheckPrivs(mstrPrivsOpt, "药品销帐申请")
    blnZL = zlCheckPrivs(mstrPrivsOpt, "诊疗销帐申请")
    blnWC = zlCheckPrivs(mstrPrivsOpt, "卫材销帐申请")
  ...
  If blnYP And blnWC And blnZL Then
    ...
    If chk项目(0).Value = 1 And chk项目(1).Value = 0 Then '只显示已执行的
        strWhere = "     And Exists (  Select 1 From 住院费用记录 B  Where A.NO = B.NO And A.记录性质 = B.记录性质 And Nvl(A.价格父号, A.序号) = Nvl(B.价格父号, B.序号)  And B.执行状态 <> 0 )" & vbNewLine
    ...
    If strKey <> "" Then
        ...
        If zlCommFun.IsNumOrChar(strKey) Then
            strIF = strIF & vbCrLf & " And  exists(Select 1 From  收费项目目录 Q1,收费项目别名 Q2 " & vbCrLf & _
            "                                      where Q1.ID=Q2.收费细目ID and A.收费细目id=Q1.id And (Q1.编码 like upper([7]) or ( Q2.简码 like upper([7]) and Q2.码类 in (3," & gbytCode + 1 & "))))  "
        Else
            strIF = strIF & vbCrLf & " And  exists(Select 1 From  收费项目目录 Q1,收费项目别名 Q2 " & vbCrLf & _
    ...
    '未结帐的(结帐并作废当未结账)
    strIF = strIF & " And (A.NO, Nvl(A.价格父号, A.序号)) In (Select A.No ,Nvl(A.价格父号, A.序号)" & vbNewLine & _
    ...
    strSQLDetail = "Select Max(ID) ID, NO, 发生时间, 序号, 执行部门id,开单部门id, 收费类别, 收费细目id, Avg(单价) 单价," & vbNewLine & _
            ...
            "       From 住院费用记录 A, 材料特性 C" & vbNewLine & _
            "       Where A.收费细目id = C.材料id(+)  " & strDosage & vbNewLine & strWhere & _
            "             And A.记录性质 = 2 " & vbCrLf & strIF & vbNewLine & _
    ...
    '可申请销帐的明细
    strSql = "" & _
    "   Select Distinct  M1.ID,M1.编码,M2.名称,M1.规格 " & _
    "   From (" & strSQLDetail & ") A, 病人费用销帐 C,收费项目目录 M1,收费项目别名 M2" & vbNewLine & _
    "   Where   A.收费细目ID=M1.ID And A.收费细目ID=M2.收费细目ID and A.ID = C.费用id(+) And  C.状态(+) = 0"
    
    
    vRect = GetControlRect(txtFeeItem.hWnd)
    Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSql, 0, "销帐费用项目", 1, " ", "请选择", False, False, True, vRect.Left, vRect.Top, txtFeeItem.Height, blnCancel, False, True, _
...
Private Function zlApplyToVerify(ByRef str费用ID As String) As Boolean
   ...
   ' strTable = "select * From (" & strTable & ")"
    ...
    "   Select    /*+ RULE */  A.费用ID ID,A.审核部门ID,A.申请类别, To_Char(A.申请时间,'YYYY-MM-DD HH24:MI:SS') 申请时间,A.数量," & _
    "               B.NO,  B.序号, B.记录性质, G.险类, A.状态,  B.姓名, B.性别,b.操作员姓名,B.登记时间" & _
    "   From 住院费用记录 B, 病案主页 G, 病人余额 Y, 病人费用销帐 A ,C1" & vbNewLine & _
    "   Where   A.费用id = B.ID and A.费用ID=C1.ID " & vbNewLine & _
    "               And B.病人id = G.病人id And B.主页id = G.主页id   " & _
    ...
    Do While Not rsTemp.EOF
            If zlCheckFeeIsValied(Val(Nvl(rsTemp!ID)), Val(Nvl(rsTemp!审核部门id)), Val(Nvl(rsTemp!申请类别))) = False Then Exit Function
            ' 申请类别_In 病人费用销帐.申请类别%Type := 1 --对药品和卫材有效,缺省为已执行的药品或卫材
            gstrSQL = "zl_病人费用销帐_Audit(" & rsTemp!ID & ",To_Date('" & rsTemp!申请时间 & "','YYYY-MM-DD HH24:MI:SS'),'" & _
                UserInfo.姓名 & "',to_date('" & rsTemp!申请时间 & "','YYYY-MM-DD HH24:MI:SS'),1,1," & Val(Nvl(rsTemp!申请类别)) & ")"
            zlAddArray cllPro, gstrSQL
            '问题:49206 --操作状态_In:0-表示直接销帐;1-表示审核销帐(通过销帐申请-->销帐审核流程)
            gstrSQL = "ZL_住院记帐记录_Delete('" & rsTemp!NO & "','" & rsTemp!序号 & ":" & rsTemp!数量 & "','" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & rsTemp!记录性质 & ",1)"
            zlAddArray cllPro, gstrSQL
            
            If Not IsNull(rsTemp!险类) And InStr(1, strMCNO, rsTemp!NO) = 0 Then
                    ...
                    strMCNO = strMCNO & IIf(strMCNO = "", "", "|") & Nvl(rsTemp!NO) & "," & Val(Nvl(rsTemp!险类)) & _
                            "," & IIf(MCPAR.记帐作废上传, "1", "0") & "," & IIf(MCPAR.记帐完成后上传, "1", "0")
            End If
            
            If InStr("," & strNos, "," & Nvl(rsTemp!NO)) = 0 Then
                strNos = strNos & "," & Nvl(rsTemp!NO) & "|" & Format(rsTemp!登记时间, "YYYY-MM-DD HH:MM:SS") & "|" & rsTemp!操作员姓名
            End If
        rsTemp.MoveNext
    Loop
    If strNos <> "" Then    '销帐审核时
        arrNO = Split(Mid(strNos, 2), ",")
        For i = 0 To UBound(arrNO)
            If Not BillOperCheck(5, CStr(Split(arrNO(i), "|")(2)), CDate(Split(arrNO(i), "|")(1)), "销帐审核", _
...
'参数:bytType=0-待审核记录,1-已审核记录
    ...
    '77686,李南春,2014/9/18,单据类别限制
    strDosage = " And Not Exists (Select  RowNum as 序号 From 药品收发记录 B1, 输液配药内容 C1 Where B1.费用id = B.ID And B1.ID = C1.收发id And instr( ',8,9,10,21,24,25,26,',','||B1.单据||',')>0) "
    '明细记录
    strDetail = "Select A.费用ID ID, B.序号, B.记录性质, A.状态,  B.姓名, B.性别, G.险类, F.名称 类别, D.名称 病人病区, G.出院病床 床号, E.名称 开单科室, A.收费细目id," & vbNewLine & _
                "       C.名称 As 项目名称, C.规格, Nvl(X.住院单位,C.计算单位) as 单位,B.NO, B.发生时间, B.登记时间, B.操作员姓名, A.数量/Nvl(X.住院包装,1) 销帐数量,A.数量 售价销帐数量,A.数量*Nvl(B.实收金额,0)/B.付数/B.数次 As 销帐金额 ," & vbNewLine & _
                " A.申请人, To_Char(A.申请时间,'YYYY-MM-DD HH24:MI:SS') 申请时间, C.产地, X.药品来源,B.结帐ID,B.执行状态,B.执行部门ID,A.审核部门ID,A.申请类别" & vbNewLine & _
                "From 住院费用记录 B, 病案主页 G, 病人余额 Y, 病人费用销帐 A, 收费项目目录 C, 药品规格 X, 部门表 D, 部门表 E, 收费项目类别 F" & vbNewLine & _
                "Where A.费用id = B.ID And B.收费细目id = C.ID And B.病人病区id = D.ID And B.开单部门ID = E.ID " & vbNewLine & _
                "      And B.收费类别 = F.编码 And B.病人id = G.病人id And B.主页id = G.主页id And A.收费细目ID=X.药品ID(+) And B.病人ID=Y.病人ID(+) And Y.性质(+)=1 And Y.类型(+)=2  " & strDosage & strIF
    ...
    strSql = "Select " & strFirstCol & "姓名, 性别, 病人病区, 床号, 类别, 项目名称,收费细目ID, 规格, 单位,申请类别, Sum(销帐数量) 销帐数量, Sum(销帐金额) 销帐金额, 申请人, 申请时间, 产地, 药品来源" & vbNewLine & _
            "From (" & strDetail & ")" & vbNewLine & _
            "Group by 收费细目id,申请类别, 状态, 姓名, 性别, 病人病区, 床号, 类别, 项目名称, 规格, 单位, 申请人, 申请时间, 产地, 药品来源" & vbNewLine & _
            "Order by 申请时间 Desc,姓名"
...
'功能：读取已申请的销帐单
    ...
    Select Case Val(cboState.ItemData(cboState.ListIndex))
        ...
        Case ESTATE.E未审核
            strFirstCol = "' ' 状态,"
            '问题:42716
            strIF = strIF & " And A.状态 = 0 " & IIf(chkOtherOperator.Value = 1 And chkOtherOperator.Visible, "", " And A.申请人 = [5]")
        Case ESTATE.E审核通过
            strFirstCol = "'√' 状态,"
    ...
    strDosage = " And Not Exists (Select  RowNum as 序号 From 药品收发记录 B1, 输液配药内容 C1 Where B1.费用id = B.ID And B1.ID = C1.收发id And instr( ',8,9,10,21,24,25,26,',','||B1.单据||',')>0)  "
    '明细记录
    strDetail = "Select A.费用ID ID, A.状态, B.姓名, B.性别, F.名称 类别,B.收费类别, A.收费细目id, C.名称 As 项目名称, C.规格," & vbNewLine & _
            "       Nvl(X.住院单位,C.计算单位) as 单位,B.NO, B.发生时间, D.名称 执行科室,E.名称 开单科室," & vbNewLine & _
            "       A.数量/Nvl(X.住院包装,1) 销帐数量,A.数量*nvl(B.标准单价,0) as 销帐金额, A.申请人, To_Char(A.申请时间,'YYYY-MM-DD HH24:MI:SS') 申请时间,C.产地, X.药品来源,B.医嘱序号" & vbNewLine & _
            "From 病人费用销帐 A, 住院费用记录 B, 收费项目目录 C, 收费项目类别 F, 药品规格 X, 部门表 D, 部门表 E" & vbNewLine & _
            "Where A.费用id = B.ID And A.收费细目id = C.ID And B.执行部门id = D.ID And B.开单部门id = E.ID And B.收费类别 = F.编码 And A.收费细目ID=X.药品ID(+)" & strDosage & strIF
    Set mrsApplied = zlDatabase.OpenSQLRecord(strDetail, Me.Caption, DatBegin, DatEnd, mlngDeptID, mstrUnitIDs, UserInfo.姓名, lng病人ID)
     
    strSql = "Select " & strFirstCol & " 姓名, 性别, 类别, 项目名称,收费细目ID, 规格, 单位, Sum(销帐数量) 销帐数量,sum(销帐金额) as 销帐金额, 申请人, 申请时间, 产地, 药品来源" & vbNewLine & _
            "From (" & strDetail & ")" & vbNewLine & _
...
Private Sub LoadApplyData(ByVal lngFeeItemID As Long, Optional ByVal lngAdviceID As Long, _
    ...
    Dim strTable As String, bln含婴儿费 As Boolean
    ...
    If lngFeeItemID <> 0 Then
        ...
        If Not blnVsfEmpt Then
            If CheckExistFeeItem(lngFeeItemID) Then
                If MsgBox("输入的销帐项目已存在于列表中,你要清除列表中的内容,只显示该项目吗?", vbInformation + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
                    blnAppend = False
                Else
                    Exit Sub
                End If
    ...
    'strIF = " And A.病人id" & IIf(chkDate.Value = 0, "+0", "") & " = [1] And A.记录状态 > 0"
        ...
        If mbytUseType <> 1 Then
            ...
            If Not mblnOperatorICU Then
                ...
                '       不再用病人科室ID=开单科室ID来判断是否为临床开的单了
                 'exists(select 1 From 部门性质说明 where A.开单部门id=部门ID And 工作性质='临床')
                strIF = strIF & _
                    " And ( exists(select 1 From 部门性质说明 where A.开单部门id=部门ID And 工作性质='临床') And  (Instr(',5,6,7,', ',' || A.收费类别 || ',') > 0 Or (A.收费类别 = '4' And Nvl(C.跟踪在用, 0) = 1))  " & _
    ...
    If lngAdviceID <> 0 Then
        strIF = strIF & " And A.NO In (Select Distinct a.No From 病人医嘱发送 A, 病人医嘱记录 B Where a.医嘱id = b.Id And (b.Id = [10] Or b.相关id = [10]))"
    End If
    If strNo <> "" Then
    ...
    strTable = " " & _
    "       Select Max(Decode(Sign(A.执行状态), -1, 0, Decode(A.价格父号, Null, A.ID, 0))) ID, " & _
    "               A.执行状态,nvl(A.婴儿费,0) as 婴儿费, A.发生时间, Max(A.登记时间) 登记时间, " & vbNewLine & _
    "              Max(Decode(Sign(A.执行状态), -1, Null,A.操作员姓名)) 操作员姓名, A.NO," & vbNewLine & _
    "              Nvl(A.价格父号, A.序号) As 序号, A.执行部门id,A.开单部门id, A.收费类别, A.收费细目id, Avg(A.标准单价) 单价," & vbNewLine & _
    "              Avg(A.付数) 付数, Avg(A.数次) 数次, Sum(A.应收金额) 应收金额, Sum(A.实收金额) 实收金额, A.结帐ID, A.医嘱序号  " & vbNewLine & _
    "       From 住院费用记录 A, 材料特性 C " & vbNewLine & _
    "       Where A.收费细目id = C.材料id(+) And  A.记录性质 = 2  " & strDosage & _
    "                   And Exists (  Select 1 From 住院费用记录 B  Where A.NO = B.NO And A.记录性质 = B.记录性质 And Nvl(A.价格父号, A.序号) = Nvl(B.价格父号, B.序号)  And B.执行状态 <> 0 )" & vbNewLine & _
    "                   And ( A.收费类别 in ('5','6','7') or (A.收费类别='4' and  nvl(C.跟踪在用,0) = 1)) " & vbCrLf & strWhereExists & _
                         strIF & vbNewLine & _
    "       Group By A.NO,A.执行状态,nvl(A.婴儿费,0),  Nvl(A.价格父号, A.序号),A.发生时间, A.执行部门id,A.开单部门id, A.收费类别, A.收费细目id,A.结帐ID,  A.医嘱序号 "
    
    strTable = "" & _
    " Select Max(ID) ID, NO, 发生时间, Max(登记时间) 登记时间, Max(操作员姓名) as 操作员姓名,max(婴儿费) as 婴儿费, 序号,  " & _
    ...
    If chk项目(0).Value = 1 Or (chk项目(0).Value = 0 And chk项目(1).Value = 0) Then
            ...
            "      Select C1.ID,-1 as 执行状态,max(C1.婴儿费) as 婴儿费,C1.发生时间,C1.登记时间,C1.操作员姓名,C1.NO,C1.序号,C1.执行部门id,C1.开单部门id, C1.收费类别, C1.收费细目id, max(C1.单价) 单价," & _
            ...
            "      Group By C1.ID,C1.发生时间,C1.登记时间,C1.操作员姓名,C1.NO,C1.序号,C1.执行部门id,C1.开单部门id, C1.收费类别, C1.收费细目id,C1.结帐ID, C1.医嘱序号"
     End If
    If chk项目(1).Value = 1 Or (chk项目(0).Value = 0 And chk项目(1).Value = 0) Then
        strSql = strSql & " Union ALL " & _
        "      Select C1.ID,0 as 执行状态,max(C1.婴儿费) as 婴儿费 ,C1.发生时间,C1.登记时间,C1.操作员姓名,C1.NO,C1.序号,C1.执行部门id,C1.开单部门id, C1.收费类别, C1.收费细目id, max(C1.单价) 单价," & _
    ...
    strSQLDetail = "" & _
            " Select Max(ID) ID,现执行状态 as 执行状态,max(婴儿费) as 婴儿费,药品卫材, NO, 发生时间, Max(登记时间) 登记时间, Max(操作员姓名) as 操作员姓名, 序号, 执行部门id,开单部门id, 收费类别, 收费细目id, Avg(单价) 单价," & vbNewLine & _
            "       Decode(Sign(Min(执行状态)), -1, 1, Sum(付数)) 付数," & vbNewLine & _
            "       Decode(Sign(Min(执行状态)), -1, Sum(付数 * 数次), Sum(数次)) 数次, Sum(应收金额) 应收金额, Sum(实收金额) 实收金额, Max(结帐ID) as 结帐ID, 医嘱序号" & vbNewLine & _
            " From (Select Max(Decode(Sign(A.执行状态), -1, 0, Decode(A.价格父号, Null, A.ID, 0))) ID, A.执行状态 ,max(nvl(A.婴儿费,0)) as 婴儿费, A.发生时间, Max(A.登记时间) 登记时间, " & vbNewLine & _
            ...
            "       From 住院费用记录 A, 材料特性 C" & vbNewLine & _
            "       Where A.收费细目id = C.材料id(+) " & vbNewLine & strDosage & strWhere & _
            "                   And A.记录性质 = 2 " & strWhereExists & strIF & vbNewLine & _
...
'            "            And   (A.收费类别 Not In ('4', '5', '6', '7') Or A.收费类别 = '4' And C.跟踪在用 = 0 Or" & vbNewLine & _
    ...
    strSql = "" & _
            "    Select A.ID,A.执行状态,婴儿费, A.NO, A.序号, A.发生时间, A.登记时间, A.操作员姓名,  " & _
            ...
            "       A.应收金额, A.实收金额, Nvl(C.数量, 0)/Nvl(X.住院包装,1) 销帐数量,nvl(C.数量,0)*A.单价 as 销帐金额,Nvl(X.住院包装,1) 住院包装, A.结帐ID, A.医嘱序号" & vbNewLine & _
            "   From (" & strSQLDetail & ") A, 病人费用销帐 C,药品规格 X, 部门表 B, 部门表 D" & vbNewLine & _
            "   Where A.执行部门id = B.ID And A.开单部门id = D.ID And A.ID = C.费用id(+) and decode(A.药品卫材,1,A.执行状态,0)=C.申请类别(+) And A.收费细目ID=X.药品ID(+) And" & vbNewLine & _
            "           C.状态(+) = 0" & strWhereOthers
    ...
    '明细按收费细目汇总
    strSql = "" & _
            "   Select A.收费细目ID, C.名称 类别, C.编码 收费类别, B.名称 项目名称, B.规格, Nvl(X.住院单位,B.计算单位) as 单位,B.产地, X.药品来源," & vbNewLine & _
            "           Sum(A.付数 * A.数次/Nvl(X.住院包装,1)) 数量, Sum(Nvl(D.数量/Nvl(X.住院包装,1), 0)) 销帐数量,sum(Nvl(D.数量,0)*nvl(A.单价,0)) as 销帐金额 " & vbNewLine & _
            "   From (" & strSQLDetail & ") A, 收费项目目录 B, 收费项目类别 C, 病人费用销帐 D, 药品规格 X" & vbNewLine & _
            "   Where A.收费细目ID = B.ID And A.收费类别 = C.编码 And A.ID = D.费用id(+) And D.状态(+) = 0  and decode(A.药品卫材,1,A.执行状态,0)=D.申请类别(+)  And A.收费细目ID=X.药品ID(+)" & strWhereOthers & vbNewLine & _
            "   Group By A.收费细目ID,A.收费类别,C.名称, C.编码, B.名称, B.规格, Nvl(X.住院单位,B.计算单位),B.产地, X.药品来源"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, Val(mrsInfo!病人ID), mlngDeptID, lngFeeItemID, DatBegin, DatEnd, mstrUnitIDs, lng主页ID, intBaby - 1, UserInfo.ID, lngAdviceID, strNo, lngSerial)
...
Private Sub ShowDetail(ByVal lngFeeItem As Long)
    ...
    rsTmp.Sort = IIf(tbsType.SelectedItem.Key = "T1", "执行状态,", "") & "发生时间 Desc"
    If rsTmp.RecordCount = 0 Then Exit Sub
            ...
            Else
                ...
                Else
                    ...
                    If Not IsNull(!结帐id) Then
                        If CheckBalance(!NO, !序号) Then    '目前未结帐的没有提取出来,下面的程序暂时没有使用
                            If Not IsNull(mrsInfo!险类) Then
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    With mrsAudited
        ...
        .Filter = "收费细目ID=" & vsfMain(1).RowData(intRow) & " And 申请类别=" & Val(vsfMain(1).Cell(flexcpData, intRow, ColAudited("状态"))) & " And 申请人='" & vsfMain(1).TextMatrix(intRow, ColAudited("申请人")) & _
                "' And 申请时间='" & vsfMain(1).TextMatrix(intRow, ColAudited("申请时间")) & "'"
        If blnCancel = True Then
            If .RecordCount <> 0 Then
                strSql = "zl_病人费用销帐_Cancel(" & !ID & ",To_Date('" & !申请时间 & "','YYYY-MM-DD HH24:MI:SS'),'" & _
                    UserInfo.姓名 & "'," & strDate & "," & IIf(blnCancel, "1", "0") & ",1," & Val(vsfMain(1).Cell(flexcpData, intRow, ColAudited("状态"))) & ")"
        ...
        Else
            If .RecordCount <> 0 Then
                ...
                arrSQL(UBound(arrSQL)) = "zl_病人费用销帐_Cancel(" & !ID & ",To_Date('" & !申请时间 & "','YYYY-MM-DD HH24:MI:SS'),'" & _
                    UserInfo.姓名 & "'," & strDate & "," & IIf(blnCancel, "1", "0") & ",1," & Val(vsfMain(1).Cell(flexcpData, intRow, ColAudited("状态"))) & ")"
                
                ReDim Preserve arrSQL(UBound(arrSQL) + 1)
                arrSQL(UBound(arrSQL)) = "ZL_住院记帐记录_Delete('" & !NO & "','" & !序号 & ":" & !售价销帐数量 & "','" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & !记录性质 & ",1)"
                ...
                If InStr("," & strNos, "," & !NO) = 0 Then
                    strNos = strNos & "," & !NO & "|" & Format(!登记时间, "YYYY-MM-DD HH:MM:SS") & "|" & !操作员姓名
                End If
                
                If strNos <> "" Then    '销帐申请和审核时
                    arrNO = Split(Mid(strNos, 2), ",")
                    For i = 0 To UBound(arrNO)
                        If Not BillOperCheck(5, CStr(Split(arrNO(i), "|")(2)), CDate(Split(arrNO(i), "|")(1)), IIf(mbytFun = E申请, "销帐申请", "销帐审核"), _
                            Split(arrNO(i), "|")(0), , 2, , False, False) Then Exit Function
                    Next
                End If
...
Private Function 药品存在配药中心(ByVal lng医嘱ID As Long) As Boolean
    ...
    gstrSQL = "Select 1 " & _
             ...
             "   Where A.相关id = B.医嘱id And B.医嘱id = D.医嘱id And B.发送号 = D.发送号 And A.ID = [1] And Rownum =1"
     Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng医嘱ID)
    药品存在配药中心 = rsTemp.RecordCount <> 0
...
'功能:检查待审核的费用项目的单据明细行是否已结帐,已结帐的不允许审核(冲销)
    ...
    CheckCanAudit = True
    With mrsAudit
        .Filter = "收费细目id=" & lngFeeItemID & " And 姓名='" & strPatient & "' And 申请人='" & strOperater & "' And 申请时间='" & strDate & "'"
        For i = 1 To .RecordCount
            If Not IsNull(!结帐id) Then
                If CheckBalance(!NO, !序号) Then
                    If Not IsNull(!险类) Then
                        If Not gclsInsure.GetCapability(support允许冲销已结帐的记帐单据, , Val(!险类)) Then
                            MsgBox "不允许冲销医保病人已结帐的单据[" & !NO & "]."
                            CheckCanAudit = False
                            Exit For
                        End If
                    Else
...
Private Sub ShowTogetherMedi(ByVal lngAdviceID As Long, ByVal lngFeeItemID As Long)
    ...
    strSql = "Select 1" & vbNewLine & _
            ...
            "Where A.ID = [1] And A.医嘱序号 is Not Null And A.NO = B.NO And A.记录性质 = B.记录性质 And A.记录状态 = B.记录状态 And A.执行状态 = B.执行状态 And" & vbNewLine & _
            "      A.收费细目id = B.收费细目id And A.登记时间 = B.登记时间 Having Count(A.ID) > 1"
        ...
        Else
            ...
            If mrsApply.RecordCount > 0 Then
                ...
                If Not IsNull(mrsApply!结帐id) Then '目前未结帐的没有提取出来,下面的程序暂时没有使用
                    If CheckBalance(mrsApply!NO, mrsApply!序号) Then
                        If Not IsNull(mrsInfo!险类) Then
    ...
    If mbytFun = E申请 Then
        If tbsType.SelectedItem.Key = "T1" Then
            With mrsApply
                ...
                For i = 1 To .RecordCount
                    If !销帐数量 <> !原始销帐数量 Then
                        str费用IDs = str费用IDs & IIf(str费用IDs = "", "", ",") & !ID
                ...
                If str费用IDs = "" Then
                    stbThis.Panels(2).Text = "所有记录都没有填写销帐数量!"
                    Exit Function
                End If
            End With
        Else
    ...
    If mbytFun = E申请 Then
            ...
            With mrsApply
                ...
                For i = 1 To .RecordCount
                    If InStr(1, "," & str费用IDs & ",", "," & !ID & ",") > 0 Then
                        ...
                        '  状态_In       住院费用记录.执行状态%Type := Null --对药品和卫材有效:0-未发药(料);1-已发药(料);NULL
                        '   删除标志_In   INTEGER :=0 --删除病人费用销帐时的条件:1-删除时不管申请类别,0-删除时,根据申请类别来进行删除(因为可能出现在申请销帐时,存在已执行和未执行两种状态)
                        arrSQL(UBound(arrSQL)) = "zl_病人费用销帐_Insert(" & !ID & "," & !收费细目ID & "," & _
                                mlngDeptID & "," & dbl数量 & ",'" & UserInfo.姓名 & "'," & strDate & "," & Val(Nvl(!执行状态)) & "," & _
                              IIf(InStr(1, "," & strKey费用IDs & ",", "," & Nvl(!ID) & ",") > 0, 0, 1) & ")"
        ...
        Else
            With mrsApplied
                For i = 1 To vsfMain(1).Rows - 1
                    If vsfMain(1).TextMatrix(i, ColApplied("选择")) = "√" Then
                        .Filter = "收费细目ID=" & vsfMain(1).RowData(i) & " And 申请人='" & vsfMain(1).TextMatrix(i, ColApplied("申请人")) & _
            ...
            While str费用IDs <> ""
                ...
                ReDim Preserve arrSQL(UBound(arrSQL) + 1)
                arrSQL(UBound(arrSQL)) = "zl_病人费用销帐_Delete('" & str费用ID & "')"
            Wend
        End If
    Else
        If tbsType.SelectedItem.Key = "T1" Then
            ...
            With mrsAudit
                For i = 1 To vsfMain(0).Rows - 1
                    ...
                    If strTmp = "√" Or strTmp = "×" Then
                        .Filter = "收费细目ID=" & vsfMain(0).RowData(i) & " And 申请类别=" & Val(vsfMain(0).Cell(flexcpData, i, ColAudit("审核"))) & " And 申请人='" & vsfMain(0).TextMatrix(i, ColAudit("申请人")) & _
                                "' And 申请时间='" & vsfMain(0).TextMatrix(i, ColAudit("申请时间")) & "'"
                                
                        For j = 1 To .RecordCount
                            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
                            If zlCheckFeeIsValied(Val(Nvl(!ID)), Val(Nvl(!审核部门id)), Val(vsfMain(0).Cell(flexcpData, i, ColAudit("审核")))) = False Then Exit Function
                            ' 申请类别_In 病人费用销帐.申请类别%Type := 1 --对药品和卫材有效,缺省为已执行的药品或卫材
                            arrSQL(UBound(arrSQL)) = "zl_病人费用销帐_Audit(" & !ID & ",To_Date('" & !申请时间 & "','YYYY-MM-DD HH24:MI:SS'),'" & _
                                UserInfo.姓名 & "'," & strDate & "," & IIf(strTmp = "√", "1", "2") & ",1," & Val(vsfMain(0).Cell(flexcpData, i, ColAudit("审核"))) & ")"
...
errH:
    ...
    Call SaveErrLog
End Function
Private Function zlCheckFeeIsValied(ByVal lng费用ID As Long, ByVal lng审核部门ID As Long, Optional int申请类别 As Integer) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查费用销帐是否有效
    '入参:int申请类别-1-已执行;0-未执行
    '出参:
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTemp As New ADODB.Recordset, lng执行部门ID As Long, strNo As String, str项目名称 As String, str部门名称 As String
    
    On Error GoTo errHandle
    
    '之所以要重新读取执行状态，因考虑并发操作这种情况
    gstrSQL = "Select A.NO,a.执行状态,a.执行部门ID,a.收费细目ID,a.收费类别, nvl(b.跟踪在用,0) as 跟踪在用 From 住院费用记录 a ,材料特性 B Where a.收费细目ID=b.材料ID(+) and a.id=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng费用ID)
    If rsTemp.EOF Then
        MsgBox "注意:" & vbCrLf & _
    ...
    gstrSQL = "Select 编码,名称 From 收费项目目录 a Where id=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(Nvl(rsTemp!收费细目ID)))
    If Not rsTemp.EOF Then
        str项目名称 = Nvl(rsTemp!编码) & "-" & Nvl(rsTemp!名称)
    End If
    
    
    gstrSQL = "Select 编码,名称 From 部门表 a Where id=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng执行部门ID)
    If Not rsTemp.EOF Then
        str部门名称 = Nvl(rsTemp!编码) & "-" & Nvl(rsTemp!名称)
    End If
    
    MsgBox "注意:" & vbCrLf & _
           "    单据号为“" & strNo & "”" & vbCrLf & _
           "    收费项目为“" & str项目名称 & "”" & vbCrLf & _
...
End Sub
Private Sub LoadBabyCombox(ByVal lng病人ID As Long)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    intCount = Val(zlDatabase.GetPara("销帐申请婴儿费显示规则", glngSys, Enum_Inside_Program.p记帐操作, "1", Array(cboBaby), InStr(mstrPrivsOpt, ";记帐选项设置;") > 0))
    On Error GoTo errHandle
    With cboBaby
        .Clear
        .AddItem "不包含婴儿费用"
