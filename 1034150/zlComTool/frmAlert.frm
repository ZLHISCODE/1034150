...
Private Declare Function pingnet Lib "pbpingnet.dll" Alias "ping" (ByVal strIP As String) As Boolean
...
Private Sub cmdRefresh_Click()
    ...
    Set rs = zlDatabase.OpenCursor(Me.Caption, "zlTools", "b_ComFunc.Get_Zlnoticerec", gstrUserName)
    If rs.BOF = False Then
        Do While Not rs.EOF
            ...
            vsf.RowData(vsf.Rows - 1) = zlCommFun.NVL(rs("序号").Value, 0)
            
            vsf.TextMatrix(vsf.Rows - 1, 2) = CStr(zlCommFun.NVL(rs("报表系统").Value, 0) & ";" & zlCommFun.NVL(rs("模块").Value, 0))
            vsf.TextMatrix(vsf.Rows - 1, 3) = zlCommFun.NVL(rs("提醒报表").Value)
            vsf.TextMatrix(vsf.Rows - 1, 4) = Format(zlCommFun.NVL(rs("检查时间").Value), "yyyy年MM月dd日 HH时mm分")
            vsf.TextMatrix(vsf.Rows - 1, 5) = zlCommFun.NVL(rs("结果内容").Value)
            
            If zlCommFun.NVL(rs("已读标志").Value, 0) = 0 Then
                vsf.Cell(flexcpFontBold, vsf.Rows - 1, 0, vsf.Rows - 1, vsf.Cols - 1) = True
            End If
            
            If zlCommFun.NVL(rs("提醒报表").Value, "") <> "" Then
...
Private Function ConnectServer() As Boolean
    ...
    '格式:服务器;端口号;状态
    '注销时，用 zlDatabase.OpenCursor(Me.Caption, "zlTools", "b_RunMana.Get_Zloption", 7) 取参数有问题。
    rs.Open "SELECT 参数值 FROM zloptions WHERE 参数号=7", gcnOracle
    If rs.BOF Then Exit Function
    varParam = Split(zlCommFun.NVL(rs("Option_Value").Value, ""), ";")
...
'    '取服务器IP地址
'    rs.Open "SELECT 参数值 FROM zloptions WHERE 参数号=8", gcnOracle
'    If rs.BOF Then Exit Function
'
''    If Ping(Trim(zlCommFun.NVL(rs("参数值").Value, ""))) = False Then Exit Function
'
'    winSock.RemoteHost = Trim(rs("参数值").Value)
'
'    '取服务器配置的端口号
'    If rs.State = adStateOpen Then rs.Close
'    rs.Open "SELECT 参数值 FROM zloptions WHERE 参数号=7", gcnOracle
'    If rs.BOF Then
'        winSock.RemotePort = 9999
'    Else
'        winSock.RemotePort = IIf(IsNull(rs("参数值").Value), 9999, IIf(Val(rs("参数值").Value) < 0, 9999, Val(rs("参数值").Value)))
'    End If
