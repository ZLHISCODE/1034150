...
Private Sub ShowCard(Optional ByVal blnNext As Boolean)
    ...
    With mrsData
        ...
        Do While Not .EOF
            ...
            If Not (lng药品id = IIf(!溶媒 = 1, !溶媒id, !药品id) And Not blnChange) Then
                lng药品id = IIf(!溶媒 = 1, !溶媒id, !药品id)
                intSum = intSum + 1
                IntCount = IntCount + 1
                Me.picDrug(i - 1).Tag = intSum - 1
                lblDrug1(Me.lblDrug1.UBound).Caption = !通用名
                lblSpec1(Me.lblSpec1.UBound).Caption = NVL(!规格)
                lblAmount1(Me.lblAmount1.UBound).Caption = Val(!单量)
...
Private Sub Show医嘱(Optional ByVal blnNext As Boolean)
    ...
    With mrsData
        ...
        Do While Not .EOF
            ...
            lblAmount1(Me.lblAmount1.UBound).Caption = Val(!单量) & NVL(!单位)
            imgOtherDrug(i - 1).Visible = (IntCount > 5)
            imgDrug(i - 1).Visible = (IntCount > 5)
            imgOtherDrug(i - 1).Tag = IntCount
