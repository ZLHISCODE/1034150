...
Private Const mconintcol构指导差价率 As Integer = 8
...
Private Sub Form_Load()
    ...
    mintSelectStock = Val(zlDataBase.GetPara("是否选择库房", glngSys, 1344))
    txtNo = mstr单据号
    txtNo.Tag = txtNo
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            gstrSQL = " SELECT * FROM " & _
                ...
                "    F.是否变价,B.指导差价率/100 AS 指导差价率 ,B.药房分批 AS 药房分批核算 " & _
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol采购价) = FormatEx(rsInitCard!成本价, intCostDigit)
                    .TextMatrix(intRow, mconIntCol采购金额) = GetFormat(rsInitCard!成本金额, intMoneyDigit)
                    .TextMatrix(intRow, mconIntCol售价) = FormatEx(rsInitCard!零售价, intPricedigit)
                    ...
                    .TextMatrix(intRow, mconIntCol原销期) = IIf(IsNull(rsInitCard!最大效期), "0", rsInitCard!最大效期) & "||" & rsInitCard!指导差价率 & "||" & rsInitCard!是否变价 & "||" & rsInitCard!药房分批核算
                    rsInitCard.MoveNext
                Loop
...
Private Sub initGrid()
    ...
    With mshStructure
        ...
        .TextMatrix(0, mconintcol构指导差价率) = "指导差价率"
...
Private Sub mshbill_CommandClick()
    ...
    gstrSQL = "" & _
        ...
        "   ,D.商品名,D.药品来源,D.基本药物,D.规格,D.产地,D.药品ID," & _
             strUnitQuantity & _
        "    S.库存金额, D.最大效期,D.是否变价,D.指导差价率,D.药房分批核算,E.库房货位 " & _
        " FROM  " & _
        ...
        "        D.药库单位,TO_CHAR(D.药库包装, '999999999990.99999') 药库包装,M.是否变价,D.指导差价率,D.药房分批 AS 药房分批核算 " & _
                    ...
                    If strkey <> "" Then
                        ...
                        ElseIf RecReturn.RecordCount = 1 Then
                            If SetColValue(.Row, RecReturn!药品ID, "[" & RecReturn!编码 & "]", RecReturn!通用名称, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), _
                               ...
                               RecReturn!比例系数, RecReturn!是否变价, RecReturn!指导差价率, RecReturn!药房分批核算) = False Then
                               RecReturn.Close
                               Cancel = True
                               Exit Sub
                            End If
                ...
                Case mconIntCol数量
                    ...
                    If strkey <> "" Then
                        ...
                        If GetStructureNum(strkey * .TextMatrix(.Row, mconIntCol比例系数), dblCostPrice) = False Then
                            ...
                            Exit Sub
                        Else
                            .TextMatrix(.Row, mconIntCol采购价) = FormatEx(dblCostPrice * .TextMatrix(.Row, mconIntCol比例系数), mintCostDigit)
                        End If
                                
                        strkey = GetFormat(strkey, mintNumberDigit)
                        .Text = strkey
                        If .TextMatrix(.Row, mconIntCol采购价) <> "" Then
                            .TextMatrix(.Row, mconIntCol采购金额) = GetFormat(.TextMatrix(.Row, mconIntCol采购价) * strkey, mintMoneyDigit)
                            If Split(.TextMatrix(.Row, mconIntCol原销期), "||")(2) = 1 Then
                                .TextMatrix(.Row, mconIntCol售价) = FormatEx(.TextMatrix(.Row, mconIntCol采购价) / (1 - Split(.TextMatrix(.Row, mconIntCol原销期), "||")(1)), mintPriceDigit)
                            End If
                        End If
    ...
    With mshStructure
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                dblConstruct = GetFormat(.TextMatrix(intRow, mconIntCol构组成数量) * dblNum, intNumberDigit)
                If mint库存检查 = 0 Then
                
                ElseIf mint库存检查 = 1 Then
                    If blnContinue = False And bln判断库存 = True Then         '一旦按了继续，后面的条件就不判了
                        If mint编辑状态 = 1 Then
                            If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)), intNumberDigit) Then
                                intReturn = MsgBox("该协定药品的对照药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName)
                                ...
                                End If
                            End If
                        ElseIf mint编辑状态 = 1 Or mint编辑状态 = 2 Then
                            numUseCount = 0
                            For Each vardrug In mcolUseCount
                                If vardrug(0) = .TextMatrix(.Row, 0) & "!" & .TextMatrix(intRow, mconintcol构药品id) Then
                                    numUseCount = vardrug(1)
                                    Exit For
                                End If
                            Next
                            
                            dbl原填写数量 = IIf(mbln下可用数量, numUseCount, 0)
                            
                            If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)) + dbl原填写数量, intNumberDigit) Then
                                intReturn = MsgBox("该协定药品的对照药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName)
                                If intReturn = vbNo Then
                                    Exit Function
                                Else
                ...
                ElseIf mint库存检查 = 2 Then
                    ...
                    numUseCount = 0
                    If Not mcolUseCount Is Nothing Then
                        For Each vardrug In mcolUseCount
                            If vardrug(0) = mshBill.TextMatrix(mshBill.Row, 0) & "!" & .TextMatrix(intRow, mconintcol构药品id) Then
                                numUseCount = vardrug(1)
                                Exit For
                            End If
                        Next
                    ...
                    If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)) + dbl原填写数量, intNumberDigit) Then
                        MsgBox "对不起，该协定药品的对照药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，请检查该原料药的库存！", vbInformation + vbOKOnly, gstrSysName
                        Exit Function
                    End If
                End If
                .TextMatrix(intRow, mconIntCol构数量) = GetFormat(dblConstruct, intNumberDigit)
                .TextMatrix(intRow, mconIntCol构售价金额) = GetFormat(dblConstruct * .TextMatrix(intRow, mconIntCol构售价), intNumberDigit)
                If .TextMatrix(intRow, mconintcol构实际金额) <= "0" Then
'                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) * Split(.TextMatrix(intRow, mconintcol构指导差价率), "||")(0) / 100, intMoneyDigit)
'                    .TextMatrix(intRow, mconIntCol构采购价) = FormatEx((.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconintCol构差价)) / (IIf(dblConstruct = 0, 1, dblConstruct)), intCostDigit)
                    .TextMatrix(intRow, mconIntCol构采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol构采购价) * dblConstruct, intMoneyDigit)
                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(Val(.TextMatrix(intRow, mconIntCol构售价金额)) - Val(.TextMatrix(intRow, mconIntCol构采购金额)), intMoneyDigit)
                Else
'                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) * (.TextMatrix(intRow, mconintcol构实际差价) / .TextMatrix(intRow, mconintcol构实际金额)), intMoneyDigit)
'                    .TextMatrix(intRow, mconIntCol构采购价) = FormatEx((.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconintCol构差价)) / (IIf(dblConstruct = 0, 1, dblConstruct)), intCostDigit)
                    .TextMatrix(intRow, mconIntCol构采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol构采购价) * dblConstruct, intMoneyDigit)
                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(Val(.TextMatrix(intRow, mconIntCol构售价金额)) - Val(.TextMatrix(intRow, mconIntCol构采购金额)), intMoneyDigit)
                End If
                dblPurchase = FormatEx(dblPurchase + .TextMatrix(intRow, mconIntCol构采购金额) / dblNum, intCostDigit)
...
Private Function SetColValue(ByVal intRow As Integer, ByVal int药品id As Long, _
    ...
    ByVal int是否变价 As Integer, ByVal dbl指导差价率 As Double, ByVal int药房分批核算 As Integer) As Boolean
    ...
    With mshBill
        ...
        .TextMatrix(intRow, mconIntCol售价) = FormatEx(num售价, mintPriceDigit)
        .TextMatrix(intRow, mconIntCol比例系数) = num比例系数
        .TextMatrix(intRow, mconIntCol原销期) = IIf(IsNull(int原效期), "0", int原效期) & "||" & dbl指导差价率 / 100 & "||" & int是否变价 & "||" & int药房分批核算
...
Private Function SetStructure(ByVal int药品id As Long) As Boolean
    ...
    If mint编辑状态 <> 4 Then
        gstrSQL = "SELECT DISTINCT B.药品ID,'[' || F.编码 || ']' As 编码,F.名称 As 通用名称,E.名称 AS 商品名称, F.规格, C.上次产地,F.计算单位 AS 单位,c.平均成本价," & _
                  ...
                  " (A.分子 / A.分母) AS 组成,C.可用数量,B.指导差价率,F.是否变价,B.药房分批 AS 药房分批核算, Nvl(F.是否变价, 0) 定价 " & _
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构售价) = FormatEx(rsStructure!售价, mintPriceDigit)
                .TextMatrix(.Row, mconIntCol构可用数量) = GetFormat(IIf(IsNull(rsStructure!可用数量), "0", rsStructure!可用数量), mintNumberDigit)
                .TextMatrix(.Row, mconIntCol构组成数量) = rsStructure!组成
                .TextMatrix(.Row, mconintcol构指导差价率) = rsStructure!指导差价率 & "||" & IIf(IsNull(rsStructure!是否变价), 0, rsStructure!是否变价) & "||" & IIf(IsNull(rsStructure!药房分批核算), 0, rsStructure!药房分批核算)
                ...
                If IsNull(rsStructure!平均成本价) Then
                    ...
                    If rs成本价.RecordCount > 0 Then
                        .TextMatrix(.Row, mconIntCol构采购价) = GetFormat(rs成本价!成本价, mintCostDigit)
                    End If
                Else
                    .TextMatrix(.Row, mconIntCol构采购价) = GetFormat(rsStructure!平均成本价, mintCostDigit)
                End If
    ...
    Else            '查看
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构单位) = rsStructure!单位
                .TextMatrix(.Row, mconIntCol构数量) = GetFormat(rsStructure!实际数量, mintNumberDigit)
                .TextMatrix(.Row, mconIntCol构采购价) = FormatEx(rsStructure!成本价, mintCostDigit)
                .TextMatrix(.Row, mconIntCol构采购金额) = GetFormat(rsStructure!成本金额, mintMoneyDigit)
                .TextMatrix(.Row, mconIntCol构售价) = FormatEx(rsStructure!零售价, mintPriceDigit)
                .TextMatrix(.Row, mconIntCol构售价金额) = GetFormat(rsStructure!零售金额, mintMoneyDigit)
                .TextMatrix(.Row, mconintCol构差价) = GetFormat(rsStructure!差价, mintMoneyDigit)
                .TextMatrix(.Row, mconintcol构药品id) = rsStructure!药品ID
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If chrNo = "" Then chrNo = zlDataBase.GetNextNo(23, lng库房id)
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                dbl填写数量 = GetFormat(.TextMatrix(intRow, mconIntCol数量) * .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_数量)
                dbl成本价 = FormatEx(.TextMatrix(intRow, mconIntCol采购价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_成本价)
                dbl成本金额 = .TextMatrix(intRow, mconIntCol采购金额)
                dbl零售价 = FormatEx(.TextMatrix(intRow, mconIntCol售价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_零售价)
...
Private Sub 显示合计金额()
    ...
    lblPurchasePrice.Caption = "购价金额合计：" & GetFormat(cur购价金额, mintMoneyDigit)
    lblSalePrice.Caption = "售价金额合计：" & GetFormat(Cur售价金额, mintMoneyDigit)
    lblDifference.Caption = "差价合计：" & GetFormat(Cur差价, mintMoneyDigit)
...
Private Sub 提示库存数()
    ...
    staThis.Panels(2).Text = "该药品当前库存数为[" & GetFormat(Dbl数量, mintNumberDigit) & "]" & RecTmp!单位
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
        Resume
