...
Begin VB.Form Frm药品批量发药 
   ...
   Begin TabDlg.SSTab TabShow 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "单据明细(&D)"
      TabPicture(0)   =   "Frm药品批量发药.frx":030A
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Msf待发明细"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "药品汇总(&T)"
      TabPicture(1)   =   "Frm药品批量发药.frx":0326
...
Private Sub GetRecipe(ByVal intType As Integer, ByVal txtInput As TextBox)
    ...
    gstrSQL = " Select Distinct S.名称 As 药房, Decode(C.单据,8,'收费',9,'记帐') 类型,C.No,C.单据,A.已收费,Decode(A.配药人,Null,'','部门发药','',A.配药人) 配药人,P.名称 科室,B.姓名,B.标识号 住院号,'' 床号," & _
             ...
             " From 未发药品记录 A,门诊费用记录 B,药品收发记录 C,部门表 P,部门表 S, 病人信息 D " & _
             " Where C.费用ID=B.ID And B.开单部门ID+0=P.ID And Nvl(C.库房ID,0)+0=S.ID and Nvl(A.库房ID,0)=Nvl(C.库房ID,0)  And A.No=C.No " & _
             IIf(mbln发其他药房处方, "", IIf(Str窗口 = "", "", " And (C.发药窗口 In (Select * From Table(Cast(f_Str2list([7]) As Zltools.t_Strlist))) Or C.发药窗口 Is NULL)")) & _
             " And C.单据 In (8,9) And C.审核人 Is Null " & IIf(mbln发药前收费或审核 = False, "", "and A.已收费=1") & " And Nvl(B.费用状态,0)<>1 " & _
...
Private Sub Ini药房()
    ...
    If IsHavePrivs(strPrivs, "所有药房") Or mbln发其他药房处方 Then
...
Private Sub cmd过滤_Click()
    ...
    gstrSQL = " Select /*+ Rule*/ Distinct S.名称 As 药房, Decode(C.单据,8,'收费',9,'记帐') 类型,C.No,C.单据,A.已收费,Decode(A.配药人,Null,'','部门发药','',A.配药人) 配药人,P.名称 科室,B.姓名,B.标识号 住院号,'' 床号," & _
             " B.开单人 开单医生,B.操作员姓名 填制人,To_Char(C.填制日期,'yyyy-MM-dd') 填制日期, S.ID As 药房id,B.记录性质,B.门诊标志,A.病人ID, d.病人类型 " & _
             " From 未发药品记录 A,门诊费用记录 B,药品收发记录 C,部门表 P,部门表 S, 病人信息 D " & IIf(Str窗口 = "", "", ",Table(Cast(f_Str2list([6]) As zlTools.t_Strlist)) E ") & _
             " Where C.费用ID=B.ID And B.开单部门ID+0=P.ID And Nvl(C.库房ID,0)+0=S.ID " & IIf(mbln发药前收费或审核 = False, "", "and A.已收费=1") & " and Nvl(A.库房ID,0)=Nvl(C.库房ID,0)  And A.No=C.No " & _
            ...
            IIf(IntSendAfterDosage = 0, " And C.配药人 is not null And C.配药日期 is not null", "") & _
             " And C.单据 In (8,9) And C.审核人 Is Null  And Nvl(B.费用状态,0)<>1 " & _
             " and Not Exists(select 1 from 药品收发记录 F where F.单据=C.单据 and F.库房id=C.库房id and F.no=C.no and 发药方式=-1) " & _
             " And C.单据=A.单据 and nvl(C.发药方式,-999)<>-1 And A.病人id=D.病人id(+)  " & strCon
    If Me.chk记账.Value <> 1 Or Me.chk收费.Value <> 1 Then
        If Me.chk记账.Value = 1 Then
            gstrSQL = gstrSQL & " And B.记录性质=2"
        Else
            gstrSQL = gstrSQL & " And B.记录性质=1"
        End If
    End If
...
Private Function Select部门(ByVal FrmMain As Form, ByVal objCtl As Control, ByVal strSearch As String, _
    ...
    For i = 1 To rsTemp.RecordCount
        ...
        strComment = IIf(strComment = "", "", strComment & ",") & NVL(rsTemp!编码) & "-" & NVL(rsTemp!名称)
...
Private Function CheckBill(ByVal intRow As Integer, ByVal IntBillStyle As Integer, ByVal strNo As String, ByVal lng药房id As Long, ByVal int记录性质 As Integer, ByVal int门诊标志 As Integer) As Integer
    ...
    With RecCheck
        ...
        If mint允许未收费处方发药 = 0 And IntBillStyle = 8 And mbln发药前收费或审核 = False Then
            If !已收费 = 0 Then
                CheckBill = 3: MsgBox "该处方[" & strNo & "]还未收费，发药操作中止！", vbInformation, gstrSysName: Exit Function
            End If
        End If
        
        If Int允许未审核处方发药 = 0 And IntBillStyle = 9 And mbln发药前收费或审核 = False Then
            If !已收费 = 0 Then
                CheckBill = 3: MsgBox "该处方[" & strNo & "]还未审核，发药操作中止！", vbInformation, gstrSysName: Exit Function
            End If
        End If
...
Private Function WriteSendListData(ByVal intType As Integer) As Boolean
    ...
    Do While Not (RecBill.EOF)
        ...
        If mint允许未收费处方发药 = 0 And RecBill!单据 = 8 And mbln发药前收费或审核 = False Then
        ...
        If Int允许未审核处方发药 = 0 And RecBill!单据 <> 8 And mbln发药前收费或审核 = False Then
            If RecBill!已收费 = 0 Then
                If intType = 0 Then
                    MsgBox "该处方还未记帐，不能执行发药操作！", vbInformation, gstrSysName
                    Exit Function
                Else
                    blnContinue = False
                End If
...
Private Function RefreshData(ByVal lng药房id As Long) As Boolean
    ...
    If Len(mstr汇总单据) > 4000 Then
        For intRow = 0 To UBound(GetArrayByStr(mstr汇总单据, 3900, "|")) - 1
            
            Set RecTotal = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, GetArrayByStr(mstr汇总单据, 3900, "|")(intRow), lng药房id)
            Call WriteTotalDataToBill(intRow > 0)
        Next
    Else
...
Private Function WriteTotalDataToBill(Optional ByVal blnFirst As Boolean) As Boolean
