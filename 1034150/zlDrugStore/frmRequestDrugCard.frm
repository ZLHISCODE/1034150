...
'        Call SetSelectorRS(2, "药品申领管理", cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), mlngStockID, , , , , , mbln检查库存)
'    End If
'End Sub
    ...
    If mbln下可用数量 Then
        For n = 1 To mshBill.rows - 1
            If Val(mshBill.TextMatrix(n, 0)) <> 0 Then
                If Not CompareUsableQuantity(n, mshBill.TextMatrix(n, mconIntCol填写数量)) Then
                    Exit Function
                End If
            End If
        Next
