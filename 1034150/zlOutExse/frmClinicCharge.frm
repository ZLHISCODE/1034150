...
Private mbytInState As gEM_ChargeEditType '0-执行,1-浏览,2-调整,3-退费(收费、记帐部份退费),4-重新收费;5-异常单据作废
Private mstrInNO As String '操作的单据号(查看，调整，退费，销帐,重新收费时)(暂未用)
...
'结算窗口
Private WithEvents mFrmBalanceWin  As frmClinicChargeBalance
...
Public Function zlEditBill(ByVal frmMain As Object, ByVal lngModule As Long, _
    ...
    '     lngModule-模块号
    '     strPrivs-权限串
    '     bytInState-操作功能(0-执行,1-浏览,2-调整,3-退费(收费、记帐部份退费),4-重新收费;5-异常单据作废)
...
Private Sub cmdSelWholeSet_Click()
    ...
    If frmWholeSelect.ShowSelect(Me, mlngModul, mstrPrivs, rsSel) = False Then Exit Sub
    ...
    Set tmpBill = ImportWholeSet(Me, IIf(mstrYBPati <> "", mintInsure, 0), rsSel, mlng西药房, mlng成药房, mlng中药房, lng病人ID, 0, gbln药房单位, lng开单部门ID, byt婴儿费, 2, chk加班.Value = 1, _
        0, gint病人来源, UserInfo.姓名, NeedName(cbo开单人.Text))
    ...
    If Not cmdAddBill.Enabled Or Not cmdAddBill.Visible Then
        ...
        mobjBill.费别 = NeedName(cbo费别.Text)
        If mobjBill.Pages(1).Details.Count > 0 Then
           If mobjBill.Pages(1).Details(1).收费类别 = "7" Then
                    bln中药 = True
           End If
        End If
        If Not zlIsCheckPrivs(mstrPrivs, "医生查询") Then mobjBill.Pages(mintPage).开单人 = ""
        '清除病人信息
       ' Call ClearmobjBill
    Else
        ...
        With mobjBill.Pages(mintPage)
            ...
            If zlIsCheckPrivs(mstrPrivs, "医生查询") Then .开单人 = tmpBill.Pages(1).开单人
...
Public Sub zlExeBalanceWinRefrshData(ByVal blnSaveOK As Boolean, ByVal bytExitMode As gExitMode, _
    ...
    If mbytInState = EM_ED_异常重收 Or mblnErrBill Then
        ...
        If Not mobjPlugIn Is Nothing Then
            On Error Resume Next
            Call mobjPlugIn.OutPatiExseAfter(objChargeInfor.病人ID, objChargeInfor.结帐ID)
            Err.Clear
        End If
    ...
    If Not blnSaveOK Then
        ...
        If mintInsure <> 0 Then
            cmdAddBill.Enabled = Not MCPAR.门诊连续收费 And MCPAR.多单据收费 _
                And zlIsCheckPrivs(mstrPrivs, "医保病人多单据收费")
        Else
            cmdAddBill.Enabled = zlIsCheckPrivs(mstrPrivs, "普通病人多单据收费")
        End If
        ...
        If gfrmMain Is Nothing And glngMain = 0 Then Me.Enabled = True
        Exit Sub
    End If
    mblnSaveData = True
    mintSucces = mintSucces + 1
    ...
    '写卡:56615
    Call WriteMzInforToCard(objChargeInfor.病人ID, objChargeInfor.结算序号)
    ...
    Else
        ...
        mstrPreDoctor = NeedName(cbo开单人.Text)
            ...
            With mobjBill.Pages(mintPage).Details(Bill.Row)
                ...
                If .执行部门ID <> Bill.ItemData(Bill.ListIndex) Then
                    ...
                    If InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药 Then
                        ...
                        Call ShowStatusCargo_space(.收费细目ID, .执行部门ID)    '显示货位
...
Private Sub Bill_CommandClick()
    ...
    End If
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, mintInsure, gbln药房单位, str类别, , , str特准项目, 0)
    If lng项目id <> 0 Then
        Bill.Text = lng项目id
        mblnSelect = True
...
Private Sub ShowStock(ByVal lng库房ID As Long, str药品 As String, dbl库存 As Double)
    ...
    On Error GoTo errHandle
    Call zlInit缺省部门
    If zlIsCheckPrivs(mstrPrivs, "显示库存") Then
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        End If
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, mintInsure, gbln药房单位, str类别, Bill.Text, Bill.TxtHwnd, str特准项目, 0, str排除类别)
                        If lng项目id <> 0 Then
                    ...
                    If InStr(",5,6,7,", mobjDetail.类别) > 0 And mbln处方职务检查 Then
                        ...
                        If cbo医疗付款.ListIndex <> -1 Then
                            '医保或公费病人
                            '问题:45605
                            If zlIsCheckMedicinePayMode(NeedName(cbo医疗付款)) Then
                    ...
                    If InStr(",5,6,7,", mobjDetail.类别) > 0 Then
                        If Not gbln分离发药 Then
                            ...
                            If gbln药房单位 Then dblStock = dblStock / mobjDetail.药房包装
                            mobjDetail.库存 = dblStock
                            Call ShowStock(lngDoUnit, mobjDetail.名称, mobjDetail.库存)
                        Else
                            str药房IDs = Decode(mobjDetail.类别, "5", gstr西药房, "6", gstr成药房, "7", gstr中药房)
                            If str药房IDs <> "" Then
                                dblStock = GetMultiStock(mobjDetail.ID, str药房IDs)
                                
                                If dblStock = 0 And zlIsCheckPrivs(mstrPrivs, "不检查库存") = False And gblnStock Then
                                    MsgBox "[" & mobjDetail.名称 & "]的可用库存为零!", vbInformation, gstrSysName
                                    Bill.TxtSetFocus: Cancel = True: Exit Sub
                                End If
                                
                                If gbln药房单位 Then dblStock = dblStock / mobjDetail.药房包装
                                mobjDetail.库存 = dblStock
                                Call ShowStock(0, mobjDetail.名称, mobjDetail.库存)
                            End If
                        End If
                    ...
                    If mstrYBPati <> "" And Not MCPAR.允许不设置医保项目 Then
                        If Not CheckMediCareItem(mobjDetail.ID, mintInsure, mobjDetail.名称, mobjDetail.变价 = False) Then
                            Bill.Text = "": Bill.TxtSetFocus: Cancel = True: Exit Sub
                        End If
                    End If
                    ...
                    If mintInsure <> 0 And MCPAR.实时监控 And mobjBill.Pages(mintPage).Details(Bill.Row).数次 <> 0 Then
                        If gclsInsure.CheckItem(mintInsure, 0, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), mintPage, Bill.Row)) = False Then
                    ...
                    If mobjBill.Pages(mintPage).Details(Bill.Row).数次 <> 0 Then
                        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, mlngModul, 0, 0, _
                            MakeDetailRecord(mobjBill, zlStr.NeedName(cbo开单人.Text), zlStr.NeedName(cbo开单科室.Text), mintPage, Bill.Row)) = False Then
                            mobjBill.Pages(mintPage).Details.Remove Bill.Row '删除刚刚想要加入的费用行
                            Bill.Text = "": Cancel = True: Exit Sub
                        End If
                    End If
            ...
            Case "数次"
                ...
                If .Details.Count >= Bill.Row And Bill.Text <> "" Then
                    ...
                    If InStr(",5,6,7,", .Details(Bill.Row).收费类别) > 0 Then
                        If Val(Bill.Text) - Int(Val(Bill.Text)) <> 0 And zlIsCheckPrivs(mstrPrivs, "药品输入小数") = False Then
                    ...
                    If CSng(Bill.Text) * .Details(Bill.Row).付数 < 0 Then
                        '权限
                        If zlIsCheckPrivs(mstrPrivs, "负数费用") = False Then
                    ...
                    With .Details(Bill.Row)
                        If (.收费类别 = "4" And .Detail.跟踪在用) Or (InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药) Then
                            ...
                            Else
                                ...
                                If colStock("_" & .执行部门ID) <> 0 And zlIsCheckPrivs(mstrPrivs, "不检查库存") = False And Bill.ColData(BillCol.执行科室) = BillColType.UnFocus Then
                                ...
                                End If
                            End If
                        ElseIf InStr(",5,6,7,", .收费类别) > 0 And zlIsCheckPrivs(mstrPrivs, "不检查库存") = False And gbln分离发药 Then
                    ...
                    With mobjBill.Pages(mintPage).Details(Bill.Row)
                        ...
                        If (.收费类别 = "4" And .Detail.跟踪在用) Or (InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药) Then
                            ...
                            Else
                                ...
                                If colStock("_" & .执行部门ID) <> 0 And zlIsCheckPrivs(mstrPrivs, "不检查库存") = False Then
                                    If .付数 * .数次 > .Detail.库存 Then
                                        If colStock("_" & .执行部门ID) = 1 Then
                                            If MsgBox("[" & .Detail.名称 & "]的当前可用库存不足输入数量,要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                                                Cancel = True
...
Private Sub SetSubItem()
    ...
    With mobjBill.Pages(mintPage).Details(lngMainRow)
        ...
        For i = 1 To mcolDetails.Count
            ...
            '保险支付项目对应检查
            If mstrYBPati <> "" And Not MCPAR.允许不设置医保项目 Then
                If Not CheckMediCareItem(mcolDetails(i).ID, mintInsure, mcolDetails(i).名称, mcolDetails(i).变价 = False) Then
                    Exit Sub
                End If
            End If
...
Private Sub CalcPItemActualIncome(ByVal lngMainRow As Long, Optional intPage As Integer)
    ...
    With mobjBill.Pages(intPage)
        ...
        str费别 = IIf(glngSys Like "8??", mobjBill.费别, TrimChar(mobjBill.费别 & "," & lbl动态费别.Tag, ","))
                    ...
                    Else
                        ...
                        If str药房IDs <> "" Then
                            If Not (mbytInState = EM_ED_收费 And mstrInNO <> "") Then
                                ...
                                If gbln药房单位 Then .Detail.库存 = .Detail.库存 / .Detail.药房包装
                            End If
                            Call ShowStock(0, .Detail.名称, .Detail.库存)
                        Else
                            sta.Panels(Pan.C2提示信息) = ""
                        End If
                    End If
        ...
        Case "数次"
            ...
            If mobjBill.Pages(mintPage).Details.Count >= Bill.Row Then
                If InStr(",5,6,7,", mobjBill.Pages(mintPage).Details(Bill.Row).收费类别) > 0 Then
                    If zlIsCheckPrivs(mstrPrivs, "药品输入小数") = False Then
                        Bill.TextMask = Replace(Bill.TextMask, ".", "")
                    End If
                    '中药快捷输入
                    If mobjBill.Pages(mintPage).Details(Bill.Row).收费类别 = "7" Then
...
Private Sub cboSex_Click()
    If Not mbytInState = EM_ED_收费 Then Exit Sub
    mobjBill.性别 = NeedName(cboSex.Text)
...
Private Sub cbo费别_Click()
    ...
    If Not (mstrYBPati <> "" Or mobjBill.费别 <> NeedName(cbo费别.Text)) Then Exit Sub
...
Private Sub LoadAndSeek费别(Optional blnNew As Boolean)
    ...
    cbo费别.Locked = (Not Bill.Active) _
            Or (mrsInfo.State = 1 And Not zlIsCheckPrivs(mstrPrivs, "调整病人费别"))
    ...
    Else
        '定位有档案病人的费别
        cbo费别.ListIndex = GetCboIndex(cbo费别, Nvl(mrsInfo!费别))
        If cbo费别.ListIndex <> -1 Then
            '费别为初诊但病人不是初诊
...
Private Sub cbo开单人_Click()
    ...
    mblnCboClick = True
    If Not (mbytInState = EM_ED_收费 And chkCancel.Value = 0) Then Exit Sub
    If mobjBill.Pages(mintPage).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text)) Then Exit Sub
    
    mobjBill.Pages(mintPage).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
...
Private Sub cbo开单人_Validate(Cancel As Boolean)
    If cbo开单人.Text <> "" Then
        If GetCboIndex(cbo开单人, NeedName(cbo开单人.Text)) = -1 Then cbo开单人.ListIndex = -1: cbo开单人.Text = ""
    End If
...
Private Sub chkCancel_Click()
    ...
    If chkCancel.Value = 1 Then
        ...
        Call NewBill(False)
        Set mobjBill = New ExpenseBill
        If fraBill.Visible Then cmdAddBill.Enabled = zlIsCheckPrivs(mstrPrivs, "普通病人多单据收费")
        
        cboNO.Text = ""

        Call SetDisible
        If Not zlIsCheckPrivs(mstrPrivs, "医生查询") Then
    ...
    Else
        ...
        If fraBill.Visible Then cmdAddBill.Enabled = zlIsCheckPrivs(mstrPrivs, "普通病人多单据收费")
        txtInvoice.Locked = Not zlIsCheckPrivs(mstrPrivs, "修改票据号") And gblnStrictCtrl
        Call SetDisible(True)
...
Private Sub AddNewBill()
    ...
    End If
    mobjBill.Pages(i).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
...
Private Sub cmdAddBill_Click()
    ...
    If mobjBill.Pages(1).NO <> "" Then cbo费别.ListIndex = GetCboIndex(cbo费别, strFirst费别)
...
Private Sub cmdDelBill_Click()
    ...
    If gTy_Module_Para.bln禁止取消挂号划价单 And mobjBill.Pages(mintPage).挂号划价单 Then
        MsgBox "第 " & mintPage & " 张单据为挂号产生的划价单据，要求必须进行收费，不允许删除！", vbInformation, gstrSysName
        Exit Sub
    End If
...
Private Sub RefreshFact()
    ...
    Else
        If (lblFact.Tag <> "" And txtInvoice.Text <> "") Or Trim(txtInvoice.Text) = "" Then
            '松散：取下一个号码
            txtInvoice.Text = IncStr(UCase(zlDatabase.GetPara("当前收费票据号", glngSys, mlngModul)))
            'Tag：问题：24363:刘兴洪：主要是解决自动生成的号是否被用户更改，主要解决：
            '    1.更改的票据号需要检查是否重复，重复后直接返回不更改发票号
            '    2.并发操作，不更改的情况下，检查是否重复，如果重复，自动取下一个号码！
        End If
...
Private Sub sta_PanelClick(ByVal Panel As MSComctlLib.Panel)
    ...
    ElseIf Panel.Key = "PatiSource" Then
        If gbln病人来源受权限控制 And zlIsCheckPrivs(mstrPrivs, "参数设置") = False Then
            '授权限控制,不能更改
            Exit Sub
        End If
...
Private Function ClearFullBill(ByVal bln提示 As Boolean, _
    ...
    cur个帐余额 = mdbl个帐余额: cur个帐透支 = mdbl个帐透支: blnYB结算作废 = mblnYB结算作废
    ...
    If blnNotClearYb And intInsure <> 0 Then
        ...
        mdbl个帐余额 = cur个帐余额: mdbl个帐透支 = cur个帐透支: mblnYB结算作废 = blnYB结算作废
        sta.Panels(Pan.C3个人帐户).Text = "个人帐户余额:" & Format(mdbl个帐余额, "0.00")
        sta.Panels(Pan.C3个人帐户).Visible = True
...
Private Function isValiedCargeFee() As Boolean
        ...
        If Not (cbo开单人.Locked And cbo开单科室.Locked) Then
            For i = 1 To mobjBill.Pages.Count
                If mobjBill.Pages(i).开单人 <> "" And mobjBill.Pages(i).NO = "" Then        '25618:And mobjBill.Pages(i).NO = "":刘兴洪加入,主要是挂号产生的划价单时,不一定开单科室时临床的,因此不能检查
                    ...
                    If mrs开单人.RecordCount = 0 Then
                        MsgBox "开单人""" & mobjBill.Pages(i).开单人 & """不属于开单科室""" & NeedName(cbo开单科室.Text) & """,请检查！", vbInformation, gstrSysName
                        Exit Function
                    End If
                End If
            Next
   ...
   '药品库存检查(仅不足禁止时或分批时价药品)
    bln检查库存 = Not zlIsCheckPrivs(mstrPrivs, "不检查库存")      '是否有权限不检查库存(分批和时价必须检查)
    For p = 1 To mobjBill.Pages.Count
        For i = 1 To mobjBill.Pages(p).Details.Count
            With mobjBill.Pages(p).Details(i)
                Set colStock = IIf(.收费类别 = "4", mcolStock2, mcolStock1)
                ...
                If InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药 Then
                    If .Detail.分批 Or .Detail.变价 Then
                        ...
                        If dblToTal > .Detail.库存 Then
                            ...
                            MsgBox strTmp & "第 " & i & " 行时价或分批药品""" & .Detail.名称 & _
                                """的当前库存" & IIf(zlIsCheckPrivs(mstrPrivs, "显示库存"), .Detail.库存, "") & "不足输入数量""" & _
                    ...
                    Else
                        ...
                        End If
                    End If
                ElseIf InStr(",5,6,7,", .收费类别) > 0 And gbln分离发药 And bln检查库存 And gblnStock Then
                    '单据行费用对象的库存是本地参数指定的药房的库存之和
                    strInfo = Decode(.Detail.类别, "5", gstr西药房, "6", gstr成药房, "7", gstr中药房)
                    If strInfo <> "" Then
                        dblToTal = GetDrugTotal(mobjBill, .收费细目ID, 0)
                        
                        If mbytInState = EM_ED_收费 And mstrInNO <> "" Then .Detail.库存 = .Detail.库存 + GetOriginalTotal(mobjBill, .收费细目ID, 0)
                        If dblToTal > .Detail.库存 Then
                            If mobjBill.Pages.Count > 1 Then strTmp = "第 " & p & " 张单据"
                            MsgBox strTmp & "第 " & i & " 行药品""" & .Detail.名称 & "]的当前库存" & IIf(zlIsCheckPrivs(mstrPrivs, "显示库存"), .Detail.库存, "") & "不足输入数量""" & _
                                dblToTal & """,请修改或检查是否有多行输入。", vbInformation, gstrSysName
                            Exit Function
                        End If
                    End If
                    ...
                    If .Detail.分批 Or .Detail.变价 Then
                        ...
                        If dblToTal > .Detail.库存 Then
                            ...
                            MsgBox strTmp & "第 " & i & " 行时价或分批卫生材料""" & .Detail.名称 & _
                                """的当前库存" & IIf(zlIsCheckPrivs(mstrPrivs, "显示库存"), .Detail.库存, "") & "不足输入数量""" & dblToTal & """。", vbInformation, gstrSysName
                            tbsBill.Tabs(p).Selected = True
                            Bill.SetFocus: Exit Function
                        End If
                    Else
                        If colStock("_" & .执行部门ID) = 2 And bln检查库存 Then
                            ...
                            If dblToTal > .Detail.库存 Then
                                ...
                                MsgBox strTmp & "第 " & i & " 行卫生材料""" & .Detail.名称 & _
                                    """的当前库存" & IIf(zlIsCheckPrivs(mstrPrivs, "显示库存"), .Detail.库存, "") & "不足输入数量""" & dblToTal & """,请修改或检查是否有多行输入。", vbInformation, gstrSysName
                                tbsBill.Tabs(p).Selected = True
                                Bill.SetFocus: Exit Function
                            End If
                        End If
    ...
    If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, mlngModul, 0, 1, _
        MakeDetailRecord(mobjBill, zlStr.NeedName(cbo开单人.Text), zlStr.NeedName(cbo开单科室.Text))) = False Then
        Exit Function
    End If
...
InvoiceHandle:
            If zlGetInvoiceGroupUseID(mlng领用ID, IIf(IsSplitPrintByNO, mobjBill.Pages.Count, 1), txtInvoice.Text) = False Then
                Exit Function
            End If
            '并发操作检查,票号是否已用
...
Private Function zlChargeFeeWin() As Boolean
    ...
    If mFrmBalanceWin.zlChargeWin(Me, EM_FUN_收费, mlngModul, mstrPrivs, mobjChargeInfor, bytReturnMode, bln连续, mlngPreBrushCardID) = False Then
       If Not gfrmMain Is Nothing Or glngMain <> 0 Then
...
Private Sub cmdOK_Click()
    ...
    If mbytInState = EM_ED_收费 And chkCancel.Value = 0 Then
        ...
        If Not mblnSaveAsPrice Then
            If gfrmMain Is Nothing And glngMain = 0 Then Me.Enabled = False
...
Private Sub LoadFeeInfor(ByVal lngPatientID As Long, Optional ByVal blnDelete As Boolean)
    ...
    If Not rsTmp Is Nothing Then
        ...
        If mbytInState = EM_ED_收费 And mstrInNO <> "" Then cmdPrint.Tag = Val(cmdPrint.Tag) + Original.冲预交款
    Else
...
Private Sub cmdPrint_Click()
    ...
    blnPrintList = False
    If zlIsCheckPrivs(mstrPrivs, "打印清单") Then
        If gint收费清单 = 1 Then
            blnPrintList = True
    ...
    If zlIsCheckPrivs(mstrPrivs, "收费执行单") Then
        If int收费执行单 = 1 Then
            blnPrintExe = True
        ...
        If strPrintNO <> "" Then
            If mblnPrint Then
                ...
                Else
                    ...
                    Call ReportOpen(gcnOracle, glngSys, "ZL" & glngSys \ 100 & "_BILL_1121_1", Me, "发票号=NO", "NO='" & strPrintNO & "'", IIf(mintInvoiceFormat = 0, "", "ReportFormat=" & mintInvoiceFormat), 2)
                End If
            End If
...
Private Sub cmd预结算_Click()
    ...
    If mintInsure <> 0 And MCPAR.实时监控 Then
        ...
        If gclsInsure.CheckItem(mintInsure, 0, 2, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text))) = False Then Exit Sub
    End If
...
Private Sub txtIn_KeyPress(KeyAscii As Integer)
    ...
    If Not cmdAddBill.Enabled Or Not cmdAddBill.Visible Then
        ...
        Set mobjBill = ImportBill(strNO, False, 0, , False)
        If mobjBill.NO = "" Then
    ...
    '不提供从后备表中导入的功能
        ...
        For i = 0 To UBound(varNos)
            ...
            Set objBill = ImportBill(strNO, False, 0, , False)
            ...
            With mobjBill.Pages(mintPage)
                ...
                If zlIsCheckPrivs(mstrPrivs, "医生查询") Then .开单人 = objBill.Pages(1).开单人
...
Private Sub CalcDrugStock(Optional intPage As Integer)
    ...
    For i = 1 To mobjBill.Pages(intPage).Details.Count
        With mobjBill.Pages(intPage).Details(i)
            ...
            ElseIf InStr(",5,6,7,", .收费类别) > 0 And gbln分离发药 And zlIsCheckPrivs(mstrPrivs, "不检查库存") = False Then
                
                '分离发药时,当前药品行的库存属性存储的是本地参数设置的药房的总库存
    ...
    If KeyAscii = 13 And cboNO.Text <> "" And Not cboNO.Locked Then
        ...
        If blnRead Then
           Bill.Active = False
            ...
            If gint病人来源 = 1 And zlIsCheckPrivs(mstrPrivs, "允许非医保病人") = False Then
...
Private Sub cbo费别_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii = 13 Then
        ...
        Else
             '即使费用相同也要重算,因为医保验卡后必须重算,预结算才正确
            If (mstrYBPati <> "" Or mobjBill.费别 <> NeedName(cbo费别.Text)) Then
                mobjBill.费别 = NeedName(cbo费别.Text)
...
Private Function isCheck开单人Exists(ByVal str姓名 As String, Optional blnLocateItem As Boolean = False) As Boolean
    ...
    For i = 0 To cbo开单人.ListCount - 1
        If NeedName(cbo开单人.List(i)) = str姓名 Then
...
Private Sub cbo开单人_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        Else
            mobjBill.Pages(mintPage).开单人 = NeedName(cbo开单人.Text)
...
Private Sub ShowCHRecipe()
    ...
    '动态费别串
    If glngSys Like "8??" Then
        str动态费别 = NeedName(cbo费别.Text)
    Else
        str动态费别 = TrimChar(NeedName(cbo费别.Text) & "," & lbl动态费别.Tag, ",")
    End If
    
    '调用窗口
    Set objDetails = frmCHRecipe.ShowMe(Me, mstrPrivs, mlngModul, 0, 0, Original.实收合计, mobjBill.病人ID, lng病人科室ID, Get开单科室ID, _
        IIf(mlng中药房 = 0, glng中药房, mlng中药房), mobjBill.Pages(mintPage).Details, NeedName(cbo费别.Text), str动态费别, _
        ...
        Case vbKeyF6 '定位到病人输入框
            If Me.ActiveControl Is txtPatient And txtPatient.Enabled And mstrYBPati = "" Then   '提取划价单后，姓名输入框是锁定的
                If mbytInState = EM_ED_收费 And (gint病人来源 = 1 Or gint病人来源 = 2) Then
                    If chkCancel.Value = 0 And zlIsCheckPrivs(mstrPrivs, "保险收费") Then
                                   ...
                                   '设置缺省执行科室
                                    If Not blnEnter Then '进入该列时保持已确定值不变
                                        ...
                                        ElseIf lngRow > 1 Then
                                            '与上一行非药品相同
                                            If mrsUnit!ID = mobjBill.Pages(mintPage).Details(lngRow - 1).执行部门ID And mobjBill.Pages(mintPage).Details(lngRow - 1).Detail.执行科室 = .Detail.执行科室 _
...
Private Sub InitFace()
    ...
    If Not (mbytInState = EM_ED_收费 And mstrInNO = "" _
        And zlIsCheckPrivs(mstrPrivs, "保险收费") _
        And gint病人来源 = 1) Then
    ...
    cmdSaveWholeSet.Visible = zlIsCheckPrivs(mstrPrivs, "增加成套项目")
    ...
    '重打(仅收费有效)
    If Not (mbytInState = EM_ED_收费 And mstrInNO = "" _
            And zlIsCheckPrivs(mstrPrivs, "重打票据")) Then
    ...
    If mbytInState = EM_ED_收费 Then Call ShowBillFormat
    ...
    '权限设置
    If Not zlIsCheckPrivs(mstrPrivs, "门诊退费") Then
        chkCancel.Visible = False
    ...
    If gbln分离发药 Or Not mbytInState = EM_ED_收费 Then
...
Private Sub MCPatientProcess(Optional ByVal lngCur病人ID As Long, Optional blnErrBill As Boolean)
    ...
    If mstrYBPati <> "" Then
        ...
        If fraBill.Visible Then
            cmdAddBill.Enabled = Not MCPAR.门诊连续收费 And MCPAR.多单据收费 And zlIsCheckPrivs(mstrPrivs, "医保病人多单据收费")
        End If
        txtPatient.ForeColor = vbRed
        If Not mrsInfo Is Nothing Then
            If mrsInfo.State = 1 Then
        ...
        '55766:文本框有一bug:如果先为密文显示,后设置成非密文显示后,不能输入五笔
        txtPatient.IMEMode = 0
        cboSex.ListIndex = GetCboIndex(cboSex, CStr(Split(mstrYBPati, ";")(4)))
        ...
        If UBound(Split(mstrYBPati, ";")) >= 25 And mobjBill.Pages(mintPage).NO = "" Then   '划价单的开单人开单科室优先
            ...
            If str开单科室 <> "" Then
                Call zlcontrol.CboSetIndex(cbo开单科室.hWnd, GetCboIndex(cbo开单科室, str开单科室)) '不触发click事件
                Call cbo开单科室_Click
            End If
        End If
        '根据病人挂号信息设置开单科室和医生
        ...
        If mrsInfo.State = 1 Then
            If Not IsNull(mrsInfo!医疗付款方式) Then
                cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, mrsInfo!医疗付款方式)
            End If
        End If
        ...
        If mbytInState = EM_ED_收费 And Visible And mstrInNO = "" And txtIn.Text = "" And mrsInfo.State = 1 And _
            ...
            blnPriceBill = LoadMultiBills(lng病人ID, MCPAR.门诊连续收费 Or Not MCPAR.多单据收费 Or zlIsCheckPrivs(mstrPrivs, "医保病人多单据收费") = False, lng挂号科室)
        End If
...
Private Sub txtPatient_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 And mbytInState = EM_ED_收费 And gint病人来源 = 1 And Not mblnValid Then
        If txtPatient.Text = "" And chkCancel.Value = 0 And zlIsCheckPrivs(mstrPrivs, "保险收费") Then
   ...
   '问题:51488
    ...
    If blnCard And Len(txtPatient.Text) = IDKind.GetCardNoLen - 1 And KeyAscii <> 8 _
            ...
            Else
                ...
                If gint病人来源 = 1 And gblnInputName And IDKind.IDKind = IDKind.GetKindIndex("姓名") And txtPatient.Text <> "" Then
                    ...
                    If txtPatient.Text = mstrPrePati And txtPatient.Text <> "" Then '同一个收费病人,此时没有病人ID
                        ...
                        If Bill.Active Then
                            Call zlcontrol.CboSetIndex(cbo开单人.hWnd, GetCboIndex(cbo开单人, mstrPreDoctor)) '不触发click事件
                            Call cbo开单人_Click
                        End If
        ...
        Else 'b.根据输入读取病人信息成功
            ...
            '102234,调用外挂部件接口
            If lng病人ID <> 0 And Not gobjPlugIn Is Nothing Then
                On Error Resume Next
                ...
                blnChecked = gobjPlugIn.PatiValiedCheck(glngSys, mlngModul, 3, lng病人ID, 0, "")
                If Err <> 0 Then
            ...
            '就诊卡密码检查
            If mbytInState = EM_ED_收费 And (blnCard Or (blnICCard Or blnIDCard) And mstrPassWord <> "") Then
            ...
            ElseIf gint病人来源 = 2 Then
                If gbyt科室医生 <> 0 And mbytInState = EM_ED_收费 And mstrInNO = "" Then
                    ...
                    Call zlcontrol.CboSetIndex(cbo开单科室.hWnd, FindCboIndex(cbo开单科室, Val("" & mrsInfo!当前科室id)))
                    Call cbo开单科室_Click
                End If
            ...
            '55766:文本框有一bug:如果先为密文显示,后设置成非密文显示后,不能输入五笔
            txtPatient.IMEMode = 0
            cboSex.ListIndex = GetCboIndex(cboSex, Nvl(mrsInfo!性别))
            ...
            ElseIf Not mblnValid Then
                If IsRegisterDept And cbo开单科室.ListIndex <> -1 Then
                    cbo费别.ListIndex = GetCboIndex(cbo费别, Nvl(mrsInfo!费别)) '挂号时确定的费别
                Else
                    If mstrInNO = "" Then Call LoadAndSeek费别
                End If
            End If
            If gstr费别 <> "" And cbo费别.ListIndex = -1 Then cbo费别.ListIndex = GetCboIndex(cbo费别, gstr费别)
            
            cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, Nvl(mrsInfo!医疗付款方式))
            If mstr付款方式 <> "" And cbo医疗付款.ListIndex = -1 Then cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, mstr付款方式)
            ...
            With mobjBill
                ...
                .病区ID = Nvl(mrsInfo!当前病区ID, 0)
                .科室ID = Nvl(mrsInfo!当前科室id, 0)
                .费别 = NeedName(cbo费别.Text) '以当前有效为准
            End With
            Call ReInitPatiInvoice
...
ReDO:
    ...
    If blnYbCheckCard = False And blnCard And IDKind.GetCurCard.名称 Like "姓名*" _
        And (InStr("-+*", Left(strInput, 1)) = 0 And IsNumeric(Mid(strInput, 2))) Then   '103563
        ...
        Select Case IDKind.GetCurCard.名称
            Case "姓名", "姓名或就诊卡"
                ...
                If Not mblnValid And gblnSeekName And gblnInputID Then
                    strPati = _
                        " Select 1 as 排序ID,A.病人ID as ID,A.病人ID,A.姓名,A.性别,A.年龄," & _
                    ...
                    Set rsTmp = zlDatabase.ShowSQLSelect(Me, strPati, 0, "病人" & 0 & gint病人来源, 1, "", "请选择病人", False, False, True, vRect.Left, vRect.Top, txtPatient.Height, blnCancel, False, True, strInput & "%", gintNameDays)
                    If Not rsTmp Is Nothing Then
...
Private Sub txtRePrint_KeyPress(KeyAscii As Integer)
    ...
    If zlIsCheckPrivs(mstrPrivs, "所有操作员") = False Then
    ...
    End If
    '调出重打的单据显示
    frmClinicDelAndView.ShowMe Me, EM_MULTI_查看, mstrPrivs, lng结算序号, True
    ...
    If Not RePrintCharge(1, strNos, Me, mlng领用ID, strReclaimInvoice, , , _
        intInvoiceFormat, blnVirtualPrint, , mlngShareUseID, mstrUseType) Then
        txtRePrint.SetFocus
    Else
...
Private Sub CalcMoney(intPage As Integer, lngRow As Long, Optional bln从项汇总折扣 As Boolean)
    ...
    Dim dblAllTime As Double, dblPrice As Double, dbl加班加价率 As Double, dblPriceSingle As Double
    ...
    gstrSQL = _
        ...
        " And A.ID=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mobjBill.Pages(intPage).Details(lngRow).收费细目ID)
    If Not rsTmp.EOF Then
            ...
            If .Detail.变价 Then
                If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                    ...
                    If dblAllTime <> 0 Then
                        dblPrice = Get时价药品应收金额(.执行部门ID, .收费细目ID, .收费类别 = "4", dblAllTime, gstrDec, dblPriceSingle)
                        If dblAllTime <> 0 Then
                            '数量未分解完毕
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            With mobjBillIncome
                ...
                If mobjBill.Pages(intPage).Details(lngRow).Detail.变价 Then
                    If InStr(",5,6,7,", mobjBill.Pages(intPage).Details(lngRow).收费类别) > 0 And gbln药房单位 Then
                ...
                Else
                    If InStr(",5,6,7,", mobjBill.Pages(intPage).Details(lngRow).收费类别) > 0 And gbln药房单位 Then
                        .标准单价 = Format(Nvl(rsTmp!现价, 0) * mobjBill.Pages(intPage).Details(lngRow).Detail.药房包装, gstrFeePrecisionFmt)
                    Else
                ...
                '应收金额=单价 * 付数 * 数次
                If mobjBill.Pages(intPage).Details(lngRow).Detail.变价 _
                    And (InStr(",5,6,7,", mobjBill.Pages(intPage).Details(lngRow).收费类别) > 0 _
                        Or mobjBill.Pages(intPage).Details(lngRow).收费类别 = "4" _
                            And mobjBill.Pages(intPage).Details(lngRow).Detail.跟踪在用) Then
...
Private Sub NewYBBill()
    ...
    With mobjBill
        ...
        End If
        .Pages(mintPage).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
        .门诊标志 = gint病人来源
        .划价人 = UserInfo.姓名
        .操作员编号 = UserInfo.编号
        .操作员姓名 = UserInfo.姓名
...
Private Function NewBill(Optional blnFact As Boolean = True, Optional bln费别 As Boolean = True) As Boolean
    ...
    If mstr付款方式 <> "" Then
        cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, mstr付款方式)
...
Private Function ReChargeFee() As Boolean
    ...
    Dim rsTemp As ADODB.Recordset, strYBPati As String
    ...
    If mintInsure <> 0 Then
        ...
        If Not rsTemp.EOF Then
            ...
            strYBPati = gclsInsure.Identify(0, mobjBill.病人ID, intCheckInsure)
            ...
            If Val(CLng(Split(strYBPati, ";")(8))) <> mobjBill.病人ID Then
    ...
    If Not frmClinicChargeBalance.zlChargeWin(Me, EM_FUN_重收, mlngModul, mstrPrivs, mobjChargeInfor, _
        ...
        If (Not gfrmMain Is Nothing Or glngMain <> 0) And Not mblnErrBill Then Unload Me
        Exit Function
    End If
    If Not gfrmMain Is Nothing Or glngMain <> 0 Then
...
Private Function DelErrBillFee() As Boolean
    ...
    '并发检查
    If zlIsCheckExistErrBill(mlng结算序号) = False Then
        MsgBox "当前异常单据可能已被处理，你不能继续，请刷新后重试！", vbInformation, gstrSysName
        Exit Function
    End If
    If zlCheckOtherSessionDoing(mlng结算序号) Then
        MsgBox "当前单据正在其它收费窗口中进行处理，你不能继续！", vbInformation, gstrSysName
...
Private Sub Set开单人开单科室(ByVal str开单人 As String, ByVal lng开单科室ID As Long)
    ...
    'a.医生确定科室
    If gbyt科室医生 = 0 Then
        Call zlcontrol.CboSetIndex(cbo开单人.hWnd, GetCboIndex(cbo开单人, str开单人))  '不触发click事件
...
Private Function ReadBill(ByVal strNO As String, ByVal bytFun As Byte, _
    ...
    If Not blnShow Then
        ...
        mobjBill.Pages(mintPage).挂号划价单 = Left(Nvl(rsTmp!摘要), 3) = "挂号:"
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        '划价收费时重新处理发药窗口
        If bytFun = 1 And InStr(",5,6,7,", rsTmp!编码) > 0 And Not gbln分离发药 Then
            j = j + 1
...
Private Function SaveModi() As Boolean
    ...
    strSQL = "zl_病人费用记录_Update('" & cboNO.Text & "'," & 1 & "," & _
        "'" & NeedName(cbo开单人.Text) & "',To_Date('" & txtDate.Text & "','YYYY-MM-DD HH24:MI:SS'))"
    On Error GoTo errH
    Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
    SaveModi = True
    Exit Function
...
'功能：根据当前病人的类型判断指定行的项目是否可以输入,适用于所有类别的项目
    ...
    '医保或公费病人
    '问题:45605
    If zlIsCheckMedicinePayMode(NeedName(cbo医疗付款), bln医保, bln公费) = False Then Exit Function
    '只检查医保病人和公费病人
    ...
    For p = 1 To mobjBill.Pages.Count
        For i = 1 To mobjBill.Pages(p).Details.Count
            If mobjBill.Pages(p).Details(i).执行部门ID = 0 Then
                If Not (InStr(",5,6,7,", mobjBill.Pages(p).Details(i).收费类别) > 0 And gbln分离发药) Then
...
'功能：检查是否可以显示挂号按钮
    ...
    '功能是否授权
    cmdRegist.Visible = zlIsCheckPrivs(strPrivs, "挂收费号") Or zlIsCheckPrivs(strPrivs, "挂免费号")
...
'功能：检查是否可以显示就诊卡按钮
    ...
    cmdIDCard.Visible = zlIsCheckPrivs(strPrivs, "发卡")
    Exit Sub
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Private Sub SetItemRowColor(ByVal intPage As Integer, ByVal lngRow As Long)
    ...
    If mobjBill.Pages(intPage).Details.Count >= lngRow And mbytInState = EM_ED_收费 Then
        With mobjBill.Pages(intPage).Details(lngRow)
            If mbln储备限额检查 And ((InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药) Or (.收费类别 = "4" And .Detail.跟踪在用)) Then
...
Private Sub ShowStatusCargo_space(ByVal lng收费细目ID As Long, lng执行库房ID As Long, _
...
RePrint:
        ...
        Call frmPrint.ReportPrint(1, strNos, strModiNos, strReclaimInvoice, mlng领用ID, mlngShareUseID, txtInvoice.Text, mobjBill.登记时间, CStr(mdbl缴款), CStr(mdbl找补), _
            IsSplitPrintByNO, mintInvoiceFormat, , , mstrUseType, blnPrintBillEmpty)
...
Private Function PatiErrBillPay(ByVal lng病人ID As Long) As Boolean
    ...
    If lng结算序号 >= 0 Then
        ...
        frmCharge.mbln退费异常 = True
        Set frmCharge.mobjMsgModule = mobjMsgModule
        frmCharge.Show GetModuleType, Me
        PatiErrBillPay = gblnOK
    End If
    '重新对异常单据进行重收费
    ...
    For p = 1 To mobjBill.Pages.Count
        ...
        If Not bln直接收费 Then
           ...
           gstrSQL = "Zl_病人划价收费_Insert("
            ...
            '获取自动发药的多个药房
            If gbln收费后自动发药 And Not gbln分离发药 Then
        ...
        ElseIf bln直接收费 Then
            ...
            For Each mobjBillDetail In mobjBill.Pages(p).Details
                If mobjBillDetail.数次 <> 0 Then
                    ...
                    With mobjBillDetail
                        If gbln收费后自动发药 Then
                            If .执行部门ID <> 0 And InStr("5,6,7", .收费类别) > 0 And Not gbln分离发药 Then
        ...
        If mintInsure <> 0 And _
            ...
            gstrSQL = "zl_门诊收费票据_Insert('" & strBillNO & "','" & strInvoice & "'," & ZVal(mlng领用ID) & "," & _
                "'" & UserInfo.姓名 & "',To_Date('" & mobjBill.登记时间 & "','YYYY-MM-DD HH24:MI:SS')," & lng打印ID & ",1," & IIf(p = 1, "1", "0") & ")"
            zlAddArray cllFeeBillItem, gstrSQL
        End If
...
Private Function Set发药窗口(ByVal p As Integer, ByRef objBillDetail As BillDetail) As Boolean
    ...
    With objBillDetail
        '收费、划价的药品行,处理发药窗口
        If Not (InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药) Then Set发药窗口 = True: Exit Function
...
Private Function CheckChargeDataValied() As Boolean
   ...
   '药品库存检查(仅不足禁止时或分批时价药品)
    bln检查库存 = (zlIsCheckPrivs(mstrPrivs, "不检查库存") = False)    '是否有权限不检查库存(分批和时价必须检查)
    For p = 1 To mobjBill.Pages.Count
        For i = 1 To mobjBill.Pages(p).Details.Count
            With mobjBill.Pages(p).Details(i)
                Set colStock = IIf(.收费类别 = "4", mcolStock2, mcolStock1)
        ...
        If bln直接收费 Then
          ...
          For Each mobjBillDetail In mobjBill.Pages(p).Details
              ...
              If InStr(1, ",5,6,7,", "," & mobjBillDetail.收费类别 & ",") > 0 _
                And Not (gbln收费后自动发药 And Not gbln分离发药) Then
                '不含自动发药
                  '药品
    ...
    If Not SaveClinicPriceBill(strNos, blnSaveBill, bln连续) Then
        ...
        If mintInsure <> 0 Then
            cmdAddBill.Enabled = Not MCPAR.门诊连续收费 And _
                MCPAR.多单据收费 And zlIsCheckPrivs(mstrPrivs, "医保病人多单据收费")
        Else
            cmdAddBill.Enabled = zlIsCheckPrivs(mstrPrivs, "普通病人多单据收费")
        End If
...
Private Sub FromBillNoReprintBill(ByVal strNO As String, ByVal blnNOMoved As Boolean)
    ...
    End If
    '调出重打的单据显示
    frmMultiBills.ShowMe Me, 0, mstrPrivs, strNO, "", True
...
Public Function zlSaveBillAndClinicSwapByNo(ByRef lng结帐ID As Long, ByRef strSavedNos As String, _
    ...
    If MCPAR.医保接口打印票据 And MCPAR.医保不走票号 = False Then
        ...
        If Not gblnStrictCtrl Then
            zlDatabase.SetPara "当前收费票据号", Trim(txtInvoice.Text), glngSys, 1121, zlIsCheckPrivs(mstrPrivs, "参数设置")
        End If
    End If
