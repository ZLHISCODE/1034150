...
Private mint退费回单打印 As Integer '退费回单打印方式 0-不打印,1-自动打印,2-选择是否重要形态打印
...
Private Sub cmdSelWholeSet_Click()
    ...
    If Not cmdAddBill.Enabled Or Not cmdAddBill.Visible Then
        ...
        If mobjBill.Pages(1).Details.Count > 0 Then
           ...
           End If
        End If
        If InStr(mstrPrivs, "医生查询") = 0 Then mobjBill.Pages(mintPage).开单人 = ""
        '清除病人信息
       ' Call ClearmobjBill
    Else
        ...
        With mobjBill.Pages(mintPage)
            ...
            If InStr(mstrPrivs, "医生查询") > 0 Then .开单人 = tmpBill.Pages(1).开单人
...
Private Sub Bill_CommandClick()
    ...
    End If
    lng批次 = -1
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, mintInsure, gbln药房单位, str类别, , , str特准项目, 0, str排除类别, False, mbln条码刷卡, lng批次)
    If lng项目id <> 0 Then
        Bill.Text = lng项目id
        Bill.Tag = lng批次
        mblnSelect = True
...
'功能：处理单据输入
    ...
    Dim dblNum As Double, lng批次 As Long
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        End If
                        lng批次 = -1
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, mintInsure, gbln药房单位, str类别, Bill.Text, Bill.TxtHwnd, str特准项目, 0, str排除类别, False, mbln条码刷卡, lng批次)
                        If lng项目id <> 0 Then
                    ...
                    If InStr(",5,6,7,", mobjDetail.类别) > 0 Then
                        ...
                        Else
                            ...
                            If str药房IDs <> "" Then
                                ...
                                If dblStock = 0 And InStr(mstrPrivs, "不检查库存") = 0 And gblnStock Then
                    ...
                    ElseIf mobjDetail.类别 = "4" And mobjDetail.跟踪在用 Then
                        dblStock = GetStock(mobjDetail.ID, lngDoUnit, mobjDetail.批次)
                        mobjDetail.库存 = dblStock
                        Call ShowStock(lngDoUnit, mobjDetail.名称, mobjDetail.库存)
                    End If
                            ...
                            '问题:36558
                             If Not mrsInfo Is Nothing Then
                                If mrsInfo.State = 1 Then
                                     If Not (InStr(",5,6,7,", .Details(Bill.Row).收费类别) > 0 And gbln分离发药) And Nvl(mrsInfo!留观, 0) = 1 Then
                    ...
                    With .Details(Bill.Row)
                        If (.收费类别 = "4" And .Detail.跟踪在用) Or (InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药) Then
                            ...
                            Else
                                ...
                                End If
                            End If
                        ElseIf InStr(",5,6,7,", .收费类别) > 0 And InStr(mstrPrivs, "不检查库存") = 0 And gbln分离发药 Then
...
With mobjBill.Pages(intPage)
    ...
    str费别 = IIf(glngSys Like "8??" Or mbytInFun = 2, mobjBill.费别, TrimChar(mobjBill.费别 & "," & lbl动态费别.Tag, ","))
...
Private Sub cbo费别_Click()
    If mbytInState = 0 Then
        ...
        Else
            ...
            If (mstrYBPati <> "" Or mobjBill.费别 <> NeedName(cbo费别.Text)) And Not mbln不重算价格 Then
                mobjBill.费别 = NeedName(cbo费别.Text)
...
ErrHand:
End Function
Private Function Write_off() As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:记帐销帐处理
    '返回:成功,返回true,否则返回False
    '编制:刘兴洪
    ...
    End If
    '问题:37307
    If (mbytBilling = 0 And Val(txt合计.Text) <> 0) And gbln消费验证 Then
        cur消费金额 = Val(txt合计.Text)
        If Not zlDatabase.PatiIdentify(Me, glngSys, mobjBill.病人ID, cur消费金额, mlngModul, 1) Then Exit Function
    End If
...
Private Function DelChargeFee() As Boolean
    ...
    '问题:37307
    If Val(txt预交冲款.Text) <> 0 And gbln消费验证 Then
...
'    End If
    ...
    strSQL = "zl_门诊收费记录_DELETE('" & mstrInNO & "','" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
        "'" & strBalance & "','" & strSQL & "','" & NeedName(cbo结算方式.Text) & "'," & cur误差金额 & _
    ...
    strSQL = strSQL & "1," & lng冲销ID & "," & lng冲销ID & ",'" & _
            strThreeBalance & IIf(strThreeBalance <> "" And strOtherBalance <> "", ",", "") & strOtherBalance & "' )"
...
'    End If
    ...
    If cbo结算方式.ListIndex >= 0 Then
        ...
        str退结算方式 = NeedName(cbo结算方式.Text)
    Else
        bln退现 = True
    ...
    If gTy_Module_Para.byt票据分配规则 <> 0 And strReclaimInvoice <> "" Then
        ...
        If mblnPrint Then
            ...
            Call RePrintCharge(1, strNos, Me, mlng领用ID, strReclaimInvoice, True, dtDateDel, _
                intInvoiceFormat, , , mlngShareUseID, mstrUseType)
        End If
        GoTo PrintList:
        Exit Sub
    End If
...
Private Function DelInsure(ByVal blnExistThreeSwap As Boolean, _
    ...
    If Not blnExistThreeSwap Then
        strSQL = "zl_门诊收费结算_Update(" & lng冲销ID & ",'" & str收费结算 & "'," & -1 * dbl本次冲预交 & ",'" & strAdvance & "'," & -1 * cur误差金额 & ",NULL,NULL,1)"
        Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
        Call gclsInsure.BusinessAffirm(交易Enum.Busi_ClinicDelSwap, True, intInsure)
        DelInsure = True: Exit Function
    End If
...
Private Function DelCharge() As Boolean
    ...
    If mbytInFun = 2 Then '销帐
        If Write_off = False Then Exit Function
...
Private Function SaveVerify() As Boolean
    ...
    If UBound(Split(strSQL, ",")) + 1 = i Then strSQL = ""
    If Val(txt合计.Text) <> 0 And gbln消费验证 Then
        If Not zlDatabase.PatiIdentify(Me, glngSys, mobjBill.病人ID, Val(txt合计.Text), mlngModul, 1) Then Exit Function
    End If
    '费用报警
        ...
        For i = 1 To mobjBill.Pages(p).Details.Count
            ...
            If mbytInFun = 2 Then
                ...
                '问题:36558
                 If Not mrsInfo Is Nothing Then
                    If mrsInfo.State = 1 Then
                        If Not (InStr(",5,6,7,", mobjBill.Pages(p).Details(i).收费类别) > 0 And gbln分离发药) And Nvl(mrsInfo!留观, 0) = 1 Then
            ...
            With mobjBill.Pages(p).Details(i)
                ...
                ElseIf InStr(",5,6,7,", .收费类别) > 0 And gbln分离发药 And bln检查库存 And gblnStock Then
                    ...
                    If strInfo <> "" Then
                        ...
                        If dblToTal > .Detail.库存 Then
                            If mobjBill.Pages.Count > 1 Then strTmp = "第 " & p & " 张单据"
                            MsgBox strTmp & "第 " & i & " 行药品""" & .Detail.名称 & "]的当前库存" & IIf(InStr(1, mstrPrivs, "显示库存") > 0, .Detail.库存, "") & "不足输入数量""" & _
                                dblToTal & """,请修改或检查是否有多行输入。", vbInformation, gstrSysName
                            Exit Function
                        End If
                    End If
...
Private Function SaveChargeFee() As Boolean
    ...
    '只有记账刷卡验证(冲预交在结算窗口处理)
    If (mbytInFun = 2 And mbytBilling = 0 And Val(txt合计.Text) <> 0) And gbln消费验证 Then
        cur消费金额 = Val(txt合计.Text)
        If Not zlDatabase.PatiIdentify(Me, glngSys, mobjBill.病人ID, cur消费金额, mlngModul, 1) Then Exit Function
    End If
    ...
    If mstrInNO = "" Or (txtModi.Visible And mbytInState = 0 And mstrInNO <> "") Then  '新增,或新增界面通过输入单据号修改单据
...
Private Sub cmdCancel_Click()
    mbln连续输入 = False
    If (Not CheckBillsEmpty Or txtPatient.Text <> "") And mbytInState = 0 Then
        ...
        If strPrintNO <> "" Then
            If mblnPrint Then
                ...
                Else
                    ...
                    Call ReportOpen(gcnOracle, glngSys, "ZL" & glngSys \ 100 & "_BILL_1121_1", Me, "NO='" & strPrintNO & "'", IIf(glngFactMediCare = 0, "", "ReportFormat=" & glngFactMediCare), 2)
                End If
            End If
...
Private Sub cmd预结算_Click()
    ...
    If mbytInFun = 0 And mintInsure <> 0 And MCPAR.实时监控 Then
        ...
        If gclsInsure.CheckItem(mintInsure, 0, 2, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text))) = False Then
            Exit Sub
        End If
    End If
...
'功能:收费或划价时导入单据
    ...
    Else
        ...
        If Not cmdAddBill.Enabled Or Not cmdAddBill.Visible Then
            ...
            Set mobjBill = ImportBill(strNO, False, mbytInFun, , False)
            If mobjBill.NO = "" Then
        ...
        '不提供从后备表中导入的功能
            ...
            For i = 0 To UBound(Split(strNos, ","))
                ...
                Set objBill = ImportBill(strNO, False, mbytInFun, , False)
                ...
                With mobjBill.Pages(mintPage)
                    ...
                    If InStr(mstrPrivs, "医生查询") > 0 Then .开单人 = objBill.Pages(1).开单人
    ...
    '重新计算每行药品库存
     ...
     For i = 1 To mobjBill.Pages(intPage).Details.Count
        With mobjBill.Pages(intPage).Details(i)
            ...
            ElseIf InStr(",5,6,7,", .收费类别) > 0 And gbln分离发药 And InStr(mstrPrivs, "不检查库存") = 0 Then
...
Private Sub txt预交冲款_Validate(Cancel As Boolean)
    ...
    ElseIf Not IsNumeric(txt预交冲款.Text) And txt预交冲款.Text <> "" Then
        ...
        SelAll txt预交冲款: Cancel = True: Exit Sub
...
'''            lbl找补.Visible = False: txt找补.Visible = False
    ...
    If gbln分离发药 Or Not (mbytInState = 0 And (mbytInFun = 0 Or mbytInFun = 1)) Then
        sta.Panels("Drugstore").Visible = False
    End If
    
    If (mbytInFun = 0 Or mbytInFun = 1) And mbytInState = 0 And mstrInNO = "" Then
   ...
   '问题:51488
    ...
    If blnCard And Len(txtPatient.Text) = IDKind.GetCardNoLen - 1 And KeyAscii <> 8 _
        ...
        Else 'b.根据输入读取病人信息成功
            ...
            '就诊卡密码检查
            If mbytInState = 0 And (blnCard Or (blnICCard Or blnIDCard) And mstrPassWord <> "") Then
...
ReDO:
    ...
    '输入病人的权限
    If mbytInFun = 2 Then
        '如果是门帐记帐新增,只能是门诊病人,如果是操作已前的单据,则不限
        If mbytInState = 0 And mstrInNO = "" Then  '新增
            '要包含门诊留观病人:35188
            strWhere = " And  (Nvl(A.当前科室ID,0)=0  or Exists(Select 1 From 病案主页 where 病人ID=A.病人id and  主页ID=A.主页ID and Nvl(病人性质,0)=1))"
        End If
    ...
    If blnYbCheckCard = False And blnCard And IDKind.GetCurCard.名称 Like "姓名*" _
        ...
        If gint病人来源 = 1 And (mbytInFun = 0 Or mbytInFun = 1) And Not gblnInputCard Then
            Set mrsInfo = New ADODB.Recordset
            Exit Function
        End If
        
        '见问题:27364
        If gint病人来源 = 1 And mbytInFun <> 2 Then strWhere = ""
    ...
    ElseIf Left(strInput, 1) = "-" And IsNumeric(Mid(strInput, 2)) Or blnYbCheckCard Then '病人ID
        If gint病人来源 = 1 And (mbytInFun = 0 Or mbytInFun = 1) And (Not gblnInputID And mstrYBPati = "") _
    ...
    ElseIf Left(strInput, 1) = "*" And IsNumeric(Mid(strInput, 2)) Then '门诊号
        If gint病人来源 = 1 And (mbytInFun = 0 Or mbytInFun = 1) And Not gblnInputID Then
            Set mrsInfo = New ADODB.Recordset
            Exit Function
        End If
        If gint病人来源 = 1 And mbytInFun <> 2 Then strWhere = ""
        ...
        Select Case IDKind.GetCurCard.名称
            Case "姓名", "姓名或就诊卡"
                ...
                If Not mblnValid And gblnSeekName And (mbytInFun <> 2 And gblnInputID Or mbytInFun = 2) Then
                    ...
                    Set rsTmp = zlDatabase.ShowSQLSelect(Me, strPati, 0, "病人" & mbytInFun & gint病人来源, 1, "", "请选择病人", False, False, True, vRect.Left, vRect.Top, txtPatient.Height, blnCancel, False, True, strInput & "%", gintNameDays)
                    If Not rsTmp Is Nothing Then
    ...
    If strSQL <> "" Then
        ...
        If Not mrsInfo.EOF Then
            ...
            If gint病人来源 = 1 And mTy_Para.bln住院病人门诊收费 = False Then
                '需要检查是否为在院病人
                '问题:27364 日期:2010-01-13 15:27:50
                If Val(Nvl(mrsInfo!在院)) = 1 And mbytInFun <> 2 Then
                        If gbln病人来源受权限控制 And InStr(1, mstrPrivs, ";参数设置;") = 0 Then
                            '29720
                            '不能转换病人
...
Private Sub txtRePrint_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        '调出重打的单据显示
        frmMultiBills.ShowMe Me, 0, mstrPrivs, txtRePrint.Text, "", True
    ...
    If Not rsTmp.EOF Then
            ...
            If .Detail.变价 Then
                If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                    ...
                    If dblAllTime <> 0 Then
                        dblPrice = Get时价药品应收金额(.执行部门ID, .收费细目ID, .收费类别 = "4", dblAllTime, gstrDec, dblPriceSingle, .Detail.批次)
                        If dblAllTime <> 0 Then
                            '数量未分解完毕
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            With mobjBillIncome
                ...
                '获取项目保险信息,门诊只有医保病人才算
                If mstrYBPati <> "" And mbytInFun = 0 Then
        ...
        With mobjBill.Pages(p)
            ...
            If mbytInFun = 0 Then
                ...
                If cbo结算方式.ListIndex = -1 And .收费结算 <> "" Then
                    Call zlcontrol.CboSetIndex(cbo结算方式.hWnd, GetCboIndex(cbo结算方式, gstr结算方式))
...
'''    txt本次应缴.Visible = False: lbl应缴.Caption = "应缴"
    ...
    If mbytInState = 0 Then
        ...
        '结算方式
        If mbytInFun = 0 Then
            i = GetCboIndex(cbo结算方式, gstr结算方式)
                ...
                If mobjBillDetail.数次 <> 0 Then
                    For Each mobjBillIncome In mobjBillDetail.InComes
                        ...
                        With mobjBillDetail
                            ...
                            '收费、划价的药品行,处理发药窗口
                            If (mbytInFun = 0 Or mbytInFun = 1) And InStr(",5,6,7,", .收费类别) > 0 And Not gbln分离发药 Then
                        ...
                        ElseIf mbytInFun = 1 Or (mbytInFun = 0 And (mblnSaveAsPrice Or mstrYBPati <> "")) Then
                            ...
                            gstrSQL = gstrSQL & "'" & UserInfo.姓名 & "'," & _
                                ...
                                str中药形态 & ",0," & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                        ElseIf mbytInFun = 2 Then
                            '门诊记帐
                            gstrSQL = gstrSQL & IIf(mbytBilling = 1, 1, 0) & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
                                IIf(mobjBillDetail.收费类别 = "4", mlng卫材类别ID, mlng药品类别ID) & ",NULL,'" & mobjBillDetail.摘要 & "'," & ZVal(lng医嘱ID) & ",Null,Null,'|" & mobjBill.Pages(mintPage).煎法 & "'," & _
                                "NULL,NULL,1," & str中药形态 & ",0," & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                        End If
            ...
            If mstrInNO <> "" Then
                ...
                If mbytInFun = 0 And Not mblnSaveAsPrice Then
                    ...
                    strDelBill = "zl_门诊收费记录_DELETE('" & mstrInNO & "','" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
                        "NULL,NULL,'" & NeedName(cbo结算方式.Text) & "',0,To_Date('" & Format(mobjBill.登记时间, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS'))"
...
'      因为多次部份退费时,每次都可能产生误差,原始的误差始终退不完。
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        '划价收费时重新处理发药窗口
        If mbytInFun = 0 And bytFun = 1 And InStr(",5,6,7,", rsTmp!编码) > 0 And Not gbln分离发药 Then
            j = j + 1
...
'     bytFlag-记录性质，1-收费或划价，2-记帐
    ...
    '-------------------------------------------------------------------------------------
    '是否已转入后备数据表中
    If mbytInFun = 0 Or mbytInFun = 2 Then
    ...
    'b.读取费用明细到单据对象
    '---------------------------------------------------------------------------------------------------
    Set mobjBill = ImportBill(strNO, True, mbytInFun, mintInsure)
    If mobjBill.NO = "" Then
    ...
    mbln不重算价格 = True
        ...
        'b.划价或收费时，未建档的病人
            ...
            cboSex.ListIndex = GetCboIndex(cboSex, mobjBill.性别)
            ...
            If strTmp <> "" Then cbo费别.ListIndex = GetCboIndex(cbo费别, strTmp)
        End If
        If cbo费别.ListIndex = -1 And cbo费别.ListCount > 0 Then cbo费别.ListIndex = 0
    mbln不重算价格 = False
...
'返回：科室个数
    ...
    If mrs开单科室 Is Nothing Then
        If mbytInFun = 1 Then
            ...
            Call GetDoctorDept(mrs开单科室, bln仅操作员部门, str性质)
        Else
            Call GetDoctorDept(mrs开单科室, bln仅操作员部门)
        End If
    End If
...
'功能：显示合约单位病人并选择
    ...
    Set rsTmp = zlDatabase.ShowSelect(Me, strSQL, 2, "合约单位病人", , , "请先选择合约单位，再选择病人", , , , , , , blnCancel)
...
Private Function ExecuteSquareUpdate(ByVal rsSquare As ADODB.Recordset, Optional blnTran As Boolean = False) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:数据保存
    '入参:rsSquare-刷卡结算数据
    '出参:
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, strTemp As String, cllPro As Collection
    ...
    ExecuteProcedureArrAy cllPro, Me.Caption, Not blnTran, Not blnTran
...
RePrint:
        ...
        Call frmPrint.ReportPrint(1, strNos, strModiNos, strReclaimInvoice, mlng领用ID, mlngShareUseID, txtInvoice.Text, mobjBill.登记时间, CStr(mdbl缴款), CStr(mdbl找补), _
            gTy_Module_Para.bln分别打印 And mbytBillSource <> 4, mintInvoiceFormat, , , mstrUseType, blnPrintBillEmpty)
        ...
        If UBound(arrSQL) >= 0 Then
            ...
            Else
                On Error GoTo errH
                    ...
                    '医保处理
                    If mstrYBPati <> "" Then
                        If zlInsure_OneBillClinicSwap(lng结帐ID, strBillNO, strInvoice, strDelBill <> "", p, Original.结帐ID, blnPriceSaved) = False Then
                           'If strSaveNos <> "" Then strSaveNos = Mid(strSaveNos, 2)
...
ErrHand:
    If ErrCenter = 1 Then Resume
End Function
Private Function zlInsure_OneBillClinicSwap(ByVal lng结帐ID As Long, _
    ...
    Err = 0: On Error GoTo errHandle:
    If blnTransMedicare Then Call gclsInsure.BusinessAffirm(交易Enum.Busi_ClinicSwap, True, mintInsure)
    zlInsure_OneBillClinicSwap = True
    Exit Function
ErrModifyTag:
