...
'            If mfrmNormal.vsfMain.TextMatrix(mfrmNormal.vsfMain.Row, mfrmNormal.vsfMain.ColIndex("结算序号")) = "" Then Exit Sub
        ...
        Case conMenu_Edit_BalanceCancel
            ...
            If mfrmErr.vsfMain.TextMatrix(mfrmErr.vsfMain.Row, mfrmErr.vsfMain.ColIndex("结算ID")) = "" Then Exit Sub
            If frmReplenishTheBalanceBill.zlEditCard _
            (Me, mlngModule, mstrPrivs, EM_Balance_Err_Cancel, mfrmErr.vsfMain.TextMatrix(mfrmErr.vsfMain.Row, mfrmErr.vsfMain.ColIndex("结算ID"))) Then
                Call RefreshData
            End If
...
Private Sub zlRptPrint(ByVal bytFunc As Integer)
    ...
    If mfrmNormal.ActiveControl Is mfrmNormal.vsfMain And tabMain.Selected.Index = 0 Then
        ...
        End With
        Set vsBill = mfrmNormal.vsfMain: strTittle = GetUnitName & "费用补充结算信息"
    End If
    ...
    If mfrmErr.ActiveControl Is mfrmErr.vsfMain And tabMain.Selected.Index = 1 Then
        With mfrmErr.vsfMain
            ...
            If .Rows = 2 And Val(.TextMatrix(1, .ColIndex("结算序号"))) = 0 Then Exit Sub
        End With
        Set vsBill = mfrmErr.vsfMain: strTittle = GetUnitName & "费用补充异常结算信息"
    End If
    ...
    If mfrmRefund.ActiveControl Is mfrmRefund.vsfMain And tabMain.Selected.Index = 2 Then
        With mfrmRefund.vsfMain
            ...
            If .Rows = 2 And Val(.TextMatrix(1, .ColIndex("结算序号"))) = 0 Then Exit Sub
        End With
        Set vsBill = mfrmErr.vsfMain: strTittle = GetUnitName & "费用补充异常退费信息"
    End If
...
'        objRow.Add "收款员:" & mfrmFeeGroupCollectFee.lvwSubWorker.SelectedItem.Text
'    End If
    ...
    With vsBill
        ...
        For i = 0 To .COLS - 1
            ...
            '82346,冉俊明,2015-2-2
            If .ColHidden(i) = True Then ' Or i = 0 Then
                .ColWidth(i) = 0
            End If
        Next
    End With
...
'        mcbrControl.Visible = zlCheckPrivs(mstrPrivs, "参数设置")
    ...
    With mcbrPopupMain.Controls
        ...
        Set mcbrControl = .Add(xtpControlButton, conMenu_File_FeeCollect, "收费员轧帐")
        mcbrControl.IconId = 3588
        mcbrControl.Visible = zlCheckPrivs(mstrPrivsRollingCurtain, "轧帐")
        Set mcbrControl = .Add(xtpControlButton, conMenu_File_Parameter, "参数设置")
'        mcbrControl.Visible = zlCheckPrivs(mstrPrivs, "参数设置")
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, Optional blnCard As Boolean) As Boolean
    ...
    strSQL = ""
    If blnCard And objCard.名称 Like "姓名*" And (InStr("-+*", Left(strInput, 1)) = 0 And IsNumeric(Mid(strInput, 2))) Then   '103563
                ...
                If Not blnSame Then
                    ...
                    Else
                       ...
                       '问题号:50485
                        ...
                        vRect = GetControlRect(txtIdentify.hWnd)
...
Private Sub RefreshFact()
    ...
    Else
        '松散：取下一个号码
        mstrInvoice = IncStr(UCase(zlDatabase.GetPara("当前收费票据号", glngSys, mlngModule)))
    End If
End Sub
