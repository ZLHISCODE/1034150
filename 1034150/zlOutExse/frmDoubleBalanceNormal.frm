    ...
    If intType = 0 Then
        If mfrmFilter.mblnInit = True Then
            With mfrmFilter
                ...
                strFilter = strFilter & IIf(.txt住院号.Text = "", "", " And C.住院号=[6] ")
                ...
                Else
                    strTable = "费用补充记录 A"
                End If
                strSQL = " Select A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费') As 附加标志, B.姓名, B.性别, B.年龄," & vbNewLine & _
                        "       Sum(B.结帐金额) As 结帐金额, A.操作员姓名, A.登记时间, A.结算序号, Max(A.记录状态) As 退费标志," & vbNewLine & _
                        "       A.病人ID,A.实际票号" & str红票已打印 & vbNewLine & _
                        " From " & strTable & ", 门诊费用记录 B, 病人信息 C " & _
                        " Where A.登记时间 Between [1] And [2] And A.病人ID=C.病人ID And A.收费结帐ID=B.结帐ID" & vbNewLine & _
                        "       And Nvl(A.费用状态,0)=0 And A.记录状态 In (1,3)" & vbNewLine & _
                        "       And Not Exists (Select 1 From 费用补充记录 Where 结算序号=A.结算序号 And 记录状态=2) " & strFilter & _
                        " Group By A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费'), B.姓名, B.性别, B.年龄," & vbNewLine & _
                        "       A.操作员姓名, A.登记时间, A.结算序号,A.病人ID,A.实际票号"
                         
                If .chkDelRecord.Value = 1 Then
                    strSQL = strSQL & " Union " & _
                        " Select NO, 附加标志, 姓名, 性别, 年龄, -1 * Sum(实收金额) As 结帐金额, " & vbNewLine & _
                        "       操作员姓名, 登记时间, 结算序号, 2 As 退费标志, 病人ID, 实际票号" & str红票已打印 & vbNewLine & _
                        " From (Select Distinct a.No, Decode(Nvl(a.附加标志, 0), 1, '挂号', '收费') As 附加标志," & vbNewLine & _
                        "           b.姓名, b.性别, b.年龄, b.实收金额,a.操作员姓名, a.登记时间, a.结算序号," & vbNewLine & _
                        "           b.No As 单据号, b.结帐id, a.病人id, a.实际票号" & _
                        "       From  " & strTable & ", 门诊费用记录 B, 病人预交记录 D, 病人信息 C" & _
                        "       Where a.登记时间 Between [1] And [2] And a.结算序号 = d.结算序号 " & vbNewLine & _
                        "             And b.结帐id = d.结帐id And a.病人id = c.病人id" & _
                        "             And Nvl(a.费用状态, 0) = 0 And a.记录状态 = 2 " & strFilter & ") A" & _
                        " Group By NO, 附加标志, 姓名, 性别, 年龄, 操作员姓名, 登记时间, 结算序号,病人ID,实际票号" & vbNewLine & _
                        " Order By 登记时间 Desc"
                End If
                Set rsMain = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, DatBegin, DatEnd, _
                                                    .txt姓名.Text, NeedName(.cbo操作员.Text), .txt门诊号.Text, .txt住院号.Text, _
        ...
        Else
            strSQL = " Select A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费') As 附加标志, B.姓名, B.性别, B.年龄, " & vbNewLine & _
                    ...
                    "       And A.操作员姓名=[1] " & _
                    " Group By A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费'), B.姓名, B.性别, B.年龄, " & vbNewLine & _
                    "       A.操作员姓名, A.登记时间, A.结算序号,A.病人ID,A.实际票号" & vbNewLine & _
                    " Order By 登记时间 Desc"
    ...
    If intType = 1 Then
        ...
        strSQL = " Select A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费') As 附加标志, B.姓名, B.性别, B.年龄," & vbNewLine & _
                ...
                "       A.病人ID,A.实际票号" & str红票已打印 & vbNewLine & _
                 ...
                 "      A.操作员姓名, A.登记时间, A.结算序号,A.病人ID,A.实际票号"
        strSQL = strSQL & " Union " & _
                " Select NO, 附加标志, 姓名, 性别, 年龄, -1 * Sum(实收金额) As 结帐金额, 操作员姓名," & vbNewLine & _
                "       登记时间, 结算序号, 2 As 退费标志, 病人ID, 实际票号" & str红票已打印 & vbNewLine & _
                " From (Select Distinct a.No, Decode(Nvl(a.附加标志, 0), 1, '挂号', '收费') As 附加标志, " & vbNewLine & _
                "           b.姓名, b.性别, b.年龄, b.实收金额, a.操作员姓名, a.登记时间, a.结算序号," & vbNewLine & _
                "           b.No As 单据号, b.结帐id, a.病人id,a.实际票号" & _
...
Private Sub ReadBalance(ByVal lngBalanceID As Long)
    ...
    strSQL = _
        "Select Decode(Mod(a.记录性质,10),1,'冲预存款',Nvl(a.结算方式,'未结金额')), Sum(a.冲预交) As 冲预交, Max(a.结算号码) As 结算号码, " & _
        "       Max(a.摘要) As 摘要, Max(a.卡号) As 卡号, Max(a.交易流水号) As 交易流水号, Max(a.交易说明) As 交易说明, Nvl(c.性质,0) As 性质" & vbNewLine & _
        " From 病人预交记录 A, 结算方式 C" & _
        " Where a.结算序号 = [1] And A.结算方式=C.名称(+) " & vbNewLine & _
        " Group By Decode(Mod(a.记录性质,10),1,'冲预存款',Nvl(a.结算方式,'未结金额')), Nvl(c.性质,0) " & _
        " Order By 性质"
...
Private Sub ReadBalanceInfo(ByVal lngBalanceID As Long)
    ...
    strSQL = _
        "   Select Min(b.交易id) As ID, a.结算方式, Max(c.名称) As 名称, Sum(a.冲预交) As 金额, b.交易项目, b.交易内容" & _
        "   From " & IIf(mblnNOMoved, "H", "") & "病人预交记录 A, " & IIf(mblnNOMoved, "H", "") & "三方结算交易 B, 医疗卡类别 C" & _
        "   Where a.Id = b.交易id And a.卡类别id = c.Id(+) And a.结算序号 = [1]" & _
        "   Group By a.结算方式, b.交易项目, b.交易内容" & _
        "   Order By ID"
...
Private Sub ReadInVoice(ByVal lngBalanceID As Long)
    ...
    If gTy_Module_Para.byt票据分配规则 <> 0 Then
        ...
        If rsInvoice.RecordCount = 0 Then GoTo ReadOld:
    Else
ReadOld:
        strSQL = _
        " Select Distinct B.ID, B.号码 as 票据号,Decode(B.原因,1,'正常发出',2,'作废收回',3,'重打发出',4,'重打收回',6,'红票正常发出') as 使用原因," & _
