...
'        str退结算方式 = NeedName(cbo退款方式.Text)
'    End If
...
Private Function CheckThreeSwapValied(ByVal blnCur部分退费 As Boolean, _
    ...
    If cbo退款方式.Visible Then
        mrsBalance.Filter = "结帐ID=" & lng结帐ID & " And 性质>=3 and 性质<=4  and 结算方式='" & NeedName(cbo退款方式.Text) & "'"
        With mrsBalance
            If .RecordCount <> 0 Then .MoveFirst
            Do While Not .EOF
        ...
        Do While Not .EOF
            ...
            If blnCur部分退费 And Val(Nvl(!是否全退)) = 1 And blnMulitNo Then
                ...
                ElseIf bln退现 = False Then
                    MsgBox "当前单据使用了第三方结算交易,所有单据必须全退！", vbInformation, gstrSysName
                    Exit Function
                ElseIf InStr(str三方结算方式 & ",", "," & NeedName(cbo退款方式.Text) & ",") > 0 Then
                    MsgBox "当前单据使用了第三方结算交易,所有单据必须全退！", vbInformation, gstrSysName
                    Exit Function
                End If
            End If
            ...
            If cbo退款方式.Visible And cbo退款方式.Enabled Then
                If bln退现 And blnCur部分退费 And InStr(str三方结算方式 & ",", "," & NeedName(cbo退款方式.Text) & ",") > 0 Then
                    MsgBox "当前单据使用了第三方结算交易,单张单据必须全退！", vbInformation, gstrSysName
                    Exit Function
                End If
                If InStr(str三方结算方式 & ",", "," & NeedName(cbo退款方式.Text) & ",") > 0 Then
            ...
            If Not bln退现 And Not blnCur部分退费 Then
                ...
                If gbln消费验证 And Val(Nvl(!性质)) = 4 Then
                    '需要验卡
...
Private Function DelInsureMulitOneBalance(ByVal blnExistThreeSwap As Boolean, _
    ...
    For k = 0 To UBound(arrNo)
        ...
        If Not blnExistThreeSwap Then
                strSQL = "zl_门诊收费结算_Update(" & lng结帐ID & ",'" & str退回结算 & "',0,'" & strBalance & "'," & -1 * cur误差金额 & ",NULL,NULL,1)"
                Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
        Else
            'Zl_医保结算校对_Update
...
Private Function DelThreeSwapFee(ByVal varNO As Variant, _
    ...
    Set cllBlance = New Collection
     If cbo退款方式.Enabled And cbo退款方式.Visible Then
        mrsBalance.Filter = "性质>=3 and  性质<=4 and 结算方式='" & NeedName(cbo退款方式.Text) & "'"
        If mrsBalance.RecordCount <> 0 Then
            With mrsBalance
                bln退指定金额 = True
                varData = Array(0, 0, "", "", "", 0, "")
    ...
    For i = UBound(arrNo) To 0 Step -1
        ...
        If colOrder("_" & strNO) <> "未选择" Then
           ...
           '60974
            ...
            strSQL = strSQL & "'" & NeedName(cbo退款方式.Text) & "',"
                    ...
                    If mintInsure <> 0 And mblnYB结算作废 Then
                        ...
                        If Not DelInsureOneBill(str医保结算, blnExistThreeSwap, lng结帐ID, lngPage, lngPages, blnCommited) Then
                            ...
                            If strDelNOs <> "" And blnExistThreeSwap And blnExistOneCardSwap Then
                                ...
                                If Not DelSawp_SpecifyNOs(varTemp, blnExistThreeSwap, blnExistOneCardSwap, strNO, blnCommited) Then
                                    If Not blnCommited Then gcnOracle.RollbackTrans
                                    Exit Function
                                End If
...
Private Function DelSawp_SpecifyNOs(ByVal varNO As Variant, _
    ...
    blnCommited = False
    If UBound(varNO) < 0 Then DelSawp_SpecifyNOs = True: Exit Function
    If blnExistOneCardSwap = False And blnExistThreeSwap = False Then DelSawp_SpecifyNOs = True:  Exit Function
    '存在一卡通
    ...
    With mrsBalance
         ...
         Do While Not .EOF
            ...
            Case Else
               ...
               If !结算方式 = NeedName(cbo退款方式) And bln原样退 = False Then
                    bln退现 = True
               End If
            ...
            With mrsBalance
                Do While Not .EOF
                    ...
                    Case Else
                        blnFind = False
                        For intCol = 1 To vsBalance.COLS - 1 Step 2
                            If vsBalance.Cell(flexcpData, 1, intCol) = NeedName(cbo退款方式.Text) _
                                And vsBalance.Cell(flexcpData, 1, intCol) = !结算方式 Then
                        ...
                        blnFind = False
                        For intCol = 1 To vsBalance.COLS - 1 Step 2
                            If NeedName(cbo退款方式.Text) <> vsBalance.Cell(flexcpData, 1, intCol) _
