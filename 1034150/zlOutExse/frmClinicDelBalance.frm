...
Private Function IsTurnFee(ByVal strNos As String) As Boolean
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "检查是否门诊转住院费用", strNos)
    If rsTmp.EOF Then
        IsTurnFee = False
    Else
        IsTurnFee = True
...
Private Function CheckThreeSwapValied(ByVal objCard As Card, Optional dblDelMoney As Double) As Boolean
    ...
    If CheckThreeSwapCanTransfer(objCard, mobjDelBalance.原结帐ID) Then
        ...
        '       </IN>
         If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModule, Nothing, objCard.接口序号, _
             ...
             True, True, bln退现, True, Nothing, False, "<IN><CZLX>1</CZLX></IN>") = False Then Exit Function
    ...
    Else
        ...
        strXMLExpend = IIf(mbytFunc = EM_FUN_重退, 1, "")
        ...
        If objCard.是否退款验卡 Then
           '弹出刷卡界面
            ...
            If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModule, Nothing, objCard.接口序号, _
                objCard.消费卡, mobjDelBalance.姓名, mobjDelBalance.性别, _
                mobjDelBalance.年龄, dblMoney, mCurBrushCard.str卡号, mCurBrushCard.str密码, _
                True, True, bln退现, True, Nothing, False, "<IN><CZLX>2</CZLX></IN>") = False Then Exit Function
        End If
...
Private Function CheckPrepayMoneyIsValied(ByVal objCard As Card, Optional ByVal intType As Integer, Optional ByVal dblMoney As Double) As Boolean
    ...
    If strTittle = "冲" Then
        If zlDatabase.PatiIdentify(Me, glngSys, mobjDelBalance.病人ID, dblMoney, mlngModule, 1) = False Then Exit Function
    ...
    '退预交款
    If gbln消费验证 = False Then CheckPrepayMoneyIsValied = True: Exit Function
    If Not zlDatabase.PatiIdentify(Me, glngSys, mobjDelBalance.病人ID, dblMoney) Then Exit Function
    CheckPrepayMoneyIsValied = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
Private Sub LoadPatiInfor()
    ...
    Set rsTemp = GetMoneyInfo(mobjDelBalance.病人ID, 0, False, 1, False)
    With mCurCarge
        .dbl预交余额 = 0
        .dbl费用余额 = 0
        If Not rsTemp.EOF Then
...
Private Function ExecuteSquareDelInterface(ByVal cllSquareCard As Collection, _
    ...
    For i = 1 To cllSquareCard.Count
        ...
        If RoundEx(Val(varTemp(2)), 6) > RoundEx(dblMoney, 6) Then
            MsgBox objCard.结算方式 & " 的退款金额超过了原始剩余结算金额！" & vbCrLf & "原结算金额:" & Format(dblMoney, "0.00") & vbCrLf & " 现退款金额:" & Format(Val(varTemp(2)), "0.00"), vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
        '卡类别ID|卡号|消费卡ID|消费金额||.
    ...
    With vsBlance
        For i = 1 To .Rows - 1
            ...
            If (Val(.TextMatrix(i, .ColIndex("删除标志"))) = 0 Or Val(.TextMatrix(i, .ColIndex("结算状态"))) = 0) Then
                Select Case int类型
                Case 1 '预交款
                    If gbln消费验证 And Val(.TextMatrix(i, .ColIndex("是否验证"))) = 0 Then
            ...
            If InStr(1, strCardTypeIDs & ",", "," & lng卡类别ID & ",") = 0 Then
                ...
                If RoundEx(dblMoney, 6) <> 0 And RoundEx(dblTemp, 6) = 0 Then
                    ...
                    For j = 1 To vsBlance.Rows - 1
                        '类型:0-普通结算;1-预交款;2-医保,3-一卡通;4-一卡通(老);5-消费卡
                        If Val(vsBlance.TextMatrix(j, vsBlance.ColIndex("类型"))) = 5 Then
    ...
    If objCard.是否退款验卡 Then
       '弹出刷卡界面
        ...
        If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModule, Nothing, objCard.接口序号, _
            ...
            True, True, False, False, cllSquareBalance, False, "<IN><CZLX>2</CZLX></IN>") = False Then Exit Function
...
Private Function CheckIsExistCashValied(ByVal objCard As Card) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    On Error GoTo errHandle
    If mCurCarge.dbl当前未退 >= 0 Then CheckIsExistCashValied = True: Exit Function
    Set rsBalance = mobjDelBalance.rsBalance
