...
' bytExitMode:操作退出模式
' bln继续输入-是否继续输入
' blnOperationSucces-当前操作是否成功
Public Event FunOperation(ByVal bytExitMode As gExitMode, ByVal bln继续输入 As Boolean, _
    ByVal blnOperationSucces As Boolean, ByRef objChargeInfor As clsClinicChargeInfor)
...
Private Sub LoadData()
 ...
 gstrSQL = gstrSQL & " Union ALL " & _
    "   Select A.ID,5 as  类型,Mod(A.记录性质,10) as 记录性质,A.结算方式,nvl(b.结算金额,0) as 冲预交,A.摘要,A.卡类别ID,A.结算卡序号," & _
    ...
    "   From 病人预交记录 A ,病人卡结算对照 C,病人卡结算记录 B, " & _
    "        卡消费接口目录 M " & _
...
Private Sub InitFace()
   ...
   With mCurCardPay
    ...
    " Where A.应用场合 = '收费' And B.名称 = A.结算方式 " & _
...
Private Function CheckOneCard() As Boolean
    ...
    False, True, False, False, Nothing, False, "<IN><CZLX>2</CZLX></IN>") = False Then Exit Function
            ...
            Case 3  '一卡通(新)
                If Val(Nvl(!校对标志)) = 2 Then
                    ...
                    If zlCheckDelValied(Val(Nvl(!卡类别ID)), CStr(Nvl(!卡类别名称)), False, _
                        Nvl(!卡号), Nvl(!交易流水号), Nvl(!交易说明), mobjChargeInfor.结帐ID, _
                        dblMoney, Val(Nvl(!是否退款验卡)) = 1) = False Then Exit Function
...
Private Sub ExcuteMainReshData(ByVal bytExitMode As gExitMode)
    ...
    If Not gfrmMain Is Nothing Or glngMain <> 0 Then Exit Sub
...
Private Sub LoadPatiInfor()
    ...
    Set rsTemp = GetMoneyInfo(mobjChargeInfor.病人ID, 0, False, 1, False)
    With mCurCarge
        .dbl预交余额 = 0
        .dbl费用余额 = 0
        If Not rsTemp.EOF Then
...
Private Function BrushcardStrikePrepay() As Boolean
    ...
    If zlDatabase.PatiIdentify(Me, glngSys, mobjChargeInfor.病人ID, Val(txt冲预交), mlngModule, 1, mlngBrushCardTypeID) Then
        lbl冲预交.Tag = "1"
       ' txt冲预交.ForeColor = d
       txt冲预交.BackColor = Me.BackColor
...
Private Function BrushCardThreeSwapCheck() As Boolean
    ...
    mobjChargeInfor.姓名, mobjChargeInfor.性别, mobjChargeInfor.年龄, dblMoney, mCurCardPay.str刷卡卡号, mCurCardPay.str刷卡密码, _
    False, True, False, False, cllSquareBalance, False, "<IN><CZLX>0</CZLX></IN>") = False Then Exit Function
    '消费卡附值
    If mCurCardPay.bln消费卡 Then
...
Private Function SaveCharge(Optional bln预交 As Boolean, _
        ...
        Else
            If dbl剩余金额 < 0 Then
                If mstr退支票 = "" Then
                    ...
                    Exit Function
                End If
                dbl退支付额 = -1 * Val(txt找补.Text)
                mobjChargeInfor.收费结算 = mobjChargeInfor.收费结算 & "|" & mstr退支票 & ":" & -1 * dbl退支付额 & ":2"
            End If
        End If
    Else
        '问题:58344
    ...
    'mCurCarge.dbl本次误差费 = Format(-1 * (mCurCarge.dbl本次实收 - mCurCarge.dbl本次已付合计 - dblMoney - dbl退支付额), gstrDec)
    '误差不能大于10块钱
...
GoOver:
    ...
    If Not blnHaveMoney Then
        ...
        If mblnElsePersonErrBill Then
            If UpdateElsePersonErrBill() = False Then blnTrans = False: Exit Function
        End If
        If ChargeOver(dbl退支付额) = False Then blnTrans = False:    Exit Function
        gcnOracle.CommitTrans:  mblnCommitBill = True
        mblnElsePersonErrBill = False '已更新
        blnTrans = False
        Call WhriteTotalDataToReCord(IIf(bln预交, dblMoney, 0), IIf(Not bln预交, dblMoney, 0), dbl退支付额)
    ...
    With vsBlance
        ...
        Else
            ...
            If Not bln预交 Then '预交款不显示结算号码、摘要、卡号、交易流水号、交易说明
     ...
     If bln是否退款验卡 Then
       '弹出刷卡界面
        ...
        If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModule, Nothing, lng卡类别ID, _
            ...
            True, True, False, True, Nothing, False, "<IN><CZLX>2</CZLX></IN>") = False Then Exit Function
    End If
goEnd:
...
Private Function zlGetInvoiceGroupUseID(ByRef lng领用ID As Long, _
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    On Error GoTo errHandle
    lng领用ID = GetInvoiceGroupID(1, intNum, lng领用ID, mobjChargeInfor.ShareUserID, strInvoiceNO, mobjChargeInfor.PatiUseType)
    If lng领用ID <= 0 Then
        Select Case lng领用ID
            Case 0 '操作失败
            Case -1
                If Trim(mobjChargeInfor.PatiUseType) = "" Then
                    MsgBox "你没有自用和共用的收费票据,请先领用一批票据或设置本地共用票据！", vbInformation, gstrSysName
                Else
                    MsgBox "你没有自用和共用的『" & mobjChargeInfor.PatiUseType & "』收费票据,请先领用一批票据或设置本地共用票据！", vbInformation, gstrSysName
                End If
                Exit Function
            Case -2
                If Trim(mobjChargeInfor.PatiUseType) = "" Then
                ...
                Else
                    MsgBox "本地的共用票据的『" & mobjChargeInfor.PatiUseType & "』收费票据已经用完,请先领用一批票据或重新设置本地共用票据！", vbInformation, gstrSysName
                End If
                Exit Function
        End Select
    End If
...
'    End If
    ...
    If mInsurePara.医保接口打印票据 And mobjChargeInfor.医保不走票号 = False Then
        ...
        If Not gblnStrictCtrl Then
            zlDatabase.SetPara "当前收费票据号", mobjChargeInfor.当前发票号, glngSys, 1121, zlIsCheckPrivs(mstrPrivs, "参数设置")
        End If
    End If
    ...
    If Not zlInsureCheck(mobjChargeInfor.预结结算, strAdvance) Or strAdvance = "" Then
        ...
        If Not mInsurePara.多单据分单据结算 Then
            Call gclsInsure.BusinessAffirm(交易Enum.Busi_ClinicSwap, True, mobjChargeInfor.intInsure)
        End If
