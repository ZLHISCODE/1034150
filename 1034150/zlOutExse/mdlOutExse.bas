...
Private Type Ty_Module_Para
    ...
    bln医保结算光标缺省定位 As Boolean '102973
    bln体检分别打印 As Boolean '104983
    byt刷卡缺省金额操作 As Byte '86853
    bln禁止取消挂号划价单 As Boolean '120836
End Type
...
'说明：在同一材质类药房的发药窗口内平均分配
    ...
    If gbytAssign = 0 Then
        ...
        "   Group by 发药窗口 " & _
        "   Order by Num"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", Curdate, lng药房ID)
        If Not rsTmp.EOF Then
            Get发药窗口 = Nvl(rsTmp!发药窗口)
        End If
    Else
        '平均分配方式
        strSQL = _
            " Select 编码,名称 as 发药窗口,Nvl(当前分配,0) as 活动 " & _
            " From 发药窗口 " & _
            " Where 上班否=1 And Nvl(专家,0)=0 And 药房ID=[1]" & _
            " Order by 编码"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", lng药房ID)
        If Not rsTmp.EOF Then
            rsTmp.Find "活动=1"
            If Not rsTmp.EOF Then rsTmp.MoveNext
            If rsTmp.EOF Then rsTmp.MoveFirst
            Get发药窗口 = Nvl(rsTmp!发药窗口)
            
            strSQL = "Update 发药窗口 Set 当前分配=Decode(编码,'" & rsTmp!编码 & "',1,0) Where 药房ID=" & lng药房ID
            Call SQLTest(App.ProductName, "mdlOutExse", strSQL)
            gcnOracle.Execute strSQL: Call SQLTest
        End If
    End If
...
Public Sub GetDoctorDept(ByRef rsTmp As ADODB.Recordset, _
    Optional ByVal bln仅操作员部门 As Boolean, Optional ByVal str性质 As String = "'临床','产科'")
    '功能：获取所有开单科室
    '入参：
    '     bln仅操作员部门-操作员的所属部门
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", UserInfo.ID, str性质)
...
Public Function CheckBillExistReplenishData(intType As Integer, Optional lngBalance As Long, Optional strNos As String, Optional lng打印ID As Long) As Boolean
    ...
    Else
        ...
        Else
            ...
            "   Where a.收费结帐id = b.结帐id And a.记录性质 = 1 And a.附加标志 = 0 And Nvl(a.费用状态,0) <> 2 And Rownum < 2"
            Set rsTmp = OpenSQLRecordByArray(strSQL, "检查二次结算", varData)
        End If
    End If
...
'功能：初始化费用模块参数
    ...
    If lngModul = 1120 Or lngModul = 1121 Then
        ...
        ElseIf lngModul = 1121 Then
            ...
            With gTy_Module_Para
                ...
                .bln禁止取消挂号划价单 = Val(zlDatabase.GetPara("禁止取消挂号划价单", glngSys, lngModul, "0")) = "1"
            End With
    ...
    If lngModul = 1120 Or lngModul = 1121 Or lngModul = 1122 Then
        If gint病人来源 = 0 Then gint病人来源 = 1   '门诊记帐
        If gint病人来源 = 1 Then
...
Public Function MakeDetailRecord(ByRef objBill As ExpenseBill, ByVal str开单人 As String, ByVal str开单科室 As String, _
    Optional ByVal intPage As Integer, Optional ByVal lngRow As Long) As ADODB.Recordset
    ...
    For p = intStartPage To intPages
        If objBill.Pages(p).NO <> "" Then '提取划价单
            strSQL = "Select 病人ID, NULL as 主页ID, 收费类别, 收费细目id, Avg(数次 * Nvl(付数, 1)) 数量," & vbNewLine & _
                    "        Sum(标准单价) As 单价, Sum(实收金额) 实收金额" & vbNewLine & _
                    " From 门诊费用记录" & vbNewLine & _
...
Public Function ImportBill(strNO As String, blnModi As Boolean, bytFlag As Byte, Optional ByVal int险类 As Integer, Optional ByVal bln工本费 As Boolean = True, Optional ByVal bln不读煎法 As Boolean) As ExpenseBill
...
'      不管是导入还是修改单据,都不应包含已停用收费细目
    ...
    Dim dblPrice As Double, dbl加班加价率 As Double
    Dim dblStock As Double, str药房IDs As String, str停用项目序号 As String, dblPriceSingle As Double
    ...
    Else
        ...
        strSQL = "Select * From (" & strSQL & ")" & _
            " Where Not(Instr(',4,5,6,7,',收费类别)>0 And (分批=1 Or 是否变价=1))" & _
            " Order by 序号"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", strNO, IIf(bytFlag = 0, 1, bytFlag), IIf(blnModi, ",0,1,", ",0,1,3,"))
...
NextRecord: Do While Not .EOF
                ...
                With objBillDetail
                    ...
                    If InStr(",5,6,7,", .收费类别) > 0 And gbln分离发药 Then
                        objBill.Pages(1).Details(objBill.Pages(1).Details.Count).Key = 1
                    End If
...
Public Function RePrintCharge(ByVal BytType As Byte, strNos As String, frmParent As Object, _
                            ...
                            Optional lngShareUseID As Long, Optional strUseType As String = "", Optional blnOnePatiPrint As Boolean = False) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:当前收款记录重新打印一张票据
    '入参:1-重打;2-补打
    ...
    Dim intFormat As Integer, lng领用IDTemp As Long
    ...
    If blnDo Then
        ...
        If blnDelOpt Then
            Call frmPrint.ReportPrint(bytPrintType, strNos, "", strReclaimInvoice, lng领用ID, lngShareUseID, strInvoice, DateDel, , , bln分别打印, intPrintFormat, blnVirtualPrint, , strUseType, , blnOnePatiPrint, lng打印ID)
        Else
            Call frmPrint.ReportPrint(bytPrintType, strNos, "", strReclaimInvoice, lng领用ID, lngShareUseID, strInvoice, zlDatabase.Currentdate, , , bln分别打印, intPrintFormat, blnVirtualPrint, blnDelRecord, strUseType, , blnOnePatiPrint, lng打印ID)
        End If
        RePrintCharge = True
    End If
End Function
...
Public Sub GetErrorItem(ByRef lng误差细目id As Long, ByRef str收据费目 As String)
...
'说明：该项目不应撤档，也应为变价项目(未管)。
    ...
    On Error GoTo errH
    strSQL = "Select a.Id As 收费细目id,c.收据费目" & vbNewLine & _
            " From 收费项目目录 a, 收费价目 b, 收入项目 c, 收费特定项目 d" & vbNewLine & _
            " Where d.特定项目 = '误差项' And d.收费细目id = a.Id And a.Id = b.收费细目id And b.收入项目id = c.Id And" & vbNewLine & _
            "       ((Sysdate Between b.执行日期 And b.终止日期) Or (Sysdate >= b.执行日期 And b.终止日期 Is Null))"
          
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "mdlOutExse")
    If Not rsTmp.EOF Then
        lng误差细目id = Val("" & rsTmp!收费细目ID)
        str收据费目 = "" & rsTmp!收据费目
    End If
    Exit Sub
errH:
...
Public Function Get收费执行科室ID( _
    ...
    Optional ByVal lng执行科室ID As Long, Optional ByVal lng病人病区ID As Long, _
    Optional ByVal lng成套缺省执行科室 As Long) As Long
'功能：获取收费项目的执行科室
'参数：int范围=1.门诊,2-住院
'      lng执行科室ID=指定的缺省执行科室ID(用于药品和卫材)
'      lng成套缺省执行科室-成套项目默认的执行科室:120717
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String, i As Integer
...
Public Function ExistRegist(ByVal lng病人ID As Long) As Boolean
'功能：判断指定病人是存在有效的挂号记录
    Dim rsTmp As ADODB.Recordset, strSQL As String
    ...
    '体检病人不限制是否挂号
    strSQL = "Select 1 From 病人医嘱记录 Where 病人ID=[1] And 开嘱时间>Trunc(Sysdate) And 病人来源=4"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", lng病人ID)
    If rsTmp.RecordCount > 0 Then ExistRegist = True: Exit Function
    
    strSQL = "" & _
    "   Select 1 From 病人挂号记录  " & _
...
Public Function CheckRegisted(ByVal lng病人ID As Long) As Boolean
'功能:收费时,检查病人是否挂号
    
    CheckRegisted = True
    If gbytUnRegevent = 0 Then Exit Function
        
    If lng病人ID <> 0 Then
        If Not ExistRegist(lng病人ID) Then lng病人ID = 0
    End If
...
Public Function GetPriceBills(ByVal lngPatient As Long, ByVal lngRegDept As Long, _
    ...
    If blnAddDiagnose Then '加入医嘱
        strTbDiagnose = ",( Select distinct A.NO, f_List2str(Cast(COLLECT(distinct Q.诊断描述 ) as t_Strlist)) as 诊断" & vbNewLine & _
                        "  From A ,病人诊断医嘱 J,病人诊断记录 Q  " & vbNewLine & _
                        "  Where A.医嘱序号=J.医嘱ID and J.诊断ID=Q.ID " & vbNewLine & _
                        "  Group by  A.NO  ) C"
    End If
    strSubTable = "Select " & strSelected & " as 选择 ,A.开单部门ID, a.执行部门ID," & vbNewLine & _
                "       A.NO,A.开单人,A.姓名,A.性别,A.年龄,A.应收金额,A.实收金额," & vbNewLine & _
                "       A.划价人,A.登记时间  As 划价时间,nvl(B.相关ID,A.医嘱序号 ) as 医嘱序号, " & vbNewLine & _
                "       decode(C.ID,NULL,0,1) as 皮试,Decode(Substr(a.摘要,1,3),'挂号:',1,0) As 挂号划价单" & vbNewLine & _
   ...
   strSQL = " Select * From (With A as (" & strSubTable & ")   " & vbNewLine & _
            " Select " & vbNewLine & _
                IIf(blnAddDiagnose, "       nvl(Max(C.诊断),' ') as 诊断,", "") & vbNewLine & _
            ...
            "       Decode(nvl(Max(A.皮试),0),1,'√','') as 皮试,Max(a.挂号划价单) As 挂号划价单" & vbNewLine & _
...
Public Function zlIsExistsSquareCard(ByVal strNos As String, Optional bln存在全退 As Boolean = False) As Boolean
    ...
    "       (  (Nvl(B.结算卡序号, 0) <> 0 And Exists(Select 1 From  卡消费接口目录 Where 编号=nvl(B.结算卡序号,0) And nvl(是否全退,0)<>[2] And Nvl(是否退现,0)=0 ) ) " & _
...
Public Function zlCheckIsPrintInvoice(ByVal strNos As String) As Boolean
    ...
    Else
        ...
        Set rsTemp = OpenSQLRecordByArray(strSQL, "mdlOutExse", varData)
    End If
...
Public Function ImportWholeSet(frmParent As Object, ByVal intInsure As Integer, rsSel As ADODB.Recordset, _
    ...
    "       F.医疗付款方式 as 付款方式,F.门诊号 as 标识号,F.病人ID,0 as 主页ID,0 as 病人病区ID,0 as 病人科室ID," & _
    ...
    If gbln分离发药 Then
        '分离发药时排开时价和分批药品或卫材
        strSQL = "Select * From (" & strSQL & ")" & _
            " Where Not(Instr(',4,5,6,7,',收费类别)>0 And (分批=1 Or 是否变价=1))"
    End If
    Set rsItems = zlDatabase.OpenSQLRecord(gstrSQL, "mdlOutExse", strValue(0), strValue(1), strValue(2), strValue(3), strValue(4), strValue(5), strValue(6), strValue(7), strValue(8), strValue(9), strValue(10), IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1), lng病人ID)
     '没有记录就是空单子
    Set objBill = New ExpenseBill
...
NextRecord: Do While Not .EOF
            ...
            lngDoUnit = Get收费执行科室ID(objBillDetail.收费类别, objBillDetail.收费细目ID, _
                ...
                lngDoUnit, lngUnitID, objBillDetail.执行部门ID)
            ...
            If InStr(",5,6,7,", rsItems!收费类别) > 0 And gbln分离发药 Then
                ...
                If str药房IDs <> "" Then dblStock = GetMultiStock(Val(Nvl(!收费细目ID)), str药房IDs)
            End If
            ...
            Do While Not rsItems.EOF
                ...
                If Val(Nvl(rsItems!是否变价)) = 1 Then
                    If InStr(",5,6,7,", rsItems!收费类别) > 0 Or (rsItems!收费类别 = "4" And Nvl(rsItems!跟踪在用, 0) = 1) Then
                        ...
                        If dblAllTime <> 0 Then
                            dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, CLng(!收费细目ID), rsItems!收费类别 = "4", dblAllTime, gstrDec, dblPriceSingle)
                            If dblAllTime <> 0 Then
                                '数量未分解完毕
...
'                                End If
                ...
                '应收金额=单价*付次*数次
                If rsItems!是否变价 = 1 And (InStr(",5,6,7,", rsItems!收费类别) > 0 Or rsItems!收费类别 = "4" And Nvl(rsItems!跟踪在用, 0) = 1) Then
                    objBillIncome.应收金额 = dblPrice '保证应收金额与零售金额没有误差
                Else
                    objBillIncome.应收金额 = objBillIncome.标准单价 * objBillDetail.付数 * objBillDetail.数次
                End If
...
Public Function zl_GetInvoicePrintFormat(ByVal lngModule As Long, _
    ...
    Optional ByVal blnPatiPrintBill As Boolean = False, Optional ByVal blnDelFeePrintBill As Boolean) As Integer
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取发票打印格式
...
Public Function GetChargeBalance(ByVal strNos As String, _
   ...
   dtDelDate = CDate("1991-01-01")
    ...
    "           A.卡类别ID,A.名称,A.是否全退,A.是否退现,A.结算号码,Max(A.摘要) as 摘要,A.卡号,A.交易流水号," & _
    "           A.交易说明, A.结算序号,A.校对标志 " & _
    "   From (" & strSQL & ") A" & _
    "   Group by  A.结帐ID,A.NO,A.性质,A.应付款,A.结算方式,A.卡类别ID ,A.名称,A.是否全退,A.是否退现,A.结算号码, " & _
    "            A.卡号,A.交易流水号,A.交易说明,A.结算序号 ,A.结算性质,A.医院编码, A.校对标志" & _
...
Public Function zlExeCuteBillNoSplit(ByVal bln模拟计算 As Boolean, ByVal int操作类型 As Integer, _
    ...
    Optional ByRef str发票号 As String, Optional int发票张数 As Integer, Optional lng打印ID As Long) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:执行分配票号的过程
    ...
    Dim objPara(0 To 11) As ADODB.Parameter
    ...
    '  发票张数_In   Out Number
   varParaName = Split("操作类型_In|N|IN,模拟计算_In|N|IN,票种_In|N|IN,领用id_In|N|IN,病人id_In|N|IN,Nos_In|C|IN,起始发票号_In|C|IN,使用人_In|C|IN,使用时间_In|D|IN,发票号_In|C|INOUT,发票张数_In|N|OUT,打印id_In|N|IN", ",")
   varParaValue = Split(int操作类型 & ";" & IIf(bln模拟计算, 1, 0) & ";1;" & lng领用ID & ";" & lng病人ID & ";" & strNos & ";" & str起始发票号 & ";" & _
                            UserInfo.姓名 & ";" & Format(datFeeDate, "YYYY-MM-DD HH:MM:SS") & ";" & str发票号 & ";" & int发票张数 & ";" & lng打印ID, ";")
...
Public Function zlGetReclaimInvoice(ByVal strNoInfor As String) As String
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Else
        ...
        Set rsTemp = OpenSQLRecordByArray(strSQL, "获取对应单据的发票号", varValue)
    End If
...
End Sub
Public Function zlIsCheckPrivs(ByVal strPrivs As String, ByVal strPriv As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查指定的权限串中是否包含指定的权限
    '入参:strPrivs-权限串
    '     strPriv-权限
    '返回:包含指定权限返回true,否则返回False
    '编制:刘兴洪
    '日期:2014-06-05 16:33:41
    '---------------------------------------------------------------------------------------------------------------------------------------------
    On Error GoTo errHandle
    zlIsCheckPrivs = InStr(";" & strPrivs & ";", ";" & strPriv & ";") > 0
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Public Function zlChargeBillIsAllDel(ByVal strNos As String, Optional ByVal lng打印ID As Long = 0, Optional ByRef blnAllDel As Boolean, Optional strNotDelNos As String) As Boolean
    ...
    Else
        ...
        strSQL = strSQL & vbCrLf & _
            ...
            "   Group By b.No,B.序号"
        strSQL = "Select Distinct  NO From (" & strSQL & ") Where nvl(数量,0)<>0  "
        Set rsTemp = OpenSQLRecordByArray(strSQL, "根据票据打印内容来获取单据是否全部退完", varValue)
     End If
