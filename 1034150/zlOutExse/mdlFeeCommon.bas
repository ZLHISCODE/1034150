...
Public Function Get结算方式(str场合 As String, Optional str性质 As String) As ADODB.Recordset
    ...
    strSql = _
        ...
        " Where A.应用场合=[1] And B.名称=A.结算方式 And" & _
        "   (B.性质<>7 Or B.性质=7 And Exists(Select 1 From 一卡通目录 C Where C.结算方式=B.名称 And C.启用=1)) and B.性质<>8 " & strIF
    'and B.性质<>8:49972
...
Public Function GetPersonnel(str性质 As String, Optional blnBaseInfo As Boolean, _
    Optional ByVal bln仅操作员部门 As Boolean, Optional ByVal lngDept As Long) As ADODB.Recordset
    '功能：读取指定性质的人员列表
    '入参:
    '   str性质=药房发药人,医生,护士,...;允许为空
    '   bln仅操作员部门-操作员的所属部门下的人员
    '   lngDept-部门ID，如果lngDept<>0,则 bln仅操作员部门 参数无效
    Dim strSql As String, strWhere As String
    On Error GoTo errH
    
    If lngDept <> 0 Then
        strWhere = " And Exists (Select 1 From 部门人员" & _
                    " Where 人员id = a.Id And 部门id = [3])"
    ElseIf bln仅操作员部门 Then
        strWhere = " And Exists (Select 1 From 部门人员 M, 部门人员 N" & _
                    " Where m.部门id = n.部门id And m.人员id = a.Id And n.人员id = [2])"
    End If
    If str性质 <> "" Then
        If InStr(1, str性质, ",") > 0 Then
            strWhere = strWhere & " And Instr(','||[1]||',',','||b.人员性质||',')>0"
        Else
            strWhere = strWhere & " And b.人员性质 = [1]"
        End If
        
        If blnBaseInfo Then
            strSql = "Select Distinct a.id,a.编号,a.简码,a.姓名 From 人员表 a,人员性质说明 b" & _
            " Where a.ID = b.人员ID" & strWhere & _
            "       And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null)" & _
            "       And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
            " Order by a.简码"
        Else
            strSql = "Select Distinct a.* From 人员表 a,人员性质说明 b" & _
            " Where a.ID = b.人员ID" & strWhere & _
            "       And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null)" & _
            "       And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
            " Order by a.简码"
    ...
    Else
        If blnBaseInfo Then
            ...
            "       And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & strWhere & _
            " Order by 简码"
        Else
        ...
        End If
    End If
    Set GetPersonnel = zlDatabase.OpenSQLRecord(strSql, App.ProductName, str性质, UserInfo.ID, lngDept)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'参数：lng科室ID=指定科室ID
    ...
    On Error GoTo errH
    If Not rs开单科室 Is Nothing Then
        Set rsTmp = rs开单科室.Clone
        rsTmp.Filter = "ID=" & lng科室ID & " And 工作性质='产科'"
    Else
        strSql = "Select 工作性质 From 部门性质说明 Where 工作性质='产科' And 部门ID=[1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng科室ID)
    End If
    
    If Not rsTmp.EOF Then is产科 = True
    Exit Function
errH:
    If ErrCenter() = 1 Then
    Resume
...
'参数：lngID=部门ID
    ...
    strSql = "Select 部门ID From 部门性质说明 Where 工作性质 in ('中药房','西药房','成药房') And 部门ID=[1]"
...
'功能:根据部门ID判断是否是临床或护理部门
    ...
    strSql = "Select 1 From 部门性质说明 Where 部门ID=[1] And 工作性质 in ('临床','护理')"
...
'参数：intNum=项目序号,为0时固定按年产生
    ...
    If Len(strNO) >= 8 Then
        ...
        Exit Function
    End If
    GetFullNO = zlCommFun.GetFullNO(strNO, intNum)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Public Function GetMoneyInfo(lng病人ID As Long, Optional dblModiMoney As Double, _
    ...
    End If
    strSql = "" & _
    "   Select " & IIf(bln按类型统计, "类型,", "") & "  nvl(sum(Nvl(费用余额,0)),0) as 费用余额,nvl(sum(Nvl(预交余额,0)),0) as 预交余额" & _
    "   From 病人余额 " & _
    "   Where 性质=1 And 病人ID=[1] " & IIf(int类型 = -1, "", " And 类型=[4]") & _
      IIf(bln按类型统计, "Group by 类型", "")
  
    If dblModiMoney <> 0 Then   '必须要用Union方式,如果直接去减,在病人余额无记录时,不会返回记录
        strSql = strSql & " Union All " & " Select " & IIf(bln按类型统计, "[4] as 类型,", "") & "  -1*[3] as 费用余额,0 as 预交余额 From Dual"
        strSql = "Select " & IIf(bln按类型统计, "类型,", "") & "nvl(Sum(费用余额),0) as 费用余额,nvl(Sum(预交余额),0) as 预交余额 From (" & strSql & ")" & vbCrLf & IIf(bln按类型统计, " Group by 类型", "")
    End If
    '如果为医保住院病人，则在费用余额中排开预结中的费用(用于报警)
    If blnInsure And bln医保 Then
        strSql = strSql & " Union All " & _
        " Select  " & IIf(bln按类型统计, "Decode(主页ID,NULL,1,0,1,2) as 类型,", "") & "   -1*Nvl(Sum(金额),0) as 费用余额,0 as 预交余额" & _
        " From 保险模拟结算" & _
        " Where 病人ID=[1] And 主页ID=[2] " & _
        IIf(bln按类型统计, "Group by Decode(主页ID,NULL,1,0,1,2)", "")
        strSql = "Select " & IIf(bln按类型统计, "类型,", "") & "nvl(Sum(费用余额),0) as 费用余额,nvl(Sum(预交余额),0) as 预交余额 From (" & strSql & ")" & vbCrLf & IIf(bln按类型统计, " Group by 类型", "")
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng病人ID, lng主页ID, dblModiMoney, int类型)
...
Public Function GetStockSet(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, Optional ByVal lng批次 As Long = 0) As Recordset
    Dim strSql As String, byt出库方式 As Byte
    
    If bln卫材 Then
        byt出库方式 = Val(zlDatabase.GetPara(156, glngSys))
    Else
        byt出库方式 = Val(zlDatabase.GetPara(150, glngSys))
    End If
    If byt出库方式 = 0 Then '分批药品出库方式：0-按批次先进先出，1-按效期最近先出,效期相同，则再按批次先进先出
                            '                  2-按批号最近先出
        strSql = "Nvl(批次,0)"
    ElseIf byt出库方式 = 2 Then
        strSql = "Nvl(上次批号,0)"
    Else
    ...
    strSql = "Select Nvl(批次,0) as 批次,Nvl(可用数量,0) as 库存," & _
        ...
        " From 药品库存" & _
        " Where 库房ID=[1] And 药品ID=[2] " & IIf(lng批次 > 0, " And Nvl(批次,0) = [3] ", "") & " And Nvl(可用数量,0)>0" & _
        " And 性质=1 And (Nvl(批次,0)=0 Or 效期 is NULL Or 效期>Trunc(Sysdate))" & _
        " Order by " & strSql
        
    On Error GoTo errH
    Set GetStockSet = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng药房ID, lng收费项目ID, lng批次)
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng收费项目ID As Long, ByVal bln卫材 As Boolean, ByRef dblAllTime As Double, _
                                    ByVal strDec As String, ByRef dblPriceSingle As Double, Optional ByVal lng批次 As Long = 0) As Currency
...
'      dblPriceSingle-只有一个批次时返回该批次的单价，避免根据数量先乘再除后因四舍五入引起不同的数量的单价不同
    ...
    Set rsPrice = GetStockSet(lng药房ID, lng收费项目ID, bln卫材, lng批次)
    '时价=总金额/总数量
    dblPrice = 0 '本笔总应收金额
...
'    Else
        strSql = "Select A.ID,A.编码,A.附码,A.名称,A.简码,A.五笔码,A.说明,A.性别限制" & _
            " From 疾病编码目录 A" & _
            " Where Rownum<=100 And A.类别=[2] And (A.撤档时间=TO_DATE('3000-01-01','YYYY-MM-DD') Or A.撤档时间 is NULL) And " & strWhere & _
            " Order by A.编码"
'    End If
    
    Set GetDiseaseCode = zlDatabase.ShowSQLSelect(frmParent, strSql, 0, "疾病编码输入", 1, "", "请选择", False, False, True, X, Y, txtHeight, blnCancel, False, True, strCode, strKind)
End Function
...
'             " Order by A.编码"
    ...
    Set GetDiseaseCodeNew = zlDatabase.ShowSQLSelect(frmParent, strSql, 0, "疾病编码输入", 1, "", "请选择", False, False, True, X, Y, txtHeight, blnCancel, False, True, strRight, strCode, strKind, strSex)
End Function
...
'功能：检查发生时间是否合法
'参数：varDate=发生时间,varPaitOrNO=病人ID或记帐单号(可能是多病人单)
'返回：错误提示
...
'返回:年龄,年龄单位
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng病人ID, DateBir)
...
Public Function zlGetSpecialItemFee(strClass As String) As Recordset
    ...
    On Error GoTo errH
    Set rsTemp = zlDatabase.OpenSQLRecord(strSql, "获取特定项目的费用集", strClass)
...
Public Function zlGetSaveCardFeeSQL(ByVal lngCardTypeID As Long, bytStyle As Byte, strNO As String, lng病人ID As Long, lng主页ID As Long, _
        ...
        Optional lng刷卡类别ID As Long, Optional bln消费卡 As Boolean, Optional str刷卡卡号 As String, Optional lng结帐ID As Long) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:产生一条医疗卡费用记录SQL语句
...
ErrHand:
    ...
    End If
End Function
Public Function GetThreeSwapBalance(lng结帐ID As Long) As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取三方结算交易
    '编制:刘兴洪
    '日期:2011-08-22 02:19:29
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTemp As ADODB.Recordset, strSql As String
    On Error GoTo errHandle
    strSql = "" & _
    "Select * " & _
    "   From (Select A.冲预交, A.卡类别id, Nvl(A. 结算卡序号, B.接口编号) As 结算卡序号, Nvl(A.卡号, B.卡号) As 卡号, " & _
    "                 Nvl(A.交易流水号, B.交易流水号) As 交易流水号, A.交易说明, A.合作单位, A.结算序号, B.消费卡id, " & _
    "                 decode(Nvl(A. 结算卡序号, B.接口编号),0,0,1) As 消费卡 " & _
    "          From 病人预交记录 A, " & _
    "               (Select Q.预交id, J.卡号, J.消费卡id, J.交易流水号, J.接口编号 " & _
    "                 From 病人卡结算对照 Q, 病人预交记录 M, 病人卡结算记录 J " & _
    "                 Where Q.预交id = M.ID And M.结帐id = [1] And Q.卡结算id = J.ID) B " & _
    "          Where A.ID = B.预交id(+) and A.结帐ID=[1]) " & _
    "   Where Nvl(卡类别id, 0) <> 0 Or Nvl(结算卡序号, 0) <> 0 " & _
    " "
  Set GetThreeSwapBalance = zlDatabase.OpenSQLRecord(strSql, "获取三方交易", lng结帐ID)
  Exit Function
errHandle:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Public Function zlIsPrintBill(ByVal lng病人ID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If int性质 = 1 Then '收费
        ...
        "      And Exists (Select 1 From 票据使用明细 M Where b.Id = m.打印id And 性质 = 1 And 原因<>6) And Rownum < 2  "
