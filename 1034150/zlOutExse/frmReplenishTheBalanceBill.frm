...
Private Sub InitFace()
    ...
    '是否可进行结算退费
    cmdDelete.Visible = (mEditType = EM_Balance_Charge Or mEditType = EM_Balance_Register) And zlIsCheckPrivs(mstrPrivs, "结算退费")
    ...
    Call ClearData
    '初始化身份认别控件
    Call PatiIdentify.zlInit(Me, glngSys, mlngModule, gcnOracle, gstrDBUser, gobjSquare.objSquareCard, strIDKindStr, gstrSysName)
End Sub
...
Private Sub vsDiagnose_Change()
    If mblnNotClick Then Exit Sub
    If mstrPreDiagnose = Trim(vsDiagnose.Text) Then Exit Sub
...
Private Sub cmdOK_Click()
    ...
    If mEditType = EM_Balance_Err_ReCharge Then
        ...
        If mFrmBalanceWin.zlChargeWin(Me, EM_Balance_Err_ReCharge, mlngModule, mstrPrivs, mobjPatiInfor, mobjPayCards, mstrNo, dtDate, mstr结算ID, _
            mstr结算序号, MCPAR.分币处理, strNos, strReclaimInvoice, mcllForceDelToCash, mstr排除结算方式, IsRegister(), mblnElsePersonErrBill) = False Then
...
RePrint:
    ...
    '打印费用清单:固定不分别打印
    If zlIsCheckPrivs(mstrPrivs, "门诊结算清单") Then
...
Private Function SaveData(ByVal strNos As String, ByVal str结帐IDs As String, ByVal str冲销IDs As String, _
     ByVal dtDate As Date, ByVal blnPrintBill As Boolean, ByRef strNO As String, Optional ByRef cur全自付 As Currency, Optional ByRef cur先自付 As Currency, Optional ByRef cur进入统筹 As Currency _
    ) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:保存单据
...
'        Next
'    End If
    '82259,冉俊明,2015-1-29,医保接口打印时未先保存票据数据
    ...
    If Not gfrmMain Is Nothing Or glngMain <> 0 Then SaveData = True
    Exit Function
errHandle:
...
Private Function isValied(ByVal strNos As String, ByRef str结帐IDs As String, str冲销IDs As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查数据有的有效性
    '出参:strNOs-本次结算的单据号
    ...
    If strNos = "" Then
        MsgBox "当前病人没有需要补充结算的" & strTittle & "费用,请选择需要补充结算的" & strTittle & "费用!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    strSQL = "" & _
        ...
        "       From 门诊费用记录" & _
        "       Where Mod(记录性质, 10)=[2] And NO In (Select Column_Value From Table(f_Str2list([1])))) B" & _
        " Where a.收费结帐id = b.结帐id And a.记录性质 = 1 And 费用状态 <> 2" & _
        "      And a.附加标志=" & IIf(mEditType = EM_Balance_Register, 1, 0) & " And Rownum < 2"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strNos, int记录性质)
    If Not rsTemp.EOF Then
        MsgBox "被选择单据中存在已补充结算了的数据或补充结算异常数据，不允许再进行补充结算！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    
    strSQL = "" & _
     "  Select A.记录性质,A.结帐ID,Max(decode(A.记录状态,2,A.结帐ID,0)) as 冲销ID " & _
     "  From 门诊费用记录 A " & _
     "  Where Mod(A.记录性质,10)=[2] And A.NO in (Select Column_Value From Table(f_str2List([1])))" & _
     "  Group by A.记录性质, A.结帐ID"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strNos, int记录性质)
    If rsTemp.EOF Then
        MsgBox "当前病人没有需要补充结算的费用,请选择需要补充结算的费用!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    If Not mEditType = EM_Balance_Register Then
        rsTemp.Filter = "记录性质=11"
        If Not rsTemp.EOF Then
            MsgBox "收费单为:" & strNos & "的收费单据中存在医保结算单据,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
    End If
    rsTemp.Filter = 0
    ...
    End With
    If str结帐IDs = "" Then
        MsgBox strTittle & "单为:" & strNos & "中未找到原始的" & strTittle & "记录,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    ...
    strSQL = "" & _
    "Select Max(Decode(Mod(a.记录性质, 10), 1, 1, 0)) As 预交, Max(Decode(Nvl(b.性质, 0), 3, 1, 4, 1, 0)) As 医保, a.结算方式, " & _
    "          Count(Distinct a.结算方式) As 统计 " & _
    "   From 病人预交记录 A, 结算方式 B " & _
    "   Where a.结算方式 = b.名称(+)  And a.结帐id In (Select Column_Value From Table(f_Num2list([1]))) " & _
    "         and not Exists(Select 1 From 结算方式 where A.结算方式=名称 and 性质=9 ) " & _
    "   Group By a.结算方式"
        
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, str结帐IDs)
    If rsTemp.EOF Then
        MsgBox strTittle & "单为:" & strNos & "的" & strTittle & "单据中未找到原始的" & strTittle & "记录,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    rsTemp.Filter = "预交=1"
    If rsTemp.EOF = False Then
        MsgBox strTittle & "单为:" & strNos & "的" & strTittle & "单据中存在使用预交款进行结算的数据,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    rsTemp.Filter = "医保=1"
    If rsTemp.EOF = False Then
        MsgBox strTittle & "单为:" & strNos & "的" & strTittle & "单据中存在医保结算的数据,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    rsTemp.Filter = "统计 >=2"
    If rsTemp.EOF = False Then
        MsgBox strTittle & "单为:" & strNos & "的" & strTittle & "单据中存在两种以上的结算方式,不允许进行医保补充结算!", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
        ...
        ElseIf cllFeeBalance(i)(5) = 1 Then '必须全退
            If cllFeeBalance(i)(2) = 1 Then '允许退现，必须全退
                If cllFeeBalance(i)(6) = 0 Then '不支持转帐及代扣
                    If MsgBox("『" & cllFeeBalance(i)(3) & "』必须全退，同时也不支持转帐及代扣，因此不能退回原卡。" & _
...
Private Function 门诊预结算(ByVal strNos As String, ByRef strNone As String) As Boolean
    ...
    If zlInsure_ClinicPreSwap(strNos, strDate, strNone) = False Then Exit Function
    '要先设置以便其它地方识别
    ...
    With vsFeeList
        For i = 1 To .Rows - 1
            If .IsSubtotal(i) Then
                If Trim(.TextMatrix(i, .ColIndex("NO"))) <> "" And Abs(Val(.Cell(flexcpChecked, i, .ColIndex("选择")))) = 1 Then
...
Private Sub PatiIdentify_FindPatiBefore(ByVal objCard As zlIDKind.Card, blnCard As Boolean, strShowText As String, objCardData As zlIDKind.PatiInfor, blnFindPatied As Boolean, blnCancel As Boolean)
    ...
    If objCard.名称 Like "*姓*名*" And Not blnCard And InStr("-*+/.", Left(Trim(PatiIdentify.Text), 1)) = 0 Then
        ...
        Else
            ...
            vRect = GetControlRect(PatiIdentify.hWnd)
            Set rsTmp = zlDatabase.ShowSQLSelect(Me, strPati, 0, "病人查找", 1, "", "请选择病人", False, False, True, vRect.Left, vRect.Top, PatiIdentify.Height, blnCancel, False, True, strNO & "%", gintNameDays, Val(zlDatabase.GetPara("缺省医疗卡类别", glngSys, glngModul, 0)))
...
Private Sub PatiIdentify_FindPatiArfter(ByVal objCard As zlIDKind.Card, ByVal blnCard As Boolean, ShowName As String, objHisPati As zlIDKind.PatiInfor, objCardData As zlIDKind.PatiInfor, strErrMsg As String, blnCancel As Boolean)
    ...
    End If
End Sub
Private Function CheckPatiInfor(ByRef objPatiInfor As zlIDKind.PatiInfor, Optional ByVal strNO As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:验证病人信息
    ...
    '加载费用信息
    If ReadBills(strNO, objPatiInfor) = False Then GoTo GoClear
    Call SetButtons '设置按钮
    CheckPatiInfor = True
    Exit Function
...
Private Function LoadErrBillData(ByRef objPati As zlIDKind.PatiInfor) As Boolean
    ...
    strSQL = "Select 结算方式,sum(冲预交) as 冲预交 From 病人预交记录 where 结算序号=[1] and mod(记录性质,10)<>1 Group by 结算方式"
...
Private Function ReadBills(ByVal strNO As String, ByRef objPati As zlIDKind.PatiInfor) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:加载单据数
    '入参:strNO-单据号
    '     blnFilter-是否按条件筛选
    ...
    Dim strTemp As String, str结算方式 As String
    ...
    str结算方式 = zlDatabase.GetPara("补结算类别设置", glngSys, mlngModule, , , True)
    
    If strNO = "" And objPati.病人ID = 0 Then Exit Function
    strWhere = "": strFields = ",'' as 诊断": strTable = ""
    If strNO = "" Then
        If mEditType = EM_Balance_Register Then
            strWhere = " And a.病人ID=[2] And a.记录性质 =4 And a.记录状态 <> 0  And a.记录状态 In (1, 3)"
        Else
            strWhere = " And a.病人ID=[2] And a.记录性质 =1 And a.记录状态 <> 0  And a.记录状态 In (1, 3)"
        End If
    Else
        If mEditType = EM_Balance_Register Then
            strWhere = " And A.NO=[1] And A.记录性质 = 4 And A.记录状态 In (1, 3)"
        Else
            strWhere = " And B.结算序号 =(Select Max(M.结算序号) from  门诊费用记录 J,病人预交记录 M Where J.结帐ID=M.结帐ID and  J.记录性质 = 1 And J.记录状态 <> 0 And J.No = [1] And J.记录状态 In (1, 3) )"
        End If
    End If
    ...
    strWithTable = "" & _
    "   With 收费单据 as ( " & _
    "   Select  a.记录性质,a.NO,max(b.结算方式) as 结算方式" & _
    "   From 门诊费用记录 A, 病人预交记录 B " & _
    "   Where a.结帐id = b.结帐id And B.结算卡序号 Is Null" & vbCrLf & strWhere & vbCrLf & _
    "         And Decode(a.记录性质, 1, a.登记时间 , a.发生时间  ) between trunc(sysdate) - " & mtyMoudlePara.int补结算有效天数 - 1 & " And trunc(sysdate) + 1 - 1 / 24 / 60 / 60" & vbCrLf & _
    "         And Exists(Select 1  From 病人预交记录 F1,结算方式 Q1  Where F1.结帐id = a.结帐ID And Mod(F1.记录性质, 10) <> 1 And (F1.结算方式=Q1.名称 Or F1.结算卡序号 Is Not Null) And Q1.性质<>9 Having Count(Distinct F1.结算方式) = 1) " & _
    "         And not Exists (Select 1  From 病人预交记录 Where 结帐id = A.结帐id And Mod(记录性质, 10) = 1  )  " & vbCrLf & _
    "         And Not Exists (Select 1 From 保险结算记录 Where 性质 = 1 And 记录id = a.结帐id)        " & vbCrLf & _
    "          " & vbCrLf & _
    IIf(str结算方式 = "", "", " And B.结算方式 in ('" & Replace(str结算方式, "|", "','") & "')") & _
    "         And Not Exists (Select 1 From 费用补充记录 Where 收费结帐ID =a.结帐ID And 费用状态 <> 2)  " & vbCrLf & _
    "         And exists(Select 1   From 门诊费用记录 where a.NO=NO And a.序号=序号 and 价格父号 is null  having sum(nvl(付数,1)*nvl(数次,0))<>0) " & _
    IIf(mEditType = EM_Balance_Charge, " ", _
        "     And Not Exists (Select 1 From 门诊费用记录 Where a.No = NO And 记录性质 = 4 And 记录状态 = 2) " & vbCrLf & _
        "     And Nvl(a.摘要,'-') Not Like '划价:%'") & _
    "   Group by  a.记录性质,a.NO )" & vbCrLf
    
    If mEditType = EM_Balance_Charge Then
       strWithTable = strWithTable & vbCrLf & _
        ",医嘱诊断 As (Select Distinct 1 As 记录性质, A2.No, f_List2str(Cast(Collect(Distinct c.诊断描述) As t_Strlist)) As 诊断" & _
        "         From (Select Distinct 1 As 记录性质, B1.No, B1.医嘱序号" & _
        "                From 门诊费用记录 B1, 收费单据 A1" & _
        "                Where B1.No = A1.No And A1.记录性质 = 1 And B1.记录性质 = 1 And B1.记录状态 In (1, 3)) A2," & _
        "              病人医嘱记录 A, 病人诊断医嘱 B, 病人诊断记录 C" & _
        "         Where A2.记录性质 = 1 And A2.医嘱序号 = a.Id And Nvl(a.相关Id,a.Id) = b.医嘱id And b.诊断id = c.Id" & _
        "         Group By A2.No)" & vbCrLf
        strTable = ",医嘱诊断 C"
        strFields = ",Max(C.诊断) as 诊断 "
    ...
    "   Group by a.记录性质, a.记录状态, A.NO,Nvl(A.价格父号,A.序号),A.从属父号,A.开单部门ID,A.执行部门ID,A.收费类别,A.费别,A.收费细目ID,A.费用类型,A.计算单位,A.结帐ID"
    ...
    Set mrsList = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strNO, objPati.病人ID)
    mrsList.Filter = ""
    
    
    '加载原收款方式
    Dim strBalance As String, str诊断 As String, blnFind As Boolean
        ...
        Do While Not .EOF
            ...
            If mEditType = EM_Balance_Charge Then
                ...
                For i = 1 To mcllDiagnose.Count
                    If mcllDiagnose(i)(0) = str诊断 Then
                ...
                mcllDiagnose.Add Array(str诊断, strTemp)
            End If
            .MoveNext
        Loop
    End With
    ...
    With rsTemp
        cboPayMode.Clear: vsDiagnose.Clear
        Do While Not .EOF
        ...
        For i = 0 To UBound(varData)
            ...
            If Trim(varData(i)) <> "" Then
                For j = 0 To cboPayMode.ListCount - 1
                    If varData(i) = cboPayMode.List(j) Then
                        blnFind = True
                    End If
                Next
                If blnFind = False Then
    ...
    If mEditType = EM_Balance_Charge Then
         vsDiagnose.Clear
         vsDiagnose.Rows = 1
         vsDiagnose.COLS = mcllDiagnose.Count
         For i = 1 To mcllDiagnose.Count
            vsDiagnose.TextMatrix(0, i - 1) = IIf(mcllDiagnose(i)(0) = "", "无诊断收费", mcllDiagnose(i)(0))
            vsDiagnose.Cell(flexcpData, 0, i - 1) = mcllDiagnose(i)(1)
            vsDiagnose.Cell(flexcpChecked, 0, i - 1) = 2
         Next
         vsDiagnose.AutoSizeMode = flexAutoSizeColWidth
         vsDiagnose.AutoSize 0, vsDiagnose.COLS - 1
         vsDiagnose.Editable = flexEDKbdMouse
     Else
         vsDiagnose.Editable = flexEDNone
     End If
    Call LoadFeeData(mrsList)
    ...
    If zlCommFun.ActualLen(strNos) < 4000 Then
        ...
        With rsTemp
            Do While Not .EOF
                ...
                mrsBalanceNO.Filter = strFilter
                If mrsBalanceNO.EOF Then
                    mrsBalanceNO.Filter = 0
                    mrsBalanceNO.AddNew
                    mrsBalanceNO!记录性质 = Val(Nvl(!记录性质))
                    mrsBalanceNO!NO = CStr(Nvl(!NO))
                    mrsBalanceNO!结帐ID = Val(Nvl(!结帐ID))
                    mrsBalanceNO!结算序号 = Val(Nvl(!结算序号))
                    mrsBalanceNO.Update
                End If
                .MoveNext
            Loop
        End With
        LoadBalanceNO = True: Exit Function
    End If
    varData = Split(strNos, "|")
    strTemp = ""
    For i = 0 To UBound(varData)
        If zlCommFun.ActualLen(strTemp & "|" & varData(i)) >= 4000 Then
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Mid(strTemp, 2))
            With rsTemp
                Do While Not .EOF
                    strFilter = "NO='" & Nvl(!NO) & "'"
                    strFilter = " And 记录性质= " & Val(Nvl(!记录性质))
                    strFilter = " And 结帐ID= " & Val(Nvl(!结帐ID))
                    strFilter = " And 结算序号= " & Val(Nvl(!结算序号))
                    mrsBalanceNO.Filter = strFilter
                    If mrsBalanceNO.EOF Then
                        mrsBalanceNO.Filter = 0
                        mrsBalanceNO.AddNew
...
Private Function LoadFeeData(ByVal rsList As ADODB.Recordset) As Boolean
    ...
    With mrsList
        If .RecordCount <> 0 Then
            .MoveFirst: strTemp = ""
            Do While Not .EOF
                str诊断 = Nvl(!诊断): str诊断 = IIf(str诊断 = "", "无诊断收费", str诊断)
...
Private Sub SetFeeListHead(Optional blnInitHead As Boolean = False)
    ...
    With vsFeeList
        ...
        If blnInitHead Then
            strHead = "单据|NO|序号|从属父号|费别|开单人|收费细目ID|收费类别|选择|类别|编码|名称|商品名|规格|费用类型|单位|医嘱序号|付数|数次|单价|应收金额|实收金额|执行科室|开单科室|操作员姓名|登记时间|摘要|结算方式|诊断|记录性质|病人ID"
...
Private Function ShowBillFormat()
    ...
    '编制:刘兴洪
    '日期:2014-09-16 12:12:00
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strReportName As String
    Dim rsTmp As ADODB.Recordset
    Dim strSQL As String, intFormat As Integer
    lblFormat.Caption = ""
    If mobjFactProperty Is Nothing Then Exit Function
    
    intFormat = mobjFactProperty.打印格式
    strReportName = "ZL" & glngSys \ 100 & "_BILL_1124"
    
    If intFormat = 0 Then
        intFormat = Val(GetSetting("ZLSOFT", "私有模块\" & UserInfo.用户名 & "\zl9Report\LocalSet\" & strReportName, "Format", 1))
    ...
    strSQL = "Select B.说明 From zlReports A,zlRptFmts B" & _
        " Where A.ID=B.报表ID And A.编号='" & strReportName & "' And B.序号=[1]"
    On Error GoTo errH
...
'    vsBalance.ColAlignment(1) = 7
    ...
    Set mcolBalance = New Collection
End Sub
Private Function zlInsure_ClinicPreSwap(ByVal strNos As String, ByVal strDate As String, _
    ...
    '咸阳医保无法返回余额
     If (mTy_Insure.dbl帐户余额 > -1 * mTy_Insure.dbl个帐透支 Or mintInsure = 61) _
        ...
        If mintInsure <> 61 Then
            ...
            End If
        End If
        dbl个帐合计 = Format(dbl个帐合计 + dbl个帐, "0.00")
...
Private Function GetDiagnoseNos() As String
    ...
    With vsDiagnose
        For i = 0 To .Rows - 1
            For j = 0 To .COLS - 1
                If Abs(Val(.Cell(flexcpChecked, i, j))) = 1 _
                    And vsDiagnose.TextMatrix(i, j) <> "" Then
...
Private Sub vsFeeList_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsFeeList
        ...
        '将选中的置为灰色状态
        SetDiagnoseSelStatu (EM_dgSeledToGray)
