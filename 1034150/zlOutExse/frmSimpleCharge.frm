...
Private Sub Bill_CommandClick()
    ...
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, 0, False, "'Z'")
    If lng项目id <> 0 Then
        Bill.Text = lng项目id
        mblnSelect = True
        ...
        Select Case Bill.TextMatrix(0, Bill.Col)
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, gint病人来源, 0, False, "'Z'", Bill.Text, Bill.TxtHwnd)
                        If lng项目id <> 0 Then
                    ...
                    If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, mlngModul, 0, 0, _
                        MakeDetailRecord(mobjBill, zlStr.NeedName(cbo开单人.Text), zlStr.NeedName(cbo开单科室.Text), 1, Bill.Row)) = False Then
...
Private Sub cbo开单人_Click()
    ...
    mblnCboClick = True
    If Not (mbytInState = 0 And chkCancel.Value = 0) Then Exit Sub
    If mobjBill.Pages(1).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text)) Then Exit Sub
    
    mobjBill.Pages(1).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
    '由医生确定科室
    If gbyt科室医生 = 0 Then
        If cbo开单人.ListIndex <> -1 Then
...
Private Sub cmdOK_Click()
    ...
    ElseIf Bill.Active Then '正常输入单据状态
        ...
        If Val(txt缴款.Text) <> 0 And txt缴款.Enabled Then
            If Val(txt缴款.Text) < Val(txt应缴.Text) Then
                ...
                Call SelAll(txt缴款): txt缴款.SetFocus: Exit Sub
            End If
        End If
        '刘兴洪:22343,缴款金额控制
        ...
        If gblnLED And Not mbln报合计 And Not gbln手工报价 Then
        ...
        If mblnPrint Then '打印门诊收据
            Call frmPrint.ReportPrint(1, "'" & mobjBill.NO & "'", "", "", mlng领用ID, mlngShareUseID, txtInvoice.Text, mobjBill.登记时间, , , , mintInvoiceFormat, , , mstrUseType)
        End If
    ...
    ElseIf Not Bill.Active Then '收取划价单费用状态
        ...
        strSQL = "zl_划价收费记录_INSERT('" & mstrInNO & "'," & Val(txtPatient.Tag) & "," & gint病人来源 & ",'" & _
            ...
            "'" & NeedName(cboSex.Text) & "','" & mobjBill.年龄 & "'," & _
             ZVal(mobjBill.科室ID, cbo开单科室.ItemData(cbo开单科室.ListIndex)) & "," & _
            cbo开单科室.ItemData(cbo开单科室.ListIndex) & ",'" & NeedName(cbo开单人.Text) & "'," & _
            "'" & NeedName(cbo结算方式.Text) & "|" & mobjBill.Pages(1).应缴金额 & "| ',"
        '预交结算
        ...
        If mblnPrint Then '打印门诊收据
            Call frmPrint.ReportPrint(1, "'" & mstrInNO & "'", "", "", mlng领用ID, mlngShareUseID, txtInvoice.Text, zlDatabase.Currentdate, , , , mintInvoiceFormat, , , mstrUseType)
        End If
        '费用清单的打印
        If InStr(mstrPrivs, "打印清单") > 0 Then
            If gint收费清单 = 1 Then
...
Private Sub Form_Load()
    ...
    Else '新增单据
        ...
        If mstrInNO <> "" Then '修改原单据
            Set mobjBill = ImportBill(mstrInNO, True, 0, , , True)
            If mobjBill.NO = "" Then
            ...
            Else
                ...
                Else
                    ...
                    If strTmp <> "" Then
                        cbo费别.ListIndex = GetCboIndex(cbo费别, strTmp)
                        If cbo费别.ListIndex = -1 Then
...
Private Sub cbo开单人_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        Else
            mobjBill.Pages(1).开单人 = NeedName(cbo开单人.Text)
...
Private Sub txtPatient_KeyPress(KeyAscii As Integer)
    ...
    If blnCard And Len(txtPatient.Text) = IDKind.GetCardNoLen - 1 And KeyAscii <> 8 Or KeyAscii = 13 And Trim(txtPatient.Text) <> "" Then
        ...
        Else
            ...
            End If
            '就诊卡密码检查
            If mbytInState = 0 And blnCard Then
            ...
            If Not rsTmp Is Nothing Then
                sta.Panels(3).Tag = Val("" & rsTmp!预交余额) - Val("" & rsTmp!费用余额)
                sta.Panels(3).Text = "预交:" & Format(Val(sta.Panels(3).Tag), "0.00")
            End If
            '预交信息初始
            ...
            cboSex.ListIndex = GetCboIndex(cboSex, IIf(IsNull(mrsInfo!性别), "", mrsInfo!性别))
            ...
            If Not mblnValid Then
                ...
                Else
                    '挂号时确定的费别
                    cbo费别.ListIndex = GetCboIndex(cbo费别, IIf(IsNull(mrsInfo!费别), "", mrsInfo!费别))
                End If
            End If
...
Private Function GetPatient(ByVal strInput As String, Optional blnCancel As Boolean, Optional ByVal blnCard As Boolean) As Boolean
    ...
    If blnCard = True And IDKind.GetCurCard.名称 Like "姓名*" And (InStr("-+*", Left(strInput, 1)) = 0 And IsNumeric(Mid(strInput, 2))) Then   '103563     '就诊卡号
    ...
    Else
        ...
        Case "姓名", "姓名或就诊卡"
                ...
                If Not mblnValid And gblnSeekName And gblnInputID Then
                    ...
                    Set rsTmp = zlDatabase.ShowSelect(Me, strPati, 0, "病人0" & gint病人来源, , , , , , True, vRect.Left, vRect.Top, txtPatient.Height, blnCancel, , True)
                    If Not rsTmp Is Nothing Then
    ...
    If KeyAscii = 13 Then
        ...
        If Val(txt缴款.Text) <> 0 Then
            If CSng(txt找补.Text) >= 0 Then
                Call zlCommFun.PressKey(vbKeyTab)
                If gblnLED And mbytInState = 0 Then  'LED显示
                    mblnHotKey = False
                    If Val(txt预交冲款.Text) = 0 Then
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    gstrSQL = _
        ...
        " And A.ID=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mobjBill.Pages(1).Details(lngRow).收费细目ID)
    ...
    If rsTmp.RecordCount <> 0 Then
        ...
        If mobjBill.Pages(1).Details(lngRow).Detail.变价 Then
            If mobjBill.Pages(1).Details(lngRow).InComes.Count = 0 Then
                '如果第一次计算金额,变价默认取原价
                dblMoney = 0    'dblMoney = IIf(IsNull(rsTmp!原价), 0, rsTmp!原价)
            Else
                dblMoney = mobjBill.Pages(1).Details(lngRow).InComes(1).标准单价
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            With mobjBillIncome
                ...
                Else
                    .标准单价 = Format(IIf(IsNull(rsTmp!现价), 0, rsTmp!现价), gstrFeePrecisionFmt)
                End If
                '.期间 = GetWhile '取当前系统时间对应的期间
                ...
                Else
                    ...
                    .实收金额 = CCur(Format(ActualMoney(str费别, .收入项目ID, .应收金额, 0, 0, 0, 0), gstrDec))
...
'      bln费别=是否重新初始化费别
    ...
    If mbytInState = 0 Then
        ...
        With mobjBill
            ...
            End If
            .门诊标志 = gint病人来源
            .Pages(1).开单人 = IIf(cbo开单人.ListIndex = -1, "", NeedName(cbo开单人.Text))
        End With
    ...
    For Each mobjBillDetail In mobjBill.Pages(1).Details
        ...
        For Each mobjBillIncome In mobjBillDetail.InComes
            ...
            gstrSQL = gstrSQL & _
                ...
                "'" & mstrInNO & "'," & lng结帐ID & ",'" & NeedName(cbo结算方式.Text) & "|" & mobjBill.Pages(1).应缴金额 & "| ',"
            '预交结算
            If Val(txt预交冲款.Text) <> 0 Then
...
'说明：为统一，退费时都不显示误差费用(虽然不可部份退费)
    ...
    If Nvl(rsTmp!门诊标志, 0) = 2 Or Not IsNull(rsTmp!医疗付款方式) Then
        cbo医疗付款.ListIndex = GetCboIndex(cbo医疗付款, rsTmp!医疗付款方式)
        If cbo医疗付款.ListIndex = -1 Then
    ...
    If Not bln划价 Then
        ...
        For i = 1 To rs结算.RecordCount
            ...
            Else
                cbo结算方式.ListIndex = GetCboIndex(cbo结算方式, rs结算!结算方式)
                If cbo结算方式.ListIndex = -1 Then
                        ...
                        For i = 1 To mrsUnit.RecordCount
                            ...
                            If Not (SendMessage(Bill.cboHwnd, CB_FINDSTRING, -1, ByVal strTmp) >= 0) Then
                                ...
                                ElseIf lngRow > 1 Then
                                    '与上一行非药品相同
                                    If mrsUnit!ID = mobjBill.Pages(1).Details(lngRow - 1).执行部门ID And _
...
Private Sub txtRePrint_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        If Not RePrintCharge(1, strNO, Me, mlng领用ID, strReclaimIvoice, , , intInvoiceFormat, , , mlngShareUseID, mstrUseType) Then
            txtRePrint.SetFocus
        Else
            Call RefreshFact
            txtRePrint.Text = ""
