...
Private Function LoadViewBills(ByVal lng结算序号 As Long) As Boolean
    ...
    If mblnPrintView And zlIsCheckPrivs(mstrPrivs, "修改姓名重打") _
    ...
    If rsTemp.EOF Then
        ...
        ElseIf mbytMode = EM_MULTI_退费申请 Then
            MsgBox "没有找到需要退费申请的单据，可能已被他人退费申请了。", vbInformation, gstrSysName
...
Private Function CheckPrivsIsValied() As Boolean
    ...
    If mCurBillType.intInsure > 0 Then
        '保险退费权限检查
        If zlIsCheckPrivs(mstrPrivs, "保险收费") = False Then
            Screen.MousePointer = 0
    ...
    '普通病人的处理
    '是否有非医保病人的退费权限
    If zlIsCheckPrivs(mstrPrivs, "允许非医保病人") = False Then
        Screen.MousePointer = 0
...
Private Sub cmdCancel_Click()
    If mblnPrintView And txtPatientPrint.Visible Then
...
Private Function isValied() As Boolean
    ...
    '105432,三方卡结算方式有效性检查
    If ThreeBalanceCheck(mrsBalance, mcllForceDelToCash) = False Then Exit Function
...
Private Function ThreeBalanceCheck(ByVal rsBalance As ADODB.Recordset, _
...
Private Function InputFactNo(ByRef lng领用ID As Long, ByRef strInvoice As String) As Boolean
    ...
    Do
        ...
        Else
            strInvoice = IncStr(UCase(zlDatabase.GetPara("当前收费票据号", glngSys, mlngModule)))
        End If
        ...
        If gblnStrictCtrl Then
            If zlGetInvoiceGroupUseID(lng领用ID, 1, strInvoice) Then blnValid = True
        Else
            blnValid = True
        End If
...
Private Function ExecDelete() As Boolean
    ...
    If blnAll部份退费 Then
        ...
        If Not (gTy_Module_Para.byt票据分配规则 <> 0 And strReclaimInvoice <> "") Then
            If zlIsCheckPrivs(mstrPrivs, "部份退费") = False Then
            ...
            If zlIsCheckPrivs(mstrPrivs, "退费核收发票") Then
...
'                zlDatabase.ExecuteProcedure strSQL, Me.Caption
    ...
    If gTy_Module_Para.byt票据分配规则 <> 0 And strReclaimInvoice <> "" Then
        ...
        If blnPrint Then
            ...
            Call RePrintCharge(1, strBillPrintNos, Me, mlng领用ID, strReclaimInvoice, True, dtDateDel, _
                intInvoiceFormat, , , mlngShareUseID, mstrUseType, blnOnePatiPrint)
        End If
        GoTo PrintList:
        Exit Sub
    End If
    If strInvoices = "无可退票据" Or strInvoices = "" Then 'a.收回并重新打印门诊收据
        ...
        If blnPrint Then
            intInvoiceFormat = IIf(strReclaimInvoice = "" And gTy_Module_Para.byt票据分配规则 <> 0 And blnOnePatiPrint = False, mintOldInvoiceFormat, mintInvoiceFormat)
            If gTy_Module_Para.bln分别打印 And blnOnePatiPrint = False Then
                Call RePrintCharge(1, strNotAllDelNos, Me, mlng领用ID, strReclaimInvoice, True, dtDateDel, _
...
Private Function zlCheckPrepayBack(ByVal lng病人ID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查是否存在预交,如果存在预交,则根据消费确认原则让用户刷卡消费
    '返回:成功,返回true,否则返回False
    '编制:刘兴洪
    '日期:2014-07-04 11:59:18
    '问题:37307
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If gbln消费验证 = False Then zlCheckPrepayBack = True: Exit Function
    
    On Error GoTo errHandle
    '       字段:类型,记录性质,结算方式,摘要,卡类别ID,卡类别名称,自制卡,结算卡序号,结算号码,卡号,交易流水号, 交易说明,结算序号,校对标志,医保,消费卡id
    '            是否密文,是否全退,是否退现,冲预交
    '       类型:0-普通结算;1-预交款;2-医保,3-一卡通;4-一卡通(老);5-消费卡)
    mrsBalance.Filter = "类型=1"
    With mrsBalance
    ...
    If Not zlDatabase.PatiIdentify(Me, glngSys, lng病人ID, dblMoney) Then Exit Function
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, blnCancel As Boolean, Optional blnCard As Boolean = False) As Boolean
    ...
    blnCancel = False
    strWhere = ""
    If blnCard And objCard.名称 Like "姓名*" And (InStr("-+*", Left(strInput, 1)) = 0 And IsNumeric(Mid(strInput, 2))) Then   '103563   '刷卡或缺省的卡
...
Private Function SelectNO(ByVal lng病人ID As Long) As Boolean
    ...
    Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSQL, 0, "退费单据", 1, "", "请选择需要退费的单据", False, False, False, 0, 0, 0, blnCancel, False, False, lng病人ID)
...
''    End If
    ...
    If MCPAR.医保接口打印票据 And MCPAR.医保不走票号 = False Then
        ...
        If Not gblnStrictCtrl Then
            zlDatabase.SetPara "当前收费票据号", mCurBillType.strInvoice, glngSys, 1121, zlIsCheckPrivs(mstrPrivs, "参数设置")
        End If
    End If
...
Public Sub zlExeBalanceWinRefrshData(ByVal blnSaveOK As Boolean, _
   ...
   '打印退费单据
    Call PrintDelBill(objDelBalance.AllNos, objDelBalance.CurDelNos, strNO, objDelBalance.病人ID, _
        ...
        '56615
    Call WriteMzInforToCard(objDelBalance.病人ID, objDelBalance.结算序号)
Completed:
...
Private Function ExecuteReDelFee() As Boolean
    ...
    End If
    '105432,三方卡结算方式有效性检查
    If ThreeBalanceCheck(zlFromIDGetChargeBalance(2, mCurBillType.strAllNOs, mblnNOMoved, , True), mcllForceDelToCash) = False Then Exit Function
...
ReDOOneCard:
    ...
    If mblnDrugMachine Then
        ...
        strSQL = "Select Max(Decode(a.记录状态, 2, a.Id, 0)) As 费用id, -1 * Nvl(Sum(a.付数 * a.数次), 0) As 退药数量" & vbNewLine & _
                ...
                " Group By NO, Nvl(价格父号, 序号)" & vbNewLine & _
                " Having Nvl(Sum(a.付数 * a.数次), 0) <> 0"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "查询本次退费项目", Val(mlng结算序号))
        Do While Not rsTemp.EOF
            strData = strData & ";" & Nvl(rsTemp!费用id) & "," & Nvl(rsTemp!退药数量)
            rsTemp.MoveNext
        Loop
    ...
    For i = 0 To UBound(varTemp)
        ...
        Select Case bytMode
            Case EM_MULTI_退费申请
                ...
                If rsTemp.RecordCount <> 0 Then
                    MsgBox "单据:" & strNO & " 已被退费申请，不用再进行退费申请！", vbInformation, gstrSysName

                    Exit Function
                End If
                rsTemp.Filter = "NO='" & strNO & "' And 状态=1" '已审核
                If rsTemp.RecordCount <> 0 Then
                    MsgBox "单据:" & strNO & " 已被退费申请并进行了审核，不能再进行退费申请！", vbInformation, gstrSysName
                    Exit Function
                End If
            Case EM_MULTI_取消申请
                rsTemp.Filter = "NO='" & strNO & "' And 状态=0" '已申请
        ...
        If .RecordCount > 0 Then
            ...
            Do While Not .EOF
                ...
                If strBXInfo <> "" Then
                    ...
                    strSQL = strSQL & IIf(Val(Split(strBXInfo, ";")(1)) = 0, "Null", Val(Split(strBXInfo, ";")(1))) & ","
...
Private Sub WriteMzInforToCard(ByVal lng病人ID As Long, ByVal lng结算序号 As Long)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:将门诊信息写入卡中
    '编制:刘兴洪
    ...
    Call gobjSquare.objSquareCard.zlMzInforWriteToCard(Me, mlngModule, lngCardTypeID, _
...
Private Function SelectMulitBalance(ByVal strNos As String, ByRef strNO As String) As Boolean
   ...
   End If
    ...
    Else
        ...
        "  Select Rownum as ID,单据号,开单科室,操作员编号,操作员姓名,收款时间 " & _
        "  From (" & strSQL & ") "
        Set rsTemp = OpenSQLRecordByArray(strSQL, "检查二次结算", varData)
    End If
