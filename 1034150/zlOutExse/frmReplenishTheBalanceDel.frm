...
Private Function LoadViewBills(ByVal str结算序号 As String) As Boolean
    ...
    "                Union All" & _
    "                Select 结帐id From 病人预交记录 T1 Where 结算序号 = [1] And 结帐id = Abs(结算序号)" & _
...
Private Function CheckPrivsIsValied() As Boolean
    ...
    End If
    '保险退费权限检查
    If zlIsCheckPrivs(mstrPrivs, "结算退费") = False Then
        Screen.MousePointer = 0
...
Private Function ExecuteDelChargeFee() As Boolean
    ...
    Dim lng结帐ID As Long, lng冲销ID As Long, lng结算序号 As Long, lng领用ID As Long
...
'    If blnAll部份退费 Then bln全退 = False
    ...
    If Not bln全退 Then
        ...
        '更新重收记录的保险信息
        If GetExcutInsureInforUpdateSQL(lng结算序号, strBalanceInfor, cllPro) = False Then Exit Function
        blnTrans = True: zlExecuteProcedureArrAy cllPro, Me.Caption, True
        '77058
        If ExcuteInsureReCharge(mCurBillType.lng病人ID, mCurBillType.intInsure, str重结ID, lng结算序号, strBalanceInfor, _
...
Private Function ShowBillFormat()
    ...
    '编制:刘兴洪
    '日期:2014-10-11 11:39:53
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strReportName As String, rsTmp As ADODB.Recordset
    Dim strSQL As String, intFormat As Integer
    
    lblFormat.Caption = ""
    If mobjFact Is Nothing Then Exit Function
    
    
    intFormat = mobjFact.打印格式
...
Private Function ExecuteClinicSwap(ByVal lng病人ID As Long, _
    ...
    If MCPAR.医保接口打印票据 And MCPAR.医保不走票号 = False Then
        ...
        If Not gblnStrictCtrl Then
            zlDatabase.SetPara "当前收费票据号", mCurBillType.strInvoice, glngSys, mlngModule, zlIsCheckPrivs(mstrPrivs, "参数设置")
        End If
    End If
    ...
    If Not mCurBillType.bln挂号 Then
        If mblnDrugMachine Then
            ...
            strSQL = "Select Max(Decode(a.记录状态, 2, a.Id, 0)) As 费用id, -1 * Nvl(Sum(a.付数 * a.数次), 0) As 退药数量" & vbNewLine & _
                    ...
                    " Group By NO, Nvl(价格父号, 序号)" & vbNewLine & _
                    " Having Nvl(Sum(a.付数 * a.数次), 0) <> 0"
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "查询本次退费项目", Val(mstr结算序号))
            Do While Not rsTemp.EOF
                strData = strData & ";" & Nvl(rsTemp!费用id) & "," & Nvl(rsTemp!退药数量)
                rsTemp.MoveNext
            Loop
...
errHandle:
    ...
    End If
End Function
Private Function ExcuteReprintFact() As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:执行重打发票功能
    '返回:打印成功,返回true,否则返回False
    '编制:刘兴洪
    '日期:2014-08-07 10:50:31
    '说明:暂不调用,后续改为在本窗体进行重打票据
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strNos As String, intInsure As Integer, lng病人ID As Long
    Dim lng结帐ID As Long, blnVirtualPrint As Boolean, strReclaimInvoice As String
    Dim intInvoiceFormat As Integer
    
    On Error GoTo errHandle
    strNos = mCurBillType.strAllNOs
    intInsure = mCurBillType.intInsure
    lng病人ID = mCurBillType.lng病人ID
    lng结帐ID = mCurBillType.lng结帐ID
    ...
    If Not RePrintCharge(1, strNos, Me, mlng领用ID, strReclaimInvoice, , , _
        intInvoiceFormat, blnVirtualPrint, , mobjFact.共享批次ID, mobjFact.使用类别) Then Exit Function
    
    Call ReInitPatiInvoice  '刷新票据号
    If txtPatient.Enabled And txtPatient.Visible Then txtPatient.SetFocus
    ExcuteReprintFact = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then Resume
End Function
